...
At the time of writing (2024-09-25) Kuro supports persistent workers for local
builds through a dedicated Kuro persistent worker gRPC protocol. However, Kuro
...
main Kuro repository. The corresponding tests will be skipped in that case. A
...
A Kuro persistent worker is created by a rule that emits the `WorkerInfo`
provider. Setting `remote = True` on this provider indicates that this worker is
remote execution capable.

Kuro actions indicate that they can utilize a persistent worker by setting the
...
Kuro actions that want to utilize a remote persistent worker must pass
command-line arguments in an argument file specified as `@argfile`,
`-flagfile=argfile`, or `--flagfile=argfile` on the command-line.

### Local Persistent Worker

A locally executed Kuro persistent worker falls under the
[Kuro persistent worker protocol](./proto/kuro/worker.proto): It is started
and managed by Kuro and passed a file path in the `WORKER_SOCKET` environment
...
A remotely executed Kuro persistent worker falls under the
