...
At the time of writing (2024-09-25) Slug supports persistent workers for local
builds through a dedicated Slug persistent worker gRPC protocol. However, Slug
...
[#776]: https://github.com/ZeromatterOSS/slug/issues/776
...
main Slug repository. The corresponding tests will be skipped in that case. A
...
A Slug persistent worker is created by a rule that emits the `WorkerInfo`
provider. Setting `remote = True` on this provider indicates that this worker is
remote execution capable.

Slug actions indicate that they can utilize a persistent worker by setting the
...
Slug actions that want to utilize a remote persistent worker must pass
command-line arguments in an argument file specified as `@argfile`,
`-flagfile=argfile`, or `--flagfile=argfile` on the command-line.

### Local Persistent Worker

A locally executed Slug persistent worker falls under the
[Slug persistent worker protocol](./proto/slug/worker.proto): It is started
and managed by Slug and passed a file path in the `WORKER_SOCKET` environment
...
A remotely executed Slug persistent worker falls under the
