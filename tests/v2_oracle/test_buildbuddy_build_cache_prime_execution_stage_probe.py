...
class ExecutionStageProbeTest(unittest.TestCase):
    ...
    def test_command_ready_replacement_and_no_bep_or_output_readers(self) -> None:
        ...
        self.assertEqual(cache.command("prime", "bazel", Path(prime[1].split("=", 1)[1]), bep, execution, nonce), prime); self.assertEqual(["bazel", "--ignore_all_rc_files", f"--output_base={prime[1].split('=', 1)[1]}", "shutdown"], calls[1]); self.assertEqual("ab285a31113a85f5a687e585088e596c552f29622b65fb991be4d591ab3886bc", hashlib.sha256(Path(cache.__file__).read_bytes()).hexdigest())
