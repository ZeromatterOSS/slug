...
def _start_clean_nativelink(
    tmp_path: Path, nativelink_bin: Path, name: str
) -> tuple[str, subprocess.Popen[str], list[str]]:
    ...
    proc, lines = _start_nativelink(nativelink_bin, config, frontend_port)
    return f"grpc://127.0.0.1:{frontend_port}", proc, lines
...
def test_stale_persistent_re_action_cache_reexecutes_through_reapi(
    ...
    try:
        endpoint, proc, nativelink_lines = _start_clean_nativelink(
            tmp_path, nativelink_bin, "nativelink-seed"
        )
        first = _build_with_reapi(slug_bin, workspace, isolation, endpoint)
        first_output = first.stdout + first.stderr
        assert "BUILD SUCCEEDED" in first_output
        ...
        endpoint, proc, nativelink_lines = _start_clean_nativelink(
            tmp_path, nativelink_bin, "nativelink-empty"
        )
        second = _build_with_reapi(slug_bin, workspace, isolation, endpoint)
        second_output = second.stdout + second.stderr
        assert "BUILD SUCCEEDED" in second_output
...
def test_stale_deferred_file_output_cache_hit_reexecutes_through_reapi(
    tmp_path: Path,
) -> None:
    slug_bin = _slug_binary()
    nativelink_bin = _nativelink_binary()
    ...
    isolation = "plan31-stale-deferred-file-output-ac"
    proc: subprocess.Popen[str] | None = None
    nativelink_lines: list[str] = []
    ...
    try:
        ...
        first = _build_with_reapi(
            slug_bin,
            workspace,
            isolation,
            endpoint,
            extra_args=["--materializations=none"],
        )
        first_output = first.stdout + first.stderr
        assert "BUILD SUCCEEDED" in first_output
        assert "Commands: 1 (cached: 0, remote: 1, local: 0)" in first_output
