...
def bxl_test(src, name = None, labels = None, buck_args: list[str] | None = None, bxl_args: list[str] | None = None, **kwargs):
    ...
    Parameters:
        ...
        buck_args: Arguments to `kuro bxl` invocation for buck specifically.
            Common examples are `--config` flags and `---modifier` flags.
            Ex. buck_args = ["--config", "build.use_limited_hybrid=false"]
        bxl_args: Arguments to `kuro bxl` invocation after `--`. These are
    ...
    kuro_e2e_test(
        name = name,
        env = env,
        srcs = {"fbcode//kuro/tests/e2e_util:test_bxl_template.py": "test_bxl_template.py"},
        labels = ["bxl_test"] + (labels if labels else []),
        test_with_compiled_kuro = False,
        test_with_deployed_kuro = True,
        skip_deployed_kuro_version_dep = True,
        **kwargs
    )
