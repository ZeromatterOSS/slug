...
../../target/release/slug audit cell

# Parse BUILD files (verify loads work)
../../target/release/slug targets root//:

# Build a target (when rules are implemented)
../../target/release/slug build //:test_target
```

Or from the slug root:

```bash
./target/release/slug --chdir tests/manual_test audit cell
```
...
3. **native.bazel_version** - Returns "9.0.0-slug" for compatibility
...
| `native.bazel_version` | Working | Returns "9.0.0-slug" |
...
3. **Verify loads**: Run `slug targets root//:` and check print output

## Learnings from Testing

### What Works

- BCR modules are fetched to `~/.cache/slug/` and copied to `bazel-external/`
- Cell resolver includes bzlmod modules alongside .buckconfig cells
- Cross-cell `load()` statements resolve correctly
- `native.bazel_version` is accessible as "9.0.0-slug"
...
1. **Before changes**: Run `slug audit cell` to verify baseline
2. **After changes**: Run `slug targets root//:` to verify loads
