...
@buck_test(data_dir="test_plan61_guardrails_data")
async def test_single_version_override_registry_source_json_parse_failure_invalidates_bzlmod_resolution(
    buck: Buck,
) -> None:
    """Bazel anchor: RegistryOverride source metadata is a resolution input."""
    cache_home = buck.cwd / "cache_home"
    default_registry = cache_home / "slug" / "registry" / "bcr.bazel.build"
    override_registry = cache_home / "slug" / "registry" / "override.example"
    default_registry.mkdir(parents=True)
    ...
    _write(
        buck.cwd / "MODULE.bazel",
        """module(name = "plan61_single_override_registry_source")
...
@buck_test(data_dir="test_plan61_guardrails_data")
async def test_multiple_version_override_registry_source_json_utf8_failure_invalidates_bzlmod_resolution(
    buck: Buck,
) -> None:
    """Bazel anchor: multiple_version_override registry source metadata is an input."""
    cache_home = buck.cwd / "cache_home"
    default_registry = cache_home / "slug" / "registry" / "bcr.bazel.build"
    override_registry = cache_home / "slug" / "registry" / "override.example"
    default_registry.mkdir(parents=True)
    ...
    _write(
        buck.cwd / "MODULE.bazel",
        """module(name = "plan61_multiple_override_registry_source_utf8")
...
@buck_test(data_dir="test_plan61_guardrails_data")
async def test_locked_registry_metadata_delete_invalidates_bzlmod_resolution(
    buck: Buck,
) -> None:
    """Bazel anchor: top-level bazel_registry.json deletion is a registry input."""
    module_name = "remote_registry_delete"
    module_version = "1.0.0"
    cache_home = buck.cwd / "cache_home"
    registry_cache = cache_home / "slug" / "registry" / "bcr.bazel.build"
    module_cache = registry_cache / "modules" / module_name / module_version
    source_dir = module_cache / "source"
    source_dir.mkdir(parents=True)
    registry_file = registry_cache / "bazel_registry.json"
    ...
    _write(
        buck.cwd / "MODULE.bazel",
        f"""module(name = "plan61_registry_metadata_delete")
...
""",
            ...
            json.dumps(
                {
                    "lockFileVersion": 26,
                    "registryFileHashes": {
                        registry_url: _sha256(registry_file),
                        module_url: _sha256(module_cache / "MODULE.bazel"),
                        source_url: _sha256(module_cache / "source.json"),
                    },
                    "selectedYankedVersions": {},
    ...
    assert "Registry file checksum mismatch" in failure
    assert "bazel_registry.json" in failure
...
@buck_test(data_dir="test_plan61_guardrails_data")
async def test_locked_registry_metadata_creation_invalidates_bzlmod_resolution(
    buck: Buck,
) -> None:
    """Bazel anchor: top-level bazel_registry.json creation is a registry input."""
    module_name = "remote_registry_create"
    module_version = "1.0.0"
    cache_home = buck.cwd / "cache_home"
    registry_cache = cache_home / "slug" / "registry" / "bcr.bazel.build"
    module_cache = registry_cache / "modules" / module_name / module_version
    ...
    registry_file = registry_cache / "bazel_registry.json"
    registry_content = "{}\n"
    registry_digest = hashlib.sha256(registry_content.encode()).hexdigest()
    _write(
        module_cache / "MODULE.bazel",
        f'module(name = "{module_name}", version = "{module_version}")\n',
    )
    ...
    _write(
        buck.cwd / "MODULE.bazel",
        f"""module(name = "plan61_registry_metadata_create")
...
@buck_test(data_dir="test_plan61_guardrails_data")
async def test_locked_registry_metadata_parse_and_utf8_failures_invalidate_bzlmod_resolution(
    buck: Buck,
) -> None:
    """Bazel anchor: IndexRegistry parses top-level bazel_registry.json metadata."""
    module_name = "remote_registry_metadata_failure"
    module_version = "1.0.0"
    cache_home = buck.cwd / "cache_home"
    registry_cache = cache_home / "slug" / "registry" / "bcr.bazel.build"
    module_cache = registry_cache / "modules" / module_name / module_version
    ...
    _write(
        buck.cwd / "MODULE.bazel",
        f"""module(name = "plan61_registry_metadata_failure")
...
""",
    ...
    _write(registry_file, "{not json}\n")
    write_lockfile()
    with pytest.raises(BuckException) as exc:
        await buck.audit("cell", env=env)
    failure_stderr = exc.value.stderr
    assert "bazel_registry.json" in failure_stderr
    assert "Failed to parse" in failure_stderr
...
@buck_test(data_dir="test_plan61_guardrails_data")
async def test_locked_registry_source_json_delete_invalidates_bzlmod_resolution(
    buck: Buck,
) -> None:
    """Bazel anchor: registry source.json deletion is a module source input."""
    module_name = "remote_source_delete"
    module_version = "1.0.0"
    repaired_module_name = "remote_source_delete_repaired_dep"
    cache_home = buck.cwd / "cache_home"
    registry_cache = cache_home / "slug" / "registry" / "bcr.bazel.build"
    module_cache = registry_cache / "modules" / module_name / module_version
    source_dir = module_cache / "source"
    ...
    _write(
        buck.cwd / "MODULE.bazel",
        f"""module(name = "plan61_registry_source_delete")
...
""",
    ...
    def write_lockfile(source_digest: str | None = None) -> None:
        _write(
            buck.cwd / "MODULE.bazel.lock",
            json.dumps(
                {
                    ...
                    "registryFileHashes": {
                        registry_url: _sha256(registry_cache / "bazel_registry.json"),
                        module_url: _sha256(module_file),
                        source_url: source_digest or _sha256(source_json),
                        repaired_module_url: _sha256(
                            repaired_module_cache / "MODULE.bazel"
                        ),
                        repaired_source_url: _sha256(
    ...
    stale_source_digest = _sha256(source_json)
    source_json.unlink()
    write_lockfile(stale_source_digest)
    with pytest.raises(BuckException) as exc:
        await buck.audit("cell", env=env)
    failure_stderr = exc.value.stderr
    assert "source.json" in failure_stderr
...
@buck_test(data_dir="test_plan61_guardrails_data")
async def test_locked_registry_source_json_creation_invalidates_bzlmod_resolution(
    buck: Buck,
) -> None:
    """Bazel anchor: registry source.json creation is a module source input."""
    module_name = "remote_source_create"
    module_version = "1.0.0"
    cache_home = buck.cwd / "cache_home"
    registry_cache = cache_home / "slug" / "registry" / "bcr.bazel.build"
    module_cache = registry_cache / "modules" / module_name / module_version
    ...
    _write(
        buck.cwd / "MODULE.bazel",
        f"""module(name = "plan61_registry_source_create")
...
@buck_test(data_dir="test_plan61_guardrails_data")
async def test_locked_registry_module_delete_invalidates_bzlmod_resolution(
    buck: Buck,
) -> None:
    """Bazel anchor: registry MODULE.bazel deletion is a module source input."""
    module_name = "remote_module_delete"
    module_version = "1.0.0"
    repaired_module_name = "remote_module_delete_repaired_dep"
    cache_home = buck.cwd / "cache_home"
    registry_cache = cache_home / "slug" / "registry" / "bcr.bazel.build"
    module_cache = registry_cache / "modules" / module_name / module_version
    source_dir = module_cache / "source"
    ...
    _write(
        buck.cwd / "MODULE.bazel",
        f"""module(name = "plan61_registry_module_delete")
...
""",
    ...
    def write_lockfile(module_digest: str | None = None) -> None:
        _write(
            buck.cwd / "MODULE.bazel.lock",
            json.dumps(
                {
                    ...
                    "registryFileHashes": {
                        registry_url: _sha256(registry_cache / "bazel_registry.json"),
                        module_url: module_digest or _sha256(module_file),
                        source_url: _sha256(module_cache / "source.json"),
                        repaired_module_url: _sha256(repaired_module_cache / "MODULE.bazel"),
                        repaired_source_url: _sha256(repaired_module_cache / "source.json"),
                    },
    ...
    stale_module_digest = _sha256(module_file)
    module_file.unlink()
    write_lockfile(stale_module_digest)
    with pytest.raises(BuckException) as exc:
        await buck.audit("cell", env=env)
    failure_stderr = exc.value.stderr
    assert "Failed to fetch MODULE.bazel" in failure_stderr
...
@buck_test(data_dir="test_plan61_guardrails_data")
async def test_locked_registry_module_creation_invalidates_bzlmod_resolution(
    buck: Buck,
) -> None:
    """Bazel anchor: registry MODULE.bazel creation is a module source input."""
    module_name = "remote_module_create"
    module_version = "1.0.0"
    cache_home = buck.cwd / "cache_home"
    registry_cache = cache_home / "slug" / "registry" / "bcr.bazel.build"
    module_cache = registry_cache / "modules" / module_name / module_version
    ...
    _write(registry_cache / "bazel_registry.json", "{}\n")
    module_file = module_cache / "MODULE.bazel"
    module_content = f'module(name = "{module_name}", version = "{module_version}")\n'
    module_digest = hashlib.sha256(module_content.encode()).hexdigest()
    _write(module_cache / "source.json", "{}\n")
    _write(source_dir / ".complete", "")
    _write(source_dir / "BUILD.bazel", 'filegroup(name = "ok", srcs = [])\n')
    _write(
        buck.cwd / "MODULE.bazel",
        f"""module(name = "plan61_registry_module_create")
...
""",
    ...
    _write(module_file, module_content)
    output, created = await _audit_cells_and_counters(buck, env=env)
    assert module_name in output
...
async def test_module_ctx_execute_label_arg_explicit_watch_reexecutes_extension(
    ...
    _write(
        buck.cwd / "module_execute_label_ext.bzl",
        """def _repo_impl(repository_ctx):
    repository_ctx.file("data.txt", repository_ctx.attr.payload + "\\n")
    repository_ctx.file("BUILD.bazel", "exports_files([\\"data.txt\\"])\\nfilegroup(name = \\"data\\", srcs = [\\"data.txt\\"])\\n")

exec_label_repo_rule = repository_rule(
    implementation = _repo_impl,
    attrs = {"payload": attr.string()},
)
...
def _module_execute_label_ext_impl(module_ctx):
    ...
    exec_label_repo_rule(name = "exec_label_repo", payload = result.stdout)

module_execute_label_ext = module_extension(
    implementation = _module_execute_label_ext_impl,
)
""",
    )
    _write(
        buck.cwd / "MODULE.bazel",
        """module(name = "plan61_module_ctx_execute_label_input")

exec_label = use_extension("//:module_execute_label_ext.bzl", "module_execute_label_ext")
use_repo(exec_label, "exec_label_repo")
""",
    )
    _write_minimal_lockfile(buck.cwd / "MODULE.bazel.lock")
    _write(
        buck.cwd / "BUILD.bazel",
        """filegroup(
    name = "uses_exec_label_repo",
    srcs = ["@exec_label_repo//:data"],
)
""",
    )

    await buck.build("//:uses_exec_label_repo")
    first = await _bzlmod_counters(buck)
    assert (repo_dir / "data.txt").read_text() == "first\n"

    _write(buck.cwd / "script.sh", "printf second\n")
    await buck.build("//:uses_exec_label_repo")
    second = await _bzlmod_counters(buck)
...
def _repo_execute_label_ext_impl(module_ctx):
    exec_label_repo_rule(name = "exec_label_repo")

repo_execute_label_ext = module_extension(
    implementation = _repo_execute_label_ext_impl,
)
""",
    )
    _write(
        buck.cwd / "MODULE.bazel",
        """module(name = "plan61_repository_ctx_execute_label_input")

exec_label = use_extension("//:repo_execute_label_ext.bzl", "repo_execute_label_ext")
use_repo(exec_label, "exec_label_repo")
""",
    )
    _write_minimal_lockfile(buck.cwd / "MODULE.bazel.lock")
...
async def test_repository_ctx_template_label_auto_watch_reexecutes_materialized_repo(
    ...
    """Bazel anchors: StarlarkRepositoryContext.createFileFromTemplate."""
    repo_dir = buck.cwd / "bazel-external" / "_main+template_watch_ext+template_watch_repo"
    recorded_inputs = repo_dir / ".slug_repo_recorded_inputs"
    _write(buck.cwd / "watched.tpl", "value = first\n")
    _write(
        buck.cwd / "template_watch_ext.bzl",
        """def _repo_impl(repository_ctx):
    repository_ctx.template("data.txt", Label("//:watched.tpl"), substitutions = {})
    repository_ctx.file("BUILD.bazel", "exports_files([\\"data.txt\\"])\\nfilegroup(name = \\"data\\", srcs = [\\"data.txt\\"])\\n")

template_watch_repo_rule = repository_rule(
    implementation = _repo_impl,
)

def _template_watch_ext_impl(module_ctx):
    template_watch_repo_rule(name = "template_watch_repo")

template_watch_ext = module_extension(
    implementation = _template_watch_ext_impl,
)
""",
    )
    _write(
        buck.cwd / "MODULE.bazel",
        """module(name = "plan61_repo_ctx_template_auto_watch_input")

template_watch = use_extension("//:template_watch_ext.bzl", "template_watch_ext")
use_repo(template_watch, "template_watch_repo")
""",
    )
    _write_minimal_lockfile(buck.cwd / "MODULE.bazel.lock")
    _write(
        buck.cwd / "BUILD.bazel",
        """filegroup(
    name = "uses_template_watch_repo",
    srcs = ["@template_watch_repo//:data"],
)
""",
    )

    await buck.build("//:uses_template_watch_repo")
    first = await _bzlmod_counters(buck)
    assert (repo_dir / "data.txt").read_text() == "value = first\n"
    assert recorded_inputs.exists()
    assert "FILE:" in recorded_inputs.read_text()

    _write(buck.cwd / "watched.tpl", "value = second\n")

    await buck.build("//:uses_template_watch_repo")
    second = await _bzlmod_counters(buck)

    assert (repo_dir / "data.txt").read_text() == "value = second\n"
    assert second["repo_materialization_miss_reason"] > first[
        "repo_materialization_miss_reason"
    ]
...
@buck_test(data_dir="test_plan61_guardrails_data")
async def test_non_root_override_repo_is_ignored(
    buck: Buck,
) -> None:
    """Bazel anchor: override_repo() is ignored outside the root module."""
    owner = buck.cwd / "owner"
    helper = buck.cwd / "helper"
    owner.mkdir()
    helper.mkdir()
...
local_path_override(module_name = "helper", path = "../helper")
ext = use_extension("//:ext.bzl", "ext")
override_repo(ext, generated = "helper")
use_repo(ext, "generated")
""",
    )
    _write(
...
""",
    ...
    _write(
        buck.cwd / "MODULE.bazel",
        """module(name = "plan61_non_root_override_repo")
