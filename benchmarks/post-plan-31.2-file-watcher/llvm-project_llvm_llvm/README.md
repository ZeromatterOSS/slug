...
Slug: HEAD `b588318b` + plan 31.2 patch (notify + watchman component filter
...
|-------|---------------------------------|--------|
| slug  | 1.05, 0.79, 0.78, 0.83, 0.89    | **0.83 s** |
...
Plan 31 success criterion: warm-daemon slug wall ≤ 1.97 s (closes ≥80%
of 4.7 s gap). **Hit.** Slug now beats bazel on this scenario.
...
the network: slug short-circuits on DICE, bazel on Skyframe. The metric
is the local-overhead floor of a no-op rebuild.

## 31.2-specific test — bazel-write events do not invalidate slug

Without the filter, every artifact bazel writes to `bazel-bin/` looks
like a source change to slug's notify watcher and forces DICE to
...
# warm-daemon slug baseline
slug build @llvm-project//llvm:llvm --config=remote      # 0.78 s
# bazel runs, writing bazel-bin/, bazel-out/, …
bazel build @llvm-project//llvm:llvm --config=remote     # 2.21 s
# next slug build — file events from bazel-bin should be dropped
slug build @llvm-project//llvm:llvm --config=remote      # 0.78 s ✓
```

The post-bazel slug build prints **no** `File changed: …bazel-bin…`
...
Single trial, both daemons just killed (`slug killall`,
...
Plan 31.1 isn't done yet, so slug has no on-disk action cache —
...
slug killall    # no on-disk cache to delete
```

| Tool  | Trial 1 | Trial 2 | Network (up / down) | actions / processes |
|-------|---------|---------|---------------------|---------------------|
| slug  | 14.48 s | 15.20 s | ~948 KiB / ~5.1 MiB | 4 853 (4 852 cached, 1 local) |
| bazel | 48.83 s | 46.05 s | not reported by bazel | 5 886 (4 739 remote cache hit, 1 147 internal) |

Median wall: slug **14.84 s** vs bazel **47.44 s** — **slug ~3.2× faster**
...
on its own end. Slug's RE client issues `GetActionResult` calls in
parallel; bazel's local action cache rebuild appears to be far less
parallel.

When plan 31.1 lands and slug persists its action cache to disk, this
scenario becomes a fast-path local lookup for slug too, and the cold-
daemon table at the top of the README closes.

BB.io invocations:

- slug #1: https://app.buildbuddy.io/invocation/9ee8a4b3-712e-4c47-adeb-e75fdd6dbb3d
- slug #2: https://app.buildbuddy.io/invocation/53badca0-d2ab-4b80-91eb-000a7e899170
...
slug  build @llvm-project//llvm:llvm --config=remote
bazel build @llvm-project//llvm:llvm --config=remote
# 5 warm-daemon trials per tool
for i in 1..5; /usr/bin/time -f "WALL=%es" slug  build @llvm-project//llvm:llvm --config=remote
...
- Notify filter: `app/slug_file_watcher/src/notify.rs`
  `RESERVED_OUTPUT_COMPONENTS` const + `is_reserved_output_path`.
- Watchman parity: `app/slug_file_watcher/src/watchman/interface.rs`
  reuses `is_reserved_output_path` in `process_one_change`.
- Tests: `app/slug_file_watcher/src/notify.rs#tests` — 6 cases covering
