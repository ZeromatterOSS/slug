...
Slug was run from `/var/mnt/dev/slug/target/debug/slug`.
...
Slug's daemon-side exposed non-external wall was about 2.32 s lower than Bazel's
server-side wall for this run. Bazel kept the action executor slightly fuller
once actions started, but Slug reached external action launch earlier.
...
Slug's daemon-side no-op path was in the same range and slightly lower than
Bazel's server wall, but Slug had more client/outer overhead in the measured CLI
wall.
...
target for this workload. Slug's measured Starlark-heavy load/analyze phases were
...
- Reduce Slug CLI/client/event-log/BES overhead on no-op builds. The daemon
...
- Separate local executor slot wait from true per-action setup in profiles. The
  aggregate per-action "overhead" in the cold Slug action events is mostly
...
- Improve local process saturation after the initial wave. Bazel averaged 14.43
  parallel actions during action execution versus Slug's 13.54 in this run.
...
- Slug cold discounted overhead: `cold-daemon-default-01/discounted-overhead.json`
- Slug cold summary: `cold-daemon-default-01/summary.json`
- Slug warm summary: `slug-local-warm-01/summary.json`
