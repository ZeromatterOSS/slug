...
| Named/automatic execution groups | Corrected implementation plus accepted initializer/effect-diagnostic/generated-path checkpoints are preserved unaccepted with R2. Generated local-JDK absence passes its joint gates at `b75291517`; the next one-selector attribution reached the fixed runtime ceiling without a typed terminal | Stages 4/5/6 | Resolve the recorded attribution resource limit without raising the inherited ceiling or blindly replaying selectors, then continue reviewed joint gates before F3 |
