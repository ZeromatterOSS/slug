...
| Ordinary source/generated/tree input transfer and output materialization | Bounded FileWrite and source-only Spawn staging/execution accepted (WP-7-31/34); verified regular-file output trees accepted (WP-7-35). WP-7-36 resolves declared producer prerequisites only. Generated file/tree CAS binding, downstream execution and validated local publication remain open | Stages 6/7 | Exact Directory topology/content/digests, declared output ownership/type/mode/symlinks, generated-output consumer/reupload and missing/corrupt data behavior; validated-root-only publication |
