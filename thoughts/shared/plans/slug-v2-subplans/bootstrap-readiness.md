...
| Named/automatic execution groups | Corrected implementation plus accepted initializer/effect-diagnostic/generated-path checkpoints are preserved unaccepted with R2. The valid path census cutoff found 471 first-Need identities and 2,054 repeats, of which 2,050 completed | Stages 4/5/6 | Review and run one capped aggregate path-frontier audit of requested/unseen/known totals and six batch-size buckets, then continue only its reviewed call-site or shard-invalidation follow-up before joint gates and F3 |
