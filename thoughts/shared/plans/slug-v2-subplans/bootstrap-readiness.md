...
| Named/automatic execution groups | Corrected implementation is preserved unaccepted with R2; 164/164 analysis tests pass. Authentic CLI/F3 traversal now clears group invocation and reaches rules_cc `_def_parser` computed-default target invocation | Stages 4/5/6 | Review the automatic qualifier/property correction, complete the narrow package-owned computed-default prerequisite, pass all joint gates, then replay F3 |
| `attr.label` computed-default invocation | Declaration retention is accepted; all three real CLI gates and supervised F3 select rules_cc `_def_parser(name, tags)`, whose result is an ordinary `cfg="exec"` dependency | Stages 4/6 | Omitted callback invocation, typed Label/None result, explicit bypass, lexical label source, atomic A/B/A publication and configured Exec dependency proof on the unaccepted combined stack |
