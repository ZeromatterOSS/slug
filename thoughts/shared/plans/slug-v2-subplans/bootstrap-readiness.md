...
The next observed execution gap is default binary runfiles support. The production
`//app/slug_cli_v2:slug` is a `rust_binary`; Analysis retains its RunfilesSupport and
selects the RunfilesTree through the hidden top-level output group, but Core's action
prerequisite planner rejects that output kind/support family. WP-7-47 supplies the
prerequisite typed logical layout and complete backing constituent view, verified through a tiny configured binary. WP-7-48 supplies native Host-source and
configured-output target resolution plus deterministic manifest bytes. WP-7-49 adds native
materialization generation leases for held prepared results. Durable materialized-source
backing, virtual support results and confined link/backing-output publication remain required
before ordinary binary Build can execute. This is required for ordinary Build; it does not activate
Run or reopen the closed F3 replay. Full production closure coverage remains a later gate.
