...
| Named/automatic execution groups | Corrected implementation plus accepted initializer/effect-diagnostic/generated-path checkpoints are preserved unaccepted with R2. Valid sampled and exact key-count audits reached the fixed runtime ceiling in `RootCompute`; path observations owned 2,543 of 8,094 same-cutoff compute starts | Stages 4/5/6 | Review and run one capped aggregate post-sharding path identity/outcome census, then continue only its reviewed causal follow-up before joint gates and F3 |
