...
| Named/automatic execution groups | Corrected implementation and independently accepted initializer checkpoint are preserved unaccepted with R2. The bounded effect diagnostic passes its owner gates and independent review; one CLI selector remains to identify the local_jdk terminal | Stages 4/5/6 | Attribute the typed repository-effect terminal, implement only its reviewed owner if required, pass all joint gates, then replay F3 |
