...
| Named/automatic execution groups | Preserved implementation and accepted loading prerequisites remain unaccepted with R2. The pinned Bazel source-invariant reconciliation is independently reviewed and Phase B ready | Stages 4/5/6 | Implement only the reviewed runtime-name, target-context, property, action-order, root-provenance/depset and subrule-boundary corrections; pass ordinary joint gates before atomic acceptance, with no diagnostic replay or F3 |
