...
| Named/automatic execution groups | Corrected implementation and independently accepted initializer checkpoint are preserved unaccepted with R2. Loading owner gates pass; the first CLI consumer now clears the initializer and exposes `[diagnostic incomplete: Effect]` for local_jdk registration | Stages 4/5/6 | Attribute the typed repository-effect terminal, implement only its reviewed owner if required, pass all joint gates, then replay F3 |
