...
| Named/automatic execution groups | Corrected implementation is preserved unaccepted with R2; 165/165 analysis tests pass after computed defaults. Authentic F3 now clears group and callback invocation and reaches rules_java configurable alias loading | Stages 4/5/6 | Review the automatic qualifier/property correction, complete the native configurable-alias prerequisite, pass all joint gates, then replay F3 |
...
| Native configurable `alias.actual` | F3 reaches rules_java `toolchains/BUILD:205`, whose mandatory label uses `select()`; Slug currently rejects it during loading | Stages 4/6 | Retain the typed expression, keep branch/condition dependencies distinct, select in alias target/exec configuration with A/B/A and query projection proofs, then replay F3 |
