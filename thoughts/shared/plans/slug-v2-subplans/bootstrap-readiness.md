...
| Named/automatic execution groups | Corrected implementation plus accepted initializer/effect-diagnostic/generated-path checkpoints are preserved unaccepted with R2. The direct-dependent audit observed 13,508 exact `PathObservationKey` events across 221 coherent committed-handoff windows | Stages 4/5/6 | Review and run one aggregate audit of legacy-delivered `PathObservationKey` callbacks by evaluated/reused kind and immediate shard dependency; rich-only reuse is excluded, and even positive cells prove no per-merge invalidation, causality or cost before joint gates or F3 |
