...
| Named/automatic execution groups | Corrected implementation plus accepted initializer/effect-diagnostic/generated-path checkpoints are preserved unaccepted with R2. The accepted activation audit observed 2,488 legacy-delivered evaluated `PathObservationKey` callbacks with an immediate exact shard dependency at a coherent 220/220/219 cutoff | Stages 4/5/6 | Stop aggregate callback audits. Any diagnostic continuation needs a separately reviewed typed invalidation-provenance design; the accepted result proves no identity, invalidation causality, necessity, cost or avoidability before joint gates or F3 |
| `attr.label` computed-default invocation | Implemented unaccepted at `dab5cb5ea`; focused proofs and loading/configuration/analysis/Build API/REAPI suites pass, and F3 advances beyond `_def_parser` | Stages 4/6 | Preserve omitted invocation, typed Label/None, explicit bypass, lexical context, atomic A/B/A and configured Exec proof through joint acceptance |
| Native configurable `alias.actual` | Implemented unaccepted with focused loading/configured/query proofs; full loading is 563 active plus one ignored and analysis is 168/168. F3 advances through the alias | Stages 4/6 | Preserve the retained expression, branch/condition split and target/exec A/B/A through joint acceptance |
