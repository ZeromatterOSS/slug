...
| Named/automatic execution groups | Corrected implementation plus accepted initializer/effect-diagnostic/generated-path checkpoints are preserved unaccepted with R2. The accepted handoff cutoff matched 218 installed merges to 218 successful injection commits carrying 1,591 aggregate potential prior exposure | Stages 4/5/6 | Review and run one aggregate armed-window audit for the six exact `PathObservationShardKey` DICE event variants; even positive counts prove no per-merge invalidation, causality or cost before joint gates or F3 |
