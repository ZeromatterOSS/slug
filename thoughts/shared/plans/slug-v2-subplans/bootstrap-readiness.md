...
| Named/automatic execution groups | Corrected implementation plus accepted initializer/effect-diagnostic/generated-path checkpoints are preserved unaccepted with R2. The shard-key audit observed zero exact events across 221 coherent committed-handoff windows | Stages 4/5/6 | Review and run one aggregate armed-window audit for the six exact `PathObservationKey` DICE event variants; a zero result ends this diagnostic chain, and even positive counts prove no per-merge invalidation, causality or cost before joint gates or F3 |
