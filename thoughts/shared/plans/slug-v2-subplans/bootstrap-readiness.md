...
| Named/automatic execution groups | Corrected implementation plus accepted initializer/effect-diagnostic checkpoints are preserved unaccepted with R2. The CLI now selects repository-local string path/get_child/exists for the absent local-JDK branch; its corrected design is accepted | Stages 4/5/6 | Implement the attempt-local generated-path owner, pass joint gates, then continue the one-selector attribution sequence before F3 |
