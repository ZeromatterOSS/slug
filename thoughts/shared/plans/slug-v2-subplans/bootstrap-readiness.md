...
| Symlink and runfiles-support action families | Typed symlink declarations, runfiles and four support actions/FilesToRun expansion are configured-only admission (`f346c209a`, `f46a009a0`, `21db5d7b8`); WP-7-47 adds a pure artifact-owned runfiles layout/constituent projection; WP-7-48 adds observed native Host-source/configured-output target resolution and deterministic source/repository mapping bytes with configured A/B/A proof; execution/aquery remain open; exact ActionKeys deferred | Stages 6/8/7 | Inventory names each required family; admit its semantic aquery/REAPI projections and outputs/runfiles behavior with explicit ActionKey classification, or prove absence from this closure |
...
configured-output target resolution plus deterministic manifest bytes. Native materialization
generation authority, virtual support results and confined link/backing-output publication
remain required before ordinary binary Build can execute. This is required for ordinary Build; it does not activate
