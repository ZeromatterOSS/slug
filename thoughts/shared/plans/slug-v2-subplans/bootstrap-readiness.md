...
| Named/automatic execution groups | Authentic F3 reaches rules_java toolchains -> rules_cc 0.2.17 `cc_library`; its authenticated declaration has named `cpp_link` and `_use_auto_exec_groups=True`. Demand for both modes is confirmed; runtime remains unimplemented | Stages 4/5/6 | Complete the shared configured-target-owned group collection and all selected resolution/dependency/provider/property/action consumers after combined R2, then pass the unchanged portable F3 proof |
