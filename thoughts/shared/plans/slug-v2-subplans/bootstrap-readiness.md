...
|---|---|---|---|
| Bazel builds the CLI root, its selected Cargo closure and generated sources | The named `//app/slug_cli_v2:slug` production root builds under pinned Bazel 9.2/nightly rules_rust, with nonempty LALRPOP and cache-leaf proto trees. Earlier static/configured/local-feature and exact embedded-input slices remain accepted. Live Cargo/Bazel closure, external feature/pin/action correspondence and Slug behavioral admission remain open | Stage 10 | Reconcile the live cache-leaf closure and prove remaining feature, pin and action correspondence without excluding a production input; preserve the successful named CLI-root build |
