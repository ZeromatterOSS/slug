...
| Named/automatic execution groups | Corrected implementation plus accepted initializer/effect-diagnostic/generated-path checkpoints are preserved unaccepted with R2. The accepted activation audit observed 2,488 legacy-delivered evaluated `PathObservationKey` callbacks with an immediate exact shard dependency at a coherent 220/220/219 cutoff | Stages 4/5/6 | Independently review the docs-first typed dependency-context invalidation-source design; do not implement or replay before acceptance, and infer no necessity, cost or avoidability before joint gates or F3 |
