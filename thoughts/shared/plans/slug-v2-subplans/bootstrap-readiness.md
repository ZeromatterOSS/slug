...
| Named/automatic execution groups | Corrected implementation plus accepted initializer/effect-diagnostic/generated-path checkpoints are preserved unaccepted with R2. Generated local-JDK absence passes its joint gates at `b75291517`; one valid sampled progress audit reached the fixed runtime ceiling in `RootCompute` without a typed terminal | Stages 4/5/6 | Capture same-replay atomic cutoff counters for the sampled external-Bzl and historically adjacent path key types in one bounded causal audit, then continue only its reviewed follow-up before joint gates and F3 |
