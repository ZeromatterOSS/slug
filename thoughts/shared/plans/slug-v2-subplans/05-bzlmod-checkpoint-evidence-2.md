...
Event contract: A separate dependency-bottom `slug_events_v2` owns only `EvaluationEvent::StarlarkPrint { text: CompactString }` and an ordered `Arc<[EvaluationEvent]>` batch with explicit empty, `Dupe`, `Allocative`, and structural equality. It has no DICE, Starlark, workspace, source-path, sink, node/version, attempt/retry, generation, terminal-policy, serialization, or publication dependency. Batches later live only in DICE evaluation data and command-owner operational tables, never domain values/equality. The unused core scaffold remains excluded until the later runtime owner removes it.
Packet split: First implement only `WP-5-m1-structural-path-sparse-owner` in `slug_workspace_v2/src/{lib.rs,path_observation.rs}` with inline focused tests and no Cargo change. Then add only the neutral event crate plus deterministic root Cargo/lockfile and archive-path metadata. Only afterward propagate structural Need leaf-up, migrate local/immutable reads, add the shared retry owner, remove legacy snapshots/adapters, capture producers, and publish terminal events.
