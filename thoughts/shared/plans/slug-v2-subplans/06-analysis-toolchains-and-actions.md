...
### Instantiated module-extension repositories observation design (2026-08-21)

Visibility implementation `0dcf2eea` closes the last prerequisite. The live
one-file design activates only
`WP-6-7A-host-instantiated-module-extension-repositories-observation-implementation`
over Rust base `0dcf2eea`. `HostInstantiatedModuleExtensionRepositoriesKey`
has one production child, the legacy pure key at line 191, and one production
consumer, validation at
`module_extension_repository_validation.rs:208`. Validation then feeds the
public generated-definition key, so it is later rather than an observation
prerequisite.

Add private `HostInstantiatedModuleExtensionRepositoriesObservationKey` as a
newtype over the legacy key, private
`ObservedHostInstantiatedModuleExtensionRepositories` retaining the exact
local semantic Result Arc plus transaction-local epoch, and private
`HostInstantiatedModuleExtensionRepositoriesObservationError` with exactly one
`Pure(HostPureModuleExtensionInvocationsObservationError)` variant. Preserve
legacy workspace identity, derive sets and `observed-{legacy Display}`. Both
keys retain Complete-only equality and validity. Keep every field, alias and
accessor private; add no export or sibling visibility.
...
semantics, both call the existing `instantiate_repositories` exactly once and
project the same local semantic Result. Do not duplicate, reorder or parallelize the pure-request receipt join,
generated canonical namespace, base/call/override mapping precedence,
repository-rule schema lookup, explicit supplied-attribute order, legacy/None
omission, mandatory/default validation, label resolution/visibility or ordered
`RepoSpec` construction.

Pure DICE compute failure remains existing semantic `InvocationsCompute` with
an empty epoch. Pure Need remains immediate Need. An observed pure opaque outer
becomes the one carrierless Pure outer; do not inspect or rewrap its private
terminal. A Complete pure carrier is accepted before its semantics: semantic
failure remains existing `Invocations` and retains the complete child epoch;
success passes the cloned pure value and the same epoch to local instantiation.
There is exactly one observed child, so this parent has no merge stage and must
not rebuild, union or validate the epoch.

Every local count/request Join, Namespace and Attribute terminal retains the
full pure epoch and preserves the existing `AfterInvocations` predecessor,
completed-request, current-repository, request and call prefixes. Success
retains that same epoch. First terminal wins in prepared-input/request/call
order. No local terminal may become a typed outer, and no existing semantic
error may become carrierless.

Instantiation stores no evaluation event data. Fresh pure-child load and
invocation-print batches remain owned and ordered by the pure subtree; the
instantiation activation row is batchless on success and every semantic
terminal. Need, opaque outer and cancellation publish no parent carrier or
batch. Warm parent reuse is silent, and an unchanged pure child reached during
parent recomputation is Reused without replay. Do not copy child events into a
new parent batch.

The only new DICE-retained state is the instantiation Result Arc plus its
transaction-local pure epoch. The Result already owns the predecessor,
completed/current repository projections and `RepoSpec`s required by existing
semantics. Repository-rule definitions/calls are those already reachable from
that Result. Driver mode, temporary pure projection and local construction
vectors remain compute scratch and drop at terminal. No evaluator heap,
event batch, side cache, task, lock or command state is retained. Equality
cutoff, invalidation and eviction remain ordinary DICE ownership; future drop
propagates cancellation through the single child compute, and recovery creates
no partially published carrier.
...
Add at most six production and five test helpers plus three observed-owner
tests; keep the shared driver below 120 and every changed helper/test below
200. The file remains cohesive despite the prospective >2,000-line trigger:
it already owns the legacy key, instantiation functions, exact schema/label
helpers, real transaction fixture and sole validation boundary. Splitting a
private sibling would expose its retained types or duplicate proof plumbing.
...
`observed_instantiation_lifecycle_cancellation_and_nonactivation`. Together
prove key/hash/Display/equality/validity; one-child/no-merge Result+epoch
projection; existing Join/Namespace/Attribute prefixes; exact legacy/observed
semantic parity for success, pure failure and local failures; exact family and
single-child dependency rows; Need and first-terminal suppression; child-only
print batches, batchless parent rows and warm silence; held Result/carrier/epoch
A -> B -> A across call/schema/mapping changes; same-semantic/different-epoch
metadata, per-transaction global-epoch subset, poll-drop recovery and zero
validation activation. Compare separate transactions semantically, reserving
Arc identity for an exact cached value proven Reused.

The pure outer is intentionally opaque in this sibling. Reuse the accepted
same-module pure HostBzl/Merge outer proof and require a static private-producer
scan plus the real observed-parent dependency row; do not add an accessor,
construct malformed epochs or expose the wrapper. Reuse pinned Bazel 9.2
`RegularRunnableExtension.run`, `SingleExtensionEvalFunction.compute` and
`ModuleExtensionResolutionTest` evidence. Buck2 DICE incrementality,
cancellation and activation-tracker tests are concept/test evidence only; no
donor scheduler, side store or fallback is admitted. Add no oracle.

Validate focused `observed_instantiation_`, protected `pure_instantiation_`,
`real_key_`, `observed_pure_` and validation `real_validation_` tests; full
`cargo test -p slug_loading_v2`; direct `cargo check -p slug_core_v2`;
`cargo fmt --all -- --check`; and `git diff --check`, serially.

Existing instantiation values/errors/order, namespaces, attribute and label
semantics, `RepoSpec` projections, DICE equality and pure-owned events remain
exact Bazel 9 compatibility. The private observation key/carrier/outer and
Result-Arc/epoch association are Slug-native. Validation observation,
generated/public/root-mapping/bootstrap activation and exact Bazel
configuration/output/ActionKey bytes remain unsupported/deferred. ACCEPT
returns only to a docs-only validation frontier audit.

STOP a second file/key/owner/adapter, any visibility/lib export or caller,
changed legacy or pure semantics, a parent event batch, epoch merge/rebuild,
retained evaluator state, lock/task across DICE, validation/generated/public/
root-mapping/bootstrap activation, fixture/oracle work, proof or cap waiver,
...
### Apparent-mapping carrier visibility design accepted (2026-08-21)

The live design audit accepts exactly one same-crate handoff. Schedule only
`WP-6-7A-host-canonical-repository-apparent-mapping-observation-carrier-visibility-implementation`.
This packet remains callerless and does not activate root apparent-definition
ownership.

Production authority is exactly generated definition at baseline 3,828
physical/tests 1,138 and SHA-256
`7b542ed4e0a661aa81a11651aba9bcd7ef62fc2a89bc947775a651c8a3d2f9db`.
Test-only sibling authority is root apparent definition at baseline 999
physical/tests 372 and SHA-256
`d13997171de5467fca54599d6c3aaf4e62eabfc95d224f91b72507401e63ae3e`.
No third file or crate-root export is authorized.

Promote exactly
`HostCanonicalRepositoryApparentMappingObservationKey`,
`ObservedHostCanonicalRepositoryApparentMapping` and
`HostCanonicalRepositoryApparentMappingObservationError` to `pub(super)`.
The key field remains private and only exact
`pub(super) fn new(workspace: NormalizedAbsolutePath, context_repo:
CanonicalRepoName, apparent_repo: ApparentRepoName) -> Self` is exposed.
Display remains exact; `/workspace`, root context and `@first` renders
`observed-host-canonical-repository-apparent-mapping:"/workspace":@@:@first`.

Keep carrier fields and `CanonicalRepositoryApparentMappingResult` private.
Expose only concrete borrowed accessors returning
`&Arc<Result<HostCanonicalRepositoryApparentMapping,
HostCanonicalRepositoryApparentMappingError>>` and
`&PathObservationEpoch`.

Effective visibility requires one opaque projection. Rename the current
private terminal enum to
`CanonicalRepositoryApparentMappingObservationError`, retaining exactly its
RootMapping/Definition variants. Inner and wrapper each derive exactly Debug,
Clone, PartialEq, Eq, Allocative and Dupe. Child outcome, driver, finisher
and terminal creation keep this private inner. Add field-private
`pub(super) struct HostCanonicalRepositoryApparentMappingObservationError(
CanonicalRepositoryApparentMappingObservationError)` with matching derives.
Wrap only observed `Complete(Err(inner))` at Key projection. Add no public
field, variant, alias, constructor, conversion or inspector.

Permit only wrapper-spelling changes in existing
`observed_canonical_repository_apparent_mapping_identity_branch_and_terminal_algebra`:
the driver/finisher evidence names the private inner while Key-Value evidence
wraps the private Definition inner. Preserve every existing terminal,
dependency and epoch assertion, exact one-per-branch inner mapping and one Key
projection. No lifecycle proof adjustment or new generated-module test is
authorized.

Add exactly one test-only sibling smoke,
`canonical_repository_apparent_mapping_observation_surface_is_sibling_usable`,
with explicit imports of the three promoted names. It constructs only the key,
asserts exact Display and defines one nonexecuted `inspect` taking
`&<HostCanonicalRepositoryApparentMappingObservationKey as Key>::Value`, the
carrier reference and opaque-error reference. Its body assigns the accessors to
the exact concrete borrowed Result Arc and epoch types; its cast spells
`&SourcePreparationOutcome<Result<ObservedHostCanonicalRepositoryApparentMapping,
HostCanonicalRepositoryApparentMappingObservationError>>` plus the same two
references. It cannot construct or inspect carrier/outer,
compute, name the private alias/inner/variants, invoke root definition or
activate semantics. Production sibling imports remain unchanged.

Caps are <=80 generated production, <=50 generated proof, <=80 sibling proof,
<=210 aggregate and physical <=3,959/1,079. Add no production helper or
generated-module test and exactly one sibling smoke. Adjusted identity remains
below 200; smoke remains below 100. Add no new `rustfmt::skip`; existing skips
remain and all bytes must be rustfmt-stable. The two files remain cohesive in
their existing owner/test roles; no hot-path or retained-container change
applies.

Validate focused observed mapping and exact sibling smoke, protected root
apparent-definition and observed canonical-definition tests, full core, direct
commands check, `cargo fmt --all -- --check`, then exact two-file allowlist/
baseline-SHA/accounting/physical/test-size/visibility/source-shape and
`git diff --check` serially. Reuse accepted owner and opaque-wrapper proof; no
oracle is needed.

Mapping behavior/equality/invalidation/epochs/lower events remains exact. The
crate-internal opaque Result-Arc+epoch handoff is Slug-native. Canonical-
definition carrier visibility, root-definition owner, route/source/public/
command/bootstrap observation and exact identity bytes remain deferred.

STOP third file/type/key/carrier/adapter, crate-public or root export, public
field/alias/private-inner/variant exposure, canonical carrier visibility,
root-definition activation, semantic/event/epoch/retention drift, proof beyond
wrapper spelling plus the exact smoke, formatter/cap/test waiver, Cargo/BUILD,
fixture/oracle, upper work, milestone closure, M8/M7B or exact identity work.
REPLAN before widening or hash drift. ACCEPT returns only to a docs-only root
...
### Root apparent-route carrier-visibility design accepted (2026-08-21)

Committed audit `7444a51e` proves that the accepted private observed route has
zero production consumers, cannot be named by its sole future sibling source-
input owner, and needs no upper or parallel prerequisite. Schedule exactly
`WP-6-7A-host-root-apparent-repository-route-observation-carrier-visibility-implementation`.
Do not activate source input or any later source/public consumer.

Authority is exactly core `root_apparent_repository_route.rs`, baseline 1,890
physical/cfg(test) line 541 and SHA-256
`aa25fa3d36c6b9ba7ff5a9bb4ca6565f2cb2e8d579e6d4ab6721efaf8139d8d8`,
plus test-only `root_apparent_repository_source_input.rs`, baseline 814
physical/cfg(test) line 271 and SHA-256
`76893b9cfd6c7358260cafe60caa8c5a6922f6b7c6e85e791c3f5603360f1dd3`.
Every third Rust/API/export/caller/fixture/oracle file is read-only.

Promote exactly `HostRootApparentRepositoryRouteObservationKey`, its existing
two-argument `Option<Self>` constructor and
`ObservedHostRootApparentRepositoryRoute` to `pub(super)`. The key tuple field
and carrier fields stay private. Preserve root rejection and
`observed-{legacy Display}`; `/workspace`, `@first` renders exactly
`observed-HostRootApparentRepositoryRouteKey { workspace: NormalizedAbsolutePath { path: "/workspace" }, apparent_repo: ApparentRepoName("first") }`.

Expose only concrete borrowed carrier accessors named `result` and
`observations`, returning respectively
`&Arc<Result<HostRootApparentRepositoryRoute,
HostRootApparentRepositoryRouteError>>` and `&PathObservationEpoch`. Add no
observation Result alias, field, constructor, conversion or inspector.

Rename the current private outer to
`RootApparentRepositoryRouteObservationError`. It retains only
`Definition(HostRootApparentRepositoryDefinitionObservationError)`, existing
Debug/Clone/PartialEq/Eq/Allocative derives and manual Dupe; driver outcome and
driver terminals use only this inner. Add a same-derived/manual-Dupe,
field-private opaque wrapper exactly:
...
Only observed Key `Complete(Err(inner))` projection constructs the wrapper.
The associated Value remains the concrete
`SourcePreparationOutcome<Result<ObservedHostRootApparentRepositoryRoute,
HostRootApparentRepositoryRouteObservationError>>`. Need/success/legacy/
equality/validity/order/Arc/epoch/event/retention/cancellation stay unchanged.

Allow only wrapper/source spelling in existing
`observed_root_apparent_repository_route_identity_finisher_and_terminal_algebra`.
Its producer scan must prove exactly one private Definition mapping and exactly
one Key wrapper; preserve every semantic assertion. Real-family/event and
lifecycle/cancellation/nonactivation proofs remain byte-unchanged. Add no
route-module test.

Add exactly one source-input sibling test,
`root_apparent_repository_route_observation_surface_is_sibling_usable`, with
explicit test-only imports of the three promoted names. Construct only the
`/workspace`, `@first` key, assert the exact Display and use a nonexecuted
function/function-pointer cast to prove associated Value, opaque outer and
concrete borrowed Result-Arc/epoch accessor types. Do not construct or inspect
carrier/outer, name inner/variant, compute, invoke source input or activate a
graph edge. Production source-input imports/compute remain byte-unchanged.

Caps are <=80 route production, <=50 route proof, <=80 source-input proof,
<=210 aggregate additions and <=1,990/894 physical. Add no production helper
or route test and exactly one sibling smoke; identity stays below 200 and smoke
below 100. Add no `rustfmt::skip`, preserve existing skips and require
rustfmt-stable bytes with no waiver. No hot-path/retained-representation change
applies.

Validate the exact identity and sibling tests, protected observed route real-
family/event/lifecycle tests, legacy route/source-input and observed root-
definition tests, full core, direct commands check, formatting, exact two-file
allowlist/SHA/accounting/physical/test-size/effective-visibility/wrapper/source-
shape and diff hygiene serially. No visibility-only oracle is needed.

Route values/five-family projection/predecessor/views/source capability/
errors/order/equality/invalidation/lower events remain exact. The opaque same-
crate Result-Arc+transaction-local epoch handoff is Slug-native. Source-input
ownership, source path/source observation, public command/bootstrap activation
and exact identity bytes remain deferred.

STOP third file/type/key/carrier/adapter, crate-public/root reexport, public
field/alias/private-inner/variant/inspector, source-input production or caller
activation, semantic/order/event/equality/epoch/retention/cancellation drift,
proof beyond wrapper spelling plus one smoke, formatter/cap/test waiver,
Cargo/BUILD, fixture/oracle, upper work, milestone closure, M8/M7B or exact
identity work. REPLAN before widening or hash drift. ACCEPT returns only to
...
### Root source-input carrier-visibility audit selects same-crate design (2026-08-21)

Accepted source-input owner `ff0728ce` changed only
`root_apparent_repository_source_input.rs` by +941/-81 and left it at 1,716
physical lines. Its observation surface at lines 176-439 remains wholly
private: `HostRootApparentRepositorySourceInputObservationKey`,
`ObservedHostRootApparentRepositorySourceInput` and the typed Route outer have
zero production consumers. The legacy source-input key has exactly one
production consumer, sibling source-path input at its lines 27/234.

Source path first normalizes the requested path and then consumes exactly that
one legacy source-input predecessor. It is the natural next owner but cannot
name the observed Key associated Value, and using the legacy child would lose
the accepted epoch. Directly promoting the existing outer would reveal its
opaque lower route terminal. Effective visibility requires a private inner
`RootApparentRepositorySourceInputObservationError` plus field-private opaque
`pub(super) HostRootApparentRepositorySourceInputObservationError(...)`.

The source-path key has exactly one production consumer, host source
observation at its line 234, and that observation key has zero production
callers. Public commands instead use the parallel Bzlmod route keys at
`runtime/dice.rs:4476-4494`; imperative root bootstrap remains dormant. These
later or parallel branches neither consume this observation nor provide
visibility and are not prerequisites.

Choose exactly
`WP-6-7A-host-root-apparent-repository-source-input-observation-carrier-visibility-design`.
It must freeze only same-crate `pub(super)` key/new, carrier with private fields
and concrete borrowed source-input Result-Arc/epoch accessors, and the opaque
outer above. Add no observation alias; existing legacy aliases and their
visibility remain unchanged. Keep only the new inner, carrier fields, variant
and every lower opaque route terminal private. Add no crate-root export,
adapter, caller or semantic owner.

The exact Display remains `observed-{legacy Display}`; `/workspace`, `@first`
must render
`observed-HostRootApparentRepositorySourceInputKey { workspace: NormalizedAbsolutePath { path: "/workspace" }, apparent_repo: ApparentRepoName("first") }`.
Exactly one source-path test-only sibling smoke may construct only the key and
prove Display, associated Value/carrier/outer types and concrete accessor
function pointers. It may not construct or inspect carrier/outer, compute a
key or activate source path. Adjust only private-inner/wrapper spelling in the
existing source-input source-shape proof; all accepted semantic, identity,
family/event and lifecycle/cancellation evidence remains unchanged.

Prospective authority is exactly source input SHA-256
`619374fed097c1423037ab80268ef714800364b2f967eb6b7b446f8b99cf4b10`
at 1,716 physical/cfg(test) line 440 and source path SHA-256
`254303b96882a68329c334662d4e07bb728b0c6b0c3eb7f78adbaf44896ff200`
at 845 physical/cfg(test) line 300. Caps are <=80 source-input production,
<=50 colocated proof, <=80 source-path proof, <=210 aggregate additions and
<=1,816/925 physical. Add no production helper or source-input test, exactly
one source-path smoke below 100, no accepted-test enlargement, no formatter
waiver and no new `rustfmt::skip`.

Validation must cover the three accepted observed source-input tests, exact
sibling smoke, protected legacy source-input/source-path and observed route
tests, full core, direct commands check, formatting, exact two-file allowlist/
SHA/accounting/physical/effective-visibility/wrapper/source-shape and diff
hygiene. Source-input values/projection/order/errors/identity/policy/
invalidation/lower events remain exact. The opaque same-crate Result-Arc+epoch
...
STOP implementation in this audit/design packet, any third file/type/key/
carrier/adapter, crate-public/root export, public inner field/alias/variant,
caller/source-path compute, semantic/order/event/equality/epoch/retention drift,
proof beyond wrapper spelling plus one smoke, formatter/cap/test waiver,
Cargo/BUILD, fixture/oracle, upper work, milestone closure, M8/M7B or exact
identity work. REPLAN before widening or hash drift. M7 remains partial and
