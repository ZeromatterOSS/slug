...
| Adopted | Stage 6 | Retained ListGen/ListLike and DictGen/DictLike, Box slices, SmallMap, Trace/Freeze/Allocative; existing Arc occurrence and publication-bijection pattern | WP-7-26 adds evaluator-local read-only collection variants without enlarging normal mutable values; HeaderInfo retains a typed Arc occurrence through publication. No new cache or global counter | Pinned shallow copy/mutation/hash source, GC and freeze, provider/depset hash, native token alias/round-trip and authentic CcInfo-to-Rustc proof; no performance claim |
| Adopted | Stages 7/11 | Existing Tokio bounded mpsc, futures, SHA-256 and pinned ByteStream types | WP-7-29 uses one-slot reserve-before-read flow control and cancellation-aware producer/RPC selection for verified reader uploads. Vec payloads, SHA-256 state and resource strings are transfer scratch; no new retained graph, cache, lock, dependency or donor code | Tiny chunks/empty/short/interrupted reads, corrupt/short/extra/error sources, early/failed RPC, cancellation/drop and bounded demand; public NativeLink round trip; no throughput claim |

| Adopted | Stage 7 | Existing SmallSet, Arc retained closure/facts, Allocative and shared PathObservationEpoch union; existing sorted REAPI Directory maps and bounded CacheClient reader | WP-7-31 uses structural source-artifact deduplication and retains validated producer state with a full epoch; no cache/interner or lock across compute. Source handles and upload maps are operation scratch; Directory order remains canonical and file bytes stream without graph retention. No donor code | Native closure selection/content restoration/conflict and invalid-shape rejection; full-frontier association; source FileNode executable policy, protected inline bytes, public verified CAS staging/hit/mutation proof. No performance improvement claim |

| Adopted | Stages 6/7 | Existing canonical labels, NormalizedAbsolutePath, Arc observation epochs, Allocative/Dupe and fixed FileContentDigest | WP-7-30 retains source label, namespace, requested/physical path and digest with a full shared-result frontier under one Core key. ResolvedPath remains the existing producer's value and transient join input; no source bytes, new string/path spelling, interner, lock or secondary digest cache | Source-typed rejection, Need/error invalidity, namespace selection, metadata versus digest equality and native request-driver content/path restoration/final validation; no throughput or retained-size improvement claim |

| Adopted | Stages 2/7 | Existing DICE keys, immutable Arc observation epochs, Allocative/Dupe and workspace SHA-256 dependency | WP-7-27 adds a 40-byte digest/size fact and semantic/observed path projection; Core uses fixed 64 KiB synchronous scratch, no file-sized retained allocation or digest cache. Reuse path resolution, read-error classification and validation owners; no donor code | Standard vectors, interrupted/short/failing bounded reads, native file/link/missing/directory/FIFO observations, complete frontier and same-DICE content cutoff/edit/restoration; no throughput claim |
| Adopted | Stage 6 | Existing ArtifactInputs/shared depset DAG, CompactString, Allocative, Trace and shared PublicationEqState | WP753 retains Cargo runfiles input identity plus exact fake artifact/workspace captures; evaluator captures are traced then lowered, rendered mappings remain scratch. No new cache, interner, DICE key or donor implementation | Exact artifact filtering and shared DAG/capture equality, unchanged pinned helper/source authentication, explicit GC, public context and forced parameter-byte proofs; derived paths stay Slug-native, no performance claim |
| Adopted | Stage 6 | Existing Arc<[AnalysisValue]>, Allocative, shared AnalysisValueLowerer/PublicationEqState and SmallSet | WP-7-25 retains complete native-link input tuples in a dedicated recipe; transient directory dedup and rendered flags stay scratch. No duplicated semantic cache, flattened retained strings or donor implementation | Pinned rustc/utils algorithms; regular-file/type rejection; unused-field and cross-Spawn depset alias discrimination; real Linux builder direct/indirect/include-flags and imported-source A/B/A; no performance claim |
| Adopted | Stages 4/6 | Existing immutable Arc slices, CompactString, SmallSet and Allocative in Bzl loading | WP-7-24 replaces flat lexical reachability rows with identity plus observed SHA-256 and carries the same row through the existing filename table. No extra per-rule index, interner, cache or donor code; +32 bytes per provenance row, shared context clones | Observed diamond exact digests/order, imported-helper A/B/A, caller mismatch and authentic configured Rustc tests; no performance claim |
...
| Adopted | Stage 6 | Existing one-byte typed configuration-field identity, structural native option vector, command overlay, ordinary/lifted configured-dependency resolver, frozen evaluator values, and root/subrule fragment lifecycle; reviewed against Bazel 9.2 coverage sources, starlark-rust/Buck2 compact enum patterns, and Zabel `0795445f3ab60f4e49070bdd0b94425c5610f73a` declaration/materialization split | Generalize the C++-only field newtype into one closed typed multi-fragment enum with `Coverage(OutputGenerator)`. Project the already-retained coverage Boolean/label through the sole resolver and an unrestricted public coverage facade. Add no caller manifest, BuiltinRestriction, raw string fallback, second label parser/store/resolver, DICE key, cache, lock, aspect execution, instrumentation/action behavior, parser, ruleset, or C++ branch | Pinned one-field CoverageConfiguration/API/global/test sources; field/layout/tools-repository A/B/A; false/true/false and mapped override; exact command forms; ordinary/lifted target/exec validation; non-allowlisted root/subrule public facade; conversion controls; 63 configuration, 28 command, 553 loading, and 119 analysis tests; authentic replay to `cpp.custom_malloc`; independent terminal implementation `ACCEPT` |
| Adopted | Stage 6 | The adopted one-byte closed configuration-field enum, structural CppOptions vector, mapping-aware command overlay, ordinary/lifted resolver, and shared root/subrule C++ fragment value; reviewed against Bazel 9.2's complete eleven-field C++ inventory and Zabel `0795445f3ab60f4e49070bdd0b94425c5610f73a` typed declaration/projection split | Add sole missing `CustomMalloc` discriminants and project the already-retained nullable label through existing owners. Give only the public `custom_malloc` facade field unrestricted access while preserving every private C++ method check. Reject known non-Boolean native `--no` forms generically. Add no string fallback, second option/label/resolver/fragment owner, DICE state, ruleset, parser, provider-constrained target invocation, or C++ rule semantics | Pinned CppConfiguration/API/Options/integration/parser sources; exact eleven-member inventory; default/label/default mapping and DICE restoration on admitted schemas; non-allowlisted public root/subrule facade plus private-method denial; exact command/no-prefix matrix; 64 configuration, 29 command, 553 loading, and 120 analysis tests; authentic replay advances to imported attribute-descriptor identity; R1 architecture, focused R2 correction, and terminal implementation `ACCEPT` |
| Adopted | Stage 6 | Existing `AttributeDefinitionGen<Value/FrozenValue>`, frozen rule schema, starlark-rust referenced-heap retention, and Slug frozen-module lifetime closure; reviewed against Bazel 9.2 immutable descriptors, Buck2's owned semantic attribute wrapper, and Zabel `0795445f3ab60f4e49070bdd0b94425c5610f73a` declaration-owner/dictionary-union pattern | Replaced the partial frozen-descriptor conversion with one complete consumer-neutral transient projection. Clone only existing compact/`Arc` facts and project embedded aspect/transition `FrozenValue` pointers; keep the original frozen schema, module heaps, consumer validation, and DICE owners. Added no dynamic registry, repr/source fallback, interner, cache, second schema, raw pointer, parser, rule semantics, or provider/aspect/transition execution | Pinned Bazel Descriptor/rule-consumer/tests; fresh two-hop dictionary-union oracle with producer-relative default, provider, aspect, and transition; local/frozen field and pointer identity; six-consumer acceptance/rejection matrix; invalid errors; source A/B/A; 556 loading passes plus one ignored and 120 analysis passes; authentic rules_rust replay advances to nonempty transition inputs; independent architecture and terminal implementation `ACCEPT` |
| Adopted | Stage 6 | Existing V2 `CanonicalLabel`, `CompactString`, immutable `Arc` slices, frozen transition value, package load fingerprint, schema equality, and typed root build-setting execution; reviewed against Bazel 9.2 regular-transition canonical maps/internal-string ordering, Buck2 `088c75c7e36805df99c3de29062baa95db700b8b` frozen transition ownership, and Zabel `0795445f3ab60f4e49070bdd0b94425c5610f73a` typed input/output capture | Replaced the scalar root-only output with one V2-owned typed setting record and canonical-sorted immutable input/output slices. Retain original spelling beside canonical definition-repository identity; apply native-name policy only to the exact raw prefix; preserve Bazel's two-phase error order and all admitted typed root execution; reject native/external/new shapes before DICE lookup or invocation. Use construction scratch only and add no retained map, second schema, repository-mapping copy, interner, cache, evaluator borrow, new DICE key, input/native/split execution, rule-level transition, parser, or ruleset branch | Pinned Bazel API/implementation/internal-string sources and tests plus fresh empty/multi/sorted/external-owner, raw-native alias, BMP/supplementary order, disabled-option, dual-invalid precedence, and raw/canonical input/output duplicate oracle; focused binding/policy/mapping/import/equality/A-B-A/pre-DICE fail-closed/typed execution proofs; 559 loading passes plus one ignored and 121 analysis passes; rebuilt rules_rust replay to generic `rule(cfg = transition)` binding; independent architecture and terminal implementation `ACCEPT` after one diagnostic-order correction |
| Adopted | Stage 6 | Existing V2 live/frozen/final regular-transition object, immutable `Arc<[TransitionSetting]>` slices, `Allocative` rule/package identity and load fingerprint; reviewed against Bazel 9.2 rule `cfg` conversion/allowlist sources, Buck2 `088c75c7e36805df99c3de29062baa95db700b8b` frozen transition lifetime, and Zabel `0795445f3ab60f4e49070bdd0b94425c5610f73a` producer-owned shared rule/attribute consumer pattern | Reuse the sole transition projection for named-only regular `rule(cfg = transition)` attachment through live/frozen/imported rule definitions and final targets. Include it in structural equality and generate the existing hidden allowlist for either rule or attribute consumers. Clone only the frozen callable and existing immutable slices; add no second transition schema, registry, ordinal, interner, cache, source/mapping reconstruction, DICE key, configuration placeholder, native/composed/parent transition, or execution semantics | Commit `6cb5ab55b`; pinned Bazel API/implementation/provider tests; focused binding/diagnostic, import/re-export, callable/setting identity, either/both/neither allowlist, equality and A/B/A proofs; pre-selector/dependency/toolchain/implementation analysis rejection; 560 loading tests plus one ignored and 122 analysis tests; rebuilt rules_rust replay; independent architecture and terminal implementation `ACCEPT` after a focused dual-invalid precedence correction |
| Adopted | Stages 4 / 6 | Existing V2 `CompactString`, immutable `Arc` slices, `Allocative`, canonical labels, generated-file package targets and frozen-module lifetime; reviewed against Bazel 9.2 `rule(outputs=...)`, `StarlarkCallbackHelper`, implicit-output, Rule and RuleContext sources, Buck2 `088c75c7e36805df99c3de29062baa95db700b8b` compact retained utilities, and Zabel `0795445f3ab60f4e49070bdd0b94425c5610f73a` separate generated-output package facts and analysis projection | Add one V2-owned ordered static-or-callback rule-output definition and one compact resolved key/label slice. Resolve complete ordinary/keyword-only/residual callback parameter names and templates synchronously during package loading; retain only final output identity plus `output_to_genfiles`; keep implicit outputs separate from attributes and borrow the same slice for generated targets, `ctx.outputs` and omitted/absent default providers. Reserve executable/test key `executable`. Add no second registry, synthetic schema, source text, mapping copy, interner, cache, DICE key, output scan or analysis fallback; import no V1/Buck2/Zabel semantics or representation. Admit only default merged genfiles; split-root option construction and bytes remain deferred | Pinned Bazel API/implementation/template/package/context sources and tests; complete binding/template/callback including a mixed ordinary/keyword-only/`*args`/`**kwargs` proof, order/direct-collision/visibility/identity/A-B-A, absent-provider/reserved-key and generated-target query-owner proofs; 360/472 production and 416/452 proof lines; complete loading, 123 analysis and 123 query tests; rebuilt rules_rust replay; R2 terminal `REPLAN`, focused R3 architecture and terminal correction implementation `ACCEPT` |
...
| Proposed | Stages 4 / 6 | Existing Starlark simple-value equality/hash plus V2 `allocative::Allocative`, closed compact enums, `SmallSet`, `SmallMap` and immutable `Arc` provider-schema storage | Add one V2-owned one-byte two-variant declaration-only Apple provider-key kind. Lower it to the existing `ProviderIdentity` only for loading schemas; reject the sibling before configured-target membership/index lookup. Import no Buck2/V1 code and add no string, map, set, interner, cache, second provider identity or graph representation | Compile-time one-byte assertion; variant/name/equality/hash and ordinary-key separation; loading schema order/dedup/freeze; configured membership/index rejection even against coincidentally named retained providers; unchanged normal-provider behavior; authenticated rules_rust replay |
...
the mandatory full suite then exposed one protected assertion that still
...
surface, representation, lifetime or cap changes; independent review was
...
R1 review returned `REPLAN` on constructor/default/root-presence semantics, not
on ownership or extraction. R2 requires an empty list beside `"all"`, one
...
`WP-4-6-7A-java-configuration-field-declaration-fail-closed-audit-r3` reuses
...
and immutable late-bound slices remain the sole declaration owners. The next
replay stop is outside this representation: an imported `.bzl` helper records
a transient filegroup before Label caller-provenance rejection, while atomic
package failure prevents publication. Its later initializer-bearing target is
not reached.
