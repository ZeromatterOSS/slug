...
| Proposed | Stage 8 | `java_regex` 0.1.0 candidate; upstream commit `ed518dc23dacbe1a88d7cb3f26f0cfe31cc91393` | Qualification/reference-only pending immutable published-crate source/checksum, license, and MSRV pin plus exact OpenJDK 25 acceptance; reject the current allocation-heavy production API and port/design only an accepted allocation-free boolean-find boundary | Independently generated OpenJDK 25.0.2/Bazel 9.2.0 corpus for compile diagnostics, `Matcher.find`, Java-only constructs, Unicode/UTF-16, supplementary and unpaired-surrogate/NUL behavior, and bounded resources; reject on any mismatch, unverifiable source, uncontrolled resource behavior, or unacceptable hot-path allocation boundary |
