...
V2 fixture: `rules-cc-basic`, `bazel-skylib-basic`, `rules-python-basic`, `protobuf-basic`, `rules-rust-basic`
Expected evidence artifact: Stage 1 oracle expected output generated by `tools/v2_oracle --update-expected` for each fixture
Implementation summary: Rewrote Plan34 `rules_cc` behavior into a Bazel 9 bzlmod fixture and added fresh skylib/python/protobuf/rules_rust public ruleset fixtures without importing V1 execution or output-root assumptions
Validation: `py -3 -B -m tools.v2_oracle run --fixture rules-cc-basic --tool bazel --bazel C:\ProgramData\chocolatey\bin\bazel.exe`; same command for `bazel-skylib-basic`, `rules-python-basic`, `protobuf-basic`, and `rules-rust-basic`
Residual risk: rules_oci fixture remains pending; output/runfiles comparisons need platform-aware oracle manifests before upgrading beyond message-shape checks
