...
| Proposed | Stage 6 | Existing `AttributeDefinitionGen<Value/FrozenValue>`, frozen rule schema, starlark-rust referenced-heap retention, and Slug frozen-module lifetime closure; reviewed against Bazel 9.2 immutable descriptors, Buck2's owned semantic attribute wrapper, and Zabel `0795445f3ab60f4e49070bdd0b94425c5610f73a` declaration-owner/dictionary-union pattern | Replace the partial frozen-descriptor conversion with one complete consumer-neutral transient projection. Clone only existing compact/`Arc` facts and project embedded aspect/transition `FrozenValue` pointers; keep the original frozen schema, module heaps, consumer validation, and DICE owners. Add no dynamic registry, repr/source fallback, interner, cache, second schema, raw pointer, parser, rule semantics, or provider/aspect/transition execution | Pinned Bazel Descriptor/rule-consumer/tests; fresh two-hop dictionary-union oracle with producer-relative default, provider, aspect, and transition; local/frozen field and pointer identity; admitted consumer/rejection matrix; source A/B/A; authentic rules_rust replay; independent architecture and terminal implementation reviews |
