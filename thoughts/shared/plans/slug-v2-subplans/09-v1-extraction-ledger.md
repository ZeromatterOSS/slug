...
| Proposed | Stages 4 / 6 | Existing V2 `CompactString`, immutable `Arc` slices, `Allocative`, canonical labels, generated-file package targets and frozen-module lifetime; reviewed against Bazel 9.2 `rule(outputs=...)`, `StarlarkCallbackHelper`, implicit-output, Rule and RuleContext sources, Buck2 `088c75c7e36805df99c3de29062baa95db700b8b` compact retained utilities, and Zabel `0795445f3ab60f4e49070bdd0b94425c5610f73a` separate generated-output package facts and analysis projection | Add one V2-owned ordered static-or-callback rule-output definition and one compact resolved key/label slice. Resolve callbacks/templates synchronously during package loading; retain only final output identity plus `output_to_genfiles`; keep implicit outputs separate from attributes and borrow the same slice for generated targets, `ctx.outputs` and default files. Add no second registry, synthetic schema, source text, mapping copy, interner, cache, DICE key, output scan or analysis fallback; import no V1/Buck2/Zabel semantics or representation | Pinned Bazel API/implementation/template/package/context sources and tests; complete binding/template/callback/order/direct-collision/visibility/identity/A-B-A/nonmerged-stop and generated-target query-owner proofs plus full loading/configuration/analysis/query gates; rebuilt rules_rust replay; independent architecture and terminal implementation review |
