...
inherited SelectedRegistry proof, real core branch/lifecycle proof,
...
Independent review accepts `f7566280` at 70/145/215 formatted Rust lines and
11,397/361 physical lines; focused proof and full Bzlmod/loading validation
pass. Run only four-ledger docs packet
`WP-4-5-6-host-root-apparent-repository-source-input-owner-design` under
40/300/220/220/780. Freeze a private Need-aware core owner over the accepted
root-apparent carrier and accepted Bzlmod certificate, retaining the complete
predecessor and projecting Main or Input once. Future Rust is exactly a new
cohesive core module, minimal `pub(super)` carrier seam, and private mod line
under 320/650/970 with 1,100/800 ceilings. Defer paths, legacy demand scope,
...
source/materialization/I/O/core/loading/command/public/wire/JVM stop. Stage 5
remains unchanged.
...
40/320/240/240/840. Map exact Builtin catalog-byte ownership and request-backed
materialization/path/file observation, the first legitimate Need, private key
visibility, legacy module-name demand scope, and choose one smallest
dependency-safe successor or prerequisite REPLAN. Authorize no Rust,
consumer/key/store, materialization/source/package/loading/command/public/
wire/I/O/reverse-edge/JVM work. Stage 5 behavior remains unchanged.
...
40/360/260/260/920. Freeze one hidden Bzlmod key over the exact source-input
certificate plus validated path. Builtin delegates once to the pinned catalog
key; Request delegates once to the existing private materialization-result key
then existing file observation. Preserve branch-specific value/error/Need and
Arc identity without legacy demand metadata. Future Rust is only
`source_preparation.rs` and hidden `lib.rs` under mandatory 420/800/1,220 and
12,250/430. No Rust/core/loading/command/public/new-I/O/reverse-edge/JVM work is
authorized. Stage 5 remains unchanged.
...
no-caller/core/loading/command/legacy-demand/second-result/new-I/O/public/wire/
reverse-edge/JVM stop. Stage 5 remains unchanged.
...
retained semantic state changed. Commit `5c3b4492f` restores the archive app
gate for the accepted crate; Stage 4 may now retry the complete rules_cc
compilation-helper freeze. No additional Stage 5 implementation is selected.
