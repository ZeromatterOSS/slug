...
WP-7-14's single CLI-root aquery exited 0 in 5.12 seconds with zero actions
run. The new cache-leaf build-script output tree, artifact 3518, appears in
the leaf Rustc input depset chain 615 → 616 → 617. The producer has all 15
pinned proto files and eight vendored protoc inputs, selecting Linux x86_64;
the old adapter build script is absent. The LALRPOP tree, artifact 3502,
remains in its Rustc chain 884 → 885 → 886. Raw JSON SHA-256 is
`ffc50a44648f85b464988eb5dd6c6dd152269f342121a006839d3e8c7b4c0be8`.
The `--nofetch` external-repository warning limits this to first-party
declaration evidence; crate freshness, generated bytes and Bazel compilation
remain unproved.
WP-7-15 then built only `//app/slug_reapi_cache_v2:build_script` with the
nightly toolchain and `--lockfile_mode=error`. It exited 0 in 31.36 seconds,
running 396 actions and producing six nonempty Rust modules matching every
leaf `tonic::include_proto!` name. This establishes materialized generated
...
The selected `WP-7-10-m7a-local-generated-input-aquery-r1` permits one
bounded offline CLI-root action query for the first-party LALRPOP and REAPI
proto build-script producer/consumer edges. It can establish configured
generated-input declaration only; no action runs and generated bytes,
compilation and M7A stay open.
The sole generated-input aquery exited 0 in 6.267 seconds with zero build
actions executed. Exact Starlark syntax and REAPI build-script `out_dir` tree
artifact IDs 3506 and 3458 appear in the respective library Rustc input
depset closures. Producer inputs include the grammar, five protos and eight
vendored protoc files; REAPI's environment selects Linux x86_64 protoc.
Receipt and parsed analysis SHA-256 values are
`f30b6ba9450cd26b36fe8bdcfd6e6bc529fb36d8bc6226b5459ac1389c3deb6e`
and `f77396e8ea9f7fdf59d05bdd18425ed6bc42c29559bc1b60e3ea3a2379021836`.
Independent final review `ACCEPT` confirmed only configured input wiring.
Generated bytes, execution, compiler success and M7A remain open.
