...
`HostRootApparentRepositorySourcePathInputKey` to that owner. Determine the
smallest private core key or computation-free adapter that can preserve Main as
an explicit no-source disposition and, only for Input, compute the hidden
`HostRepositorySourceObservationKey` once with the exact accepted source input
and relative path. Freeze whether the natural owner retains the exact completed
source-path predecessor Arc plus the exact observation Arc, how inherited Need
is forwarded, and how path/source-input versus Builtin/Request errors remain
ordered and structurally typed.

Map the concrete visibility and dependency edges among the existing private
core source-path module, the hidden Bzlmod observation key/outcome/view, and
later loading package/source consumers. Prove core already depends on Bzlmod
and can consume the hidden ABI without a reverse edge; Bzlmod must not depend on
core. Determine whether an opaque `pub(super)` predecessor seam is sufficient
or whether a smaller borrowed view/accessor prerequisite is required. Main
must not fabricate a source input or observation. Builtin must retain the exact
catalog value/error/integrity fields; Request must retain the exact accepted
materialization/source value/error and first Need. No consumer may reconstruct
a request, path, module name, route, or `RepositorySourceScope`.

Resolve the ownership decision explicitly: the core predecessor currently
publishes borrowed `Main | Input(&HostRepositorySourceInput)` and
`&HostRepositoryRelativePath`, while the observation-key constructor consumes
owned certificates. Either admit the compact Slug-native clones at this DICE
boundary and prove that the retained request and path Arcs are pointer-shared,
or REPLAN to a smaller Arc-retention/borrowed-constructor prerequisite. Do not
claim no-copy, deep-copy RepoSpec/catalog state, or authorize both alternatives.

Audit equality, validity, lifetime and activation requirements for request
workspace/apparent/path plus predecessor/observation identity, including
Main/Builtin/Request, Need invalid/self-unequal, complete structural equality,
A/B/A restoration, zero event data, and no second observation/result lookup.
Classify accepted path/source-input association and Builtin/Request observation
as exact; private carrier/key/error/lifetime scheduling as Slug-native; and
loading/package/command/public migration, legacy demand replacement, generated
custom repository execution, new transport/materialization behavior and I/O as
unsupported/deferred.

Completion evidence is a concrete ownership/call map, one smallest atomic
successor with exact future files/ABI/caps/proof/stops, or a precise smaller
prerequisite REPLAN. Authorize only these four ledgers under documentation caps
40 canonical/320 current/240 Stage 4/240 Stage 5/840 total. Authorize no Rust,
new key/store, caller activation, source/package/loading/core behavior change,
legacy demand mutation, public/server/command surface, new materialization/I/O,
reverse dependency, JVM, or fifth file. Require independent acceptance before
completion.
...
docs-only audit contract above. Implementation `cbc44e43` completes the hidden
Bzlmod source-observation owner: exact two-file scope, 748 formatted added
lines within 420/800/1,220, physical 12,247/379 within 12,250/430, focused 6/6
and full 365/365 Bzlmod tests, clean diff hygiene, and three independent final
acceptances. The key preserves exact Builtin and Request branches, one
request-result dependency, first Need, full structural identity/errors and
callerless no-legacy-demand boundaries.

## Accepted docs-only design contract

This section and everything below is historical context only, grants no file,
action, cap, or schedule authority, and is interpreted only through the active
docs-only audit contract above. Design `7ef0c353` is the complete predecessor.

Independent review accepts audit `b6a90390`. The smallest dependency-safe
successor is Bzlmod-owned: core cannot access the private materialization-result
owner without visibility widening, while Bzlmod can consume the accepted hidden
source input without a reverse edge.

Freeze hidden key `HostRepositorySourceObservationKey { input:
HostRepositorySourceInput, relative_path: HostRepositoryRelativePath }` with
exact constructor `new(input, relative_path) -> Self`. Derive
Debug/Clone/PartialEq/Eq/Allocative and implement Hash manually over the full
capability structural Hash, disposition discriminant, Request ID framing on
the Request branch, and exact validated relative-path bytes. Capability Hash
already covers workspace/apparent/canonical, Builtin identity or complete
RepoSpec attributes plus local-path policy; request kind is deterministically
derived from that exact capability by the private constructor. Thus same-ID
requests with changed RepoSpec/policy/kind must hash differently without a
second request classifier or invented request hasher. The input remains the sole capability/request association
owner; do not reconstruct a request, classify a rule/policy, or revalidate the
path. Key `provide` publishes no `RepositorySourceScope` or module name.

Freeze value `HostRepositorySourceObservation::{Builtin(
BuiltinBazelToolsSourceFileValue), Request(HostRepositorySourceFileValue)}` in
exact aliases `HostRepositorySourceObservationResult =
Result<HostRepositorySourceObservation, HostRepositorySourceObservationError>`
and `HostRepositorySourceObservationOutcome =
SourcePreparationOutcome<Arc<HostRepositorySourceObservationResult>>`.
Freeze borrowed `HostRepositorySourceObservationView<'a>::{Builtin(&'a
BuiltinBazelToolsSourceFileValue), Request(&'a
HostRepositorySourceFileValue)}` and exact `view(&self) -> ...View<'_>`.
Preserve the branch
rather than flattening it: Builtin retains exact catalog path/bytes/SHA-256/
executable; Request retains exact Present bytes/logical path or Absent.
Structural Complete equality; Need is invalid and self-unequal; no event data.

Compute inspects `input.view().disposition()` exactly once. Builtin converts
the already-validated path to UTF-8 without rewriting; non-UTF-8 completes a
typed `BuiltinPath` terminal retaining the full input/path. Otherwise compute
only `BuiltinBazelToolsSourceFileKey::new(identity.snapshot(), exact_path)` and
wrap its exact success/error or compute message; it never creates a request or
filesystem observation. Request computes only the existing private
`RepositoryMaterializationResultKey { request: exact Arc }`, forwards its Need
unchanged, then calls existing
`observed_repository_source_file_from_materialization` once and projects with
existing `host_repository_source_file_value`. Preserve existing materialization
error, resolved-path, lstat/file-byte Need/error ordering and Local/Immutable
namespaces. Do not compute `HostRepositoryPathKey` and do not repeat the result
dependency as current `HostRepositorySourceFileKey` does.

Freeze exact error `HostRepositorySourceObservationError { input:
HostRepositorySourceInput, relative_path: HostRepositoryRelativePath, kind:
HostRepositorySourceObservationErrorKind }` and private kind exactly
`BuiltinPath | Builtin(BuiltinBazelToolsSourceFileError) |
BuiltinCompute(Arc<str>) | Request(RepositorySourceFileError) |
RequestCompute(Arc<str>)`. `Builtin` and `Request` retain the exact typed inner
terminal; only `BuiltinPath` has no subordinate key/error and only the two
Compute variants replace a failed key computation with its exact message.
Derive Debug/Clone/PartialEq/Eq/Allocative; implement Display/Error without
parsing Display; expose borrowed `input(&self) -> &HostRepositorySourceInput`
and `relative_path(&self) -> &HostRepositoryRelativePath`. No error may discard
input, path, request Arc, Builtin identity, or existing typed terminal. If
existing private errors make this exact representation infeasible without
copying or Display parsing, REPLAN before activation.

Future Rust is exactly existing `app/slug_bzlmod_v2/src/source_preparation.rs`
and hidden exports in existing `app/slug_bzlmod_v2/src/lib.rs`; mandatory caps
are 420 production/800 tests/1,220 total with mandatory physical ceilings
12,250/430 against measured live 11,511/367; REPLAN on excess. Proof must cover
Builtin file/directory/UnsupportedCatalog/integrity and non-UTF-8 path; Request first Need,
Local/Immutable present/absent/wrong-kind/file errors and recovery; exact
request/path/input Arc provenance; workspace/apparent/canonical/source/policy/
spec/path A/B/A, including same request ID with changed RepoSpec/policy/kind;
branch inequality; Hash/Eq consistency; lifecycle/no events; external hidden ABI;
and zero route/core/package/loading/command or second result activation.

Exact compatibility is accepted branch selection, Builtin catalog content/
integrity, request materialization and source-observation behavior. Hidden key,
branch/error algebra, Hash and scheduling are Slug-native. Main handling, core
caller/dispatch, legacy key migration and `RepositorySourceScope`, package/
loading/command/public API, generated-local/custom rules, transport execution,
and new I/O remain unsupported/deferred. STOP on core/loading/server/command
edits, third Rust file, public API, legacy caller/key/demand mutation, second
key/store/result lookup, copied request/RepoSpec/catalog, path rewriting,
classification, new materialization/I/O semantics, reverse edge, JVM, or cap/
ceiling excess. Require independent design acceptance and later activation.

## Accepted docs-only audit contract

This section and everything below is historical context only, grants no file,
action, cap, or schedule authority, and is interpreted only through the active
docs-only design contract above. Audit `b6a90390` selects the Bzlmod-owned
...
This section and everything below is historical context only, grants no file,
action, cap, or schedule authority, and is interpreted only through the active
docs-only design contract above. Design `68349398` and implementation
`bd337622` complete the private path-first source-input owner.
...
Independent review accepts `b46c2c63`: the pure hidden Bzlmod relative-path
owner calls the sole checker once, retains one exact Arc, has no callers or
DICE/I/O edge, and passes all 359 Bzlmod tests. Physical files are 11,511/367;
three independent final reviews accept.

Run only four-ledger docs packet
`WP-4-5-6-host-root-apparent-repository-source-path-input-owner-design` under
40/300/240/240/820. Authorize no Rust yet. Freeze private core key
`HostRootApparentRepositorySourcePathInputKey { workspace, apparent_repo,
requested_path }`, rejecting root apparent requests. Compute order: call
`host_repository_relative_path(self.requested_path.clone())` synchronously
before any await; Path error completes immediately with zero source-input/route
activation; then compute only `HostRootApparentRepositorySourceInputKey`,
forward Need exactly, wrap compute/completed terminals, validate association,
and retain the exact completed predecessor Arc plus validated relative path.

Freeze the key fields as `workspace: NormalizedAbsolutePath`, `apparent_repo:
ApparentRepoName`, `requested_path: PathBuf`, with exact constructor
`new(workspace: NormalizedAbsolutePath, apparent_repo: ApparentRepoName,
requested_path: PathBuf) -> Option<Self>`. The sibling seam freezes
`HostRootApparentRepositorySourceInputResult =
Result<HostRootApparentRepositorySourceInput,
HostRootApparentRepositorySourceInputError>` and its existing outcome type.

Value is `SourcePreparationOutcome<Arc<Result<certificate,error>>>`.
Freeze aliases `HostRootApparentRepositorySourcePathInputResult =
Result<HostRootApparentRepositorySourcePathInput,
HostRootApparentRepositorySourcePathInputError>` and
...
Certificate fields are `workspace: NormalizedAbsolutePath`, `apparent_repo:
ApparentRepoName`, `predecessor:
Arc<HostRootApparentRepositorySourceInputResult>`, and `relative_path:
HostRepositoryRelativePath`.

Freeze `HostRootApparentRepositorySourcePathInputView<'a>` and
`HostRootApparentRepositorySourcePathInputDispositionView<'a> { Main,
Input(&'a HostRepositorySourceInput) }`. Exact accessors are
`apparent_repo(self) -> &'a ApparentRepoName`, `canonical_repo(self) -> &'a
CanonicalRepoName`, `relative_path(self) -> &'a HostRepositoryRelativePath`,
and `disposition(self) -> ...DispositionView<'a>`; certificate
`view(&self) -> Option<...View<'_>>` fails closed on association mismatch.

Freeze error fields `workspace: NormalizedAbsolutePath`, `apparent_repo:
ApparentRepoName`, and private kind exactly
`Path(HostRepositoryRelativePathError) | Source { relative_path:
HostRepositoryRelativePath, predecessor:
Arc<HostRootApparentRepositorySourceInputResult> } | InvalidSource {
relative_path: HostRepositoryRelativePath, predecessor: Arc<...Result> } |
Compute { relative_path: HostRepositoryRelativePath, message: Arc<str> }`.
Path owns raw input. Only Path occurs before a predecessor is requested; only
Compute lacks a completed predecessor after that request. Success, Source, and
InvalidSource retain the exact completed Arc. Structural traits; Need
invalid/self-unequal; Complete structural; no event data.

Minimal sibling seam may widen only source-input key/new/outcome/opaque result,
view and disposition accessors to `pub(super)`. Future Rust is exactly new
`root_apparent_repository_source_path_input.rs`, minimal seam in existing
source-input module, and `runtime/mod.rs`. Mandatory caps 340 production/700
tests/1,040 total; physical ceilings 840 predecessor/850 new. REPLAN on excess.

Proof: exhaustive path-before-source dispatch; invalid path zero activation;
valid path exact source Need forwarding; Source/InvalidSource/Compute identity;
real Main/Builtin/SelectedNonregistry root+command/Generated and inherited
Registry; predecessor/request/path Arc provenance; association corruption;
workspace/apparent/path/canonical/spec/policy/mapping/order A/B/A; warmed source
owner Reused then path owner Evaluated→Reused, no events; tracked and structural
zero result/path/source/package/loading/registry/filesystem/I/O edges.

Exact compatibility is accepted relative-path/error ordering and source-input
identity. Private key/error/certificate/lifetime/scheduling is Slug-native.
Materialization result/ResolvedPath/Builtin bytes/legacy demand/source/package/
loading/command/public/I/O remains deferred. STOP on Bzlmod/loading/server/
command/Cargo edits, fourth Rust file, public ABI, second validation/projection/
lookup/store, copied graph state, materialization/source behavior, module-name
synthesis, reverse edge, JVM, or cap excess. Require acceptance and activation.

## Accepted implementation contract

This section and everything below is historical context only, grants no file,
action, cap, or schedule authority, and is interpreted only through the active docs-only audit contract above. Design `4d96d094` and implementation
`b46c2c63` complete the relative-path prerequisite.

## Accepted docs-only design contract

This section and everything below is historical context only, grants no file,
action, cap, or schedule authority, and is interpreted only through the active docs-only audit contract above.
...
Design `e659b0e3` activated exactly the new private core source-input module,
minimal `pub(super)` route-carrier seam, private mod declaration, and ledgers
under 320/650/970 and 1,100/800. Implementation `e4292de7` satisfies that
contract and is complete. No path/result/source/package/loading/command/public
API/I/O/reverse-dependency/JVM behavior was activated.
...
Independent review accepts implementation `f7566280`: the hidden Bzlmod
certificate calls the accepted materialization-request projection exactly once,
retains the exact capability plus Builtin identity or request Arc, and adds no
key, path, Need, result compute, source consumer, or I/O. Focused proof, all 358
Bzlmod tests, and all 110 loading tests pass. Formatted accounting is 70
production/145 tests/215 total; physical files are 11,397/361, within
140/320/460 and 11,400/390. Clippy reaches the crate and stops only on the
existing `approx_constant` baseline in `module_eval.rs:1968,2030`. Three
independent latest-diff reviews accept. The archive-status utility retains its
existing missing archive-ref/non-V2-thoughts baseline and confirms only the two
authorized V2 files were dirty before commit.

Run only docs packet
`WP-4-5-6-host-root-apparent-repository-source-input-owner-design` in the four
ledgers under mandatory 40/300/220/220/780 documentation caps. Authorize no Rust
yet. The live five-domain truth is private core
`HostRootApparentRepositoryRouteKey`; its computation-free
`source_capability()` is ephemeral and no owner retains its Need/terminal
predecessor together with the accepted Bzlmod `HostRepositorySourceInput`.
A Bzlmod path input therefore has no truthful five-domain caller yet, while
loading migration would also disturb legacy `RepositorySourceScope` demand
identity. Design this smaller owner first.

Freeze private core `HostRootApparentRepositorySourceInputKey { workspace,
apparent_repo }`, rejecting root apparent requests in its constructor. Its
value is `SourcePreparationOutcome<Arc<Result<certificate, typed error>>>`.
Compute only `HostRootApparentRepositoryRouteKey` in exact order: key shape;
route Need forwarded unchanged; route compute error; completed route terminal;
borrow the route source disposition; Main completes as `Main`; Builtin and the
three spec-backed kinds call `host_repository_source_input` exactly once and
complete as `Input(HostRepositorySourceInput)`; absent/corrupt route view and
projection error are typed complete terminals. No second mapping, definition,
capability, request, policy, or rule/path classification occurs.

Freeze sibling alias `HostRootApparentRepositoryRouteResult =
Result<HostRootApparentRepositoryRoute, HostRootApparentRepositoryRouteError>`.
Freeze exact success fields `workspace: NormalizedAbsolutePath`,
`apparent_repo: ApparentRepoName`, `predecessor:
Arc<HostRootApparentRepositoryRouteResult>`, and `disposition:
HostRootApparentRepositorySourceInputDisposition`, whose variants are exactly
`Main` and `Input(HostRepositorySourceInput)`. Freeze exact error struct fields
`workspace`, `apparent_repo`, and `kind`; its private kind variants are exactly
`Route(Arc<HostRootApparentRepositoryRouteResult>)`,
`InvalidRoute(Arc<HostRootApparentRepositoryRouteResult>)`,
`Projection { predecessor: Arc<HostRootApparentRepositoryRouteResult>, error:
HostRepositorySourceInputError }`, and `Compute(Arc<str>)`. `Route` wraps every
ordinary completed carrier terminal; `InvalidRoute` wraps absent/corrupt
carrier disposition or request/view mismatch; `Projection` retains the exact
accepted certificate error; only `Compute` lacks a completed predecessor.

Success and errors derive structural Debug/Clone/PartialEq/Eq/Allocative.
Freeze a private borrowed success view of apparent repo, canonical repo, and
disposition `Main | Input(&HostRepositorySourceInput)`. The canonical target is
always borrowed from the retained carrier. Need remains invalid and
self-unequal; Complete equality is structural; no event data.

The minimal sibling seam in existing
`root_apparent_repository_route.rs` may be only `pub(super)` key/new/outcome,
opaque completed types, borrowed view/source disposition, and accessors needed
by the new owner. Fields, error variants, definition/mapping predecessors, and
cross-crate/public exports remain private. Future Rust is exactly new
`app/slug_core_v2/src/runtime/root_apparent_repository_source_input.rs`, the
minimal seam in existing `root_apparent_repository_route.rs`, and one private
`runtime/mod.rs` declaration. Mandatory future caps are 320 production/650
tests/970 total; physical ceilings are 1,100 for the existing route file and
800 for the new module. REPLAN if the seam or proof cannot fit.

Proof must combine a production-used pure dispatch/error matrix with reachable
real DICE rows: route Need/terminal/compute ordering; Main; Builtin exact
identity; SelectedNonregistry workspace-relative and command-absolute;
Generated LocalUnsupported projection error or admitted immutable success;
SelectedRegistry inherited at the accepted route ABI; exact predecessor and
request-Arc provenance; workspace/apparent/canonical/spec/policy/mapping/order
A/B/A; warmed route predecessor Reused before owner Evaluated then owner Reused;
no event data. Runtime trackers must prove zero activation of every nameable
public/hidden route, path, source-file, package-load, registry, filesystem,
execution, and I/O owner. Because `RepositoryMaterializationResultKey` is
private inside Bzlmod and must not be published for proof, a production
structural import/call-edge assertion must prove the new module invokes only
`HostRootApparentRepositoryRouteKey` as a DICE predecessor and the pure
`host_repository_source_input` constructor; it must contain no result key,
path/source key, compute helper, debug-string parsing, or downstream import.

Exact compatibility is the accepted root apparent selection and accepted
Builtin/local/http/git source-input association. The private key, retained
certificate/error, lifetime, and DICE scheduling are Slug-native. Repository
path validation, legacy demand scope/module-name, first materialization Need,
result observation, Builtin bytes, source/package/loading migration, commands,
public API/wire, arbitrary generated/custom rules, and I/O remain
unsupported/deferred.

STOP on Bzlmod/loading/server/command/Cargo edits, a fourth Rust file, public or
cross-crate ABI, a second key/store or semantic lookup, copied route/request/
RepoSpec/mapping/catalog, path/ResolvedPath/result input, rule/path inference,
materialization/source/package behavior, reverse dependency, JVM work, or
cap/ceiling excess. Require independent acceptance and explicit activation
before Rust.
...
Independent review accepts implementation `65b1dd89` at 170 production, 292
tests, and 462 total changed Rust lines, within 320/700/1,020. Physical lines
are 2,919/2,426/999/1,069, within 3,000/2,450/1,040/1,100. Focused capability,
root-definition, and route tests pass; full Bzlmod, loading, and server suites
pass. Core remains 188/189 only because the accepted unrelated deferred
external-visibility assertion expects a broad diagnostic while the live graph
returns its more specific wrong-kind deferred diagnostic.

Run only docs packet
`WP-4-5-host-repository-source-capability-materialization-request-projection-design` in
canonical, this manifest, Stage 4, and Stage 5 under mandatory
40/260/220/220/740 documentation caps. Authorize no Rust yet. Live `RepoSpec`
already retains its attribute graph as `Arc<SmallMap<...>>`; cloning it for one
owned request copies only the compact `RepoRuleId` shell and shares attributes.
Treat that one bounded shell clone as Slug-native representation. Do not widen
the exported request layout or schedule an eight-file ownership migration
without measured evidence.

Freeze hidden computation-free ABI
`HostRepositoryMaterializationDisposition::{Builtin(BuiltinBazelToolsRouteIdentity),
Request(Arc<RepositoryMaterializationRequest>)}` and
`host_repository_materialization_request(capability:
&HostRepositorySourceCapability) -> Result<HostRepositoryMaterializationDisposition,
RepositoryMaterializationError>`. Owner is the existing `source_preparation.rs`
beside `RepositoryMaterializationRequest` and `request_kind`; `lib.rs` supplies
only hidden named exports. Builtin returns its exact retained identity and
allocates no request. RepoSpec success uses capability workspace/canonical for
the existing request ID, performs exactly one compact-shell RepoSpec clone,
and returns one Arc request. Apparent name is validated by the capability but
does not enter request identity.

Freeze classification order and policy exactly. Match `local_repository`
first. `WorkspaceRelative` admits only the existing normalized nonempty
workspace-relative path and joins workspace; `CommandAbsolute` admits only the
existing normalized nonempty absolute path; `LocalUnsupported` rejects the
local rule before inspecting its attributes or path with exact Slug-native
`RepositoryMaterializationError::Spec("local_repository is unsupported for this repository source")`.
That terminal wins over missing, malformed, or invalid local attributes/path
only for `LocalUnsupported`; WorkspaceRelative and CommandAbsolute retain the
existing attribute and path diagnostics. Then match `http_archive`,
then `git_repository`, both Immutable under every policy. Every other rule
retains the existing unsupported-rule `Spec` terminal. Do not infer provenance
from path shape, rule, source kind, or apparent spelling. Replace the existing
private route-typed helper name with the hidden capability-typed
`host_repository_materialization_request` above. Add private
`root_repository_materialization_request(route: &RootRepositoryRoute) ->
Result<Arc<RepositoryMaterializationRequest>, RepositoryMaterializationError>`
and update only its existing same-file legacy call sites. That wrapper first
calls `route.source_capability()`, delegates once, returns `Request` unchanged,
and translates `Builtin(_)` to exact existing
`Spec("built-in bazel_tools source requires its immutable source owner")`.
Preserve call-site ordering and every observable request/error.

Future Rust is exactly existing
`app/slug_bzlmod_v2/src/source_preparation.rs` and existing
`app/slug_bzlmod_v2/src/lib.rs` hidden exports, plus four ledgers, under
mandatory 180 production/420 tests/600 total formatted changed Rust lines
against the accepted design commit. Physical ceilings are 11,080 and 380.
No third Rust file, Cargo, core, loading, server, command, fixture, or consumer
edit is allowed.

Required proof: exhaustive Builtin and RepoSpec disposition; local policy by
relative/absolute/empty/dot/dot-dot/root/prefix shape with exact logical roots
and error order; http then git Immutable under all three policies; unsupported
rule and malformed local attributes preserve existing errors; exact request
workspace/canonical/spec/kind and attribute order; identical RepoSpec bytes
under WorkspaceRelative versus CommandAbsolute do not self-classify; apparent
`bazel_tools` with nonbuiltin canonical remains spec-backed; the renamed legacy
route wrapper and all callers are structurally/error identical;
field/policy/path/spec A/B/A; one
compact-shell clone with shared attribute Arc, repeated projections structurally
equal, and clones of one returned Arc request pointer-identical; external-style
hidden ABI; full Bzlmod and direct dependents; and zero DICE activation, Need,
events, registry, filesystem, package/source, transport, materialization,
execution, or I/O.

Compatibility: admitted local/http/git classification, path-policy semantics,
request content/identity, and legacy error order are exact. The hidden
disposition, one compact-shell clone, diagnostics, and Arc wrapper are
Slug-native. Generated-local admission, arbitrary repository-rule execution,
consumer-key migration, first Need, package/source/materialization, commands,
public wire/API, lockfile, and JVM behavior remain unsupported/deferred.

STOP and REPLAN on a new key/store/request layout, deep attribute-map copy,
path/rule/source inference, generated-local admission, changed request ID or
legacy observable behavior, consumer migration, Need/DICE computation,
package/source/materialization/execution/I/O, a third Rust file, cap or ceiling
excess, public stable/wire ABI, reverse dependency, or JVM work.
Require independent acceptance and explicit implementation activation.

## Accepted r2 implementation contract

This section and everything below is historical context only, grants no file,
action, cap, or schedule authority, and is interpreted only through the active docs-only audit contract above.

Independent review accepted proof correction `5e88f2ad` over design
`c5853ecf` and implementation `65b1dd89`. The implementation was limited to
the four named source-capability policy files under 320/700/1,020 and
3,000/2,450/1,040/1,100, with inherited Registry proof and real
Generated/Nonregistry/Main/Builtin consumer proof.
...
Retain the unaccepted four-file Rust diff over design `c5853ecf`, but authorize
no Rust until independent acceptance and explicit r2 activation. Run only
docs packet
`WP-4-5-6-host-repository-source-capability-policy-threading-r2-proof-design`
in the four ledgers under 35/220/180/180/615 documentation caps. Preserve the
same future four paths, 320/700/1,020 Rust caps, 3,000/2,450/1,040/1,100
ceilings, production semantics, ABI, and stops.

Correct the mandatory proof split. Real Bzlmod predecessor proof remains
authoritative for Root, SelectedRegistry `LocalUnsupported`, and root/command
SelectedNonregistry policy. Real core consumer proof covers Generated
`LocalUnsupported`, root-origin and command-origin SelectedNonregistry through
capability publication, plus Main/Builtin absence. The production-used pure
kind/RepoSpec/policy tables must exhaust SelectedRegistry and all corrupt
polarity rows through canonical definition, root-apparent definition, route,
and capability construction. Do not fabricate a real core SelectedRegistry
fixture: successful registry construction requires Bzlmod-private injected
mirror policy that core cannot name without a forbidden ABI/fixture widening.

The root-apparent definition view itself, not only the later route view, must
fail closed on every wrong kind/policy pair. Its production-used discriminator
and pure table prove Registry/Generated require `LocalUnsupported` and
SelectedNonregistry requires WorkspaceRelative or CommandAbsolute. No admitted
real mapping success may silently normalize a mismatch.

Acceptance of this correction does not reactivate implementation. Require a
separate r2 activation anchored to this proof correction and design `c5853ecf`.
REPLAN if pure discrimination needs public/private payload leakage, another
Rust file, a Bzlmod-private input export, or any cap/ceiling/breadth change.
...
Independent review accepts design `c5853ecf`. Implement only
`WP-4-5-6-host-repository-source-capability-policy-threading-implementation`
in existing `app/slug_bzlmod_v2/src/host_module.rs`, existing
`app/slug_core_v2/src/runtime/generated_repository_definition.rs`, existing
`app/slug_core_v2/src/runtime/root_apparent_repository_definition.rs`, existing
`app/slug_core_v2/src/runtime/root_apparent_repository_route.rs`, and four
completion ledgers under mandatory 320 production/700 tests/1,020 total
formatted net Rust lines against `c5853ecf` and physical ceilings
3,000/2,450/1,040/1,100. Preserve the exact constructor/accessors, borrowed
view chain, policy identity/hash/polarity, proof, compatibility split, and all
no-fifth-file/key/store/request/source/materialization/loading/command/public/
I/O/JVM stops below. Keep `lib.rs`, `runtime/mod.rs`, and all other Rust
unchanged.
...
Independent review accepts selected-policy implementation `63de458b`: 43
production and 64 test lines, 107 total, inside 220/450/670. Full Bzlmod,
loading, and server suites pass; core is 188/189 with only the unchanged,
directly reproduced deferred external-visibility assertion. Run only docs
packet `WP-4-5-6-host-repository-source-capability-policy-threading-design` in
canonical, this manifest, Stage 4, and Stage 5 under mandatory
40/280/220/220/760 formatted net documentation lines. Authorize no Rust,
Cargo, fixture, DICE key/store, request/source/package/materialization,
loading/server/command, public API, execution/I/O, lockfile, wire, or JVM work.

Freeze one compile-safe vertical hidden chain. In Bzlmod, change the existing
`HostRepositorySourceCapabilitySource::RepoSpec` identity to retain the sole
shared `Arc<RepoSpec>` plus explicit `HostRepositoryLocalPathPolicy` as named
fields `repo_spec` and `local_path_policy`. Freeze
`HostRepositorySourceCapability::from_repo_spec(workspace,
apparent_repo, canonical_repo, repo_spec: &RepoSpec,
local_path_policy: HostRepositoryLocalPathPolicy) -> Option<Self>`, plus
`repo_spec(&self) -> Option<&RepoSpec>` and `local_path_policy(&self)
-> Option<HostRepositoryLocalPathPolicy>`. Include policy in structural Eq and
manual Hash. Existing
`RootRepositoryRoute::source_capability()` supplies `WorkspaceRelative` for
legacy DirectLocal. Builtin retains no policy.

In core, thread policy only through existing borrowed views, never through a
new retained store. `HostCanonicalRepositoryDefinitionView` delegates selected
policy to the accepted hidden selected-definition ABI, returns `None` for Root,
and returns `Some(LocalUnsupported)` for Generated. The root-apparent
definition and route views pass that value through. Main and Builtin require
no policy; SelectedRegistry and Generated require `LocalUnsupported`;
SelectedNonregistry requires its exact retained `WorkspaceRelative` or
`CommandAbsolute`. Missing or mismatched policy fails closed through the
existing optional view/projection boundary. The computation-free source
capability projection passes the exact policy into the constructor. Do not
infer from path, rule name, source kind, logical root, or materialization.

Freeze the core borrowed ABI exactly:
`HostCanonicalRepositoryDefinitionView::local_path_policy(self)
-> Option<HostRepositoryLocalPathPolicy>`; successful
`HostRootApparentRepositoryDefinitionView::local_path_policy(self)
-> HostRepositoryLocalPathPolicy`; and
`HostRootApparentRepositoryRouteView::local_path_policy(self)
-> Option<HostRepositoryLocalPathPolicy>`. The canonical view's `None` is Root;
the route view's `None` is only Main/Builtin. A successful root-apparent
definition cannot represent missing policy: absence or a wrong kind/policy
pair fails closed before its view publishes. Each Copy view may carry the
borrowed projection field needed by its accessor, but no certificate or DICE
value gains a retained policy field.

Future Rust is exactly existing
`app/slug_bzlmod_v2/src/host_module.rs`, existing
`app/slug_core_v2/src/runtime/generated_repository_definition.rs`, existing
`app/slug_core_v2/src/runtime/root_apparent_repository_definition.rs`, and
existing `app/slug_core_v2/src/runtime/root_apparent_repository_route.rs`, plus
four ledgers, under mandatory 320 production/700 tests/1,020 total formatted
net Rust lines against the accepted design commit. `lib.rs` and `runtime/mod.rs`
remain unchanged. Physical ceilings are 3,000/2,450/1,040/1,100 lines in that
order; REPLAN on a fifth Rust path, new edge/key/store, cap, or ceiling excess.

Required proof: pure kind by RepoSpec by policy polarity table; exact legacy
DirectLocal WorkspaceRelative; real selected root-relative and command-absolute
policy through Bzlmod selected definition, canonical definition, root-apparent
definition, route, and capability; selected Registry and Generated
LocalUnsupported without rule inspection; Main/Builtin no-policy; spec-backed
apparent `bazel_tools`; identical RepoSpec content with policy inequality,
manual-hash participation, and A/B/A restoration; shared RepoSpec Arc behavior;
corrupt missing/wrong policy fails closed; lifecycle/reuse/no events; and zero
request/package/source/materialization activation after accepted predecessor
warming. Full Bzlmod/loading/core/server dependents remain mandatory.

Exact is Bazel 9.2 root-versus-command local-path provenance and its association
with selected repository identity. Hidden enum storage, ABI, hash framing, and
borrowed view chain are Slug-native. Generated local-repository admission,
materialization-request projection, package/source migration, commands/public
API, execution/I/O, lockfile, wire, and JVM are deferred. STOP on policy
inference, generated-local admission, request construction, `request_kind`,
materialization Need/I/O, loading/command consumer edits, copied graph/mapping
state, RepoSpec copies beyond the existing single capability allocation,
module-name reconstruction, reverse edges, or public stable API.

## Accepted selected-policy implementation

This section and everything below is historical context only, grants no file,
action, cap, or schedule authority, and is interpreted only through the active docs-only audit contract above.

Independent review accepts `63de458b`: policy is derived once from the existing
effective Root/Command override, retained structurally in the nonregistry
closure, and published as Root `None`, Registry `LocalUnsupported`, or the
exact Nonregistry value. Proof covers identical RepoSpec content with distinct
provenance, real relative/absolute policy, equality restoration, lifecycle,
and the forbidden downstream boundary. Full Bzlmod/loading/server pass; the
single core failure is the directly reproduced deferred visibility baseline.
...
Independent review accepts `0cba8fb8` at 42 production, 111 tests, and 153
total formatted net Rust lines against `7b6484ee`; the file is 931/960 lines.
The exact private `Option<Disposition>` projection is computation-free, stores
nothing, preserves carrier equality/DICE behavior, fails closed on corruption,
and returns Main or an owned exact Builtin/spec-backed Bzlmod capability. The
five-kind 30-row polarity table, full builtin identity, spec-backed apparent
`bazel_tools`, Arc clone provenance, A/B/A/lifecycle, and forbidden-owner proof
pass. Core is 188/189 with only the unchanged deferred external-visibility
assertion; prior full Bzlmod/loading/server suites are green.
...
Independent review accepts design `7b6484ee` over capability implementation
`3faa90dd`. Implement only
`WP-4-5-6-host-repository-source-capability-publication-implementation` in
existing `app/slug_core_v2/src/runtime/root_apparent_repository_route.rs` with
colocated tests, plus four completion ledgers, under mandatory 180 production/
360 tests/540 total formatted net Rust lines against `7b6484ee` and a 960-line
file ceiling. Preserve the exact private `Option<Disposition>` ABI, one owned
computation-free projection per call, fail-closed corruption behavior,
unchanged carrier state/equality/DICE semantics, allocation/clone proof,
compatibility split, and every no-second-file/key/store/source/materialization/
consumer/command/public/I/O/JVM stop below. Keep `runtime/mod.rs`, Bzlmod,
loading, server, commands, Cargo, and all other Rust unchanged.
...
Independent review accepts source-capability implementation below. Run only
docs packet `WP-4-5-6-host-repository-source-capability-publication-design` in
canonical, this manifest, Stage 4, and Stage 5 under mandatory
40/240/180/180/640 formatted net documentation lines. Authorize no Rust,
Cargo, fixture, DICE key/store, source/package/materialization, command/server,
public stable API, execution/I/O, lockfile, wire, or JVM work.

The dependency-safe owner is existing private core
`runtime/root_apparent_repository_route.rs`: core already depends on Bzlmod and
the accepted five-domain carrier is the first exact apparent-to-canonical
definition certificate. Freeze a computation-free publication inside that
existing carrier, not a new DICE key or parallel lookup. Main has a typed
no-capability disposition. Builtin and SelectedRegistry/SelectedNonregistry/
Generated publish one owned `HostRepositorySourceCapability` retaining exact
workspace/apparent/canonical/source identity. Builtin uses the pinned
`BuiltinBazelToolsSnapshot::CURRENT.route_identity()` after the carrier's exact
Builtin consistency check. The three spec-backed kinds use the accepted
capability constructor over the exact borrowed original RepoSpec; an apparent
spelling of `bazel_tools` remains admitted when canonical/source are
non-builtin. Any missing view, kind/source polarity, canonical, request, or
constructor mismatch fails closed before publication.

Freeze exact private
`HostRootApparentRepositorySourceDisposition::{Main,
Capability(HostRepositorySourceCapability)}` and
`HostRootApparentRepositoryRoute::source_capability(&self)
-> Option<HostRootApparentRepositorySourceDisposition>`. Validated Main and
capability rows return `Some`; every absent view, request/canonical/kind/source
polarity mismatch, or constructor rejection returns `None` without panic,
fallback, or normalization. A future consumer calls the computation-free
projection once and owns or Arc-clones its result across awaits. Do not
store the derived capability in the carrier: preserve its complete definition
predecessor as the sole semantic state and keep route-key compute order, Need,
completed errors, equality, events, and validity unchanged. Add no map,
definition, RepoSpec, target, or source catalog and expose no error payload,
mapping, or constructor. No module name or source/materialization kind is
synthesized.

Each successful spec-backed projection call performs exactly the one authorized
boundary RepoSpec allocation inside the returned capability. Clone of that one
returned value is Arc-only and pointer-sharing; repeated projection calls may
allocate independently but must be structurally equal.

Future Rust is exactly existing
`app/slug_core_v2/src/runtime/root_apparent_repository_route.rs` with colocated
tests under mandatory 180 production/360 tests/540 total formatted net lines
against accepted implementation `3faa90dd`; keep the file at or below 960 physical
lines. `runtime/mod.rs`, Bzlmod, loading, server, commands, Cargo, and every
second Rust path remain unchanged. REPLAN on a second file, cap, or ceiling.

Required proof: pure exhaustive Main/Builtin/three-spec-kind/corrupt polarity
table; real Main no-capability, Builtin exact identity, SelectedNonregistry and
Generated original RepoSpec content, with SelectedRegistry inherited from the
accepted Bzlmod hidden ABI; spec-backed apparent `bazel_tools`; accessor
provenance and `Arc::ptr_eq` across clones of one returned capability, while
repeated projection is structurally equal but need not be pointer-equal;
complete workspace/apparent/canonical/kind/source/RepoSpec and mapping/order/
override/inject A/B/A inherited from the unchanged carrier predecessor;
Evaluated-to-Reused with no event data; and zero added
RootRepositoryRoute/package/source/materialization/registry/filesystem/
execution activation after warming the sole predecessor. Exact is the admitted
Bazel 9.2 root-visible repository identity and original RepoSpec association.
Owned Arc publication, private disposition, diagnostics, and DICE storage are
Slug-native. RepoSpec-to-materialization classification, generic request,
loading consumer migration, package/source preparation, route replacement,
commands/public API, execution/I/O, lockfile, wire, and JVM are deferred. STOP
on guessed rule classification, module-name reconstruction, copied retained
state, a new key/store, reverse edge, source/materializer work, or scope excess.

## Accepted repository source-capability implementation

This section and everything below is historical context only, grants no file,
action, cap, or schedule authority, and is interpreted only through the active docs-only audit contract above.

Independent review accepts implementation `3faa90dd` at 111 production,
148 tests, and 259 total formatted net Rust lines against `538b5231`. The hidden
value owns exact workspace/apparent/canonical plus Builtin identity or one shared
`Arc<RepoSpec>` allocation; Clone is Arc-only, manual Hash covers every field,
and legacy DirectLocal/Builtin projection is exact. Spec-backed apparent
`bazel_tools` remains admitted under non-builtin canonical/source identity.
Focused and full Bzlmod, loading, and server suites pass. Core is 188/189 with
only the unchanged deferred external-visibility assertion reproduced in its
accepted message family.
...
`0c0c2402`; definition/carrier modules are 954/778 physical lines. Focused
carrier proof passes, Bzlmod/loading/server suites are green, and core is
188/189 with only the unchanged deferred external-visibility assertion.

Run only docs packet
`WP-4-5-6-host-repository-route-consumer-boundary-design` in canonical, this
manifest, Stage 4, and Stage 5 under mandatory 40/240/200/200/680 formatted net
documentation lines. Authorize no Rust, Cargo, fixture, public API, source,
package, materialization, execution/I/O, command/server, or JVM work.

Audit exactly core's accepted private `root_apparent_repository_route.rs` and
live external consumer in `runtime/dice.rs`, Bzlmod's `host_module.rs` and
`source_preparation.rs`, and loading's `bzl_module.rs`. Freeze the first
dependency-safe consumer seam for Main, Builtin, SelectedRegistry,
SelectedNonregistry, and Generated without duplicating root mapping or
canonical-definition lookup. Decide whether a neutral lower-crate borrowed
...
Independent review accepts design `0c0c2402`. Run only
`WP-4-5-6-host-root-apparent-repository-route-carrier-owner-implementation` in
new private `app/slug_core_v2/src/runtime/root_apparent_repository_route.rs`,
existing `root_apparent_repository_definition.rs` only for the frozen minimal
`pub(super)` seam, and `runtime/mod.rs` only for its private declaration, plus
four ledgers, under mandatory 320 production/650 tests/970 total formatted net
Rust lines against `0c0c2402`, a 960-line definition-module ceiling, and an
800-line new route-module ceiling. Implement exactly the sole-predecessor
five-domain carrier, opaque deferred projection, fail-closed consistency,
retained full Arc/request identity, no-copy view, proof, compatibility, and
stops below. Keep Bzlmod/loading/server/commands/Cargo/fixtures/public route,
source/package/materialization/I/O/JVM unchanged. REPLAN on a fourth Rust path,
cap, ceiling, or semantic-boundary excess.
...
active docs-only audit contract above is its sole successor authority.
...
validated canonical generated-repository definition inputÃ¢â‚¬â€internal name,
...
schedule authority, and is interpreted through the active docs-only audit contract above.
...
cap, or schedule authority, and is interpreted only through the active docs-only audit contract above.
...
This section is historical context only, grants no independent file, action,
cap, or schedule authority, and is interpreted through the active docs-only audit contract above.
...
active docs-only audit contract above.
...
This section is historical context only, grants no file, action, cap, or
schedule authority, and is interpreted only through the active docs-only audit contract above.
...
interpreted solely through the active docs-only audit contract above.
...
cap, or schedule authority, and is interpreted solely through the active docs-only audit contract above.
...
This section is historical context only, grants no file, action, cap, or
schedule authority, and is interpreted solely through the active docs-only audit contract above.
...
action, cap, or schedule authority, and is interpreted through the active docs-only audit contract above.
...
authority; it is interpreted solely through the active docs-only audit contract above.
...
or schedule authority, and is interpreted only through the active docs-only audit contract above.
