...
Packet: `WP-6-7A-repository-declaration-documentation-category-design-r1`

Milestone: M7A generic Starlark/ruleset closure; loading declaration metadata.

Status: docs-only architecture review active after terminal acceptance of
...
- the complete declaration-builtin audit must cover `attr.*`, `rule`, `aspect`,
  `provider`, symbolic `macro`, `module_extension`, `tag_class`, and
  `repository_rule`. Existing members retain their accepted typed binding and
  ownership. `repository_rule` is the sole current declaration-time rejection
  of a valid non-`None` doc argument; and
...
The API fixes the exact public parameter list and string/None type. The
...
The category audit records, rather than rewrites, existing declaration owners.
If testing discovers another valid declaration doc argument still rejected, the
implementation packet may include only its equivalent binding correction when
the value is already classified nonsemantic for the admitted build surface.
Any new retained public metadata, extractor consumer, repository projection or
cross-crate owner is `REPLAN` and requires an independently reviewed broader
design.
...
- at most one existing focused declaration-doc test file if the audit exposes a
  real unproved sibling; and
...
compiler-required production consumer outside `package.rs`, a valid sibling
rejection needing retained state, or any remote-execution coupling is `REPLAN`.
...
- doc changes do not change final repository-rule projection equality or
  materialized repository semantics, while source changes still invalidate the
  owning loaded module; and
- the declaration-family audit explicitly records every doc-bearing builtin and
  confirms no additional build-loading reject is hidden.
...
terminally accepted. It covers all five label-bearing constructors and default
