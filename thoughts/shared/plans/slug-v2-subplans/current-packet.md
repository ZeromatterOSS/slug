...
Packet: `WP-5-host-canonical-selected-module-definition-publication-implementation`
...
Result: implement the accepted hidden borrowed publication ABI.

## Active implementation contract

Independent review accepts design `1d8758d5`. Run only
`WP-5-host-canonical-selected-module-definition-publication-implementation`
in existing `app/slug_bzlmod_v2/src/selected_repo_spec.rs` and
`app/slug_bzlmod_v2/src/lib.rs` solely for hidden re-exports, plus four
ledgers, under mandatory 180 production/380 tests/560 total formatted net Rust
lines against `1d8758d5`.

Implement exactly the named hidden ABI matrix below over the sole existing key,
predecessor, ordinal, map, order spine, and RepoSpec storage. Preserve opaque
structural errors, Need/nonpublication, no-copy lifetimes, external-style and
A/B/A proof, compatibility classification, and every no-new-key/store,
no-third-file, no-route/source/materializer/I/O, no-loading/core/server-edit,
no-stable-public-API/JVM stop. `REPLAN` on any boundary or cap excess.
