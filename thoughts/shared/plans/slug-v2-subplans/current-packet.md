...
Packet: `WP-4-5-host-repository-source-input-certificate-design`
Milestone: M7 canonical repository routing prerequisites
Owner: `slug-v2-subplans/05-bzlmod-and-repository-graph.md`
Result: freeze the computation-free owned source input before consumer migration.

## Active docs-only source-input certificate design contract

The accepted consumer audit finds that `HostRepositoryPathKey` cannot be the
five-domain first owner. Its value is a materialized `ResolvedPath`, while
Builtin is catalog-backed and has no request/path; it also publishes legacy
module-name `RepositorySourceScope`, which the capability intentionally omits.
Migrating it now would fabricate identity, combine path and byte-source owners,
or require a core demand migration.

Run only four-ledger docs packet
`WP-4-5-host-repository-source-input-certificate-design` under mandatory
40/260/220/220/740 documentation caps. Authorize no Rust. Freeze hidden Bzlmod
computation-free `HostRepositorySourceInput` with exact private fields
`capability: HostRepositorySourceCapability` and
`disposition: HostRepositoryMaterializationDisposition`. Exact constructor:
`host_repository_source_input(capability: HostRepositorySourceCapability) ->
Result<HostRepositorySourceInput, HostRepositorySourceInputError>`. It calls
only the accepted pure projection once and stores that returned disposition
unchanged. Builtin retains the exact capability
identity and builtin route identity without a request. Request retains the
exact capability plus returned `Arc<RepositoryMaterializationRequest>`; no
second projection, classification, path, module name, or route. Its request
shell clone is the single clone created by that sole accepted projection and is
not reconstructed inside the certificate.

Freeze exact `view(&self) -> HostRepositorySourceInputView<'_>` and hidden
borrowed accessors exposing `capability()` and disposition
`Builtin(&BuiltinBazelToolsRouteIdentity) | Request(&Arc<RepositoryMaterializationRequest>)`.
Freeze typed error
`HostRepositorySourceInputError::Projection(RepositoryMaterializationError)`.
The private fields and sole constructor make association correct by
construction: Builtin/Request identity and request kind are certified by the
single accepted projection and are never reconstructed or reclassified here.
Proof asserts them from the stored value; there is no certificate-level corrupt
kind/association state or second classifier. Derive/freeze structural
Debug/Clone/PartialEq/Eq/Allocative; clones share the request Arc and existing
attribute Arc. Future Rust is exactly existing Bzlmod `source_preparation.rs`
and hidden `lib.rs` exports under mandatory 140 production/320 tests/460 total,
with physical ceilings 11,400/390.

Proof: exhaustive Builtin/Request/projection-error disposition; exact
retained capability and request Arc identity; workspace/apparent/canonical/
source/policy/RepoSpec A/B/A; Builtin full identity; repeated construction
equality and Arc clone sharing; external hidden ABI; zero DICE/Need/events,
result/path/builtin-source/package/loading/core activation, filesystem or I/O.
Exact compatibility is the accepted Builtin/local/http/git association;
certificate/error/lifetime representation is Slug-native. Path validation,
legacy demand scope, materialization result/first Need, builtin byte read,
source-file/package/loading/core migration, generated-local/custom rules,
commands/public API and I/O remain deferred.

STOP on any Rust edit before acceptance/activation, new key/store, path or
ResolvedPath input, legacy scope/module-name invention, result/materializer/
builtin-source compute, copied request/RepoSpec/catalog, reclassification,
third Rust file, loading/core/server/command change, public stable/wire ABI,
JVM breadth, cap/ceiling excess, or consumer/source/materialization behavior.
Require independent acceptance and explicit activation.
...
docs-only source-input certificate design contract above.
...
This section and everything below is historical context only, grants no file,
action, cap, or schedule authority, and is interpreted only through the active docs-only source-input certificate design contract above.
...
active docs-only source-input certificate design contract above is its sole successor authority.
...
schedule authority, and is interpreted through the active docs-only source-input certificate design contract above.
...
cap, or schedule authority, and is interpreted only through the active docs-only source-input certificate design contract above.
...
This section is historical context only, grants no independent file, action,
cap, or schedule authority, and is interpreted through the active docs-only source-input certificate design contract above.
...
active docs-only source-input certificate design contract above.
...
This section is historical context only, grants no file, action, cap, or
schedule authority, and is interpreted only through the active docs-only source-input certificate design contract above.
...
interpreted solely through the active docs-only source-input certificate design contract above.
...
cap, or schedule authority, and is interpreted solely through the active docs-only source-input certificate design contract above.
...
This section is historical context only, grants no file, action, cap, or
schedule authority, and is interpreted solely through the active docs-only source-input certificate design contract above.
...
action, cap, or schedule authority, and is interpreted through the active docs-only source-input certificate design contract above.
...
authority; it is interpreted solely through the active docs-only source-input certificate design contract above.
...
or schedule authority, and is interpreted only through the active docs-only source-input certificate design contract above.
