...
Packet: `WP-4-5-7A-innate-repository-rule-owner-certificate-architecture`
...
scheduling commit `61bad75f5`. The stopped canonical Host-capability draft and
the older dirty selected-context R2 candidate remain unaccepted worktree state.

## Observable result

Freeze one generic selected-owner boundary for Bazel 9.2 innate
...
This is a zero-Rust architecture packet. It does not execute
`winsdk_configure`, expose a repository-context capability, change MODULE
evaluation, or resume the stopped Host-capability or selected-context drafts.
...
## Implementation successor allowlist and caps

After independent acceptance of this architecture, activate only
`WP-4-5-7A-innate-repository-rule-owner-certificate-implementation` against
the then-current blobs. Its maximum allowlist is:
