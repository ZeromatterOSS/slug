...
Packet: `WP-6-7A-noncallback-vector-args-paramfiles-implementation-r1`

Milestone: M7A generic Starlark/ruleset closure; Stage 6 action declaration.

Status: Independent pre-review returned `REPLAN` for one bounded identity miss:
Args-backed write must ignore `use_param_file` policy. The corrected contract
separates the common recipe/write format from Spawn-only policy; focused
rereview returned `ACCEPT`, so implementation is authorized within this packet.

Base: `78b94789c`, which terminally accepts typed `DefaultInfo.files`, scalar
Args, generic Spawn/artifact-symlink/absolute-symlink declaration, configured
caller authentication, and action publication cutoff. The unrelated dirty
...
3. regular source/generated Files with generated bytes classified Slug-native,
   ...
   source cases before action publication;
...
Independent pre-review must confirm the evaluator tracing/lowering seam,
manual publication equality, typed ArgsWrite payload, compatibility split,
allowlist, and caps before Rust edits. Independent terminal review is mandatory
for the retained command-line/action representation. One focused correction is
allowed; a second material contract or implementation correction is `REPLAN`.
