...
Commit `97f2bfeeb` accepts loading-owned canonical package-context label
conversion and converts the complete external-query consumer category to final
repository/package identities. Canonical BUILD lowering now consumes the
selected route mapping as evaluator scratch and retains only final
`CanonicalLabel` values; root BUILD apparent mapping remains deferred.

The parked analysis correction already computes both registration expansion
families with the required outer-before-Need-before-semantic polarity. Resume
it from this accepted prerequisite and redesign the selected-nonroot fixture so
platform/toolchain declarations and their internal references are loaded from
their canonical BCR repository packages. Do not add root BUILD mapping or
analysis-side label repair.
