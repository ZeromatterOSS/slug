...
Packet: `WP-4-5-host-repository-source-consumer-boundary-design`
Milestone: M7 canonical repository routing prerequisites
Owner: `slug-v2-subplans/05-bzlmod-and-repository-graph.md`
Result: choose the first capability-owned materialization/source consumer boundary.

## Active docs-only consumer-boundary design contract

Independent review accepts implementation `06a5aa99` at 40 production, 189
tests, and 229 total formatted net Rust lines. Physical lines are 11,192/351,
within 11,240/380. Full Bzlmod and focused proof pass; loading/server remain
green and core retains only its accepted unrelated 188/189 deferred diagnostic.

Run only four-ledger docs packet
`WP-4-5-host-repository-source-consumer-boundary-design` under mandatory
40/280/220/220/760 documentation caps. Authorize no Rust. Read-only audit
anchors are Bzlmod `source_preparation.rs` and `host_package.rs`, loading
`bzl_module.rs`, and core `runtime/dice.rs`. Decide exactly one first future
implementation: atomically migrate the existing `HostRepositoryPathKey` input
to an owned capability/materialization disposition, or REPLAN to a smaller
capability-owned repository-source certificate prerequisite if Builtin and
request-backed repositories cannot share the current path-result algebra.

Freeze ownership across the first DICE await. Builtin must use only the pinned
builtin source owner and never fabricate a RepoSpec/request. Request-backed
Local/Immutable repositories must retain the exact accepted capability and
`Arc<RepositoryMaterializationRequest>` without reconstructing module names,
root overrides, policy, or classification. Main fails before this boundary.
Relative-path validation retains current precedence; the first legitimate Need
is only the existing materialization-result observation for Request input.
Completed values/errors retain exact owned input and path identity; Need stays
invalid/self-unequal. Decide whether the existing path key can change input
atomically without a parallel semantic key/store, then freeze source-file and
package-source ordering with loading explicitly last.

Required successor contract: exact future paths/caps, hidden constructors and
accessors, Builtin/Request output and error algebra, request/capability/path
equality and A/B/A, no-copy identity, Need/order/lifecycle/no-event proof, and
zero loading/core/command behavior change. Classify existing Builtin and
local/http/git semantics as exact, private carrier/key/error shape as
Slug-native, and generated-local/custom rules, package/loading migration,
materialization execution, commands/public API and I/O as deferred.

STOP on any Rust edit, loading/core/server/command mutation, package/source-file
migration, new key/store, duplicate request/RepoSpec/catalog, path/rule/policy
inference, actual materializer/transport/filesystem work, reverse dependency,
public stable/wire API, JVM breadth, or choosing both consumers. Require
independent acceptance before scheduling a successor.
...
docs-only consumer-boundary design contract above.
...
This section and everything below is historical context only, grants no file,
action, cap, or schedule authority, and is interpreted only through the active docs-only consumer-boundary design contract above.
...
active docs-only consumer-boundary design contract above is its sole successor authority.
...
schedule authority, and is interpreted through the active docs-only consumer-boundary design contract above.
...
cap, or schedule authority, and is interpreted only through the active docs-only consumer-boundary design contract above.
...
This section is historical context only, grants no independent file, action,
cap, or schedule authority, and is interpreted through the active docs-only consumer-boundary design contract above.
...
active docs-only consumer-boundary design contract above.
...
This section is historical context only, grants no file, action, cap, or
schedule authority, and is interpreted only through the active docs-only consumer-boundary design contract above.
...
interpreted solely through the active docs-only consumer-boundary design contract above.
...
cap, or schedule authority, and is interpreted solely through the active docs-only consumer-boundary design contract above.
...
This section is historical context only, grants no file, action, cap, or
schedule authority, and is interpreted solely through the active docs-only consumer-boundary design contract above.
...
action, cap, or schedule authority, and is interpreted through the active docs-only consumer-boundary design contract above.
...
authority; it is interpreted solely through the active docs-only consumer-boundary design contract above.
...
or schedule authority, and is interpreted only through the active docs-only consumer-boundary design contract above.
