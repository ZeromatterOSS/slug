...
Packet: WP-4-5-6-7A-root-repository-load-route-publication-replan-r1
...
Status: terminally accepted; integration commit pending. Independent Sol
review accepted the producer-owned namespace correction on 2026-09-01. R1 and its focused
R2 source-certificate correction were accepted on 2026-09-01, but exact
recursive generated-repository mapping proof then found a second material
contract miss. The generated repository mapping correctly resolves a sibling
name to the owner's canonical namespace, while selected demand authenticates
only repositories named by a `use_repo` import. Bazel 9.2 succeeds on the same
root BUILD -> generated `.bzl` -> unimported sibling-generated `.bzl` load and
Slug fails with a typed missing generated definition. This superseding packet
keeps the reviewed route/publication work and adds only producer-owned
extension-namespace demand plus the demonstrated discriminator. The completed
candidate stays within this superseding reviewed boundary and changes no
ruleset, parser, builtin or C++ owner.
...
## Replan discriminator and corrected ownership

The accepted fixture proves an imported generated repository, but did not
exercise the adjacent `generatedReposHaveCorrectMappings` sibling case named by
the packet. A scratch projection of that fixture added an unimported `sibling`
repository, made the imported repository's `defs.bzl` load
`@sibling//:value.bzl`, and loaded `@generated//:defs.bzl` from the root BUILD.
Pinned Bazel 9.2 returns `//:loaded`; the rebuilt Slug route reaches canonical
`@@+ext+sibling` and then reports `HostGeneratedRepositoryDefinitionError::Missing`.

Bazel's `SingleExtensionEvalFunction` retains the complete ordered
`generatedRepoSpecs` map from one extension evaluation before validating root
imports. `SingleExtensionValue` derives the canonical-to-internal-name map for
every generated repository as `extensionUniqueName + "+" + internalName`.
`ModuleExtensionResolutionTest.generatedReposHaveCorrectMappings` proves an
imported generated repository can load an unimported sibling generated by the
same owner.

Slug already has the same natural owner boundary: selected mappings retain the
graph-derived unique name for every extension owner, and the owner certificate
retains every validated repository call from that one evaluation. Correct
`HostSelectedExtensionDemand` so a canonical request is authenticated by the
exact owner namespace `unique_name + "+"`, not only by root/module import rows.
The certificate remains the sole proof that the requested internal name was
actually generated; an unknown name within an authentic namespace evaluates
the owner and returns the existing typed `Missing` result. Do not add a global
canonical-name index, scan source trees, infer an owner from materialized paths,
or copy generated definitions into the route adapter.
...
`SingleExtensionEvalFunction.java` is
`e97d01acd89834147bef7825e25563a1af8787edd59ae4307fbe97d3f23000a2`,
`SingleExtensionValue.java` is
`6e50b0fc9df9def0770296d26584c8d609a02de750702aa6f5bbdbe1cd10c427`,
...
Extend the already accepted `module-extension-use-repo` generated build fixture
with the demonstrated sibling-recursive root load and external query commands;
do not create a second fixture. Regenerate its expected result only with pinned
Bazel 9.2. The pinned Java tests are not copied: their public Starlark behavior
is covered by the compact CLI discriminator plus focused Rust tests, while
Skyframe key-shape assertions would be implementation detail. Reuse the
authentic rules_rust replay unchanged.
...
- root-apparent mapping to a canonical generated definition and that owner's
  complete generated-repository mapping during recursive Bzl loads, including
  an unimported sibling repository;
...
- owner-namespace demand accepts an imported and unimported sibling from the
  same owner, rejects a delimiter-prefix collision and foreign namespace, and
  leaves actual-name existence to the complete owner certificate;
...
- root BUILD direct load of a generated `.bzl`, including an unimported sibling
  repository resolved through the generated repository's own mapping;
...
tests, `app/slug_core_v2/src/runtime/tests/build_command_tests.rs`, and the
existing `tests/v2_oracle/fixtures/module-extension-use-repo/{fixture.toml,workspace/*,expected/oracle.json}`.
No Cargo, repository BUILD metadata, new fixture, ruleset, C++, CLI production
or parked-proof edit is allowed.

R2 permits exactly one additional production change in that source-preparation
file: make the existing `request_error()` accessor doc-hidden public without
changing its result, representation or classification. Core uses it only to
preserve the accepted `WrongKind::Directory` success polarity; all other
canonical source errors remain typed terminals with the same certificate.

The replan additionally permits one bounded semantic change in
`selected_extension_demand.rs`: authenticate a request against the exact
graph-derived owner namespace boundary rather than only the owner's import
values. Keep the key, retained demand value, owner-input key, certificate,
validation, error kinds and observed carrier unchanged. Add no retained index
or collection. The certificate lookup remains the existence check.
...
trigger. Their edits are restricted to existing consumer call sites, frontier
projection matches and the existing selected-demand helper. The new key/driver
lives in its own cohesive loading module; do not add another semantic key to
those large files. The smaller observation-error owners remain cohesive
because their changes are one read-only projection per existing error algebra.
...
- implementation exceeds the allowlist/caps or needs any further material
  correction;
- a new fixture is needed instead of the bounded demonstrated expansion of the
  accepted fixture; or
...
Independent Sol review returns `ACCEPT`: the exact `unique_name + "+"` boundary
matches Bazel 9.2's owner namespace, preserves certificate-owned existence and
DICE invalidation, adds no retained state or semantic owner, and the bounded
fixture expansion is discriminating. Residual implementation risk is delimiter
collision handling and preservation of typed `Missing`; both are mandatory
proof cases.

## Terminal acceptance (2026-09-01)

Implementation and correction rereview return `ACCEPT`. The loading-owned
adapter now serves root BUILD direct `.bzl` loading, exported-source build and
external query through the same retained `HostRepositorySourceRoute`. The
selected-demand producer authenticates the exact nonempty
`unique_name + "+" + internal_name` namespace while the owner certificate
continues to prove repository existence. Core's 997-line private
`generated_package_route.rs` bridge is deleted.

The adapter-specific proof covers admitted and nonactivating root routes,
neutral fallback with the exact original error, typed mapping/load/frontier
failures, contradictory epoch union, complete-only warm equality and `Arc`
reuse, carrierless Need, cancellation/recovery and generated/missing/generated
A/B/A. Delimiter-prefix collision, foreign namespace and typed missing-name
cases pass. The expanded accepted oracle proves root BUILD -> imported
generated `.bzl` -> unimported sibling-generated `.bzl` for both pinned Bazel
9.2 and the rebuilt Slug binary.

Serial terminal validation passes:

- `slug_bzlmod_v2`: 596 passed;
- `slug_loading_v2`: 503 passed, 1 ignored;
- `slug_query_v2`: 55 passed;
- focused root-route adapter proof: 6 passed;
- `cargo build -p slug_cli_v2`, `cargo fmt --check` and `git diff --check`;
- current `slug_core_v2`: 286 passed, 8 failed, 1 ignored, where an exact clean
  `d788af2ce` worktree has 285 passed, 17 failed, 1 ignored and every current
  failure is a member of that baseline failure set.

The rebuilt authentic rules_rust replay clears `@slug_crates` and the recursive
owner-namespace gap. Its next honest terminal is the generic
`repository_rule` attribute schema rejecting `build_file` while evaluating
verbatim `@@bazel_tools//tools/build_defs/repo:git.bzl`. Select a docs-first
complete repository-rule declaration/schema category audit next; do not add a
`build_file`, rules_rust, `cc_common`, `cc_internal`, parser or C++ special
case. The parked registration-expansion proof remains outside this packet at
