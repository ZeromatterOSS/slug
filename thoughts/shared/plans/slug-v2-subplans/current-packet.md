...
Packet: `WP-4-5-6-host-root-apparent-repository-source-path-input-owner-implementation`
...
Result: implement the accepted private path-first core source-input owner.

## Active implementation contract

Independent review accepts design `68349398`. Implement only new
`app/slug_core_v2/src/runtime/root_apparent_repository_source_path_input.rs`,
the minimal `pub(super)` seam in existing
`root_apparent_repository_source_input.rs`, the private `runtime/mod.rs`
declaration, and completion ledgers. Mandatory caps are 340 production/700
tests/1,040 total; physical ceilings are 840 predecessor/850 new. Preserve the
complete private ABI matrix, path-before-await/source ordering, exact path and
predecessor ownership, association gate, proof matrix, compatibility classes,
and every stop in design `68349398`. No Bzlmod/loading/server/command/Cargo,
fourth file, public seam, second validation/projection/lookup/store,
materialization/result/source/package/I/O, copied graph state, module-name,
reverse-edge, or JVM change is authorized. Require independent acceptance.
