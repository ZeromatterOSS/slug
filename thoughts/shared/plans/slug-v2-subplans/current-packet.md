...
Packet: `WP-4-5-6-host-root-repository-source-observation-consumer-owner-audit`
...
Result: identify the first dependency-safe source-observation owner after the
accepted path-first five-domain input.

## Active docs-only audit contract

Independent review accepts implementation `bd337622`: the private core
path-first owner validates once before its sole source-input await, forwards
Need, retains exact path/predecessor/request identity, and adds no result,
source, loading, command, or I/O behavior. Focused proof passes 6/6; the core
suite passes 198 tests with the single accepted unrelated external-visibility
deferred-message baseline. Physical files are 814/840 and 843/850, formatted
growth is within 340 production/700 tests/1,040 total, and three independent
reviews accept.

Run only four-ledger packet
`WP-4-5-6-host-root-repository-source-observation-consumer-owner-audit` under
40/320/240/240/840 documentation caps. Authorize no Rust. Map the exact live
call and dependency chain from `HostRootApparentRepositorySourcePathInputKey`
to the two real observation branches: Builtin through
`BuiltinBazelToolsSourceFileKey`, and Request through the existing private
`RepositoryMaterializationResultKey`, resolved-path observation, and
`HostRepositorySourceFileValue`. Include the existing
`HostRepositoryPathKey`/`HostRepositorySourceFileKey` legacy route inputs and
their `RepositorySourceScope { workspace, module_name }` demand publication.

The audit must decide one smallest dependency-safe successor or REPLAN to one
representation/visibility prerequisite. Compare at least: a private core
source-observation owner dispatching both branches; a hidden Bzlmod-owned
request-backed observation certificate/owner followed by core dispatch; and an
atomic migration or hidden generalization of the existing Bzlmod path/source
keys. Freeze which owner stores one await-safe input, which exact existing key
owns the first new Need, and whether the current path-then-source double
materialization-result dependency can be reused without a second semantic
lookup, request reconstruction, or parallel result store.

For Builtin, establish the exact retained route identity/snapshot/path input,
catalog bytes/kind/integrity output, absence/error ordering, and whether a
common source-value algebra can preserve those fields without fabricating a
materialization request or filesystem path. For Request, establish exact
request Arc/path/predecessor retention, materialization Need/error ordering,
ResolvedPath and file-observation ownership, Local/Immutable behavior, and the
private visibility changes required without reversing Bzlmod -> core
dependencies. Main remains a typed no-source disposition. Registry,
SelectedNonregistry, and Generated use only the accepted request/policy
classification; generated local/custom execution remains fail-closed.

Freeze how legacy `RepositorySourceScope` demand metadata remains exact and
whether generic five-domain activation must stay blocked until its module-name
consumer is replaced. Classify compatibility: accepted branch selection,
path/error order, materialization and Builtin content are exact; private
certificate/key/error/Arc scheduling is Slug-native; loading/package/command
visibility, public API, arbitrary custom/generated rules, transport, and I/O
activation remain unsupported/deferred. Completion evidence is a concrete
owner/dependency/error/lifetime map, one bounded successor with exact future
files/caps/proof/stops, or an explicit smaller REPLAN.

STOP on Rust, a new key/store, activating any consumer, source/materialization
or Builtin-byte computation, loading/package/server/command edits, public ABI,
module-name inference, `RepositorySourceScope` changes, copied request/RepoSpec/
catalog/mapping state, path revalidation, second projection/classification,
reverse dependencies, I/O, wire/lockfile, JVM, or more than one successor.
Require independent audit acceptance before scheduling implementation.
...
docs-only audit contract above. Design `68349398` and implementation
...
This section and everything below is historical context only, grants no file,
action, cap, or schedule authority, and is interpreted only through the active
docs-only audit contract above.
