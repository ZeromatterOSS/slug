...
Packet: `WP-4-5-host-repository-source-capability-materialization-request-projection-r2-proof-design`
Milestone: M7 canonical repository routing prerequisites
Owner: `slug-v2-subplans/05-bzlmod-and-repository-graph.md`
Result: correct the proof boundary and physical ceiling before implementation resumes.

## Active docs-only proof-correction contract

Retain the unaccepted two-file Rust implementation over design `9df81032`, but
authorize no Rust. Run only this manifest, canonical, Stage 4, and Stage 5 under
mandatory 40/260/220/220/740 documentation caps. Independent implementation
review accepts the production ABI, classifier, error order, compact-shell clone,
legacy wrapper, scope, and 180 production/420 tests/600 total caps, but rejects
the incomplete frozen proof. The live source file is 11,078 lines, leaving only
two lines under the activated 11,080 ceiling; the required table cannot fit
truthfully. Freeze a corrected future source ceiling of 11,240, retain the 380
`lib.rs` ceiling, and preserve every production semantic and breadth stop.

The r2 proof must exercise the production projection with normalized absolute
CommandAbsolute success and exact logical root; relative/absolute/empty/dot/
dot-dot/root and platform-prefix shapes where constructible; missing, malformed,
and extra local attributes; exact WorkspaceRelative/CommandAbsolute diagnostics;
and `LocalUnsupported` winning before every attribute/path error. It must retain
http-before-git policy-independent Immutable rows, unsupported-rule failure,
exact workspace/canonical/spec/kind/attribute-order identity, apparent-name
exclusion from request identity, compact-shell attribute-Arc sharing, repeated
structural equality and returned-Arc sharing, field/policy/path/spec A/B/A, and
legacy-wrapper parity. Reuse existing classifier evidence where exact and add a
compact production-used table; do not weaken or fabricate any row.
...
docs-only proof-correction contract above.
...
This section and everything below is historical context only, grants no file,
action, cap, or schedule authority, and is interpreted only through the active docs-only proof-correction contract above.
...
active docs-only proof-correction contract above is its sole successor authority.
...
schedule authority, and is interpreted through the active docs-only proof-correction contract above.
...
cap, or schedule authority, and is interpreted only through the active docs-only proof-correction contract above.
...
This section is historical context only, grants no independent file, action,
cap, or schedule authority, and is interpreted through the active docs-only proof-correction contract above.
...
active docs-only proof-correction contract above.
...
This section is historical context only, grants no file, action, cap, or
schedule authority, and is interpreted only through the active docs-only proof-correction contract above.
...
interpreted solely through the active docs-only proof-correction contract above.
...
cap, or schedule authority, and is interpreted solely through the active docs-only proof-correction contract above.
...
This section is historical context only, grants no file, action, cap, or
schedule authority, and is interpreted solely through the active docs-only proof-correction contract above.
...
action, cap, or schedule authority, and is interpreted through the active docs-only proof-correction contract above.
...
authority; it is interpreted solely through the active docs-only proof-correction contract above.
...
or schedule authority, and is interpreted only through the active docs-only proof-correction contract above.
