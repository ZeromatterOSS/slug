...
Packet: `WP-4-5-6-host-root-apparent-repository-definition-owner-implementation`
Milestone: M7 canonical repository routing prerequisites
Owner: `slug-v2-subplans/04-starlark-loading-and-build-packages.md`
Result: implement the accepted cohesive private mapping-to-definition owner.
...
new private `app/slug_core_v2/src/runtime/root_apparent_repository_definition.rs`,
existing `generated_repository_definition.rs` solely for the frozen minimal
`pub(super)` predecessor seam, and `runtime/mod.rs` solely for its private module
declaration, plus four ledgers, under mandatory 340 production/700 tests/1,040
total formatted net Rust lines against `512e40ed`. Keep the old/new modules at
or below 2,400/900 physical lines. Implement exactly the mapping-first,
Root/builtin-short-circuit, retained no-copy predecessor, typed error/equality,
proof, compatibility, and stop contract below. Keep Bzlmod, loading, server,
commands, Cargo, fixtures, route/source/materializer/I/O/public API, and JVM
unchanged. REPLAN on any fourth Rust path, cap, ceiling, or semantic boundary.
...
predecessors plus request; copy no target, map, RepoSpec, row, or certificate.
