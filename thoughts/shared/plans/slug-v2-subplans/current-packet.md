...
Packet: `WP-4-5-7A-subrule-configured-hidden-dependencies-and-query-r5`

Milestone: M7A bootstrap-critical generic Starlark/ruleset closure.

Base: `ebd19e3b1`, which accepts the generic selected-toolchain context on top
of loading producer `965cfde5e` and command/configuration producer `4425d3bfb`.
The unrelated dirty registration-expansion proof remains parked. Stage and
validate only this packet's exact hunks.
...
configured-child/Need/alias pipeline, preserve one descriptor-ordinal stream
across interleaved Target and Exec rows, validate providers, single-file shape
and executability, and retain ordered phase-scratch dependency facts. One
parent cycle guard covers selected-toolchain and declared-child analysis.
...
R4 terminal review established two implementation facts. First, partitioning
Target rows ahead of Exec rows changes descriptor/error order when the two are
interleaved. Second, `ConfiguredAnalysisCycleGuard::guard_this` is deliberately
one-shot: consuming it inside selected-toolchain preparation leaves ordinary or
hidden configured children unguarded and permits a self/cross dependency to
stall. R5 corrects both without changing the compatibility surface.
...
Resolve ordinary configured attributes and the borrowed hidden/late-bound
descriptors before child computation. Resolve literal labels directly and
typed defaults from the owner's structural `SlugConfiguration`. Preserve one
ordinal stream: ordinary attribute rows first as already frozen, followed by
configured dependency descriptors and each descriptor's label-list elements
in source order. Never partition that stream by configuration kind.
...
independently. A target-only batch retains the established dependency-
preparation-before-toolchain-resolution precedence. If any Exec row is present,
resolve the selected platform first, then construct Target and Exec keys in the
single ordinal pass and prepare/deduplicate the complete declared-child batch
once. Feed both row kinds through the existing configured child key, observed
Need union, requested/actual alias, and publication path.
...
`ConfiguredNodeAnalysisKey` remains the sole retained computation. Its existing
one-shot `ConfiguredAnalysisCycleGuard` surrounds one aggregate future that
first computes selected toolchain implementations when present and then every
prepared declared child. Selected-toolchain preparation must not acquire or
consume a second guard. This preserves a single parent edge set for Legacy and
Observed key families and detects direct or cross hidden-dependency cycles
without a silent wait. The guard is a DICE detector capability, not a shared
lock; no mutex guard is held across a compute.

Loaded descriptors and `SlugConfiguration` remain DICE-retained semantic
inputs; resolved rows, child-key batches, aggregate-future outputs and
validation scratch die with the compute; no new configured result is retained
on the terminal error. No new key, cache, side registry, lock, task, watcher or
request carrier is allowed. Same-DICE source or command A/B/A must invalidate
and restore error precedence without stale children, results or actions.
...
- interleaved Target/Exec descriptors retain source-order failure precedence;
- direct and cross hidden-dependency cycles terminate in both Legacy and
  Observed families, and the same DICE graph succeeds to the pre-call boundary
  after source repair without stale cycle state;
- target-only outer frontier errors still terminate before toolchain-resolution
  child-event publication;
...
- `app/slug_loading_v2/src/subrule.rs`,
  ...
  fail-closed/accessor hunk of `src/package.rs`,
  `bee28ca831f3d71ab3b8f1a29fab1559c4ce299f`;
- new `app/slug_analysis_v2/src/subrule.rs`, only exact orchestration hunks of
  `src/dice.rs`, `2718df1109a4a543f5ac57c99b8c52aed52c8de3`, and the
  module/export hunk of `src/lib.rs`,
...
regression exists. Conservative physical-path accounting classifies the inline
query proof with its production file: the R5 candidate measures 1,218
production additions, 1,062 proof additions and 2,280 aggregate additions.
...
R4 terminal implementation review returned `REPLAN` solely because it
partitioned Target/Exec rows and consumed the one-shot guard before declared
child computation. R5 requires focused independent plan review of the ordinal
stream, aggregate guard ownership, and new proofs before correction. Independent
terminal implementation review remains mandatory. `REPLAN`
...
mechanically exact. R4 made those corrections, but terminal review found its
Target-before-Exec partition and split cycle-guard ownership. R5 changes only
those two orchestration decisions and their proof. After this packet, activate
only the direct-call successor that materializes evaluator values, invocation
and successful configured edges while preserving this packet's dependency
owner and pre-call validation.

Commit `ebd19e3b1` then accepted the generic selected-toolchain context and
structural Target-or-Exec requester ownership required by R5's Exec children.
The rebased candidate removes the 16 additions now owned by that prerequisite and
otherwise preserves the reviewed implementation; no narrower Target-only or
C++-specific slice is substituted.
