...
Packet: `WP-4-5-6-host-root-apparent-repository-source-input-owner-implementation`
...
Result: implement the accepted private Need-aware core source-input owner.

## Active implementation contract

Independent review accepts design `e659b0e3`. Implement only new
`app/slug_core_v2/src/runtime/root_apparent_repository_source_input.rs`, the
minimal `pub(super)` sibling seam in existing
`root_apparent_repository_route.rs`, the private declaration in `runtime/mod.rs`,
and four completion ledgers. Mandatory limits are 320 production/650 tests/970
total formatted Rust lines; physical ceilings are 1,100 for the route carrier
and 800 for the new module. Preserve the exact key/value/success/error/view ABI,
route-only DICE ordering, single accepted Bzlmod certificate projection,
pure/inherited/reachable-real proof split, structural private-result edge proof,
exact/Slug-native/deferred classification, and every stop in design `e659b0e3`.
No Bzlmod, loading, server, command, Cargo, path, materialization-result, source,
package, public API, I/O, reverse-dependency, second-key/store, copied semantic
state, or JVM change is authorized. Require independent acceptance before
completion.
