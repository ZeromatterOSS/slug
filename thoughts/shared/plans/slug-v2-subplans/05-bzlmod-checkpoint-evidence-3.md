...
### WP-5-m1 external Starlark test-base query-closure decision (2026-08-04)

**Status: REPLAN — no test-rule implementation is authorized.** The direct
unconfigured test-base edge set is finite and source-pinned, but its transitive
query closure is not loadable under the existing route and package owners.
Pinned Bazel 9.2 `StarlarkRuleClassFunctions.java` lines 257-376 and
`BaseRuleClasses.java` lines 125-150 and 198-210 supply the test wrapper, XML
writer, runtime, setup/XML, coverage, and null-default run-under attributes;
`RuleClass.java` lines 153-158 and `PlatformConstants.java` lines 26-28 add the
default test toolchain-type edge.
`AggregatingAttributeMapper.java` lines 132-168 supplies the unconfigured
late-bound fallback used by query, so this decision does not claim configured
analysis or toolchain resolution is needed merely to enumerate those direct
edges.

The boundary appears in their transitive targets instead. Pinned installed
`tools/test/BUILD.tools` loads `@rules_shell` for `collect_coverage`, aliases
the report generator to the `remote_coverage_tools` repository generated by
`tools/test/extensions.bzl`, and reaches `@bazel_tools` JDK, launcher, build-
info, and condition packages. The generated coverage package loads
`@rules_java`; its source-defined Java rules add JDK, launcher, platform, and
toolchain-type labels. The accepted Bazel 9.2 probe observed 44 targets across
`@bazel_tools`, platforms, rules_shell, rules_java, and the generated coverage
repository. This is a finite result for that resolved invocation, not a static
graph that may be hard-coded without repository identity and invalidation.

Live Slug cannot own that traversal. `RootRepositoryRouteKey` accepts only a
root dependency with a direct `local_path_override`; it cannot route the
built-in `bazel_tools` repository, a dependency in the tools repository's own
mapping, or an extension-generated canonical repository. The checkout has no
tracked Bazel 9.2 installed `@bazel_tools` tree; the unused
`scripts/sync_bazel_tools.sh` defaults to 9.0 source files and invents root
metadata, so it is not an admissible content owner. `RepositoryPackageLoadKey`
retains one such direct-local route and still rejects repository-qualified and
cross-package loads, test/executable capabilities, dependency labels, and
mixed targets. `ExternalUnconfiguredPackageGraphKey` consequently has no DICE
dependency on the resolved module graph, contextual repository mappings,
lockfile/registry policy, installed tools content/version, or extension-
generated source lifecycle. A copied edge list would make all enabled generic
query consumers observe nodes without their real packages, attributes,
visibility, siblings, and BUILD/Bzl provenance.

Independent route, source, and consumer audits all returned `REPLAN`.
Reserved-boundary review and a correction rereview returned `ACCEPT`; the
correction explicitly keeps the finite direct unconfigured edge result and
removes the broader configuration claim. No Rust, fixture, tool, generated
evidence, Bazel command, Cargo command, public API, DICE key, or production
behavior changed.

The only authorized continuation is read-only design:
`WP-5-m1-bazel-tools-query-repository-closure-owner-design`. Design the
smallest DICE-owned Bazel 9.2 installed `@bazel_tools` source/route plus
contextual resolved and extension-generated repository routes needed to load
the query closure. Reuse the existing Bzlmod discovery, repository mapping,
source preparation, materialization, and lockfile owners; define exact key
identity, equality/invalidation, BUILD.tools installation semantics,
cross-package/repository BUILD/Bzl loading, consumer scope, and lifecycle
gates. Port upstream tools content verbatim only. Do not edit Rust, fixtures,
tools, or generated evidence, and do not activate external tests, suites,
analysis, actions, execution, configured toolchain resolution, JVM, Java
