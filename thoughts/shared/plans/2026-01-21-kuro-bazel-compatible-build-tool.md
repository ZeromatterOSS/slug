...
| ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | --------------- |
| [10-module-extension-execution.md](./kuro-bazel-subplans/10-module-extension-execution.md)           | Let real module extensions execute via DICE instead of synthetic stubs | **Complete** (extensions execute end-to-end; toolchain alias-resolution moved to Plan 11 Phase 8; Phase 6.3 cell-registration cleanup deferred) |
...
| [13-lazy-toolchain-loading.md](./kuro-bazel-subplans/13-lazy-toolchain-loading.md)                   | Filter dev_dependency, make toolchain loading resilient, reduce eager loading | **Phases 1–2 Complete; Phase 3 open** (demand-driven per-target reachability — kills 30-min toolchain-loading wall) |
...
| [36-extension-spoke-lazy-materialization.md](./kuro-bazel-subplans/36-extension-spoke-lazy-materialization.md) | Trigger DICE materialization of an extension spoke repo when a sibling extension dereferences it via `mctx.path(Label)` / `mctx.read(Label)`. Picks up the gap Plan 10 Phase 2 closed at the resolve-string layer but left open at the make-it-exist-on-disk layer. Discovered while verifying Plan 13 Phase 3 against `zeromatter//sdk:sdk_contents` (rules_rs `crate` extension fails on `mctx.path(Label("@cargo_linux_..."))` because the spoke is registered but not on disk). | **Not Started** |
...
| ----- | -------------------------------------- | -------- | --------------- |
| 19    | Module Extension Execution             | [10-module-extension-execution.md](./kuro-bazel-subplans/10-module-extension-execution.md) | [x] Complete (extensions execute end-to-end; alias-resolution gap moved to Plan 11 Phase 8) |
