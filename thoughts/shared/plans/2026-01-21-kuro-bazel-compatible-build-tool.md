...
| 7c    | Bazel Top-Level Modules | [~] Partial (config module done, platform_common done, testing.analysis_test() done, coverage_common done; cc_common compile passes flags, compilation context preserves all include types, linking_context extracts objects; Label.workspace_root returns "external/<repo>" for external repos, Label.repo_name added; cc_common.is_cc_toolchain_resolution_enabled_do_not_use() added; cc_common.create_cc_toolchain_config_info() creates real provider instances; cc_common.create_compile_action() public API with proper params; ctx.configuration has real default_shell_env/short_id/test_env; ctx.var.values()/items() added; ctx.tokenize() shell tokenizer; DefaultInfo.files_to_run attribute; java_common.INCOMPATIBLE_ENABLE_JAVA_TOOLCHAIN_RESOLUTION=true; cc_common.merge_compilation_contexts() merges all 7 fields; merge_cc_infos() now merges all include types; CcToolchainInfo.built_in_include_directories returns real paths; ctx.resolve_command() expands $(location) and collects tool inputs; actions.write() accepts mnemonic/execution_requirements params; 2026-03-12) |
