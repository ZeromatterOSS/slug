...
| [35-buckconfig-removal.md](./kuro-bazel-subplans/35-buckconfig-removal.md)                           | Retire the Buck-shaped bootstrap from kuro's own source tree: rename 277 `BUCK` → `BUILD.bazel`, drain every `.buckconfig` section into `MODULE.bazel` (cells), `.bazelrc` (flags), or `.bazelignore` (paths), and delete the `legacy_configs` parser. No new `.kurorc` — `.bazelrc` already covers every surviving knob. Picks up where Plan 09 §9d/9e left off. | **Not Started** |
