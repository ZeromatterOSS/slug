...
[WP-4-5-7A-repository-qualified-external-bzl-load-route-implementation-r3](./slug-v2-subplans/current-packet.md).

Commit `5fd9b9f0d` freezes the independently reviewed generic Root/Canonical
child-route handoff. R1 implements it within the reviewed five files and proves
the selected parent resolves the extension-generated canonical child. Its
first generated-child source test then stops earlier in the canonical route's
effect: `HostSelectedExtensionOwnerInputs` deliberately rejects a nonroot-only
ordinary extension and always selects Root mappings.

Run only R3. Retain the R1 route implementation, then project the extension
definition host from the canonical `.bzl` repository, use that host's fully
substituted mapping for both namespace and definition loading, and keep each
usage owner's mapping for its invocation module row. Do not change loading,
the legacy root-wide aggregate, parser,
`set`, `cc_common`, `cc_internal`, rules_cc, or C++ rule semantics. Bazel 9.2
remains sole behavior authority; clean Zabel remains concept/optimization
guidance only.

### M7 definition-host R2 review REPLAN; corrected R3 active (2026-08-30)
...
uses the same conceptual grouping but is not an authority. Only the existing
Bzlmod owner-input projection changes; the route implementation, owner
certificate, loading evaluator, legacy aggregate, and selected/built-in direct
Root dependencies remain unchanged.
