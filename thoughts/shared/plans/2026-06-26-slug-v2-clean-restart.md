...
Run only
`WP-5-m1-root-module-include-composition-event-oracle-design`.

Design only a focused Bazel 9.2 retained-daemon oracle strengthening the
existing `module-include-change-invalidation` fixture. Freeze exact rows and
assertions for one root-plus-inline-includes event stream: nested and repeated
include order, unchanged warm nonreplay, print-only include edit whole-closure
reexecution, missing/parse include preflight suppression, runtime-failure
prefix events, and complete recovery. Do not edit fixtures, expected output,
Rust, Cargo, commands, server/runtime paths, loading/analysis evaluators,
nonroot evaluation, or discovery behavior.
