...
| M7: command/ruleset breadth | **partial; repository-source-file observation implementation active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, `cb5073e0`'s immutable action-owner row, `b7390392`'s rules_rust 0.73 evidence, `a3efa1b7`'s root MODULE-files epoch, `3d174006`'s effective-override epoch, and `ae8aa35e`'s request/materialization epoch are accepted | nonregistry source reads still cross carrierless resolution/FileBytes state, while registry preparation remains a separate later frontier | implement the bounded repository-source sibling, then resume the selected-module-graph frontier and preserve M7A -> M8 -> M7B |
...
[WP-6-7A-repository-source-file-observation-implementation](./slug-v2-subplans/current-packet.md).
