...
`WP-5-m1-runtime-path-observation-producer-design-correction`.

Preserve the private synchronous producer, compact sorted materialization-root
snapshot, all-instance preflight before I/O, exact physical demands, raw
OS-native values, operation-specific Missing/WrongKind/I/O results, and
two-file implementation boundary from the replanned design. An instance
authorizes its namespace but does not impose containment; the future caller
must keep the retained materialization owner alive for the whole call.

Correct the exact desktop-Windows `Lstat` contract against Bazel 9.2. Freeze a
private `cfg(windows)` FFI seam that applies Bazel's raw UTF-16
`asLongPath`/separator transformation, rejects interior NUL without lossy
conversion, opens no-follow with `CreateFileW(0, READ|WRITE|DELETE,
OPEN_EXISTING, BACKUP_SEMANTICS|OPEN_REPARSE_POINT)`, reads one
`FILE_BASIC_INFO`, captures failures before RAII close, and converts mtime and
ctime with their distinct signed Bazel/OpenJDK formulas. Specify exact
file-not-found, path-not-found, access-denied, invalid-input, and other raw-code
projection into the existing `Lstat` operation result. Freeze kind, size,
permission, symlink/junction, device, and non-name-surrogate reparse handling
without the unavailable desktop `MetadataExt::change_time()`.

Recheck every operation-specific auxiliary-lstat race, `Interrupted` retry,
whole-directory `EIO` retry/partial-result discard, exact epoch equality, and
the boundary that defers ctime-only external-repository cleanliness to the
later materializer. Return `REPLAN` if exact Windows behavior requires Cargo,
a public/cross-crate API, another source file, or a repository/DICE owner.

Do not edit Rust, Cargo, fixtures, oracle artifacts, plans, or routing during
this correction packet; retain roots, perform repository materialization,
inject epochs, add retries/effects/events, activate source preparation, or
broaden discovery/package/loading/analysis/query.
