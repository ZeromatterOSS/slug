...
[WP-4-5-7A-registration-expansion-prerequisite-owners](./slug-v2-subplans/current-packet.md).

### M7 registration architecture accepted; prerequisite owners active (2026-08-27)

Commit `ae46e1cf5` independently accepts one shared contextual canonical
parser, a general package inventory beneath current consumer-policy adapters,
independent family keys and exact Bazel stable-postorder recursive priority.
Warning facts participate in equality, and the old adapter remains only until
every command/query/configured consumer migrates its policy.

Activate only `WP-4-5-7A-registration-expansion-prerequisite-owners`. Factor
one identity grammar while preserving the command-facing apparent boundary,
add borrowed selected-mapping point lookup, and move general Root/Canonical
BUILD evaluation under a behavior-preserving old policy projection. Do not add
or activate registration keys. Bazel 9 BCR Starlark owns all rules including
`cc_internal`; `cc_common` remains a reusable host-ABI consumer. Zabel is peer
