...
| M1: one semantic spine | partial | retained runtime/loading spine plus the accepted Host root-module and public root-loading-anchor projection, accepted typed no-follow dirent observation, accepted and implemented 13-row focused ASCII Host-dirent glob oracle, and accepted Bazel-internal string oracle/feasibility design, Bazel v28 visible-lockfile, registry-input, pure RegistryFunction, Registry IO bridge with native local paths, focused registry-file vendor oracle, exact ASCII local-directory registry oracle, accepted default-launch collation/charset oracle, and exact host-JVM startup/reuse oracle; pinned source proves BUILD/ordinary `.bzl` Latin-1 byte strings, greedy octal-only escapes, byte-counted locations, and a feasible token-local dormant carrier with a separate opt-in byte-column reporter; the typed observation retains raw names plus direct kind in one shared value, preserves duplicate name order, refines only unknown Unix entries after close, and uses reparse-aware no-follow Windows classification; the required five-packet fixture-growth checkpoint is accepted through `22de3631`, and the accepted dirent oracle is post-checkpoint packet three | production retry/publication activation remains gated on Host migrations, typed production roots, and reviewed byte-aware adapters for every string ingress; the eight-row Bazel-internal string oracle is next, followed by the separately reviewed dormant parser seam, raw-name/pattern-lazy oracle breadth, pure Host glob owner, and transactional package-evaluation retry/suspension boundary; native Windows and lone-surrogate ordering remain activation gates; a reusable regular-or-special Host byte projection remains separate; the full Bazel-valid JVM surface disproves every bounded standalone registry-byte executor found, so the private Host registry-file owner is separately blocked unless a genuinely original-process-equivalent Slug mechanism appears; HTTP and Git PAX/GNU/link/special parity, resolved repository path state, deleted-package request ownership, repository-ignore ownership, package lookup, omitted-`module()` defaults, and discovery composition remain | `WP-5-m1-loading-bazel-internal-string-oracle` |
...
Implement the accepted eight-row, six-file Bazel-only fixture from the Stage 5
owner plan. Pin the static/dynamic ASCII success projections, the five ordered
bad-file escape failures, the byte-counted location row, and exact epoch 1
across generation plus two fresh-root replays. Do not change the harness,
invoke Slug, add raw filesystem names or `glob()`, implement the parser seam,
touch loading/Host/DICE, or broaden beyond the exact caps and stop gates.
