...
Run only `WP-5-m1-runtime-path-observation-producer`.

Add only `app/slug_core_v2/src/runtime/path_observation.rs` and its private
module declaration in `runtime/mod.rs`. Add no Cargo dependency, crate feature,
public reexport, DICE key, runtime/materializer wiring, or workspace change.

Implement a private `MaterializationRootSnapshot` as one sorted compact
`Arc<[(PathObservationInstanceId, NormalizedAbsolutePath)]>` with binary
search, rejecting zero and duplicate IDs. Implement one synchronous
`observe_path_demands` batch that sorts and rejects duplicate demands and
prevalidates every materialization instance before any I/O. An existing
instance authorizes the namespace only: observe the demand's exact normalized
absolute path without joining it to, canonicalizing it under, or checking
containment within the stored root. Return one complete exact
`PathObservationEpoch` or a typed outer runtime error for malformed authority,
unknown instance, invalid/duplicate directory names, unsupported platform, or
epoch construction.

Produce exact operation results for Lstat, raw ReadLink targets, FileBytes, and
sorted raw OS-native DirectoryEntries. Retry every syscall on `Interrupted`.
Lstat maps NotFound/NotADirectory to Missing. ReadLink and DirectoryEntries use
auxiliary no-follow stat only to distinguish a present wrong kind from a
missing target/ancestor, otherwise preserve the original I/O error. FileBytes
maps missing/non-directory ancestors to Missing and may classify a present
directory after `IsADirectory`; special files remain readable. Restart and
discard the whole directory collection without a cap on `Interrupted` or Unix
EIO. Never fabricate InconsistentState. Retain exact ctime and permissions in
the epoch; later repository dirtiness, not this producer, owns ctime-only
filtering.

On Unix, use no-follow metadata and Bazel's signed/wrapping millisecond
arithmetic, signed size/node, and `mode & 0777`. On Windows, validate NUL, use
the first retried `symlink_metadata` for size, mtime, attributes/permissions,
and kind, and map any other initial metadata failure to Missing. Classify
ReparsePoint→Symlink, Device→SpecialFile, Directory→Directory, otherwise
RegularFile. A private safe wrapper over a contained `cfg(windows)` raw ABI
must apply Bazel's raw UTF-16 long-path transform, open no-follow with the exact
sharing/flags, query `FILE_BASIC_INFO` for ctime only, capture errors before an
RAII close, and retain exact missing/access-denied/other operation errors.
Node is `-1`; mtime and ctime use their distinct signed formulas.

Test empty/sorted batches; zero/duplicate roots and duplicate/unknown demands
before I/O; Host versus Materialization identity; inside and escaped
materialization paths; exact file/missing/wrong-kind/raw-error behavior;
nofollow lstat; relative, absolute, dangling, and non-UTF-8 link targets; raw
non-UTF-8 sorted directory entries; partial-directory discard and retry seams;
Unix metadata fields; host-runnable Windows long-path/time/kind helpers
including overlapping reparse+device and device+directory bits; same-demand
create/delete/recreate and symlink-retarget freshness; and epoch operation
validation. Run focused and full `slug_core_v2` tests/doctests, formatting,
diff, exact two-file scope, forbidden ownership/containment/map scans, and
archive status. Stop rather than expand files, APIs, dependencies, owners, or
runtime activation.
