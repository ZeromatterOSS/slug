...
[Rule-initializer declaration-retention implementation
active](./slug-v2-subplans/current-packet.md).

Commit `10aaed332` terminally accepts the fail-closed `apple_common`
declaration-provider packet at 141/180/321 gross production/proof/total Rust
additions. Loading passes 533 tests with one ignored plus all integration
targets; analysis passes 19/19, query 55/55, and CLI/fmt/diff/archive/daemon
gates pass.

The rebuilt bounded-PATH replay clears `apple_common` and stops while loading
selected rules_cc 0.2.4 `cc_shared_library.bzl` because Slug's generic `rule()`
signature lacks the named `initializer` parameter. Exact release scanning finds
three lazy initializer-bearing rules across shared-library, binary and test
sources; none is invoked at the replay boundary.

The audit accepts only the complete declaration-retention category: store one
optional callable in the transient/frozen rule definition and preserve it
through freeze/import lifetime. Every initializer-bearing target call must fail
before package mutation. Bazel's argument lifting, invocation, dictionary merge,
private/allowlist handling, parent chaining and configured semantics remain a
separate unsupported category. Independently review
`WP-4-7A-rule-initializer-declaration-retention-implementation-r1` before Rust.
