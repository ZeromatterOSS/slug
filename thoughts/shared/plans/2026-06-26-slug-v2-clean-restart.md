...
| M8: bootstrap | **active (developer graph)** | exact 33-package CLI boundary plus accepted Gates A-B; Gate C1 has 43 green test targets; one-label BuildBuddy cache is `PROVED_BUILD_CACHE` and managed RBE is `PROVED_BUILD_RBE`, both with clean lifecycle | Reconcile the older full gate with the accepted one-label command/parser/lifecycle semantics | `WP-10-m8-bazel-buildbuddy-full-gate-driver-reconciliation-design` |

### Current packet

`WP-10-m8-bazel-buildbuddy-full-gate-driver-reconciliation-design`.
