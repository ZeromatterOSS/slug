...
| M7: command/ruleset breadth | **active (built-in bazel_tools owner implementation)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, plus the accepted POSIX executable FileWrite `run` vertical with client-only launch | implement the reviewed immutable canonical `@@bazel_tools` routing/source owner before its test package and external closure | `WP-5-builtin-bazel-tools-repository-owner-implementation` |
...
[`WP-5-builtin-bazel-tools-repository-owner-implementation`](./slug-v2-subplans/current-packet.md).
