...
| M4: `cquery` | **active (`DefaultInfo.executable` prerequisite accepted)** | projected label/Starlark-label output, ordered sets, Rust-native `filter`, full `some`, typed evaluation exit 1, retained configured rule capability, and non-test executable-rule analysis | activate non-test configured `executables(expr)` without claiming test-rule/runfiles breadth | `WP-6-m4-cquery-executables-nontest-successor-audit` |
...
[`WP-6-m4-cquery-executables-nontest-successor-audit`](./slug-v2-subplans/current-packet.md).
