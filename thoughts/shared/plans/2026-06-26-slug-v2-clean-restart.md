...
Run only
`WP-5-m1-runtime-materializer-session-archive-implementation`.

Edit exactly:

- `app/slug_core_v2/src/runtime/repository_io.rs`;
- `app/slug_core_v2/src/runtime/path_observation.rs`, for one `pub(super)`
  native-observer/root-authority entrypoint only; and
- `app/slug_core_v2/src/runtime/dice.rs`, for one private
  `Arc<RepositoryMaterializer>` field and initialization only.

Implement the accepted single-lease `RepositoryMaterializer`, exact
full-request accepted cache and validation snapshots, append-only accepted
roots, owner-quarantined provisional roots, monotonic burned instance IDs,
logical Local results, cumulative complete epochs, exact per-repository
dirtiness and offline reuse, and token-checked terminal promotion/discard.
Implicit session-handle drop must not release provisional roots.

Replace mutable caller-path archive reuse with one private capture and the
accepted stage/error precedence. Implement normalize/relativize-before-exact-
prefix selection and containment for the existing regular-file/directory tar
subset; skip outside-prefix entries and reject absent prefixes. Do not claim or
add symlink/hardlink breadth.

Add the focused scripted and real lifecycle evidence frozen in the owner plan,
including lock instrumentation and archive mutation/dual-failure/path cases.
Run focused core tests, full core tests, GNU-Windows no-run compilation,
formatting, diff, archive, and exact-scope/forbidden-reference checks. Return
`REPLAN` on a fourth file, dependency/public/DICE-key/source-preparation
change, lock across I/O/await/DICE, caller-path reopen, reusable exposed ID,
implicit provisional-root release, component-count-only stripping, sidecar,
retry/publication, command activation, fixture, or discovery expansion.
