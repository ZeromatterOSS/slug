...
| M1: one semantic spine | partial | retained runtime/loading spine plus the accepted Host root-module and public root-loading-anchor projection, accepted typed no-follow dirent observation, accepted and implemented 13-row focused ASCII Host-dirent glob oracle, accepted and implemented eight-row Bazel-internal string oracle, accepted Bazel-internal string oracle/feasibility design, accepted dormant Bazel-internal string parser seam in `c8f13ee9`, and accepted Linux raw-name pattern-lazy oracle design, Bazel v28 visible-lockfile, registry-input, pure RegistryFunction, Registry IO bridge with native local paths, focused registry-file vendor oracle, exact ASCII local-directory registry oracle, accepted default-launch collation/charset oracle, and exact host-JVM startup/reuse oracle; pinned source, the exact oracle, and the dormant token-local implementation prove BUILD/ordinary `.bzl` Latin-1 byte carriers, greedy octal-only escapes, byte-counted opt-in locations, unchanged ordinary parsing/rendering, and frozen/unfrozen runtime pass-through; pinned source and native observation also correct positive BUILD `?` matching to exact rejection and isolate Linux raw-name evidence from macOS and Windows; the typed observation retains raw names plus direct kind in one shared value, preserves duplicate name order, refines only unknown Unix entries after close, and uses reparse-aware no-follow Windows classification; the required five-packet fixture-growth checkpoint is accepted through `22de3631`, and the accepted string oracle is post-checkpoint packet four | production retry/publication activation remains gated on Host migrations, typed production roots, and reviewed byte-aware adapters for every string ingress; the dormant parser seam remains unactivated, and the four-row Linux raw-name oracle plus its mandatory packet-five fixture-growth review is next before pure Host glob ownership and the transactional package-evaluation retry/suspension boundary; native Windows and lone-surrogate ordering remain activation gates; a reusable regular-or-special Host byte projection remains separate; the full Bazel-valid JVM surface disproves every bounded standalone registry-byte executor found, so the private Host registry-file owner is separately blocked unless a genuinely original-process-equivalent Slug mechanism appears; HTTP and Git PAX/GNU/link/special parity, resolved repository path state, deleted-package request ownership, repository-ignore ownership, package lookup, omitted-`module()` defaults, and discovery composition remain | `WP-5-m1-loading-raw-name-pattern-lazy-glob-oracle` |
...
Run only implementation packet
`WP-5-m1-loading-raw-name-pattern-lazy-glob-oracle`.

Implement the accepted exact ten-path, four-row Linux-only Bazel oracle from
the Stage 5 owner plan. Keep the accepted 13-row POSIX fixture unchanged; add
only the bounded final-component `raw_create`/`raw_delete` harness operations,
ASCII-only projections, exact BUILD `?` rejection, one-server
create/delete/recreate evidence, and pinned generation plus two fresh-root
replays. Before terminal acceptance, complete and record the mandatory fifth
packet fixture-growth review from baseline `22de3631`. Stop on broader raw
path capability, lossy decoding, platform overclaim, Slug/Rust/DICE/loading
scope, existing-evidence drift, server restart, or cap/allowlist expansion.
