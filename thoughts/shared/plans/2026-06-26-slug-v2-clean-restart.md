...
| M4: `cquery` | **partial (root forward traversal accepted)** | projected label/Starlark-label output, direct retained-state functions, exact toolchain plus delegation graph evidence, one Need-aware analysis key/result, structural/null Target/Exec node identity, classified ordered edges, fixture-proven root topology, semantic error publication, and single-root depth-limited `deps --noimplicit_deps` over the sole retained graph | select the next exact noimplicit forward consumer without synthesizing the stopped external host-platform tail or a second graph | `WP-6-m2j-configured-query-forward-traversal-successor-audit` |
...
[`WP-6-m2p-configured-query-forward-successor-audit`](./slug-v2-subplans/current-packet.md).
