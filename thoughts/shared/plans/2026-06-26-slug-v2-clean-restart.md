...
[Repository template accepted; generic `repository_ctx.which` audit selected](./slug-v2-subplans/current-packet.md).
...
independent `repository_ctx.which("bash")` call in rules_shell. Run only
docs-first `WP-5-7A-repository-context-which-audit`; add no repository API or
ruleset/toolchain special case.
