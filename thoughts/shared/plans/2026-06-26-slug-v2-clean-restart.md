...
| M7: command/ruleset breadth | **partial; selected repository file-effect producer implementation active** | accepted owner-first design `b97d6372`; retained passing selected-owner/source-preflight candidates; independently accepted producer split | Slug has no selected-call `repository_ctx.file` plan producer; route handoff and immutable-root application follow separately | implement only `WP-4-5-selected-repository-file-effect-producer-implementation`; preserve M7A -> M8 -> M7B |
...
[WP-4-5-selected-repository-file-effect-producer-implementation](./slug-v2-subplans/current-packet.md).

Implement only the callerless selected-call producer: one compact Bzlmod file-
effect value plus one loading legacy/observed key pair that reloads and
authenticates the exact rule before bounded execution. Core handoff and root
application remain the next separately reviewed packet.
