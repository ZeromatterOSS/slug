...
| M4: `cquery` | **partial (root topology activation accepted)** | projected label/Starlark-label output, direct retained-state functions, exact toolchain plus delegation graph evidence, one Need-aware analysis key/result, structural/null Target/Exec node identity, classified ordered edges, fixture-proven root delegation/platform/constraint/selected-toolchain activation, and semantic error publication without valid error nodes | audit the smallest root configured `deps` traversal slice without synthesizing the external host-platform tail | `WP-6-m2h-configured-query-root-traversal-activation-audit` |
