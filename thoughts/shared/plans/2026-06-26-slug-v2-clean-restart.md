...
[`WP-6-7A-selected-toolchain-request-implementation-r1`
prerequisite contract](./slug-v2-subplans/current-packet.md).
...
replay reviews accept; full loading, query-library and analysis-library gates,
rebuilt CLI, formatting and hygiene pass. Bare BUILD genrule, complete query
projection and configured genrule/action semantics remain unsupported.

The targeted root-alias replay clears all six authenticated rules_java 9.1.0
genrule calls and finishes in 62.74 seconds, exit 2, at BUILD:138 ->
rules_cc 0.2.17 cc/cc_library.bzl:19:
`target invocation for named execution-group semantics is unsupported`.
No package publishes or Java runtime/genrule analysis starts. Stage 4 records
exact source, command and binary provenance. The earlier BUILD:365 prediction
was not the observed next boundary.

The named-only runtime audit returns `REPLAN`: the authenticated selected
cc_library declares `cpp_link` and `_use_auto_exec_groups=True`. Its composed
...
The shared-runtime design resolves the remaining automatic-policy/action
decisions, then returns REPLAN to a concrete semantic prerequisite: selected
toolchain implementations must retain parent configuration plus a separate
execution-platform preference, not Slug's current Exec substitution. Stage 6
freezes full-key/retained identity, edge-specific propagation, resolver priority,
topology provenance and downstream proofs. The manifest bounds seven production
files and five additional proof files at 900/1,800/2,700 gross Rust additions.
Independent architecture review ACCEPTS this prerequisite contract; no Rust
changed in the docs-only design milestone. Implementation is selected next.
After the correction, resume the complete shared named/automatic group design.
Do not erase group names, substitute the default platform or move the loading
guard. Computed-default and later C++/Java semantics remain unsupported.
...
additions. Full loading/query, CLI, formatting, diff and daemon gates pass.
Authenticated bounded-PATH replay clears the selected computed-default closure
and stops at missing `exec_group` in selected rules_cc 0.2.4 `cc_binary.bzl`.

The complete selected archive closure has four constructors across binary,
test and library, two named `config.exec` references and one configured
`ctx.exec_groups` consumer. Bazel couples these declarations to per-group
toolchain/platform resolution and action routing; Slug currently resolves only
the default group. Audit result is `ACCEPT` only for an accepted compact
declaration carrier plus pre-publication target-invocation guard. It must retain
both groups and named transition references and may not enter final package,
analysis, actions or DICE. Independent architecture and retained-state review
returns `ACCEPT`; implementation is active within the frozen manifest.
...
It reuses routed canonical source reads and their observed epochs, keeps all
template bytes and substitutions in invocation scratch, and retains output only
through the existing generated-file plan. Full loading and query gates pass.
The rebuilt authentic replay clears `repository_ctx.template` and stops at the
independent `repository_ctx.which("bash")` call in rules_shell. That checkpoint
selected docs-first `WP-5-7A-repository-context-which-audit` and authorized no
repository API or ruleset/toolchain special case.

The docs-only `WP-5-7A-repository-context-which-audit` returns `ACCEPT` and
selects `WP-4-5-7A-repository-context-which-implementation-r1`. Pinned Bazel
9.2 and an isolated installed-Bazel 9.2 oracle establish basename-only ordered
Unix `PATH` lookup, ignored relative entries, Java U+0020/control trimming,
symlink-following regular-or-special-file checks, owner-executable `0100`,
candidate-path return identity and `None` behavior. Tracked host invalidation is
Slug-native integrity behavior; Windows extension lookup remains deferred.

Reuse the existing repository environment frontier plus legacy/observed
resolved-path owners. Add only bounded invocation-local candidate demand/result
scratch and provenance that prevents a `which` path from widening the accepted
template source surface. The outer effect driver must verify `PATH`, drop the
synchronous evaluator before every DICE await, and merge all path observations.
Add no DICE key, direct filesystem/process-environment read, retained cache,
rules_shell branch, shell-name branch or toolchain special case. Implement only
the frozen packet.
...
Terminal implementation rereview returns `ACCEPT` at 13/150/163 gross
production/proof/total Rust additions. The runtime-only composition passes all
loading, Bzlmod and query gates, CLI rebuild, hygiene and archive checks. The
authenticated replay clears `Label()` and next stops at the independent
`repository_ctx.path(Label(label))` call in the same rules_cc source
expression. Run docs-only `WP-5-7A-repository-context-path-audit`; add no
rules_cc or toolchain special case.

The path audit returns `REPLAN`. Bazel package-looks-up the Label but does not
inspect its target, resolve symlinks or implicitly watch it under 9.2 defaults;
Slug's existing routed path key does all three and is not reusable. Select
docs-only `WP-2-4-5-7A-repository-label-path-owner-design-r1` to compose the
existing route, package and materialization owners with a lock-safe synchronous
invocation retry. String/generated-root paths, built-in catalog paths, path
filesystem methods and repository effects remain deferred; no Rust is
authorized by the audit.

Independent architecture review accepts
`WP-2-4-5-7A-repository-label-path-owner-design-r1`. Add one lexical
root-or-routed package/materialization key and a 256-address invocation-local
prepared map. A missing path demand aborts only that synchronous attempt; all
evaluator state and speculative events/effects are dropped before the outer
effect key awaits DICE and retries. Path equality uses normalized physical path
bytes while DICE retains observation namespace. Built-in catalog paths,
...
The template audit returns `ACCEPT` and selects
`WP-4-5-7A-repository-context-template-implementation-r1`. Bazel 9.2 reads the
template as ISO-8859-1 bytes, applies literal substitutions sequentially in
dictionary insertion order, writes the replaced bytes with a default executable
mode, and auto-watches an external source. Slug's existing routed source-read
legacy/observed keys already own those bytes and epochs; the invocation-local
retry already drops speculative evaluator state before DICE; and the generated
file-effect plan already owns output path/content/mode/order. Implement only the
bounded normalized-string destination plus canonical external `path(Label)`
source and Latin-1 substitutions. Root/self-generated/string/Label template
sources, explicit watch modes and every other repository effect remain deferred.
...
replay clears verbatim Bazel-tools `build_file` before stopping at the ordinary
descriptor default `attr.label(default = "@rust_host_tools")`.

Select docs-only
`WP-4-5-7A-package-context-label-string-category-design-r2`. Audit and converge
the complete Bazel 9.2 package-context dependency-label string grammar across
`Label()`, all five dependency attribute constructors and their defaults,
ordinary BUILD values, explicit module tags, explicit repository-rule values
and toolchain declarations. Retain one `CanonicalLabel` result and caller-owned
mapping contexts; add no second representation, DICE key, filesystem lookup,
ruleset, C++, `cc_common` or `cc_internal` special case. Output labels,
`load()`, transition-setting labels and command target patterns remain separate
contexts. Independent architecture review must accept the shared syntax owner,
context ownership, proof matrix and bounds before Rust begins. R1 independent
review returned `REPLAN` for four bounded corrections: distinguish ordinary
extension from innate repository-call conversion, admit `@//` separately from
`@@//`, reject canonicalized label-key collisions, and protect the complete
existing `PackageRecorder` consumer family. R2 makes those corrections without
...
R1 implementation ends `REPLAN` on an overbroad package-publication proof:
both `tools/res` catalog globs now evaluate, then the existing independent
`toolchain()` schema rejects its `toolchain_type` `Label` as non-`str`. R2
changes no production architecture or glob compatibility claim. Prove the two
exact raw match slices through the shared request adapter, require package
loading and authentic replay to reach the later typed schema error, and keep
successful package/frozen-target publication deferred. Resume Rust only after
focused R2 correction review.

R2 implements that source-routed traversal without a new DICE owner,
materialization or consumer branch and passes the complete serial loading,
Bzlmod, query, Windows cross-target, CLI-build and authentic rules_rust replay
gates at 228/302/530 gross production/proof/total Rust lines. The replay clears
the old catalog-glob stop and selects the generic builtin declaration-schema
boundary where `toolchain_type` receives a `Label` but Slug expects `str`.
Independent terminal implementation review returns `ACCEPT`. Audit the
complete label-like builtin parameter category next; do not add a `tools/res`,
ruleset, C++, `cc_common` or `cc_internal` special case.

Select docs-first
`WP-4-7A-native-builtin-label-like-parameter-category-design-r1`. Bazel 9.2's
generic label conversion preserves an existing `Label` and resolves only
strings through the package converter. Freeze the complete admitted direct
native parameter inventory across package metadata, package groups, alias,
test suite, config setting, constraint/platform and toolchain declarations;
reuse Slug's existing raw String-or-Label coercer and sole `CanonicalLabel`
result. Add no parser, mapping policy, DICE key, retained representation,
selector/configured behavior, output/path grammar or consumer special case.
Pinned-source regression plus the accepted verbatim catalog replay are the
evidence basis. Independent architecture review must accept the inventory,
owner, proof, bounds and stops before Rust begins.
Initial review corrected three bounded contract details: actual pinned source
owner filenames, canonical duplicate rejection for package metadata, and
ordinary label target names that resemble target patterns. Focused correction
rereview returns `ACCEPT`; implement only the corrected generic conversion
packet.

Terminal implementation and focused correction rereviews return `ACCEPT` at
186/315/501 gross production/proof/total additions. All required loading,
Bzlmod and query gates pass. The rebuilt authentic rules_rust replay clears the
typed direct-parameter boundary and next stops at missing predeclared
`analysis_test_transition` in `@@bazel_skylib+//lib:unittest.bzl`. Audit that
complete Bazel 9.2 category docs-first; do not add a bazel_skylib or rules_rust
consumer special case.

The completed category audit finds that Bazel's value is a fixed literal patch,
not a callable regular transition. Full support couples `rule(analysis_test =
True)`, a distinct configuration marker, action/provider restrictions, nested-
test prevention and a transitive dependency cap. Select the bounded docs-first
`WP-4-7A-analysis-test-transition-loading-declaration-design-r1`: add only the
BZL predeclared constructor, policy-aware validation/repr/freeze and transient
dependency-attribute descriptor retention, then reject ordinary rule
consumption before package publication. Configured analysis-test semantics,
`testing.analysis_test` and `AnalysisTestResultInfo` remain explicitly
deferred. Independent retained-lifetime and boundary review is required before
Rust.

Independent review corrected the dictionary-duplicate and native-option phase
claims. A focused source rereview confirms that first-phase build-setting labels
remain absolute and that only analysis-test native-option policy differs from
regular transitions. It returns `ACCEPT`; implement only the bounded loading
declaration packet.

Terminal implementation rereview returns `ACCEPT` at 152/265/417 gross added
production/proof/total Rust lines. The distinct fixed-patch value retains its
arbitrary dictionary on the frozen module heap and canonical output identity in
an immutable shared slice. All five dependency constructors preserve it across
live, frozen and imported descriptors; every unsupported consumer fails closed
before schema/package publication. Complete serial loading, Bzlmod-lib and
query-lib gates, rebuilt CLI, formatting, hygiene and archive-baseline checks
pass.

The authenticated rules_rust replay clears the missing-global stop and reaches
`@@bazel_skylib+//toolchains/unittest:BUILD`, where package declaration rejects
the generic `default_applicable_licenses` parameter. The complete source audit
accepts `WP-4-7A-applicable-licenses-loading-alias-design-r1`: canonicalize the
two deprecated inputs immediately into the existing package-default and rule-
attribute owners. Keep REPO.bazel package arguments, repeated package calls,
symbolic-macro aliases, rules_license self-edge suppression, materializers and
configured license semantics deferred. Add no Skylib, toolchain or consumer
special case.

Terminal implementation rereview returns `ACCEPT` at 66/214/280 gross
production/proof/total additions. All loading, Bzlmod and query gates, rebuilt
CLI, formatting, hygiene and archive-baseline checks pass. The authenticated
replay clears the Skylib package alias and reaches rules_cc toolchain
registration row 8, where a `.bzl`-defined repository-rule implementation calls
`Label(label)` but Slug's repository evaluator lacks the defining BZL context.
Run docs-only `WP-4-7A-repository-rule-label-constructor-context-audit` next;
add no rules_cc or toolchain special case.
...
label-bearing constructors, selected-platform configuration, typed tool-edge
identity and `ctx.executable`; BCR Starlark owns the rule bodies and rules_rust,
`cc_common` and `cc_internal` remain downstream consumers.

`WP-6-7A-repository-declaration-documentation-category-implementation-r2` is
terminally accepted. The complete exposed declaration family now has exact
typed doc binding for the admitted build/loading surface without adding
metadata identity; Stardoc and materializer/remote-repository categories remain
deferred. The rebuilt authentic replay clears documentation and stops at
`glob(["**"])` in `@platforms//host:BUILD`.
...
R2 then ended `REPLAN` when the mandatory full suite exposed one protected
...
The resulting R1 design covers all fourteen ordinary Bazel 9.2 tag kinds with
one shared attribute-value graph, but independent review returns `REPLAN`
before Rust: its exact non-visible-label ordering claim cannot be represented
...
Commit `493be79b6` terminally accepts complete provider-constrained
dependencies across all five Bazel 9.2 label-bearing constructors. The
canonical provider DNF reaches final package equality and every ordinary
configured edge uses the shared actual-provider validator. Direct
source/generated files remain exempt after file admissibility, rule aliases
validate forwarded providers, and alias-to-file remains explicitly deferred.
Complete loading/analysis/query, permanent Bazel oracle, caps and terminal
rereview pass. The rebuilt authentic rules_rust replay clears `link_deps` and
stops at the independent rule-level transition-execution boundary selected
above.

Commit `096653548` terminally accepts complete generic aspect declarations.
One typed live/frozen/importable owner retains the full default-enabled family;
fixed propagation preserves source order with set-semantic equality. Complete
loading/analysis and replay gates pass. The fresh rules_rust 0.73 replay clears
`rust_unpretty_aspect` and exposes the provider-constrained `link_deps` boundary
selected above. Bazel 9.2 remains sole semantic authority, Buck2-derived
compact utilities are reused, and Zabel remains peer architecture and
...
imported and final rule ownership, participates in structural package equality,
and drives the shared hidden transition allowlist. Complete loading and
analysis suites pass, terminal correction rereview returns `ACCEPT`, and the
rebuilt authentic rules_rust replay clears `rule(cfg=transition)` and stops at
generic `rule(outputs={...})` binding.
...
provider validation, transitions, tools, constraints, licenses and dependency
resolution. Unowned names therefore fail closed and move later with their
complete consumer category; the one-word schema avoids future representation
...
Implementation found one pre-existing generic gap: ordinary `rule()` schemas
did not reject public late-bound attributes before publication. Bazel 9.2's
`AttributeValueSource.LATE_BOUND` requires the `_` prefix. Focused R3 review
returns `ACCEPT` for adding only `package.rs`'s generic attribute-publication
seam and public coverage/C++ controls; no fragment-specific path, resolver,
representation, or DICE scope changes.

The R3 candidate passes all 63 configuration, 28 command, 553 loading (one
ignored), and 119 analysis tests plus formatting, metadata, hygiene, pinned-
source, rebuilt-CLI, clean-Zabel and parked-file gates. The archive checker
retains only its three documented thought-path failures. Fresh rebuilt
...
The corrected R2 candidate passes all 64 configuration, 29 command, 553
loading (one ignored), and 120 analysis tests plus rebuilt-CLI, formatting,
metadata, hygiene, pinned-source, clean-Zabel and parked-file gates. Growth is
45 production and 257 proof net lines, 312 total gross. Fresh rebuilt
rules_rust 0.73.0 cquery clears `cpp.custom_malloc` and advances to
`rust/private/rust.bzl:1001`, where imported
...
The terminally accepted `WP-6-7A-cross-module-attribute-descriptor-identity-r1`
preserves
...
clean Zabel supplies declaration-owner guidance only. Focused field/pointer,
consumer-policy, error, and same-DICE A/B/A proofs pass; complete loading is
556 passed with one ignored and analysis is 120 passed. Rebuilt rules_rust
0.73.0 clears the imported allocator descriptor and advances to its nonempty
transition `inputs` at `rust/private/rust.bzl:1110`, before rule-body or C++
semantics. Independent design and terminal implementation reviews returned
`ACCEPT`; generic transition declaration-input breadth is next.
...
supported-depth equality and both external-owner lifecycle corrections. Commit
`94fd24e9f` freezes the complete non-callback
Args/run/run_shell/artifact-symlink category and its bounded successor order.
Initial review returned `REPLAN` for generated-path
...
architecture schedules a bounded configured-action-environment prerequisite before
...
rereview both returned `ACCEPT`. Terminal implementation review now accepts the
three admitted command options, one process-latched Host fact, the Exec rewrite,
and canonical configured/per-action environment projection; the packet
registers no action. The bounded basic Args/run/symlink FDO successor is now in
implementation after focused correction rereview returned `ACCEPT`. The first
review accepted its dependency
and sink boundaries but returned `REPLAN` for Args lifetime, Windows short-path
observation, depset publication equality, and the missing generic custom-
allowlist/depth check used before absolute symlinks. Its line-level preflight
admits one acyclic internal
`slug_build_api_v2 -> slug_configuration_v2` edge so the action retains the
accepted environment owner directly, and promotes the predecessor's reserved
Host-flavored path normalizer instead of copying it. Zabel remains peer design
guidance, and
`cc_common`/`cc_internal` remain ordinary BCR Starlark consumers rather than
Rust-native or parser special cases.

The R1 implementation candidate then passed the serial configuration,
build-API, loading, analysis and REAPI suites, CLI build, format/cap checks and
authenticated source ledger. A relocated unchanged rules_cc FDO body reached
all basic action paths when a wrapper supplied a typed File, but the direct
source target failed at `DefaultInfo.files` because Slug retained strings.
Terminal review returned `REPLAN`: migrate `DefaultInfo.files` generically to
the accepted typed dense depset, hand the recursive source manifest into all
configured evaluation, prove actual parent-key DICE cutoff rather than value
equality, and correct the omitted proof allowlist and exhausted line cap. R2
freezes those owners; independent architecture review returned `ACCEPT`, so
the corrective implementation is active. It adds no rules_cc/FDO/parser branch;
runfiles, output groups, `DefaultInfo.executable` and FilesToRun typed migration
remain an explicit standard-provider successor. Bazel 9.2 remains authority,
with Zabel used only for peer design and optimization guidance.

The completed R2 implementation now retains source, generated, and declared
Files in one checked dense `DefaultInfo.files` owner, carries the recursive
source manifest into configured evaluation, proves actual parent-key DICE
...
R1 review returned `REPLAN` for an unrepresentable exact non-visible-label
failure-order claim, incorrect tag enforcement of `allow_empty`, and internal
...
membership and root usage kinds; and exposes the general
`root_module_has_non_dev_dependency` input. Facts hydration/persistence,
...
### M7A rule-class restrictions accepted; shared root repository load route selected (2026-09-01)

`WP-6-7D-rule-class-restriction-category-implementation-r2` is terminally
accepted at 955 added Rust lines within its frozen cap. The complete admitted
Bazel 9.2 `allow_rules` predicate is shared across all five dependency
constructors and existing rule, aspect, macro and subrule projections.
Configured validation uses effective configured-target class/provider OR,
keeps silent projections out of analysis views without deleting graph
topology, and preserves the packet's explicit alias-to-file/configured-aspect
and JVM-crash boundaries. No query, ruleset, C++, `cc_common`, `cc_internal` or
parser specialization was added.

The authentic rules_rust replay now reaches root BUILD loading of
`@slug_crates//:defs.bzl` and fails because the selected/direct-local root
route does not publish an extension-generated source. Loading already owns the
canonical apparent mapping, canonical definition/effect load route and shared
Root/Canonical repository-source carrier. Select docs-only
`WP-4-5-6-7A-root-repository-load-route-publication-implementation-r1`: one
admission-aware loading key must compose those owners, cut over root direct
`.bzl`, external query and exported-source build together, and delete core's
temporary generated-package bridge. Bazel 9.2 is compatibility authority;
Zabel `0795445f...` informs only the composed-source ownership shape.

Independent Sol architecture review returns `ACCEPT` with no correction.
Implementation is active only within the frozen packet boundary.

R1 implementation exposes one allowlist correction before terminal review:
core's accepted exported-source behavior treats a repository source directory
as present, but the shared canonical observer wraps the same typed
`WrongKind::Directory` error. Select focused R2 review for making the existing
read-only `request_error()` projection doc-hidden public; this moves no owner
and changes no classification, carrier or identity.

Focused R2 correction review returns `ACCEPT`; implementation may expose only
that existing read-only projection and preserve the typed directory polarity.

Recursive generated-repository mapping proof then returns `REPLAN`. Bazel 9.2
successfully loads root BUILD -> imported generated `defs.bzl` -> unimported
sibling-generated `value.bzl`; Slug resolves the sibling canonical name but its
selected-demand owner lookup considers only `use_repo` imports and reports the
definition missing. Select
`WP-4-5-6-7A-root-repository-load-route-publication-replan-r1`: preserve the
reviewed route/cutover and make the existing selected-demand producer
authenticate the exact graph-derived owner namespace, while the existing owner
certificate remains the sole generated-name existence proof. Extend only the
accepted generated-repository fixture. Independent architecture review is
required before further Rust.

Independent Sol replan review returns `ACCEPT`. Implementation may resume only
inside the superseding namespace-demand, shared-route and bounded fixture
contract; delimiter-prefix collision and typed missing behavior are required
terminal proof.

### Proto facade accepted; selected-BCR read-only mode audited (2026-09-02)

Commit `06ddf8252` accepts the exact default one-member proto-common facade at
5/31/36 gross production/proof/total Rust additions. The focused proof passes
1/1; loading passes 530 unit tests with one ignored and all integration
targets, query passes 55/55, and the CLI build, formatting, diff and daemon
hygiene gates pass. Only three longstanding thought-path failures remain.

Authenticated replay now reproducibly clears rules_shell with both explicit
bounded PATH variants. The earlier `which` limit result is transient evidence,
not a compatibility gap; no resolver or cap change is selected. Both replays
reach rules_java 9.1.0 archive materialization and stop on its first regular
`0444` entry. The exact archive has 94 regular `0444` entries and 20 `0755`
directories only. Bazel 9.2 accepts and retains `0444` by applying
`entry.getMode() | 0400`.

Audit result: `ACCEPT`. Select
`WP-5-7A-selected-bcr-regular-0444-mode-implementation-r1`: add only generic
regular `0444` to the existing selected-BCR materializer's fail-closed mode
...
### Apple-common fail-closed provider implementation selected (2026-09-02)
...
Commit `94738a9a2` accepts the corrected design. Select
`WP-4-6-7A-apple-common-declaration-provider-fail-closed-implementation-r1`:
add a closed two-variant sibling token that lowers normally for loading-time
provider schemas but is rejected explicitly before configured target
membership/indexing. Existing retained provider identities remain unchanged;
configured Apple providers/actions remain deferred.

### Apple facade accepted; generic rule-initializer declaration audited (2026-09-03)

Commit `10aaed332` accepts the Apple packet at 141 production and 180 proof
gross Rust additions, 321 total. Full loading, analysis and query validation,
the CLI build and hygiene gates pass. Authenticated bounded-PATH replay clears
all three selected Apple declarations and reaches rules_cc 0.2.4
`cc_shared_library.bzl` at the generic missing `rule(initializer = ...)`
signature before any configured Apple operation.

Pinned Bazel 9.2 separates initializer declaration from a materially larger
invocation protocol: explicit public-attribute lifting, isolated evaluation,
`None`/dictionary merge, name and private/base-attribute checks, configured
allowlisting and parent chaining. The selected rules_cc archive has exactly
three lazy initializer-bearing rules; the replay executes none of their bodies.

Audit result: `ACCEPT` only the generic declaration-retention category. Add one
optional callable to the existing transient/frozen rule definition, freeze it
in the same module heap and preserve it through existing import lifetime
ownership. Any initializer-bearing target invocation must return the fixed
Slug-native diagnostic before package mutation. Runtime initializer semantics,
`native.package_relative_label`, allowlisting and all configured C++ behavior
remain deferred. No callable enters final package/configured identity, no DICE
owner changes and no V1/Buck2 extraction is selected. Independent packet review
returns `ACCEPT`; implement only the two-file production-plus-proof packet
frozen in `current-packet.md`.

### Rule initializer accepted; label computed-default retention audited (2026-09-03)

Commit `db9e693e7` terminally accepts generic initializer declaration retention
at 21/131/152 gross production/proof/total Rust additions. Loading passes 535
unit tests with one ignored and all integration targets, query passes 55/55,
and the direct pinned-nightly CLI, formatting, diff, archive and daemon-hygiene
gates pass. Authenticated bounded-PATH replay clears the three selected
initializer declarations and stops at rules_cc 0.2.4 `_def_parser` because its
`attr.label` default is a Starlark function.

Pinned Bazel 9.2 separates descriptor retention from package-time computed-
default invocation, parameter dependency ordering, configurable precomputation,
`None` fallback and return coercion. Exact scanning of all 400 selected archive
entries finds one callback, three construction sites and four rule consumers.

Audit result: `ACCEPT` for generic `attr.label` function-default declaration
retention only. Replace the descriptor's boolean with its existing-heap
callable pointer and retain actual rule uses in a compact schema-indexed sparse
slice. Every consuming target call fails with a fixed Slug-native diagnostic
before recorder/coercion/publication. Other constructors and all callback,
package/configured/C++ behavior remain deferred. Independent packet and
retained-representation review returns `ACCEPT`; the two-file 80/180/260 packet
in `current-packet.md` is active without scope change.

### Computed default accepted; execution-group declaration closure audited (2026-09-03)

Commit `64a0f29f6` terminally accepts the computed-default packet at
59/178/237 gross production/proof/total Rust additions. Loading passes 537
unit tests with one ignored and every integration target; query passes 55/55,
and the CLI, formatting, diff, daemon and archive-baseline gates pass.
Authenticated bounded-PATH replay clears that closure and stops at missing
`exec_group` in selected rules_cc 0.2.4 `cc_binary.bzl` before target
invocation.

The complete 400-entry scan finds four `exec_group` calls in three rule
declarations, two `config.exec(exec_group = "test")` attribute descriptors and
one configured `ctx.exec_groups` consumer. Bazel 9.2 couples these to named
transitions, per-group toolchain/platform resolution, context projection and
action routing. Slug has named action-owner identity but resolves only a
default context; a constructor-only shim would silently lose semantics.

Audit result: `ACCEPT` for
`WP-4-6-7A-exec-group-declaration-closure-fail-closed-implementation-r1`.
Retain the
selected constructor values and named transition references in compact
source/schema-ordered slices, then reject every rule that could consume either
before recorder access or package publication. Exactness covers only selected
toolchain-list/empty-constraint declaration shapes, lazy freeze/import and
unaffected no-group rules. Compact representation and the fixed invocation
error are Slug-native. Configured named transitions, platform/toolchain
resolution, `ctx.exec_groups`, actions, automatic groups and C++ behavior stay
unsupported/deferred. Independent architecture and retained-state review
returns `ACCEPT`; implementation is active without scope change.

### Execution groups accepted; provider declaration identity audited (2026-09-03)

Commit `2945accbe` terminally accepts the fail-closed execution-group
declaration closure. Its validation gates pass and authenticated bounded-PATH
replay next stops in rules_java 9.1.0 `java_runtime.bzl` at
`provides = [JavaRuntimeInfo, platform_common.TemplateVariableInfo]` with
`rule provides must contain exported provider constructors`.

Audit result: `ACCEPT`, pending independent review, for one generic declaration
identity closure. The immediate first failure is the exported live initialized
JavaRuntimeInfo callable: Slug projects only its frozen sibling today. The
existing TemplateVariableInfo callable is a second deterministic miss because
its builtin provider identity is not admitted. Add both to the shared provider
classifier, but reject TemplateVariableInfo explicitly before configured Target
membership/indexing. This also closes the selected alias's required-provider
list without a Java or rules_java special case. Selected JavaInfo and
JavaPluginInfo advertisements already import frozen initialized callables and
remain regression-preserved rather than newly closed.

The existing compact ProviderIdentity and Arc advertised-provider slice remain
the sole retained representation; no owner, DICE input, lock, cache, interner
or Stage 9 row changes. Exactness covers exported initialized and builtin
declaration keys. The compact representation and configured diagnostic are
Slug-native. Template instances/configured semantics and all Java execution
remain deferred. Freeze only the two-file 16/140/156 successor after review.

### Provider identities accepted; java_common private loading facade audited (2026-09-03)

Commit `60528af77` accepts provider declaration identity at 16/139/155 gross
production/proof/total Rust additions. Authenticated replay clears those
declarations and next stops at rules_java 9.1.0 `java/private/native.bzl:19`
because the Bzl-only `java_common` global is missing.

The complete six-file selected closure has 26 literal
`get_internal_java_common` occurrences: five load imports plus 21 definition/
call-form occurrences comprising the bridge definition and 20 returned-member
call sites using ten names. Under Bazel's default-false flag, one legacy-only
call is unreachable, one `google_legacy_api_enabled` call eagerly constructs
the public rules_java struct, and 18 calls are lazy. Two are exact false queries
inside JavaInfo helpers; only the other 16 invoke deferred members, and the
enclosing provider behavior remains deferred. Bazel exposes eleven members;
`incompatible_disable_non_executable_java_binary` is the sole unreferenced
member.

Audit result: `ACCEPT`, pending independent review, only for the default-false
loading facade. Two zero-sized values expose `internal_DO_NOT_USE()` and
`google_legacy_api_enabled()` in `.bzl`, enforce at both calls one allocation-
free package-local predicate admitting only a real rules_java `java`-package
caller from the recursive manifest, and survive freeze/re-export. Selected
positive callers are exact. Rejecting rules_java packages outside `java/**`,
the main-repository `javatests/com/google/devtools/grok/kythe/analyzers/build/testdata/pkg`
and `third_party/bazel_rules/rules_java` prefixes, or stricter callerless forms
is Slug-native fail-closed, as is the sparse Rust representation. Every other
member,
legacy-true configuration and all Java provider/configured/toolchain/action
semantics remain deferred and fail closed. No retained field, DICE owner,
cache, fixture or Stage 6 value changes. Freeze only the two-file 72/160/232
successor in `current-packet.md` after review.

### Java common accepted; Java configuration fields audited (2026-09-03)

Commit `c8b4c9e86` terminally accepts the sparse default-false rules_java
`java_common` facade at 71/154/225 gross production/proof/total Rust additions.
Authenticated bounded-PATH replay clears it and stops during loading at
rules_java 9.1.0 `java/common/rules/java_toolchain.bzl:602`, where the missing
Java fragment rejects `java_toolchain_bytecode_optimizer`.
...
anchor 262-282); its later BUILD invocation remains deferred. Bazel 9.2
`BazelBuildApiGlobals`, `StarlarkLateBoundDefault`, `JavaConfiguration` and its
tests establish typed fragment/field/tools-repository identity and configured
reflection.

R1 implementation review returned `REPLAN` for crossing the authentic runtime
boundary; R2 was accepted at `6542169e8`. Its parked candidate clears both
selected declarations. Replay then corrects only the next-source ordering:
`toolchains/BUILD` loads `java_runtime_files` at 9-15 and first calls it at 102.
The imported helper in `default_java_toolchain.bzl:201-217` records a transient
`native.filegroup` at 204-208, then fails on its first Label string at 212 with
`Label() may only be called in a .bzl module`; line 213 is unreached. Package
failure is atomic, so no target publishes. The later initializer-bearing
`default_java_toolchain` call at BUILD line 365 is deferred and unreached; do
not claim pre-recorder rejection or absence of transient authentic mutation.
Corrected R3 remains exact rule-declaration/freeze/import/re-export retention
...
Commit `4d59d7451` terminally accepts that five-file implementation at
75-production/205-proof/280-total gross Rust additions. It preserves the
one-byte closed field identity, exact declaration/freeze/import/re-export
surface and Slug-native configured error. Focused and terminal validation pass.

### Java fields accepted; imported-Bzl Label caller provenance audited (2026-09-03)

Authenticated replay of `4d59d7451` clears both selected Java declarations,
then `toolchains/BUILD:102` calls imported `java_runtime_files`. Its lexical
body in `default_java_toolchain.bzl:201-217` records a transient filegroup and
fails at the first `Label()` on line 212; atomic package failure publishes
nothing. The second Label and later initializer-bearing BUILD line 365 are
unreached.

Pinned Bazel 9.2 `StarlarkRuleFunctionsApi:1031-1057`,
`StarlarkRuleClassFunctions:2122-2143` and
`BazelModuleContext:140-190` resolve Label against the innermost executing
`.bzl` source. `StarlarkIntegrationTest.java:4338-4356` directly freezes
re-exported Label-alias rejection and its
`Label() can only be used during .bzl initialization (top-level evaluation)`
diagnostic. Audit result is `ACCEPT`, pending independent review, for that
generic lexical category. Reuse the already loaded recursive manifest in one
attempt-only sparse `Option<Arc<[(filename, BzlModuleIdentity)]>>` on
`PackageRecorder`; select the call source with
`native_call_source_filename().or_else(native_caller_function_filename)` so
compiler inlining cannot change identity. Direct BUILD and a merely aliased
builtin remain rejected. Add no provider/global Bzl context, DICE key, map,
cache, interner or fixture.

Exactness covers ordinary/Bzlmod BUILD calls into imported/re-exported defs,
including relative/apparent/canonical resolution against the lexical source.
The compact attempt carrier and missing/ambiguous-source diagnostics are
Slug-native. Other callbacks and configured Java semantics remain deferred.
The five-file 120-production/260-proof/380-total successor, ownership,
complexity waivers, proof matrix and stops are frozen in `current-packet.md`.
