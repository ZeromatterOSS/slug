...
| M8: bootstrap | **active (developer graph)** | exact 33-package CLI boundary plus accepted Gates A-B; Gate C0 freezes the CLI runfiles boundary and passes the CLI library unit test; Gate C1 inventories all 1,005 transitive V2 cases, with 36 accepted Bazel targets covering 458 source cases and passing all 454 default-active Linux Cargo cases with stable locks; the Bzlmod lockfile scratch adapter is accepted; the core host-tool audit isolates its runtime integration | query unit and Bzlmod unit are whole-target **REPLAN** at clean-baseline failures; the CLI integrations, query loading integration, and server unit target share the nested-fixture **REPLAN**; the core unit target lacks pinned cross-platform `git`/`tar` owners; cache/RBE and self-hosting remain | `WP-10-m8-bazel-core-runtime-test-implementation` |

### Current packet

`WP-10-m8-bazel-core-runtime-test-implementation`.
