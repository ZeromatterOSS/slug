...
| M7: command/ruleset breadth | **partial; selected-module-graph proof-cap correction active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, `cb5073e0`'s immutable action-owner row, `b7390392`'s rules_rust 0.73 evidence, `a3efa1b7`'s root MODULE-files epoch, `3d174006`'s effective-override epoch, `ae8aa35e`'s request/materialization epoch, `12f68983`'s repository-source epoch, `b08b7f2e`'s REPO-file epoch, `754e7619`'s repository-ignore epoch, `18166691`'s package-preflight epoch, `28fab9b0`'s nonregistry closure epoch, `d0ebd79d`'s registry-policy epoch, `0f9a0559`'s registry-file epoch; `223c8112`'s complete module-source-preparation epoch and `c6b1e108`'s discovered-module epoch/events are accepted | design `09177514` selects the selected graph; the production-sound retained candidate needs bounded live-stage/topology/event/lifecycle proof headroom | correct only `WP-6-7A-host-selected-module-graph-observation-proof-cap-correction-design`, then retry the same owner and preserve M7A -> M8 -> M7B |
...
[WP-6-7A-host-selected-module-graph-observation-proof-cap-correction-design](./slug-v2-subplans/current-packet.md).
