...
Latest M1 accepted evidence: the private Bazel 9.2 lockfile-v28 pure owner is
retained. It implements the complete immutable six-field value, streaming
Gson-compatible typed reader, direct canonical renderer, exact adapter
identity/order, semantic equality, typed caught/uncaught failures,
domain-sensitive surrogate normalization, and Java decimal/suffix/hex numeric
fallback. The exact 15-row oracle remains the source of truth. Focused 98,
complete crate 177 plus all integration suites, doctest, GNU-Windows linking,
format/archive/diff/scope gates, and all three terminal reviews passed. Only
the existing workspace `base64` dependency changed; the owner is private and
dormant with no live caller, Host/DICE change, or `Cargo.lock` drift.

### Current packet

Implement only
`WP-5-m1-bazel-lockfile-v28-raw-read-live-cutover-design`.

Design only the smallest atomic cutover that feeds raw observed lockfile bytes
through the accepted Java-compatible v28 reader before version scanning. Audit
the existing `WorkspaceRawFileKey` boundary and provisionally add only
`module_eval.rs` plus `tests/root_module_dice.rs` to the prior four-file
cutover. Freeze exact caught/direct-adapter/delimiter errors, all-mode
semantics, semantic equality pruning, retained A-to-B-to-A behavior, and the
complete validation/forbidden gates. Do not edit Rust or fixtures and do not
run Cargo.
