...
| M3: `query` | **active (`attr` oracle design)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; leaf attr formatting is source-closed | close selector/default order, typed combination, canonical-label, and total native-attribute evidence before representation design; rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-candidate-order-oracle-design` |
...
[`WP-4-8-m3-attr-candidate-order-oracle-design`](./slug-v2-subplans/current-packet.md).
