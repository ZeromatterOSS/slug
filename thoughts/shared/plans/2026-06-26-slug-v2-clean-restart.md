...
Run only
`WP-5-m1-local-repository-lifecycle-oracle`.

Extend the existing retained-daemon `module-source-preparation` fixture with
the smallest exact Local lifecycle suffix. Move the existing three
`local-route` payload files to `local-route-a`, make the unchanged logical
`local-route` path a relative symlink to A, add one parked B symlink and a
minimal B `MODULE.bazel` with a B-only graph marker, and add one parked symlink
to a checked existing directory whose only file is `BUILD.bazel`. Reuse the
existing safe rename/delete/create mutation operations; do not change the
runner or fixture schema.

After the existing Local main/include mutations, keep the exact
`local_path_override(module_name = "route", path = "local-route")` request
while proving A-to-B symlink retargeting, missing logical root, regular-file
wrong kind, existing-directory-without-repository-boundary, and exact A
recovery. Missing and regular-file rows must pin Bazel's path
missing/not-directory diagnostic and exclude the boundary diagnostic plus
downstream graph/evaluation markers. The boundaryless row must pin the distinct
`No MODULE.bazel, REPO.bazel, or WORKSPACE` diagnostic and exclude the path
diagnostic and both A/B markers. Recovery must restore the already-mutated A
graph and exclude every prior failure/B marker. Preserve every existing row
exactly and keep the later archive/Git transitions valid.

Edit only
`tests/v2_oracle/fixtures/module-source-preparation/fixture.toml`,
its `expected/oracle.json`, the three moved A payload paths, one B
`MODULE.bazel`, one boundaryless `BUILD.bazel`, and the three relative symlink
entries. Add exact provenance anchors for `LocalPathRepoSpecs`,
`local_repository`, fetched-root boundary validation, logical symlink
freshness, and MODULE-read ordering. Generate with pinned Bazel 9.2, replay
twice, run the focused oracle harness tests, record net file/line growth and
reused scaffolding, and obtain an independent terminal fixture review.

Do not edit Rust, Cargo, oracle runner/schema code, registry scaffolding, or
unrelated fixtures; add an unused-invalid override; mutate the output base; or
activate source preparation. Stop on existing-row drift, inability to
distinguish preflight from MODULE evaluation, platform-dependent symlink
behavior, or any need for new registry topology. Fixture-growth review is not
yet due: only `9fa4fbde` has been accepted after checkpoint `42e38bc3`.
