...
Run only
`WP-5-m1-runtime-http-archive-captured-ustar-design-correction`.

Perform a read-only correction of the one-file HTTP archive contract before
retrying Rust. Preserve private capture, one caller-source read, raw USTAR
regular/directory extraction, exact prefix selection, Git byte identity, and
all existing scope exclusions.

Freeze a two-phase inspect/select/plan-then-extract owner so any later malformed
or unsupported selected entry wins before filesystem writes. Pin physical EOF
to Commons Compress 1.26.1: any initial nonempty short record is EOF, while a
partial nonzero record after a complete padded entry is an error. Pin octal
leading-NUL behavior and make base-256 rejection an explicit subset boundary.

Freeze Latin-1/raw-like `strip_prefix` encoding, platform-neutral Windows
drive-absolute relativization, per-component Windows drive/UNC/backslash
rejection, and final root containment. Move private capture persistence
write/flush failures to `Transport` and specify an injectable capture writer.
Require discriminating tests for every correction and preserve the accepted
checksum/capture precedence.

Return `REPLAN` if the correction needs another file, dependency, public API,
Git change, retained-session/runtime/path/DICE behavior, or weakens any
accepted raw-name, normalization, collision, containment, type, checksum, or
single-read gate.
