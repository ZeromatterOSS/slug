...
| M3: `query` | **active (`attr` six-ownership-mismatch evidence)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; the corrected 18-lane/165-pair manifest at SHA-256 `99b772e6…42300d`, five-file layout, transition allowlist, computed timeout, and package-license construction remain viable | the executable reconstruction reached two operational roots but strict ownership missed four positives and selected two negatives; use minimal focused evidence to classify each construction versus manifest boundary before another full source replay | `WP-4-8-m3-attr-six-ownership-mismatch-evidence` |
...
[`WP-4-8-m3-attr-six-ownership-mismatch-evidence`](./slug-v2-subplans/current-packet.md).
