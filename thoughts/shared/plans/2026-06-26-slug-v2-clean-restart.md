...
Run only `WP-5-m1-workspace-demand-provenance-bootstrap`.

Edit only new `app/slug_core_v2/src/runtime/demands.rs` plus
`app/slug_core_v2/src/runtime/mod.rs`,
`app/slug_core_v2/src/runtime/events.rs`, and
`app/slug_core_v2/src/runtime/dice.rs`. Add the sparse workspace-lifetime
node-metadata owner; the sole passive/eventful runtime activation tracker; a
centralized production user-data factory used before any retained bzlmod graph
warming; exact-closure semantic scope selection; and one closure read shared
with event selection. Preserve marker-absent direct printing. Stop on native
I/O, epoch injection, retry, accept/discard, current-command event capture,
publication, public command/server APIs, Cargo, DICE-core, or extra-file edits.
