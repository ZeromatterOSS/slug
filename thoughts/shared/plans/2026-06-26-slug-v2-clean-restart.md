...
Correct the stopped oracle around explicit BEP-producing `build //...`
diagnostic companions, absent-`combinedForm` empty projection,
fixture-opt-in-only epoch observation, exact Bazel cleanup, and an explicit
no-Slug-lifecycle-evidence boundary. Freeze the exact command count, expected
records, scope, and growth before any harness, fixture, RC, expected-file, or
production edit.
