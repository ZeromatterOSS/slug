...
| M3: `query` | **active (`attr` source-template retry 3)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; the corrected 18-lane/165-pair manifest at SHA-256 `99b772e6…42300d`, five-file layout, transition allowlist, computed timeout, and package-license construction remain viable | retry-2 evidence had a second literal-argv transcription defect after its allowed correction; freeze executable argv once, execute it before docs, then recreate and review the five bodies, two-root 18-lane replay, nine controls, and focused probes | `WP-4-8-m3-attr-five-source-template-oracle-design-retry-3` |
...
[`WP-4-8-m3-attr-five-source-template-oracle-design-retry-3`](./slug-v2-subplans/current-packet.md).
