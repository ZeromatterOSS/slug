...
Implement only `WP-5-m1-loading-event-producer`.

Add marker-conditional local event batches for loading only. Edit
`app/slug_loading_v2/Cargo.toml`, `src/bzl_module.rs`, and focused evidence in
`tests/{bzl_invalidation.rs,build_file_loading.rs}`. Preserve direct printing
when no marker is installed, keep dependency batches on their own nodes, store
explicit empty batches, and retain only the executed prefix on runtime failure.
Do not change key values/equality, typed Need, runtime, commands, or
publication.
