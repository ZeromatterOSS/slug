...
| M8: bootstrap | **active (developer graph)** | exact 33-package CLI boundary plus accepted Gates A-B; Gate C1 has 43 accepted Bazel targets covering 906 source cases; one canonical payload owns all 14 shared fixture workspaces; the cache gate selects nightly; verified actiond reached one remote spawn; the next BuildBuddy gate still stopped at an opaque pre-target command-line error | Token-free user diagnosis, prime-runner classifier drift, successful live cache/RBE evidence, core host-tool owners, and self-hosting remain | `WP-10-m8-bazel-buildbuddy-command-stderr-user-decision-after-nightly-repair` |
