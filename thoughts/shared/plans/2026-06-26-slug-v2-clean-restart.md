...
| M7: command/ruleset breadth | **partial; repository context, symbolic-macro lifecycle, subrule loading, FDO command configuration, selected-toolchain context, configured hidden dependencies, direct subrule calls, the first generic fragment category, dense retained depsets, configured action environments, and scalar Args/run/symlink declaration accepted** | accepted M7A loading/repository closure through `c83e70f0f`; symbolic macros/providers and configured namespaces in `e34cfdc7a`/`541fcfaf2`; corrected subrule architecture/loading in `4900ce46b`/`965cfde5e`; lawful FDO command/DICE producer in `4425d3bfb`; generic selected-toolchain context in `ebd19e3b1`; configured hidden dependencies in `2bf86bfa8`; direct invocation/value materialization in `da6865a3b`; real starlark-rust `set` in `cb71a302d`; generic fragment projection in `683538254`; dense depset/action-input owner in `7b0db03e1`; configured-action-environment owner in `71d34affa`; the current packet terminally accepts typed `DefaultInfo.files`, generic scalar Args/run/symlink owners, configured caller authentication, and action publication cutoff | vector Args transforms, param-file policy/write, the remaining spawn envelope and typed standard-provider breadth remain | author the bounded non-callback vector Args/param-file successor |
