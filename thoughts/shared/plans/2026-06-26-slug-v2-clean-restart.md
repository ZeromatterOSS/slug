...
Preserve the compact two-file producer, authority/preflight model, exact
operations, Unix/Windows lstat contract, lifetime boundary, and exclusions.
Do not restore the reverted implementation draft.

Audit Bazel 9.2 `WindowsFileOperations.readSymlinkOrJunction`,
`ReadSymlinkOrJunction` and its JNI path against pinned Rust
`std::fs::read_link`. Freeze an exact private Windows ReadLink seam for NTFS
symlinks, junctions/mount points, WSL `IO_REPARSE_TAG_LX_SYMLINK`, ProjFS,
ordinary files, unknown reparse tags, sharing violations, missing paths, and
other open/query errors. Specify the exact raw UTF-16 input transform,
no-follow handle flags, `DeviceIoControl(FSCTL_GET_REPARSE_POINT)` buffer
alignment and bounds checks, substitute-name extraction, relative/absolute
target and namespace-prefix output normalization, RAII/error capture, and
projection into the existing ReadLink operation schema.

Resolve the apparent schema edge where Bazel lstat classifies every reparse
point as Symlink but ReadLink treats ProjFS as not-a-link. Return `REPLAN`
rather than fabricate a contradictory WrongKind or widen public workspace
schema without review. On Unix, split directory opening from iteration so
only iterator `EIO` restarts/discards; opening `EIO` remains exact I/O.
Make focused tests platform-correct by using an absolute temp-root sentinel
and cfg-specific raw access-denied codes.

Freeze pure host-runnable buffer/parser/error/output tests plus any required
Windows-only evidence, exact implementation allowlist, and stop conditions.
Do not edit Rust, Cargo, fixtures, plans, or routing during the design packet;
add dependencies/files/APIs, wire runtime/DICE/materialization, or broaden
