...
Run only
`WP-5-m1-runtime-path-observation-windows-adapter`.

After the accepted neutral and Unix checkpoints, edit only
`app/slug_core_v2/src/runtime/path_observation.rs`. Add no dependency, Cargo,
public export, caller, DICE, materializer, retry/publication, fixture, or
consumer change, and do not change non-Windows behavior.

Under `cfg(any(test, windows))`, add safe host-pure numeric kind, permission,
FILETIME, raw UTF-16 long-path/find-name, checked reparse parser, scripted call
traits, and handle-owner state machines. This layer has no OS imports, externs,
or unsafe. Under `cfg(windows)`, copy only the exact used pinned
`windows-sys 0.61.2` HANDLE/BOOL/FILETIME/FILE_BASIC_INFO/WIN32_FIND_DATAW
layouts, constants, and CreateFileW/CloseHandle/GetLastError/
GetFileInformationByHandleEx/DeviceIoControl/FindFirstFileExW/FindNextFileW/
FindClose signatures in one kernel32 extern block. Native RAII, `OsString`
WTF-16 conversion, adapter, and private Windows entrypoint live only here.

Lstat rejects interior NUL, retries initial `symlink_metadata`, collapses every
initial non-Interrupted failure to Missing, and stages size/mtime/attributes/
permissions/kind before a no-follow native ctime query. Preserve reparse,
device, directory kind precedence; `0555`/`0755`; exact OpenJDK mtime and native
wrapping ctime formulas; and raw query 2/3 Missing, 32 PermissionDenied, other
exact I/O.

ReadLink uses the accepted no-follow open flags and 16-KiB aligned reparse
buffer. Capture native failure before close; map raw 4390/1 to typed NotALink.
Check returned length against capacity before any slice, then parse checked
NTFS/LX/mount substitute targets, ProjFS NotALink, and unknown-tag Unsupported,
preserving lone surrogates and exact prefix/backslash normalization.

Raw Find enumeration uses one Option-handle owner. Initial raw 2 is empty
success without auxiliary lstat; every other initial or iterator error is
captured before ignored close and classified through the neutral table. Raw 18
is EOF. Copy each fixed UTF-16 name immediately, reject unterminated buffers,
skip exact dot entries, discard all partial names on failure, sort/project only
after complete enumeration, disarm explicit close, and use Drop only as a
single fallback.

Exhaust host-pure/scripted attributes, permissions, time boundaries/overflow,
long-path forms, parser tags/layouts/offsets/ranges/returned lengths, raw names,
file-handle open/query/close precedence, and Find initial/first/next/EOF/error/
partial/close/Drop/refinement behavior. Add Windows ABI size/alignment
assertions. Validate host focused and full core tests plus the provisioned real
`cargo test -p slug_core_v2 --target x86_64-pc-windows-gnu --no-run` link that
produces unit and integration executables against kernel32, formatting, diff,
archive, exact one-file scope, and forbidden references. Return `REPLAN` on a
neutral-trait change, dependency/public/non-Windows expansion, unsafe outside
the ABI/RAII boundary, copied-layout mismatch, or unavailable link gate.
