...
The sole CLI-root Bazel aquery exited 0 in 5.12 seconds and showed new cache
leaf build-script tree artifact 3518 in the leaf Rustc input depset, all 15
proto inputs, Linux x86_64 protoc selection, no old adapter build-script
action, and the retained LALRPOP edge. It ran zero actions. Raw query and
analysis SHA-256 values are recorded in the current packet. Bazel's
`--nofetch` warning leaves external crate-repository freshness unproved;
generated bytes, compilation and M7A remain open. The accepted cache-core
predecessor is `5437a1474`; the callback candidate remains unaccepted at
local `review/wp-7-11-rustc-crate-root-map-each` (`ea5fcc6fd`).
...
offline action query may prove declared output-tree reachability, without
running build actions or proving generated bytes, compilation or M7A.
The sole query exited 0 in 6.267 seconds and showed both build-script
`out_dir` tree artifacts as inputs to their respective library Rustc actions
by matching artifact IDs. The grammar, five protos and eight vendored protoc
executables are producer inputs; REAPI selects Linux x86_64 protoc. Receipt
SHA-256 is
...
Independent final review `ACCEPT` confirmed configured input wiring only;
generated bytes, compilation and M7A remain open.
