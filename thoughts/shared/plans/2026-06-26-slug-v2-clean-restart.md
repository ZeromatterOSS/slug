...
Run only
`WP-5-m1-runtime-materializer-implementation-checkpoint-split`.

Perform a read-only split of the accepted three-file implementation union into
three serial terminal checkpoints:

1. one-file captured archive and byte-oriented USTAR regular/directory subset
   in `runtime/repository_io.rs`;
2. one-file pure retained-session, accepted-cache, validation-dirtiness,
   lifetime, and cumulative-epoch kernel in `runtime/repository_io.rs`, using
   scripted materialization and observation seams; and
3. the native bridge: only the accepted `pub(super)` entrypoint in
   `runtime/path_observation.rs`, the private owner field/init in
   `runtime/dice.rs`, and the minimum bridge in `runtime/repository_io.rs`.

For each checkpoint return the exact functions/types it may add or change,
focused evidence, downstream validation, and stop conditions. Preserve the
accepted archive ordering/raw-name/normalization behavior, single-session
token state machine, owner-held provisional roots, append-only lifetime,
burned IDs, full request equality, dirtiness rules, Local behavior, complete
epochs, three-file union, and every public/DICE/activation exclusion. Do not
edit Rust or add a fourth lifecycle packet unless a concrete dependency proves
one of these three cannot land independently.
