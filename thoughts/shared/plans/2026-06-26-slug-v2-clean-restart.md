...
The accepted WP-7-14 declaration checkpoint at `757a96799` found the new
cache-leaf build-script output tree in the CLI-root leaf Rustc input depset,
with all pinned proto inputs and Linux x86_64 protoc selected. It ran zero
actions. The current packet built only that script under a 60-second cap. It
exited 0 in 31.36 seconds and produced six nonempty Rust modules matching the
leaf's `include_proto!` names. Leaf/CLI compilation and M7A remain open.
The callback candidate remains unaccepted at local
