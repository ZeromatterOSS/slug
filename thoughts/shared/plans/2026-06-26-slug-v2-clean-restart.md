...
| M4: `cquery` | **partial (analysis-error publication accepted)** | projected label/Starlark-label output, direct retained-state functions, exact toolchain plus delegation graph evidence, one Need-aware analysis key/result, structural/null node identity, classified ordered edges, fixture-proven root delegating activation, and build/cquery semantic error publication without valid error nodes | audit the bounded root toolchain/platform topology activation that must precede traversal breadth | `WP-6-m2f-root-toolchain-platform-topology-activation-audit` |
