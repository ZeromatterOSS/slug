...
| M7: command/ruleset breadth | **partial; nonregistry REPO-file observation retry active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, `cb5073e0`'s immutable action-owner row, `b7390392`'s rules_rust 0.73 evidence, `a3efa1b7`'s root MODULE-files epoch, `3d174006`'s effective-override epoch, `ae8aa35e`'s request/materialization epoch, and `12f68983`'s repository-source epoch are accepted | selected graph still crosses the carrierless, event-owning nonregistry REPO-file then ignore/preflight/closure chain; registry preparation is separate | complete the bounded REPO-file observation retry before ignore/preflight/closure; then resume the selected-module-graph frontier and preserve M7A -> M8 -> M7B |
...
[WP-6-7A-host-nonregistry-repo-file-observation-implementation-retry](./slug-v2-subplans/current-packet.md).
