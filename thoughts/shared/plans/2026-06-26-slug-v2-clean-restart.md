...
| M7: command/ruleset breadth | **active (selected registry RepoSpec cap correction)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite run, complete Host discovered leaves, normalized/effective overrides, exact shared versions, accepted callerless selected graph in 216a0be8, and accepted selected RepoSpec design in 1b4b4996; the first compiling implementation exceeded its production cap | freeze corrected caps plus whitespace registry-JSON and normalized file/local-path semantics before resuming the retained unaccepted diff | WP-5-host-selected-registry-repo-spec-owner-implementation-r2-cap-design |
...
[WP-5-host-selected-registry-repo-spec-owner-implementation-r2-cap-design](./slug-v2-subplans/current-packet.md).
