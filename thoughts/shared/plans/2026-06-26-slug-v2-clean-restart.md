...
| M8: bootstrap | **active (developer graph)** | exact 33-package CLI boundary plus accepted Gates A-B; Gate C0 freezes the CLI runfiles boundary and passes the CLI library unit test; Gate C1 inventories all 1,005 transitive V2 cases, with 35 accepted Bazel targets covering 447 source cases and passing all 443 default-active Linux Cargo cases with stable locks; the Bzlmod lockfile scratch successor is source-exact | query unit and Bzlmod unit are whole-target **REPLAN** at clean-baseline failures; the CLI integrations, query loading integration, and server unit target share the nested-fixture **REPLAN**; core host-tool ownership needs a separate design; cache/RBE and self-hosting remain | `WP-10-m8-bazel-bzlmod-lockfile-test-semantic-adapter-implementation` |

### Current packet

`WP-10-m8-bazel-bzlmod-lockfile-test-semantic-adapter-implementation`.
