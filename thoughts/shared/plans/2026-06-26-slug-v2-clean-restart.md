...
| M3: `query` | **active (`attr` executable reconstruction retry 3)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; the corrected 18-lane/165-pair manifest at SHA-256 `99b772e6…42300d`, five-file layout, transition allowlist, computed timeout, package-license construction, six-row source fixes, and generated-owner identity remain viable | retry-2's fresh emitter fails Bazel package load on invented `attr.label(..., allow_none=True)`; assert it absent and pass package load before replay | `WP-4-8-m3-attr-five-source-executable-reconstruction-retry-3` |
...
[`WP-4-8-m3-attr-five-source-executable-reconstruction-retry-3`](./slug-v2-subplans/current-packet.md).
