...
Freeze the corrected exact desktop-Windows `Lstat` staging and classification.
Validate interior NUL first. Retry `symlink_metadata` on `Interrupted`, then
map every other initial metadata failure to Missing as Bazel's Windows
`statIfFound` does. That first metadata read alone supplies size, mtime,
attributes/permissions, and kind. Classify every `FILE_ATTRIBUTE_REPARSE_POINT`
as Symlink exactly like Bazel's native `IsSymlinkOrJunction`, then Device as
SpecialFile, then Directory, otherwise RegularFile. Do not use Rust's
name-surrogate-only `FileType::is_symlink()` decision.

The later private raw long-path/no-follow `FILE_BASIC_INFO` query contributes
only ctime. Preserve its exact missing/access-denied/other I/O projection,
distinct signed time formulas, all generic operation rules, focused pure and
platform tests, and the two-file allowlist. Return `REPLAN` if this exact
correction changes the ownership, dependency, unsafe, or file boundary.
