...
Design only `WP-5-m1-first-leaf-up-structural-propagation-read-migration`.

Perform a read-only live-checkout audit from `ModuleSourcePreparationKey`
through every intended discovery/graph consumer and from the legacy
`WorkspaceRawFileKey` plus immutable `std::fs::read` paths through their
repository-source callers. Name the first coherent leaf-up packet that can
carry `PathOutcome::Need` without converting it to domain strings or starting
the runtime retry loop.

Freeze exact owner files, public/private value schemas, error conversions,
DICE equality/validity, materialization-instance authority, local versus
immutable path namespaces, observation operations, and retained-engine
transition evidence. Decide whether structural propagation and the first
local/immutable read migration fit one bounded packet; split them if either
would introduce a second owner or require runtime epoch production.

Do not edit Rust, Cargo, fixtures, or oracles. Preserve package lookup,
directory/loading/analysis/query breadth, command retry, producer capture,
events, and terminal publication as later serial owners unless the live caller
closure proves a smaller prerequisite must precede them.
