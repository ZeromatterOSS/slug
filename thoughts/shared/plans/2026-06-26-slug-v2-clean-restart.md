...
| M3: `query` | **active (`attr` source-template retry)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; pinned Bazel 9.2 closes the corrected 18-lane/165-pair families, reviewed five-file two-package layout, transition allowlist, and computed timeout manifest at SHA-256 `99b772e6…42300d` | rebuild exact five source bodies with the four retained review corrections, then validate hidden constructions plus all 18 argv/stdout bindings twice; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats stay deferred | `WP-4-8-m3-attr-five-source-template-oracle-design-retry` |
...
[`WP-4-8-m3-attr-five-source-template-oracle-design-retry`](./slug-v2-subplans/current-packet.md).
