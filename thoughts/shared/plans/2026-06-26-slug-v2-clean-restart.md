...
| M8: bootstrap | **active (developer graph)** | exact 33-package CLI boundary plus accepted Gates A-B; Gate C1 has 43 accepted Bazel targets; normal-RC minimal/RBE-profile builds succeed; the source-pinned execution-log replacement contract is accepted | Run its probe once, then resume structured cache/RBE proof and the full 43-test gate | `WP-10-m8-bazel-buildbuddy-prime-execution-artifact-contract-live-evidence` |
