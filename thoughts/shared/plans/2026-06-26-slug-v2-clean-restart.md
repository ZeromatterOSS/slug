...
| M5: `aquery` | **active (sole-candidate action platform design accepted)** | recursive action ownership, complete Slug configuration identity, strict closure-resolved toolchain-backed FileWrite semantics, exact framed Slug FileWrite identity, declaration-ordered root-only literal text, bounded aspect-free `deps()` ownership, and a one-candidate selected-toolchain implementation action-platform contract | `deps()` oracle/implementation and broader zero-toolchain/multi-platform/action/expression/output breadth remain open | `WP-8-m5-filewrite-aquery-deps-owner-set-platform-oracle-implementation` |
...
[`WP-8-m5-filewrite-aquery-deps-owner-set-platform-oracle-implementation`](./slug-v2-subplans/current-packet.md).
