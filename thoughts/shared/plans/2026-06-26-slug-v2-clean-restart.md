...
[WP-4-5-7A-external-bzl-source-observation-cutover-implementation](./slug-v2-subplans/current-packet.md).

Commit `ecee4aca5` terminally accepts the complete Bazel 9.2
TestingBootstrap loading ABI and generic builtin-provider identity. Two fresh-
root real rules_rust replays clear that category and expose a distinct generic
source-consumer boundary: root-apparent external `.bzl` loading still bypasses
the accepted Root/Canonical observation owner for built-in `@bazel_tools`.
R1 full convergence was rejected because it changed the accepted exact Root-
request DICE child identity/order. Freeze only R2's disposition-aware typed
projection: Root requests retain legacy keys; Root built-ins and Canonical
routes use the shared observation owner. Independent R2 review returns
`ACCEPT`; implement only that four-file contract. The proof-only registration and selected-
context candidates remain parked and read-only. If the cutover reaches the
catalog boundary, the next packet must import the complete direct
`tools/build_defs/cc` package category, never only `action_names.bzl`.
