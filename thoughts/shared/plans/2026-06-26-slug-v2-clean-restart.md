...
[WP-6-7A-bootstrap-critical-repository-ruleset-frontier-audit-r2](./slug-v2-subplans/current-packet.md).

Audit the exact bootstrap-critical developer graph from the newly accepted
selected source owner through the existing rules_rust
`repository_rule(doc=...)` terminal. Select only the uniquely smallest next
M7A semantic owner or one bounded evidence prerequisite; authorize no Rust.
