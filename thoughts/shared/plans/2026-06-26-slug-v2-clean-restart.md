...
validation. It replaces split attribute edge variants with one typed public
attribute dependency role and shares a general default/named exec-group
identity with action contexts so later named/composed groups extend the same
owner. Named/automatic groups, configured aspects and cquery tool traversal
remain deferred. Bazel 9.2 is sole semantic authority, Zabel is peer design and
optimization guidance only, Buck2-derived compact utilities are reused, BCR
Starlark owns all rule bodies, and rules_rust/`cc_common` are downstream
consumers rather than implementation branches.
