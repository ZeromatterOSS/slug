...
Implement only `WP-5-m1-analysis-event-producer`.

Edit only `app/slug_analysis_v2/Cargo.toml`,
`app/slug_analysis_v2/src/{dice.rs,starlark_rule.rs}`, and focused evidence in
`app/slug_analysis_v2/tests/starlark_rule.rs`.
`ConfiguredTargetAnalysisKey` owns one marker-conditional local rule
implementation batch. Marker absence preserves direct printing and dependency
targets retain separate nodes. Do not change analysis values/equality, loading,
runtime, execution, or publication.
