...
| M3: `query` | **active (`attr` two-package oracle generation)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; pinned Bazel 9.2 closes the corrected 18-lane/165-pair matrix and the reviewed five-file positive-default/external-baseline layout | generate and clean-replay the bounded isolated Bazel-only attr fixture; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-two-package-observable-candidate-oracle-generation` |
