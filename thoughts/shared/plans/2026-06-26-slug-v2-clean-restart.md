...
Run only
`WP-5-m1-root-module-include-composition-event-oracle`.

Strengthen only `module-include-change-invalidation` under the exact five-file
allowlist in the owning plan. Add the accepted eight retained-daemon rows for
one root-plus-inline-includes event stream, exact repeated nested execution,
semantic and print-only edits, warm nonreplay, missing/parse preflight
suppression, runtime prefix, and recovery. Generate and independently replay
the Bazel 9.2 oracle; do not edit the harness, Rust, Cargo, another fixture,
BUILD/module payloads, registry data, commands, server/runtime paths,
loading/analysis evaluators, nonroot evaluation, or discovery behavior.
