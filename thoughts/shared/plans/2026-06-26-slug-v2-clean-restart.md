...
`WP-5-m1-runtime-materialization-preflight-retry-design`.

Audit the retained command/runtime boundary, `LocalRepositoryIo`, structural
path epoch production, exact-version activation closure, and the now-landed
repository materialization request/result graph. Freeze the smallest serial
runtime owners that can preflight pure materialization Needs outside DICE,
retain exact immutable results and instances across later path retries, inject
complete replacement result/path epochs, and publish only the terminal
attempt's effects.

The design must preserve demand-driven unused-repository laziness, logical
Local Host roots and pre-observation validation, failure-only request
generation, exact per-repository immutable dirtiness, ctime-only non-dirtiness,
unchanged-repository offline reuse, and materialization-before-source
observation ordering. Resolve the archive race where source identity can hash
captured bytes while extraction rereads a mutable path. Name the exact
request/result retention table, dirty detector, retry states, epoch replacement
rules, terminal-demand retention, event/effect policy, file allowlist, and
focused retained-daemon evidence.

Do not edit Rust, Cargo, fixtures, or oracle artifacts; activate source
preparation; broaden discovery/package/loading/analysis/query; or claim a
runtime detector, ctime policy, offline reuse, or publication owner without
pinned Bazel 9.2 and live-code support. Return `REPLAN` if final-demand or
terminal-publication ownership is a prerequisite that cannot fit one bounded
