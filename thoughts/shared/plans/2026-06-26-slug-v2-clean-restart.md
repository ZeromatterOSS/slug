...
Run only
`WP-5-m1-operational-path-resolution-implementation-checkpoint-split`.

Design a serial two-owner implementation split without changing the accepted
public schema, final two-file boundary, algorithms, fixtures, equality, or stop
gates. Owner A must leave an independently useful and reviewable operational
`ResolvedPathKey`: schemas, iterative parent/target frame machine, exact
operational errors, and all pure plus cumulative observation-backed A/B
chain/link/provenance/error evidence. Owner B must then add the semantic
`PathFileBytesKey`/`PathFileBytesError`, direct semantic tables, and retained
selector/projection/counter C evidence.

Freeze each owner's exact public/reexport surface, test inventory, validation,
commit boundary, and residual risk. Decide whether Owner A may land a
temporarily resolver-only `path_resolution.rs` and how Owner B extends that same
file without rewriting accepted tests. Preserve no Cargo/dependency,
`path_observation.rs`, consumer, runtime, repository, retry, event, or
publication edits. Obtain independent architecture and implementation-feasibility
review before Rust.
