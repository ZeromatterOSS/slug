...
[WP-4-5-7A-command-registration-overlay-consumer](./slug-v2-subplans/current-packet.md).

### Contextual command setting preparation accepted; registration consumer active (2026-08-28)

Commit `f76576ccd` implements the first half of the independently accepted
category-3 architecture. One compact immutable occurrence carrier now crosses
build/cquery one-shot and daemon paths; one post-Bzlmod DICE owner resolves all
admitted Starlark occurrences and both native registration families into the
sole typed structural configuration; and the fixed `@@//:setting` bridge is
deleted.

Activate only `WP-4-5-7A-command-registration-overlay-consumer`. Reuse the
existing loading target-pattern/package walker for signed root-context command
rows; keep the positive MODULE keys unchanged; reverse only the normalized
toolchain command list; and merge command expansions before MODULE labels in
the sole configured consumer. Keep configured eligibility and providers closed.
