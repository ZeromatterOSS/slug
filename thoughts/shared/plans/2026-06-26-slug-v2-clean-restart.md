...
Run only read-only `WP-5-m1-nonroot-preparation-provenance-design`.

Inspect the pinned Bazel 9.2 `ModuleFileFunction`, registry download/event,
`RepoSpec`, and module-file value sources; the accepted registry transport,
local replay, source-preparation, and evaluation-ordering oracles; and the live
`ModuleSourcePreparationKey`, registry IO errors, patch errors, and equality
owners. Freeze one compact typed boundary for registry and non-registry
preparation success, ordered registry URL attempts with SHA-256 or explicit
absence, complete exhaustion, and fatal registry/materialization/patch causes.

The design must keep source bytes and selected provenance under the sole
preparation owner, exclude operational roots and generations from semantic
equality, preserve typed fallback versus fatality without string parsing, and
remain consumable by the later stable effective-`ModuleKey` discovery owner.
Decide exact success/error types, allocation/equality rules, lifecycle tests,
and a narrow implementation allowlist. Return `REPLAN` if accepted evidence
cannot distinguish a required cause or if the design would duplicate registry
iteration, source IO, patching, or discovery. Do not edit Rust, Cargo, fixtures,
expected artifacts, DICE, commands, server, or lockfiles in this packet.
