...
[WP-5-7A-selected-bcr-archive-transform-implementation-r2](./slug-v2-subplans/current-packet.md).
...
Run only `WP-5-7A-selected-bcr-archive-transform-implementation-r2` under the
same frozen nine-file core allowlist and 900/1,100/2,000 caps. Retain R1's
complete phase-scratch implementation and add bounded, well-formed global PAX
`comment` metadata only. Local PAX, malformed records and every semantic or
unknown global key remain unsupported. Do not change Bzlmod, loading,
registration, action or REAPI files.
