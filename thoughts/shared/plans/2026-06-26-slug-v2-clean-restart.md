...
[WP-4-5-7A-root-package-external-bzl-load-owner-design](./slug-v2-subplans/current-packet.md).

Design a structural selected-registry projection in the existing root route
owner, then let root package loading consume that route for external Bzl
children. Loading must not reconstruct mappings or definitions; authorize no
Rust.
