...
| M7: command/ruleset breadth | **partial; bootstrap-critical frontier audit active** | generated `repository_ctx.file` producer, demand-side route/request, atomic immutable-root application and tool-aware Bazel/Slug generated-source fixture are accepted through `3ac0a85b` | the next uniquely smallest bootstrap-critical repository/ruleset owner has not yet been selected | audit only `WP-6-7A-bootstrap-critical-repository-ruleset-frontier-audit`; preserve M7A -> M8 -> M7B |
...
[WP-6-7A-bootstrap-critical-repository-ruleset-frontier-audit](./slug-v2-subplans/current-packet.md).

Audit the accepted M7A/generated-source boundary against the exact M8 developer
graph and select one uniquely smallest missing repository/ruleset owner or one
bounded evidence prerequisite. Authorize no implementation during the audit.
