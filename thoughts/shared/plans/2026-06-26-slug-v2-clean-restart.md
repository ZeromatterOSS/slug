...
The pinned source audit at SHA-256
`0befd1d6b4187984859ee3a07769a50ff314f092217d8b1d303f5bc14251bb17`
finds that `ahash` and `rustix` cannot add named `feature=...` cfgs through
their generated flags; the former captures child stdout and the latter
discards it. `num-traits` explicitly emits only `has_total_cmp`, but its
`autocfg` probe child inherits stdout and may use environment-selected
wrappers, leaving that path opaque. Independent final review `ACCEPT`
confirmed the partial source bound. No build, query or test ran; all-five
feature closure and actual generated bytes remain open.
The selected successor permits one offline CLI-root action query filtered
to the `num-traits` build-script run action. It inspects the configured
environment for rustc wrappers used by the remaining opaque probe-child
path. It executes no action and cannot infer `_bs.flags` bytes or compilation.
Independent design review `ACCEPT` confirmed the exact action filter and
required that absent aquery environment keys remain unknown when inherited
default-shell values are not exposed.
