...
| M7: command/ruleset breadth | **active (repository local-path policy ownership design)** | consumer audit `1cd140a6` accepted | retain root-vs-command path provenance before request projection | WP-4-5-host-repository-local-path-policy-owner-design |
...
[WP-4-5-host-repository-local-path-policy-owner-design](./slug-v2-subplans/current-packet.md).
