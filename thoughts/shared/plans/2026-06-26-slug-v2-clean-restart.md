...
| M7: command/ruleset breadth | **partial; file-effect producer implementation retry active** | independently accepted 930/850/1,780 cap correction over the exact retained seven-file candidate; four focused tests pass | producer ABI/DICE/lifecycle proof remains incomplete | implement only `WP-4-5-selected-repository-file-effect-producer-implementation-retry`; preserve M7A -> M8 -> M7B |
...
[WP-4-5-selected-repository-file-effect-producer-implementation-retry](./slug-v2-subplans/current-packet.md).

Complete only the already-designed producer's ABI/DICE/lifecycle proof under
the corrected caps. Preserve its owner structure and do not activate a caller.
