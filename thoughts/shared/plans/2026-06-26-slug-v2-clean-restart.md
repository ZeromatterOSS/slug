...
orchestration-policy audits froze the complete lockfile-v28 defaults, typed
semantic value/equality, adapter and renderer behavior, exceptional catch
holes, oracle matrix, and private-owner-plus-atomic-cutover sequence. The
current live schema is intentionally still lossy, so Host lockfile ownership
remains blocked until the schema oracle, dormant exact owner, and live cutover
accept. No Rust, fixture, Cargo, public API, or activation changed.

### Current packet

Implement only `WP-5-m1-bazel-lockfile-v28-schema-oracle`.

Add the isolated `tests/v2_oracle/fixtures/bazel-lockfile-v28-schema/**`
fixture with no harness or Rust change. Generate one comprehensive v28 value
with pinned Bazel 9.2.0, then retain the exact default/null/unknown/version-gate
rows, complete typed extension shape and canonical rewrite, caught failures,
uncaught custom-adapter holes, JSON grammar pair, and malformed recorded-input
replacement frozen in the Stage 5 owner plan. Stop and replan on a required
harness change or a field real Bazel cannot generate and retain.
