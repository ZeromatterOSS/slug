...
| M7: command/ruleset breadth | **partial; M7A immutable configured-action owner implementation active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI and generated Bazel 9.2 default/named exec-group owner evidence are accepted | one immutable creation-time configured-action row must replace projection-time owner/platform reconstruction before bootstrap action breadth | implement that owner without public named-group breadth, then follow M7A -> M8 -> M7B |
...
[WP-6-7A-immutable-configured-action-owner-context-implementation](./slug-v2-subplans/current-packet.md).
