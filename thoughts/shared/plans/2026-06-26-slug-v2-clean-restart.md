...
| M7: command/ruleset breadth | **active (bounded run implementation)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, plus accepted client-launch/daemon-wire design | implement the first executable FileWrite `run` vertical without inventing runfiles or direct-local build actions | `WP-8-m7-filewrite-run-handoff-implementation` |
...
[`WP-8-m7-filewrite-run-handoff-implementation`](./slug-v2-subplans/current-packet.md).
