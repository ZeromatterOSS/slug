...
[WP-4-5-7A-contextual-command-setting-preparation](./slug-v2-subplans/current-packet.md).

### Contextual command-overlay architecture accepted; setting preparation active (2026-08-28)

Commit `d5362b099` freezes the independently accepted category-3 architecture:
one compact command occurrence carrier, one post-Bzlmod structural
configuration preparation owner, and one later shared signed registration
consumer. It plans all Starlark-setting kinds and both native registration
families together while retaining one configuration identity and one loading
target-pattern engine.

Activate only `WP-4-5-7A-contextual-command-setting-preparation`. Replace the
singular `@@//:setting` bridge across build/cquery one-shot and daemon paths;
resolve main-repository flags without an unrelated Bzlmod-graph dependency and
apparent-external flags through the final root mapping; convert all five
declaration kinds and both native extra-registration lists into the sole typed
structural configuration; and leave registration consumption closed for the
following packet.
