...
| M7: command/ruleset breadth | **partial; root MODULE-files observation implementation active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, generated Bazel 9.2 owner evidence, `cb5073e0`'s immutable configured-action row, and `b7390392`'s exact rules_rust 0.73 generated-registration/provider/action evidence are accepted | the accepted root-module observation drops extension usages and legacy visible-lockfile completion has no epoch, so selected graph and extension-generated repository owners cannot yet retain their first complete lower prefix | implement and accept the frozen two-file owner from `335cfa45`, then design the selected-module graph frontier and preserve M7A -> M8 -> M7B |
...
[WP-6-7A-root-module-files-observation-completion-implementation](./slug-v2-subplans/current-packet.md).
