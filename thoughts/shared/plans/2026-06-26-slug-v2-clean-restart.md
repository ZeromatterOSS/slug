...
[Exact one-file built-in C++ configure facade audited; bounded catalog
implementation selected](./slug-v2-subplans/current-packet.md).

The docs-only catalog audit returns `ACCEPT`. Pinned and installed Bazel 9.2
contain the same 784-byte `tools/cpp/lib_cc_configure.bzl` at SHA-256
`da7e4ae162120582a7a703b5657286dffe61fdf37cc489a4fc7625608517370c`.
Its sole load is the external rules_cc toolchain-config utility and its sole
export is `escape_string`; no second built-in member enters the closure. The
installed tree's `0755` is the same extraction artifact seen for the accepted
byte-identical sibling, while source/archive and Slug catalog mode remain
`0644`/non-executable.

Run only
`WP-5-7A-bazel-tools-lib-cc-configure-catalog-implementation-r1`. Add the
verbatim file plus one entry to the existing versioned catalog, update its
sorted direct-listing and manifest proof, rebuild, and replay. Do not add a
DICE key, BUILD file, install-tree fallback, evaluator/C++ behavior or
apple_support/rules_cc/toolchain special case. The replay's next independent
failure must become a new docs-first packet.
