...
Add the private Unix-dormant `HostGlobSegmentCandidatesKey` in exactly
`slug_loading_v2/src/host_glob/{mod.rs,tests.rs}` plus the private `lib.rs`
module declaration, within +1,750/-20. Consume only Host
`PathDirectoryListingKey` and matched/literal `ResolvedPathKey`; retain raw
byte components, direct kinds, matched-only symlink laziness, wildcard-only
ancestor-expansion failure, semantic errors, complete-error-before-Need
precedence, complete-only equality, and zero events/callers. Support only the
accepted literal/simple non-adjacent-`*` segment language; keep package
boundaries, multi-segment/`**`, parser activation, evaluator retry, native
