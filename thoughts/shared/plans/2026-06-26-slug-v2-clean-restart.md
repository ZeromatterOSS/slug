...
| M7: command/ruleset breadth | **partial; terminal source-carrier visibility design active** | accepted M7A repository-observation chain through `597df31b`; `b3eba6df` accepts the exact private terminal source epoch | the terminal matching-family surface is same-file private; generated package policy/lookup/source/load and public publication remain separate later owners | design only `WP-6-7A-host-root-apparent-repository-source-observation-observation-carrier-visibility-design`, Linux-first; then implement it before the generated package/publication frontier audit; preserve M7A -> M8 -> M7B |
...
[WP-6-7A-host-root-apparent-repository-source-observation-observation-carrier-visibility-design](./slug-v2-subplans/current-packet.md).
