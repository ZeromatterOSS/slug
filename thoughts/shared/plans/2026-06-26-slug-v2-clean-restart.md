...
[Bounded direct-external-Label repository read audited; two-owner implementation
selected](./slug-v2-subplans/current-packet.md).

Commit `3592fbfd1` terminally accepts the exact 12,993-byte
`tools/osx/xcode_configure.bzl` source at 6/16/22 gross production/proof/total
Rust additions plus its 329-line asset. The authenticated replay clears the
catalog miss and stops in apple_support 1.24.2 at
`repository_ctx.read(Label("@build_bazel_apple_support//crosstool:BUILD.toolchains"))`.

The docs-only audit returns `ACCEPT` for the direct canonical-external Label,
omitted/default auto-watch, present ASCII <=2 MiB category: regular-file
success is exact, while success for an existing-owner special-file source is
explicitly Slug-native because the retained source value does not preserve
that kind distinction. Preserve Bazel's
package-before-file ordering with the existing Label-path retry, then reuse the
already accepted routed source-byte retry and observed epoch. Run only
`WP-4-5-7A-repository-context-read-implementation-r1` under its two-file
allowlist and caps. String/path/explicit-watch/non-ASCII sources and all
apple_support/Xcode/C++ special cases remain deferred.
