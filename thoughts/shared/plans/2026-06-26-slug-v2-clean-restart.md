...
[Implement complete generic rule-level Starlark transition execution](./slug-v2-subplans/current-packet.md).

Implement the independently accepted
`WP-6-7A-rule-level-starlark-transition-execution-architecture-r1`. Apply a
rule's retained regular transition before final configured attributes,
dependencies, toolchains, fragments and implementation. The complete bounded
category includes every already-admitted typed Starlark build setting, the
target-platform native option, raw configured transition attrs, exact
identity/idempotent/non-idempotent delegation and rule-before-attribute
composition. One shared evaluator replaces the current narrow attribute-only
executor; one compact apply/skip bit joins configured-target/DICE identity.
Other native options, platform flags/mappings, aliased settings, split
attribute topology and configured aspects remain explicit later categories.
Bazel 9.2 is sole semantic authority, Zabel is peer ownership/optimization
guidance only, and BCR Starlark owns all rule bodies including `cc_internal`;
rules_rust and `cc_common` are downstream consumers, never special cases.
