...
Correct the generic oracle harness so a retained remote-executor fixture
appends Slug remote flags only to build commands and requires positive
structured REAPI evidence exactly for build rows whose command contract expects
exit zero. Preserve independent actual-exit validation and every current
successful REAPI build fixture. Edit only `tools/v2_oracle_lib/{runner.py,
compare.py}` and `tests/v2_oracle/test_v2_oracle.py`; do not add schema,
fixture-name, command-name, tool-output, fixture, or Slug production changes.
