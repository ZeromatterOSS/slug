...
Run only `WP-5-m1-nonroot-include-composition-design-rereview`.

This is a read-only packet. Redesign supplied-file composition over the landed
syntax inspector and one-file evaluator against `203cdaac`. Preserve Bazel's
one logical Starlark call stack across root→outer→nested execution, complete
closure parse/restricted-syntax/scope compilation before any directive runs,
per-file bindings with shared semantic state, exact raw-label-keyed
Program/Module reuse, file-local source spans, multi-heap rooting/reread, and
the already accepted registry-include ordering boundary.

Inspect the exact supported starlark-rust execution/compilation APIs and, if
necessary, a bounded upstream seam rather than accepting a diagnostic
divergence. Name the future implementation allowlist, ownership, focused tests,
oracle replay, and stop conditions. The confined Bazel cycle probe timed out
after 10 seconds without a diagnostic; that is non-normative characterization,
not permission to invent cycle rejection. If no exact common-stack and
compile-before-execute seam is feasible, return `REPLAN`. Do not edit Rust,
Cargo, fixtures, expected artifacts, harness code, DICE, preparation,
discovery, commands, server, or lockfiles.
