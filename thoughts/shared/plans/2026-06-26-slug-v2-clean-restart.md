...
Freeze the Bazel-internal byte representation spanning OS-native directory
components and evaluated Starlark glob patterns/results, plus the exact
pattern-lazy no-follow classification and symlink/special resolution boundary
required by the Stage 5 owner plan. Reuse compact Buck2-derived collections,
shared storage, hashing, and interning where applicable; identify any
parser/evaluator seam needed to keep literal UTF-8 bytes distinct from octal
byte escapes. Freeze the exact production/test allowlist, typed
Need/error/equality model, retained-DICE lifecycle, and Bazel-source/oracle
evidence before Rust. Do not implement code, change Cargo, activate loading,
or combine the separate regular-or-special Host file-byte projection.
