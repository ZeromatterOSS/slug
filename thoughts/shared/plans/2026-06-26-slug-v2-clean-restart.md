...
| M1: one semantic spine | **active** | Host loading observations, dormant typed root-loading/root-analysis/build/query command boundaries, accepted preactivation Host closure gate, and opaque accepted-command envelope; full evidence is in the Stage 5 owner plan | source-aware event publication, then query-first activation; external mapping, native Windows glob ordering, registry transports, and discovery composition remain | design source-aware command events |
...
`WP-5-m1-source-aware-command-event-design`.
