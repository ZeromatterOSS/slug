...
Audit all five generated-target glob calls and freeze explicit `allow_empty`
semantics for every row. Reproduce regular-to-directory behavior in isolated
cold and same-daemon controls, trace why the warm row retained the files-only
target despite `exclude_directories = 1`, and distinguish Bazel incremental
semantics from a mutation/visibility artifact. Then refreeze every row/output,
topology, allowlist, cap, and the typed-observation follow-up. Permit only
pinned-source inspection and confined temporary Bazel probes. Do not edit the
harness, fixture, expectation, assets, Rust, Cargo, APIs, owners, consumers,
or activation, and do not resume implementation before terminal acceptance.
