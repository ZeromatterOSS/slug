...
[Typed FilesToRun/runfiles provider-category architecture](./slug-v2-subplans/current-packet.md).

Commit `bfe6f2690` terminally accepts the common default-context non-callback
Spawn envelope. Its corrected public binding layer follows Bazel's signature
order before typed lowering; `run` and `run_shell` share one retained Spawn
owner, and scoped executable provenance prevents associated Files from losing
FilesToRun/runfiles semantics. The next design owns that typed provider
category generically; independent architecture/identity/lifecycle review
returned `ACCEPT`. It is not a `cc_common`, rules_cc, or C++ rule packet.
