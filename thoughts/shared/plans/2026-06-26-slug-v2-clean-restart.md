...
| M3: `query` | **active (`attr` transition-allowlist correction)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; pinned Bazel 9.2 closes the corrected 18-lane/165-pair families and reviewed five-file two-package layout; the semantic manifest remains structurally accepted | temp-oracle source synthesis disproved the frozen `l12_a003` transition-allowlist label, so correct that one row and digest before source-template design resumes; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-transition-allowlist-manifest-correction` |
...
[`WP-4-8-m3-attr-transition-allowlist-manifest-correction`](./slug-v2-subplans/current-packet.md).
