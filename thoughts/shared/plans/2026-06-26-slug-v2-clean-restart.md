...
| M5: `aquery` | **active (`deps()` implementation replanned)** | recursive action ownership, complete Slug configuration identity, strict closure-resolved toolchain-backed FileWrite semantics, exact framed Slug FileWrite identity, declaration-ordered root-only literal text, and a bounded aspect-free `deps()` owner-set contract | zero-toolchain action execution-platform assignment blocks the `deps()` implementation; later action/expression/output breadth remains open | `WP-8-m5-filewrite-aquery-zero-toolchain-platform-design` |
...
[`WP-8-m5-filewrite-aquery-zero-toolchain-platform-design`](./slug-v2-subplans/current-packet.md).
