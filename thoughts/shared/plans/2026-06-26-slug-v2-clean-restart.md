...
| M7: command/ruleset breadth | **partial; effective-module-override implementation retry active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, `cb5073e0`'s immutable action-owner row, `b7390392`'s rules_rust 0.73 evidence, and `a3efa1b7`'s complete root MODULE-files epoch are accepted | selected graph and discovered-module owners still consume carrierless effective override, registry preparation and nonregistry closure branches | accept the bounded effective-override sibling under corrected proof caps, then resume the selected-module graph frontier and preserve M7A -> M8 -> M7B |
...
[WP-6-7A-effective-module-override-observation-implementation-retry](./slug-v2-subplans/current-packet.md).
