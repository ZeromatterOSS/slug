...
| M7: command/ruleset breadth | **partial; extension definition/evaluation frontier audit active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, `cb5073e0`'s immutable action-owner row, `b7390392`'s rules_rust 0.73 evidence, the accepted repository-observation chain through `e155d74f`, `ccf7421e`'s selected registry repo-spec epoch, `9d2f7a7d`'s selected-module-routes epoch, `2e0a19ae`'s selected-extension-mappings epoch, and `094ba075`'s selected-extension definition-request epoch are accepted | audit definition loading/evaluation ownership and the private carrier handoff; root mapping/generated/public remain later | audit only `WP-6-7A-extension-definition-evaluation-observation-frontier-audit`, then preserve M7A -> M8 -> M7B |
...
[WP-6-7A-extension-definition-evaluation-observation-frontier-audit](./slug-v2-subplans/current-packet.md).
