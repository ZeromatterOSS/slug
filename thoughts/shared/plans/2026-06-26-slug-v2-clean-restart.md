...
[WP-4-7A-rules-cc-helper-internal-complete-loading-proof](./slug-v2-subplans/current-packet.md).

Prove exact complete `cc_helper_internal.bzl` loads its three complete children,
evaluates its admitted eager rows and freezes all lazy helpers without invoking
them. Add no production behavior or complete proxy claim.

### M7 complete private paths accepted; helper-internal proof selected (2026-08-26)

Commit `c4e49424c` adds 66 proof lines and no production. It embeds/hashes exact
complete private `paths.bzl`, evaluates its canonical `rules_cc+` owner and
freezes public lazy `is_path_absolute` without calling it. All 240 loading
library tests, 24 invalidation tests, 31 BUILD-loading tests, locked checks and
CLI build pass; independent review accepts bytes, caps and boundaries.

All three 383-line `cc_helper_internal.bzl` children are now exact complete.
Its eager constants, list extensions, structs, initialized provider and 22
instances use accepted evaluator shapes; lines 216-383 are lazy functions. Run
only `WP-4-7A-rules-cc-helper-internal-complete-loading-proof` under 0/480/480
caps. Prove exact full-source freeze and child pointer identities without
