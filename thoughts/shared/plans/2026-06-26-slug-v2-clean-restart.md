...
| M3: `query` | **active (`attr` observable-candidate oracle design)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; leaf attr formatting is source-closed; candidate order/multiplicity are proven unobservable | close typed correlation/combination, canonical-label, normalization-order, and total native-attribute evidence before representation design; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-observable-candidate-oracle-design` |
...
[`WP-4-8-m3-attr-observable-candidate-oracle-design`](./slug-v2-subplans/current-packet.md).
