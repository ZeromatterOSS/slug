...
| M7: command/ruleset breadth | **active (bounded FileWrite test design)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, plus the accepted POSIX executable FileWrite `run` vertical with client-only launch | freeze the first executable FileWrite `test` boundary without inventing broader test, runfiles, or BEP semantics | `WP-8-m7-filewrite-test-handoff-design` |
...
[`WP-8-m7-filewrite-test-handoff-design`](./slug-v2-subplans/current-packet.md).
