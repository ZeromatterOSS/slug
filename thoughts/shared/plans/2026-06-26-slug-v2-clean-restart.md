...
| M3: `query` | **active (`attr` observable-candidate oracle retry)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; pinned Bazel 9.2 closes the complete Starlark/native attr-visible loading schema, renderers, defaults, and removals; candidate order/multiplicity are proven unobservable | derive the smallest paired ordinary-query discriminator matrix from the accepted total-schema ledger; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-observable-candidate-oracle-design-retry` |
...
[`WP-4-8-m3-attr-observable-candidate-oracle-design-retry`](./slug-v2-subplans/current-packet.md).
