...
Run only `WP-5-m1-nonroot-owned-include-dispatcher`.
...
prepared program and a one-shot evaluator-owned
`Rc<[PreparedModule<'v>]>` registry. The setter consumes the prepared `Vec`;
opaque-index dispatch dupes the single `Rc` before borrowing an entry and
mutably executing it. Native `include()` resolves an exact raw label only in
compact Value-free `extra` state, passes its opaque index to the evaluator, and
never downcasts a prepared program.
...
supplied closure without effects; then move prepared storage into the one
execution evaluator. Dupe the registry `Rc` before mutably executing an index.
Preserve one call stack, repeated Program/Module
