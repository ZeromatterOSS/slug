...
Design only `WP-5-m1-host-registry-io-bridge-design`.

Audit the active wrappers and exact mode/expectation/generation matrix in
`registry_dice.rs`, then freeze a one-file private bridge contract that
factors a Bazel-shaped expectation-aware remote executor and a typed local
executor while preserving every active legacy wrapper and legacy Off behavior
byte-for-byte. The later Host remote wrapper must use the accepted
RegistryFunction hash-mode table. The local wrapper must retain sticky Found
and generation-gated absence/read failure. Do not edit Rust, implement the
Host registry-file owner, add a dependency or public API, or activate a Host
consumer in this design packet.
