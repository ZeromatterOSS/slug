...
[`WP-4-6-7A-rule-execution-group-runtime-design-r1`
docs/source design](./slug-v2-subplans/current-packet.md).
...
The successor designs one complete generic rule execution-group runtime,
including the selected automatic policy, before freezing implementation files,
caps and tests for independent architecture review. Do not erase group names,
substitute the default platform or merely move the pre-publication guard.
No new Rust or runtime behavior is authorized. Computed-default and later
C++/Java semantics remain independent unsupported categories.
