...
| M7: command/ruleset breadth | **active (selected registry RepoSpec owner design)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite run, complete Host discovered leaves, normalized/effective overrides, exact shared versions, and the accepted callerless selected graph in 216a0be8; the route audit proves canonical mappings are derivable but registry RepoSpecs are not yet owned | freeze the missing post-selection registry source.json-to-RepoSpec owner without activating the legacy supplied-file catalog | WP-5-host-selected-registry-repo-spec-owner-design |
...
[WP-5-host-selected-registry-repo-spec-owner-design](./slug-v2-subplans/current-packet.md).
