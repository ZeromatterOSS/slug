...
Run only `WP-5-m1-nonroot-package-policy-oracle`.

Append exactly 15 retained-daemon rows to `nonroot-include-composition`. Edit
only its `fixture.toml`, generated `expected/oracle.json`, a new
`modules/subject/pkg2/order.MODULE.bazel`, and a new checked-in symlink
`modules/subject/pkg/BUILD.link -> ../BUILD.bazel`.

The rows must prove primary/fallback BUILD selection, horizon-wide source-order
package preflight before either fragment executes, successful recovery with
exact boundary-then-order prints, a symlink-only BUILD marker and unchanged
selected-name nonreplay, canonical `--deleted_packages=@@subject+//pkg`
failure versus apparent `@subject//pkg` recovery, repository `.bazelignore`,
and `REPO.bazel` `ignore_directories()` failure/recovery. Deleted-policy rows
must retain the include-site wrapper and Bazel's exact deleted-package cause;
missing-marker rows must retain the exact no-BUILD cause. Every failure excludes
both print sentinels. Every two-fragment success retains marker digest
`f052c5616758f335713070ac968b140a71a32a2ea5a9be46871f720323eba3a8`.

Preserve every field of the 11 existing records byte-for-byte except the seven
successful manifest modes proven stale by the isolated HEAD replay. Change only
those exact `0o544` scalars to pinned Bazel's canonical read-only `0o555`.
Generate locally with pinned Bazel 9.2, replay twice independently, require
exactly two new files and fewer than 800 net newline-counted lines, and run the
full oracle harness plus source, manifest, provenance, symlink, scope, diff,
credential, and archive checks. Stop on any other old-row drift, unstable source
order or print count, a different marker digest, network input, a harness
feature, or any edit outside the four-file allowlist. No Rust, Cargo, command,
server, lockfile, or registry scaffold edit is authorized.
