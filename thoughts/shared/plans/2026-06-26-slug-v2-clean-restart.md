...
[Typed runfiles and complete DefaultInfo implementation](./slug-v2-subplans/current-packet.md).

The second implementation successor of the accepted generic
DefaultInfo/runfiles/FilesToRun architecture is terminally accepted. It owns a
typed immutable runfiles value, `ctx.runfiles`, `merge`/`merge_all`, typed
SymlinkEntry depsets, all five `DefaultInfo` parameters, executable insertion,
predeclared-output fallback, and alias-aware structural publication equality.
Support Artifacts/actions and Spawn expansion remain closed. The next bounded
successor designs generic runfiles support construction and atomic support-
action registration. This remains a generic standard-provider category, not a
`cc_common`, rules_cc, parser, or C++ rule packet.
