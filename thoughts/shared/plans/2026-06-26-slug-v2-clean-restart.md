...
| M1: one semantic spine | **active** | Host loading observations, dormant typed root-loading/root-analysis/build boundaries, accepted preactivation Host closure gate, opaque accepted-command publication, source-aware command events, and typed one-shot/daemon query activation; full evidence is in the Stage 5 owner plan | typed build activation; external mapping, native Windows glob ordering, registry transports, and discovery composition remain | design typed build activation without execution |
...
`WP-5-m1-build-activation-design`.
