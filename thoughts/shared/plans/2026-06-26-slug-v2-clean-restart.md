...
[Complete generic aspect declarations](./slug-v2-subplans/current-packet.md).

`WP-6-7A-generic-aspect-declaration-implementation-r1` is terminally
`ACCEPTED`. One typed live/frozen/importable declaration owns generic aspect
attributes, fixed/callback propagation, provider predicates, direct required
aspects, fragments/toolchains, predicate/generating policy, constraints and
subrules. Fixed propagation retains source order with explicit set-semantic
equality. Complete loading and serial analysis suites pass within every cap.
A rebuilt fresh rules_rust 0.73 replay clears `rust_unpretty_aspect` and stops
at the independent provider-constrained `link_deps` target-invocation
boundary. Materialize that complete five-constructor provider-policy category
next; aspect attachment and configured aspect execution remain separate.
Bazel 9.2 remains sole semantic authority, Buck2-derived compact utilities are
reused, and Zabel remains peer architecture/optimization guidance only.
