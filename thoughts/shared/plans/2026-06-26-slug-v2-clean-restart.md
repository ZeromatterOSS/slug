...
Run only `WP-5-m1-nonroot-include-dispatcher`.

Edit only `starlark-rust/starlark/src/eval.rs`,
`starlark-rust/starlark/src/eval/compiler/module.rs`,
`starlark-rust/starlark/src/eval/runtime/evaluator.rs`, and
`app/slug_bzlmod_v2/src/module_eval.rs`. Add an opaque module-bound reusable
prepared program and a one-shot evaluator registry borrowing
`&'a [PreparedModule<'v>]`. Native `include()` resolves an exact raw label only
in compact Value-free `extra` state, passes its opaque index to the evaluator,
and never downcasts a prepared program.

Allocate every file Module before preparation and never grow that collection.
Use a temporary preparation evaluator to scope-check and compile the complete
supplied closure without effects; then declare prepared storage before the one
execution evaluator. Copy the registry slice from the evaluator field before
mutably executing an index. Preserve one call stack, repeated Program/Module
reuse, per-file bindings/spans/roots, scoped foreign-heap GC suspension,
Module/DefInfo/frame/logical-file restoration on success and error,
Bazel-shaped include-site diagnostics, final per-Module GC/reread, builder
validation, and Bazel's nonterminating cycle horizon.

Add the reviewed upstream and Slug focused regressions first, then run focused
and full `starlark`/`slug_bzlmod_v2` suites serially, formatting, diff/archive
checks, and the existing Bazel oracle replay. Do not edit Cargo, public
consumers, fixtures, expected artifacts, DICE, preparation, discovery,
commands, server, or lockfiles.
