...
| M7: command/ruleset breadth | **active (selected extension-mapping design correction)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite run, complete Host discovered leaves, normalized/effective overrides, exact shared versions, accepted callerless selected graph in `216a0be8`, selected registry RepoSpecs in `e8ad58dd`, selected routes in `6f72baaf`, and private root extension usages in `11be92b9` | correct root override targets to resolve through the full no-overrides extension mapping before Rust resumes | WP-5-host-selected-extension-mapping-owner-design-r3-correction |
