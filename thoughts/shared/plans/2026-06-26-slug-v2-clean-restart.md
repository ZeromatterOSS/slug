...
| M3: `query` | **active (`attr` oracle generation)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; pinned Bazel 9.2 closes the corrected 18-lane/165-pair families, reviewed five-file two-package layout, and complete stable-ID manifest at SHA-256 `3352106d…faf2` | generate and validate the manifest-bound loading oracle; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-two-package-observable-candidate-oracle-generation` |
...
[`WP-4-8-m3-attr-two-package-observable-candidate-oracle-generation`](./slug-v2-subplans/current-packet.md).
