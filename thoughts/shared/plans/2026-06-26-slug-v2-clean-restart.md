...
Design only `WP-5-m1-shared-retry-publication-driver-checkpoint`.

Freeze a bounded residual-sidecar checkpoint for the shared build/query
retry/publication driver. Reconcile typed Need propagation through entrypoints,
source-preparation/discovery activation, legacy snapshot retirement,
cancellation ownership, and terminal REAPI/event publication order with the
accepted runtime native-demand session. Do not edit Rust, activate a production
command surface, or combine independently large entrypoint, discovery, legacy
retirement, and publication changes before the checkpoint is independently
reviewed.
