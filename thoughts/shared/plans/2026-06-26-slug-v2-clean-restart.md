...
| M1: one semantic spine | partial | retained runtime/loading spine plus the accepted Host root-module and public root-loading-anchor projection, accepted typed no-follow dirent observation, accepted and implemented 13-row focused ASCII Host-dirent glob oracle, accepted and implemented eight-row Bazel-internal string oracle, and accepted Bazel-internal string oracle/feasibility design, Bazel v28 visible-lockfile, registry-input, pure RegistryFunction, Registry IO bridge with native local paths, focused registry-file vendor oracle, exact ASCII local-directory registry oracle, accepted default-launch collation/charset oracle, and exact host-JVM startup/reuse oracle; pinned source and the exact oracle prove BUILD/ordinary `.bzl` Latin-1 byte strings, greedy octal-only escapes, byte-counted locations, and a feasible token-local dormant carrier with a separate opt-in byte-column reporter; the typed observation retains raw names plus direct kind in one shared value, preserves duplicate name order, refines only unknown Unix entries after close, and uses reparse-aware no-follow Windows classification; the required five-packet fixture-growth checkpoint is accepted through `22de3631`, and the accepted string oracle is post-checkpoint packet four | production retry/publication activation remains gated on Host migrations, typed production roots, and reviewed byte-aware adapters for every string ingress; the separately reviewed dormant parser seam is next, followed by raw-name/pattern-lazy oracle breadth, pure Host glob owner, and transactional package-evaluation retry/suspension boundary; whichever oracle next reaches acceptance is post-checkpoint packet five and must perform the focused fixture-growth review; native Windows and lone-surrogate ordering remain activation gates; a reusable regular-or-special Host byte projection remains separate; the full Bazel-valid JVM surface disproves every bounded standalone registry-byte executor found, so the private Host registry-file owner is separately blocked unless a genuinely original-process-equivalent Slug mechanism appears; HTTP and Git PAX/GNU/link/special parity, resolved repository path state, deleted-package request ownership, repository-ignore ownership, package lookup, omitted-`module()` defaults, and discovery composition remain | `WP-5-m1-starlark-bazel-internal-string-seam` |
...
Implement the accepted exact eight-file dormant opt-in parser seam from the
Stage 5 owner plan, now that the prerequisite Bazel oracle is accepted in
`98b8b0e1`. Preserve ordinary parser, source, span, location, and formatted
rendering behavior; expose only the explicit parse-encoding entrypoint and the
separate opt-in byte-column reporting resolver. Keep zero production callers
and stop on any required compiler/value/hash/repr/source-storage edit,
whole-source transformation, mixed-ingress activation, allowlist/cap
expansion, or loading/Host/DICE/consumer adoption.
