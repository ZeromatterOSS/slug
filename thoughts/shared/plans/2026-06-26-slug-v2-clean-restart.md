...
| M7: command/ruleset breadth | **active (built-in module value)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite `run`, and the immutable canonical `@@bazel_tools` route/seven-file source owner | retain the complete embedded MODULE value before the later selected Host discovery graph | `WP-5-builtin-bazel-tools-module-value-implementation` |
...
[`WP-5-builtin-bazel-tools-module-value-implementation`](./slug-v2-subplans/current-packet.md).
