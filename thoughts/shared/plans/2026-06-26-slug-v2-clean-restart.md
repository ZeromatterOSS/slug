...
Implement only `WP-5-m1-repository-materialization-request-result`.

In `slug_bzlmod_v2`, add the accepted complete-only source-preparation carrier,
pure repository request key, exact per-workspace injected result epoch, and
per-request projection. Remove repository IO from DICE. Local success carries
no root: derive its normalized logical Host root from the exact request.
Immutable success carries source identity, generation root, and observation
instance. Keep persistent spec errors generation-independent and validate
generation-tagged transient transport/materialization errors before returning
Complete; stale or missing results return materialization Need.

Make materialization equality exact across every operational field, then prune
only at source bytes, absence, or typed errors. Prove request/path Need
propagation, lawful request hashing without hashing or ordering `RepoSpec`,
epoch duplicate/conflict/kind/workspace rejection, unused-override laziness,
workspace/spec isolation, two-repository omission, no DICE IO, transient retry,
logical-root symlink retarget, exact immutable instance selection, cumulative
path retries, byte pruning/change/restoration, and complete-only validity.
Edit only `source_preparation.rs`, its public reexports in `lib.rs`, and
`source_preparation_dice.rs`. Do not edit core/runtime/workspace/Cargo,
activate preparation, or add events, publication, fixtures, or command retry.
