...
Run only `WP-5-m1-demand-key-metadata`.

Edit only `app/slug_workspace_v2/src/path_observation.rs` and
`app/slug_bzlmod_v2/src/source_preparation.rs`. Add direct `DynKey` regressions
first, then let `PathObservationKey`, private
`RepositoryMaterializationResultKey`, and `RepositorySourceFileKey` provide
their exact key-static path demand, full request, and workspace/module
repository-source scope through `Key::provide`. Do not use evaluation data or
change compute/value/equality behavior. Stop on any runtime, Cargo, command,
fixture, oracle, DICE-core, or extra-file edit.
