...
[WP-4-5-7A-repository-rule-attribute-family-architecture](./slug-v2-subplans/current-packet.md).

Commit `3023718a0` accepts the complete direct Bazel 9.2
`tools/build_defs/repo` embedded package: exact `BUILD.repo`-to-`BUILD` plus all
eight `.bzl` siblings, derived manifest/listing/package identity, full serial
suites, two clean replays, and independent design/terminal review. Both replays
advance beyond `utils.bzl` and next reject the scalar-only repository-rule
bridge at `globals = attr.string_list_dict(mandatory = True)` in bazel_features.

Run only the docs/evidence architecture packet for the complete thirteen-kind
repository-rule attribute value family. Freeze one recursive heap-independent
call carrier, kind-directed RepoSpec coercion, ordinary/innate parity, complete
default validation, a generic recursively ordered RepoSpec publication
projection, descriptor-policy ledger, dirty-hunk isolation, caps, and proof
matrix. Preserve name-addressed top-level attribute membership equality, but
replace the built-in `remote_patches` identity special case so every nested map
order participates in Slug-native semantic cutoff/hash identity. Do not patch
only `string_list_dict`; do not change parser, `set`, repository Host methods,
`cc_common`, `cc_internal`, rules_cc, or C++ rule semantics. Bazel 9.2 remains
sole authority; clean Zabel remains peer guidance.
