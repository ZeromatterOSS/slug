...
[Design repository declaration documentation-category closure](./slug-v2-subplans/current-packet.md).

`WP-6-7A-default-exec-configured-label-dependency-implementation-r2` is
terminally accepted. The complete default-group category spans all five
...
Activate only
`WP-6-7A-repository-declaration-documentation-category-design-r1`. Audit the
complete declaration-builtin documentation family and freeze the bounded
`repository_rule(doc = ...)` loading closure exposed by authentic replay.
