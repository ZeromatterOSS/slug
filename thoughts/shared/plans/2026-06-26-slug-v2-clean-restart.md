...
Run only `WP-5-m1-root-module-composed-evaluator`.

Implement the private composed root-MODULE evaluation key below
`RootModuleFilesKey` in exactly:
...
Add the opaque `ignore_dev_dependency` projection, complete reachable-source
preflight and preparation, one inline prepared-program evaluator with
distinct-file binding isolation and repeated-call execution, aggregate
Bazel-shaped root-module semantics and module-file paths, and retained-DICE
evidence. Retire the exported per-file evaluator key/type and the public
root/includes partition without a shim. Keep the default direct-print handler;
the serial event-producer packet adds the marker and dependency afterward.
Do not edit Cargo, DICE, runtime implementation, command/server, registry or
source preparation, loading/analysis, nonroot evaluation, discovery, fixtures,
expected output, oracle harness, or any file outside the allowlist.
