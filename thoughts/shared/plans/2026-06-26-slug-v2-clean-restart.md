...
| M5: `aquery` | **active (`deps()` owner-set design accepted)** | recursive action ownership, complete Slug configuration identity, strict closure-resolved toolchain-backed FileWrite semantics, exact framed Slug FileWrite identity, declaration-ordered root-only literal text, and a bounded aspect-free `deps()` owner-set contract | `deps()` oracle/implementation and later action/expression/output breadth remain open | `WP-8-m5-filewrite-aquery-deps-owner-set-oracle-implementation` |
...
[`WP-8-m5-filewrite-aquery-deps-owner-set-oracle-implementation`](./slug-v2-subplans/current-packet.md).
