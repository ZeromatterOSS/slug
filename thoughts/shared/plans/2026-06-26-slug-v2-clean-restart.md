...
Run only
`WP-5-m1-operational-path-resolution-evidence-script-correction`.

Perform one read-only correction to the preserved semantic-error and integrated
evidence contract. Integrated `PathFileBytesError::WrongKind` cases cover only
Directory and SpecialFile; a symlink lstat must proceed to ReadLink and cannot
terminate as byte wrong-kind. Any theoretical semantic-equality construction
with `actual=Symlink` is helper-only and must not be presented as an
observation-backed resolver result.

Separate terminal assertions by owner. Every `ResolvedPathKey` error asserts
the outer namespace and original requested path in addition to its exact
physical demand or chain. Every `PathFileBytesKey` error asserts only the
stable logical path and semantic operation/cause/status/kinds, and proves that
namespace, demand object, physical path/root, route, raw target, chain,
instance, and generation are absent from its value. Preserve every other
schema, topology, demand script, split, provenance order, counter topology,
count, file boundary, and exclusion unchanged. Obtain one fresh terminal
