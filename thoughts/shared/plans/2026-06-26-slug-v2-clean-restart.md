...
[`WP-6-7A-configured-action-output-conflict-design-r1`
docs/source prerequisite](./slug-v2-subplans/current-packet.md).
...
Stage 6 records the consumer trace and missing producer-owned conflict boundary.
Design that boundary, including legitimate action sharing and all consumers,
before restoring the request correction as one complete implementation.
No new Rust is authorized by the selected docs-only packet. Then resume the
shared named/automatic group contract; loading/computed-default/C++/Java guards
remain unchanged. Do not substitute an Exec transition or output-path suffix
to conceal the missing integrity check.
Independent terminal review accepts the REPLAN and docs-only successor, not
the parked runtime implementation.
