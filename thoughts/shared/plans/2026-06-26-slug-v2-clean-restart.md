...
Run only `WP-5-m1-nonroot-package-policy-oracle-design`.

This is a read-only design packet. Reuse the accepted
`nonroot-include-composition` fixture where possible and inspect the oracle
harness plus pinned Bazel 9.2 `PackageLookupFunction`,
`IgnoredSubdirectoriesFunction`, `PackageLookupValue`, and deleted-package
option parsing.

Design the smallest retained-daemon row matrix that discriminates
`BUILD.bazel` priority and fallback, regular-file versus symlink package
markers, `--deleted_packages`, repository `.bazelignore`, `REPO.bazel`
`ignore_directories()`, exact deleted/no-BUILD errors, source-order failure,
and recovery before fragment execution. Specify exact mutations, outputs,
source anchors, fixture/file allowlist, growth, and whether an existing fixture
can express symlink transitions without a harness feature.

Stop and replan rather than add a harness feature, command transport, Rust,
registry scaffold, or nondiscriminating row. No fixture, expected artifact,
Cargo, command, server, lockfile, or Rust edit is authorized.
