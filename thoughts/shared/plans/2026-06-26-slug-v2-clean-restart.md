...
| M3: `query` | **active (`attr` executable reconstruction retry)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; the corrected 18-lane/165-pair manifest at SHA-256 `99b772e6…42300d`, five-file layout, transition allowlist, computed timeout, and package-license construction remain viable | focused Bazel 9.2 evidence accepted the six disputed rows unchanged and freezes the required source corrections; recreate the one executable representation and prove strict ownership before rendering | `WP-4-8-m3-attr-five-source-executable-reconstruction-retry` |
...
[`WP-4-8-m3-attr-five-source-executable-reconstruction-retry`](./slug-v2-subplans/current-packet.md).
