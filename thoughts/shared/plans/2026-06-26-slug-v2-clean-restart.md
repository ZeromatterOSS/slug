...
[WP-4-5-7A-builtin-bazel-tools-repo-package-catalog-implementation](./slug-v2-subplans/current-packet.md).

Commit `f747507f6` accepts the generic generated-repository `.bzl` route,
canonical definition-host final-mapping projection, and nonroot-only ordinary
extension execution. Full serial suites, CLI build, two clean replays, and
independent terminal review pass. Both replays advance through the real
rules_cc compatibility-proxy owner and next stop at missing immutable built-in
content `tools/build_defs/repo/utils.bzl`.

Run only the complete direct Bazel 9.2 `tools/build_defs/repo` embedded package
catalog packet. Import `BUILD.repo` as `BUILD` and all eight sibling `.bzl`
files verbatim with exact paths, digests, modes, derived manifest/listing, and
package-set proof. Do not patch only `utils.bzl`; do not change parser, `set`,
Host ABI, repository execution, `cc_common`, `cc_internal`, rules_cc, or C++
rule semantics. Bazel 9.2 remains sole authority; clean Zabel remains peer
