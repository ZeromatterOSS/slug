...
sequence. The first oracle attempt stopped before retained evidence because
DIRTREE is repository-context-only and cannot be naturally generated by
`module_ctx`; an ignored working draft is not accepted state. Correct the
oracle contract before resuming it. No Rust, Cargo, harness, public API, or
activation changed.

### Current packet

Design-correct only
`WP-5-m1-bazel-lockfile-v28-schema-oracle-correction`.

Preserve the exact twelve-file/no-harness boundary and 15-row cap. Narrow the
generated baseline to the four recorded-input kinds `module_ctx` can produce;
hand-inject DIRTREE into the current factor, prove stale replay and forced
replacement, and retain a separate unevaluated foreign factor. Use only the
fixture-local loopback registry plus embedded modules. Do not resume the
ignored draft until the corrected contract receives terminal rereview.
