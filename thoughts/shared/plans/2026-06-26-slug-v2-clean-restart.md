...
`WP-5-m1-path-observation-native-schema-and-unix-directory-design`.

Audit the accepted structural observation owner and every exhaustive
`PathObservationError` consumer. Freeze the smallest public schema correction
for Bazel's distinct Windows not-a-link result when lstat still reports
Symlink, including operational and semantic equality, diagnostics, downstream
resolver/source-preparation behavior, reexports, file allowlist, and tests.
Prefer one fieldless `NotALink`-like discriminator if it remains exact; do not
invent a false actual kind, collapse it into generic I/O, or fabricate
InconsistentState.

Audit the smallest exact Unix directory seam against Bazel 9.2 native
`opendir`/`readdir`/`closedir` and pinned Rust std. Bazel retries opendir EINTR,
reports opendir EIO, retains the same handle and partial names across readdir
EINTR/EIO, and ignores only closedir EINTR. Decide whether a reviewed existing
crate can provide this portably or whether a target-specific dependency/raw
ABI owner and Cargo expansion are required. Freeze handle lifetime, errno
capture, dirent-name extraction without UTF-8 loss, dot-entry filtering,
iterator/error/close precedence, platform coverage, safe wrapper, tests, and
stop conditions.

Preserve the accepted checked private Windows reparse-buffer parser contract
for NTFS, LX, mount-point, ProjFS, unknown tags, exact output normalization,
and raw errors; do not redesign it unless the schema decision requires it.
Also preserve the compact producer authority, preflight, lstat/FileBytes
semantics, and no runtime/DICE/materializer wiring.

Do not edit Rust, Cargo, fixtures, plans, or routing during the design packet;
wire runtime/DICE/materialization, or broaden repository/source preparation.
Return `REPLAN` if exact Unix parity requires vendoring platform layouts or if
the public schema correction expands beyond the observation owner and its
direct exhaustive consumers.
