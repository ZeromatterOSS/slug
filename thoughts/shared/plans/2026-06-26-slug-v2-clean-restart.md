...
| M7: command/ruleset breadth | **active (bounded run implementation retry 2)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, plus accepted client-launch/daemon-wire, endpoint, and POSIX-fixture designs | complete the first executable FileWrite `run` vertical without adding host-configuration or direct-local build actions | `WP-8-m7-filewrite-run-handoff-implementation-retry-2` |
...
[`WP-8-m7-filewrite-run-handoff-implementation-retry-2`](./slug-v2-subplans/current-packet.md).
