...
| M7: command/ruleset breadth | **partial; nonregistry REPO-file proof-cap correction active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, `cb5073e0`'s immutable action-owner row, `b7390392`'s rules_rust 0.73 evidence, `a3efa1b7`'s root MODULE-files epoch, `3d174006`'s effective-override epoch, `ae8aa35e`'s request/materialization epoch, and `12f68983`'s repository-source epoch are accepted | the sound REPO-file candidate exceeds the first corrected proof envelope before exact terminal/event/family/lifecycle discrimination is complete; ignore/preflight/closure and registry preparation remain later | correct only the measured REPO-file proof envelope, retry the same one-file implementation, then observe ignore/preflight/closure and preserve M7A -> M8 -> M7B |
...
[WP-6-7A-host-nonregistry-repo-file-observation-proof-cap-correction-2-design](./slug-v2-subplans/current-packet.md).
