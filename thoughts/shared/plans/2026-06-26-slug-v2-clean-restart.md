...
| M4: `cquery` | **active (vacuous configured `visible` accepted)** | projected label/Starlark-label output, ordered sets, Rust-native `filter`, full `some`, non-test `executables`, exported rule-class `kind`, exact `siblings`/`visible` post-analysis behavior, typed exits, and retained configured rule capability | select the next exact vertical from retained configured state | `WP-6-m4-configured-query-successor-audit-8` |
...
[`WP-6-m4-configured-query-successor-audit-8`](./slug-v2-subplans/current-packet.md).
