...
Perform a read-only correction of the retained-materializer design. Preserve
the per-`WorkspaceRuntime` owner, command-scoped provisional session, full
request identity, terminal-only promotion, append-only accepted roots,
monotonic burned instance IDs, logical Host-only Local result, exact
per-repository dirtiness, cumulative complete epochs, single private archive
capture, and exact three-file private implementation boundary.

Freeze exactly one active session lease per runtime. Concurrent
`begin_session` returns one typed internal busy error without changing accepted
state. Epoch construction, terminal acceptance, and discard all require the
current nonzero token; stale-token operations fail without mutation. A
terminal reject must restore accepted injected state and end every transaction
that can reference provisional roots before the token-checked discard drops
them. Test overlapping begin and stale accept/discard explicitly.

Freeze the supported archive order from Bazel 9.2:
structural strip/URL validation; private output root and capture-artifact
creation; exactly one caller-source capture/hash; delayed malformed-checksum
error or streamed checksum mismatch; inspection and prefix selection; then
extraction. Checksum, identity, inspection, and extraction use only the private
artifact. Add dual-failure precedence for private-root creation versus missing
source and malformed checksum versus capture failure.

Normalize and relativize every archive member before exact component-prefix
selection and destination containment, matching `StripPrefixedPath`; skip
outside-prefix entries and error when no member matches. Cover absolute
members, `foo/../bar`, sibling same-depth names, outside-prefix entries, absent
prefix, normalized collisions, and escape containment. Retain regular-file and
directory entries only in this packet and record Bazel symlink/hardlink archive
support as residual parity breadth.

Do not edit Rust, dependencies, public APIs, source-preparation types, DICE
keys, sidecars, producers, retry/publication, command activation, fixtures, or
discovery. Return `REPLAN` on a fourth file, new dependency, public expansion,
lock across I/O/await/DICE, reusable abandoned ID, caller-path reopen, or
component-count-only prefix handling.
