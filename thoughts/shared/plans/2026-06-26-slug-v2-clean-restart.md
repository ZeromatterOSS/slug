...
| M7: command/ruleset breadth | **active (selected extension-mapping owner implementation)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite run, complete Host discovered leaves, normalized/effective overrides, exact shared versions, accepted callerless selected graph in `216a0be8`, selected registry RepoSpecs in `e8ad58dd`, selected routes in `6f72baaf`, private root extension usages in `11be92b9`, and accepted selected-extension mapping design | implement the private pre-evaluation extension ID/unique-name/full-mapping owner without evaluation or consumers | WP-5-host-selected-extension-mapping-owner-implementation |
...
[WP-5-host-selected-extension-mapping-owner-implementation](./slug-v2-subplans/current-packet.md).
