...
Run only
`WP-5-m1-root-module-event-producer-design-correction`.

Perform a read-only correction for the first event producer. Determine whether
the current separate-key root-MODULE include layout can preserve the accepted
one-local-batch-per-file include rule while capture is conditional on
`CaptureEvaluationEvents` and remains outside semantic equality. Freeze the
smallest exact producer packet or stop/replan if that layout cannot preserve
the rule. Do not edit Rust, Cargo, fixtures, oracles, runtime/command/server
paths, loading/analysis evaluators, nonroot evaluation, or discovery behavior.
