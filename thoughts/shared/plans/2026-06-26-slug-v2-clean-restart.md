...
| M8: bootstrap | **active (developer graph)** | exact 33-package CLI boundary plus accepted Gates A-B; Gate C1 has 43 accepted Bazel targets covering 906 source cases; one canonical payload owns all 14 shared fixture workspaces; the cache gate selects nightly; verified actiond `v0.0.6` reached one uncached remote spawn, then failed process setup before Rust compilation | Successful live cache/RBE evidence, core host-tool owners, and self-hosting remain | `WP-10-m8-bazel-buildbuddy-cache-live-evidence-after-nightly-repair` |

### Current packet

`WP-10-m8-bazel-buildbuddy-cache-live-evidence-after-nightly-repair`.
