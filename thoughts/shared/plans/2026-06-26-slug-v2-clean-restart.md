...
[WP-6-7A-complete-noncallback-spawn-envelope-implementation-r3](./slug-v2-subplans/current-packet.md).

R1/R2 design reviews exposed and corrected the `resource_set` callback/result
boundary, context-scoped executable-to-FilesToRun association, and Bazel's
distinct top-level versus sequence-nested tool-depset lookup. Independent R3
review returned `ACCEPT`; implementation is authorized only within that frozen
default-context non-callback envelope.
