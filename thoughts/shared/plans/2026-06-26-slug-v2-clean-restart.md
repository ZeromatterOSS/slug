...
| M7: command/ruleset breadth | **active (embedded test-tools prerequisite design)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, plus the accepted POSIX executable FileWrite `run` vertical with client-only launch | admit the exact embedded `@bazel_tools//tools/test` closure and repository/DICE ownership before TestRunner semantics | `WP-4-6-8-bazel-tools-test-closure-design` |
