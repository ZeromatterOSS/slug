...
Run only
`WP-5-m1-runtime-path-observation-native-producer-final`.

Implement the accepted private outside-DICE producer in exactly:

- `app/slug_core_v2/src/runtime/path_observation.rs`;
- `app/slug_core_v2/src/runtime/mod.rs`;
- `app/slug_core_v2/Cargo.toml`; and
- `Cargo.lock`.

Use one lifetime-bound `Allocative` sorted `Arc` slice of normalized retained
materialization roots. Sort and completely preflight demands before executing
that same order; duplicates, zero/unknown instances, and an Lstat request on an
unsupported platform must perform no filesystem operation. Retained roots
authorize instance identity only: observe each demand's exact path without
containment, canonicalization, or rewriting.

Perform ReadLink, FileBytes, and DirectoryEntries first. Only their accepted
candidate failures run an auxiliary no-follow lstat, with the frozen
Missing/WrongKind/original-error race tables. Preserve successful special-file
reads, typed NotALink, raw same-handle Unix `opendir`/`readdir`/`closedir`
ownership and EINTR/EIO/close precedence, and complete-result publication.

On Windows, preserve exact Bazel 9.2 lstat kind order, `0555`/`0755`
permissions, separate OpenJDK mtime and native ctime formulas, literal
`asLongPath` behavior, checked native reparse parsing, and explicit kernel32
linkage. Implement raw `FindFirstFileExW`/`FindNextFileW`/`FindClose`
enumeration so error capture precedes ignored close, raw 2 is an empty
directory, raw 18 is EOF, partial names are discarded on failure, and owned
WTF-16 names survive. Bounds-check every returned reparse byte count before
forming a slice.

Add the accepted pure/scripted operation, authority, allocation, Unix-owner,
Windows ABI/parser/time/path/directory, and real create/edit/delete/recreate,
directory, symlink, and Host/materialization lifecycle evidence. Portable tests
derive absolute paths from temporary directories. Add only target-Unix
`nix = { workspace = true }`; add no Windows dependency and no public schema,
DICE, materializer, retry, publication, fixture, or consumer wiring.

Terminal acceptance requires a native or correctly provisioned Windows
`cargo test -p slug_core_v2 --no-run` that links the test binary against
kernel32; cross-target `cargo check` alone is insufficient. Return `REPLAN` if
that gate is unavailable, the copied pinned Windows ABI diverges, or the
implementation needs a fifth file or excluded owner.
