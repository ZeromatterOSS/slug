...
| M3: `query` | **active (`attr` license-default evidence)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; the 18-lane/165-pair structure, five-file layout, transition allowlist, and computed timeout are retained | Bazel 9.2 accepts `licenses([notice])` but did not render the manifest's package-derived notice license; pin exact source and disposable-oracle behavior before choosing a one-row/family correction or source construction | `WP-4-8-m3-attr-license-default-source-evidence` |
...
[`WP-4-8-m3-attr-license-default-source-evidence`](./slug-v2-subplans/current-packet.md).
