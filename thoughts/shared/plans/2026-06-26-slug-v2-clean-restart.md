...
| M8: bootstrap | **active (developer graph)** | exact 33-package CLI boundary plus accepted Gates A-B; Gate C1 has 43 accepted Bazel targets; repaired live prime evidence reaches `BEP_READY` and its execution-stage probe is accepted offline | Deliver one execution-stage result, then resume output/cache/RBE proof and the full 43-test gate | `WP-10-m8-bazel-buildbuddy-build-cache-prime-execution-stage-probe-transported-live-evidence` |
