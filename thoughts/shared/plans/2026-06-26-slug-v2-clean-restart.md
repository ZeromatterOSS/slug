...
| M3: `query` | **active (`attr` isolated-oracle design)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; pinned Bazel 9.2 and the reviewed 18-lane matrix close the finite observable attr schema; a stopped generation run proves the existing shared fixture cannot admit required constructors without breaking protected Slug rows | design a complete isolated Bazel-only attr fixture with unique atom pairs and no protected Slug consumer; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-isolated-observable-candidate-oracle-design` |
...
[`WP-4-8-m3-attr-isolated-observable-candidate-oracle-design`](./slug-v2-subplans/current-packet.md).
