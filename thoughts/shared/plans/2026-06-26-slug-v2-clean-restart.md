...
Add only `WP-5-m1-root-patch-special-file-two-phase-oracle`.

Extend the existing retained-daemon
`tests/v2_oracle/fixtures/module-source-preparation` fixture by two rows before
the local-override transition. First, route the root patch label through a
checked symlink to `/dev/null`; Bazel 9.2 must accept the special-file target,
treat its empty bytes as a no-op, and retain `patch_base@1.0.0`. Second, select
a context-invalid first patch and absent second patch; Bazel must report the
second path as missing before attempting the first patch.

Modify only `fixture.toml`, `expected/oracle.json`, the checked special symlink
and invalid patch asset, and deterministic `patch_base` registry metadata.
Preserve all nine existing rows exactly, update the later local-override
mutation from the new two-patch state, record exact net file/line growth and
reuse, and run generation plus two fresh Bazel 9.2 replays and the focused
oracle harness. Add no Rust, Cargo, shared harness, network, or unrelated
fixture breadth.
