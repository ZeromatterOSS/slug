...
| M4: `cquery` | **partial (forward composition accepted)** | projected label/Starlark-label/label-kind/graph output, direct retained-state functions, exact toolchain plus delegation graph evidence, one Need-aware analysis key/result, structural/null Target/Exec node identity, classified ordered edges, fixture-proven root topology, semantic error publication, arbitrary-depth single-root `deps --noimplicit_deps`, and exact `executables(deps(...))` composition over the sole retained graph | select the next exact noimplicit forward consumer without broadening general nested expressions or synthesizing stopped topology | `WP-6-m2t-configured-query-forward-successor-audit` |
...
[`WP-6-m2t-configured-query-forward-successor-audit`](./slug-v2-subplans/current-packet.md).
