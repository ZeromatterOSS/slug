...
[Bounded Unix `repository_ctx.which` accepted; exact built-in C++ configure catalog audit selected](./slug-v2-subplans/current-packet.md).

The bounded Unix `repository_ctx.which` implementation returns `ACCEPT` after
one correction rereview at 240/498/738 gross production/proof/total Rust
additions. It composes the existing verified repository-environment frontier
with Host resolved-path owners, returns the logical first executable candidate,
and keeps tracked PATH/metadata/symlink invalidation as explicit Slug-native
integrity. No DICE key, filesystem/process-environment read, retained candidate
cache or rules_shell/toolchain special case was added.

The full loading suite passes, the V2 CLI rebuilds, and the bounded-PATH
authenticated rules_rust replay clears rules_shell's generic Unix lookup. It
next stops at missing exact built-in source
`@@bazel_tools//tools/cpp:lib_cc_configure.bzl` while evaluating apple_support's
local C++ configuration. Run only docs-first
`WP-5-7A-bazel-tools-lib-cc-configure-catalog-audit`; any admitted content must
be copied verbatim from pinned Bazel 9.2, never reconstructed or specialized
for apple_support.
