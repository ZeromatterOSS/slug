...
Design only
`WP-5-m1-loading-host-package-boundary-projection-design`.

Freeze a public opaque Host boundary projection that separately preserves the
selected package root, ordinary no-package, deleted-package continue,
ignored-directory stop, actual-subpackage stop, incorrect nested-repository
stop, and typed operational error/Need propagation. Keep recursive glob
traversal, multi-segment/`**`, parser activation, evaluator retry, native
