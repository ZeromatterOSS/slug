...
Run only
`WP-5-m1-runtime-path-observation-neutral-kernel`.

Add new private `app/slug_core_v2/src/runtime/path_observation.rs` and its
unconditional private declaration in `runtime/mod.rs`; edit no other file. Add
no native cfg, dependency, public export, caller, DICE, materializer,
retry/publication, fixture, or consumer wiring. The module is deliberately
dormant but compiled on ordinary and Windows targets.

Implement the lifetime-bound `Allocative` sorted `Arc` retained-root authority
without Clone/Dupe/map/interner. Add a private adapter trait with
`supports_lstat`, direct Lstat, and primary ReadLink/FileBytes/DirectoryEntries
methods. Primary failure is only `Refine(original)` or `Final(original)`; it
cannot directly fabricate Missing.

The batch kernel collects and sorts demands, rejects the first duplicate,
validates zero/unknown materialization instances in sorted order, and
preflights only unsupported Lstat before any operation call. Unsupported
non-Lstat remains an adapter-provided final operation error. Execute the exact
sorted demands only after complete preflight. Root paths authorize instances
only; pass every exact demanded path unchanged. Operation-level errors remain
inside the complete epoch and do not abort later demands.

Apply the accepted auxiliary no-follow lstat tables only to `Refine`: ReadLink
maps Missing and truthful non-Symlink WrongKind while preserving the original
for Symlink/aux-I/O; FileBytes maps Missing and Directory WrongKind while
preserving other kinds/aux-I/O; DirectoryEntries maps Missing and non-Directory
WrongKind while preserving Directory/aux-I/O. Present and Final never run the
auxiliary operation, and the kernel never invents InconsistentState.

Use only a test scripted adapter to exhaust compact root allocation,
zero-operation invalid preflight, exact sorted execution, Host/materialization
identity and no-rewrite behavior, error continuation, primary-before-auxiliary
ordering, every node-kind/race/refinement/no-aux row, typed NotALink, special
file bytes, and matching epoch variants. Derive all paths from temporary
directories.

Validate focused and full core tests, host all-target compilation, Windows GNU
all-target compilation, formatting, diff, archive status, exact two-file scope,
and forbidden references. Return `REPLAN` if either accepted native adapter
would require changing this neutral contract, any native type/dependency is
needed now, or the module must become public or callable.
