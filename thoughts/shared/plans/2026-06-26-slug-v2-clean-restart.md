...
[WP-4-5-7A-innate-repository-rule-owner-certificate-architecture](./slug-v2-subplans/current-packet.md).
...
### M7 canonical Host-capability implementation REPLAN; innate owner architecture active (2026-08-29)
...
Independent architecture review returns `REPLAN`: removing the guard would
load the wrong file and callable type. Run only
`WP-4-5-7A-innate-repository-rule-owner-certificate-architecture`. Freeze a
generic singular-owner projection and distinct loading certificate that
resolves the actual label in the owning module mapping, authenticates the
exported repository rule and feeds retained calls into the existing namespace,
RepoSpec and validation owners. No winsdk special case, MODULE evaluator
change, or repository Host capability is authorized. Preserve both dirty
candidates recoverably. Bazel 9.2 remains sole semantic authority; clean Zabel
