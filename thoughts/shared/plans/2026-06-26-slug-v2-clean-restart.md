...
| M1: one semantic spine | **active** | Host loading observations, dormant public typed root-loading/root-analysis boundaries, and the accepted typed query-root design; full evidence is in the Stage 5 owner plan | typed command-root implementation and later activation; external mapping, native Windows glob ordering, registry transports, and discovery composition remain | implement the dormant typed query command root |
...
`WP-5-m1-query-typed-command-root`.
