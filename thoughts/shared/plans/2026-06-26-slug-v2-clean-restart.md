...
Design only `WP-5-m1-loading-event-producer-globals-owner-correction`.

The first implementation proved that `loading_globals()` omits `print`; a
copied-value overlay then caused a downstream frozen-value lifetime SIGSEGV.
Define one corrected retry that adds
`app/slug_loading_v2/src/package.rs` to the prior exact four-file allowlist,
constructs standard loading globals plus only `LibraryExtension::Print` in one
owning `Globals` heap, and removes every copied-value/process-static overlay.
Preserve the otherwise green local-batch implementation and evidence. Do not
change key values/equality, typed Need, runtime, commands, publication, or any
other Starlark extension.
