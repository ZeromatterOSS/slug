...
Against pinned Bazel 9.2 and the live Rust/runtime types, freeze:

- primary-operation-first ReadLink/FileBytes/DirectoryEntries failure tables,
  auxiliary no-follow lstat timing, race precedence, and exact
  Missing/WrongKind/I/O results;
- deterministic sorted execution after complete duplicate/instance/platform
  preflight, plus normalized compact retained-root ownership and allocation;
- exact Windows writable permissions, distinct mtime/ctime overflow formulas,
  slash replacement and literal long-path prefix rules, kernel32 linkage, and
  a bounds check before any reparse-buffer slice;
- complete host-pure Windows layout/error/normalization tests, portable path
  construction, Unix open/read/close scripted evidence, and create/edit/delete,
  directory, symlink-retarget, inside/escaped materialization lifecycle rows.

Preserve the accepted fieldless NotALink schema, raw same-handle Unix owner,
checked native Windows parser, private four-file boundary, and exclusions.
Do not edit Rust, Cargo, fixtures, plans, or routing during this design packet.
Return `REPLAN` if exact correction needs a public schema, another dependency,
or runtime/DICE/materializer wiring.
