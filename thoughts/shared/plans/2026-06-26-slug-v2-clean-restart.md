...
[WP-4-5-7A-module-extension-native-existing-rules-family](./slug-v2-subplans/current-packet.md).

Commit `0c3a172ed` accepts the complete thirteen-kind repository-rule attribute
family, ordinary/innate convergence, recursive ordered-map publication
identity, full owner suites, two fresh rules_rust replays, and independent
terminal review. Both replays advance beyond the former
`attr.string_list_dict` rejection and now stop at missing
`native.existing_rule` in verbatim `tools/build_defs/repo/utils.bzl:318`.

Implement the complete module-extension no-op family together:
`native.existing_rule(name)` returns `None` and `native.existing_rules()`
returns an immutable empty dictionary only in authenticated module-extension
invocation contexts. BUILD/finalizer snapshots remain unsupported and fail
closed. Do not change parser or `set` semantics, repository Host methods,
`cc_common`, `cc_internal`, rules_cc, or C++ rules/actions. Bazel 9.2 remains
sole authority; clean Zabel has no corresponding implementation and remains
