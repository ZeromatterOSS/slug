...
Design only `WP-5-m1-runtime-materialization-observation-retry-design`.

Audit the retained build/query, one-shot/daemon, repository materialization,
and sparse-observation paths. Freeze the smallest shared command owner that
can satisfy Host and exact materialization-instance `PathNeed` batches outside
DICE, retry the same semantic request with a complete replacement epoch, and
rematerialize a previously successful immutable repository when its owned
source inputs change.

Name the exact request/epoch state, retry bounds, source-input dependency,
materialization-instance lifecycle, event/publication boundary, error
precedence, implementation allowlist, and retained-daemon evidence before any
Rust edit. Preserve failure-only `RepositoryMaterializationGeneration` unless
the audit proves a distinct successful-refetch owner is required. Do not
activate the callerless preparation key, add discovery/package/loading/
analysis/query breadth, or perform filesystem IO inside DICE.
