...
`WP-5-m1-direct-local-nonroot-source-preparation-handoff-design`.
...
UTF-16 identity. The next packet is read-only and asks whether the already
accepted direct-local route can supply one closed nonroot source-preparation
handoff without pulling in the absent full discovery owner graph.
