...
| M7: command/ruleset breadth | **partial; first post-config aspect-definition packet selected** | selected-registry source/root-load and exact rules_rust root realization are accepted through `2f373248`; commits through `68e458b4` accept bounded keyword-only syntax, exact live `.bzl` `struct` placement/operations, provider/rule docs and typed string/bool/list definitions with structural repeatability; `a8e18278` authenticates the next source-order stop | expose/freeze the first fixed `aspect` constructor subset, advancing the live expression to missing `Label`; aspect application and bool/list targets/analysis remain deferred | run only `WP-4-7A-bazel-aspect-definition-loading`; preserve M7A -> M8 -> M7B |
...
Expose the first missing `.bzl` `aspect` constructor and freeze its admitted
fixed definition subset with implementation lifetime, ordered propagation
attributes, canonical toolchain requirement and producer export identity. The
live rules_rust expression advances to missing `Label`; stop before adding it
or any aspect attachment, selection, propagation or analysis.
