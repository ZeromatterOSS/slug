...
[`WP-4-5-8-7A-native-genrule-imported-declaration-audit-r1`
...
provenance at 119/241/360 gross production/proof/total Rust additions. The
rebuilt replay reaches selected rules_java
`default_java_toolchain.bzl:210` and stops at missing `native.genrule`.
Because native member lookup precedes keyword-expression evaluation, that
failure is not positive evidence for either Label argument.

The docs/oracle audit uses a pinned Bazel 9.2 query over the authenticated
rules_java closure plus an isolated two-package discriminator. It accepts,
pending independent review, only imported `.bzl` calls to
`native.genrule` using required `name`, `cmd`, and nonempty `outs` plus optional
`srcs`, `toolchains`, and `tags`, with omitted `executable=false`. Raw strings
resolve in the BUILD package, typed Labels
preserve their lexical identity, and outputs become generated files owned by
the genrule. Bare BUILD `genrule`, complete query projection, configured
toolchains/providers, command expansion, actions and execution remain
unsupported/deferred. The six-file 170/260/430 successor and its fail-closed
stops are frozen in `current-packet.md`; the early empty-output rejection is
Slug-native because Bazel reports it only during configured genrule creation.
