...
| M3: `query` | **active (`attr` total-schema source ledger)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; leaf attr formatting is source-closed; candidate order/multiplicity are proven unobservable | close the complete inherited/hidden/computed `RuleClass` schema and loading-value sources before observable-candidate oracle design; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-total-ruleclass-schema-source-ledger-design` |
...
[`WP-4-8-m3-attr-total-ruleclass-schema-source-ledger-design`](./slug-v2-subplans/current-packet.md).
