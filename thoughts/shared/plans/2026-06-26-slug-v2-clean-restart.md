...
| M1: one semantic spine | **partial; root-module anchor loading-consumer design next** | Host loading observations, typed command/event ownership, direct local-override external query loading, the query-only unsupported-cycle boundary in `ea2019f8`, direct-local exported-source build completion in `42f4a64b`, the first private core repository source-observation consumer in `53152727`, the pinned in-flight loading/source-lock oracle in `2ffad088`, the private request-revision/source-certificate vertical in `207fe438`, the sole-root native publication bridge in `f0849151`, the exact callerless observed-path/Host-file frontier in `308b409a`, the accepted observed root REPO, repository-ignore, and package-marker frontiers in `f2c7305f`, `43adf74b`, and `0875728b`, the finite private root-MODULE include-progress terminal in `53833591`, the complete callerless root-module frontier in `2640d1c0`, and the doc-hidden Bzlmod-to-loading anchor carrier in `c6e61d60` | loading does not consume the anchor certificate, while its package-source, parse, load, and glob dependencies lack a complete shared frontier; directory/glob unions, repositories/materialization, external test-base/tools closure, JVM-dependent registry directory transport, and native-Windows byte-glob ordering remain unsupported or `REPLAN` | design the smallest complete loading-side consumer or its uniquely required prerequisite |
...
[WP-2A-m1-root-module-anchor-frontier-loading-consumer-design](./slug-v2-subplans/current-packet.md).
