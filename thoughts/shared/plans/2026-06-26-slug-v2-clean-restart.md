...
| M1: one semantic spine | **active** | Host loading observations, dormant typed root-loading/root-analysis boundaries, accepted preactivation Host closure gate, opaque accepted-command publication, source-aware command events, typed one-shot/daemon query and build activation, and accepted direct local-override external source-file, `filegroup`, and direct `alias` queries; full evidence is in the Stage 5 owner plan | external native-rule/query breadth, native Windows glob ordering, registry transports, and discovery composition remain | implement one same-package external native `config_setting` query |
...
`WP-5-m1-external-repository-config-setting-query`.
