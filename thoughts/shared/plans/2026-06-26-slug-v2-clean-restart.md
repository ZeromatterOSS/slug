...
| M1: one semantic spine | **active** | Host loading observations, dormant typed root-loading/root-analysis/build/query command boundaries, accepted preactivation Host closure gate, and accepted opaque-envelope design; full evidence is in the Stage 5 owner plan | opaque terminal envelope, source-aware event publication, then query-first activation; external mapping, native Windows glob ordering, registry transports, and discovery composition remain | implement the private opaque terminal envelope |
