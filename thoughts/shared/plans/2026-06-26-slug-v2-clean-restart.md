...
| M3: `query` | **active (`attr` two-package oracle design)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; pinned Bazel 9.2 and the corrected 18-lane/165-pair matrix close the finite observable attr schema; generation proved one package cannot expose both positive package deprecation and a same-schema null control | design the smallest isolated positive-default/baseline package layout and recalculate fixture caps; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-two-package-observable-candidate-oracle-design` |
...
[`WP-4-8-m3-attr-two-package-observable-candidate-oracle-design`](./slug-v2-subplans/current-packet.md).
