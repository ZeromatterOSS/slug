...
| M1: one semantic spine | **active** | Host loading observations, dormant typed root-loading/root-analysis/build/query command boundaries, accepted preactivation Host closure gate, opaque accepted-command publication, source-aware command events, and accepted query-first activation design; full evidence is in the Stage 5 owner plan | query-first activation; external mapping, native Windows glob ordering, registry transports, and discovery composition remain | implement query-first activation |
...
`WP-5-m1-query-first-activation`.
