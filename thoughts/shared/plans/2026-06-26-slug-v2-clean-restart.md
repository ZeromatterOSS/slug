...
Preserve all 11 existing records byte-for-byte. Generate locally with pinned
Bazel 9.2, replay twice independently, require exactly two new files and fewer
than 800 net newline-counted lines, and run the full oracle harness plus source,
manifest, provenance, symlink, scope, diff, credential, and archive checks.
Stop on old-row drift, unstable source order or print count, a different marker
digest, network input, a harness feature, or any edit outside the four-file
allowlist. No Rust, Cargo, command, server, lockfile, or registry scaffold edit
is authorized.
