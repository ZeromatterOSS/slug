...
| M7: command/ruleset breadth | **partial; repository context, symbolic-macro lifecycle, subrule loading, FDO command configuration, selected-toolchain context, configured hidden dependencies, direct subrule calls, the first generic fragment category, dense retained depsets, configured action environments, scalar/vector Args, param-file/write, common non-callback Spawn, typed FilesToRun/runfiles/complete DefaultInfo, symlink declaration, complete runfiles-package ownership, four runfiles-support actions, FilesToRun Spawn expansion, and Bzlmod declaration-call signatures accepted** | accepted M7A loading/repository closure through `c83e70f0f`; symbolic macros/providers and configured namespaces in `e34cfdc7a`/`541fcfaf2`; corrected subrule architecture/loading in `4900ce46b`/`965cfde5e`; lawful FDO command/DICE producer in `4425d3bfb`; generic selected-toolchain context in `ebd19e3b1`; configured hidden dependencies in `2bf86bfa8`; direct invocation/value materialization in `da6865a3b`; real starlark-rust `set` in `cb71a302d`; generic fragment projection in `683538254`; dense depset/action-input owner in `7b0db03e1`; configured-action-environment owner in `71d34affa`; scalar generic actions in `78b94789c`; vector Args and parameter files in `a01a23fe7`; complete default-context non-callback Spawn envelope in `bfe6f2690`; terminally accepted typed FilesToRun provider core in `8e7234b82`; typed runfiles and complete DefaultInfo in `f346c209a`; complete loading/package metadata in `80a6bfd3a`; configured transitive-package closure in `2483dd7e2`; four support actions in `f46a009a0`; complete FilesToRun Spawn expansion in `21db5d7b8`; exact Bzlmod declaration-call signatures in `817d017b6` | Bzlmod declaration selector/producer identity, other standard-provider breadth, named exec groups/callbacks, and execution projection remain | implement the complete assigned-global/private/alias/reexport identity slice for Bzlmod declaration selectors, then replay the bootstrap frontier |
...
[Bzlmod declaration selection and producer-identity parity](./slug-v2-subplans/current-packet.md).

Commit `817d017b6` terminally accepts exact first-parameter call binding across
`repository_rule`, `module_extension`, and `tag_class`. The rebuilt real BCR
replay passes the prior positional declaration failure and now stops when the
repository-effect owner tries to reacquire rules_cc's private
`_compatibility_proxy_repo_rule` through public-only lookup.

The active `WP-6-7A-bzlmod-declaration-selection-identity-parity-r1` packet
closes the whole selector category rather than special-casing that name. One
compact starlark-rust assigned-global capability distinguishes private/local
assignments and assignment reexports from raw load bindings for every
`use_extension` load/reacquisition path. Internal repository-rule calls retain
their complete first-export producer projection at either visibility;
`use_repo_rule` remains public-only but accepts aliases/reexports while keeping
the underlying producer label/name. No parser, set, rule body, DICE key,
action, `cc_common`, `cc_internal`, rules_cc or C++ branch is admitted. Bazel
9.2 is semantic authority; Zabel is peer producer-identity/compactness guidance
only.
