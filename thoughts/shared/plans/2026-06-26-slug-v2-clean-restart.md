...
| M7: command/ruleset breadth | **partial; nonregistry REPO-file retry-2 active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, `cb5073e0`'s immutable action-owner row, `b7390392`'s rules_rust 0.73 evidence, `a3efa1b7`'s root MODULE-files epoch, `3d174006`'s effective-override epoch, `ae8aa35e`'s request/materialization epoch, and `12f68983`'s repository-source epoch are accepted | the accepted second proof-cap correction preserves the sound one-file owner while exact terminal/event/family/lifecycle discrimination is completed; ignore/preflight/closure and registry preparation remain later | finish the bounded REPO-file retry-2, then observe ignore/preflight/closure and preserve M7A -> M8 -> M7B |
...
[WP-6-7A-host-nonregistry-repository-ignore-observation-proof-cap-correction-design](./slug-v2-subplans/current-packet.md).
