...
[Built-in public-label load-routing audit accepted; bounded Stage 4
implementation selected](./slug-v2-subplans/current-packet.md).

`WP-4-5-7A-builtin-external-bzl-load-routing-audit` returns `ACCEPT`. Pinned
Bazel 9.2 compiles the importer before requesting its full repository mapping,
then resolves source-ordered load labels and retains the mapping/load DAG.
Slug's existing canonical built-in route already owns the complete graph-
derived mapping, including `rules_cc -> rules_cc+`, plus exact catalog source
identity, recursive external-Bzl loading and manifest lifetime.

Run only
`WP-4-5-7A-builtin-external-bzl-load-routing-implementation-r1`. After exact
built-in source decode/parse, promote only the bounded shape of exactly one
nonempty apparent-repository load (`@name//...`, excluding `@@...` and `@//...`)
to the existing canonical `bazel_tools` route as invocation scratch. All other
built-in shapes remain unchanged and multi-load public-label prevalidation
stays deferred. Add no Stage 5 edit, key, retained type, fixture, root-route
semantic change or rules_cc/apple_support/C++/toolchain branch. The two-file
allowlist, ordering, lifecycle proof, caps and replay stop are frozen.
