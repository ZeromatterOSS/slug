...
| M4: `cquery` | **active (Slug-native display)** | bounded one-root `str(target.label)` success/missing/recovery is accepted in `135b0567`; complete structural identity is now available | admit the public default/`label` formatter and wire contract using the Slug projection while preserving the existing Starlark bytes | `WP-6-m4-root-cquery-label-slug-projection-design` |
...
[`WP-6-m4-root-cquery-label-slug-projection-design`](./slug-v2-subplans/current-packet.md).
