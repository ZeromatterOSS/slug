...
| M7: command/ruleset breadth | **active (root extension override semantic correction implementation)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite run, complete Host discovered leaves, normalized/effective overrides, exact shared versions, accepted callerless selected graph in `216a0be8`, selected registry RepoSpecs in `e8ad58dd`, selected routes in `6f72baaf`, and private root extension usages in `11be92b9` | restore pinned root global ignore-dev and override finalization validation before resuming two-phase selected extension mappings | WP-5-root-extension-override-semantic-correction-implementation |
...
[WP-5-root-extension-override-semantic-correction-implementation](./slug-v2-subplans/current-packet.md).
