...
Add new `app/slug_workspace_v2/src/path_resolution.rs` and only the six
operational public reexports in `app/slug_workspace_v2/src/lib.rs`. Implement
the exact `ResolvedPathKey`, outer-context operational errors, iterative
parent/target frame machine, and every pure plus cumulative L/R A/B case named
by the accepted split, including the explicit direct-link, escape/clamp,
target-parent, and SpecialFile scripts.

Do not add `PathFileBytes`, `PathFileBytesError`, `PathFileBytesKey`, any
FileBytes demand, semantic equality, selector/projection/counter test key, or
consumer. Run full workspace tests/doctests, format, diff, exact two-file
allowlist, and archive guards; require independent pinned-source and
architecture acceptance before one implementation commit.
