...
Implement only `WP-5-m1-host-registry-io-bridge`.

Edit exactly `app/slug_bzlmod_v2/src/registry_dice.rs` within the accepted
850-addition/120-deletion cap. Factor the private closed execution plan and
add dormant crate-private Host remote/local entrypoints with exact typed
errors and generation ordering. Preserve every public legacy item, caller,
URL/error/dependency order, output, and legacy Off behavior byte-for-byte.
Add only inline focused evidence and run the accepted serial legacy,
full-crate, downstream, cross-target, archive, scope, public-API, credential,
and forbidden-edge gates. Do not add a second file, dependency, public API,
production key, Host file owner/consumer, direct filesystem/path Need, vendor
or mirror claim, or activation.
