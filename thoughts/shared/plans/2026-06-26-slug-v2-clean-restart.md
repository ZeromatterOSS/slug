...
| M1: one semantic spine | **active** | Host loading observations, dormant typed root-loading/root-analysis/build/query command boundaries, and accepted preactivation Host closure gate; full evidence is in the Stage 5 owner plan | opaque terminal output envelope and later activation; external mapping, native Windows glob ordering, registry transports, and discovery composition remain | design the private opaque terminal envelope |
...
`WP-5-m1-private-opaque-terminal-envelope-design`.
