...
| M7: command/ruleset breadth | **active (root extension-usage owner implementation)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite run, complete Host discovered leaves, normalized/effective overrides, exact shared versions, accepted callerless selected graph in `216a0be8`, selected registry RepoSpecs in `e8ad58dd`, selected routes in `6f72baaf`, and accepted root-usage design plus pinned eight-file Bazel 9.2 oracle | implement the private root MODULE extension-usage leaf without selected mapping or consumer activation | WP-5-root-extension-usage-semantic-owner-implementation |
...
[WP-5-root-extension-usage-semantic-owner-implementation](./slug-v2-subplans/current-packet.md).
