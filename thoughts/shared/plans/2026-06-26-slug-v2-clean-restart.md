...
Implement only `WP-5-m1-registry-remote-bridge`.

Edit exactly `app/slug_bzlmod_v2/src/registry_dice.rs`. Extract the accepted
crate-private policy-input-only remote bridge and exact remote-only error;
leave every public type/export, legacy key/value/result, installed IO owner,
generation order, and caller unchanged. Run the full unchanged registry suite,
the exact bridge/error/call-site scans, full bzlmod and downstream validation,
and cross-target no-run gates before terminal review.
