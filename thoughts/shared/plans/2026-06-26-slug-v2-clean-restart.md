...
Design only `WP-5-m1-immutable-materialization-instance-authority-design`.

Audit the live immutable `RepositoryIoOutcome` and
`RepositoryMaterialization` path, retained temporary-root ownership, existing
`PathObservationInstanceId` authority, DICE equality, and every caller. Freeze
the smallest serial contract that gives each retained immutable
materialization root a unique operational observation instance before the
immutable read migration.

The request-scoped `RepositoryMaterializationGeneration` is retry input, not
instance authority. Preserve semantic materialization equality over canonical
repository, RepoSpec, and source identity while excluding operational
generation roots and instances. Do not edit Rust, tests, Cargo, fixtures,
oracles, runtime producers, retry/publication, discovery, package lookup, or
loading/analysis/query consumers in this design packet.
