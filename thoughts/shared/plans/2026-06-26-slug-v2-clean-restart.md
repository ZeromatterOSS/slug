...
| M7: command/ruleset breadth | **partial; first post-config aspect-definition packet selected** | selected-registry source/root-load and exact rules_rust root realization are accepted through `2f373248`; commits through `68e458b4` accept bounded keyword-only syntax, exact live `.bzl` `struct` placement/operations, provider/rule docs and typed string/bool/list definitions with structural repeatability; `a8e18278` authenticates the next source-order stop | load/freeze the first live `rust_analyzer_aspect` definition; aspect application and bool/list targets/analysis remain deferred | run only `WP-4-7A-bazel-aspect-definition-loading`; preserve M7A -> M8 -> M7B |
...
[WP-4-7A-bazel-aspect-definition-loading](./slug-v2-subplans/current-packet.md).

Load, bind, export, freeze and recursively import the first live rules_rust
`rust_analyzer_aspect` declaration. Retain implementation lifetime, ordered
propagation attributes, the canonical toolchain requirement and producer export
identity, but stop before aspect attachment, selection, propagation or
analysis.
...
`WP-4-7A-bazel-aspect-definition-loading`, retaining the declaration and first
producer export identity but no application semantics. Pinned Zabel
