...
| M1: one semantic spine | **partial; request-revision design active after the accepted focused oracle** | Host loading observations, typed command/event ownership, direct local-override external query loading, the query-only unsupported-cycle boundary in `ea2019f8`, direct-local exported-source build completion in `42f4a64b`, the first private core repository source-observation consumer in `53152727`, and the pinned in-flight loading/source-lock oracle in `2ffad088`; full evidence is in the owner plans | production still depends on coarse whole-workspace snapshots and lacks request-private source certificates, final reobservation, atomic revision acceptance, and overlapping-request proof; external test-base/tools closure, JVM-dependent registry directory transport, and native-Windows byte-glob ordering remain unsupported or `REPLAN` rather than M1 blockers | design, then implement, the smallest private root-host request-revision/source-certificate vertical |
...
[WP-2-m1-request-revision-source-certificate-design](./slug-v2-subplans/current-packet.md).
