...
| M3: `query` | **active (`attr` atomic manifest design)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; pinned Bazel 9.2 closes the corrected 18-lane/165-pair families and reviewed five-file positive-default/external-baseline layout | freeze the complete stable-ID attr/regex/yes/no/support manifest before generation; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-atomic-discriminator-manifest-design` |
...
[`WP-4-8-m3-attr-atomic-discriminator-manifest-design`](./slug-v2-subplans/current-packet.md).
