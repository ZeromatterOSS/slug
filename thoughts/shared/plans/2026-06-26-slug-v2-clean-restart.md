...
[WP-4-5-7A-configured-toolchain-selection-architecture](./slug-v2-subplans/current-packet.md).

### Command registration overlays accepted; configured selection architecture active (2026-08-28)

Commit `e1d0723ea` completes category 3. Command toolchain and execution-
platform rows now use one configuration-keyed loading family over the existing
contextual target-pattern/package walker, including signed removal and
reinsertion. Configured analysis merges command results before unchanged
positive MODULE results, preserves all four input frontiers and retains only
canonical immutable label slices.

Activate only the zero-Rust
`WP-4-5-7A-configured-toolchain-selection-architecture` review. Freeze the
complete provider-independent category-4 owner/key/value and algorithm for the
target platform, configured aliases, target settings, target/exec constraints,
target-to-exec policy, mandatory/optional type identity, order and derived exec
configuration. Preserve the current marker payload only as an explicitly
temporary post-selection bridge; providers and implementation analysis remain
categories 5 and 6.
