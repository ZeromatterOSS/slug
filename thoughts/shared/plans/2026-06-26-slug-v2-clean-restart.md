...
[Implement the shared root repository load-route publication owner](./slug-v2-subplans/current-packet.md).

The shared route candidate is terminally accepted and awaits its integration
commit. One loading-owned adapter now serves root BUILD direct `.bzl` loads,
exported-source build and external query, including recursive unimported
sibling repositories from the same generated owner. The authentic rules_rust
replay clears that boundary and next reaches the generic `repository_rule`
attribute schema rejecting `build_file` in verbatim `@bazel_tools`. After
integration, select only a docs-first complete repository-rule
declaration/schema category audit; do not add a consumer, ruleset, C++, parser
or single-attribute special case.
