...
| M1: one semantic spine | **active** | Host loading observations plus dormant public typed root-loading and root-analysis boundaries; full evidence is in the Stage 5 owner plan | typed command roots and later activation; external mapping, native Windows glob ordering, registry transports, and discovery composition remain | design the typed query command root |
...
`WP-5-m1-query-typed-command-root-design`.
