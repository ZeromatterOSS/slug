...
| M1: one semantic spine | partial | retained runtime/loading spine plus the accepted Host root-module and public root-loading-anchor projection, accepted typed no-follow dirent observation, accepted and implemented 13-row focused ASCII Host-dirent glob oracle, accepted and implemented eight-row Bazel-internal string oracle, accepted Bazel-internal string oracle/feasibility design, accepted dormant Bazel-internal string parser seam in `c8f13ee9`, accepted and implemented four-row Linux raw-name pattern-lazy oracle in `03684d84` with hygiene correction `e2cc891d`, accepted corrected private Host glob segment-candidate owner design, and accepted simple segment matcher oracle design, Bazel v28 visible-lockfile, registry-input, pure RegistryFunction, Registry IO bridge with native local paths, focused registry-file vendor oracle, exact ASCII local-directory registry oracle, accepted default-launch collation/charset oracle, and exact host-JVM startup/reuse oracle; pinned source, the exact oracles, and the dormant token-local implementation prove BUILD/ordinary `.bzl` Latin-1 byte carriers, greedy octal-only escapes, byte-counted opt-in locations, unchanged ordinary parsing/rendering, frozen/unfrozen runtime pass-through, Linux raw-name order, literal/octal/dynamic wildcard matching, warm delete/recreate, and exact BUILD `?` rejection; the typed observation retains raw names plus direct kind in one shared value, preserves duplicate name order, refines only unknown Unix entries after close, and uses reparse-aware no-follow Windows classification; the required fixture-growth checkpoint is accepted through `e2cc891d` at 1,314 regular files, 24 symlinks, and 39,304 lines after the exact 11-path prune and two affected-fixture replays | production retry/publication activation remains gated on Host migrations, typed production roots, and reviewed byte-aware adapters for every string ingress; the dormant parser seam remains unactivated; the five-path simple segment matcher oracle implementation is next before its private candidate owner, while recursive/package-aware traversal additionally requires a Host package-boundary projection that separates ignored, deleted, subpackage, and nested-repository states; native Windows and lone-surrogate ordering remain activation gates; a reusable regular-or-special Host byte projection remains separate; the full Bazel-valid JVM surface disproves every bounded standalone registry-byte executor found, so the private Host registry-file owner is separately blocked unless a genuinely original-process-equivalent Slug mechanism appears; HTTP and Git PAX/GNU/link/special parity, resolved repository path state, deleted-package request ownership, repository-ignore ownership, package lookup, omitted-`module()` defaults, and discovery composition remain | `WP-5-m1-loading-host-glob-segment-matcher-oracle` |
...
Implement only
`WP-5-m1-loading-host-glob-segment-matcher-oracle`.

Use the accepted five-path/+100/-25 extension of
`glob-callable-contract`: one isolated `//segment_matcher` package, two
one-line assets, four top-level glob comprehensions, one exact fifth query
row, and one regenerated pinned Bazel 9.2 expectation. Prove bare-star hidden
membership, ordinary-star hidden exclusion, explicit-dot inclusion, and
multiple non-adjacent nonempty star spans. Keep the first four callable rows
semantically unchanged and do not touch the raw-name, typed-dirent, or
package-boundary fixtures, harness, Rust, or Stage 9. After the oracle accepts,
implement only the private three-file
