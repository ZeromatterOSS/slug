...
| M8: bootstrap | **active (developer graph)** | exact 33-package CLI boundary plus accepted Gates A-B; Gate C1 has 43 accepted Bazel targets covering 906 source cases; one canonical payload owns all 14 shared fixture workspaces; the cache gate selects nightly; verified actiond reached one remote spawn; the user-confirmed normal-RC BuildBuddy minimal build succeeds without exposing home auth | Direct single-binary RBE-profile build, prime-runner classifier repair, full live cache/RBE gates, core host-tool owners, and self-hosting remain | `WP-10-m8-bazel-buildbuddy-rbe-developer-build-after-user-minimal-success` |
