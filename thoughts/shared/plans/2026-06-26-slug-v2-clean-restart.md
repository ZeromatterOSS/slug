...
| M3: `query` | **active (`attr` five-source executable reconstruction design)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; the corrected 18-lane/165-pair manifest at SHA-256 `99b772e6…42300d`, five-file layout, transition allowlist, computed timeout, and package-license construction remain viable | retry-3 correctly stopped: its deleted exact source bodies are unrecoverable from HEAD, log, and unreachable-object audit, so recovery would require prohibited manual transcription; reconstruct one fresh executable source from the accepted semantic manifest, then run and mechanically render it | `WP-4-8-m3-attr-five-source-executable-reconstruction-design` |
...
[`WP-4-8-m3-attr-five-source-executable-reconstruction-design`](./slug-v2-subplans/current-packet.md).
