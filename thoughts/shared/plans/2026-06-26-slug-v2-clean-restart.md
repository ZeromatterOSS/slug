...
Latest M1 accepted evidence: the corrected raw-read v28 live-cutover design
passed exact-source, implementation-feasibility, and architecture/DICE review.
It reuses the existing `WorkspaceRawFileKey` production snapshot path, gives
all three direct-DICE test helpers matching same-updater raw state, and freezes
one sole v28 value, typed caught/direct error surfaces, complete semantic
planning, registry tri-state projection, and deletion of the old raw/general
schema. The implementation boundary is exactly eight files; no new key, lock,
filesystem read, text fallback, production registry/loading edit, Cargo,
fixture, Host, exact-Off, or merge-conflict work is admitted.
...
Atomically cut the accepted Java-compatible v28 owner into the live raw DICE
read and make it the sole lockfile schema. Change only `lockfile_v28.rs`,
`lockfile.rs`, `lib.rs`, `tests/lockfile.rs`, `module_eval.rs`,
`tests/root_module_dice.rs`, `tests/registry_dice.rs`, and loading's
`tests/glob_invalidation.rs`. Preserve legacy Off's early no-file edge, inject
raw snapshots in every direct-DICE helper, retain caught versus direct failure
surfaces, compare complete semantic values before writes, and run the frozen
serial validation and forbidden-scan matrix. Stop on any ninth implementation
file, new key/lock/read/fallback, production registry/loading change, Cargo or
fixture edit, Host/exact-Off activation, merge-conflict claim, or second
