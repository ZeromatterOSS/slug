...
Run only `WP-5-m1-root-module-composite-event-producer`.

Add the existing workspace `slug_events_v2` dependency and make the private
`RootModuleEvaluationKey` the sole root-MODULE event producer in exactly:
...
Marker absence must retain the evaluator's direct-print behavior and store no
event data. Marker presence must capture one local `EventBatch`, including
explicit empty batches for missing/read/parse/prepare/no-print outcomes and
the exact executed prefix for runtime failure. Keep marker and event state
outside semantic values and equality; do not put event data on file leaves,
`RootModuleFilesKey`, or graph keys. Prove cold nested/repeated order, warm and
fresh-owner nonreplay, print-only semantic equality, failure/recovery,
policy/lockfile reuse, ignore-dev/source reevaluation, marker absence, and
exactly one event-bearing activation node. Do not edit runtime publication,
fixtures, oracle output, harness, or files outside the allowlist.
