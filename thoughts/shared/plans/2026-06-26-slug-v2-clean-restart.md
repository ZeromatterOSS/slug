...
Freeze the smallest extension of the existing retained-server/REAPI
`load-invalidation` fixture that observes exact Bazel 9.2 root-MODULE,
dependency/parent `.bzl`, BUILD, and rule-implementation print order across
success, eligible Starlark failure, warm reuse, semantic edit, execution
failure, and valid empty query. Keep stdout/stderr channel order
discriminating, prove whether semantic output precedes an execution diagnostic,
and forbid warm replay. Do not edit Slug Rust or expand the oracle runner unless
fixture-local capture is proved insufficient and a corrected exact allowlist is
