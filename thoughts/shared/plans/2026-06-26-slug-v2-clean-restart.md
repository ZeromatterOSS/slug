...
| M4: `cquery` | **active (post-analysis `siblings` accepted)** | projected label/Starlark-label output, ordered sets, Rust-native `filter`, full `some`, non-test `executables`, exported rule-class `kind`, exact `siblings` terminal, typed exits, and retained configured rule capability | select the next exact vertical from retained configured state | `WP-6-m4-configured-query-successor-audit-7` |
...
[`WP-6-m4-configured-query-successor-audit-7`](./slug-v2-subplans/current-packet.md).
