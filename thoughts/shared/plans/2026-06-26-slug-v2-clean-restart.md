...
| M4: `cquery` | **active (ordered configured sets accepted)** | one-root Starlark label, projected label output, C0/C1/C0 restoration, and function-free `set`/`let`/`union`/`intersect`/`except` over configured roots | dependency traversal remains gated by missing observable platform/configuration nodes; provider projection and broader functions/formats remain unselected | `WP-6-m4-configured-query-successor-audit` |
...
[`WP-6-m4-configured-query-successor-audit`](./slug-v2-subplans/current-packet.md).
