...
Design only `WP-5-m1-bazel-lockfile-v28-schema-design`.

The Host visible-lockfile boundary audit proved the live `BazelLockfile` is a
lossy prerequisite: it drops extension factors/metadata and uses weaker raw
identities for checksums, selected-yanked keys, facts, versions, digests,
recorded inputs, and repo specs. Freeze the exact Bazel 9.2 v28 schema,
parser/renderer equality and errors, oracle-first evidence, and bounded live
migration before returning to the separate Host read/mode owner. Do not edit
