...
| M7: command/ruleset breadth | **partial; materialization-request observation retry active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, `cb5073e0`'s immutable action-owner row, `b7390392`'s rules_rust 0.73 evidence, `a3efa1b7`'s root MODULE-files epoch, and `3d174006`'s effective-override epoch are accepted | the sound request sibling needs its corrected observed terminal/lifecycle proof before acceptance | complete the bounded request proof, then resume the selected-module-graph frontier and preserve M7A -> M8 -> M7B |
...
[WP-6-7A-repository-materialization-request-observation-implementation-retry](./slug-v2-subplans/current-packet.md).
