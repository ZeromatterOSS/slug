...
[WP-4-5-7A-shared-registration-expander-architecture](./slug-v2-subplans/current-packet.md).

### M7 canonical loading source address accepted; registration architecture active (2026-08-27)

Commit `79a36c580` accepts one Root/Canonical loading carrier through subtree,
repository package, recursive `.bzl` and cycle owners. Built-in catalog bytes
remain zero-copy and catalog-relative, canonical evaluator names remain
distinct from published package paths, complete mappings come from their sole
route owner, mapped child route/effect observations precede source, and root
behavior remains exact. Independent terminal review returned `ACCEPT` after
the complete mapped-child frontier and proof-size corrections.

Activate only `WP-4-5-7A-shared-registration-expander-architecture` at zero
Rust. Freeze one shared contextual canonical parser, a general package
inventory beneath current consumer-policy adapters, independent family keys
and exact Bazel stable-postorder recursion before implementation. Both MODULE
registration families are designed together. Bazel 9 BCR Starlark still owns
all rules including `cc_internal`; `cc_common` remains a reusable host-ABI
consumer. Zabel is peer design/optimization guidance only.
