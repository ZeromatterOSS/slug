...
[Built-in Xcode configure catalog audit accepted; exact one-file Stage 5
implementation selected](./slug-v2-subplans/current-packet.md).

Commit `5d5991634` terminally accepts bounded built-in public-label routing at
80/360/440 gross production/proof/total Rust additions. Full loading, focused
query and CLI build gates pass. The authentic replay clears the rules_cc
facade and recursive utility, then stops at missing exact
`@@bazel_tools//tools/osx:xcode_configure.bzl` content.

The docs-only catalog audit returns `ACCEPT`: pinned and installed Bazel 9.2
agree on one 12,993-byte source with no syntactic loads, and existing evaluator
APIs already own its declarations. Run only
`WP-5-7A-bazel-tools-xcode-configure-catalog-implementation-r1` under its exact
three-file allowlist and caps. Add no BUILD, locator, sibling asset, runtime
repository behavior, route/key or apple_support/toolchain special case.
