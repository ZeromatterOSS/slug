...
| M3: `query` | **active (`attr` source-template retry 2)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; the corrected 18-lane/165-pair manifest at SHA-256 `99b772e6…42300d`, five-file layout, transition allowlist, computed timeout, and package-license construction are accepted | rebuild exact five source bodies with every reviewed hidden construction, then validate all 18 primary and focused queries twice; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats stay deferred | `WP-4-8-m3-attr-five-source-template-oracle-design-retry-2` |
...
[`WP-4-8-m3-attr-five-source-template-oracle-design-retry-2`](./slug-v2-subplans/current-packet.md).
