...
Run only
`WP-5-m1-runtime-http-archive-captured-ustar-subset-retry`.

Edit only `app/slug_core_v2/src/runtime/repository_io.rs`. Implement the exact
private capture, failure ordering, two-phase USTAR inspection/extraction,
numeric/format/type subset, ISO-8859-1 prefix conversion, Unix/Windows-flavor
normalization, prefix/containment, collision, and evidence contract in
**Stage 5 HTTP captured-USTAR design correction** of the owner plan. Add the
narrow regressions first. Keep `materialize_git` and its existing
stdout/NamedTempFile/`extract_tar` path byte-identical.

Run focused and full core tests, real GNU-Windows no-run compilation,
formatting, diff, archive, exact one-file scope, Git byte-identity, and
forbidden-reference gates. Return `REPLAN` on any second file, dependency,
public/session/runtime/path/DICE/source-preparation change, caller-source
reopen, destination write during inspection, external HTTP tar invocation,
lossy raw-name conversion, or missing discriminating row.
