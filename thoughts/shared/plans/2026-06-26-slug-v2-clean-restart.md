...
Run only `WP-5-m1-host-registry-inputs-design`; it is design-only.

Freeze the smallest serial private owners for raw registry/mirror input
normalization, a vendor-only package-policy projection, and a dedicated
Refresh invalidation token. Audit pinned Bazel 9.2 source and the live Slug
seams, specify exact value/equality/Need/error and retained-DICE evidence, and
terminally rereview the complete contract before any Rust, Cargo, fixture,
dependency, API, consumer, or activation change.
