...
| M7: command/ruleset breadth | **active (root extension-usage owner design)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite run, complete Host discovered leaves, normalized/effective overrides, exact shared versions, accepted callerless selected graph in `216a0be8`, selected registry RepoSpecs in `e8ad58dd`, and selected routes in `6f72baaf` | root MODULE evaluation does not retain extension usages, imports, isolation, tags, override/inject, or innate repo-rule identity required by selected extension mappings | WP-5-root-extension-usage-semantic-owner-design |
...
[WP-5-root-extension-usage-semantic-owner-design](./slug-v2-subplans/current-packet.md).
