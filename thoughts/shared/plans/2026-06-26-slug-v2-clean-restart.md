...
| M1: one semantic spine | partial | retained runtime/loading spine plus the accepted Host root-module and public root-loading-anchor projection, accepted typed no-follow dirent observation, accepted and implemented 13-row focused ASCII Host-dirent glob oracle, accepted and implemented eight-row Bazel-internal string oracle, accepted Bazel-internal string oracle/feasibility design, and accepted dormant Bazel-internal string parser seam in `c8f13ee9`, Bazel v28 visible-lockfile, registry-input, pure RegistryFunction, Registry IO bridge with native local paths, focused registry-file vendor oracle, exact ASCII local-directory registry oracle, accepted default-launch collation/charset oracle, and exact host-JVM startup/reuse oracle; pinned source, the exact oracle, and the dormant token-local implementation prove BUILD/ordinary `.bzl` Latin-1 byte carriers, greedy octal-only escapes, byte-counted opt-in locations, unchanged ordinary parsing/rendering, and frozen/unfrozen runtime pass-through; the typed observation retains raw names plus direct kind in one shared value, preserves duplicate name order, refines only unknown Unix entries after close, and uses reparse-aware no-follow Windows classification; the required five-packet fixture-growth checkpoint is accepted through `22de3631`, and the accepted string oracle is post-checkpoint packet four | production retry/publication activation remains gated on Host migrations, typed production roots, and reviewed byte-aware adapters for every string ingress; the dormant parser seam remains unactivated, and raw-name/pattern-lazy oracle breadth is next before pure Host glob ownership and the transactional package-evaluation retry/suspension boundary; whichever oracle next reaches acceptance is post-checkpoint packet five and must perform the focused fixture-growth review; native Windows and lone-surrogate ordering remain activation gates; a reusable regular-or-special Host byte projection remains separate; the full Bazel-valid JVM surface disproves every bounded standalone registry-byte executor found, so the private Host registry-file owner is separately blocked unless a genuinely original-process-equivalent Slug mechanism appears; HTTP and Git PAX/GNU/link/special parity, resolved repository path state, deleted-package request ownership, repository-ignore ownership, package lookup, omitted-`module()` defaults, and discovery composition remain | `WP-5-m1-loading-raw-name-pattern-lazy-glob-oracle-design` |
...
`WP-5-m1-loading-raw-name-pattern-lazy-glob-oracle-design`.

Freeze the next focused Bazel 9 raw-name/pattern-lazy glob oracle boundary
after the dormant parser seam accepted in `c8f13ee9`. Do not silently reuse
the historical byte-capable pattern-lazy design that ended in `REPLAN`.
Separate the eventual post-checkpoint packet-five oracle from the later pure
Host glob owner and transactional evaluator retry/suspension work, and include
the required focused fixture-growth review in that eventual oracle's terminal
acceptance contract.
