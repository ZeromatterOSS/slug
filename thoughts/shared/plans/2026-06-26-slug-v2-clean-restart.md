...
Extend only accepted `app/slug_workspace_v2/src/path_resolution.rs` and add
exactly three semantic public reexports in `app/slug_workspace_v2/src/lib.rs`:
`PathFileBytes`, `PathFileBytesError`, and `PathFileBytesKey`. Do not rewrite
Owner A's operational schemas, machine, errors, routes, or A/B tests.

Implement the frozen total projection and semantic-only error equality, exact
Directory/SpecialFile WrongKind and FileBytes error/inconsistent behavior,
direct operational-versus-semantic tables, and the retained test-only
selector/projection/pointer-counter sequences proving actual downstream
pruning and A→B→Missing→typed-error→A counts. Run focused C tests, full
workspace tests/doctests, format, diff, the same exact two-file allowlist,
forbidden-surface checks, and archive guards; require independent DICE/equality
and pinned-source acceptance before one implementation commit.
