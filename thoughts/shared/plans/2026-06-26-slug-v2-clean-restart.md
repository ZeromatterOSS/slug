...
Run only `WP-5-m1-nonroot-include-composition-oracle`.

Create only
`tests/v2_oracle/fixtures/nonroot-include-composition/**`. Use one local-path
nonroot module whose included fragments, extension, generated marker, and
consumer target are all self-contained. The retained-daemon oracle must pin
successful nested inline tag order, exact repeated raw-label execution,
direct included-fragment A→B→A invalidation without a masking root edit, a
nested runtime failure with Bazel's exact include-parent stack/location shape,
a later-fragment scope failure before an earlier invalid directive executes,
and final same-daemon recovery. Generate and verify with Bazel 9.2 before
claiming exact exits or diagnostic regexes.

Run the two-fragment include cycle only as a separate temporary Bazel
`--batch` subprocess with a fresh output root and a hard process-group timeout.
Record it in the handoff only; it is not an oracle row or a Slug diagnostic.
Stop rather than weakening the packet if the stable parent include frame is
not observable or if registry/source-preparation/raw-value/harness expansion
is required. Do not edit Rust, Cargo, DICE, preparation, discovery, commands,
server, lockfiles, any existing fixture, or the oracle harness. Slug replay and
include implementation remain deferred.
