...
| M5: `aquery` | **active (root-local FileWrite order accepted)** | recursive action ownership, complete Slug configuration identity, strict closure-resolved toolchain-backed FileWrite semantics, exact framed Slug FileWrite identity, and declaration-ordered root-only literal CLI/daemon text | `deps()` owner-set/order design and later action/expression/output breadth remain open | `WP-8-m5-filewrite-aquery-deps-owner-set-design` |
...
[`WP-8-m5-filewrite-aquery-deps-owner-set-design`](./slug-v2-subplans/current-packet.md).
