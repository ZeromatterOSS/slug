...
[Typed FilesToRun provider-core implementation](./slug-v2-subplans/current-packet.md).

The first implementation successor of the accepted generic
DefaultInfo/runfiles/FilesToRun architecture is terminally accepted. It owns
typed Artifact/depset identity, a dedicated nonconstructible Starlark
FilesToRun value, scope-local typed provider provenance, structural
publication equality, an explicit incomplete-support guard, and the final
retained runfiles/support schema. It does not open runfiles construction or
action expansion. The next successor owns typed `ctx.runfiles`, merge
operations, and all five `DefaultInfo` constructor parameters. This remains a
generic standard-provider category, not a `cc_common`, rules_cc, parser, or
C++ rule packet.
