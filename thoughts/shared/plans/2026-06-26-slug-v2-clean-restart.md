...
[WP-4-5-7A-module-bzl-native-context-implementation](./slug-v2-subplans/current-packet.md).

Commit `cc93ce4e7` accepts the complete exact module-extension
`native.existing_rule`/`native.existing_rules` no-op family with BUILD and
unauthenticated evaluation fail-closed. Two fresh rules_rust replays advance
beyond that former rejection and now stop identically at missing
`native.bazel_version` in `@@bazel_features+//private:globals_repo.bzl:22`.

Commit `612126b6f` independently accepts the general module-loaded
native-context architecture. Implement it by carrying a private BUILD/Bzlmod
context in root and external `.bzl` evaluation, observation, and cycle keys;
inherit it recursively; and expose two
native namespace values backed by one common method table. Only the Bzlmod
namespace exposes the pinned Bazel 9.2 version owned by
`BuiltinBazelToolsSnapshot`. Package loads remain BUILD-context; ordinary,
selected, innate, and repository-definition module-extension paths use Bzlmod
context. Do not change parser or `set` semantics, `cc_common`, `cc_internal`,
rules_cc, or C++ rules/actions. Bazel 9.2 remains sole semantic authority;
clean Zabel is peer implementation guidance for namespace separation only.
