...
| M2: analysis graph | **active** | recursive configured analysis, bounded root cquery in `135b0567`, implicit empty-default normalization in `7c6eeae5`, internal string build-setting transitions in `dfc1705e`, first-compatible toolchain evidence in `ed4baf08`, and ordered root registration retention in `4a3af8df` | native toolchain declarations, rule requirements/builtin loading, real DICE selection/context ownership, then general identity and action gates | implement `WP-6-m2-native-toolchain-target-loading-implementation` |
...
`WP-6-m2-native-toolchain-target-loading-implementation`.
...
registration-retention implementation is accepted in `4a3af8df`. The current
read-only design packet now isolates fixture-bounded native constraint,
platform, toolchain-type, toolchain-declaration, and rule-class requirement
loading before any DICE selection or prepared `ctx.toolchains`. Reserved review
accepted two serial loading owners: first the five native target declarations
with fail-closed query boundaries, then frozen Starlark rule requirements plus
a load-only `platform_common.ToolchainInfo` symbol. The native target packet is
current; invalid transition and toolchain failure diagnostics remain deferred.
...
platform inputs, plus Starlark options/scopes; Slug's opaque `first-build`
