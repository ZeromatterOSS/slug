...
[WP-4-5-7A-repository-rule-attribute-family-implementation-r2](./slug-v2-subplans/current-packet.md).
...
Architecture commit `a286f0b04` independently accepts the complete thirteen-
kind repository-rule attribute family and its corrected generic nested-map
publication identity. Implement only the frozen six-file R2 packet: extend the
loading-private recursive call carrier, converge ordinary and innate coercion,
validate all defaults without publishing them, and replace the built-in
`remote_patches` identity exception. Preserve name-addressed top-level
membership equality and both existing route hash consumers. Do not patch only
`string_list_dict`; do not change parser, `set`, repository Host methods,
