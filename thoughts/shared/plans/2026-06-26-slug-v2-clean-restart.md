...
| M7: command/ruleset breadth | **active (discovered-module prerequisite)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite `run`, the immutable canonical `@@bazel_tools` route/seven-file source owner, and the complete callerless embedded MODULE value in `3bc745de` | implement the reviewed embedded/registry per-module owner before selected-graph ownership | `WP-5-host-discovered-module-owner-implementation` |
...
[`WP-5-host-discovered-module-owner-implementation`](./slug-v2-subplans/current-packet.md).
