...
| M7: command/ruleset breadth | **active (embedded bazel_tools closure design)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite `run`, and the immutable canonical `@@bazel_tools` route/seven-file source owner | freeze the complete pinned Bazel 9.2 repository/source/package closure required by `@@bazel_tools//tools/test` before consumer dispatch | `WP-4-6-8-bazel-tools-test-closure-design` |
...
[`WP-4-6-8-bazel-tools-test-closure-design`](./slug-v2-subplans/current-packet.md).
