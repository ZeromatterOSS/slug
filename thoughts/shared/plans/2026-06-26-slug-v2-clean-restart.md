...
| M7: command/ruleset breadth | **active (selected extension mapping owner design)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite run, complete Host discovered leaves, normalized/effective overrides, exact shared versions, accepted callerless selected graph in `216a0be8`, accepted selected registry RepoSpec owner in `e8ad58dd`, and accepted callerless selected routes in `6f72baaf` (328 production/439 tests/767 total; full owner/loading suites and independent review pass) | audit the additive post-selection extension mapping owner without extension evaluation or consumers | WP-5-host-selected-extension-mapping-owner-design |
...
[WP-5-host-selected-extension-mapping-owner-design](./slug-v2-subplans/current-packet.md).
