...
Run only `WP-5-m1-nonroot-directive-evaluator-redesign`.

Freshly redesign the single-supplied-file nonroot directive evaluator against
the accepted compact schema and post-file snapshot. Freeze the exact
starlark-rust globals and evaluator-local context for `module`, dependencies,
registrations, flag aliases, ordinary extension proxies/tags/imports, isolated
usages, repo redirection calls, and innate repo rules; pin mutation,
validation, nonroot-dev suppression, built-in insertion, and final snapshot
ordering. The design must supply exact builtin-`print` and extension-proxy
identities to the private snapshot helper and must keep update-mode adapter
failure before later ordinary schema validation without retaining the heap.

This packet is read-only. It may supersede the earlier directive design that
replanned for raw-value evidence, but it must not edit Rust, add public or DICE
seams, activate includes, change source preparation/discovery, or implement
lockfile hashing. Stop if a single-file evaluator cannot produce the accepted
`EvaluatedNonrootModule` and bounded deferred diagnostics through existing
private ownership, or if any additional raw value/cycle form needs support
without a new Bazel oracle.
