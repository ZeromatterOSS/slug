...
| M3: `query` | **active (`attr` observable-candidate oracle generation)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; pinned Bazel 9.2 closes the complete Starlark/native attr-visible loading schema; the reviewed 18-lane paired matrix covers its finite observable classes without union-dedup rescue | generate and independently replay the bounded ordinary-query fixture; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-observable-candidate-oracle-generation` |
...
[`WP-4-8-m3-attr-observable-candidate-oracle-generation`](./slug-v2-subplans/current-packet.md).
