...
Latest M1 accepted evidence: the exact 15-row Bazel 9.2 lockfile-v28 schema
oracle is retained. It pins the complete six-field value, canonical rewrite,
current and foreign factors, generated FILE/DIRENTS/ENV/REPO_MAPPING plus
injected DIRTREE stale/replacement behavior, defaults and Gson leniency,
caught failures, and three uncaught adapter holes. Semantic comparison now
enforces all five visible-lockfile manifests. Its 500-entry registry scaffold
matches the pinned tools lockfile exactly: 184 BCR successes, 312 paired
module-discovery misses, and four checked fixture-local entries. Generation,
two final fresh-root replays, exact provenance/scope/growth/archive/diff
checks, and all three terminal reviews passed. No Rust, Cargo, harness, public
API, or activation changed.

### Current packet

Implement only
`WP-5-m1-bazel-lockfile-v28-pure-owner`.

Add the accepted private immutable v28 value, exact Gson-compatible reader and
canonical renderer, typed parse/render errors, semantic equality, and
parse-render-parse evidence in only `app/slug_bzlmod_v2/Cargo.toml`,
`src/lockfile_v28.rs`, `src/lockfile_v28_tests.rs`, and `src/lib.rs`. Add only
the existing workspace `base64` dependency. Do not edit the live lockfile
owner, add a public re-export, activate Host, or create a second production
schema. The later atomic cutover remains serial.
