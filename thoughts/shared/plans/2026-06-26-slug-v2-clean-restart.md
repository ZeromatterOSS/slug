...
Design only `WP-5-m1-immutable-repository-source-read-migration-design`.

Audit the now-instance-bearing immutable `RepositoryMaterialization`, the
remaining direct `std::fs::read` branch in `RepositorySourceFileKey`, Bazel 9.2
file-kind/read behavior, the accepted resolver/FileBytes contracts, and the
test/runtime observation boundaries. Freeze the smallest packet that reads
beneath `generation_root` through
`PathObservationNamespace::Materialization(observation_instance)` while
preserving semantic source equality and Need propagation.

Do not implement Rust or tests in this design packet. Keep successful-refetch
invalidation, runtime epoch production, retry/event/publication, Local/Host
behavior, discovery, package lookup, and loading/analysis/query propagation
separate unless the audit proves one is a strict prerequisite.
