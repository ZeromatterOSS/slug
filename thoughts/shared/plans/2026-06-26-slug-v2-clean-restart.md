...
Design only `WP-5-m1-repository-materialization-request-result-design`.

Replace the current inside-DICE archive/Git materialization boundary with a
pure structural request and injected operational result design. Freeze exact
request identity as normalized workspace runtime plus canonical repository and
complete `RepoSpec`, a unified invalid/unequal transient carrier that
propagates materialization and path Needs through materialization, source-file,
and preparation keys, and an immutable cumulative complete-replacement result
epoch with lawful per-request lookup and projection despite `RepoSpec` lacking
`Hash` and `Ord`.

Freeze result lifetime and precedence: successes remain retained across
commands, failures are omitted and retried once outside DICE on a later
command, and immutable path observation starts only after a Complete
materialization result. Successful operational equality must include the
generation root and observation instance so DICE cannot return a stale path;
repository source bytes, absence, and typed errors remain the semantic pruning
boundary. Preserve demand-driven unused overrides, failure-only
`RepositoryMaterializationGeneration`, per-repository dirtiness, and
ctime-only non-dirtiness. Specify local-repository symlink-retarget freshness
and error retry semantics. Do not add the runtime driver, observer, event
publication, CLI/server change, production preparation caller, or filesystem
IO inside DICE.
