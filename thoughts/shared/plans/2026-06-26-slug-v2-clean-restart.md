...
| M7: command/ruleset breadth | **partial; repository-source-file proof-cap correction design active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, `cb5073e0`'s immutable action-owner row, `b7390392`'s rules_rust 0.73 evidence, `a3efa1b7`'s root MODULE-files epoch, `3d174006`'s effective-override epoch, and `ae8aa35e`'s request/materialization epoch are accepted | the sound repository-source candidate reaches its exact +500 proof ceiling before the full parent reducer/event/immutable lifecycle matrix; registry preparation remains a separate later frontier | correct only the proof cap, retry the same repository-source owner, then resume the selected-module-graph frontier and preserve M7A -> M8 -> M7B |
...
[WP-6-7A-repository-source-file-observation-proof-cap-correction-design](./slug-v2-subplans/current-packet.md).
