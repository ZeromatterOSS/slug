...
| M4: `cquery` | **active (direct retained-state functions exhausted)** | projected label/Starlark-label output, ordered sets, Rust-native `filter`, full `some`, non-test `executables`, exported rule-class `kind`, exact post-analysis terminals/vacuity, typed exits, and retained configured rule capability | design complete configured-graph ownership before traversal | `WP-6-m4-configured-query-graph-ownership-design` |
...
[`WP-6-m4-configured-query-graph-ownership-design`](./slug-v2-subplans/current-packet.md).
