...
| M7: command/ruleset breadth | **partial; selected-registry extension source implementation active** | generated `repository_ctx.file` producer, demand-side route/request, atomic immutable-root application and tool-aware Bazel/Slug generated-source fixture are accepted through `3ac0a85b`; the bootstrap frontier audit selects the external selected-definition source owner | root-owned `@rules_rust//rust:extensions.bzl` cannot yet consume its selected registry source/repository view; repository-rule and ruleset semantics remain later | implement only `WP-4-5-6-7A-selected-registry-extension-bzl-source-owner-implementation`; preserve M7A -> M8 -> M7B |
...
[WP-4-5-6-7A-selected-registry-extension-bzl-source-owner-implementation](./slug-v2-subplans/current-packet.md).

Associate the first root-owned selected-registry extension definition with its
Bzlmod-owned canonical source/repository view, then load same-repository and
mapped-repository `.bzl` dependencies through loading's existing source and
evaluation owners. Do not widen repository-rule execution.
