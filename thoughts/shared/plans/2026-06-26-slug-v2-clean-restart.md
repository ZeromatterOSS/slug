...
| M1: one semantic spine | **active** | Host loading observations and dormant public typed root-loading/root-analysis/query-command boundaries, including accepted query Host migration; full evidence is in the Stage 5 owner plan | build command root and later activation; external mapping, native Windows glob ordering, registry transports, and discovery composition remain | design the build typed command root |
...
`WP-5-m1-build-typed-command-root-design`.
