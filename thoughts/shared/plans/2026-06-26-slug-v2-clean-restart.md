...
[WP-6-7A-testing-bootstrap-loading-implementation-r3](./slug-v2-subplans/current-packet.md).
...
input ownership after terminal correction review. Commit `7bcac3da3` now
accepts the distinct generic innate owner projection and certificate,
including exact root/nonroot ownership, actual-rule versus synthetic-MODULE
label domains, shared validation and external `@@rules_cc+` identity. The
recoverably parked Host-capability draft is now active; registration proof and
selected-context Rust remain stopped.
...
Independent correction review accepts R3's complete five-binding/member
inventory, generic `ProviderIdentity` declaration/rematerialization path, exact
callability matrix, stable top-level identity, unsupported `@_builtins`
boundary, and three explicit user-provider proof projections. Implement only
`WP-6-7A-testing-bootstrap-loading-implementation-r3` under the manifest's
frozen allowlist and 500/700/1,200 caps. Do not add `ProviderIdentity: Display`,
a rules_shell-only token, universal-environment or parser/`set` changes, a
configured coverage stub, or a `cc_common` special case.
