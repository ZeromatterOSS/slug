...
[WP-4-5-7A-builtin-bazel-tools-cc-direct-package-catalog-implementation](./slug-v2-subplans/current-packet.md).

Commit `1b997c5ef` terminally accepts the generic external-`.bzl` source
projection: exact Root requests retain their legacy DICE child identity/order,
while Root built-ins and Canonical routes use the shared immutable observation
owner. Focused/full validation and independent corrected terminal review pass;
two fresh-root rules_rust replays clear the old source-owner guard and stop
identically at exact `UnsupportedCatalog` for
`tools/build_defs/cc/action_names.bzl`.

Implement only the complete pinned Bazel 9.2 direct package category:
`tools/build_defs/cc/{BUILD,action_names.bzl,cc_import.bzl}`. Preserve the
existing catalog, manifest, directory-listing and zero-copy source owners. Do
not add parser, `set`, `cc_common`, `cc_internal`, C++ rule-engine,
rules_rust, or demanded-file special cases. The proof-only registration and
selected-context candidates remain parked and read-only. Bazel 9.2 is
semantic/content authority; clean Zabel remains peer design/optimization
guidance only.
