...
[WP-4-5-7A-repository-qualified-external-bzl-load-route-design](./slug-v2-subplans/current-packet.md).

Commit `84190d95c` terminally accepts the complete exact pinned Bazel 9.2
`tools/build_defs/cc/{BUILD,action_names.bzl,cc_import.bzl}` category,
including derived manifest and built-in package-set identity. Focused/full
validation and independent terminal review pass; two fresh-root rules_rust
replays clear `action_names.bzl` and stop identically while resolving
`@cc_compatibility_proxy//:symbols.bzl` from rules_cc's public
`cc_common.bzl`.

Design only the generic repository-qualified external-`.bzl` route/mapping
handoff. Audit the real defining-repository final mapping and canonical
extension-generated child route before selecting an owner. Do not add parser,
`set`, `cc_common`, `cc_internal`, compatibility-proxy, rules_cc, or C++
rule-engine special cases. The proof-only registration and selected-context
candidates remain parked and read-only. Bazel 9.2 is semantic authority; clean
Zabel remains peer design/optimization guidance only.
