...
| M7: command/ruleset breadth | **partial; M7A exec-group action-owner evidence active** | `53152727` accepts the first private core source consumer; M1 now supplies the shared observation/publication spine; bounded FileWrite aquery and REAPI paths are accepted | immutable creation-time action ownership lacks generated Bazel 9.2 default/named exec-group evidence; the bootstrap-critical closure remains M7A | generate the focused owner-context oracle before its Rust design, then follow M7A -> M8 -> M7B |
...
[WP-1-6-7A-exec-group-action-owner-context-evidence](./slug-v2-subplans/current-packet.md).
