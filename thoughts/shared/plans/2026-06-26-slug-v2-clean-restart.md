...
| M1: one semantic spine | **active** | Host loading observations, dormant public typed root-loading/root-analysis/query-command boundaries, accepted query Host migration, and accepted build command-root design; full evidence is in the Stage 5 owner plan | build command-root implementation and later activation; external mapping, native Windows glob ordering, registry transports, and discovery composition remain | implement the build typed command root |
...
`WP-5-m1-build-typed-command-root`.
