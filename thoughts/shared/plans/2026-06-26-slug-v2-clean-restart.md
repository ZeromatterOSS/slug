...
repeat unchanged and require the marker without any replayed print; delete
`pkg/BUILD` for the same pre-execution failure; then create
`pkg/BUILD.bazel` for recovery with the same print and marker.
...
count/location/nonreplay, manifests, source/provenance/closure, fixture
listing, diff, archive, credentials, and net growth. Stop on unstable print
attribution, a warm replay, a missing-file rather than missing-package cause,
any execution before package failure, nonempty omitted defaults or
version-first failure, network access, old-row drift, or scope expansion. No
Rust is authorized.
