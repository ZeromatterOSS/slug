...
| M7: command/ruleset breadth | **partial; rules_rust 0.73 toolchain/action-owner evidence active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, generated Bazel 9.2 owner evidence, and `cb5073e0`'s immutable configured-action row with explicit selected-toolchain, selected-platform-only and unresolved-default states are accepted | direct nonroot registration, synthetic root selection, and action-owner evidence do not expose rules_rust 0.73's extension-generated `@rust_toolchains//:all` mapping or selected Rust action context | generate only the isolated Bazel 9.2/rules_rust 0.73 discriminator, then design the external toolchain owner and preserve M7A -> M8 -> M7B |
...
[WP-1-6-7A-rules-rust-0.73-toolchain-action-owner-evidence](./slug-v2-subplans/current-packet.md).
