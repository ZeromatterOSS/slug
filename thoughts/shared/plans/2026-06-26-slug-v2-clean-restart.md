...
Run only `WP-5-m1-nonroot-preparation-provenance`.

Edit only `app/slug_bzlmod_v2/src/source_preparation.rs`,
`app/slug_bzlmod_v2/src/lib.rs`, and
`app/slug_bzlmod_v2/tests/source_preparation_dice.rs`. Replace bare found bytes
with compact `NonRegistry { bytes }` or `Registry { bytes,
selected_registry, module_file_attempts }`. Each ordered attempt retains its
exact `RegistryFileUrl` plus `Some([u8; 32])` for downloaded bytes or `None`
for a completed not-found. Capture the downloaded digest before root patches.
Project away the lower registry value's absence source and recordable
expectation; their lifecycle remains owned by `RegistryFileKey`.

Make complete exhaustion typed and retain its ordered not-found attempts.
Preserve `MissingVersion` before IO. Remove fatal registry string erasure:
policy value errors retain `RegistryFileError`; every per-file value or DICE
failure retains the exact attempted URL and prior completed not-found attempts
in a distinct typed variant. Do not add a terminal failure to successful
lockfile provenance. Only `RegistryFileValue::NotFound` may fall through;
registry, source, and patch failures remain fatal.

Structural DICE equality includes bytes, selected normalized registry, ordered
URL/checksum attempts, exhaustion attempts, and typed failures. It excludes
materialization roots and request generations. Add exact focused regressions
for miss→success provenance, miss→fatal stop, exhaustion order, override
selection/bypass, nonregistry provenance, downloaded-versus-patched digest
identity, and A→B→A lifecycle reuse. Run the full bzlmod suite, formatting,
diff/archive checks, and fresh independent review. Do not edit registry IO,
evaluator, discovery, Cargo, fixtures, expected artifacts, commands, server,
or lockfiles.
