...
| M3: `query` | **active (`attr` computed-timeout correction)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; pinned Bazel 9.2 closes the corrected 18-lane/165-pair families and reviewed five-file two-package layout; transition-allowlist spelling is corrected | source-template correction proved `l11_a003`'s frozen `size=short` is invalid; change only that negative size to Bazel's valid `small` computed-timeout source and recompute the digest before source-template retry; remaining native-toolchain graph projection, rank order, external repositories, patterns, and formats stay deferred | `WP-4-8-m3-attr-test-timeout-manifest-correction` |
...
[`WP-4-8-m3-attr-test-timeout-manifest-correction`](./slug-v2-subplans/current-packet.md).
