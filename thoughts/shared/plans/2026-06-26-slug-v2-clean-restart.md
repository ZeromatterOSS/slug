...
Run only `WP-5-m1-nonroot-deferred-attribute-snapshot-design`.

Freeze a heap-independent post-file snapshot boundary for raw ordinary-tag and
innate repository-rule kwargs. It must preserve mutations made after calls,
retain structural sharing and bounded cycles without a live/frozen Starlark
heap, distinguish later ordinary schema validation from innate repo-rule
instantiation, and reproduce the update-mode adapter failure before ordinary
tag validation. Define exact semantic equality and identity-free diagnostic
projection for the oracle-proven float, builtin callable, extension proxy,
nested invalid list/dict key, and self-cycle forms; do not generalize to an
unobserved opaque-value universe.

This packet is read-only design. It may supersede the rejected call-time schema
but must not edit Rust, add a DICE key or public seam, activate the directive
evaluator, compose includes, or change preparation/discovery ownership. Stop
if aliasing, cycles, or stable diagnostics require retaining evaluator-owned
values or if the compact representation cannot express every accepted oracle
