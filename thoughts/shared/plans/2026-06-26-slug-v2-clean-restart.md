...
[WP-4-5-7A-innate-repository-rule-owner-certificate-implementation-r2](./slug-v2-subplans/current-packet.md).
...
registration rows. Commit `10e6f1a8b` now terminally accepts generic
`repository_rule` declaration metadata. Architecture R5 is independently
accepted in `3dbd937a4`, and `64878a1be` accepts shared command/wire/DICE Host-
input ownership after terminal correction review. The Host-capability draft
then proved that selected innate `use_repo_rule` owners are still rejected and
would otherwise be loaded as ordinary module extensions. The active zero-Rust
packet freezes a distinct generic innate owner certificate before that draft
can resume. Registration proof and selected-context Rust remain stopped.

### M7 innate owner architecture accepted; implementation active (2026-08-29)

The stopped loading draft implements generic `repository_ctx.os`, `getenv` and
staged `file`, and its original focused effect suite passes. Its first exact
authenticated BCR winsdk proof fails earlier at selected-owner inputs:
`use_repo_rule` is represented as Bazel's synthetic `//:MODULE.bazel` owner
with compound `"<bzl label> <rule name>"` identity, while Slug rejects the
compound name and nonroot owner, then its loading owner authenticates only
`module_extension` exports.

Independent architecture review first returned `REPLAN`: removing the guard
would load the wrong file and callable type. Commit `30118d7fd` now freezes the
corrected generic singular-owner projection and distinct loading certificate,
including separate actual-rule versus synthetic-MODULE label domains and
Root/built-in/selected/generated source routing through existing owners. The
corrected architecture received independent `ACCEPT`.

Run only
`WP-4-5-7A-innate-repository-rule-owner-certificate-implementation-r2` under its
exact nine-file allowlist and 1,250/1,400/2,650 caps. No winsdk special case,
MODULE evaluator change, or repository Host capability is authorized. Park
the stopped Host draft recoverably and preserve the selected-context candidate
unchanged. Bazel 9.2 remains sole semantic authority; clean Zabel is concept/
optimization guidance only. BCR Starlark owns all rule logic, including
`cc_internal`, and `cc_common` stays a generic Host/provider ABI.
