...
| M7: command/ruleset breadth | **partial; nonrepeatable Bazel `config.string_list` definition loading selected** | selected-registry source/root-load and exact rules_rust root realization are accepted through `2f373248`; commits `54d28477`, `1a527089`, `a81b5823`, `6ab6f35d` and `573c25c7` accept bounded keyword-only syntax, exact live `.bzl` `struct` placement/operations, provider/rule docs and typed bool definition loading; the StringList audit authenticates the next source-ordered descriptor | retain normalized nonrepeatable StringList identity/schema and fail before list-target recording; repeatable descriptors plus bool/list invocation and analysis remain deferred | run only `WP-4-7A-bazel-config-string-list-nonrepeatable-loading`; preserve M7A -> M8 -> M7B |
...
[WP-4-7A-bazel-config-string-list-nonrepeatable-loading](./slug-v2-subplans/current-packet.md).

Accept the first live nonrepeatable `config.string_list(flag = True)`
definitions, retain StringList distinctly through freeze/equality/schema, keep
BUILD string-only, and reject repeatable descriptors and every list-setting
target before target recording.

### M7 StringList audit selects nonrepeatable definition loading (2026-08-26)
...
The audit selects the first two source-ordered nonrepeatable declarations as
one bounded definition-only slice. Add a distinct StringList kind to the sole
retained rule-definition owner, derive list-default schema, expose only named
`.bzl` calls with `flag=True` and omitted/false `repeatable`, keep BUILD
string-only, and reject every list target before recording. Omission and
explicit false normalize to one identity; admitting `repeatable=True` later
requires a structurally retained identity dimension and is not authorized now.

Run only `WP-4-7A-bazel-config-string-list-nonrepeatable-loading` within its
two-file 90/90/180 caps. Pinned Zabel `c7298478…` directly guides the one
complete typed declaration owner and structural rule/invocation comparison;
its separate typed repeatability fields confirm that later repeatability may
not be hidden in evaluator or analysis state. No Zabel code, representation or
behavior is copied. Bazel 9.2 remains sole behavior authority, and M7A -> M8 ->
M7B is unchanged.
