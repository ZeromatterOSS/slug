...
| M7: command/ruleset breadth | **active (bounded run implementation retry)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, plus accepted client-launch/daemon-wire and oracle-endpoint designs | complete the first executable FileWrite `run` vertical without inventing runfiles or direct-local build actions | `WP-8-m7-filewrite-run-handoff-implementation-retry` |
...
[`WP-8-m7-filewrite-run-handoff-implementation-retry`](./slug-v2-subplans/current-packet.md).
