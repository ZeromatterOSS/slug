...
[WP-4-5-7A-effective-repository-host-input-implementation](./slug-v2-subplans/current-packet.md).
...
`repository_rule` declaration metadata. The active zero-Rust packet freezes
command-scoped Host environment/OS ownership, exact per-name DICE invalidation,
retry/restoration, canonical repository execution and three bounded successor
packets. Architecture R5 is independently accepted in `3dbd937a4`; the active
packet implements only shared command/wire/DICE Host-input ownership. Repository
evaluation/effects and selected-context Rust remain stopped.
