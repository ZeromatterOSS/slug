...
| M1: one semantic spine | **active** | Host loading observations, dormant typed root-loading/root-analysis/build/query command boundaries, and accepted preactivation Host-gate design; full evidence is in the Stage 5 owner plan | executable Host closure gate, opaque output envelope, and later activation; external mapping, native Windows glob ordering, registry transports, and discovery composition remain | implement the preactivation Host gate |
...
`WP-5-m1-preactivation-host-gate`.
