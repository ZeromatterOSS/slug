...
Run only
`WP-5-m1-runtime-path-observation-unix-adapter`.

After the accepted neutral kernel, edit only
`app/slug_core_v2/src/runtime/path_observation.rs`,
`app/slug_core_v2/Cargo.toml`, and `Cargo.lock` if dependency resolution
actually changes. Add only target-Unix `nix = { workspace = true }`, using
`nix::libc` and `nix::errno::Errno`. Do not edit `runtime/mod.rs` or add public,
Windows, DICE, materializer, retry/publication, fixture, or consumer behavior.

Add a cfg-Unix adapter and private Unix batch entrypoint implementing the
neutral trait. Lstat retries interrupted `symlink_metadata`, maps
NotFound/NotDirectory to Missing, and preserves no-follow kind, signed
size/inode, wrapping millisecond times, and `mode & 0777`. ReadLink and
FileBytes perform their primary operation first, retry Interrupted, and classify
only the accepted candidate failures as `Refine`; every other native error is
`Final`.

Add a private generic `UnixDirectoryApi` and Option-handle owner. The sole
unsafe libc implementation immediately captures errno and copies transient
`d_name` bytes. Interior-NUL paths are final InvalidInput. Retry `opendir` only
on EINTR. Clear errno before every `readdir`; retain the same handle and
partial names across EINTR/EIO; null plus zero is EOF. Other iterator errors
discard partial results. Explicit close disarms before calling; iterator or
validation errors win over ignored cleanup, EOF accepts close EINTR and
surfaces other close errors, and Drop closes only a remaining handle. Skip only
exact dot entries, preserve non-UTF-8 names, and project invalid/duplicate names
to operation-level InvalidData.

Script opener EINTR/EIO/other errors, handle identity, errno clearing/staleness,
partial-name retention/discard, iterator-versus-close precedence, EOF close
success/EINTR/error, invalid/duplicate names, explicit disarming, Drop fallback,
and exactly-once close. Add temp-derived real Unix evidence for lstat kinds and
metadata, relative ReadLink, FileBytes, file create/edit/delete/recreate,
directory child mutation/delete/recreate and sorting, symlink
create/retarget/delete/recreate, non-UTF-8 names, same-path Host/materialization,
and authorized inside-root and escaped exact paths.

Validate focused owner/operation/lifecycle tests, full core tests, Windows GNU
target compilation of the neutral-only boundary, formatting, diff, archive
status, exact scope, and forbidden `std::fs::ReadDir`, `nix::dir::Dir`, public
or runtime wiring. Return `REPLAN` if the neutral trait needs a material change,
Windows gains any stub or entrypoint, another dependency/file is needed, or the
raw owner and lifecycle matrix cannot be proved in this checkpoint.
