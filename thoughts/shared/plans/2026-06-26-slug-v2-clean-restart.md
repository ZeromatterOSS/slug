...
| M2: analysis graph | **active** | recursive configured analysis, bounded root Starlark-label cquery in `135b0567`, and positive string build-setting/transition evidence in `b12774b9` | implementable semantic configuration ownership, then general option and failure-diagnostic identity, toolchain/platform, repository-mapping, and broader action ownership gates | design `WP-6-m2-positive-string-build-setting-transition-design` |
| M3: `query` | partial | 13/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; full evidence is in the Stage 8 owner plan | Pattern functions remain `REPLAN`; rank auto order needs original dependency order absent from the selected graph; external repositories, pattern breadth, and remaining formats require new representation | pause while the M2 string-transition design is current |
...
`WP-6-m2-positive-string-build-setting-transition-design`.
...
checksum-free. The active design now decides whether that semantic slice has a
bounded internal owner and truthful command-observability boundary; invalid
transition diagnostics remain deferred.
