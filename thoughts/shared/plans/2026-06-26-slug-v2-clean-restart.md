...
Implement only `WP-5-m1-immutable-repository-source-read-migration`.

Edit exactly `app/slug_bzlmod_v2/src/source_preparation.rs` and
`app/slug_bzlmod_v2/tests/source_preparation_dice.rs`. Factor the accepted
Local resolver/FileBytes sequence into one private observed-read helper.
Local calls it with Host/source root; Immutable calls it with
`Materialization(observation_instance)`/generation root. Preserve exact Need,
Regular/Special/Missing/wrong-kind, symlink, typed error, and semantic
projection behavior.

Remove immutable direct filesystem reads and the obsolete `ImmutableRead`
error. Add exact materialization-namespace cumulative, terminal, instance
anti-alias, pruning, and fixed-instance lifecycle evidence from the accepted
owner contract. Do not add `PathFileBytesKey`, a production selector, a
successful-generation dependency, runtime epoch/refetch/event/publication
work, core/Cargo/oracle edits, or discovery/loading/analysis/query breadth.
