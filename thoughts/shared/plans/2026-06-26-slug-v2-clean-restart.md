...
Run only `WP-5-m1-retained-native-materialization-bridge`.

Edit only `app/slug_core_v2/src/runtime/repository_io.rs`. Add the retained
session `materialize_native` entrypoint with exact stable Spec versus
current-generation Transport/Materialization classification; batch accepted
validation into one deduplicated native observation pass; return the complete
preflight path epoch plus clean reusable repository results; preserve rootless
Local success; and expose only the private session operations required by the
next packet. Preserve existing root snapshots and post-I/O token checks. Stop
and replan rather than collapsing exact HTTP or Git staging failures into
Transport if they require the deferred PAX/GNU/link/special parity surface.
