...
`WP-5-m1-runtime-path-observation-producer-design`.

Audit the accepted structural observation schema and operational resolver,
the runtime module boundary, `WorkspaceRuntime`, and the retained
materialization instance/root authority. Freeze the smallest additive
outside-DICE producer for exact `Lstat`, `ReadLink`, `FileBytes`, and
`DirectoryEntries` demands in both Host and Materialization namespaces.

Specify the exact runtime error/result API, namespace-to-physical-root
authority, symlink escape and absolute-target semantics, non-UTF-8 handling,
stable I/O-kind mapping, directory-entry ordering, lstat metadata collection,
unknown-instance behavior, concurrency/locking boundary, file allowlist, and
focused unit/downstream evidence. Materialization paths may resolve through
symlinks outside their generation root; an existing instance authorizes the
namespace, not containment. Unknown instances must fail as typed runtime
infrastructure errors rather than fabricated path-operation observations.
Design the producer so the later retained materializer can supply instance
roots without a lock held across filesystem work or DICE computation.

Use the accepted 16-row Local lifecycle oracle plus pinned Bazel 9.2
`FileFunction`, `FileStateValue`, `FileContentsProxy`, and external-repository
dirtiness sources. Preserve exact OS-native path bytes and the existing
structural observation/equality contracts. Decide whether the first
implementation is one additive runtime file with a test fake for
materialization roots, or requires one minimal private owner seam; return
`REPLAN` rather than coupling it to the materializer correction or retry loop.

Do not edit Rust, Cargo, fixtures, oracle artifacts, plans, or routing during
this design packet; retain roots, perform repository materialization, inject
epochs, add retries/effects/events, activate source preparation, or broaden
discovery/package/loading/analysis/query. Stop on a public/cross-crate API,
inside-DICE filesystem work, canonicalized Local identity, containment checks
that reject Bazel-compatible symlink escapes, or a lock across observation.
