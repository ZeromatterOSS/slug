...
| M4: `cquery` | **active (configured `kind` accepted)** | projected label/Starlark-label output, ordered sets, Rust-native `filter`, full `some`, non-test `executables`, exported rule-class `kind`, typed evaluation/analysis exits, and retained configured rule capability | select the next exact vertical from retained configured state | `WP-6-m4-configured-query-successor-audit-6` |
...
[`WP-6-m4-configured-query-successor-audit-6`](./slug-v2-subplans/current-packet.md).
