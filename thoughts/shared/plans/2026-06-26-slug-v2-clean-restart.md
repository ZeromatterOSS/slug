...
| M7: command/ruleset breadth | **partial; selected-registry source observable scope correction active** | generated `repository_ctx.file` producer, demand-side route/request, atomic immutable-root application and tool-aware Bazel/Slug generated-source fixture are accepted through `3ac0a85b`; the external selected-definition source ownership design is sound | `dc85f527` incorrectly required successful rules_rust export while deferring the top-level repository-rule declaration/schema semantics that source evaluates first | design only `WP-4-5-6-7A-selected-registry-extension-bzl-source-observable-scope-correction-design`; preserve M7A -> M8 -> M7B |
...
[WP-4-5-6-7A-selected-registry-extension-bzl-source-observable-scope-correction-design](./slug-v2-subplans/current-packet.md).

Correct the selected-registry source owner's observable so a bounded focused
source proves same/mapped loads while actual rules_rust stops exactly at its
still-deferred repository-rule declaration boundary.
