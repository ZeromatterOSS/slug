...
Run only `WP-5-m1-nonroot-include-composition`.

Edit only `starlark-rust/starlark/src/eval.rs`,
`starlark-rust/starlark/src/eval/compiler/module.rs`, and
`app/slug_bzlmod_v2/src/module_eval.rs`. Add an opaque module-bound prepared
program that performs exact scope resolution and reusable bytecode compilation
without execution. Execute root and included prepared programs on one
`Evaluator`, with nested execution switching and restoring the target Module
and `DefInfo` without adding a synthetic module frame. Suspend automatic GC
only while a foreign heap is active, restore it at every nesting depth and
error boundary, then force GC and reread hidden roots independently in every
file Module.

Compose only supplied local/nonregistry files. Preserve exact raw-label keys,
per-file bindings, repeated stored Program/Module execution, shared compact
state containing no Starlark `Value`, current-file spans, Bazel-shaped
include-site diagnostics, and final builder validation. Match Bazel's horizon
behavior without adding a cycle visited set or diagnostic. Add the reviewed
upstream and Slug focused tests before implementation, then run the focused and
full `starlark`/`slug_bzlmod_v2` suites serially, formatting, diff/archive
checks, and the Bazel oracle replay. Do not edit Cargo, public consumers,
fixtures, expected artifacts, DICE, preparation, discovery, commands, server,
or lockfiles.
