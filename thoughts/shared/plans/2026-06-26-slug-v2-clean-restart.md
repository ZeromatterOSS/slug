...
`WP-5-m1-loading-pure-host-glob-owner-design`.

Freeze the smallest pure Host glob ownership boundary that consumes the
accepted typed no-follow directory observation, 13-row POSIX kind/symlink
oracle, dormant Bazel-internal string carrier, and four-row Linux raw-name
oracle. Keep pattern matching and matched-path dependency ownership pure and
incremental without activating loading callers or the dormant parser seam.
Separate this owner from transactional package-evaluation retry/suspension,
regular-or-special BUILD/`.bzl` byte acquisition, native Windows and
lone-surrogate evidence, and consumer publication. Stop before Rust until the
design has pinned source/API, implementation/evidence, and architecture/
orchestration terminal review.
