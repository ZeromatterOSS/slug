...
| M8: bootstrap | **active (developer graph)** | exact 33-package CLI boundary plus accepted Gates A-B; Gate C1 has 43 accepted Bazel targets; normal-RC minimal/RBE-profile builds succeed; private prime artifacts are usable but the repaired gate still fails closed | Add a fixed-enum one-prime parser-stage probe, then resume cache/RBE proof and the full 43-test gate | `WP-10-m8-bazel-buildbuddy-build-cache-prime-stage-probe-implementation` |
