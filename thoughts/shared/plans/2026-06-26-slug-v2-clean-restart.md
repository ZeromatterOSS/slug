...
Retry only corrected `WP-5-m1-terminal-event-execution-oracle`.

Replace the two retained `load-invalidation` commands with the accepted
nine-row Bazel 9.2 matrix for cold producer order, warm build/query nonreplay,
empty-query dependency edit, analysis-only follow-up, eligible analysis
failure, action diagnostic order, and post-failure reuse. Edit only its
`fixture.toml`, `expected/oracle.json`, `workspace/MODULE.bazel`,
`workspace/pkg/{BUILD.bazel,defs.bzl,message.bzl}`, and only the direct parser
regression in `tests/v2_oracle/test_v2_oracle.py`. That regression must assert
the nine ordered command names and the two row-4 mutations. Add no file,
symlink, scaffold, schema, other harness/fixture, or Slug production change;
stop above 350 net fixture lines or on unstable/cross-channel ordering.
