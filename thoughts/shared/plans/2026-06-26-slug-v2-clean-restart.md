...
Implement the combined accepted contracts in the owner plan's corrected
state-machine, semantic-error/integrated-evidence, and evidence-script sections.
The exact allowlist is new
`app/slug_workspace_v2/src/path_resolution.rs` plus public reexports in
`app/slug_workspace_v2/src/lib.rs`; add no dependency and edit no accepted
observation owner.

Proceed in the frozen test-first checkpoints. Implement the operational schemas
and pure frame machine, then pass every exact cumulative observation-backed
chain/link/provenance/error script, then add the semantic byte projection and
retained-engine selector/projection/counter pruning sequences. Do not substitute
helper calls or direct equality for integrated evidence. Run full
`slug_workspace_v2` tests/doctests, format, diff, exact allowlist, and archive
guards. Stop on any schema/equality/script/count drift, direct IO, recursive
resolver key, flattened frame loop, fabricated Symlink wrong-kind, physical
identity in byte errors, missing outer resolver-error context, or
consumer/runtime/repository/retry/publication expansion.
