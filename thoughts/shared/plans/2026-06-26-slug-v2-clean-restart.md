...
| M3: `query` | **active (typed `attr` projection design)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance | `attr` needs a complete typed attribute-string projection; rank order, external repositories, patterns, and remaining formats still need representation | `WP-4-8-m3-attr-typed-attribute-string-design` |
...
[`WP-4-8-m3-attr-typed-attribute-string-design`](./slug-v2-subplans/current-packet.md).
