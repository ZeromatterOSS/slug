...
| M7: command/ruleset breadth | **active (built-in bazel_tools typed source-kind design)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, plus the accepted POSIX executable FileWrite `run` vertical with client-only launch | freeze the typed source-kind boundary before retrying immutable canonical `@@bazel_tools` routing/source ownership before its test package and external closure | `WP-5-builtin-bazel-tools-typed-source-kind-design` |
...
[`WP-5-builtin-bazel-tools-typed-source-kind-design`](./slug-v2-subplans/current-packet.md).
