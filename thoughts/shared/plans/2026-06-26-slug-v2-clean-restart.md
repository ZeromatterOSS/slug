...
[Rule `attr.label` computed-default declaration-retention implementation
active](./slug-v2-subplans/current-packet.md).

Commit `db9e693e7` terminally accepts generic rule-initializer declaration
retention at 21/131/152 gross production/proof/total Rust additions. Loading
passes 535 tests with one ignored plus every integration target, query passes
55/55, and CLI/fmt/diff/archive/daemon gates pass. Authenticated replay clears
all selected initializer declarations and next stops at selected rules_cc 0.2.4
`cc_shared_library.bzl`: `_def_parser` uses an `attr.label` Starlark-function
default.

The complete 400-entry archive scan finds one callback, three descriptor sites
and four rule consumers. The audit accepts only generic `attr.label` function-
default declaration retention: keep the callable and parameter metadata in the
existing frozen-module lifetime, project actual computed attributes into a
compact schema-indexed sparse slice, and reject every consuming target call
before package mutation. Callback execution, label context, configurable
precomputation, result coercion, other constructors and configured C++ behavior
remain deferred. Independent packet and retained-representation review returns
`ACCEPT`; implementation is active only within the frozen manifest.
