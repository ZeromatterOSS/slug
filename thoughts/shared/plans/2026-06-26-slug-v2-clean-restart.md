...
| M2: analysis graph | **active** | recursive configured analysis, bounded root cquery in `135b0567`, implicit empty-default normalization in `7c6eeae5`, internal string build-setting transitions in `dfc1705e`, root-only integrated first-compatible toolchain selection/context in `1533569f`, command-retained recursive action closure in `afd2a606`, four-domain action-query identity evidence in `f00e99db`, the complete target-configuration input ledger, the 17-class/341-option metadata/cache grammar in `b043d54d`, accepted exact Java/Guava renderer authority, forced-sharding structural semantic-identity evidence, and exact family/16-enum source anchors | retry 6 must preserve retry-5 facts while freezing unadmitted-list, runs-attachment, and Dotted full-input identity distinctions before mechanical descriptor attachment and Rust | run `WP-6-m2-pure-native-family-byte-contract-ledger-retry-6` |
...
`WP-6-m2-pure-native-family-byte-contract-ledger-retry-6`.
