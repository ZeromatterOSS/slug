...
Run only `WP-5-m1-workspace-directory-owner-lift`.

Edit exactly `app/slug_workspace_v2/Cargo.toml`,
`app/slug_workspace_v2/src/lib.rs`, `app/slug_loading_v2/src/keys.rs`,
`app/slug_loading_v2/src/bzl_module.rs`, and `Cargo.lock`.

Move the six accepted directory observation types and every existing method,
`Display`, `InjectedKey`, and `Key` implementation body-for-body into
`slug_workspace_v2`. Add its direct workspace `compact_str` dependency and
regenerate only the corresponding lockfile dependency list. Replace the old
loading definitions with real public re-exports so every
`slug_loading_v2::keys::WorkspaceDirectory*` path and the DICE key `TypeId`
remain unchanged. Add focused owner tests in the existing workspace `lib.rs`
test module for compact sorting, structural equality, exact present/absent/
read-error propagation, and missing-directory defaulting.

Do not edit consumer imports or core injection. Do not add a second snapshot,
generation, filesystem read, cache, containment policy, manual lock, or reverse
workspace dependency. Validate serially with the full workspace/loading
suites, affected core/server/query/analysis crates, formatting, body
comparison, diff, and archive checks. Stop on public-path/TypeId drift,
semantic or request-commit-order change, an unexpected root manifest change,
or any required file outside the five-file allowlist. Package lookup remains
deferred.
