...
`WP-5-m1-loading-host-glob-segment-matcher-oracle-design`.

Freeze the smallest oracle-only extension proving the accepted candidate
owner's remaining simple-segment matcher semantics: bare `*` including a
leading-dot name, `*.txt` excluding it, an explicit-dot wildcard including
it, and multiple non-adjacent stars in one segment. Reuse a maintainable
pinned Bazel 9.2 fixture, keep the raw-name and typed-dirent oracles unchanged,
and do not edit Rust. After the oracle accepts, implement only the private
three-file `WP-5-m1-loading-pure-host-glob-segment-candidates-owner`; keep
package-boundary projection, multi-segment/`**` traversal, parser activation,
transactional evaluator retry, and every consumer separate.
