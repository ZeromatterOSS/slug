...
Run only
`WP-5-m1-root-module-composed-evaluator-event-design`.

Design only the bzlmod-local composed root-MODULE evaluator and its event
producer against the accepted eight-row oracle. Freeze complete include
preflight/preparation, one inline evaluator/thread with per-file binding
isolation and repeated-call execution, one marker-conditional local
`EventBatch` outside semantic equality, exact DICE key ownership, and bounded
retained-engine tests. Reconcile or retire the current exported separate-file
evaluation key before naming an implementation allowlist. Do not edit Rust,
Cargo, fixtures, expected output, harness, runtime/command/server paths,
loading/analysis/nonroot evaluators, source preparation, or discovery.
