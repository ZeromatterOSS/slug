...
Implement only `WP-5-m1-immutable-materialization-instance-authority`.

Edit exactly `app/slug_bzlmod_v2/src/source_preparation.rs`,
`app/slug_core_v2/src/runtime/repository_io.rs`, and
`app/slug_bzlmod_v2/tests/source_preparation_dice.rs`. Add
`observation_instance: PathObservationInstanceId` only to immutable
`RepositoryIoOutcome` and `RepositoryMaterialization` values. The installed
`LocalRepositoryIo` owns one mutex-protected allocator/root-retention state:
allocate a nonzero, checked, never-reused instance and retain its `TempDir` in
one synchronous critical section after materialization returns.

Keep successful materialization independent of
`RepositoryMaterializationGeneration`. Keep semantic materialization equality
exactly Local repository/spec/root or Immutable repository/spec/source identity;
exclude immutable generation root and observation instance. Add exact
zero/MAX no-wrap allocator tests, sequential/concurrent unique-root-instance
evidence, field-complete direct equality, and a retained injected-key
downstream counter with `A/i1 → A/i2 → B → A` counts `1,1,2,3`.

Do not migrate immutable `std::fs::read`, allocate globally or inside DICE,
change Host/local semantics, add observation producers or successful-refetch
retry ownership, or edit Cargo, workspace observation types, fixtures, or
discovery/loading/analysis/query code.
