...
| M7: command/ruleset breadth | **partial; terminal source-carrier visibility implementation active** | accepted M7A repository-observation chain through `597df31b`; `b3eba6df` accepts the private terminal source-observation owner; `3d865737` selects the same-crate handoff | the terminal matching-family surface is same-file private; generated package policy/lookup/source/load and public publication remain separate later owners | implement only `WP-6-7A-host-root-apparent-repository-source-observation-observation-carrier-visibility-implementation`, Linux-first; then audit the generated package/publication frontier; preserve M7A -> M8 -> M7B |
...
[WP-6-7A-host-root-apparent-repository-source-observation-observation-carrier-visibility-implementation](./slug-v2-subplans/current-packet.md).
