...
Run only
`WP-5-m1-path-observation-not-a-link-schema`.

Add the fieldless `PathObservationError::NotALink` discriminator and exact
equality/propagation evidence in:
...
Prove direct distinction from every existing observation error, exact
ReadLink-demand propagation through the resolver, semantic byte equality and
transition invalidation, and complete repository-source projection. Do not
edit reexports or production source preparation, add fields or diagnostics,
restore the producer, change Cargo, or wire runtime/DICE/materialization.
After independent acceptance, implement the already-frozen four-file native
producer as a separate packet.
