...
| M7: command/ruleset breadth | **active (selected extension evaluation-input design)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite run, complete Host discovered leaves, normalized/effective overrides, exact shared versions, selected graph/routes/extension mappings, hidden definition-load requests in `d0d7bde7`, and loading-owned heap-independent definitions in `bf2c36e9` | design the Bzlmod-owned ordered root module/tag input projection before schema composition or execution | WP-5-host-selected-extension-evaluation-input-requests-design |
...
[WP-5-host-selected-extension-evaluation-input-requests-design](./slug-v2-subplans/current-packet.md).
