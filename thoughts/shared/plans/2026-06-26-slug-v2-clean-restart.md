...
Design only `WP-5-m1-host-root-patch-path-projection-migration`.

Audit the current root-patch read in `slug_bzlmod_v2/src/source_preparation.rs`
and its exact DICE callers/tests. Freeze the smallest Host-only consumer
boundary that replaces only its `WorkspaceRawFileKey` read with
`PathFileBytesKey(PathObservationNamespace::Host, ...)`, carries outer
`PathOutcome` through `ModuleSourcePreparationKey` and private helpers, and
retains typed patch-path failures.

Name the exact files, public/private type changes, equality and Need behavior,
cumulative observation scripts, Missing/error handling, and same-engine
A→B→A evidence. Keep immutable-instance reads, local registries, repository
materialization, directory/package/loading/analysis/query propagation, retry
ownership, producer capture, events, and publication out of scope. Require
independent DICE/consumer and pinned-source review before scheduling Rust.
