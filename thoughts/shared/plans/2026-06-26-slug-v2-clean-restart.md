...
| M2: analysis graph | **active** | recursive configured analysis, bounded root cquery in `135b0567`, implicit empty-default normalization in `7c6eeae5`, internal string build-setting transitions in `dfc1705e`, root-only integrated first-compatible toolchain selection/context in `1533569f`, command-retained recursive action closure in `afd2a606`, and four-domain action-query identity evidence in `f00e99db` | Bazel 9.2 has 17 native fragments/341 cache-key options plus graph-derived inputs; their complete ownership ledger is the first of four serial action-identity prerequisites | specify `WP-6-m2-bazel-9-target-configuration-input-ledger` |
...
`WP-6-m2-bazel-9-target-configuration-input-ledger`.
