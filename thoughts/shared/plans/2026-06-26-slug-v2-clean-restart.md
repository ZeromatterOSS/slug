...
Latest M1 accepted evidence: the corrected private Host root-module-file owner
passed three terminal latest-diff reviews against exact Bazel 9.2.0 source.
Focused 10, full bzlmod 79+214, loading 54, core 102+13, zero doctests, and all
20 GNU-Windows executable links pass. The value owns only evaluated root
semantics, compact overrides, and set-equivalent canonical relative
module-file paths; breadth-first dependency reuse remains distinct from
per-occurrence validation, expansion, and inline execution. Lockfile,
environment/mode policy, resolution, mappings, public consumers, and
activation remain deferred.

### Current packet

Design only `WP-5-m1-host-visible-lockfile-boundary-design`.

Follow exact Bazel 9.2.0 `BazelLockFileFunction` and `RegistryFunction`
ownership after the accepted root-module-file owner. Registry must request the
separate lockfile owner even in Off mode; parsed/empty lockfile state remains
distinct from whether a consumer uses recorded hashes. Do not inherit the
legacy `VisibleLockfileRead::Ignored` shortcut, add Rust or Cargo changes, or
pull final repository mappings forward from the post-selection graph owner.
