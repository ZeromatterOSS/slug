...
Implement only
`WP-5-m1-bazel-lockfile-v28-live-cutover`.

Atomically replace the incomplete live parse/render surface with the accepted
private v28 owner in only `app/slug_bzlmod_v2/src/lockfile_v28.rs`,
`src/lockfile.rs`, `src/lib.rs`, and `tests/lockfile.rs`. Preserve the accepted
typed value, errors, equality, streaming reader, and canonical renderer. Do not
add Host/DICE ownership, a second schema, a generic JSON tree, new dependencies,
