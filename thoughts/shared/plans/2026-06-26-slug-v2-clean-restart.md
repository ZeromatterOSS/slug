...
| M3: `query` | **active (`attr` isolated-oracle generation)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; pinned Bazel 9.2 and the reviewed 18-lane/170-pair matrix close the finite observable attr schema; the five-file isolated fixture has no Rust projection or protected Slug consumer | generate and clean-replay the bounded isolated Bazel-only attr fixture; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-isolated-observable-candidate-oracle-generation` |
...
[`WP-4-8-m3-attr-isolated-observable-candidate-oracle-generation`](./slug-v2-subplans/current-packet.md).
