...
Design only `WP-5-m1-root-raw-host-migration-design`.

Freeze either parallel typed Host root/policy/file keys while legacy
production keys remain, or the complete propagation allowlist through
`module_eval.rs`, `source_preparation.rs`, `registry_dice.rs`, exports, and
every live `RootModuleFilesKey`/`RootModuleGraphKey` consumer. No Host Need may
be stringified into `RegistryFileError`. Name exact tests, forbidden scans,
implementation packets, and stop gates before Rust; do not combine this design
with nonroot discovery.
