...
| M7: command/ruleset breadth | **active (run handoff design)** | early command/ruleset scaffolding plus accepted query/cquery/aquery and bounded FileWrite execution | design the first executable FileWrite `run` vertical without inventing runfiles or direct-local build actions | `WP-8-m7-filewrite-run-handoff-design` |
...
[`WP-8-m7-filewrite-run-handoff-design`](./slug-v2-subplans/current-packet.md).
