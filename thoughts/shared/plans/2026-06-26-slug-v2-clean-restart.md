...
| M1: one semantic spine | **active** | Host loading observations and dormant typed root-loading/root-analysis/build/query command boundaries, including accepted query Host migration; full evidence is in the Stage 5 owner plan | preactivation Host gate, opaque output envelope, and later activation; external mapping, native Windows glob ordering, registry transports, and discovery composition remain | design the preactivation Host gate |
...
`WP-5-m1-preactivation-host-gate-design`.
