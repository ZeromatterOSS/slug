...
| M7: command/ruleset breadth | **active (built-in module injection design)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite `run`, and the immutable canonical `@@bazel_tools` route/seven-file source owner | own Bazel 9.2's hidden built-in module graph and contextual mappings before package/source consumer dispatch | `WP-5-builtin-bazel-tools-module-injection-design` |
...
[`WP-5-builtin-bazel-tools-module-injection-design`](./slug-v2-subplans/current-packet.md).
