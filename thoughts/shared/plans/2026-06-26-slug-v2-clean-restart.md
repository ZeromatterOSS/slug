...
[Implement complete provider-constrained dependencies](./slug-v2-subplans/current-packet.md).

Implement
`WP-6-7A-provider-constrained-dependency-implementation-r1`. Bazel 9.2 exposes
`providers` on exactly five default-enabled label-bearing constructors. Retain
their existing canonical provider DNF on the final package `AttributeSchema`,
bind the three missing dictionary forms, admit ordinary target invocation, and
feed every configured label edge into the existing actual-provider validator.
Provider-policy-only changes must participate in package equality and DICE
invalidation. Aspect attachment remains independently fail-closed; this adds
no configured aspect, `CcInfo`, rules_rust, `cc_common`, `cc_internal`, parser,
native/C++ rule or action special case. R1 architecture review required the
alias-to-file gap to remain explicitly deferred and the permanent oracle to be
fully allowlisted. Corrected R2 makes only those changes and focused rereview
returns `ACCEPT`. Implement its frozen allowlists, caps, proofs and stops.
