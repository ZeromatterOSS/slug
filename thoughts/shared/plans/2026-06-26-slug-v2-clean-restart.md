...
| M7: command/ruleset breadth | **active (selected registry RepoSpec owner implementation)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite run, complete Host discovered leaves, normalized/effective overrides, exact shared versions, accepted callerless selected graph in 216a0be8, and accepted selected RepoSpec design in 1b4b4996 | implement the callerless selected-only registry source.json-to-RepoSpec owner without routes/materialization/consumers | WP-5-host-selected-registry-repo-spec-owner-implementation |
...
[WP-5-host-selected-registry-repo-spec-owner-implementation](./slug-v2-subplans/current-packet.md).
