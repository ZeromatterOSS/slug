...
Implement only the accepted pure
`WP-5-m1-host-registry-function` owner.

Run a terminal exact-scope pre-gate against the accepted root-free
RegistryFunction contract, then add the new private `host_registry.rs`, its
private module declaration, and only prerequisite-owner edits explicitly
required by that contract. Prove exact mode/vendor/Refresh/visible/mirror
ordering, complete-only semantic equality and pruning, all scheme/mode cells,
typed errors and Needs, original/resolved identity, and the absence of root,
IO, request-generation, mapping, source-preparation, write, or activation
edges. Do not add a public API, dependency, fetch, file owner, IO bridge,
command/server/core consumer, clock lifecycle, or production injection.
