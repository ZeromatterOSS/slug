...
[Private proto-common declaration facade audited; one-owner implementation
selected](./slug-v2-subplans/current-packet.md).

Commit `f7c365234` terminally accepts bounded direct-external-Label
`repository_ctx.read` at 98/265/363 gross production/proof/total Rust
additions. Full loading and query gates and the CLI build pass. The
authenticated replay clears the read and generated apple repository, then
stops while recursively loading protobuf 33.4 at
`@@protobuf+//bazel/private:native.bzl:3` because
`proto_common_do_not_use` is not predeclared.

The docs-only audit returns `ACCEPT` for exactly Bazel 9.2's default injected
one-field `struct`: the private top-level exists in ordinary and Bzlmod `.bzl`
environments, exposes only
`INCOMPATIBLE_ENABLE_PROTO_TOOLCHAIN_RESOLUTION = True`, and remains absent
from direct BUILD globals. Add it only in the existing loading-globals owner.
Native proto methods/providers, nondefault flags and all configured proto,
toolchain and authenticity semantics remain deferred. Run only
`WP-4-7A-proto-common-predeclared-facade-implementation-r1` under its one-file
allowlist and caps.
