...
| M3: `query` | **active (`attr` executable reconstruction retry 2)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; the corrected 18-lane/165-pair manifest at SHA-256 `99b772e6…42300d`, five-file layout, transition allowlist, computed timeout, package-license construction, and six-row source fixes remain viable | correction rereview found a second material producer-identity miss: the generated-file owner must be `l01_generated_owner`, not `l01_generated_nonrule_owner`; recreate one executable representation with a direct source assertion before replay | `WP-4-8-m3-attr-five-source-executable-reconstruction-retry-2` |
...
[`WP-4-8-m3-attr-five-source-executable-reconstruction-retry-2`](./slug-v2-subplans/current-packet.md).
