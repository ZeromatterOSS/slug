...
| M4: `cquery` | **partial (configured-node substrate accepted)** | projected label/Starlark-label output, direct retained-state functions, exact toolchain plus delegation graph evidence, one Need-aware analysis key, structural/null node identity, classified ordered edges, and one retained configured-node result | activate the fixture-admitted root alias/source/generated/filegroup/package-group nodes before traversal breadth | `WP-6-m2d-root-delegating-native-node-activation` |
...
[`WP-6-m2d-root-delegating-native-node-activation`](./slug-v2-subplans/current-packet.md).
