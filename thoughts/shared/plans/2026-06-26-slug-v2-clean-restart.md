...
Run only
`WP-5-m1-runtime-attempt-effect-sidecar`.

Add only the dependency-bottom `CaptureEvaluationEvents` marker, the private
core command/serial-attempt effect sidecar, its exact-version closure-selection
tests, and the required core dependencies/module privacy change. Use the exact
four-file allowlist in the owning plan. Do not add a runtime field, producer,
retry loop, sink, publication, command/server/CLI wiring, path/repository
operation, source-preparation/discovery behavior, fixture, or oracle.
