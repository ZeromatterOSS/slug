...
Implement only `WP-5-m1-repository-source-local-host-read-migration`.

Edit exactly `app/slug_bzlmod_v2/src/source_preparation.rs`,
`app/slug_bzlmod_v2/src/lib.rs`, and
`app/slug_bzlmod_v2/tests/source_preparation_dice.rs`. Replace only the Local
`WorkspaceRawFileKey` edge with Host `ResolvedPathKey` plus resolved-real-path
FileBytes. Accept RegularFile and SpecialFile, return semantic Absent for
resolved Missing, and keep the immutable `std::fs::read` branch operationally
unchanged.

Change `RepositorySourceFileKey` to the accepted direct `PathResult`, add and
reexport the cheap-`Dupe` semantic `RepositorySourceFileError`, and propagate
Need unchanged into `ModuleSourcePreparationKey`. Retain only shared
repository-relative paths, shared messages/materialization failures, operation
and typed observation state, wrong-kind, or cycle/expansion class. Erase every
namespace, absolute source/real path, demand path, symlink route/target, chain,
root/instance, and generation from semantic equality.

Add the exact cumulative, regular/special/symlink/Missing/wrong-kind/error,
complete-only equality/validity, pruning, and retained lifecycle evidence from
the accepted owner contract. Add no `PathFileBytesKey`, new key, direct IO,
immutable instance/read migration, core/runtime/discovery/package/loading
breadth, dependency, Cargo, fixture, or oracle edit.
