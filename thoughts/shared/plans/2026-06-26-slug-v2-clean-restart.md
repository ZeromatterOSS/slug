...
| M4: `cquery` | **active (configured `some` accepted)** | projected label and exact `str(target.label)` output, C0/C1/C0 restoration, ordered configured sets, Rust-native `filter`, full `some(expr[, count])`, and typed evaluation exit 1 | traversal and configured metadata/provider boundaries remain incomplete | `WP-6-m4-configured-query-successor-audit-4` |
...
[`WP-6-m4-configured-query-successor-audit-4`](./slug-v2-subplans/current-packet.md).
