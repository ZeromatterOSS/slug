...
| M4: `cquery` | **partial (root delegating nodes accepted)** | projected label/Starlark-label output, direct retained-state functions, exact toolchain plus delegation graph evidence, one Need-aware analysis key/result, structural/null node identity, classified ordered edges, and fixture-proven root alias/source/generated/package-group activation | preserve complete analysis errors outside success-only DICE validity so command-side activation selection can publish the intended terminal | `WP-6-m2e-analysis-error-activation-sidecar-prerequisite` |
...
[`WP-6-m2e-analysis-error-activation-sidecar-prerequisite`](./slug-v2-subplans/current-packet.md).
