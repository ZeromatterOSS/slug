...
Run only `WP-5-m1-nonroot-repository-package-lookup-design-rereview`.

This is a read-only design packet. Reinspect the prior package-lookup `REPLAN`,
the neutral workspace-directory owner, the accepted discovery-boundary and
package-policy oracles, pinned Bazel 9.2 package/ignored-directory/deleted-option
sources, and live bzlmod/core/command request inputs.

Freeze the smallest Bazel-complete ownership sequence for normalized external
repository/package identity, horizon-wide deduplicated preflight, BUILD.bazel
priority/fallback and selected-name equality, symlink-aware markers,
`--deleted_packages`, repository `.bazelignore`, and `REPO.bazel`
`ignore_directories()`. Specify stable keys/values, DICE dependencies,
request-local policy transport, parsing/error boundaries, source-order adapter
diagnostics, exact implementation/test allowlists, and whether package lookup
can now be one bounded implementation or requires a smaller prerequisite.

Stop and return `REPLAN` if any request input, repository-root observation,
ignore parser, symlink state, or ownership edge remains missing. Do not edit
Rust, Cargo, tests, fixtures, expected artifacts, commands, server, or lockfile.
