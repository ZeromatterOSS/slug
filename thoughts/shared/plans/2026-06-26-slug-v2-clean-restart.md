...
| M8: bootstrap | **active (developer graph)** | exact 33-package CLI boundary plus accepted Gates A-B; Gate C1 has 43 accepted Bazel targets; repaired live prime evidence reaches both `BEP_READY` and `EXECUTION_READY`, and the final prime probe is accepted offline | Deliver one output/prime-semantics result, then resume replay/cache/RBE proof and the full 43-test gate | `WP-10-m8-bazel-buildbuddy-build-cache-prime-output-semantics-probe-transported-live-evidence` |
