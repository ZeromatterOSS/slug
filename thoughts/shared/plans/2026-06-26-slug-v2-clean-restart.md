...
[Implement corrected repository declaration documentation-category closure](./slug-v2-subplans/current-packet.md).
...
Implementation R1 returned `REPLAN`: starlark-rust `Option<&str>` rejects
explicit `None`, exposing the same latent gap in `tag_class` and
`module_extension`. Review only the bounded correction that routes those two
siblings and `repository_rule` through one typed, nonretaining string/None
union binding. Independent correction rereview returns `ACCEPT`; resume only
implementation R2.
