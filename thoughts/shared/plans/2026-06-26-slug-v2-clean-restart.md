...
[WP-4-5-7A-canonical-source-policy-convergence-implementation-r2](./slug-v2-subplans/current-packet.md).
...
Activate only
`WP-4-5-7A-canonical-source-policy-convergence-implementation-r2`. Generalize
...
wrappers. The corrected twelve-file allowlist includes one crate-private
built-in byte-`Arc` accessor and the canonical load-route proof so catalog bytes
remain zero-copy and wrapper deletion is discriminating. Stage B loading/package
adaptation remains read-only. Bazel 9 BCR Starlark owns all rules including
