...
Run only `WP-5-m1-nonroot-include-dispatcher-design`.

This is a read-only design packet. Inspect the repo-local evaluator runtime,
native callback ABI, `AnyLifetime`, module compiler/bytecode ownership, and the
private nonroot evaluator. Design an evaluator-owned dispatcher whose lifetime
is coupled to `Evaluator<'v>` and whose native `include()` surface passes only
an exact raw key or opaque index. The dispatcher must retrieve and execute the
module-bound prepared program without placing it in `Evaluator.extra`, unsafe
lifetime erasure, self-referential storage, or a second evaluator.

Freeze the smallest reviewed implementation allowlist, including the evaluator
runtime definition only if required. Preserve closure-wide preparation before
effects, one call stack, per-file Modules and roots, scoped foreign-heap GC
suspension, success/error restoration, repeated exact-key execution,
Bazel-shaped include-site diagnostics, and Bazel's nonterminating cycle
horizon. Keep semantic state in `extra` compact and free of Starlark `Value`.
Do not edit Rust, Cargo, fixtures, expected artifacts, DICE, preparation,
