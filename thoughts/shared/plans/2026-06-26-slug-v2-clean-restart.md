...
Run only
`WP-5-m1-runtime-path-observation-native-producer`.

Implement the private outside-DICE producer in exactly:
...
Use target-Unix workspace `nix` only for `libc` and errno. Preserve the frozen
compact authority, complete-batch preflight, exact Lstat/ReadLink/FileBytes/
DirectoryEntries results, platform metadata, operation-level failures, and
lifecycle evidence. On Unix, retain one raw directory handle and partial names
across `readdir` EINTR/EIO with exact iterator/close precedence and contained
unsafe. On Windows, use the accepted checked native reparse-buffer parser,
UTF-16 long-path/no-follow handle seam, exact NTFS/LX/mount/ProjFS/unknown-tag
behavior, output normalization, lstat kind order, metadata formulas, and raw
errors.

Keep the module private and callerless. Do not edit workspace schemas, wire
DICE/runtime materialization/retries/publication, add another dependency, use
`std::fs::ReadDir`/`nix::dir::Dir`, or broaden the allowlist. Stop rather than
retain partial behavior if the exact four-file packet cannot be completed.
