...
Run only `WP-5-m1-demand-driven-path-observation-request-boundary-design`.

This is a read-only architecture packet. Inspect the retained
`WorkspaceRuntime` request flow, DICE transaction boundary, all three current
workspace file/directory snapshots and their consumers, immutable repository
materialization generation, and the accepted 16-row repository path-state
oracle. Design one typed missing-path demand protocol whose observations occur
outside DICE and whose retries restart the whole semantic request.

Specify the exact demand identity and payload for `lstat`, `readlink`, regular
file bytes, and directory entries; closure expansion for relative and absolute
symlinks; immutable per-epoch observation ownership; request inputs and
external request revision across retries; event/result buffering; typed
failure propagation; DICE dependencies and semantic equality; and convergence
by strictly growing unique demanded raw/logical paths. If more than one
observation batch can otherwise mix filesystem states, require a filesystem
revision or watch token and restart the entire request when it changes. No
partial semantic result or user-visible event may escape a failed epoch.

The design must decide whether the demand owner replaces all three existing
snapshots directly or requires one bounded wrapper migration, but the old and
new observation owners must not coexist terminally. Preserve the finding that
immutable reads depend operationally on
`RepositoryMaterializationGenerationKey` while generation roots stay out of
semantic equality. Name exact types, owners, dependency direction, error
classification, migration allowlist, focused tests, retained-daemon oracle
replay, platform limits, and hot-path allocation/performance expectations.

Stop and `REPLAN` on filesystem reads from inside DICE, generation-only lazy
reads, duplicate terminal snapshots, arbitrary retry/traversal caps,
repository-root escape rejection, request-visible partial execution, or a
design without a mutation-consistency and convergence argument. Do not edit
Rust, Cargo, tests, fixtures, expected artifacts, commands, server behavior,
