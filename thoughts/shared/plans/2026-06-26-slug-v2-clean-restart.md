...
Run only
`WP-5-m1-runtime-retained-session-kernel`.

Edit only `app/slug_core_v2/src/runtime/repository_io.rs`. Add the callerless
single-lease retained-session owner, owner-held provisional and append-only
accepted roots, burned nonzero IDs, full-request accepted cache, exact
validation dirtiness, logical Local result, cumulative complete epochs, and
explicit token-checked accept/discard from checkpoint 2 of **Stage 5 retained
materializer implementation checkpoint split**. Use only scripted
materialization/observation evidence; do not add native observation or a
runtime field.

Add no dependency, public API, DICE key, source-preparation change, sidecar,
producer, retry/publication, command activation, fixture, or discovery
behavior. Preserve the accepted HTTP/Git materialization implementation
byte-identically.
