...
Run only
`WP-5-m1-runtime-attempt-effect-sidecar-checkpoint-design`.

Reconcile the accepted demand-retry event owner and exact-version activation
closure with the live retained materializer/native observer. Freeze the
smallest serial implementation packets for the command-owned attempt/effect
sidecar, then event and demand producers. Do not edit Rust, Cargo, fixtures,
oracles, commands, server paths, source preparation, or discovery behavior.
