...
[WP-4-5-7A-target-platform-and-exec-configuration-prerequisite](./slug-v2-subplans/current-packet.md).
...
Activate only
`WP-4-5-7A-target-platform-and-exec-configuration-prerequisite`. Add typed
target-platform and selected-platform exec projections over the existing
configuration rows, port the two required pinned `@bazel_tools` files
verbatim, publish configured platform/target-platform DICE facts through the
existing alias recursion, and activate constraint-category matching in the
sole condition key. Toolchain selection and provider/implementation analysis
remain the next packet and categories 5/6.
