...
Design only `WP-5-m1-root-raw-host-registry-boundary-redesign`.

Preserve the reviewed dormant parallel Host spine and audit only the live
private registry policy/transport closure. Freeze either an exact crate-private
policy-free remote bridge or a bounded shared-module move, including the exact
allowlist, unchanged-legacy tests, a local-Host forbidden scan, and a Host-only
remote error schema that cannot contain legacy root/local erasure. Resubmit the
complete serial root/raw Host contract before Rust; do not implement the
otherwise viable private byte leaf in isolation or activate discovery.
