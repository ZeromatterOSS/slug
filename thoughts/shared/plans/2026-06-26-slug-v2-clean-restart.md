...
| M4: `cquery` | **active (non-test `executables` accepted)** | projected label/Starlark-label output, ordered sets, Rust-native `filter`, full `some`, non-test `executables`, typed evaluation/analysis exits, retained configured rule capability, and executable-rule analysis | select the next exact vertical from retained configured state | `WP-6-m4-configured-query-successor-audit-5` |
...
[`WP-6-m4-configured-query-successor-audit-5`](./slug-v2-subplans/current-packet.md).
