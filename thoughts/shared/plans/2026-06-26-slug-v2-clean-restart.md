...
| M7: command/ruleset breadth | **partial; nonregistry module-closure observation implementation active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, `cb5073e0`'s immutable action-owner row, `b7390392`'s rules_rust 0.73 evidence, `a3efa1b7`'s root MODULE-files epoch, `3d174006`'s effective-override epoch, `ae8aa35e`'s request/materialization epoch, `12f68983`'s repository-source epoch, `b08b7f2e`'s REPO-file epoch, `754e7619`'s repository-ignore epoch, and `18166691`'s package-preflight epoch are accepted | the nonregistry closure remains carrierless before discovery; registry preparation remains a separate later frontier | implement accepted design `0ffa79cd`, then preserve M7A -> M8 -> M7B |
...
[WP-6-7A-host-nonregistry-module-closure-observation-implementation](./slug-v2-subplans/current-packet.md).
