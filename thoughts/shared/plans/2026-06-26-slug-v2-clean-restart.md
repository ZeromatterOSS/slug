...
Run only
`WP-5-m1-runtime-retained-native-owner-bridge`.

Add only the `pub(super)` retained-root/native-observer entrypoint in
`app/slug_core_v2/src/runtime/path_observation.rs`, the minimum call bridge in
`app/slug_core_v2/src/runtime/repository_io.rs`, and one private
`Arc<RepositoryMaterializer>` field plus initialization in
`app/slug_core_v2/src/runtime/dice.rs`.

Add no DICE key, injection, retry, sidecar, command activation, public API,
dependency, source-preparation change, fixture, or discovery behavior.
