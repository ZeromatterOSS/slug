...
Run only
`WP-5-m1-runtime-http-archive-captured-ustar-subset`.

Edit only `app/slug_core_v2/src/runtime/repository_io.rs`. Replace only the
`http_archive` mutable caller-path/external extraction path with one private
captured artifact and a byte-oriented USTAR parser/extractor for regular files
and directories. Leave `materialize_git` and its entire existing
stdout/NamedTempFile/`extract_tar` path byte-identical.

Preserve exact supported-surface order: validate stripping/URL and save
checksum validation; create unpublished output root then capture artifact;
open/read/hash the caller once; report capture before saved malformed checksum
and valid mismatch before parsing; inspect, normalize/select, then extract from
the private bytes only. Keep the accepted Spec/Transport/Materialization
classification.

Parse raw USTAR name plus prefix, octal size, declared payload and 512-byte
padding. Accept regular type NUL/`0` and directories from type `5` or a trailing
slash; type `5` without a slash is still a directory. Match Bazel's physical
EOF and checksum-only-corruption acceptance, but reject malformed numeric
fields and truncated declared payload/padding. Explicitly reject PAX/GNU
metadata, links, and special types without partial output.

Normalize raw member and prefix components, make absolute paths relative, then
apply exact component prefix, containment, and archive-order extraction.
Prefix-root entries mark found but skip output; outside-prefix entries skip
before type/containment checks; absent prefix fails after the stream; later
normalized collisions win. Cover raw non-UTF-8 names, USTAR prefix assembly,
absolute and parent paths, sibling prefixes, outside unsupported/escape rows,
implicit parents, directory/file collisions, capture mutation/deletion,
dual-stage errors, no-terminator/checksum acceptance, and every rejected
format/type.

Run focused and full core tests, GNU-Windows no-run compilation, formatting,
diff, archive, exact one-file scope, and forbidden-reference scans. Return
`REPLAN` on any Git diff, dependency/public/session/runtime/path/DICE change,
lossy subprocess listing, caller-path reopen, component-count stripping, PAX/
GNU/link/special acceptance, or missing discriminating row.
