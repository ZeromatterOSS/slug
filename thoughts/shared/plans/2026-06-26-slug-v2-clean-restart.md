...
`WP-5-m1-runtime-path-observation-native-producer-checkpoint-split`.

Preserve the fully accepted final producer contract and four-file terminal
boundary, but split implementation into the smallest serial checkpoints that
each land complete private behavior and discriminating evidence. At minimum
adjudicate:

1. a platform-neutral authority, sorted-preflight, operation/refinement, and
   scripted-backend substrate with exhaustive pure race/order evidence;
2. the Unix native adapter and raw same-handle directory owner with its complete
   scripted and real-filesystem lifecycle matrix; and
3. the Windows lstat/ReadLink/Find adapters, checked reparse parser, exact
   formulas/path/name handling, copied ABI, scripted owner matrix, and real
   kernel32-linked test build.

Decide whether real Host/materialization escaped-path, directory, and symlink
lifecycle evidence belongs wholly to the Unix checkpoint or needs a final
tests-only checkpoint. Every landed checkpoint must compile on its supported
targets, expose no partial public behavior, and be independently reviewable.
Name exact files, cfg boundaries, traits/helpers, tests, validation, and stop
conditions for each owner. Preserve target-Unix workspace `nix` as the only
dependency addition and keep all DICE, runtime materializer, retry/publication,
fixtures, public schema, and consumer wiring excluded.

Do not edit Rust, Cargo, fixtures, plans, or routing during this design packet.
Return `REPLAN` if a serial checkpoint would require a public API, temporary
wrong behavior on an activated platform, another dependency, or a fifth
terminal file.
