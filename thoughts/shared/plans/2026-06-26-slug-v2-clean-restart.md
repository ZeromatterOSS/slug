...
| M2: analysis graph | **active** | recursive configured analysis, bounded root cquery in `135b0567`, implicit empty-default normalization in `7c6eeae5`, internal string build-setting transitions in `dfc1705e`, first-compatible toolchain evidence in `ed4baf08`, ordered root registration retention in `4a3af8df`, and native toolchain target loading in `6a457406` | rule requirements/builtin loading, real DICE selection/context ownership, then general identity and action gates | implement `WP-6-m2-toolchain-rule-provider-loading-implementation` |
...
`WP-6-m2-toolchain-rule-provider-loading-implementation`.
...
external query boundaries. The current second serial loading packet retains
frozen Starlark rule requirements and adds only a load-capable,
invocation-unsupported `platform_common.ToolchainInfo` symbol. It still adds no
ToolchainInfo provider value, selection, DICE owner, or prepared
`ctx.toolchains`; invalid transition and toolchain failure diagnostics remain
deferred.
