...
| M8: bootstrap | **active (developer graph)** | exact 33-package CLI boundary plus accepted Gates A-B; Gate C1 has 43 accepted Bazel targets; repaired live prime evidence reaches `BEP_READY` and `EXECUTION_READY`, guarded live prime evidence is lifecycle-clean, and its semantic discriminator is accepted offline | Deliver one guarded fixed semantic branch before replay/cache/RBE proof and the full 43-test gate | `WP-10-m8-bazel-buildbuddy-build-cache-prime-semantic-branch-discriminator-transported-live-evidence` |
