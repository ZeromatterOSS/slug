...
| M7: command/ruleset breadth | **partial; registry-file observation implementation active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, `cb5073e0`'s immutable action-owner row, `b7390392`'s rules_rust 0.73 evidence, `a3efa1b7`'s root MODULE-files epoch, `3d174006`'s effective-override epoch, `ae8aa35e`'s request/materialization epoch, `12f68983`'s repository-source epoch, `b08b7f2e`'s REPO-file epoch, `754e7619`'s repository-ignore epoch, `18166691`'s package-preflight epoch, `28fab9b0`'s nonregistry closure epoch, and `d0ebd79d`'s registry-policy epoch are accepted | shared registry-file IO is the smallest remaining carrierless producer; patches, source preparation, discovery and selected graph remain later | implement the accepted private one-file registry-file Result-Arc+epoch owner, then preserve M7A -> M8 -> M7B |
...
[WP-6-7A-registry-file-observation-implementation](./slug-v2-subplans/current-packet.md).
