...
Run only
`WP-5-m1-runtime-materializer-captured-archive-design`.

Perform a read-only design correction of the retained outside-DICE repository
materializer before editing Rust. Inspect the accepted pure request/result
epoch, `LocalRepositoryIo` and its retained immutable-instance roots, current
source-preparation/materializer call graph, and the exact Bazel 9.2
`RepositoryFunction`/`RepoRecordedInput` cache and Local-repository behavior.

Freeze one runtime owner that consumes exact materialization requests without
performing repository IO inside DICE. Specify per-workspace retained success
reuse, exact request/spec identity, dirty-observation invalidation, transient
generation failures, immutable instance/root lifetime, abandoned-attempt
discard, and complete replacement epoch construction. For archives, freeze a
single private captured artifact whose same bytes feed checksum, canonical
identity, tar inspection, and extraction; no later reopen of a mutable caller
path is permitted.

Return an exact implementation file allowlist, API/state ownership, lock and
lifetime rules, Local versus immutable state machine, error precedence,
concurrency/cancellation behavior, and focused scripted plus real lifecycle
evidence. Preserve the accepted path producer, pure DICE projection, and
six-state Local oracle. Exclude attempt/effect sidecars, event/demand
producers, retry/publication, ordinary build/query activation, public surface
expansion, and discovery ownership. Return `REPLAN` if one bounded retained
owner cannot cover archive capture and repository reuse without IO under a
DICE compute or a lock held across DICE.
