...
[Exact built-in C++ configure facade accepted; external selected-repository
load-routing audit selected](./slug-v2-subplans/current-packet.md).

`WP-5-7A-bazel-tools-lib-cc-configure-catalog-implementation-r1` returns
`ACCEPT` at 6 production, 11 proof and 17 total gross Rust additions plus the
exact 784-byte/18-line upstream asset. The frozen SHA-256, non-executable
catalog mode, sorted direct listing and complete manifest digest pass. No DICE
key, BUILD file, install-tree fallback, evaluator/C++ behavior or ruleset
special case was added.

The rebuilt authentic replay clears `UnsupportedCatalog` and loads the facade.
Its next independent failure is the generic external Bzl loader's typed
rejection of
`@rules_cc//cc/toolchains:toolchain_config_utils.bzl` as a
repository-qualified external load. Run only docs-first
`WP-4-5-7A-builtin-external-bzl-load-routing-audit`. Determine how the built-in
repository's selected mapping, canonical route and existing external-source
owners can resolve that public label without copying rules_cc, inventing a
C++/apple_support branch or widening unqualified external-load semantics.
