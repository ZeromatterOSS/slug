...
Implement only `WP-5-m1-root-patch-host-observation-migration`.

Edit only `app/slug_bzlmod_v2/src/source_preparation.rs` and
`app/slug_bzlmod_v2/tests/source_preparation_dice.rs`. Change
`ModuleSourcePreparationKey::Value` to the accepted outer `PathOutcome`,
use complete-only equality/validity, and propagate Need through the private
registry/patch helpers. Preserve local `WorkspaceRawFileKey` and immutable
direct reads unchanged.

For main-repository root patches, resolve every normalized patch path first
with `ResolvedPathKey(Host, ...)`: Missing and Directory are typed failures;
RegularFile and SpecialFile continue. Only after every path validates, demand
`PathObservationKey(FileBytes)` at each resolved real path immediately before
sequential application. Propagate Need, treat empty bytes as a no-op, and add
no `PathFileBytesKey`, projection key, direct IO, fresh graph, new production
key, dependency, fixture, or oracle breadth.

Add exact cumulative L/R/F, two-phase ordering, special/symlink/failure,
nonmain/patch-command exclusion, and retained-engine lifecycle evidence from
the accepted owner contract. Run focused tests first, then the owner crate,
format, exact two-file allowlist, forbidden-surface scans, and archive guards;
obtain independent DICE/consumer and pinned-source reviews before acceptance.
