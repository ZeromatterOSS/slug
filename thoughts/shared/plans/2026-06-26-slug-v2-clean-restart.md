...
| M7: command/ruleset breadth | **active (selected-module-graph prerequisite)** | early command/ruleset scaffolding, accepted query/cquery/aquery and FileWrite execution, accepted POSIX executable FileWrite `run`, the immutable canonical `@@bazel_tools` route/seven-file source owner, and the complete callerless embedded MODULE value in `3bc745de` | freeze the sole Host discovery-to-MVS owner before any built-in injection or consumer | `WP-5-builtin-bazel-tools-selected-graph-owner-design` |
...
[`WP-5-builtin-bazel-tools-selected-graph-owner-design`](./slug-v2-subplans/current-packet.md).
