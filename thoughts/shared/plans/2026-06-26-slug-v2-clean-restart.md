...
| M8: bootstrap | **active (developer graph)** | exact 33-package CLI boundary plus accepted Gates A-B; Gate C1 has 43 accepted Bazel targets; the pinned Bazel 9 omitted-zero BEP repair is accepted offline | Deliver one repaired BEP-stage result, then resume execution/cache/RBE proof and the full 43-test gate | `WP-10-m8-bazel-buildbuddy-bep-zero-default-transported-live-evidence` |
