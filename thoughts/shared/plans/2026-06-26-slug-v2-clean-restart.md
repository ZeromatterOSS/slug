...
Run only `WP-5-m1-repository-path-state-owner-design`.

This is a read-only design packet. Inspect the neutral workspace file/raw/
directory owners, production observation and path validation, local and
immutable repository materialization, pinned Bazel 9.2 `FileValue` symlink
resolution, and accepted package-policy evidence.

Design the smallest neutral DICE owner that can answer Bazel-shaped resolved
path state for a logical repository path: regular or special file, directory,
absent/dangling, symlink cycle, and ordinary read error. It must handle symlinked
ancestors rather than only directly listed symlink leaves, preserve logical key
identity, keep local state request-atomic with the existing observation batch,
and make immutable state depend operationally on materialization generation
without putting generation roots in semantic equality. Specify content-byte
ownership for symlinked regular files, containment/escape policy, exact types,
dependencies, equality, errors, implementation/test allowlist, transitions,
and performance/memory bounds.

Stop and return `REPLAN` on a shallow leaf-only model, filesystem reads inside a
DICE compute without a tracked immutable/generation owner, symlink-directory
traversal without bounded cycle handling, request-order changes, duplicate
snapshots, unclear cross-platform classification, or a containment-policy
change not justified by Bazel parity. Do not edit Rust, Cargo, tests, fixtures,
expected artifacts, commands, server, or lockfile.
