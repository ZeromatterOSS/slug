...
Run only
`WP-5-m1-runtime-native-demand-producer-design-correction`.

Design only the separately corrected runtime-native demand producer. Reconcile
the accepted retained repository materializer, exact-version DICE activation
closure, and dormant attempt/effect sidecar into one bounded
workspace-lifetime demand-provenance contract. Freeze node-keyed semantic
repository scope, repository-first/path-second scheduling, cumulative complete
epochs, fixed per-command generations, strict nonprogress, retained root pins,
explicit accept/discard transitions, and lock-free DICE/native-I/O boundaries.
Explain how untouched cached closure nodes retain demand authority across
commands and why repository-source scope, not path prefix, groups Local
validation. Name exact implementation packets and retained-engine evidence
before any Rust, Cargo, fixture, oracle, command/server, source-preparation, or
discovery edit.
