...
Implement the accepted five-file typed no-follow directory-entry observation
contract in the Stage 5 owner plan. Preserve the existing single directory
observation demand, raw OS-native names, stable name-only ordering with
duplicate retention, explicit platform-specific kind classification, complete
typed-payload equality, invalid-Need behavior, and the exact caller/test
migration. Do not add loading glob ownership, byte-string/parser work, direct
filesystem IO in DICE, compatibility shims, retained standard collections, or
consumer activation.
