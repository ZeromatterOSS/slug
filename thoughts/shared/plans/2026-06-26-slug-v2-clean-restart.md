...
Implement only `WP-5-m1-host-registry-inputs`.

Edit exactly the new private
`app/slug_bzlmod_v2/src/host_registry_inputs.rs`,
`app/slug_bzlmod_v2/src/package_policy.rs`, and the private module declaration
in `app/slug_bzlmod_v2/src/lib.rs`, adding at most 900 lines. Implement and
validate the registry/mirror inputs first, the vendor-only projection second,
and the opaque Refresh token third. Keep all injection test-only and dormant;
do not add a public export, dependency, fixture, production consumer,
RegistryFunction, IO, clock, command/server/core edit, or activation.
