...
| M7: command/ruleset breadth | **partial; nonregistry package-preflight proof-cap correction active** | `53152727` accepts the first private core source consumer; M1 supplies the shared publication spine; bounded FileWrite aquery/REAPI, `cb5073e0`'s immutable action-owner row, `b7390392`'s rules_rust 0.73 evidence, `a3efa1b7`'s root MODULE-files epoch, `3d174006`'s effective-override epoch, `ae8aa35e`'s request/materialization epoch, `12f68983`'s repository-source epoch, `b08b7f2e`'s REPO-file epoch, and `754e7619`'s repository-ignore epoch are accepted | package preflight remains carrierless before the nonregistry closure; registry preparation remains a separate later frontier | correct the bounded package-preflight proof envelope, retry the same owner, then continue closure/registry preparation and preserve M7A -> M8 -> M7B |
...
[WP-6-7A-host-nonregistry-package-preflight-observation-proof-cap-correction-design](./slug-v2-subplans/current-packet.md).
