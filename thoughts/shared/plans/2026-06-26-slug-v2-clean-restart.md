...
| M3: `query` | **active (`attr` source-template design)** | 15/16 default functions plus default/explicit `label`, graph, `label_kind`, and `package` output; accepted Rust-native `filter`/`kind` preserve streamed provenance; pinned Bazel 9.2 closes the corrected 18-lane/165-pair families, reviewed five-file two-package layout, and corrected semantic manifest at SHA-256 `8ae8899e…1ff9e` | freeze and temp-oracle-validate the exact five source bodies plus 18 argv/stdout bindings before payload generation; native-toolchain graph projection, rank order, external repositories, patterns, and remaining formats remain | `WP-4-8-m3-attr-five-source-template-oracle-design` |
...
[`WP-4-8-m3-attr-five-source-template-oracle-design`](./slug-v2-subplans/current-packet.md).
