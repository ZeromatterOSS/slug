...
- Slug provides proven, high-performance build infrastructure
...
| Execution boundary | Direct local / Buck executors | REAPI for remote and local executor backends | Make NativeLink-local REAPI proof routine |
...
**Critical**: Slug must report itself as Bazel 9.0+ for compatibility with modern rules:

1. **`native.bazel_version`** must return a version string >= "9.0.0" (e.g., "9.0.0" or "9.0.0-slug")
...
3. **Abort on incompatible version**: If Slug is somehow configured to report < 9.0.0, it should abort with a clear error
4. **Test with rules_cc 0.2.16** - This is the minimum version that works properly with Bazel 9.0

The `bazel_features` module (https://github.com/bazel-contrib/bazel_features) provides feature detection that many rules depend on. Slug must satisfy these version checks to be compatible with the modern rules\_\* ecosystem.

### Depset And TransitiveSet Shared Core

The depset/transitive_set migration is tracked in
[Plan 54](./slug-bazel-subplans/54-depset-transitive-set-shared-core.md). The
current direction is not a broad best-effort alias between Bazel `depset` and
Slug `transitive_set`:

1. Bazel-facing APIs consume and return Bazel-compatible `depset` values.
2. Slug `transitive_set` remains a separate nominal type for projections,
   reductions, and execution-time shared graph behavior.
3. Shared nested-DAG traversal code is used where semantics overlap.
4. The explicit native bridge helpers are Slug-specific and lossy:
   `depset -> transitive_set` stores each depset node's direct values as a tuple
   payload in an internal `BazelDepsetTset`, and `transitive_set -> depset`
   materializes values and loses projections, reductions, definition identity,
   and tset node identity.
5. No implicit coercion or unknown depset-like fallback should silently convert
   between these types.

### Verification Criteria

- [x] `slug build //...` works on a project using rules_cc
- [x] `slug build //...` works on a project using rules_rust
- [x] `slug build //...` works on a project using rules_python
- [x] `slug build //...` works on a project using rules_oci
- [x] `slug run //:target` executes binaries (verified: hello_rust_bin prints "Hello, Slug!")
- [x] `slug query //...` returns dependency information
...
**Slug approach**: Generate synthetic `@cc_compatibility_proxy` repo matching the Bazel 9.0+ path. Only the `cc_common` module needs native implementation.
...
**Slug approach**: Generate synthetic `@rules_python_internal` with `enable_pystar = True`. Provide `py_internal` stubs for the Starlark implementations. Do NOT implement `native.py_library` (removed in Bazel 9.0).
...
**Slug approach**: Ensure `hasattr(native, "proto_library")` returns False (don't register native proto rules). protobuf's Starlark implementation handles everything. Need `ProtoInfo` provider and `proto_common` module stubs.

### Implications for Slug
...
BXL (Buck Extension Language) is a powerful build graph introspection system inherited from Buck2. It will be preserved as a **Slug-specific extension feature** that enhances Bazel-compatible builds.
...
4. **No Bazel Conflict** - BXL uses separate `.bxl` files and `slug bxl` command

### BXL vs Bazel Aspects

| Feature        | BXL                              | Bazel Aspects                       |
| -------------- | -------------------------------- | ----------------------------------- |
| **Invocation** | External (`slug bxl`)            | Internal (during analysis)          |
...
1. **Keep `slug bxl` command** - Separate from Bazel build commands
2. **Keep `.bxl` file extension** - No conflict with `.bzl` files
3. **Keep `prelude/bxl/` directory** - BXL support files
4. **Document as extension** - Position as "Slug Extension Language"
...
1. **Fork and rebrand** - slug identity
...
- **String storage discipline:** New Bazel-compatibility code should reuse Slug's existing interning patterns for stable identifiers and scoped attr data. Before adding new long-lived `String`, `HashMap<String, ...>`, or `FxHashMap<String, ...>` fields, check [Plan 26: String interning cleanup](./slug-bazel-subplans/26-string-interning.md). Raw strings are fine for user output, command lines, URLs, lockfile text, and low-cardinality local data, but hot graph identifiers should be typed and interned or explicitly justified.
...
|----------|------|---------|---------------|
| 1 | [34-sandboxed-execution-strategy.md](./slug-bazel-subplans/34-sandboxed-execution-strategy.md) | Plan 64's bzlmod remediation guardrail is now clean; current steer is to close the REAPI executor boundary before opening new compatibility lanes. | Hosted Linux CI records the NativeLink-backed REAPI smoke with `executor_boundary=reapi`, zero direct-local actions, materialized outputs, and upload/AC evidence; fallback remains behind REAPI. |
| 2 | [46-directory-paths-in-filegroup-srcs.md](./slug-bazel-subplans/46-directory-paths-in-filegroup-srcs.md) | Local source-directory attr fixes are implemented and the Bazel 9 provider shape is confirmed; external LLVM toolchain verification is still the unclosed proof. | External LLVM smoke advances past `llvm-toolchain-minimal...//:lib/clang/22` and records the next blocker, if any. |
| 3 | [45-per-args-paramfile-and-cargo-runfiles.md](./slug-bazel-subplans/45-per-args-paramfile-and-cargo-runfiles.md) | Local and REAPI per-`Args` paramfile slot delivery are covered by repo-owned fixtures, including a cargo-runfiles-shaped directory handoff; public BCR/rules_rust scan found no consumer needing more than per-`Args` slot materialization, and the rules_python label-keyed tag read blocker is cleared. | Public cargo-build-script smoke gets past the current registered-toolchain blocker, then observes `--cargo_manifest_args=@...` and advances to a distinct layer. |
...
| 5 | [36-extension-spoke-lazy-materialization.md](./slug-bazel-subplans/36-extension-spoke-lazy-materialization.md) follow-ups | Extension materialization is a recurring external-workspace blocker; Phases 3b/4/5 are still open. | `repository_ctx` Label-path audit complete, missing `repository_rule_attr` accessors backfilled, stubbed sub-extension failures are loud and direct. |
...
|------|------------|------------------|-------|
| 46 | Existing source/dep coercion code | 45 and public cargo-build-script verification | Independent small correctness fix; should land before relying on LLVM-backed toolchains in public smokes. |
| 45 | Current Plan 44 Phase 2.5 execroot stopgap | External cargo-build-script execution | Does not wait for Plan 44 Phase 3; Phase 4 in Plan 45 is cleanup after full execroot. |
...
| 63 | Plan 62 Phase 1 replay work, Plan 61 watched-absolute input tracking | Warm-build cacheability after removing broad `validity=false` stopgaps | Owns DICE validity un-poisoning for Project*/WatchedAbs file keys and the external-tree generation marker edge. |
| 64 | Plans 62 and 63 live implementation review | Actual Plan 62/63 closure | Owns remediation for status drift, materialization lock/publish correctness, download/auth completion, lockfile lifecycle, digest honesty, semantic replay guardrails, and bzlmod string/data-structure audit work. |
...
- Every active plan must list:
  ...
  - one real-world target when the plan is motivated by external LLVM behavior;
...
|-----|-------------------------|----------------|
| `genquery` execution is a touch/empty-output stub | Remaining Stub Behavior / High Priority | [Plan 48](./slug-bazel-subplans/48-small-parity-gaps.md) |
| build status stamping (`StampFile`, stable/volatile status files) | Remaining Stub Behavior / High Priority | [Plan 48](./slug-bazel-subplans/48-small-parity-gaps.md) |
| `proto_common.compile()` and protoc toolchain lookup | Remaining Stub Behavior / Medium Priority | [Plan 48](./slug-bazel-subplans/48-small-parity-gaps.md) |
...
| [13-lazy-toolchain-loading.md](./slug-bazel-subplans/13-lazy-toolchain-loading.md)                   | Filter dev_dependency, make toolchain loading resilient, reduce eager loading | **Partial** (Phase 3 landed and a large external smoke gets past the 30-min wall; focused smoke harness still open) |
| [19-configuration-transitions.md](./slug-bazel-subplans/19-configuration-transitions.md)             | Real Bazel-compatible configuration transitions: build settings on ConfigurationData, transition() impl fn execution, built-in exec transition (incl. compilation_mode=opt), select() on build settings, cc_toolchain_config flag-set selection | **Not Started** |
| [22-cli-flag-compat.md](./slug-bazel-subplans/22-cli-flag-compat.md)                                 | CLI flag parity with Bazel: `--config=NAME` vs `--config=SECTION.KEY=VALUE` disambiguation, remote-header stubs, `common --*` flags from real-world `.bazelrc` files. Discovered during Plan 18 end-to-end testing. | **In Progress** (22.1 done; 22.2–22.3 open) |
| [23-module-extension-realworld.md](./slug-bazel-subplans/23-module-extension-realworld.md)           | Real-world bzlmod module-extension parity (`toolchains_buildbuddy` / `mctx.modules` / root-tag / macro-wrapped `repository_rule` pattern). Canonical extension id, rctx.template/symlink Label handling, loud extension-exec errors. | **Complete** (23.1–23.5 landed; next blocker is Plan 13 / rules_java / remote-exec client) |
| [25-remote-execution-buildbuddy.md](./slug-bazel-subplans/25-remote-execution-buildbuddy.md)         | Make `slug build … --config=remote` actually dispatch actions to BuildBuddy's RBE workers (currently runs locally despite RE config). Daemon constraint refresh, executor-factory promotion of `Executor::Local` → remote when RE configured, CAS upload smoke test. | **In Progress** (RE wire smoke-test landed; 25.1 next) |
| [24-exec-platform-resolution.md](./slug-bazel-subplans/24-exec-platform-resolution.md)               | Constraint-based execution-platform resolution: surface `register_execution_platforms()` + `--extra_execution_platforms` candidates into `compute_execution_platforms`, drive the existing `check_execution_platform` constraint matcher, retire the `legacy_execution_platform` host-fallback for registered workspaces, per-target `exec_properties` overrides, per-exec-group platform selection, per-action `exec_properties` kwarg, hermetic BEP RE Platform assertion. Supersedes Plan 11's "no remote exec platform selection" exclusion and Plan 25.3.E/F point-fixes. | **Partial** (Phases 1–9 landed; Phase 10 covered at executor-config grain, full BEP wire assertion deferred) |
| [26-string-interning.md](./slug-bazel-subplans/26-string-interning.md)                               | Audit and clean up string-heavy Bazel-compat code written after the Buck2 fork. Extend existing `static_interner`, `ConcurrentTargetLabelInterner`, and scoped attr interning patterns to stable graph identifiers and hot string-keyed maps; document guardrails for future AI-agent work. | **Not Started** |
| [27-native-language-rule-removal.md](./slug-bazel-subplans/27-native-language-rule-removal.md)       | Remove/quarantine remaining Buck2-era native language-rule implementations. Convert removed Bazel 9 symbols such as no-load `cc_*` into Bazel-shaped removed-rule diagnostics, migrate slug-owned fixtures to explicit loads, and keep native modules such as `cc_common` available for external Starlark rulesets. | **Complete** (Phases 27.1–27.6 landed; native language rules are removed-rule stubs with guardrails) |
| [28-builtins-module-architecture.md](./slug-bazel-subplans/28-builtins-module-architecture.md)       | Add a bundled Bazel builtins module layer, inspired by Bonanza's exports/wrappers pattern but sourced from Bazel 9. Export selected Starlark builtins into BUILD globals, `native`, and external `.bzl` environments; add a rule-implementation wrapper for incremental `ctx` compatibility migration; integrate or remove remaining Buck2 prelude machinery. | **Complete** (all phases landed 2026-05-01; migration discipline remains ongoing) |
| [29-cc-include-dir-determinism.md](./slug-bazel-subplans/29-cc-include-dir-determinism.md)           | Retire the `EXTERNAL_INCLUDE_DIRS` process-global mutable registry in cc_common; route every `-I` / `-iquote` / `-isystem` / `-idirafter` flag through `CcCompilationContext` providers (matches Bazel + Bonanza). Closes the remaining 25% BB action-cache miss rate from Plan 18.10.3 — the global's *set membership* still races with parallel action prep even after the sort fix. Target: slug→slug warm = 99%+ cache hit on `@llvm-project//llvm`. | **Complete** (29.2 + 29.3 + 29.4 landed; full llvm warm = 100% cache hit, 57s wall, digest match=4853/4853; copts now plumbed through `compile_build_variables.user_compile_flags` + `WORKSPACE_ROOT` hardcoded) |
| [30-bes-upload-throughput.md](./slug-bazel-subplans/30-bes-upload-throughput.md)                     | Close the BES-upload performance gap to bazel on warm `@llvm-project//llvm` (57 s slug vs 50 s bazel; profiling pinpointed 23 s post-build BES drain wait as the entire delta). Earlier stream open, tonic flow-control tuning, daemon-resident uploader, parallel lifecycle handshakes. Substrate (tonic / prost) is fine; Connect-rs and grpcio are not viable replacements. | **30.1 + 30.2 + 30.5 done 2026-04-29** (warm wall 57.4 s → 14.81 s, post-build wait 23.2 s → ~2.9 s); 30.3 deferred to plan 31.3; 30.4 done |
| [31-bazel-perf-parity.md](./slug-bazel-subplans/31-bazel-perf-parity.md)                             | Successor to plan 30. Close the remaining slug-vs-bazel gaps on warm-RE `@llvm-project//llvm:llvm` (cold-daemon 14.81 s vs 5.92 s; warm-daemon 5.72 s vs 1.03 s). Persistent on-disk action cache (the structural cold-daemon win), file-watcher filter for bazel convenience symlinks (the structural warm-daemon win), daemon-resident BES uploader (delivers plan 30 §30.3). Target: close ≥80% of each gap. | **Proposed** |
| [32-local-overhead-parity.md](./slug-bazel-subplans/32-local-overhead-parity.md)                     | Capture the 2026-04-30 `@llvm-project//llvm:Support` local-build overhead profile and close the remaining local/no-op gaps: fake external-action harness, queue-wait vs setup split, warm no-op CLI/client overhead, and local executor saturation. Baseline: Slug exposed cold overhead 3.33 s vs Bazel 5.65 s, but Slug warm CLI wall 0.87 s vs Bazel 0.46 s and action parallelism 13.54 vs 14.43. | **Proposed** |
| [33-dynamic-dependency-discovery.md](./slug-bazel-subplans/33-dynamic-dependency-discovery.md)       | Add a generic DICE-backed discovery substrate for Slug-specific dynamic dependencies: C++20 modules, proto imports, Go/Haskell imports, Cargo.toml/rules_rs-style manifest discovery, and future scanner-driven rules. Preserves Bazel-compatible rule behavior by making discovery explicit opt-in, then tracks correctness, hermeticity, remote execution, sandboxing, caching, and performance guardrails. | **Proposed** |
| [34-sandboxed-execution-strategy.md](./slug-bazel-subplans/34-sandboxed-execution-strategy.md)       | Production sandboxed local execution strategy: fail-closed filesystem isolation, undeclared input/write detection, final output overlap claims, Linux Landlock/namespace backends, network policy, and second-tier Windows AppContainer support. Supersedes the current `pre_exec` prototype as the strict strategy. | **Proposed** |
| [36-extension-spoke-lazy-materialization.md](./slug-bazel-subplans/36-extension-spoke-lazy-materialization.md) | Trigger DICE materialization of an extension spoke repo when a sibling extension dereferences it via `mctx.path(Label)` / `mctx.read(Label)`. Picks up the gap Plan 10 Phase 2 closed at the resolve-string layer but left open at the make-it-exist-on-disk layer. Discovered while verifying Plan 13 Phase 3 against an external SDK aggregation target whose rules_rs `crate` extension reads a registered-but-unmaterialized spoke via `mctx.path(Label("@cargo_linux_..."))`. | **Partial** (Phases 1–3 landed; 3b/4/5 still open) |
| [35-buckconfig-removal.md](./slug-bazel-subplans/35-buckconfig-removal.md)                           | Retire the Buck-shaped bootstrap from slug's own source tree: rename 277 `BUCK` → `BUILD.bazel`, drain every `.buckconfig` section into `MODULE.bazel` (cells), `.bazelrc` (flags), or `.bazelignore` (paths), and delete the `legacy_configs` parser. Migration guide for users: [`thoughts/shared/docs/2026-05-04-buckconfig-to-bazel-migration.md`](../docs/2026-05-04-buckconfig-to-bazel-migration.md). No new `.slugrc` — `.bazelrc` already covers every surviving knob. Picks up where Plan 09 §9d/9e left off. | **Complete** (Phases 35.1–35.7 landed 2026-05-01..05-04) |
| [37-canonical-cell-prefix-routing.md](./slug-bazel-subplans/37-canonical-cell-prefix-routing.md)     | Route `@@//pkg` to the root cell without conflating it with `@//pkg`. | **Complete** |
| [38-spoke-registration-without-lockfile.md](./slug-bazel-subplans/38-spoke-registration-without-lockfile.md) | Register extension spokes from lockfile/startup data even when hub materialization is warm. | **Complete** |
| [39-git-repository-keep-dot-git.md](./slug-bazel-subplans/39-git-repository-keep-dot-git.md)         | Preserve `.git` for rules_rs `git worktree` consumers. | **Partial** |
| [40-bazel-lib-relative-file.md](./slug-bazel-subplans/40-bazel-lib-relative-file.md)                 | Fix bazel_lib relative symlink path behavior via Bazel version feature path. | **Complete** |
| [41-config-transition-allowlist-resolution.md](./slug-bazel-subplans/41-config-transition-allowlist-resolution.md) | Compare transitioned label attrs modulo cfg for `_allowlist_function_transition`. | **Complete** |
| [42-expand-template-deferred-action.md](./slug-bazel-subplans/42-expand-template-deferred-action.md) | Implement `ctx.actions.expand_template` as a deferred action. | **Complete** |
| [43-rules-rust-toolchain-resolution.md](./slug-bazel-subplans/43-rules-rust-toolchain-resolution.md) | Surface dict/toolchain analysis failures that masked rules_rust toolchain resolution. | **Complete** |
| [44-workspace-layout-parity.md](./slug-bazel-subplans/44-workspace-layout-parity.md)                 | Move toward Bazel-shaped workspace symlinks, external repo location, and execroot layout. | **Partial** (Phase 2.5 landed; 2.6/3 proposed) |
| [45-per-args-paramfile-and-cargo-runfiles.md](./slug-bazel-subplans/45-per-args-paramfile-and-cargo-runfiles.md) | Per-Args paramfile materialization for cargo build script runfiles. | **Partial** (local slot materialization + NativeLink REAPI nested `Args.use_param_file` and cargo-runfiles directory handoff regressions landed; public smoke and BCR scan remain open) |
| [46-directory-paths-in-filegroup-srcs.md](./slug-bazel-subplans/46-directory-paths-in-filegroup-srcs.md) | Accept directory paths in `filegroup.srcs` / `one_of(dep, source)` attrs. | **Partial** (local fixes and Bazel 9 provider-shape proof landed; external LLVM smoke remains open) |
...
| [62-bzlmod-replay-parity-followups.md](./slug-bazel-subplans/62-bzlmod-replay-parity-followups.md)     | Bzlmod replay and Bazel 9 parity follow-ups: external-cell DICE tracking, refresh metadata, multiple-version identity, extension unique names, lockfile shape, guardrail gaps, write/fetch safety, MVS semantics, and lifecycle. | **Partial** (major phases landed; Plan 64 still owns lockfile lifecycle, bzlmod string/data-structure audit, and workspace hygiene) |
| [63-dice-validity-unpoison.md](./slug-bazel-subplans/63-dice-validity-unpoison.md)                     | Un-poison DICE validity overrides for warm-build cacheability. Remove `validity=false` stopgaps from WatchedAbs and Project\* file keys; add `ExternalTreeGenerationKey` dep edge for mutable tree roots. | **Complete** (unit surface plus Plan 64.7 same-daemon external-tree replay guardrail landed 2026-06-25) |
| [64-plan62-implementation-review-remediation.md](./slug-bazel-subplans/64-plan62-implementation-review-remediation.md) | Plan 62 implementation review remediation: status drift repair, materialization lock/publish correctness, download/auth completion, lockfile lifecycle, digest honesty, same-daemon replay guardrails, bzlmod string/data-structure audit, workspace hygiene. | **Partial** (Phases 64.1–64.8 landed; Phase 64.9 pending) |
...
| `transition(impl=...)` impl fn never runs | `app/slug_transition/src/transition/starlark.rs` | Transition dict returned from `impl` actually mutates the outgoing cfg | Plan 19 |
...
- [rules_cc Native Requirements](../research/2026-01-26-rules-cc-native-requirements.md) - What Slug must provide for rules_cc (Bazel 9.0+)
- [Sync Extension Executor Architecture Analysis](../research/2026-02-18-sync-extension-executor-architecture-analysis.md) - Comparison of Slug's sync executor vs Bazel/Buck2 approaches
- Native → Starlark Migration: Each rules\_\* repo's version detection is documented inline in [07-rules-integration.md](./slug-bazel-subplans/07-rules-integration.md#bazel-90-native--starlark-migration-architecture)
- [Sandboxed Execution Strategy](./slug-bazel-subplans/34-sandboxed-execution-strategy.md) - Rust and OS sandboxing research plus implementation plan for strict local action isolation
...
### Foundation (Phases 1-3) - [Sub-plan](./slug-bazel-subplans/01-foundation.md)
...
### bzlmod (Phases 4a-5c) - [Sub-plan](./slug-bazel-subplans/02-bzlmod.md)
...
### Rule Primitives (Phases 6a, 6c) - [Sub-plan](./slug-bazel-subplans/03-rule-primitives.md)
...
### Prelude Architecture (Phase 6b) - [Sub-plan](./slug-bazel-subplans/04-prelude-architecture.md)

| Phase | Title                                            | Status          |
| ----- | ------------------------------------------------ | --------------- | --- |
| 6b.1  | Preserve Buck2 Prelude Loading Mechanism         | [x] Already working (prelude.bzl → native.bzl → __slug_builtins__ flow preserved) |
...
### Builtins Compatibility (Phases 7a-7d) - [Sub-plan](./slug-bazel-subplans/05-builtins-compatibility.md)
...
| 7b    | Bazel Global Functions  | [~] Partial (audit done, glob exclude_directories added, missing functions implemented; package_group visibility resolution working with cross-package deps; existing_rules()/existing_rule() return "kind" field; repository_name() returns "@" for root cell; module_name()/module_version() added; macro() now returns callable MacroCallable that invokes implementation function; 2026-03-12) |
| 7c    | Bazel Top-Level Modules | [~] Partial (config module done, platform_common done, testing.analysis_test() done, coverage_common done; cc_common compile passes flags, compilation context preserves all include types, linking_context extracts objects; Label uses @@ canonical format (bzlmod); bazel_features globals reference real symbols (RunEnvironmentInfo, CcSharedLibraryInfo, subrule, macro); ctx.fragments.cpp has compiler/cpu/target_cpu/host_system_name/target_system_name/target_libc methods; ctx.resolve_command() expands $(location) and collects tool inputs; actions.write() accepts mnemonic/execution_requirements params; actions.run/run_shell progress_message wired through pipeline with %{output} substitution; genrule $(GENDIR)/$(BINDIR) fixed to bin output root; 2026-03-12) |
...
### Aspects (Phases 8a-8d) - [Sub-plan](./slug-bazel-subplans/06-aspects.md)

| Phase | Title                              | Status          |
| ----- | ---------------------------------- | --------------- |
| 8a    | Stub aspect() Function             | [x] Complete    |
| 8b    | Aspect Context and Basic Execution | [x] Complete    |
| 8c    | Shadow Graph Propagation           | [~] Partial (propagation, shadow graph, required_providers work; DICE caching and incremental recomputation remain open) |
| 8d    | Advanced Features                  | [~] Partial (required_aspect_providers works; requires/toolchains/apply_to_generating_rules/complex chains remain open) |

### Unified Execution Architecture (Phases 9a-9f) - [Sub-plan](./slug-bazel-subplans/09-unified-execution-architecture.md)
...
### Rules Integration (Phases 10-14) - [Sub-plan](./slug-bazel-subplans/07-rules-integration.md)
...
### Infrastructure (Phases 16-18) - [Sub-plan](./slug-bazel-subplans/08-infrastructure.md)
...
### Real-World Compatibility and Bazel 9 Follow-ups (Plans 19-64) — CURRENT
...
| 21    | Lazy Toolchain Loading & dev_dep      | [13-lazy-toolchain-loading.md](./slug-bazel-subplans/13-lazy-toolchain-loading.md) | [~] Partial: Phase 3 landed and a large external smoke gets past the 30-min toolchain-loading wall; focused smoke harness still open |
| 22    | CLI Flag Compatibility                 | [22-cli-flag-compat.md](./slug-bazel-subplans/22-cli-flag-compat.md) | [~] In Progress |
| 23    | Module Extension Real-World Patterns   | [23-module-extension-realworld.md](./slug-bazel-subplans/23-module-extension-realworld.md) | [x] Complete |
| 24    | Exec Platform Resolution               | [24-exec-platform-resolution.md](./slug-bazel-subplans/24-exec-platform-resolution.md) | [~] Partial: Phases 1–9 landed; Phase 10 covered at executor-config grain, full BEP wire assertion deferred |
| 25    | Remote Execution against BuildBuddy    | [25-remote-execution-buildbuddy.md](./slug-bazel-subplans/25-remote-execution-buildbuddy.md) | [~] In Progress |
| 26    | String Interning Cleanup               | [26-string-interning.md](./slug-bazel-subplans/26-string-interning.md) | [ ] Not Started |
| 27    | Native Language Rule Removal           | [27-native-language-rule-removal.md](./slug-bazel-subplans/27-native-language-rule-removal.md) | [x] Complete (removed-rule stubs + guardrails landed 2026-04-30) |
| 28    | Bazel Builtins Module Architecture     | [28-builtins-module-architecture.md](./slug-bazel-subplans/28-builtins-module-architecture.md) | [x] Complete (all phases landed 2026-05-01; migration discipline remains ongoing) |
| 29    | cc Include-Dir Determinism             | [29-cc-include-dir-determinism.md](./slug-bazel-subplans/29-cc-include-dir-determinism.md) | [x] Complete |
| 30    | BES Upload Throughput                  | [30-bes-upload-throughput.md](./slug-bazel-subplans/30-bes-upload-throughput.md) | [~] Partial: 30.1+30.2+30.4+30.5 done; daemon-resident BES moved to Plan 31.3 |
| 31    | Bazel Perf Parity (warm-RE)            | [31-bazel-perf-parity.md](./slug-bazel-subplans/31-bazel-perf-parity.md) | [ ] Proposed: persistent action cache, watcher filter, daemon-resident BES |
| 32    | Local Overhead Parity                  | [32-local-overhead-parity.md](./slug-bazel-subplans/32-local-overhead-parity.md) | [ ] Proposed |
| 33    | Generic Dynamic Dependency Discovery   | [33-dynamic-dependency-discovery.md](./slug-bazel-subplans/33-dynamic-dependency-discovery.md) | [ ] Proposed |
| 34    | Sandboxed Execution Strategy           | [34-sandboxed-execution-strategy.md](./slug-bazel-subplans/34-sandboxed-execution-strategy.md) | [ ] Proposed |
| 35    | Remove BUCK and `.buckconfig`          | [35-buckconfig-removal.md](./slug-bazel-subplans/35-buckconfig-removal.md) | [x] Complete through parser deletion, fixture/example disposition, docs, and `slug init` template updates |
| 36    | Extension Spoke Lazy Materialization   | [36-extension-spoke-lazy-materialization.md](./slug-bazel-subplans/36-extension-spoke-lazy-materialization.md) | [~] Phases 1–3 done (2026-05-05). A large external SDK aggregation smoke reaches deep analysis with ~550 crate spokes lazily materialized; new blocker is a package-file-tree-loading hang in a local-vendored crate, out of plan scope. Phases 3b (`repository_ctx` audit), 4 (`repository_rule_attr` backfill), 5 (stubbed sub-extension loud-fail) still open. |
...
| 62    | Bzlmod Replay and Bazel 9 Parity Follow-ups | [62-bzlmod-replay-parity-followups.md](./slug-bazel-subplans/62-bzlmod-replay-parity-followups.md) | [~] In Progress: major phases landed; Plan 64 still owns lockfile lifecycle, bzlmod string/data-structure audit, and workspace hygiene |
| 63    | DICE Validity Un-poison Warm-Build Fix | [63-dice-validity-unpoison.md](./slug-bazel-subplans/63-dice-validity-unpoison.md) | [x] Complete: unit surface plus Plan 64.7 same-daemon external-tree replay guardrail landed 2026-06-25 |
| 64    | Plan 62 Implementation Review Remediation | [64-plan62-implementation-review-remediation.md](./slug-bazel-subplans/64-plan62-implementation-review-remediation.md) | [~] In Progress: Phases 64.1–64.8 landed; Phase 64.9 pending |
...
| 42    | Deferred `ctx.actions.expand_template` | [42-expand-template-deferred-action.md](./slug-bazel-subplans/42-expand-template-deferred-action.md) | [x] Complete |
| 43    | rules_rust toolchain resolution        | [43-rules-rust-toolchain-resolution.md](./slug-bazel-subplans/43-rules-rust-toolchain-resolution.md) | [x] Complete |
| 44    | Workspace layout parity                | [44-workspace-layout-parity.md](./slug-bazel-subplans/44-workspace-layout-parity.md) | [~] Partial: Phase 2.5 stopgap landed; Phase 2.6 and Phase 3 proposed |
| 45    | Per-Args paramfile + cargo_build_script runfiles | [45-per-args-paramfile-and-cargo-runfiles.md](./slug-bazel-subplans/45-per-args-paramfile-and-cargo-runfiles.md) | [~] Partial: local and NativeLink REAPI per-`Args` slot materialization plus cargo-runfiles directory handoff are covered; public smoke and BCR scan remain open |
| 46    | Directory paths in `filegroup.srcs`    | [46-directory-paths-in-filegroup-srcs.md](./slug-bazel-subplans/46-directory-paths-in-filegroup-srcs.md) | [~] Partial: local fixes and Bazel 9 provider-shape proof landed; external LLVM smoke remains open |
...
The Slug codebase inherits Buck2's extensive pytest-based test infrastructure. As we adopt Bazel semantics, tests must be migrated accordingly:
...
**Root Cause:** The `slug` symlink at project root points to `target/debug/slug`, but development was using `cargo build --release` which outputs to `target/release/slug`.

**Solution:** Always match the build type to the symlink target:

- Use `cargo build -p slug` (debug) when using the `./slug` symlink
...
- **`slug run` RUNFILES_DIR**: Sets RUNFILES_DIR and RUNFILES_MANIFEST_FILE env vars when runfiles tree exists
- **`slug info` improvements**: Single-key queries output value only (Bazel compat); added output_path, bazel-genfiles, server_pid, server_log, build-language keys
...
- **New Bazel-compatible commands**: `slug version` (multi-line build label format), `slug shutdown` (alias for kill), `slug fetch` (no-op, deps resolved lazily)
...
- **DLL linking on Windows**: Fixed action name mismatch — rules_cc uses underscore-separated names (cpp_link_dynamic_library) while slug expected hyphen-separated (c++-link-dynamic-library); normalize_action_name() helper added
...
- **Improved slug init**: Better BUILD template, Bazel docs links, bazel-external/bin/testlogs in .gitignore

### CI Infrastructure (2026-03-05)

- **`test.py` accepts `--slug` arg** (was failing with argparse error in CI)
...
- **Source location tests + `test_local_incompatible`** (6 tests in `test_error_categorization.py`) - Fixed by:
  1. Updating test assertions to use `slug_*` crate paths instead of `buck2_*`
  2. Updating `SLUG` vs `BUCK2` source area
  ...
  4. Fixed `slug_client` Windows build error (unix-only symlink calls in `run.rs`)
  5. Added `.exe` extension support for slug binary path detection in conftest.py
