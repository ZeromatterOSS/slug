...
| 3 | [45-per-args-paramfile-and-cargo-runfiles.md](./slug-bazel-subplans/45-per-args-paramfile-and-cargo-runfiles.md) | Local and REAPI per-`Args` paramfile slot delivery are covered by repo-owned fixtures; external cargo-build-script and BCR evidence remain open. | Public cargo-build-script smoke observes `--cargo_manifest_args=@...` and advances to a distinct layer; public BCR scan records whether any consumer needs more than per-`Args` slot materialization. |
...
| 45    | Per-Args paramfile + cargo_build_script runfiles | [45-per-args-paramfile-and-cargo-runfiles.md](./slug-bazel-subplans/45-per-args-paramfile-and-cargo-runfiles.md) | [~] Partial: local and NativeLink REAPI per-`Args` slot materialization are covered; external smoke and BCR scan remain open |
