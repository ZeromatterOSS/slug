...
| 2 | [46-directory-paths-in-filegroup-srcs.md](./slug-bazel-subplans/46-directory-paths-in-filegroup-srcs.md) | Local source-directory attr fixes are implemented; external LLVM toolchain verification is still the unclosed proof. | `filegroup.srcs` and Bazel `allow_files=True` label attrs accept source directories; external LLVM smoke advances past `llvm-toolchain-minimal...//:lib/clang/22`. |
...
| [36-extension-spoke-lazy-materialization.md](./slug-bazel-subplans/36-extension-spoke-lazy-materialization.md) | Trigger DICE materialization of an extension spoke repo when a sibling extension dereferences it via `mctx.path(Label)` / `mctx.read(Label)`. Picks up the gap Plan 10 Phase 2 closed at the resolve-string layer but left open at the make-it-exist-on-disk layer. Discovered while verifying Plan 13 Phase 3 against `zeromatter//sdk:sdk_contents` (rules_rs `crate` extension fails on `mctx.path(Label("@cargo_linux_..."))` because the spoke is registered but not on disk). | **Partial** (Phases 1–3 landed; 3b/4/5 still open) |
...
| [46-directory-paths-in-filegroup-srcs.md](./slug-bazel-subplans/46-directory-paths-in-filegroup-srcs.md) | Accept directory paths in `filegroup.srcs` / `one_of(dep, source)` attrs. | **Proposed** |
...
| 46    | Directory paths in `filegroup.srcs`    | [46-directory-paths-in-filegroup-srcs.md](./slug-bazel-subplans/46-directory-paths-in-filegroup-srcs.md) | [ ] Proposed |
