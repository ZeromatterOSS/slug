...
| 3 | [45-per-args-paramfile-and-cargo-runfiles.md](./slug-bazel-subplans/45-per-args-paramfile-and-cargo-runfiles.md) | Local per-`Args` paramfile slots are implemented; external cargo-build-script and REAPI slot delivery evidence remain open. | Public cargo-build-script smoke observes `--cargo_manifest_args=@...` and advances to a distinct layer; RE use of `ParamFileSlot` is either implemented or explicitly routed to Plan 34/25. |
...
| [45-per-args-paramfile-and-cargo-runfiles.md](./slug-bazel-subplans/45-per-args-paramfile-and-cargo-runfiles.md) | Per-Args paramfile materialization for cargo build script runfiles. | **Partial** (local slot materialization + repo-owned nested `Args.use_param_file` regression landed; external smoke, BCR scan, and RE delivery remain open) |
...
| 45    | Per-Args paramfile + cargo_build_script runfiles | [45-per-args-paramfile-and-cargo-runfiles.md](./slug-bazel-subplans/45-per-args-paramfile-and-cargo-runfiles.md) | [~] Partial: local per-`Args` slot materialization is covered; external smoke, BCR scan, and RE delivery remain open |
