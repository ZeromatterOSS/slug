...
- **Directory**: `tools/`
- **Destination**: `bazel_tools/` in Slug source tree
...
├── scripts/
│   └── sync_bazel_tools.sh
└── app/slug_external_cells_bundled/
```
...
- [x] `slug_external_cells_bundled` builds successfully with bazel_tools (3 tests passing)
...
- [x] Verify `@bazel_tools` is available via `slug audit cell`
