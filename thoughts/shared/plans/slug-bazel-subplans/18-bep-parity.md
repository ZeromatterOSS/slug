...
Make Slug speak Bazel's Build Event Protocol (BEP) and Build Event
Service (BES) well enough that third-party dashboards — BuildBuddy,
EngFlow, Trunk, BitRise, custom BES collectors — receive a Slug
...
Slug's event infrastructure (confirmed in plan-1 research):

- `app/slug_data/data.proto` — Slug-native `BuckEvent` schema, ~2400
  lines, spans + instants + records.
- `app/slug_events/src/dispatch.rs` — central pub-sub.
- `app/slug_events/src/sink/*.rs` — channel, smart-truncate, remote
  (Scribe), tee, null. All file sinks write `.pb.zst`. No JSON sink.
- `app/slug_event_log/src/{read,write}.rs` — zstd-framed proto varint
  stream.
...
- No BEP translation layer. `BuckEvent` is Slug's dialect; BEP consumers
...
Landed as `app/slug_build_event_stream/` (Bazel 9.0.0rc1+834 + googleapis
...
Create `app/slug_build_event_stream/` crate. Copy protos verbatim into
`proto/`. Wire `build.rs` to compile via `prost` (match existing slug
...
- `ParsedTargetPatterns` (instant) → `PatternExpanded`. Pattern strings
  ...
  as `//:foo` (Bazel-shape) instead of slug's internal `<cell>//:foo`.
...
| `cacheStats` | empty (out of scope until slug records cache stats) |
...
- `Progress` events fire only when the daemon's tailer pushes
  stdout/stderr; for a quiet slug build (no tailer activity) the BB
...
New `app/slug_build_event_stream/src/translate.rs`. Maps `BuckEvent` →
BEP `BuildEvent`. One visitor per Slug event kind; lossy cases
documented inline.

Required coverage (minimum set for BuildBuddy dashboard to render
correctly):

| Slug event | BEP event | Notes |
|---|---|---|
...
| (configuration resolution) | `Configuration` | One per platform; id = Slug config hash. |
...
| `BuildGraphExecutionInfo` | — or Slug-specific BEP extension | Critical path is not in BEP core; emit as a custom `BuildEvent.Event.build_metadata` or a BuildBuddy-specific extension. |

Slug-specific events with no BEP analogue (`DiceStateUpdate`,
...
`slug_client_ctx/src/subscribers/bep_file_sink.rs` (`BepFileSubscriber`) wire
...
(`app/slug_build_event_stream/src/file_sink.rs`) into the dispatcher.
...
`BuildEventId` parent/child relationships. Verify that Slug's dispatcher
preserves emission order (it should — the dispatcher is synchronous by
default per `app/slug_events/src/dispatch.rs`).
...
"NotFound" for every slug invocation.
...
Verified post-fix: `slug build //:main --bes_backend=grpcs://remote.buildbuddy.io ...`
...
`slug_client_ctx/src/subscribers/bep_bes_sink.rs` (`BesSubscriber`).
...
Validated against BuildBuddy: `slug build //:main --bes_backend=grpcs://remote.buildbuddy.io --bes_header=x-buildbuddy-api-key=<KEY>` produced an accessible invocation page at
...
New sink `app/slug_build_event_stream/src/grpc_sink.rs` using `tonic`
(already in the slug dep tree? verify; if not, add).
...
- `build_tool_version` = `slug_build_info::revision()`
...
- `invocation_id` — map to Slug's `trace_id` (already a UUIDv4).
- `build_id` — separate UUID per build attempt (BEP distinguishes
  invocation from attempt).
- `user` — from `$USER` / `whoami`.
- `host` — hostname.
- `workspace` — project root (Slug already has this).
- `cmdline` — argv.
- `build_tool_version` — Slug version string.
- `role=CI` / `role=DEV` — from `$BUILDBUDDY_ROLE` or `--bes_keywords`.

Extend `ClientContext` (`app/slug_cli_proto/daemon.proto`) if any fields
...
- Signal handler in `app/slug_client/src/main.rs` (or wherever the
...
binary files (typically Bazel vs Slug). Initial snapshot captured in
`app/slug_build_event_stream/README.md` — gap list in that table is the
...
already builds under both Slug and Bazel and is a workable stopgap.
...
# Capture Slug's BEP.
slug build --build_event_json_file=/tmp/slug.json //...
# Diff (ignoring volatile fields).
python3 tools/bep_diff.py /tmp/bazel.json /tmp/slug.json
```
...
BuildBuddy invocation page actually render every panel slug had
half-populated, and to make slug's RE action digests stable enough
...
discovered (and verified) by running `slug build @llvm-project//llvm`
...
|---|---|---|---|
| Flamegraph timeline | `command.profile.gz` `X` events | Stuck on "Build is in progress…" for clang-scale | Captured `local_thread_id` + tokio thread name on `ActionExecutionEnd`, used as chrome-trace `tid`; lane labels read off `ThreadName` so BB shows `slug-rt-N` instead of `Worker N`. Required `data.proto` fields `local_thread_id`/`local_thread_name` (47/48), per-worker tokio name via `thread_name_fn`, monotonic per-thread index in `slug_util::threads::thread_index()` (mirrors `java.lang.Thread.getId()`). |
| Phase Breakdown pie (Launch / Evaluation / Analysis / Execution) | `traceEvents` whose `name` is `buildTargets` / `runAnalysisPhase` / `evaluateTargetPatterns` / `Launch Blaze` | Card invisible (filter array filters to empty) | Synthetic `buildTargets` X-event covering `[command_start_us, command_end_us]` so BB sees ≥1 phase slice; remaining phase markers deferred until slug plumbs per-phase timestamps to the translate layer. |
...
- **`build_id` cross-correlation**: chrome-trace `otherData.build_id`
  populated from `BesConfig.invocation_id` (was hardcoded `"slug"`).
  ...
  completed.
- **`bazel_version` shape**: `release 8.0.0-slug` (was bare `slug`).
...
`slug build @llvm-project//clang:clang --config=remote`: Timing tab
flamegraph populated with named `slug-rt-N` lanes, Phase Breakdown +
...
Files: `app/slug_build_event_stream/src/translate.rs` (the bulk of the
state-keeping + JSON emit), `app/slug_build_event_stream/src/grpc_sink.rs`
(timeout fix), `app/slug_client_ctx/src/subscribers/bep_bes_sink.rs` +
`bep_file_sink.rs` (`observe_slug_event` wiring), `app/slug_data/data.proto`
(thread fields), `app/slug_build_api/src/actions/calculation.rs`
(thread capture site), `app/slug_util/src/{threads.rs,tokio_runtime.rs}`
...
`slug killall && rm -rf buck-out/v2/cache buck-out/v2/forkserver`,
...
**Root cause** — *two* code paths in slug disagreed on the canonical
...
Result: slug materialized one path under bazel-external (the `_main+…`
...
"package `llvm-project//llvm` does not exist" after `slug clean`
...
making *every* compile action's digest unique to the slug daemon
...
from 0/183 cache hits to 183/183 cache hits — and *the slug digests
matched bazel's*, because the `_main+…` prefix is what bazel's RE
client stores too.

Files: `app/slug_bzlmod/src/extension_execution_dice.rs`,
`app/slug_bzlmod/src/pending_repo_cells.rs` (test fixtures),
`app/slug_external_cells/src/extension_repo.rs`,
...
across slug→slug restarts. Per-field hashing of every
...
**Root cause.** `app/slug_build_api/src/interpreter/rule_defs/cc_common/mod.rs`
...
Verified: slug→slug warm went from 23% hits / 232 s wall to 75% hits /
...
Files: `app/slug_build_api/src/interpreter/rule_defs/cc_common/mod.rs`
(sort + FIXME comment naming the architectural issue),
`app/slug_execute/src/execute/command_executor.rs` (per-field
diagnostic tracing under `action_digest_debug` target),
`app/slug_execute_impl/src/executors/action_cache.rs` (HIT/MISS/ERR
...
- **Phase markers in chrome trace.** Today only the synthetic
  ...
  when slug grows per-phase timestamps in `BepStreamState`.
- **Memory / package / artifact metrics in `BuildMetrics`.** The
  Bazel proto exposes them; slug doesn't yet record the underlying
...
New sink `app/slug_build_event_stream/src/otlp_sink.rs`. Maps Slug
span-start/span-end pairs to OTLP spans, with trace_id = Slug trace_id.
...
- How much of BuildBuddy's "Invocation diff" feature relies on fields
  Slug doesn't carry? (e.g., per-action input digests, remote-cache
  instance-name metadata). Answer during 18.8 by diffing a real
  BuildBuddy invocation for identical workspace.
- Should Slug emit a BuildBuddy-specific BEP extension for critical
