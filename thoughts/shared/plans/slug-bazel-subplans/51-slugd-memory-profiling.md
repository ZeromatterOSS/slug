# Plan 51: Slugd Memory Profiling
...
After that bounded curl wait cleared, Slug resumed Rust crate analysis.
...
env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep "slugd\\[zeromatter\\].*sdk-parity-20260510-active-snapshot-1" \
    -- \
    timeout 1200s \
...
This smoke also exposed a measurement artifact: `SLUG_MEMORY_CHECKPOINTS=1`
...
env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep "slugd\\[zeromatter\\].*sdk-parity-20260510-rust-sampler-1" \
    -- \
    timeout 1200s \
      /var/mnt/dev/slug/target/debug/slug \
...
The expanded sampler in `app/slug_analysis/src/analysis/env.rs` worked: the log
...
timeout 900s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep "slugd\\[zeromatter\\].*sdk-parity-20260510-003745" \
    -- \
    /var/mnt/dev/slug/target/debug/slug \
...
peaked around `858236` KiB and `slugd` reached about 699 MiB, so this is still
...
`app/slug_analysis/src/analysis/env.rs` so long-running Rust rule evaluation
...
Result: Slug status `3` after 158s,
...
`object_files + additional_object_files` because Slug does not support
`tuple + list`.
...
Result: Slug status `3` after 190s,
...
Result: Slug status `3` after 184s,
...
`rules_cc+0.2.17/cc/private/rules_impl/cc_static_library.bzl:174`; Slug rejects
...
Result: Slug status `3` after 164s,
...
wait. With configured-node and analysis-dep checkpoints enabled, Slug exited
...
`app/slug_configured/src/nodes.rs` and
`app/slug_analysis/src/analysis/calculation.rs` should remain useful for the
next bounded zeromatter smoke.

## Goal

Profile and reduce `slugd` memory usage during large bzlmod/zeromatter
builds. Recent local runs showed `slugd` consuming more than 20 GB of
...
```sh
heaptrack /var/mnt/dev/slug/target/debug/slug \
  --isolation-dir verify-canonical-label-architecture \
  build //sdk:sdk_contents
```
...
- First implementation pass:
  ...
  - added `SLUG_MEMORY_CHECKPOINTS`-gated tracing checkpoints for lockfile
...
- Short zeromatter repro capture from `/var/mnt/dev/zeromatter`:
  - command:
    `SLUG_MEMORY_CHECKPOINTS=1 scripts/memory_smoke.sh --interval 5 --include-pgrep 'slugd\[zeromatter\].*verify-canonical-label-architecture-memory-profile-3' -- target/debug/slug --isolation-dir verify-canonical-label-architecture-memory-profile-3 build //sdk:sdk_contents`;
...
`app/slug_common/src/package_listing/interpreter.rs`, which preallocated three
...
directory. Slug still has package-level parallelism, but a large external repo
...
- root-cell native platform labels now render in Bazel form (`//pkg:tgt`)
  instead of Slug cell form (`zeromatter//pkg:tgt`);
...
- extension repo stubs now record a RepoSpec hash in `.slug_repo_complete`, so
...
and Slug's current Bazel depset implementation is independent from Buck's
...
Added `SLUG_MEMORY_CHECKPOINTS`-gated checkpoints around Bazel depset
...
This is intentionally diagnostic-only: when `SLUG_MEMORY_CHECKPOINTS` is not
...
- [x] Ran `cargo fmt -- app/slug_build_api/src/interpreter/rule_defs/depset.rs`
- [x] Ran `cargo check -p slug_build_api`
...
```sh
SLUG_MEMORY_CHECKPOINTS=1 /var/mnt/dev/slug/scripts/memory_smoke.sh \
  --interval 5 \
  --include-pgrep 'slugd\[zeromatter\].*plan54-followup' \
  -- /var/mnt/dev/slug/target/debug/slug \
...
zeromatter package-file-tree loading. The checkpoint is emitted when Slug builds a
...
- `cargo fmt -- app/slug_common/src/dice/cells.rs app/slug_interpreter/src/import_paths.rs`
- `cargo check -p slug_common -p slug_interpreter`
- `cargo test -p slug_common dice::cells --lib` (compiled; 0 matching tests)
- `cargo test -p slug_interpreter import_paths --lib` (compiled; 0 matching
  tests)
- `cargo check -p slug`
- `cargo build -p slug`
- `git diff --check`
...
SLUG_MEMORY_CHECKPOINTS=1 /var/mnt/dev/slug/scripts/memory_smoke.sh \
  --interval 5 \
  --include-pgrep 'slugd\[zeromatter\].*plan51-bzlmod-fast-path' \
...
Added `SLUG_MEMORY_CHECKPOINTS`-gated instrumentation for the package-file-tree
...
SLUG_MEMORY_CHECKPOINTS=1 /var/mnt/dev/slug/scripts/memory_smoke.sh \
  --interval 5 \
  --include-pgrep 'slugd\[zeromatter\].*plan51-package-loading-instrumented' \
...
SLUG_MEMORY_CHECKPOINTS=1 /var/mnt/dev/slug/scripts/memory_smoke.sh \
  --interval 5 \
  --include-pgrep 'slugd\[zeromatter\].*plan51-interpreter-results-instrumented' \
...
- `cargo fmt -- app/slug_common/src/file_ops/dice.rs app/slug_common/src/package_listing/dice.rs app/slug_common/src/package_listing/interpreter.rs app/slug_common/src/package_listing/listing.rs app/slug_interpreter_for_build/src/interpreter/calculation.rs`
- `cargo check -p slug_common`
- `cargo check -p slug_interpreter_for_build -p slug_common`
- `cargo check -p slug`
- `cargo build -p slug`
- `cargo test -p slug_common package_listing --lib`
- `cargo test -p slug_common file_ops --lib`
- `cargo test -p slug_interpreter_for_build interpreter::calculation --lib`
- `git diff --check`
...
host-resource based and feeds loading/analysis parallelism. Slug currently
accepts `--loading_phase_threads` for Bazel CLI compatibility but marks it
ignored in `app/slug_client_ctx/src/common.rs`, so this slice implemented only
...
SLUG_MEMORY_CHECKPOINTS=1 /var/mnt/dev/slug/scripts/memory_smoke.sh \
  --interval 5 \
  --include-pgrep 'slugd\[zeromatter\].*plan51-package-eval-gated-compact-load' \
  -- timeout --signal=INT --kill-after=15s 180s \
     /var/mnt/dev/slug/target/debug/slug \
...
- `cargo fmt -- app/slug_interpreter_for_build/src/interpreter/calculation.rs app/slug_build_signals_impl/src/lib.rs`
- `cargo check -p slug_interpreter_for_build -p slug_build_signals_impl`
- `cargo test -p slug_build_signals_impl -p slug_interpreter_for_build interpreter::calculation --lib`
- `cargo build -p slug`
- `git diff --check`
...
and direct import paths. The audit server's `slug audit starlark package-deps`
...
- `cargo fmt -- app/slug_interpreter/src/file_loader.rs app/slug_interpreter_for_build/src/interpreter/calculation.rs`
- `cargo check -p slug_interpreter -p slug_interpreter_for_build`
- `cargo test -p slug_interpreter file_loader --lib`
- `cargo test -p slug_interpreter_for_build interpreter::calculation --lib`
- `cargo build -p slug`
- `cargo fmt -- app/slug_interpreter/src/file_loader.rs app/slug_cmd_audit_server/src/starlark/package_deps.rs`
- `cargo check -p slug_cmd_audit_server -p slug_interpreter`
- `cargo fmt -- app/slug_interpreter_for_build/src/interpreter/dice_calculation_delegate.rs app/slug_interpreter_for_build/src/interpreter/calculation.rs`
...
SLUG_MEMORY_CHECKPOINTS=1 /var/mnt/dev/slug/scripts/memory_smoke.sh \
  --interval 5 \
  --include-pgrep 'slugd\[zeromatter\].*plan51-module-eval-gate' \
  -- timeout --signal=INT --kill-after=15s 180s \
     /var/mnt/dev/slug/target/debug/slug \
...
- `cargo fmt -- app/slug_interpreter/src/file_loader.rs app/slug_interpreter_for_build/src/interpreter/dice_calculation_delegate.rs`
- `cargo check -p slug_interpreter -p slug_interpreter_for_build`
- `cargo test -p slug_interpreter file_loader --lib`
- `cargo test -p slug_interpreter_for_build interpreter::calculation --lib`
- `cargo build -p slug`
...
- `rules_cc` and `@slug_builtins` autoloads are bzlmod-only and use canonical
...
- `cargo fmt -- app/slug_interpreter_for_build/src/interpreter/calculation.rs app/slug_interpreter_for_build/src/interpreter/dice_calculation_delegate.rs app/slug_interpreter_for_build/src/interpreter/interpreter_for_dir.rs app/slug_interpreter_for_build/src/interpreter/testing.rs app/slug_interpreter_for_build_tests/src/tests.rs`
- `cargo check -p slug_interpreter_for_build`
- `cargo test -p slug_interpreter_for_build_tests test_eval_import -- --nocapture`
- `cargo test -p slug_interpreter_for_build_tests test_load -- --nocapture`
- `cargo build -p slug`
...
timeout 180s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan51-bzlmod-import-key-2' \
    -- /var/mnt/dev/slug/target/debug/slug \
...
- `//sdk:sdk_contents` still does not build under Slug. The bounded smoke was
...
- `cargo fmt -- app/slug_analysis/src/analysis/calculation.rs app/slug_build_api/src/analysis.rs app/slug_build_api/src/analysis/registry.rs app/slug_build_api/src/actions/registry.rs`
- `cargo check -p slug_analysis -p slug_build_api`
...
timeout 180s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan51-analysis-retention-1' \
    -- /var/mnt/dev/slug/target/debug/slug \
...
timeout 180s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan51-analysis-batched-deps-1' \
    -- /var/mnt/dev/slug/target/debug/slug \
...
- Peak sampled RSS was `38574352` KiB at `2026-05-09T10:50:32-07:00`.
  The `slug_memory` max RSS checkpoint recorded `max_rss_bytes=39520804864`.
...
- The likely source Starlark path is
  ...
  `CcInfo` providers. A fix should be systemic in Slug's provider/depset/
...
Added `SLUG_MEMORY_CHECKPOINTS`-gated live-heap checkpoints around the native
...
- `cargo fmt -- app/slug_build_api/src/interpreter/rule_defs/depset.rs app/slug_build_api/src/interpreter/rule_defs/cc_common/actions.rs`
...
timeout 180s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan51-cc-liveheap-1' \
    -- /var/mnt/dev/slug/target/debug/slug \
...
```sh
timeout 150s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan51-cc-freeze-1' \
    -- /var/mnt/dev/slug/target/debug/slug \
...
- The previous 38 GiB live heap burst did not reproduce under the added
  ...
  fix.
- The current stuck target still does not build under Slug. The failure shape
...
- The suspected `cc_common.create_*` calls in
  ...
  `rules_cc+0.2.17`'s Starlarkified `cc_common` provider wrappers, not Slug's
...
- Added an embedder-owned `CallStackCheckpoint` hook to the vendored Starlark
  evaluator. Slug installs it only when `SLUG_MEMORY_CHECKPOINTS=1`.
...
cargo fmt -- \
  ...
  app/slug_build_api/src/interpreter/rule_defs/provider/user.rs \
  app/slug_build_api/src/interpreter/rule_defs/provider/callable.rs

cargo check -p starlark -p slug_analysis -p slug_build_api
cargo build -p slug
```
...
```sh
timeout 110s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan51-starlark-progress-2' \
    -- /var/mnt/dev/slug/target/debug/slug \
...
- The vendored Starlark `CallStackCheckpoint` hook now also has an
  ...
  checkpoint.
- Analysis installs the callback only under `SLUG_MEMORY_CHECKPOINTS=1`.
...
```sh
cargo fmt -- app/slug_analysis/src/analysis/env.rs starlark-rust/starlark/src/eval/runtime/evaluator.rs
cargo check -p starlark -p slug_analysis
cargo check -p slug_build_api
cargo build -p slug
git diff --check
```
...
timeout 120s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan51-eval-heartbeat-1' \
    -- /var/mnt/dev/slug/target/debug/slug \
         --isolation-dir plan51-eval-heartbeat-1 \
         build //sdk:sdk_contents \
  2>&1 | tee /tmp/plan51-eval-heartbeat-1-memory.log

timeout 75s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan51-eval-heartbeat-2' \
    -- /var/mnt/dev/slug/target/debug/slug \
         --isolation-dir plan51-eval-heartbeat-2 \
         build //sdk:sdk_contents \
  > /tmp/plan51-eval-heartbeat-2-memory.log 2>&1

timeout 60s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan51-eval-heartbeat-3' \
    -- /var/mnt/dev/slug/target/debug/slug \
...
- `//sdk:sdk_contents` still did not complete under Slug.
...
- The next slice should instrument that block per resolved toolchain type and
  ...
  `app/slug_analysis/src/analysis/env.rs`, specifically the loop that calls
...
`app/slug_analysis/src/analysis/env.rs` with
...
timeout 90s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan51-toolchain-await-1' \
    -- /var/mnt/dev/slug/target/debug/slug \
...
- This is outside Plan 51's memory scope. Existing Plan 15 owns the relevant
  Bazel 9 parity gap because Slug still treats `cc_toolchain` as a native
...
already an active analysis key, Slug now returns a minimal synthetic
`ToolchainInfo(cc=..., cc_provider_in_toolchain=True)` / `CcToolchainInfo`
provider instead of awaiting that same analysis result.

Focused verification from `/var/mnt/dev/slug`:

```sh
cargo fmt -- app/slug_analysis/src/analysis/calculation.rs app/slug_analysis/src/analysis/env.rs app/slug_build_api/src/interpreter/rule_defs/context.rs app/slug_build_api/src/interpreter/rule_defs/cc_common/feature_config.rs app/slug_build_api/src/interpreter/rule_defs/platform_common.rs
cargo check -p slug_build_api
cargo check -p slug_analysis
cargo build -p slug
```
...
the 180s bound and exited with Slug status `3` after analysis produced a
real error:
...
Focused verification from `/var/mnt/dev/slug` passed:
...
directory`, then Slug created a stub. This still leaves Plan 51/15's
...
`mandatory = False`. Slug's deferred toolchain retry predicate treated any
...
Result: Slug exited status `3` after 179s, with
