...
`slug build @llvm-project//clang:clang` should produce an llvm-tblgen
...
- **Attribute-level `cfg`.** `attr.label(cfg="exec")` parses and
  ...
  (`app/slug_interpreter_for_build/src/attrs/attrs_global.rs:1026`).
...
- **Rule-level `cfg`.** `rule(cfg=transition)` is parsed and stored
  ...
  `::FromAttribute` (`app/slug_interpreter_for_build/src/rule.rs:348`,
  `app/slug_node/src/rule.rs:26`).
- **Transition provider scaffolding.** `transition()` Starlark
  function exists in `app/slug_transition/src/transition/starlark.rs`.
...
- **Per-target exec platform resolution.** Plan 11 lands real
  ...
  (`app/slug_configured/src/execution.rs`,
  `app/slug_configured/src/nodes.rs:910-941`).
  `AttrConfigurationContextImpl` already carries a separate `exec_cfg`
  (`app/slug_node/src/attrs/configuration_context.rs:45`).
- **Build setting rules.** `rule(build_setting=config.bool(flag=True))`
  ...
  `app/slug_node/src/rule.rs:85-87`).
...
2. **`--compilation_mode` is parsed-and-dropped.**
   `app/slug_client_ctx/src/common.rs:261` declares the flag with a
...
4. **No built-in exec transition.** Bazel's `cfg="exec"` corresponds
   to a specific transition that:
    - switches the platform to the chosen exec platform (slug does),
    - sets `compilation_mode=opt` unless `--host_compilation_mode` says
      otherwise (slug doesn't),
    - sets `--cpu` to the exec platform's CPU constraint value (slug
      doesn't),
    - clears `--features` that don't apply in exec (slug doesn't).
...
5. **cc_toolchain_config's flag sets aren't selected on compilation_mode.**
   ...
   isn't a configured value in slug, none of the mode-specific flags
   get selected.
...
6. **`ctx.var["COMPILATION_MODE"]`, `ctx.fragments.cpp.compilation_mode`.**
   ...
   configuration state (`app/slug_build_api/.../context.rs:939-943`).
   `ctx.fragments.cpp.compilation_mode` would need a configured
   cpp fragment.

### The execution path for a cc_binary tool today

Trace of what happens when slug builds `llvm-tblgen` (a `cc_binary`
...
3. Slug's rules_cc (from the same source) also says `cfg="exec"`.
4. Slug coerces that to `AttrType::exec_dep(...)`.
5. At dep gathering, `exec_dep` uses `exec_cfg` from the rule's
   exec-platform resolution — slug replaces the target label's config
...
Closing the gap is about making steps 9-11 work the same in slug.
...
Starlark (as in Bazel), not in slug's Rust engine. Slug's Rust layer
only knows:
...
Extend `ConfigurationData` (`app/slug_core/src/configuration/data.rs`)
to hold `build_settings: Arc<SortedMap<BuildSettingLabel, BuildSettingValue>>`.
`BuildSettingValue` is an enum over the build-setting types slug
...
- `RuleIncomingTransition::Fixed(id)` in
  `slug_configured/src/nodes.rs:1062-1064` already reads the
...
3. Apply **transition defaults from the exec platform's
   ...
   This is how rules_cc specifies the opt default without slug
   hardcoding anything.
...
"@bazel_tools//tools/cpp:compilation_mode": "opt"})`. Slug just
reads and applies.
...
Change `app/slug_client_ctx/src/common.rs:261` from accepted-and-ignored
...
Test: `slug build --compilation_mode=dbg //:foo` produces actions
...
Test: `slug log what-ran` on a clang:clang cold build shows
...
- **Where do exec_properties defaults live?** Plan assumes rules_cc's
  ...
  hardcoded in CcConfiguration.java), we need to confirm slug's
...
- Fresh cold `@llvm-project//clang:clang` baseline after 19.1–19.6
  ...
  - `slug log critical-path` shows td_generate entries at ~30 s each
...
- No hardcoded tool-name or binary-path special cases in Rust.
  ...
  Starlark are unchanged after the landings (only the slug Rust
  plumbing that exposes cfg to Starlark changes).
- `slug log diff summary` between baseline and post-landing shows
...
- Baseline showing the 296 s wall gap:
  `benchmarks/post-plan-17-fixed-aggregator/FINDINGS.md`
- Existing transition scaffolding: `app/slug_transition/src/transition/starlark.rs`
- Existing RuleIncomingTransition enum: `app/slug_node/src/rule.rs:26`
- Existing exec platform resolution: `app/slug_configured/src/execution.rs`
