...
Close the fixed per-invocation overhead slug pays on *warm* queries
and builds. Every slug command — even a no-op cquery on a trivially
...
- **`setup_interpreter`.** Creates a fresh `BuildInterpreterConfiguror`
  per transaction (`app/slug_server/src/ctx.rs:636`). If the
...
2. Gate with `SLUG_LOG_DICE=<path>` (mirror of
   `SLUG_LOG_ADMISSION`). Zero cost when unset.
3. Run a single back-to-back `slug cquery @llvm-project//llvm:Demangle`
...
- `slug log summary` reports 100% DICE hits on the second warm
...
beat Bazel here, but ~2.5 s of slug's cold is daemon-process startup
...
- `slug_daemon::daemon::exec` timeline: what takes the seconds between
...
- Does slug's file watcher track buck-out/ writes? Our own log writes
...
- Relevant source:
  - `app/slug_server/src/ctx.rs:624-686` (`DiceUpdater::update`)
  - `app/slug_node/src/load_patterns.rs:350-390`
    (`load_patterns_with_modifiers`)
  - `app/slug_build_api/src/configure_targets.rs:226-260`
    (`load_compatible_patterns_with_modifiers`)
  - `app/slug_query_impls/src/cquery/evaluator.rs:38-160` (`eval_cquery`)
  - `app/slug_query_impls/src/dice.rs:351-370`
