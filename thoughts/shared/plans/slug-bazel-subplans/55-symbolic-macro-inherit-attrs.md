...
Slug previously ignored `inherit_attrs` in `macro()`, so omitted inherited
...
- Store `inherit_attrs` on Slug's symbolic macro callable instead of discarding
...
- Preserve the `StarlarkAttribute::implicit_default` marker on rule callables so
  ...
  still sees Slug's existing coerced default.
...
```sh
bash -o pipefail -c 'timeout 260s env SLUG_MEMORY_CHECKPOINTS=1 \
