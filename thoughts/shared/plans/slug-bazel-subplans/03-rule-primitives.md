...
This sub-plan covers ensuring Slug's rule execution API matches every member of Bazel's `ctx`, `ctx.actions`, and `repository_ctx` interfaces.
...
| Component | Slug Location |
|-----------|--------------|
| `AnalysisContext` (ctx) | `app/slug_build_api/src/interpreter/rule_defs/context.rs` |
| `AnalysisActions` (ctx.actions) | `app/slug_build_api/src/interpreter/rule_defs/context.rs:68` (struct) |
| Actions method registration | `app/slug_action_impl/src/context.rs:33` (late-binding init) |
| `ctx.actions.run()` | `app/slug_action_impl/src/context/run.rs` |
| `ctx.actions.write()` | `app/slug_action_impl/src/context/write.rs` |
| `ctx.actions.expand_template()` | `app/slug_action_impl/src/context/write.rs:404` |
| `ctx.actions.symlink()` | `app/slug_action_impl/src/context/copy.rs:153` |
| Other actions | `app/slug_action_impl/src/context/unsorted.rs` |
| `StarlarkCmdArgs` (Args) | `app/slug_build_api/src/interpreter/rule_defs/cmd_args/typ.rs` |
...
| # | Bazel Member | Type | Slug Status | Notes |
|---|---|---|---|---|
...
| 14 | `ctx.genfiles_dir` | `root` | **Done** | Same as bin_dir (no separate genfiles dir in Slug, context.rs:641). |
...
| # | Bazel Method | Returns | Slug Status | Notes |
|---|---|---|---|---|
...
| # | Bazel Method | Slug Status | Notes |
|---|---|---|---|
...
The `Args` object is returned by `ctx.actions.args()`. In Slug this is `StarlarkCmdArgs`.
...
- [x] depset ↔ TransitiveSet bridge has explicit Slug-only lossy conversion
...
Bazel's `repository_ctx` object (passed to `repository_rule` implementation functions) has **5 attributes** and **18 methods**. **IMPLEMENTED** — full implementation in `app/slug_interpreter_for_build/src/repository_ctx.rs`. Starlark execution of custom repository rules wired via late binding in `slug_bzlmod/src/starlark_repo_rule_executor.rs` + `slug_interpreter_for_build/src/starlark_repo_rule_executor_impl.rs`.
...
All methods are **Missing** in Slug.
...
**Implementation approach**: Create a new `RepositoryContext` Starlark type in a new file (e.g., `app/slug_bzlmod/src/repository_ctx.rs`) that:
...
- [x] rules_oci oci_image builds — **verified 2026-02-19**: `slug build //:hello_bin_image` works end-to-end
