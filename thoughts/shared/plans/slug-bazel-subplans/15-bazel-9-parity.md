...
>
> Per repo-local `AGENTS.md`: slug targets Bazel 9 exclusively. No
> backwards-compatibility, no migration shims. Break slug workspaces
> freely; no external users depend on the prototype's current surface.

## Scope

Bring slug into full compliance with Bazel 9.0.x behaviour across four
...
single-file projection for `allow_single_file` label attrs. Therefore Slug must
...
Slug's target-name validator still used an older, narrower punctuation set and
...
through Slug live Starlark list values:
...
Slug still validates direct depset elements with `Value::get_hashed()` and
...
`app/slug_build_api_tests/src/interpreter/rule_defs/cc_common.rs`:
...
Outcome: Slug exited status `3` after 158s
...
Slug now expands list and tuple values returned by `map_each` for
...
Outcome: Slug exited status `3` after 190s
...
`generate_glibc_stubs` relative to the listed generated outputs and fix Slug's
...
`old: root//:iphone#...` while Slug now prints `old: //:iphone#...`.
...
Outcome: Slug exited status `3` after 184s
...
teach Slug's `cmd_args.add_all` map_each handling to accept Bazel-compatible
...
The Rust toolchain type label canonicalization blocker is cleared. Slug now
...
Outcome: Slug exited status `3` after 164s
...
The instrumentation shows Slug resolved/analyzed a provider for
...
- slug's "using lockfile specs anyway" digest-mismatch fallback
  (masked Bazel-compat bugs elsewhere — remove).
- slug's `register_bazel_provider_globals` exposing `CcSharedLibraryInfo`,
...
1. **Provider globals**: slug exposes `CcSharedLibraryInfo`,
   ...
   Location: `app/slug_build_api/src/interpreter/rule_defs/cc_common.rs:7676`
   and adjacent.

2. **Native `cc_*` / `java_*` / `py_*` rules**: slug implements
   ...
   `app/slug_analysis/src/analysis/native_rule_analysis.rs:152-170`.

3. **Lockfile v24 digest enforcement**: slug accepts lockfile entries
   ...
   `app/slug_bzlmod/src/lockfile.rs:474-479`.

4. **`@bazel_tools` content gaps**: missing `src/conditions/` was
   patched in commit 2260f5f but there are likely more — slug's
...
- `app/slug_build_api/src/interpreter/rule_defs/cc_common.rs:7676`:
...
- Callers in slug's own .bzl / BUILD files that referenced these as
...
- slug prints the same "use load(...)" hint shape that Bazel 9 does
  when a user-written .bzl references a removed global.
- existing slug-owned .bzl files (bazel_tools, prelude) pass their own
...
phase to cover slug's current `sh_*`, `execution_platform(s)`, and
...
- `app/slug_node/src/rule_type.rs`: delete `NativeRuleKind::CcLibrary`,
...
- `app/slug_interpreter_for_build/src/interpreter/native_rules.rs`:
  ...
  dead code).
- Rules_cc Starlark implementations: ensure slug can evaluate
  ...
  confirm slug can load rules_cc's Starlark version).
...
- Rules_cc's Starlark `cc_library` depends on cc_common API surface
  that may have gaps in slug (toolchain resolution, compile/link
...
- Slug's native cc_* had Bazel-incompatible behaviour (e.g., a
...
(`app/slug_bzlmod/src/lockfile.rs:474-479`). On digest mismatch, force
...
Comparing slug's emitted lockfile against a fresh
...
| Stale committed slug lock | `//pkg:file%name` (no `@`) | `//host:extension.bzl%host_platform` |
| Slug (current) | `@<apparent>//pkg:file%name` | `@platforms//host:extension.bzl%host_platform` |

Source: `app/slug_bzlmod/src/extensions.rs:209-226`
...
**Over-locking:** slug emits 12 entries for `multi_package`'s lock;
fresh bazel emits 4. The 8 slug-only entries are extensions that bazel
treats as internal/version-bound and intentionally does not lock,
because their output is fully determined by the bazel/slug version
...
(*"Slug-generated lockfile entries are re-readable by Bazel 9 on the
...
- `app/slug_bzlmod/src/lockfile.rs:445-509`: modify
  `get_extension_cache` to return `None` on digest mismatch, same as
  empty-specs.
- `app/slug_bzlmod/src/extension_execution_dice.rs:406-438`: when
...
- **Skip internal/version-bound extensions** when writing the
  ...
  (`SLUG_INTERNAL_EXTENSIONS`) keyed by canonical extension id. Initial
  ...
  their implementation with slug; locking them is noise that
  ...
  **Blocker (2026-05-05):** a naive skip-list breaks
  `examples/multi_package` because slug's spoke-materialization path
...
- Bazel-generated `MODULE.bazel.lock` (from a fresh Bazel 9 build) is
  read cleanly by slug. Shared entries hit without digest mismatch.
...
- The llvm-project v24 lockfile uses mixed SRI/bare-base64 for
  different fields. Slug must emit exactly the same format per field.
...
Replace slug's `bazel_tools/` tree with content derived verbatim from
...
- Script: walk `/var/mnt/dev/bazel/src/` and `embedded_tools/` for
  files named `BUILD.tools` and `BUILD`. For each, check if slug has
...
- High-priority subpackages (most commonly referenced from rules and
  ...
  - `tools/test/`
- Each port updates
  `app/slug_external_cells_bundled/build.rs` (touch) and rebuilds.
...
- `slug cquery @llvm-project//llvm:config` does not error with
...
- Some Bazel `BUILD.tools` reference `@bazel_tools//...` targets that
  recursively depend on `@bazel_tools` subpackages not yet in slug —
...
- Rules in `tools/cpp/BUILD.tools` reference platform-specific
  toolchains (`@local_config_cc//...`). Those already work via slug's
...
After Phases 1-4, run `slug cquery` / `slug build` against representative
...
- Module extension API (`module_ctx.*`, `tag_class.*`,
  ...
  tracking that slug doesn't implement
...
Slug's `cc_toolchain` is a native stub
(`app/slug_analysis/src/analysis/native_rule_analysis.rs:166` —
...
therefore not published, and `ctx.var` falls back to slug's hardcoded
builtin list in `app/slug_build_api/src/interpreter/rule_defs/context.rs`
...
Slug's eager `ctx.toolchains` provider construction plus minimal native
...
2. Implement the Bazel-parity behavior in Slug's C++ toolchain path. Likely
...
- Added active-analysis-key tracking and used it while constructing
  ...
  in analysis, Slug now returns a cycle-safe minimal C++ toolchain provider for
...
- Latest bounded zeromatter smoke:
  ...
  `--experimental_cc_implementation_deps` to true, and Slug now preserves,
  ...
  ```sh
  set -o pipefail
  timeout 90s env SLUG_MEMORY_CHECKPOINTS=1 \
    /var/mnt/dev/slug/target/debug/slug \
  ...
  Slug now passes the invocation workspace root to repository rules for
  ...
  not supplied, but Slug currently exposes it as present with value `None`.
  ...
  fallback; explicitly supplied `None` remains present. Slug now tracks provider
  field presence separately from the stored field value in
  `app/slug_build_api/src/interpreter/rule_defs/provider/user.rs`, treats
  Bazel-style list/doc schema fields as optional in
  `app/slug_build_api/src/interpreter/rule_defs/provider/callable.rs`, and has
  focused tests in
  `app/slug_build_api_tests/src/interpreter/rule_defs/provider/tests.rs`.

  Verification passed:

  ```sh
  cargo fmt
  cargo test -p slug_build_api_tests creates_providers -- --nocapture
  cargo test -p slug_build_api_tests test_schema_provider_missing_fields_are_absent -- --nocapture
  cargo test -p slug_build_api_tests test_runtime_constructor_error_on_missing_required -- --nocapture
  cargo test -p slug_build_api_tests interpreter::rule_defs::provider::tests -- --nocapture
  ...
  timeout 120s env SLUG_MEMORY_CHECKPOINTS=1 \
    /var/mnt/dev/slug/target/debug/slug \
...
- Investigated the apparent
  ...
  the actual stall was Slug's missing-directory diagnostic path:
...
- Fixed the diagnostic path in `app/slug_common/src/file_ops/error.rs` by
  skipping external cells during cross-cell missing-path suggestion probes.
  Focused coverage:

  ```sh
  cargo test -p slug_common missing_path_suggestion_probe_skips_external_cells -- --nocapture
  ```
...
- Rebuilt and reran a targeted repro:
  ...
  timeout 30s /var/mnt/dev/slug/target/debug/slug \
...
- Bazel 9 parity source: `StarlarkNativeModule.BINDINGS_FOR_BUILD_FILES`
  ...
  name/version. Slug already exposed `native.module_name()` and
  `native.module_version()`, but omitted the direct BUILD global forms.
- Fixed the systemic gap in
  `app/slug_interpreter_for_build/src/interpreter/functions/path.rs` by adding
...
- Plan 55 fixed Slug's missing symbolic macro `inherit_attrs = <rule>` handling
  ...
  Slug now stores `inherit_attrs`, injects omitted inherited rule attrs into
...
- Focused verification passed:
  ...
  ```sh
  cargo fmt -- app/slug_interpreter_for_build/src/rule.rs \
    app/slug_interpreter_for_build/src/macro_callable.rs \
...
- Bounded ZeroMatter smoke:
  ...
  ```sh
  timeout 260s env SLUG_MEMORY_CHECKPOINTS=1 \
    /var/mnt/dev/slug/scripts/memory_smoke.sh \
      --interval 5 \
      --include-pgrep 'slugd\[zeromatter\].*plan55-symbolic-macro-inherit-1' \
      -- /var/mnt/dev/slug/target/debug/slug \
...
- Verification:
  ...
  ```sh
  cargo fmt
  cargo test -p slug_interpreter_for_build repository_ctx::tests::test_ --lib
  cargo test -p slug_external_cells extension_repo::tests::stub_marker_detection_accepts_plain_and_hashed_stubs --lib
  cargo check -p slug_interpreter_for_build
  cargo build -p slug
  ```
...
1. **Path bridging**: `BuckOutPathResolver::resolve_external_cell_source`
   in `app/slug_core/src/fs/buck_out_path.rs:346-348` places action
   ...
   `app/slug_common/src/legacy_configs/cells.rs:822`. Parity fix:
   `app/slug_external_cells/src/extension_repo.rs::ensure_buck_out_extension_repo_symlink`.
...
2. **Symlink-aware directory listing**: `ExtensionRepoFileOpsDelegate::read_dir`
   ...
   in `app/slug_common/src/package_listing/interpreter.rs:414` only
...
`app/slug_common/src/io/fs.rs:149` has the same behaviour — symlinks
...
slug diverges. File as its own sub-plan if a workspace surfaces it.
...
No return statement. Under slug's previous behaviour,
`ProviderCollection::try_from_value_subtarget` auto-injected an
**empty** DefaultInfo, so `slug build :abi_breaking_h_gen` reported
...
After the DefaultInfo auto-inject lands, `slug build @llvm-project//llvm:abi_breaking_h_gen`
...
Two path deltas between slug output and the cc_library include path:

1. **Extra `__<target>__/` segment.** slug's `BaseDeferredKey::make_hashed_path`
   in `app/slug_core/src/deferred/base_deferred_key.rs:148` always appends
...
2. **`external/<canonical>/` prefix missing.** For external-cell
   ...
   `bazel-bin/external/<canonical_name>/<pkg>/<out_path>`. slug's buck-out
...
- (B) After action execution, symlink from the Bazel-layout path to
  the actual slug buck-out path. Cheap, but introduces two truth
  sources and a clean/rebuild coherence concern.
- (C) Change cc_library's include-dir computation to point at the
  slug-layout path (`<bin_dir>/<cell>/<pkg>/__<target>__/<include_dir>`).
...
- `BuckOutPathKind::BazelOutput` variant in `app/slug_core/src/fs/buck_out_path.rs`.
- `BaseDeferredKey::make_hashed_path` in
  `app/slug_core/src/deferred/base_deferred_key.rs` handles the new
...
- `ArtifactPath::with_full_path` in
  `app/slug_execute/src/path/artifact_path.rs` mirrors the new layout
...
- `CtxOutputs::declare_file` in
  `app/slug_build_api/src/interpreter/rule_defs/context.rs` passes the
...
Slug's lexer (`starlark-rust/starlark_syntax/src/lexer.rs:393-403`)
...
- Native stub (`app/slug_analysis/src/analysis/native_rule_analysis.rs`)
...
`app/slug_interpreter_for_build/src/interpreter/natives.rs::extract_cell_and_package_from_filename`:
...
Verified via `rm -rf bazel-external && slug build @llvm-project//llvm:Support`:
...
`app/slug_interpreter_for_build/src/repository_ctx.rs::resolve_label_to_path`
...
**Impact:** Blocks any `slug clean` + rebuild scenario for
...
`expand_template`) were not declared as action inputs, so slug's
...
### 15.5.9 rules_cc `cc_common.compile` is Starlark, not slug's native (IN PROGRESS)
...
Placing `fail("SLUG_TRACE ...")` verifies _cc_library_impl enters
...
1. **`BuckOutPathKind::Shareable` variant**
   (`app/slug_core/src/fs/buck_out_path.rs`) — resolves to
   ...
   (`app/slug_core/src/deferred/base_deferred_key.rs`) and mirrored in
   `ArtifactPath::with_full_path`
   (`app/slug_execute/src/path/artifact_path.rs`). Aspect and
...
2. **`declare_shareable_artifact` uses `Shareable`**
   (`app/slug_action_impl/src/context/unsorted.rs:229, 257`). Removed
...
3. **`ctx.bin_dir.path` returns real per-target bin_dir**
   (`app/slug_build_api/src/interpreter/rule_defs/cc_common.rs`:
...
5. **Threaded transitive headers into `create_cc_compile_action`
   ...
   slug-native `fn compile` (line ~3108, added in 5f64d82) threaded
...
registered via `copy_file_impl` (`app/slug_action_impl/src/context/copy.rs:76`)
...
yet slug runs the compile without waiting for the symlink. Empty
...
the slug-native `fn compile`'s 5f64d82 threading actually works for
...
double quotes. slug emits `-DLLVM_VERSION_STRING=23.0.0git` (quotes
...
(`app/slug_action_impl/src/actions/impls/run.rs:478`) showed that
...
(`app/slug_execute_impl/src/materializers/deferred/command_processor.rs:927`)
...
**Fix #1** (`app/slug_action_impl/src/actions/impls/run.rs`): in
...
`materialize_inputs`
(`app/slug_execute_impl/src/executors/local.rs:1579`) calls
...
**Fix #2** (`app/slug_core/src/fs/buck_out_path.rs`):
...
action's `CommandExecutionRequest` inputs. The bug was a Bazel/slug
translation gap: Bazel rules pass inputs via an explicit `inputs=`
kwarg on `ctx.actions.run`, but slug's `RunAction` treated that kwarg
...
failed: Failed to spawn a process". Slug already had machinery
(`ensure_external_symlink` in
`app/slug_core/src/cells.rs`) to create `<project_root>/external/<cell>`
...
`symlink_metadata()` returned `Ok`, slug's creator bailed out and
never replaced it.
...
(`app/slug_core/src/cells.rs`):** if the entry is a symlink and
...
before the actual linker flags. slug's `get_link_args` produces only
...
**Bug #2:** slug's default `FeatureConfiguration` enabled
...
`maybe_inject_implicit_default_info` in `slug_analysis/src/analysis/env.rs`
...
for the target becomes the list of all predeclared outputs."). In slug's
...
(`slug_build_api/src/interpreter/rule_defs/cmd_args/traits.rs:58-67`).
...
declared output of the gentbl_rule, but slug never registered it in
the package's output-file → target map, so the label couldn't resolve.

**Root cause.** Two collaborating bugs in slug:
1. `attr.output_list` didn't set `is_output = true` on the attribute
   (`slug_interpreter_for_build/src/attrs/attrs_global.rs::output_list`).
...
2. Even with the attribute flagged, the registration loop in
   `slug_interpreter_for_build/src/rule.rs::call` only handled
...
slug computed `root.path` as `full_path - short_path` (suffix-strip).
...
slug's `declare_shareable_artifact` already dedupes via
...
not an error. Slug matches the no-op semantics for the common case
...
or bool there. Slug's native `genrule` typed the parameter as `&str`
...
content. Slug already ignored the attribute's semantics; this is
...
Rationale: slug does not yet execute Starlark config transitions;
...
the source file of the same name. Slug treats `:python_bootstrap_template.txt`
...
`PYTHON_VERSIONS`. Slug doesn't yet run the extension; its stub
...
slug's per-module `output_file_registry` tracks predeclared outputs
...
`py_internal.get_label_repo_runfiles_path(ctx.label)`). Slug's
...
1. **Mandatory flag ignored.** slug's `rule(toolchains=[...])`
   ...
   py toolchain on py_library) — slug promoted those to mandatory,
...
2. **Empty `ResolvedToolchains` swallowed optional misses.** Even
   ...
   entry (None) for declared-but-unresolved optional types; slug now
   matches.
...
3. **No host Python toolchain registered.** LLVM's MODULE.bazel
   ...
   Bazel historically auto-generated `@local_config_python`; slug
   now does the same.

**Landed changes.**

- `app/slug_node/src/rule.rs`, `app/slug_interpreter_for_build/src/rule.rs`,
...
- `app/slug_external_cells_bundled/build.rs` generates a new
...
- `app/slug_external_cells_bundled/src/lib.rs` adds
...
- `bazel_tools/tools/launcher/BUILD` adds the `launcher_toolchain_type`
  ...
  `BUILD.tools`/`BUILD.bootstrap`). slug's `toolchain()` rule requires
...
1. **py_binary runfiles tree missing at action time.** Once analysis
   ...
   a `.runfiles/` directory next to the executable. Slug does not
...
slug declared the implicit `toolchains` attribute as
...
- `app/slug_interpreter_for_build/src/rule.rs`: implicit
  `toolchains` attribute changed to
  `AttrType::list(AttrType::dep(ProviderIdSet::EMPTY, PluginKindSet::EMPTY))`.
  Slug now resolves the deps and exposes them as
  `Dependency`/`FrozenDependency` values on `ctx.attr.toolchains`.

- `app/slug_build_api/src/interpreter/rule_defs/platform_common.rs`:
...
`slug cquery "deps(@llvm-project//clang:clang)"` resolves **916
targets** without analysis failures. `slug build @llvm-project//clang:clang`
...
`app/slug_analysis/src/analysis/calculation.rs::check_config_setting_flag_values`:
...
1. **Bare target names in `flag_values` keys rejected.** LLVM's
   ...
   accepts bare names as relative labels; slug routed them through
...
2. **`build_setting_default` of list type fell through to the
   ...
   is a Starlark list (all 23 tool names). slug's lookup only
...
- Default-value lookup now handles `CoercedAttr::List` and
  ...
  slug parses `string_list_flag` CLI values today.
...
**Verification.**
`slug build @llvm-project//llvm:llvm` now compiles
...
- `app/slug_build_api/src/interpreter/rule_ctx_storage.rs` (new): moved the
  thread-local that holds the current rule's `ctx` value out of
  `slug_interpreter_for_build::subrule` so that `default_info_creator` in
  `slug_build_api` can reach it.
- `app/slug_build_api/src/interpreter.rs:12`: declares the new module.
- `app/slug_interpreter_for_build/src/subrule.rs:80-85`: re-exports the
  storage API so existing callers (`slug_analysis::analysis::env`) keep
  compiling unchanged.
- `app/slug_build_api/src/interpreter/rule_defs/provider/builtin/default_info.rs`:
...
- `app/slug_build_api/src/interpreter/rule_defs/context.rs:251-280`: adds
...
- `app/slug_build_api/src/interpreter/rule_defs/depset.rs:448`: relaxes
  `collect_depset_elements` from `pub(crate)` to `pub` so the synthesis can
  unpack the runfiles depset.
- `app/slug_build_api/src/interpreter/rule_defs/artifact/starlark_artifact.rs`:
  adds `StarlarkArtifact::new_with_associated_artifacts(...)` constructor.
- `app/slug_action_impl/src/context/runfiles_tree.rs` (new): late-binding
...
- `app/slug_action_impl/src/context.rs:24`: declares the new
  `runfiles_tree` submodule.
- `app/slug_action_impl/src/lib.rs:28`: wires
  `context::runfiles_tree::init_synthesize_runfiles_tree()` into
  `init_late_bindings`.

Unblocker that also landed with §15.5.23:
- `app/slug_external_cells/src/bzlmod.rs:319-349`: `declare_all_source_artifacts`
...
`/var/mnt/dev/llvm-project/utils/bazel`, `slug build
...
**Observed.** After §15.5.23 landed, `slug build
...
raw token through to the tool. Slug's `expand_location` only looked at the
`targets` list, so output labels in `outs=[...]` and source-file labels in
`srcs=[...]` were never resolvable.

**Fix, file:line.**
- `app/slug_build_api/src/interpreter/rule_defs/context.rs:expand_location`
...
**Milestone verification.** `slug build
...
1. **Runfiles aren't built.** slug's
   `app/slug_build_api/src/build/outputs.rs::get_outputs_for_top_level_target`
...
2. **No symlink-tree action type.** Slug has
   ...
   what produces this; slug doesn't emit it from any rule today.

3. **`ctx.actions.run` doesn't propagate tool runfiles.**
   `app/slug_action_impl/src/context/run.rs` accepts `executable=`
   ...
   next to X. Slug's run action sees only the stub File.
...
`slug_interpreter_for_build::subrule::get_current_rule_ctx_raw` but
not idiomatic at that layer). Expect ~1 day of plumbing + testing
before any llvm-project result. Until landed,
`slug build @llvm-project//clang:clang` analyzes cleanly but can't
...
to `rule()`. Slug creates a `Transition` object for the inline
...
3. Treat anonymous rule-cfg transitions as no-ops (identity transition)
   and skip the whole fetch path. Correct for slug today because we
...
**Recommended direction.** Option 3. Slug doesn't execute
...
name. Slug's detour through `(path, name)` is a slug-specific
...
Slug resolved that load to the canonical repo
...
**Slug fix.** In bzlmod load resolution, equivalent apparent/canonical
...
Slug now has focused repository-rule attr coverage for bare repo-label
...
`deps = ["@@zstd//:"]` for
`crates__zstd-sys-2.0.16-zstd.1.5.7`, so Slug pre-registered a bad
RepoSpec even though `Lockfile::get_extension_cache` would have rejected
the same cache. `app/slug_bzlmod/src/pending_repo_cells.rs` now
...
`cargo test -p slug_bzlmod invalid_empty_target_label -- --nocapture`,
`cargo build -p slug`.
...
`cc_linking_outputs.library_to_link` in a depset and Slug reports
...
`cargo build -p slug` passed. A fresh bounded zeromatter smoke from
...
`ResolvedToolchainContext` only rejects missing mandatory toolchains. Slug was
...
```sh
cargo fmt -- app/slug_analysis/src/analysis/env.rs
cargo test -p slug_analysis deferred_retry --lib
...
The command exited with Slug status `3` after 179s, peak RSS 771808 KiB.
...
pic_static_library = library)`, and Slug still reports
...
`depset(lib.pic_objects)`, where `lib.pic_objects` was Slug's hashable
...
```sh
cargo fmt -- app/slug_build_api/src/interpreter/rule_defs/depset.rs \
  app/slug_build_api/src/interpreter/rule_defs/cc_common/actions.rs \
  app/slug_build_api/src/interpreter/rule_defs/cc_common/mod.rs \
  app/slug_build_api_tests/src/interpreter/rule_defs/cc_common.rs
cargo test -p slug_build_api_tests interpreter::rule_defs::cc_common -- --nocapture
cargo test -p slug_build_api_tests interpreter::rule_defs::depset -- --nocapture
cargo build -p slug
```
...
timeout 220s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan15-depset-frozen-list-direct-1' \
    -- \
    /var/mnt/dev/slug/target/debug/slug \
...
```sh
cargo fmt -- app/slug_build_api/src/interpreter/rule_defs/cmd_args/typ.rs \
  app/slug_build_api_tests/src/interpreter/rule_defs/cmd_args/tests.rs
cargo test -p slug_build_api_tests map_each_sequence_returns_expand_as_items -- --nocapture
cargo test -p slug_build_api_tests cc_common -- --nocapture
...
the inherited `target_triple` attr, and Slug reports a missing implementation
...
Slug now uses the existing C++ NativeShim provider facade at the
...
limits the shortcut to the C++ provider surface Slug already constructs
intrinsically.
`app/slug_analysis/src/analysis/calculation.rs` also drops the now-unused
...
That work is broader than C++: every Bazel-native provider/API surface that Slug
...
```sh
cargo fmt -- app/slug_analysis/src/analysis/env.rs app/slug_analysis/src/analysis/calculation.rs
cargo check -p slug_analysis
cargo build -p slug
timeout 90s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/target/debug/slug \
...
timeout 260s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan15-cpp-toolchain-native-shim-sdk-1' \
    -- \
    /var/mnt/dev/slug/target/debug/slug \
...
```sh
timeout 420s env SLUG_MEMORY_CHECKPOINTS=1 \
  /var/mnt/dev/slug/scripts/memory_smoke.sh \
    --interval 5 \
    --include-pgrep 'slugd\[zeromatter\].*plan15-target-name-punctuation-zeromatter-1' \
    -- \
    /var/mnt/dev/slug/target/debug/slug \
...
Slug checkpoint RSS was ~815 MiB. The smoke wrapper killed the
`slugd[zeromatter]` daemon after timeout, and no `slugd[...]` process
remained.
...
same `../reactor-repo-slug` checkout succeeds for `bazel build --config=linux
...
headers. Slug's generated `_bs.env` for the same crate was empty, so this is a
...
- Phase 1 unblocks Phase 2 (removing globals exposes which rules_cc
  load paths slug needs to honour).
...
behaviour. Phase 1 next to force all the load() fixes in slug's own
...
- **rules_cc readiness**: can slug build a trivial `cc_library` via
...
- **Lockfile round-trip**: is there a reasonable test fixture (pair of
  ...
  that slug-regenerates) to validate Phase 3 parity? If not, build
...
- `.bazelversion`-9.0-pinned Bazel workspaces build unchanged under
  slug.
- Slug's error messages for removed symbols and rules match Bazel 9's
  exactly (compared mechanically).
- Lockfiles are round-trip-compatible between slug and Bazel 9.
- `examples/*` in slug are updated to use Bazel 9 load patterns. No
