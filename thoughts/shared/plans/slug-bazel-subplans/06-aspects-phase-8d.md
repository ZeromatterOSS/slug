...
**File:** `app/slug_node/src/attrs/attr_type/mod.rs`
...
- [x] `cargo check -p slug_node` - is_label_type() compiles
- [x] `cargo check -p slug_analysis` - compute_dep_aspects() compiles
- [x] `cargo test -p slug_node` - existing tests pass
- [x] `cargo test -p slug_analysis` - existing tests pass
...
#### Automated Verification:
- [x] `cargo check -p slug_analysis` - modified execute_aspect() compiles
...
./slug.py build //tests/manual_test:c
...
#### Manual Verification:
- [x] `./slug.py build //tests/manual_test:c` succeeds
...
#### 3b.2 Create Unit Test in slug_build_api_tests

**File:** `app/slug_build_api_tests/src/analysis/aspect_deduplication.rs`
...
use slug_analysis::analysis::aspect_calculation::{
...
use slug_interpreter_for_build::aspect::register_aspect_function;
...
#[tokio::test]
async fn test_aspect_dice_deduplication_diamond() -> slug_error::Result<()> {
...
#[tokio::test]
async fn test_aspect_dice_deduplication_deep_diamond() -> slug_error::Result<()> {
    reset_aspect_compute_counter();
...
#### 3b.3 Add Module to slug_build_api_tests

**File:** `app/slug_build_api_tests/src/analysis/mod.rs`
...
- [ ] ~~`cargo test -p slug_build_api_tests -- aspect_deduplication` passes~~
- [ ] ~~Test asserts exactly 4 compute() calls for diamond (a, b, c, d)~~
- [ ] ~~Test fails if count is 5 (would indicate :a computed twice)~~
- [ ] ~~Counter functions exported from `slug_analysis` crate~~
...
**File:** `app/slug_node/src/attrs/attr_type/mod.rs` (add to existing tests module)
...
**File:** `app/slug_analysis/src/analysis/aspect_calculation.rs` (add test module)
...
#### Automated Verification:
- [x] `cargo test -p slug_node -- is_label_type` passes (18 tests)
- [x] `cargo test -p slug_analysis` passes (existing tests)
- [x] `cargo test -p slug_build_api` passes (no regressions)
...
| File | Changes |
|------|---------|
| `app/slug_analysis/src/analysis/aspect_calculation.rs` | Implement compute_dep_aspects(), add deduplication counter, modify execute_aspect() |
| `app/slug_node/src/attrs/attr_type/mod.rs` | Add is_label_type() helper method |
| `app/slug_build_api_tests/src/analysis/mod.rs` | Add aspect_deduplication module |

### New Files

| File | Purpose |
|------|---------|
| `app/slug_build_api_tests/src/analysis/aspect_deduplication.rs` | Unit test for DICE deduplication with diamond dependency pattern |
...
- `app/slug_configured/src/nodes.rs:499-555` - gather_deps() aspect collection pattern
- `app/slug_analysis/src/analysis/calculation.rs` - AnalysisKey DICE pattern
