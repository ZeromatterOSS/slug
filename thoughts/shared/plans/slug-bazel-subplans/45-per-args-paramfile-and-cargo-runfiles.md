...
(`bin.rs:366-370`). On the slug invocation it is `None`. The runner gets
...
Slug currently honours `param_file` only on the top-level cmd_args:
`app/slug_action_impl/src/actions/impls/run.rs:929-933`:
...
`app/slug_execute_impl/src/executors/local.rs:836-870` detects this exact
...
**Files**:
- `app/slug_build_api/src/interpreter/rule_defs/cmd_args/typ.rs` — keep
...
- `app/slug_action_impl/src/actions/impls/run.rs` — rebuild
...
- `app/slug_execute/src/execute/request.rs` — rename
  `with_param_file(Option<ParamFileSpec>)` → `with_param_files(Vec<...>)`.
- `app/slug_execute_impl/src/executors/local.rs` — replace lines
...
After Phase 1, `slug build //sdk:sdk_contents` from
`zeromatter/` should:
...
Acceptance:
- `slug build //sdk:sdk_contents` advances past `target-triple` and
...
`set_dynamic_project_root` (`app/slug_core/src/cells.rs`) becomes
...
(`app/slug_file_watcher/src/notify.rs`) stays — Plan 44 Phase 3 may
