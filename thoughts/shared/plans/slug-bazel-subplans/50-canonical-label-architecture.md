...
Slug has repeatedly hit bugs where `@repo`, `@@repo`, `//pkg:target`,
...
Slug already has a partial foundation:

- `app/slug_bzlmod/src/repo_mapping.rs` defines `CanonicalRepoName`,
  `CanonicalLabel`, and `BzlmodRepoMapping::canonicalize_label`.
- `app/slug_bzlmod/src/lib.rs` exports `CanonicalLabel`.

But canonicalization is not actually centralized:

- `app/slug_bzlmod/src/globals.rs` separately canonicalizes MODULE.bazel
  label values.
- `app/slug_bzlmod/src/pending_repo_cells.rs` separately canonicalizes
  lockfile-seeded repo spec labels.
- `app/slug_interpreter_for_build/src/repository_rule.rs` canonicalizes
  repo-rule attr labels using call-stack filename heuristics.
- `app/slug_interpreter_for_build/src/repository_ctx.rs` parses labels
  to paths with `trim_start_matches('@')` and `bazel-external` scans.
- `app/slug_interpreter_for_build/src/module_ctx/context.rs` has an
  independent label-to-filesystem resolver.
- `app/slug_bzlmod/src/repository_executor.rs` has another
  `build_file` label resolver.
- `app/slug_common/src/legacy_configs/cells.rs` and
  `app/slug_interpreter_for_build/src/starlark_repo_rule_executor_impl.rs`
...
interpreter, likely `slug_bzlmod` unless dependency direction requires a
smaller label crate.
...
- Added `app/slug_interpreter_for_build/src/label_filesystem.rs` as the
...
- Moved toolchain repo-name extraction in
  `app/slug_common/src/legacy_configs/cells.rs` onto
