...
- The resolved graph, repo-mapping snapshots, cell graph, and registered
  ...
  current workspace identity inside its named cell graph, while repo-mapping
  and extension-aggregation data also carry source workspace provenance so
  stale cross-workspace maps cannot be paired with that graph. It carries
  resolution facts, module versions, repo env, repo mappings, extension
