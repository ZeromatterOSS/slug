...
**Status: COMPLETE (2026-05-05)** — fix landed in `app/slug_core/src/pattern/pattern.rs`. Follow-up `crates__yaml-rust2-0.8.1//` wait tracked separately.
...
- `@@<n>//`  → canonical-name `<n>` (in slug this is the same alias map).

## The bug

`lex_provider_pattern` in `app/slug_core/src/pattern/pattern.rs` did:
...
`app/slug_core/src/cells.rs:405`).
...
cell, but slug tried to load `lib/wirebuf` as a sub-package of the
...
Two-line change in `app/slug_core/src/pattern/pattern.rs`:
...
lookup), which is correct in slug because canonical and apparent
names share the same alias resolver.

`BazelLabel::parse` (`app/slug_build_api/src/interpreter/rule_defs/bazel_label.rs:217`)
already normalised `@@//pkg` correctly at analysis time — only the
*coerce-time* path was broken.

## Verification

- `cargo test -p slug_bzlmod --lib` → 163/163 pass (baseline).
- `cargo test -p slug_core --lib pattern` → all pattern tests that
