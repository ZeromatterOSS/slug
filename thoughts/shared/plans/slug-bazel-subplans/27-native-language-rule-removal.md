...
>
> Per repo-local `AGENTS.md`, slug targets Bazel 9 parity only. This
> plan intentionally breaks slug workspaces that still rely on Buck2-era
...
Remove or quarantine slug's remaining native language-rule
implementations so BUILD files see the same surface that Bazel 9 exposes.
In practice:

- `cc_*` rules no longer analyze through slug's Rust
  `create_cc_analysis_result()` path.
- `sh_*` rules no longer analyze through slug's Rust shell-rule path
...
- removed Bazel native rules produce Bazel-shaped "add this `load()`"
  diagnostics instead of silently building through a slug-only native
  implementation.
...
- Slug should not adopt Bonanza-only public platform fields or its
  custom remote-execution model.

## Current State

Slug still has native rule variants and analysis branches for rules that
are not real Bazel 9 native language rules:

| Area | Current slug location | Problem |
|------|-----------------------|---------|
| Native enum entries | `app/slug_node/src/rule_type.rs` | `NativeRuleKind` includes `cc_library`, `cc_binary`, `cc_test`, `cc_import`, `cc_shared_library`, `cc_toolchain`, `cc_toolchain_suite`, `sh_binary`, `sh_test`, `sh_library`, and Buck2 execution-platform rules. |
| Rule registration | `app/slug_interpreter_for_build/src/interpreter/native_rules.rs` | BUILD globals register callable Rust native implementations for the above names. |
| Analysis dispatch | `app/slug_analysis/src/analysis/native_rule_analysis.rs` | `cc_*` dispatches to `create_cc_analysis_result()`, `sh_*` dispatches to native shell analysis, and `cc_toolchain*` returns minimal native providers. |
| CC side effects | `app/slug_build_api/src/interpreter/rule_defs/cc_common/mod.rs` | The process-global external include-dir registry is a remote-cache determinism risk. Removing native cc analysis should make it easier to replace or delete. |
...
- A BUILD file containing:
  ...
  calls the Starlark rule from `@rules_cc`, not slug's Rust native
  rule implementation.
...
3. **No Bonanza platform extensions.** `exec_pkix_public_key` and
   `repository_os_*` are Bonanza-specific and do not belong in slug's
...
slug that is not a true Bazel 9 native rule, including the exact Bazel 9
...
| `proto_library`, `cc_proto_library` | Must not be real native implementations | Slug should keep `hasattr(native, "proto_library")` behavior aligned with protobuf's Bazel 9 path. |
| `py_library`, `py_binary`, `py_test` | Must not be real native implementations | Slug should keep rules_python on the pystar path. |
| `java_library`, `java_binary`, `java_test`, `objc_library`, `objc_import` | EmptyRule or absent, per Bazel 9 source | Slug may not currently implement these, but tests should lock that down. |
...
- The checked-in inventory names every slug native rule that is not in
...
in `app/slug_node/src/rule_type.rs`.

**True Bazel 9 native rules — keep as native, no migration needed:**

| Rule name | Slug analyzer | Bazel 9 status |
|-----------|---------------|----------------|
...
| Rule name | Slug analyzer (current) | Replacement load |
|-----------|-------------------------|------------------|
...
| Rule name | Slug analyzer (current) | Migration path |
|-----------|-------------------------|----------------|
...
**Stubs / slug-internal — review separately, out of Plan 27 scope:**
...
and is already a no-op analyzer in slug. The same infrastructure is then
...
- `RemovedNativeRule` enum carries metadata + `diagnostic_message()` for
  each removed rule (`app/slug_node/src/rule_type.rs`).
- `NativeRuleKind::Removed(RemovedNativeRule)` variant dispatches to
  `analyze_removed_rule`
  (`app/slug_analysis/src/analysis/native_rule_analysis.rs`). Deleted
...
- New tests:
  ...
  - 3 unit tests in `slug_node` for the `parity_category` guardrail and
...
Represent removed native rules explicitly so slug can accept package
...
4. Add an analysis branch that returns a structured error with Bazel's
   message shape. Include target label context using slug's normal
...
- `app/slug_node/src/rule_type.rs`
- `app/slug_interpreter_for_build/src/interpreter/native_rules.rs`
...
- All ten removed-rule analysis branches in
  `app/slug_analysis/src/analysis/native_rule_analysis.rs` are gone:
...
## Phase 27.4: Migrate Slug-Owned BUILD Files and Examples  [DONE 2026-04-30]
...
- **Out of scope:**
  - `tests/manual_test/` is git-ignored per top-level `.gitignore`
    ...
    Slug-owned tracked tests. (Local copies that hit the removed-rule
...
- No tracked Slug test relies on no-load `cc_*` or `sh_*` BUILD
  globals.
...
Make slug's own fixtures and examples obey Bazel 9's explicit-load
rules before the removed-rule stubs land broadly.

### Work

1. Audit slug-owned BUILD files:
   - `examples/`
   - `tests/`
   - `bazel_tools/`
   - `prelude/`
...
- Slug-owned tests do not rely on no-load `cc_*` or `sh_*` BUILD
  globals.
- Fixture changes are minimal and Bazel 9-compatible.
...
- **rules_cc smoke**: `@llvm-project//llvm:Demangle` (8 cc_compile +
  ...
  end-to-end via `slug test` blocked on the pre-existing slug test
  runner clap panic on duplicate `test_summary` (filed as a separate
  bug in `app/slug_client/src/commands/test.rs`); not Plan 27.
...
1. **rules_cc smoke**
   ...
   - Build a loaded `cc_test` and run it through `slug test`.
...
- `parity_category` exhaustive match in
  `app/slug_node/src/rule_type.rs` forces every `NativeRuleKind`
  variant to be classified as `true_native`, `removed_stub`,
  `slug_internal`, or `slug_internal_apple`. Adding a new variant
...
2. Add a test that `NativeRuleKind` variants are either:
   ...
   - explicitly marked slug-internal and unavailable in BUILD globals.
...
- **Wrong error timing.** Bazel's removed-rule diagnostics may appear
  during loading or analysis depending on the rule. Slug should match
  the message shape first, then refine timing if tests show observable
  divergence.
- **Fixture churn.** Many slug tests may rely on no-load native language
