...
Starlark. Slug must model those as explicit NativeShim boundaries, not as
...
surface. Slug's current C++ workaround fixes the analysis cycle only for that
surface; this plan generalizes the model and naming.

## Naming

- Use `Native` for behavior implemented by Slug's native engine layer.
- Use `Intrinsic` for Bazel behavior that is inherently built in rather than
  authored as ordinary Starlark.
- Use `NativeShim` for a Slug object that presents a Bazel-intrinsic provider or
...
- Do not make C++ provider handling a public alias for Buck/Slug internals.
...
1. Inventory intrinsic Bazel API boundaries currently approximated in Slug.
   Initial candidates:
...
7. Verify performance and memory after each expansion:
   ...
   - `cargo check -p slug_analysis` or the affected crate;
   - bounded `//sdk:sdk_contents` smoke with `SLUG_MEMORY_CHECKPOINTS=1`;
...
So Slug must not relax arbitrary mutable user-provider fields. The systemic
fix is that C++ intrinsic provider values exposed through Slug's NativeShim
...
A gated Slug diagnostic on the same SDK frontier showed the remaining mutable
...
That `HeaderInfo` is produced by Slug's implementation of
