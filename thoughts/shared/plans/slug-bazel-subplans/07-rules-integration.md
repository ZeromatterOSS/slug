...
**Critical context for ALL rules_* phases**: In Bazel 9.0, all language rules are pure Starlark. The native (C++/Java) implementations have been removed from Bazel's core. Each rules_* repo has a different mechanism for detecting Bazel 9.0+ and switching to Starlark implementations. Slug intercepts these mechanisms via synthetic repositories.

### Pattern: Each rules_* repo checks Bazel version differently

| Rules Repo | Detection Mechanism | What Slug Must Provide |
|---|---|---|
...
- [x] **`slug run` with cc_binary** - Bazel-compatible `DefaultInfo(executable=...)` support
  - Added `executable` field to `DefaultInfoGen` struct
  - `slug run` now checks `DefaultInfo.executable` as fallback when no `RunInfo` is present
...
- [x] **Bazel default target pattern support** - `@repo//path` now resolves to `@repo//path:path`
  - Added `ParsedPattern::parse_infer_target()` in `slug_core/src/pattern/pattern.rs`
  ...
  - Files: `app/slug_core/src/pattern/pattern.rs`, `app/slug_interpreter_for_build/src/attrs/coerce/ctx.rs`
- [x] **Starlark filegroup rule for bazel_tools** - `@bazel_tools//tools/test:collect_cc_coverage` now works
  ...
  - Files: `bazel_tools/tools/build_rules/filegroup.bzl`, `bazel_tools/tools/test/BUILD`, `app/slug_node/src/rule_type.rs`
...
- `slug test //:hello_test` and `slug test //:hello_test_dynamic` both pass

**Completed (2026-02-05) - Dynamic linking & test runner:**
- [x] **Test runner integration** - `slug test` works with cc_test rules end-to-end
...
**Files:**
- `app/slug_build_api/src/interpreter/rule_defs/cc_common.rs` - cc_common implementation
- `app/slug_execute/src/path/artifact_path.rs` - artifact path resolution (full buck-out paths)
- `app/slug_build_api/src/interpreter/rule_defs/artifact/methods.rs` - artifact `.path` attribute
...
**Implication for Bazel Compatibility:** Bazel rules often pass string paths (from `.path`) through providers, expecting them to work in commands. For Slug compatibility, either:
...
**Problem:** Bazel binaries (cc_binary, etc.) use `DefaultInfo(executable=binary)` to mark targets as runnable. Buck2/Slug uses a separate `RunInfo` provider. When rules_cc's cc_binary returns DefaultInfo with executable, `slug run` failed because it only looked for RunInfo.

**Fix:** Modified Slug to support Bazel's pattern:
...
**Files:**
- `app/slug_build_api/src/interpreter/rule_defs/provider/builtin/default_info.rs` - Store executable
- `app/slug_build_api/src/build/outputs.rs` - Include executable as run output
- `app/slug_server_commands/src/build/result_report.rs` - Build run_args from executable

### Overview

Get rules_cc working to compile C and C++ code. For Bazel 9.0+, rules_cc is **almost entirely pure Starlark**. The key insight from research is that Slug does NOT need to implement native providers or rules—only the native `cc_common` module that the Starlark implementations call into.
...
The `_bazel_version_ge()` function comes from `@bazel_features//private:util.bzl`. Slug reports `"9.0.0"` (no prerelease suffix), which compares greater than `"9.0.0-pre.20250911"` in semver (released > prerelease).

**Slug approach**: Generate synthetic `@cc_compatibility_proxy` repo matching the Bazel 9.0+ path (lines 61-109 of extensions.bzl). Implemented in `synthetic_repos.rs:386-478`.
...
These are implemented in rules_cc's Starlark code—Slug does NOT need native implementations:
...
- [x] `slug build //:main` compiles and links successfully
- [x] `slug run //:hello_bin` executes cc_binary (via DefaultInfo.executable)
- [ ] Header dependencies tracked correctly
- [ ] Incremental builds work
- [ ] `slug test //:mylib_test` runs tests
...
- [ ] `slug build //:main` compiles Rust code
- [x] `slug test //:hello_rust_test` runs tests (2026-02-23)
...
Get rules_python working for Python projects. **Critical**: rules_python 1.8.0+ uses a config-driven switch (`enable_pystar`) to choose between native rules and Starlark implementations. For Bazel 9.0 compatibility, Slug MUST use the Starlark path (`enable_pystar = True`), NOT implement `native.py_library`.
...
```starlark
# internal_config_repo.bzl - Repository rule (NOT executed by Slug - we generate synthetic repo)
...
- Would require Slug to implement native Python rule analysis
...
**File**: `app/slug_bzlmod/src/synthetic_repos.rs` - `generate_rules_python_internal_config_repo()`

#### 3. Implement `py_internal` Stubs

The Starlark implementations load `@rules_python_internal//:py_internal.bzl` which re-exports Bazel's `py_internal` global. Slug must provide this as a stub.

Generate `@rules_python_internal//:py_internal.bzl`:
```python
# Stub py_internal for Slug compatibility
...
Note: Since Slug generates `@rules_python_internal` synthetically (bypassing the repo rule), the `hasattr` check only matters in `util.bzl` where `IS_BAZEL_7_OR_HIGHER` is used for other purposes (like `py_runtime.bzl:21`).
...
- [x] `slug build //:hello_py_lib` analyzes py_library via Starlark rules
- [x] `slug build //:hello_py_bin` creates executable Python binary
- [x] `slug run //:hello_py_bin` executes Python (prints "Hello, Slug!")
- [x] `slug test //:hello_py_test` runs test (2 tests passed)
...
Since Slug targets Bazel 9.0 and does NOT register `native.proto_library`, `hasattr(native, "proto_library")` returns False → Starlark path is used automatically.
...
Slug must NOT register `proto_library` as a functioning native rule. It may be stubbed as `= None` on the `native` module if needed to avoid crashes, but `hasattr(native, "proto_library")` should return False (or the value should be None), so protobuf's Starlark path is triggered.

#### 3. Provide `ProtoInfo` Provider

protobuf's Starlark code references `ProtoInfo` (re-exported from `bazel/common/proto_info.bzl`). In newer versions this is defined in Starlark; in transitional versions it may reference the native `ProtoInfo`. Slug may need a `ProtoInfo` provider stub similar to `PyInfo`.
...
Slug needs native stubs for this module, similar to `cc_common`.
...
- [x] `slug build //:hello_proto` compiles .proto files (283 commands, builds protoc_minimal from source)
- [x] `slug build //:hello_cc_proto` generates C++ code from protos (313 commands, 2026-02-23)
- [x] `slug build //:hello_py_proto` generates Python code from protos (2026-02-23)
...
- [x] **rules_pkg 1.0.1 + pkg_tar**: `slug build //:hello_bin_tar` succeeds, produces valid tar
...
- [x] `slug build //:hello_bin_tar` creates valid tar file (`hello_bin_tar.tar` contains `/usr/local/bin/hello_bin`) — **DONE (2026-02-19)**
- [x] `slug build //:hello_bin_image` creates OCI image — **DONE (2026-02-19)**
