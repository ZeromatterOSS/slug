...
Slug runs ~1/3 fewer actions but takes 19% longer wall. The throughput gap
...
1. **Scheduler parallelism.** 5.6 → 3.2 act/sec is the biggest single
   number. Investigate `app/slug_execute/src/scheduler*.rs` and
   `app/slug_build_api/src/execute/*`. Specific symptoms observed:
...
5. **File watcher catch-up.** Every `slug cquery` / `slug build` warm
   ...
   world. `app/slug_file_watcher/*`. Suspect AI-written shortcut.
...
7. **Starlark re-compilation on daemon cold start.** Bazel persists
   compiled bzl across invocations; Slug's bundled-cell cache has a
   `touch app/slug_external_cells_bundled/build.rs` escape hatch which
...
- **17.2-instrument** (commit `c9e2de7f`): side-channel CSV logger in
  `app/slug_build_api/src/actions/admission_log.rs`, gated by
  `SLUG_LOG_ADMISSION=<path>`. Captures ready_us / start_us / delta_us
  per action.
...
- **17.2-compile-outlier.** Is slug's 279 s on SemaConcept slug-specific
  or intrinsic? Time the single file isolated in both Bazel and slug,
  compare.
...
**Parity source.** None — slug-specific hot path in
`slug_build_signals_impl`.
...
`slug_external_cells/src/*.rs`. On daemon cold start, do we re-hash every
...
Bazel's Skylark module cache persists across invocations. Verify slug's
equivalent. If bundled-cell bzls get re-parsed on every daemon start,
measure and fix.

`app/slug_interpreter/src/*`, `app/slug_external_cells_bundled/*`.
...
### 17.9 `slug_error` allocation sites (OPEN)

`slug_error::Error` boxing + backtrace capture can dominate error paths
