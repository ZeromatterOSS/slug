...
> slug's BES upload land in BuildBuddy; this plan makes slug's *actions*
...
trace upload). The *exec platform selection* problem — how slug
...
Make `slug build … --config=remote` (with a Bazel-shape `.bazelrc`
declaring a public BuildBuddy remote executor and credentials supplied
outside the repo) dispatch action execution to BuildBuddy's RBE workers,
with action inputs uploaded to the BuildBuddy CAS and outputs downloaded
back. The build report should read
`Commands: N (cached: X, remote: Y, local: Z)` with `Y > 0`.
...
`cargo run -p slug_re_configuration --example probe_re -- grpcs://remote.buildbuddy.io`
with credentials supplied outside the repo):

- `SlugOssReConfiguration` populates from buckconfig section
  `slug_re_client` (`address`, `cas_address`, `engine_address`,
...
- `CommonBuildConfigurationOptions::config_overrides` translates
  ...
  `--remote_*_header=KEY=VALUE` into the matching `slug_re_client.*`
...
- **Daemon's `static_metadata` is bound at daemon-init from
  `root_config`.** `app/slug_server/src/daemon/state.rs:542` calls
  `RemoteExecutionStaticMetadata::from_legacy_config(root_config)?`
  exactly once. Per-invocation buckconfig overrides (the
  `slug_re_client.address=…` we now emit from `--remote_executor`)
...
- **Action scheduler always picks Local.** Looking at
  `app/slug_server/src/daemon/common.rs::get_command_executor`,
  ...
  executor. Slug's per-rule configs were written for Buck2's "rules
  ...
  Result: `slug build … --config=remote` against an llvm-project target
...
restarts when the user invokes `slug build … --remote_executor=URI`.

**Implementation.**

- `app/slug_common/src/init.rs`: added `ReConfigSnapshot` struct
  ...
  `from_legacy_config` reads `slug_re_client` section so a daemon
...
- `app/slug_client_ctx/src/common.rs`: added `cli_re_config_snapshot()`
...
- `app/slug_client_ctx/src/bazelrc.rs`: removed `remote_executor` and
...
- `app/slug_client_ctx/src/streaming.rs`: in `BuckSubcommand::exec_impl`,
...
- `app/slug_server/src/daemon/state.rs`: added `apply_re_config_overlay`
  (cfg-gated `not(fbcode_build)`) that mutates the
  `SlugOssReConfiguration` inside `RemoteExecutionStaticMetadata` using
...
- `slug build … --remote_executor=grpcs://X` against a daemon started
  with no `slug_re_client.address` triggers a "Startup config mismatch;
...
- `app/slug_re_configuration/src/lib.rs`: added
...
- `app/slug_execute/src/re/manager.rs`: added
  `ReConnectionManager::is_re_configured()` delegating to the static
  metadata.

- `app/slug_server/src/daemon/common.rs::get_default_executor_config`:
  takes a new `re_configured: bool` argument. The previous
  `if slug_core::is_open_source()` branch becomes
  `if !slug_core::is_open_source() || re_configured`. OSS without RE
...
single workspace. Promote to default once slug's local-required-first
rules opt back in.
...
- `slug build //:foo` (no RE config) continues to run locally.
...
(documented in 25.3.A below) prevents full green: slug's command-line
...
re-materialize after `slug clean`. The repo's
`_overlay_directories(repository_ctx)` calls
`repository_ctx.path(Label("@llvm-raw//:WORKSPACE")).dirname` to find
its sibling repo's source tree. Slug's `resolve_label_to_path` in
`app/slug_interpreter_for_build/src/repository_ctx.rs` could not find
...
and the rule failed; slug stubbed `bazel-external/llvm-project/` as
...
`slug build` rematerializes it correctly via `llvm_configure`, and
...
2. Command-line argument resolution
   ...
   `buck-out/v2/external_cells/<origin>/<cell>/<rel>` — slug's
   on-disk layout.

Locally these resolve to the same file because slug creates a
...
- **A.** Change slug's command-line resolution for source artifacts
...
Option A is the right long-term fix. It would also let slug drop the
...
`slug clean` (see "Known issues" below).
...
- `llvm_configure` repository_rule's `_overlay_directories` resolves
  ...
  CMakeLists.txt and slug stubs the cell. Out of scope for Plan 25;
  needs investigation in `slug_external_cells`/`slug_bzlmod` repo-rule
  handling.
...
mnemonics through specific strategies. Slug accepts the flag today
...
- Should we keep slug's per-rule local/remote split or move toward
...
- Do we want `--remote_executor` to imply
  ...
  meant it." Slug could do the same.
...
- `slug build` without RE config remains 100% local with no
