...
> sort got slug→slug warm cache hit rate from 23% → 75% on
...
> *On a `slug killall && rm -rf buck-out/v2/{cache,gen,forkserver}` cycle
> (i.e. cold daemon, hot BB CAS), `slug build @llvm-project//llvm
...
`app/slug_build_api/src/interpreter/rule_defs/cc_common/mod.rs:118`:
...
The slug-Bonanza gap is small in concept — slug already has
...
- Cross-tool digest parity with Bazel. Even with this fix, slug's
...
- `cargo test -p slug_build_api` green.
- `slug test fbcode//slug/tests/core/...` green (all cc-related
  tests).
- `slug build hello_world//:main` green; per-action digest stable
  across daemon restarts (verify via `action_digest_debug` trace
  that `args_hash` matches across two builds with `slug killall &&
  rm -rf buck-out/v2/{cache,gen,forkserver}` between).

#### Manual

- `slug build @llvm-project//llvm --config=remote` (warm BB cache);
...
- `slug build @llvm-project//clang:clang --config=remote`: full clang
  build still succeeds, no `fatal error: <header>: No such file or
  directory` regressions caused by losing an include path.
- (Stretch) Cross-tool: slug→bazel hits — *not expected to improve
...
2. **`external_include_paths` feature gating**: Bazel and Bonanza
   ...
   of `includes`" branch on a toolchain feature. Slug's toolchain
   features list almost certainly doesn't enable it. Just emit as
   `includes` (Bonanza's else-branch at line 234-236) for now — it
   matches what slug emits today, and the `external_include_paths`
...
- Output-path normalization (would be Plan 30 if filed) — needed
  for cross-tool BB cache sharing, *not* needed for slug→slug.
...
- `slug build @llvm-project//llvm --config=remote` (warm BB CAS):
  ≥99% action cache hit rate.
- `slug→slug warm` (kill daemon + clear `buck-out/v2/{cache,gen,forkserver}`):
  digest match=N/N (no `differ`).
- `static EXTERNAL_INCLUDE_DIRS` deleted; `git grep` for it returns
  zero hits in `app/`.
- `cargo test -p slug_build_api` green.
- `slug build @llvm-project//clang:clang --config=remote` still
  succeeds end-to-end.
...
targets. Slug hardcoded `BINDIR`/`GENDIR`/`TARGET_CPU`/`COMPILATION_MODE` in
`slug_build_api::rule_defs::context.rs` but not `WORKSPACE_ROOT`. Added it as
...
Slug's `create_cc_compile_action` accepted the parameter and **ignored it**
...
`preprocessor_defines`. Slug's `create_cc_compile_action` already gets
...
`slug→slug warm` on `@llvm-project//llvm` (4853 actions):
...
Stated goal of plan: ≥99% slug→slug warm cache hit rate. **Achieved at 100%.**

#### 29.4 follow-up legacy notes (kept for historical reference)

**Partial fix landed (2026-04-29):** Added `WORKSPACE_ROOT` to slug's
hardcoded built-in make-variable defaults in
`slug_build_api::rule_defs::context.rs` (both `ctx.var` and
...
compile command produced by slug, *none* of `clang:basic`'s declared
...
`-idirafter` flags from deps' `includes = ["include"]`, and slug's
...
This is a pre-existing slug bug and is out of scope for Plan 29. The
...
empty for clang:basic in slug. Likely candidates: slug's
`cc_library` attr spec drops `copts` somewhere; or `ctx.attr.copts`
returns empty under slug's coercion; or
...
plumbing fix. Once that lands, the slug→slug warm benchmark will
...
root-cell targets. Slug hardcodes `BINDIR`/`GENDIR`/`TARGET_CPU`/
`COMPILATION_MODE` in `slug_build_api::rule_defs::context.rs:952`
...
The previous behaviour of slug made this *appear* to work because
...
when it lands, re-run the full `slug→slug warm` benchmark on
