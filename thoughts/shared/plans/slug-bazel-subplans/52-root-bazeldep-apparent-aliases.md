...
gap: slug was only emitting aliases for `bazel_dep(repo_name = "...")`, and
...
2. For every root `bazel_dep`, register its Bazel apparent name
   (`repo_name` when present, otherwise `name`) as an alias when slug's selected
   cell identity differs from that apparent name.
3. Resolve alias targets through the selected module graph, including
   disambiguated `name+version` cell identities used by slug for selected
   modules.

## Verification

- `cargo test -p slug_bzlmod parser::tests::test_parse_module_with_compatibility_level --lib`
- `cargo check -p slug_common`

ZeroMatter verification is run separately after rebuilding `target/debug/slug`.
