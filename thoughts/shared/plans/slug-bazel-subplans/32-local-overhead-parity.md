...
Make Slug's local build overhead measurable, keep the daemon-side advantage over
...
- Slug's exposed daemon-side overhead is about **2.32 s lower** than Bazel's
  server-side overhead on this run.
- Slug reaches the first external action earlier.
...
- Slug's daemon-side no-op path is competitive and slightly lower than Bazel's
  server wall.
- Slug pays too much outside the daemon: about **570 ms** of CLI/client/event
  overhead versus about **48 ms** for Bazel.

### Starlark interpreter finding

Slug embeds `starlark-rust`. Build-file and `.bzl` loading parse source to
...
- Slug-vs-Bazel overhead after discounting external process execution.
...
- A wrapper toolchain or fake compiler/archive/linker setup for both Slug and
  Bazel.
- A repeatable script under `thoughts/shared/` or `benchmarks/` that records:
  - Slug event log + `slug log summary --format=json`
...
- A validation row proving the fake workload preserves action count and graph
  ...
  - Slug: target/action graph nodes, actions by mnemonic
...
The 2026-04-30 Slug action-event rollup shows large aggregate "pre-exec"
...
- Updated `slug log summary --format=json` schema or a sidecar post-processor
...
Slug daemon wall is 0.300 s while CLI wall is 0.87 s. Bazel's server wall is
0.412 s while CLI wall is 0.46 s. The Slug gap is too large for a no-op build.
...
- `app/slug_client_ctx/src/events_ctx.rs`
- `app/slug_client_ctx/src/subscribers/`
- `app/slug_cmd_log_client/`
...
Slug averaged 13.54 with the same peak of 16. The gap is not huge, but it is
visible and should be measurable with the no-op harness.

Questions to answer:

- Is Slug's lower average caused by real graph dependencies or scheduler/local
...
The cold Slug run reports load around 3.0 s and analysis around 0.12-0.18 s on
...
- Cold exposed non-external Slug overhead remains below Bazel by at least 20%
  on `@llvm-project//llvm:Support`.
- Warm no-op Slug CLI wall <= 0.55 s.
- Warm no-op Slug daemon wall <= 0.300 s or any regression is explained by a
