...
**Research conducted:**
1. **Module loading in Slug** - No global registry; rules use `StarlarkRuleType = (path, name)`
...
This matches Bazel's architecture and follows the existing rule loading pattern in Slug.

**Files affected by the update:**
- `app/slug_node/src/aspect_type.rs` (NEW)
- `app/slug_interpreter_for_build/src/aspect.rs` (add aspect_path field)
- `app/slug_node/src/attrs/attr.rs` (change aspects field type)
- `app/slug_interpreter_for_build/src/attrs/attrs_global.rs` (extract full type)
- `app/slug_analysis/src/analysis/aspect_key.rs` (use StarlarkAspectType)
- `app/slug_analysis/src/analysis/aspect_calculation.rs` (implement module loading)
- `app/slug_configured/src/nodes.rs` (update aspect key creation)
...
**File:** `app/slug_build_api/src/interpreter/rule_defs/aspect/execution.rs`
...
**File:** `app/slug_node/src/attrs/attr.rs`
...
**File:** `app/slug_interpreter_for_build/src/attrs/attrs_global.rs`
...
**New File:** `app/slug_analysis/src/analysis/aspect_key.rs`
...
**Current File:** `app/slug_analysis/src/analysis/aspect_calculation.rs`
...
**New Type:** `app/slug_node/src/aspect_type.rs`
...
use derive_more::Display;
use dupe::Dupe;
use slug_core::bzl_or_bxl_path::BzlOrBxlPath;
...
**File:** `app/slug_analysis/src/analysis/aspect_key.rs`
...
**File:** `app/slug_analysis/src/analysis/aspect_calculation.rs`
...
fn get_aspect_from_module(
    module: &LoadedModule,
    name: &str,
) -> slug_error::Result<&FrozenStarlarkAspectCallable> {
...
**File:** `app/slug_configured/src/nodes.rs`
...
### Step 7: Add Aspect Modules to slug_analysis ✅

**File:** `app/slug_analysis/src/analysis/mod.rs`
...
cargo check -p slug_build_api -p slug_node -p slug_interpreter_for_build -p slug_analysis

# Run unit tests for slug_build_api
...
slug build //tests/manual_test:c
...
slug build //tests/manual_test:d
...
**File:** `app/slug_node/src/aspect_type.rs` (already has basic test)
...
**New File:** `app/slug_analysis/src/analysis/aspect_calculation_tests.rs`

Add async DICE tests following `slug_build_api_tests` patterns:
...
mod tests {
    ...
    use slug_core::cells::paths::CellRootPathBuf;
    ...
    async fn test_dice_ctx() -> (ProjectRootTemp, DiceTransaction) {
        ...
        // ... standard DICE setup following slug_interpreter_for_build_tests pattern
        // Return (fs, ctx)
    }
...
cargo test -p slug_node -- aspect_type
cargo test -p slug_analysis -- aspect
cargo test -p slug_node -- attr::tests

# Run with verbose output
cargo test -p slug_node -p slug_analysis -- --nocapture

# Full test suite to ensure no regressions
cargo test -p slug_node -p slug_analysis -p slug_interpreter_for_build
```
...
| File | Purpose |
|------|---------|
| `app/slug_node/src/aspect_type.rs` | StarlarkAspectType definition + unit tests |
| `app/slug_analysis/src/analysis/aspect_key.rs` | AspectKey, AspectValue DICE types + unit tests |
| `app/slug_analysis/src/analysis/aspect_calculation.rs` | DICE Key implementation |
| `app/slug_analysis/src/analysis/aspect_calculation_tests.rs` | DICE integration tests (Step 10) |

### Modified Files

| File | Changes |
|------|---------|
| `app/slug_node/src/attrs/attr.rs` | Add `aspects: Vec<Arc<StarlarkAspectType>>` field + unit tests |
| `app/slug_interpreter_for_build/src/aspect.rs` | Add `aspect_path` field, `aspect_type()` getter |
| `app/slug_interpreter_for_build/src/attrs/attrs_global.rs` | Store aspects in label()/label_list() |
| `app/slug_configured/src/nodes.rs` | Wire aspect computation into gather_deps() |
| `app/slug_analysis/src/analysis/mod.rs` | Add aspect modules |
| `app/slug_build_api/src/interpreter/rule_defs/aspect/execution.rs` | Add unit tests |
...
- slug_build_api: 23 tests
- slug_node: 10 aspect tests (5 StarlarkAspectType + 5 Attribute)
- slug_analysis: 4 AspectKey tests
...
This prevents `slug build` from running any targets. The issue is in the prelude's typing annotations, not in Phase 8c code.

**Next Steps:**
1. Fix prelude typing issue in attrs_validators.bzl
2. Run manual verification: `slug build root//tests/manual_test:c`
...
- [x] `cargo test -p slug_build_api` passes (23 tests)
...
- [x] StarlarkAspectType created in slug_node
...
- [x] Run `slug build //tests/manual_test:c` - **BUILD SUCCEEDS**
...
- [ ] Run `slug build //tests/manual_test:d` (wider graph test) - Deferred to Phase 8d
...
```
[2026-01-31T19:52:43.970-08:00] Aspect visiting: gh_facebook_slug//tests/manual_test:a (<unspecified>)
[2026-01-31T19:52:43.970-08:00]   Rule kind: simple_rule
[2026-01-31T19:52:43.971-08:00] Aspect visiting: gh_facebook_slug//tests/manual_test:b (<unspecified>)
...
- [ ] Integration test passes with `cargo test -p slug_core_tests`

### Unit Tests (Step 10)

Unit tests allow verifying Phase 8c infrastructure without full build environment:

- [x] `cargo test -p slug_node` passes (aspect_type tests)
- [x] `cargo test -p slug_analysis` passes (AspectKey tests)
- [x] `cargo test -p slug_interpreter_for_build` passes (aspect.rs tests)
...
**Research findings:**
- Slug has no global module registry
...
- `app/slug_analysis/src/analysis/calculation.rs` - AnalysisKey DICE pattern to follow
- `app/slug_analysis/src/analysis/env.rs` - RuleSpec and run_analysis pattern
- `app/slug_configured/src/nodes.rs` - gather_deps() integration point
