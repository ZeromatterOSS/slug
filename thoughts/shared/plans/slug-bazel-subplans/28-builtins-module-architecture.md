...
All seven phases landed. The `@slug_builtins` bundled cell is the
...
- ~~Delete the `PreludePath` type and the `@prelude` cell registration.~~
  **Done 2026-05-01** (this session). Deleted
  ...
  `cmd_audit_server/prelude.rs`, `slug_server/src/ctx.rs`). Also
  removed the `prelude_is_included` test in
  `slug_interpreter_for_build_tests`. The `@prelude` cell is still
  bundled (via `app/slug_external_cells_bundled`) so explicit
  `load("@prelude//utils:argfile.bzl", ...)` still resolves; what
  went away is the legacy implicit-prelude path.
- ~~Restrict or rename the `__slug_builtins__` Rust namespace.~~
  **Done 2026-05-01**. Removed the `__slug_builtins__` namespace
  ...
  `slug_interpreter_for_build_tests/src/interpreter.rs` off
  `__slug_builtins__.X` references — they now exercise the
  top-level versions directly. `__internal__` namespace retained
  for slug-private registrations. Same session: stripped ~90
  gratuitous "Plan 28..." migration markers across 17 source
  files (the plan history lives in git log + this doc, not in
  inline comments).
- ~~Delete `prelude/toolchains/` after `slug init` migrates to
  `rules_*` references.~~ **Done 2026-05-01**. Investigation
  surfaced that `initialize_toolchains_build` in
  `app/slug_client/src/commands/init.rs` was never called — pure
  ...
  broken init tests so they match what `slug init` actually
  generates. (Plan 35.7 finished the scaffolding: `slug init` now
...
Add a first-class, Bazel-9-compatible builtins module layer for slug.
...
`wrappers.bzl`, but slug must not copy Bonanza's semantics wholesale.
The public behavior must be sourced from Bazel 9 and upstream
`rules_*`.

## Why This Exists

Slug currently has two mechanisms:
...
- Slug has a bundled builtins Starlark package, loaded deterministically
...
- The remaining Buck2 prelude machinery is either integrated into the
  builtins module, moved to a Slug-specific extension surface such as
...
3. **No Starlark `cc_common` action engine.** `cc_common.compile()`,
   ...
   remain Rust-backed.
4. **No language rules in slug builtins.** `cc_library`, `py_library`,
   `proto_library`, etc. come from external `rules_*` repos, not from
   slug's builtins package.
5. **No Buck2 prelude compatibility promise.** `@prelude//...` is an
   implementation detail unless a Slug-specific command such as BXL
...
4. Build a no-op prototype export such as
   `_slug_builtins_probe = "ok"` that is visible in a dedicated test
...
- New top-level `slug_builtins/` directory ships an `exports.bzl`
  entry-point (kept minimal — single probe symbol) plus an empty
  `BUILD.bazel`.
- `app/slug_external_cells_bundled/build.rs` and `lib.rs` register the
  contents as bundled cell `SLUG_BUILTINS`. `get_bundled_data()`
  returns it alongside `prelude` / `bazel_tools` /
  `local_config_platform` / `local_config_python`.
- `app/slug_common/src/legacy_configs/cells.rs` auto-registers
  `@slug_builtins` for every bzlmod project (mirrors the Phase 17
  `@local_config_platform` registration). Legacy non-bzlmod workspaces
  can opt in via `[external_cells] slug_builtins = bundled`.
- `app/slug_interpreter_for_build/src/interpreter/interpreter_for_dir.rs`:
  ...
  - appended to `implicit_imports` for both BUILD and `.bzl` paths in
    `parse()` (skipped inside the `slug_builtins` cell itself);
...
- Acceptance test
  ...
  (Bazel-mode fixture, no prelude) references `slug_builtins_probe`
  without a `load()` and gets `"slug-28-2-loader-ok"` written to a
  build artifact.
...
- The Plan 28.2 acceptance bullet about "rejecting loads outside the
  ...
  an external file into another file inside `@slug_builtins` is not
...
5. Keep `__slug_builtins__` as the Rust primitive namespace for now,
...
- `slug_builtins/exports.bzl` defines an explicit
...
- `app/slug_interpreter_for_build/src/interpreter/interpreter_for_dir.rs::create_env`
...
- Phase 28.4 hook in place: `rule_implementation_wrapper = _invoke_rule`
  ...
  `slug_analysis::run_analysis` to call this wrapper and start
...
- New tests:
  ...
  - `test_28_2_slug_builtins_visible_in_external_bzl` continues to
...
wrapper through `slug_analysis::run_analysis`. Once the wrapper
...
|-----------|--------------|-------|
| small helper globals used only by slug-owned builtins tests | proves export plumbing | Remove probes after tests or keep under `_slug_*`. |
...
`@slug_builtins//:exports.bzl` now installs a Starlark `struct`
...
helper (`_slug_target_platform_has_constraint`); the Rust impls in
`app/slug_build_api/src/interpreter/rule_defs/context.rs` and
`app/slug_build_api/src/interpreter/rule_defs/aspect/context.rs`
...
slug build time:
`app/slug_external_cells_bundled/build.rs` stages the
`slug_builtins/` source into `OUT_DIR/slug_builtins_staged/` and
...
- `slug_analysis::analysis::calculation::get_slug_builtins_module`
  loads `@slug_builtins//:exports.bzl` via DICE. Returns `None` for
...
- Acceptance test
  ...
  builds a Starlark rule that asserts `ctx.slug_facade_active ==
...
- `slug_analysis::analysis::aspect_calculation::execute_aspect`
  fetches `@slug_builtins` via the now-`pub(crate)`
  `super::calculation::get_slug_builtins_module` and looks up
...
- `_invoke_aspect(implementation, target, raw_ctx)` in
  ...
  Reuses the same `_slug_target_platform_has_constraint` shim, so
...
- Acceptance test
  ...
  `ctx.slug_facade_active == True`, `ctx.slug_facade_kind ==
...
- A second TLS slot in
  `slug_build_api::interpreter::rule_ctx_storage` carries the
...
- `_invoke_subrule(implementation, raw_ctx, **kwargs)` in
  ...
  `slug_facade_kind = "subrule"` so acceptance tests can prove
...
- `_slug_package_relative_label(raw_ctx, label_str)` in
...
top-level `_slug_tokenize` in `exports.bzl` mirrors the algorithm
byte-for-byte for ASCII input. Bound directly into the facade
without a closure (same pattern as
`_slug_target_platform_has_constraint`).
...
**runtime global-state access via a slug-internal Starlark builtin**.
The Rust impl in `context.rs` was deleted.

- New module
  `app/slug_interpreter_for_build/src/interpreter/functions/slug_runtime.rs`
  registers `slug_collect_code_coverage()` as an analysis-time
  Starlark global. The function reads
  `slug_build_api::interpreter::rule_defs::build_config::get_collect_code_coverage()`,
  the per-build `--collect_code_coverage` flag.
- Wired via `register_slug_runtime` in `register_analysis_natives`,
  so the global is reachable at module-eval time of
  `@slug_builtins//:exports.bzl` and at every analysis call.
- Naming: every global in this module is `slug_*`-prefixed. End-user
  code can technically call them (Starlark globals are flat) but the
  contract is "internal to `@slug_builtins`" — treat as private.
  Future slug-runtime hooks (e.g. `slug_compilation_mode()` for the
  upcoming `var` migration) follow the same naming.
- `_slug_coverage_instrumented(dep = None)` in `exports.bzl` reads
  the flag and returns it, ignoring `dep` (matches the Rust impl,
  which also ignored `dep`). When slug grows per-target
...
table now lives in **one** Starlark function — `_slug_make_substitutions`
...
- **Runtime global-state hook** (Stage 8):
  `app/slug_interpreter_for_build/src/interpreter/functions/slug_runtime.rs`
  gains four new globals — `slug_host_target_cpu`,
  `slug_host_cc_path`, `slug_get_all_defines`, and
  `slug_compilation_mode_for_label`. The first two delegate to the
  still-`pub` `host_target_cpu()` / `host_cc_path()` helpers in
  `context.rs` (still used by `cc_common`'s `ctx_cheat`).
  `slug_get_all_defines` allocates a Starlark dict from the
  `BUILD_CONFIG` defines map. `slug_compilation_mode_for_label`
...
- **TemplateVariableInfo provider lookup**: a fifth hook,
  `slug_collect_toolchains_template_vars`, takes the
...
Priority order in `_slug_make_substitutions` mirrors the deleted
...
The `$(VAR)` parser in `_slug_expand_make_variables` uses the same
"for-loop with explicit cursor and break" idiom Stage 7 introduced
for `_slug_tokenize` (Starlark has no `while`). Each iteration
...
`$(location ...)` expansion via `_slug_expand_location` (post-merge
...
slug_runtime hooks keep the unimplementable-in-Starlark pieces
Rust-side:

- `slug_collect_location_pool(raw_ctx, targets)` — builds the
...
- `slug_lookup_output_path(raw_ctx, label_str)` — lazily resolves
...
slug_runtime hooks:
- `slug_create_runfiles(files, transitive_files, symlinks,
  root_symlinks)` — delegates to the already-`pub`
  `default_info::create_runfiles`.
- `slug_collect_runfiles_into(runfiles, attr_value, want_data)` —
...
`slug_create_runfiles`, then walks `deps` / `runtime_deps` /
`data` attrs through `slug_collect_runfiles_into` based on the
...
- **The wrapper frame is visible in tracebacks.** Every error
  ...
  `slug_builtins/exports.bzl:152, in _invoke_rule` (or
...
- **Type-name regression on missing-attribute errors.** Before the
  ...
  later by switching the facade from `struct()` to a slug-internal
...
- **Migrated-method errors expose internal names.** A wrong-arg
  ...
  slug_builtins/exports.bzl._slug_target_platform_has_constraint`.
...
2. Change the analysis invocation path so Starlark rules call the
   ...
   - `app/slug_analysis/src/analysis/native_rule_analysis.rs`
...
The BUCK-file injection path is wired. `@slug_builtins//:exports.bzl`
...
`__slug_builtins__` in Phase 28.6.

Precedence in BUCK files (lowest to highest, last-write wins):

1. Rust primitives via `prelude/native.bzl`'s `native = struct(...)`
   scrape of `__slug_builtins__`.
2. `@slug_builtins//:exports.bzl::exported_native` entries.
3. User `load()` bindings at the use site (Starlark scope rules
   take care of this).

The probe entry `_slug_exported_native_probe` in `exported_native`
proves both halves:
...
- `tests/core/analysis/test_native_rules.py::test_28_5_exported_native_hidden_in_bzl`
  ...
  with "Variable _slug_exported_native_probe not found".
...
- A real migration that uses the new `exported_native` injection
  ...
  through `__slug_builtins__`. Pick one as a worked example before
...
pipeline and route BUILD-global construction through `@slug_builtins`.
...
`@slug_builtins//`. New BUILD package
`slug_builtins/utils/`. Load paths inside the copies rewritten
`@prelude//utils:X` → `@slug_builtins//utils:X`. New test
`test_28_6_slug_builtins_helpers_loadable` proves the new
namespace works.

**PR 3** (commit `7072e9bd`): drop the dead prelude rule pipeline.
Investigation surfaced that `prelude/rules.bzl::169` calls
`load_symbols(rules)` but slug stubs `load_symbols` as a no-op
(see
`app/slug_interpreter_for_build/src/interpreter/functions/load_symbols.rs`).
That means the entire `prelude_rule` → BUILD-callable chain does
nothing in slug — native rules (`alias`, `filegroup`, `genrule`,
`sh_*`, `test_suite`, etc.) come from
`app/slug_analysis/src/analysis/native_rule_analysis.rs::NativeRuleKind`,
...
- Deleted the `extra_globals_from_prelude_for_buck_files` method in
  ...
  (`slug_server/src/lsp.rs`, `slug_cmd_starlark_server/src/util/environment.rs`)
  to drop the now-redundant scrape.
- Deleted `prelude/native.bzl` and `prelude/paths.bzl` (PR 2
  migrated `paths.bzl` to `@slug_builtins//`).
...
- `asserts.bzl`, `utils/` (six files: source_listing, source_listing_impl,
  ...
  `examples/persistent_worker`, and `app/slug_external_cells_bundled`.
- `toolchains/` — referenced by `slug init` template strings.
- `bxl/` — Slug extension namespace.
...
- Delete `app/slug_interpreter/src/prelude_path.rs` and the `PreludePath`
  ...
  load-resolver chain. Multi-file Rust refactor; defer until slug init
  templates also drop their `@prelude` references.
- Restrict or delete the `__slug_builtins__` namespace in
  `globals.rs::base_globals`. Tests in
  `app/slug_interpreter_for_build_tests/src/interpreter.rs` use
  `__slug_builtins__.json.encode(...)` as a stable internal handle; those
  must migrate first.
- Delete `prelude/toolchains/` and update `slug init` templates to point
...
- moved behind a Slug-specific extension boundary;
...
| `prelude/native.bzl` | Constructs `native = struct(...)` from `__slug_builtins__` | Integrate into builtins export merge; shrink to compatibility shim, then delete if no caller needs it. |
...
| `prelude/bxl/` | Slug-specific BXL support | Keep, but make it explicitly owned by `slug bxl`, not by Bazel-compatible BUILD loading. |
| `prelude/toolchains/` and any remaining language/toolchain trees | Buck2 language/toolchain support | Remove from Bazel-compatible preload path; replace with Bazel `rules_*`, `@bazel_tools`, bzlmod, or Slug toolchain internals. |
| `app/slug_interpreter/src/prelude_path.rs` | Resolves `@prelude//:prelude.bzl` | Keep only if BXL or another Slug extension still needs it; otherwise delete or restrict to extension mode. |
| `app/slug_interpreter/src/file_loader.rs::get_native_symbols_from_prelude` | Scrapes `native` members from prelude evaluation | Delete once BUILD globals come from the builtins export merge. |
| `__slug_builtins__` namespace | Rust primitive namespace exposed to prelude | Rename or restrict after migration so it is not a public compatibility surface. |
...
2. Change BUILD-global construction so it no longer depends on:
   ...
   - `prelude/native.bzl` merging `__slug_builtins__`.
...
4. Move Slug-only helpers behind explicit Slug extension boundaries:
   - BXL helpers remain reachable from `slug bxl`;
   - non-Bazel debug/testing helpers use `_slug_*` names or test-only
     fixtures;
...
- Ordinary `slug build` does not need to evaluate
...
- `@prelude//...` loads in user BUILD/.bzl files are either unsupported
  with a clear Slug/Bazel-compatibility error or explicitly documented
  as Slug extension APIs.
...
- **Hidden BXL/prelude coupling.** Some `prelude/` files may still be
  needed by `slug bxl` or internal tooling even though Bazel-compatible
