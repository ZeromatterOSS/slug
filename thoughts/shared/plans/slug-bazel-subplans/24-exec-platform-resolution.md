...
`slug build <target> --config=remote` should "just work" against BuildBuddy
...
`compute_execution_platforms` (`app/slug_configured/src/execution.rs:571–636`)
is the only DICE key that produces the `ExecutionPlatforms` struct. It
reads exactly one input: the buckconfig key
`build.execution_platforms` (`app/slug_node/src/execution.rs:25–28`).
...
1. **`register_execution_platforms()` from MODULE.bazel.** Parsed in
   `app/slug_bzlmod/src/globals.rs:784–798`, accumulated through bzlmod
   resolution at `app/slug_common/src/legacy_configs/cells.rs:1050–1059,
   1092`, stored in the global
   `slug_bzlmod::REGISTERED_EXECUTION_PLATFORMS` (`app/slug_bzlmod/src/lib.rs:163`).
   Read accessor `get_registered_execution_platforms()` exists
   (`app/slug_bzlmod/src/lib.rs:174–178`) but is **never called from
   `compute_execution_platforms`**.

2. **`--extra_execution_platforms` CLI flag.** Defined in clap at
   `app/slug_client_ctx/src/common/build.rs:275–282`, serialized into
   the proto request at `build.rs:360`. The daemon receives the value
   in `slug_cli_proto::CommonBuildOptions` but **no daemon code reads
   it**.

### What constraint-matching machinery already exists

Once `ExecutionPlatforms` is non-empty, slug's constraint-based
resolver runs: `resolve_execution_platform_from_constraints`
(`app/slug_configured/src/execution.rs:740–784`) iterates candidates
and `check_execution_platform`
(`app/slug_configured/src/execution.rs:640–730`) verifies for each:

- `exec_compatible_with` constraints from the target node (read via
  `EXEC_COMPATIBLE_WITH_ATTRIBUTE`, internal id 4 at
  `app/slug_node/src/attrs/spec/internal.rs:92–105`).
...
when nothing matches (`app/slug_core/src/execution_types/execution_platforms.rs:21–25`).
...
(`app/slug_configured/src/execution.rs:538–568`) skips the constraint
...
(`app/slug_build_api/src/interpreter/rule_defs/provider/builtin/execution_platform_info.rs:77–172`).
...
`app/slug_node/src/attrs/spec/internal.rs`. Bazel allows individual
targets to override the platform's exec_properties via an
`exec_properties = {…}` attribute — slug can't read this today.
...
Sources read: `app/slug_bzlmod/src/globals.rs`, the executed Bazel
...
- `slug build <any target> --config=remote` against a Bazel-shape
...
2. **Automatic Execution Groups (AEGs).** `--incompatible_auto_exec_groups`
   is opt-in and has no current users among slug's tested
...
**Files**:
- `app/slug_cli_proto/daemon.proto`: add
  `repeated string extra_execution_platforms = 23;` to
  `CommonBuildOptions`. (Field 23 is the next free id.)
- `app/slug_client_ctx/src/common/build.rs::to_proto`: populate it from
  `self.extra_execution_platforms.clone()`.
- `app/slug_server/src/ctx.rs`: in `setup_data` (the path that builds
...
**File**: `app/slug_configured/src/execution.rs`.
...
Wired from `app/slug_server/src/ctx.rs::dice_updater` against the
`DiceTransactionUpdater`, parallel to `set_enabled_optional_validations`
(`app/slug_validation/src/enabled_optional_validations_key.rs` is the
canonical model).
...
async fn compute_execution_platforms(
    ...
    let registered = slug_bzlmod::get_registered_execution_platforms();
    ...
    Ok(Some(Arc::new(ExecutionPlatformsData::new(
        ...
        TargetLabel::testing_parse("@slug_settings//:registered_execution_platforms"),
        candidates,
...
async fn load_platform_candidate(
    ...
    if let Some(pi) = collection.builtin_provider::<FrozenPlatformInfo>() {
        ...
        let frozen_epi = FrozenExecutionPlatformInfo::for_native_execution_platform(
            ...
            // scoped allocation — see slug_node's arena patterns.
            heap,
        );
        return Ok(Some(
...
`app/slug_analysis/src/analysis/env.rs`) handles materialization of
...
- [x] `cargo check -p slug` passes
- [x] `slug_cli_proto::CommonBuildOptions` has field 23
...
- [ ] Unit test in `slug_configured/src/execution.rs` that registers two
...
#### Manual Verification:
- [x] `slug build //:print --extra-execution-platforms=@local_config_platform//:host`
...
- [ ] `slug audit execution-platforms` (if such a command exists, else
...
See `app/slug_validation/src/enabled_optional_validations_key.rs` for
...
**File**: `app/slug_node/src/attrs/spec/internal.rs`.
...
**File**: `app/slug_action_impl/src/context/run.rs` (the
...
**File**: `app/slug_core/src/execution_types/executor_config.rs`.
...
- [x] Unit test (`re_platform_fields_merged_with_target_overrides_platform_and_preserves_others`
      in `slug_core::execution_types::executor_config`) verifies
...
- [ ] Per-action `exec_properties` (`actions.run` kwarg) — **moved to
      ...
      exec_properties=...)`; slug's action API has the
...
**File**: `app/slug_action_impl/src/context/run.rs`.
...
- `ResolvedExecGroups` (in
  `app/slug_build_api/src/interpreter/rule_defs/context.rs:3547-3555`)
...
Code change: `app/slug_configured/src/execution.rs::legacy_exec_cfg`.
Commit: `01ce01f5`.
...
1. `slug killall && cd /var/mnt/dev/llvm-project/utils/bazel`.
2. `slug build @llvm-project//clang:clang --config=remote`.
...
- [x] `@llvm-project//clang:clang --config=remote` →
  ...
  Cold run (no prior bazel/slug had populated BB's cache for clang).
  ...
  *Follow-up*: the first clang invocation's Timing tab stayed stuck at
  ...
  `otherData.build_id` was hardcoded `"slug"` instead of the BES
  ...
  `app/slug_build_event_stream/src/translate.rs`) into the trace's
  `build_id` field, and pinned with two unit tests
  (`chrome_trace_build_id_uses_invocation_id` and
  `chrome_trace_default_build_id_is_slug` in `translate.rs`) so a
...
4. **Main plan** (`2026-01-21-slug-bazel-compatible-build-tool.md`):
...
- [x] Main plan's subplan table includes Plan 24
      (`2026-01-21-slug-bazel-compatible-build-tool.md` line 292) with
...
1. **Candidate starvation in the multi-group resolver.**
   `resolve_toolchain_types` (`app/slug_analysis/src/analysis/env.rs:1314-1317`)
   ...
   (`app/slug_configured/src/execution.rs:693-734`) never reach the
...
2. **Result discarded at storage time.**
   ...
   `app/slug_analysis/src/analysis/toolchain_resolution.rs:43-49`) is
...
fn resolve_toolchain_types(
    ...
    exec_group_defs: Vec<(String, slug_node::rule::ExecGroupDef)>,
...
`app/slug_analysis/src/analysis/toolchain_resolution.rs`.
...
**File**: `app/slug_analysis/src/analysis/toolchain_resolution.rs`.
...
**File**: `app/slug_build_api/src/actions/registry.rs`.
...
**File**: `app/slug_build_api/src/actions/registry.rs` and
`app/slug_action_impl/src/context/run.rs`.
...
slug_analysis` after the change and grepping for the
...
- [x] Three Rust unit tests in
      `app/slug_build_api/src/actions/registry.rs::select_action_executor_config_tests`
...
- [ ] On a real Bazel-shape workspace, declare a rule with two exec
      groups, run `slug build … --config=remote`, and confirm via the
...
- **Where the resolved platform is materialized.** Phase 8's
  ...
  (`app/slug_analysis/src/analysis/env.rs:923-1004`). The map is
...
- **Routing decision lives in `select_action_executor_config`**
  (`app/slug_build_api/src/actions/registry.rs`). Extracted from
...
- **cc_test as motivating consumer**: slug's prelude currently does
...
message it receives. Configure slug with `--bes_backend=grpc://127.0.0.1:<port>`.
...
information; for slug's emitter, extract from the action's executor
config in the action-completed event (verify the exact field by
inspecting `app/slug_build_event_stream/src/translate.rs`).
...
- **Skip if BES is disabled.** If slug runs with no BES configured,
...
slug/Buck2. Globals leak across builds, race under concurrency, and
...
- Plan 11: `thoughts/shared/plans/slug-bazel-subplans/11-toolchain-resolution.md`
- Plan 12: `thoughts/shared/plans/slug-bazel-subplans/12-stub-cleanup-and-exec-groups.md`
- Plan 25: `thoughts/shared/plans/slug-bazel-subplans/25-remote-execution-buildbuddy.md`
...
- Current resolver: `app/slug_configured/src/execution.rs:538–784`
- Registration globals: `app/slug_bzlmod/src/lib.rs:163–178`
- ExecutionPlatformInfo: `app/slug_build_api/src/interpreter/rule_defs/provider/builtin/execution_platform_info.rs`
- CommonBuildOptions proto: `app/slug_cli_proto/daemon.proto`
- Internal attributes: `app/slug_node/src/attrs/spec/internal.rs`
