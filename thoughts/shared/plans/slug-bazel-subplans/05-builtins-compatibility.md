...
This sub-plan covers ensuring Slug has all Bazel built-in rules, functions, and modules while removing Buck2-specific builtins that conflict with Bazel compatibility.

---

## Overview

Bazel 9.0 completed the "Starlarkification" effort, moving most language-specific rules to external repositories. Slug must:
...
| Rule | Description | Slug Status | Location |
|------|-------------|-------------|----------|
...
- [x] Implement `sh_test` rule (like sh_binary + ExternalRunnerTestInfo, `slug test` works)
...
| Function | Description | Slug Status | Location |
|----------|-------------|-------------|----------|
...
| Function | Description | Slug Status |
|----------|-------------|-------------|
...
`with_cfg.bzl` migrates away. Slug must match the Starlark builtins export that
...
| `host_info().slug` | `functions/host_info.rs` | Buck2 detection field | Rename to `host_info().buck2` or remove |
...
|------|----------|--------|
| `plugins.*` namespace | `plugins.rs` | Useful Slug-specific extension |
| `bxl.*` namespace | `slug_bxl/` | Separate feature, no conflict |
...
- [x] Remove `rules.bzl` load from `prelude/native.bzl` (2026-02-26): Buck2 language-specific rules (android, apple, cxx, erlang, etc.) are no longer loaded at startup. `native` struct is now `__slug_builtins__ + user_rules` only. All 356 build targets and 5 tests pass.
...
- [x] No Buck2-specific functions pollute Bazel-style BUILD files (2026-02-26: native.bzl only exposes __slug_builtins__ which contains Bazel-compatible rules)
...
Slug currently supports native `config_setting`, `constraint_value`,
...
- `app/slug_configured/src/configuration.rs`
...
- `app/slug_node/src/attrs/coerced_attr.rs`
...
/var/mnt/dev/slug/target/debug/slug --isolation-dir verify-config-setting-group build //sdk:sdk_contents
```
