...
Premise: Slug already captures the data. The event stream
(`app/slug_data/data.proto`, ~2400 lines) emits timed spans for every DICE
key — analysis, load, action, listing, materialization — with async poll-
time metrics per span. The observer layer (`slug_event_observer`) aggregates
...
- DICE activation data carries `TimeSpan` on every key completion; used by
  `app/slug_build_signals_impl/src/lib.rs:415-467`.
- Action stats (cache-hit %, local/remote/cached/fallback counts, excess
  cache misses): `app/slug_event_observer/src/action_stats.rs:30-106`.
- Critical path + slowest path with per-node potential-improvement:
  `app/slug_build_signals_impl/src/lib.rs:534-765`, backends at
  `app/slug_build_signals_impl/src/backend/`.
- Event log format: `.pb.zst` under `buck-out/v2/log/`, queryable via
  `slug log {what-ran,critical-path,slowest-path,summary,diff,replay}`.
...
- `slug log summary` (`app/slug_cmd_log_client/src/summary.rs`) is minimal.
- `slug log diff` (`app/slug_cmd_log_client/src/diff.rs`) exists but does
...
- `output_file_target: value=... registry.len=42` ERROR fires on every
  llvm build — `app/slug_interpreter_for_build/src/attrs/coerce/ctx.rs`.
  Non-fatal but cluttering and possibly indicative of an attr-coercion gap.
- Unbounded MPSC on build signals
  (`app/slug_build_signals_impl/src/lib.rs:942`) → OOM under long runs.
- Span tracker never evicts (`app/slug_event_observer/src/span_tracker.rs`)
  → daemon memory drifts across invocations.
- Event log reader decompresses entire file into memory
  (`app/slug_event_log/src/read.rs`) → 5-10s tax per `slug log …` call on
  llvm-sized logs.
- `EventSink::send` panics on failure (`app/slug_events/src/sink/*.rs:254-259`)
...
New module `app/slug_event_observer/src/build_summary.rs`. Pure aggregation
...
### 16.4 `slug log diff <before> <after>` — rollup diff (OPEN)

Parity source: no exact bazel equivalent; inspired by
`bazel analyze-profile --dump=text` + hand-rolled scripts CI teams write.

Extend `app/slug_cmd_log_client/src/diff.rs` (or add `--summary` mode if the
...
`app/slug_interpreter_for_build/src/attrs/coerce/ctx.rs`. Investigate,
...
`app/slug_build_signals_impl/src/lib.rs:942`. Switch to
...
`app/slug_event_observer/src/span_tracker.rs`. Evict on matching
`SpanEnd`; optionally cap total entries with an LRU for long-running
daemons.

**16.5.4** Streaming zstd reader — `app/slug_event_log/src/read.rs`. Use
`zstd::stream::read::Decoder` wrapping the file reader; don't allocate a
full `Vec<u8>`. Cuts 5-10s off every `slug log` invocation on llvm-sized
logs.

**16.5.5** `EventSink::send` returns `Result` —
`app/slug_events/src/sink/*.rs:254-259`. Existing call sites `.expect("…")`
...
Proto changes in `app/slug_data/data.proto` — new
`AnalysisConfigureStart/End`, `AnalysisAttrEvalStart/End`,
`AnalysisImplStart/End` under `SpanStartEvent.data`. Emission in
`app/slug_analysis/src/analysis/env.rs` around the three phases (current
...
`app/slug_build_signals_impl/src/lib.rs` emission). Fix if gaps.
...
- `run.sh <target> [--runs=N] [--cold|--warm|--both]` — invokes
  `/var/mnt/dev/slug/slug kill` between cold runs, drops caches between
  cold runs (with user confirmation), emits JSON rollup per run under
  `benchmarks/<YYYY-MM-DD>-<git-sha>/<target>/<run>/summary.json`.
- `compare.sh <baseline-dir> <current-dir>` — wraps `slug log diff` over
...
- Does `slug log diff` today do event-level diff, or is it a stub? If the
...
- `slug log diff baseline.pb.zst head.pb.zst` prints a ≥12-column table,
  stable across identical workspaces, `--fail-on-regression` usable in CI.
- `slug log summary` on the clang:clang llvm log produces a full rollup
