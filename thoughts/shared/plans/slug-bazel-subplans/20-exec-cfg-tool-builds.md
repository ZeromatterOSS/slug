...
**Concrete goal.** After this plan, `slug build
...
`app/slug_interpreter_for_build/src/interpreter/native_rules.rs:451-466`
...
`slug build --compilation_mode=opt @llvm-project//llvm:Support` fails
with:
...
Slug has two native compile-action entry points that rules_cc can
reach:
...
the Starlark-level rules_cc code that slug's native stub translates to
...
- Plan 19.3: `@local_config_platform//:host` declares
  ...
  "opt"}` via `app/slug_external_cells_bundled/build.rs`. The
...
- `slug build --compilation_mode=opt @llvm-project//llvm:Support`
...
failing build under a fresh daemon with `slug --verbose audit providers
...
1. `AttrConfigurationContextImpl::configure_target` vs
   `configure_exec_target` (`app/slug_node/src/attrs/configuration_context.rs`).
...
**Success criteria.** `slug build @llvm-project//llvm:Support`
succeeds with `genrule.tools` = `exec_dep`. `slug log what-ran` on a
...
1. rules_cc's `cc_common.compile` iterates `srcs` *plus* some
   ...
   is active. If slug's stub for that derived list isn't returning
...
3. rules_cc's `opt_compile_flags` feature implies
   `generate_dsym_file` which in turn requests a second compile. Slug
...
**Success criteria.** `slug build --compilation_mode=opt
...
After 20.1 and 20.2 land, re-run the canonical cold benchmark:
    ...
    BUCKD_STARTUP_TIMEOUT=180 SLUG=/var/mnt/dev/slug/slug \
      tools/bench/run.sh \
...
- `slug log what-ran | grep c_compile` on an exec-cfg tool-compile
  action contains `-O2 -D_FORTIFY_SOURCE=1 -DNDEBUG
  -ffunction-sections -fdata-sections -g0` plus the always-on
  baseline flags.
- `slug log what-ran` on a target-cfg library compile (llvm:Support
...
- Relevant source surface:
  - `app/slug_interpreter_for_build/src/interpreter/native_rules.rs:451`
    (`genrule.tools` declaration — flip site)
  - `app/slug_build_api/src/interpreter/rule_defs/cc_common/actions.rs:167`
    (`create_cc_compile_action`)
  - `app/slug_build_api/src/interpreter/rule_defs/cc_common/actions.rs:3432`
    (`create_compile_action`)
  - `app/slug_node/src/attrs/configuration_context.rs`
