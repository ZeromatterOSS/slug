...
conditions: hub `.slug_repo_complete` marker absent **and**
...
1. **Lockfile pre-seed at startup**
   ...
   - Each seeded extension is marked via
     `slug_bzlmod::mark_extension_spokes_seeded(ext_id)`.
...
3. **Single lockfile parse**
   - New `slug_bzlmod::cached_lockfile(workspace_root)` accessor backed
...
4. **Stale-stub recovery** in `extension_repo.rs`: detect
   `.slug_repo_complete` content of `"stub"` and re-materialise when a
...
- `cargo test -p slug_bzlmod --lib` → 163/163
- `cargo test -p slug_external_cells --lib` → 4/4
- `cargo test -p slug_common --lib` → 83/83
- `cargo test -p slug_interpreter_for_build --lib` → 50/50
...
- `app/slug_common/src/legacy_configs/cells.rs` — wire lockfile pre-seed
- `app/slug_interpreter_for_build/src/repository_ctx.rs` — `ctx.patch`
