...
Fixed by bumping `SLUG_BAZEL_VERSION` from `9.0.0` to `9.0.1`. That
...
`rctx.symlink(target_path, link_path)` branch (which slug implements
correctly with absolute paths) instead of the
`ln -sf relative_file(...)` fallback (which produces broken
`var/...`-prefixed targets in slug's stubbed bazel_lib).
...
its `.slug_repo_complete` marker prevents slug from re-running on
...
`relative_file` is loaded from `@bazel_lib//lib:paths.bzl`. In slug,
...
guard would route to `rctx.symlink` (which slug implements correctly)
...
2. Determine why `relative_file(absolute_target, absolute_link)`
   ...
   b. Slug's `rctx.path(string)` doesn't preserve the leading `/` when
...
- **If slug's `RepositoryPath::Display` strips a leading `/`**: fix
...
- **If `bazel_features.external_deps.repo_rules_relativize_symlinks`
  is the wrong default in slug's `bazel_features` stub**: flip it to
...
A more thorough fix is to implement `relative_file` natively in slug's
