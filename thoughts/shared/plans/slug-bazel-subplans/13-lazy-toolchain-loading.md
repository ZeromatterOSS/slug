...
Slug currently loads ALL registered toolchain packages eagerly on every build,
...
**`register_toolchains()`** — `app/slug_bzlmod/src/globals.rs:728-740`:
```rust
...
**`use_extension()`** — `app/slug_bzlmod/src/globals.rs:650`: records `dev_dependency`
on `ExtensionUsage`, but `aggregate_extensions()` (`app/slug_bzlmod/src/extensions.rs:128`)
...
`app/slug_common/src/legacy_configs/cells.rs:1114-1131`:
```rust
...
`app/slug_analysis/src/analysis/env.rs:386-491`:
...
1. `slug build //sdk:all_sdk_sources` succeeds on zeromatter
2. `slug build //sdk:sdk` reaches the analysis phase (may still fail for
...
- [x] `slug build //sdk:all_sdk_sources` → BUILD SUCCEEDED (2026-04-11)
- [x] `slug build //sdk:sdk` reaches "Error running analysis" (not loading errors) (2026-04-11)
...
**File**: `app/slug_bzlmod/src/globals.rs` (ModuleContext struct, ~line 55):
Update the field types in `ModuleContext` to match.

**File**: `app/slug_bzlmod/src/parser.rs` (~line 143):
Update the clone to use the new type.

#### 1b. Filter non-root dev_dependency items during collection

**File**: `app/slug_common/src/legacy_configs/cells.rs` (~line 1114-1131)
...
**File**: `app/slug_bzlmod/src/extensions.rs` (`aggregate_extensions()`, ~line 128)
...
**File**: `app/slug_bzlmod/src/pending_repo_cells.rs` (`pre_compute_extension_repo_cells`, ~line 148)
...
- [x] `cargo build -p slug` compiles without errors (2026-04-11)
...
**File**: `app/slug_analysis/src/analysis/env.rs` (~line 452)
...
#### Automated Verification:
- [x] `cargo build -p slug` compiles (2026-04-11)
- [x] Existing test suite passes (no regressions, 2026-04-11)

#### Manual Verification:
- [x] zeromatter: `slug build //sdk:all_sdk_sources` → BUILD SUCCEEDED (2026-04-11)
...
This is a more aggressive optimization that brings slug closer to Bazel's model.
...
Slug's DICE is also demand-driven, but `ensure_registered_toolchains_loaded()`
...
- Phases 1+2 successfully eliminated the *failure* mode (no more BUILD FAILED
  on irrelevant load errors), but did nothing for the *latency* mode — slug
...
`app/slug_bzlmod/src/lib.rs::REGISTERED_TOOLCHAINS` becomes
...
`app/slug_analysis/src/analysis/toolchain_resolution.rs::resolve_toolchains`
...
- `app/slug_bzlmod/src/lib.rs` — type change for `REGISTERED_TOOLCHAINS`,
  add `RegisteredToolchain` struct, update getter signature.
- `app/slug_common/src/legacy_configs/cells.rs:843-902` — populate structured
  list with `is_root` and `module_name`.
- `app/slug_analysis/src/analysis/env.rs::ensure_registered_toolchains_loaded`
  — filter to root-only; populate `DEFERRED_TOOLCHAINS`.
- `app/slug_analysis/src/analysis/native_rule_analysis.rs` — add
...
#### Automated Verification:
- [x] `cargo build -p slug` compiles (2026-05-04)
- [x] `cargo test -p slug_bzlmod --lib` — 161 passed (2026-05-04)
- [x] `cargo test -p slug_analysis --lib` — 11 passed (2026-05-04)
...
- `app/slug_bzlmod/src/lib.rs` — added `RegisteredToolchain { module, label,
  is_root }`; `REGISTERED_TOOLCHAINS` is now `Vec<RegisteredToolchain>`;
  `set_registered_toolchains` / `get_registered_toolchains` updated.
- `app/slug_common/src/legacy_configs/cells.rs` — collection loop now
...
- `app/slug_analysis/src/analysis/native_rule_analysis.rs` — added
...
Acceptance:
- `slug build //:bin` completes in < 60s (warm daemon)
- `bazel-external/` count after build is < 50 repos (vs. ~200+ pre-Phase-3)
- `slug build //:bin -v 5 2>&1 | grep "Skipping deferred"` shows transitive
...
- Prior slug work: `thoughts/shared/plans/slug-bazel-subplans/11-toolchain-resolution.md`
- Prior slug work: `thoughts/shared/plans/slug-bazel-subplans/12-stub-cleanup-and-exec-groups.md`
