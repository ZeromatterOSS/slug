...
Common unstable features Slug may use:
...
- Code under `@oss-disable` should be **deleted** (Meta-internal code not needed for Slug)
...
This cleanup removes the dual-mode OSS/internal distinction inherited from Buck2, since Slug is purely open source.
...
**File**: `slug_sandbox/src/linux.rs`
...
**File**: `slug_sandbox/src/macos.rs`
...
**File**: `slug_sandbox/src/windows.rs`
...
slug build --sandbox=true //...   # Default on
slug build --sandbox=false //...  # For debugging

# Sandbox strategy
slug build --sandbox_strategy=linux-sandbox //...
slug build --sandbox_strategy=symlink //...
```
...
Ensure slug works on Linux, Windows, and macOS.
...
#### 1. Query Command (`slug query`)

Query the unconfigured target graph:

```bash
slug query "deps(//src:main)"
slug query "rdeps(//..., //lib:foo)"
slug query "//..." --output=label
slug query "//..." --output=build
```

#### 2. Configured Query (`slug cquery`)

Query with configurations applied:

```bash
slug cquery "deps(//src:main)" --output=json
```

#### 3. Action Query (`slug aquery`)

Query the action graph:

```bash
slug aquery "//src:main" --output=jsonproto
```
...
- [ ] `slug query "deps(//...)"` returns dependencies
- [ ] `slug cquery` shows configured targets
- [ ] `slug aquery` shows actions
...
- `app/slug_build_api_tests/src/attrs.rs` - Update for `attr.*` API
- `app/slug_build_api_tests/src/actions.rs` - Update for `ctx.actions.*` API
- `app/slug_build_api_tests/src/nodes.rs` - Preserve DICE node tests
- ADD new module: `app/slug_bzlmod_tests/` for bzlmod resolution
...
- Slug repository: https://github.com/facebook/slug
