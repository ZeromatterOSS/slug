...
configuration instead of relying on Slug's current native approximation.

The immediate parity failure is `../reactor-repo-slug`'s `//sdk:sdk_contents`:
Slug builds the target, but Rust link actions miss LLVM toolchain-declared
...
Slug only preserves shallow feature/action names.
...
- Reactor SDK LLVM toolchain declarations:
...
- `app/slug_build_api/src/interpreter/rule_defs/cc_common/feature_config.rs`
...
- Slug physical output paths (`buck-out/...`) leak into Rust debug/source data
...
environments and Slug has historically returned an empty `_bs.env`.
...
- `cargo test -p slug_build_api_tests cc_common_feature_env_sets_expand_for_matching_action -- --nocapture`
- `cargo test -p slug_build_api_tests interpreter::rule_defs::cc_common -- --nocapture`
- `cargo test -p slug_build_api feature_config -- --nocapture`
- `cargo check -p slug`
- `cargo build -p slug`
- `git diff --check`
- `/run/host/var/mnt/dev/reactor-repo-slug`: bounded SDK smoke
...
The latest `../reactor-repo-slug` SDK smokes build `//sdk:sdk_contents` under
Slug, so the active Plan 58 frontier is output parity rather than a terminal
...
directories), while Slug leaves local action outputs writable (`755`). This is
not an SDK rule issue: it is local action output materialization/metadata
behavior. The systemic boundary is Slug's local executor after a successful
...
before output hashing/metadata capture so Slug records the same `555` modes
that Bazel exposes in the SDK tree, and make cleanup robust by allowing removal
of previously read-only output directories.

Verification for this slice:

- `cargo test -p slug_execute_impl test_normalize_local_output_permissions_sets_bazel_output_mode -- --nocapture`
- `cargo test -p slug_fs remove_all_readonly_dir -- --nocapture`
- `cargo test -p slug_build_api_tests cc_common_feature_env_sets_expand_for_matching_action -- --nocapture`
- `cargo check -p slug_build_api -p slug_execute_impl -p slug_fs`
- `cargo build -p slug`
- `/run/host/var/mnt/dev/reactor-repo-slug`: 30-minute-bounded SDK smoke
...
and libc++/libc++abi/libunwind runtime archive/search args. Slug's params only
...
content hashes different. Its Rust final-link params showed only Slug's old
...
public `cc_common.configure_features(ctx, cc_toolchain, ...)`, and Slug's public
...
Slug's analysis layer unconditionally returns `CcToolchainInfoNativeShim` for
...
startfile targets, Slug stayed in analysis on
...
Slug's metadata extractor currently converts every label substitution into a
...
`create_cc_compile_action`; Slug's native action bridge currently places output
...
`c++_header_parsing`, which Slug rejects because action categories must be
snake-case identifiers. The missing semantic is generic normalization from
Bazel C++ action names to Slug action categories for action registration only;
...
Slug action category, without changing feature-configuration action matching.
...
- Missing Bazel semantic: rule-based C++ toolchain metadata is provider data,
  but Slug currently requires normal target analysis to observe it.
...
`libc.so.6`; Slug still needs `libgcc_s.so.1`, `libm.so.6`, `libc.so.6`, and
...
- The selected LLVM toolchain legitimately default-enables the `module_maps`
  ...
  when the toolchain's `module_map` attr is present. Slug's native shim exposed
...
Slug's `EmptyCompilationContext` returned an empty list for that private field.
...
`map_each` callbacks that declare the second parameter; Slug always invokes
...
Bazel does not request that feature. Slug's
...
parity fix. Preserve default-enabled features for Slug's public
...
returns its private `_ModuleMapInfo` provider instance, which Slug's depset
typing currently reports as `struct`; Slug's native toolchain module-map shim
...
shows Slug had already configured LLVM runtime data targets such as
...
After rebuilding `target/debug/slug`, the restored baseline smoke
...
Additional ELF comparison of the last successful Slug SDK smoke
...
- `bin/zerobuf`, `bin/zerosystem`, and `bin/zm` are the same ELF kind in both
  ...
  remaining final-link content, including embedded Slug physical paths such as
...
- `lib/libzeromatter_ffi.so` is still a real link-surface mismatch. Bazel
  ...
  `libc.so.6`; Slug records `libgcc_s.so.1`, `libm.so.6`, `libc.so.6`, and
  `ld-linux-x86-64.so.2`. Both are `DYN` shared objects with `BIND_NOW`.
- Fresh Slug Rust final-link params still only show the plain rule-based flags
...
2. Implement Bazel feature selection
   ...
   Keep Slug's compilation-mode features (`fastbuild`, `dbg`, `opt`) aligned
...
6. Separate logical exec paths from physical materialization paths
   ...
   Slug may materialize the corresponding files under `buck-out/...`, but Rust,
...
reactor SDK still had four content hash differences. `rules_rust` does not call
the internal `CcToolchainFeatures.configure_features` method directly; it calls
public `cc_common.configure_features(ctx, cc_toolchain, ...)`. Slug's public
...
- `cargo test -p slug_build_api_tests interpreter::rule_defs::cc_common -- --nocapture`
  passed.
- `cargo test -p slug_build_api feature_config -- --nocapture` passed.
- `cargo check -p slug && cargo build -p slug && git diff --check` passed.
- Reactor `//sdk:sdk_contents` Slug smoke
...
- The three executables have no dynamic `NEEDED` entries in either Bazel or
  Slug output, so their hash drift is from different static/final link command
...
- `lib/libzeromatter_ffi.so` is still linked differently:
  ...
  `libc.so.6`; Slug has `libgcc_s.so.1`, `libm.so.6`, `libc.so.6`,
  `ld-linux-x86-64.so.2`.
- Slug params now include some rule-based link args such as
  ...
  wired.
- Slug params still omit the larger LLVM/rules_cc path-backed args:
...
- Plan item 4 is now the main blocker: rule-based `cc_args(data = ..., format =
  ...
  enough in Slug. Some plain action-config flags survive, but path/data-backed
...
- Latest smoke `plan58-actioncategory-sdk-20260511-235143` reaches execution with
  ...
  such as `@llvm//platforms/config:linux_x86_64_musl`. Slug's lightweight
...
- Owning subsystem: `cc_common_internal.cc_toolchain_features()` conversion from
  Starlark `ToolchainConfigInfo` into Slug's `CcToolchainFeatures`.
...
The referenced Slug path for `@llvm//runtimes:resource_directory` does not
...
`cc_toolchain.all_files`; Slug's shim is over-approximating the rule-based
toolchain `files` map.
...
... app/slug_build_api/src/interpreter/rule_defs/context.rs:2289
```
...
with `SIGSEGV` rather than returning a Slug error. The strace files show the
...
only converting them into Slug artifact groups at the command-line/input visitor
...
`SLUG_DISABLE_CC_TOOLCHAIN_FILE_SHIM=1` and did not crash at the 26s boundary.
...
return a Slug-only command-line/input carrier from `cc_toolchain.all_files` and
...
`/tmp/plan58-carrier-sdk-20260512-111124.log`, returned the Slug-only carrier
...
member is the crash-free Slug-only command-line/input carrier. That should let
...
single direct member is the Slug-only carrier reporting Starlark type `File`.
...
`llvm+llvm+llvm-project//vars.bzl`; Slug reported that file missing. The next
...
can analyze are also analyzable in Slug.
...
canonicalization; it is coming from Slug's global apparent-repo aliasing for
...
- A path-mapping test that verifies command-line artifact strings are
  Bazel-shaped while materialized files remain in Slug's output tree.
- A materialization test for Bazel-compatible output executable/read-only modes.

Then validate against reactor:

1. Build the relevant LLVM runtime/search-directory targets with Bazel and Slug.
2. Compare Rust final-link param files for `zerobuf`, `zerosystem`, `zm`, and
   `libzeromatter_ffi.so`.
3. Build `../reactor-repo-slug` `//sdk:sdk_contents` with Slug.
...
- Slug command-line artifact strings use Bazel exec paths where Bazel embeds
  them in outputs.
- `../reactor-repo-slug` `//sdk:sdk_contents` builds successfully with Slug and
...
- Output mode normalization can disturb incremental cleanup if files become
  ...
  Slug already knows an action has completed.
