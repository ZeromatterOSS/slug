...
The immediate parity failure is `../zeromatter-slug`'s `//sdk:sdk_contents`:
...
- `cargo build -p slug`
- `git diff --check`
- `/run/host/var/mnt/dev/zeromatter-slug`: bounded SDK smoke
...
The latest `../zeromatter-slug` SDK smokes build `//sdk:sdk_contents` under
...
- `cargo build -p slug`
- `/run/host/var/mnt/dev/zeromatter-slug`: 30-minute-bounded SDK smoke
...
- Zeromatter `//sdk:sdk_contents` Slug smoke
...
3. Build `../zeromatter-slug` `//sdk:sdk_contents` with Slug.
