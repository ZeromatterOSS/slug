...
- [x] `cargo test -p slug_build_api` passes
...
- [ ] Run `slug build //tests/manual_test:c`
...
- [ ] `ctx.rule.kind` returns "test_rule"
- [ ] `ctx.rule.attr.deps` contains aspect results (shadow graph)
- [ ] Run `slug build //tests/manual_test:d` (wider graph test)
...
- [ ] `cargo test -p slug_node` passes (aspect_type tests)
- [ ] `cargo test -p slug_analysis` passes (AspectKey tests)
- [ ] `cargo test -p slug_interpreter_for_build` passes (aspect.rs tests)
