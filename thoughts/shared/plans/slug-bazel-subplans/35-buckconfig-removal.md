...
> **Main Plan**:
> [2026-01-21-slug-bazel-compatible-build-tool.md](../2026-01-21-slug-bazel-compatible-build-tool.md)
>
> **Related**:
...
Today the slug source tree has roughly:
...
Slug is positioning itself as a Bazel-compatible build system but its own
...
- The `legacy_configs` parser in `app/slug_common/src/legacy_configs/`
  becomes dead code and is deleted.

**No `.slugrc`**, see "Knob disposition" below for the analysis.
...
1. The `legacy_configs` parser carries Slug-specific knobs (cell registration,
   ...
   existing `.bazelrc` parser at `app/slug_client_ctx/src/bazelrc.rs`
...
2. Prior plans treated `BUCK` as the bootstrap convention. The build-file
   discovery code at `app/slug_common/src/buildfiles.rs:26` already defaults
   to `["BUILD.bazel", "BUILD"]`; only an explicit `[buildfile] name = BUCK`
   opt-in keeps `BUCK` working in slug's own tree.
3. There was no MODULE.bazel at the slug repo root, so cell registration had
...
| `[external_cells]` | `prelude = bundled`, etc. | drop; `slug_external_cells_bundled` Rust crate auto-registers them |
| `[buildfile]` | `name`, `name_v2`, `extra_for_test`, `includes`, `package_includes` | drop `name`/`name_v2` (default to `BUILD.bazel`); `extra_for_test` survives as a `.slugrc`-style escape valve only if needed |
...
| `[rust]` | `default_edition` | drop (slug's own bootstrap config; not a slug build-system concern) |
...
| `[slug]` | ~50 daemon-tuning keys | `.bazelrc` flags with `--slug_*` prefix where they aren't already named that way |
| `[slug_re_client]` | RE backend address, TLS, headers | `.bazelrc` flags |
| `[slug_resource_control]` | cgroup/memory limits | `.bazelrc` flags |
| `[slug_health_check]`, `[slug_system_warning]` | diagnostics thresholds | `.bazelrc` flags |
...
resolve because slug's root `.buckconfig` opts in via
...
Considered + rejected. A parallel `.slugrc` was the obvious candidate for
Slug-only knobs that don't map to `.bazelrc`'s flag-form shape. But:
...
3. **One file, one parser, one source of truth.** A separate `.slugrc` would
   ...
   is worse than one.
4. **The `--slug_*` prefix is a sufficient namespace.** Bazel-compatible
   flags pass through to whatever Slug accepts; Slug-specific flags are
   visibly distinct.

If a future Slug feature genuinely needs structured config (unlikely for
build-tool knobs), introduce `.slugrc` then. Today: `.bazelrc` only.
...
3. Cross-reference with the Rust `BuckconfigKeyRef` consumers in
   `app/slug_common/src/legacy_configs/` and elsewhere. Any key with no
...
4. For knobs migrating to `.bazelrc`, decide flag names. Bazel-compatible
   ...
   `--watchfs_*`); Slug-prefixed otherwise (`--slug_starlark_max_callstack_size`).
...
- The list of `.bazelrc` flags Slug must accept is finalized; cross-checked
  against the existing `app/slug_client_ctx/src/bazelrc.rs` parser.
...
- Regression test added at `app/slug_common/src/buildfiles.rs::tests::test_buildfiles`:
  explicit `assert!(!default.contains("BUCK"))`.


#### Goal

Drop the Buck-shaped naming opt-in from slug's active source tree. Test
...
3. Update any explicit references in source: `grep -rn '"BUCK"' app/` for
   ...
   `slug_cmd_completion_client/src/complete/package.rs`). Test fixtures
...
4. Drop `[buildfile] name = BUCK` from `./.buckconfig`. Leave the file
   ...
   goes.
5. Verify each cell still resolves: `cargo build -p slug`,
...
- `cargo build -p slug` clean. Analysis suite at the same baseline as
...
- `examples/bzlmod_local_test/.buckconfig`: stripped `[cells]`,
  ...
  35.5. Verified `slug build //...` still passes.
...
1. **Multi-aliasing in root `.buckconfig.d/common.buckconfig`**: 8+ cell
   ...
   `gh_facebook_slug_shims_meta` cell. `bazel_dep` is keyed by `name`, so
   ...
   **skipped entirely** — so partial migration breaks aliases that haven't
   ...
   `app/slug/transition.bzl` — none of which are loaded by OSS users — but
...
- Root `.buckconfig` + `.buckconfig.d/common.buckconfig` cells/aliases.
  ...
  don't load the `.bzl` files that need them), or (c) extend `slug_bzlmod`
...
1. For workspaces that don't yet have a `MODULE.bazel`: create one. The slug
...
3. Translate `[cell_aliases]` → `bazel_dep(... repo_name = "alias")`
   ...
   `slug_common::legacy_configs::cells`).
4. `[external_cells] X = bundled` becomes a no-op: Plan 28's
   `slug_external_cells_bundled` Rust crate auto-registers the bundled
   cells.
...
- `cargo build -p slug` clean. Test suite + LLVM clean.
- `slug audit cell` (or equivalent) reports the same cell topology
  pre/post-migration for each active workspace.

### Phase 35.4: `.bazelignore` adoption (active workspaces)  [~½ day]  ✅ COMPLETE 2026-05-01

**Notes**:
- New module `slug_common::ignores::bazelignore` parses
...
- Interpreter ignores wired in `app/slug_common/src/ignores/all_cells.rs`:
  `.bazelignore` (read via `DiceFileComputations::read_file_if_exists`)
  takes precedence; falls back to `[project] ignore` during the
  deprecation window.
- File-watcher ignores wired in `app/slug_server/src/daemon/state.rs`:
  same precedence, reads via `slug_fs::fs_util::read_to_string_if_exists`
  at daemon startup.
...
1. Add `.bazelignore` parser to `slug_common`. Bazel's syntax is one path
...
3. Migrate slug's root `.buckconfig` ignore list (`app/slug_explain`,
   `app_dep_graph_rules`, `examples`, `integrations/rust-project/tests`)
   to a new `slug/.bazelignore`.
...
- Step (b): `[slug_re_client]` → `--remote_executor`/`--remote_header`/`--remote_instance_name`/`--tls_client_certificate` plus URL-scheme TLS detection. 5 examples migrated (vscode, buildbarn, buildbuddy, engflow, nativelink). End-to-end BuildBuddy RE smoke from `llvm-project/utils/bazel --config=remote` clean (8 actions, all remote). ✅ commit `33ebcc97`.
...
- Step (d): `[slug] file_watcher` → `--slug_file_watcher`. Field added to `DaemonStartupConfig` so a flag change forces a daemon restart; `FileWatcher::new` takes the value via an explicit `watcher_override: Option<&str>` param. 2 examples migrated (`toolchains/{go,python}_toolchain`).
...
1. Drop the `BuckconfigKeyRef` parse in
   `slug_configured::target_platform_resolution::get_target_platform_detector`;
...
2. Each migrated example gets a `.bazelrc` line `build --platforms=X`,
   ...
   `app/slug_client_ctx/src/common/target_cfg.rs`. It threads through
...
`slug_configured::execution`) accepts individual platform labels.
Every slug example wraps a single platform in the registration rule,
...
2. Drop the `BuckconfigKeyRef` parse in
   `slug_configured::execution::compute_execution_platforms` (the
...
without a `[parser]` or `[build]` section; `cargo build -p slug`
clean; LLVM Demangle + LLVM Support smoke clean; pytest analysis
baseline 122/5/3/1; per-example smoke via `slug audit cell` (offline)
and `slug build //...` (where buildable) for the 9 migrated examples.


**Drain (delete dead lookups) — DONE 2026-05-04**:
- `init.rs`: 39 → 6 lookups (kept `digest_algorithms` + `[slug_re_client]` snapshot)
- `slug_re_configuration/lib.rs`: 17 → 7 (kept addresses, tls, headers, instance_name);
...
- `slug_server/ctx.rs`: 25 → 0
- `slug_server/daemon/state.rs`: 17 → 0
- Smaller consumers (one big batch): slug_server_commands/build.rs,
  ...
  build_api/artifact_groups/calculation.rs, slug_test/command.rs

**Rename phase — DEFERRED**:
The remaining live `BuckconfigKeyRef` call sites outside `legacy_configs/`:
- `init.rs::DaemonStartupConfig::new`: `[slug] digest_algorithms` (3 examples
  set it → `--digest_function` Bazel flag)
- `init.rs::ReConfigSnapshot::from_config`: 5 `[slug_re_client]` keys
  (5 examples set them → `--remote_executor`/`--remote_header`/
  `--remote_instance_name`/`--tls_client_certificate`)
- `slug_re_configuration/lib.rs::SlugOssReConfiguration::from_legacy_config`:
  same 5 keys (read at daemon startup; overlaid by ReConfigSnapshot)
- `slug_node/execution.rs::EXECUTION_PLATFORMS_BUCKCONFIG`:
  `[build] execution_platforms` (9 examples → `--platforms`)
- `slug_configured/target_platform_resolution.rs`:
  `[parser] target_platform_detector_spec` (12 setters → `--platforms`)
- `slug_file_watcher/file_watcher.rs`: `[slug] file_watcher` (2 examples →
  `--slug_file_watcher`)
- `slug_common/buildfiles.rs`: `[buildfile] name` (shim, examples/bxl_tutorial),
  `[buildfile] extra_for_test` (test fixtures only — bucket-(B) candidate
  for 35.6a)

Doing this rename right requires:
1. Adding the new CLI flags to `app/slug_client_ctx/src/bazelrc.rs` /
...
this repo with no Bazel equivalent get a `--slug_*` flag.
...
1. **Delete** if no active-workspace `.buckconfig` in the slug repo sets
...
2. **Rename to Bazel-compat flag** if the key *is* set in an active
   ...
   `slug_re_client.engine_address` → `--remote_executor`,
   `slug.digest_algorithms` → `--digest_function`, etc.).
3. **`--slug_*` flag** only if the key is set AND there is no Bazel
...
1. `app/slug_re_configuration/src/lib.rs` (~80 keys, all
   `[slug_re_client]`) — most are Meta-internal RE knobs with no live
...
2. `app/slug_server/src/ctx.rs` — `[slug]`, `[build]`, `[ui]`, `[scuba]`,
   `[log]`, `[client]` keys.
3. `app/slug_server/src/daemon/state.rs` — `[slug]`, `[build]`,
   `[project]` (already covered by 35.4).
4. `app/slug_common/src/init.rs` — `[http]`, `[slug_system_warning]`.
5. `app/slug_file_watcher/src/{file_watcher,watchman/interface,edenfs/interface}.rs` —
   `[slug]`, `[project]` (.bazelignore-covered).
6. Smaller consumers: `slug_test/src/command.rs` (`[test]`),
   `slug_interpreter/src/{factory,allow_relative_paths,import_paths}.rs`,
   `slug_interpreter_for_build/src/interpreter/{buckconfig,interpreter_for_dir}.rs`,
   `slug_build_api/src/{configure_dice,materialize,build/build_report,artifact_groups/calculation}.rs`,
   `slug_execute_impl/src/materializers/deferred/clean_stale.rs`,
   `slug_configured/src/{nodes,target_platform_resolution}.rs`,
   `slug_node/src/execution.rs`,
   `slug_server_commands/src/build.rs`.
...
- Every `BuckconfigKeyRef` call site outside `legacy_configs/` is
  ...
  flag, or kept as `--slug_*` (rare).
- Every `.bazelrc` flag introduced/renamed has a help string and a
  documented default.
- No active-workspace `.buckconfig` contains any `[slug*]`, `[build]`,
...
3. **Bucket (B) — migrate via script.** Write a one-shot migration
   ...
   - Preserve `[buildfile] name = TARGETS.fixture` by emitting a
     fixture-local `.bazelrc` with `--slug_buildfile_name=TARGETS.fixture`
...
- `45dfe16a` — `slug init` scaffolds MODULE.bazel + .bazelignore (no .buckconfig); daemon logs deprecation warning on stray `.buckconfig` at workspace root.

Net delta: ~3000 LOC deleted from `app/slug_common/src/legacy_configs/`. Plan 02's bzlmod path is now the only cell-resolution path. The `BuckconfigKeyRef` lookup API (still used by `read_config()` Starlark API and `config_setting` matching) survives but is backed by an in-memory map populated only from `-c section.key=value` CLI flags — no file parsing.

Per Q1=B option (b), `read_config()` and `config_setting` buckconfig matching are intentionally retained as user-facing slug features (no Bazel equivalent for `read_config`; `config_setting` is migrated long-term but keeping the path costs ~10 LOC). The 34 bucket-C fixtures retain their `.buckconfig` files solely to feed values via the surviving lookup path; this is OK because `LegacyBuckConfig` no longer parses them — the fixture-relevant CLI overrides come through the test runner's `-c` flag.
...
Remove `app/slug_common/src/legacy_configs/` once nothing reads from
`.buckconfig` anymore.
...
3. Replace the `slug_common::legacy_configs::cells` cell-resolution path
...
- No `.buckconfig` files in the slug source tree (modulo bucket-(C)
  fixtures, which must each justify their existence in the appendix).
- `cargo build -p slug` clean. Full pytest + LLVM smoke clean.
- The deprecation warning fires for any user workspace that still ships
  a `.buckconfig`.

### Phase 35.7: Migration docs + slug init templates  [~½ day]  ✅ COMPLETE 2026-05-04

**Outputs**:
- `slug init` template rewritten in `45dfe16a` (Phase 35.6b commit): scaffolds `MODULE.bazel`, `BUILD.bazel`, `.bazelignore` instead of `.buckconfig` + `BUCK`. The "leave existing project alone" guard now keys on `MODULE.bazel`.
...
- Cross-references updated: main plan (Plan 35 marked complete + linked to migration guide), Plan 04 (prelude `.buckconfig` annotated as vestigial bundled-cell artifact, `BUCK` → `BUILD.bazel`), Plan 28 (`slug init` description updated to match the new scaffold).
...
1. Update the `slug init` templates in
   `app/slug_client/src/commands/init.rs` to scaffold `MODULE.bazel`,
...
2. Migration guide: a single doc in `thoughts/shared/docs/` (or the
   slug-build user docs) explaining the per-knob mapping. Includes an
...
- `slug init` produces a Bazel-shaped scaffold by default.
...
- **Plan 28 (Builtins)**: bundled-cell registration via
  `slug_external_cells_bundled` must auto-register the cells that
  `[external_cells] = bundled` lists today. Done in Plan 28.
- **`.bazelrc` parser**: already shipped at
  `app/slug_client_ctx/src/bazelrc.rs` (1122 LOC, full Bazel-compatible
...
- **User workspaces still on `.buckconfig`**: external projects that adopted
  Slug before this migration. Mitigation: keep parsing for one release with
...
- **Tests that explicitly construct `LegacyBuckConfig` in Rust**
  (`app/slug_interpreter_for_build_tests/` and similar): these aren't
...
- LLVM Demangle smoke clean (`cd /var/mnt/dev/llvm-project/utils/bazel
  && /var/mnt/dev/slug/slug build --config=generic_gcc --remote_executor=
...
- [x] **Delete `use_fbcode_metadata` and all Meta-internal-only code paths**
      ...
      `slug_re_configuration` sweep: deleted the field + parse, the
      ...
      blocks in `app/slug_execute/src/re/client.rs` and similar — most
      are gated at the workspace level by `is_open_source()` and don't
      compile in OSS today.

## Out of Scope

- Renaming `slug` itself: this plan keeps the tool name and its CLI shape;
  what changes is the per-workspace config format. Repo-wide renames are
  separate.
- Migrating users' workspaces: this plan only converts the slug source tree
...
- New `.slugrc` format: explicitly rejected (see "Why no `.slugrc`"). If a
  future feature truly needs structured config, file a follow-up plan.
- Buck1 compatibility: slug long since dropped Buck1 parity; this plan
