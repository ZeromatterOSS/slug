...
|-----------|------|--------|
| `module_ctx` Starlark object | `slug_interpreter_for_build/src/module_ctx.rs` | ✅ Complete interface, stub methods |
| Extension execution framework | `slug_interpreter_for_build/src/extension_execution.rs` | ✅ `build_module_context()`, tag serialization |
| Repository executor | `slug_bzlmod/src/repository_executor.rs` | ✅ http_archive, git_repository execution |
| DICE infrastructure | `slug_bzlmod/src/repository_execution.rs` | ✅ `RepositoryRuleExecutionKey` |
| Lockfile entries | `slug_bzlmod/src/lockfile.rs` | ✅ `RepositoryRuleLockEntry` |
| Repository invocations | `slug_bzlmod/src/repository_invocations.rs` | ✅ Thread-local registry |
...
**File**: `slug_bzlmod/src/repo_spec.rs` (new)
...
1. [x] Create `slug_bzlmod/src/repo_spec.rs`:
...
**Files to modify**:
- `slug_bzlmod/src/extension_execution_dice.rs` (new)
- `slug_bzlmod/src/lib.rs`
- `slug_interpreter_for_build/src/extension_execution.rs`
...
2. [x] Export from `slug_bzlmod/src/lib.rs`:
...
**Files modified**:
- `slug_bzlmod/src/repository_execution.rs` - DONE
- `slug_bzlmod/src/lib.rs` - DONE
- `slug_common/src/legacy_configs/cells.rs` - FUTURE (Phase 5e-5)
...
**Files modified**:
- `slug_bzlmod/src/lockfile.rs` - DONE
...
1. [x] Add `ExtensionRepoCellSetup` to `ExternalCellOrigin`:
   - [x] New struct in `slug_core/src/cells/external.rs` with canonical_name, extension_id, internal_name, spec_hash, materialized fields
   - [x] New `ExtensionRepo` variant in `ExternalCellOrigin` enum
   - [x] Update `Display` impl and match statements in `buck_out_path.rs`

2. [x] Create `slug_bzlmod/src/pending_repo_cells.rs`:
...
3. [x] Export new types from `slug_bzlmod/src/lib.rs`:
   - [x] `PendingRepoCell`, `RepoAlias`, `ExtensionCellDefinitions`
   - [x] All builder functions

4. [x] Integration with `cells.rs`:
   - [x] Import `ExtensionRepoCellSetup` from slug_core
   - [x] Add `register_extension_cells()` function for DICE integration

**Files modified**:
- `slug_core/src/cells/external.rs` - Added `ExtensionRepoCellSetup` and `ExtensionRepo` variant
- `slug_core/src/fs/buck_out_path.rs` - Handle new variant in match statements
- `slug_bzlmod/src/pending_repo_cells.rs` - NEW: Cell registration infrastructure
- `slug_bzlmod/src/lib.rs` - Export new types
- `slug_common/src/legacy_configs/cells.rs` - `register_extension_cells()` function
...
**Files to modify**:
- `slug_interpreter_for_build/src/module_ctx.rs`
- `slug_bzlmod/src/extension_execution_dice.rs`
...
| File | Status | Changes |
|------|--------|---------|
| `slug_bzlmod/src/repo_spec.rs` | ✅ **Done** | RepoSpec type, thread-local capture registry |
| `slug_bzlmod/src/extension_execution_dice.rs` | ✅ **Done** | ModuleExtensionExecutionKey (captures specs), late binding call, lockfile caching |
| `slug_bzlmod/src/module_extension_executor.rs` | ✅ **New** | ModuleExtensionExecutorImpl trait, LateBinding |
| `slug_bzlmod/src/repository_execution.rs` | ✅ **Done** | Add ExtensionRepoExecutionKey (lazy execution) |
| `slug_interpreter_for_build/src/repository_rule.rs` | ✅ **Done** | Capture RepoSpec in extension context |
| `slug_interpreter_for_build/src/module_ctx.rs` | ✅ **Done** | Temp working dir, delete_on_close, with_temp_working_dir() |
| `slug_interpreter_for_build/src/module_extension_executor_impl.rs` | ✅ **New** | ConcreteModuleExtensionExecutor |
| `slug_bzlmod/src/lockfile.rs` | ✅ **Done** | generatedRepoSpecs format |
| `slug_common/src/legacy_configs/cells.rs` | ✅ **Done** | Pending cell registration, lockfile-based extension cell resolution (Phase 5e-12) |
| `slug_bzlmod/src/pending_repo_cells.rs` | ✅ **New** | Cell definitions from extension results |
| `slug_core/src/cells/external.rs` | ✅ **Done** | ExtensionRepoCellSetup + ExtensionRepo variant + repo_spec_json field |
| `slug_bzlmod/src/lib.rs` | ✅ **Done** | Export new types |
| `slug_interpreter_for_build/src/lib.rs` | ✅ **Done** | Export + init late binding |
| `slug_external_cells/src/extension_repo.rs` | ✅ **Done** | Lazy materialization via ExtensionRepoExecutionKey |
...
**File**: `slug_bzlmod/src/extension_execution_dice.rs`
...
- [x] Documented the full Starlark execution flow (requires slug_interpreter_for_build)
...
`slug_interpreter_for_build::extension_execution::build_module_context()`. This will be
...
### 3. New Exports from slug_bzlmod

**File**: `slug_bzlmod/src/lib.rs`
...
**Goal**: Allow `ModuleExtensionExecutionKey::compute()` in `slug_bzlmod` to call
`build_module_context()` from `slug_interpreter_for_build` without a direct dependency.

**Problem**: The crate dependency direction is `slug_interpreter_for_build` → `slug_bzlmod`.
Extension execution needs to use `AggregatedExtension` from `slug_bzlmod` AND
`build_module_context()` from `slug_interpreter_for_build`.

**Solution**: Late binding pattern (like `EXTERNAL_CELLS_IMPL` in `slug_common`).

**Files created/modified**:
- [x] `slug_bzlmod/src/module_extension_executor.rs` (NEW):
...
- [x] `slug_bzlmod/src/lib.rs`: Export new types
- [x] `slug_bzlmod/Cargo.toml`: Add `slug_util` dependency for `LateBinding`
- [x] `slug_bzlmod/src/extension_execution_dice.rs`:
  - Updated `compute()` to call via late binding
  - Falls back to stub mode if late binding not initialized
- [x] `slug_interpreter_for_build/src/module_extension_executor_impl.rs` (NEW):
...
- [x] `slug_interpreter_for_build/src/lib.rs`:
  - Export new module
...
**Files modified**:
- `slug_bzlmod/src/extension_execution_dice.rs` - Added lockfile caching
...
**Implementation**:
- [x] Add `repo_spec_json: Arc<str>` field to `ExtensionRepoCellSetup` in `slug_core/src/cells/external.rs`
  - Stores serialized RepoSpec for lazy execution
- [x] Add `repo_spec_json: String` field to `PendingRepoCell` in `slug_bzlmod/src/pending_repo_cells.rs`
- [x] Update `build_extension_cells()` to serialize RepoSpec to JSON when creating pending cells
- [x] Update `register_extension_cells()` in `slug_common/src/legacy_configs/cells.rs` to include repo_spec_json
- [x] Add `slug_bzlmod` and `serde_json` dependencies to `slug_external_cells`
- [x] Modify `get_file_ops_delegate()` in `slug_external_cells/src/extension_repo.rs`:
...
**Files modified**:
- `slug_core/src/cells/external.rs` - Added repo_spec_json field
- `slug_bzlmod/src/pending_repo_cells.rs` - Added repo_spec_json field, updated build_extension_cells()
- `slug_common/src/legacy_configs/cells.rs` - Updated register_extension_cells()
- `slug_external_cells/Cargo.toml` - Added slug_bzlmod, serde_json, tracing dependencies
- `slug_external_cells/src/extension_repo.rs` - Implemented lazy materialization
...
**Implementation**:
- [x] Add `resolve_extension_repos_from_lockfile()` function in `slug_common/src/legacy_configs/cells.rs`:
...
- [x] Export `compute_bzl_transitive_digest()` from `slug_bzlmod` for consistent digest format

**Files modified**:
- `slug_common/src/legacy_configs/cells.rs`:
...
- `slug_bzlmod/src/extension_execution_dice.rs` - Made `compute_bzl_transitive_digest()` public
- `slug_bzlmod/src/lib.rs` - Export `compute_bzl_transitive_digest`
...
**Slug Patterns:**
- `slug_bzlmod/src/repository_executor.rs` - Repository execution (reuse for lazy execution)
- `slug_bzlmod/src/repository_execution.rs` - DICE key pattern
- `slug_bzlmod/src/repository_invocations.rs` - Thread-local registry pattern
- `slug_interpreter_for_build/src/repository_ctx.rs` - repository_ctx I/O methods
