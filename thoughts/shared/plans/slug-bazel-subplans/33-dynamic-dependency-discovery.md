...
>
> Adds a Slug-specific superset feature while preserving Bazel 9 parity for
...
seed inputs + toolchain + resolver scope
  ...
  -> normal Slug execution and caching
```

## Compatibility Position

Slug should treat dynamic dependency discovery as an explicit Slug extension,
...
- Existing rules_* integrations should not silently start depending on inferred
  deps unless the ruleset opts into a Slug-specific API.

Slug-native rules and future Slug extensions may opt into discovery APIs. Those
...
conservative input sets while still letting Slug build the actual required deps
before execution.
...
- **Query and target determination precision**: before discovery runs, Slug may
...
If discovery is modeled as a first-class DICE computation, Slug can sandbox the
...
- **Path rewriting errors**: scanner output may use exec-root, project-relative,
  absolute, symlinked, or virtual include paths. Slug must canonicalize these to
...
ask Slug/DICE for additional inputs while the action is running.
...
continuing after Slug uploads them. That is not compatible with the RE action
model.
...
- path output can be mapped back to Slug artifacts independent of the remote
...
- If the scanner runs as an RE action, its output artifact must be downloaded or
  parsed from CAS before Slug resolves discovered deps.
...
- A warm no-op dynamic-discovery fixture spends near-zero time in scanner
  execution.
- Discovery graph memory and validation cost are visible in `slug log summary`.
...
- If a discovery scanner runs remotely, Slug downloads or CAS-reads the scanner
...
- Should analysis-time discovery reuse anon targets exclusively, or should Slug
