...
- `use_extension()` parsing in `slug_bzlmod/src/globals.rs:614-644`
...
- `module_extension()` Starlark global in `slug_interpreter_for_build/src/module_extension.rs`
- `tag_class()` Starlark global with attrs parameter
- `module_ctx` Starlark object in `slug_interpreter_for_build/src/module_ctx.rs` with:
...
- Extension execution framework in `slug_interpreter_for_build/src/extension_execution.rs`:
  - `build_module_context()` - creates module_ctx from aggregated extension data
  - `tag_value_to_serialized()` - converts slug_bzlmod TagValue to SerializedTagValue
...
- `repository_rule()` Starlark global in `slug_interpreter_for_build/src/repository_rule.rs`
...
- `repository_ctx` Starlark object in `slug_interpreter_for_build/src/repository_ctx.rs` with:
