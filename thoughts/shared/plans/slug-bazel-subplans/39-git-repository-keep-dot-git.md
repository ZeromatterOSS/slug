...
`app/slug_bzlmod/src/repository_executor.rs:982-986`:
...
`app/slug_bzlmod/src/starlark_repo_rule_executor.rs:43-50` lists
...
is never executed; slug silently swaps in the native impl.

The `_external_repo_for_git_source` repos that Cargo crates depend on
are therefore created by slug's native code with `.git` stripped, even
...
returning the path. Reuse `slug_bzlmod::materialize_spoke_sync` (Plan
...
- `app/slug_bzlmod/src/repository_executor.rs` — drop `.git` removal
  block
- `app/slug_bzlmod/src/starlark_repo_rule_executor.rs` — Phase 2
  builtin-override logic
- (Possibly) `app/slug_bzlmod/src/repository_invocations.rs` — surface
