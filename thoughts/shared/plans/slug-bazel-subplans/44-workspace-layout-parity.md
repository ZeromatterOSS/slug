...
## Progress 2026-05-15: workspace-root external watcher invalidation

A focused `zeromatter-kuro //sdk:ffi_cpp_headers` smoke showed an avoidable
Plan 44 synchronization cost before target analysis:

```text
File changed: workspace//external
Directory changed: workspace//external
File changed: workspace//external/abseil-cpp
63487 additional file change events
```

The notify watcher already filters Slug/Bazel generated output components such
as `buck-out`, `bazel-external`, and `execroot`, but it did not filter
workspace-root `external/`. In the current Bzlmod-only layout that directory is
generated by Slug as Bazel-style external-repository convenience symlinks; it is
not source authority. Treating those symlink repair/materialization events as
source changes forces DICE invalidation and makes warm SDK smokes spend time
resynchronizing unrelated generated state.

This is a systemic workspace-layout fix, not an SDK workaround. The watcher
filter should classify `external/` with the other generated layout components
for Bzlmod Slug workspaces. The eventual Phase 3 move to Bazel's output-base
layout should reduce the need for a workspace-root generated `external/`, but
until then events under it must not invalidate source packages.
