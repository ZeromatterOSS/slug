...
`bazel_skylib` `select_file` looked for `glibc/build/c.s`, while Slug's
...
`object_files = object_files + additional_object_files`, and Slug rejects
`tuple + list`.

## Context

Discovered while investigating MODULE.bazel.lock churn (commit
`f9c5a705`): slug and bazel disagree on what files appear at the
...
- 178 slug-generated `external/<apparent>` symlinks were committed to
...
- New users running `bazel build` and `slug build` in the same
...
- External repos: materialized extension/BCR repos still live under the
  current slug external location until Phase 2 / Phase 3 moves them.
...
slug-prefix equivalents) at workspace root pointing into slug's
...
- Bazel's symlinks point into `<output_base>` because actions
  *execute* there (execroot). Slug runs actions from the workspace
  root, so `bazel-<ws>` becomes a self-pointing symlink. Either drop
  `bazel-<ws>` from slug's set or make it a no-op until Phase 3
  introduces an execroot.
- `bazel-bin` requires a stable target. Bazel uses `<config>/bin`
  with `<config>` being the host or target config. Slug's
  `<cfg_hash>` rotates per configuration — pick one (host) and
  document.

**Files**: one new module under `slug_server` or `slug_core`,
...
`<output_base>/external/<canonical>`. Slug currently writes
`<workspace>/bazel-external/<canonical>`. Move slug's location to the
equivalent of bazel's output_base.

**Target location** (proposal): `<workspace>/buck-out/v2/external/<canonical>`,
or a process-wide cache like `~/.cache/slug/external/<canonical>`
...
- `app/slug_bzlmod/src/repository_execution.rs` — materialization target path
- `app/slug_bzlmod/src/repository_executor.rs` — repo-rule output base
- `app/slug_bzlmod/src/spoke_materialization.rs` — spoke creation target
- `app/slug_client/src/commands/clean.rs` — clean command knows the path
- `app/slug_client/src/commands/run.rs` — runfile resolution
- `app/slug_common/src/legacy_configs/cells.rs` — symlink writer's target side
- `app/slug_core/src/cells.rs` — `ensure_external_symlink` target side
- `app/slug_external_cells/src/extension_repo.rs` — extension repo path
- `app/slug_file_watcher/src/notify.rs` — watch-or-skip logic
- `app/slug_interpreter_for_build/src/module_ctx/context.rs` — module_ctx working dir
- `app/slug_interpreter_for_build/src/module_ctx/methods.rs` — `mctx.path()` resolution
- `app/slug_interpreter_for_build/src/module_extension_executor_impl.rs` — extension exec path
- `app/slug_interpreter_for_build/src/repository_ctx.rs` — `rctx.path()` resolution
...
**Landed:**
- `app/slug_core/src/cells.rs::ensure_execroot_layout` builds
...
- `app/slug_execute_impl/src/executors/local.rs::exec` routes action
  cwd through that execroot.
- Tests: `slug_execute_impl test_exec_cmd_environment` updated to
...
- `slug_core --lib execroot` — 4/4 unit tests pass
...
- `slug_execute_impl --lib` — 44/44 tests pass.

**Collision-name filter** (`is_likely_runfiles_collision` in
`app/slug_core/src/cells.rs`): excludes `ci`, `docs`, `examples`,
...
- `16191ed9` moves that digest execroot to
  ...
  layout while preserving slug's `buck-out` output convention.
...
staging. Slug's shared synthesized execroot mirrors the workspace's
...
5. Garbage-collect `<buck-out>/v2/execroot/` on `slug clean`. No
...
- `app/slug_execute/src/execute/request.rs` — `CommandExecutionRequest`
  already exposes `inputs()`, `outputs()`. Add a helper that
  extracts the sorted top-level prefix set from inputs+tools.
- `app/slug_execute_impl/src/executors/local.rs::exec_once` — call
  the helper, hash the prefix set, build/reuse the execroot dir,
  pass its absolute path to `exec`.
- `app/slug_execute_impl/src/executors/local.rs::exec` — accept the
  per-action execroot path (replaces the
  `slug_core::cells::execroot_path` call from Phase 2.5).
- `app/slug_core/src/cells.rs` — keep `execroot_path` as a fallback
  ...
  for any action path.
- `app/slug_action_impl/` — confirm input-set extraction has
...
- **Tests asserting on `cwd`**: a few in `slug_execute_impl` already
...
- ZeroMatter `slug build //sdk:sdk_contents` advances past
...
external repos. Slug runs actions with `cwd = <workspace>` and
...
**Why this is large**:
- Action staging touches `slug_execute_impl` materialization, the
...
3. Drop `slug_core::ensure_external_symlink` — execroot has the
...
- Migrating `buck-out/` itself to `<output_base>/execroot/...` —
  buck-out is buck2's convention, slug inherits it intentionally.
...
- Phase 1: `slug build :foo && readlink bazel-bin && readlink bazel-out`
...
- Phase 2: `git ls-files | grep bazel-external` empty; clean checkout
  + `slug build` creates no `bazel-external/` at workspace root;
