...
Kuro's `filegroup` rule rejects this with
...
`app/kuro_interpreter_for_build/src/interpreter/native_rules.rs:184-216`
declares:
...
`app/kuro_interpreter_for_build/src/attrs/coerce/ctx.rs::coerce_path:519-525`
...
`app/kuro_node/src/attrs/coerced_path.rs::CoercedPath::Directory`
...
analysis at
`app/kuro_analysis/src/analysis/native_rule_analysis.rs::collect_source_files_from_configured_attr:813-820`
...
`app/kuro_interpreter_for_build/src/interpreter/native_rules.rs:192`
...
`app/kuro_interpreter_for_build/src/attrs/coerce/ctx.rs:400-433`: when
...
files; add a unit test in `kuro_analysis` to lock in the behavior.

## Tests

### Unit (kuro_interpreter_for_build)

In `app/kuro_interpreter_for_build/src/attrs/coerce/ctx.rs` under
`#[cfg(test)]`:
...
list. Run via `kuro test fbcode//kuro/tests/core/build:dir_filegroup_test`
(naming TBD).

### End-to-end (zeromatter)

Re-run `kuro build crates__zerocopy-0.8.42//:_bs --target-platforms=//bazel/platforms:linux-gnu-host`
...
`app/kuro_interpreter_for_build/src/attrs/`):
...
- `cargo test -p kuro_interpreter_for_build --lib` green (existing
  + new tests).
- `cargo test -p kuro_analysis --lib` green (existing + new test).
...
- ZeroMatter `kuro build crates__zerocopy-0.8.42//:_bs` advances past
...
- `app/kuro_interpreter_for_build/src/attrs/coerce/ctx.rs` — ~15
  lines (defer-to-source check + cleanup of unused
  `is_source_file` local).
- `app/kuro_analysis/src/analysis/native_rule_analysis.rs` — test
  only (~30 lines).
- New test fixture under `tests/core/build/` — small.
- `thoughts/shared/plans/kuro-bazel-subplans/44-workspace-layout-parity.md`
