...
Kuro currently has **4 distinct .bzl loading paths**, a **synchronous pre-DICE extension executor** with a subset of Starlark globals, and **deep dependency on `.buckconfig` files** for cell definitions. This diverges from Bazel's unified Skyframe-based architecture where:
...
Additionally, Kuro's lockfile format diverges from Bazel 9.0's actual format in several ways that break cross-tool compatibility.

This plan unifies Kuro's execution environments to match Bazel's architecture and ensures lockfile format compatibility.
...
Make Kuro's `MODULE.bazel.lock` format match Bazel 9.0's lockfile format exactly. A lockfile written by Kuro should be parseable by Bazel, and vice versa.
...
**Kuro current format** (`lockFileVersion: 24`):
```json
{
  "lockFileVersion": 24,
...
| Aspect | Kuro Current | Bazel 9.0 | Fix |
|--------|-------------|-----------|-----|
...
| `repositoryRules` | Present (Kuro-specific) | **Not present** | Remove or move to separate file |
...
**File**: `app/kuro_bzlmod/src/lockfile.rs:60`

- [x] Change `LOCKFILE_VERSION` from `24` to `26`

#### Step 2: Remove deprecated top-level fields

**File**: `app/kuro_bzlmod/src/lockfile.rs`

Bazel 8.0+ removed `moduleFileHash` and `moduleDepGraph` from the lockfile. Kuro should match.
...
`repositoryRules` is a Kuro-specific addition not in Bazel's format. Move to a separate Kuro-specific cache file if still needed.
...
- [ ] If repo rule caching is still needed, create a separate `MODULE.bazel.lock.kuro` or use DICE-based caching
...
- [x] `cargo test -p kuro_bzlmod` passes (update lockfile tests for new format) — all 158 tests pass
- [x] Lockfile written by Kuro matches Bazel 9.0 JSON structure (validate with `jq` or test)
- [x] Old-format lockfiles are read without error (treated as needing refresh)

**Manual:**
- [ ] Generate a lockfile with Kuro, validate the JSON structure matches Bazel 9.0
- [ ] Compare field names, nesting, and value formats with a real Bazel 9.0 lockfile
- [ ] Verify Bazel 9.0 can read a Kuro-generated lockfile (at least without error)
...
**File**: `app/kuro_bzlmod/src/parser.rs`
...
**New function**: `pre_compute_extension_repo_cells()` in `app/kuro_common/src/legacy_configs/cells.rs`
...
**Automated:**
- [x] `cargo build -p kuro` succeeds
- [x] `cd tests/manual_test && ../../kuro build //:hello_bin` succeeds (extension repos pre-registered)
- [x] `cd tests/manual_test && ../../kuro build //:hello_cc_proto` succeeds (313 commands)
- [x] All 158 kuro_bzlmod unit tests pass
...
**File**: `app/kuro_bzlmod/src/repository_execution.rs`
...
**Files to modify:**
- `app/kuro_bzlmod/src/sync_extension_executor.rs` — DELETE
- `app/kuro_interpreter_for_build/src/sync_extension_executor_impl.rs` — DELETE
- `app/kuro_bzlmod/src/lib.rs` — Remove `SYNC_EXTENSION_EXECUTOR_IMPL` export
- `app/kuro_interpreter_for_build/src/lib.rs` — Remove `sync_extension_executor_impl` module and init
- `app/kuro_common/src/legacy_configs/cells.rs` — Remove `try_execute_extension_sync()` function
- `app/kuro_bzlmod/src/repo_spec.rs` — Remove `CURRENT_BZL_CONTEXT` thread-local, `set_bzl_context()`, `get_bzl_context()`
...
**Automated:**
- [x] `cargo build -p kuro` succeeds
- [x] `cd tests/manual_test && ../../kuro build //:hello_bin` succeeds with DICE-only extension execution
- [x] `cd tests/manual_test && ../../kuro build //:hello_cc_proto` succeeds (313 commands)
- [x] 159 kuro_bzlmod unit tests pass
...
**File**: `app/kuro_interpreter/src/prelude_path.rs`
...
**File**: `app/kuro_common/src/invocation_roots.rs`
...
**Automated:**
- [x] `cargo build -p kuro` succeeds
- [x] `cd tests/manual_test && ../../kuro build //:hello_bin` succeeds without `[cells]` in `.buckconfig`
- [x] `cd tests/manual_test && ../../kuro build //:hello_cc_proto` succeeds (313 commands)

**Manual:**
- [ ] Verify project works with empty `.buckconfig` (or no `.buckconfig` at all)
- [ ] Verify `kuro audit cell` shows all cells from MODULE.bazel
...
Move build configuration from `.buckconfig` to `.bazelrc` and/or `MODULE.bazel` where Bazel has equivalents. Keep `.buckconfig` as a Kuro-specific config file for settings with no Bazel equivalent.
...
**Keep in Kuro config** (no Bazel equivalent):
| Setting | Why |
|---------|-----|
| `[kuro] daemon_buster` | Kuro-specific daemon management |
| `[kuro] digest_algorithms` | Kuro-specific hash config |
| `[kuro] materializations` | Kuro-specific materialization strategy |
| `[kuro_resource_control] *` | Kuro-specific cgroup management |
| `[kuro_system_warning] *` | Kuro-specific health monitoring |
| `[kuro] file_watcher` | Kuro-specific file watcher backend |
...
After removing the sync executor, Kuro should have **2** loading paths (down from 4):
...
|-----------|------|-----------|
| Lockfile format | `app/kuro_bzlmod/src/lockfile.rs` | entire file |
| Cell resolver construction | `app/kuro_common/src/legacy_configs/cells.rs` | 356-573 (main flow) |
| CellsAggregator | `app/kuro_common/src/legacy_configs/aggregator.rs` | 48-158 |
| Sync extension executor (DELETE) | `app/kuro_interpreter_for_build/src/sync_extension_executor_impl.rs` | entire file |
| Sync executor trait (DELETE) | `app/kuro_bzlmod/src/sync_extension_executor.rs` | entire file |
| DICE extension execution | `app/kuro_bzlmod/src/extension_execution_dice.rs` | 336-488 |
| Repo execution key | `app/kuro_bzlmod/src/repository_execution.rs` | `ExtensionRepoExecutionKey::compute()` |
| Canonical name construction | `app/kuro_bzlmod/src/extension_execution_dice.rs` | 556-594 |
| CellResolverKey (DICE) | `app/kuro_common/src/dice/cells.rs` | 54-84 |
| ExternalCellOrigin | `app/kuro_core/src/cells/external.rs` | 22-34 |
| Pending repo cells | `app/kuro_bzlmod/src/pending_repo_cells.rs` | 123-207 |
| MODULE.bazel parser | `app/kuro_bzlmod/src/parser.rs` | ExtensionUsage parsing |
| Project root discovery | `app/kuro_common/src/invocation_roots.rs` | 72-115 |
| Prelude discovery | `app/kuro_interpreter/src/prelude_path.rs` | 41-50 |
| .buckconfig parsing | `app/kuro_common/src/legacy_configs/configs.rs` | entire file |
| Config file paths | `app/kuro_common/src/legacy_configs/path.rs` | 39-57 |
| Thread-local bzl context (DELETE) | `app/kuro_bzlmod/src/repo_spec.rs` | 126+ |
