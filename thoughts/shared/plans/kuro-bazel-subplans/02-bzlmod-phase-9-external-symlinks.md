...
ensuring Bazel-compatible external repository access for tools outside of Kuro.
...
Kuro downloads bzlmod modules to:
```
~/.cache/kuro/registry/bcr.bazel.build/modules/{name}/{version}/source/
```
...
The `FileOpsDelegate` approach works for Kuro's internal file operations, but fails when:
...
3. **Build actions** - Commands spawned by Kuro can't use `FileOpsDelegate`
...
**`app/kuro_bzlmod/src/synthetic_repos.rs`**
...
3. **Manual cleanup**: `kuro clean` should optionally remove `bazel-external/`
...
1. **First run after update**: Kuro creates `bazel-external/` with symlinks
2. **Manual symlinks**: Can be removed after Kuro creates them
3. **`.gitignore`**: Add `bazel-external/` (it's generated, like Bazel's)

### Recommended `.gitignore` Addition

```gitignore
# Kuro/Bazel external repositories
bazel-external/
```
...
```bash
# After running kuro build
ls -la bazel-external/
# Should show symlinks to ~/.cache/kuro/...

# Verify symlink targets
readlink bazel-external/rules_cc+0.2.16
# Should show ~/.cache/kuro/registry/bcr.bazel.build/modules/rules_cc/0.2.16/source
```
...
1. **Should we support both `~` and `+` delimiters?** Bazel 7.x uses `~`, Bazel 8.0+ uses `+`.
   Kuro targets Bazel 9.0 compatibility, so `+` is recommended.
