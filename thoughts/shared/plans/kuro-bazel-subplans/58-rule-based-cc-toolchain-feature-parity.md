...
The immediate parity failure is `../reactor-repo-kuro`'s `//sdk:sdk_contents`:
...
- `cargo build -p kuro`
- `git diff --check`
- `/run/host/var/mnt/dev/reactor-repo-kuro`: bounded SDK smoke
...
The latest `../reactor-repo-kuro` SDK smokes build `//sdk:sdk_contents` under
...
- `cargo build -p kuro`
- `/run/host/var/mnt/dev/reactor-repo-kuro`: 30-minute-bounded SDK smoke
...
- Reactor `//sdk:sdk_contents` Kuro smoke
...
3. Build `../reactor-repo-kuro` `//sdk:sdk_contents` with Kuro.
...
- Kuro command-line artifact strings use Bazel exec paths where Bazel embeds
  them in outputs.
- `../reactor-repo-kuro` `//sdk:sdk_contents` builds successfully with Kuro and
