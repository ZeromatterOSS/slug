...
prevents undeclared or overlapping writes, and gives Kuro a clear network
...
Kuro already has a `kuro_sandbox` crate and a `--sandbox` flag, but the current
implementation is a prototype rather than the final strategy.

Observed implementation shape:

- `app/kuro_sandbox/src/lib.rs` applies Linux user and mount namespace setup via
...
- `app/kuro_execute_impl/src/executors/local.rs` builds a `SandboxSpec` from
...
Kuro should borrow the behavioral contract and tests, not necessarily the exact
runner structure.
...
Landlock is the best candidate for Kuro's default Linux fast path.
...
- It does not create a virtual execroot. Kuro still needs a per-action input and
...
not a good default sandbox substrate for Kuro.
...
`cap-std` and related crates are useful for Kuro-owned code that can be written
...
- Job Objects for process-tree lifetime and resource control. Kuro already has
  Job Object process management in `app/kuro_execute_local/src/win/`.
...
Implement a small `kuro-sandbox-helper` binary plus a Rust library API used by
the local executor.
...
- Final `buck-out` output paths should not be writable by the action. The action
  writes private outputs; Kuro commits them after validation.
...
- Move namespace setup from `pre_exec` into `kuro-sandbox-helper`.
...
Sandboxing must not permanently give up Kuro's local execution performance.
...
- Should `--sandbox_default_allow_network` follow Bazel's flag name exactly but
  default to deny for ordinary actions, or should Kuro keep Bazel's default and
