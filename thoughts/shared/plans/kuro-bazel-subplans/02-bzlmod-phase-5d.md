...
**Kuro Alignment:**
- Download cache: `~/.cache/kuro/downloads/{integrity}` (SRI format - compatible)
...
**File**: `kuro_bzlmod/src/repository_invocations.rs` (new)
...
**File**: `kuro_bzlmod/src/repository_execution.rs` (new)

Follow the `GitFileOpsDelegateKey` pattern from `app/kuro_external_cells/src/git.rs:377-408`:
...
async fn execute_repository_rule(
    ...
    cancellations: &CancellationContext,
) -> kuro_error::Result<Arc<RepositoryRuleResult>> {
...
- `kuro_bzlmod/src/fetch.rs` - `SourceFetcher` for HTTP downloads with retry
- `kuro_bzlmod/src/cache.rs` - `ModuleCache` for content-addressable download storage
- `kuro_bzlmod/src/integrity.rs` - SRI integrity verification
- `kuro_http/src/client.rs` - HTTP client with redirect handling
...
**File**: `kuro_common/src/legacy_configs/cells.rs`
...
1. Create `kuro_bzlmod/src/repository_invocations.rs`
...
1. Create `kuro_bzlmod/src/repository_execution.rs`
...
|------|---------|---------|
| `kuro_bzlmod/src/repository_invocations.rs` | Invocation registry | New file |
| `kuro_bzlmod/src/repository_execution.rs` | DICE key + execution | New file |
| `kuro_interpreter_for_build/src/repository_rule.rs` | Rule definition | Modify `invoke()` to record |
| `kuro_interpreter_for_build/src/repository_ctx.rs` | I/O methods | May need async variants |
| `kuro_common/src/legacy_configs/cells.rs` | Cell registration | Add RepositoryRule origin |
| `kuro_bzlmod/src/lockfile.rs` | Caching | Add repository rule entries |
...
- [ ] Downloaded archives cached in `~/.cache/kuro/downloads/`
...
**Kuro Patterns:**
- `app/kuro_external_cells/src/git.rs:377-408` - `GitFileOpsDelegateKey` DICE pattern
- `app/kuro_build_api/src/actions/calculation.rs:656-683` - `BuildKey` DICE pattern
- `app/kuro_bzlmod/src/fetch.rs` - Existing download infrastructure
