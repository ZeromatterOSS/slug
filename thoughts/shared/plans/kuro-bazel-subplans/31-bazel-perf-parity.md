...
> down to ~2.9 s, dropping kuro's default-mode wall on
...
| Scenario             | kuro (post-30) | bazel  | gap    | target after plan 31 |
|----------------------|----------------|--------|--------|----------------------|
...
across daemon restarts. Kuro's DICE — the type that holds
...
kuro's `execute=8.2 s` and a large slice of the `materialize=4.6 s`
...
1. **Spurious file-watcher invalidation work.** Back-to-back kuro runs
   logged `File changed: llvm-project-overlay//bazel-bin/llvm/llvm`
   and `File changed: llvm-project-overlay//kuro_warm1.log` — files we
   ...
   **not** filtered, so when kuro runs in a workspace that has been
...
- Skipping BES connect on no-op builds (was a candidate phase 31.4).
  ...
  invocation page even on no-op builds, so kuro's "skip if zero
...
```
DICE BuildKey(ActionKey)  — app/kuro_build_api/src/actions/calculation.rs:686
  → build_action_impl                         (line 81)
  → BuckActionExecutor::execute
  → CommandExecutor::action_cache             — app/kuro_execute/src/execute/command_executor.rs:124
  → ActionCacheChecker::maybe_execute         — app/kuro_execute_impl/src/executors/action_cache.rs:266
  → query_action_cache_and_download_result    (line 74)
  → re_client.action_cache(digest.dupe())     (line 107)
  → RemoteExecutionClientImpl::action_cache   — app/kuro_execute/src/re/client.rs:228
  → get_action_result(...)                    (line 977-990)  ← network round-trip
```

Existing on-disk SQLite infra to copy from:

- `app/kuro_execute_impl/src/sqlite/materializer_db.rs` — schema v7,
  table at `buck-out/v2/cache/materializer_state`. Holds materializer
  artifact metadata.
- `app/kuro_execute_impl/src/sqlite/incremental_state_db.rs` — schema
  v0, table at `buck-out/v2/cache/incremental_state`. Holds
  `(run_action_key, short_path) → content_hash_path` for content-based
  output paths.
- `kuro_common::sqlite::sqlite_db::{SqliteDb, SqliteTable, SqliteTables}`
  — generic infrastructure both use. Identity-based corruption
  detection, `kuro.sqlite_<name>_version` buckconfig escape hatch.

Action digest type: `ActionDigest = CasDigest<ActionDigestKind>`
(`app/kuro_execute/src/execute/action_digest.rs:27`). SHA256 of the
serialized RE `Action` proto, computed in `re_create_action`
(`app/kuro_execute/src/execute/command_executor.rs:258`). Globally
...
`app/kuro_file_watcher/src/notify.rs:368`, calls
...
`bazel-external`). When kuro runs in a workspace that bazel also
...
archives there for **kuro's own use** (created by
`resolve_bzlmod_dependencies` at
`app/kuro_common/src/legacy_configs/cells.rs:751`). Filtering it
...
```
run_streaming_fallible       — app/kuro_server/src/daemon/server.rs:407
...
`app/kuro_server_ctx/src/template.rs:68` wraps every command in a
...
(`app/kuro_server/src/daemon/state.rs`) is long-lived (`Arc<DaemonState>`).
...
- BES subscriber moves to a daemon-side `EventSubscriber`-equivalent
  ...
  (`app/kuro_client_ctx/src/events_ctx.rs:453`) is the structural
...
**File**: `app/kuro_execute_impl/src/sqlite/action_cache_db.rs` (new)
...
buckconfig key: `kuro.sqlite_action_cache_state_version`.

**File**: `app/kuro_execute_impl/src/sqlite/tables/action_cache_table.rs` (new)

Companion table-helper, modeled on
`tables/materializer_state_table.rs`.

**File**: `app/kuro_execute_impl/src/sqlite/mod.rs`

Add `pub mod action_cache_db;`.

**File**: `app/kuro_execute_impl/src/executors/action_cache.rs`
...
`kuro.local_action_cache_ttl_days`.

**File**: `app/kuro_server/src/daemon/state.rs`
...
the existing `kuro_data::CacheQuery` event so the BES feed reports
...
- [ ] `cargo build -p kuro_execute_impl` clean.
- [ ] New unit tests in `action_cache_db.rs`:
      put/get/expire/identity-reset round-trips.
- [ ] Existing test suite passes:
      `kuro test fbcode//kuro/tests/core/build_command/...`.
...
- [ ] `rm -rf ~/.cache/buildbuddy_cache_test_namespace/` (or
      ...
      then re-run: kuro detects the CAS-evicted local hit, falls back
...
**File**: `app/kuro_file_watcher/src/notify.rs`
...
/// out before reaching DICE invalidation. `buck-out` is kuro's own
...
**File**: `app/kuro_file_watcher/src/watchman/interface.rs`
...
**File**: `app/kuro_file_watcher/src/notify.rs` (tests, new)
...
kuro build @llvm-project//llvm:llvm --config=remote   # warm
RUST_LOG=kuro_file_watcher=debug kuro build @llvm-project//llvm:llvm --config=remote 2>watch.log
...
- [x] `cargo test -p kuro_file_watcher` passes the new component-match
      tests (5 cases listed above; 6 actually shipped).
- [x] `cargo build -p kuro_file_watcher` clean.
- [ ] Existing FS-related core tests pass:
      `kuro test fbcode//kuro/tests/core/file_watcher/...` if such a
...
- [x] In `RUST_LOG=kuro_file_watcher=debug` output for a back-to-back
      kuro run, no `FileWatcher: …bazel-bin…` or
      `FileWatcher: …bazel-out…` lines appear. Verified by running
      kuro after an intervening `bazel build`: no bazel-* paths appear
...
**File**: `app/kuro_server/src/daemon/server.rs`
...
**File**: `app/kuro_server_commands/src/build/bes_subscriber.rs` (new)

Daemon-side equivalent of
`app/kuro_client_ctx/src/subscribers/bep_bes_sink.rs`. Same logic,
...
**File**: `app/kuro_client_ctx/src/subscribers/bep_bes_sink.rs`

Delete the file. Update `app/kuro_client_ctx/src/streaming.rs:124`
...
**File**: `app/kuro_build_event_stream/src/grpc_sink.rs`
...
**File**: `app/kuro_server/src/daemon/state.rs` (graceful shutdown)
...
**Flag semantics** (`app/kuro_client_ctx/src/common.rs`):
...
- [ ] `cargo build -p kuro_server -p kuro_server_commands` clean
      after the move.
- [ ] BES integration tests pass: `tests/core/event_log/...` (if the
      BES subscriber has any), or grep for `BesSubscriber` in the
      test tree.
- [ ] Two back-to-back `kuro build` runs complete without daemon
...
- [ ] Cold-daemon `@llvm-project//llvm:llvm` warm-RE: client wall
      ...
      plan 30.
- [ ] `kuro build … && kuro build …` pipeline: second invocation
...
- [ ] `kuro killall` mid-upload cleanly cancels the in-flight BES
...
- **Local action-cache TTL.** Default 6 days, buckconfig
  `kuro.local_action_cache_ttl_days` overrides. Resolved.
...
- Plan 30 benchmark dir:
  `/var/mnt/dev/kuro/benchmarks/post-plan-30-bes-throughput/llvm-project_llvm_llvm/README.md`
  — the kuro-vs-bazel numbers this plan targets closing.
- Action cache lookup site:
  `app/kuro_execute_impl/src/executors/action_cache.rs:107`.
- File-watcher buck-out filter:
  `app/kuro_file_watcher/src/notify.rs:105` (already
  component-match; the memory note at
  `~/.claude/projects/-var-mnt-dev-kuro/memory/file_watcher_buck_out_alias.md`
  is stale).
- Daemon command lifecycle:
  `app/kuro_server_ctx/src/template.rs:68`.
- BES finalize path:
  `app/kuro_client_ctx/src/subscribers/bep_bes_sink.rs:238–327`.
