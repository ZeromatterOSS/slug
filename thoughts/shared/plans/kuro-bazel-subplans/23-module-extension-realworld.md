...
Make Kuro's module-extension executor handle the extension shape used by
...
Today Kuro silently captures zero repo specs from this extension and
...
- `app/kuro_external_cells/src/extension_repo.rs:502` logs
...
- Kuro's execution path:
  `kuro_external_cells::extension_repo::...` →
  `ModuleExtensionExecutionKey::compute()` (kuro_bzlmod) →
  `MODULE_EXTENSION_EXECUTOR_IMPL` late-binding (set by
  `kuro_interpreter_for_build`) →
  `ConcreteModuleExtensionExecutor::execute_extension`
  (`app/kuro_interpreter_for_build/src/module_extension_executor_impl.rs:324`)
  → `try_execute_starlark` (line 185).
- Repo rules invoked during the extension body are captured via
  `with_repo_spec_registry` (`kuro_bzlmod::repo_spec::RepoSpecRegistry`).
...
1. **`native.bazel_version` unsupported** — referenced in `rules.bzl:46`
   inside the toolchain's repo-rule impl. Kuro may not provide it; the
   expression `native.bazel_version and native.bazel_version < "6.0.0"`
   would raise.
2. **`ModuleContext.modules[i].tags.<class>`** shape mismatch — kuro's
...
`app/kuro_interpreter_for_build/src/module_extension_executor_impl.rs:384`
...
Changes in `app/kuro_interpreter_for_build/src/module_extension_executor_impl.rs`:
...
**Success criteria.** `kuro build @llvm-project//llvm:Demangle --config=remote`
...
New helper `kuro_bzlmod::extensions::canonical_extension_id(bzl_file,
...
Candidate fixes, in decreasing likelihood based on kuro memory notes:
...
`repository_rule`). If kuro's Starlark invocation of a repository rule
doesn't walk back up to the active `RepoSpecRegistry`, macro-wrapped
registrations go nowhere.

- Fix location: `kuro_bzlmod::repo_spec` or the `repository_rule`
...
dispatch. Kuro's `ModuleInfo` has `tags: HashMap<String, Vec<ExtensionTag>>`
(see `app/kuro_bzlmod/src/extensions.rs:236`). The Starlark-facing view
...
(`"9.0.0-kuro"` or the live kuro version) during repo-rule evaluation,
...
1. `kuro build @llvm-project//llvm:Demangle --config=remote` no longer
...
Two small fixes in `app/kuro_interpreter_for_build/src/repository_ctx.rs`:
...
must run end-to-end against kuro's repository-context surface:
...
**Success criteria.** Running kuro through the extension produces a
...
Rerun: `kuro build @llvm-project//llvm:Demangle --config=remote`.
...
This is kuro eagerly analyzing Java toolchains from the buildbuddy_toolchain
...
Re-run `kuro build @llvm-project//llvm:Demangle --config=remote`. Expect
one of two outcomes:

- **Build succeeds locally** (kuro ignores the `--remote_executor` flag
...
- `kuro build @llvm-project//llvm:Demangle --config=remote` either
...
- New unit test covering the identified gap lives in
  `app/kuro_bzlmod/` or `app/kuro_interpreter_for_build/`.
