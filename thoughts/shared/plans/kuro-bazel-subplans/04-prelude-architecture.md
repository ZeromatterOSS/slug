...
Kuro inherits Buck2's prelude architecture which provides a powerful mechanism for injecting symbols into BUILD files. This architecture should be **preserved** while being **adapted** for Bazel compatibility.
...
│    ├── CcInfo = None                    ├── .update(__kuro_builtins__)      │
...
| `app/kuro_interpreter/src/file_loader.rs` | Extracts `native` struct → BUILD globals |
| `app/kuro_interpreter_for_build/src/interpreter/globals.rs` | Creates `__kuro_builtins__` namespace |
| `app/kuro_build_api/src/interpreter/more.rs` | Registers Bazel shim modules |
...
**File**: `app/kuro_interpreter/src/file_loader.rs:116-134`
...
#### 3. Base Globals with __kuro_builtins__
**File**: `app/kuro_interpreter_for_build/src/interpreter/globals.rs:154-162`

```rust
pub fn base_globals() -> GlobalsBuilder {
    let mut builder = GlobalsBuilder::standard().with(register_all_natives);
    // __kuro_builtins__ namespace makes native functions available to prelude
    builder.namespace("__kuro_builtins__", register_all_natives);
    builder
}
```
...
Native Rust modules in `app/kuro_build_api/src/interpreter/rule_defs/`:
