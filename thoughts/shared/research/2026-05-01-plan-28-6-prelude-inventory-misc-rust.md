...
| prelude/dist/ | varies | DistInfo provider for runtime artifact tracking | **extension-only** | Lines 9-50 define `DistInfo` provider. Used by Buck2-era runtime tracking; mark @internal or move into a Slug extension module. |
...
| prelude/oss/ | 0 | Buck2 OSS-specific overrides (deleted Phase 7) | **remove** | Empty. Slug uses `is_open_source()` Rust flag instead. |
...
### `app/slug_interpreter/src/prelude_path.rs`

**51 lines.** Wrapper around `ImportPath` for `@prelude//:prelude.bzl`.

**Callers:**
- `app/slug_interpreter_for_build/src/interpreter/interpreter_for_dir.rs:572-590` — `prelude_import()` method.
...
### `app/slug_interpreter/src/file_loader.rs::extra_globals_from_prelude_for_buck_files`
...
### `__slug_builtins__` namespace

**Registered in:** `app/slug_interpreter_for_build/src/interpreter/globals.rs:173-175` via `global_env.namespace("__slug_builtins__", |x| { register_all_natives(x); })`.
...
**Current usage:**
- `prelude/native.bzl:21` — sole caller. Scrapes `__slug_builtins__` via `__struct_to_dict()`, wraps in struct, exports as `native`.
- Tests in `app/slug_interpreter_for_build_tests/src/interpreter.rs` use `__slug_builtins__.json.encode(...)` etc. as a stable internal namespace for testing.

**Disposition:** **restrict** → eventually **remove**.

**Rationale:** Once Phase 28.5 moves all Bazel-shaped names to `exported_native`, the only Starlark caller of `__slug_builtins__` is `prelude/native.bzl`, which is deleted in Phase 28.6. Tests can be migrated to read names directly from their owning module.

**Deletion condition:** After (a) `prelude/native.bzl` deletion, (b) test migration off `__slug_builtins__.X`, the namespace becomes dead code. It can either be deleted or renamed to `__slug_internal__` to indicate non-public status while internal code transitions.
...
- `__slug_builtins__` namespace — restrict, then remove after test migration.
