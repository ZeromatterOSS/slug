...
> **Plan**: [Plan 28: Bazel Builtins Module Architecture](../plans/slug-bazel-subplans/28-builtins-module-architecture.md)
>
...
when `@slug_builtins//:exports.bzl` exposes `rule_implementation_wrapper`.
...
`slug_builtins/exports.bzl` was extended (and reverted) with:
...
some entries in slug's starlark-rust fork. Candidate root causes,
...
2. **`**` only forwards Starlark-native kwargs, not arbitrary dict
   ...
   only forwards the dict's "value-typed-as-kwarg" entries, slug
...
3. **`struct()` only allows valid-identifier kwargs and silently
   ...
   `struct(**...)` discards them. (`slug_facade_active` is a valid
...
- **Bypass `struct()` entirely, write a Rust facade.** Add a
  `SlugFacadeCtx` Rust value that wraps `raw_ctx` and overrides
...
2. Reproduce the field loss in a starlark-rust unit test (no slug
...
- `slug_builtins/exports.bzl` keeps the `rule_implementation_wrapper`
