...
**Date:** 2026-04-22
**Scope:** `app/kuro_interpreter/src/`, `app/kuro_external_cells_bundled/`
...
dice graph" (`app/kuro_interpreter/src/load_module.rs:58`). This
...
`app/kuro_external_cells_bundled/build.rs` embeds prelude and
...
- Developer machine after `kuro kill` or reboot
...
kuro's is ~2.1s. Starlark compile time might be part of that gap.

## Fix sketch (deferred)

A fix would need:
1. Serialize frozen `LoadedModule`s to disk, keyed by (source hash,
   kuro binary hash). Binary hash invalidates cache across version
   bumps.
...
3. Invalidate on bundled-cell source changes (already triggered by
   `touch app/kuro_external_cells_bundled/build.rs` convention).
...
- `app/kuro_interpreter/src/load_module.rs:58` — DICE caching docstring
- `app/kuro_external_cells_bundled/build.rs:36` —
  `cargo:rerun-if-changed=prelude` (invalidates bundled data only on
  prelude changes, not unrelated source edits)
- `app/kuro_external_cells_bundled/src/lib.rs` — embedded file
  catalog
