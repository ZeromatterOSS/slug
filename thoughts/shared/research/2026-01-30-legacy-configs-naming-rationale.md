...
branch: main
repository: slug
topic: "Why is slug_common/legacy_configs called 'legacy'? Is there a newer config system?"
...
Why is `slug_common/legacy_configs` called "legacy"? Is there a newer configuration system that should be used preferentially?
...
**Key finding**: There is NOT a direct replacement for `legacy_configs`. It's still actively used and required for all Slug projects. However, there IS a different configuration system (`slug_core/configuration`) that serves a completely different purpose:

| System | Purpose | Files | Status |
|--------|---------|-------|--------|
| `legacy_configs` | Project-level settings | `.buckconfig` INI files | Active, required |
| `slug_core/configuration` | Build-level configuration | Starlark rules (`platform()`, `constraint_*`) | Active, modern |
...
The `legacy_configs` module (`app/slug_common/src/legacy_configs/`) handles:
...
The `slug_core/configuration` module handles **build-time configuration** - how targets are built for different platforms:
...
This is the Bazel-compatible configuration model that Buck2/Slug uses.

### Why "Legacy" is Misleading

The term "legacy" suggests deprecation, but:

1. **Still required** - Every Slug project needs a `.buckconfig` file
...
- Buck v2/Slug (2020+): Rust-based, rules in Starlark, but .buckconfig retained for project settings

### Future Direction: Removing .buckconfig Requirement

From `thoughts/shared/plans/slug-bazel-subplans/02-bzlmod-phase-5b.md`:
...
- `app/slug_common/src/legacy_configs.rs:11-12` - Module documentation with "buckv1" reference
- `app/slug_common/src/legacy_configs/configs.rs:33-34` - `LegacyBuckConfig` main type definition
- `app/slug_common/src/legacy_configs/cells.rs:252-434` - `BuckConfigBasedCells` with bzlmod stub
- `app/slug_core/src/configuration.rs:11-19` - Modern configuration system documentation
...
### Historical Context: Buck v1 vs Buck v2/Slug

| Aspect | Buck v1 | Buck v2/Slug |
|--------|---------|--------------|
...
- `thoughts/shared/plans/slug-bazel-subplans/02-bzlmod-phase-5b.md` - Cell system integration
- `thoughts/shared/plans/slug-bazel-subplans/02-bzlmod-phase-5e.md` - Module extension execution
- `thoughts/shared/research/2026-01-29-dice-incremental-computation-engine.md` - DICE engine context
- `docs/about/benefits/compared_to_buck1.md` - Buck v1 vs Slug comparison
...
**Should you use slug_core/configuration?** Yes, it's the correct system for:
