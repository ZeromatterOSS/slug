...
> _"`slug build SOME_TARGET_I_ALREADY_BUILT_BEFORE` is basically instantaneous
> and is a super delightful experience. 🙂"_ - End user experience
> <FbInternalOnly>([source](https://fb.prod.workplace.com/groups/slugusers/posts/3030704467185914))</FbInternalOnly>

> _"Slug is largely faster and more memory efficient than buck1, and where I’ve
> seen counter-examples, the slug team quickly optimizes and fixes that.🙂"_ -
...
their development inner loop), switching to Slug provides the following
benefits:

- **Performance** - the performance of Slug is better in four ways:
  - **_Fast things are fast_** - in Buck1, simply typing `buck build` when there
    is nothing to do can be expensive (23 seconds in some benchmarks). In Slug,
  ...
  - **_Slow things are faster_** - when there is real work to do, Slug is
  ...
  - **_Users contribute to the shared cache_** - with Buck1, only trusted CI
    builds write to the network cache, while with Slug everyone writes to the
...
- **Correctness** - in Slug, rules are hermetic by default. Missing
  dependencies are errors. These restrictions apply to both the user-written
  `BUCK` files and the language rules.
  - During the process of migrating to Slug, a huge number of missing
    ...
    track all deps). The end result is that Slug gives the right answer more
    often, cutting down on user surprises.
- **Rule features** - the rules in Slug, especially for less commonly used
...
- **Actively developed** - the Meta build team is putting all its efforts behind
  Slug; it's vastly easier to develop than Buck1. While Slug is already ahead
...
- **Support** - Meta can provide much better support to those having
  difficulties with Slug than to those using Buck1.

## Benefits for Rule Authors

If you write language-specific rules, then Slug is in a different league to
Buck1.

> _"This is all rather fun! Slug rules are so much more hackable than
...
There are a number of reasons why Slug excels for Rule Authors:
...
- **Faster developer cycle** - in Buck1, the time from changing a rule to seeing
  ...
  Slug, it takes seconds, you don’t even need to restart the daemon.
- **Simple API** - Slug rules use a small and documented Starlark API, which is
  dependency-correct by construction. In Buck1, the rules must obey a lot of
  subtle side conditions with a much larger API.
- **Easier deployment** - for Slug, deployment is just checking the rules in,
...
- **Low barrier to entry** - writing rules in Slug is vastly easier than Buck1,
...
For those people who integrate Slug into larger systems, in addition to many of
the above benefits apply, Slug provides the following benefits:

- **Faster queries** - many integrators make extensive use of `buck uquery` and
  `cquery`. In Slug, these commands are **faster** and use **less memory**.
  - For example, on CI target determination (a bunch of targets/queries), Slug
    is 25% faster at P50 (moving to 40% faster at P95) with 25% less memory
    (saving over 20Gb, and crossing below the 64Gb threshold).
- **Profiling** - Slug already ships with five types of profiling for both
  loading and analysis (flame graphs, statement breakdown, heap profiles etc).
  With Slug, these tools are much more easily accessible to people not on the
  Build Infra team.

<FbInternalOnly>

- **Eden friendly** - Slug is tuned for the Eden architecture, performing fewer
...
- **Better observability** - Slug populates many Scuba tables with information
  about
  [loading](https://www.internalfb.com/intern/scuba/query/?dataset=slug_loads),
  [analysis](https://www.internalfb.com/intern/scuba/query/?dataset=slug_analyses),
  [builds](https://www.internalfb.com/intern/scuba/query/?dataset=slug_builds)
  and
  [errors](https://www.internalfb.com/intern/scuba/query/?dataset=slug_action_errors),
  and more. The architecture of Slug ensures that all important information can
...
- **Stability** - Slug is under active development, which means the risk of
...
- **Corner cases** - Buck1 has been battle-tested for nearly a decade, which has
  ...
  Only time and user feedback will enable Meta to bring Slug to the same level.
  Please share all such feedback!

<FbInternalOnly>

- **Slug Web UI** - there isn’t yet a working Web UI equivalent to the one
