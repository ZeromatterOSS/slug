...
See `examples/prelude/cpp/conan` in the Kuro source repository for a full
working example.

## Motivation

Kuro has the ability to build C/C++ libraries natively. However, some C/C++
projects have complex build systems and are difficult to migrate to a native
Kuro build. Other programming languages often have established standard
package managers and such dependencies can be imported into a Kuro project
...
control to support an integration into Kuro, supports toolchain configuration
...
The Kuro integration of Conan should fulfill the following design goals:

* The overall build should be controlled by Kuro:
    ...
    This enables the use of Kuro's own incremental build and caching
    functionality. It also enables cross-platform and cross-compilation with
    the help of Kuro's platforms and toolchains.
...
* Command-Line
    ...
    this capability to build Conan packages in separate Kuro build actions.
...
* Install Location
    ...
    directory under Kuro's output directory and copies needed dependencies
...
* Profiles
    ...
    settings. The integration uses profiles to expose Kuro's own cxx toolchain
...
* Generators
    ...
    Conan generator that creates Kuro targets that import Conan built packages
    and can be depended upon by native Kuro C/C++ targets.
...
def conan_component(
    ...
    them as a target that can be depended on by native Kuro C/C++ targets such
    as `cxx_library`.
    """
...
conan_generate = rule(
    ...
    doc = "Generate Kuro import targets for Conan packages using the Buckler generator.",
)
...
def _profile_env_var(name, value) -> cmd_args:
    ...
    #   Setting it causes Kuro to escape the `$PROFILE_DIR` prefix set in the
