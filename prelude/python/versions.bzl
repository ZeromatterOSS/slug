...
versioned_library = rule(
    doc = """
        Represents multiple versions of the same library. This is a less flexible option than simply using [`select()`](https://kuro.build/docs/rule_authors/configurations/) in `deps` attributes of targets, and setting the right constraints on the build to allow [select resolution](https://kuro.build/docs/rule_authors/configurations_by_example/) to resolve the desired version.
