...
def cxx_binary_impl(ctx: AnalysisContext) -> list[Provider]:
    ...
    # unpacked external debuginfo that goes with it. This makes `kuro build
    # :main` equivalent to `kuro build :main :main[debuginfo]`.
    #
    # This is wasted work if we are building an executable together with its dwp
    # subtarget (`kuro build :main :main[dwp]`) in which case a large number of
    ...
    # means the output of `kuro build :main` is always immediately usable in a
    # debugger.
    #
    ...
    providers.extend(inject_test_run_info(
        ...
        ExternalRunnerTestInfo(
            ...
            run_from_project_root = (
                "kuro_run_from_project_root" in (ctx.attrs.labels or []) or
                re_executor != None
            ),
