# Distributed ThinLTO in Kuro

Sean Gillespie, April 2022

This document is a technical overview into Kuro's implementation of a
distributed ThinLTO. Like all rules in Kuro, this implementation is written
...
Kuro has the ability to take a step further than Buck1 could ever have - Kuro
...
## Kuro's Implementation

Kuro's role in a distributed ThinLTO compilation is to construct a graph of
...
Unlike Buck1 (or even Blaze/Bazel), Kuro has explicit support for this paradigm
[("dynamic dependencies")](https://fburl.com/gdoc/zklwhkll). Therefore, for
Kuro, the basic strategy looks like:
...
produced as part of action `1`. Furthermore, Kuro's support of `1` is
complicated by the fact that certain Kuro rules can produce an archive of
object files as an output (namely, the Rust compiler). As a result, Kuro's
implementation of Distributed ThinLTO is highly dynamic.

Kuro's implementation contains four phases of actions:
...
It is a reality of Kuro today that some rules don't produce a statically-known
...
the archive, Kuro has no way of knowing what the contents of the archive are,
...
Kuro dispatches a `thin_lto_prepare` action for every archive. Each prepare
...
unpacking each archive, Kuro keeps track of the list of archives as a Starlark
...
For the purposes of Kuro, the indexer looks like a linker; because of this,
Kuro must construct a reasonable link line. Kuro does this by iterating over
the list of linkables that it has been given and constructing a link line from
them. Uniquely for distributed ThinLTO, Kuro must wrap all objects that were
...
Invoking the indexer is relatively straightforward in that Kuro invokes it like
it would any other linker. However, once the indexer returns, Kuro must
post-process its output into a format that Kuro's Starlark can understand and
translate into a graph of dynamic `opt` actions. The first thing that Kuro is
...
After `thin_lto_index` completes, Kuro launches `thin_lto_opt` actions for
every object file and for every archive. For each object file, Kuro reads that
object file's optimization plan. At this phase, it is Kuro's responsibility to
...
For each archive, Kuro reads the archive's optimization plan and constructs
additional `thin_lto_opt` actions for each object file contained in the archive.
Kuro creates a directory of symlinks (`opt_objects`) that either contains
...
calculated. In cases where Kuro would put an archive on the link line, it
