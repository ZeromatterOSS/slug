...
def _ignore_artifacts(ctx: AnalysisContext) -> bool:
    return "kuro_ignore_artifacts" in ctx.attrs.labels
...
def genrule_impl(ctx: AnalysisContext) -> list[Provider]:
    ...
    # Kuro clears the output directory before execution, and thus src/sh too.
...
def process_genrule(
    ...
    if type(ctx.attrs.srcs) == type([]):
        ...
        # See fbcode//kuro/tests/targets/rules/genrule:genrule-dot-input for a test that exposes it.
