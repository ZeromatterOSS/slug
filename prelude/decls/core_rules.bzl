...
test_suite = prelude_rule(
    ...
    attrs = (
        # @unsorted-dict-items
        {
            # On buck1 query, tests attribute on test_suite is treated as deps, while on kuro it is not.
            # While kuro's behavior makes more sense, we want to preserve buck1 behavior on test_suite for now to make TD behavior match between buck1 and kuro.
            # This diff makes the behaviors match by adding a test_deps attribute to test_suite on kuro that is used as a deps attribute. In the macro layer, we set test_deps = tests if we are using kuro.
