...
common_shell_attributes = (
    {
        ...
        "shell_libs": attrs.set(
            ...
            default = ["prelude//erlang/shell:kuro_shell_utils"],
            doc = """
            This attribute allows to define additional dependencies for the shell. By default this is
            set to `["prelude//erlang/shell:kuro_shell_utils"]` which includes a `user_default` module
            that loads and compiles modules with kuro mechanisms.
        """,
        ),
    }
)
...
common_application_attributes = {
    "applications": attrs.list(attrs.dep(providers = [ErlangAppInfo]), default = [], doc = """
        ...
        **NOTE**: _If you use the `app_src` field and the references application resource file template specifies
        `applications` or `included_applications` kuro checks that the target definitions and information in the template are
...
rules_attributes = {
    "erlang_app": {
        ...
        "app_src": attrs.option(attrs.source(), default = None, doc = """
                ...
                kuro to generate the `*.app` output file in the applications `ebin/` directory. This is useful during the migration from
                rebar3 to kuro to avoid duplicated entries, of e.g. the `version`.

                Kuro will use or check all fields present in the template, and fill out the fields with the information provided in the
                target, e.g. if the `version` is specified in both, kuro will check that they are identical. Otherwise, it uses the
                ...
                **NOTE**: _If you use the `app_src` field and the references application resource file template specifies `applications`
                or `included_applications` kuro checks that the target definitions and information in the template are equivalent to
...
erlang_app = prelude_rule(
    ...
    docs = """
        ...
        Erlang Applications are the basic building block of our kuro integration and used by many other Erlang
...
erlang_test = prelude_rule(
    ...
    docs = """
        ...
        One can call
        - `kuro build //my_app:test_SUITE` to compile the test files together with its dependencies.
        - `kuro test //my_app:other_test_SUITE` to run the test.
        - `kuro run //my_app:other_test_SUITE` to open an interactive test shell, where tests can be run iteratively.


        kuro test will rely on tpx to run the suite. To get access to tpx commands, add `--` after the
        target. For example:

        - `kuro test //my_app:other_test_SUITE -- --help` will print the list of tpx available
        command line parameters.
        - `kuro test //my_app:other_test_SUITE -- group.mycase` will only run those test cases
