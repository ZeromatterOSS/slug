---
name: kuro-rule-basics
description: Guide users through writing their first Kuro rule to learn fundamental concepts including rules, actions, targets, configurations, analysis, and select(). Use this skill when users want to learn Kuro basics hands-on or need help understanding rule writing.
---

@nolint

# Kuro Rule Basics - Interactive Tutorial

## Overview

This is an **interactive, step-by-step tutorial** that teaches Kuro
...
- **`references/concepts.md`** - Deep dive into Kuro core concepts including:
  - The Kuro build model (load, configuration, analysis, execution phases)
...
- User asks "how does X work in Kuro?" → Check `concepts.md`
...
## Important: Use System Kuro Command

This tutorial uses the **system `kuro` command**, NOT `./kuro.py`.

- Use: `kuro build`, `kuro test`, `kuro cquery`, etc.
- Do NOT use: `./kuro.py` (that's for Kuro development/self-bootstrap)

This ensures the tutorial works for all users with Kuro installed.
...
output but haven't created an action to produce it. Kuro is telling us: "You
...
"""
    ...
    ctx.actions.run(
        ...
        category = "uppercase",  # Shows in kuro output
    )
...
```bash
kuro build :hello --show-full-output
...
- **Action**: A command Kuro will execute (bash script + Python)
- **cmd_args()**: Builds command line, handles artifacts properly
- **artifact.as_output()**: Marks artifact as output (Kuro creates parent dirs)
- **ctx.actions.run()**: Registers action - doesn't execute yet, just declares
- **Shell redirection**: Use `>` in shell scripts (Kuro doesn't support
  redirect_stdout)
- **Lazy execution**: Kuro decides when/if to run based on what's needed
...
1. List available targets with `kuro targets`
...
# fbcode//kuro/kuro-tutorial:hello
# fbcode//kuro/kuro-tutorial:goodbye
```

**Explain target name anatomy:**

A full Kuro target name has three parts:
...
- `:hello`
  ...
  - When you run `kuro build :hello` from the `kuro-tutorial` directory, Kuro knows you mean `fbcode//kuro/kuro-tutorial:hello`

**Now query the targets:**

```bash
# Unconfigured - shows raw attributes
kuro uquery :hello --output-attribute=src

# Configured - shows with platform config applied
kuro cquery :hello --output-attribute=src
```
...
- Unconfigured: `fbcode//kuro/kuro-tutorial:hello`
- Configured: `fbcode//kuro/kuro-tutorial:hello (cfg:dev-linux-x86_64-...)`

Notice the configuration suffix `(cfg:...)` is added when Kuro applies platform-specific settings.
...
kuro uquery :hello --output-attribute=output_name

# See the resolved value for your platform
kuro cquery :hello --output-attribute=output_name
...
- **Resolution**: During configuration phase, Kuro picks one branch
...
kuro build :goodbye --show-full-output
...
- **Automatic ordering**: Kuro builds deps first, ensuring outputs are ready
...
```bash
kuro build :hello_macro --show-full-output
...
- Core Kuro primitives declared with `rule()`
- Run during analysis phase
- Create actions that produce artifacts
- Can be queried with `kuro cquery`/`uquery`
...
**User:** "I want to learn Kuro"

**Claude:** "Great! I'll guide you through writing your first Kuro rule. We'll
...
**Claude:** "Perfect! Step 1 is about creating the simplest possible Kuro
...
2. **Normalize errors**: "This error is expected!", "Let's see what Kuro is
   telling us"
...
1. **Read the reference materials**: This skill includes detailed documentation:
   - `references/concepts.md` - Deep dive into Kuro core concepts (build model, targets, artifacts, actions, providers, configurations, build graph)
...
2. **Explore Kuro's prelude**: See real production rules in `fbcode/kuro/prelude/`
