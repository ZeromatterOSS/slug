# Kuro Core Concepts - Deep Dive

This reference provides detailed explanations of Kuro core concepts for users
who want to go beyond the basic tutorial.

## Table of Contents

1. [The Kuro Build Model](#the-kuro-build-model)
...
## The Kuro Build Model

Kuro uses a **declarative, graph-based build model**:
...
should be run. Kuro decides when to actually run them.
...
**Target** is an overloaded term in Kuro. Be precise:
...
# kuro uquery //path:app --output-attribute=srcs
...
# kuro cquery //path:app --output-attribute=srcs
...
A **cell** is a repository or unit of code in Kuro. Cells allow Kuro to work
with multiple repositories or isolated parts of a monorepo.

- **Explicit cell reference:** `fbcode//kuro/app:kuro`
  - `fbcode` = cell name
  - `kuro/app` = package path within that cell
  - `kuro` = target name
- **Implicit cell reference:** `//kuro/app:kuro`
  - Uses the current cell based on your working directory
  - Equivalent to `fbcode//kuro/app:kuro` if your current directory is under
...
- **Cross-cell dependencies:** Depend on targets in a different cell
  - Example: `fbcode//kuro:kuro` depending on `prelude//rules.bzl`
...
Artifacts represent files in Kuro's build model.
...
Kuro tracks artifacts symbolically during analysis:

```starlark
output = ctx.actions.declare_output("foo.txt")
# At this point, output is just a promise - file doesn't exist yet!
# Kuro knows: "If anyone needs foo.txt, run the action that produces it"
```

### Bound vs Unbound Artifacts

- **Bound artifact:** Has an action that produces it
- **Unbound artifact:** Declared but no action produces it (Kuro error!)
...
Kuro tracks dependencies automatically:
...
Kuro caches action results based on:
...
If inputs haven't changed, Kuro reuses cached outputs.
...
**Used by `kuro build`:** When you run `kuro build //target:name`, Kuro
...
# Access sub-targets
kuro build //app:main[foo]  # Builds artifacts from sub_targets["foo"]
```
...
**Used by `kuro run`:** When you run `kuro run //target:name`, Kuro executes
...
kuro run //app:main -- additional_args
...
This is not actual Kuro syntax - it's a conceptual representation.
...
1. User specifies a target: `kuro build //app:main`
2. Kuro applies default configuration (or user-specified one)
...
```bash
kuro build //app:main --target-platforms fbcode//platform/linux:x86_64
kuro build //app:main --target-platforms fbcode//platform/macos:arm64
```
...
1. Kuro walks the configured build graph
...
def my_rule_impl(ctx):
    print("Analysis!")  # Prints during kuro build (analysis)
...
Kuro provides tools to inspect the graph:

#### uquery - Unconfigured target graph

```bash
kuro uquery "deps(//app:main)"        # All dependencies
kuro uquery "rdeps(//..., //lib:foo)" # Reverse dependencies
```

#### cquery - Configured target graph

```bash
kuro cquery "deps(//app:main)"                    # With configs applied
kuro cquery //app:main --output-attribute=srcs   # Show resolved attributes
```

#### aquery - Action graph

```bash
kuro aquery "deps(//app:main)"  # Show actions that will run
```

### Incremental Builds

Kuro uses the graph for incrementality:

1. User changes `src/util.cpp`
2. Kuro identifies affected artifacts (util.o)
...
These concepts work together to make Kuro fast, correct, and scalable.
