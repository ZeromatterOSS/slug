"""Fail-closed one-prime output and aggregate-semantics discriminator."""
...
from tools.v2_oracle_lib import buildbuddy_build_cache_prime_stage_probe as prime_stage
...
def run_probe(bazel: str = "bazel", runner: Callable[..., subprocess.CompletedProcess[bytes]] = subprocess.run) -> dict[str, object]:
    ...
    try:
        ...
        try:
            ...
            bep, execution = phase / "bep.json", phase / "execution.json"; bep_id = _private(bep); _private(execution)
        ...
        if process != "ZERO": result = record("STAGE_RECORDED", process, "PROCESS_NONZERO")
            ...
            try: output_count = cache._outputs(output)
            except Exception: result = record("STAGE_RECORDED", process, "OUTPUT_SCAN_REJECTED")
            else:
                if output_count != 1: result = record("STAGE_RECORDED", process, "OUTPUT_MATERIALIZATION_REJECTED")
                elif not cache._anchored(root, root_id, root_fd, "prime", phase_id, output_fd, output_id): result = record("STAGE_RECORDED", process, "POST_OUTPUT_ANCHOR_REJECTED")
                else:
                    try: bep_bytes = cache._private_bytes(phase_fd, bep.name, bep_id)
