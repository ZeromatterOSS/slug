...
This folder houses Kuro's Scribe client, which Kuro uses to send information
that powers all of our internal tooling around Kuro. Despite this client
serving the needs of Kuro, there is no Kuro-specific logic contained within
this library.

See
[this post](https://fb.workplace.com/groups/kuroprototyping/posts/2829650903999058)
for justification of why this library exists and why it is here. This library is
intended to be an implementation detail of Kuro; please do not depend directly
on this library without speaking to us first.

Kuro writes to Scribe by interfacing directly with the Thrift service running
...
1. On Linux, this library
   ...
   which is not acceptable for Kuro (which often runs in environments where
   ServiceRouter cannot function).
2. `ScribeProducer` presents an asynchronous API for pushing messages, which is
   not acceptable for Kuro.
3. Kuro needs functionality that exists in the C++ Scribe client -
