...
enum BuckdCommunicationError {
    ...
    #[error(
        "The Kuro daemon was shut down while executing your command. This happened because: {0}"
    )]
    #[kuro(tag = InterruptedByDaemonShutdown)]
    InterruptedByDaemonShutdown(kuro_data::DaemonShutdown),
...
pub trait PartialResultHandler {
    type PartialResult: TryFrom<
            kuro_cli_proto::partial_result::PartialResult,
            Error = kuro_cli_proto::partial_result::PartialResult,
        >;
...
impl PartialResultCtx<'_> {
    pub async fn stdout(&mut self, bytes: &[u8]) -> kuro_error::Result<()> {
        self.inner
...
/// Manages incoming event streams from the daemon for the kuro client and
...
impl<'a> DaemonEventsCtx<'a> {
    ...
    async fn handle_stream_next<Handler>(
        ...
        next: Option<Vec<kuro_error::Result<StreamValue>>>,
        shutdown: &mut Option<kuro_data::DaemonShutdown>,
    ) -> kuro_error::Result<ControlFlow<Box<CommandResult>, ()>>
    where
    ...
    async fn dispatch_tailer_event(&mut self, event: FileTailerEvent) -> kuro_error::Result<()> {
        match event {
            FileTailerEvent::Stdout(out) | FileTailerEvent::Stderr(out) => {
                ...
                // But if daemon does, it should not be used as standard output of kuro.
    ...
    where
        S: Stream<Item = kuro_error::Result<StreamValue>>,
        Handler: PartialResultHandler,
    {
        ...
        let command_result: kuro_error::Result<CommandResult> = try {
            loop {
                tokio::select! {
                    next = stream.next() => {
    ...
    ) -> kuro_error::Result<CommandOutcome<Res>>
    where
        S: Stream<Item = kuro_error::Result<StreamValue>>,
    ...
    async fn handle_error_owned(&mut self, error: kuro_error::Error) -> kuro_error::Error {
        let error: kuro_error::Error = error.into();
        let result: Result<(), kuro_error::Error> = self
            .inner
    ...
    pub async fn flush(&mut self) -> kuro_error::Result<()> {
...
fn convert_result<R: TryFrom<command_result::Result, Error = command_result::Result>>(
    value: CommandResult,
) -> kuro_error::Result<CommandOutcome<R>> {
    match value.result {
...
impl EventsCtx {
    ...
    async fn handle_tailer_stderr(&mut self, stderr: &[u8]) -> kuro_error::Result<()> {
    ...
    async fn handle_events(
        ...
        let events = events.into_map(|mut event| {
            ...
            if let kuro_data::buck_event::Data::Instant(instant_event) = event.data_mut() {
                match &mut instant_event.data {
                    Some(kuro_data::instant_event::Data::Snapshot(snapshot)) => {
                    ...
                    Some(kuro_data::instant_event::Data::DaemonShutdown(msg)) => {
    ...
    where
        Fut: Future<Output = kuro_error::Result<()>> + 'b,
    {
    ...
    pub(crate) fn handle_daemon_started(&mut self, reason: kuro_data::DaemonWasStartedReason) {
    ...
    pub(crate) async fn eprintln(&mut self, message: &str) -> kuro_error::Result<()> {
