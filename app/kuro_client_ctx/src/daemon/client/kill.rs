...
pub async fn kill_command_impl(
    ...
    let pid = match buckd {
        ...
        Ok(Err(e)) => {
            ...
            if e.has_tag(ErrorTag::ServerTransportError) {
                ...
                crate::eprintln!(
                    "unexpected error connecting to Kuro: {:#} \
    ...
    if let Some(pid) = pid {
        crate::eprintln!("Kuro daemon pid {} has exited", pid)?;
    }
...
pub(crate) async fn kill(
    ...
    match tokio::time::timeout(KILL_REQUEST_TIMEOUT, request_fut).await {
            ...
            match inner_result {
                Ok(_) => loop {
                    ...
                    if Instant::now() - time_req_sent > GRACEFUL_SHUTDOWN_TIMEOUT {
                        crate::eprintln!(
                            "Timed out waiting for graceful shutdown of kuro daemon pid {}",
                            pid
                        )?;
                        break;
                    }
                ...
                Err(e) => {
                    ...
                    // authenticate to it.
                    crate::eprintln!(
                        "Error requesting graceful shutdown of kuro daemon pid {}: {}",
                        pid,
                        e
                    )?;
                }
        ...
        Err(e) => {
            ...
            crate::eprintln!(
                "Timed out requesting graceful shutdown of kuro daemon pid {}",
                pid
            )?;
        }
    };
...
pub(crate) async fn hard_kill(info: &DaemonProcessInfo) -> kuro_error::Result<()> {
    let pid = Pid::from_i64(info.pid)?;
