...
use kuro_common::buckd_connection::ConnectionType;
use kuro_common::client_utils::RetryError;
use kuro_common::client_utils::get_channel_tcp;
use kuro_common::client_utils::get_channel_uds;
...
use kuro_resource_control::spawn_daemon::replace_unit_delimiter;
...
impl ConstraintUnsatisfiedReason {
    pub(crate) fn to_daemon_was_started_reason(&self) -> kuro_data::DaemonWasStartedReason {
        match self {
            ConstraintUnsatisfiedReason::Version => {
                kuro_data::DaemonWasStartedReason::ConstraintMismatchVersion
            }
            ConstraintUnsatisfiedReason::UserVersion => {
                kuro_data::DaemonWasStartedReason::ConstraintMismatchUserVersion
            }
            ConstraintUnsatisfiedReason::StartupConfig => {
                kuro_data::DaemonWasStartedReason::ConstraintMismatchStartupConfig
            }
            ConstraintUnsatisfiedReason::RejectDaemonId => {
                kuro_data::DaemonWasStartedReason::ConstraintRejectDaemonId
            }
            ConstraintUnsatisfiedReason::TraceIo => {
                kuro_data::DaemonWasStartedReason::ConstraintMismatchTraceIo
            }
            ConstraintUnsatisfiedReason::SqliteIdentity => {
                kuro_data::DaemonWasStartedReason::ConstraintMismatchSqliteIdentity
            }
        }
    }
}
...
impl DaemonConstraintsRequest {
    ...
    fn satisfied(
        &self,
        daemon: &kuro_cli_proto::DaemonConstraints,
...
pub fn buckd_startup_timeout() -> kuro_error::Result<Duration> {
    Ok(Duration::from_secs(
        kuro_env!("BUCKD_STARTUP_TIMEOUT", type=u64)?.unwrap_or(10),
    ))
}

pub fn buckd_startup_init_timeout() -> kuro_error::Result<Duration> {
    Ok(Duration::from_secs(
        kuro_env!("BUCKD_STARTUP_INIT_TIMEOUT", type=u64)?.unwrap_or(90),
    ))
}
...
impl<'a> BuckdLifecycle<'a> {
    async fn lock_with_timeout(
        ...
        constraints: &'a DaemonConstraintsRequest,
    ) -> kuro_error::Result<BuckdLifecycle<'a>> {
        Ok(BuckdLifecycle::<'a> {
            paths,
    ...
    fn clean_daemon_dir(&self) -> kuro_error::Result<()> {
        self.lock
    ...
    async fn start_server(&self) -> kuro_error::Result<()> {
        ...
        //   ```
        //   kuro --isolation-dir=xx audit providers fbcode//kuro:kuro --quiet
        //   ```
        ...
        daemon_env_vars.push((
            ...
            OsStr::new(kuro_env!("BUCK2_LIB_BACKTRACE")?.unwrap_or("0")),
        ));
    ...
    async fn start_server_unix(
        ...
        if kuro_env!("BUCK_DAEMON_LOG_TO_FILE", type=u8)? == Some(1) {
            ...
            match result {
                Err(_elapsed) => {
                    ...
                    child.kill().await.buck_error_context(
                        "Error killing process after kuro daemon launch timing out",
                    )?;
                    ...
                    kuro_error::Ok(child.wait().await?)
                }
                Ok(result) => result.map_err(|e| kuro_error::Error::from(e).into()),
            }
        };
...
impl BuckdChannel {
    ...
    pub async fn upgrade(self) -> kuro_error::Result<BootstrapBuckdClient> {
        let Self {
            info,
            daemon_dir,
            mut client,
...
pub struct BootstrapBuckdClient {
    ...
    constraints: kuro_cli_proto::DaemonConstraints,
}
...
impl BootstrapBuckdClient {
    pub async fn connect(
        ...
        kuro_fs::fs_util::create_dir_all(&daemon_dir.path)
    ...
    pub(crate) async fn kill(&mut self, reason: &str) -> kuro_error::Result<Pid> {
        kill::kill(&mut self.client, &self.info, reason).await?;
        Pid::from_i64(self.info.pid)
    }

    async fn kill_for_constraints_mismatch(&mut self) -> kuro_error::Result<Pid> {
...
pub async fn connect_buckd(
    ...
    events_ctx: &mut EventsCtx,
    paths: &InvocationPaths,
) -> kuro_error::Result<BuckdClientConnector> {
...
pub async fn establish_connection_existing(
    daemon_dir: &DaemonDir,
) -> kuro_error::Result<BootstrapBuckdClient> {
...
fn explain_failed_to_connect_reason(reason: kuro_data::DaemonWasStartedReason) -> &'static str {
    match reason {
        ...
        DaemonWasStartedReason::NoDaemonProcess => "kuro daemon is not running",
    }
}
...
async fn establish_connection_inner(
    ...
    let daemon_was_started_reason = {
        match BuckdProcessInfo::load_if_exists(&daemon_dir) {
            Ok(Some(buckd_info)) => {
                match try_connect_existing(&buckd_info, &deadline, &lifecycle_lock).await {
                    Ok(channel) => {
                        ...
                        events_ctx
                            .eprintln(&format!(
                                "kuro daemon constraint mismatch: {reason}; killing daemon..."
                            ))
                            .await?;
                        ...
                        events_ctx.eprintln("Starting new kuro daemon...").await?;
                    ...
                    Err(reason) => {
                        events_ctx
                            .eprintln(&format!(
                                "Could not connect to kuro daemon ({}), killing daemon..",
            ...
            Ok(None) => {
                ...
                kuro_data::DaemonWasStartedReason::NoBuckdInfo
            }
            Err(e) => {
                events_ctx
                    .eprintln(&format!(
                        "Could not load buckd.info: {e}, starting new kuro daemon..."
                    ))
                    .await?;

                kuro_data::DaemonWasStartedReason::CouldNotLoadBuckdInfo
            }
        }
    };
    ...
    deadline
        .down(
            &format!(
                "starting new kuro daemon for reason: {}",
...
async fn start_new_buckd_and_connect(
    ...
    constraints: &DaemonConstraintsRequest,
    events_ctx: &mut EventsCtx,
    daemon_was_started_reason: kuro_data::DaemonWasStartedReason,
) -> kuro_error::Result<BootstrapBuckdClient> {
    ...
    lifecycle_lock
        .start_server()
        .await
        .buck_error_context("Error starting kuro daemon")?;
...
) -> Result<BuckdChannel, kuro_data::DaemonWasStartedReason> {
    let timeout: kuro_error::Result<_> = try { timeout.min(buckd_startup_timeout()?)? };
    let Ok(timeout) = timeout else {
        return Err(kuro_data::DaemonWasStartedReason::TimeoutCalculationError);
    };
    let Ok(rem_duration) = timeout.rem_duration("connect existing buckd") else {
        return Err(kuro_data::DaemonWasStartedReason::TimedOutConnectingToDaemon);
    };
    match tokio::time::timeout(rem_duration, buckd_info.create_channel()).await {
        Ok(Ok(channel)) => Ok(channel),
        Ok(Err(_)) => {
            let Ok(pid) = buckd_info.pid() else {
                return Err(kuro_data::DaemonWasStartedReason::CouldNotLoadBuckdInfo);
            };
            ...
            if !buckd_process_exists {
                ...
                Err(kuro_data::DaemonWasStartedReason::NoDaemonProcess)
            } else {
                Err(kuro_data::DaemonWasStartedReason::CouldNotConnectToDaemon)
            }
        }
        Err(e) => {
            let _assert_type: tokio::time::error::Elapsed = e;
            Err(kuro_data::DaemonWasStartedReason::TimedOutConnectingToDaemon)
        }
    }
}
...
impl<'a> BuckdProcessInfo<'a> {
    ...
    async fn load_and_create_channel(
        daemon_dir: &'a DaemonDir,
    ) -> kuro_error::Result<BuckdChannel> {
        Self::load(daemon_dir)?.create_channel().await
    }

    pub fn load(daemon_dir: &'a DaemonDir) -> kuro_error::Result<Self> {
    ...
    pub fn load_if_exists(daemon_dir: &'a DaemonDir) -> kuro_error::Result<Option<Self>> {
    ...
    pub async fn create_channel(&self) -> kuro_error::Result<BuckdChannel> {
    ...
    pub async fn hard_kill(&self) -> kuro_error::Result<()> {
        kill::hard_kill(&self.info).await
    }

    pub fn pid(&self) -> kuro_error::Result<Pid> {
        Pid::from_i64(self.info.pid)
    }
}
...
async fn get_constraints(
...
) -> kuro_error::Result<kuro_cli_proto::DaemonConstraints> {
    ...
    let status = DaemonEventsCtx::without_tailers(&mut events_ctx)
        .unpack_oneshot({
            client.status(tonic::Request::new(kuro_cli_proto::StatusRequest {
                snapshot: false,
    ...
    let status: kuro_cli_proto::StatusResponse = match status {
        CommandOutcome::Success(r) => Ok(r),
        CommandOutcome::Failure(_) => Err(kuro_error!(
...
#[allow(clippy::large_enum_variant)]
#[kuro(tag = DaemonConnect)]
enum BuckdConnectError {
    #[error(
        "buck daemon startup failed with exit code {code}\nstdout:\n{stdout}\nstderr:\n{stderr}"
    )]
    #[kuro(tag = DaemonStartupFailed)]
    BuckDaemonStartupFailed {
        code: String,
        stdout: String,
        stderr: String,
    },
    #[error("Failed to launch Kuro daemon: {error:#}")]
    #[kuro(tag = DaemonLaunchFailed)]
    ...
    #[kuro(tag = DaemonConstraintsWrongAfterStart)]
    BuckDaemonConstraintWrongAfterStart {
        reason: ConstraintUnsatisfiedReason,
        expected: DaemonConstraintsRequest,
        actual: kuro_cli_proto::DaemonConstraints,
    },
    #[error("kuro daemon constraint mismatch during nested invocation: {reason}")]
    #[kuro(tag = DaemonNestedConstraintsMismatch)]
    NestedConstraintMismatch { reason: ConstraintUnsatisfiedReason },
    #[error("buckd info {path} does not exist")]
    #[kuro(tag = BuckdInfoMissing)]
    BuckdInfoMissing { path: AbsNormPathBuf },
    #[error("Error parsing daemon info in `{}`. \
                Try deleting that file and running `kuro killall` before running your command again",
                location.display())]
    #[kuro(tag = BuckdInfoParseError)]
    BuckdInfoParseError {
...
async fn daemon_connect_error(
    ...
    let error_report: Result<kuro_data::ErrorReport, RetryError<kuro_error::Error>> = retrying(
        ...
        || async {
            ...
            let error_report = kuro_data::ErrorReport::deserialize(
    ...
    let error = if let Ok(error_report) = error_report {
        ...
        let daemon_error: kuro_error::Error = error_report.into();
    ...
    // FIXME(ctolliday) we should check if the pid at daemon_dir.buckd_pid is still running before suggesting kuro kill.
    let error_message = format!(
        "Failed to connect to buck daemon.
    Try running `kuro kill` and your command afterwards.
...
fn is_nested_invocation(
    kuro_daemon_uuid: Option<&String>,
    daemon: &kuro_cli_proto::DaemonConstraints,
) -> bool {
    kuro_daemon_uuid == Some(&daemon.daemon_id)
}
...
mod tests {
    ...
    fn constraints(trace_io_enabled: bool) -> kuro_cli_proto::DaemonConstraints {
        kuro_cli_proto::DaemonConstraints {
            version: "version".to_owned(),
            user_version: Some("test".to_owned()),
            daemon_id: "foo".to_owned(),
            extra: Some(kuro_cli_proto::ExtraDaemonConstraints {
                trace_io_enabled,
    ...
    fn test_reject_daemon() {
        ...
        let daemon = kuro_cli_proto::DaemonConstraints {
            version: "foo".to_owned(),
            user_version: None,
