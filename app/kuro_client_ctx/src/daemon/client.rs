...
use kuro_cli_proto::daemon_api_client::*;
...
impl BuckdClientConnector {
    ...
    pub fn daemon_constraints(&self) -> &kuro_cli_proto::DaemonConstraints {
...
impl BuckdLifecycleLock {
    ...
    pub async fn lock_with_timeout(
        ...
        async fn lock_inner(
            daemon_dir: DaemonDir,
            deadline: StartupDeadline,
        ) -> kuro_error::Result<BuckdLifecycleLock> {
    ...
    pub fn clean_daemon_dir(&self, keep_prev: bool) -> kuro_error::Result<()> {
...
pub(crate) fn tonic_status_to_error(status: tonic::Status) -> kuro_error::Error {
    ...
    kuro_error::Error::from(status).tag(tags)
}

/// Translates a tonic streaming response into a stream of StreamValues, the set of things that can flow across the gRPC
/// event stream.
fn grpc_to_stream(
    response: kuro_error::Result<tonic::Response<tonic::Streaming<MultiCommandProgress>>>,
) -> impl Stream<Item = kuro_error::Result<StreamValue>> {
    ...
    let stream = stream
        ...
        .map_ok(|e| stream::iter(e.messages.into_iter().map(kuro_error::Ok)))
        .try_flatten();
...
impl BuckdClient {
    ...
    pub async fn status<'a>(
    ...
    ) -> kuro_error::Result<StatusResponse> {
        let outcome = events_ctx
            ...
            .await;
        // TODO(nmj): We have a number of things that wish to use status() and return an kuro_error::Result,
        // for now we'll just turn a "CommandMessage" into a error, but that's really not what we
        // want long term.
        match outcome? {
            CommandOutcome::Success(r) => Ok(r),
            CommandOutcome::Failure(_) => Err(kuro_error::kuro_error!(
                kuro_error::ErrorTag::DaemonStatus,
...
impl TryFrom<kuro_cli_proto::partial_result::PartialResult> for NoPartialResult {
    type Error = kuro_cli_proto::partial_result::PartialResult;

    fn try_from(v: kuro_cli_proto::partial_result::PartialResult) -> Result<Self, Self::Error> {
        Err(v)
    }
}
...
impl PartialResultHandler for StdoutPartialResultHandler {
    type PartialResult = kuro_cli_proto::StdoutBytes;
...
macro_rules! wrap_method {
    ($method: ident ($($param: ident : $param_type: ty),*), $res: ty) => {
        pub async fn $method(&mut self, events_ctx: &mut EventsCtx, $($param: $param_type)*) -> kuro_error::Result<$res> {
            let mut events_ctx = DaemonEventsCtx::new(self.inner, events_ctx)?;
            let out = self
                .inner
...
impl FlushingBuckdClient<'_> {
    ...
    stream_method!(bxl, BxlRequest, BxlResponse, kuro_cli_proto::StdoutBytes);
    ...
    pub async fn new_generic(
        &mut self,
        context: kuro_cli_proto::ClientContext,
        req: NewGenericRequest,
        events_ctx: &mut EventsCtx,
        stdin: Option<ConsoleInteractionStream<'_>>,
    ) -> kuro_error::Result<CommandOutcome<NewGenericResponse>> {
        let req = serde_json::to_string(&req)
            .buck_error_context("Could not serialize `NewGenericRequest`")?;
        let req = kuro_cli_proto::NewGenericRequestMessage {
            context: Some(context),
            new_generic_request: req,
        };
        let command_outcome: CommandOutcome<kuro_cli_proto::NewGenericResponseMessage> = self
