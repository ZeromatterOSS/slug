...
enum ArgExpansionError {
    ...
    StdinReadError { source: kuro_error::Error },
}
...
pub fn log_relative_path_from_cell_root(requested_path: &str) -> kuro_error::Result<()> {
...
mod tests {
    ...
    #[test]
    fn test_resolve_argfile_kind_with_project() -> kuro_error::Result<()> {
    ...
    #[test]
    fn test_expand_argfile_content() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
        let cwd =
    ...
    #[test]
    fn test_comment() -> kuro_error::Result<()> {
        let content = indoc! {"
            --# Usage: kuro build @mode/mrustc ...
            --#
