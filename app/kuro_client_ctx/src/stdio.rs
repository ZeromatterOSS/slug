...
fn stdout() -> kuro_error::Result<io::Stdout> {
...
pub fn _print(fmt: Arguments) -> kuro_error::Result<()> {
    stdout()?
        .lock()
        .write_fmt(fmt)
...
pub fn _eprint(fmt: Arguments) -> kuro_error::Result<()> {
    io::stderr()
        .lock()
        .write_fmt(fmt)
        .map_err(|e| ClientIoError::from(e).into())
...
pub fn print_bytes(bytes: &[u8]) -> kuro_error::Result<()> {
    stdout()?
        .lock()
        .write_all(bytes)
        .map_err(|e| ClientIoError::from(e).into())
...
pub fn eprint_line(line: &Line) -> kuro_error::Result<()> {
    let line = line.render();
    crate::eprintln!("{}", line)
}

pub fn flush() -> kuro_error::Result<()> {
    stdout()?.flush().map_err(|e| ClientIoError::from(e).into())
}

fn stdout_to_file(stdout: &Stdout) -> kuro_error::Result<File> {
    #[cfg(not(windows))]
    {
        use std::os::fd::AsFd;
...
pub async fn print_with_writer<E, F>(f: F) -> kuro_error::Result<()>
where
    E: Into<kuro_error::Error>,
