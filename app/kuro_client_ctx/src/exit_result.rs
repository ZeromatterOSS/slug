...
use kuro_error::classify::ErrorTagExtra;
use kuro_error::classify::best_error;
...
///
/// We can easily turn a kuro_error::Result (or kuro_error::Error, or even a message) into a ExitResult,
...
impl ExitResult {
    ...
    pub fn get_error(&self) -> Option<kuro_error::Error> {
    ...
    pub fn bail(msg: impl Display) -> Self {
        Self::err(kuro_error::kuro_error!(
            kuro_error::ErrorTag::Bail,
            "Command failed: {}",
            msg
        ))
    }
    ...
    pub fn err(err: kuro_error::Error) -> Self {
        Self {
    ...
    pub fn err_with_exit_code(err: kuro_error::Error, exit_code: ExitCode) -> Self {
        Self {
    ...
    /// Kuro supports being built as both a "full" binary as well as a "client-only" binary.
    ///
    ...
    pub fn retry_command_with_full_binary() -> kuro_error::Result<Option<Self>> {
...
/// We can produce a ExitResult from a `kuro_error::Result` for convenience.
impl From<kuro_error::Result<()>> for ExitResult {
    fn from(e: kuro_error::Result<()>) -> Self {
        match e {
            Ok(()) => Self::success(),
            Err(e) => Self::err(e),
        }
...
impl From<kuro_error::Result<ExitCode>> for ExitResult {
    fn from(e: kuro_error::Result<ExitCode>) -> Self {
        match e {
...
impl From<kuro_error::Error> for ExitResult {
    fn from(e: kuro_error::Error) -> Self {
        Self::err(e)
    }
}

impl FromResidual<kuro_error::Error> for ExitResult {
    #[track_caller]
    fn from_residual(residual: kuro_error::Error) -> ExitResult {
        Self::err(residual)
    }
}

impl<E: Into<::kuro_error::Error>> FromResidual<Result<Infallible, E>> for ExitResult {
    #[track_caller]
    fn from_residual(residual: Result<Infallible, E>) -> ExitResult {
        match residual {
            // E -> kuro_error::Error -> ExitResult
...
impl ExitResultVariant {
    pub fn report(self) -> ! {
        ...
        let mut exit_code = match self {
            Self::Status(v) => v,
            Self::Exec(args) => {
                // Terminate by exec-ing a new process - usually because of `kuro run`.
                //
        ...
        // So the result is non-reproducible crash of the kuro client.
...
#[derive(kuro_error::Error, derivative::Derivative)]
...
pub enum ClientIoError {
    ...
    /// An easy way to trigger this is `kuro audit config | head`
    #[kuro(tag = IoClientBrokenPipe)]
    #[kuro(environment)]
    BrokenPipe(io::Error),
    #[kuro(tier0)]
    OtherIo(io::Error),
    #[kuro(tier0)]
    Other(kuro_error::Error),
}

impl From<kuro_error::Error> for ClientIoError {
    fn from(error: kuro_error::Error) -> Self {
...
#[cfg(windows)]
fn do_exec(command: &mut Command) -> kuro_error::Error {
...
fn execv(args: ExecArgs) -> ! {
    // patternlint-disable-next-line kuro-no-command-new
