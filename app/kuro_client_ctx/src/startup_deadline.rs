...
/// Utility to time out properly with context during kuro client startup.
///
...
impl StartupDeadline {
    pub fn duration_from_now(duration: Duration) -> kuro_error::Result<StartupDeadline> {
        Ok(StartupDeadline {
    ...
    pub(crate) fn down_deadline(&self) -> kuro_error::Result<StartupDeadline> {
    ...
    pub(crate) fn rem_duration(&self, op: &str) -> kuro_error::Result<Duration> {
        self.deadline
    ...
    pub(crate) async fn down<R, Fut, F>(&self, op: &str, f: F) -> kuro_error::Result<R>
    where
        F: FnOnce(StartupDeadline) -> Fut,
        Fut: Future<Output = kuro_error::Result<R>>,
    {
    ...
    pub(crate) async fn run<R, Fut>(&self, op: &str, f: Fut) -> kuro_error::Result<R>
    where
        Fut: Future<Output = kuro_error::Result<R>>,
    {
    ...
    pub(crate) fn min(&self, other: Duration) -> kuro_error::Result<StartupDeadline> {
    ...
    pub(crate) fn half(&self) -> kuro_error::Result<StartupDeadline> {
        let now = Instant::now();
