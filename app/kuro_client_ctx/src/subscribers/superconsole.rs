...
use kuro_event_observer::event_observer::DebugEventObserverExtra;
use kuro_event_observer::session_info::SessionInfo;
use kuro_event_observer::unpack_event::VisitorError;
use kuro_event_observer::unpack_event::unpack_event;
use kuro_event_observer::verbosity::Verbosity;
use kuro_event_observer::what_ran::command_to_string;
use kuro_event_observer::what_ran::worker_command_as_fallback_to_string;
...
impl Component for BuckRootComponent<'_> {
    ...
    fn draw_unchecked(&self, dimensions: Dimensions, mode: DrawMode) -> kuro_error::Result<Lines> {
...
impl StatefulSuperConsole {
    ...
    /// Construct a console suitable for use by the Kuro CLI. We use non-blocking output here
    ...
    pub fn render_result_errors(result: &kuro_cli_proto::CommandResult) -> Lines {
        let mut lines = Lines::new();
        if let kuro_cli_proto::CommandResult {
            result: Some(kuro_cli_proto::command_result::Result::Error(e)),
        } = result
        {
    ...
    async fn handle_event(&mut self, ev: &Arc<BuckEvent>) -> kuro_error::Result<()> {
        match self {
    ...
    fn finalize(&mut self) -> kuro_error::Result<()> {
        let mut res = Ok(());
...
impl SuperConsoleState {
    pub fn new(
        ...
        config: SuperConsoleConfig,
        health_check_reports_receiver: Option<Receiver<Vec<DisplayReport>>>,
    ) -> kuro_error::Result<SuperConsoleState> {
...
impl StatefulSuperConsoleImpl {
    ...
    async fn handle_stderr(&mut self, msg: &str) -> kuro_error::Result<()> {
        self.super_console
    ...
    async fn handle_console_message(
        &mut self,
        message: &kuro_data::ConsoleMessage,
    ) -> kuro_error::Result<()> {
    ...
    async fn handle_streaming_output(
        &mut self,
        message: &kuro_data::StdoutStreamingOutput,
    ) -> kuro_error::Result<()> {
        self.super_console
    ...
    async fn handle_console_warning(
        &mut self,
        message: &kuro_data::ConsoleWarning,
    ) -> kuro_error::Result<()> {
        let style = ContentStyle {
    ...
    async fn handle_console_preferences(
        &mut self,
        prefs: &kuro_data::ConsolePreferences,
    ) -> kuro_error::Result<()> {
    ...
    async fn tick(&mut self, tick: &Tick) -> kuro_error::Result<()> {
    ...
    fn finalize(
        ...
        SuperConsoleState,
        Option<superconsole::Error<kuro_error::Error>>,
    ) {
        let err = self
            .super_console
...
impl EventSubscriber for StatefulSuperConsole {
    ...
    async fn handle_error(&mut self, _error: &kuro_error::Error) -> kuro_error::Result<()> {
        self.finalize()?;
        Ok(())
    }
}
    ...
    if let Some(command_kind) = command_failed.command_kind.as_ref() {
        ...
        match command_kind.command.as_ref() {
                ...
                let command = if verbosity.print_failure_full_command() {
                    ...
                    match truncate(command) {
                        ...
                        Some(short) => Cow::Owned(format!(
                            "{short} (run `kuro log what-failed` to get the full command)"
                        )),
                    }
                };
            ...
            Some(Command::RemoteCommand(remote_command)) => {
                let help_message = if kuro_core::is_open_source() {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_transfer_state_to_simpleconsole() -> kuro_error::Result<()> {
        ...
        let event = Arc::new(BuckEvent::new(
            ...
            Some(id),
            None,
            kuro_data::buck_event::Data::SpanStart(SpanStartEvent {
                data: Some(kuro_data::span_start_event::Data::Load(
                    LoadBuildFileStart {
        ...
        console
            .handle_command_result(&CommandResult {
                result: Some(kuro_cli_proto::command_result::Result::GenericResponse(
                    GenericResponse {},
                )),
            })
            .await
        ...
        let event = Arc::new(BuckEvent::new(
            ...
            Some(id),
            None,
            kuro_data::buck_event::Data::SpanEnd(SpanEndEvent {
                data: Some(kuro_data::span_end_event::Data::Load(LoadBuildFileEnd {
                    module_id: "foo".to_owned(),
                    cell: "bar".to_owned(),
                    error: None,
    ...
    #[tokio::test]
    async fn test_default_layout() -> kuro_error::Result<()> {
        let trace_id = TraceId::new();
        ...
        console
            .handle_event(&Arc::new(BuckEvent::new(
                ...
                kuro_data::buck_event::Data::SpanStart(SpanStartEvent {
                    data: Some(
                        kuro_data::CommandStart {
                            data: Some(kuro_data::BuildCommandStart {}.into()),
                            ..Default::default()
                        }
                        .into(),
                    ),
        ...
        console
            .handle_event(&Arc::new(BuckEvent::new(
                ...
                kuro_data::InstantEvent {
                    data: Some(
                        kuro_data::RemoteExecutionSessionCreated {
        ...
        console
            .handle_command_result(&kuro_cli_proto::CommandResult { result: None })
            .await?;
    ...
    #[test]
    fn test_session_info() -> kuro_error::Result<()> {
        let info = SessionInfo {
            trace_id: TraceId::null(),
            test_session: Some(kuro_data::TestSessionInfo {
    ...
    #[tokio::test]
    async fn test_tailer_stderr() -> kuro_error::Result<()> {
        let trace_id = TraceId::new();
        let tick = Tick::now();
