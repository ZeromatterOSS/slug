...
use kuro_event_log::wait_for_child_and_log;
...
impl ReLog {
    ...
    fn log_upload(
        &mut self,
    ) -> impl Future<Output = kuro_error::Result<()>> + 'static + Send + Sync + use<> {
...
async fn log_upload_impl(session_id: String, isolation_dir: FileNameBuf) -> kuro_error::Result<()> {
    ...
    let mut buck = kuro_util::process::async_background_command(std::env::current_exe()?);
    let command = buck
        .arg("--isolation-dir")
