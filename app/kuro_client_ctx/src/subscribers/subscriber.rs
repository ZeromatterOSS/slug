...
pub trait EventSubscriber: Send {
    ...
    /// stdout.
    async fn handle_output(&mut self, _raw_output: &[u8]) -> kuro_error::Result<()> {
        Ok(())
    }
    /// Fired by the tailer for stderr.
    async fn handle_tailer_stderr(&mut self, _stderr: &str) -> kuro_error::Result<()> {
        Ok(())
    }
    ...
    async fn handle_events(&mut self, _event: &[Arc<BuckEvent>]) -> kuro_error::Result<()> {
        Ok(())
    }
    async fn handle_command_result(
        &mut self,
        _result: &kuro_cli_proto::CommandResult,
    ) -> kuro_error::Result<()> {
        Ok(())
    }
    ...
    async fn tick(&mut self, _tick: &Tick) -> kuro_error::Result<()> {
        Ok(())
    }
    ...
    fn handle_daemon_started(&mut self, _reason: kuro_data::DaemonWasStartedReason) {}
