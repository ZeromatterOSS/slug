...
use kuro_health_check::health_check_client::HealthCheckClient;
use kuro_health_check::health_check_client::StreamingHealthCheckClient;
use kuro_health_check::interface::HealthCheckContextEvent;
use kuro_health_check::interface::HealthCheckEvent;
...
impl HealthCheckSubscriber {
    ...
    async fn handle_event(&mut self, event: &Arc<BuckEvent>) -> kuro_error::Result<()> {
        ...
        let health_check_event = match event.data() {
            SpanStart(start) => match &start.data {
                Some(kuro_data::span_start_event::Data::Command(command)) => {
                    Some(HealthCheckEvent::HealthCheckContextEvent(
                        HealthCheckContextEvent::CommandStart(kuro_data::CommandStartWithTraceId {
            ...
            Instant(instant) => {
                    ...
                    .buck_error_context("Missing `data` in `Instant`")?
                {
                    ...
                    Snapshot(_snapshot) => {
                        ...
                        let snapshot_data = kuro_health_check::interface::HealthCheckSnapshotData {
    ...
    pub fn close_client_connection_with_error_report(&mut self, error: &str) {
        ...
        let _ignored = soft_error!(
            ...
            kuro_error::kuro_error!(kuro_error::ErrorTag::HealthCheck, "{}", error)
        );
    }
}
...
mod tests {
    ...
    use kuro_health_check::report::DisplayReport;
    use kuro_health_check::report::HealthIssue;
    use kuro_health_check::report::Message;
    use kuro_health_check::report::Severity;
    ...
    fn test_event(data: kuro_data::buck_event::Data) -> Arc<BuckEvent> {
    ...
    fn event_for_excess_cache_miss() -> kuro_data::buck_event::Data {
        let action_end = ActionExecutionEnd {
            kind: ActionKind::Run as i32,
            invalidation_info: Some(kuro_data::CommandInvalidationInfo {
                changed_file: None,
                changed_any: None,
            }),
            ..Default::default()
        ...
        kuro_data::buck_event::Data::SpanEnd(kuro_data::SpanEndEvent {
            data: Some(kuro_data::span_end_event::Data::ActionExecution(
                Box::new(action_end).into(),
            )),
            ..kuro_data::SpanEndEvent::default()
        })
    }

    #[tokio::test]
    async fn test_health_check_subscriber() -> kuro_error::Result<()> {
        ...
        let event = test_event(
            kuro_data::InstantEvent {
                data: Some(Box::new(kuro_data::Snapshot::default()).into()),
            }
            .into(),
        );
    ...
    #[tokio::test]
    async fn test_event_limit_exceeded() -> kuro_error::Result<()> {
        let (events_tx, mut events_rx) = mpsc::channel::<HealthCheckEvent>(EVENT_CHANNEL_SIZE);
        ...
        let event = test_event(kuro_data::buck_event::Data::Instant(
            kuro_data::InstantEvent {
                data: Some(Box::new(kuro_data::Snapshot::default()).into()),
            },
        ));
    ...
    #[tokio::test]
    async fn test_slow_client() -> kuro_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_dropped_client() -> kuro_error::Result<()> {
        let (events_tx, events_rx_guard) = mpsc::channel::<HealthCheckEvent>(EVENT_CHANNEL_SIZE);
