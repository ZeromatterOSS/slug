...
use kuro_event_observer::event_observer::EventObserver;
use kuro_event_observer::event_observer::EventObserverExtra;
...
use kuro_event_observer::what_ran::WhatRanCommandConsoleFormat;
use kuro_event_observer::what_ran::WhatRanOutputCommand;
use kuro_event_observer::what_ran::WhatRanOutputWriter;
...
/// kuro daemon info is printed to stderr if there are no other updates available
...
macro_rules! echo {
    () => {
        {
            // patternlint-disable-next-line kuro-cli-simpleconsole-echo
    ...
    E: EventObserverExtra,
{
    ...
    fn print_stats_while_waiting(&mut self) -> kuro_error::Result<()> {
        ...
        if is_snapshot_stale {
            ...
            if let Some((_, snapshot)) = &snapshots.last {
                if let Some(kuro_rss) = snapshot.kuro_rss {
                    parts.push(format!("RSS: {}", HumanizedBytes::new(kuro_rss)));
                }
            }
    ...
    fn print_action_error(&mut self, error: &kuro_data::ActionError) -> kuro_error::Result<()> {
    ...
    pub(crate) async fn handle_event(&mut self, event: &Arc<BuckEvent>) -> kuro_error::Result<()> {
    ...
    async fn handle_event_inner(&mut self, event: &BuckEvent) -> kuro_error::Result<()> {
        match unpack_event(event)? {
            kuro_event_observer::unpack_event::UnpackedBuckEvent::SpanStart(_, _, data) => {
                match data {
    ...
    async fn handle_command_start(
        &mut self,
        _command: &kuro_data::CommandStart,
        event: &BuckEvent,
    ) -> kuro_error::Result<()> {
        if cfg!(fbcode_build) {
            echo!(
                "Buck UI: https://www.internalfb.com/kuro/{}",
                event.trace_id()?
            )?;
        } else {
    ...
    async fn handle_test_discovery(
        ...
        if let Some(data) = &test_info.data {
            match data {
                kuro_data::test_discovery::Data::Session(kuro_data::TestSessionInfo { info }) => {
                    echo!("Test session: {}", info)?;
                    self.notify_printed();
                }
                kuro_data::test_discovery::Data::Tests(..) => {}
            }
        }
    ...
    pub(crate) async fn handle_stderr(&mut self, stderr: &str) -> kuro_error::Result<()> {
        echo!("{}", stderr)?;
        self.notify_printed();
        Ok(())
    }
    ...
    fn echo_health_check_warning(&self, report: &DisplayReport) -> kuro_error::Result<()> {
    ...
    E: EventObserverExtra,
{
    ...
    async fn tick(&mut self, _: &Tick) -> kuro_error::Result<()> {
        if self.verbosity.print_status()
