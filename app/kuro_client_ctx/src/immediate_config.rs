...
impl ImmediateConfig {
    ...
    fn parse(roots: &InvocationRoots) -> kuro_error::Result<ImmediateConfig> {
...
pub struct ImmediateConfigContext<'a> {
    ...
    // us to have a shared reference to the underlying `kuro_error::Error` which
...
impl<'a> ImmediateConfigContext<'a> {
    ...
    pub fn daemon_startup_config(&self) -> kuro_error::Result<&DaemonStartupConfig> {
    ...
    fn data(&self) -> kuro_error::Result<&ImmediateConfigContextData> {
        self.data
            .get_or_try_init(|| {
                ...
                kuro_error::Ok(ImmediateConfigContextData {
    ...
    pub(crate) fn resolve_argfile_kind(
        ...
        flag: Option<&str>,
    ) -> Result<kuro_common::argv::ArgFileKind, kuro_error::Error> {
...
fn is_paranoid_enabled(path: &AbsPath) -> kuro_error::Result<bool> {
    if let Some(p) = kuro_env!("BUCK_PARANOID", type=bool)? {
        return Ok(p);
    }
    ...
    let info = kuro_cli_proto::ParanoidInfo::decode(bytes.as_slice())
