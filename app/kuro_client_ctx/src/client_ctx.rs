...
use kuro_cli_proto::client_context::HostPlatformOverride as GrpcHostPlatformOverride;
use kuro_cli_proto::client_context::PreemptibleWhen as GrpcPreemptibleWhen;
...
use kuro_core::error::kuro_hard_error_env;
...
pub struct ClientCommandContext<'a> {
    ...
    pub(crate) start_in_process_daemon:
        Option<Box<dyn FnOnce() -> kuro_error::Result<()> + Send + Sync>>,
    pub(crate) argv: Argv,
...
impl<'a> ClientCommandContext<'a> {
    pub fn new(
        ...
        start_in_process_daemon: Option<Box<dyn FnOnce() -> kuro_error::Result<()> + Send + Sync>>,
        argv: Argv,
        trace_id: TraceId,
        stdin: &'a mut Stdin,
    ...
    pub fn paths(&self) -> kuro_error::Result<&InvocationPaths> {
        match &self.paths {
    ...
    pub fn maybe_paths(&self) -> kuro_error::Result<Option<&InvocationPaths>> {
        match &self.paths {
    ...
    pub fn client_context<T: StreamingCommand>(
        ...
        cmd: &T,
    ) -> kuro_error::Result<ClientContext> {
    ...
    pub fn empty_client_context(&self, command_name: &str) -> kuro_error::Result<ClientContext> {
        ...
        Ok(ClientContext {
            ...
            argfiles: Vec::new(),
            kuro_hard_error: kuro_hard_error_env()?.unwrap_or_default().to_owned(),
    ...
    pub fn log_download_method(&self) -> kuro_error::Result<LogDownloadMethod> {
        Ok(self
            .immediate_config
