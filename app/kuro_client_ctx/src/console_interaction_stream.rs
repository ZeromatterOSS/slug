...
mod interactive_terminal {
    ...
    impl InteractiveTerminal {
        pub fn enable() -> kuro_error::Result<Option<Self>> {
        ...
        pub fn disable(&mut self) -> kuro_error::Result<()> {
            let fd = std::io::stdin().as_raw_fd();
...
mod interactive_terminal {
    ...
    fn get_console_mode(handle: HANDLE) -> kuro_error::Result<DWORD> {
    ...
    fn set_console_mode(handle: HANDLE, mode: DWORD) -> kuro_error::Result<()> {
...
pub trait SuperConsoleInteraction: Send + Sync {
    async fn toggle(&mut self) -> kuro_error::Result<Option<SuperConsoleToggle>>;
}
...
impl SuperConsoleInteraction for ConsoleInteractionStream<'_> {
    async fn toggle(&mut self) -> kuro_error::Result<Option<SuperConsoleToggle>> {
        match self.stdin.read_u8().await {
            ...
            Err(e) => Err(kuro_error::Error::from(e).context("Error reading char from console")),
        }
    }
}
