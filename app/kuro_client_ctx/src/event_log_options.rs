...
use kuro_event_log::file_names::find_log_by_trace_id;
use kuro_event_log::file_names::retrieve_nth_recent_log;
use kuro_event_log::read::EventLogPathBuf;
use kuro_event_log::utils::Encoding;
use kuro_fs::fs_util;
...
impl EventLogOptions {
    ...
    async fn download_remote_id(
        ...
        let log_file_name = FileNameBuf::try_from(format!(
            "{}{}",
            trace_id,
            // TODO(nga): hardcoded default, should at least use the same default kuro uses,
        ...
        let (command_name, command) = match ctx.log_download_method()? {
            LogDownloadMethod::Manifold => {
                let args = [
                    "get",
                    &format!("kuro_logs/flat/{log_file_name}"),
                    temp_path
                        .path()
                        .as_os_str()
                        .to_str()
