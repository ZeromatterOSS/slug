...
use kuro_downward_api_proto::LogRequest;
use kuro_downward_api_proto::downward_api_client;
use kuro_downward_api_proto::downward_api_server;
...
use kuro_test_proto::AttachInfoMessageRequest;
use kuro_test_proto::Empty;
use kuro_test_proto::EndOfTestResultsRequest;
use kuro_test_proto::ExecuteResponse2;
use kuro_test_proto::PrepareForLocalExecutionResponse;
use kuro_test_proto::ReportTestResultRequest;
use kuro_test_proto::ReportTestSessionRequest;
use kuro_test_proto::ReportTestsDiscoveredRequest;
...
/// Test runner client to kuro test orchestrator.
///
/// When running `kuro test`, kuro starts a gRPC server
/// and spawns a test runner which connects to kuro process using this client.
...
impl DownwardApi for TestOrchestratorClient {
    async fn console(&self, level: Level, message: String) -> kuro_error::Result<()> {
    ...
    async fn log(&self, level: Level, message: String) -> kuro_error::Result<()> {
        let level = level.try_into().buck_error_context("Invalid `level`")?;
    ...
    async fn external(&self, data: HashMap<String, String>) -> kuro_error::Result<()> {
        let event = data.into();
...
impl TestOrchestratorClient {
    pub async fn execute2(
        ...
        let req: kuro_test_proto::ExecuteRequest2 = req
            .try_into()
        ...
        let response = match response.buck_error_context("Missing `response`")? {
            kuro_test_proto::execute_response2::Response::Result(res) => {
                ExecuteResponse::Result(res.try_into().buck_error_context("Invalid `result`")?)
            }
            kuro_test_proto::execute_response2::Response::Cancelled(
                kuro_test_proto::Cancelled { reason },
            ) => {
                let reason = match reason {
                    Some(reason) => kuro_test_proto::CancellationReason::try_from(reason)
                        .map(|proto_reason| match proto_reason {
                            kuro_test_proto::CancellationReason::NotSpecified => {
                                crate::data::CancellationReason::NotSpecified
                            }
                            kuro_test_proto::CancellationReason::ReQueueTimeout => {
    ...
    pub async fn report_test_result(&self, result: TestResult) -> kuro_error::Result<()> {
    ...
    pub async fn report_test_session(&self, session_info: String) -> kuro_error::Result<()> {
        self.test_orchestrator_client
            .clone()
    ...
    pub async fn end_of_test_results(&self, exit_code: i32) -> kuro_error::Result<()> {
        self.test_orchestrator_client
            .clone()
    ...
    pub async fn prepare_for_local_execution(
        ...
        let executable: kuro_test_proto::TestExecutable = executable
            .try_into()
            .buck_error_context("Invalid prepare_for_local_execution request")?;

        let request = kuro_test_proto::PrepareForLocalExecutionRequest {
    ...
    pub async fn attach_info_message(&self, message: String) -> kuro_error::Result<()> {
        self.test_orchestrator_client
            .clone()
    ...
    T: TestOrchestrator + Send + Sync + 'static,
{
    async fn execute2(
        &self,
        request: tonic::Request<kuro_test_proto::ExecuteRequest2>,
            ...
            let response = match response {
                ...
                ExecuteResponse::Cancelled(reason) => {
                    let reason = if let Some(reason) = reason {
                        match reason {
                            crate::data::CancellationReason::NotSpecified => {
                                Some(kuro_test_proto::CancellationReason::NotSpecified.into())
                            }
                            crate::data::CancellationReason::ReQueueTimeout => {
                                Some(kuro_test_proto::CancellationReason::ReQueueTimeout.into())
                            }
                        }
                    } else {
                        None
    ...
    async fn prepare_for_local_execution(
        &self,
        request: tonic::Request<kuro_test_proto::PrepareForLocalExecutionRequest>,
    ) -> Result<tonic::Response<PrepareForLocalExecutionResponse>, tonic::Status> {
        to_tonic(async move {
            let kuro_test_proto::PrepareForLocalExecutionRequest {
                test_executable,
                required_local_resources,
            } = request.into_inner();
    ...
    T: DownwardApi + Send + Sync + 'static,
{
    ...
    ) -> Result<tonic::Response<kuro_downward_api_proto::Empty>, tonic::Status> {
        to_tonic(async move {
            ...
            Ok(kuro_downward_api_proto::Empty {})
        })
        .await
    }
