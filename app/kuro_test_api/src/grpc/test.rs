...
impl TestExecutor for MockExecutor {
    async fn external_runner_spec(&self, _: ExternalRunnerSpec) -> kuro_error::Result<()> {
        Ok(())
    }
...
#[tokio::test]
async fn test_basic() -> kuro_error::Result<()> {
...
#[tokio::test]
async fn test_client_disconnect() -> kuro_error::Result<()> {
    let (client_io, server_io) = tokio::io::duplex(64);
    let mut server =
