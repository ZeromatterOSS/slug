...
use slug_cli_proto::targets_request;
use slug_cli_proto::targets_request::OutputFormat;
...
use slug_client_ctx::console_interaction_stream::ConsoleInteractionStream;
use slug_client_ctx::daemon::client::BuckdClientConnector;
use slug_client_ctx::daemon::client::NoPartialResultHandler;
use slug_client_ctx::daemon::client::StdoutPartialResultHandler;
...
use slug_client_ctx::query_args::CommonAttributeArgs;
...
impl Compression {
    fn to_proto(&self) -> slug_cli_proto::targets_request::Compression {
        match self {
            Compression::None => slug_cli_proto::targets_request::Compression::Uncompressed,
            Compression::Gzip => slug_cli_proto::targets_request::Compression::Gzip,
            Compression::Zstd => slug_cli_proto::targets_request::Compression::Zstd,
        }
    }
}
...
impl TargetsCommand {
    ...
    fn output_format(&self) -> slug_error::Result<OutputFormat> {
        if self.json {
    ...
    fn package_values_as_regexes(&self) -> slug_error::Result<Vec<String>> {
...
impl StreamingCommand for TargetsCommand {
    ...
    async fn exec_impl(
        ...
        let target_hash_use_fast_hash = match self.target_hash_function {
            TargetHashFunction::Sha1 | TargetHashFunction::Sha256 => {
                slug_client_ctx::eprintln!(
                    "slug only supports \"fast\" and \"strong\" target hash functions. Using the \"strong\" hash."
                )?;
                false
            }
            TargetHashFunction::Murmur_Hash3 => {
                slug_client_ctx::eprintln!(
                    "slug only supports \"fast\" and \"strong\" target hash functions. Using the \"fast\" hash."
                )?;
                true
            }
        ...
        let target_request = TargetsRequest {
            ...
            concurrency: self
                .num_threads
                .map(|num| slug_cli_proto::Concurrency { concurrency: num }),
...
async fn targets_show_outputs(
    ...
    slug_client_ctx::stdio::print_with_writer::<ClientIoError, _>(async move |w| {
...
async fn targets(
    ...
    if !response.serialized_targets_output.is_empty() {
        slug_client_ctx::print!("{}", response.serialized_targets_output)?;
    }
    ExitResult::success()
}
