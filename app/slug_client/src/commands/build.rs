...
use slug_cli_proto::BuildTarget;
use slug_cli_proto::TargetCfg;
use slug_cli_proto::build_request::BuildProviders;
use slug_cli_proto::build_request::ResponseOptions;
use slug_cli_proto::build_request::build_providers;
...
use slug_client_ctx::common::PrintOutputsFormat;
use slug_client_ctx::common::build::CommonBuildOptions;
use slug_client_ctx::common::build::CommonOutputOptions;
use slug_client_ctx::common::target_cfg::TargetCfgWithUniverseOptions;
...
pub trait MaterializationsToProto {
    fn to_proto(&self) -> slug_cli_proto::build_request::Materializations;
}
impl MaterializationsToProto for Option<FinalArtifactMaterializations> {
    fn to_proto(&self) -> slug_cli_proto::build_request::Materializations {
        match self {
            Some(FinalArtifactMaterializations::All) => {
                slug_cli_proto::build_request::Materializations::Materialize
            }
            Some(FinalArtifactMaterializations::None) => {
                slug_cli_proto::build_request::Materializations::Skip
            }
            None => slug_cli_proto::build_request::Materializations::Default,
        }
    }
}
...
pub trait UploadsToProto {
    fn to_proto(&self) -> slug_cli_proto::build_request::Uploads;
}
impl UploadsToProto for Option<FinalArtifactUploads> {
    fn to_proto(&self) -> slug_cli_proto::build_request::Uploads {
        match self {
            Some(FinalArtifactUploads::Always) => slug_cli_proto::build_request::Uploads::Always,
            Some(FinalArtifactUploads::Never) => slug_cli_proto::build_request::Uploads::Never,
            None => slug_cli_proto::build_request::Uploads::Never,
        }
    }
}
...
impl StreamingCommand for BuildCommand {
    ...
    async fn exec_impl(
        ...
        if slug_env!("BUCK2_TEST_BUILD_ERROR", bool, applicability = testing)? {
            return slug_error!(
                slug_error::ErrorTag::TestOnly,
...
pub(crate) fn print_build_failed(console: &FinalConsole) -> slug_error::Result<()> {
...
mod tests {
    ...
    fn parse(args: &[&str]) -> slug_error::Result<BuildCommand> {
    ...
    #[test]
    fn infos_default() -> slug_error::Result<()> {
        let opts = parse(&[])?;
    ...
    #[test]
    fn infos_noop() -> slug_error::Result<()> {
        let opts = parse(&[
            "--skip-test-info",
            "--build-default-info",
            "--build-run-info",
    ...
    #[test]
    fn infos_configure() -> slug_error::Result<()> {
    ...
    #[test]
    fn infos_validation() -> slug_error::Result<()> {
        // Test duplicate args
        assert_matches!(
