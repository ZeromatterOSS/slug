...
use slug_client_ctx::daemon::client::BuckdLifecycleLock;
use slug_client_ctx::daemon::client::kill::kill_command_impl;
...
use slug_client_ctx::startup_deadline::StartupDeadline;
...
///
/// The command also kills the slug daemon.
...
impl BuckSubcommand for InnerCleanCommand {
    ...
    async fn exec_impl(
        ...
        ctx: ClientCommandContext<'_>,
        _events_ctx: &mut slug_client_ctx::events_ctx::EventsCtx,
    ) -> ExitResult {
        let paths = ctx.paths()?;
        ...
        kill_command_impl(&lifecycle_lock, "`slug clean` was invoked").await?;
...
async fn clean(
    ...
    if background {
        ...
        console.print_stderr("You can run other slug commands while this completes.")?;
...
) -> slug_error::Result<Vec<AbsNormPathBuf>> {
...
/// add a single retry.
fn clean_buck_out_with_retry(path: &AbsNormPathBuf) -> slug_error::Result<()> {
...
fn clean_buck_out(path: &AbsNormPathBuf) -> slug_error::Result<()> {
    let walk = WalkDir::new(path);
    let thread_pool = ThreadPool::new(slug_util::threads::available_parallelism());
