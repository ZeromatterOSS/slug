...
use slug_cli_proto::CounterWithExamples;
use slug_cli_proto::TestRequest;
use slug_cli_proto::TestSessionOptions;
use slug_client_ctx::client_ctx::ClientCommandContext;
use slug_client_ctx::common::BuckArgMatches;
use slug_client_ctx::common::CommonBuildConfigurationOptions;
use slug_client_ctx::common::CommonCommandOptions;
use slug_client_ctx::common::CommonEventLogOptions;
use slug_client_ctx::common::CommonStarlarkOptions;
use slug_client_ctx::common::build::CommonBuildOptions;
use slug_client_ctx::common::target_cfg::TargetCfgOptions;
use slug_client_ctx::common::timeout::CommonTimeoutOptions;
use slug_client_ctx::common::ui::CommonConsoleOptions;
use slug_client_ctx::daemon::client::BuckdClientConnector;
use slug_client_ctx::daemon::client::NoPartialResultHandler;
use slug_client_ctx::events_ctx::EventsCtx;
use slug_client_ctx::exit_result::ExitResult;
use slug_client_ctx::final_console::FinalConsole;
use slug_client_ctx::output_destination_arg::OutputDestinationArg;
use slug_client_ctx::path_arg::PathArg;
use slug_client_ctx::stdio::eprint_line;
use slug_client_ctx::streaming::StreamingCommand;
use slug_client_ctx::subscribers::subscriber::EventSubscriber;
use slug_client_ctx::subscribers::superconsole::test::TestCounterColumn;
use slug_client_ctx::subscribers::superconsole::test::span_from_build_failure_count;
use slug_error::BuckErrorContext;
use slug_error::ErrorTag;
use slug_error::ExitCode;
use slug_error::slug_error;
use slug_event_observer::display::TestOutputMode;
use slug_event_observer::display::set_test_output_mode;
...
///
/// The `slug test --xml <path>` flag triggers collection of test result events
...
impl EventSubscriber for XmlTestResultWriter {
    ...
    async fn handle_events(
        &mut self,
        events: &[Arc<slug_events::BuckEvent>],
    ) -> slug_error::Result<()> {
        for event in events {
            if let Ok(slug_event_observer::unpack_event::UnpackedBuckEvent::Instant(
                _,
                _,
                slug_data::instant_event::Data::TestResult(result),
            )) = unpack_event(event)
            {
                let status_enum = slug_data::TestStatus::try_from(result.status)
                    .unwrap_or(slug_data::TestStatus::Unknown);
                // Listing results are metadata, not test executions — skip them.
                if matches!(
                    status_enum,
                    slug_data::TestStatus::ListingSuccess | slug_data::TestStatus::ListingFailed
                ) {
                    continue;
                }
                let (status_str, type_str) = match status_enum {
                    slug_data::TestStatus::Pass => ("pass", "SUCCESS"),
                    slug_data::TestStatus::Fail => ("fail", "FAILURE"),
                    slug_data::TestStatus::Fatal => ("fail", "FAILURE"),
                    slug_data::TestStatus::Timeout => ("fail", "FAILURE"),
                    slug_data::TestStatus::InfraFailure => ("fail", "FAILURE"),
                    slug_data::TestStatus::Skip => ("skip", "EXCLUDED"),
                    slug_data::TestStatus::Omitted => ("skip", "EXCLUDED"),
                    _ => ("fail", "FAILURE"),
                };
                let name = result
                    .name
...
Example: --test_tag_filters=small,-slow (include 'small', exclude 'slow')"
    ...
    ///
    /// Examples:
    ///   slug test //foo:bar --test_filter=MyClass.TestMethod
    ///   slug test //foo:bar --test_filter=test_
    ...
    ///
    /// slug test //foo:bar -- --env PRIVATE_KEY=123
    #[clap(name = "TEST_EXECUTOR_ARGS", raw = true)]
    test_executor_args: Vec<String>,

    /// Also build DefaultInfo provider, which is what `slug build` command builds (this is not the default)
    ...
    /// Also build RunInfo provider, which builds artifacts needed for `slug run` (this is not the default)
...
impl StreamingCommand for TestCommand {
    ...
    async fn exec_impl(
        ...
        if let Some(build_report) = response.serialized_build_report {
            slug_client_ctx::println!("{}", build_report)?;
        }
