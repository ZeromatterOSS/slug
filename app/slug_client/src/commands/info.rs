...
//! `slug info` - Bazel-compatible info command.
//!
...
/// Known info keys supported by `slug info`.
const INFO_KEYS: &[(&str, &str)] = &[
    ...
    ("server_pid", "PID of the running slug daemon"),
    ("server_log", "Path to the daemon log file"),
    ("release", "Version info for this Slug release"),
...
\n\
Usage:\n  slug info            # print all keys\n  slug info workspace  # print just workspace path\n\
\n\
...
impl InfoCommand {
    pub fn exec(
        ...
        let resolve_key = |key: &str| -> slug_error::Result<String> {
            match key {
