...
use chrono::TimeZone;
use chrono::Utc;
use slug_cli_proto::CleanStaleRequest;
use slug_cli_proto::CleanStaleResponse;
...
) -> slug_error::Result<Option<KeepSinceArg>> {
    let arg = match (stale, keep_since_time) {
        (Some(Some(human_duration)), None) => {
            let duration = chrono::Duration::from_std(human_duration.into())
                .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::InvalidDuration))?;
...
fn format_result_stats(stats: slug_data::CleanStaleStats) -> String {
...
impl StreamingCommand for CleanStaleCommand {
    ...
    async fn exec_impl(
                    ...
                    humantime::format_duration(
                        duration
                            ...
                            .map_err(|e| from_any_with_tag(
                                e,
                                slug_error::ErrorTag::InvalidDuration
                            ))
        ...
        if let Some(message) = response.message {
            slug_client_ctx::eprintln!("{}", message)?;
        }
        if let Some(stats) = response.stats {
            slug_client_ctx::eprintln!("{}", format_result_stats(stats))?;
        }
        ExitResult::success()
    }
