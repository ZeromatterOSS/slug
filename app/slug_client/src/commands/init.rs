...
/// Initializes a slug project at the provided path.
#[derive(Debug, clap::Parser)]
#[clap(name = "init", about = "Initialize a slug project")]
    ...
    pub fn exec(self, _matches: BuckArgMatches<'_>, ctx: ClientCommandContext<'_>) -> ExitResult {
        ...
        match exec_impl(self, ctx, &console) {
            ...
            Err(e) => {
                ...
                slug_error!(ErrorTag::Tier0, "{:?}", e).into()
            }
        }
    }
...
fn initialize_module_bazel(repo_root: &AbsPath) -> slug_error::Result<()> {
    let mut module = std::fs::File::create(repo_root.join("MODULE.bazel"))?;
    writeln!(module, "module(name = \"root\")")?;
    Ok(())
}

fn initialize_bazelignore(repo_root: &AbsPath, git: bool) -> slug_error::Result<()> {
    if !git {
        return Ok(());
    }
...
fn initialize_root_build(repo_root: &AbsPath, prelude: bool) -> slug_error::Result<()> {
    let mut build = std::fs::File::create(repo_root.join("BUILD.bazel"))?;

    if prelude {
        writeln!(build, "# Slug build file - compatible with Bazel syntax")?;
        ...
        writeln!(build, "    cmd = \"echo 'Built by Slug!' > $@\",")?;
        writeln!(build, ")")?;
    }
    Ok(())
}

fn set_up_gitignore(repo_root: &AbsPath) -> slug_error::Result<()> {
...
fn set_up_buckroot(repo_root: &AbsPath) -> slug_error::Result<()> {
    fs_util::write(repo_root.join(".buckroot"), "")?;
    Ok(())
}

fn set_up_project(repo_root: &AbsPath, git: bool, prelude: bool) -> slug_error::Result<()> {
    ...
    // already initialized it (manually or via a previous `slug init`).
...
mod tests {
    ...
    #[test]
    fn test_set_up_project_with_prelude_no_git() -> slug_error::Result<()> {
    ...
    #[test]
    fn test_default_gitignore() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let tempdir_path = tempdir.path();
        let tempdir_path = AbsPath::new(tempdir_path)?;
        fs_util::create_dir_all(tempdir_path)?;
    ...
    #[test]
    fn test_bazelignore_generated_with_git() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let tempdir_path = tempdir.path();
        let tempdir_path = AbsPath::new(tempdir_path)?;
        fs_util::create_dir_all(tempdir_path)?;
    ...
    #[test]
    fn test_bazelignore_skipped_without_git() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let tempdir_path = tempdir.path();
        let tempdir_path = AbsPath::new(tempdir_path)?;
        fs_util::create_dir_all(tempdir_path)?;
    ...
    #[test]
    fn test_buildfile_generation_with_prelude() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let tempdir_path = tempdir.path();
        let tempdir_path = AbsPath::new(tempdir_path)?;
        fs_util::create_dir_all(tempdir_path)?;
        ...
        let expected_build = "# Slug build file - compatible with Bazel syntax\n# See: https://bazel.build/concepts/build-files\n\ngenrule(\n    name = \"hello_world\",\n    outs = [\"out.txt\"],\n    cmd = \"echo 'Built by Slug!' > $@\",\n)\n";
