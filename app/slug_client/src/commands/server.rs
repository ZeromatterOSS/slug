...
#[clap(
    ...
    long_about = "Start, query, and control the slug server, a long-lived process, spanning slug command line invocations.
Using this command can ensure the daemon is running.

To stop a specific server, use `slug kill` and add `--isolation-dir` for a specific instance.
To stop all instances, use `slug killall`."
)]
...
impl StreamingCommand for ServerCommand {
    ...
    async fn exec_impl(
        ...
        let status = buckd
            ...
            .await?;
        slug_client_ctx::println!("buckd.endpoint={}", status.process_info.unwrap().endpoint)?;
        ExitResult::success()
    }
