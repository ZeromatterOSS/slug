...
use slug_core::env::registry::Applicability;
use slug_core::env::registry::ENV_INFO;
use slug_core::env::registry::EnvInfoEntry;
use slug_error::BuckErrorContext;

/// Print help for environment variables used by slug.
#[derive(Debug, clap::Parser)]
pub struct HelpEnvCommand {
    /// Also print those environment variables that are only used for slug integration tests.
    ///
    ...
    pub fn exec(self, _matches: BuckArgMatches<'_>, _ctx: ClientCommandContext<'_>) -> ExitResult {
        ...
        let mut env_info: Vec<EnvInfoEntry> = ENV_INFO
            ...
            .filter(|x| match x.applicability {
                ...
                Applicability::Internal => !slug_core::is_open_source(),
            })
            .collect();
        env_info.sort();
        env_info.dedup();
        ...
        for (name, ty, default) in rows {
            ...
            slug_client_ctx::println!("{}", line.trim_end())?;
        }
        ExitResult::success()
    }
}
