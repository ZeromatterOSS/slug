...
//! `slug fetch` - Bazel-compatible fetch command.
//!
//! In Bazel, `fetch` downloads external dependencies. In Slug, external
...
In Slug, external dependencies are resolved lazily, so this command\n\
is a no-op that exists for Bazel script compatibility.\n\
Use `slug build` to trigger actual dependency resolution."
)]
...
impl FetchCommand {
    pub fn exec(
        ...
        // Slug resolves external deps lazily during build.
        // This command exists for Bazel script compatibility.
        slug_client_ctx::eprintln!(
            "INFO: External dependencies are resolved lazily in Slug. No fetch needed."
        )?;
        Ok(())
    }
