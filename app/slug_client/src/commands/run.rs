...
pub struct RunCommand {
    ...
    /// formatted for shell interpolation, use as: $(slug run --emit-shell ...)
...
impl StreamingCommand for RunCommand {
    ...
    async fn exec_impl(
        ...
        // Slug. We're not a wrapper script, so we unset it to prevent `run` from inheriting it.
...
pub enum RunCommandError {
    ...
    #[error("`slug run` only supports a single target, but multiple targets were requested.")]
    MultipleTargets,
