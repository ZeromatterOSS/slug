...
use slug_cli_proto::StatusResponse;
use slug_client_ctx::client_ctx::ClientCommandContext;
use slug_client_ctx::common::BuckArgMatches;
use slug_client_ctx::daemon::client::connect::BuckdConnectConstraints;
use slug_client_ctx::daemon::client::connect::connect_buckd;
use slug_client_ctx::daemon::client::connect::establish_connection_existing;
use slug_client_ctx::events_ctx::EventsCtx;
use slug_client_ctx::subscribers::stdout_stderr_forwarder::StdoutStderrForwarder;
    ...
    pub fn exec(
        ...
        ctx.with_runtime(|ctx| async move {
            ...
            if self.all {
                ...
                for entry in walker {
                    let entry =
                        entry.map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::Tier0))?;
                ...
                slug_client_ctx::println!("{}", serde_json::to_string_pretty(&statuses)?)?;
            } else {
                match connect_buckd(
                ...
                {
                    Err(_) => {
                        slug_client_ctx::eprintln!("no buckd running")?;
...
fn timestamp_to_string(seconds: u64, nanos: u32) -> slug_error::Result<String> {
...
fn process_status(status: StatusResponse) -> slug_error::Result<serde_json::Value> {
