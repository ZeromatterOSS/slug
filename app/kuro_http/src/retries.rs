...
where
    Exec: Fn() -> F,
    E: AsKuroError + HttpErrorForRetry + std::fmt::Display + Send + Sync + 'static,
        ...
        if err.is_retryable() {
            if let Some(b) = backoff.peek() {
                ...
                tracing::warn!(
                    ...
                    b.as_secs(),
                    // Print as a kuro_error to make sure we get the source
                    err.as_kuro_error()
                );
                continue;
            }
...
mod tests {
    ...
    impl AsKuroError for HttpTestError {
        fn as_kuro_error(self) -> kuro_error::Error {
    ...
    fn ok_response() -> Result<String, HttpTestError> {
        Ok("Success".to_owned()).map_err(|_: kuro_error::Error| test_error(StatusCode::IM_A_TEAPOT))
    }
