...
impl LocalResourcesSetupResult {
    pub(crate) fn into_state(
        ...
        ) -> slug_error::Result<LocalResource> {
            let env_vars = env_var_to_alias
                .iter()
                .map(|(env_var, alias)| {
                    let value = alias_to_value.get(alias).ok_or_else(|| slug_error::slug_error!(
                ...
                .collect::<Result<_, slug_error::Error>>()?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_into_state() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_missing_value() -> slug_error::Result<()> {
        let setup_result = LocalResourcesSetupResult {
            pid: Some(42),
            resources: vec![
