...
//! Implementation of the `TestOrchestrator` from `slug_test_api`.
...
use slug_build_api::actions::execute::dice_data::CommandExecutorResponse;
use slug_build_api::actions::execute::dice_data::DiceHasCommandExecutor;
use slug_build_api::actions::execute::dice_data::GetReClient;
...
use slug_build_api::artifact_groups::ArtifactGroupValues;
use slug_build_api::artifact_groups::calculation::ArtifactGroupCalculation;
use slug_build_api::build_signals::HasBuildSignals;
use slug_build_api::context::HasBuildContextData;
use slug_build_api::interpreter::rule_defs::cmd_args::AbsCommandLineContext;
use slug_build_api::interpreter::rule_defs::cmd_args::ArtifactPathMapperImpl;
...
use slug_build_api::interpreter::rule_defs::cmd_args::CommandLineBuilder;
use slug_build_api::interpreter::rule_defs::cmd_args::CommandLineContext;
...
use slug_build_api::interpreter::rule_defs::cmd_args::space_separated::SpaceSeparatedCommandLineBuilder;
use slug_build_api::interpreter::rule_defs::provider::builtin::external_runner_test_info::FrozenExternalRunnerTestInfo;
use slug_build_api::interpreter::rule_defs::provider::builtin::external_runner_test_info::TestCommandMember;
use slug_build_api::interpreter::rule_defs::provider::builtin::local_resource_info::FrozenLocalResourceInfo;
use slug_build_api::keep_going::KeepGoing;
...
use slug_core::execution_types::executor_config::RemoteExecutorCustomImage;
use slug_core::fs::artifact_path_resolver::ArtifactFs;
use slug_core::fs::buck_out_path::BuckOutTestPath;
...
use slug_data::EndOfTestResults;
use slug_data::SetupLocalResourcesEnd;
use slug_data::SetupLocalResourcesStart;
use slug_data::TestDiscovery;
use slug_data::TestDiscoveryEnd;
use slug_data::TestDiscoveryStart;
use slug_data::TestRunEnd;
use slug_data::TestRunStart;
use slug_data::TestSessionInfo;
use slug_data::TestSuite;
...
use slug_execute::execute::request::OutputCreationBehavior;
use slug_execute::execute::request::WorkerId;
use slug_execute::execute::request::WorkerSpec;
use slug_execute::execute::result::CommandCancellationReason;
...
use slug_execute::execute::target::CommandExecutionTarget;
use slug_execute::materialize::materializer::HasMaterializer;
use slug_execute_impl::executors::local::EnvironmentBuilder;
use slug_execute_impl::executors::local::apply_local_execution_environment;
use slug_execute_impl::executors::local::create_output_dirs;
use slug_execute_impl::executors::local::materialize_inputs;
use slug_execute_impl::executors::local::prep_scratch_path;
...
use slug_test_api::data::ConfiguredTargetHandle;
use slug_test_api::data::DeclaredOutput;
use slug_test_api::data::ExecuteResponse;
use slug_test_api::data::ExecutionDetails;
use slug_test_api::data::ExecutionResult2;
use slug_test_api::data::ExecutionStatus;
use slug_test_api::data::ExecutionStream;
use slug_test_api::data::ExecutorConfigOverride;
use slug_test_api::data::ExternalRunnerSpecValue;
use slug_test_api::data::LocalExecutionCommand;
use slug_test_api::data::Output;
use slug_test_api::data::PrepareForLocalExecutionResult;
use slug_test_api::data::RequiredLocalResources;
use slug_test_api::data::TestResult;
use slug_test_api::data::TestStage;
use slug_test_api::data::convert::host_sharing_requirements_to_grpc;
use slug_test_api::protocol::TestOrchestrator;
...
pub struct BuckTestOrchestrator<'a: 'static> {
    ...
    results_channel: UnboundedSender<slug_error::Result<ExecutorMessage>>,
...
impl<'a> BuckTestOrchestrator<'a> {
    pub async fn new(
        ...
        results_channel: UnboundedSender<slug_error::Result<ExecutorMessage>>,
        cancellations: &'a CancellationContext,
    ) -> slug_error::Result<BuckTestOrchestrator<'a>> {
...
impl TestOrchestrator for BuckTestOrchestrator<'_> {
    async fn execute2(
        ...
        executor_override: Option<ExecutorConfigOverride>,
        required_local_resources: RequiredLocalResources,
    ) -> slug_error::Result<ExecuteResponse> {
            ...
            Err(ExecuteError::Cancelled(cancelled)) => {
                Ok(ExecuteResponse::Cancelled(match cancelled.reason {
                    Some(CancellationReason::NotSpecified) => {
                        Some(slug_test_api::data::CancellationReason::NotSpecified)
                    }
                    Some(CancellationReason::ReQueueTimeout) => {
                        Some(slug_test_api::data::CancellationReason::ReQueueTimeout)
                    }
                    None => None,
                }))
            }
    ...
    async fn report_test_result(&self, r: TestResult) -> slug_error::Result<()> {
        let event = slug_data::instant_event::Data::TestResult(translations::convert_test_result(
            r.clone(),
            &self.session,
        )?);
        ...
        self.results_channel
            ...
            .map_err(|_| {
                slug_error::internal_error!("Test result was received after end-of-tests")
            })?;
        Ok(())
    }
    ...
    async fn report_tests_discovered(
        ...
        self.events.instant_event(TestDiscovery {
            data: Some(slug_data::test_discovery::Data::Tests(TestSuite {
                suite_name: suite,
                test_names: names,
    ...
    async fn report_test_session(&self, session_info: String) -> slug_error::Result<()> {
        self.events.instant_event(TestDiscovery {
            data: Some(slug_data::test_discovery::Data::Session(TestSessionInfo {
                info: session_info,
            })),
        });
    ...
    async fn end_of_test_results(&self, exit_code: i32) -> slug_error::Result<()> {
        self.events.instant_event(EndOfTestResults { exit_code });
        self.results_channel
            .unbounded_send(Ok(ExecutorMessage::ExitCode(exit_code)))
            .map_err(|_| slug_error::internal_error!("end_of_tests was received twice"))?;
    ...
    async fn prepare_for_local_execution(
        ...
        pre_create_dirs: Vec<DeclaredOutput>,
        required_local_resources: RequiredLocalResources,
    ) -> slug_error::Result<PrepareForLocalExecutionResult> {
        let test_target = self.session.get(test_target)?;
    ...
    async fn attach_info_message(&self, message: String) -> slug_error::Result<()> {
        self.results_channel
            .unbounded_send(Ok(ExecutorMessage::InfoMessage(message)))
            .map_err(|_| slug_error::internal_error!("Message received after end-of-tests"))?;
        Ok(())
    }
}
...
impl BuckTestOrchestrator<'_> {
    ...
    ) -> slug_error::Result<ExecutorPreference> {
    ...
    fn executor_config_with_remote_cache_override<'a>(
        ...
        stage: &TestStage,
    ) -> slug_error::Result<Cow<'a, CommandExecutorConfig>> {
    ...
    async fn get_command_executor(
        ...
        stage: &TestStage,
    ) -> slug_error::Result<CommandExecutor> {
    ...
    async fn get_test_info(
        dice: &mut DiceComputations<'_>,
        test_target: &ConfiguredProvidersLabel,
    ) -> slug_error::Result<OwnedFrozenValueTyped<FrozenExternalRunnerTestInfo>> {
    ...
    async fn get_test_executor(
        ...
        fs: &ArtifactFs,
        stage: &TestStage,
    ) -> slug_error::Result<TestExecutor> {
    ...
    async fn expand_test_executable<'a>(
        ...
        prefix: TestExecutionPrefix,
        opts: TestSessionOptions,
    ) -> slug_error::Result<ExpandedTestExecutable> {
        ...
        let (expanded_cmd, expanded_env, ensured_inputs, expanded_worker) = {
            ...
            let ensured_inputs = KeepGoing::try_compute_join_all(dice, inputs, |dice, input| {
                async move {
                    ...
                    slug_error::Ok((input, artifact_group_value))
                }
                .boxed()
            })
            .await?;
    ...
    async fn setup_local_resources(
        ...
        let result: slug_error::Result<Vec<_>> = required_targets
            .iter()
            .map(|t| lock.get(t).unwrap().clone())
            .collect();
        Ok(result.map_err(slug_error::Error::from)?)
    }
    ...
    async fn prepare_local_resource(
        ...
        executor_fs: &ExecutorFs<'_>,
        default_timeout: Duration,
    ) -> slug_error::Result<PreparedLocalResourceSetupContext> {
        let digest_config = dice.global_data().get_digest_config();
    ...
    async fn start_local_resource(
        ...
        cancellation: &CancellationContext,
    ) -> slug_error::Result<LocalResourceState> {
        let manager = CommandExecutionManager::new(
            Box::new(MutexClaimManager::new()),
            events.dupe(),
            liveliness_observer,
...
impl Drop for BuckTestOrchestrator<'_> {
    fn drop(&mut self) {
        ...
        let _ignored = self
            .results_channel
            .unbounded_send(Err(slug_error::internal_error!(
...
) -> impl for<'a> Fn(&'a mut dyn CommandLineArtifactVisitor<'v>, &'a ArgValue) -> slug_error::Result<()>
{
...
impl<'a> Execute2RequestExpander<'a> {
    fn get_inputs(&self) -> slug_error::Result<IndexSet<ArtifactGroup>> {
        ...
        B: CommandLineContextExt<'a>,
    {
        ...
        let expanded_worker = match test_info.worker() {
            Some(worker) => {
                ...
                let worker_env: slug_error::Result<SortedVectorMap<_, _>> = worker
                    .env()
                    .into_iter()
                    .map(|(k, v)| {
...
async fn resolve_output_root(
    ...
    test_target: &ConfiguredProvidersLabel,
    prefix: TestExecutionPrefix,
) -> Result<ForwardRelativePathBuf, slug_error::Error> {
...
impl CommandExecutionTarget for TestTarget<'_> {
    ...
    fn as_proto_action_key(&self) -> slug_data::ActionKey {
        slug_data::ActionKey {
            id: Default::default(),
            owner: Some(slug_data::action_key::Owner::TestTargetLabel(
    ...
    fn as_proto_action_name(&self) -> slug_data::ActionName {
        slug_data::ActionName {
...
impl CommandExecutionTarget for LocalResourceTarget<'_> {
    ...
    fn as_proto_action_key(&self) -> slug_data::ActionKey {
        slug_data::ActionKey {
            id: Default::default(),
            owner: Some(slug_data::action_key::Owner::LocalResourceSetup(
                self.target.as_proto(),
            )),
            key: Default::default(),
        }
...
mod tests {
    ...
    async fn make() -> slug_error::Result<(
        BuckTestOrchestrator<'static>,
        UnboundedReceiver<slug_error::Result<ExecutorMessage>>,
    )> {
    ...
    #[tokio::test]
    async fn orchestrator_results() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn orchestrator_attach_info_messages() -> slug_error::Result<()> {
        let (orchestrator, channel) = make().await?;
    ...
    #[tokio::test]
    async fn test_orchestrator_channel_drop() -> slug_error::Result<()> {
        let (orchestrator, channel) = make().await?;
        drop(orchestrator);
    ...
    #[tokio::test]
    async fn test_orchestrator_closes_channel() -> slug_error::Result<()> {
        let (orchestrator, channel) = make().await?;
