...
use dupe::Dupe;
use slug_common::kill_util::try_terminate_process_gracefully;
...
use slug_data::ReleaseLocalResourcesEnd;
use slug_data::ReleaseLocalResourcesStart;
...
pub struct LocalResourceRegistry(
    pub Arc<Mutex<HashMap<ConfiguredTargetLabel, slug_error::Result<LocalResourceState>>>>,
);

impl LocalResourceRegistry {
    pub(crate) async fn release_all_resources(&self) -> slug_error::Result<()> {
        let resources = {
...
pub trait HasLocalResourceRegistry {
    fn get_local_resource_registry(&self) -> slug_error::Result<Arc<LocalResourceRegistry>>;
}
