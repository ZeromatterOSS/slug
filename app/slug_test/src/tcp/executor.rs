...
/// Environment variable used to pass the actual username from Slug client to the test executor.
/// This is necessary because in some scenarios slugd may run as a different user than the user who invoked `slug test`.
...
pub(crate) async fn spawn(
    ...
    args: Vec<String>,
    tpx_args: Vec<String>,
) -> slug_error::Result<(ExecutorFuture, TcpStream, TcpStream)> {
    ...
    // Pass the actual username from Slug client to the executor.
    ...
    let conns = async {
        ...
        slug_error::Ok((orchestrator_tcp_stream, executor_tcp_stream))
    };
    ...
    match futures::future::select(exec, conns).await {
        Either::Left((output, _)) => Err(slug_error::internal_error!(
...
async fn create_tcp_listener() -> slug_error::Result<(String, TcpListener)> {
