...
use slug_test_api::grpc::TestExecutorClient;
use slug_test_api::grpc::spawn_orchestrator_server;
...
pub struct ExecutorFuture {
    fut: BoxFuture<'static, slug_error::Result<ExecutorOutput>>,
}
...
impl Future for ExecutorFuture {
    type Output = slug_error::Result<ExecutorOutput>;
...
pub trait ExecutorLauncher: Send + Sync {
    async fn launch(&self, tpx_args: Vec<String>) -> slug_error::Result<ExecutorLaunch>;
}
...
impl ExecutorLauncher for OutOfProcessTestExecutor {
    async fn launch(&self, tpx_args: Vec<String>) -> slug_error::Result<ExecutorLaunch> {
        // Declare outside of `cfg(unix)` so `slug help-env` would include it on Windows
        // even if it is no-op on Windows.
        let use_tcp = slug_env!("BUCK2_TEST_TPX_USE_TCP", bool)?;
...
) -> slug_error::Result<ExecutorLaunch> {
