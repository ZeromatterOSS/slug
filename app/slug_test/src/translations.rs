...
use slug_test_api::data::ConfiguredTarget;
use slug_test_api::data::RemoteObject;
...
) -> slug_error::Result<ConfiguredTarget> {
...
pub(crate) fn convert_test_result(
    test_result: slug_test_api::data::TestResult,
    session: &TestSession,
) -> slug_error::Result<slug_data::TestResult> {
    let slug_test_api::data::TestResult {
        name,
        status,
        msg,
        duration,
    ...
    Ok(slug_data::TestResult {
        name,
        status: status.try_into().buck_error_context("Invalid `status`")?,
        msg: msg.map(|msg| slug_data::test_result::OptionalMsg { msg }),
...
fn convert_digest(digest: &FileDigest) -> slug_test_api::data::CasDigest {
    let hash = format!("{}", digest.raw_digest());
    slug_test_api::data::CasDigest {
        hash,
