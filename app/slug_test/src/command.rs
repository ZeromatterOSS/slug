...
use slug_build_api::actions::calculation::get_target_rule_type_name;
use slug_build_api::analysis::calculation::RuleAnalysisCalculation;
use slug_build_api::build::AsyncBuildTargetResultBuilder;
use slug_build_api::build::BuildConfiguredLabelOptions;
use slug_build_api::build::BuildEvent;
use slug_build_api::build::BuildEventConsumer;
use slug_build_api::build::BuildTargetResult;
use slug_build_api::build::BuildTargetResultBuilder;
use slug_build_api::build::ConfiguredBuildEventVariant;
use slug_build_api::build::ProvidersToBuild;
use slug_build_api::build::build_configured_label;
use slug_build_api::build::build_report::build_report_opts;
use slug_build_api::build::build_report::write_build_report;
use slug_build_api::interpreter::rule_defs::provider::builtin::run_info::FrozenRunInfo;
use slug_build_api::interpreter::rule_defs::provider::collection::FrozenProviderCollectionValue;
use slug_build_api::interpreter::rule_defs::provider::test_provider::TestProvider;
use slug_build_api::materialize::MaterializationAndUploadContext;
use slug_cli_proto::HasClientContext;
use slug_cli_proto::TestRequest;
use slug_cli_proto::TestResponse;
use slug_cli_proto::representative_config_flag;
use slug_common::dice::cells::HasCellResolver;
use slug_common::events::HasEvents;
use slug_common::liveliness_observer::LivelinessGuard;
use slug_common::liveliness_observer::LivelinessObserver;
use slug_common::liveliness_observer::LivelinessObserverExt;
use slug_common::liveliness_observer::TimeoutLivelinessObserver;
use slug_common::pattern::parse_from_cli::parse_patterns_with_modifiers_from_cli_args;
...
use slug_core::package::PackageLabelWithModifiers;
use slug_core::pattern::pattern::Modifiers;
...
use slug_data::BuildResult;
use slug_error::BuckErrorContext;
use slug_error::ErrorTag;
use slug_events::dispatch::console_message;
...
use slug_interpreter_for_build::interpreter::context::HasInterpreterContext;
...
use slug_server_ctx::commands::send_target_cfg_event;
...
use slug_server_ctx::test_command::TEST_COMMAND;
use slug_server_ctx::tpx_experiment_util::get_tpx_experiments;
...
struct TestOutcome {
    errors: Vec<slug_data::ErrorReport>,
...
impl TestOutcome {
    fn exit_code(&self) -> slug_error::Result<i32> {
...
impl CounterWithExamples {
    ...
    fn to_cli_proto_counter(self) -> slug_cli_proto::CounterWithExamples {
        slug_cli_proto::CounterWithExamples {
            count: self.count,
            max: self.max,
...
#[derive(Debug, slug_error_derive::Error)]
#[slug(tag = TestDeadlineExpired)]
...
async fn test_command(
    ...
    req: TestRequest,
) -> slug_error::Result<TestResponse> {
...
struct TestServerCommand {
    req: slug_cli_proto::TestRequest,
}

#[async_trait]
impl ServerCommandTemplate for TestServerCommand {
    type StartEvent = slug_data::TestCommandStart;
    type EndEvent = slug_data::TestCommandEnd;
    type Response = slug_cli_proto::TestResponse;
    type PartialResult = NoPartialResult;

    fn build_result(&self, response: &Self::Response) -> Option<BuildResult> {
        let build_completed =
            if let Some(slug_cli_proto::test_response::TestStatuses { build_errors, .. }) =
    ...
    fn end_event(&self, _response: &slug_error::Result<Self::Response>) -> Self::EndEvent {
        slug_data::TestCommandEnd {
            unresolved_target_patterns: self
                .req
                .target_patterns
                .iter()
                .map(|p| slug_data::TargetPattern { value: p.clone() })
                .collect(),
        }
    }
...
fn test_executor_errors(
    ...
    test_statuses: &slug_cli_proto::test_response::TestStatuses,
) -> Vec<slug_data::ErrorReport> {
    // FIXME: These errors should be derived from exit code only
    let mut errors = Vec::new();
    if let Some(failed) = &test_statuses.failed {
        if failed.count > 0 {
            errors.push(slug_data::ErrorReport::from(&TestError::TestFailed.into()));
        }
    }
    if let Some(infra_failure) = &test_statuses.infra_failure {
        if infra_failure.count > 0 {
            errors.push(slug_data::ErrorReport::from(
    ...
    if let Some(fatal) = &test_statuses.fatals {
        if fatal.count > 0 {
            errors.push(slug_data::ErrorReport::from(&TestError::Fatal.into()));
        }
    }
    ...
    if let Some(skipped) = &test_statuses.skipped {
        if skipped.count > 0 {
            errors.push(slug_data::ErrorReport::from(&TestError::TestSkipped.into()));
        }
    }
    if let Some(omitted) = &test_statuses.omitted {
        if omitted.count > 0 {
            errors.push(slug_data::ErrorReport::from(&TestError::TestOmitted.into()));
        }
    }
    if errors.is_empty() {
        errors.push(slug_data::ErrorReport::from(&slug_error::slug_error!(
            slug_error::ErrorTag::TestExecutor,
...
async fn test(
    ...
    let test_statuses = slug_cli_proto::test_response::TestStatuses {
        passed: Some(
            test_outcome
                .executor_report
                .statuses
...
async fn test_targets(
    ...
    let test_server = tokio::spawn({
        ...
        with_dispatcher_async(
            ...
            async move {
                ...
                slug_error::Ok((driver.build_target_result, test_statuses))
            },
        )
    });
    ...
    let _ignored = test_status_sender.unbounded_send(Err(slug_error::slug_error!(
        ErrorTag::TestExecutor,
    ...
    let mut errors = convert_error(&build_target_result)
        .iter()
        .map(slug_data::ErrorReport::from)
        .unique_by(|e| e.message.clone())
        .collect::<Vec<_>>();

    if let Some(timeout_observer) = timeout_observer {
        if !timeout_observer.is_alive().await {
            errors.push(slug_data::ErrorReport::from(&DeadlineExpired.into()));
        }
    }
...
async fn build_target_result(
    ...
    build_default_info: bool,
    build_run_info: bool,
) -> slug_error::Result<(BuildTargetResult, FrozenProviderCollectionValue)> {
...
async fn test_target(
    ...
    test_config_unification_rollout: bool,
    oncall: Option<String>,
) -> slug_error::Result<Option<ConfiguredProvidersLabel>> {
...
fn convert_error(build_result: &BuildTargetResult) -> Vec<slug_error::Error> {
    let mut errors = Vec::new();
...
fn run_tests<'a, 'b>(
    ...
    test_config_unification_rollout: bool,
    oncall: Option<String>,
) -> BoxFuture<'a, slug_error::Result<ConfiguredProvidersLabel>> {
...
fn generate_config_entry_args(
    ...
    representative_config_flags: &[slug_cli_proto::RepresentativeConfigFlag],
) {
...
fn post_process_test_executor(s: &str) -> slug_error::Result<PathBuf> {
    match s.split_once("$BUCK2_BINARY_DIR/") {
        Some(("", rest)) => {
            let exe = AbsPathBuf::new(
                std::env::current_exe().buck_error_context("Cannot get Slug executable")?,
            )?;
            let exe = fs_util::canonicalize(&exe).buck_error_context(
                "Failed to canonicalize path to Slug executable. Try running `slug kill`.",
            )?;

            let exe = exe.as_abs_path();
            let exe_dir = exe
                .parent()
                .buck_error_context("Slug executable directory has no parent")?;

            Ok(exe_dir.join(rest).to_path_buf())
        }
        Some(..) => Err(slug_error::slug_error!(
...
#[cfg(test)]
mod tests {
    use slug_cli_proto::RepresentativeConfigFlag;
    use slug_cli_proto::representative_config_flag;
