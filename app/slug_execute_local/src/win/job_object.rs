...
impl JobObject {
    pub(crate) fn new() -> slug_error::Result<Self> {
    ...
    pub(crate) fn assign_process(&self, process: HANDLE) -> slug_error::Result<()> {
        result_bool(unsafe { jobapi2::AssignProcessToJobObject(self.job_handle.handle(), process) })
    }

    pub(crate) async fn terminate(&self, exit_code: u32) -> slug_error::Result<()> {
    ...
    async fn wait(&self) -> slug_error::Result<()> {
...
fn set_job_limits(job: &WinapiHandle) -> slug_error::Result<()> {
