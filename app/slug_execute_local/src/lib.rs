...
pub struct CollectedExecutionStats {
    ...
    pub userspace_events: Option<slug_data::CpuCounter>,
    pub kernel_events: Option<slug_data::CpuCounter>,
}
...
struct CommandEventStream<Status, Stdio> {
    exit: Option<slug_error::Result<GatherOutputStatus>>,
...
where
    Status: Future<Output = slug_error::Result<GatherOutputStatus>>,
    Stdio: Stream<Item = slug_error::Result<StdioEvent>> + InterruptNotifiable,
{
    type Item = slug_error::Result<CommandEvent>;
...
) -> slug_error::Result<GatherOutputStatus> {
    match timeout {
        Some(t) => {
...
) -> slug_error::Result<impl Stream<Item = slug_error::Result<CommandEvent>>>
where
    T: Future<Output = slug_error::Result<GatherOutputStatus>> + Send + Unpin,
{
    ...
    let process_details = if retry_on_txt_busy {
        ...
        cmd.spawn().map_err(|x| slug_error::Error::from(x.0))
    };
    ...
    let status = async move {
        ...
        let execute = async {
            ...
            slug_error::Ok(match futures::future::select(status, cancellation).await {
...
pub async fn decode_command_event_stream<S>(stream: S) -> slug_error::Result<CommandResult>
where
...
pub(crate) trait KillProcess {
    async fn kill(self, process: &mut ProcessGroup) -> slug_error::Result<()>;
}
...
impl KillProcess for DefaultKillProcess {
    async fn kill(self, process_group: &mut ProcessGroup) -> slug_error::Result<()> {
...
async fn spawn_retry_txt_busy<F, D>(
    ...
    mut delay: F,
) -> slug_error::Result<ProcessGroup>
where
    F: FnMut() -> D,
    D: Future<Output = ()>,
{
...
mod tests {
    ...
    #[tokio::test]
    async fn test_gather_output() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_gather_does_not_wait_for_children() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_gather_output_timeout() -> slug_error::Result<()> {
        let now = Instant::now();
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_spawn_retry_txt_busy() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_spawn_retry_other_error() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
    ...
    #[tokio::test]
    async fn test_kill_terminates_process_group() -> slug_error::Result<()> {
        use sysinfo::Pid;
        use sysinfo::System;
    ...
    #[tokio::test]
    async fn test_stream_command_events_ends() -> slug_error::Result<()> {
        let mut cmd = if cfg!(windows) {
            background_command("powershell")
        } else {
            background_command("sh")
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_signal_exit_code() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn timeout_kills_before_dropping_decoder() -> slug_error::Result<()> {
        struct Kill {
        ...
        impl StatusDecoder for Decoder {
            async fn decode_status(self, _status: ExitStatus) -> slug_error::Result<DecodedStatus> {
                panic!("Should not be called in this test since we timeout")
            }

            async fn cancel(self) -> slug_error::Result<()> {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_no_stdio_stream_command_events() -> slug_error::Result<()> {
        let mut cmd = background_command("sh");
        cmd.args(["-c", "echo hello"]);
