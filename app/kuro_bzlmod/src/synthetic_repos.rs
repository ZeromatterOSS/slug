...
fn generate_crate_universe_repos(
    ...
    // Instead of stubs, materialize actual crate sources from the Cargo registry cache
    // at bazel-external/{spoke_name}/ so they can be compiled.
    if let Some(data) = &crate_data {
        for ext_crate in &data.external_crates {
            ...
            // Try to materialize from Cargo registry cache on disk
            if let Some(root) = project_root {
                let dest = root.join("bazel-external").join(&spoke_name);
                if !dest.exists() {
                    if let Some(src) = find_cargo_registry_source(&ext_crate.name, &ext_crate.version, &ext_crate.source) {
                        if let Err(e) = materialize_spoke_repo(&dest, &src, &ext_crate.name) {
                            tracing::warn!(
                                "Failed to materialize spoke repo {}: {}",
                                spoke_name, e
                            );
                        }
                    }
            ...
            // Still create a synthetic repo entry with minimal BUILD
            // (acts as fallback and provides the target definition)
            let mut files = HashMap::new();
            let edition = if let Some(root) = project_root {
                let dest = root.join("bazel-external").join(&spoke_name);
                read_crate_edition(&dest)
            } else {
                "2021".to_string()
            };
            let build_content = generate_spoke_build_file_for_real_sources(&ext_crate.name, &edition);
            files.insert("BUILD.bazel".to_owned(), build_content);
            repos.push(SyntheticRepo {
                name: spoke_name,
                files,
...
struct ExternalCrate {
    name: String,
    version: String,
    /// Source URL from Cargo.lock (e.g., "registry+...", "git+https://...#rev")
    source: String,
}
...
/// Generate a BUILD.bazel for a spoke repo that has real crate sources.
fn generate_spoke_build_file_for_real_sources(crate_name: &str, edition: &str) -> String {
    let target_name = crate_name.replace('-', "_");
    format!(
        r#"# Auto-generated by kuro crate universe for: {crate_name}
package(default_visibility = ["//visibility:public"])
...
/// Read the Rust edition from a crate's Cargo.toml. Defaults to "2021".
fn read_crate_edition(crate_dir: &std::path::Path) -> String {
...
/// Find the Cargo source directory for a crate (supports both registry and git).
fn find_cargo_registry_source(crate_name: &str, version: &str, source: &str) -> Option<std::path::PathBuf> {
    let cargo_home = std::env::var("CARGO_HOME")
        .ok()
        .or_else(|| {
            if cfg!(windows) {
                std::env::var("USERPROFILE").ok().map(|h| format!("{}\\.cargo", h))
            } else {
                std::env::var("HOME").ok().map(|h| format!("{}/.cargo", h))
            }
        })?;
    ...
    if source.starts_with("git+") {
        // Format: "git+https://github.com/user/repo.git?rev=abc123#abc123"
        // Cargo stores git checkouts at: ~/.cargo/git/checkouts/<repo-hash>/<short-rev>/
        let git_checkouts = cargo_home.join("git").join("checkouts");
        if !git_checkouts.exists() {
            return None;
        }

        // Extract the revision hash from the source URL (after #)
        let rev = source.rsplit_once('#').map(|(_, r)| r)?;
        let short_rev = &rev[..std::cmp::min(7, rev.len())];

        // Find the checkout directory that contains this crate
        for entry in std::fs::read_dir(&git_checkouts).ok()? {
            let entry = entry.ok()?;
            if !entry.file_type().ok()?.is_dir() {
                continue;
            }
            // Check for the revision subdirectory
            let rev_dir = entry.path().join(short_rev);
            if !rev_dir.exists() {
                continue;
            }
            // Search all subdirectories for a Cargo.toml matching this crate name
            if let Some(found) = find_crate_in_dir(&rev_dir, crate_name) {
                return Some(found);
            }
        }
        return None;
    ...
    // Check the full rev directory too (some repos have the checkout at a longer rev)
    if source.starts_with("git+") {
        let git_checkouts = cargo_home.join("git").join("checkouts");
        if git_checkouts.exists() {
            // Also try with longer rev prefixes
            if let Some(rev) = source.rsplit_once('#').map(|(_, r)| r) {
                for entry in std::fs::read_dir(&git_checkouts).ok()? {
                    let entry = entry.ok()?;
                    if !entry.file_type().ok()?.is_dir() { continue; }
                    // Try full rev hash as directory name
                    let rev_dir = entry.path().join(rev);
                    if rev_dir.exists() {
                        if let Some(found) = find_crate_in_dir(&rev_dir, crate_name) {
                            return Some(found);
                        }
    ...
    for entry in std::fs::read_dir(&registry_src).ok()? {
        let entry = entry.ok()?;
        let crate_dir = entry.path().join(&dir_name);
        if crate_dir.exists() {
...
/// Search a directory tree for a Cargo.toml that declares a crate with the given name.
fn find_crate_in_dir(dir: &std::path::Path, crate_name: &str) -> Option<std::path::PathBuf> {
    // Check root first
    let cargo_toml = dir.join("Cargo.toml");
    if cargo_toml.exists() {
        if let Ok(toml) = std::fs::read_to_string(&cargo_toml) {
            if toml.contains(&format!("name = \"{}\"", crate_name)) {
                return Some(dir.to_path_buf());
            }
        }
    }

    // Search immediate subdirectories (workspace members are typically 1 level deep)
    for entry in std::fs::read_dir(dir).ok()? {
        let entry = entry.ok()?;
        if !entry.file_type().ok()?.is_dir() { continue; }
        let sub_toml = entry.path().join("Cargo.toml");
        if sub_toml.exists() {
            if let Ok(toml) = std::fs::read_to_string(&sub_toml) {
                if toml.contains(&format!("name = \"{}\"", crate_name)) {
...
/// Materialize a spoke repo on disk by copying crate sources from the Cargo registry.
fn materialize_spoke_repo(
    dest: &std::path::Path,
    src: &std::path::Path,
    crate_name: &str,
) -> kuro_error::Result<()> {
    ...
    std::fs::create_dir_all(dest).map_err(|e| {
        kuro_error::kuro_error!(
            ...
            "Failed to create spoke repo dir {:?}: {}",
            dest,
            e
        )
    })?;
    ...
    // Write the BUILD.bazel that defines the rust_library target
    let edition = read_crate_edition(src);
    let build_content = generate_spoke_build_file_for_real_sources(crate_name, &edition);
    std::fs::write(dest.join("BUILD.bazel"), build_content).map_err(|e| {
        kuro_error::kuro_error!(
            kuro_error::ErrorTag::Environment,
            "Failed to write BUILD.bazel for spoke repo {:?}: {}",
            dest,
            e
        )
    })?;
...
fn copy_dir_recursive(src: &std::path::Path, dest: &std::path::Path) -> kuro_error::Result<()> {
