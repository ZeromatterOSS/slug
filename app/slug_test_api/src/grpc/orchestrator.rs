...
use slug_downward_api::DownwardApi;
use slug_downward_api_proto::ConsoleRequest;
use slug_downward_api_proto::ExternalEventRequest;
use slug_downward_api_proto::LogRequest;
use slug_downward_api_proto::downward_api_client;
use slug_downward_api_proto::downward_api_server;
use slug_error::BuckErrorContext;
use slug_events::dispatch::EventDispatcher;
use slug_events::dispatch::with_dispatcher_async;
use slug_grpc::ServerHandle;
use slug_grpc::make_channel;
use slug_grpc::spawn_oneshot;
use slug_grpc::to_tonic;
use slug_test_proto::AttachInfoMessageRequest;
use slug_test_proto::Empty;
use slug_test_proto::EndOfTestResultsRequest;
use slug_test_proto::ExecuteResponse2;
use slug_test_proto::PrepareForLocalExecutionResponse;
use slug_test_proto::ReportTestResultRequest;
use slug_test_proto::ReportTestSessionRequest;
use slug_test_proto::ReportTestsDiscoveredRequest;
use slug_test_proto::Testing;
use slug_test_proto::test_orchestrator_client;
use slug_test_proto::test_orchestrator_server;
...
/// Test runner client to slug test orchestrator.
///
/// When running `slug test`, slug starts a gRPC server
/// and spawns a test runner which connects to slug process using this client.
...
impl TestOrchestratorClient {
    pub async fn new<T>(io: T) -> slug_error::Result<Self>
    where
...
impl DownwardApi for TestOrchestratorClient {
    async fn console(&self, level: Level, message: String) -> slug_error::Result<()> {
    ...
    async fn log(&self, level: Level, message: String) -> slug_error::Result<()> {
        let level = level.try_into().buck_error_context("Invalid `level`")?;
    ...
    async fn external(&self, data: HashMap<String, String>) -> slug_error::Result<()> {
        let event = data.into();
...
impl TestOrchestratorClient {
    pub async fn execute2(
        ...
        let req: slug_test_proto::ExecuteRequest2 = req
            .try_into()
        ...
        let response = match response.buck_error_context("Missing `response`")? {
            slug_test_proto::execute_response2::Response::Result(res) => {
                ExecuteResponse::Result(res.try_into().buck_error_context("Invalid `result`")?)
            }
            slug_test_proto::execute_response2::Response::Cancelled(
                slug_test_proto::Cancelled { reason },
            ) => {
                let reason = match reason {
                    Some(reason) => slug_test_proto::CancellationReason::try_from(reason)
                        .map(|proto_reason| match proto_reason {
                            slug_test_proto::CancellationReason::NotSpecified => {
                                crate::data::CancellationReason::NotSpecified
                            }
                            slug_test_proto::CancellationReason::ReQueueTimeout => {
    ...
    pub async fn report_test_result(&self, result: TestResult) -> slug_error::Result<()> {
    ...
    pub async fn report_test_session(&self, session_info: String) -> slug_error::Result<()> {
        self.test_orchestrator_client
            .clone()
    ...
    pub async fn end_of_test_results(&self, exit_code: i32) -> slug_error::Result<()> {
        self.test_orchestrator_client
            .clone()
    ...
    pub async fn prepare_for_local_execution(
        ...
        let executable: slug_test_proto::TestExecutable = executable
            .try_into()
            .buck_error_context("Invalid prepare_for_local_execution request")?;

        let request = slug_test_proto::PrepareForLocalExecutionRequest {
    ...
    pub async fn attach_info_message(&self, message: String) -> slug_error::Result<()> {
        self.test_orchestrator_client
            .clone()
    ...
    T: TestOrchestrator + Send + Sync + 'static,
{
    async fn execute2(
        &self,
        request: tonic::Request<slug_test_proto::ExecuteRequest2>,
        ...
        to_tonic(async move {
            ...
            let response = match response {
                ExecuteResponse::Result(r) => slug_test_proto::execute_response2::Response::Result(
                    r.try_into()
                ...
                ExecuteResponse::Cancelled(reason) => {
                    let reason = if let Some(reason) = reason {
                        match reason {
                            crate::data::CancellationReason::NotSpecified => {
                                Some(slug_test_proto::CancellationReason::NotSpecified.into())
                            }
                            crate::data::CancellationReason::ReQueueTimeout => {
                                Some(slug_test_proto::CancellationReason::ReQueueTimeout.into())
                            }
                        }
                    } else {
                        None
    ...
    async fn prepare_for_local_execution(
        &self,
        request: tonic::Request<slug_test_proto::PrepareForLocalExecutionRequest>,
    ) -> Result<tonic::Response<PrepareForLocalExecutionResponse>, tonic::Status> {
        to_tonic(async move {
            let slug_test_proto::PrepareForLocalExecutionRequest {
                test_executable,
                required_local_resources,
            } = request.into_inner();
    ...
    T: DownwardApi + Send + Sync + 'static,
{
    ...
    ) -> Result<tonic::Response<slug_downward_api_proto::Empty>, tonic::Status> {
        to_tonic(async move {
            ...
            Ok(slug_downward_api_proto::Empty {})
        })
        .await
    }
