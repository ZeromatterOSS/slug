...
impl InvocationRoots {
    pub fn common_buckd_dir(&self) -> kuro_error::Result<AbsNormPathBuf> {
        Ok(home_buck_dir()?.join(FileName::unchecked_new("buckd")))
    }

    pub fn paranoid_info_path(&self) -> kuro_error::Result<AbsPathBuf> {
        // Used in tests
        if let Some(p) = kuro_env!("BUCK2_PARANOID_PATH")? {
...
fn get_roots(from: &AbsWorkingDir) -> kuro_error::Result<Option<InvocationRoots>> {
...
pub fn find_invocation_roots(from: &AbsWorkingDir) -> kuro_error::Result<InvocationRoots> {
...
pub(crate) fn home_buck_dir() -> kuro_error::Result<&'static AbsNormPath> {
    fn find_dir() -> kuro_error::Result<AbsNormPathBuf> {
        let home =
    ...
    static DIR: Lazy<kuro_error::Result<AbsNormPathBuf>> =
        Lazy::new(|| find_dir().map_err(kuro_error::Error::from));
