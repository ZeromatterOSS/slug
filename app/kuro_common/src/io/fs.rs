...
impl FsIoProvider {
    ...
    ) -> kuro_error::Result<RawPathMetadata<ProjectRelativePathBuf>> {
        let fs = self.fs.dupe();
...
impl IoProvider for FsIoProvider {
    ...
    async fn read_dir_impl(
        &self,
        path: ProjectRelativePathBuf,
    ) -> kuro_error::Result<Vec<RawDirEntry>> {
        // Don't want to totally saturate the executor with these so that some other work can progress.
    ...
    async fn read_path_metadata_if_exists_impl(
        &self,
        path: ProjectRelativePathBuf,
    ) -> kuro_error::Result<Option<RawPathMetadata<ProjectRelativePathBuf>>> {
        let fs = self.fs.dupe();
        let path = path.into_forward_relative_path_buf();
        let file_digest_config = FileDigestConfig::source(self.cas_digest_config);
    ...
    async fn settle(&self) -> kuro_error::Result<()> {
        Ok(())
    }
    ...
    async fn eden_version(&self) -> kuro_error::Result<Option<String>> {
        Ok(None)
    }
...
) -> kuro_error::Result<Option<RawPathMetadata<ForwardRelativePathBuf>>> {
...
fn convert_metadata(
    ...
    file_digest_config: FileDigestConfig,
) -> kuro_error::Result<RawPathMetadata<ForwardRelativePathBuf>> {
...
impl ExactPathMetadata {
    fn from_exact_path(curr: &PathAndAbsPath) -> kuro_error::Result<Self> {
        Ok(
...
mod tests {
    ...
    #[test]
    fn test_read_not_symlink() -> kuro_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_symlink() -> kuro_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_symlink_in_dir() -> kuro_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_symlink_remaining_path() -> kuro_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_symlink_out_of_project() -> kuro_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_unchecked_symlink() -> kuro_error::Result<()> {
        let t = TempDir::new()?;
        let root = AbsPath::new(t.path())?;
