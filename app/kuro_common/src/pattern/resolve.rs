...
use kuro_core::pattern::pattern::display_precise_pattern;
...
impl ResolvedPattern<ConfiguredProvidersPatternExtra> {
    pub fn convert_pattern<U: PatternType>(self) -> kuro_error::Result<ResolvedPattern<U>> {
            ...
            let spec = match spec {
                PackageSpec::Targets(targets) => {
                    PackageSpec::Targets(targets.into_try_map(|(target_name, extra)| {
                        ...
                        kuro_error::Ok((target_name, extra))
                    })?)
                }
...
impl ResolveTargetPatterns {
    ...
    ) -> kuro_error::Result<ResolvedPattern<P>> {
...
mod tests {
    ...
    impl TestPatternResolver {
        fn new(cells: &[(&str, &str)], files: &[&str]) -> kuro_error::Result<Self> {
            let resolver = {
        ...
        async fn resolve<T>(&self, patterns: &[&str]) -> kuro_error::Result<ResolvedPattern<T>>
        where
            T: PatternType,
        {
        ...
        async fn resolve_with_modifiers<T>(
            &self,
            patterns: &[&str],
        ) -> kuro_error::Result<ResolvedPattern<T>>
        where
            T: PatternType,
        {
    ...
    #[tokio::test]
    async fn test_simple_specs_targets() -> kuro_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_simple_specs_providers() -> kuro_error::Result<()> {
        let tester = TestPatternResolver::new(&[("root", ""), ("child", "child/cell")], &[])?;
        tester
                            ...
                            ProvidersPatternExtra {
                                providers: ProvidersName::NonDefault(triomphe::Arc::new(
                                    NonDefaultProvidersName::Named(
                                        kuro_util::arc_str::ArcSlice::new([ProviderName::new(
                                            "my-label".to_owned(),
                                        )
                                        .unwrap()]),
                                    ),
    ...
    #[tokio::test]
    async fn test_simple_specs_targets_with_modifiers() -> kuro_error::Result<()> {
        let tester = TestPatternResolver::new(&[("root", ""), ("child", "child/cell")], &[])?;
    ...
    #[tokio::test]
    async fn test_simple_specs_providers_with_modifiers() -> kuro_error::Result<()> {
        let tester = TestPatternResolver::new(&[("root", ""), ("child", "child/cell")], &[])?;
        ...
        tester
            ...
            .assert_eq(&[(
                ...
                PackageSpec::Targets(vec![(
                    ...
                    ProvidersPatternExtra {
                        providers: ProvidersName::NonDefault(triomphe::Arc::new(
                            NonDefaultProvidersName::Named(kuro_util::arc_str::ArcSlice::new([
