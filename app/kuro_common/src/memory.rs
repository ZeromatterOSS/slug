...
//! Utilities for interacting with the jemalloc heap used by kuro.
//!
...
mod imp {
    ...
    pub fn write_heap_to_file(filename: &str) -> kuro_error::Result<()> {
        ...
        let prof_enabled: bool = memory::mallctl_read("opt.prof")
            .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::Mallctl))?;
        if !prof_enabled {
    ...
    pub fn allocator_stats(options: &str) -> kuro_error::Result<String> {
        allocator_stats::malloc_stats(options)
            .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::MallocStats))
    }
    ...
    pub fn enable_background_threads() -> kuro_error::Result<()> {
        if memory::is_using_jemalloc() {
...
mod imp {
    pub fn write_heap_to_file(_filename: &str) -> kuro_error::Result<()> {
        ...
        Err(kuro_error::kuro_error!(
            kuro_error::ErrorTag::Unimplemented,
            "not implemented: heap dump for Cargo builds"
        ))
    }

    pub fn allocator_stats(_: &str) -> kuro_error::Result<String> {
        Err(kuro_error::kuro_error!(
            kuro_error::ErrorTag::Unimplemented,
