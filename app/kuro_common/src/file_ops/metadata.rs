...
impl FileDigest {
    ...
    pub fn from_file(file: &AbsPath, config: FileDigestConfig) -> kuro_error::Result<Self> {
        if !kuro_env!("BUCK2_DISABLE_FILE_ATTR", bool)? {
    ...
    fn from_file_disk(file: &AbsPath, config: FileDigestConfig) -> kuro_error::Result<Self> {
