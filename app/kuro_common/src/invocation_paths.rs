...
//!
//! Defines utilities to obtain the basic paths for kuro client and the daemon.
...
impl InvocationPaths {
    pub fn daemon_dir(&self) -> kuro_error::Result<DaemonDir> {
        #[cfg(windows)]
        ...
        let root_relative: Cow<ForwardRelativePath> = self
            ...
            .project_root
            .root()
            .strip_prefix(kuro_fs::paths::abs_norm_path::AbsNormPath::new("/")?)?;
