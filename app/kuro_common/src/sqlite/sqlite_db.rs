...
#[derive(kuro_error::Error, Debug, PartialEq, Eq)]
#[kuro(tag = Input)]
enum SqliteDbError {
...
pub trait SqliteTable {
    ...
    fn create_table(&self) -> kuro_error::Result<()>;
}
...
pub trait SqliteDb {
    ...
    fn new(tables: SqliteTables<Self::TableType>) -> kuro_error::Result<Self>
    where
        Self: Sized;

    /// Open and configure the database tables
    fn open_tables(path: &AbsNormPath) -> kuro_error::Result<SqliteTables<Self::TableType>>;
...
pub struct SqliteTables<T: SqliteTable> {
    ...
    /// versions this kuro binary expects. If the versions don't match, we throw
    ...
    /// Table for logging metadata associated with the kuro that created the db.
...
impl<T: SqliteTable> SqliteTables<T> {
    ...
    pub fn get_identity(&self) -> kuro_error::Result<SqliteIdentity> {
        let identity = self
            .created_by_table
            .get(IDENTITY_KEY)
    ...
    pub fn create_all_tables(&self) -> kuro_error::Result<()> {
    ...
    pub fn create_connection(path: &AbsNormPath) -> kuro_error::Result<Arc<Mutex<Connection>>> {
...
mod tests {
    ...
    impl TestSqliteTable {
        ...
        fn insert(&self, value: &str) -> kuro_error::Result<()> {
        ...
        fn read_all(&self) -> kuro_error::Result<TestState> {
    ...
    impl TestSqliteDb {
        fn insert_test_data(&mut self, value: &str) -> kuro_error::Result<()> {
    ...
    #[test]
    fn test_create_new_sqlite_db() -> kuro_error::Result<()> {
    ...
    #[test]
    fn test_load_existing_sqlite_db() -> kuro_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let db_dir = fs
            .path()
            .resolve(ProjectRelativePath::unchecked_new("test_db_dir"));
    ...
    #[test]
    fn test_version_mismatch_error() -> kuro_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let db_dir = fs
            .path()
            .resolve(ProjectRelativePath::unchecked_new("test_db_dir"));
    ...
    #[test]
    fn test_rejected_identity() -> kuro_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let db_dir = fs
            .path()
            .resolve(ProjectRelativePath::unchecked_new("test_db_dir"));
    ...
    #[test]
    fn test_path_does_not_exist_error() -> kuro_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
    ...
    #[test]
    fn test_sqlite_tables_functionality() -> kuro_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let db_path = fs
            .path()
