...
impl LegacyBuckConfigOnDice<'_, '_> {
    pub fn parse<T: FromStr>(&mut self, key: BuckconfigKeyRef) -> kuro_error::Result<Option<T>>
    where
        kuro_error::Error: From<<T as FromStr>::Err>,
    {
...
impl LegacyBuckConfigView for LegacyBuckConfigOnDice<'_, '_> {
    fn get(&mut self, key: BuckconfigKeyRef) -> kuro_error::Result<Option<Arc<str>>> {
...
pub trait HasInjectedLegacyConfigs {
    fn get_injected_external_buckconfig_data(
        &mut self,
    ) -> impl Future<Output = kuro_error::Result<Arc<ExternalBuckconfigData>>>;

    fn is_injected_external_buckconfig_data_key_set(
        &mut self,
    ) -> impl Future<Output = kuro_error::Result<bool>>;
}
...
pub trait HasLegacyConfigs {
    ...
    async fn get_legacy_config_on_dice(
        &mut self,
        cell_name: CellName,
    ) -> kuro_error::Result<OpaqueLegacyBuckConfigOnDice>;
...
pub trait SetLegacyConfigs {
    ...
    fn set_none_legacy_config_external_data(&mut self) -> kuro_error::Result<()>;
}
...
impl Key for LegacyBuckConfigForCellKey {
    type Value = kuro_error::Result<LegacyBuckConfig>;
    ...
    async fn compute(
        ...
        let event = kuro_data::CellHasNewConfigs {
...
impl ProjectionKey for LegacyBuckConfigErrorKey {
    ...
    type Value = Option<kuro_error::Error>;

    fn compute(
        &self,
        config: &kuro_error::Result<LegacyBuckConfig>,
        _ctx: &DiceProjectionComputations,
    ) -> Option<kuro_error::Error> {
...
impl HasInjectedLegacyConfigs for DiceComputations<'_> {
    async fn get_injected_external_buckconfig_data(
        &mut self,
    ) -> kuro_error::Result<Arc<ExternalBuckconfigData>> {
    ...
    async fn is_injected_external_buckconfig_data_key_set(&mut self) -> kuro_error::Result<bool> {
        Ok(self
