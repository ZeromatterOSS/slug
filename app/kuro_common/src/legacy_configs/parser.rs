...
impl LegacyConfigParser {
    ...
    pub(crate) fn finish(self) -> kuro_error::Result<LegacyBuckConfig> {
    ...
    pub(crate) fn to_proto_external_config_values(
        &self,
        is_cli: bool,
    ) -> Vec<kuro_data::ConfigValue> {
        self.values
            .iter()
            .map(|(k, v)| {
                v.values.iter().map(|(key, value)| kuro_data::ConfigValue {
                    section: k.to_owned(),
                    key: key.to_owned(),
...
impl<'p> LegacyConfigFileParser<'p> {
    ...
    fn push_file(&mut self, line: usize, path: &ConfigPath) -> kuro_error::Result<()> {
    ...
    ) -> BoxFuture<'a, kuro_error::Result<bool>> {
        async move {
    ...
    fn parse_section_marker(line: &str) -> kuro_error::Result<Option<&str>> {
