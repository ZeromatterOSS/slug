...
impl Default for Ttl {
    fn default() -> Self {
        Self::from_secs(164 * 86_400) // 164 days, equals scuba kuro_builds retention
    }
}
...
impl AsKuroError for HttpWriteError {
    fn as_kuro_error(self) -> kuro_error::Error {
        kuro_error::Error::from(self)
    }
}

impl AsKuroError for HttpAppendError {
    fn as_kuro_error(self) -> kuro_error::Error {
        kuro_error::Error::from(self)
    }
}
...
impl Bucket {
    ...
    pub const RAGE_DUMPS: Bucket = Bucket {
        name: "kuro_rage_dumps",
        key: "kuro_rage_dumps-key",
    };

    pub const RE_LOGS: Bucket = Bucket {
        name: "kuro_re_logs",
        key: "kuro_re_logs-key",
    };

    pub const INSTALLER_LOGS: Bucket = Bucket {
        name: "kuro_installer_logs",
        key: "kuro_installer_logs-key",
    };
}
...
impl ManifoldChunkedUploader<'_> {
    pub async fn write(&mut self, chunk: Bytes) -> kuro_error::Result<()> {
