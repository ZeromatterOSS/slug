...
pub struct CycleGuardResult<R, E>(kuro_error::Result<Result<R, E>>);

impl<R, E> CycleGuardResult<R, E> {
    /// Converts the result from GuardThis into an kuro_error::Result.
    ///
...
impl<D: CycleAdapterDescriptor> CycleGuard<D> {
    pub fn new(ctx: &DiceComputations<'_>) -> kuro_error::Result<Self> {
    ...
    pub async fn guard_this<R: Send, Fut: Future<Output = R> + Send>(
        ...
        let res: kuro_error::Result<Result<R, D::Error>> = (|| async move {
            match &self.0 {
