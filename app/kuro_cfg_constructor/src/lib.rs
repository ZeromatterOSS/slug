...
) -> kuro_error::Result<(Vec<String>, Value<'v>)> {
...
async fn analyze_constraints(
    ...
    refs: Vec<String>,
) -> kuro_error::Result<SmallMap<String, FrozenProviderCollectionValue>> {
...
fn eval_post_constraint_analysis<'v>(
    ...
    eval.with_evaluator(|eval| -> kuro_error::Result<ConfigurationData> {
...
impl CfgConstructorImpl for CfgConstructor {
    ...
    ) -> Pin<Box<dyn Future<Output = kuro_error::Result<ConfigurationData>> + Send + 'a>> {
