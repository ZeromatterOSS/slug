...
impl<'v> StarlarkSelector<'v> {
    ...
    pub fn from_concat<I>(iter: I, heap: Heap<'v>) -> kuro_error::Result<Value<'v>>
    where
        I: IntoIterator<Item = Value<'v>>,
    {
        fn values_to_selector<'v, I>(
            ...
            values: &mut I,
            heap: Heap<'v>,
        ) -> kuro_error::Result<NoneOr<StarlarkSelector<'v>>>
        where
    ...
    fn select_map<'a, const RECURSE: bool>(
        ...
        kuro_util::threads::check_stack_overflow()?;
...
/// The `Select` type represents a conditional attribute value in Kuro.
///
...
/// # Resolution Timing
///
/// `Select` objects are resolved during Kuro's **configuration phase**, which happens after
...
/// # Platform Selection
///
/// Kuro determines the target platform through:
///
...
/// Example:
/// ```bash
/// kuro build //app:main --target-platforms //platforms:linux-x86_64
/// ```
///
/// # Target Compatibility
///
/// Kuro provides two attributes for platform compatibility:
///
...
/// - Kuro docs: "Configurations By Example"
#[starlark_module]
