...
use kuro_interpreter::file_loader::ModuleDeps;
...
pub trait HasCalculationDelegate<'c, 'd> {
    ...
    ) -> kuro_error::Result<DiceCalculationDelegate<'c, 'd>>;
}
...
impl<'c, 'd> HasCalculationDelegate<'c, 'd> for DiceComputations<'d> {
    async fn get_interpreter_calculator(
        ...
        impl Key for InterpreterConfigForDirKey {
            type Value = kuro_error::Result<Arc<InterpreterForDir>>;
            async fn compute(
                &self,
...
impl<'c, 'd: 'c> DiceCalculationDelegate<'c, 'd> {
    ...
    pub async fn prepare_package_file_eval(
    ...
    ) -> kuro_error::Result<Option<(PackageFilePath, AstModule, ModuleDeps)>> {
        // Note:
        ...
        impl Key for PackageFileLookupKey {
            type Value = kuro_error::Result<Option<Arc<PackageFilePath>>>;
    ...
    pub(crate) async fn eval_package_file(
        ...
        impl Key for PackageFileKey {
            type Value = kuro_error::Result<SuperPackage>;
    ...
    pub async fn eval_build_file(
        ...
        package: PackageLabel,
        cancellation: &CancellationContext,
    ) -> (TimeSpan, kuro_error::Result<Arc<EvaluationResult>>) {
        let mut now = None;
        let eval_kind = StarlarkEvalKind::LoadBuildFile(package.dupe());
        let eval_result: kuro_error::Result<_> = try {
            let ((), listing) = self
                .ctx
                .try_compute2(
            ...
            let start_event = kuro_data::LoadBuildFileStart {
                cell: cell_str.clone(),
