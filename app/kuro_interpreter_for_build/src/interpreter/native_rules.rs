...
pub(crate) mod rule_defs {
    ...
    /// Standard attributes shared by most language rules: srcs, deps, data.
    fn standard_language_rule_attributes() -> AttributeSpec {
        let srcs_attr = Attribute::new(
            Some(Arc::new(CoercedAttr::List(
                ListLiteral(ArcSlice::default()),
            ))),
        ...
        .expect("standard language rule attributes should be valid")
    }

    fn make_language_rule(kind: NativeRuleKind, is_test: bool) -> Arc<Rule> {
        Arc::new(Rule {
            attributes: standard_language_rule_attributes(),
    ...
    pub static PROTO_LIBRARY_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_language_rule(NativeRuleKind::ProtoLibrary, false));
    pub static JAVA_LIBRARY_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_language_rule(NativeRuleKind::JavaLibrary, false));
    pub static JAVA_BINARY_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_language_rule(NativeRuleKind::JavaBinary, false));
    pub static JAVA_TEST_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_language_rule(NativeRuleKind::JavaTest, true));
    pub static JAVA_IMPORT_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_language_rule(NativeRuleKind::JavaImport, false));
    pub static PY_LIBRARY_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_language_rule(NativeRuleKind::PyLibrary, false));
    pub static PY_BINARY_RULE: Lazy<Arc<Rule>> =
...
pub fn register_native_rules(globals: &mut GlobalsBuilder) {
    ...
    /// Protocol buffer library rule (Bazel-compatible).
    fn proto_library<'v>(
        #[starlark(require = named)] name: &str,
        #[starlark(require = named, default = starlark::values::none::NoneType)] srcs: Value<'v>,
        #[starlark(require = named, default = starlark::values::none::NoneType)] deps: Value<'v>,
        ...
        register_standard_language_rule(
            ...
            deps,
            visibility,
            rule_defs::PROTO_LIBRARY_RULE.clone(),
        )
    }

    /// Java library rule (Bazel-compatible).
    fn java_library<'v>(
        #[starlark(require = named)] name: &str,
        #[starlark(require = named, default = starlark::values::none::NoneType)] srcs: Value<'v>,
        #[starlark(require = named, default = starlark::values::none::NoneType)] deps: Value<'v>,
        ...
        register_standard_language_rule(
            ...
            deps,
            visibility,
            rule_defs::JAVA_LIBRARY_RULE.clone(),
        )
    }

    /// Java binary rule (Bazel-compatible).
    fn java_binary<'v>(
        #[starlark(require = named)] name: &str,
        #[starlark(require = named, default = starlark::values::none::NoneType)] srcs: Value<'v>,
        #[starlark(require = named, default = starlark::values::none::NoneType)] deps: Value<'v>,
        ...
        register_standard_language_rule(
            ...
            deps,
            visibility,
            rule_defs::JAVA_BINARY_RULE.clone(),
        )
    }

    /// Java test rule (Bazel-compatible).
    fn java_test<'v>(
        #[starlark(require = named)] name: &str,
        #[starlark(require = named, default = starlark::values::none::NoneType)] srcs: Value<'v>,
        #[starlark(require = named, default = starlark::values::none::NoneType)] deps: Value<'v>,
        ...
        register_standard_language_rule(
            ...
            deps,
            visibility,
            rule_defs::JAVA_TEST_RULE.clone(),
        )
    }

    /// Java import rule (Bazel-compatible).
    fn java_import<'v>(
        #[starlark(require = named)] name: &str,
        #[starlark(require = named, default = starlark::values::none::NoneType)] srcs: Value<'v>,
        #[starlark(require = named, default = starlark::values::none::NoneType)] deps: Value<'v>,
        ...
        register_standard_language_rule(
            ...
            deps,
            visibility,
            rule_defs::JAVA_IMPORT_RULE.clone(),
        )
    }

    /// Python library rule (Bazel-compatible).
    fn py_library<'v>(
        #[starlark(require = named)] name: &str,
        #[starlark(require = named, default = starlark::values::none::NoneType)] srcs: Value<'v>,
        #[starlark(require = named, default = starlark::values::none::NoneType)] deps: Value<'v>,
        ...
        register_standard_language_rule(
            ...
            deps,
            visibility,
            rule_defs::PY_LIBRARY_RULE.clone(),
        )
    }

    /// Python binary rule (Bazel-compatible).
    fn py_binary<'v>(
        #[starlark(require = named)] name: &str,
        #[starlark(require = named, default = starlark::values::none::NoneType)] srcs: Value<'v>,
        #[starlark(require = named, default = starlark::values::none::NoneType)] deps: Value<'v>,
        ...
        register_standard_language_rule(
            ...
            deps,
            visibility,
            rule_defs::PY_BINARY_RULE.clone(),
        )
    }

    /// Python test rule (Bazel-compatible).
    fn py_test<'v>(
        #[starlark(require = named)] name: &str,
        #[starlark(require = named, default = starlark::values::none::NoneType)] srcs: Value<'v>,
        #[starlark(require = named, default = starlark::values::none::NoneType)] deps: Value<'v>,
...
/// Helper to register a standard language rule with srcs, deps, data attributes.
fn register_standard_language_rule<'v>(
    eval: &mut Evaluator<'v, '_, '_>,
    rule_name: &str,
    name: &str,
    srcs: Value<'v>,
    ...
    // Coerce srcs
    let list_dep_type = AttrType::list(AttrType::one_of(vec![
        AttrType::dep(ProviderIdSet::EMPTY, PluginKindSet::EMPTY),
        AttrType::source(false),
    ]));
    let srcs_value = if srcs.is_none() {
    ...
    let coerced_srcs =
        list_dep_type.coerce(AttrIsConfigurable::Yes, coercion_ctx, srcs_value)?;
    attrs.push(("srcs".to_owned(), coerced_srcs));
    ...
    // Empty data
    let data_value = eval.heap().alloc(Vec::<Value>::new());
    let coerced_data =
        list_dep_type.coerce(AttrIsConfigurable::Yes, coercion_ctx, data_value)?;
    attrs.push(("data".to_owned(), coerced_data));
