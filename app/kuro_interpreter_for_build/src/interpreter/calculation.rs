...
use kuro_interpreter::load_module::InterpreterCalculationImpl;
...
use kuro_node::nodes::frontend::TargetGraphCalculationImpl;
...
impl TargetGraphCalculationImpl for TargetGraphCalculationInstance {
    ...
    fn get_interpreter_results<'a>(
        ...
        package: PackageLabel,
    ) -> BoxFuture<'a, kuro_error::Result<Arc<EvaluationResult>>> {
...
impl Key for EvalImportKey {
    type Value = kuro_error::Result<LoadedModule>;
    async fn compute(
        &self,
        ctx: &mut DiceComputations,
        cancellation: &CancellationContext,
