...
impl<'v> InterpreterExtraValue<'v> {
    pub(crate) fn get(module: &'v Module) -> kuro_error::Result<&'v InterpreterExtraValue<'v>> {
        Ok(&module
            .extra_value()
...
impl FrozenInterpreterExtraValue {
    pub(crate) fn get(
        module: &FrozenModule,
    ) -> kuro_error::Result<OwnedFrozenValueTyped<StarlarkAnyComplex<FrozenInterpreterExtraValue>>>
    {
        module
            .owned_extra_value()
            .internal_error("Extra value is missing")?
