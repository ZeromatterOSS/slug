...
use kuro_node::attrs::attr_type::query::QueryAttrBase;
...
pub trait QueryAttrTypeExt {
    fn coerce(
        ...
        query: String,
    ) -> kuro_error::Result<QueryAttrBase<ProvidersLabel>>;
}
...
impl QueryAttrTypeExt for QueryAttrType {
    fn coerce(
        ...
        impl<'q> QueryLiteralVisitor<'q> for Collector<'q> {
            fn target_pattern(&mut self, pattern: &'q str) -> kuro_error::Result<()> {
