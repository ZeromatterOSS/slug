...
impl SuperPackageValuesImpl {
    pub fn get(values: &dyn SuperPackageValues) -> kuro_error::Result<&SuperPackageValuesImpl> {
        values
            .as_any()
    ...
    ) -> kuro_error::Result<Arc<dyn SuperPackageValues>> {
...
impl SuperPackageValues for SuperPackageValuesImpl {
    ...
    fn package_values_json(&self) -> kuro_error::Result<SmallMap<MetadataKey, serde_json::Value>> {
...
impl<'v> StarlarkPackageValue<'v> {
    pub(crate) fn new(value: Value<'v>) -> kuro_error::Result<StarlarkPackageValue<'v>> {
...
impl OwnedFrozenStarlarkPackageValue {
    ...
    pub(crate) fn to_json_value(&self) -> kuro_error::Result<serde_json::Value> {
        self.0
            .value()
            .to_json_value()
...
pub(crate) fn write_package_value_impl<'v>(
    ...
    let key = MetadataKeyRef::new(key).map_err(kuro_error::Error::from)?;
    ...
    if package_file_extra.package_values.borrow().contains_key(key) {
        return Err(
            kuro_error::Error::from(PackageValueError::KeyAlreadySetInThisFile(key.to_owned()))
                .into(),
        );
    }
    ...
    if !overwrite {
        if package_ctx.parent.package_values().contains_key(key) {
            return Err(
                kuro_error::Error::from(PackageValueError::KeySetInParentFile(key.to_owned()))
                    .into(),
            );
        }
    }
