...
use kuro_data::ReleaseLocalResourcesEnd;
...
pub struct LocalResourceRegistry(
    pub Arc<Mutex<HashMap<ConfiguredTargetLabel, kuro_error::Result<LocalResourceState>>>>,
);

impl LocalResourceRegistry {
    pub(crate) async fn release_all_resources(&self) -> kuro_error::Result<()> {
        let resources = {
...
pub trait HasLocalResourceRegistry {
    fn get_local_resource_registry(&self) -> kuro_error::Result<Arc<LocalResourceRegistry>>;
}
