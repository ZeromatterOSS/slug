...
impl DownwardApi for BuckTestDownwardApi {
    async fn console(&self, _level: Level, msg: String) -> kuro_error::Result<()> {
        // TODO(brasselsprouts): use the level and hook it up with our superconsole
        eprintln!("{}", msg);
        Ok(())
    }

    async fn log(&self, _level: Level, _msg: String) -> kuro_error::Result<()> {
        unimplemented!("TODO(bobyf)")
    }

    async fn external(&self, _data: HashMap<String, String>) -> kuro_error::Result<()> {
