...
use kuro_build_api::interpreter::rule_defs::provider::test_provider::TestProvider;
...
use kuro_cli_proto::TestResponse;
...
use kuro_core::pattern::pattern::ProvidersLabelWithModifiers;
...
use kuro_interpreter_for_build::interpreter::context::HasInterpreterContext;
...
use kuro_server_ctx::commands::send_target_cfg_event;
...
use kuro_server_ctx::tpx_experiment_util::get_tpx_experiments;
...
struct TestOutcome {
    errors: Vec<kuro_data::ErrorReport>,
...
impl TestOutcome {
    fn exit_code(&self) -> kuro_error::Result<i32> {
...
impl CounterWithExamples {
    ...
    fn to_cli_proto_counter(self) -> kuro_cli_proto::CounterWithExamples {
        kuro_cli_proto::CounterWithExamples {
            count: self.count,
            max: self.max,
...
#[derive(Debug, kuro_error_derive::Error)]
#[kuro(tag = TestDeadlineExpired)]
...
async fn test_command(
    ...
    req: TestRequest,
) -> kuro_error::Result<TestResponse> {
...
struct TestServerCommand {
    req: kuro_cli_proto::TestRequest,
}

#[async_trait]
impl ServerCommandTemplate for TestServerCommand {
    type StartEvent = kuro_data::TestCommandStart;
    type EndEvent = kuro_data::TestCommandEnd;
    type Response = kuro_cli_proto::TestResponse;
    type PartialResult = NoPartialResult;

    fn build_result(&self, response: &Self::Response) -> Option<BuildResult> {
        let build_completed =
            if let Some(kuro_cli_proto::test_response::TestStatuses { build_errors, .. }) =
...
fn test_executor_errors(
    ...
    test_statuses: &kuro_cli_proto::test_response::TestStatuses,
) -> Vec<kuro_data::ErrorReport> {
    // FIXME: These errors should be derived from exit code only
    let mut errors = Vec::new();
    if let Some(failed) = &test_statuses.failed {
        if failed.count > 0 {
            errors.push(kuro_data::ErrorReport::from(&TestError::TestFailed.into()));
        }
    }
    ...
    if let Some(fatal) = &test_statuses.fatals {
        if fatal.count > 0 {
            errors.push(kuro_data::ErrorReport::from(&TestError::Fatal.into()));
        }
    }
    ...
    if errors.is_empty() {
        errors.push(kuro_data::ErrorReport::from(&kuro_error::kuro_error!(
            kuro_error::ErrorTag::TestExecutor,
...
async fn test(
    ...
    let test_statuses = kuro_cli_proto::test_response::TestStatuses {
        passed: Some(
            test_outcome
                .executor_report
                .statuses
...
async fn test_targets(
    ...
    let test_server = tokio::spawn({
        ...
        with_dispatcher_async(
            ...
            async move {
                ...
                kuro_error::Ok((driver.build_target_result, test_statuses))
            },
        )
    });
    ...
    let _ignored = test_status_sender.unbounded_send(Err(kuro_error::kuro_error!(
        ErrorTag::TestExecutor,
    ...
    let mut errors = convert_error(&build_target_result)
        .iter()
        .map(kuro_data::ErrorReport::from)
        .unique_by(|e| e.message.clone())
        .collect::<Vec<_>>();

    if let Some(timeout_observer) = timeout_observer {
        if !timeout_observer.is_alive().await {
            errors.push(kuro_data::ErrorReport::from(&DeadlineExpired.into()));
        }
    }
...
async fn build_target_result(
    ...
    build_default_info: bool,
    build_run_info: bool,
) -> kuro_error::Result<(BuildTargetResult, FrozenProviderCollectionValue)> {
...
async fn test_target(
    ...
    test_config_unification_rollout: bool,
    oncall: Option<String>,
) -> kuro_error::Result<Option<ConfiguredProvidersLabel>> {
...
fn convert_error(build_result: &BuildTargetResult) -> Vec<kuro_error::Error> {
    let mut errors = Vec::new();
...
fn run_tests<'a, 'b>(
    ...
    test_config_unification_rollout: bool,
    oncall: Option<String>,
) -> BoxFuture<'a, kuro_error::Result<ConfiguredProvidersLabel>> {
...
fn generate_config_entry_args(
    ...
    representative_config_flags: &[kuro_cli_proto::RepresentativeConfigFlag],
) {
...
fn post_process_test_executor(s: &str) -> kuro_error::Result<PathBuf> {
    match s.split_once("$BUCK2_BINARY_DIR/") {
        Some(("", rest)) => {
            let exe = AbsPathBuf::new(
                std::env::current_exe().buck_error_context("Cannot get Kuro executable")?,
            )?;
            let exe = fs_util::canonicalize(&exe).buck_error_context(
                "Failed to canonicalize path to Kuro executable. Try running `kuro kill`.",
            )?;

            let exe = exe.as_abs_path();
            let exe_dir = exe
                .parent()
                .buck_error_context("Kuro executable directory has no parent")?;

            Ok(exe_dir.join(rest).to_path_buf())
        }
        Some(..) => Err(kuro_error::kuro_error!(
