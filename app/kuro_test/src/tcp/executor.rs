...
/// Environment variable used to pass the actual username from Kuro client to the test executor.
/// This is necessary because in some scenarios kurod may run as a different user than the user who invoked `kuro test`.
...
pub(crate) async fn spawn(
    ...
    args: Vec<String>,
    tpx_args: Vec<String>,
) -> kuro_error::Result<(ExecutorFuture, TcpStream, TcpStream)> {
    ...
    // Pass the actual username from Kuro client to the executor.
    ...
    let conns = async {
        ...
        kuro_error::Ok((orchestrator_tcp_stream, executor_tcp_stream))
    };
    ...
    match futures::future::select(exec, conns).await {
        Either::Left((output, _)) => Err(kuro_error::internal_error!(
...
async fn create_tcp_listener() -> kuro_error::Result<(String, TcpListener)> {
