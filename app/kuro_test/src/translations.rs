...
use kuro_test_api::data::RemoteObject;
...
) -> kuro_error::Result<ConfiguredTarget> {
...
pub(crate) fn convert_test_result(
    test_result: kuro_test_api::data::TestResult,
    session: &TestSession,
) -> kuro_error::Result<kuro_data::TestResult> {
    let kuro_test_api::data::TestResult {
        name,
        status,
        msg,
        duration,
    ...
    Ok(kuro_data::TestResult {
        name,
        status: status.try_into().buck_error_context("Invalid `status`")?,
        msg: msg.map(|msg| kuro_data::test_result::OptionalMsg { msg }),
...
fn convert_digest(digest: &FileDigest) -> kuro_test_api::data::CasDigest {
    let hash = format!("{}", digest.raw_digest());
    kuro_test_api::data::CasDigest {
        hash,
