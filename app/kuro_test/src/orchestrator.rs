...
//! Implementation of the `TestOrchestrator` from `kuro_test_api`.
...
use kuro_build_api::actions::execute::dice_data::GetReClient;
use kuro_build_api::actions::impls::run_action_knobs::HasRunActionKnobs;
...
use kuro_build_api::build_signals::HasBuildSignals;
use kuro_build_api::context::HasBuildContextData;
use kuro_build_api::interpreter::rule_defs::cmd_args::AbsCommandLineContext;
use kuro_build_api::interpreter::rule_defs::cmd_args::ArtifactPathMapperImpl;
...
use kuro_build_api::interpreter::rule_defs::provider::builtin::external_runner_test_info::TestCommandMember;
...
use kuro_core::fs::buck_out_path::BuckOutTestPath;
...
use kuro_data::EndOfTestResults;
use kuro_data::SetupLocalResourcesEnd;
use kuro_data::SetupLocalResourcesStart;
use kuro_data::TestDiscovery;
use kuro_data::TestDiscoveryEnd;
use kuro_data::TestDiscoveryStart;
use kuro_data::TestRunEnd;
use kuro_data::TestRunStart;
use kuro_data::TestSessionInfo;
use kuro_data::TestSuite;
...
use kuro_execute::execute::blocking::HasBlockingExecutor;
use kuro_execute::execute::cache_uploader::CacheUploadInfo;
use kuro_execute::execute::cache_uploader::NoOpCacheUploader;
use kuro_execute::execute::claim::MutexClaimManager;
use kuro_execute::execute::command_executor::CommandExecutor;
...
use kuro_execute::execute::request::OutputCreationBehavior;
use kuro_execute::execute::request::WorkerId;
use kuro_execute::execute::request::WorkerSpec;
use kuro_execute::execute::result::CommandCancellationReason;
...
use kuro_execute_impl::executors::local::EnvironmentBuilder;
use kuro_execute_impl::executors::local::apply_local_execution_environment;
use kuro_execute_impl::executors::local::create_output_dirs;
use kuro_execute_impl::executors::local::materialize_inputs;
use kuro_execute_impl::executors::local::prep_scratch_path;
...
pub struct BuckTestOrchestrator<'a: 'static> {
    ...
    results_channel: UnboundedSender<kuro_error::Result<ExecutorMessage>>,
...
impl<'a> BuckTestOrchestrator<'a> {
    pub async fn new(
        ...
        results_channel: UnboundedSender<kuro_error::Result<ExecutorMessage>>,
        cancellations: &'a CancellationContext,
    ) -> kuro_error::Result<BuckTestOrchestrator<'a>> {
...
impl TestOrchestrator for BuckTestOrchestrator<'_> {
            ...
            Err(ExecuteError::Cancelled(cancelled)) => {
                Ok(ExecuteResponse::Cancelled(match cancelled.reason {
                    Some(CancellationReason::NotSpecified) => {
                        Some(kuro_test_api::data::CancellationReason::NotSpecified)
                    }
                    Some(CancellationReason::ReQueueTimeout) => {
                        Some(kuro_test_api::data::CancellationReason::ReQueueTimeout)
                    }
                    None => None,
                }))
            }
    ...
    async fn report_test_result(&self, r: TestResult) -> kuro_error::Result<()> {
        let event = kuro_data::instant_event::Data::TestResult(translations::convert_test_result(
            r.clone(),
            &self.session,
        )?);
        ...
        self.results_channel
            ...
            .map_err(|_| {
                kuro_error::internal_error!("Test result was received after end-of-tests")
            })?;
        Ok(())
    }
    ...
    async fn report_tests_discovered(
        ...
        self.events.instant_event(TestDiscovery {
            data: Some(kuro_data::test_discovery::Data::Tests(TestSuite {
                suite_name: suite,
                test_names: names,
    ...
    async fn report_test_session(&self, session_info: String) -> kuro_error::Result<()> {
        self.events.instant_event(TestDiscovery {
            data: Some(kuro_data::test_discovery::Data::Session(TestSessionInfo {
                info: session_info,
            })),
        });
    ...
    async fn end_of_test_results(&self, exit_code: i32) -> kuro_error::Result<()> {
        self.events.instant_event(EndOfTestResults { exit_code });
        self.results_channel
            .unbounded_send(Ok(ExecutorMessage::ExitCode(exit_code)))
            .map_err(|_| kuro_error::internal_error!("end_of_tests was received twice"))?;
    ...
    async fn prepare_for_local_execution(
        ...
        pre_create_dirs: Vec<DeclaredOutput>,
        required_local_resources: RequiredLocalResources,
    ) -> kuro_error::Result<PrepareForLocalExecutionResult> {
        let test_target = self.session.get(test_target)?;
    ...
    async fn attach_info_message(&self, message: String) -> kuro_error::Result<()> {
        self.results_channel
            .unbounded_send(Ok(ExecutorMessage::InfoMessage(message)))
            .map_err(|_| kuro_error::internal_error!("Message received after end-of-tests"))?;
        Ok(())
    }
}
...
impl BuckTestOrchestrator<'_> {
    ...
    fn executor_config_with_remote_cache_override<'a>(
        ...
        stage: &TestStage,
    ) -> kuro_error::Result<Cow<'a, CommandExecutorConfig>> {
    ...
    async fn get_command_executor(
        ...
        stage: &TestStage,
    ) -> kuro_error::Result<CommandExecutor> {
    ...
    async fn get_test_info(
        dice: &mut DiceComputations<'_>,
        test_target: &ConfiguredProvidersLabel,
    ) -> kuro_error::Result<OwnedFrozenValueTyped<FrozenExternalRunnerTestInfo>> {
    ...
    async fn get_test_executor(
        ...
        fs: &ArtifactFs,
        stage: &TestStage,
    ) -> kuro_error::Result<TestExecutor> {
    ...
    async fn expand_test_executable<'a>(
        ...
        prefix: TestExecutionPrefix,
        opts: TestSessionOptions,
    ) -> kuro_error::Result<ExpandedTestExecutable> {
        ...
        let (expanded_cmd, expanded_env, ensured_inputs, expanded_worker) = {
            ...
            let ensured_inputs = KeepGoing::try_compute_join_all(dice, inputs, |dice, input| {
                async move {
                    ...
                    kuro_error::Ok((input, artifact_group_value))
                }
                .boxed()
            })
            .await?;
    ...
    async fn setup_local_resources(
        ...
        let result: kuro_error::Result<Vec<_>> = required_targets
            .iter()
            .map(|t| lock.get(t).unwrap().clone())
            .collect();
        Ok(result.map_err(kuro_error::Error::from)?)
    }
    ...
    async fn prepare_local_resource(
        ...
        executor_fs: &ExecutorFs<'_>,
        default_timeout: Duration,
    ) -> kuro_error::Result<PreparedLocalResourceSetupContext> {
        let digest_config = dice.global_data().get_digest_config();
    ...
    async fn start_local_resource(
        ...
        cancellation: &CancellationContext,
    ) -> kuro_error::Result<LocalResourceState> {
        let manager = CommandExecutionManager::new(
            Box::new(MutexClaimManager::new()),
            events.dupe(),
            liveliness_observer,
...
impl Drop for BuckTestOrchestrator<'_> {
    fn drop(&mut self) {
        ...
        let _ignored = self
            .results_channel
            .unbounded_send(Err(kuro_error::internal_error!(
...
) -> impl for<'a> Fn(&'a mut dyn CommandLineArtifactVisitor<'v>, &'a ArgValue) -> kuro_error::Result<()>
{
...
impl<'a> Execute2RequestExpander<'a> {
    fn get_inputs(&self) -> kuro_error::Result<IndexSet<ArtifactGroup>> {
...
async fn resolve_output_root(
    ...
    test_target: &ConfiguredProvidersLabel,
    prefix: TestExecutionPrefix,
) -> Result<ForwardRelativePathBuf, kuro_error::Error> {
...
impl CommandExecutionTarget for TestTarget<'_> {
    ...
    fn as_proto_action_key(&self) -> kuro_data::ActionKey {
        kuro_data::ActionKey {
            id: Default::default(),
            owner: Some(kuro_data::action_key::Owner::TestTargetLabel(
...
impl CommandExecutionTarget for LocalResourceTarget<'_> {
    ...
    fn as_proto_action_key(&self) -> kuro_data::ActionKey {
        kuro_data::ActionKey {
            id: Default::default(),
            owner: Some(kuro_data::action_key::Owner::LocalResourceSetup(
                self.target.as_proto(),
            )),
            key: Default::default(),
        }
...
mod tests {
    ...
    async fn make() -> kuro_error::Result<(
        BuckTestOrchestrator<'static>,
        UnboundedReceiver<kuro_error::Result<ExecutorMessage>>,
    )> {
    ...
    #[tokio::test]
    async fn orchestrator_results() -> kuro_error::Result<()> {
    ...
    #[tokio::test]
    async fn orchestrator_attach_info_messages() -> kuro_error::Result<()> {
        let (orchestrator, channel) = make().await?;
    ...
    #[tokio::test]
    async fn test_orchestrator_channel_drop() -> kuro_error::Result<()> {
        let (orchestrator, channel) = make().await?;
        drop(orchestrator);
    ...
    #[tokio::test]
    async fn test_orchestrator_closes_channel() -> kuro_error::Result<()> {
        let (orchestrator, channel) = make().await?;
