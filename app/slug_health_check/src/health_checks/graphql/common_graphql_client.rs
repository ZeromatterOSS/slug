...
pub fn get_graphql_client(fb: FacebookInit) -> slug_error::Result<InternGraphClient> {
...
pub(crate) fn get_interngraph_client_subdomain_and_auth(
    fb: FacebookInit,
) -> slug_error::Result<(Subdomain, Authentication)> {
    match x2p::current()
        .ok()
        ...
        .flatten()
    {
        Some(proxy_port_url) => Ok((
            ...
            Authentication::x2p_cat_token_for_current_user(fb, APP_ID)
                .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::HealthCheck))?,
        )),
