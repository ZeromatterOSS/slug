...
/// Trait to generalize a slug health check.
...
pub(crate) struct HealthCheckContext {
    ...
    pub command_data: Option<slug_data::command_start::Data>,
    ...
    pub parsed_target_patterns: Option<slug_data::ParsedTargetPatterns>,
    ...
    pub experiment_configurations: Option<slug_data::SystemInfo>,
}

/// A subset of the Snapshot data specifically for health check use.
/// This struct contains timing metrics extracted from slug_data::Snapshot.
#[derive(Dupe, Clone)]
...
pub enum HealthCheckEvent {
    ...
    // This snapshot passes a subset of the slug_data::Snapshot data to health checks.
...
pub enum HealthCheckContextEvent {
    CommandStart(slug_data::CommandStartWithTraceId),
    ParsedTargetPatterns(slug_data::ParsedTargetPatterns),
    BranchedFromRevision(String),
    /// Sent only once and communicates if slug is experiencing excess cache misses.
    HasExcessCacheMisses(),
    /// Configuration about the health checks.
    ExperimentConfigurations(slug_data::SystemInfo),
}
