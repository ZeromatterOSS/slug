...
use slug_health_check_proto::health_check_client::HealthCheckClient;
use slug_util::properly_reaped_child::ProperlyReapedChild;
use slug_util::properly_reaped_child::reap_on_drop_command;
...
const CLI_NAME: &str = "slug-health-check";
const CLI_INFO_FILE: &str = "slug-health-check-server-info";
...
pub(crate) struct HealthCheckRpcClient {
    ...
    connection: AsyncOnceCell<slug_error::Result<HealthCheckServerConnection>>,
...
impl HealthCheckRpcClient {
    ...
    async fn connection(&self) -> slug_error::Result<&HealthCheckServerConnection> {
    ...
    fn get_cli_path() -> slug_error::Result<String> {
    ...
    async fn get_state_info_file_path(&self) -> slug_error::Result<AbsNormPathBuf> {
...
impl HealthCheckService for HealthCheckRpcClient {
    async fn update_context(&mut self, event: HealthCheckContextEvent) -> slug_error::Result<()> {
        let rpc_event: slug_health_check_proto::HealthCheckContextEvent = event.try_into()?;
        self.rpc_client()
            .await?
    ...
    ) -> slug_error::Result<Vec<Report>> {
        let snapshot: slug_health_check_proto::HealthCheckSnapshotData = snapshot.try_into()?;
...
mod tests {
    ...
    async fn temp_state_info(
        temp_dir: &TempDir,
    ) -> slug_error::Result<(AbsNormPathBuf, AbsNormPathBuf)> {
    ...
    #[tokio::test]
    async fn test_server_state_reset() -> slug_error::Result<()> {
