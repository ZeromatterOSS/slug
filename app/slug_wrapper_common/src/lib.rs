...
//! Code shared between `slug_wrapper` and `slug`.
//!
...
/// Find all slug processes in the system.
fn find_slug_processes(who_is_asking: WhoIsAsking) -> Vec<ProcessInfo> {
    ...
    let mut slug_processes = Vec::new();
    for (pid, process) in system.processes() {
        ...
        if is_slug_exe(exe, who_is_asking) && !current_parents.contains(pid) {
            let Ok(pid) = Pid::from_u32(pid.as_u32()) else {
                continue;
            };
            slug_processes.push(ProcessInfo {
                pid,
...
/// Kills all running Slug processes, except this process's hierarchy. Returns whether it
/// succeeded without errors.
pub fn killall(who_is_asking: WhoIsAsking, write: impl Fn(String)) -> bool {
    let slug_processes = find_slug_processes(who_is_asking);

    if slug_processes.is_empty() {
        write("No slug processes found".to_owned());
        return true;
    }
    ...
    impl<F: Fn(String)> Printer<F> {
        ...
        fn failed_to_kill(&mut self, process: &ProcessInfo, error: slug_error::Error) {
            ...
            for process in processes_still_alive {
                printer.failed_to_kill(
                    ...
                    slug_error::slug_error!(
                        slug_error::ErrorTag::DaemonWontDieFromKill,
