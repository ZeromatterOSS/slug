...
use slug_cli_proto::representative_config_flag::Source as RepresentativeConfigFlagSource;
use slug_common::argv::ArgFileKind;
use slug_common::argv::ArgFilePath;
use slug_common::argv::ExpandedArgSource;
use slug_common::argv::ExpandedArgv;
use slug_common::argv::FlagfileArgSource;
...
#[slug(tag = slug_error::ErrorTag::InternalError)]
...
pub struct CommonBuildConfigurationOptions {
    ...
    ///
    /// Normally, when you run two commands - from different terminals, say - slug will attempt
    /// to run them in parallel. However, if the two commands are based on different state, that
    /// is they either have different configs or different filesystem states, slug cannot run them
    ...
    /// Use `--console` to control Slug's output style.
    ...
    /// alongside (or in lieu of) the slug defaults from
    /// `[slug_re_client]` buckconfig. Useful for steering BuildBuddy's
    ...
    /// the public certificate and the private key, matching slug's
    /// existing `[slug_re_client] tls_client_cert` semantics.
    #[clap(
    ...
    /// `--digest_function`).
    ///
    /// Accepts a comma-separated list (slug extension; first entry is the
    ...
    /// File-watcher backend (slug-specific; no Bazel equivalent).
    ///
    ...
    #[clap(
        long = "slug-file-watcher",
        alias = "slug_file_watcher",
        hide = true,
        value_name = "BACKEND"
    )]
    pub slug_file_watcher: Option<String>,
    ...
    /// ignored — slug's RE client does not yet plumb custom headers).
    ...
    /// `grpcs://remote.buildbuddy.io`). When set, Slug streams BEP events
    ...
    /// URL prefix for BES invocation links; Slug logs
...
impl CommonBuildConfigurationOptions {
    ...
    pub fn config_overrides(
        ...
        cwd: &AbsWorkingDir,
    ) -> slug_error::Result<Vec<ConfigOverride>> {
        fn with_indices<'a, T>(
            collection: &'a [T],
            name: &str,
            matches: BuckArgMatches<'a>,
        ) -> slug_error::Result<impl Iterator<Item = (usize, &'a T)> + use<'a, T>> {
            ...
            if indices.len() != collection.len() {
                return Err(slug_error::Error::from(IndicesLengthMismatchError {
        ...
        // `slug_re_client` buckconfig overrides that
        // `SlugOssReConfiguration::from_legacy_config` reads. Without
            ...
            && !uri.is_empty()
        {
            result.push(ConfigOverride {
                cell: None,
                config_override: format!("slug_re_client.address={uri}"),
                config_type: ConfigType::Value as i32,
            });
            ...
            && !uri.is_empty()
        {
            result.push(ConfigOverride {
                cell: None,
                config_override: format!("slug_re_client.cas_address={uri}"),
                config_type: ConfigType::Value as i32,
            });
            result.push(ConfigOverride {
                cell: None,
                config_override: format!("slug_re_client.action_cache_address={uri}"),
                config_type: ConfigType::Value as i32,
            });
        }
        ...
        if !header_strs.is_empty() {
            result.push(ConfigOverride {
                cell: None,
                config_override: format!("slug_re_client.http_headers={}", header_strs.join(",")),
                config_type: ConfigType::Value as i32,
            });
        }
    ...
    pub fn cli_re_config_snapshot(&self) -> Option<slug_common::init::ReConfigSnapshot> {
        let header_strs: Vec<String> = self
            .remote_header
            .iter()
            .chain(self.remote_cache_header.iter())
        ...
        Some(slug_common::init::ReConfigSnapshot {
...
pub struct CommonStarlarkOptions {
    ...
    ///    analysis/cell//slug/app/slug_action_impl:slug_action_impl (cfg:linux-x86_64#27ac5723e0c99706)
    ...
    ///    anon_analysis/anon//:_anon_link_rule (anon: 766183dc9b6f680a) (fbcode//slug/platform/execution:linux-x86_64#08961b14cfb182aa)
...
impl CommonStarlarkOptions {
    ...
    ) -> Option<slug_cli_proto::client_context::ProfilePatternOptions> {
        self.profile_patterns.as_ref().map(|v| {
            slug_cli_proto::client_context::ProfilePatternOptions {
...
#[cfg(test)]
mod tests {
    use slug_common::argv::ExpandedArgvBuilder;
    ...
    #[test]
    fn test_get_representative_config_flags_for_flagfiles() -> slug_error::Result<()> {
    ...
    #[test]
    fn test_get_representative_config_flags_stops_at_double_dash() -> slug_error::Result<()> {
        let mut argv = ExpandedArgvBuilder::new();
