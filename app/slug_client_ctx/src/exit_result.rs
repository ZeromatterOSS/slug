...
use slug_error::classify::ErrorTagExtra;
use slug_error::classify::best_error;
...
///
/// We can easily turn a slug_error::Result (or slug_error::Error, or even a message) into a ExitResult,
...
impl ExitResult {
    ...
    pub fn get_error(&self) -> Option<slug_error::Error> {
    ...
    pub fn bail(msg: impl Display) -> Self {
        Self::err(slug_error::slug_error!(
            slug_error::ErrorTag::Bail,
            "Command failed: {}",
            msg
        ))
    }
    ...
    pub fn err(err: slug_error::Error) -> Self {
        Self {
    ...
    pub fn err_with_exit_code(err: slug_error::Error, exit_code: ExitCode) -> Self {
        Self {
    ...
    /// Slug supports being built as both a "full" binary as well as a "client-only" binary.
    ///
    ...
    pub fn retry_command_with_full_binary() -> slug_error::Result<Option<Self>> {
...
/// We can produce a ExitResult from a `slug_error::Result` for convenience.
impl From<slug_error::Result<()>> for ExitResult {
    fn from(e: slug_error::Result<()>) -> Self {
        match e {
            Ok(()) => Self::success(),
            Err(e) => Self::err(e),
        }
...
impl From<slug_error::Result<ExitCode>> for ExitResult {
    fn from(e: slug_error::Result<ExitCode>) -> Self {
        match e {
...
impl From<slug_error::Error> for ExitResult {
    fn from(e: slug_error::Error) -> Self {
        Self::err(e)
    }
}

impl FromResidual<slug_error::Error> for ExitResult {
    #[track_caller]
    fn from_residual(residual: slug_error::Error) -> ExitResult {
        Self::err(residual)
    }
}

impl<E: Into<::slug_error::Error>> FromResidual<Result<Infallible, E>> for ExitResult {
    #[track_caller]
    fn from_residual(residual: Result<Infallible, E>) -> ExitResult {
        match residual {
            // E -> slug_error::Error -> ExitResult
...
impl ExitResultVariant {
    pub fn report(self) -> ! {
        ...
        let mut exit_code = match self {
            Self::Status(v) => v,
            Self::Exec(args) => {
                // Terminate by exec-ing a new process - usually because of `slug run`.
                //
        ...
        // So the result is non-reproducible crash of the slug client.
...
#[derive(slug_error::Error, derivative::Derivative)]
...
pub enum ClientIoError {
    ...
    /// An easy way to trigger this is `slug audit config | head`
    #[slug(tag = IoClientBrokenPipe)]
    #[slug(environment)]
    BrokenPipe(io::Error),
    #[slug(tier0)]
    OtherIo(io::Error),
    #[slug(tier0)]
    Other(slug_error::Error),
}

impl From<slug_error::Error> for ClientIoError {
    fn from(error: slug_error::Error) -> Self {
...
#[cfg(windows)]
fn do_exec(command: &mut Command) -> slug_error::Error {
...
fn execv(args: ExecArgs) -> ! {
    // patternlint-disable-next-line slug-no-command-new
