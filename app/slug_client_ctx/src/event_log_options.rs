...
use slug_event_log::file_names::find_log_by_trace_id;
use slug_event_log::file_names::retrieve_nth_recent_log;
use slug_event_log::read::EventLogPathBuf;
use slug_event_log::utils::Encoding;
use slug_fs::fs_util;
...
impl EventLogOptions {
    pub async fn get(&self, ctx: &ClientCommandContext<'_>) -> slug_error::Result<EventLogPathBuf> {
    ...
    async fn download_remote_id(
        ...
        let log_file_name = FileNameBuf::try_from(format!(
            "{}{}",
            trace_id,
            // TODO(nga): hardcoded default, should at least use the same default slug uses,
        ...
        let (command_name, command) = match ctx.log_download_method()? {
            LogDownloadMethod::Manifold => {
                let args = [
                    "get",
                    &format!("slug_logs/flat/{log_file_name}"),
                    temp_path
                        .path()
                        .as_os_str()
                        .to_str()
