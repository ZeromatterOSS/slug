...
impl ClientMetadata {
    pub fn to_proto(&self) -> slug_data::ClientMetadata {
        slug_data::ClientMetadata {
            key: self.key.clone(),
            value: self.value.clone(),
        }
    }

    pub fn from_env() -> slug_error::Result<Vec<Self>> {
        let client_metadata_str = slug_env!("BUCK2_CLIENT_METADATA")?.unwrap_or_default();
...
pub fn parse_client_metadata(value: &str) -> slug_error::Result<ClientMetadata> {
    ...
    if !REGEX.is_match(key) {
        return Err(
            slug_error::Error::from(ClientMetadataError::InvalidKey(key.to_owned())).into(),
        );
    }
