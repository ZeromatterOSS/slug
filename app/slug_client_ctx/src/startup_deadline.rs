...
/// Utility to time out properly with context during slug client startup.
///
...
impl StartupDeadline {
    pub fn duration_from_now(duration: Duration) -> slug_error::Result<StartupDeadline> {
        Ok(StartupDeadline {
    ...
    pub(crate) fn down_deadline(&self) -> slug_error::Result<StartupDeadline> {
    ...
    pub(crate) fn rem_duration(&self, op: &str) -> slug_error::Result<Duration> {
        self.deadline
    ...
    pub(crate) async fn down<R, Fut, F>(&self, op: &str, f: F) -> slug_error::Result<R>
    where
        F: FnOnce(StartupDeadline) -> Fut,
        Fut: Future<Output = slug_error::Result<R>>,
    {
    ...
    pub(crate) async fn run<R, Fut>(&self, op: &str, f: Fut) -> slug_error::Result<R>
    where
        Fut: Future<Output = slug_error::Result<R>>,
    {
    ...
    pub(crate) fn min(&self, other: Duration) -> slug_error::Result<StartupDeadline> {
    ...
    pub(crate) fn half(&self) -> slug_error::Result<StartupDeadline> {
        let now = Instant::now();
