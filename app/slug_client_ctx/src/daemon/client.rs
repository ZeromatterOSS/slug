...
use futures::pin_mut;
use futures::stream;
use slug_cli_proto::daemon_api_client::*;
use slug_cli_proto::new_generic::NewGenericRequest;
use slug_cli_proto::new_generic::NewGenericResponse;
use slug_cli_proto::*;
...
impl BuckdClientConnector {
    ...
    pub fn daemon_constraints(&self) -> &slug_cli_proto::DaemonConstraints {
...
impl BuckdLifecycleLock {
    ...
    pub async fn lock_with_timeout(
        ...
        async fn lock_inner(
            daemon_dir: DaemonDir,
            deadline: StartupDeadline,
        ) -> slug_error::Result<BuckdLifecycleLock> {
    ...
    pub fn clean_daemon_dir(&self, keep_prev: bool) -> slug_error::Result<()> {
...
pub(crate) fn tonic_status_to_error(status: tonic::Status) -> slug_error::Error {
    ...
    slug_error::Error::from(status).tag(tags)
}

/// Translates a tonic streaming response into a stream of StreamValues, the set of things that can flow across the gRPC
/// event stream.
fn grpc_to_stream(
    response: slug_error::Result<tonic::Response<tonic::Streaming<MultiCommandProgress>>>,
) -> impl Stream<Item = slug_error::Result<StreamValue>> {
    ...
    let stream = stream
        ...
        .map_ok(|e| stream::iter(e.messages.into_iter().map(slug_error::Ok)))
        .try_flatten();
...
impl BuckdClient {
    ...
    ) -> slug_error::Result<CommandOutcome<Res>>
    where
    ...
    pub async fn status<'a>(
    ...
    ) -> slug_error::Result<StatusResponse> {
        let outcome = events_ctx
            ...
            .await;
        // TODO(nmj): We have a number of things that wish to use status() and return an slug_error::Result,
        // for now we'll just turn a "CommandMessage" into a error, but that's really not what we
        // want long term.
        match outcome? {
            CommandOutcome::Success(r) => Ok(r),
            CommandOutcome::Failure(_) => Err(slug_error::slug_error!(
                slug_error::ErrorTag::DaemonStatus,
...
impl TryFrom<slug_cli_proto::partial_result::PartialResult> for NoPartialResult {
    type Error = slug_cli_proto::partial_result::PartialResult;

    fn try_from(v: slug_cli_proto::partial_result::PartialResult) -> Result<Self, Self::Error> {
        Err(v)
    }
}
...
macro_rules! wrap_method {
    ($method: ident ($($param: ident : $param_type: ty),*), $res: ty) => {
        pub async fn $method(&mut self, events_ctx: &mut EventsCtx, $($param: $param_type)*) -> slug_error::Result<$res> {
            let mut events_ctx = DaemonEventsCtx::new(self.inner, events_ctx)?;
            let out = self
                .inner
...
impl FlushingBuckdClient<'_> {
    ...
    stream_method!(bxl, BxlRequest, BxlResponse, slug_cli_proto::StdoutBytes);
    ...
    pub async fn new_generic(
        &mut self,
        context: slug_cli_proto::ClientContext,
        req: NewGenericRequest,
        events_ctx: &mut EventsCtx,
        stdin: Option<ConsoleInteractionStream<'_>>,
    ) -> slug_error::Result<CommandOutcome<NewGenericResponse>> {
        let req = serde_json::to_string(&req)
            .buck_error_context("Could not serialize `NewGenericRequest`")?;
        let req = slug_cli_proto::NewGenericRequestMessage {
            context: Some(context),
            new_generic_request: req,
        };
        let command_outcome: CommandOutcome<slug_cli_proto::NewGenericResponseMessage> = self
