...
use slug_cli_proto::daemon_api_client::DaemonApiClient;
use slug_common::buckd_connection::BUCK_AUTH_TOKEN_HEADER;
use slug_common::buckd_connection::ConnectionType;
use slug_common::client_utils::RetryError;
use slug_common::client_utils::get_channel_tcp;
use slug_common::client_utils::get_channel_uds;
use slug_common::client_utils::retrying;
use slug_common::daemon_dir::DaemonDir;
use slug_common::init::DaemonStartupConfig;
use slug_common::invocation_paths::InvocationPaths;
use slug_core::slug_env;
use slug_data::DaemonWasStartedReason;
...
use slug_resource_control::spawn_daemon::ResourceControlRunner;
use slug_resource_control::spawn_daemon::replace_unit_delimiter;
use slug_util::process::async_background_command;
use slug_util::truncate::truncate;
use slug_wrapper_common::kill::process_exists;
use slug_wrapper_common::pid::Pid;
...
impl ConstraintUnsatisfiedReason {
    pub(crate) fn to_daemon_was_started_reason(&self) -> slug_data::DaemonWasStartedReason {
        match self {
            ConstraintUnsatisfiedReason::Version => {
                slug_data::DaemonWasStartedReason::ConstraintMismatchVersion
            }
            ConstraintUnsatisfiedReason::UserVersion => {
                slug_data::DaemonWasStartedReason::ConstraintMismatchUserVersion
            }
            ConstraintUnsatisfiedReason::StartupConfig => {
                slug_data::DaemonWasStartedReason::ConstraintMismatchStartupConfig
            }
            ConstraintUnsatisfiedReason::RejectDaemonId => {
                slug_data::DaemonWasStartedReason::ConstraintRejectDaemonId
            }
            ConstraintUnsatisfiedReason::TraceIo => {
                slug_data::DaemonWasStartedReason::ConstraintMismatchTraceIo
            }
            ConstraintUnsatisfiedReason::SqliteIdentity => {
                slug_data::DaemonWasStartedReason::ConstraintMismatchSqliteIdentity
            }
        }
    }
}
...
impl DaemonConstraintsRequest {
    ...
    fn satisfied(
        &self,
        daemon: &slug_cli_proto::DaemonConstraints,
...
pub async fn new_daemon_api_client(
    endpoint: ConnectionType,
    auth_token: String,
) -> slug_error::Result<DaemonApiClient<InterceptedService<Channel, BuckAddAuthTokenInterceptor>>> {
...
pub fn buckd_startup_timeout() -> slug_error::Result<Duration> {
    Ok(Duration::from_secs(
        slug_env!("BUCKD_STARTUP_TIMEOUT", type=u64)?.unwrap_or(10),
    ))
}

pub fn buckd_startup_init_timeout() -> slug_error::Result<Duration> {
    Ok(Duration::from_secs(
        slug_env!("BUCKD_STARTUP_INIT_TIMEOUT", type=u64)?.unwrap_or(90),
    ))
}
...
impl<'a> BuckdLifecycle<'a> {
    async fn lock_with_timeout(
        ...
        constraints: &'a DaemonConstraintsRequest,
    ) -> slug_error::Result<BuckdLifecycle<'a>> {
        Ok(BuckdLifecycle::<'a> {
            paths,
    ...
    fn clean_daemon_dir(&self) -> slug_error::Result<()> {
        self.lock
    ...
    async fn start_server(&self) -> slug_error::Result<()> {
        ...
        //   ```
        //   slug --isolation-dir=xx audit providers fbcode//slug:slug --quiet
        //   ```
        ...
        daemon_env_vars.push((
            ...
            OsStr::new(slug_env!("BUCK2_LIB_BACKTRACE")?.unwrap_or("0")),
        ));
    ...
    async fn start_server_unix(
        ...
        if slug_env!("BUCK_DAEMON_LOG_TO_FILE", type=u8)? == Some(1) {
            ...
            match result {
                Err(_elapsed) => {
                    ...
                    child.kill().await.buck_error_context(
                        "Error killing process after slug daemon launch timing out",
                    )?;
                    ...
                    slug_error::Ok(child.wait().await?)
                }
                Ok(result) => result.map_err(|e| slug_error::Error::from(e).into()),
            }
        };
...
impl BuckdChannel {
    ...
    pub async fn upgrade(self) -> slug_error::Result<BootstrapBuckdClient> {
        let Self {
            info,
            daemon_dir,
            mut client,
...
pub struct BootstrapBuckdClient {
    ...
    constraints: slug_cli_proto::DaemonConstraints,
}
...
impl BootstrapBuckdClient {
    pub async fn connect(
        ...
        slug_fs::fs_util::create_dir_all(&daemon_dir.path)
    ...
    pub(crate) async fn kill(&mut self, reason: &str) -> slug_error::Result<Pid> {
        kill::kill(&mut self.client, &self.info, reason).await?;
        Pid::from_i64(self.info.pid)
    }

    async fn kill_for_constraints_mismatch(&mut self) -> slug_error::Result<Pid> {
...
pub async fn connect_buckd(
    ...
    events_ctx: &mut EventsCtx,
    paths: &InvocationPaths,
) -> slug_error::Result<BuckdClientConnector> {
...
pub async fn establish_connection_existing(
    daemon_dir: &DaemonDir,
) -> slug_error::Result<BootstrapBuckdClient> {
...
fn explain_failed_to_connect_reason(reason: slug_data::DaemonWasStartedReason) -> &'static str {
    match reason {
        ...
        DaemonWasStartedReason::NoDaemonProcess => "slug daemon is not running",
    }
}
...
async fn establish_connection_inner(
    ...
    let daemon_was_started_reason = {
        match BuckdProcessInfo::load_if_exists(&daemon_dir) {
            Ok(Some(buckd_info)) => {
                match try_connect_existing(&buckd_info, &deadline, &lifecycle_lock).await {
                    Ok(channel) => {
                        ...
                        events_ctx
                            .eprintln(&format!(
                                "slug daemon constraint mismatch: {reason}; killing daemon..."
                            ))
                            .await?;
                        ...
                        events_ctx.eprintln("Starting new slug daemon...").await?;
                    ...
                    Err(reason) => {
                        events_ctx
                            .eprintln(&format!(
                                "Could not connect to slug daemon ({}), killing daemon..",
            ...
            Ok(None) => {
                ...
                slug_data::DaemonWasStartedReason::NoBuckdInfo
            }
            Err(e) => {
                events_ctx
                    .eprintln(&format!(
                        "Could not load buckd.info: {e}, starting new slug daemon..."
                    ))
                    .await?;

                slug_data::DaemonWasStartedReason::CouldNotLoadBuckdInfo
            }
        }
    };
    ...
    deadline
        .down(
            &format!(
                "starting new slug daemon for reason: {}",
...
async fn start_new_buckd_and_connect(
    ...
    constraints: &DaemonConstraintsRequest,
    events_ctx: &mut EventsCtx,
    daemon_was_started_reason: slug_data::DaemonWasStartedReason,
) -> slug_error::Result<BootstrapBuckdClient> {
    ...
    lifecycle_lock
        .start_server()
        .await
        .buck_error_context("Error starting slug daemon")?;
    ...
    events_ctx.eprintln("Connected to new slug daemon.").await?;
...
) -> Result<BuckdChannel, slug_data::DaemonWasStartedReason> {
    let timeout: slug_error::Result<_> = try { timeout.min(buckd_startup_timeout()?)? };
    let Ok(timeout) = timeout else {
        return Err(slug_data::DaemonWasStartedReason::TimeoutCalculationError);
    };
    let Ok(rem_duration) = timeout.rem_duration("connect existing buckd") else {
        return Err(slug_data::DaemonWasStartedReason::TimedOutConnectingToDaemon);
    };
    match tokio::time::timeout(rem_duration, buckd_info.create_channel()).await {
        Ok(Ok(channel)) => Ok(channel),
        Ok(Err(_)) => {
            let Ok(pid) = buckd_info.pid() else {
                return Err(slug_data::DaemonWasStartedReason::CouldNotLoadBuckdInfo);
            };
            ...
            if !buckd_process_exists {
                ...
                Err(slug_data::DaemonWasStartedReason::NoDaemonProcess)
            } else {
                Err(slug_data::DaemonWasStartedReason::CouldNotConnectToDaemon)
            }
        }
        Err(e) => {
            let _assert_type: tokio::time::error::Elapsed = e;
            Err(slug_data::DaemonWasStartedReason::TimedOutConnectingToDaemon)
        }
    }
}
...
impl<'a> BuckdProcessInfo<'a> {
    ...
    async fn load_and_create_channel(
        daemon_dir: &'a DaemonDir,
    ) -> slug_error::Result<BuckdChannel> {
        Self::load(daemon_dir)?.create_channel().await
    }

    pub fn load(daemon_dir: &'a DaemonDir) -> slug_error::Result<Self> {
    ...
    pub fn load_if_exists(daemon_dir: &'a DaemonDir) -> slug_error::Result<Option<Self>> {
    ...
    pub async fn create_channel(&self) -> slug_error::Result<BuckdChannel> {
    ...
    pub async fn hard_kill(&self) -> slug_error::Result<()> {
        kill::hard_kill(&self.info).await
    }

    pub fn pid(&self) -> slug_error::Result<Pid> {
        Pid::from_i64(self.info.pid)
    }
}
...
async fn get_constraints(
...
) -> slug_error::Result<slug_cli_proto::DaemonConstraints> {
    ...
    let status = DaemonEventsCtx::without_tailers(&mut events_ctx)
        .unpack_oneshot({
            client.status(tonic::Request::new(slug_cli_proto::StatusRequest {
                snapshot: false,
    ...
    let status: slug_cli_proto::StatusResponse = match status {
        CommandOutcome::Success(r) => Ok(r),
        CommandOutcome::Failure(_) => Err(slug_error!(
...
#[allow(clippy::large_enum_variant)]
#[slug(tag = DaemonConnect)]
enum BuckdConnectError {
    #[error(
        "buck daemon startup failed with exit code {code}\nstdout:\n{stdout}\nstderr:\n{stderr}"
    )]
    #[slug(tag = DaemonStartupFailed)]
    BuckDaemonStartupFailed {
        code: String,
        stdout: String,
        stderr: String,
    },
    #[error("Failed to launch Slug daemon: {error:#}")]
    #[slug(tag = DaemonLaunchFailed)]
    ...
    #[slug(tag = DaemonConstraintsWrongAfterStart)]
    BuckDaemonConstraintWrongAfterStart {
        reason: ConstraintUnsatisfiedReason,
        expected: DaemonConstraintsRequest,
        actual: slug_cli_proto::DaemonConstraints,
    },
    #[error("slug daemon constraint mismatch during nested invocation: {reason}")]
    #[slug(tag = DaemonNestedConstraintsMismatch)]
    NestedConstraintMismatch { reason: ConstraintUnsatisfiedReason },
    #[error("buckd info {path} does not exist")]
    #[slug(tag = BuckdInfoMissing)]
    BuckdInfoMissing { path: AbsNormPathBuf },
    #[error("Error parsing daemon info in `{}`. \
                Try deleting that file and running `slug killall` before running your command again",
                location.display())]
    #[slug(tag = BuckdInfoParseError)]
    BuckdInfoParseError {
...
async fn daemon_connect_error(
    ...
    let error_report: Result<slug_data::ErrorReport, RetryError<slug_error::Error>> = retrying(
        ...
        || async {
            ...
            let error_report = slug_data::ErrorReport::deserialize(
    ...
    let error = if let Ok(error_report) = error_report {
        ...
        let daemon_error: slug_error::Error = error_report.into();
    ...
    // FIXME(ctolliday) we should check if the pid at daemon_dir.buckd_pid is still running before suggesting slug kill.
    let error_message = format!(
        "Failed to connect to buck daemon.
    Try running `slug kill` and your command afterwards.
...
fn is_nested_invocation(
    slug_daemon_uuid: Option<&String>,
    daemon: &slug_cli_proto::DaemonConstraints,
) -> bool {
    slug_daemon_uuid == Some(&daemon.daemon_id)
}
...
mod tests {
    ...
    fn constraints(trace_io_enabled: bool) -> slug_cli_proto::DaemonConstraints {
        slug_cli_proto::DaemonConstraints {
            version: "version".to_owned(),
            user_version: Some("test".to_owned()),
            daemon_id: "foo".to_owned(),
            extra: Some(slug_cli_proto::ExtraDaemonConstraints {
                trace_io_enabled,
    ...
    fn test_reject_daemon() {
        ...
        let daemon = slug_cli_proto::DaemonConstraints {
            version: "foo".to_owned(),
            user_version: None,
