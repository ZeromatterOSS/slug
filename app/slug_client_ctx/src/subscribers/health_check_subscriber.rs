...
use slug_health_check::health_check_client::HealthCheckClient;
use slug_health_check::health_check_client::StreamingHealthCheckClient;
use slug_health_check::interface::HealthCheckContextEvent;
use slug_health_check::interface::HealthCheckEvent;
...
impl HealthCheckSubscriber {
    ...
    async fn handle_event(&mut self, event: &Arc<BuckEvent>) -> slug_error::Result<()> {
        ...
        let health_check_event = match event.data() {
            SpanStart(start) => match &start.data {
                Some(slug_data::span_start_event::Data::Command(command)) => {
                    Some(HealthCheckEvent::HealthCheckContextEvent(
                        HealthCheckContextEvent::CommandStart(slug_data::CommandStartWithTraceId {
            ...
            Instant(instant) => {
                    ...
                    .buck_error_context("Missing `data` in `Instant`")?
                {
                    ...
                    Snapshot(_snapshot) => {
                        ...
                        let snapshot_data = slug_health_check::interface::HealthCheckSnapshotData {
    ...
    pub fn close_client_connection_with_error_report(&mut self, error: &str) {
        ...
        let _ignored = soft_error!(
            ...
            slug_error::slug_error!(slug_error::ErrorTag::HealthCheck, "{}", error)
        );
    }
}
...
mod tests {
    ...
    use slug_health_check::report::DisplayReport;
    use slug_health_check::report::HealthIssue;
    use slug_health_check::report::Message;
    use slug_health_check::report::Severity;
    ...
    fn test_event(data: slug_data::buck_event::Data) -> Arc<BuckEvent> {
    ...
    fn event_for_excess_cache_miss() -> slug_data::buck_event::Data {
        let action_end = ActionExecutionEnd {
            kind: ActionKind::Run as i32,
            invalidation_info: Some(slug_data::CommandInvalidationInfo {
                changed_file: None,
                changed_any: None,
            }),
            ..Default::default()
        ...
        slug_data::buck_event::Data::SpanEnd(slug_data::SpanEndEvent {
            data: Some(slug_data::span_end_event::Data::ActionExecution(
                Box::new(action_end).into(),
            )),
            ..slug_data::SpanEndEvent::default()
        })
    }

    #[tokio::test]
    async fn test_health_check_subscriber() -> slug_error::Result<()> {
        ...
        let event = test_event(
            slug_data::InstantEvent {
                data: Some(Box::new(slug_data::Snapshot::default()).into()),
            }
            .into(),
        );
    ...
    #[tokio::test]
    async fn test_event_limit_exceeded() -> slug_error::Result<()> {
        let (events_tx, mut events_rx) = mpsc::channel::<HealthCheckEvent>(EVENT_CHANNEL_SIZE);
        ...
        let event = test_event(slug_data::buck_event::Data::Instant(
            slug_data::InstantEvent {
                data: Some(Box::new(slug_data::Snapshot::default()).into()),
            },
        ));
    ...
    #[tokio::test]
    async fn test_slow_client() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_dropped_client() -> slug_error::Result<()> {
        let (events_tx, events_rx_guard) = mpsc::channel::<HealthCheckEvent>(EVENT_CHANNEL_SIZE);
