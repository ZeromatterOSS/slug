...
/// Subscribes to Slug's event stream and mirrors each event into one or more
/// BEP-format files.
...
impl BepFileSubscriber {
    ...
    fn write_event(&mut self, event: &slug_data::BuckEvent) {
        // See bep_bes_sink: slug-side stats (critical path,
        // BuildGraph node/edge counts, Command span timestamps) feed
        // BuildMetrics but don't translate to BEP.
        self.stream_state.observe_slug_event(event);
