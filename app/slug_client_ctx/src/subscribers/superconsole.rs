...
use slug_event_observer::action_sub_error_display::ActionSubErrorDisplay;
use slug_event_observer::display;
use slug_event_observer::display::TargetDisplayOptions;
use slug_event_observer::display::display_file_watcher_end;
use slug_event_observer::event_observer::DebugEventObserverExtra;
use slug_event_observer::session_info::SessionInfo;
use slug_event_observer::unpack_event::VisitorError;
use slug_event_observer::unpack_event::unpack_event;
use slug_event_observer::verbosity::Verbosity;
use slug_event_observer::what_ran::command_to_string;
use slug_event_observer::what_ran::worker_command_as_fallback_to_string;
...
impl Component for BuckRootComponent<'_> {
    ...
    fn draw_unchecked(&self, dimensions: Dimensions, mode: DrawMode) -> slug_error::Result<Lines> {
...
impl StatefulSuperConsole {
    ...
    /// Construct a console suitable for use by the Slug CLI. We use non-blocking output here
    ...
    pub fn render_result_errors(result: &slug_cli_proto::CommandResult) -> Lines {
        let mut lines = Lines::new();
        if let slug_cli_proto::CommandResult {
            result: Some(slug_cli_proto::command_result::Result::Error(e)),
        } = result
        {
    ...
    async fn handle_event(&mut self, ev: &Arc<BuckEvent>) -> slug_error::Result<()> {
        match self {
    ...
    fn finalize(&mut self) -> slug_error::Result<()> {
        let mut res = Ok(());
...
impl SuperConsoleState {
    pub fn new(
        ...
        config: SuperConsoleConfig,
        health_check_reports_receiver: Option<Receiver<Vec<DisplayReport>>>,
    ) -> slug_error::Result<SuperConsoleState> {
...
impl StatefulSuperConsoleImpl {
    ...
    async fn handle_inner_event(&mut self, event: &BuckEvent) -> slug_error::Result<()> {
        match unpack_event(event)? {
            slug_event_observer::unpack_event::UnpackedBuckEvent::SpanStart(_, _, _) => Ok(()),
            slug_event_observer::unpack_event::UnpackedBuckEvent::SpanEnd(_, _, data) => match data
            {
            ...
            slug_event_observer::unpack_event::UnpackedBuckEvent::Instant(_, _, data) => match data
            {
                slug_data::instant_event::Data::ConsoleMessage(message) => {
                    self.handle_console_message(message).await
                }
                slug_data::instant_event::Data::ConsoleWarning(message) => {
                ...
                slug_data::instant_event::Data::ConsolePreferences(preferences) => {
                    self.handle_console_preferences(preferences).await
                }
                slug_data::instant_event::Data::ActionError(error) => {
                    self.handle_action_error(error).await
                }
                slug_data::instant_event::Data::StreamingOutput(message) => {
                    self.handle_streaming_output(message).await
                }
                _ => Ok(()),
            },
            slug_event_observer::unpack_event::UnpackedBuckEvent::UnrecognizedSpanStart(_, _)
            | slug_event_observer::unpack_event::UnpackedBuckEvent::UnrecognizedSpanEnd(_, _)
            | slug_event_observer::unpack_event::UnpackedBuckEvent::UnrecognizedInstant(_, _) => {
                Err(VisitorError::MissingField(event.clone()).into())
            }
        }
    }

    async fn handle_stderr(&mut self, msg: &str) -> slug_error::Result<()> {
        self.super_console
    ...
    async fn handle_console_message(
        &mut self,
        message: &slug_data::ConsoleMessage,
    ) -> slug_error::Result<()> {
    ...
    async fn handle_streaming_output(
        &mut self,
        message: &slug_data::StdoutStreamingOutput,
    ) -> slug_error::Result<()> {
        self.super_console
    ...
    async fn handle_console_warning(
        &mut self,
        message: &slug_data::ConsoleWarning,
    ) -> slug_error::Result<()> {
        let style = ContentStyle {
    ...
    async fn handle_action_error(
        ...
        if let Some(error_diagnostics) = error_diagnostics {
            match error_diagnostics.data.as_ref().unwrap() {
                slug_data::action_error_diagnostics::Data::SubErrors(sub_errors) => {
                ...
                slug_data::action_error_diagnostics::Data::HandlerInvocationError(error) => {
    ...
    async fn handle_console_preferences(
        &mut self,
        prefs: &slug_data::ConsolePreferences,
    ) -> slug_error::Result<()> {
    ...
    async fn tick(&mut self, tick: &Tick) -> slug_error::Result<()> {
    ...
    fn finalize(
        ...
        SuperConsoleState,
        Option<superconsole::Error<slug_error::Error>>,
    ) {
        let err = self
            .super_console
...
impl EventSubscriber for StatefulSuperConsole {
    ...
    async fn handle_output(&mut self, raw_output: &[u8]) -> slug_error::Result<()> {
        self.finalize()?;
        match self {
    ...
    async fn handle_error(&mut self, _error: &slug_error::Error) -> slug_error::Result<()> {
        self.finalize()?;
        Ok(())
    }
}
    ...
    if let Some(command_kind) = command_failed.command_kind.as_ref() {
        ...
        match command_kind.command.as_ref() {
                ...
                let command = if verbosity.print_failure_full_command() {
                    ...
                    match truncate(command) {
                        ...
                        Some(short) => Cow::Owned(format!(
                            "{short} (run `slug log what-failed` to get the full command)"
                        )),
                    }
                };
            ...
            Some(Command::RemoteCommand(remote_command)) => {
                let help_message = if slug_core::is_open_source() {
...
mod tests {
    ...
    use slug_cli_proto::GenericResponse;
    use slug_data::LoadBuildFileEnd;
    use slug_data::LoadBuildFileStart;
    ...
    #[tokio::test]
    async fn test_transfer_state_to_simpleconsole() -> slug_error::Result<()> {
        ...
        let event = Arc::new(BuckEvent::new(
            ...
            Some(id),
            None,
            slug_data::buck_event::Data::SpanStart(SpanStartEvent {
                data: Some(slug_data::span_start_event::Data::Load(
                    LoadBuildFileStart {
        ...
        console
            .handle_command_result(&CommandResult {
                result: Some(slug_cli_proto::command_result::Result::GenericResponse(
                    GenericResponse {},
                )),
            })
            .await
        ...
        let event = Arc::new(BuckEvent::new(
            ...
            Some(id),
            None,
            slug_data::buck_event::Data::SpanEnd(SpanEndEvent {
                data: Some(slug_data::span_end_event::Data::Load(LoadBuildFileEnd {
                    module_id: "foo".to_owned(),
                    cell: "bar".to_owned(),
                    error: None,
    ...
    #[tokio::test]
    async fn test_default_layout() -> slug_error::Result<()> {
        let trace_id = TraceId::new();
        ...
        console
            .handle_event(&Arc::new(BuckEvent::new(
                ...
                slug_data::buck_event::Data::SpanStart(SpanStartEvent {
                    data: Some(
                        slug_data::CommandStart {
                            data: Some(slug_data::BuildCommandStart {}.into()),
                            ..Default::default()
                        }
                        .into(),
                    ),
        ...
        console
            .handle_event(&Arc::new(BuckEvent::new(
                ...
                None,
                None,
                slug_data::InstantEvent {
                    data: Some(
                        slug_data::RemoteExecutionSessionCreated {
        ...
        console
            .handle_command_result(&slug_cli_proto::CommandResult { result: None })
            .await?;
    ...
    #[test]
    fn test_session_info() -> slug_error::Result<()> {
        let info = SessionInfo {
            trace_id: TraceId::null(),
            test_session: Some(slug_data::TestSessionInfo {
    ...
    #[tokio::test]
    async fn test_tailer_stderr() -> slug_error::Result<()> {
        let trace_id = TraceId::new();
        let tick = Tick::now();
