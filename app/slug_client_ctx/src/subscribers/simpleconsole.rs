...
use slug_event_observer::event_observer::EventObserver;
use slug_event_observer::event_observer::EventObserverExtra;
...
use slug_event_observer::what_ran::WhatRanCommandConsoleFormat;
...
/// slug daemon info is printed to stderr if there are no other updates available
...
macro_rules! echo {
    () => {
        {
            // patternlint-disable-next-line slug-cli-simpleconsole-echo
...
fn echo_system_warning_exponential(warning: &HealthCheckType, msg: &str) -> slug_error::Result<()> {
...
pub struct SimpleConsole<E> {
    ...
    action_errors: Vec<slug_data::ActionError>,
    ...
    E: EventObserverExtra,
{
    ...
    fn print_stats_while_waiting(&mut self) -> slug_error::Result<()> {
        ...
        if is_snapshot_stale {
            ...
            if let Some((_, snapshot)) = &snapshots.last {
                if let Some(slug_rss) = snapshot.slug_rss {
                    parts.push(format!("RSS: {}", HumanizedBytes::new(slug_rss)));
                }
            }
    ...
    fn print_action_error(&mut self, error: &slug_data::ActionError) -> slug_error::Result<()> {
    ...
    pub(crate) async fn handle_event(&mut self, event: &Arc<BuckEvent>) -> slug_error::Result<()> {
    ...
    async fn handle_event_inner(&mut self, event: &BuckEvent) -> slug_error::Result<()> {
        match unpack_event(event)? {
            slug_event_observer::unpack_event::UnpackedBuckEvent::SpanStart(_, _, data) => {
                match data {
            ...
            slug_event_observer::unpack_event::UnpackedBuckEvent::Instant(_, _, data) => match data
            {
                ...
                slug_data::instant_event::Data::TagEvent(tags) => {
    ...
    async fn handle_command_start(
        &mut self,
        _command: &slug_data::CommandStart,
        event: &BuckEvent,
    ) -> slug_error::Result<()> {
        if cfg!(fbcode_build) {
            echo!(
                "Buck UI: https://www.internalfb.com/slug/{}",
                event.trace_id()?
            )?;
        } else {
    ...
    async fn handle_command_end(
        ...
        if self.verbosity.print_status() && self.observer().action_stats().log_stats() {
            ...
            // line. Set SLUG_BUILD_SUMMARY=off to suppress. std::mem::take the
            ...
            if slug_env!("SLUG_BUILD_SUMMARY").unwrap_or(None).as_deref() != Some("off") {
    ...
    async fn handle_test_discovery(
        ...
        if let Some(data) = &test_info.data {
            match data {
                slug_data::test_discovery::Data::Session(slug_data::TestSessionInfo { info }) => {
                    echo!("Test session: {}", info)?;
                    self.notify_printed();
                }
                slug_data::test_discovery::Data::Tests(..) => {}
            }
        }
    ...
    pub(crate) async fn handle_stderr(&mut self, stderr: &str) -> slug_error::Result<()> {
        echo!("{}", stderr)?;
        self.notify_printed();
        Ok(())
    }
    ...
    fn echo_health_check_warning(&self, report: &DisplayReport) -> slug_error::Result<()> {
    ...
    E: EventObserverExtra,
{
    ...
    async fn tick(&mut self, _: &Tick) -> slug_error::Result<()> {
        if self.verbosity.print_status()
...
impl WhatRanOutputWriter for PrintDebugCommandToStderr {
    fn emit_command(&mut self, command: WhatRanOutputCommand<'_>) -> slug_error::Result<()> {
        echo!(
            "{}",
