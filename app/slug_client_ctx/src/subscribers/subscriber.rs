...
pub trait EventSubscriber: Send {
    ...
    /// stdout.
    async fn handle_output(&mut self, _raw_output: &[u8]) -> slug_error::Result<()> {
        Ok(())
    }
    /// Fired by the tailer for stderr.
    async fn handle_tailer_stderr(&mut self, _stderr: &str) -> slug_error::Result<()> {
        Ok(())
    }
    ...
    async fn handle_events(&mut self, _event: &[Arc<BuckEvent>]) -> slug_error::Result<()> {
        Ok(())
    }
    async fn handle_command_result(
        &mut self,
        _result: &slug_cli_proto::CommandResult,
    ) -> slug_error::Result<()> {
        Ok(())
    }
    ...
    async fn tick(&mut self, _tick: &Tick) -> slug_error::Result<()> {
        Ok(())
    }
    ...
    fn handle_daemon_started(&mut self, _reason: slug_data::DaemonWasStartedReason) {}
