...
use slug_build_event_stream::grpc_sink::BesConfig;
use slug_build_event_stream::grpc_sink::BesSink;
use slug_build_event_stream::grpc_sink::UploadMode;
use slug_build_event_stream::translate::BepStreamState;
...
enum State {
    ...
    Connecting(JoinHandle<slug_error::Result<BesSink>>),
...
impl BesSubscriber {
    ...
    pub fn maybe_new(
        ...
        // here so a vanilla `slug build … --config=remote` running in a
        ...
        let config = BesConfig {
            ...
            // until Slug grows a real notion of retry-within-a-build.
...
fn is_command_end(event: &slug_data::BuckEvent) -> bool {
    matches!(
        event.data.as_ref(),
        Some(slug_data::buck_event::Data::SpanEnd(se))
            if matches!(
                se.data.as_ref(),
                Some(slug_data::span_end_event::Data::Command(_))
            )
    )
}

fn parse_kv(entry: &str) -> slug_error::Result<(String, String)> {
