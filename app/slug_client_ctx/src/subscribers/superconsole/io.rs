...
use slug_event_observer::two_snapshots::TwoSnapshots;
...
fn do_render(
    ...
    if let Some(slug_rss) = snapshot.slug_rss {
        ...
        // slug_rss is only available on Linux. On other platforms, slug keeps track of slug_max_rss so show that instead.
        parts.push(format!(
            "Max RSS = {}",
            HumanizedBytes::new(snapshot.slug_max_rss)
        ));
    }

    // We prefer to display malloc_bytes_active instead of malloc_bytes_allocated
    // because it represents active pages which is more than allocated and better reflects actual memory use of slug.
