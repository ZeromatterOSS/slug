...
use slug_event_observer::span_tracker::BuckEventSpanHandle;
use slug_event_observer::span_tracker::BuckEventSpanTracker;
...
impl TimedListBody<'_> {
    ...
    fn draw_root(&self, root: &BuckEventSpanHandle) -> slug_error::Result<Vec<TimedRow>> {
...
impl Component for TimedListHeader {
    ...
    fn draw_unchecked(&self, dimensions: Dimensions, _mode: DrawMode) -> slug_error::Result<Lines> {
...
mod tests {
    ...
    use slug_data::FakeStart;
    use slug_data::SpanStartEvent;
    use slug_event_observer::action_stats::ActionStats;
    ...
    #[test]
    fn test_normal() -> slug_error::Result<()> {
        let tick = Tick::now();
        ...
        let label = Arc::new(BuckEvent::new(
            ...
            slug_data::buck_event::Data::SpanStart(SpanStartEvent {
                data: Some(slug_data::span_start_event::Data::Fake(FakeStart {
    ...
    #[test]
    fn test_remaining() -> slug_error::Result<()> {
        let tick = Tick::now();
    ...
    #[tokio::test]
    async fn test_remaining_with_pending() -> slug_error::Result<()> {
        let tick = Tick::now();
    ...
    #[test]
    fn test_children() -> slug_error::Result<()> {
        let tick = Tick::now();
        ...
        let prepare = Arc::new(BuckEvent::new(
            ...
            SpanStartEvent {
                data: Some(
                    slug_data::ExecutorStageStart {
                        stage: Some(slug_data::PrepareAction {}.into()),
                    }
                    .into(),
                ),
            }
            ...
            SpanStartEvent {
                data: Some(
                    slug_data::ExecutorStageStart {
                        stage: Some(
                            slug_data::ReStage {
                                stage: Some(slug_data::ReDownload {}.into()),
                            }
                            .into(),
                        ),
                    }
    ...
    fn dice_snapshot(time: SystemTime) -> Arc<BuckEvent> {
        Arc::new(BuckEvent::new(
            ...
            slug_data::InstantEvent {
                data: Some(
                    slug_data::DiceStateSnapshot {
                        key_states: {
                            let mut map = HashMap::new();
                            map.insert(
                                "BuildKey".to_owned(),
                                slug_data::DiceKeyState {
                                    started: 5,
                                    finished: 2,
    ...
    fn span_start_event(parent_span: Option<SpanId>, time: SystemTime) -> Arc<BuckEvent> {
        ...
        Arc::new(BuckEvent::new(
            ...
            SpanStartEvent {
                data: Some(
                    slug_data::ActionExecutionStart {
                        key: Some(slug_data::ActionKey {
                            id: Default::default(),
                        ...
                        kind: slug_data::ActionKind::NotSet as i32,
                    }
                    .into(),
                ),
            }
