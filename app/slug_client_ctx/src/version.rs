...
impl BuckVersion {
    ...
    fn extract_unique_id(file: &object::File) -> Option<String> {
        if let Ok(Some(build_id)) = file.build_id() {
            ...
            slug_build_info::win_internal_version().map(|s| s.to_owned())
        } else {
            None
        }
    }
    ...
    fn compute() -> BuckVersion {
        ...
        let file_m = match unsafe { memmap2::Mmap::map(&file) } {
            Ok(mmap) => mmap,
            Err(err) => {
                panic!("Failed to map slug binary for version extraction: {err:?}");
            }
        };
        ...
        let file_object = match object::File::parse(&*file_m) {
            Ok(file) => file,
            Err(err) => {
                panic!("Failed to parse slug file for version extraction: {err:?}");
            }
        };
            ...
            Self::extract_unique_id(&file_object)
        {
            ...
            if !(slug_core::is_open_source() || slug_env!("BUCK2_IGNORE_VERSION_EXTRACTION_FAILURE", type=bool, default=false, applicability=testing).unwrap_or(false)) {
                let _ignored = crate::eprintln!(
                    "version extraction failed. This indicates an issue with the slug release, will fallback to binary hash"
                );
            }
            (Self::hash_binary(&mut file), "<exe-hash>")
        };

        let version = if let Some(version) = slug_build_info::revision() {
            version.to_owned()
        } else {
