...
mod interactive_terminal {
    ...
    impl InteractiveTerminal {
        pub fn enable() -> slug_error::Result<Option<Self>> {
        ...
        pub fn disable(&mut self) -> slug_error::Result<()> {
            let fd = std::io::stdin().as_raw_fd();
...
mod interactive_terminal {
    ...
    fn get_console_mode(handle: HANDLE) -> slug_error::Result<DWORD> {
    ...
    fn set_console_mode(handle: HANDLE, mode: DWORD) -> slug_error::Result<()> {
...
pub trait SuperConsoleInteraction: Send + Sync {
    async fn toggle(&mut self) -> slug_error::Result<Option<SuperConsoleToggle>>;
}
...
impl SuperConsoleInteraction for ConsoleInteractionStream<'_> {
    async fn toggle(&mut self) -> slug_error::Result<Option<SuperConsoleToggle>> {
        match self.stdin.read_u8().await {
            ...
            Err(e) => Err(slug_error::Error::from(e).context("Error reading char from console")),
        }
    }
}
