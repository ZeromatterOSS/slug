...
enum ArgExpansionError {
    ...
    StdinReadError { source: slug_error::Error },
}
...
pub fn log_relative_path_from_cell_root(requested_path: &str) -> slug_error::Result<()> {
...
mod tests {
    ...
    #[test]
    fn test_resolve_argfile_kind_with_project() -> slug_error::Result<()> {
    ...
    #[test]
    fn test_expand_argfile_content() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
        let cwd =
    ...
    #[test]
    fn test_comment() -> slug_error::Result<()> {
        let content = indoc! {"
            --# Usage: slug build @mode/mrustc ...
            --#
