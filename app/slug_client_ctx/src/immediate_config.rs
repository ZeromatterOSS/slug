...
pub struct ImmediateConfigContext<'a> {
    ...
    // us to have a shared reference to the underlying `slug_error::Error` which
...
impl<'a> ImmediateConfigContext<'a> {
    ...
    pub fn daemon_startup_config(&self) -> slug_error::Result<&DaemonStartupConfig> {
    ...
    pub fn resolve_alias_to_path_in_cwd(&self, alias: &str) -> slug_error::Result<CellRootPathBuf> {
        let data = self.data()?;
        let cell = data.cwd_cell_alias_resolver.resolve(alias)?;
        Ok(data.cell_resolver.get(cell)?.path().to_buf())
    }

    fn data(&self) -> slug_error::Result<&ImmediateConfigContextData> {
        self.data
            .get_or_try_init(|| {
                ...
                slug_error::Ok(ImmediateConfigContextData {
    ...
    pub(crate) fn resolve_argfile_kind(
        ...
        flag: Option<&str>,
    ) -> Result<slug_common::argv::ArgFileKind, slug_error::Error> {
...
fn is_paranoid_enabled(path: &AbsPath) -> slug_error::Result<bool> {
    if let Some(p) = slug_env!("BUCK_PARANOID", type=bool)? {
        return Ok(p);
    }
    ...
    let info = slug_cli_proto::ParanoidInfo::decode(bytes.as_slice())
