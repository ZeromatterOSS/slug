...
enum BuckdCommunicationError {
    ...
    #[error(
        "The Slug daemon was shut down while executing your command. This happened because: {0}"
    )]
    #[slug(tag = InterruptedByDaemonShutdown)]
    InterruptedByDaemonShutdown(slug_data::DaemonShutdown),
...
pub trait PartialResultHandler {
    type PartialResult: TryFrom<
            slug_cli_proto::partial_result::PartialResult,
            Error = slug_cli_proto::partial_result::PartialResult,
        >;
...
impl PartialResultCtx<'_> {
    pub async fn stdout(&mut self, bytes: &[u8]) -> slug_error::Result<()> {
        self.inner
...
/// Manages incoming event streams from the daemon for the slug client and
...
impl<'a> DaemonEventsCtx<'a> {
    ...
    async fn handle_stream_next<Handler>(
        ...
        next: Option<Vec<slug_error::Result<StreamValue>>>,
        shutdown: &mut Option<slug_data::DaemonShutdown>,
    ) -> slug_error::Result<ControlFlow<Box<CommandResult>, ()>>
    where
    ...
    async fn dispatch_tailer_event(&mut self, event: FileTailerEvent) -> slug_error::Result<()> {
        match event {
            FileTailerEvent::Stdout(out) | FileTailerEvent::Stderr(out) => {
                ...
                // But if daemon does, it should not be used as standard output of slug.
    ...
    where
        S: Stream<Item = slug_error::Result<StreamValue>>,
        Handler: PartialResultHandler,
    {
        ...
        let command_result: slug_error::Result<CommandResult> = try {
            loop {
                tokio::select! {
                    next = stream.next() => {
    ...
    async fn handle_error_owned(&mut self, error: slug_error::Error) -> slug_error::Error {
        let error: slug_error::Error = error.into();
        let result: Result<(), slug_error::Error> = self
            .inner
    ...
    pub async fn flush(&mut self) -> slug_error::Result<()> {
...
fn convert_result<R: TryFrom<command_result::Result, Error = command_result::Result>>(
    value: CommandResult,
) -> slug_error::Result<CommandOutcome<R>> {
    match value.result {
...
impl EventsCtx {
    ...
    async fn handle_tailer_stderr(&mut self, stderr: &[u8]) -> slug_error::Result<()> {
    ...
    async fn handle_events(
        ...
        let events = events.into_map(|mut event| {
            ...
            if let slug_data::buck_event::Data::Instant(instant_event) = event.data_mut() {
                match &mut instant_event.data {
                    ...
                    Some(slug_data::instant_event::Data::DaemonShutdown(msg)) => {
    ...
    where
        Fut: Future<Output = slug_error::Result<()>> + 'b,
    {
    ...
    pub(crate) fn handle_daemon_started(&mut self, reason: slug_data::DaemonWasStartedReason) {
    ...
    pub(crate) async fn eprintln(&mut self, message: &str) -> slug_error::Result<()> {
