...
//! build --jobs=8          # applies to `slug build` only
//! test --test_output=all  # applies to `slug test` only
...
/// Known slug subcommands (used to find injection point in argv).
...
/// Normalize a Bazel-style flag argument to slug-compatible form.
///
/// Bazel allows both `--keep_going` and `--keep-going` interchangeably.
/// Slug's clap flags use hyphens, so we normalize underscores → hyphens
...
/// strip Bazel-only transitional flags that slug doesn't understand.
///
/// Bazel treats `--flag_name` and `--flag-name` as equivalent; slug uses hyphens.
///
/// Transitional flags like `--incompatible_*` and `--noincompatible_*` are Bazel
/// migration flags that enable future-default behavior. Slug already implements
...
/// Returns true for Bazel-only transitional flags that slug should silently ignore.
///
/// These include:
...
fn is_bazel_transitional_flag(arg: &str) -> bool {
    ...
    // Bazel-specific flags that have no slug equivalent and should be silently dropped.
...
/// remote execution details, compilation modes) that have no direct slug
/// equivalent. Silently dropping them allows slug to work with existing
...
fn is_bazel_specific_flag(normalized_flag: &str) -> bool {
    ...
    matches!(
        base,
        // Workspace/bzlmod toggles (slug always uses bzlmod)
        "enable_workspace"
            | "enable_bzlmod"
...
/// ```text
/// slug build --from-cmdline
/// # ~/.bazelrc: build --from-bazelrc
/// # becomes: slug build --from-bazelrc --from-cmdline
/// ```
...
mod tests {
    ...
    fn test_is_bazel_transitional_flag() {
        ...
        // experimental_* flags are stripped unless Slug models their Starlark-visible semantics.
        assert!(is_bazel_transitional_flag(
    ...
    fn test_normalize_args_strips_transitional() {
        ...
        let result = normalize_args(args);
        assert_eq!(result, vec!["slug", "build", "//..."]);
    }
    ...
    fn filter_handles_two_arg_config_form() {
        ...
        assert_eq!(
            args,
            strs(vec!["slug", "build", "--config", "cell.foo=bar", "//x"])
        );
    }
    ...
    fn filter_strips_enable_platform_specific_config() {
        ...
        filter_bazelrc_only_cli_flags(&mut args);
        assert_eq!(args, strs(vec!["slug", "build", "//x"]));
    }

    #[test]
    fn filter_preserves_bare_short_c_flag() {
        // `-c` is buck-only; never mis-identified.
        let mut args = strs(vec!["slug", "build", "-c", "cell.foo=bar", "//x"]);
        filter_bazelrc_only_cli_flags(&mut args);
        assert_eq!(
            args,
            strs(vec!["slug", "build", "-c", "cell.foo=bar", "//x"])
        );
    }
    ...
    fn filter_ignores_trailing_bare_config_without_value() {
        ...
        let mut args = strs(vec!["slug", "build", "//x", "--config"]);
        filter_bazelrc_only_cli_flags(&mut args);
        assert_eq!(args, strs(vec!["slug", "build", "//x", "--config"]));
    }
}
