...
use slug_core::error::slug_hard_error_env;
...
pub struct ClientCommandContext<'a> {
    ...
    pub(crate) start_in_process_daemon:
        Option<Box<dyn FnOnce() -> slug_error::Result<()> + Send + Sync>>,
    pub(crate) argv: Argv,
...
impl<'a> ClientCommandContext<'a> {
    pub fn new(
        ...
        start_in_process_daemon: Option<Box<dyn FnOnce() -> slug_error::Result<()> + Send + Sync>>,
        argv: Argv,
        trace_id: TraceId,
        stdin: &'a mut Stdin,
    ...
    pub fn paths(&self) -> slug_error::Result<&InvocationPaths> {
        match &self.paths {
    ...
    pub fn maybe_paths(&self) -> slug_error::Result<Option<&InvocationPaths>> {
        match &self.paths {
    ...
    pub fn client_context<T: StreamingCommand>(
        ...
        cmd: &T,
    ) -> slug_error::Result<ClientContext> {
    ...
    pub fn empty_client_context(&self, command_name: &str) -> slug_error::Result<ClientContext> {
        ...
        Ok(ClientContext {
            ...
            argfiles: Vec::new(),
            slug_hard_error: slug_hard_error_env()?.unwrap_or_default().to_owned(),
    ...
    pub fn log_download_method(&self) -> slug_error::Result<LogDownloadMethod> {
        Ok(self
            .immediate_config
