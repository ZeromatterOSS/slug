...
impl Invocation {
    ...
    pub(crate) fn parse_json_line(json: &str) -> kuro_error::Result<Invocation> {
        let i = serde_json::from_str::<kuro_data::Invocation>(json)
            .with_buck_error_context(|| format!("Invalid header: {}", json.trim_end()))?;
        Ok(Invocation::from_proto(i))
    }

    pub fn to_proto(self) -> kuro_data::Invocation {
        kuro_data::Invocation {
    ...
    pub(crate) fn from_proto(proto: kuro_data::Invocation) -> Self {
        Invocation {
...
mod tests {
    ...
    fn test_parse_json_line() {
        ...
        let line = r#"{"command_line_args":["/some/path/kuro","test","@//mode/mac","app/..."],"working_dir":"/Users/nga/dir45","trace_id":"281d1c16-8930-40cd-8fc1-7d71355c20f5"}"#;
        let line = Invocation::parse_json_line(line).unwrap();
        let expected = Invocation {
            command_line_args: vec![
                "/some/path/kuro".to_owned(),
                "test".to_owned(),
                "@//mode/mac".to_owned(),
                "app/...".to_owned(),
            ],
