...
impl EventLogPathBuf {
    pub fn infer(path: AbsPathBuf) -> kuro_error::Result<Self> {
    ...
    fn file_name(path: &AbsPathBuf) -> kuro_error::Result<&str> {
        let name = path
            .file_name()
    ...
    pub fn uuid_from_filename(&self) -> kuro_error::Result<TraceId> {
    ...
    pub fn command_from_filename(&self) -> kuro_error::Result<&str> {
    ...
    pub(crate) fn infer_opt(path: &AbsPathBuf) -> kuro_error::Result<Option<Self>> {
    ...
    ) -> kuro_error::Result<(Invocation, BoxStream<'a, kuro_error::Result<StreamValue>>)> {
    ...
    async fn unpack_stream_protobuf<'a>(
        ...
        let invocation = kuro_data::Invocation::decode_length_delimited(invocation)
        ...
        let events = stream.and_then(|data| async move {
            let val = kuro_cli_proto::CommandProgress::decode_length_delimited(data)
            ...
            match val.progress {
                ...
                None => Err(kuro_error::kuro_error!(
                    kuro_error::ErrorTag::InvalidEvent,
    ...
    async fn unpack_stream_inner<'a>(
        ...
        Invocation,
        impl Stream<Item = kuro_error::Result<StreamValue>> + use<'a>,
    )> {
    ...
    pub async fn unpack_stream(
        ...
        Invocation,
        impl Stream<Item = kuro_error::Result<StreamValue>> + 'static + use<>,
    )> {
    ...
    async fn open<'a>(
        &self,
        stats: Option<&'a ReaderStats>,
    ) -> kuro_error::Result<EventLogReader<'a>> {
        tracing::info!(
            "Open {} using encoding {:?}",
            self.path.display(),
            self.encoding
    ...
    pub async fn get_summary(&self) -> kuro_error::Result<EventLogSummary> {
...
mod tests {
    ...
    #[test]
    fn test_get_uuid_from_logfile_name() -> kuro_error::Result<()> {
    ...
    fn buck_event() -> Result<BuckEvent, kuro_error::Error> {
