...
pub fn should_upload_log() -> kuro_error::Result<bool> {
...
pub fn should_block_on_log_upload() -> kuro_error::Result<bool> {
    // `BUCK2_TEST_BLOCK_ON_UPLOAD` is used by our tests.
    Ok(is_ci()? || kuro_env!("BUCK2_TEST_BLOCK_ON_UPLOAD", bool, applicability = internal)?)
}
...
pub async fn wait_for_child_and_log(child: FutureChildOutput, reason: &str) {
    async fn inner(child: FutureChildOutput) -> kuro_error::Result<()> {
        ...
        if !res.status.success() {
            ...
            return Err(kuro_error::kuro_error!(
                kuro_error::ErrorTag::EventLogUpload,
