...
impl NamedEventLogWriter {
    ...
    pub(crate) async fn flush(&mut self) -> kuro_error::Result<()> {
        match self.file.flush().await {
            ...
            Err(e) => Err(kuro_error::Error::from(e).context(format!(
    ...
    fn serialize_event<'b, T>(&self, mut buf: &mut Vec<u8>, event: &T) -> kuro_error::Result<()>
    where
        T: SerializeForLog + 'b,
    {
    ...
    async fn write_all(&mut self, buf: &[u8]) -> kuro_error::Result<()> {
        match self.file.write_all(buf).await {
            ...
            Err(e) => Err(kuro_error::Error::from(e).context("Failed to write event")),
        }
    }
