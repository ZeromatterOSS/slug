...
impl WriteEventLog {
    ...
    async fn log_invocation(&mut self, trace_id: TraceId) -> kuro_error::Result<()> {
    ...
    async fn write_ln<'b, T, I>(&'b mut self, events: I) -> kuro_error::Result<()>
    where
        T: SerializeForLog + 'b,
    ...
    async fn ensure_log_writers_opened(&mut self, event: &BuckEvent) -> kuro_error::Result<()> {
...
) -> kuro_error::Result<NamedEventLogWriter> {
    let current_exe = std::env::current_exe().buck_error_context("No current_exe")?;
    let mut command = kuro_util::process::async_background_command(current_exe);
...
impl WriteEventLog {
    pub async fn write_events(&mut self, events: &[Arc<BuckEvent>]) -> kuro_error::Result<()> {
    ...
    pub async fn flush_files(&mut self) -> kuro_error::Result<()> {
...
impl SerializeForLog for Invocation {
    fn serialize_to_json(&self, buf: &mut Vec<u8>) -> kuro_error::Result<()> {
        serde_json::to_writer(buf, &self.clone().to_proto())
            .buck_error_context("Failed to serialize event")
    }

    fn serialize_to_protobuf_length_delimited(&self, buf: &mut Vec<u8>) -> kuro_error::Result<()> {
        self.clone().to_proto().encode_length_delimited(buf)?;
        Ok(())
    }

    // Always log invocation record to user event log for `kuro log show` compatibility
    fn maybe_serialize_user_event(&self, buf: &mut Vec<u8>) -> kuro_error::Result<bool> {
        serde_json::to_writer(buf, &self.clone().to_proto())
            .buck_error_context("Failed to serialize event")?;
        Ok(true)
    }
...
impl SerializeForLog for StreamValueForWrite<'_> {
    ...
    fn serialize_to_protobuf_length_delimited(&self, buf: &mut Vec<u8>) -> kuro_error::Result<()> {
        ...
        let stream_val = kuro_cli_proto::CommandProgressForWrite {
...
mod tests {
    ...
    impl WriteEventLog {
        async fn new_test(log: EventLogPathBuf) -> kuro_error::Result<Self> {
            Ok(Self {
                ...
                sanitized_argv: Argv {
                    argv: vec!["kuro".to_owned()],
                    expanded_argv: ExpandedArgv::from_literals(vec!["kuro".to_owned()]),
                }
                .no_need_to_sanitize(),
    ...
    #[tokio::test]
    async fn test_protobuf_decoding_gzip() -> kuro_error::Result<()> {
        test_protobuf_decoding(Encoding::PROTO_GZIP).await
    }

    #[tokio::test]
    async fn test_protobuf_decoding_zstd() -> kuro_error::Result<()> {
        test_protobuf_decoding(Encoding::PROTO_ZSTD).await
    }

    async fn test_protobuf_decoding(encoding: Encoding) -> kuro_error::Result<()> {
        //Create log dir
    ...
    #[tokio::test]
    async fn test_tick_makes_valid_log_zstd() -> kuro_error::Result<()> {
        test_tick_makes_valid_log(Encoding::PROTO_ZSTD).await
    }

    async fn test_tick_makes_valid_log(encoding: Encoding) -> kuro_error::Result<()> {
        if cfg!(windows) {
    ...
    fn test_stream_value_serialize_to_protobuf_length_delimited() {
        ...
        StreamValueForWrite::Event(event.event())
            ...
            .unwrap();
        let expected = kuro_cli_proto::CommandProgress {
