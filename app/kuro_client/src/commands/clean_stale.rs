...
) -> kuro_error::Result<Option<KeepSinceArg>> {
    let arg = match (stale, keep_since_time) {
        (Some(Some(human_duration)), None) => {
            let duration = chrono::Duration::from_std(human_duration.into())
                .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::InvalidDuration))?;
...
fn format_result_stats(stats: kuro_data::CleanStaleStats) -> String {
...
impl StreamingCommand for CleanStaleCommand {
    ...
    async fn exec_impl(
                    ...
                    humantime::format_duration(
                        duration
                            ...
                            .map_err(|e| from_any_with_tag(
                                e,
                                kuro_error::ErrorTag::InvalidDuration
                            ))
        ...
        if let Some(message) = response.message {
            kuro_client_ctx::eprintln!("{}", message)?;
        }
        if let Some(stats) = response.stats {
            kuro_client_ctx::eprintln!("{}", format_result_stats(stats))?;
        }
        ExitResult::success()
    }
