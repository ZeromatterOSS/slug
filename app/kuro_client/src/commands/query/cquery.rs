...
use kuro_core::if_else_opensource;
...
Run `kuro docs cquery` or
"#,
        if_else_opensource!(
            "https://kuro.build/docs/users/query/cquery/",
            "https://www.internalfb.com/intern/staticdocs/kuro/docs/users/query/cquery/",
        ),
        r#"
...
`kuro cquery //java/com/example/app:amazing --output-all-attributes`

List the deps of a target (special characters in a target will
require quotes):

`kuro cquery 'deps("//java/com/example/app:amazing+more")'`
"#
    )
}
            ...
            .cquery(
                CqueryRequest {
                    ...
                    profile_output: self
                        ...
                        .profile_output
                        .as_ref()
                        .map(|p| kuro_error::Ok(p.resolve(&ctx.working_dir).to_str()?.to_owned()))
                        .transpose()?,
                },
                events_ctx,
