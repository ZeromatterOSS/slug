...
///
/// The command also kills the kuro daemon.
...
impl BuckSubcommand for InnerCleanCommand {
    ...
    async fn exec_impl(
        ...
        ctx: ClientCommandContext<'_>,
        _events_ctx: &mut kuro_client_ctx::events_ctx::EventsCtx,
    ) -> ExitResult {
        let paths = ctx.paths()?;
        ...
        kill_command_impl(&lifecycle_lock, "`kuro clean` was invoked").await?;
...
) -> kuro_error::Result<Vec<AbsNormPathBuf>> {
...
/// add a single retry.
fn clean_buck_out_with_retry(path: &AbsNormPathBuf) -> kuro_error::Result<()> {
...
fn clean_buck_out(path: &AbsNormPathBuf) -> kuro_error::Result<()> {
    let walk = WalkDir::new(path);
    let thread_pool = ThreadPool::new(kuro_util::threads::available_parallelism());
