        ...
        let single_default_output = match default_outputs.len() {
            0 => {
                return Err(kuro_error!(
                    kuro_error::ErrorTag::CopyOutputs,
...
async fn copy_directory<P, Q>(src: P, dst: Q, context: &CopyContext) -> kuro_error::Result<()>
where
...
async fn copy_file(src: &Path, dst: &Path) -> kuro_error::Result<()> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_copy_directory() -> kuro_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_copy_directory_with_symlink_pointing_externally() -> kuro_error::Result<()> {
        let src_dir = tempfile::tempdir()?;
    ...
    #[tokio::test]
    async fn test_copy_symlink() -> kuro_error::Result<()> {
    ...
    fn test_copy_symlink_parametrized(
        prepare_dst_dir: &dyn Fn(&TempDir) -> kuro_error::Result<()>,
    ...
    mod unix {
        ...
        #[tokio::test]
        async fn test_copy_file() -> kuro_error::Result<()> {
            let dir = tempfile::tempdir()?;
            let out = dir.path().join("sleep");

            let res = kuro_util::process::async_background_command("cp")
            ...
            let mut proc = kuro_util::process::async_background_command(&out)
                .arg("10000")
                .kill_on_drop(true)
                .spawn()
