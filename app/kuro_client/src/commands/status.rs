...
use kuro_client_ctx::daemon::client::connect::BuckdConnectConstraints;
use kuro_client_ctx::daemon::client::connect::connect_buckd;
use kuro_client_ctx::daemon::client::connect::establish_connection_existing;
use kuro_client_ctx::events_ctx::EventsCtx;
use kuro_client_ctx::subscribers::stdout_stderr_forwarder::StdoutStderrForwarder;
    ...
    pub fn exec(
        ...
        ctx.with_runtime(|ctx| async move {
            ...
            if self.all {
                ...
                for entry in walker {
                    let entry =
                        entry.map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::Tier0))?;
                ...
                kuro_client_ctx::println!("{}", serde_json::to_string_pretty(&statuses)?)?;
            } else {
                match connect_buckd(
                ...
                {
                    Err(_) => {
                        kuro_client_ctx::eprintln!("no buckd running")?;
...
fn timestamp_to_string(seconds: u64, nanos: u32) -> kuro_error::Result<String> {
...
fn process_status(status: StatusResponse) -> kuro_error::Result<serde_json::Value> {
