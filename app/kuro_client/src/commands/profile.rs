...
use kuro_client_ctx::common::profiling::BuckProfileMode;
...
impl StreamingCommand for ProfileSubcommand {
    ...
    async fn exec_impl(
        ...
        let elapsed = elapsed
            ...
            .and_then(|d| {
                Duration::try_from(d).map_err(|_| {
                    kuro_error::kuro_error!(kuro_error::ErrorTag::Input, "Duration is negative")
                })
            })
        ...
        kuro_client_ctx::println!("Elapsed: {:.3}s", elapsed.as_secs_f64())?;
        kuro_client_ctx::println!("Total retained bytes: {}", total_retained_bytes)?;
