...
use kuro_core::env::registry::Applicability;
use kuro_core::env::registry::ENV_INFO;
use kuro_core::env::registry::EnvInfoEntry;
use kuro_error::BuckErrorContext;

/// Print help for environment variables used by kuro.
#[derive(Debug, clap::Parser)]
pub struct HelpEnvCommand {
    /// Also print those environment variables that are only used for kuro integration tests.
    ///
    ...
    pub fn exec(self, _matches: BuckArgMatches<'_>, _ctx: ClientCommandContext<'_>) -> ExitResult {
        ...
        let mut env_info: Vec<EnvInfoEntry> = ENV_INFO
            ...
            .filter(|x| match x.applicability {
                ...
                Applicability::Internal => !kuro_core::is_open_source(),
            })
            .collect();
        env_info.sort();
        env_info.dedup();
        ...
        for (name, ty, default) in rows {
            ...
            kuro_client_ctx::println!("{}", line.trim_end())?;
        }
        ExitResult::success()
    }
}
