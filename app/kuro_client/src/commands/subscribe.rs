...
use kuro_client_ctx::common::ui::ConsoleType;
use kuro_client_ctx::daemon::client::BuckdClientConnector;
use kuro_client_ctx::events_ctx::EventsCtx;
use kuro_client_ctx::events_ctx::PartialResultCtx;
use kuro_client_ctx::events_ctx::PartialResultHandler;
...
/// Open a subscription channel to the Kuro daemon. This allows you to interact with the Kuro
...
/// `kuro_subscription_proto/subscription.proto`. The client writes `SubscriptionRequest` and
...
#[clap(about = "Subscribe to updates from the Kuro daemon")]
    ...
    async fn exec_impl(
        ...
        let stream = FramedRead::new(ctx.stdin(), ProtobufSplitter)
            ...
            .map(|res| match res {
                ...
                Err(e) => {
                    ...
                    let _ignored = kuro_client_ctx::eprintln!("{}", reason);
                    SubscriptionRequest {
                        request: Some(
                            kuro_subscription_proto::Disconnect { reason, ok: false }.into(),
                        ),
                    }
                }
            });
        ...
        let stream = if self.active_commands {
            futures::stream::once(futures::future::ready(SubscriptionRequest {
                request: Some(kuro_subscription_proto::SubscribeToActiveCommands {}.into()),
            }))
            .chain(stream)
            .left_stream()
        } else {
        ...
        let stream = stream.map(|request| kuro_cli_proto::SubscriptionRequestWrapper {
...

        {
            ...
            reborrow_stream_for_static(
                ...
                || {
                    Some(kuro_cli_proto::SubscriptionRequestWrapper {
                        request: Some(SubscriptionRequest {
                            request: Some(
                                kuro_subscription_proto::Disconnect {
...
impl PartialResultHandler for SubscriptionPartialResultHandler {
    type PartialResult = kuro_cli_proto::SubscriptionResponseWrapper;
    ...
    async fn handle_partial_result(
        ...
        if let Some(kuro_subscription_proto::subscription_response::Response::Goodbye(goodbye)) =
            &response.response
        {
