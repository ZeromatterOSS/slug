...
use kuro_client_ctx::console_interaction_stream::ConsoleInteractionStream;
use kuro_client_ctx::daemon::client::BuckdClientConnector;
use kuro_client_ctx::daemon::client::NoPartialResultHandler;
use kuro_client_ctx::daemon::client::StdoutPartialResultHandler;
...
impl Compression {
    fn to_proto(&self) -> kuro_cli_proto::targets_request::Compression {
        match self {
            Compression::None => kuro_cli_proto::targets_request::Compression::Uncompressed,
            Compression::Gzip => kuro_cli_proto::targets_request::Compression::Gzip,
            Compression::Zstd => kuro_cli_proto::targets_request::Compression::Zstd,
        }
    }
}
...
impl TargetsCommand {
    ...
    fn output_format(&self) -> kuro_error::Result<OutputFormat> {
        if self.json {
    ...
    fn package_values_as_regexes(&self) -> kuro_error::Result<Vec<String>> {
        ...
        let target_hash_use_fast_hash = match self.target_hash_function {
            TargetHashFunction::Sha1 | TargetHashFunction::Sha256 => {
                kuro_client_ctx::eprintln!(
                    "kuro only supports \"fast\" and \"strong\" target hash functions. Using the \"strong\" hash."
                )?;
                false
            }
            TargetHashFunction::Murmur_Hash3 => {
                kuro_client_ctx::eprintln!(
                    "kuro only supports \"fast\" and \"strong\" target hash functions. Using the \"fast\" hash."
                )?;
                true
            }
...
async fn targets_show_outputs(
    ...
    kuro_client_ctx::stdio::print_with_writer::<ClientIoError, _>(async move |w| {
...
async fn targets(
    ...
    if !response.serialized_targets_output.is_empty() {
        kuro_client_ctx::print!("{}", response.serialized_targets_output)?;
    }
    ExitResult::success()
}
