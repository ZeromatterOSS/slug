...
use kuro_wrapper_common::BUCK_WRAPPER_UUID_ENV_VAR;
...
pub struct RunCommand {
    ...
    /// formatted for shell interpolation, use as: $(kuro run --emit-shell ...)
...
impl StreamingCommand for RunCommand {
    ...
    async fn exec_impl(
        ...
        // Kuro. We're not a wrapper script, so we unset it to prevent `run` from inheriting it.
...
pub enum RunCommandError {
    ...
    #[error("`kuro run` only supports a single target, but multiple targets were requested.")]
    MultipleTargets,
