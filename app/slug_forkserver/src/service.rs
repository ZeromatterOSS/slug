...
use slug_execute_local::StdRedirectPaths;
...
use slug_execute_local::status_decoder::MiniperfStatusDecoder;
use slug_forkserver_proto::CommandRequest;
use slug_forkserver_proto::RequestEvent;
use slug_forkserver_proto::SetLogFilterRequest;
use slug_forkserver_proto::SetLogFilterResponse;
use slug_forkserver_proto::forkserver_server::Forkserver;
use slug_fs::fs_util;
...
type RunStream =
    Pin<Box<dyn Stream<Item = Result<slug_forkserver_proto::CommandEvent, Status>> + Send>>;

struct ValidatedCommand {
    exe: PathBuf,
    argv: Vec<Vec<u8>>,
    env: Vec<slug_forkserver_proto::EnvDirective>,
    cwd: PathBuf,
...
impl ValidatedCommand {
    fn try_from(cmd_request: CommandRequest) -> slug_error::Result<Self> {
        let CommandRequest {
            exe,
            argv,
            env,
...
impl UnixForkserverService {
    ...
    ) -> slug_error::Result<CommandRequest> {
    ...
    fn configure_environment(
        ...
        env_directives: &[slug_forkserver_proto::EnvDirective],
    ) -> slug_error::Result<()> {
        use slug_forkserver_proto::env_directive::Data;
    ...
    ) -> slug_error::Result<(Command, Option<AbsNormPathBuf>)> {
    ...
    async fn create_command_stream(
        cmd: Command,
        validated_cmd: ValidatedCommand,
        cancellation: impl futures::Future<Output = slug_error::Result<GatherOutputStatus>>
        + Send
        + Unpin
        + 'static,
        miniperf_output: Option<AbsNormPathBuf>,
        freeze_rx: impl ActionFreezeEventReceiver,
    ) -> slug_error::Result<RunStream> {
            ...
            tokio::spawn(async move {
                ...
                while let Some(m) = stream.next().await {
                    let err = match m {
                        Ok(m) => match m.data {
                            Some(d) => match d {
                                slug_forkserver_proto::request_event::Data::CommandRequest(_) => {
                                    slug_error::internal_error!("Non-leading command request")
                                }
                                slug_forkserver_proto::request_event::Data::CancelRequest(_) => {
                                    drop(cancel_tx.send(Ok(GatherOutputStatus::Cancelled)));
                                    break;
                                }
                                slug_forkserver_proto::request_event::Data::FreezeRequest(_) => {
                                    drop(freeze_tx.send(ActionFreezeEvent::Freeze));
                                    continue;
                                }
                                slug_forkserver_proto::request_event::Data::UnfreezeRequest(_) => {
...
impl MiniperfContainer {
    fn new(forkserver_state_dir: &AbsNormPath) -> slug_error::Result<Option<Self>> {
        let miniperf_bin: Option<&'static [u8]>;

        #[cfg(all(fbcode_build, target_os = "linux"))]
        {
            miniperf_bin = Some(slug_miniperf_data::get());
        }
