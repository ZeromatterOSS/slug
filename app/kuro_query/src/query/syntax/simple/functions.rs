...
pub trait QueryLiteralVisitor<'a> {
    fn target_pattern(&mut self, pattern: &'a str) -> kuro_error::Result<()>;
}
...
impl<Env: QueryEnvironment> DefaultQueryFunctionsModule<Env> {
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery "allpaths('//foo:bar', '//foo/bar/lib:baz')"
    /// ```
    ...
    /// ```text
    /// $ kuro uquery "allpaths(kind(java_library, '//...'), '//foo:bar')"
    /// ```
    ...
    ///
    /// ```text
    /// $ kuro uquery "allpaths(//kuro:kuro, //kuro/app/kuro_validation:kuro_validation)" --output-format=dot > result.dot
    ...
    ///
    /// ```text
    /// $ kuro uquery 'somepath(//kuro:kuro, //kuro/app/kuro_node:kuro_node)'
    ///
    /// //kuro:kuro
    /// //kuro/app/kuro:kuro-bin
    /// //kuro/app/kuro_analysis:kuro_analysis
    /// //kuro/app/kuro_node:kuro_node
    /// ```
    async fn somepath(
        &self,
        evaluator: &QueryEvaluator<'_, Env>,
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery "attrfilter(deps, '//kuro/app/kuro_validation:kuro_validation', '//...')"
    ///
    /// //kuro/app/kuro:kuro-bin
    /// //kuro/app/kuro_server:kuro_server
    /// //kuro/app/kuro_server:kuro_server-unittest
    /// ```
    /// returns targets that contain `//kuro/app/kuro_validation:kuro_validation` target in their `deps` attribute.
    async fn attrfilter(
        &self,
        attr: String,
        value: String,
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery "nattrfilter(deps, '//foo:bar', '//...')"
    /// ```
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery "attrregexfilter(deps, '.+validation$', '//...')"
    ///
    /// //kuro/app/kuro:kuro-bin
    /// //kuro/app/kuro_server:kuro_server
    /// //kuro/app/kuro_server:kuro_server-unittest
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery 'buildfile(//kuro:kuro)'
    ///
    /// kuro/BUCK
    /// ```
    ///
    ...
    /// Examples:
    /// ```text
    /// $ kuro uquery "buildfile(//kuro/app/kuro_action_impl_tests:kuro_action_impl_tests)"
    /// ```
    /// and
    /// ```text
    /// $ kuro uquery "buildfile(owner(app/kuro_action_impl_tests/src/context.rs))"
    /// ```
    /// both return `kuro/app/kuro_action_impl_tests/TARGETS`.
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery "rbuildfiles(//kuro/BUCK, //kuro/defs.bzl)"
    ///
    /// kuro/defs.bzl
    /// kuro/BUCK
    /// ```
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery 'allbuildfiles(//foo:bar)'
    ///
    /// foo/BUCK
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery "filter(validation$, //kuro/app/...)"
    ///
    /// //kuro/app/kuro_validation:kuro_validation
    /// ```
    /// returns all targets within `//kuro/app` that have a label with a `validation` suffix.
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery "inputs(//kuro/dice/...)"
    /// ```
    /// returns the direct inputs for the `//kuro/dice/...` targets.
    ...
    /// For example:
    /// ```text
    /// $ kuro query "kind('java.*', deps('//foo:bar'))"
    /// ```
    ...
    /// Not implemented.
    ///
    /// This function won't be implemented in the future, because kuro query core does not support returning both files and targets from a single function.
    ///
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery "owner('app/kuro/src/lib.rs')"
    ///
    /// //kuro/app/kuro:kuro-unittest
    /// //kuro/app/kuro:kuro
    /// ```
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery 'targets_in_buildfile(buildfile(//kuro:kuro))'
    ///
    /// //kuro:kuro
    /// //kuro:kuro_bundle
    /// //kuro:symlinked_kuro_and_tpx
    /// ```
    ...
    ///
    /// ```text
    /// $ kuro uquery "rdeps(//kuro/..., //kuro/dice/dice:dice, 1)"
    /// ```
    /// returns all targets under `//kuro/...` that depend on `//kuro/dice/dice:dice`.
    async fn rdeps(
        &self,
        evaluator: &QueryEvaluator<'_, Env>,
        universe: TargetSet<Env::Target>,
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery "testsof(set(//kuro/dice/dice:dice //kuro/app/kuro:kuro))"
    ///
    /// //kuro/dice/dice:dice-unittest
    /// //kuro/app/kuro:kuro-unittest
    /// ```
    /// returns the tests associated with both `//kuro/dice/dice:dice` and `//kuro/app/kuro:kuro`.
    ///
    ...
    /// For example:
    /// ```text
    /// $ kuro uquery "testsof(deps(//kuro/app/kuro:kuro))"
    /// ```
    /// first finds the transitive closure of `//kuro/app/kuro:kuro`,
    ...
    ///
    /// Example:
    /// `kuro cquery "deps('//foo:bar', 1, first_order_deps())"` is equivalent to `kuro cquery "deps('//foo:bar', 1)"`
    ...
    ///
    /// Example:
    /// `kuro cquery "deps('//foo:bar', 1, target_deps())"`
    ...
    ///
    /// Example:
    /// `kuro cquery "deps('//foo:bar', 1, exec_deps())"`
    ...
    ///
    /// Example:
    /// `kuro cquery "deps('//foo:bar', 1, configuration_deps())"`
    ...
    ///
    /// Example:
    /// `kuro cquery "deps('//foo:bar', 1, toolchain_deps())"`
    ...
    ///
    /// Example:
    /// `kuro aquery "deps('//foo:bar') intersect deps('//baz:lib')"` is the same as
    /// `kuro aquery "deps('//foo:bar') ^ deps('//baz:lib')"`
    ...
    ///
    /// Example:
    /// `kuro aquery "deps('//foo:bar') except deps('//baz:lib')"` is the same as
    /// `kuro aquery "deps('//foo:bar') - deps('//baz:lib')"`
    ...
    ///
    /// Example:
    /// `kuro aquery "deps('//foo:bar') union deps('//baz:lib')"` is the same as
    /// `kuro aquery "deps('//foo:bar') + deps('//baz:lib')"`
...
impl<Env: QueryEnvironment> DefaultQueryFunctions<Env> {
    ...
    pub fn attrfilter(
        ...
        value: &str,
        targets: &TargetSet<Env::Target>,
    ) -> kuro_error::Result<TargetSet<Env::Target>> {
    ...
    pub fn filter_file_set(&self, regex: &str, files: &FileSet) -> kuro_error::Result<FileSet> {
        files.filter_name(regex)
    }

    pub fn inputs(&self, targets: &TargetSet<Env::Target>) -> kuro_error::Result<FileSet> {
        targets.inputs()
    }
    ...
    pub async fn testsof_with_default_target_platform(
        ...
        env: &Env,
        targets: &TargetSet<Env::Target>,
    ) -> kuro_error::Result<Vec<MaybeCompatible<Env::Target>>> {
    ...
    where
        ...
        Ff: Fn(&FileSet, &FileSet) -> kuro_error::Result<FileSet>,
    {
        match (left, right) {
