...
impl<T: QueryTarget> QueryEvaluationResult<T> {
    ...
    pub fn targets(&self) -> impl Iterator<Item = kuro_error::Result<&T>> {
        match self {
...
impl<T: QueryTarget> QueryEvaluationValue<T> {
    pub fn try_into_targets(self) -> kuro_error::Result<TargetSet<T>> {
        match self {
    ...
    pub(crate) fn targets(&self) -> impl Iterator<Item = kuro_error::Result<&T>> {
        match self {
...
pub trait QueryResultExt<T> {
    fn into_kuro_error(self, query: &str) -> kuro_error::Result<T>;
}
