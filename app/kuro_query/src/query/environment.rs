...
pub trait TraversalFilter<T: QueryTarget>: Send + Sync {
    ...
    async fn get_children(&self, target: &T) -> kuro_error::Result<TargetSet<T>>;
}
...
pub trait QueryEnvironment: Send + Sync {
    ...
    async fn testsof_with_default_target_platform(
        &self,
        targets: &TargetSet<Self::Target>,
    ) -> kuro_error::Result<Vec<MaybeCompatible<Self::Target>>> {
        let target_tests = targets
            .iter()
            .map(|target| {
                let tests = target
...
{
    async fn get(&self, label: &<Q::Target as LabeledNode>::Key) -> kuro_error::Result<Q::Target> {
