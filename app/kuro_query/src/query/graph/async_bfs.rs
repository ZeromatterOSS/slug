...
impl<N: LabeledNode + 'static> BfsVisited<N> {
    fn take_path(mut self, last: &N::Key, mut item: impl FnMut(N)) -> kuro_error::Result<()> {
        let node = self
            .visited
            .remove(last)
...
) -> kuro_error::Result<Option<Vec<N>>> {
    let lookup = &lookup;
...
mod tests {
    ...
    impl TestGraph {
        async fn bfs_find_path(
            ...
            target: u32,
        ) -> kuro_error::Result<Option<Vec<u32>>> {
    ...
    impl AsyncNodeLookup<TestNode> for TestGraph {
        async fn get(&self, label: &TestNodeKey) -> kuro_error::Result<TestNode> {
