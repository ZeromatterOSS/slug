...
/// using:
/// - `source <(kuro completion bash)`
/// - `source <(kuro completion zsh)`
/// - `source (kuro completion fish | psub)`
    ...
    for line in wrapper_iter.by_ref() {
        match line {
            ...
            COMPLETION_INSERTION_POINT => {
                ...
                kuro_client_ctx::println!("{}", option_completions(shell, cmd)?)?;
            }
            s => {
                kuro_client_ctx::println!("{}", s)?;
            }
        }
    }
