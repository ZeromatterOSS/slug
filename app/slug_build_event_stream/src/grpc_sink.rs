...
//!
//! Wires Slug into BuildBuddy / EngFlow / Trunk / custom BES collectors via
...
impl BesSink {
    ...
    pub async fn start(config: BesConfig) -> slug_error::Result<Self> {
    ...
    pub async fn enqueue(&self, event: BepBuildEvent) -> slug_error::Result<()> {
    ...
    /// the full event stream before the slug process exits.
    ///
    ...
    pub async fn shutdown(&self) -> slug_error::Result<()> {
...
async fn build_endpoint(config: &BesConfig) -> slug_error::Result<Endpoint> {
    ...
    let endpoint = if tls {
        endpoint
            ...
            .map_err(|e| {
                slug_error::slug_error!(slug_error::ErrorTag::Environment, "BES TLS config: {e}")
            })?
    } else {
        endpoint
    };
...
impl HeaderInterceptor {
    fn new(headers: Vec<(String, String)>) -> slug_error::Result<Self> {
