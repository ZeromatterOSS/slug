...
use kuro_events::dispatch::with_dispatcher;
...
use kuro_interpreter::allow_relative_paths::HasAllowRelativePaths;
...
impl DocsCacheManager {
    async fn new(fs: ProjectRoot, mut dice_ctx: DiceTransaction) -> kuro_error::Result<Self> {
        Ok(Self {
    ...
    ) -> kuro_error::Result<MutexGuard<'_, DocsCache>> {
...
impl DocsCache {
    async fn get_prelude_uri(
        ...
        Url::from_file_path(abs_path)
            .unwrap()
            .try_into()
            .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::Lsp))
    }
    ...
    async fn new_with_lookup<
        ...
        Fut: Future<Output = kuro_error::Result<LspUrl>>,
    >(
...
impl<'a> BuckLspContext<'a> {
    async fn new(
        server_ctx: &'a dyn ServerCommandContextTrait,
    ) -> kuro_error::Result<BuckLspContext<'a>> {
    ...
    async fn with_dice_ctx<F, Fut, R>(&self, exec: F) -> kuro_error::Result<R>
    where
        F: FnOnce(DiceTransaction) -> Fut + Send,
        Fut: Future<Output = kuro_error::Result<R>> + Send,
        R: Send,
    {
        self.server_ctx
    ...
    async fn import_path(&self, path: &Path) -> kuro_error::Result<OwnedStarlarkModulePath> {
    ...
    async fn parse_file_from_contents_and_handle_diagnostic(
        ...
        uri: &LspUrl,
        content: String,
    ) -> kuro_error::Result<LspEvalResult> {
    ...
    async fn parse_file_from_string(
        ...
        literal: &str,
    ) -> kuro_error::Result<Option<StringLiteralResult>> {
                        ...
                        Ok(DicePackageListingResolver(&mut dice_ctx)
                            ...
                            .and_then(|listing| {
                                ...
                                match Url::from_file_path(path).unwrap().try_into() {
                                    ...
                                    Err(e) => Err(from_any_with_tag(e, kuro_error::ErrorTag::Lsp)),
                                }
                            }))
                    })
                    .await?;
        ...
        self.runtime
            .block_on(with_dispatcher_async(dispatcher, async {
                match current_file {
                    ...
                    _ => Err(kuro_error::Error::from(ResolveLoadError::WrongScheme(
                        "file://".to_owned(),
                        current_file.clone(),
                    ))
                    .into()),
    ...
    fn resolve_string_literal(
        ...
        self.runtime
            .block_on(with_dispatcher_async(dispatcher, async {
                ...
                //
                // kuro_error::Error sort of obscures the root causes, so we just have
        ...
        self.runtime
            .block_on(with_dispatcher_async(dispatcher, async {
                let docs_cache = self
                    ...
                    .await?;
                kuro_error::Ok(docs_cache.url_for_symbol(symbol).cloned())
            }))
            .map_err(|e| format!("{:#}", e))
    }
...
pub(crate) async fn run_lsp_server_command(
    ...
    partial_result_dispatcher: PartialResultDispatcher<kuro_cli_proto::LspMessage>,
    req: StreamingRequestHandler<kuro_cli_proto::LspRequest>,
) -> kuro_error::Result<kuro_cli_proto::LspResponse> {
    let start_event = ctx
        .command_start_event(kuro_data::LspCommandStart {}.into())
        .await?;
    span_async(start_event, async move {
        let result = run_lsp_server(ctx, partial_result_dispatcher, req)
            .await
            .map_err(Into::into);
        let end_event = command_end(&result, kuro_data::LspCommandEnd {});
...
async fn run_lsp_server(
    ctx: &dyn ServerCommandContextTrait,
    mut partial_result_dispatcher: PartialResultDispatcher<kuro_cli_proto::LspMessage>,
    mut req: StreamingRequestHandler<LspRequest>,
) -> kuro_error::Result<LspResponse> {
...
async fn recv_from_lsp(
    ...
    mut event_sender: UnboundedSender<kuro_cli_proto::LspMessage>,
) -> kuro_error::Result<()> {
    loop {
        let msg = to_client
            .recv()
        ...
        let res = kuro_cli_proto::LspMessage { lsp_json };
        match event_sender.send(res).await {
            Ok(_) => {}
            Err(e) if e.is_disconnected() => break Ok(()),
            Err(e) => break Err(from_any_with_tag(e, kuro_error::ErrorTag::Lsp)),
        }
    }
}
...
fn handle_incoming_lsp_message(
    ...
    message: kuro_error::Result<LspRequest>,
) -> kuro_error::Result<Option<LspResponse>> {
    if let Ok(m) = message {
...
fn handle_outgoing_lsp_message(
    partial_result_dispatcher: &mut PartialResultDispatcher<kuro_cli_proto::LspMessage>,
    event: Option<kuro_cli_proto::LspMessage>,
) -> Option<LspResponse> {
    match event {
        Some(event) => {
...
mod tests {
    ...
    #[test]
    fn cache_builds() -> kuro_error::Result<()> {
        ...
        async fn lookup_function(location: &ImportPath) -> kuro_error::Result<LspUrl> {
