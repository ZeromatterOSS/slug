...
use kuro_event_observer::span_tracker::RootData;
use kuro_event_observer::span_tracker::Roots;
...
pub fn broadcast_shutdown(shutdown: &kuro_data::DaemonShutdown) {
    for cmd in ACTIVE_COMMANDS.lock().values() {
...
pub struct ActiveCommandHandle {
    ...
    daemon_shutdown_channel: Arc<Mutex<Option<oneshot::Sender<kuro_data::DaemonShutdown>>>>,
...
impl ActiveCommandHandle {
    fn notify_shutdown(&self, shutdown: kuro_data::DaemonShutdown) {
...
pub struct ActiveCommand {
    ...
    pub daemon_shutdown_channel: oneshot::Receiver<kuro_data::DaemonShutdown>,
}
...
impl ActiveCommand {
    pub fn new(event_dispatcher: &EventDispatcher, sanitized_argv: Vec<String>) -> Self {
        ...
        if let Some(commands) = result {
            ...
            event_dispatcher.instant_event(kuro_data::ConcurrentCommands {
...
mod tests {
    ...
    fn test_active_command_state() {
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            Some(root),
            None,
            kuro_data::SpanStartEvent {
                data: Some(kuro_data::AnalysisStart::default().into()),
            }
            .into(),
        ));
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            Some(root),
            kuro_data::SpanStartEvent {
                data: Some(kuro_data::AnalysisStageStart::default().into()),
            }
            .into(),
        ));
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            Some(child),
            Some(root),
            kuro_data::SpanEndEvent {
                data: Some(kuro_data::AnalysisStageEnd::default().into()),
                ..Default::default()
            }
            .into(),
        ));
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            None,
            kuro_data::SpanEndEvent {
                data: Some(kuro_data::AnalysisEnd::default().into()),
                ..Default::default()
            }
            .into(),
        ));
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            kuro_data::InstantEvent {
                data: Some(
                    kuro_data::DiceStateSnapshot {
                        key_states: {
                            let mut map = HashMap::new();
                            map.insert(
                                "BuildKey".to_owned(),
                                kuro_data::DiceKeyState {
                                    started: 4,
                                    finished: 2,
        ...
        assert_matches!(event, Some(Event::Buck(event)) => {
            assert_matches!(
                ...
                kuro_data::buck_event::Data::Instant(kuro_data::InstantEvent {
                    data: Some(kuro_data::instant_event::Data::ConcurrentCommands(
                        kuro_data::ConcurrentCommands {
                            trace_ids,
                        }
                    ))
                }) => {
