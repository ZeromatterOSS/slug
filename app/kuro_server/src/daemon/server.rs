...
use kuro_cli_proto::daemon_api_server::*;
use kuro_cli_proto::*;
...
use kuro_fs::cwd::WorkingDirectory;
use kuro_fs::fs_util;
use kuro_fs::fs_util::DiskSpaceStats;
use kuro_fs::fs_util::disk_space_stats;
use kuro_fs::paths::abs_path::AbsPathBuf;
use kuro_interpreter::starlark_profiler::config::StarlarkProfilerConfiguration;
use kuro_profile::proto_to_profile_mode;
use kuro_profile::starlark_profiler_configuration_from_request;
use kuro_resource_control::buck_cgroup_tree::BuckCgroupTree;
use kuro_resource_control::buck_cgroup_tree::PreppedBuckCgroups;
use kuro_server_ctx::bxl::BXL_SERVER_COMMANDS;
use kuro_server_ctx::late_bindings::AUDIT_SERVER_COMMAND;
use kuro_server_ctx::late_bindings::OTHER_SERVER_COMMANDS;
use kuro_server_ctx::late_bindings::QUERY_SERVER_COMMANDS;
use kuro_server_ctx::late_bindings::STARLARK_SERVER_COMMAND;
use kuro_server_ctx::late_bindings::TARGETS_SERVER_COMMANDS;
...
use kuro_server_ctx::test_command::TEST_COMMAND;
use kuro_server_starlark_debug::run::run_dap_server_command;
...
impl DaemonShutdown {
    ...
    fn start_shutdown(&self, reason: kuro_data::DaemonShutdown, timeout: Option<Duration>) {
...
impl Interceptor for BuckCheckAuthTokenInterceptor {
    fn call(&mut self, request: Request<()>) -> Result<Request<()>, Status> {
        ...
        if kuro_env!("BUCK2_TEST_FAIL_BUCKD_AUTH", bool, applicability = testing).unwrap() {
...
pub(crate) struct BuckdServerData {
    ...
    base_daemon_constraints: kuro_cli_proto::DaemonConstraints,
...
impl BuckdServer {
    pub async fn run(
        ...
        #[cfg(fbcode_build)]
        {
            ...
            if !kuro_env!("BUCK2_DISABLE_EDEN_HEALTH_CHECK", bool)?
    ...
    where
        F: for<'a> FnOnce(
            ...
            ) -> BoxFuture<'a, kuro_error::Result<Res>>
            + Send
            + 'static,
        Req: TryFrom<StreamingRequest, Error = kuro_error::Error> + Send + Sync + 'static,
        ...
        PartialRes: Into<partial_result::PartialResult> + Send + 'static,
    {
        ...
        let init_request = match req.message().await? {
            Some(
                m @ StreamingRequest {
                    request: Some(kuro_cli_proto::streaming_request::Request::Context(_)),
                },
            ) => Ok(m),
    ...
    async fn run_streaming_fallible<Req, Res, PartialRes, F>(
        ...
        func: F,
    ) -> kuro_error::Result<Response<ResponseStream>>
    where
        F: for<'a> FnOnce(
                &'a ServerCommandContext,
                PartialResultDispatcher<PartialRes>,
        ...
        PartialRes: Into<partial_result::PartialResult> + Send + 'static,
    {
        ...
        if kuro_env!("BUCK2_TEST_FAIL_STREAMING", bool, applicability = testing).unwrap() {
        ...
        reload_hard_error_config(&client_ctx.kuro_hard_error)?;
        ...
        dispatch.instant_event(kuro_data::SystemInfo {
        ...
        let memory_reporter = daemon_state.data.memory_tracker.as_ref().map(|t| {
            kuro_resource_control::memory_tracker::spawn_memory_reporter(dispatch.dupe(), t.dupe())
        });
        ...
        let resp = streaming(
            ...
            move |req, cancellations| {
                async move {
                    let result: kuro_error::Result<Res> = try {
                        let base_context =
    ...
    async fn oneshot<
        ...
        Fut: Future<Output = kuro_error::Result<Res>> + Send,
        F: FnOnce(Req) -> Fut,
    >(
        &self,
        req: Request<Req>,
...
fn error_to_command_progress(e: kuro_error::Error) -> CommandProgress {
    CommandProgress {
...
fn error_to_response_stream(e: kuro_error::Error) -> Response<ResponseStream> {
    tonic::Response::new(Box::pin(stream::once(future::ready(Ok(
        kuro_cli_proto::MultiCommandProgress {
...
fn pump_events(
    ...
    output_send: tokio::sync::mpsc::UnboundedSender<
        Result<kuro_cli_proto::CommandProgress, tonic::Status>,
    >,
) {
...
fn streaming<Req, F>(
    ...
    daemon_shutdown_channel: oneshot::Receiver<kuro_data::DaemonShutdown>,
    func: F,
    rt: &Handle,
    ...
    F: for<'a> FnOnce(Req, &'a CancellationContext) -> BoxFuture<'a, ()> + Send + 'static,
{
    ...
    if let Err(e) = merge_task {
        return error_to_response_stream(
            kuro_error::Error::from(e).context("failed to spawn pump-events"),
        );
    };
...
impl<Req> StreamingCommandOptions<Req> for QueryCommandOptions {
    fn starlark_profiler_instrumentation_override(
        ...
        match self.profile_mode {
            ...
            Some(mode) => {
                let mode = kuro_cli_proto::ProfileMode::try_from(mode)
...
impl DaemonApi for BuckdServer {
    async fn kill(&self, req: Request<KillRequest>) -> Result<Response<CommandResult>, Status> {
        ...
        self.oneshot(req, KillRunCommandOptions, move |req| async move {
            ...
            let reason = kuro_data::DaemonShutdown {
                reason: req.reason,
                callers: req.callers,
            };
    ...
    async fn status(&self, req: Request<StatusRequest>) -> Result<Response<CommandResult>, Status> {
        ...
        self.oneshot(req, DefaultCommandOptions, move |req| async move {
            ...
            let extra_constraints = kuro_cli_proto::ExtraDaemonConstraints {
    ...
    async fn flush_dep_files(
        ...
        self.oneshot(req, DefaultCommandOptions, move |req| async move {
            ...
            if retain_locally_produced_dep_files {
                kuro_file_watcher::dep_files::flush_non_local_dep_files();
            } else {
                kuro_file_watcher::dep_files::flush_dep_files();
            }
            Ok(GenericResponse {})
        })
        .await
    ...
    async fn unstable_dice_dump(
        ...
        let res: kuro_error::Result<_> = try {
            ...
            let format_proto =
                kuro_cli_proto::unstable_dice_dump_request::DiceDumpFormat::try_from(inner.format)
            ...
            |ctx, partial_result_dispatcher, req| {
                Box::pin(async {
                    match req.profile_opts.as_ref().expect("Missing profile opts") {
                        kuro_cli_proto::profile_request::ProfileOpts::TargetProfile(_) => {
                            profile_command(ctx, partial_result_dispatcher, req).await
                        }
                        kuro_cli_proto::profile_request::ProfileOpts::BxlProfile(_) => {
...
) -> kuro_error::Result<impl Future<Output = ()>> {
...
mod eden_health {
    ...
    use kuro_eden::connection::EdenConnectionManager;
    use kuro_eden::error::ErrorFromHangingMount;
    use kuro_eden::semaphore;
    ...
    pub(crate) async fn edenfs_health_check(fb: fbinit::FacebookInit, root: ProjectRoot) {
        tokio::task::spawn(async move {
            ...
            loop {
                ...
                match EdenConnectionManager::new(fb, &root, Some(semaphore::kuro_default())) {
                    Ok(Some(conn)) => {
                        let info = conn
                            .with_eden(|eden| {
                        ...
                        match info {
                            Err(e) if e.is_caused_by_hanging_mount() => {
                                ...
                                soft_error!(
                                    ...
                                    kuro_error!(kuro_error::ErrorTag::Input, "check failed with: {:#}", e),
                                    quiet: true
                                )
                                .ok();
                            }
                    ...
                    Err(e) => {
                        ...
                        soft_error!(
                            ...
                            kuro_error!(kuro_error::ErrorTag::Input, "client creation failed with: {:#}", e),
                            quiet: true
                        )
                        .ok();
                    }
