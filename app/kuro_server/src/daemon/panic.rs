...
fn get_panic_dump_dir() -> PathBuf {
    temp_dir().join("kuro-dumps")
}

async fn remove_old_panic_dumps() -> kuro_error::Result<()> {
...
fn daemon_panic_hook(daemon_state: &Arc<dyn DaemonStatePanicDiceDump>, info: &PanicHookInfo) {
    if !kuro_core::is_open_source()
        && kuro_env!("BUCK2_DICE_DUMP_ON_PANIC", bool).unwrap_or_default()
...
fn maybe_dice_dump(
    ...
    if is_dice_panic {
        ...
        eprintln!(
            "Kuro panicked and DICE may be responsible. Please be patient as we try to dump DICE graph to `{dice_dump_folder:?}` and create an archive file"
        );
        if let Err(e) = daemon_state.dice_dump(&dice_dump_folder, DiceDumpFormat::Tsv) {
            ...
            let maybe_report_msg = if cfg!(fbcode_build) {
                format!(
                    "Please upload the report via `jf upload {}.tar.gz` and then report to https://fb.workplace.com/groups/kurousers. ",
                    dice_dump_folder.display()
                )
            } else {
                "".to_owned()
