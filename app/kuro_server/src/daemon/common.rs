...
use kuro_execute_impl::executors::action_cache::ActionCacheChecker;
use kuro_execute_impl::executors::action_cache::RemoteDepFileCacheChecker;
use kuro_execute_impl::executors::action_cache_upload_permission_checker::ActionCacheUploadPermissionChecker;
use kuro_execute_impl::executors::caching::CacheUploader;
use kuro_execute_impl::executors::hybrid::FallbackTracker;
use kuro_execute_impl::executors::hybrid::HybridExecutor;
use kuro_execute_impl::executors::local::ForkserverAccess;
use kuro_execute_impl::executors::local::LocalExecutor;
use kuro_execute_impl::executors::re::ReExecutor;
use kuro_execute_impl::executors::stacked::StackedExecutor;
use kuro_execute_impl::executors::to_re_platform::RePlatformFieldsToRePlatform;
...
impl HasCommandExecutor for CommandExecutorFactory {
    fn get_command_executor(
        ...
        if !kuro_core::is_open_source() && !cfg!(fbcode_build) {
        ...
        let response = match &executor_config.executor {
            ...
            Executor::RemoteEnabled(remote_options) => {
                ...
                let disable_caching =
                    kuro_env!("BUCK2_TEST_DISABLE_CACHING", type=bool, applicability=testing)?
                ...
                let only_remote_dep_file_cache = kuro_env!(
        ...
        let response = response
            .with_buck_error_context(|| format!("The desired execution strategy (`{:?}`) is incompatible with the executor config that was selected: {:?}", self.strategy, executor_config)).tag(kuro_error::ErrorTag::Input)?;
