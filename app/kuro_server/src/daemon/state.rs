...
use kuro_common::init::SystemWarningConfig;
use kuro_common::init::Timeout;
...
use kuro_execute::execute::blocking::BuckBlockingExecutor;
use kuro_execute::execute::blocking::DirectIoExecutor;
...
use kuro_execute_impl::materializers::deferred::AccessTimesUpdates;
use kuro_execute_impl::materializers::deferred::DeferredMaterializer;
use kuro_execute_impl::materializers::deferred::DeferredMaterializerConfigs;
use kuro_execute_impl::materializers::deferred::TtlRefreshConfiguration;
use kuro_execute_impl::materializers::deferred::clean_stale::CleanStaleConfig;
...
use kuro_file_watcher::file_watcher::FileWatcher;
...
use kuro_resource_control::memory_tracker;
...
pub struct DaemonStateData {
    ...
    /// What kuro state to store on disk, ex. materializer state on sqlite
...
impl DaemonStateData {
    pub fn dice_dump(&self, path: &Path, format: DiceDumpFormat) -> kuro_error::Result<()> {
...
impl DaemonStatePanicDiceDump for DaemonStateData {
    fn dice_dump(&self, path: &Path, format: DiceDumpFormat) -> kuro_error::Result<()> {
...
impl DaemonState {
    ...
    async fn init_data(
        ...
        cgroup_tree: Option<BuckCgroupTree>,
        daemon_id: DaemonId,
    ) -> kuro_error::Result<Arc<DaemonStateData>> {
        if kuro_env!(
        ...
        rt.spawn(async move {
            ...
            let default_digest_algorithm =
                kuro_env!("BUCK_DEFAULT_DIGEST_ALGORITHM", type=DigestAlgorithmFamily)?;
    ...
    ) -> kuro_error::Result<Arc<dyn Materializer>> {
    ...
    fn init_scribe_sink(
        fb: FacebookInit,
        config: ScribeConfig,
    ) -> kuro_error::Result<Option<Arc<dyn EventSinkWithStats>>> {
        facebook_only();
    ...
    pub async fn prepare_events(
        &self,
        trace_id: TraceId,
    ) -> kuro_error::Result<(ChannelEventSource, EventDispatcher)> {
        // facebook only: logging events to Scribe.
        facebook_only();
        let (events, sink) = kuro_events::create_source_sink_pair();
        let data = self.data();
    ...
    pub async fn prepare_command(
    ...
    ) -> kuro_error::Result<BaseServerCommandContext> {
        let data = self.data();

        dispatcher.instant_event(kuro_data::RestartConfiguration {
        ...
        dispatcher.instant_event(kuro_data::TagEvent {
        ...
        dispatcher.instant_event(kuro_data::IoProviderInfo { eden_version });
    ...
    pub fn validate_cwd(&self) -> kuro_error::Result<()> {
    ...
    pub fn validate_buck_out_mount(&self) -> kuro_error::Result<()> {
        #[cfg(fbcode_build)]
        {
...
) -> kuro_error::Result<HttpClientBuilder> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_from_startup_config_defaults_internal() -> kuro_error::Result<()> {
        ...
        assert_eq!(
            ...
            kuro_certs::certs::supports_vpnless()
        );
        assert_eq!(
