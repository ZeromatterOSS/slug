...
use kuro_cli_proto::UnstableCrashRequest;
use kuro_cli_proto::unstable_crash_request::CrashType;

pub(crate) fn crash(req: UnstableCrashRequest) -> kuro_error::Result<GenericResponse> {
    let crash_type = CrashType::try_from(req.crash_type).map_err(|_| {
        kuro_error::kuro_error!(kuro_error::ErrorTag::CrashRequested, "{}", "bad request")
    })?;
    match crash_type {
        CrashType::Panic => {
