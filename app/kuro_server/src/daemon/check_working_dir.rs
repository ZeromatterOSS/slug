...
pub fn check_working_dir() -> kuro_error::Result<()> {
    use std::fs;
    use std::io;
    ...
    if err.kind() == io::ErrorKind::NotConnected {
        let err = "Kuro is running in an Eden mount but Eden restarted uncleanly. \
            This error is unrecoverable and you should restart Buck using `kuro killall`.";
    ...
    if err.kind() != io::ErrorKind::NotFound {
        tracing::warn!(
            "Kuro is unable to read its current working directory: {}. Consider restarting",
            err
        );
    }
