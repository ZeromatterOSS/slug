...
use kuro_cli_proto::new_generic::MaterializeRequest;
use kuro_cli_proto::new_generic::MaterializeResponse;
...
pub(crate) async fn materialize_command(
...
) -> kuro_error::Result<MaterializeResponse> {
    let start_event = context
        .command_start_event(kuro_data::MaterializeCommandStart {}.into())
        .await?;
    span_async(start_event, async move {
        let result = materialize(&context.base_context, req.paths)
            ...
            .map_err(Into::into);
        let end_event = command_end(&result, kuro_data::MaterializeCommandEnd {});
