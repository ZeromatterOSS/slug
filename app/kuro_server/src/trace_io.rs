...
use kuro_cli_proto::trace_io_response;
...
pub(crate) async fn trace_io_command(
    ...
    req: kuro_cli_proto::TraceIoRequest,
) -> kuro_error::Result<kuro_cli_proto::TraceIoResponse> {
    let start_event = context
        .command_start_event(kuro_data::TraceIoCommandStart {}.into())
        .await?;
    span_async(start_event, async move {
        ...
        let result = match (tracing_provider, respond_with_trace) {
            ...
            (Some(_), false) => Ok(kuro_cli_proto::TraceIoResponse {
                enabled: true,
                trace: Vec::new(),
            ...
            (None, _) => Ok(kuro_cli_proto::TraceIoResponse {
                enabled: false,
                trace: Vec::new(),
                external_entries: Vec::new(),
                relative_symlinks: Vec::new(),
        ...
        let end_event = command_end(&result, kuro_data::TraceIoCommandEnd {});
...
async fn build_response_with_trace(
    ...
    Ok(kuro_cli_proto::TraceIoResponse {
        enabled: true,
        trace,
        external_entries,
        relative_symlinks,
