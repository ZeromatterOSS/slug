...
pub(crate) async fn clean_stale_command(
    ...
    req: kuro_cli_proto::CleanStaleRequest,
...
impl ServerCommandTemplate for CleanStaleServerCommand {
    type StartEvent = kuro_data::CleanCommandStart;
    type EndEvent = kuro_data::CleanCommandEnd;
    type Response = kuro_cli_proto::CleanStaleResponse;
    ...
    fn end_event(&self, response: &kuro_error::Result<Self::Response>) -> Self::EndEvent {
        let clean_stale_stats = if let Ok(res) = response {
            res.stats
        } else {
            None
        };
        kuro_data::CleanCommandEnd { clean_stale_stats }
    }
}
