...
pub(crate) async fn file_status_command(
    ...
    req: kuro_cli_proto::FileStatusRequest,
...
impl ServerCommandTemplate for FileStatusServerCommand {
    type StartEvent = kuro_data::FileStatusCommandStart;
    type EndEvent = kuro_data::FileStatusCommandEnd;
    type Response = kuro_cli_proto::GenericResponse;
    ...
    async fn command(
        ...
        if result.bad != 0 {
            ...
            Ok(kuro_cli_proto::GenericResponse {})
        }
    }
}
    ...
    match (fs_metadata, dice_metadata) {
        ...
        (RawPathMetadata::Directory, RawPathMetadata::Directory) => {
            ...
            let mut fs_names = fs_read_dir
                .iter()
                .map(|f| kuro_error::Ok(FileName::new(&f.file_name)?.to_owned()))
