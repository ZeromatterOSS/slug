        ...
        while let Some(event) = tasks.next().await {
            if let Some(error) = &event.command_error {
                soft_error!(
                    ...
                    kuro_error::kuro_error!(kuro_error::ErrorTag::Input, "{}", error),
                    quiet: true
                )
                .ok();
            }
...
) -> kuro_data::VersionControlRevision {
    let mut revision = kuro_data::VersionControlRevision::default();
...
async fn create_hg_data(
    revision: &mut kuro_data::VersionControlRevision,
    revision_type: RevisionDataType,
    repo_root: &AbsNormPathBuf,
) {
    match revision_type {
...
async fn get_hg_status(revision: &mut kuro_data::VersionControlRevision) {
...
async fn repo_type(repo_root: &AbsNormPathBuf) -> kuro_error::Result<&'static RepoVcs> {
    static REPO_TYPE: OnceCell<kuro_error::Result<RepoVcs>> = OnceCell::const_new();
    async fn repo_type_impl(repo_root: &AbsNormPathBuf) -> kuro_error::Result<RepoVcs> {
