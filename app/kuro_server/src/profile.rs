...
use kuro_cli_proto::target_profile::Action;
use kuro_common::pattern::parse_from_cli::parse_and_resolve_patterns_from_cli_args;
...
use kuro_profile::get_profile_response;
...
) -> kuro_error::Result<Arc<StarlarkProfileDataAndStats>> {
...
impl ServerCommandTemplate for ProfileServerCommand {
    type StartEvent = kuro_data::ProfileCommandStart;
    type EndEvent = kuro_data::ProfileCommandEnd;
    type Response = kuro_cli_proto::ProfileResponse;
...
async fn generate_profile(
    ...
    match action {
        ...
        Action::Loading => {
            ...
            let profiles = kuro_util::future::try_join_all(resolved.specs.into_iter().map(
