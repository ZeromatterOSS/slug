...
use kuro_build_api::build_signals::BuildSignalsInstaller;
use kuro_build_api::build_signals::SetBuildSignals;
use kuro_build_api::build_signals::create_build_signals;
use kuro_build_api::context::SetBuildContextData;
use kuro_build_api::keep_going::HasKeepGoing;
use kuro_build_api::materialize::HasMaterializationQueueTracker;
...
use kuro_build_signals::env::HasCriticalPathBackend;
...
use kuro_cli_proto::client_context::HostArchOverride;
...
use kuro_execute::re::client::RemoteExecutionClient;
use kuro_execute::re::manager::ReConnectionHandle;
use kuro_execute::re::manager::ReConnectionObserver;
...
use kuro_server_ctx::bxl::InitBxlStreamingTracker;
use kuro_server_ctx::concurrency::DiceUpdater;
use kuro_server_ctx::ctx::DiceAccessor;
use kuro_server_ctx::ctx::LockedPreviousCommandData;
use kuro_server_ctx::ctx::PrivateStruct;
use kuro_server_ctx::ctx::ServerCommandContextTrait;
use kuro_server_ctx::stderr_output_guard::StderrOutputGuard;
use kuro_server_ctx::stderr_output_guard::StderrOutputWriter;
use kuro_server_starlark_debug::BuckStarlarkDebuggerHandle;
use kuro_server_starlark_debug::create_debugger_handle;
use kuro_test::local_resource_registry::InitLocalResourceRegistry;
use kuro_util::arc_str::ArcS;
use kuro_util::truncate::truncate_container;
use kuro_validation::enabled_optional_validations_key::SetEnabledOptionalValidations;
use tracing::warn;
...
fn parse_concurrency(requested: u32) -> Option<usize> {
    let ret: usize = requested
        .try_into()
        .expect("Kuro isn't built for 16 bit systems");
...
impl<'a> ServerCommandContext<'a> {
    pub fn new(
        ...
        let working_dir_project_relative = working_dir
            ...
            .map_err(|_| {
                Into::<kuro_error::Error>::into(DaemonCommunicationError::InvalidWorkingDirectory(
        ...
        impl ReConnectionObserver for Observer {
            fn session_created(&self, client: &RemoteExecutionClient) {
                ...
                self.events
                    .instant_event(kuro_data::RemoteExecutionSessionCreated {
        ...
        kuro_build_api::interpreter::rule_defs::build_config::set_copts(&client_context.copts);
        kuro_build_api::interpreter::rule_defs::build_config::set_cxxopts(&client_context.cxxopts);
        ...
        kuro_build_api::interpreter::rule_defs::build_config::set_strip(&client_context.strip_mode);
        ...
        kuro_build_api::interpreter::rule_defs::build_config::set_stamp(stamp_enabled);
    ...
    ) -> kuro_error::Result<DiceCommandUpdater<'s, 'a>> {
    ...
    pub(crate) async fn finalize(mut self) -> kuro_error::Result<()> {
...
impl ServerCommandContext<'_> {
    ...
    ) -> kuro_error::Result<BuckConfigBasedCells> {
...
impl DiceCommandUpdater<'_, '_> {
    fn make_user_computation_data(
        ...
        // They currently use either a usecase specified in actions (cas_artifact), or a global default (kuro.default_remote_execution_use_case).
        ...
        self.cmd_ctx
            .events()
            .instant_event(kuro_data::TagEvent { tags });

        self.cmd_ctx
            .events()
            .instant_event(kuro_data::CommandOptions {
                configured_parallelism: concurrency as _,
                available_parallelism: kuro_util::threads::available_parallelism() as _,
            });
...
impl ServerCommandContextTrait for ServerCommandContext<'_> {
    ...
    ) -> kuro_error::Result<DiceAccessor<'s>> {
    ...
    fn stderr(&self) -> kuro_error::Result<StderrOutputGuard<'_>> {
    ...
    async fn command_start_event(
        &self,
        data: kuro_data::command_start::Data,
    ) -> kuro_error::Result<kuro_data::CommandStart> {
        Ok(kuro_data::CommandStart {
    ...
    async fn request_metadata(&self) -> kuro_error::Result<HashMap<String, String>> {
        // Facebook only: metadata collection for Scribe writes
        facebook_only();
    ...
    ) -> kuro_error::Result<HashMap<String, String>> {
        ctx.per_transaction_data()
            .data
            .get::<ConfigMetadataHolder>()
            .map(|holder| holder.0.clone())
            .map_err(|_| kuro_error::internal_error!("Config metadata not set"))
    }
    ...
    fn log_target_pattern(
        ...
        let patterns = providers_patterns.map(|pat| kuro_data::TargetPattern {
            value: format!("{pat}"),
        });

        self.events()
            .instant_event(kuro_data::ParsedTargetPatterns {
    ...
    fn log_target_pattern_with_modifiers(
        ...
        let patterns = seen_values
            .into_iter()
            .map(|pat| kuro_data::TargetPattern { value: pat })
            .collect();
