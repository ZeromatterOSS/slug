...
use slug_common::cas_digest::RawDigest;
...
use slug_execute::materialize::http::Checksum;
use slug_execute::materialize::http::http_download;
use slug_execute::materialize::http::http_head;
...
impl DownloadFileAction {
    ...
    ) -> slug_error::Result<Option<FileMetadata>> {
        ...
        let size = match self.inner.size_bytes {
            ...
            None => {
                ...
                let head = http_head(client, url).await.map_err(|e| {
                    slug_error::Error::from(e).tag([ErrorTag::DownloadFileHeadRequest])
                })?;
                ...
                head.headers()
                    ...
                    .map(|content_length| {
                        let content_length = content_length
                            .to_str()
                            .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::Http))
                        ...
                        slug_error::Ok(content_length_number)
                    })
                    .transpose()
    ...
    ) -> slug_error::Result<(ActionOutputs, ActionExecutionMetadata)> {
...
impl Action for DownloadFileAction {
    fn kind(&self) -> slug_data::ActionKind {
        slug_data::ActionKind::DownloadFile
    }
