...
impl UnregisteredSymlinkedDirAction {
    ...
    pub(crate) fn new<'v>(
        ...
        match copy {
            CopyMode::Symlink | CopyMode::SymlinkToArtifactPath => Self::validate_args(&mut args)?,
            CopyMode::Copy { .. } => (),
        };
        Ok(Self {
            copy,
...
impl Action for SymlinkedDirAction {
    fn kind(&self) -> slug_data::ActionKind {
        slug_data::ActionKind::SymlinkedDir
    }
    ...
    async fn execute(
        ...
        for (group, relative_dest) in &self.args {
            ...
            match self.copy {
                ...
                CopyMode::Symlink | CopyMode::SymlinkToArtifactPath => {
                    if self.output().get_path().is_content_based_path() {
                        builder.add_external_symlinked(value, src, temp_dest.as_ref())?;
                    } else {
...
mod tests {
    ...
    fn test_symlinked_dir_validation() {
        fn validate(paths: &[&str]) -> slug_error::Result<()> {
