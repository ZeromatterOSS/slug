...
use slug_build_api::actions::execute::action_execution_target::ActionExecutionTarget;
...
use slug_build_api::actions::impls::expanded_command_line::ExpandedCommandLine;
use slug_build_api::actions::impls::expanded_command_line::ExpandedCommandLineDigest;
use slug_build_api::actions::impls::expanded_command_line::ExpandedCommandLineFingerprinter;
...
use slug_build_api::interpreter::rule_defs::artifact::starlark_artifact_value::StarlarkArtifactValue;
use slug_build_api::interpreter::rule_defs::artifact::starlark_output_artifact::FrozenStarlarkOutputArtifact;
...
use slug_build_api::interpreter::rule_defs::cmd_args::FrozenParamFileData;
use slug_build_api::interpreter::rule_defs::cmd_args::FrozenStarlarkCmdArgs;
use slug_build_api::interpreter::rule_defs::cmd_args::ParamFileFormat as StarlarkParamFileFormat;
...
use slug_build_api::interpreter::rule_defs::provider::builtin::worker_info::FrozenWorkerInfo;
use slug_build_api::interpreter::rule_defs::provider::builtin::worker_info::WorkerInfo;
...
use slug_execute::execute::cache_uploader::force_cache_upload;
...
use slug_execute::execute::request::ParamFileSlot;
use slug_execute::execute::request::RemoteWorkerSpec;
...
impl FrozenStarlarkRunActionValues {
    pub(crate) fn worker<'v>(
        &'v self,
    ) -> slug_error::Result<Option<ValueOf<'v, &'v WorkerInfo<'v>>>> {
...
impl RunAction {
    ...
    ) -> slug_error::Result<UnpackedRunActionValues<'v>> {
    ...
    fn expand_command_line_and_worker<'v>(
            ...
            let worker_key = if worker.supports_bazel_remote_persistent_worker_protocol {
                    ...
                    && !worker_visitor.frozen_outputs.is_empty()
                {
                    ...
                    return Err(slug_error!(
                        slug_error::ErrorTag::ActionMismatchedOutputs,
        ...
        let remote_worker = if let Some(remote_worker) = values.remote_worker {
            ...
            let remote_worker_env: slug_error::Result<SortedVectorMap<_, _>> = remote_worker
                .env
                .into_iter()
                .map(|(k, v)| {
                    let mut env = String::new();
        ...
        let cli_env: slug_error::Result<SortedVectorMap<_, _>> = values
            .env
            .into_iter()
            .map(|(k, v)| {
                let mut env = String::new();
    ...
    ) -> slug_error::Result<ControlFlow<CommandExecutionResult, ()>> {
    ...
    async fn output_paths_as_inputs(
        &self,
        ctx: &dyn ActionExecutionCtx,
    ) -> slug_error::Result<Vec<CommandExecutionInput>> {
        let executor_fs = ctx.executor_fs();
        let fs = executor_fs.fs();
        let output_paths = {
    ...
    fn outputs_for_error_handler(&self) -> slug_error::Result<Vec<BuildArtifactPath>> {
        self.starlark_values
            ...
            .map(|artifact| {
                ...
                match a.as_parts().0 {
                    BaseArtifactKind::Source(s) => Err(slug_error::slug_error!(
...
fn rendered_action_path_rewrites(
    ...
    cli_ctx: &DefaultCommandLineContext,
    fs: &ExecutorFs<'_>,
) -> slug_error::Result<Vec<(String, String)>> {
...
impl RunAction {
    ...
    async fn execute_for_offline(
        &self,
        ctx: &mut dyn ActionExecutionCtx,
    ) -> slug_error::Result<Option<(ActionOutputs, ActionExecutionMetadata)>> {
...
impl Action for RunAction {
    ...
    ) -> slug_error::Result<ValueOfUnchecked<'v, DictType<StarlarkArtifact, StarlarkArtifactValue>>>
    {
    ...
    async fn execute(
        ...
        let incremental_kind = match (
            ...
            (true, true) => slug_data::IncrementalKind::IncrementalLocalAndRemote,
            (false, true) => slug_data::IncrementalKind::IncrementalRemote,
            (true, false) => slug_data::IncrementalKind::IncrementalLocal,
            (false, false) => slug_data::IncrementalKind::NonIncremental,
        };
