...
use slug_artifact::artifact::artifact_type::BoundBuildArtifact;
...
impl DynamicActionsOutputArtifactBinder {
    ...
    ) -> slug_error::Result<BoundBuildArtifact> {
...
) -> slug_error::Result<Box<[ValueTyped<'v, StarlarkOutputArtifact<'v>>]>> {
...
pub(crate) fn analysis_actions_methods_dynamic_output(methods: &mut MethodsBuilder) {
    ...
    /// https://slug.build/docs/rule_authors/dynamic_dependencies/.
    ///
    ...
    fn dynamic_output<'v>(
        ...
        if outputs.items.is_empty() {
            return Err(slug_error::Error::from(DynamicOutputError::EmptyOutput).into());
        }
    ...
    /// overview of dynamic dependencies in slug.
    ///
