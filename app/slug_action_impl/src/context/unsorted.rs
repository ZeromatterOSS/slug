...
use slug_build_api::interpreter::rule_defs::artifact::starlark_artifact_like::StarlarkArtifactLike;
...
use slug_build_api::interpreter::rule_defs::cmd_args::value::CommandLineArg;
use slug_build_api::interpreter::rule_defs::context::AnalysisActions;
use slug_build_api::interpreter::rule_defs::digest_config::StarlarkDigestConfig;
...
/// `<output_base>/external/<repo>/<path>`. Slug's path normalizer rejects
...
pub(crate) fn analysis_actions_methods_unsorted(builder: &mut MethodsBuilder) {
    ...
    /// Android split configurations). In Slug, this is an alias for `declare_file`
    ...
    /// since Buck2/Slug handles symlinks transparently during execution.
    fn declare_symlink<'v>(
        ...
        // Symlink artifacts are treated as regular file artifacts in Slug.
    ...
    /// Creates a new transitive set. For details, see https://slug.build/docs/rule_authors/transitive_sets/.
    fn tset<'v>(
        this: &AnalysisActions<'v>,
    ...
    /// 3. **Subsequent builds**: Slug only triggers rebuilds when files listed in the depfile change
    ///
    /// ### Usage Pattern
    ///
    ...
    /// ```
    ///
    /// This tells Slug that `output.o` depends on these specific files. Slug reads this file
    ...
    /// for equality, allowing Slug to match tagged inputs with their corresponding depfiles.
    ///
    /// ### See Also
    ///
    ...
    /// This is equivalent to Slug's cmd_args() global function.
    ///
    ...
    /// In Slug, we fail immediately at analysis time since there's no benefit
