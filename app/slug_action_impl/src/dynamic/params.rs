...
use slug_build_api::interpreter::rule_defs::plugins::FrozenAnalysisPlugins;
...
pub(crate) struct DynamicLambdaStaticFields {
    ...
    /// Execution platform inherited from the owner to use for actionsfbcode/slug/app/slug_action_impl/src/dynamic/deferred.rs
...
impl FrozenDynamicLambdaParams {
    pub(crate) fn attributes<'v>(
        &'v self,
    ) -> slug_error::Result<Option<ValueOfUnchecked<'v, StructRef<'static>>>> {
    ...
    pub(crate) fn plugins<'v>(
        &'v self,
    ) -> slug_error::Result<Option<ValueTypedComplex<'v, AnalysisPlugins<'v>>>> {
