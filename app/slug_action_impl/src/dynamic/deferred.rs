...
use slug_build_api::dynamic::calculation::dynamic_lambda_result;
use slug_build_api::dynamic_value::DynamicValue;
...
async fn execute_lambda(
    ...
    TimeSpan,
    SmallVec<[SpanId; 1]>,
    slug_error::Result<RecordedAnalysisValues>,
) {
    if let BaseDeferredKey::BxlLabel(key) = self_key.owner().dupe() {
        ...
        let (res, spans) = record_root_spans(|| {
            span(start_event, || {
                ...
                let output: slug_error::Result<_> = try {
                    let env = Module::new();
                ...
                (
                    ...
                    slug_data::AnalysisEnd {
                        target: Some(slug_data::analysis_end::Target::DynamicLambda(
                            self_key.owner().to_proto().into(),
                        )),
                        rule: proto_rule,
                        profile: None,
    ...
    span_async_simple(
        ...
        async move {
            ...
            let (input_artifacts_materialized, resolved_dynamic_values) = span_async_simple(
                slug_data::DeferredPreparationStageStart {
                    stage: Some(slug_data::MaterializedArtifacts {}.into()),
                },
                ctx.try_compute2(
                ...
                slug_data::DeferredPreparationStageEnd {},
            )
            .await?;
        ...
        slug_data::DeferredEvaluationEnd {},
    )
    .await
}
...
async fn ensure_artifacts_built(
    ...
    ctx: &mut DiceComputations<'_>,
) -> slug_error::Result<Vec<ArtifactGroupValues>> {
...
async fn materialize_inputs(
    ensured_artifacts: &IndexMap<&Artifact, &ArtifactValue>,
    ctx: &mut DiceComputations<'_>,
) -> slug_error::Result<InputArtifactsMaterialized> {
...
async fn resolve_dynamic_values(
    ...
    ctx: &mut DiceComputations<'_>,
) -> slug_error::Result<HashMap<DynamicValue, FrozenProviderCollectionValue>> {
        ...
        .try_compute_join(dynamic_values, |ctx, dynamic_value| {
            Box::pin(async {
                let result = dynamic_lambda_result(ctx, &dynamic_value.dynamic_lambda_results_key)
                    ...
                    .provider_collection()?
                    .to_owned();
                slug_error::Ok((dynamic_value.dupe(), result))
            })
        })
        .await?;
...
fn new_attr_values<'v>(
    ...
    resolved_dynamic_values: &HashMap<DynamicValue, FrozenProviderCollectionValue>,
    env: &'v Module,
) -> slug_error::Result<Box<[(String, Value<'v>)]>> {
...
pub fn dynamic_lambda_ctx_data<'v>(
    ...
    digest_config: DigestConfig,
    env: &'v Module,
) -> slug_error::Result<DynamicLambdaCtxData<'v>> {
