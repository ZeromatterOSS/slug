...
impl<'v> DynamicAttrValue<Value<'v>> {
    fn bind(&self, bind: &mut DynamicActionsOutputArtifactBinder) -> slug_error::Result<()> {
        match self {
...
impl<'v> DynamicAttrValues<Value<'v>> {
    pub(crate) fn bind(&self, key: &DynamicLambdaResultsKey) -> slug_error::Result<()> {
...
impl DynamicAttrType {
    ...
    pub(crate) fn coerce<'v>(
        &self,
        value: Value<'v>,
    ) -> slug_error::Result<DynamicAttrValue<Value<'v>>> {
        match self {
