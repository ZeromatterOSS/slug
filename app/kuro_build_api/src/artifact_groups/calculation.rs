...
use kuro_artifact::artifact::artifact_type::ArtifactKind;
...
use kuro_directory::directory::directory_data::DirectoryData;
...
use kuro_execute::directory::insert_artifact;
...
pub trait ArtifactGroupCalculation {
    ...
    async fn ensure_artifact_group(
        &mut self,
        input: &ArtifactGroup,
    ) -> kuro_error::Result<ArtifactGroupValues>;
}
...
impl EnsureArtifactGroupReady {
    ...
    fn unpack_single(self) -> kuro_error::Result<ArtifactValue> {
        match self {
...
async fn dir_artifact_value(
    ...
    // `PathMetadataKey` of every file inside that directory, blowing up Kuro's memory use.
    ...
    impl Key for DirArtifactValueKey {
        type Value = kuro_error::Result<ArtifactValue>;
            ...
            let entry_values = ctx
                .try_compute_join(files.iter(), |ctx, x| {
                    async move {
                        ...
                        .await?;
                        kuro_error::Ok((x.file_name.clone(), value))
                    }
                    .boxed()
                })
                .await?;
...
enum ProjectedArtifactError {
    ...
    #[kuro(tag = kuro_error::ErrorTag::ProjectMissingPath)]
...
impl Key for EnsureTransitiveSetProjectionKey {
    type Value = kuro_error::Result<ArtifactGroupValues>;
