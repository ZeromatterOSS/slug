...
impl AnalysisData {
    pub fn providers(&self) -> kuro_error::Result<FrozenProviderCollectionValue> {
...
impl ActionQueryNodeRef {
    pub fn require_action(&self) -> kuro_error::Result<&ActionKey> {
        match self {
            Self::Analysis(a) => Err(kuro_error::kuro_error!(
...
impl QueryTarget for ActionQueryNode {
    ...
    fn buildfile_path(&self) -> &BuildFilePath {
        // TODO(cjhopman): In addition to implementing this, we should be able to return an kuro_error::Error here rather than panicking.
...
pub static FIND_MATCHING_ACTION: LateBinding<
    for<'c> fn(
        ...
        Box<dyn Future<Output = kuro_error::Result<Option<ActionQueryNode>>> + Send + 'c>,
    >,
> = LateBinding::new("FIND_MATCHING_ACTION");

/// Hook to link printer in `kuro_server_commands` from `kuro_cmd_audit_server`.
pub static PRINT_ACTION_NODE: LateBinding<
    ...
    ) -> Pin<Box<dyn Future<Output = kuro_error::Result<()>> + Send + 'a>>,
> = LateBinding::new("PRINT_ACTION_NODE");

/// Use of "configured_attr_to_value" in `kuro_transition` from `kuro_analysis`.
