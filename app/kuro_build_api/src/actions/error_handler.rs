...
fn action_error_context_methods(builder: &mut MethodsBuilder) {
    ...
    /// should not go here - kuro tries to categorize these types of errors automatically.
    ...
    fn parse_with_errorformat<'v>(
        ...
        let error_lines = kuro_errorformat::split_lines(&error);
        let error_entries = kuro_errorformat::parse_error_format(errorformats.items, error_lines)
