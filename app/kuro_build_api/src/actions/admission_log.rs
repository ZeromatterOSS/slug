...
//!
//! When the `KURO_LOG_ADMISSION` env var is set to a path, every action
...
//! latency — how long a ready action waits before kuro's scheduler
//! dispatches it.
//!
...
fn writer() -> Option<&'static Mutex<BufWriter<File>>> {
    WRITER
        .get_or_init(|| match std::env::var("KURO_LOG_ADMISSION") {
