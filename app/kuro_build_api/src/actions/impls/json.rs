...
struct KuroErrorResultOfSerializedValue<'a, 'v> {
    result: kuro_error::Result<SerializeValue<'a, 'v>>,
}

impl<'a, 'v> Serialize for KuroErrorResultOfSerializedValue<'a, 'v> {
...
impl<'a, 'v> SerializeValue<'a, 'v> {
    fn with_value(&self, x: Value<'v>) -> KuroErrorResultOfSerializedValue<'a, 'v> {
        KuroErrorResultOfSerializedValue {
...
fn err<R, E: serde::ser::Error>(res: kuro_error::Result<R>) -> Result<R, E> {
    match res {
        Ok(v) => Ok(v),
        Err(e) => Err(serde::ser::Error::custom(format!("{e:#}"))),
    }
...
impl<'v> JsonArtifact<'v> {
    fn artifact(&self) -> kuro_error::Result<Artifact> {
        match self {
...
pub fn validate_json(x: JsonUnpack) -> kuro_error::Result<()> {
