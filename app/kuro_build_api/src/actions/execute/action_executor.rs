...
use kuro_common::http::HasHttpClient;
...
use kuro_execute::output_size::OutputCountAndBytes;
...
pub enum ActionExecutionKind {
    ...
    /// This action is simple and executed inline within kuro (e.g. write, symlink_dir)
    #[display("simple")]
    Simple,
...
pub struct CommandExecutionRef<'a> {
    ...
    pub incremental_kind: kuro_data::IncrementalKind,
}
...
impl ActionExecutionKind {
    pub fn as_enum(&self) -> kuro_data::ActionExecutionKind {
        match self {
            ...
            ActionExecutionKind::Simple => kuro_data::ActionExecutionKind::Simple,
            ActionExecutionKind::Deferred => kuro_data::ActionExecutionKind::Deferred,
            ActionExecutionKind::LocalDepFile => kuro_data::ActionExecutionKind::LocalDepFile,
            ActionExecutionKind::LocalActionCache => {
                kuro_data::ActionExecutionKind::LocalActionCache
            }
        }
    }
...
pub trait HasActionExecutor {
    ...
    ) -> kuro_error::Result<Arc<BuckActionExecutor>>;
}
...
mod tests {
    ...
    #[test]
    fn test_cleanup_path_missing() -> kuro_error::Result<()> {
    ...
    #[test]
    fn test_cleanup_path_present() -> kuro_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let fs = fs.path();
        fs_util::create_dir_all(fs.resolve(ProjectRelativePath::unchecked_new("foo/bar/qux")))?;
    ...
    #[test]
    fn test_cleanup_path_overlap() -> kuro_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let fs = fs.path();
    ...
    #[test]
    fn test_cleanup_path_overlap_deep() -> kuro_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let fs = fs.path();
        fs.write_file(ProjectRelativePath::unchecked_new("foo/bar"), "xx", false)?;
        cleanup_path(
