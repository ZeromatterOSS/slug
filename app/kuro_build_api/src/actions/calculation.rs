...
use kuro_data::ActionErrorDiagnostics;
use kuro_data::ActionSubErrors;
use kuro_data::ToProtoMessage;
use kuro_data::get_action_digest;
use kuro_error::BuckErrorContext;
use kuro_event_observer::action_util::get_execution_time_ms;
...
async fn build_action_impl(
    ...
    key: &ActionKey,
) -> kuro_error::Result<ActionOutputs> {
...
async fn build_action_inner(
    ...
    let is_expected_eligible_for_dedupe = match action.is_expected_eligible_for_dedupe() {
        Some(v) => {
            if v {
                kuro_data::ExpectedEligibleForDedupe::ExpectedEligible
            } else {
                kuro_data::ExpectedEligibleForDedupe::ExpectedIneligible
            }
        }
        None => kuro_data::ExpectedEligibleForDedupe::UnknownEligibility,
    };
    ...
    let mut kuro_revision = None;
    let mut kuro_build_time = None;
    ...
    let error_diagnostics = match execute_result {
        ...
        Err(e) => {
            ...
            // so as to reduce Scribe traffic and log it in kuro_action_errors
            kuro_revision = kuro_build_info::revision().map(|s| s.to_owned());
            kuro_build_time = kuro_build_info::time_iso8601().map(|s| s.to_owned());
            hostname = kuro_events::metadata::hostname();
            ...
            action_result = Err(kuro_error::Error::from(e)
    ...
    let invalidation_info = if executor.invalidation_tracking_enabled() {
        fn to_proto(
        ...
        ) -> Option<kuro_data::command_invalidation_info::InvalidationSource> {
            match invalidation_path {
                ...
                dice::DiceTrackedInvalidationPath::Invalidated(_) => {
                    Some(kuro_data::command_invalidation_info::InvalidationSource {})
                }
            }
        }
        let invalidation_paths = ctx.get_invalidation_paths();
        Some(kuro_data::CommandInvalidationInfo {
    ...
    let execution_kind = execution_kind.unwrap_or(kuro_data::ActionExecutionKind::NotSet);
    ...
    (
        ...
        Box::new(kuro_data::ActionExecutionEnd {
            key: Some(action_key),
            kind: action.kind().into(),
...
fn is_action_eligible_for_dedupe(
...
) -> kuro_data::EligibleForDedupe {
    if !action.all_outputs_are_content_based() {
        return kuro_data::EligibleForDedupe::IneligibleOutput;
    }
    ...
    for (ag, _agv) in inputs.iter() {
        if !ag.is_eligible_for_dedupe(target_platform) {
            return kuro_data::EligibleForDedupe::IneligibleInput;
        }
    }

    kuro_data::EligibleForDedupe::Eligible
}
...
fn try_run_error_handler(
    ...
    artifact_fs: kuro_error::Result<ArtifactFs>,
    outputs: Option<&ActionOutputs>,
) -> Option<ActionErrorDiagnostics> {
    use kuro_data::action_error_diagnostics::Data;

    fn create_error(
        e: kuro_error::Error,
    ) -> (
        Option<ActionErrorDiagnostics>,
        kuro_data::ActionErrorHandlerExecutionEnd,
    ) {
        (
    ...
    match action.action.error_handler() {
        Some(error_handler) => {
            ...
            dispatcher
                .clone()
                .span(kuro_data::ActionErrorHandlerExecutionStart {}, || {
                    let env = Module::new();
                    let heap = env.heap();
                    ...
                    let data = match error_handler_result {
                        ...
                        Err(e) => {
                            let e = kuro_error::Error::from(e).context("Error handler failed");
...
pub struct ActionExtraData {
    pub execution_kind: kuro_data::ActionExecutionKind,
    pub execution_time_ms: Option<u64>,
    pub output_size: u64,
    pub target_rule_type_name: Option<String>,
    pub action_digest: Option<String>,
    pub invalidation_info: Option<kuro_data::CommandInvalidationInfo>,
}

struct ActionExecutionData {
    action_result: kuro_error::Result<ActionOutputs>,
...
impl ActionCalculation {
    pub async fn get_action(
    ...
    ) -> kuro_error::Result<Arc<RegisteredAction>> {
        ...
        match deferred_holder.lookup_action(action_key)? {
            ...
            ActionLookup::Deferred(action_key) => {
                ...
                ) -> BoxFuture<'a, kuro_error::Result<Arc<RegisteredAction>>> {
    ...
    pub fn build_action<'a>(
        ctx: &'a mut DiceComputations<'_>,
        action_key: &ActionKey,
    ) -> impl Future<Output = kuro_error::Result<ActionOutputs>> + use<'a> {
        ...
        ctx.compute(BuildKey::ref_cast(action_key))
            .map(|v| v?.map_err(kuro_error::Error::from))
    }
...
impl Key for BuildKey {
    type Value = kuro_error::Result<ActionOutputs>;
...
async fn command_execution_report_to_proto(
    ...
    let status = match &report.status {
        CommandExecutionStatus::Success { .. } => kuro_data::command_execution::Success {}.into(),
        CommandExecutionStatus::Cancelled { .. } => {
            kuro_data::command_execution::Cancelled {}.into()
        }
        CommandExecutionStatus::Failure { .. } => kuro_data::command_execution::Failure {}.into(),
        CommandExecutionStatus::WorkerFailure { .. } => {
            kuro_data::command_execution::WorkerFailure {}.into()
        }
        CommandExecutionStatus::TimedOut { duration, .. } => {
            kuro_data::command_execution::Timeout {
...
pub async fn command_details(
    ...
    allow_omit_details: bool,
) -> kuro_data::CommandExecutionDetails {
