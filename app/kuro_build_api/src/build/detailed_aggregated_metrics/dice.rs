...
pub trait HasDetailedAggregatedMetrics {
    fn action_executed(&self, ev: ActionExecutionMetrics) -> kuro_error::Result<()>;
    fn analysis_started(&self, key: &DeferredHolderKey) -> kuro_error::Result<()>;
    fn analysis_complete(
        &self,
        key: &DeferredHolderKey,
        result: &DeferredHolder,
    ) -> kuro_error::Result<()>;
    fn top_level_target(&self, spec: TopLevelTargetSpec) -> kuro_error::Result<()>;
    fn take_per_build_events(&self) -> kuro_error::Result<PerBuildEvents>;
    fn compute_detailed_metrics(
        &self,
        events: PerBuildEvents,
    ) -> impl Future<Output = kuro_error::Result<DetailedAggregatedMetrics>> + Send;
}
...
impl HasDetailedAggregatedMetrics for DiceComputations<'_> {
    ...
    async fn compute_detailed_metrics(
        &self,
        events: PerBuildEvents,
    ) -> kuro_error::Result<DetailedAggregatedMetrics> {
        span_async_simple(
...
) -> kuro_error::Result<&'a Option<DetailedAggregatedMetricsEventHandler>> {
    ctx.global_data()
...
fn get_per_build_events_holder<'a>(
    ctx: &'a DiceComputations<'_>,
) -> kuro_error::Result<&'a DetailedAggregatedMetricsPerBuildEventsHolder> {
