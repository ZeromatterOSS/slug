...
impl<'v, V: ValueLike<'v>> TransitiveSetGen<V> {
    ...
    pub fn projection_name(&'v self, projection: usize) -> kuro_error::Result<&'v str> {
    ...
    pub fn get_projection_value(&self, projection: usize) -> kuro_error::Result<Option<V>> {
        match &self.node {
            None => Ok(None),
            Some(node) => Ok(Some(
                *node
    ...
    pub(crate) fn definition(
        &self,
    ) -> kuro_error::Result<ValueTypedComplex<'v, TransitiveSetDefinition<'v>>> {
        ...
        let node = value.try_map(|value| {
            let projections = def
                ...
                .map(|(name, spec)| {
                    ...
                    kuro_error::Ok(projected_value)
                })
                .collect::<Result<Box<[_]>, _>>()?;

            kuro_error::Ok(NodeGen { value, projections })
        })?;
...
fn transitive_set_methods(builder: &mut MethodsBuilder) {
    ...
    fn reduce<'v>(
        ...
        let index = match def.operations().reductions.get_index_of(reduction) {
            ...
            None => {
                return Err(
                    kuro_error::Error::from(TransitiveSetError::ReductionDoesNotExist {
