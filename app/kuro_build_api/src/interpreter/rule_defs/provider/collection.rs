...
enum ProviderCollectionError {
    ...
    #[error(
        "Cannot handle flavor `{flavor}` on target `{target}`. Most flavors are unsupported in Kuro."
    )]
...
impl<'v, V: ValueLike<'v>> ProviderCollectionGen<V> {
    ...
    fn try_from_value_impl(
        mut value: Value<'v>,
    ) -> kuro_error::Result<SmallMap<Arc<ProviderId>, Value<'v>>> {
    ...
    pub fn try_from_value(value: Value<'v>) -> kuro_error::Result<ProviderCollection<'v>> {
    ...
    fn get_impl(
        ...
        index: Value<'v>,
        op: GetOp,
    ) -> kuro_error::Result<Either<Value<'v>, Arc<ProviderId>>> {
    ...
    pub(crate) fn get(
        &self,
        index: Value<'v>,
    ) -> kuro_error::Result<NoneOr<ValueOfUnchecked<'v, AbstractProvider>>> {
...
impl FrozenProviderCollectionValue {
    ...
    pub fn try_from_value(value: OwnedFrozenValue) -> kuro_error::Result<Self> {
        Ok(Self {
    ...
    ) -> kuro_error::Result<FrozenProviderCollectionValueRef<'f>> {
...
impl<'f> FrozenProviderCollectionValueRef<'f> {
    ...
    pub fn lookup_inner(
        ...
        match label.name() {
            ProvidersName::Default => kuro_error::Ok(self),
            ...
            let collection = frozen
                ...
                .ok_or_else(|| {
                    kuro_error::kuro_error!(
                        kuro_error::ErrorTag::StarlarkError,
