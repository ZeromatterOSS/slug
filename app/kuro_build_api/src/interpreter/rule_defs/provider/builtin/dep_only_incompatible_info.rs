...
use kuro_core::error::validate_logview_category;
...
/// ```
///
/// Then register a target of this rule in root .buckconfig under `kuro.dep_only_incompatible_info`
    ...
    fn DepOnlyIncompatibleInfo<'v>(
        ...
        for (category, value) in custom_soft_errors.entries {
            let category_str = category.to_value().unpack_str().ok_or_else(|| {
                starlark::Error::from(kuro_error::internal_error!(
...
impl FrozenDepOnlyIncompatibleInfo {
    ...
    ) -> kuro_error::Result<DepOnlyIncompatibleCustomSoftErrors> {
        let value_to_target_pattern =
            |target_pattern: Value<'_>| -> kuro_error::Result<ParsedPattern<TargetPatternExtra>> {
