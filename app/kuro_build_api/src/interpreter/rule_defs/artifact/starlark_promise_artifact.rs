...
impl StarlarkPromiseArtifact {
    ...
    fn short_path_err(&self) -> kuro_error::Result<&ForwardRelativePath> {
        self.short_path
            .as_deref()
            .ok_or_else(|| PromiseArtifactError::NoShortPathPromised(self.clone()).into())
    }

    fn file_name_err(&self) -> kuro_error::Result<&FileName> {
...
impl<'v> StarlarkInputArtifactLike<'v> for StarlarkPromiseArtifact {
    fn get_bound_artifact(&self) -> kuro_error::Result<Artifact> {
        match self.artifact.get() {
            Some(v) => Ok(v.dupe()),
    ...
    fn as_output_error(&self) -> kuro_error::Error {
    ...
    fn get_artifact_group(&self) -> kuro_error::Result<ArtifactGroup> {
        Ok(self.as_artifact())
    }

    fn as_output(&'v self, _this: Value<'v>) -> kuro_error::Result<StarlarkOutputArtifact<'v>> {
    ...
    fn project(
        ...
        hide_prefix: bool,
    ) -> kuro_error::Result<EitherStarlarkInputArtifact<'v>> {
