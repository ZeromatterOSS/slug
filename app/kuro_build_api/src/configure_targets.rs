...
use kuro_node::load_patterns::load_patterns_with_modifiers;
...
fn split_compatible_incompatible(
    targets: impl IntoIterator<Item = kuro_error::Result<MaybeCompatible<ConfiguredTargetNode>>>,
    keep_going: bool,
) -> kuro_error::Result<(
...
pub async fn get_maybe_compatible_targets<'a, T>(
    ...
    impl Iterator<Item = kuro_error::Result<MaybeCompatible<ConfiguredTargetNode>>> + use<T>,
                ...
                let local_cfg_options = match package_with_modifiers.modifiers.as_slice() {
                    Some(modifiers) => {
                        if !global_cfg_options.cli_modifiers.is_empty() {
                            let error = kuro_error::Error::from(
                    ...
                    DiceComputations::declare_closure(|ctx| {
                        async move {
                            let target = ctx
                                ...
                                .await?;
                            kuro_error::Ok(ctx.get_configured_target_node(&target).await?)
                        }
                        .boxed()
                    })
                });
...
pub async fn get_compatible_targets(
    ...
    global_cfg_options: &GlobalCfgOptions,
    keep_going: bool,
) -> kuro_error::Result<ConfiguredTargetsWithErrors> {
