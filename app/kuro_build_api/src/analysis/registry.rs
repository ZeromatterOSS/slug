...
impl<'v> AnalysisRegistry<'v> {
    ...
    ) -> kuro_error::Result<(ArtifactDeclaration<'v>, OutputArtifact<'v>)> {
        let declaration_location = eval.call_stack_top_location();
        let heap = eval.heap();
    ...
    pub fn finalize(
        ...
        impl FnOnce(&FrozenModule) -> kuro_error::Result<RecordedAnalysisValues> + use<>,
    > {
...
impl<'v> AnalysisValueStorage<'v> {
    ...
    fn write_to_module(self, module: &'v Module) -> kuro_error::Result<()> {
    ...
    pub(crate) fn register_transitive_set<
        F: FnOnce(TransitiveSetKey) -> kuro_error::Result<ValueTyped<'v, TransitiveSet<'v>>>,
    >(
        &mut self,
        func: F,
    ) -> kuro_error::Result<ValueTyped<'v, TransitiveSet<'v>>> {
...
impl AnalysisValueFetcher {
    fn extra_value(
        &self,
    ) -> kuro_error::Result<Option<(&FrozenAnalysisValueStorage, &FrozenHeapRef)>> {
    ...
    pub fn get_action_data(
        &self,
        id: &ActionKey,
    ) -> kuro_error::Result<(Option<OwnedFrozenValue>, Option<OwnedFrozenValue>)> {
...
impl RecordedAnalysisValues {
    ...
    ) -> kuro_error::Result<OwnedFrozenValueTyped<FrozenTransitiveSet>> {
    ...
    pub fn lookup_action(&self, key: &ActionKey) -> kuro_error::Result<ActionLookup> {
        if key.holder_key() != &self.self_key {
            return Err(internal_error!(
    ...
    pub fn analysis_storage(
        &self,
    ) -> kuro_error::Result<OwnedRefFrozenRef<'_, FrozenAnalysisValueStorage>> {
        Ok(self
            .analysis_storage
            .as_ref()
    ...
    pub fn provider_collection(&self) -> kuro_error::Result<FrozenProviderCollectionValueRef<'_>> {
