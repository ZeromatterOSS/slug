...
pub static GET_PROMISED_ARTIFACT: LateBinding<
    for<'c> fn(
        &'c PromiseArtifact,
        &'c mut DiceComputations,
    ) -> Pin<Box<dyn Future<Output = kuro_error::Result<Artifact>> + Send + 'c>>,
> = LateBinding::new("GET_PROMISED_ARTIFACT");

async fn lookup_deferred_inner(
    key: &BaseDeferredKey,
    dice: &mut DiceComputations<'_>,
) -> kuro_error::Result<DeferredHolder> {
    match key {
...
impl DeferredHolder {
    ...
    pub(crate) fn lookup_action(&self, key: &ActionKey) -> kuro_error::Result<ActionLookup> {
