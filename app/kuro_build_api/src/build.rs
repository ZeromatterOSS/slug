...
pub struct ConfiguredBuildTargetResultGen<T> {
    ...
    pub graph_properties: Option<kuro_error::Result<MaybeCompatible<GraphPropertiesValues>>>,
    pub errors: Vec<kuro_error::Error>,
}

pub type ConfiguredBuildTargetResult =
    ConfiguredBuildTargetResultGen<kuro_error::Result<ProviderArtifacts>>;
...
impl AsyncBuildTargetResultBuilder {
    ...
    ) -> kuro_error::Result<BuildTargetResult> {
        let mut fut = pin!(fut);
        loop {
            tokio::select! {
...
pub struct BuildTargetResultBuilder {
    res: HashMap<
        ...
        Option<ConfiguredBuildTargetResultGen<(usize, kuro_error::Result<ProviderArtifacts>)>>,
    >,
    configured_to_pattern_modifiers: HashMap<ConfiguredProvidersLabel, Vec<Modifiers>>,
    other_errors: BTreeMap<Option<ProvidersLabel>, Vec<kuro_error::Error>>,
    build_failed: bool,
...
impl BuildTargetResultBuilder {
    ...
    pub fn event(&mut self, event: BuildEvent) -> kuro_error::Result<FailFastState> {
        ...
        match variant {
            ...
            ConfiguredBuildEventVariant::Timeout => {
                self.res.get_mut(&label)
                     ...
                     .errors.push(kuro_error::Error::from(BuildDeadlineExpired));
                // TODO(cjhopman): Why don't we break here?
                self.build_failed = true;
            }
...
pub struct BuildTargetResult {
    ...
    pub other_errors: BTreeMap<Option<ProvidersLabel>, Vec<kuro_error::Error>>,
...
pub enum ConfiguredBuildEventExecutionVariant {
    BuildOutput {
        output: kuro_error::Result<ProviderArtifacts>,
...
pub enum ConfiguredBuildEventVariant {
    ...
    GraphProperties {
        graph_properties: kuro_error::Result<MaybeCompatible<GraphPropertiesValues>>,
    },
    Error {
...
async fn build_configured_label_inner<'a>(
    ...
    if !opts.skippable && outputs.is_empty() {
        let docs = "https://kuro.build/docs/users/faq/common_issues/#why-does-my-target-not-have-any-outputs"; // @oss-enable
        // @oss-disable: let docs = "https://www.internalfb.com/intern/staticdocs/kuro/docs/users/faq/common_issues/#why-does-my-target-not-have-any-outputs";
            ...
            |(index, (output, provider_type))| {
                ...
                Either::Left(async move {
                    ...
                    .await
                    {
                        ...
                        Err(e) => Err(kuro_error::Error::from(e)),
                    };
