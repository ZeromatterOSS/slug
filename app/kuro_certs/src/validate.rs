...
#[kuro(environment, tag = NoValidCerts)]
...
async fn verify(path: &OsString) -> kuro_error::Result<()> {
...
pub async fn validate_certs() -> kuro_error::Result<()> {
...
pub async fn check_cert_state(cert_state: CertState) -> Option<kuro_error::Error> {
