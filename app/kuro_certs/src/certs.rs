...
/// certificate store.
async fn load_system_root_certs() -> kuro_error::Result<RootCertStore> {
    let root_certs = if let Some(path) = find_root_ca_certs() {
        ...
        if !native_certs_results.certs.is_empty() {
            ...
            let native_certs_error = native_certs_results
                .errors
                .pop()
                .map(kuro_error::Error::from)
                .unwrap_or(kuro_error!(
                    kuro_error::ErrorTag::NoValidCerts,
            ...
            if cfg!(fbcode_build) {
                ...
                let context = format!(
                    ...
                    Please try `getchef -reason 'chef broken'`{windows_message}, `Fix My <OS>` via the f-menu, then `kuro killall`.
...
async fn load_key<P: AsRef<Path>>(key: P) -> kuro_error::Result<PrivateKeyDer<'static>> {
    let key = key.as_ref();
...
async fn load_cert_pair<P: AsRef<Path>>(
    cert: P,
    key: P,
) -> kuro_error::Result<(Vec<CertificateDer<'static>>, PrivateKeyDer<'static>)> {
...
pub async fn tls_config_with_system_roots() -> kuro_error::Result<ClientConfig> {
...
pub async fn tls_config_with_single_cert<P: AsRef<Path>>(
    cert_path: P,
    key_path: P,
) -> kuro_error::Result<ClientConfig> {
    let system_roots = load_system_root_certs().await?;
    ...
    ClientConfig::builder()
        ...
        .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::Certs))
...
pub(crate) async fn load_certs<P: AsRef<Path>>(
    cert_path: P,
) -> kuro_error::Result<Vec<CertificateDer<'static>>> {
