...
use slug_node::cfg_constructor::CFG_CONSTRUCTOR_CALCULATION_IMPL;
use slug_node::configuration::target_platform_detector::TargetPlatformDetector;
...
use slug_node::target_calculation::CONFIGURED_TARGET_CALCULATION;
use slug_node::target_calculation::ConfiguredTargetCalculationImpl;
...
) -> slug_error::Result<Arc<TargetPlatformDetector>> {
...
async fn apply_cli_build_settings(
    ...
    let starlark_flags: Vec<(String, Vec<String>)> =
        slug_build_api::interpreter::rule_defs::build_config::get_all_starlark_flag_values()
            .into_iter()
            .collect();
    let resolver = ctx.get_cell_resolver().await?;
...
/// `@slug_settings` routing in `BuildSettingLabel::from_bazel_label` still
...
mod tests {
    ...
    #[test]
    fn cli_compilation_mode_lands_in_build_settings() -> slug_error::Result<()> {
    ...
    #[test]
    fn starlark_flags_land_in_build_settings() -> slug_error::Result<()> {
        let flags = vec![
    ...
    #[test]
    fn repeated_starlark_flags_land_as_string_list() -> slug_error::Result<()> {
        let flags = vec![(
    ...
    #[test]
    fn differing_compilation_modes_produce_distinct_cfgs() -> slug_error::Result<()> {
    ...
    #[test]
    fn cell_alias_is_canonicalized_at_storage_time() -> slug_error::Result<()> {
    ...
    #[test]
    fn invalid_flag_label_is_skipped() -> slug_error::Result<()> {
        let flags = vec![
