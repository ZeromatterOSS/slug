...
use slug_build_api::interpreter::rule_defs::provider::builtin::dep_only_incompatible_info::DepOnlyIncompatibleCustomSoftErrors;
...
use slug_core::configuration::compatibility::IncompatiblePlatformReasonCause;
...
use slug_core::plugins::PluginListElemKind;
...
use slug_core::target::configured_or_unconfigured::ConfiguredOrUnconfiguredTargetLabel;
...
use slug_node::attrs::configuration_context::PlatformConfigurationError;
...
use slug_node::attrs::spec::AttributeId;
use slug_node::attrs::spec::internal::INCOMING_TRANSITION_ATTRIBUTE;
use slug_node::attrs::spec::internal::LEGACY_TARGET_COMPATIBLE_WITH_ATTRIBUTE;
use slug_node::attrs::spec::internal::TARGET_COMPATIBLE_WITH_ATTRIBUTE;
...
use slug_node::nodes::configured_frontend::CONFIGURED_TARGET_NODE_CALCULATION;
use slug_node::nodes::configured_frontend::ConfiguredTargetNodeCalculation;
use slug_node::nodes::configured_frontend::ConfiguredTargetNodeCalculationImpl;
...
use slug_node::rule::RuleIncomingTransition;
use slug_node::visibility::VisibilityError;
...
fn configured_checkpoint(
    ...
    slug_util::memory_checkpoint::checkpoint(checkpoint, checkpoint_fields);
    tracing::warn!(
        target: "slug_memory",
        checkpoint,
...
enum NodeCalculationError {
    ...
    #[error(
        "Target {0} configuration transition is not idempotent
         ...
         Use `slug audit configurations {1} {2} {3}` to see the configurations."
    )]
...
fn unpack_target_compatible_with_attr(
    ...
    attr_id: AttributeId,
) -> slug_error::Result<Option<ConfiguredAttr>> {
...
fn check_compatible(
    ...
    target_node: TargetNodeRef,
    resolved_cfg: &MatchedConfigurationSettingKeysWithCfg,
) -> slug_error::Result<MaybeCompatible<()>> {
    ...
    let check_compatibility = |attr| -> slug_error::Result<(Vec<_>, Vec<_>)> {
...
impl ErrorsAndIncompatibilities {
    fn unpack_dep_into(
        &mut self,
        target_label: &TargetConfiguredTargetLabel,
        result: slug_error::Result<MaybeCompatible<ConfiguredTargetNode>>,
    ...
    pub(crate) fn finalize<T>(mut self) -> Option<slug_error::Result<MaybeCompatible<T>>> {
...
pub(crate) struct GatheredDeps {
    ...
    pub(crate) aspect_results: std::collections::HashMap<
        (
            ...
            std::sync::Arc<slug_node::aspect_type::StarlarkAspectType>,
        ),
        slug_analysis::analysis::aspect_key::AspectValue,
    >,
}
...
pub(crate) async fn gather_deps(
    ...
    ctx: &mut DiceComputations<'_>,
) -> slug_error::Result<(GatheredDeps, ErrorsAndIncompatibilities)> {
    ...
    impl ConfiguredAttrTraversal for Traversal {
        ...
        fn exec_dep(&mut self, dep: &ConfiguredProvidersLabel) -> slug_error::Result<()> {
            self.exec_deps.insert(dep.dupe(), CheckVisibility::Yes);
            Ok(())
        }

        fn toolchain_dep(&mut self, dep: &ConfiguredProvidersLabel) -> slug_error::Result<()> {
            self.toolchain_deps
        ...
        fn plugin_dep(&mut self, dep: &TargetLabel, kind: &PluginKind) -> slug_error::Result<()> {
            self.plugin_lists
        ...
        if !a.attr.aspects().is_empty() {
            ...
            for dep_label in collector.deps {
                for aspect_type in a.attr.aspects() {
                    aspect_keys.push(slug_analysis::analysis::aspect_key::AspectKey::new(
                        dep_label.dupe(),
                        aspect_type.dupe(),
                    ));
                }
    ...
    for (key, result) in aspect_results_with_keys {
        match result {
            ...
            Err(e) => {
                // DICE error - convert to slug_error::Error
...
async fn resolve_transition_attrs<'a>(
    ...
    ctx: &mut DiceComputations<'_>,
) -> slug_error::Result<OrderedMap<&'a str, Arc<ConfiguredAttr>>> {
    ...
    impl AttrConfigurationContext for AttrConfigurationContextToResolveTransitionAttrs<'_> {
        ...
        fn platform_cfg(&self, label: &TargetLabel) -> slug_error::Result<ConfigurationData> {
...
fn configured_attrs_equal_modulo_cfg(a: &ConfiguredAttr, b: &ConfiguredAttr) -> bool {
    use slug_node::attrs::configured_attr::ConfiguredAttr as CA;
    match (a, b) {
...
async fn compute_configured_target_node_no_transition(
    ...
    ctx: &mut DiceComputations<'_>,
) -> slug_error::Result<MaybeCompatible<ConfiguredTargetNode>> {
    let started = Instant::now();
...
impl slug_error::TypedContext for LookingUpConfiguredNodeContext {
    fn eq(&self, other: &dyn slug_error::TypedContext) -> bool {
...
impl Key for ConfiguredTargetNodeKey {
    type Value = slug_error::Result<MaybeCompatible<ConfiguredTargetNode>>;
    async fn compute(
        &self,
...
) -> slug_error::Result<Option<Vec<ArcStr>>> {
    #[derive(Clone, Dupe, Display, Debug, Eq, Hash, PartialEq, Allocative)]
    struct GetDepOnlyIncompatibleInfo;

    #[async_trait]
    impl Key for GetDepOnlyIncompatibleInfo {
        type Value = slug_error::Result<Option<Arc<DepOnlyIncompatibleCustomSoftErrors>>>;
