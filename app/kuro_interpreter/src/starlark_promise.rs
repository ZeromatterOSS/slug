...
struct Validate<'v>(#[trace(unsafe_ignore)] fn(Value<'v>) -> kuro_error::Result<()>);
...
impl<'v> StarlarkPromise<'v> {
    ...
    pub fn map(
        ...
        f: StarlarkCallable<'v, (Value<'v>,), Value<'v>>,
        eval: &mut Evaluator<'v, '_, '_>,
    ) -> kuro_error::Result<ValueTyped<'v, StarlarkPromise<'v>>> {
        match x.get() {
            Some(x) => Ok(eval
                .heap()
    ...
    pub fn validate(&self, f: fn(Value<'v>) -> kuro_error::Result<()>) -> kuro_error::Result<()> {
        match self.get() {
            Some(x) => f(x),
            _ => {
...
mod tests {
    ...
    fn assert_promise<'v>(modu: &'v Module, content: &str) -> kuro_error::Result<Value<'v>> {
        alloc_promises(modu);
    ...
    fn assert_promise_err<'v>(modu: &'v Module, content: &str, err: &str) -> kuro_error::Error {
