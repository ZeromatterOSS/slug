...
impl StarlarkBuckRegex {
    ...
    fn is_match(&self, s: &str) -> kuro_error::Result<bool> {
        match self {
...
pub fn register_buck_regex(builder: &mut GlobalsBuilder) {
    ...
    /// ## Fanciness
    ///
    /// Kuro regexes support two backing implementations:
    ///
    ...
    fn regex<'v>(
        ...
        match fancy {
            false => Ok(StarlarkBuckRegex::Regular(
                regex::Regex::new(regex).map_err(kuro_error::Error::from)?,
            )),
            true => Ok(StarlarkBuckRegex::Fancy(
                fancy_regex::Regex::new(regex).map_err(kuro_error::Error::from)?,
            )),
        }
    }
