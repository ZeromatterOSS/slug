...
pub struct CollectedExecutionStats {
    ...
    pub userspace_events: Option<kuro_data::CpuCounter>,
    pub kernel_events: Option<kuro_data::CpuCounter>,
}
...
struct CommandEventStream<Status, Stdio> {
    exit: Option<kuro_error::Result<GatherOutputStatus>>,
...
where
    Status: Future<Output = kuro_error::Result<GatherOutputStatus>>,
    Stdio: Stream<Item = kuro_error::Result<StdioEvent>> + InterruptNotifiable,
{
    type Item = kuro_error::Result<CommandEvent>;
...
) -> kuro_error::Result<GatherOutputStatus> {
    match timeout {
        Some(t) => {
...
) -> kuro_error::Result<impl Stream<Item = kuro_error::Result<CommandEvent>>>
where
    T: Future<Output = kuro_error::Result<GatherOutputStatus>> + Send + Unpin,
{
    ...
    let process_details = if retry_on_txt_busy {
        ...
        cmd.spawn().map_err(|x| kuro_error::Error::from(x.0))
    };
    ...
    let status = async move {
        ...
        let execute = async {
            ...
            kuro_error::Ok(match futures::future::select(status, cancellation).await {
...
pub async fn decode_command_event_stream<S>(stream: S) -> kuro_error::Result<CommandResult>
where
...
pub(crate) trait KillProcess {
    async fn kill(self, process: &mut ProcessGroup) -> kuro_error::Result<()>;
}
...
impl KillProcess for DefaultKillProcess {
    async fn kill(self, process_group: &mut ProcessGroup) -> kuro_error::Result<()> {
...
async fn spawn_retry_txt_busy<F, D>(
    ...
    mut delay: F,
) -> kuro_error::Result<ProcessGroup>
where
    F: FnMut() -> D,
    D: Future<Output = ()>,
{
...
mod tests {
    ...
    #[tokio::test]
    async fn test_gather_output() -> kuro_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_gather_does_not_wait_for_children() -> kuro_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_gather_output_timeout() -> kuro_error::Result<()> {
        let now = Instant::now();
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_spawn_retry_txt_busy() -> kuro_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_spawn_retry_other_error() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
    ...
    #[tokio::test]
    async fn test_kill_terminates_process_group() -> kuro_error::Result<()> {
        use sysinfo::Pid;
        use sysinfo::System;
    ...
    #[tokio::test]
    async fn test_stream_command_events_ends() -> kuro_error::Result<()> {
        let mut cmd = if cfg!(windows) {
            background_command("powershell")
        } else {
            background_command("sh")
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_signal_exit_code() -> kuro_error::Result<()> {
    ...
    #[tokio::test]
    async fn timeout_kills_before_dropping_decoder() -> kuro_error::Result<()> {
        struct Kill {
        ...
        impl StatusDecoder for Decoder {
            ...
            async fn cancel(self) -> kuro_error::Result<()> {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn test_no_stdio_stream_command_events() -> kuro_error::Result<()> {
        let mut cmd = background_command("sh");
        cmd.args(["-c", "echo hello"]);
