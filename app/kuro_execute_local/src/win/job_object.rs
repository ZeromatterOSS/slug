...
impl JobObject {
    pub(crate) fn new() -> kuro_error::Result<Self> {
    ...
    pub(crate) fn assign_process(&self, process: HANDLE) -> kuro_error::Result<()> {
        result_bool(unsafe { jobapi2::AssignProcessToJobObject(self.job_handle.handle(), process) })
    }

    pub(crate) async fn terminate(&self, exit_code: u32) -> kuro_error::Result<()> {
    ...
    async fn wait(&self) -> kuro_error::Result<()> {
...
fn set_job_limits(job: &WinapiHandle) -> kuro_error::Result<()> {
