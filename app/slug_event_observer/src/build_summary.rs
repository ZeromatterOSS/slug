...
//! Rollup aggregator for the Slug event stream.
//!
...
//! `slug log summary` / `slug log diff` commands (Plan 16.3 / 16.4). The
...
impl BuildSummaryBuilder {
    ...
    pub fn handle_event(&mut self, event: &BuckEvent) {
        ...
        match event.data() {
            ...
            Data::Instant(instant) => {
                if let Some(slug_data::instant_event::Data::BuildGraphInfo(info)) = &instant.data {
    ...
    fn on_span_start(&mut self, event: &BuckEvent, span: &slug_data::SpanStartEvent) {
    ...
    fn on_span_end(&mut self, event: &BuckEvent, span: &slug_data::SpanEndEvent) {
        ...
        match &span.data {
            ...
            Some(Data::Analysis(analysis)) => {
                ...
                let target = match &analysis.target {
                    Some(slug_data::analysis_end::Target::StandardTarget(t)) => {
                        format_target_label(t)
                    }
                    Some(slug_data::analysis_end::Target::AnonTarget(_)) => "<anon>".to_owned(),
                    Some(slug_data::analysis_end::Target::DynamicLambda(_)) => {
    ...
    fn on_build_graph_info(&mut self, info: &slug_data::BuildGraphExecutionInfo) {
        self.num_dice_nodes = info.num_nodes;
        self.num_dice_edges = info.num_edges;

        let sum_path = |entries: &[slug_data::CriticalPathEntry2]| -> u64 {
            entries
                .iter()
        ...
        for entry in &info.critical_path2 {
            ...
            let category = match &entry.entry {
                Some(slug_data::critical_path_entry2::Entry::ActionExecution(a)) => a
                    .name
                    .as_ref()
                    .map(|n| n.category.clone())
                    .unwrap_or_else(|| "action".to_owned()),
                Some(slug_data::critical_path_entry2::Entry::Analysis(_)) => "analysis".to_owned(),
                Some(slug_data::critical_path_entry2::Entry::Load(_)) => "load".to_owned(),
                Some(slug_data::critical_path_entry2::Entry::Listing(_)) => "listing".to_owned(),
                _ => continue,
            };
            *self
        ...
        self.action_graph_size = info
            ...
            .filter(|e| {
                matches!(
                    e.entry,
                    Some(slug_data::critical_path_entry2::Entry::ActionExecution(_))
                )
            })
            .count() as u64;
    }
...
fn format_target_label(label: &slug_data::ConfiguredTargetLabel) -> String {
    let name = label
        .label
        .as_ref()
...
mod tests {
    ...
    use slug_data::CommandExecutionKind;
    use slug_data::RemoteCommand;
    use slug_data::SpanEndEvent;
    use slug_data::SpanStartEvent;
    use slug_data::command_execution_kind::Command as CmdKind;
    use slug_events::span::SpanId;
    use slug_wrapper_common::invocation_id::TraceId;

    use super::*;

    fn span_start(data: slug_data::span_start_event::Data) -> BuckEvent {
        span_start_at(data, 0)
    }

    fn span_start_at(data: slug_data::span_start_event::Data, ts_us: u64) -> BuckEvent {
        BuckEvent::new(
    ...
    fn span_end(data: slug_data::span_end_event::Data, duration_us: u64) -> BuckEvent {
        span_end_at(data, duration_us, 0)
    }

    fn span_end_at(
        data: slug_data::span_end_event::Data,
        duration_us: u64,
        ts_us: u64,
    ) -> BuckEvent {
        BuckEvent::new(
    ...
    fn remote_cache_hit() -> slug_data::ActionExecutionEnd {
        slug_data::ActionExecutionEnd {
            ...
            commands: vec![slug_data::CommandExecution {
    ...
    fn rollup_tracks_mnemonic_and_cache_hits() {
        ...
        for _ in 0..3 {
            b.handle_event(&span_end(
                slug_data::span_end_event::Data::ActionExecution(Box::new(remote_cache_hit())),
                2_000,
            ));
        }
    ...
    fn rollup_splits_exec_and_queue_wall() {
        ...
        let mut b = BuildSummaryBuilder::new();
        b.handle_event(&span_end(
            slug_data::span_end_event::Data::ActionExecution(Box::new(action)),
    ...
    fn rollup_peak_in_flight_actions() {
        let mut b = BuildSummaryBuilder::new();
        let start = slug_data::span_start_event::Data::ActionExecution(
            slug_data::ActionExecutionStart::default(),
        );
        let end = slug_data::span_end_event::Data::ActionExecution(Box::new(
            slug_data::ActionExecutionEnd::default(),
        ));
    ...
    fn short_lines_format() {
        ...
        b.handle_event(&span_start_at(
            slug_data::span_start_event::Data::Load(slug_data::LoadBuildFileStart {
                ..Default::default()
            }),
            0,
        ));
        b.handle_event(&span_end_at(
            slug_data::span_end_event::Data::Load(slug_data::LoadBuildFileEnd {
                ..Default::default()
            }),
            1_500_000,
            1_500_000,
        ...
        let action_start = || {
            slug_data::span_start_event::Data::ActionExecution(
                slug_data::ActionExecutionStart::default(),
            )
        };
