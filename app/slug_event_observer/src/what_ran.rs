...
impl<'a> WhatRanOptionsRegex<'a> {
    pub fn from_options(options: &'a WhatRanOptions) -> slug_error::Result<Self> {
...
pub enum WhatRanRelevantAction {
    ActionExecution(slug_data::ActionExecutionStart),
    TestDiscovery(slug_data::TestDiscoveryStart),
    TestRun(slug_data::TestRunStart),
    SetupLocalResources(slug_data::SetupLocalResourcesStart),
}

impl WhatRanRelevantAction {
    /// Extract a relevant action from an event's data, if we can find one.
    pub fn from_buck_data(data: &slug_data::buck_event::Data) -> Option<Self> {
        match data {
            slug_data::buck_event::Data::SpanStart(span) => match &span.data {
                Some(slug_data::span_start_event::Data::ActionExecution(action)) => {
                    Some(Self::ActionExecution(action.clone()))
                }
                Some(slug_data::span_start_event::Data::TestDiscovery(suite)) => {
                    Some(Self::TestDiscovery(suite.clone()))
                }
                Some(slug_data::span_start_event::Data::TestStart(test)) => {
                    Some(Self::TestRun(test.clone()))
                }
                Some(slug_data::span_start_event::Data::LocalResources(setup)) => {
...
pub enum CommandReproducer {
    CacheQuery(slug_data::CacheQuery),
    CacheHit(slug_data::CacheHit),
    LocalDepFileCacheHit,
    ReExecute(slug_data::ReExecute),
    LocalExecute(slug_data::LocalExecute),
    WorkerExecute(slug_data::WorkerExecute),
    WorkerInit(slug_data::WorkerInit),
}
...
impl CommandReproducer {
    pub fn executor(&self) -> String {
        match self {
            ...
            Self::CacheHit(cache) => {
                ...
                match slug_data::CacheHitType::try_from(cache_type) {
                    Ok(slug_data::CacheHitType::RemoteDepFileCache) => {
    ...
    pub fn from_buck_data(
        data: &slug_data::buck_event::Data,
        options: &WhatRanOptions,
    ) -> Option<Self> {
        match data {
            slug_data::buck_event::Data::SpanStart(span) => match &span.data {
                Some(slug_data::span_start_event::Data::ExecutorStage(executor_stage)) => {
                    match &executor_stage.stage {
                        Some(slug_data::executor_stage_start::Stage::CacheQuery(cache_hit))
                            if options.emit_cache_queries =>
                        {
                            return Some(CommandReproducer::CacheQuery(cache_hit.clone()));
                        }
                        Some(slug_data::executor_stage_start::Stage::CacheHit(cache_hit))
                            if !options.skip_cache_hits =>
                        {
                            return Some(CommandReproducer::CacheHit(cache_hit.clone()));
                        }
                        Some(slug_data::executor_stage_start::Stage::Re(re_stage))
                            if !options.skip_remote_executions =>
                        {
                            match &re_stage.stage {
                                Some(slug_data::re_stage::Stage::Execute(execute)) => {
                                    return Some(CommandReproducer::ReExecute(execute.clone()));
                                }
                                _ => {}
                            }
                        }
                        Some(slug_data::executor_stage_start::Stage::Local(local_stage)) => {
                            if !options.skip_local_executions {
                                match &local_stage.stage {
                                    Some(slug_data::local_stage::Stage::Execute(local_execute)) => {
                                        return Some(CommandReproducer::LocalExecute(
                                            local_execute.clone(),
                                        ));
                                    }
                                    Some(slug_data::local_stage::Stage::WorkerExecute(
                                        worker_execute,
                                    )) => {
                                    ...
                                    Some(slug_data::local_stage::Stage::WorkerInit(
                                        worker_init,
                                    )) => {
...
pub struct Command<'a> {
    env: &'a Vec<slug_data::EnvironmentEntry>,
    argv: &'a Vec<String>,
}

impl<'a> From<&'a slug_data::LocalCommand> for Command<'a> {
    fn from(command: &'a slug_data::LocalCommand) -> Self {
        Command {
            env: &command.env,
            argv: &command.argv,
        }
...
impl<'a> From<&'a slug_data::WorkerCommand> for Command<'a> {
    fn from(command: &'a slug_data::WorkerCommand) -> Self {
        Command {
            env: &command.env,
            argv: &command.argv,
        }
...
impl<'a> From<&'a slug_data::WorkerInitCommand> for Command<'a> {
    fn from(command: &'a slug_data::WorkerInitCommand) -> Self {
        Command {
            env: &command.env,
            argv: &command.argv,
        }
...
pub fn worker_command_as_fallback_to_string(command: &slug_data::WorkerCommand) -> String {
...
pub fn command_to_string<'a>(command: impl Into<Command<'a>>) -> String {
    ...
    let mut cmd = "env --chdir=\"$(slug root --kind project)\" --".to_owned();
...
fn executor_with_platform(execute: &slug_data::ReExecute) -> String {
...
mod tests {
    ...
    fn test_executor_with_platform_no_platform() {
        let execute = slug_data::ReExecute::default();
        let result = executor_with_platform(&execute);
