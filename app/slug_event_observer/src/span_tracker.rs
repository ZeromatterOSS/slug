...
impl<T: SpanTrackable + Dupe> Span<T> {
    ...
    fn remove_child(&mut self, child: Span<T>, roots: &mut Roots<T>) -> slug_error::Result<()> {
...
impl<T: SpanTrackable + Dupe> SpanTracker<T> {
    ...
    pub fn start_at(&mut self, event: &T) -> slug_error::Result<()> {
        if !event.is_shown() {
            return Ok(());
        }
    ...
    fn end(&mut self, event: &T) -> slug_error::Result<()> {
        let span_id = event
            .span_id()
            .ok_or_else(|| SpanTrackerError::NonSpanEvent(event.dupe()))?;
...
impl BuckEventSpanTracker {
    pub fn handle_event(&mut self, event: &Arc<BuckEvent>) -> slug_error::Result<()> {
...
mod tests {
    ...
    #[test]
    fn test_boring_via_self() -> slug_error::Result<()> {
    ...
    #[test]
    fn test_boring_via_child() -> slug_error::Result<()> {
    ...
    #[test]
    fn test_iter_roots_len() -> slug_error::Result<()> {
        let e1 = TestSpan::new();
    ...
    #[test]
    fn test_dice_counts() -> slug_error::Result<()> {
