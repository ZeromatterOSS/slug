...
use slug_data::ActionExecutionStart;
use slug_data::AnalysisEnd;
use slug_data::AnalysisStageStart;
use slug_data::AnalysisStart;
use slug_data::ExecutorStageStart;
use slug_data::LoadBuildFileEnd;
use slug_data::LocalStage;
use slug_data::ReStage;
use slug_data::analysis_stage_start;
use slug_data::executor_stage_start;
use slug_data::instant_event;
use slug_data::local_stage;
...
impl BuildProgressStateTracker {
    ...
    pub fn handle_event(&mut self, event: &BuckEvent) -> slug_error::Result<()> {
    ...
    fn handle_load(&mut self, ev: &UnpackedBuckEvent) -> slug_error::Result<()> {
        match ev {
    ...
    fn handle_analysis(&mut self, ev: &UnpackedBuckEvent) -> slug_error::Result<()> {
        match ev {
            UnpackedBuckEvent::SpanStart(
                BuckEvent {
                    span_id: Some(span_id),
    ...
    fn handle_actions(&mut self, ev: &UnpackedBuckEvent) -> slug_error::Result<()> {
        match ev {
            UnpackedBuckEvent::SpanStart(
                BuckEvent {
                    span_id: Some(span_id),
...
mod tests {
    ...
    #[test]
    fn test_span_map() -> slug_error::Result<()> {
