...
impl VerbosityItem {
    fn from(value: &str) -> slug_error::Result<Self> {
...
impl Verbosity {
    pub fn try_from_cli(value: &str) -> slug_error::Result<Verbosity> {
        ...
        if levels.len() > 1 {
            return Err(slug_error::Error::from(VerbosityError::MoreThan1Level).into());
        }
