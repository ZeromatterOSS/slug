...
use slug_data::ActionKey;
use slug_data::ActionName;
use slug_data::AnonTarget;
use slug_data::BxlFunctionKey;
use slug_data::BxlFunctionLabel;
use slug_data::ConfiguredTargetLabel;
use slug_data::FileWatcherKind;
use slug_data::TargetLabel;
use slug_data::action_key;
use slug_data::span_start_event::Data;
use slug_error::BuckErrorContext;
use slug_events::BuckEvent;
use slug_test_api::data::TestStatus;
use slug_util::commas::commas;
...
pub fn display_anon_target(ctl: &AnonTarget) -> slug_error::Result<String> {
    if let AnonTarget {
...
pub fn display_analysis_target(
    target: &slug_data::analysis_start::Target,
    opts: TargetDisplayOptions,
) -> slug_error::Result<String> {
    use slug_data::analysis_start::Target;
    match target {
        ...
        Target::DynamicLambda(dynamic) => {
            use slug_data::dynamic_lambda_owner::Owner;
            match dynamic
                .owner
                .as_ref()
...
pub fn display_bxl_key(ctl: &BxlFunctionKey) -> slug_error::Result<String> {
...
pub fn display_event(event: &BuckEvent, opts: TargetDisplayOptions) -> slug_error::Result<String> {
    let res: slug_error::Result<_> = try {
        let data = match event.data() {
            slug_data::buck_event::Data::SpanStart(start) => start.data.as_ref().unwrap(),
            _ => Err(slug_error::Error::from(ParseEventError::UnexpectedEvent))?,
        };

        let res: slug_error::Result<_> = match data {
            ...
            Data::MatchDepFiles(slug_data::MatchDepFilesStart {
            ...
            Data::SharedTask(slug_data::SharedTaskStart { owner_trace_id }) => Ok(format!(
            ...
            Data::DiceSynchronizeSection(..) => Ok("Synchronizing slug internal state".to_owned()),
            Data::DiceCleanup(..) => Ok("Cleaning up graph state".to_owned()),
            Data::ExclusiveCommandWait(slug_data::ExclusiveCommandWaitStart { command_name }) => {
            ...
            Data::DeferredPreparationStage(prep) => {
                use slug_data::deferred_preparation_stage_start::Stage;
            ...
            Data::DynamicLambda(lambda) => {
                use slug_data::dynamic_lambda_start::Owner;
            ...
            Data::ConnectToInstaller(slug_data::ConnectToInstallerStart { tcp_port }) => {
...
fn display_file_watcher(provider: i32) -> &'static str {
    match slug_data::FileWatcherProvider::try_from(provider) {
        Ok(slug_data::FileWatcherProvider::Watchman) => "Watchman",
        Ok(slug_data::FileWatcherProvider::RustNotify) => "notify",
        Ok(slug_data::FileWatcherProvider::FsHashCrawler) => "fs_hash_crawler",
        Ok(slug_data::FileWatcherProvider::EdenFs) => "EdenFS",
        Err(_) => "unknown mechanism",
    }
}

pub fn display_analysis_stage(stage: &slug_data::analysis_stage_start::Stage) -> &'static str {
    use slug_data::analysis_stage_start::Stage;
...
pub fn display_file_watcher_end(file_watcher_end: &slug_data::FileWatcherEnd) -> Vec<String> {
...
pub fn display_executor_stage(
    stage: &slug_data::executor_stage_start::Stage,
) -> Option<&'static str> {
    use slug_data::executor_stage_start::Stage;

    let label = match stage {
        Stage::Prepare(..) => "prepare",
        Stage::CacheQuery(cache_query) => {
            match slug_data::CacheType::try_from(cache_query.cache_type).unwrap() {
                slug_data::CacheType::ActionCache => "re_action_cache",
                slug_data::CacheType::RemoteDepFileCache => "re_dep_file_cache",
            }
        }
        Stage::CacheHit(..) => "re_download",
        Stage::Re(re) => {
            use slug_data::re_stage::Stage;
        ...
        Stage::Local(local) => {
            use slug_data::local_stage::Stage;
...
) -> slug_error::Result<Option<Lines>> {
    let slug_data::TestResult {
        name,
        status,
        duration,
        details,
...
pub struct ActionErrorDisplay<'a> {
    ...
    pub reason: String,
    pub command: Option<&'a slug_data::CommandExecutionDetails>,
    pub error_diagnostics: Option<&'a slug_data::ActionErrorDiagnostics>,
}
...
pub fn get_action_error_reason(error: &slug_data::ActionError) -> slug_error::Result<String> {
    use slug_data::action_error::Error;
    ...
    Ok(
            ...
            .buck_error_context("Internal error: Missing error in action error")?
        {
            ...
            Error::CommandExecutionError(slug_data::CommandExecutionError {}) => {
...
pub fn display_action_error(
    ...
    opts: TargetDisplayOptions,
) -> slug_error::Result<ActionErrorDisplay<'_>> {
...
fn failure_reason_for_command_execution(
    command_execution: &slug_data::CommandExecution,
) -> slug_error::Result<String> {
    use slug_data::command_execution::Cancelled;
    use slug_data::command_execution::Error;
    use slug_data::command_execution::Failure;
    use slug_data::command_execution::Status;
    use slug_data::command_execution::Success;
    use slug_data::command_execution::Timeout;
    use slug_data::command_execution::WorkerFailure;
