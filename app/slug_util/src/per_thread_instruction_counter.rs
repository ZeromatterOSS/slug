...
impl PerThreadInstructionCounter {
    ...
    pub fn init() -> slug_error::Result<Option<PerThreadInstructionCounter>> {
        Self::init_impl()
    }

    #[cfg(target_os = "linux")]
    fn init_impl() -> slug_error::Result<Option<PerThreadInstructionCounter>> {
    ...
    pub fn collect(self) -> slug_error::Result<u64> {
        self.collect_impl()
    }

    #[cfg(target_os = "linux")]
    fn collect_impl(mut self) -> slug_error::Result<u64> {
    ...
    #[cfg(not(target_os = "linux"))]
    fn collect_impl(self) -> slug_error::Result<u64> {
