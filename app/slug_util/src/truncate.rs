...
mod tests {
    ...
    const MSG: &str = "rdeps(set(fbcode//slug/... fbsource//xplat/slug/..., fbsource//fbobjc/slug/...), fbcode//slug/cli:slug)";

    #[test]
    fn test_truncate() {
        assert_eq!(&truncate(MSG, 0), "<<omitted>>");
        assert_eq!(&truncate(MSG, TRUNCATION_MSG.len()), "<<omitted>>");
        assert_eq!(&truncate(MSG, 30), "rdeps(set<<omitted>>cli:slug)");
        assert_eq!(
            &truncate(MSG, 50),
            "rdeps(set(fbcode//k<<omitted>>ode//slug/cli:slug)"
        );
    }
