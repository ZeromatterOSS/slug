...
///
/// slug compilation speed is very important for us, so we try to make smaller crates
/// with as few dependencies between them as possible. So when we change one line of code,
/// we recompile as little as possible.
///
/// # Example
///
/// We have a crate `slug_node` that defines target graph data types
...
/// `slug_interpreter_for_build`, but code that needs to compute a target node,
/// calls the function defined in `slug_node` crate without compile time dependency
/// on `slug_interpreter_for_build` or `starlark` crates.
///
/// The best example of this pattern is probably `slug_bxl` crate:
/// the only target depending on `slug_bxl` is the final binary with `slug.rs` file.
/// So when `slug_bxl` changes, we recompile only `slug_bxl` and `slug.rs`.
/// At the moment of writing this comment, `slug_bxl` had only one public function,
...
/// * in the file `app/slug/bin/slug.rs` call `init_late_bindings()` of the corresponding crate
...
impl<T> LateBinding<T> {
    ...
    #[inline]
    pub fn get(&self) -> slug_error::Result<&T> {
        self.symbol
            .get()
