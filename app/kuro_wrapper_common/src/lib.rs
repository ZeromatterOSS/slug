...
//! Code shared between `kuro_wrapper` and `kuro`.
//!
...
use is_kuro::WhoIsAsking;
use sysinfo::ProcessesToUpdate;
use sysinfo::System;

use crate::is_kuro::is_kuro_exe;
use crate::pid::Pid;
...
/// Find all kuro processes in the system.
fn find_kuro_processes(who_is_asking: WhoIsAsking) -> Vec<ProcessInfo> {
    ...
    let mut kuro_processes = Vec::new();
    for (pid, process) in system.processes() {
        ...
        if is_kuro_exe(exe, who_is_asking) && !current_parents.contains(pid) {
            let Ok(pid) = Pid::from_u32(pid.as_u32()) else {
                continue;
            };
            kuro_processes.push(ProcessInfo {
                pid,
...
/// Kills all running Kuro processes, except this process's hierarchy. Returns whether it
/// succeeded without errors.
pub fn killall(who_is_asking: WhoIsAsking, write: impl Fn(String)) -> bool {
    let kuro_processes = find_kuro_processes(who_is_asking);

    if kuro_processes.is_empty() {
        write("No kuro processes found".to_owned());
        return true;
    }
    ...
    impl<F: Fn(String)> Printer<F> {
        ...
        fn failed_to_kill(&mut self, process: &ProcessInfo, error: kuro_error::Error) {
            ...
            for process in processes_still_alive {
                printer.failed_to_kill(
                    ...
                    kuro_error::kuro_error!(
                        kuro_error::ErrorTag::DaemonWontDieFromKill,
