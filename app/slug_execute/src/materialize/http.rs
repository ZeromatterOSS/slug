...
use slug_common::cas_digest::SHA1_SIZE;
use slug_common::cas_digest::SHA256_SIZE;
...
use slug_http::retries::AsSlugError;
use slug_http::retries::HttpError;
use slug_http::retries::HttpErrorForRetry;
use slug_http::retries::http_retry;
use sha1::Digest;
use sha1::Sha1;
use sha2::Sha256;
...
impl Checksum {
    pub fn new(sha1: Option<&str>, sha256: Option<&str>) -> slug_error::Result<Self> {
...
impl AsSlugError for HttpHeadError {
    fn as_slug_error(self) -> slug_error::Error {
        slug_error::Error::from(self)
    }
}

impl AsSlugError for HttpDownloadError {
    fn as_slug_error(self) -> slug_error::Error {
        slug_error::Error::from(self)
    }
}

pub async fn http_head(client: &HttpClient, url: &str) -> slug_error::Result<Response<()>> {
...
pub async fn http_download(
    ...
    checksum: &Checksum,
    executable: bool,
) -> slug_error::Result<TrackedFileDigest> {
    ...
    Ok(http_retry(
        || async {
            ...
            let file = fs_util::create_file(&abs_path)
                .map_err(|e| HttpDownloadError::IoError(slug_error::Error::from(e)))?;
...
mod tests {
    ...
    use futures::stream;
    use slug_common::cas_digest::testing;
    ...
    #[tokio::test]
    async fn test_copy_and_hash_ok() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_copy_and_hash_invalid_primary_hash() -> slug_error::Result<()> {
        assert_matches!(
    ...
    #[tokio::test]
    async fn test_copy_and_hash_invalid_secondary_hash() -> slug_error::Result<()> {
        assert_matches!(
