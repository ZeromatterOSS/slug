...
use slug_directory::directory::walk::ordered_entry_walk;
...
#[derive(slug_error::Error, Debug, Clone, Dupe)]
#[error(
    ...
    To proceed, you should restart Buck using `slug killall`.

Debug information:
  Path: {}
  Digest origin: {}
  Directory:\n{}", .path, .info.origin.as_display_for_not_found(), format_directory_entry_leaves(.directory))]
#[slug(tag = MaterializationError)]
pub struct CasNotFoundError {
    ...
    #[source]
    pub error: Arc<slug_error::Error>,
}
...
pub trait Materializer: Allocative + Send + Sync + 'static {
    ...
    async fn invalidate(&self, path: ProjectRelativePathBuf) -> slug_error::Result<()> {
    ...
    /// Slug significantly slower, after we stop deferring local copies of
    ...
    /// time.
    fn add_snapshot_stats(&self, _snapshot: &mut slug_data::Snapshot) {}
...
impl dyn Materializer {
    ...
    fn check_declared_external_symlink(&self, value: &ArtifactValue) -> slug_error::Result<()> {
        match value.entry() {
...
pub enum MaterializationMethodError {
    #[error(
        "Invalid value for buckconfig `[slug] materializations`. Got `{0}`. Expected one of `all`, `deferred`, or `deferred_skip_final_artifacts`."
    )]
...
pub trait DeferredMaterializerExtensions: Send + Sync {
    fn iterate(&self) -> slug_error::Result<BoxStream<'static, DeferredMaterializerIterItem>>;

    fn list_subscriptions(&self) -> slug_error::Result<BoxStream<'static, ProjectRelativePathBuf>>;

    /// Obtain a list of files that don't match their in-memory representation. This may not catch
    /// all discrepancies.
    fn fsck(
        &self,
    ) -> slug_error::Result<BoxStream<'static, (ProjectRelativePathBuf, slug_error::Error)>>;

    async fn refresh_ttls(&self, min_ttl: i64) -> slug_error::Result<()>;

    async fn get_ttl_refresh_log(&self) -> slug_error::Result<String>;
    ...
    async fn test_iter(&self, count: usize) -> slug_error::Result<String>;
    async fn flush_all_access_times(&self) -> slug_error::Result<String>;

    /// Create a new DeferredMaterializerSubscription.
    async fn create_subscription(
        &self,
    ) -> slug_error::Result<Box<dyn DeferredMaterializerSubscription>>;
}
