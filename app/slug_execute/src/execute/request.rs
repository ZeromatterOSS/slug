...
use slug_core::fs::buck_out_path::BuckOutScratchPath;
...
impl ExecutorPreference {
    pub fn and(self, other: Self) -> slug_error::Result<Self> {
...
impl CommandExecutionPaths {
    pub fn new(
        ...
        let input_files_bytes = if slug_core::faster_directories::is_enabled() {
...
pub struct CommandExecutionRequest {
    ...
    /// What environment variables to inherit from the Slug daemon.
...
pub enum ParamFileFormat {
    ...
    #[default]
    Multiline,
    /// Flag name and value on separate lines (functionally same as Multiline for Slug).
    FlagPerLine,
...
impl CommandExecutionOutputRef<'_> {
    ...
    ) -> slug_error::Result<ResolvedCommandExecutionOutput> {
        match self {
            Self::BuildArtifact {
                path,
                output_type,
