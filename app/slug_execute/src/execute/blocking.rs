...
pub trait BlockingExecutor: Allocative + Send + Sync + 'static {
    ...
    async fn execute_dyn_io_inline<'a>(
        &self,
        f: Box<dyn FnOnce() -> slug_error::Result<()> + Send + 'a>,
    ) -> slug_error::Result<()>;
...
impl dyn BlockingExecutor {
    pub async fn execute_io_inline<F, T>(&self, f: F) -> slug_error::Result<T>
    where
        F: FnOnce() -> slug_error::Result<T> + Send,
        T: Send,
    {
        let mut res = None;
        ...
        .await
        .tag(slug_error::ErrorTag::IoBlockingExecutor)?;
...
struct ThreadPoolIoRequest {
    io: Box<dyn IoRequest>,
    sender: oneshot::Sender<slug_error::Result<()>>,
}
...
impl BuckBlockingExecutor {
    ...
    pub fn default_concurrency(fs: ProjectRoot) -> slug_error::Result<Self> {
        let io_threads = slug_env!("BUCK2_IO_THREADS", type=usize, default=4)?;
        let io_semaphore = slug_env!("BUCK2_IO_SEMAPHORE", type=usize, default=slug_util::threads::available_parallelism())?;
...
impl DirectIoExecutor {
    pub fn new(project_fs: ProjectRoot) -> slug_error::Result<Self> {
        Ok(Self { project_fs })
    }
}
