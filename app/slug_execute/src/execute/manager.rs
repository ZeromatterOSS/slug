...
pub trait CommandExecutionManagerExt: Sized {
    ...
    fn error(
        self,
        stage: &'static str,
        error: impl Into<slug_error::Error>,
    ) -> CommandExecutionResult {
...
fn inline_environment_metadata() -> slug_data::InlineCommandExecutionEnvironmentMetadata {
    slug_data::InlineCommandExecutionEnvironmentMetadata {
        sandcastle_instance_id:
            slug_env!("SANDCASTLE_INSTANCE_ID", type = u64, applicability = internal)
                .ok()
                .flatten(),
    }
}
