...
pub struct CommandExecutionMetadata {
    ...
    /// Additional stats.
    pub execution_stats: Option<slug_data::CommandExecutionStats>,
...
impl CommandExecutionMetadata {
    ...
    pub fn to_proto(&self) -> slug_data::CommandExecutionMetadata {
        let metadata = self.dupe();
        slug_data::CommandExecutionMetadata {
...
pub struct CommandExecutionResult {
    ...
    /// Whether this was uploaded to cache, by Slug.
    pub did_cache_upload: bool,
    /// Whether dep file information for this action was uploaded to cache, by Slug.
...
impl CommandExecutionResult {
    ...
    pub fn resolve_outputs<'a>(
        &'a self,
        fs: &'a ArtifactFs,
    ) -> impl Iterator<Item = slug_error::Result<(ResolvedCommandExecutionOutput, &'a ArtifactValue)>> + 'a
    {
...
pub struct CommandExecutionReport {
    ...
    pub inline_environment_metadata: slug_data::InlineCommandExecutionEnvironmentMetadata,
}
...
impl CommandExecutionReport {
    pub async fn to_command_execution_proto(
        ...
        let status = match &self.status {
            CommandExecutionStatus::Success { .. } => {
                slug_data::command_execution::Success {}.into()
            }
            CommandExecutionStatus::Cancelled { .. } => {
                slug_data::command_execution::Cancelled {}.into()
            }
            CommandExecutionStatus::Failure { .. } => {
                slug_data::command_execution::Failure {}.into()
            }
            ...
            CommandExecutionStatus::Error { stage, error, .. } => {
                slug_data::command_execution::Error {
...
mod tests {
    ...
    fn make_simple_report() -> CommandExecutionReport {
        ...
        let status = CommandExecutionStatus::Success {
            execution_kind: CommandExecutionKind::Local {
                ...
                command: vec!["fake_slug".to_owned()],
                env: {
        ...
        let timing = CommandExecutionMetadata {
            ...
            execution_stats: Some(slug_data::CommandExecutionStats {
                cpu_instructions_user: Some(4),
                cpu_instructions_kernel: Some(5),
                userspace_events: Some(slug_data::CpuCounter {
                    count: 4,
                    time_enabled: 100,
                    time_running: 100,
                }),
                kernel_events: Some(slug_data::CpuCounter {
                    count: 10,
                    time_enabled: 50,
                    time_running: 100,
                }),
        ...
        CommandExecutionReport {
            ...
            inline_environment_metadata: slug_data::InlineCommandExecutionEnvironmentMetadata {
                sandcastle_instance_id: Some(123),
            },
        }
    }

    fn make_simple_proto() -> slug_data::CommandExecution {
        // The field values correspond to what `make_simple_report()` builds.
        let command_execution_kind = slug_data::CommandExecutionKind {
            command: Some(slug_data::command_execution_kind::Command::LocalCommand(
                slug_data::LocalCommand {
                    argv: vec!["fake_slug".to_owned()],
                    env: vec![slug_data::EnvironmentEntry {
        ...
        let command_execution_stats = slug_data::CommandExecutionStats {
            cpu_instructions_user: Some(4),
            cpu_instructions_kernel: Some(5),
            userspace_events: Some(slug_data::CpuCounter {
                count: 4,
        ...
        let command_execution_metadata = slug_data::CommandExecutionMetadata {
        ...
        let command_execution_details = slug_data::CommandExecutionDetails {
        ...
        slug_data::CommandExecution {
            ...
            status: Some(slug_data::command_execution::Status::Success(
                slug_data::command_execution::Success {},
            )),
            inline_environment_metadata: Some(
                slug_data::InlineCommandExecutionEnvironmentMetadata {
                    sandcastle_instance_id: Some(123),
                },
            ),
        }
    ...
    async fn test_to_command_execution_proto_omit_command_details() {
        ...
        expected_proto
            ...
            .unwrap()
            .command = Some(
            slug_data::command_execution_kind::Command::OmittedLocalCommand(
                slug_data::OmittedLocalCommand {
                    action_digest: format!("{}:{}", "0".repeat(64), "123"),
                },
            ),
        );
