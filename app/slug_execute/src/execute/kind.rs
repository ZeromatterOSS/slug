...
impl CommandExecutionKind {
    pub fn as_enum(&self) -> slug_data::ActionExecutionKind {
        match self {
            Self::Local { .. } => slug_data::ActionExecutionKind::Local,
            Self::LocalWorker { .. } | Self::LocalWorkerInit { .. } => {
                slug_data::ActionExecutionKind::LocalWorker
            }
            Self::Remote { details, .. } => match details.persistent_worker {
                false => slug_data::ActionExecutionKind::Remote,
                true => slug_data::ActionExecutionKind::RemoteWorker,
            },
            Self::ActionCache { .. } => slug_data::ActionExecutionKind::ActionCache,
            Self::RemoteDepFileCache { .. } => slug_data::ActionExecutionKind::RemoteDepFileCache,
        }
    }

    pub fn to_proto(&self, omit_details: bool) -> slug_data::CommandExecutionKind {
        ...
        let command = Some(match self {
            Self::Local {
                ...
                if omit_details {
                    Command::OmittedLocalCommand(slug_data::OmittedLocalCommand {
                        action_digest: digest.to_string(),
                    })
                } else {
                    Command::LocalCommand(slug_data::LocalCommand {
                        action_digest: digest.to_string(),
                        argv: command.to_owned(),
                        env: env
                            .iter()
                            .map(|(key, value)| slug_data::EnvironmentEntry {
                                key: key.clone(),
                                value: value.clone(),
                            })
                            .collect(),
            ...
            Self::Remote {
                ...
                cache_hit: false,
                cache_hit_type: slug_data::CacheHitType::Executed.into(),
            ...
            Self::RemoteDepFileCache { details } => {
                Command::RemoteCommand(slug_data::RemoteCommand {
                    action_digest: details.action_digest.to_string(),
                    cache_hit: true,
                    cache_hit_type: slug_data::CacheHitType::RemoteDepFileCache.into(),
                    queue_time: None,
                    details: details.to_proto(omit_details),
            ...
            Self::LocalWorkerInit { command, env } => {
                Command::WorkerInitCommand(slug_data::WorkerInitCommand {
                    argv: command.to_owned(),
                    env: env
                        .iter()
                        .map(|(key, value)| slug_data::EnvironmentEntry {
            ...
            Self::LocalWorker {
                ...
                digest,
                fallback_exe,
            } => Command::WorkerCommand(slug_data::WorkerCommand {
                action_digest: digest.to_string(),
                argv: command.to_owned(),
                env: env
                    .iter()
        ...
        slug_data::CommandExecutionKind { command }
    }
}
...
impl RemoteCommandExecutionDetails {
    ...
    fn to_proto(&self, omit_details: bool) -> Option<slug_data::RemoteCommandDetails> {
        if omit_details {
            return None;
        }

        Some(slug_data::RemoteCommandDetails {
