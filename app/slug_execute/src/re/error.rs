...
fn re_error(
    ...
    let error = slug_error::Error::from(re_error.clone()).context(re_error);
...
pub fn test_re_error(message: &str, code: TCode) -> slug_error::Error {
    re_error(
        "test",
        "test",
        message.to_owned(),
...
mod tests {
    ...
    fn test_extract_re_error() {
        let error: slug_error::Error = re_error(
            "test",
            "test",
            "test".to_owned(),
            TCode::UNKNOWN,
