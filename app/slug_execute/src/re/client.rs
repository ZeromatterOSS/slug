...
impl RemoteExecutionClient {
    pub async fn new(re_config: &RemoteExecutionConfig) -> slug_error::Result<Self> {
        if slug_env!("BUCK2_TEST_FAIL_CONNECT", bool, applicability = testing)? {
    ...
    pub async fn new_retry(re_config: &RemoteExecutionConfig) -> slug_error::Result<Self> {
    ...
    ) -> slug_error::Result<Option<ActionResultResponse>> {
        self.data
            .action_cache
    ...
    ) -> slug_error::Result<ExecuteResponseOrCancelled> {
        self.data
            .executes
    ...
    ) -> slug_error::Result<GetDigestsTtlResponse> {
        self.data
            .get_digests_ttl
    ...
    pub async fn get_digest_expirations(
        ...
        digests: Vec<TDigest>,
        metadata: RemoteExecutionMetadata,
    ) -> slug_error::Result<Vec<(TDigest, DateTime<Utc>)>> {
        let now = Utc::now();
    ...
    ) -> slug_error::Result<WriteActionResultResponse> {
        self.data
    ...
    pub fn get_experiment_name(&self) -> slug_error::Result<Option<String>> {
        self.data
            .client
            .client()
            .get_experiment_name()
            .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::ReExperimentName))
    }
...
fn anticipated_queue_duration(
    ...
    if let Some(duration) = slug_env!(
        "BUCK2_TEST_RE_QUEUE_ESTIMATE_S",
        type=u64,
        applicability = testing
    )
    // Stringify the error because we can't deal with slug_errors here
...
impl RemoteExecutionClientImpl {
    async fn new(re_config: &RemoteExecutionConfig) -> slug_error::Result<Self> {
        let op_name = "REClientBuilder";
        tracing::info!("Creating a new RE client");

        let res: slug_error::Result<Self> = try {
            let download_concurrency =
                slug_env!("BUCK2_RE_DOWNLOAD_CONCURRENCY", type=usize, default=256)?;
            ...
            let client = {
                ...
                // slug makes find_missing calls for the same blobs
                    ...
                    let remote_cache_config = {
                        let mut remote_cache_config = RemoteCacheConfig {
                            ...
                            sync_copy_policy: match static_metadata
                                ...
                                .clone()
                                .unwrap_or(SlugCopyPolicy::Copy)
                            {
                                SlugCopyPolicy::Copy => CopyPolicy::FULL_COPY,
                                SlugCopyPolicy::Reflink => CopyPolicy::SOFT_COPY,
                                SlugCopyPolicy::Hybrid => CopyPolicy::HYBRID_COPY,
                            },
                            ..Default::default()
                        };
                    ...
                    .unwrap_or(
                        if static_metadata.use_zippy_rich_client && cfg!(target_os = "linux") {
                            if static_metadata.shared_casd_address.is_some() {
                                "remote_execution/features/client_slug_pc"
                            } else {
                                "remote_execution/features/client_slug"
                            }
                        } else {
                            "remote_execution/features/client_slug_alternative"
                        },
                    )
                    .to_owned();
    ...
    async fn execute_impl(
        ...
        use slug_data::ReAfterAction;
        use slug_data::ReBeforeAction;
        use slug_data::ReExecute;
        use slug_data::ReQueue;
        use slug_data::ReUnknown;
        use slug_data::ReWorkerDownload;
        use slug_data::ReWorkerUpload;
        use slug_data::re_stage;
    ...
    pub async fn execute<'a>(
        ...
        if slug_env!("BUCK2_TEST_FAIL_RE_EXECUTE", bool, applicability = testing)? {
        ...
        let metadata = RemoteExecutionMetadata {
            ...
            buck_info: Some(BuckInfo {
                version: slug_build_info::revision()
                    .map(|s| s.to_owned())
                    .unwrap_or_default(),
    ...
    async fn download_typed_blobs<T: Message + Default>(
        ...
        digests: Vec<TDigest>,
        use_case: RemoteExecutorUseCase,
    ) -> slug_error::Result<(Vec<T>, TLocalCacheStats)> {
        if digests.is_empty() {
        ...
        if blobs.len() != expected_blobs {
            return Err(slug_error!(
                slug_error::ErrorTag::CasBlobCountMismatch,
    ...
    pub async fn download_blob(
        ...
        digest: &TDigest,
        use_case: RemoteExecutorUseCase,
    ) -> slug_error::Result<(Vec<u8>, TLocalCacheStats)> {
    ...
    async fn materialize_files(
        ...
        files: Vec<NamedDigestWithPermissions>,
        use_case: RemoteExecutorUseCase,
    ) -> slug_error::Result<TLocalCacheStats> {
        if slug_env!(
        ...
        let futs = chunks(files, self.download_chunk_size).map(|chunk| async move {
            ...
            slug_error::Ok(response.local_cache_stats)
        });

        let stat = slug_util::future::try_join_all(futs).await?.iter().fold(
