...
impl Uploader {
    async fn find_missing<'a>(
        ...
        let now = Utc::now();
        let ttl_wanted = if slug_core::is_open_source() {
            1
        } else {
            600i64
        };
        ...
        let digests_and_ttls_iterator = if deduplicate_get_digests_ttl_calls {
            ...
            {
                type Item = slug_error::Result<(&'a TrackedFileDigest, i64)>;

                fn next(&mut self) -> Option<slug_error::Result<(&'a TrackedFileDigest, i64)>> {
    ...
    pub async fn upload(
                            ...
                            if file.digest.to_re() == digest {
                                if should_error_for_missing_digest(info) {
                                    ...
                                    return Err(slug_error::slug_error!(
                                        slug_error::ErrorTag::ReCasArtifactExpired,
        ...
        if !upload_files.is_empty() || !upload_blobs.is_empty() {
            ...
            .await
            .map_err(|e| {
                if e.tags().contains(&slug_error::ErrorTag::ReInvalidArgument) {
                    slug_error::slug_error!(
                        slug_error::ErrorTag::ReInvalidArgument,
...
fn error_for_missing_file(
    ...
    slug_error::slug_error!(
        slug_error::ErrorTag::ReInvalidGetCasResponse,
        ...
        `slug killall`. We would appreciate a bug report. Debug information: {:#}",
        digest,
        cause,
    )
}
...
fn add_injected_missing_digests<'a>(
    ...
    fn convert_digests(val: &str) -> slug_error::Result<Vec<FileDigest>> {
        val.split(' ')
            .map(|digest| {
                let digest = TDigest::from_str(digest)
                    .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::InvalidDigest))
                ...
                slug_error::Ok(digest)
            })
            .collect()
    }

    let ingested_digests = slug_env!(
...
struct GetDigestsTtlDeduper<'s> {
    ...
    queries: HashMap<
        RequestId,
        Shared<BoxFuture<'s, slug_error::Result<HashMap<TrackedFileDigest, i64>>>>,
    >,
}
...
impl<'s> GetDigestsTtlDeduper<'s> {
    ...
    fn get_ttls<'a>(
        ...
        impl Future<Output = slug_error::Result<HashMap<TrackedFileDigest, i64>>> + 's,
        usize,
        usize,
    ) {
...
) -> BoxFuture<'s, slug_error::Result<HashMap<TrackedFileDigest, i64>>> {
...
fn process_get_digest_ttls_response<T>(
    ...
    digest_config: DigestConfig,
) -> slug_error::Result<impl Iterator<Item = slug_error::Result<(T, i64)>>>
where
    T: Borrow<TrackedFileDigest> + Ord,
{
    ...
    let mut digest_ttls = digest_ttls.into_try_map(|d| {
        slug_error::Ok((
            FileDigest::from_re(&d.digest, digest_config).map_err(slug_error::Error::from)?,
            d.ttl,
        ))
    })?;
    digest_ttls.sort();
