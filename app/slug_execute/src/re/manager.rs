...
impl RemoteExecutionConfig {
    async fn connect_now(&self) -> slug_error::Result<RemoteExecutionClient> {
...
struct LazyRemoteExecutionClient {
    client: AsyncOnceCell<slug_error::Result<RemoteExecutionClient>>,
...
impl LazyRemoteExecutionClient {
    ...
    async fn get(&self) -> slug_error::Result<&RemoteExecutionClient> {
    ...
    async fn init(&self) -> slug_error::Result<RemoteExecutionClient> {
...
impl ReConnectionManager {
    ...
    /// pointed slug at an RE service via `--remote_executor` /
    /// `[slug_re_client] address`.
    ...
    /// equivalent `[slug_re_client] default_exec_properties` buckconfig
    ...
    pub fn get_network_stats(&self) -> slug_error::Result<RemoteExecutionClientStats> {
...
impl ReGetSessionId for ReConnectionManager {
    async fn get_session_id(&self) -> slug_error::Result<String> {
...
impl UnconfiguredRemoteExecutionClient {
    ...
    fn lock(&self) -> slug_error::Result<Arc<Arc<LazyRemoteExecutionClient>>> {
        self.data
            .upgrade()
    ...
    pub async fn get_session_id(&self) -> slug_error::Result<String> {
...
impl ManagedRemoteExecutionClient {
    ...
    pub async fn download_blob(&self, digest: &TDigest) -> slug_error::Result<Vec<u8>> {
        self.lock()?
            .get()
            .await?
    ...
    pub async fn upload_blob(&self, blob: InlinedBlobWithDigest) -> slug_error::Result<TDigest> {
        self.lock()?
            .get()
            .await?
