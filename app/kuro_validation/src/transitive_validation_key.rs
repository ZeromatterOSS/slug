...
impl TransitiveValidationKey {
    ...
    async fn validate_current_node(
        ...
        let artifacts = info
            ...
            .collect::<kuro_error::Result<Vec<Artifact>>>()?;
...
impl Key for TransitiveValidationKey {
    type Value = kuro_error::Result<CachedValidationResult>;
...
enum TreatValidationFailureAsError {
    ...
    Transient(kuro_error::Error),
}

impl From<kuro_error::Error> for TreatValidationFailureAsError {
...
impl From<DiceError> for TreatValidationFailureAsError {
    fn from(value: DiceError) -> Self {
        TreatValidationFailureAsError::Transient(kuro_error::Error::from(value))
    }
}
...
fn tighten_cached_validation_result(
    result: kuro_error::Result<CachedValidationResult>,
) -> Result<(), TreatValidationFailureAsError> {
    match result {
