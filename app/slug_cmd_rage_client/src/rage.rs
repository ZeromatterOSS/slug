...
use slug_client_ctx::daemon::client::connect::BuckdProcessInfo;
use slug_client_ctx::exit_result::ExitResult;
use slug_client_ctx::stdin::Stdin;
use slug_client_ctx::thread_dump::thread_dump_command;
use slug_client_ctx::upload_re_logs::upload_re_logs;
...
use slug_data::RageResult;
use slug_data::instant_event::Data;
use slug_error::BuckErrorContext;
use slug_event_log::file_names::do_find_log_by_trace_id;
use slug_event_log::file_names::get_local_logs;
use slug_event_log::read::EventLogPathBuf;
use slug_event_log::read::EventLogSummary;
use slug_events::BuckEvent;
use slug_events::sink::remote::RemoteEventSink;
...
#[clap(
    name = "rage",
    about = "Record information about the previous failed slug command",
...
pub struct RageCommand {
    ...
    /// Collect rage report about slug in general, not about specific invocation
    #[clap(long, group = "invocation")]
    no_invocation: bool,
...
impl RageCommand {
    ...
    async fn exec_impl(self, mut ctx: ClientCommandContext<'_>) -> slug_error::Result<()> {
        let paths = ctx.paths()?;
        ...
        let info = BuckdProcessInfo::load(&daemon_dir).map_err(slug_error::Error::from);
        ...
        slug_client_ctx::eprintln!("Collecting debug info...")?;
    ...
    async fn send_to_scuba(
        ...
        let slug_revision = build_info.get_field(|o| Some(o.slug_revision.to_owned()));
        ...
        insert_if_some(&mut string_data, "slug_revision", slug_revision.clone());
    ...
    where
        Fut: Future<Output = slug_error::Result<T>> + 'a,
        T: std::fmt::Display + 'a,
    {
...
fn create_scribe_sink(ctx: &ClientCommandContext) -> slug_error::Result<Option<RemoteEventSink>> {
...
async fn maybe_select_invocation(
    ...
    command: &RageCommand,
) -> slug_error::Result<Option<EventLogPathBuf>> {
...
async fn user_prompt_select_log<'a>(
...
) -> slug_error::Result<usize> {
    slug_client_ctx::eprintln!("Which buck invocation would you like to report?\n")?;
    ...
    slug_client_ctx::eprintln!("Selected invocation {}\n", selection)?;
    Ok(selection)
}
...
async fn output_rage(no_paste: bool, output: &str) -> slug_error::Result<()> {
    if no_paste {
        slug_client_ctx::println!("{}", output)?;
    } else {
        match generate_paste("Slug Rage", output).await {
            Err(e) => {
                slug_client_ctx::eprintln!(
            ...
            Ok(paste) => slug_client_ctx::eprintln!(
                "\nPlease post in https://fb.workplace.com/groups/slugusers with the following link:\n\n{}\n",
                paste
            )?,
        }
    };
...
async fn generate_paste(title: &str, content: &str) -> slug_error::Result<String> {
...
async fn upload_thread_dump(
    buckd: &slug_error::Result<BuckdProcessInfo<'_>>,
    manifold: &ManifoldClient,
    manifold_id: &String,
) -> slug_error::Result<String> {
...
async fn get_trace_id(invocation: &Option<EventLogPathBuf>) -> slug_error::Result<Option<TraceId>> {
