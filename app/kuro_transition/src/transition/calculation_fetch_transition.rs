...
pub(crate) trait FetchTransition {
    ...
    async fn fetch_transition(&mut self, id: &TransitionId) -> kuro_error::Result<TransitionData>;
}
        ...
        match id {
            TransitionId::MagicObject { path, name } => {
                ...
                let transition = module
                    ...
                    .map_err(|_| {
                        kuro_error::Error::from(FetchTransitionError::NotFound(id.clone()))
                    })?
                    .0;
...
impl Key for TransitionAttrsKey {
    type Value = kuro_error::Result<Option<Arc<[String]>>>;
...
impl TransitionAttrProvider for TransitionGetAttrs {
    ...
    ) -> kuro_error::Result<Option<Arc<[String]>>> {
