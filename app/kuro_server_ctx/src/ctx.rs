...
use kuro_data::CommandCriticalEnd;
use kuro_data::CommandCriticalStart;
use kuro_data::DiceCriticalSectionEnd;
use kuro_data::DiceCriticalSectionStart;
...
pub struct PreviousCommandDataInternal {
    pub external_and_local_configs: Vec<kuro_data::BuckconfigComponent>,
...
impl PreviousCommandData {
    pub fn process_current_command(
        ...
        current_external_and_local_configs: Vec<kuro_data::BuckconfigComponent>,
        ...
        }) = self.data.as_ref()
        {
            if *current_external_and_local_configs != *external_configs {
                event_dispatcher.instant_event(kuro_data::PreviousCommandWithMismatchedConfig {
...
pub trait ServerCommandContextTrait: Send + Sync {
    ...
    ) -> kuro_error::Result<DiceAccessor<'a>>;
...
pub trait ServerCommandDiceContext {
    async fn with_dice_ctx<'v, F, Fut, R>(&'v self, exec: F) -> kuro_error::Result<R>
    where
                            ...
                            |mut dice, early_command_timing| async move {
                                ...
                                events
                                    .span_async(
                                        ...
                                        async move {
                                            let res = kuro_build_signals::env::scope(
                                                build_signals,
                                                self.events().dupe(),
