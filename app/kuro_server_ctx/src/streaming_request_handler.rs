...
#[pin_project]
pub struct StreamingRequestHandler<T: TryFrom<StreamingRequest, Error = kuro_error::Error>> {
    #[pin]
    client_stream: tonic::Streaming<StreamingRequest>,
    _phantom: PhantomData<T>,
}

impl<T: TryFrom<StreamingRequest, Error = kuro_error::Error>> StreamingRequestHandler<T> {
    ...
    pub async fn message(&mut self) -> kuro_error::Result<T> {
    ...
    fn map_poll(v: Option<Result<StreamingRequest, Status>>) -> Option<kuro_error::Result<T>> {
        match v {
            None => None,
...
impl<T: TryFrom<StreamingRequest, Error = kuro_error::Error>> Stream
    for StreamingRequestHandler<T>
{
    type Item = kuro_error::Result<T>;
