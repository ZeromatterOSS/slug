...
impl<'a> StdoutPartialOutput<'a> {
    pub fn new(dispatcher: &'a mut PartialResultDispatcher<kuro_cli_proto::StdoutBytes>) -> Self {
        Self {
...
struct WriterWrapper<'a> {
    inner: &'a mut PartialResultDispatcher<kuro_cli_proto::StdoutBytes>,
}
...
impl Write for WriterWrapper<'_> {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        ...
        for chunk in buf.chunks(CHUNK_SIZE) {
            self.inner.emit(kuro_cli_proto::StdoutBytes {
                data: chunk.to_owned(),
            });
        }
