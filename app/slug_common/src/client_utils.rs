...
#[cfg(unix)]
async fn get_channel_uds_no_symlink(connect_to: &Path) -> slug_error::Result<Channel> {
...
#[cfg(windows)]
pub async fn get_channel_uds(_unix_filename: &Path, _chg_dir: bool) -> slug_error::Result<Channel> {
    Err(slug_error::slug_error!(
        slug_error::ErrorTag::WindowsUnsupported,
        "Unix domain sockets are not supported on Windows",
    ))
}

pub async fn get_channel_tcp(socket_addr: Ipv4Addr, port: u16) -> slug_error::Result<Channel> {
...
mod tests {
    ...
    async fn test_retrying_error_forever() {
        ...
        let future = retrying(
            ...
            Duration::from_millis(1),
            Duration::from_millis(1),
            || async { Err(slug_error::slug_error!(slug_error::ErrorTag::Input, "test")) },
        );
        let result: Result<(), RetryError<slug_error::Error>> = future.await;
