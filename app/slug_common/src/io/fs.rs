...
impl FsIoProvider {
    ...
    ) -> slug_error::Result<RawPathMetadata<ProjectRelativePathBuf>> {
        let fs = self.fs.dupe();
...
) -> slug_error::Result<Option<RawPathMetadata<ForwardRelativePathBuf>>> {
...
fn convert_metadata(
    ...
    file_digest_config: FileDigestConfig,
) -> slug_error::Result<RawPathMetadata<ForwardRelativePathBuf>> {
...
impl ExactPathMetadata {
    fn from_exact_path(curr: &PathAndAbsPath) -> slug_error::Result<Self> {
        Ok(
...
mod tests {
    ...
    #[test]
    fn test_read_not_symlink() -> slug_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_symlink() -> slug_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_symlink_in_dir() -> slug_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_symlink_remaining_path() -> slug_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_symlink_out_of_project() -> slug_error::Result<()> {
        let t = TempDir::new()?;
    ...
    #[test]
    fn test_read_unchecked_symlink() -> slug_error::Result<()> {
        let t = TempDir::new()?;
        let root = AbsPath::new(t.path())?;
