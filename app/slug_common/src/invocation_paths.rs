...
//!
//! Defines utilities to obtain the basic paths for slug client and the daemon.
...
impl InvocationPaths {
    pub fn daemon_dir(&self) -> slug_error::Result<DaemonDir> {
        #[cfg(windows)]
        ...
        let root_relative: Cow<ForwardRelativePath> = self
            ...
            .project_root
            .root()
            .strip_prefix(slug_fs::paths::abs_norm_path::AbsNormPath::new("/")?)?;
