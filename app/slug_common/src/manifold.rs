...
impl Default for Ttl {
    fn default() -> Self {
        Self::from_secs(164 * 86_400) // 164 days, equals scuba slug_builds retention
    }
}
...
impl AsSlugError for HttpWriteError {
    fn as_slug_error(self) -> slug_error::Error {
        slug_error::Error::from(self)
    }
}

impl AsSlugError for HttpAppendError {
    fn as_slug_error(self) -> slug_error::Error {
        slug_error::Error::from(self)
    }
}
...
impl Bucket {
    ...
    pub const RAGE_DUMPS: Bucket = Bucket {
        name: "slug_rage_dumps",
        key: "slug_rage_dumps-key",
    };

    pub const RE_LOGS: Bucket = Bucket {
        name: "slug_re_logs",
        key: "slug_re_logs-key",
    };

    pub const INSTALLER_LOGS: Bucket = Bucket {
        name: "slug_installer_logs",
        key: "slug_installer_logs-key",
    };
}
...
impl ManifoldChunkedUploader<'_> {
    pub async fn write(&mut self, chunk: Bytes) -> slug_error::Result<()> {
