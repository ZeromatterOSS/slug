...
impl FileOps for TestFileOps {
    ...
    async fn read_dir(&self, path: CellPathRef<'async_trait>) -> slug_error::Result<ReadDirOutput> {
        let included = self
            .entries
            .get(&path.to_owned())
            .and_then(|e| match e {
    ...
    async fn buildfiles<'a>(&self, _cell: CellName) -> slug_error::Result<Arc<[FileNameBuf]>> {
        Ok(Arc::from_iter([
