...
mod unix {
    ...
    enum StoppedWaiting {
        Success,
        UnexpectedError(slug_error::Error),
    }
    ...
    fn check_result(pid: nix::unistd::Pid, result: nix::Result<()>) -> ControlFlow<StoppedWaiting> {
        match result {
            ...
            Err(e) => ControlFlow::Break(StoppedWaiting::UnexpectedError(slug_error::slug_error!(
...
mod tests {
    ...
    #[tokio::test]
    async fn test_graceful_termination() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_sigkill_when_graceful_termination_fails() -> slug_error::Result<()> {
        let dir = tempfile::tempdir()?;
