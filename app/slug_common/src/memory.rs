...
//! Utilities for interacting with the jemalloc heap used by slug.
//!
...
mod imp {
    ...
    pub fn write_heap_to_file(filename: &str) -> slug_error::Result<()> {
        ...
        let prof_enabled: bool = memory::mallctl_read("opt.prof")
            .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::Mallctl))?;
        if !prof_enabled {
    ...
    pub fn allocator_stats(options: &str) -> slug_error::Result<String> {
        allocator_stats::malloc_stats(options)
            .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::MallocStats))
    }
    ...
    pub fn enable_background_threads() -> slug_error::Result<()> {
        if memory::is_using_jemalloc() {
...
mod imp {
    pub fn write_heap_to_file(_filename: &str) -> slug_error::Result<()> {
        ...
        Err(slug_error::slug_error!(
            slug_error::ErrorTag::Unimplemented,
            "not implemented: heap dump for Cargo builds"
        ))
    }

    pub fn allocator_stats(_: &str) -> slug_error::Result<String> {
        Err(slug_error::slug_error!(
            slug_error::ErrorTag::Unimplemented,
