...
impl PatternParser {
    ...
    fn parse_pattern<T: PatternType>(&self, pattern: &str) -> slug_error::Result<ParsedPattern<T>> {
...
pub async fn parse_patterns_from_cli_args<T: PatternType>(
    ...
    cwd: &ProjectRelativePath,
) -> slug_error::Result<Vec<ParsedPattern<T>>> {
...
pub async fn parse_patterns_with_modifiers_from_cli_args<T: PatternType>(
    ...
    target_patterns: &[String],
    cwd: &ProjectRelativePath,
) -> slug_error::Result<Vec<ParsedPatternWithModifiers<T>>> {
    let parser = PatternParser::new(ctx, cwd).await?;
