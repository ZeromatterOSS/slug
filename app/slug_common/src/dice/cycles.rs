...
use futures::Future;
use slug_util::cycle_detector::CycleDescriptor;
use slug_util::cycle_detector::LazyCycleDetector;
use slug_util::cycle_detector::LazyCycleDetectorGuard;
use tracing::debug;
...
pub struct CycleGuardResult<R, E>(slug_error::Result<Result<R, E>>);

impl<R, E> CycleGuardResult<R, E> {
    /// Converts the result from GuardThis into an slug_error::Result.
    ///
...
impl<D: CycleAdapterDescriptor> CycleGuard<D> {
    pub fn new(ctx: &DiceComputations<'_>) -> slug_error::Result<Self> {
    ...
    pub async fn guard_this<R: Send, Fut: Future<Output = R> + Send>(
        ...
        let res: slug_error::Result<Result<R, D::Error>> = (|| async move {
            match &self.0 {
