...
#[derive(slug_error::Error, Debug, PartialEq, Eq)]
#[slug(tag = Input)]
enum SqliteDbError {
...
pub struct SqliteTables<T: SqliteTable> {
    ...
    /// versions this slug binary expects. If the versions don't match, we throw
    ...
    /// Table for logging metadata associated with the slug that created the db.
...
impl<T: SqliteTable> SqliteTables<T> {
    ...
    pub fn get_identity(&self) -> slug_error::Result<SqliteIdentity> {
        let identity = self
            .created_by_table
            .get(IDENTITY_KEY)
    ...
    pub fn create_all_tables(&self) -> slug_error::Result<()> {
    ...
    pub fn create_connection(path: &AbsNormPath) -> slug_error::Result<Arc<Mutex<Connection>>> {
...
mod tests {
    ...
    impl TestSqliteTable {
        ...
        fn insert(&self, value: &str) -> slug_error::Result<()> {
        ...
        fn read_all(&self) -> slug_error::Result<TestState> {
    ...
    impl TestSqliteDb {
        fn insert_test_data(&mut self, value: &str) -> slug_error::Result<()> {
    ...
    #[test]
    fn test_create_new_sqlite_db() -> slug_error::Result<()> {
    ...
    #[test]
    fn test_load_existing_sqlite_db() -> slug_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let db_dir = fs
            .path()
            .resolve(ProjectRelativePath::unchecked_new("test_db_dir"));
    ...
    #[test]
    fn test_version_mismatch_error() -> slug_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let db_dir = fs
            .path()
            .resolve(ProjectRelativePath::unchecked_new("test_db_dir"));
    ...
    #[test]
    fn test_rejected_identity() -> slug_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let db_dir = fs
            .path()
            .resolve(ProjectRelativePath::unchecked_new("test_db_dir"));
    ...
    #[test]
    fn test_path_does_not_exist_error() -> slug_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
    ...
    #[test]
    fn test_sqlite_tables_functionality() -> slug_error::Result<()> {
        let fs = ProjectRootTemp::new()?;
        let db_path = fs
            .path()
