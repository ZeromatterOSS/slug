...
fn resolve_config_flag_arg(
    ...
    raw_arg: &str,
) -> slug_error::Result<ResolvedConfigFlag> {
...
pub(crate) async fn resolve_config_args(
    args: &[ConfigOverride],
) -> slug_error::Result<Vec<ResolvedLegacyConfigArg>> {
        ...
        .map(|arg| {
            let data = match arg {
                ResolvedLegacyConfigArg::Flag(resolved_config_flag) => {
                    ConfigValue(slug_data::ConfigValue {
            ...
            slug_data::BuckconfigComponent { data: Some(data) }
        })
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn test_argument_pair() -> slug_error::Result<()> {
        // Valid Formats
