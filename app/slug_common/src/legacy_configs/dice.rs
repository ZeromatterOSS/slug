...
impl LegacyBuckConfigOnDice<'_, '_> {
    pub fn parse<T: FromStr>(&mut self, key: BuckconfigKeyRef) -> slug_error::Result<Option<T>>
    where
        slug_error::Error: From<<T as FromStr>::Err>,
    {
...
impl LegacyBuckConfigView for LegacyBuckConfigOnDice<'_, '_> {
    fn get(&mut self, key: BuckconfigKeyRef) -> slug_error::Result<Option<Arc<str>>> {
...
pub trait HasInjectedLegacyConfigs {
    fn get_injected_external_buckconfig_data(
        &mut self,
    ) -> impl Future<Output = slug_error::Result<Arc<ExternalBuckconfigData>>>;

    fn is_injected_external_buckconfig_data_key_set(
        &mut self,
    ) -> impl Future<Output = slug_error::Result<bool>>;
}
...
pub trait SetLegacyConfigs {
    ...
    fn set_none_legacy_config_external_data(&mut self) -> slug_error::Result<()>;
}
...
impl Key for LegacyBuckConfigForCellKey {
    type Value = slug_error::Result<LegacyBuckConfig>;
...
impl ProjectionKey for LegacyBuckConfigErrorKey {
    ...
    type Value = Option<slug_error::Error>;

    fn compute(
        &self,
        config: &slug_error::Result<LegacyBuckConfig>,
        _ctx: &DiceProjectionComputations,
    ) -> Option<slug_error::Error> {
...
impl HasInjectedLegacyConfigs for DiceComputations<'_> {
    async fn get_injected_external_buckconfig_data(
        &mut self,
    ) -> slug_error::Result<Arc<ExternalBuckconfigData>> {
    ...
    async fn is_injected_external_buckconfig_data_key_set(&mut self) -> slug_error::Result<bool> {
        Ok(self
...
mod tests {
    ...
    #[test]
    fn config_equals() -> slug_error::Result<()> {
