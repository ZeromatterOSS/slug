...
pub struct SystemWarningConfig {
    /// A threshold that is used to determine the percent of memory slug uses to display memory pressure warnings.
    /// If None, we don't warn the user.
    /// The corresponding buckconfig is `slug_system_warning.memory_pressure_threshold_percent`.
    pub memory_pressure_threshold_percent: Option<u64>,
    /// A threshold that is used to determine remaining disk space slug uses to display disk space warnings.
    /// If None, we don't warn the user.
    /// The corresponding buckconfig is `slug_system_warning.remaining_disk_space_threshold`.
    pub remaining_disk_space_threshold_gb: Option<u64>,
    /// Minimum number of bytes downloaded to measure average download speed.
    /// If None, we don't warn the user.
    /// The corresponding buckconfig is `slug_system_warning.min_re_download_bytes_threshold`.
    ...
    /// If None, we don't warn the user.
    /// The corresponding buckconfig is `slug_system_warning.avg_re_download_bytes_per_sec_threshold`.
    pub avg_re_download_bytes_per_sec_threshold: Option<u64>,
    /// A regex that controls which targets are opted into the vpn check.
    /// The corresponding buckconfig is `slug_health_check.optin_vpn_check_targets_regex`.
...
impl SystemWarningConfig {
    pub fn serialize(&self) -> slug_error::Result<String> {
        serde_json::to_string(&self).buck_error_context("Error serializing SystemWarningConfig")
    }

    pub fn deserialize(s: &str) -> slug_error::Result<Self> {
...
pub struct ResourceControlConfig {
    ...
    /// The corresponding buckconfig is `slug_resource_control.status` that can take
    /// one of `{off | if_available | required}`.
    pub status: ResourceControlStatus,
    /// Maximum allowed memory usage for all work slug manages.
    ///
    /// Accepts either a number of bytes or a percentage of the available resources.
    ///
    /// The corresponding buckconfig is `slug_resource_control.memory_max`.
    pub memory_max: Option<String>,
    /// Like `memory_max`, but controls cgroupv2's `memory.high`
    ///
    /// The corresponding buckconfig is `slug_resource_control.memory_high`.
    ...
    /// The corresponding buckconfig is `slug_resource_control.memory_high_action_cgroup_pool`.
...
pub enum ResourceControlStatus {
    ...
    /// slug errors it out and the command returns with an error exit code.
    Required,
}
...
impl ResourceControlConfig {
    pub fn default_or_from_env() -> slug_error::Result<Self> {
        if let Some(env_conf) = slug_env!(
...
pub struct DaemonStartupConfig {
    ...
    /// `edenfs`). Sourced from `--slug_file_watcher`. `None` means
    ...
    /// Snapshot of the `[slug_re_client]` buckconfig section at daemon
...
/// Subset of `[slug_re_client]` buckconfig that's load-bearing for the
/// daemon's remote-execution client. Not the full
/// `SlugOssReConfiguration` — just the fields that, if changed,
...
pub struct ReConfigSnapshot {
    ...
    /// compatibility) and merged with the slug defaults at action
...
impl ReConfigSnapshot {
    /// `[slug_re_client]` is no longer parsed from `.buckconfig`; the
    ...
    pub fn from_config(_config: &LegacyBuckConfig) -> slug_error::Result<Self> {
        Ok(Self::default())
    }
}

impl DaemonStartupConfig {
    pub fn new(config: &LegacyBuckConfig) -> slug_error::Result<Self> {
        ...
        Ok(Self {
            ...
            daemon_buster: None,
            // `[slug] digest_algorithms` is dead — the user-facing knob
