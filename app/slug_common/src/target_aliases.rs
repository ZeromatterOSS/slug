...
use slug_core::target_aliases::TargetAliasResolver;
...
impl TargetAliasResolver for BuckConfigTargetAliasResolver {
    fn get<'a>(&'a self, name: &str) -> slug_error::Result<Option<&'a str>> {
        match self.resolve_alias(name) {
            ...
            ) => Err(slug_error::Error::from(e).context(format!("Error resolving alias `{name}`"))),
        }
    }
}
...
pub trait HasTargetAliasResolver {
    async fn target_alias_resolver(&mut self) -> slug_error::Result<BuckConfigTargetAliasResolver>;
}
...
impl Key for TargetAliasResolverKey {
    type Value = slug_error::Result<BuckConfigTargetAliasResolver>;

    async fn compute(
        &self,
        ctx: &mut DiceComputations,
        _cancellations: &CancellationContext,
    ) -> slug_error::Result<BuckConfigTargetAliasResolver> {
