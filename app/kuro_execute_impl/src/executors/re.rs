...
use kuro_execute::execute::kind::RemoteCommandExecutionDetails;
...
use kuro_execute::execute::result::CommandExecutionErrorType;
...
use kuro_execute::re::action_identity::ReActionIdentity;
use kuro_execute::re::client::CancellationReason;
use kuro_execute::re::client::ExecuteResponseOrCancelled;
use kuro_execute::re::error::RemoteExecutionError;
use kuro_execute::re::error::get_re_error_tag;
...
use kuro_execute::re::remote_action_result::ExecuteResponseWithQueueStats;
use kuro_execute::re::remote_action_result::RemoteActionResult;
...
impl ReExecutor {
    async fn upload(
        ...
        let upload_response = span_async(kuro_data::ReUploadStart {}, async move {
            let res = re_client
                .upload(
                    &self.project_fs,
                    &self.materializer,
            ...
            match res {
                ...
                Err(e) => (Err(e), kuro_data::ReUploadEnd::default()),
            }
        })
        .await;
    ...
    ) -> ControlFlow<CommandExecutionResult, (CommandExecutionManager, ExecuteResponseWithQueueStats)>
    {
        ...
        let execute_response =
            if let Some(timeout) = kuro_common::self_test_timeout::maybe_cap_timeout(None) {
    ...
    async fn exec_cmd(
        ...
        let res = download_action_results(
            ...
            *digest_config,
            ...
            &identity,
            kuro_data::ReStage {
                stage: Some(kuro_data::ReDownload {}.into()),
            }
            .into(),
            request.paths(),
            request.outputs(),
...
#[kuro(tier0, tag = get_re_error_tag(&inner.code))]
