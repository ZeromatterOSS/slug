...
use kuro_execute::directory::directory_to_re_tree;
...
use kuro_execute::execute::cache_uploader::UploadCache;
...
fn error_on_cache_upload() -> kuro_error::Result<bool> {
    kuro_env!(
...
impl CacheUploader {
    ...
    async fn upload_local_outputs(
        ...
        span_async(
            ...
            async {
                ...
                .await
                .map_err(|e: kuro_error::Error| kuro_error::Error::from(e))
                .unwrap_or_else(CacheUploadOutcome::Failed);

                let cache_upload_end_event = kuro_data::CacheUploadEnd {
                    key: Some(info.target.as_proto_action_key()),
                    name: Some(info.target.as_proto_action_name()),
                    action_digest: digest_str.clone(),
    ...
    async fn upload_dep_file(
        ...
        span_async(
            ...
            async {
                ...
                let end_event = kuro_data::DepFileUploadEnd {
                    key: Some(info.target.as_proto_action_key()),
                    name: Some(info.target.as_proto_action_name()),
                    remote_dep_file_key: remote_dep_file_key.clone(),
                    success: outcome.uploaded(),
    ...
    async fn check_upload_permission(
        &self,
        info: &CacheUploadInfo<'_>,
    ) -> kuro_error::Result<Result<(), CacheUploadOutcome>> {
    ...
    async fn upload_files_and_directories(
    ...
    ) -> kuro_error::Result<Result<TActionResult2, CacheUploadRejectionReason>> {
        ...
        let uploads = async {
            kuro_util::future::try_join_all(upload_futs)
                .await
    ...
    fn log_and_create_result(
        ...
        if !self.uploaded() && error_on_cache_upload {
            Err(kuro_error::kuro_error!(
                kuro_error::ErrorTag::CacheUploadFailed,
                "cache_upload_failed"
            ))
        } else {
            Ok(self)
...
fn systemtime_to_ttimestamp(time: SystemTime) -> kuro_error::Result<TTimestamp> {
