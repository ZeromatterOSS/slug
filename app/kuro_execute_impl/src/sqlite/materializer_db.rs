...
use kuro_common::sqlite::sqlite_db::SqliteTables;
...
/// then you can fix forward by bumping the `kuro.sqlite_materializer_state_version`
...
impl MaterializerStateSqliteDb {
    ...
    /// or the DB has a different set of versions than the versions this kuro expects, we
    ...
    ) -> kuro_error::Result<(Self, kuro_error::Result<MaterializerState>)> {
        io_executor
            .execute_io_inline(|| {
...
mod tests {
    ...
    #[test]
    fn test_initialize_sqlite_db() -> kuro_error::Result<()> {
        fn testing_metadatas() -> Vec<HashMap<String, String>> {
            let metadata = kuro_events::metadata::collect(&DaemonId::new());
