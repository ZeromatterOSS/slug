...
) -> kuro_error::Result<IncrementalState> {
...
impl IncrementalStateSqliteTable {
    ...
    pub(crate) fn read_incremental_state(&self) -> kuro_error::Result<IncrementalState> {
    ...
    pub(crate) fn delete(&self, run_action_key: String) -> kuro_error::Result<usize> {
...
mod tests {
    ...
    #[test]
    fn test_insert_and_read_single_action() -> kuro_error::Result<()> {
    ...
    #[test]
    fn test_insert_and_read_multiple_actions() -> kuro_error::Result<()> {
        let conn = Connection::open_in_memory()?;
        let table = IncrementalStateSqliteTable::new(Arc::new(Mutex::new(conn)));
        table.create_table()?;
    ...
    #[test]
    fn test_delete_action() -> kuro_error::Result<()> {
        let conn = Connection::open_in_memory()?;
        let table = IncrementalStateSqliteTable::new(Arc::new(Mutex::new(conn)));
        table.create_table()?;
