...
) -> kuro_error::Result<MaterializerState> {
    #[derive(Default)]
...
fn convert_non_directory_sqlite_entry_to_materializer_state_entry(
    ...
    digest_config: DigestConfig,
) -> kuro_error::Result<MaterializerStateEntry> {
...
fn convert_non_directory_sqlite_entry_to_action_directory_member(
    sqlite_entry: SqliteEntry,
    digest_config: DigestConfig,
) -> kuro_error::Result<ActionDirectoryMember> {
...
impl MaterializerStateSqliteTable {
    ...
    pub(crate) fn create_table(&self) -> kuro_error::Result<()> {
        let sql = format!(
    ...
    fn create_parent_path_index(&self) -> kuro_error::Result<()> {
        let sql = format!(
    ...
    fn read_all_entries(&self) -> kuro_error::Result<Vec<SqliteEntry<'_>>> {
        static SQL: Lazy<String> = Lazy::new(|| {
            format!(
    ...
    pub(crate) fn delete(&self, paths: Vec<ProjectRelativePathBuf>) -> kuro_error::Result<usize> {
        if paths.is_empty() {
            return Ok(0);
        }
...
mod tests {
    ...
    #[test]
    fn test_delete_many() -> kuro_error::Result<()> {
