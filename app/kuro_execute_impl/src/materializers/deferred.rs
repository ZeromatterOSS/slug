...
use kuro_directory::directory::directory_iterator::DirectoryIteratorPathStack;
use kuro_directory::directory::entry::DirectoryEntry;
use kuro_directory::directory::walk::unordered_entry_walk;
...
use kuro_execute::materialize::materializer::ArtifactNotMaterializedReason;
use kuro_execute::materialize::materializer::CasDownloadInfo;
use kuro_execute::materialize::materializer::CasNotFoundError;
use kuro_execute::materialize::materializer::CopiedArtifact;
use kuro_execute::materialize::materializer::DeclareArtifactPayload;
use kuro_execute::materialize::materializer::DeclareMatchOutcome;
...
fn access_time_update_max_buffer_size() -> kuro_error::Result<usize> {
    kuro_env!("BUCK_ACCESS_TIME_UPDATE_MAX_BUFFER_SIZE", type=usize, default=5000)
}
...
pub enum AccessTimesUpdatesError {
    #[error(
        "Invalid value for buckconfig `[kuro] update_access_times`. Got `{0}`. Expected one of `full`, `partial`  or `disabled`."
    )]
...
struct TtlRefreshHistoryEntry {
    at: DateTime<Utc>,
    outcome: Option<kuro_error::Result<()>>,
}
...
impl From<kuro_error::Error> for MaterializeEntryError {
    fn from(e: kuro_error::Error) -> MaterializeEntryError {
        Self::Error(e)
    }
}
...
impl<T: IoHandler + Allocative> Materializer for DeferredMaterializerAccessor<T> {
    ...
    async fn invalidate_many(&self, paths: Vec<ProjectRelativePathBuf>) -> kuro_error::Result<()> {
        ...
        invalidate_fut.await.map_err(kuro_error::Error::from)
    }
    ...
    fn add_snapshot_stats(&self, snapshot: &mut kuro_data::Snapshot) {
...
impl DeferredMaterializerAccessor<DefaultIoHandler> {
    ...
    pub fn new(
        ...
        let materializer_state_info = kuro_data::MaterializerStateInfo {
