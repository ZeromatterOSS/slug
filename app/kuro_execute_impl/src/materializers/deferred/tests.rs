...
mod state_machine {
    ...
    use kuro_execute::execute::blocking::IoRequest;
    use kuro_fs::fs_util;
    use kuro_fs::fs_util::ReadDir;
    ...
    impl IoHandler for StubIoHandler {
        ...
        async fn immediate_write<'a>(
            self: &Arc<Self>,
            _gen: Box<dyn FnOnce() -> kuro_error::Result<Vec<WriteRequest>> + Send + 'a>,
        ...
        fn clean_path<'a>(
            ...
            command_sender: Arc<MaterializerSender<Self>>,
            _cancellations: &'a CancellationContext,
        ) -> BoxFuture<'a, Result<(), kuro_error::Error>> {
        ...
        async fn materialize_entry(
            ...
            if (*self.fail_paths.lock()).contains(&path) || *self.fail.lock() {
                ...
                Err(kuro_error::kuro_error!(
                    kuro_error::ErrorTag::MaterializationError,
                    "Injected error"
                )
                .into())
            } else {
        ...
        fn create_ttl_refresh(
            ...
            _tree: &ArtifactTree,
            _min_ttl: Duration,
        ) -> Option<BoxFuture<'static, kuro_error::Result<()>>> {
            unimplemented!()
        }
    ...
    fn make_processor_for_io(
        ...
        let (daemon_dispatcher_events, daemon_dispatcher_sink) =
            kuro_events::create_source_sink_pair();
    ...
    async fn make_materializer(
        ...
        let command_thread = thread_spawn("kuro-dm", {
            move || {
    ...
    #[tokio::test]
    async fn test_declare_reuse() -> kuro_error::Result<()> {
    ...
    fn make_artifact_value_with_symlink_dep(
        ...
        digest_config: DigestConfig,
    ) -> kuro_error::Result<ArtifactValue> {
    ...
    #[tokio::test]
    async fn test_materialize_symlink_and_target() -> kuro_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
    ...
    #[tokio::test]
    async fn test_materialize_symlink_first_then_target() -> kuro_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
    ...
    #[tokio::test]
    async fn test_subscription_subscribe_also_materializes() -> kuro_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let (mut dm, mut channel) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
            let value = ArtifactValue::file(digest_config.empty_file());
    ...
    #[tokio::test]
    async fn test_invalidate_error() -> kuro_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async{
            let (mut dm, _) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
    ...
    #[tokio::test]
    async fn test_materialize_dep_error() -> kuro_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            // Construct a tree with a symlink and its target, materialize both at once
            let symlink_path = make_path("foo/bar_symlink");
            let target_path = make_path("foo/bar_target");
            ...
            dm.materialize_artifact(&symlink_path, EventDispatcher::null())
                .buck_error_context("Expected a future")?
                .await
                .map_err(|err| kuro_error!(kuro_error::ErrorTag::MaterializationError, "error materializing {:?}", err))?;
            assert_eq!(
                dm.io.take_log(),
                &[
            ...
            dm.materialize_artifact(&symlink_path, EventDispatcher::null())
                .buck_error_context("Expected a future")?
                .await
                .map_err(|err| kuro_error!(kuro_error::ErrorTag::MaterializationError, "error materializing 2 {:?}", err))?;
    ...
    #[tokio::test]
    async fn test_retry() -> kuro_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let (mut dm, mut channel) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
    ...
    #[tokio::test]
    async fn test_clean_stale() -> kuro_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
    ...
    #[tokio::test]
    async fn test_clean_stale_interrupt() -> kuro_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let path = make_path("buck-out/v2/gen/foo/bar");
            let project_root = temp_root();
            let io = Arc::new(StubIoHandler::new(project_root.clone()));
    ...
    #[tokio::test]
    async fn test_clean_stale_schedule() -> kuro_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let path = make_path("buck-out/v2/gen/foo/bar");
            let project_root = temp_root();
            ...
            let receive_clean_result = |events: &mut ChannelEventSource| {
                ...
                match event.unpack_buck().unwrap().data() {
                    kuro_data::buck_event::Data::Instant(instant) => match instant.data.as_ref() {
                        Some(kuro_data::instant_event::Data::CleanStaleResult(res)) => {
                            Some(res.clone())
                        }
                        _ => None,
                    },
            ...
            while i < 5 {
                ...
                if let kuro_data::CleanStaleStats {
    ...
    #[tokio::test]
    async fn test_has_artifact_at() -> kuro_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let (mut dm, _) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
