...
use kuro_data::CleanStaleResultKind;
use kuro_data::CleanStaleStats;
...
use kuro_execute::execute::clean_output_paths::cleanup_path;
use kuro_fs::fs_util;
...
pub struct CleanStaleArtifactsExtensionCommand {
    ...
    pub sender: Sender<BoxFuture<'static, kuro_error::Result<CleanResult>>>,
}
...
enum PendingCleanResult {
    ...
    Pending(BoxFuture<'static, kuro_error::Result<CleanResult>>),
}
...
impl From<CleanResult> for kuro_cli_proto::CleanStaleResponse {
    fn from(result: CleanResult) -> Self {
        let message = match result.kind {
            ...
            CleanStaleResultKind::SkippedDeferWriteDisabled => {
                Some("Skipping clean, set kuro.defer_write_actions to use clean --stale")
            }
            CleanStaleResultKind::SkippedSqliteDisabled => {
                Some("Skipping clean, set kuro.sqlite_materializer_state to use clean --stale")
            }
...
fn create_result(
    result: Result<CleanResult, kuro_error::Error>,
    trace_id: Option<TraceId>,
    daemon_id: &DaemonId,
    total_duration_s: u64,
) -> kuro_data::CleanStaleResult {
...
impl CleanStaleArtifactsCommand {
    pub(super) fn create_clean_fut<T: IoHandler>(
        ...
        trace_id: Option<TraceId>,
        daemon_id: DaemonId,
    ) -> BoxFuture<'static, kuro_error::Result<CleanResult>> {
        ...
        async move {
            ...
            let result: Result<CleanResult, kuro_error::Error> = result.map_err(|e| e.into());
            let result_event: kuro_data::CleanStaleResult = create_result(
                result.clone(),
                trace_id,
                &daemon_id,
    ...
    fn create_pending_clean_result<T: IoHandler>(
        &self,
        processor: &mut DeferredMaterializerCommandProcessor<T>,
    ) -> kuro_error::Result<PendingCleanResult> {
    ...
    fn scan_and_create_clean_fut<T: IoHandler>(
        ...
        for (path, file_type) in found_paths
            ...
            .take(2000)
        {
            self.dispatcher.instant_event(kuro_data::UntrackedFile {
                path: path.to_string(),
...
#[derive(Debug, Clone, kuro_error::Error)]
...
pub(crate) struct CleanStaleError {
    db_size: usize,
    stats: kuro_data::CleanStaleStats,
}

fn stats_for_paths(paths: &Vec<FoundPath>) -> kuro_data::CleanStaleStats {
    let mut stats = kuro_data::CleanStaleStats::default();
    for path in paths {
        match path {
...
fn create_clean_fut<T: IoHandler>(
    ...
    cancellations: &'static CancellationContext,
    liveliness_observer: Arc<dyn LivelinessObserverSync>,
) -> kuro_error::Result<BoxFuture<'static, kuro_error::Result<CleanResult>>> {
    let io = io.dupe();
    ...
    let fut = async move {
        ...
        let res = kuro_util::future::try_join_all(
            found_paths
                .into_iter()
                .filter_map(|x| match x {
...
impl IoRequest for CleanInvalidatedPathRequest {
    fn execute(self: Box<Self>, project_fs: &ProjectRoot) -> kuro_error::Result<()> {
        if !self.liveliness_observer.is_alive_sync() {
            return Err(kuro_error!(ErrorTag::CleanInterrupt, "Interrupt").into());
        }
...
pub fn get_size(path: &AbsNormPath) -> kuro_error::Result<u64> {
    let mut result = 0;
    if path.is_dir() {
