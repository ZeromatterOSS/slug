...
pub(crate) type CleaningFuture = Shared<BoxFuture<'static, kuro_error::Result<()>>>;
    ...
    pub(crate) fn matches_entry(
        ...
        match (&self.0, entry) {
            ...
            (DirectoryEntry::Leaf(l1), DirectoryEntry::Leaf(l2)) => {
                // In Windows, the 'executable bit' absence can cause Kuro to re-download identical artifacts.
...
pub(crate) trait MaterializationMethodToProto {
    fn to_proto(&self) -> kuro_data::MaterializationMethod;
}
    ...
    fn to_proto(&self) -> kuro_data::MaterializationMethod {
        match self {
            ArtifactMaterializationMethod::LocalCopy { .. } => {
                kuro_data::MaterializationMethod::LocalCopy
            }
            ArtifactMaterializationMethod::CasDownload { .. } => {
                kuro_data::MaterializationMethod::CasDownload
            }
            ArtifactMaterializationMethod::Write { .. } => kuro_data::MaterializationMethod::Write,
            ArtifactMaterializationMethod::HttpDownload { .. } => {
                kuro_data::MaterializationMethod::HttpDownload
            }
            #[cfg(test)]
...
impl ArtifactTree {
    ...
    ) -> kuro_error::Result<Vec<(ProjectRelativePathBuf, ProcessingFuture)>> {
        ...
        #[cfg(test)]
        {
            ...
            for path in &invalidated_paths {
                if path.as_str() == "test/invalidate/failure" {
                    return Err(kuro_error!(kuro_error::ErrorTag::Tier0, "Injected error"));
                }
            }
        }
