...
use kuro_util::threads::check_stack_overflow;
...
pub(super) struct DeferredMaterializerCommandProcessor<T: 'static> {
    ...
    ttl_refresh_instance: Option<oneshot::Receiver<(DateTime<Utc>, kuro_error::Result<()>)>>,
...
struct CommandStream<T: 'static> {
    ...
    clean_stale_fut: Option<BoxFuture<'static, kuro_error::Result<CleanResult>>>,
}
...
impl<T: IoHandler> DeferredMaterializerCommandProcessor<T> {
                ...
                Op::CleanStaleRequest => {
                    if let Some(config) = clean_stale_config.as_ref() {
                        ...
                        soft_error!(
                            ...
                            kuro_error!(kuro_error::ErrorTag::Tier0, "clean scheduled without being configured").into(),
                            quiet: true
                        )
                            .unwrap();
                    }
        ...
        match command {
            ...
            MaterializerCommand::Declare(
                ...
                self.maybe_log_command(&event_dispatcher, || {
                    kuro_data::materializer_command::Data::Declare(
                        kuro_data::materializer_command::Declare {
                            path: path.to_string(),
                        },
                    )
                });
            ...
            MaterializerCommand::InvalidateFilePaths(paths, sender, event_dispatcher) => {
                ...
                self.maybe_log_command(&event_dispatcher, || {
                    kuro_data::materializer_command::Data::InvalidateFilePaths(
                        kuro_data::materializer_command::InvalidateFilePaths {
            ...
            MaterializerCommand::Ensure(paths, event_dispatcher, fut_sender) => {
                self.maybe_log_command(&event_dispatcher, || {
                    kuro_data::materializer_command::Data::Ensure(
                        kuro_data::materializer_command::Ensure {
                            paths: paths.iter().map(|p| p.to_string()).collect::<Vec<_>>(),
                        },
                    )
                });
        ...
        self.ttl_refresh_instance = match self.ttl_refresh_instance.take() {
            Some(mut curr) => match curr.try_recv() {
                ...
                Err(TryRecvError::Closed) => {
                    ...
                    self.ttl_refresh_history.push(TtlRefreshHistoryEntry {
                        at: Utc::now(),
                        outcome: Some(Err(kuro_error!(kuro_error::ErrorTag::Tier0, "Shutdown"))),
                    });
                    None
                }
            },
    ...
    fn materialize_artifact_inner(
        ...
        path: &ProjectRelativePath,
        event_dispatcher: EventDispatcher,
    ) -> kuro_error::Result<Option<MaterializingFuture>> {
    ...
    where
        F: FnOnce() -> kuro_data::materializer_command::Data,
    {
        if self.verbose_materializer_log {
            let data = Some(f());
            event_dispatcher.instant_event(kuro_data::MaterializerCommand { data });
        }
    }
}
...
fn clean_path<T: IoHandler>(
    ...
    .map(|r| match r {
        Ok(r) => r,
        Err(e) => Err(e.into()), // Turn the JoinError into a kuro_error::Error.
    })
    .boxed()
    .shared()
}
...
struct ExistingFutures(kuro_error::Result<Vec<(ProjectRelativePathBuf, ProcessingFuture)>>);
...
impl ExistingFutures {
    ...
    fn into_result(self) -> kuro_error::Result<Vec<(ProjectRelativePathBuf, ProcessingFuture)>> {
        self.0
    }
