...
use kuro_execute::digest::CasDigestFromReExt;
...
use kuro_execute::materialize::http::http_download;
...
impl IoHandler for DefaultIoHandler {
    ...
    fn clean_path<'a>(
        ...
        self.io_executor
            ...
            .map(|r| r.map_err(kuro_error::Error::from))
            .boxed()
    }
    ...
    async fn materialize_entry(
        ...
        let materialization_start = kuro_data::MaterializationStart {
...
fn maybe_tombstone_digest(digest: &FileDigest) -> kuro_error::Result<&FileDigest> {
    ...
    fn convert_digests(val: &str) -> kuro_error::Result<HashSet<FileDigest>> {
        val.split(' ')
            .map(|digest| {
                ...
                kuro_error::Ok(digest)
            })
            .collect()
    }

    let tombstoned_digests = kuro_env!(
...
pub(super) fn create_ttl_refresh(
    ...
    min_ttl: Duration,
    digest_config: DigestConfig,
) -> Option<impl Future<Output = kuro_error::Result<()>> + use<>> {
        ...
        for (use_case, digests_to_refresh) in digests_to_refresh {
            ...
            for chunk in digests_to_refresh.as_slice().chunks(REFRESH_CHUNK_SIZE) {
                ...
                let mut digests_expires = digests_expires.into_try_map(|(digest, expires)| {
                    kuro_error::Ok((FileDigest::from_re(&digest, digest_config)?, expires))
                })?;
...
impl WriteIoRequest {
    fn execute_inner(&self, project_fs: &ProjectRoot) -> kuro_error::Result<()> {
...
impl IoRequest for CleanIoRequest {
    fn execute(self: Box<Self>, project_fs: &ProjectRoot) -> kuro_error::Result<()> {
        // NOTE: No spans here! We should perhaps add one, but this needs to be considered
        // carefully as it's a lot of spans, and we haven't historically emitted those for writes.
        let res = cleanup_path(project_fs, &self.path).map_err(kuro_error::Error::from);
