...
use kuro_execute::directory::re_tree_to_directory;
use kuro_execute::execute::action_digest::TrackedActionDigest;
...
use kuro_util::time_span::TimeSpanBuilder;
        ...
        e => {
            let materialized_inputs = if materialize_failed_re_action_inputs {
                executor_stage_async(
                    kuro_data::ReStage {
                        stage: Some(kuro_data::MaterializeFailedInputs {}.into()),
                    },
                    async move {
...
impl CasDownloader<'_> {
    ...
    async fn extract_artifacts<'a>(
        ...
        requested_outputs: impl IntoIterator<Item = CommandExecutionOutputRef<'a>>,
        output_spec: &dyn RemoteActionResult,
    ) -> kuro_error::Result<ExtractedArtifacts> {
        let now = Utc::now();
    ...
    ) -> kuro_error::Result<IndexMap<CommandExecutionOutput, ArtifactValue>> {
...
fn re_forward_path(re_path: &str) -> kuro_error::Result<&ForwardRelativePath> {
