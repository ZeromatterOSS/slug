...
pub enum IsMountReadyError {
    ...
    #[kuro(tag = IoEdenMountDoesNotExist)]
    MountDoesNotExist { mount: AbsPathBuf },
    #[error(transparent)]
    #[kuro(tag = IoEdenRequestError)]
...
pub enum ConnectAndRequestError<E> {
    #[error(transparent)]
    #[kuro(tag = IoEdenConnectionError)]
    ConnectionError(kuro_error::Error),
