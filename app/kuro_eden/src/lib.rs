...
pub mod semaphore {
    ...
    /// A kuro-specific semaphore that is used to limit the number of outstanding requests to
    /// EdenFS. Reads kuro specific environment variable "BUCK2_EDEN_SEMAPHORE" to determine the
    /// number of permits.
    pub fn kuro_default() -> Semaphore {
        let permits =
            kuro_env!("BUCK2_EDEN_SEMAPHORE", type=usize, default=DEFAULT_MAX_OUTSTANDING_REQUESTS, applicability=internal)
