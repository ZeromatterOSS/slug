...
use kuro_common::io::fs::ReadUncheckedOptions;
use kuro_core;
...
use crate::semaphore::kuro_default;
...
impl EdenIoProvider {
    pub async fn new(
        ...
        let eden_semaphore = kuro_default();
        ...
        if let Some(eden_version) = &eden_version {
            if eden_version.as_str() < min_eden_version {
                tracing::warn!(
                    ...
                    Update Eden then restart Kuro to use Eden I/O.",
                    eden_version,
                    min_eden_version
                );
                return Ok(None);
    ...
    ) -> kuro_error::Result<PathMetadataResult> {
    ...
    async fn read_file_if_exists_impl(
        ...
        match res {
            Ok(res) => match res.blob {
                ...
                ScmBlobOrError::UnknownField(code) => Err(kuro_error::kuro_error!(
                    kuro_error::ErrorTag::IoEdenUnknownField,
...
impl IoProvider for EdenIoProvider {
    ...
    async fn read_file_if_exists_impl(
        ...
        if kuro_env!("BUCK2_ENABLE_EDEN_THRIFT_READ", bool).unwrap_or(false)
