...
impl EdenConnectionManager {
    ...
    fn get_eden_connector(
    ...
    ) -> kuro_error::Result<EdenConnector> {
        ...
        if cfg!(windows) {
            ...
            let config: EdenConfig = toml::from_str(&config_contents)
                .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::IoEdenConfigError))?;
    ...
    pub async fn get_eden_version(&self) -> kuro_error::Result<Option<String>> {
        let fb303 = self.connector.connect_fb303()?;
        let values = fb303
            .getRegexExportedValues("^build_.*")
            .await
            .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::IoEdenVersionError))?;
                ...
                Ok(res) => {
                    ...
                    if retries > 0 {
                        soft_error!(
                            ...
                            kuro_error!(kuro_error::ErrorTag::Input, "Eden IO retried {} times", retries),
                            quiet: true
                        ).ok();
                    }
                    break Ok(res);
...
type EdenClientFuture =
    Shared<BoxFuture<'static, kuro_error::Result<Arc<dyn EdenService + Send + Sync>>>>;
...
fn thrift_builder(
    fb: FacebookInit,
    socket: &AbsPathBuf,
) -> kuro_error::Result<::thriftclient::ThriftChannelBuilder> {
    ...
    Ok(
        ::thriftclient::ThriftChannelBuilder::from_path(fb, socket.as_path())
            .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::IoEdenThriftError))?
...
impl EdenConnector {
    ...
    fn connect_fb303(&self) -> kuro_error::Result<Arc<dyn BaseService + Send + Sync>> {
