...
use kuro_cmd_audit_client::includes::AuditIncludesCommand;
    ...
    async fn server_execute(
        ...
        Ok(server_ctx
            .with_dice_ctx(|server_ctx, mut ctx| async move {
                ...
                let results: Vec<(_, kuro_error::Result<Vec<_>>)> = futures.collect().await;
                // This is expected to not return any errors, and so we're not careful about not propagating it.
                let to_absolute_path = move |include: ImportPath| -> kuro_error::Result<_> {
                ...
                let absolutize_paths =
                    |paths: Vec<ImportPath>| -> kuro_error::Result<Vec<AbsNormPathBuf>> {
                        paths.into_try_map(&to_absolute_path)
                    };
                let results: Vec<(String, kuro_error::Result<Vec<AbsNormPathBuf>>)> = results
