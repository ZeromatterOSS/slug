...
use kuro_cmd_audit_client::config::OutputFormat;
use kuro_cmd_audit_client::config::ValueStyle;
use kuro_common::dice::cells::HasCellResolver;
use kuro_common::legacy_configs::configs::LegacyBuckConfig;
use kuro_common::legacy_configs::configs::LegacyBuckConfigLocation;
use kuro_common::legacy_configs::configs::LegacyBuckConfigValue;
...
impl<'a> Match<'a> {
    fn parse(resolver: &CellAliasResolver, spec: &'a str) -> kuro_error::Result<Self> {
...
impl<'a> Matches<'a> {
    fn parse(resolver: &CellAliasResolver, specs: &'a [String]) -> kuro_error::Result<Self> {
        Ok(Self {
...
trait CellConfigRenderer {
    fn render_cell_header(&mut self, cell: CellName) -> kuro_error::Result<()>;
    fn render_section_header(&mut self, section: &str) -> kuro_error::Result<()>;
...
impl CellConfigRenderer for JsonCellConfigRenderer<'_> {
    fn render_cell_header(&mut self, _cell: CellName) -> kuro_error::Result<()> {
        Ok(())
    }

    fn render_section_header(&mut self, _section: &str) -> kuro_error::Result<()> {
        Ok(())
    }
