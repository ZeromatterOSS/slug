...
pub enum Tier {
    ...
    Environment,
    // Unexpected errors in kuro or core dependencies.
...
impl ErrorLike for kuro_data::ErrorReport {
    fn best_tag(&self) -> Option<ErrorTag> {
    ...
    fn error_rank(self: &kuro_data::ErrorReport) -> u32 {
...
pub fn best_error<'a>(
    tags: impl IntoIterator<Item = &'a kuro_data::ErrorReport>,
) -> Option<&'a kuro_data::ErrorReport> {
...
// Kuro is the fallback/default source area, use the first non-kuro source area.
pub fn source_area(tags: impl IntoIterator<Item = ErrorTag>) -> ErrorSourceArea {
    tags.into_iter()
        .find_map(|tag| {
            let area = tag.source_area();
            if area != ErrorSourceArea::Kuro {
                Some(area)
            } else {
                None
            }
        })
        .unwrap_or(ErrorSourceArea::Kuro)
}
