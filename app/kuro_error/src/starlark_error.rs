...
//! Starlark::Error <-> kuro_error::Error conversion implementation.
...
fn error_with_starlark_context(
    ...
    let mut kuro_error = e.clone();

    loop {
        match kuro_error.0.as_ref() {
            ErrorKind::Root(root) => {
            ...
            ErrorKind::WithContext(context_value, inner) => {
                match context_value {
                    ...
                    ContextValue::StarlarkError(_) => {
                        return kuro_error.context_for_starlark_backtrace(starlark_context);
                    }
...
mod tests {
    ...
    fn test_roundtrip_starlark() {
        // Tests kuro_error->starlark->kuro_error
    ...
    fn test_pop_base_error_from_context() {
        ...
        let e = kuro_error!(crate::ErrorTag::StarlarkError, "{}", base_error);
