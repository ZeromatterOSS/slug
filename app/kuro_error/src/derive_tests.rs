...
#[derive(kuro_error_derive::Error, Debug)]
#[error("foo")]
#[kuro(input)]
pub struct Error1;
...
fn test_source_location_no_attrs() {
    ...
    assert_eq!(
        ...
        "kuro_error/src/derive_tests.rs::NoAttrsStruct"
    );
    let e: crate::Error = NoAttrsEnum::Variant.into();
    assert_eq!(
        e.source_location().to_string(),
        "kuro_error/src/derive_tests.rs::NoAttrsEnum::Variant"
    );
}
...
fn test_enum_with_type_option() {
    ...
    assert_eq!(
        e.source_location().to_string(),
        "kuro_error/src/derive_tests.rs::EnumWithTypeOption::Variant",
    );
}
...
fn test_error_tags_vec_fn() {
    ...
    #[derive(kuro_error_derive::Error, Debug)]
    #[error("Unused")]
    #[kuro(tag = WatchmanTimeout, tags = calc_tags(*extra_tag))]
    struct TaggedError {
        extra_tag: bool,
    }
...
#[test]
fn test_recovery_through_transparent_kuro_error() {
    #[derive(kuro_error_derive::Error, Debug)]
    #[error("base_display")]
    #[kuro(tag = Environment)]
    struct BaseError;
