...
/// entirely replaced it in the Kuro codebase. It has `From` impls from many common error types.
...
impl Error {
    ...
    pub fn new(
        ...
        let kuro_error = crate::Error(Arc::new(ErrorKind::Root(Box::new(error_root))));
        kuro_error.tag([error_tag])
    }
    ...
    pub fn action_error(&self) -> Option<&kuro_data::ActionError> {
