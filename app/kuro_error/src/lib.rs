...
/// you can call [`.tag()`](`crate::BuckErrorContext::tag`) on a [`kuro_error::Result`][`Result`].
///
...
pub use kuro_data::error::ErrorTag;
...
/// `thiserror` in `kuro/app` will be replaced with this macro.
///
/// Currently, the only distinction from `thiserror::Error` is that an additional impl of
/// `AnyError` is generated for the type, which makes some of the interactions with `kuro_error` more
...
/// #[error("My error type")]
/// #[kuro(tag = kuro_error::ErrorTag::Input)]
/// struct MyError;
///
/// let e = kuro_error::Error::from(MyError);
/// assert_eq!(&format!("{}", e), "My error type");
/// ```
#[doc(inline)]
pub use kuro_error_derive::Error;
