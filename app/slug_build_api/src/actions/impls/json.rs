...
use slug_interpreter::types::cell_path::StarlarkCellPath;
...
struct SlugErrorResultOfSerializedValue<'a, 'v> {
    result: slug_error::Result<SerializeValue<'a, 'v>>,
}

impl<'a, 'v> Serialize for SlugErrorResultOfSerializedValue<'a, 'v> {
...
impl<'a, 'v> SerializeValue<'a, 'v> {
    fn with_value(&self, x: Value<'v>) -> SlugErrorResultOfSerializedValue<'a, 'v> {
        SlugErrorResultOfSerializedValue {
...
fn err<R, E: serde::ser::Error>(res: slug_error::Result<R>) -> Result<R, E> {
    match res {
        Ok(v) => Ok(v),
        Err(e) => Err(serde::ser::Error::custom(format!("{e:#}"))),
    }
...
impl<'v> JsonArtifact<'v> {
    fn artifact(&self) -> slug_error::Result<Artifact> {
        match self {
...
pub fn validate_json(x: JsonUnpack) -> slug_error::Result<()> {
