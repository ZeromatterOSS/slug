...
use slug_event_observer::display::display_action_error;
...
pub struct ActionError {
    ...
    name: slug_data::ActionName,
    key: slug_data::ActionKey,
    last_command: Option<slug_data::CommandExecution>,
    error_diagnostics: Option<slug_data::ActionErrorDiagnostics>,
}

impl From<ActionError> for slug_error::Error {
    fn from(this: ActionError) -> slug_error::Error {
            ...
            ExecuteError::CommandExecutionError { error, .. } => {
                ...
                if is_command_failure {
                    if let Some(diagnostic) = &this.error_diagnostics {
                        if let Some(slug_data::action_error_diagnostics::Data::SubErrors(
                            sub_errors,
...
impl ActionError {
    ...
    pub(crate) fn as_proto_field(&self) -> slug_data::action_execution_end::Error {
        match &self.execute_error {
            ExecuteError::MissingOutputs { declared } => slug_data::CommandOutputsMissing {
                message: format!("Action failed to produce outputs: {}", error_items(declared)),
            }
            .into(),
            ExecuteError::MismatchedOutputs { declared, real } => slug_data::CommandOutputsMissing {
                message: format!(
            ...
            .into(),
            ExecuteError::WrongOutputType {path, declared, real} => slug_data::CommandOutputsMissing {
                message: format!(
            ...
            ExecuteError::CommandExecutionError { .. } => slug_data::CommandExecutionError {}.into(),
        }
    }

    pub(crate) fn as_proto_event(&self) -> slug_data::ActionError {
        let field = match self.as_proto_field() {
            slug_data::action_execution_end::Error::Unknown(e) => e.into(),
            slug_data::action_execution_end::Error::MissingOutputs(e) => e.into(),
            slug_data::action_execution_end::Error::CommandExecutionError(e) => e.into(),
        };
        slug_data::ActionError {
            error: Some(field),
...
mod tests {
    ...
    fn test_error_conversion() {
        let error = slug_error!(ErrorTag::Http, "error");
        ...
        let action_error = ActionError::new(
            ...
            slug_data::ActionKey {
                id: vec![],
                key: "key".to_owned(),
                owner: Some(slug_data::action_key::Owner::TargetLabel(
                    slug_data::ConfiguredTargetLabel {
                        label: Some(slug_data::TargetLabel {
                            package: "package".to_owned(),
                            name: "name".to_owned(),
                        }),
                        configuration: Some(slug_data::Configuration {
                            full_name: "conf".into(),
                        }),
        ...
        let slug_error = slug_error::Error::from(action_error);
