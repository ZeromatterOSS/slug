...
use slug_data::ActionErrorDiagnostics;
use slug_data::ActionSubErrors;
use slug_data::ToProtoMessage;
use slug_data::get_action_digest;
use slug_error::BuckErrorContext;
use slug_event_observer::action_util::get_execution_time_ms;
...
async fn build_action_impl(
    ...
    key: &ActionKey,
) -> slug_error::Result<ActionOutputs> {
...
async fn build_action_no_redirect(
    ...
    // when `SLUG_LOG_ADMISSION` is unset (inner fn short-circuits via
    // `OnceLock` load).
    let ready_admission = crate::actions::admission_log::now_if_enabled();

    let start_event = slug_data::ActionExecutionStart {
        key: Some(action.key().as_proto()),
        kind: action.kind().into(),
        name: Some(slug_data::ActionName {
...
async fn build_action_inner(
...
) -> (ActionExecutionData, Box<slug_data::ActionExecutionEnd>) {
    ...
    // measures slug's build-api admission latency.
    ...
    let is_expected_eligible_for_dedupe = match action.is_expected_eligible_for_dedupe() {
        Some(v) => {
            if v {
                slug_data::ExpectedEligibleForDedupe::ExpectedEligible
            } else {
                slug_data::ExpectedEligibleForDedupe::ExpectedIneligible
            }
        }
        None => slug_data::ExpectedEligibleForDedupe::UnknownEligibility,
    };
    ...
    let action_name = slug_data::ActionName {
        category: action.category().as_str().to_owned(),
        identifier: action.identifier().unwrap_or("").to_owned(),
        progress_message: action.progress_message().unwrap_or("").to_owned(),
    };
    ...
    let mut slug_revision = None;
    let mut slug_build_time = None;
    ...
    let error_diagnostics = match execute_result {
        ...
        Err(e) => {
            ...
            // so as to reduce Scribe traffic and log it in slug_action_errors
            slug_revision = slug_build_info::revision().map(|s| s.to_owned());
            slug_build_time = slug_build_info::time_iso8601().map(|s| s.to_owned());
            hostname = slug_events::metadata::hostname();
            ...
            action_result = Err(slug_error::Error::from(e)
    ...
    let invalidation_info = if executor.invalidation_tracking_enabled() {
        fn to_proto(
        ...
        ) -> Option<slug_data::command_invalidation_info::InvalidationSource> {
            match invalidation_path {
                ...
                dice::DiceTrackedInvalidationPath::Invalidated(_) => {
                    Some(slug_data::command_invalidation_info::InvalidationSource {})
                }
            }
        }
        let invalidation_paths = ctx.get_invalidation_paths();
        Some(slug_data::CommandInvalidationInfo {
    ...
    let execution_kind = execution_kind.unwrap_or(slug_data::ActionExecutionKind::NotSet);
    ...
    (
        ...
        Box::new(slug_data::ActionExecutionEnd {
            key: Some(action_key),
            kind: action.kind().into(),
            ...
            local_thread_id: slug_util::threads::thread_index(),
...
fn is_action_eligible_for_dedupe(
...
) -> slug_data::EligibleForDedupe {
    if !action.all_outputs_are_content_based() {
        return slug_data::EligibleForDedupe::IneligibleOutput;
    }
    ...
    for (ag, _agv) in inputs.iter() {
        if !ag.is_eligible_for_dedupe(target_platform) {
            return slug_data::EligibleForDedupe::IneligibleInput;
        }
    }

    slug_data::EligibleForDedupe::Eligible
}
...
fn try_run_error_handler(
    action: Arc<RegisteredAction>,
    last_command: Option<&slug_data::CommandExecution>,
    artifact_fs: slug_error::Result<ArtifactFs>,
    outputs: Option<&ActionOutputs>,
) -> Option<ActionErrorDiagnostics> {
    use slug_data::action_error_diagnostics::Data;

    fn create_error(
        e: slug_error::Error,
    ) -> (
        Option<ActionErrorDiagnostics>,
        slug_data::ActionErrorHandlerExecutionEnd,
    ) {
        (
    ...
    match action.action.error_handler() {
        Some(error_handler) => {
            ...
            dispatcher
                .clone()
                .span(slug_data::ActionErrorHandlerExecutionStart {}, || {
                    let env = Module::new();
                    let heap = env.heap();
                    ...
                    let data = match error_handler_result {
                        ...
                        Err(e) => {
                            let e = slug_error::Error::from(e).context("Error handler failed");
...
pub struct ActionExtraData {
    pub execution_kind: slug_data::ActionExecutionKind,
    pub execution_time_ms: Option<u64>,
    pub output_size: u64,
    pub target_rule_type_name: Option<String>,
    pub action_digest: Option<String>,
    pub invalidation_info: Option<slug_data::CommandInvalidationInfo>,
}

struct ActionExecutionData {
    action_result: slug_error::Result<ActionOutputs>,
...
impl ActionCalculation {
    pub async fn get_action(
    ...
    ) -> slug_error::Result<Arc<RegisteredAction>> {
        ...
        match deferred_holder.lookup_action(action_key)? {
            ...
            ActionLookup::Deferred(action_key) => {
                ...
                ) -> BoxFuture<'a, slug_error::Result<Arc<RegisteredAction>>> {
    ...
    pub fn build_action<'a>(
        ctx: &'a mut DiceComputations<'_>,
        action_key: &ActionKey,
    ) -> impl Future<Output = slug_error::Result<ActionOutputs>> + use<'a> {
...
impl Key for BuildKey {
    type Value = slug_error::Result<ActionOutputs>;
...
async fn command_execution_report_to_proto(
    ...
    let status = match &report.status {
        CommandExecutionStatus::Success { .. } => slug_data::command_execution::Success {}.into(),
        CommandExecutionStatus::Cancelled { .. } => {
            slug_data::command_execution::Cancelled {}.into()
        }
        CommandExecutionStatus::Failure { .. } => slug_data::command_execution::Failure {}.into(),
        CommandExecutionStatus::WorkerFailure { .. } => {
            slug_data::command_execution::WorkerFailure {}.into()
        }
        CommandExecutionStatus::TimedOut { duration, .. } => {
            slug_data::command_execution::Timeout {
                duration: (*duration).try_into().ok(),
            }
            .into()
        }
        CommandExecutionStatus::Error { stage, error, .. } => slug_data::command_execution::Error {
...
pub async fn command_details(
    ...
    allow_omit_details: bool,
) -> slug_data::CommandExecutionDetails {
