...
impl AnalysisData {
    pub fn providers(&self) -> slug_error::Result<FrozenProviderCollectionValue> {
...
impl ActionQueryNodeRef {
    pub fn require_action(&self) -> slug_error::Result<&ActionKey> {
        match self {
            Self::Analysis(a) => Err(slug_error::slug_error!(
...
impl QueryTarget for ActionQueryNode {
    ...
    fn buildfile_path(&self) -> &BuildFilePath {
        // TODO(cjhopman): In addition to implementing this, we should be able to return an slug_error::Error here rather than panicking.
...
pub static FIND_MATCHING_ACTION: LateBinding<
    for<'c> fn(
        ...
        Box<dyn Future<Output = slug_error::Result<Option<ActionQueryNode>>> + Send + 'c>,
    >,
> = LateBinding::new("FIND_MATCHING_ACTION");

/// Hook to link printer in `slug_server_commands` from `slug_cmd_audit_server`.
...
/// Use of "configured_attr_to_value" in `slug_transition` from `slug_analysis`.
