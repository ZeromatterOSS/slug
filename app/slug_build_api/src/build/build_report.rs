...
use slug_artifact::artifact::artifact_dump::FileInfo;
use slug_artifact::artifact::artifact_dump::SymlinkInfo;
...
use slug_core::pattern::pattern::TargetLabelWithModifiers;
...
use slug_error::UniqueRootId;
use slug_error::classify::ErrorLike;
use slug_error::classify::Tier;
...
pub struct BuildReportCollector<'a> {
    ...
    error_cause_cache: HashMap<slug_error::UniqueRootId, usize>,
...
#[derive(slug_error::Error)]
#[error(transparent)]
#[slug(tag = BuildReport)]
pub struct BuildReportGenerationError(slug_error::Error);

impl From<slug_error::Error> for BuildReportGenerationError {
...
impl<'a> BuildReportCollector<'a> {
    ...
    fn assemble_build_report(
        ...
        // Determine error category using existing Slug error classification
        ...
        Ok(BuildReport {
            ...
            // Setting this to false since we don't currently truncate slug's build report.
            truncated: false,
    ...
    ) -> slug_error::Result<BuildReportEntry> {
    ...
    fn collect_results_for_configured<'b>(
        ...
        metrics: &mut HashMap<&'b ConfiguredProvidersLabel, Arc<TargetBuildMetrics>>,
        all_error_reports: &mut Vec<ErrorReport>,
    ) -> slug_error::Result<ConfiguredBuildReportEntry> {
    ...
    fn collect_per_configuration_data(
        &mut self,
    ) -> slug_error::Result<Option<HashMap<String, PerConfigurationEntry>>> {
        ...
        let per_configuration_data = total_per_configuration_sketcher
            ...
            .collect::<slug_error::Result<HashMap<String, PerConfigurationEntry>>>()?;
...
pub async fn build_report_opts<'a>(
    ...
    graph_properties_opts: GraphPropertiesOptions,
) -> slug_error::Result<BuildReportOpts> {
...
) -> Result<Option<String>, slug_error::Error> {
