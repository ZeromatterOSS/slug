...
impl ToProtoMessage for DetailedAggregatedMetrics {
    type Message = slug_data::DetailedAggregatedMetrics;

    fn as_proto(&self) -> Self::Message {
        slug_data::DetailedAggregatedMetrics {
...
impl ToProtoMessage for AggregatedBuildMetrics {
    type Message = slug_data::AggregatedBuildMetrics;

    fn as_proto(&self) -> Self::Message {
        slug_data::AggregatedBuildMetrics {
...
impl ToProtoMessage for TopLevelTargetAggregatedData {
    type Message = slug_data::TopLevelTargetMetrics;

    fn as_proto(&self) -> Self::Message {
        slug_data::TopLevelTargetMetrics {
...
impl AggregatedBuildMetrics {
    fn aggregate_execution(&mut self, factor: f64, ev: &ActionExecutionMetrics, when: BuiltWhen) {
        ...
        if let BuiltWhen::ThisBuild = when {
            ...
            match ev.execution_kind {
                slug_data::ActionExecutionKind::Local
                ...
                slug_data::ActionExecutionKind::Remote
                | slug_data::ActionExecutionKind::RemoteWorker => {
                    self.remote_execution_time_ms += factor * (ev.execution_time_ms as f64);
                    self.remote_executions += factor;
                }
                slug_data::ActionExecutionKind::ActionCache
                | slug_data::ActionExecutionKind::RemoteDepFileCache => {
                    self.remote_cache_hits += factor;
                }
                slug_data::ActionExecutionKind::NotSet
                | slug_data::ActionExecutionKind::Simple
                | slug_data::ActionExecutionKind::Deferred
                | slug_data::ActionExecutionKind::LocalDepFile
...
impl ToProtoMessage for AllTargetsAggregatedData {
    type Message = slug_data::AllTargetsBuildMetrics;

    fn as_proto(&self) -> Self::Message {
        slug_data::AllTargetsBuildMetrics {
            action_graph_size: self.action_graph_size,
            metrics: Some(self.metrics.as_proto()),
