...
use slug_data::ComputeDetailedAggregatedMetricsEnd;
use slug_data::ComputeDetailedAggregatedMetricsStart;
...
pub trait HasDetailedAggregatedMetrics {
    fn action_executed(&self, ev: ActionExecutionMetrics) -> slug_error::Result<()>;
    fn analysis_started(&self, key: &DeferredHolderKey) -> slug_error::Result<()>;
    fn analysis_complete(
        &self,
        key: &DeferredHolderKey,
        result: &DeferredHolder,
    ) -> slug_error::Result<()>;
    fn top_level_target(&self, spec: TopLevelTargetSpec) -> slug_error::Result<()>;
    fn take_per_build_events(&self) -> slug_error::Result<PerBuildEvents>;
    fn compute_detailed_metrics(
        &self,
        events: PerBuildEvents,
    ) -> impl Future<Output = slug_error::Result<DetailedAggregatedMetrics>> + Send;
}
...
impl HasDetailedAggregatedMetrics for DiceComputations<'_> {
    ...
    async fn compute_detailed_metrics(
        &self,
        events: PerBuildEvents,
    ) -> slug_error::Result<DetailedAggregatedMetrics> {
        span_async_simple(
...
) -> slug_error::Result<&'a Option<DetailedAggregatedMetricsEventHandler>> {
    ctx.global_data()
...
fn get_per_build_events_holder<'a>(
    ctx: &'a DiceComputations<'_>,
) -> slug_error::Result<&'a DetailedAggregatedMetricsPerBuildEventsHolder> {
