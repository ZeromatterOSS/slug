...
impl<'a> Graph<'a> {
    ...
    fn lookup_node(&self, key: Key) -> slug_error::Result<Node> {
        match key {
    ...
    fn lookup_artifact(&self, artifact: &ArtifactGroup) -> slug_error::Result<Option<Key>> {
        match artifact {
    ...
    fn visit_deps(&self, node: &Node, mut visit_dep: impl FnMut(Key)) -> slug_error::Result<()> {
        match node {
            Node::Action(action) => {
            ...
            Node::TransitiveSetProjection(proj_node) => {
                ...
                struct Visitor<'a, 'b, F: FnMut(Key)>(F, &'a Graph<'b>, slug_error::Result<()>);
...
) -> slug_error::Result<(bool, fxhash::FxHashSet<ActionKey>)> {
