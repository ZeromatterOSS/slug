...
use slug_directory::directory::directory_data::DirectoryData;
...
pub trait ArtifactGroupCalculation {
    ...
    async fn ensure_artifact_group(
        &mut self,
        input: &ArtifactGroup,
    ) -> slug_error::Result<ArtifactGroupValues>;
}
...
) -> impl Future<Output = slug_error::Result<EnsureArtifactGroupReady>> + use<'a, 'd> {
    async move {
        match input {
...
) -> impl Future<Output = slug_error::Result<EnsureArtifactGroupReady>> + use<'a> {
    match artifact {
...
impl EnsureArtifactGroupReady {
    ...
    fn unpack_single(self) -> slug_error::Result<ArtifactValue> {
        match self {
...
fn ensure_target_default_outputs_staged<'a, 'd>(
    ctx: &'a mut DiceComputations<'d>,
    label: &'a slug_core::target::configured_target_label::ConfiguredTargetLabel,
) -> impl Future<Output = slug_error::Result<EnsureArtifactGroupReady>> + use<'a, 'd> {
    async move {
...
async fn dir_artifact_value(
    ...
    // `PathMetadataKey` of every file inside that directory, blowing up Slug's memory use.
    ...
    impl Key for DirArtifactValueKey {
        type Value = slug_error::Result<ArtifactValue>;
            ...
            let entry_values = ctx
                .try_compute_join(files.iter(), |ctx, x| {
                    async move {
                        ...
                        .await?;
                        slug_error::Ok((x.file_name.clone(), value))
                    }
                    .boxed()
                })
                .await?;
...
async fn path_artifact_value(
    ...
    match raw {
        RawPathMetadata::Symlink {
            ...
            let target_path = match slug_fs::paths::abs_path::AbsPath::new(&abs_target) {
                Ok(p) => p.to_owned(),
                Err(_) => {
            ...
            let digest_config = ctx.global_data().get_digest_config();
            let file_digest_config = slug_common::file_ops::metadata::FileDigestConfig::source(
                digest_config.cas_digest_config(),
            );
            match slug_common::file_ops::metadata::FileDigest::from_file(
                &target_path,
                file_digest_config,
            ) {
                Ok(digest) => {
                    ...
                    let is_executable = metadata
                        ...
                        .unwrap_or(false);
                    let tracked = slug_common::cas_digest::TrackedCasDigest::new(
                        digest,
                        digest_config.cas_digest_config(),
                    );
                    Ok(ArtifactValue::new(
                        ActionDirectoryEntry::Leaf(ActionDirectoryMember::File(
                            slug_common::file_ops::metadata::FileMetadata {
                                digest: tracked,
                                is_executable,
                            },
                        )),
...
enum ProjectedArtifactError {
    ...
    #[slug(tag = slug_error::ErrorTag::ProjectMissingPath)]
...
impl Key for EnsureTransitiveSetProjectionKey {
    type Value = slug_error::Result<ArtifactGroupValues>;
