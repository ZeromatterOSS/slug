...
fn create_callable_function_signature(
    ...
    schemaless: bool,
    ret_ty: Ty,
) -> slug_error::Result<(ParametersSpec<FrozenValue>, TyCallable)> {
...
impl<'v> StarlarkValue<'v> for UserProviderCallable {
    ...
    fn export_as(
        ...
        self.callable.get_or_try_init(|| {
            ...
            slug_error::Ok(UserProviderCallableNamed {
                id: provider_id.dupe(),
                signature,
    ...
    fn invoke(
        ...
        match self.callable.get() {
            ...
            None => Err(slug_error::Error::from(ProviderCallableError::NotBound).into()),
        }
    }
...
fn provider_field_parse_type<'v>(
    ty: Value<'v>,
    eval: &mut Evaluator<'v, '_, '_>,
) -> slug_error::Result<TypeCompiled<FrozenValue>> {
