...
/// Slug; the fallback is kept defensively for edge cases encountered by
...
enum ProviderCollectionError {
    ...
    #[error(
        "Cannot handle flavor `{flavor}` on target `{target}`. Most flavors are unsupported in Slug."
    )]
...
impl<'v, V: ValueLike<'v>> ProviderCollectionGen<V> {
    ...
    fn try_from_value_impl(
        mut value: Value<'v>,
    ) -> slug_error::Result<SmallMap<Arc<ProviderId>, Value<'v>>> {
    ...
    pub fn try_from_value(value: Value<'v>) -> slug_error::Result<ProviderCollection<'v>> {
    ...
    pub fn try_from_aspect_value(value: Value<'v>) -> slug_error::Result<ProviderCollection<'v>> {
        let providers = Self::try_from_value_impl(value)?;
    ...
    fn get_impl(
        ...
        index: Value<'v>,
        op: GetOp,
    ) -> slug_error::Result<Either<Value<'v>, Arc<ProviderId>>> {
    ...
    pub(crate) fn get(
        &self,
        index: Value<'v>,
    ) -> slug_error::Result<NoneOr<ValueOfUnchecked<'v, AbstractProvider>>> {
    ...
    fn at(&self, index: Value<'v>, _heap: Heap<'v>) -> starlark::Result<Value<'v>> {
        match self.get_impl(index, GetOp::At)? {
            ...
            Either::Right(provider_id) => {
                Err(slug_error::Error::from(ProviderCollectionError::AtNotFound(
...
impl FrozenProviderCollection {
    ...
    pub fn default_info<'a>(&'a self) -> slug_error::Result<FrozenRef<'a, FrozenDefaultInfo>> {
...
impl FrozenProviderCollectionValue {
    ...
    pub fn try_from_value(value: OwnedFrozenValue) -> slug_error::Result<Self> {
        Ok(Self {
    ...
    ) -> slug_error::Result<FrozenProviderCollectionValueRef<'f>> {
...
impl<'f> FrozenProviderCollectionValueRef<'f> {
    ...
    pub fn lookup_inner(
        ...
        match label.name() {
            ProvidersName::Default => slug_error::Ok(self),
            ...
            let collection = frozen
                ...
                .ok_or_else(|| {
                    slug_error::slug_error!(
                        slug_error::ErrorTag::StarlarkError,
