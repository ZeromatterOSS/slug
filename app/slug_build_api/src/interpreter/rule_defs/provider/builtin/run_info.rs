...
/// provides `RunInfo`, it can be executed with `slug run <target>`. The provider wraps the
...
/// ```bash
/// slug run <target>                    # Run with the args specified in RunInfo
/// slug run <target> -- [extra args]    # Pass additional args after '--' to the command
/// ```
/// ## Examples
///
