...
impl FrozenExternalRunnerTestInfo {
    ...
    pub fn use_project_relative_paths(&self) -> bool {
        NoneOr::<bool>::unpack_value(self.use_project_relative_paths.get().to_value())
            ...
            .unwrap()
            .into_option()
            .unwrap_or_else(slug_core::is_open_source)
    }
...
fn iter_value<'v>(value: Value<'v>) -> slug_error::Result<impl Iterator<Item = Value<'v>> + 'v> {
...
fn iter_test_command<'v>(
    command: Value<'v>,
) -> impl Iterator<Item = slug_error::Result<TestCommandMember<'v>>> {
    if command.is_none() {
...
fn iter_test_env<'v>(
    env: Value<'v>,
) -> impl Iterator<Item = slug_error::Result<(&'v str, &'v dyn CommandLineArgLike<'v>)>> {
    if env.is_none() {
        return Either::Left(Either::Left(empty()));
    }
    ...
    let env = match DictRef::from_value(env) {
        Some(env) => env,
        None => {
            return Either::Left(Either::Right(once(Err(slug_error!(
...
fn iter_opt_str_list<'v>(
    list: Value<'v>,
    name: &'static str,
) -> impl Iterator<Item = slug_error::Result<&'v str>> {
    if list.is_none() {
        return Either::Left(Either::Left(empty()));
    }
...
) -> impl Iterator<Item = slug_error::Result<(&'v str, &'v StarlarkCommandExecutorConfig)>> {
...
) -> impl Iterator<Item = slug_error::Result<(&'v str, Option<&'v ConfiguredProvidersLabel>)>> {
...
fn unpack_opt_executor<'v>(
    executor: Value<'v>,
) -> slug_error::Result<Option<&'v StarlarkCommandExecutorConfig>> {
    if executor.is_none() {
        return Ok(None);
    }
...
fn unpack_opt_worker<'v>(worker: Value<'v>) -> slug_error::Result<Option<&'v WorkerInfo<'v>>> {
    if worker.is_none() {
        return Ok(None);
    }
...
fn check_all<I, T>(it: I) -> slug_error::Result<()>
where
    I: IntoIterator<Item = slug_error::Result<T>>,
{
    for e in it {
        e?;
    }
    ...
    V: ValueLike<'v>,
{
    ...
    let provided_local_resources =
        iter_local_resources(info.local_resources.get().to_value())
            .collect::<slug_error::Result<IndexMap<&str, Option<&ConfiguredProvidersLabel>>>>()?;
    ...
    if !required_local_resources.is_none() {
        for resource_type in iter_value(required_local_resources).buck_error_context("`required_local_resources` should be a list or a tuple of `RequiredTestLocalResource` objects")? {
            let resource_type = StarlarkRequiredTestLocalResource::from_value(resource_type)
                .ok_or_else(|| slug_error!(slug_error::ErrorTag::Input, "`required_local_resources` should only contain `RequiredTestLocalResource` values, got {}", resource_type))?;
