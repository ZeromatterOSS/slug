...
///
/// Initialized in `slug_action_impl::context::runfiles_tree`. See §15.5.23 of
/// `thoughts/shared/plans/slug-bazel-subplans/15-bazel-9-parity.md` for why this
/// lives here and not in `slug_action_impl`: `DefaultInfo(executable=..., default_runfiles=...)`
...
///   append it to `other_outputs` as a safety net (ensures `slug build :target`
...
pub static SYNTHESIZE_RUNFILES_TREE: LateBinding<
    for<'v> fn(
        ...
        Value<'v>,
        &str,
    ) -> slug_error::Result<(Value<'v>, Value<'v>)>,
...
pub struct DefaultInfoGen<V: ValueLifetimeless> {
    ...
    /// subtarget, such as when doing `slug build cell//foo:bar[baz]`. Just like any
    ...
    /// e.g.: `slug build cell//foo:bar[baz][qux]`.
    ...
    /// with `slug run`. Stored as a list with 0 or 1 elements. When present and no
...
fn validate_default_info(info: &FrozenDefaultInfo) -> slug_error::Result<()> {
...
impl FrozenDefaultInfo {
    ...
    fn get_sub_target_providers_impl(
        &self,
        name: &str,
    ) -> slug_error::Result<Option<FrozenValueTyped<'static, FrozenProviderCollection>>> {
    ...
    fn default_outputs_impl(
        &self,
    ) -> slug_error::Result<impl Iterator<Item = slug_error::Result<StarlarkArtifact>> + '_> {
    ...
    fn sub_targets_impl(
        ...
        impl Iterator<Item = slug_error::Result<(&str, FrozenRef<'static, FrozenProviderCollection>)>>
        + '_,
    > {
    ...
    fn for_each_in_list(
        &self,
        value: FrozenValue,
        mut processor: impl FnMut(Value) -> slug_error::Result<()>,
    ) -> slug_error::Result<()> {
...
fn default_info_methods(builder: &mut MethodsBuilder) {
    ...
    /// - `runfiles_manifest`: the runfiles manifest File (always None in Slug)
    ///
    ...
    /// implicit filegroup slug creates for a `.sh` source file reached via
    ...
    fn DefaultInfo<'v>(
        ...
        let valid_default_outputs: ValueOfUnchecked<ListType<ValueIsInputArtifactAnnotation>> =
            match (default_outputs.into_option(), default_output.into_option()) {
                ...
                (Some(_), Some(_)) => {
                    return Err(
                        slug_error::Error::from(DefaultOutputError::ConflictingArguments).into(),
                    );
                }
            };
        ...
        let valid_sub_targets = sub_targets
            ...
            .collect::<slug_error::Result<Vec<(StringValue<'v>, _)>>>()?;
        ...
        // to `other_outputs` so `slug build :target` materialises it even if the exe
...
fn normalize_runfiles_files<'v>(
    ...
    if !files.is_none() && !files_is_depset {
        if let Some(list) = ListRef::from_value(files) {
            ...
            return Err(slug_error::Error::from(RunfilesError::ExpectedIterable(
                "files",
                files.to_repr(),
...
fn normalize_runfiles_dict<'v>(
    ...
    if DictRef::from_value(value).is_some() {
        ...
        Err(slug_error::Error::from(RunfilesError::ExpectedDict(
            field,
            value.to_repr(),
...
fn merge_runfiles_dicts<'v>(
    ...
    let left_dict = DictRef::from_value(left).ok_or_else(|| {
        slug_error::Error::from(RunfilesError::ExpectedDict(
            field,
            left.to_repr(),
...
pub fn merge_runfiles_values<'v>(
    ...
    let left_runfiles = Runfiles::from_value(left).ok_or_else(|| {
        slug_error::Error::from(RunfilesError::ExpectedRunfiles(
            left.to_repr(),
            left.get_type().to_owned(),
        ))
    })?;
...
fn validate_runfiles_value<'v>(value: Value<'v>) -> starlark::Result<Value<'v>> {
    if Runfiles::from_value(value).is_some() {
        ...
        Err(slug_error::Error::from(RunfilesError::ExpectedRunfiles(
            value.to_repr(),
            value.get_type().to_owned(),
        ))
        .into())
...
fn runfiles_methods(builder: &mut MethodsBuilder) {
    ...
    fn merge_all<'v>(
        ...
        let iter = runfiles_list.iterate(heap).map_err(|_| {
            slug_error::Error::from(RunfilesError::ExpectedIterable(
                "runfiles_list",
