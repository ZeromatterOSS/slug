...
use slug_core::error::validate_logview_category;
...
/// ```
///
/// Then register a target of this rule in root .buckconfig under `slug.dep_only_incompatible_info`
    ...
    fn DepOnlyIncompatibleInfo<'v>(
        ...
        for (category, value) in custom_soft_errors.entries {
            let category_str = category.to_value().unpack_str().ok_or_else(|| {
                starlark::Error::from(slug_error::internal_error!(
...
impl FrozenDepOnlyIncompatibleInfo {
    ...
    ) -> slug_error::Result<DepOnlyIncompatibleCustomSoftErrors> {
        let value_to_target_pattern =
            |target_pattern: Value<'_>| -> slug_error::Result<ParsedPattern<TargetPatternExtra>> {
