...
//! union operations. This is similar to Slug's transitive_set but with
//! a different API.
...
-> slug_error::Result<&'static OwnedFrozenValueTyped<FrozenTransitiveSetDefinition>> {
    static DEF: OnceLock<OwnedFrozenValueTyped<FrozenTransitiveSetDefinition>> = OnceLock::new();
    DEF.get_or_try_init(|| {
        let cell = CellName::unchecked_new("slug_builtin")?;
...
impl<'v> StarlarkValue<'v> for Depset {
    ...
    fn length(&self) -> starlark::Result<i32> {
        Err(slug_error::Error::from(DepsetError::LenUnsupported).into())
    }

    fn bit_or(&self, _other: Value<'v>, _heap: Heap<'v>) -> starlark::Result<Value<'v>> {
        Err(slug_error::Error::from(DepsetError::BitOrUnsupported).into())
    }
...
pub fn depset_direct_and_transitive<'v>(
    ...
    Err(slug_error::Error::from(DepsetError::TransitiveNotDepset {
        index: 0,
...
pub fn depset_to_artifact_inputs<'v>(
    ...
    for (index, element) in elements.iter().enumerate() {
        if <&dyn StarlarkInputArtifactLike<'v>>::unpack_value(*element)?.is_none() {
            return Err(slug_error::Error::from(DepsetError::ArtifactInputExpected {
                index,
...
fn merge_depset_element_type(
    ...
    match expected {
        ...
        Some(depset_type) => Err(slug_error::Error::from(DepsetError::ElementTypeMismatch {
            item_type,
...
fn validate_depset_elements<'v>(
    ...
    if depth > MAX_DEPSET_DEPTH {
        return Err(slug_error::Error::from(DepsetError::DepthExceeded {
            depth,
...
pub fn register_depset(globals: &mut GlobalsBuilder) {
    ...
    fn depset<'v>(
        ...
        // bazel's `depset(transitive = None)` works, slug's used to reject
