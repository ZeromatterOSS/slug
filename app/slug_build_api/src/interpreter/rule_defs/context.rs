...
impl<'v> AnalysisActions<'v> {
    pub fn state(&self) -> slug_error::Result<RefMut<'_, AnalysisRegistry<'v>>> {
        let state = self
            .state
            .try_borrow_mut()
...
impl<'v> AnalysisContext<'v> {
    ...
    pub fn workspace_name_str(&self) -> String {
    ...
    pub fn prepare(
        ...
        let bazel_label = configured_label.map(|label| {
            heap.alloc_typed(bazel_label_from_configured_with_alias_resolver_and_root(
                label.inner(),
    ...
    pub fn assert_no_promises(&self) -> slug_error::Result<()> {
...
fn analysis_context_methods(builder: &mut MethodsBuilder) {
    ...
    ///
    /// In Bazel, attributes are accessed via `ctx.attr.foo`. In Slug (Buck2),
    ...
    /// Since Slug does not implement split transitions, each attribute value is
    ...
    /// configuration fragments. Since Slug doesn't distinguish host from target
    ...
    ///
    /// In Slug, derives the path from the active buck-out root, target's cell
    ...
    ///
    /// In Slug, this returns the same root as `bin_dir` since there is no
    ...
    // and `new_file` are served from `@slug_builtins//:exports.bzl`
    ...
    // `expand_location` is served from `@slug_builtins//:exports.bzl`
    // through the facade. Two runtime hooks bridge the Rust-only logic:
    //   - `slug_collect_location_pool` — builds the label→paths pool
    //     (targets list + implicit attrs walk).
    //   - `slug_lookup_output_path` — lazily resolves attr.output /
    ...
    fn expand_location<'v>(
        ...
        // `_expand_location_bound`, which calls `_slug_expand_location`
...
/// Pub entry point for the `slug_collect_location_pool` Starlark
/// global. Builds the label→paths pool used by `_slug_expand_location`
...
/// `slug_lookup_output_path` hook rather than being included in this pool.
...
/// Pub entry point for the `slug_lookup_output_path` Starlark global.
...
///
/// In Slug, the output path is `<buck-out>/gen/<cell>/<cfg_hash>`.
/// Falls back to `<buck-out>/gen` if no label is available.
pub fn bin_dir_path_from_label(
    label: Option<
        starlark::values::ValueTyped<
            '_,
            slug_interpreter::types::configured_providers_label::StarlarkConfiguredProvidersLabel,
        >,
    >,
    cell_alias_resolver: Option<&CellAliasResolver>,
    root_cell_name: Option<CellName>,
...
mod tests {
    ...
    #[test]
    fn analysis_context_repo_name_uses_runtime_alias_snapshot() -> slug_error::Result<()> {
...
/// with `slug_configured::target_platform_resolution`.
...
/// Pub entry point for the `slug_compilation_mode_for_label` Starlark
...
/// `slug_collect_runfiles_into` to serve
...
/// Adds target-platform identity to real `CcToolchainInfo` values when Slug has
...
/// convert to Slug artifact groups only when command-line inputs are visited.
#[derive(Debug, ProvidesStaticType, NoSerialize, Allocative)]
...
/// artifacts through Starlark. Slug only needs these records as hidden action
...
/// Since Slug doesn't implement split transitions, each value is wrapped in
...
/// Single source of truth for Slug's builtin Make-variable (`$(VAR)`) bindings
