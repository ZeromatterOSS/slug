...
use slug_core::execution_types::executor_config::ImagePackageIdentifier;
...
use slug_core::execution_types::executor_config::RemoteExecutionPolicy;
use slug_core::execution_types::executor_config::RemoteExecutorCafFbpkg;
        ...
        let command_executor_config = {
            ...
            let re_properties = if remote_execution_properties.is_none() {
                ...
                let re_properties = DictRef::from_value(remote_execution_properties.to_value())
                    .ok_or_else(|| {
                        slug_error::Error::from(CommandExecutorConfigErrors::RePropertiesNotADict(
            ...
            let remote_options = if remote_enabled {
                ...
                let re_max_queue_time = slug_common::self_test_timeout::maybe_cap_timeout(
            ...
            let remote_cache_default = if slug_core::is_open_source() {
                remote_enabled
            } else {
                true
            };
            ...
            let executor = match (local_options, remote_options, remote_cache_enabled) {
                (local, Some(remote), remote_cache_enabled) => {
                    ...
                    Executor::RemoteEnabled(RemoteEnabledExecutorOptions {
                        executor,
                        re_properties: re_properties.ok_or(slug_error::Error::from(
                            CommandExecutorConfigErrors::MissingField(
                                "remote_execution_properties",
                            ),
                        ))?,
                        re_use_case: re_use_case.ok_or(slug_error::Error::from(
                ...
                (Some(local), None, true) => {
                    Executor::RemoteEnabled(RemoteEnabledExecutorOptions {
                        ...
                        re_use_case: re_use_case
                            .unwrap_or_else(RemoteExecutorUseCase::slug_default),
                        re_action_key,
                        cache_upload_behavior,
...
pub fn parse_custom_re_image(
    ...
    value: Value,
) -> slug_error::Result<Option<Box<RemoteExecutorCustomImage>>> {
    if value.is_none() {
        return Ok(None);
    }

    fn dict_ref<'v>(field_name: &'static str, value: Value<'v>) -> slug_error::Result<DictRef<'v>> {
    ...
    fn get_value<'v>(dict_ref: &DictRef<'v>, name: &'static str) -> slug_error::Result<Value<'v>> {
    ...
    fn get_string<'v>(dict_ref: &DictRef<'v>, name: &'static str) -> slug_error::Result<String> {
...
fn parse_remote_execution_policy(
    policy: Option<Value>,
) -> slug_error::Result<RemoteExecutionPolicy> {
    if policy.is_none() {
        Ok(RemoteExecutionPolicy::default())
    } else {
        let re_policy_dict = DictRef::from_value(policy.unwrap().to_value()).ok_or_else(|| {
            slug_error::Error::from(CommandExecutorConfigErrors::RePolicyNotADict(
...
) -> slug_error::Result<Vec<RemoteExecutorCafFbpkg>> {
    if caf_fbpkgs.is_none() {
        Ok(vec![])
    } else {
        let re_caf_fbpkgs_list =
            ListRef::from_value(caf_fbpkgs.unwrap().to_value()).ok_or_else(|| {
                slug_error::Error::from(CommandExecutorConfigErrors::ReCafFbpkgsNotAList(
        ...
        Ok(re_caf_fbpkgs_list
            ...
            .collect::<slug_error::Result<Vec<RemoteExecutorCafFbpkg>>>()?)
    }
}

pub fn parse_meta_internal_extra_params<'v>(
    params: Option<DictRef<'v>>,
) -> slug_error::Result<MetaInternalExtraParams> {
