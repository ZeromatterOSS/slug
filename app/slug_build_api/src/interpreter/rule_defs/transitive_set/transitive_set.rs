...
impl<'v, V: ValueLike<'v>> TransitiveSetGen<V> {
    ...
    pub fn projection_name(&'v self, projection: usize) -> slug_error::Result<&'v str> {
    ...
    pub fn get_projection_value(&self, projection: usize) -> slug_error::Result<Option<V>> {
        match &self.node {
            None => Ok(None),
            Some(node) => Ok(Some(
                *node
    ...
    pub(crate) fn definition(
        &self,
    ) -> slug_error::Result<ValueTypedComplex<'v, TransitiveSetDefinition<'v>>> {
    ...
    TransitiveSetGen<V>: StarlarkValue<'v> + TransitiveSetLike<'v>,
{
    ...
    pub fn iter_values<'a>(
        &'a self,
        ordering: TransitiveSetOrdering,
    ) -> slug_error::Result<Box<dyn Iterator<Item = Value<'v>> + 'a>>
    where
        'v: 'a,
    {
        Ok(Box::new(
        ...
        let node = value.try_map(|value| {
            let projections = def
                ...
                .map(|(name, spec)| {
                    ...
                    slug_error::Ok(projected_value)
                })
                .collect::<Result<Box<[_]>, _>>()?;

            slug_error::Ok(NodeGen { value, projections })
        })?;
...
fn transitive_set_methods(builder: &mut MethodsBuilder) {
    ...
    fn reduce<'v>(
        ...
        let index = match def.operations().reductions.get_index_of(reduction) {
            ...
            None => {
                return Err(
                    slug_error::Error::from(TransitiveSetError::ReductionDoesNotExist {
