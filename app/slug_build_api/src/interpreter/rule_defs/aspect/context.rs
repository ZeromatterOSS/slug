...
fn aspect_context_methods(builder: &mut MethodsBuilder) {
    ...
    ///
    /// Returns the same fragments as `ctx.fragments` since Slug doesn't
    ...
    /// Returns the genfiles_dir (same as bin_dir in Slug).
    #[starlark(attribute)]
    ...
    // installed by `_invoke_aspect` in `@slug_builtins//:exports.bzl`,
    // sharing the rule-side `_slug_target_platform_has_constraint`
    // shim.
