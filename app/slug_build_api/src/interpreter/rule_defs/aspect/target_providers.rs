...
fn aspect_target_methods(builder: &mut MethodsBuilder) {
    ...
    fn get<'v>(
        ...
        match provider.as_provider_callable() {
            ...
            None => Err(
                slug_error::Error::from(AspectTargetError::IndexTypeNotProvider(
                    provider.get_type(),
                ))
                .into(),
            ),
...
impl<'v> StarlarkValue<'v> for AspectTargetProviders<'v> {
        ...
        match index.as_provider_callable() {
            Some(callable) => {
                ...
                match self.provider_collection().get_provider_raw(&provider_id) {
                    ...
                    None => Err(
                        slug_error::Error::from(AspectTargetError::ProviderNotFound {
            ...
            None => Err(
                slug_error::Error::from(AspectTargetError::IndexTypeNotProvider(index.get_type()))
                    .into(),
            ),
        }
    }
    ...
    fn is_in(&self, other: Value<'v>) -> starlark::Result<bool> {
        match other.as_provider_callable() {
            ...
            None => Err(
                slug_error::Error::from(AspectTargetError::IndexTypeNotProvider(other.get_type()))
                    .into(),
            ),
        }
    }
