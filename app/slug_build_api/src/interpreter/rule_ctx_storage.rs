...
//!
//! Set by the analysis pipeline in `slug_analysis::analysis::env::get_user_defined_rule_spec`
//! before calling a rule's implementation function, and cleared afterwards. Consumers:
//!
//! - `slug_interpreter_for_build::subrule`: looks it up to inject `ctx` as the first
//!   positional argument to subrule implementation functions.
//! - `slug_build_api::interpreter::rule_defs::provider::builtin::default_info`:
...
thread_local! {
    ...
    /// Subrule-wrapper `Value` for the current rule analysis. Set
    /// alongside `CURRENT_RULE_CTX` so subrule invocations can route
    /// through the bundled
    /// `subrule_implementation_wrapper(impl, ctx, **kwargs)`. `None`
    /// when @slug_builtins isn't registered or doesn't expose the
...
/// Stash the bundled `subrule_implementation_wrapper` alongside the
/// current rule's ctx so subrule invocations can route through the
/// same Starlark facade rule contexts already see.
///
