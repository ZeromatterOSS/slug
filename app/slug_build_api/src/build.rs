...
pub struct ConfiguredBuildTargetResultGen<T> {
    ...
    pub graph_properties: Option<slug_error::Result<MaybeCompatible<GraphPropertiesValues>>>,
    pub errors: Vec<slug_error::Error>,
}

pub type ConfiguredBuildTargetResult =
    ConfiguredBuildTargetResultGen<slug_error::Result<ProviderArtifacts>>;
...
pub struct BuildTargetResultBuilder {
    res: HashMap<
        ...
        Option<ConfiguredBuildTargetResultGen<(usize, slug_error::Result<ProviderArtifacts>)>>,
    >,
    configured_to_pattern_modifiers: HashMap<ConfiguredProvidersLabel, Vec<Modifiers>>,
    other_errors: BTreeMap<Option<ProvidersLabel>, Vec<slug_error::Error>>,
    build_failed: bool,
...
impl BuildTargetResultBuilder {
    ...
    pub fn event(&mut self, event: BuildEvent) -> slug_error::Result<FailFastState> {
        ...
        match variant {
            ...
            ConfiguredBuildEventVariant::Timeout => {
                self.res.get_mut(&label)
                     ...
                     .errors.push(slug_error::Error::from(BuildDeadlineExpired));
                // TODO(cjhopman): Why don't we break here?
                self.build_failed = true;
            }
...
pub struct BuildTargetResult {
    ...
    pub other_errors: BTreeMap<Option<ProvidersLabel>, Vec<slug_error::Error>>,
...
pub enum ConfiguredBuildEventExecutionVariant {
    BuildOutput {
        output: slug_error::Result<ProviderArtifacts>,
...
pub enum ConfiguredBuildEventVariant {
    ...
    GraphProperties {
        graph_properties: slug_error::Result<MaybeCompatible<GraphPropertiesValues>>,
    },
    Error {
...
async fn build_configured_label_inner<'a>(
        ...
        .cloned()
    {
        ...
        let node_keys = resolved_artifacts
            ...
            .filter_map(|resolved| match resolved.dupe() {
                ...
                ResolvedArtifactGroup::InputSymlink(_) => None,
            })
            .collect();
    ...
    if !opts.skippable && outputs.is_empty() {
        let docs = "https://slug.build/docs/users/faq/common_issues/#why-does-my-target-not-have-any-outputs"; // @oss-enable
        // @oss-disable: let docs = "https://www.internalfb.com/intern/staticdocs/slug/docs/users/faq/common_issues/#why-does-my-target-not-have-any-outputs";
            ...
            |(index, (output, provider_type))| {
                ...
                Either::Left(async move {
                    ...
                    .await
                    {
                        ...
                        Err(e) => Err(slug_error::Error::from(e)),
                    };
