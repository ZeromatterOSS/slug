...
use kuro_query::query::environment::QueryTargets;
...
impl<'a, T: QueryCommandTarget> DotDigraph<'a> for DotTargetGraph<T> {
    ...
    fn for_each_node<F: FnMut(&Self::Node) -> kuro_error::Result<()>>(
        &'a self,
        mut f: F,
    ) -> kuro_error::Result<()> {
    ...
    fn for_each_edge<F: FnMut(&DotEdge) -> kuro_error::Result<()>>(
        &'a self,
        node: &Self::Node,
        mut f: F,
    ) -> kuro_error::Result<()> {
...
impl<T: QueryCommandTarget> DotNode for DotTargetGraphNode<'_, T> {
    fn attrs(&self) -> kuro_error::Result<DotNodeAttrs> {
        let extra = match &self.1.attributes {
            Some(attr_regex) => {
                let mut extra = SmallMap::new();
                QueryTargets::for_all_attrs::<kuro_error::Error, _, _>(
                    self.0,
