...
use kuro_cli_proto::CqueryRequest;
use kuro_cli_proto::CqueryResponse;
...
pub(crate) async fn cquery_command(
    ...
    req: CqueryRequest,
) -> kuro_error::Result<CqueryResponse> {
...
impl ServerCommandTemplate for CqueryServerCommand {
    type StartEvent = kuro_data::CQueryCommandStart;
    type EndEvent = kuro_data::CQueryCommandEnd;
    type Response = CqueryResponse;
    type PartialResult = kuro_cli_proto::StdoutBytes;

    fn start_event(&self) -> kuro_data::CQueryCommandStart {
        kuro_data::CQueryCommandStart {
...
async fn cquery(
    ...
    let profile_mode = request
        .profile_mode
        .map(|i| kuro_cli_proto::ProfileMode::try_from(i).internal_error("Invalid profile mode"))
        .transpose()?;
...
impl ProviderLookUp<ConfiguredTargetNode> for LinearRecomputeDiceComputations<'_> {
    ...
    ) -> kuro_error::Result<MaybeCompatible<FrozenProviderCollectionValue>> {
        Ok(self
            .get()
