...
impl<'a, T: QueryCommandTarget> DotDigraph<'a> for DotTargetGraph<T> {
    ...
    fn for_each_node<F: FnMut(&Self::Node) -> slug_error::Result<()>>(
        &'a self,
        mut f: F,
    ) -> slug_error::Result<()> {
    ...
    fn for_each_edge<F: FnMut(&DotEdge) -> slug_error::Result<()>>(
        &'a self,
        node: &Self::Node,
        mut f: F,
    ) -> slug_error::Result<()> {
...
impl<T: QueryCommandTarget> DotNode for DotTargetGraphNode<'_, T> {
    fn attrs(&self) -> slug_error::Result<DotNodeAttrs> {
        let extra = match &self.1.attributes {
            Some(attr_regex) => {
                let mut extra = SmallMap::new();
                QueryTargets::for_all_attrs::<slug_error::Error, _, _>(
                    self.0,
