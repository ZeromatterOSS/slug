...
impl ServerCommandTemplate for CqueryServerCommand {
    type StartEvent = slug_data::CQueryCommandStart;
    type EndEvent = slug_data::CQueryCommandEnd;
    type Response = CqueryResponse;
    type PartialResult = slug_cli_proto::StdoutBytes;

    fn start_event(&self) -> slug_data::CQueryCommandStart {
        slug_data::CQueryCommandStart {
...
async fn cquery(
    ...
    let profile_mode = request
        .profile_mode
        .map(|i| slug_cli_proto::ProfileMode::try_from(i).internal_error("Invalid profile mode"))
        .transpose()?;
...
impl ProviderLookUp<ConfiguredTargetNode> for LinearRecomputeDiceComputations<'_> {
    ...
    ) -> slug_error::Result<MaybeCompatible<FrozenProviderCollectionValue>> {
        Ok(self
            .get()
