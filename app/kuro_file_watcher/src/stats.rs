...
pub(crate) struct FileWatcherStats {
    stats: kuro_data::FileWatcherStats,
    // Bounded by MAX_FILE_CHANGE_RECORDS
    changes: Vec<kuro_data::FileWatcherEvent>,
    // Did we not insert things into changes
    changes_missed: bool,
}

impl FileWatcherStats {
    pub(crate) fn new(stats: kuro_data::FileWatcherStats, min_count: usize) -> Self {
    ...
    pub(crate) fn add(
        &mut self,
        path: String,
        event: kuro_data::FileWatcherEventType,
        kind: kuro_data::FileWatcherKind,
    ) {
        self.stats.events_total += 1;
        self.stats.events_processed += 1;

        if self.changes.len() < MAX_FILE_CHANGE_RECORDS {
            self.changes.push(kuro_data::FileWatcherEvent {
                event: event as i32,
                kind: kind as i32,
                path,
            });
    ...
    pub(crate) fn finish(self) -> kuro_data::FileWatcherStats {
        let Self {
            mut stats,
            changes,
            changes_missed,
