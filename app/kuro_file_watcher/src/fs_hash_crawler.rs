    ...
    async fn sync(
        ...
        span_async(
            kuro_data::FileWatcherStart {
                provider: kuro_data::FileWatcherProvider::FsHashCrawler as i32,
            },
            async {
...
impl FsSnapshot {
    fn build(root: &ProjectRoot, cells: &CellResolver) -> kuro_error::Result<Self> {
    ...
    fn get_updates(&self, new_snapshot: &FsSnapshot) -> kuro_error::Result<Vec<FsEvent>> {
    ...
    fn get_updates_for_dice(
        ...
        ignore_specs: &HashMap<CellName, IgnoreSet>,
    ) -> kuro_error::Result<(kuro_data::FileWatcherStats, FileChangeTracker)> {
...
fn file_hash(path: &Path) -> kuro_error::Result<Hash> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_fs_snapshot() -> kuro_error::Result<()> {
        ...
        let get_path = |path| -> kuro_error::Result<(AbsPathBuf, CellPath)> {
