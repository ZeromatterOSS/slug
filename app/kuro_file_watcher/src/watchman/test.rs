...
async fn wait_for_watchman(watchman_sock: &Path) -> kuro_error::Result<()> {
    ...
    loop {
        ...
        match connector.connect().await {
            Ok(..) => return Ok(()),
            Err(e) if i >= 100 => return Err(kuro_error!(kuro_error::ErrorTag::Tier0, "{}", e)),
            _ => {
...
impl WatchmanInstance {
    pub async fn shutdown(&mut self) -> kuro_error::Result<()> {
        let child = self
            .child
            .as_mut()
...
async fn spawn_watchman(watchman_dir: &Path) -> kuro_error::Result<WatchmanInstance> {
...
#[tokio::test]
async fn test_syncable_query() -> kuro_error::Result<()> {
