...
use kuro_data::FileWatcherEventType as Type;
use kuro_data::FileWatcherKind as Kind;
...
pub(crate) enum EdenFsWatcherError {
    #[kuro(tag = IoNotConnected)]
    #[error("Failed to connect to EdenFS {0}")]
    EdenConnectionError(kuro_error::Error),
    #[kuro(tag = Input)]
...
impl EdenFsFileWatcher {
    ...
    ) -> kuro_error::Result<ProcessChangeStatus> {
                        ...
                        soft_error!(
                            ...
                            kuro_error::kuro_error!(
                                ...
                                "EdenFS reported SmallChangeNotification::renamed directory: '{}' -> '{}'. \
                                 ...
                                 EdenFS Thrift API has changed and the kuro code needs to be updated.",
    ...
    async fn skip_change(&self, change: &ChangeNotification) -> kuro_error::Result<()> {
    ...
    async fn process_sapling_status(
            ...
            .buck_error_context("Failed to get Sapling status.")?
        {
            ...
            SaplingGetStatusResult::Normal(status) => {
                ...
                let results: kuro_error::Result<Vec<_>> = status
                    .into_iter()
    ...
    async fn on_large_or_unknown_change(
        &self,
        dice: DiceTransactionUpdater,
    ) -> kuro_error::Result<(FileWatcherStats, FileChangeTracker, DiceTransactionUpdater)> {
    ...
    async fn update_mergebase(&self, to: &str) -> kuro_error::Result<bool> {
    ...
    async fn base_file_watcher_stats(&self) -> kuro_error::Result<kuro_data::FileWatcherStats> {
    ...
    async fn sync(
        ...
        span_async(
            kuro_data::FileWatcherStart {
                provider: kuro_data::FileWatcherProvider::EdenFs as i32,
            },
            async {
