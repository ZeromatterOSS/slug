...
//! The panic hook, shared by the kuro CLI and daemon.
//!
//! The Kuro CLI unwinds the stack on a panic, while the Kuro daemon terminates without unwinding.
...
pub fn initialize() -> kuro_error::Result<()> {
    ...
    kuro_core::error::initialize(Box::new(move |category, err, loc, options| {
        let fb = kuro_common::fbinit::get_or_init_fbcode_globals();
...
mod imp {
    ...
    use kuro_events::daemon_id::get_daemon_id_for_panics;
        ...
        trace
            ...
            .map(|frame| {
                let symbols = frame
                    .symbols()
                    .iter()
                    .map(|symbol| kuro_data::structured_error::Symbol {
                        name: symbol
                            .name()
                ...
                kuro_data::structured_error::StackFrame {
    ...
    fn get_metadata_for_panic(options: &StructuredErrorOptions) -> HashMap<String, String> {
        ...
        #[cfg(not(client_only))]
        if let Some(commands) = kuro_server::active_commands::try_active_commands() {
    ...
    pub(crate) fn write_soft_error(
        ...
        // commands.
        match kuro_events::dispatch::get_dispatcher_opt() {
            Some(dispatcher) => {
            ...
            None => {
                ...
                #[cfg(not(client_only))]
                let warn = !kuro_server::active_commands::broadcast_instant_event(&event)
                    && !options.quiet;
                if warn {
    ...
    fn panic_payload(
        ...
        message: String,
        backtrace: Vec<kuro_data::structured_error::StackFrame>,
        options: &StructuredErrorOptions,
        soft_error_category: Option<kuro_data::SoftError>,
        ...
        kuro_data::StructuredError {
            ...
            soft_error_category: soft_error_category
                .map(|arg0: kuro_data::SoftError| ToOwned::to_owned(&arg0)),
    ...
    fn write_to_scribe(fb: FacebookInit, data: kuro_data::StructuredError) {
        ...
        let _err = thread_spawn("kuro-write-panic-to-scribe", move || {
