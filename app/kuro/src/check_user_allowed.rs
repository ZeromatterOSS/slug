...
#[cfg(windows)]
pub(crate) fn check_user_allowed() -> kuro_error::Result<()> {
    use std::io;
    use std::mem;
    ...
    if elevation_struct.TokenIsElevated == 1 {
        // In CI, if kuro got run from an admin shell, we need not worry that a
        // subsequent invocation might come from a non-admin shell. It almost
        // certainly will not.
        if !is_ci()? {
            tracing::warn!(
                "You're running kuro from an admin shell. Invocations from non-admin shells will likely fail going forward. To remediate, run `kuro clean` in this admin shell, then switch to a non-admin shell."
            );
        }
    }
    Ok(())
...
pub(crate) fn check_user_allowed() -> kuro_error::Result<()> {
    ...
    #[error("kuro is not allowed to run as root (unless home dir is owned by root)")]
    #[kuro(tag = Input)]
    struct RootError;
