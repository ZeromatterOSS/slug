...
use kuro_critical_path::Graph;
use kuro_critical_path::GraphBuilder;
use kuro_critical_path::OptionalVertexId;
use kuro_critical_path::PushError;
use kuro_critical_path::VertexData;
use kuro_critical_path::VertexKeys;
...
pub(crate) struct LongestPathGraphBackend {
    builder: kuro_error::Result<GraphBuilder<NodeKey, NodeData>>,
...
impl BuildListenerBackend for LongestPathGraphBackend {
    ...
    fn finish(self) -> kuro_error::Result<BuildInfo> {
        let (graph, keys, data) = {
...
) -> kuro_error::Result<(DetailedCriticalPath, Vec<(ConfiguredTargetLabel, Duration)>)> {
    ...
    let critical_path_for_top_level_targets = top_level_targets
        ...
        .filter_map(|t| {
            let max_cost = (|| {
                ...
                kuro_error::Result::Ok(Duration::from_micros(path_cost.runtime))
            })();
...
fn compute_slowest_paths(
    ...
    keys: &VertexKeys<NodeKey>,
    data: &VertexData<NodeData>,
) -> kuro_error::Result<DetailedCriticalPath> {
    let mut last = None;
