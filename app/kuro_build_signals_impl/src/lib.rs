...
use kuro_build_api::actions::calculation::ActionExtraData;
use kuro_build_api::actions::calculation::ActionWithExtraData;
use kuro_build_api::actions::calculation::BuildKey;
use kuro_build_api::actions::calculation::BuildKeyActivationData;
use kuro_build_api::artifact_groups::ResolvedArtifactGroupBuildSignalsKey;
use kuro_build_api::artifact_groups::calculation::EnsureProjectedArtifactKey;
use kuro_build_api::artifact_groups::calculation::EnsureTransitiveSetProjectionKey;
use kuro_build_api::build_signals::BuildSignals;
...
use kuro_build_signals::env::EarlyCommandTiming;
use kuro_build_signals::env::FinishBuildSignals;
...
use kuro_common::package_listing::dice::PackageListingKey;
use kuro_common::package_listing::dice::PackageListingKeyActivationData;
...
impl NodeKey {
    ...
    ) -> kuro_data::critical_path_entry2::Entry {
        match self {
            NodeKey::BuildKey(ref key) => {
                ...
                match extra_data {
                    NodeExtraData::Action(ActionNodeData {
                        ...
                        action_digest,
                        invalidation_info,
                    }) => kuro_data::critical_path_entry2::ActionExecution {
                        owner: Some(owner),
            ...
            NodeKey::AnalysisKey(key) => kuro_data::critical_path_entry2::Analysis {
            ...
            NodeKey::FinalMaterialization(key) => {
                ...
                kuro_data::critical_path_entry2::FinalMaterialization {
                    owner: Some(owner),
                    path: key.get_path().path().to_string(),
                }
                .into()
            }
            NodeKey::InterpreterResultsKey(key) => kuro_data::critical_path_entry2::Load {
                package: key.0.to_string(),
            }
            .into(),

            NodeKey::PackageListingKey(key) => kuro_data::critical_path_entry2::Listing {
                package: key.0.to_string(),
            }
            .into(),
            ...
            NodeKey::TestExecution(t) => kuro_data::critical_path_entry2::TestExecution {
            ...
            NodeKey::TestListing(t) => kuro_data::critical_path_entry2::TestListing {
                target_label: Some(t.target.as_proto()),
                suite: t.suite.to_string(),
            }
            .into(),
    ...
    fn into_generic_entry(self) -> kuro_data::critical_path_entry2::Entry {
        kuro_data::critical_path_entry2::GenericEntry {
            kind: match self {
...
pub(crate) struct FinishBuildSignalsImpl {
    sender: Arc<BuildSignalSender>,
    handle: JoinHandle<kuro_error::Result<()>>,
}
    ...
    T: BuildListenerBackend,
{
    ...
    pub(crate) async fn run_and_log(mut self, ctx: BuildSignalsContext) -> kuro_error::Result<()> {
        ...
        let top_level_targets = top_level_targets.try_map(|(key, duration)| {
            kuro_error::Ok(kuro_data::TopLevelTargetCriticalPath {
                target: Some(key.as_proto()),
                duration: Some((*duration).try_into()?),
            })
        })?;

        instant_event(kuro_data::BuildGraphExecutionInfo {
            critical_path2,
            slowest_path,
    ...
    fn process_test_execution(&mut self, signal: TestExecutionSignal) -> kuro_error::Result<()> {
    ...
    fn process_test_listing(&mut self, signal: TestListingSignal) -> kuro_error::Result<()> {
...
impl DetailedCriticalPath {
    ...
    fn create_proto_entries_for_early_timings(
        ...
        let generic_entry = |kind: &str| -> kuro_data::critical_path_entry2::Entry {
            kuro_data::critical_path_entry2::GenericEntry {
                kind: kind.to_owned(),
            }
            .into()
    ...
    ) -> kuro_error::Result<Vec<kuro_data::CriticalPathEntry2>> {
...
struct ActionNodeData {
    ...
    execution_kind: kuro_data::ActionExecutionKind,
    target_rule_type_name: Option<String>,
    action_digest: Option<String>,
    invalidation_info: Option<kuro_data::CommandInvalidationInfo>,
}
