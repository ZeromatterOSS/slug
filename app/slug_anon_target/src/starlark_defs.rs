...
use slug_build_api::interpreter::rule_defs::context::ANALYSIS_ACTIONS_METHODS_ANON_TARGET;
...
use slug_interpreter_for_build::rule::FrozenArtifactPromiseMappings;
...
impl<'v> StarlarkAnonTarget<'v> {
    ...
    ) -> slug_error::Result<StarlarkAnonTarget<'v>> {
...
fn anon_target_methods(builder: &mut MethodsBuilder) {
    ...
    fn artifact<'v>(
        ...
        match this.artifacts.get(name) {
            ...
            None => {
                let slug_error: slug_error::Error =
...
fn analysis_actions_methods_anon_target(builder: &mut MethodsBuilder) {
    ...
    ///
    /// For more details see https://slug.build/docs/rule_authors/anon_targets/
    fn anon_target<'v>(
