...
use slug_analysis::analysis::calculation::get_rule_spec;
use slug_analysis::analysis::env::RuleSpec;
use slug_analysis::analysis::env::transitive_validations;
use slug_artifact::artifact::artifact_type::Artifact;
use slug_build_api::analysis::AnalysisResult;
use slug_build_api::analysis::anon_promises_dyn::AnonPromisesDyn;
use slug_build_api::analysis::anon_promises_dyn::RunAnonPromisesAccessorPair;
use slug_build_api::analysis::anon_targets_registry::ANON_TARGET_REGISTRY_NEW;
use slug_build_api::analysis::anon_targets_registry::AnonTargetsRegistryDyn;
use slug_build_api::analysis::registry::AnalysisRegistry;
use slug_build_api::anon_target::AnonTargetDependentAnalysisResults;
use slug_build_api::anon_target::AnonTargetDyn;
use slug_build_api::artifact_groups::promise::PromiseArtifact;
use slug_build_api::artifact_groups::promise::PromiseArtifactId;
use slug_build_api::artifact_groups::promise::PromiseArtifactResolveError;
...
use slug_build_api::deferred::calculation::EVAL_ANON_TARGET;
use slug_build_api::deferred::calculation::GET_PROMISED_ARTIFACT;
...
use slug_configured::execution::find_execution_platform_by_configuration;
...
use slug_core::deferred::base_deferred_key::BaseDeferredKeyDyn;
...
use slug_core::pattern::pattern::PatternData;
use slug_core::pattern::pattern::lex_target_pattern;
...
use slug_interpreter::starlark_promise::StarlarkPromise;
...
use slug_node::attrs::spec::internal::is_internal_attr;
...
impl Key for AnonTargetKey {
    type Value = slug_error::Result<AnalysisResult>;
...
impl AnonTargetKey {
    fn downcast(key: Arc<dyn BaseDeferredKeyDyn>) -> slug_error::Result<Self> {
        Ok(AnonTargetKey(
            key.into_any()
                .downcast()
                .ok()
    ...
    fn parse_target_label(x: &str) -> slug_error::Result<TargetLabel> {
        let err = || {
            format!(
    ...
    fn create_name(rule_name: &str) -> slug_error::Result<TargetLabel> {
    ...
    fn coerce_name(x: Value) -> slug_error::Result<TargetLabel> {
    ...
    fn coerce_to_anon_target_attr(
        ...
        x: Value,
        ctx: &AnonAttrCtx,
    ) -> slug_error::Result<AnonTargetAttr> {
    ...
    ) -> BoxFuture<'a, slug_error::Result<AnalysisResult>> {
...
impl<'v> AnonTargetsRegistry<'v> {
    ...
    ) -> slug_error::Result<&'v mut AnonTargetsRegistry<'v>> {
    ...
    pub(crate) fn anon_target_key(
        ...
        attributes: UnpackDictEntries<&'v str, Value<'v>>,
        owner_key: &BaseDeferredKey,
    ) -> slug_error::Result<AnonTargetKey> {
    ...
    pub(crate) fn register_artifact(
        ...
        id: usize,
    ) -> slug_error::Result<PromiseArtifact> {
...
impl<'v> AnonTargetsRegistryDyn<'v> for AnonTargetsRegistry<'v> {
    ...
    */
    ...
    fn assert_no_promises(&self) -> slug_error::Result<()> {
