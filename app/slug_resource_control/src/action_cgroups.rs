...
use dupe::Dupe;
use slug_common::init::ActionSuspendStrategy;
...
pub struct ActionCgroupResult {
    ...
    pub error: Option<slug_error::Error>,
...
impl ActionCgroupResult {
    ...
    pub fn from_error(e: slug_error::Error) -> Self {
        Self {
            memory_peak: None,
            swap_peak: None,
            error: Some(e),
...
impl ActionCgroupSession {
    ...
    ) -> slug_error::Result<Option<(Self, RetryFuture)>> {
...
fn suspend_cgroup(
    ...
    now: Instant,
) -> (SuspendedActionCgroup, slug_data::ResourceControlEventKind) {
    let (wake_implementation, event_kind) = match cgroup.suspend_implementation {
        SuspendImplementation::CgroupFreeze(kill_sender, freeze_sender) => {
            ...
            (
                ...
                slug_data::ResourceControlEventKind::SuspendFreeze,
            )
        }
        SuspendImplementation::KillAndRetry(kill_sender) => {
            ...
            (
                ...
                slug_data::ResourceControlEventKind::SuspendKill,
            )
        }
    };
    (
...
fn wake_cgroup(
    cgroup: ActionCgroup,
    wake_implementation: WakeImplementation,
) -> (RunningActionCgroup, slug_data::ResourceControlEventKind) {
    let (retry_tx, event_kind) = match wake_implementation {
        WakeImplementation::CgroupUnfreeze {
            ...
            return (
                ...
                slug_data::ResourceControlEventKind::WakeUnfreeze,
            );
        }
        WakeImplementation::KillAndRetry(retry_tx) => {
            (retry_tx, slug_data::ResourceControlEventKind::WakeRetry)
        }
        WakeImplementation::UnblockStart(retry_tx) => (
            retry_tx,
            slug_data::ResourceControlEventKind::WakeDelayedStart,
        ),
    };
...
mod tests {
    ...
    #[tokio::test]
    async fn test_peak_memory_and_swap() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_freeze() -> slug_error::Result<()> {
        let cgroup_1 = tempfile::tempdir()?;
        let cgroup_2 = tempfile::tempdir()?;
        let cgroup_1 =
            CgroupPathBuf::new(AbsNormPathBuf::unchecked_new(cgroup_1.path().to_path_buf()));
