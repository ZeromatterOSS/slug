...
impl<M: MemoryMonitoring, K: CgroupKind> Cgroup<M, K> {
    ...
    pub(crate) async fn read_enabled_controllers(&self) -> slug_error::Result<EnabledControllers> {
    ...
    pub async fn set_memory_high(&self, memory_high: &str) -> slug_error::Result<()> {
        CgroupFile::open(
            self.dir.dupe(),
    ...
    pub async fn set_memory_max(&self, memory_max: &str) -> slug_error::Result<()> {
        CgroupFile::open(
            self.dir.dupe(),
    ...
    async fn read_resource_constraints(&self) -> slug_error::Result<EffectiveResourceConstraints> {
        let read = |f| async move {
            let f = CgroupFile::open(self.dir.dupe(), FileNameBuf::unchecked_new(f), false).await?;
            slug_error::Ok(f.read_max_or_int().await?)
        };
    ...
    pub(crate) async fn read_effective_resouce_constraints(
        &self,
    ) -> slug_error::Result<EffectiveResourceConstraints> {
...
impl Cgroup<NoMemoryMonitoring, CgroupKindUndecided> {
    pub(crate) fn sync_try_from_path(path: CgroupPathBuf) -> slug_error::Result<Self> {
    ...
    pub async fn try_from_path(path: CgroupPathBuf) -> slug_error::Result<Self> {
        tokio::task::spawn_blocking(|| Self::sync_try_from_path(path)).await?
    }

    /// Treat this cgroup as a leaf cgroup
    pub async fn into_leaf(self) -> slug_error::Result<Cgroup<NoMemoryMonitoring, CgroupKindLeaf>> {
        Ok(Cgroup {
            kind: CgroupKindLeaf {
                procs: Arc::new(
                    CgroupFile::open(
    ...
    ) -> slug_error::Result<Cgroup<NoMemoryMonitoring, CgroupKindInternal>> {
...
impl<M: MemoryMonitoring, K: CgroupKind> Cgroup<M, K> {
    fn sync_discouraged_make_child_impl(
        ...
        path: &CgroupPath,
        child: &FileName,
    ) -> slug_error::Result<CgroupMinimal> {
...
impl<M: MemoryMonitoring> Cgroup<M, CgroupKindInternal> {
    ...
    pub(crate) async fn make_child(&self, child: FileNameBuf) -> slug_error::Result<CgroupMinimal> {
    ...
    pub(crate) async fn make_leaf_child(
        &self,
        child: FileNameBuf,
    ) -> slug_error::Result<Cgroup<NoMemoryMonitoring, CgroupKindLeaf>> {
        let c = self.make_child(child).await?;
        c.into_leaf().await
    }
}
...
impl<K: CgroupKind> Cgroup<NoMemoryMonitoring, K> {
    pub(crate) async fn enable_memory_monitoring(
        self,
    ) -> slug_error::Result<Cgroup<WithMemoryMonitoring, K>> {
        let open = |f| {
            let d = &self.dir;
...
impl<K: CgroupKind> Cgroup<WithMemoryMonitoring, K> {
    pub async fn read_memory_stat(&self) -> slug_error::Result<MemoryStat> {
        self.memory.memory_stat.read_memory_stat().await
    }

    pub async fn read_memory_current(&self) -> slug_error::Result<u64> {
        self.memory.memory_current.read_int().await
    }

    pub async fn read_swap_current(&self) -> slug_error::Result<u64> {
        self.memory.swap_current.read_int().await
    }

    pub async fn read_memory_pressure(&self) -> slug_error::Result<f64> {
        Ok(self
            .memory
            .memory_pressure
...
impl<M: MemoryMonitoring, K: CgroupKind> Cgroup<M, K> {
    pub async fn freeze(&self) -> slug_error::Result<CgroupFreezeGuard> {
