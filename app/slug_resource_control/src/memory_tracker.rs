...
pub fn spawn_memory_reporter(
    ...
    tokio::task::spawn(slug_util::async_move_clone!(cancel, {
...
pub async fn create_memory_tracker(
    ...
    daemon_id: &DaemonId,
) -> slug_error::Result<Option<MemoryTrackerHandle>> {
...
impl MemoryTracker {
    ...
    #[doc(hidden)]
    pub(crate) async fn spawn(self, duration: Duration) -> slug_error::Result<()> {
...
async fn read_memory_file(file: &mut File, file_name: &str) -> slug_error::Result<u64> {
    file.rewind().await?;
...
pub async fn read_memory_current(memory_current: &mut File) -> slug_error::Result<u64> {
    read_memory_file(memory_current, "memory.current").await
}

pub async fn read_memory_swap_current(memory_swap_current: &mut File) -> slug_error::Result<u64> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_current_memory_changes_tracked() -> slug_error::Result<()> {
        ...
        let assert_max_over = |f: fn(&slug_data::ResourceControlEvents) -> u64, val, name: &str| {
