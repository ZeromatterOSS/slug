...
impl EventSenderState {
    pub(crate) fn new(
        ...
        Self {
            metadata: slug_events::metadata::collect(daemon_id),
    ...
    pub(crate) fn maybe_send_scheduled_event(
            ...
            .is_none_or(|last| now.duration_since(last) > freq)
        {
            ...
            self.send_event(
                slug_data::ResourceControlEventKind::Scheduled,
                None,
                actions_running,
                actions_suspended,
            );
    ...
    pub(crate) fn send_event(
        &mut self,
        kind: slug_data::ResourceControlEventKind,
...
impl ResourceControlEventMostly {
    pub(crate) fn complete(self, uuid: &TraceId) -> slug_data::ResourceControlEvents {
        slug_data::ResourceControlEvents {
            uuid: uuid.to_string(),
            kind: self.kind.into(),
            ...
            ancestor_cgroup_constraints: Some(slug_data::AncestorCgroupConstraints {
