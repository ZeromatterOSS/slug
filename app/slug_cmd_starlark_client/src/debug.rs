...
pub fn write_dap_message(out: &mut impl Write, msg: &[u8]) -> slug_error::Result<()> {
...
fn send_message_to_dap_client(msg: &[u8]) -> slug_error::Result<()> {
...
impl StreamingCommand for StarlarkDebugAttachCommand {
        ...
        let stream = ide_message_stream::<_, debugserver_types::Request>(ctx.stdin()).filter_map(
            |m| async move {
                match m {
                    ...
                    Err(e) => {
                        let _ignored = slug_client_ctx::eprintln!(
    ...
    fn extra_subscribers(&self) -> Vec<Box<dyn EventSubscriber>> {
        ...
        impl ConvertToDap {
            fn write_console(&self, msg: &str) -> slug_error::Result<()> {
        ...
        impl EventSubscriber for ConvertToDap {
                ...
                for ev in events {
                    match unpack_event(ev)? {
                        UnpackedBuckEvent::Instant(_, _, data) => match data {
                            slug_data::instant_event::Data::StructuredError(soft_error) => {
                                if !soft_error.quiet {
            ...
            async fn handle_error(&mut self, error: &slug_error::Error) -> slug_error::Result<()> {
                Ok(self
                    .write_console(&format!("slug starlark-attach debugserver error: {error}"))?)
            }
        }
...
impl PartialResultHandler for DapPartialResultHandler {
    type PartialResult = slug_cli_proto::DapMessage;

    async fn handle_partial_result(
        &mut self,
        mut _ctx: PartialResultCtx<'_>,
        partial_res: slug_cli_proto::DapMessage,
    ) -> slug_error::Result<()> {
