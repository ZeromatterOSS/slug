...
impl QueryFrontend for QueryFrontendImpl {
    async fn eval_uquery(
        ...
        query: &str,
        query_args: &[String],
    ) -> kuro_error::Result<QueryEvaluationResult<TargetNode>> {
        Ok(ctx
    ...
    async fn eval_cquery(
        ...
        Ok(ctx
            .with_linear_recompute(|ctx| async move {
                ...
                //   ```
                //   kuro cquery --target-universe android//:binary 'deps("some//:lib (<arm32>)")'
                //   ```
                eval_cquery(
                    dice_query_delegate,
                    query,
    ...
    async fn eval_aquery(
        ...
        query_args: &[String],
        global_cfg_options: GlobalCfgOptions,
    ) -> kuro_error::Result<QueryEvaluationResult<ActionQueryNode>> {
        Ok(ctx
            .with_linear_recompute(|ctx| async move {
