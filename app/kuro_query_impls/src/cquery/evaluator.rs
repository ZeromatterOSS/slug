            ...
            let (resolved_literals, universe) = match target_universe {
                None => {
                    ...
                    if let Some(universes_tx) = universes_tx {
                        universes_tx.send(universe.dupe()).map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::Tier0)).internal_error("Must be open")?;
                    }
...
async fn resolve_literals_in_universe(
...
) -> kuro_error::Result<PreresolvedQueryLiterals<ConfiguredTargetNode>> {
    ...
    let resolution_futs: FuturesUnordered<_> = literals
        ...
        .map(|lit| async move {
            ...
            (lit.to_owned(), result.map_err(kuro_error::Error::from))
        })
        .collect();
