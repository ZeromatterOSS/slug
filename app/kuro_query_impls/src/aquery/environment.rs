...
pub(crate) trait AqueryDelegate: Send + Sync {
    ...
    async fn get_node(&self, key: &ActionKey) -> kuro_error::Result<ActionQueryNode>;

    async fn expand_artifacts(
        &self,
        artifacts: &[ArtifactGroup],
    ) -> kuro_error::Result<Vec<ActionQueryNode>>;

    async fn get_target_set_from_analysis(
        &self,
        configured_label: &ConfiguredProvidersLabel,
        analysis: AnalysisResult,
    ) -> kuro_error::Result<TargetSet<ActionQueryNode>>;
}
...
impl<'c> AqueryEnvironment<'c> {
    ...
    async fn get_node(&self, label: &ActionQueryNodeRef) -> kuro_error::Result<ActionQueryNode> {
...
impl QueryEnvironment for AqueryEnvironment<'_> {
    ...
    async fn get_node(&self, node_ref: &ActionQueryNodeRef) -> kuro_error::Result<Self::Target> {
...
impl AsyncNodeLookup<ActionQueryNode> for AqueryNodeLookup<'_, '_> {
    async fn get(&self, label: &ActionQueryNodeRef) -> kuro_error::Result<ActionQueryNode> {
