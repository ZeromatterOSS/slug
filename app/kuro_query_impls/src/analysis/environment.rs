...
use kuro_node::nodes::configured_node_ref::ConfiguredTargetNodeRefNode;
use kuro_node::nodes::configured_node_ref::ConfiguredTargetNodeRefNodeDeps;
...
use kuro_query::query::environment::TraversalFilter;
use kuro_query::query::environment::deps;
use kuro_query::query::graph::dfs::dfs_postorder;
...
use kuro_query::query::syntax::simple::eval::values::QueryValue;
...
use kuro_query::query::syntax::simple::functions::helpers::QueryBinaryOp;
use kuro_query::query::syntax::simple::functions::helpers::QueryFunction;
use kuro_query::query::traversal::NodeLookupId;
...
pub(crate) trait ConfiguredGraphQueryEnvironmentDelegate: Send + Sync {
    fn eval_literal(&self, literal: &str) -> kuro_error::Result<ConfiguredTargetNode>;

    async fn get_targets_from_template_placeholder_info(
        &self,
        template_name: &'static str,
        targets: TargetSet<ConfiguredGraphNodeRef>,
    ) -> kuro_error::Result<TargetSet<ConfiguredGraphNodeRef>>;
}
...
impl QueryEnvironment for ConfiguredGraphQueryEnvironment<'_> {
    ...
    async fn owner(&self, _paths: &FileSet) -> kuro_error::Result<TargetSet<Self::Target>> {
...
async fn get_template_info_provider_artifacts(
    ...
    template_name: &str,
) -> kuro_error::Result<Vec<ArtifactGroup>> {
    let providers_label =
...
) -> kuro_error::Result<IndexMap<ConfiguredTargetLabel, Artifact>> {
