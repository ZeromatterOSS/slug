...
use kuro_common::package_listing::resolver::PackageListingResolver;
...
use kuro_query::query::environment::QueryEnvironmentAsNodeLookup;
...
use kuro_query::query::graph::successors::AsyncChildVisitor;
...
use kuro_query::query::syntax::simple::functions::HasModuleDescription;
...
pub(crate) trait UqueryDelegate: Send + Sync {
    async fn get_buildfile_names_by_cell(
        &self,
    ) -> kuro_error::Result<HashMap<CellName, Arc<[FileNameBuf]>>>;

    /// Resolves a target pattern.
    async fn resolve_target_patterns(
        &self,
        pattern: &[&str],
    ) -> kuro_error::Result<ResolvedPattern<TargetPatternExtra>>;
...
pub(crate) struct PreresolvedQueryLiterals<T: QueryTarget> {
    resolved_literals: HashMap<String, kuro_error::Result<TargetSet<T>>>,
}
...
impl<T: QueryTarget> PreresolvedQueryLiterals<T> {
    ...
    pub(crate) async fn pre_resolve(
        ...
        for (literal, result) in resolved_literal_results {
            resolved_literals.insert(literal, result.map_err(kuro_error::Error::from));
        }
        Self { resolved_literals }
    }

    /// All the literals, or error if resolution of any failed.
    pub(crate) fn literals(&self) -> kuro_error::Result<TargetSet<T>> {
...
impl<'c> UqueryEnvironment<'c> {
    ...
    async fn get_node(&self, target: &TargetLabel) -> kuro_error::Result<TargetNode> {
        let package = self
            .delegate
            .ctx()
...
impl QueryEnvironment for UqueryEnvironment<'_> {
    ...
    async fn get_node(&self, node_ref: &TargetLabel) -> kuro_error::Result<Self::Target> {
    ...
    async fn eval_literals(&self, literals: &[&str]) -> kuro_error::Result<TargetSet<TargetNode>> {
        self.literals
    ...
    async fn dfs_postorder(
        ...
        visit: impl FnMut(TargetNode) -> kuro_error::Result<()> + Send,
    ) -> kuro_error::Result<()> {
    ...
    async fn depth_limited_traversal(
        ...
        visit: impl FnMut(Self::Target) -> kuro_error::Result<()> + Send,
        depth: u32,
    ) -> kuro_error::Result<()> {
    ...
    async fn owner(&self, paths: &FileSet) -> kuro_error::Result<TargetSet<Self::Target>> {
...
pub(crate) async fn rbuildfiles<'c>(
    ...
    impl AsyncNodeLookup<Node> for Lookup {
        async fn get(&self, label: &NodeRef) -> kuro_error::Result<Node> {
...
async fn split_universe_files<'c>(
    ...
    delegate: &'c dyn UqueryDelegate,
) -> kuro_error::Result<(Vec<ArcCellPath>, Vec<ArcCellPath>)> {
...
async fn top_level_imports_by_build_file<'c>(
    ...
    delegate: &'c dyn UqueryDelegate,
) -> kuro_error::Result<HashMap<ArcCellPath, Vec<ImportPath>>> {
...
async fn first_order_imports<'c>(
    ...
    delegate: &'c dyn UqueryDelegate,
) -> kuro_error::Result<HashMap<ImportPath, Vec<ImportPath>>> {
...
) -> kuro_error::Result<Vec<ImportPath>> {
    #[derive(Clone, Dupe)]
    struct Node(Arc<ImportPath>);
