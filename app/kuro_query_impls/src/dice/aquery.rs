...
use kuro_build_api::actions::query::ActionInput;
use kuro_build_api::actions::query::ActionQueryNode;
use kuro_build_api::actions::query::ActionQueryNodeRef;
use kuro_build_api::actions::query::SetProjectionInputs;
use kuro_build_api::actions::query::iter_action_inputs;
...
) -> BoxFuture<'c, kuro_error::Result<SetProjectionInputs>> {
    async move {
...
async fn get_tset_node<'c>(
    ...
    ctx: &'c mut DiceComputations<'_>,
    key: TransitiveSetProjectionKey,
) -> kuro_error::Result<SetProjectionInputs> {
...
fn compute_action_node<'c>(
    ...
    key: ActionKey,
    fs: Arc<ArtifactFs>,
) -> BoxFuture<'c, kuro_error::Result<ActionQueryNode>> {
    async move {
...
async fn get_action_node<'c>(
    ...
    key: ActionKey,
    fs: Arc<ArtifactFs>,
) -> kuro_error::Result<ActionQueryNode> {
    let copied_node_cache = node_cache.dupe();
    node_cache
        .action_nodes
        .get_or_compute(key, move |key| {
...
impl<'c, 'd> DiceAqueryDelegate<'c, 'd> {
    pub(crate) async fn new(
        base_delegate: DiceQueryDelegate<'c, 'd>,
    ) -> kuro_error::Result<DiceAqueryDelegate<'c, 'd>> {
...
impl AqueryDelegate for DiceAqueryDelegate<'_, '_> {
    ...
    async fn expand_artifacts(
        ...
        kuro_util::future::try_join_all(refs.iter().map(|n| self.get_node(n))).await
    }
