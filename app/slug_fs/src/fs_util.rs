...
/// Slug having full control over how FS IO works is beneficial for implementing
/// IO counters and retry policies that are optimized for Slug and the EdenFS
...
impl IoError {
    ...
    pub fn categorize_for_source_file(self) -> slug_error::Error {
        if self.e.kind() == io::ErrorKind::NotFound {
            slug_error::Error::from(self).tag([ErrorTag::Input]).into()
        } else {
            self.into()
        }
    }
...
#[slug(tags = io_error_tags(e, *is_eden))]
#[error("{op}")]
pub struct IoError {
    op: String,
    #[source]
            ...
            Ok(v) => {
                if let Some(err) = last_error_kind {
                    ...
                    soft_error!(
                        ...
                        slug_error!(slug_error::ErrorTag::Input, "{}", err.to_string()),
                        quiet: true
                    )
                    .ok();
                }
...
pub fn symlink<P, Q>(original: P, link: Q) -> slug_error::Result<()>
where
    P: AsRef<Path>,
    Q: AsRef<AbsPath>,
{
...
#[cfg(unix)]
fn symlink_impl(original: &Path, link: &AbsPath) -> slug_error::Result<()> {
...
fn symlink_impl(original: &Path, link: &AbsPath) -> slug_error::Result<()> {
    ...
    fn permission_check(result: io::Result<()>) -> slug_error::Result<()> {
        match result {
            ...
            link.parent()
                .ok_or_else(|| {
                    slug_error::slug_error!(
                        slug_error::ErrorTag::SymlinkParentMissing,
...
pub fn set_current_dir<P: AsRef<AbsPath>>(path: P) -> slug_error::Result<()> {
...
pub fn set_executable<P: AsRef<AbsPath>>(path: P, executable: bool) -> slug_error::Result<()> {
    let path = path.as_ref();
...
pub fn canonicalize<P: AsRef<AbsPath>>(path: P) -> slug_error::Result<AbsNormPathBuf> {
...
pub fn canonicalize_if_exists<P: AsRef<AbsPath>>(
    path: P,
) -> slug_error::Result<Option<AbsNormPathBuf>> {
    let _guard = IoCounterKey::Canonicalize.guard();
...
pub fn simplified(path: &AbsPath) -> slug_error::Result<&AbsPath> {
...
pub fn disk_space_stats<P: AsRef<AbsPath>>(path: P) -> slug_error::Result<DiskSpaceStats> {
    #[cfg(not(windows))]
    fn disk_space_stats_impl(path: &AbsPath) -> slug_error::Result<DiskSpaceStats> {
        use std::ffi::CString;
    ...
    fn disk_space_stats_impl(path: &AbsPath) -> slug_error::Result<DiskSpaceStats> {
        ...
        use slug_util::os::win::os_str::os_str_to_wide_null_term;
...
// TODO(T143971518) Avoid RelativePath usage in slug
pub fn relative_path_from_system(path: &Path) -> slug_error::Result<Cow<'_, RelativePath>> {
...
mod tests {
    ...
    #[test]
    fn if_exists_read_dir() -> slug_error::Result<()> {
    ...
    #[test]
    fn create_and_remove_symlink_dir() -> slug_error::Result<()> {
    ...
    #[test]
    fn create_and_remove_symlink_file() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = tempdir.path().join("root");
        let root = AbsPath::new(&root)?;
        create_dir_all(root)?;
    ...
    #[test]
    fn test_symlink_with_target_length_over_max_path() -> slug_error::Result<()> {
    ...
    #[test]
    fn symlink_to_file_which_doesnt_exist() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn symlink_to_symlinked_dir() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn relative_symlink_to_nonexistent_file() -> slug_error::Result<()> {
    ...
    #[test]
    fn relative_symlink_to_nonexistent_dir() -> slug_error::Result<()> {
    ...
    #[test]
    fn relative_symlink_from_symlinked_dir_windows() -> slug_error::Result<()> {
    ...
    #[test]
    fn absolute_symlink_to_nonexistent_file_in_nonexistent_dir() -> slug_error::Result<()> {
        // tmp -- dir1 (doesn't exists) -- file1 (doesn't exist)
        //     \
    ...
    #[test]
    fn remove_file_removes_symlink_to_directory() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn remove_file_does_not_remove_directory() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
        let dir_path = root.join("dir");
    ...
    #[test]
    fn remove_file_broken_symlink() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
    ...
    #[test]
    fn remove_file_non_existing_file() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn remove_all_nonexistent() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn remove_all_regular() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn remove_all_dir() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[cfg(unix)]
    #[test]
    fn remove_all_readonly_dir() -> slug_error::Result<()> {
        use std::os::unix::fs::PermissionsExt;
    ...
    #[test]
    fn remove_all_broken_symlink() -> slug_error::Result<()> {
        fn ls(path: &Path) -> slug_error::Result<Vec<PathBuf>> {
    ...
    #[cfg(unix)]
    #[test]
    fn remove_all_path_contains_regular_file() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn remove_dir_all_does_not_remove_file() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn test_read_to_string_if_exists() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
        let f1 = root.join("f1");
        let f2 = root.join("f2");
    ...
    #[test]
    fn test_read_if_exists() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
        let f1 = root.join("f1");
        let f2 = root.join("f2");
    ...
    #[cfg(windows)]
    #[test]
    fn test_windows_relative_path() -> slug_error::Result<()> {
        assert_eq!(
    ...
    #[cfg(unix)]
    #[test]
    fn test_relative_path() -> slug_error::Result<()> {
        assert_eq!(
            fs_util::relative_path_from_system(Path::new("foo/bar"))?,
            RelativePath::new("foo/bar")
        );
    ...
    #[cfg(unix)]
    #[test]
    fn test_remove_all_removes_readonly_path() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn test_create_file_if_not_exists() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn test_disk_space_stats() -> slug_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn test_retry_io() -> slug_error::Result<()> {
        use std::io::ErrorKind;
    ...
    #[test]
    fn test_io_error_tag() -> slug_error::Result<()> {
        ...
        let slug_error = slug_error::Error::from(file.err().unwrap());
