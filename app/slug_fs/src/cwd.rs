...
impl<T: WorkingDirectoryImpl> WorkingDirectoryGen<T> {
    pub fn open(path: AbsNormPathBuf) -> slug_error::Result<Self> {
        let imp = T::open(&path)?;
        Ok(Self { path, imp })
    }

    pub fn chdir_and_promise_it_will_not_change(&self) -> slug_error::Result<()> {
        self.imp
            .chdir(&self.path)
    ...
    pub fn is_stale(&self) -> slug_error::Result<bool> {
...
pub trait WorkingDirectoryImpl: Sized {
    fn open(path: &AbsNormPath) -> slug_error::Result<Self>;
    fn chdir(&self, _path: &AbsNormPath) -> slug_error::Result<()>;
    fn is_stale(&self, path: &AbsNormPath) -> slug_error::Result<bool>;
}
...
impl WorkingDirectoryImpl for PortableWorkingDirectoryImpl {
    fn open(_path: &AbsNormPath) -> slug_error::Result<Self> {
        Ok(Self)
    }

    fn chdir(&self, path: &AbsNormPath) -> slug_error::Result<()> {
        fs_util::set_current_dir(path)?;
        Ok(())
    }

    /// Not supported for PortableWorkingDirectoryImpl
    fn is_stale(&self, _path: &AbsNormPath) -> slug_error::Result<bool> {
        Ok(false)
    }
}
...
fn cwd_will_not_change(cwd: &AbsNormPath) -> slug_error::Result<()> {
...
pub(crate) fn assert_cwd_is_not_set() -> slug_error::Result<()> {
