...
impl AbsNormPath {
    ...
    /// }
    /// ```
    pub fn new<P: ?Sized + AsRef<Path>>(p: &P) -> slug_error::Result<&AbsNormPath> {
    ...
    fn strip_prefix_impl(&self, base: &AbsNormPath) -> slug_error::Result<&Path> {
        if self.windows_prefix()? == base.windows_prefix()? {
            ...
            Err(slug_error::slug_error!(
                slug_error::ErrorTag::InvalidAbsPath,
    ...
    /// # slug_error::Ok(())
    /// ```
    pub fn windows_prefix(&self) -> slug_error::Result<OsString> {
        use std::os::windows::ffi::OsStringExt;
        use std::path::Prefix;

        match self.0.components().next().ok_or_else(|| {
            slug_error::slug_error!(slug_error::ErrorTag::InvalidAbsPath, "AbsPath is empty.")
        })? {
            std::path::Component::Prefix(prefix_component) => match prefix_component.kind() {
                ...
                prefix => Err(slug_error::slug_error!(
                    slug_error::ErrorTag::InvalidAbsPath,
                    "Unknown prefix kind: {:?}.",
                    prefix
                )),
    ...
    /// # slug_error::Ok(())
    /// ```
    pub fn strip_windows_prefix(&self) -> slug_error::Result<&Path> {
...
impl AbsNormPathBuf {
    pub fn new(path: PathBuf) -> slug_error::Result<AbsNormPathBuf> {
    ...
    pub fn from(s: String) -> slug_error::Result<Self> {
    ...
    /// # slug_error::Ok(())
    /// ```
    pub fn push_normalized<P: AsRef<RelativePath>>(&mut self, path: P) -> slug_error::Result<()> {
...
impl TryFrom<String> for AbsNormPathBuf {
    ...
    /// }
    /// ```
    fn try_from(s: String) -> slug_error::Result<AbsNormPathBuf> {
...
impl TryFrom<OsString> for AbsNormPathBuf {
    ...
    // no allocation
    fn try_from(s: OsString) -> slug_error::Result<AbsNormPathBuf> {
...
impl TryFrom<PathBuf> for AbsNormPathBuf {
    ...
    /// }
    /// ```
    fn try_from(p: PathBuf) -> slug_error::Result<AbsNormPathBuf> {
...
impl FromStr for AbsNormPathBuf {
    ...
    fn from_str(s: &str) -> slug_error::Result<Self> {
...
fn verify_abs_path(path: &AbsPath) -> slug_error::Result<()> {
...
mod tests {
    ...
    #[test]
    fn abs_paths_work_in_maps() -> slug_error::Result<()> {
    ...
    #[test]
    fn abs_path_is_comparable() -> slug_error::Result<()> {
        let foo_string = make_absolute("/foo");
        let bar_string = make_absolute("/bar");
    ...
    #[cfg(not(windows))]
    #[test]
    fn absolute_path_display_is_readable() -> slug_error::Result<()> {
