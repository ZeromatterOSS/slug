...
pub(crate) trait DebugServer {
    ...
    fn attach(&mut self, x: dap::AttachRequestArguments) -> slug_error::Result<()>;
    ...
    fn disconnect(&mut self, x: dap::DisconnectArguments) -> slug_error::Result<()>;

    /// See <https://microsoft.github.io/debug-adapter-protocol/specification#Requests_Source>
    fn source(&mut self, x: dap::SourceArguments) -> slug_error::Result<dap::SourceResponseBody>;
}
...
pub(crate) fn err_response(req: &dap::Request, err: &slug_error::Error) -> dap::Response {
    dap::Response {
...
pub(crate) fn dispatch(
    ...
    r: &dap::Request,
) -> slug_error::Result<dap::Response> {
    fn arg<T: for<'a> Deserialize<'a>>(r: &dap::Request) -> slug_error::Result<T> {
    ...
    fn ret_some<T: Serialize>(
        r: &dap::Request,
        v: slug_error::Result<T>,
    ) -> slug_error::Result<dap::Response> {
        ret(r, v.map(|v| Some(v)))
    }

    fn ret_none(r: &dap::Request, v: slug_error::Result<()>) -> slug_error::Result<dap::Response> {
        ret::<()>(r, v.map(|_| None))
    }
    ...
    match r.command.as_str() {
        ...
        _ => Err(slug_error::slug_error!(
            slug_error::ErrorTag::Input,
            "Slug debugserver didn't recognize command: {}",
            r.command
        )),
    }
}
