...
/// Messages from the debugger server to its client (the cli `slug starlark debug-attach` which
/// then forwards them along through its stdout).
#[derive(Debug)]
pub(crate) enum ToClientMessage {
    Event(dap::Event),
    Response(dap::Response),
    Shutdown(slug_error::Result<()>),
}

impl ToClientMessage {
    fn pretty_string(&self) -> slug_error::Result<String> {
        match self {
...
pub async fn run_dap_server_command(
    ...
    partial_result_dispatcher: PartialResultDispatcher<slug_cli_proto::DapMessage>,
    req: StreamingRequestHandler<slug_cli_proto::DapRequest>,
) -> slug_error::Result<slug_cli_proto::DapResponse> {
    let start_event = ctx
        .command_start_event(slug_data::StarlarkDebugAttachCommandStart {}.into())
        .await?;
    span_async(start_event, async move {
        let result = run_dap_server(ctx, partial_result_dispatcher, req)
            .await
            .map_err(Into::into);
        let end_event = command_end(&result, slug_data::StarlarkDebugAttachCommandEnd {});
...
async fn run_dap_server(
    ctx: &dyn ServerCommandContextTrait,
    mut partial_result_dispatcher: PartialResultDispatcher<slug_cli_proto::DapMessage>,
    mut req: StreamingRequestHandler<slug_cli_proto::DapRequest>,
) -> slug_error::Result<slug_cli_proto::DapResponse> {
        ...
        select! {
            request = req.next() => {
                let request = match request {
                    ...
                    None => {
                        // client disconnected.
                        break slug_cli_proto::DapResponse {};
                    }
                };
...
fn handle_outgoing_message(
    seq: &mut u32,
    partial_result_dispatcher: &mut PartialResultDispatcher<slug_cli_proto::DapMessage>,
    message: ToClientMessage,
) -> slug_error::Result<Option<slug_cli_proto::DapResponse>> {
    ...
    let (dap_json, response) = match message {
        ...
        ToClientMessage::Shutdown(res) => {
            ...
            (
                ...
                Some(slug_cli_proto::DapResponse {}),
            )
        }
    };

    partial_result_dispatcher.emit(slug_cli_proto::DapMessage { dap_json });
    Ok(response)
}
