...
use slug_interpreter::starlark_debug::StarlarkDebugController;
...
impl BuckStarlarkDebuggerServer {
    pub(crate) fn new(
        ...
        Self {
            to_state,
            eval_semaphore: Arc::new(Semaphore::new(slug_util::threads::available_parallelism())),
    ...
    pub(crate) async fn start_eval(
        ...
        description: &str,
    ) -> slug_error::Result<Box<dyn StarlarkDebugController>> {
        ...
        let (hook_id, eval_wrapper) = match recv.await {
            ...
            Err(..) => {
                ...
                // That could be due to the debugger detaching from slug. This does not indicate
    ...
    pub(crate) fn send_request(&self, req: dap::Request) -> slug_error::Result<()> {
    ...
    pub(crate) fn detach(&self) -> slug_error::Result<()> {
...
impl VariableId {
    ...
    pub fn new(top_frame: bool, thread_id: u32, variable_id: u32) -> slug_error::Result<Self> {
        if thread_id > 0xFFFFF {
            return Err(slug_error::slug_error!(
                slug_error::ErrorTag::StarlarkServer,
                "{}",
                format!(
...
impl DebugServer for ServerState {
    ...
    ) -> slug_error::Result<Option<serde_json::Value>> {
    ...
    fn set_breakpoints(
    ...
    ) -> slug_error::Result<dap::SetBreakpointsResponseBody> {
        ...
        let resolved = resolve_breakpoints(&x, &self.get_ast(&project_relative)?)
            .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::StarlarkServer))?;
    ...
    fn attach(&mut self, _x: dap::AttachRequestArguments) -> slug_error::Result<()> {
        Ok(())
    }

    fn threads(&mut self) -> slug_error::Result<dap::ThreadsResponseBody> {
    ...
    fn configuration_done(&mut self) -> slug_error::Result<()> {
        Ok(())
    }
    ...
    ) -> slug_error::Result<dap::StackTraceResponseBody> {
    ...
    fn scopes(&mut self, x: dap::ScopesArguments) -> slug_error::Result<dap::ScopesResponseBody> {
    ...
    ) -> slug_error::Result<dap::VariablesResponseBody> {
    ...
    fn source(&mut self, _x: dap::SourceArguments) -> slug_error::Result<dap::SourceResponseBody> {
        Err(StarlarkDebuggerError::Unimplemented.into())
    }

    fn continue_(&mut self, x: ContinueArguments) -> slug_error::Result<dap::ContinueResponseBody> {
        let hook = self.find_hook_by_pseudo_thread(x.thread_id)?;
        hook.adapter
            .continue_()
            .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::StarlarkServer))?;
    ...
    fn next(&mut self, x: dap::NextArguments) -> slug_error::Result<()> {
        let hook = self.find_hook_by_pseudo_thread(x.thread_id)?;
        hook.adapter
            .step(StepKind::Over)
            .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::StarlarkServer))?;
    ...
    fn step_in(&mut self, x: dap::StepInArguments) -> slug_error::Result<()> {
        let hook = self.find_hook_by_pseudo_thread(x.thread_id)?;
        hook.adapter
            .step(StepKind::Into)
            .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::StarlarkServer))?;
    ...
    fn step_out(&mut self, x: dap::StepOutArguments) -> slug_error::Result<()> {
        let hook = self.find_hook_by_pseudo_thread(x.thread_id)?;
        hook.adapter
            .step(StepKind::Out)
            .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::StarlarkServer))?;
    ...
    ) -> slug_error::Result<dap::EvaluateResponseBody> {
    ...
    fn disconnect(&mut self, _x: dap::DisconnectArguments) -> slug_error::Result<()> {
        Ok(())
    }
}
...
impl ServerState {
    ...
    fn get_snapshot(&self) -> slug_data::DebugAdapterSnapshot {
        ...
        for hook_state in self.current_hooks.values() {
            if let Some(v) = &hook_state.stopped_at {
                let handle_snapshot = current_handles
                    ...
                    .or_insert_with(|| slug_data::DebugAdapterCommandSnapshot {
                        stopped_evals: Vec::new(),
                    });
                handle_snapshot
                    .stopped_evals
                    .push(slug_data::DebugAdapterStoppedEval {
        ...
        slug_data::DebugAdapterSnapshot {
    ...
    fn handle_message(&mut self, msg: ServerMessage) -> slug_error::Result<bool> {
        match msg {
            ServerMessage::NewHook {
                ...
                response_channel.send(resp).map_err(|_| {
                    slug_error::slug_error!(slug_error::ErrorTag::StarlarkServer, "channel closed")
                })?;
            }
    ...
    fn new_hook(
        ...
        handle: BuckStarlarkDebuggerHandle,
        description: String,
    ) -> slug_error::Result<(HookId, Option<Box<dyn DapAdapterEvalHook>>)> {
    ...
    fn drop_hook(&mut self, hook_id: HookId) -> slug_error::Result<()> {
    ...
    fn eval_stopped(&mut self, hook_id: HookId) -> slug_error::Result<()> {
    ...
    fn find_hook_by_pseudo_thread(&self, thread_id: i64) -> slug_error::Result<&HookState> {
    ...
    fn get_ast(&self, source: &ProjectRelativePath) -> slug_error::Result<AstModule> {
