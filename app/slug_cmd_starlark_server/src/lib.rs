...
use slug_cmd_starlark_client::StarlarkSubcommand;
...
use slug_server_ctx::late_bindings::StarlarkServerCommand;
...
impl StarlarkServerCommand for StarlarkServerCommandImpl {
    async fn starlark(
        ...
        let start_event = ctx
            .command_start_event(slug_data::StarlarkCommandStart {}.into())
            .await?;
        span_async(
            start_event,
...
pub(crate) trait StarlarkServerSubcommand: Send + Sync + 'static {
    async fn server_execute(
        ...
        stdout: PartialResultDispatcher<slug_cli_proto::StdoutBytes>,
...
async fn server_starlark_command_inner(
    ...
    let result = parse_command_and_execute(context, partial_result_dispatcher, req)
        .await
        .map_err(Into::into);
    let end_event = command_end(&result, slug_data::StarlarkCommandEnd {});
