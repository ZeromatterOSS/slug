...
use slug_cmd_starlark_client::typecheck::StarlarkTypecheckCommand;
...
impl Cache<'_> {
    async fn typecheck(&mut self, path: OwnedStarlarkPath) -> slug_error::Result<()> {
        self.run(path).await?;
        Ok(())
    }
    ...
    async fn get(&mut self, path: OwnedStarlarkModulePath) -> slug_error::Result<Interface> {
    ...
    #[async_recursion]
    async fn run(&mut self, path: OwnedStarlarkPath) -> slug_error::Result<Interface> {
