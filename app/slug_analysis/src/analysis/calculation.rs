...
use slug_build_api::analysis::calculation::EVAL_ANALYSIS_QUERY;
use slug_build_api::analysis::calculation::RULE_ANALYSIS_CALCULATION;
use slug_build_api::analysis::calculation::RuleAnalysisCalculation;
use slug_build_api::analysis::calculation::RuleAnalysisCalculationImpl;
use slug_build_api::build::detailed_aggregated_metrics::dice::HasDetailedAggregatedMetrics;
use slug_build_api::deferred::calculation::DeferredHolder;
...
use slug_events::dispatch::record_root_spans;
...
use slug_node::attrs::attr_type::query::ResolvedQueryLiterals;
...
fn analysis_checkpoint(
    ...
    slug_util::memory_checkpoint::checkpoint(checkpoint, fields.clone());
    tracing::warn!(
        target: "slug_memory",
        checkpoint,
        target_label = %target,
...
impl RuleAnalysisCalculationImpl for RuleAnalysisCalculationInstance {
    async fn get_analysis_result(
        ...
        target: &ConfiguredTargetLabel,
    ) -> slug_error::Result<MaybeCompatible<AnalysisResult>> {
...
) -> slug_error::Result<HashMap<String, Arc<AnalysisQueryResult>>> {
    ...
    for (key, expected_value) in pairs.iter() {
        ...
        let (scalar_actual, list_actual): (Option<String>, Option<Vec<String>>) =
            if let Some(value) = &cfg_value {
                ...
                slug_build_api::interpreter::rule_defs::build_config::get_starlark_flag(
                    &flag_label_str,
                )
            {
...
fn check_config_setting_define_values(
    ...
    for (key, expected_value) in pairs.iter() {
        ...
        match slug_build_api::interpreter::rule_defs::build_config::get_define(&key_str) {
...
fn resolve_bazel_config_value(key: &str, expected: &str) -> bool {
    match key {
        "compilation_mode" => {
            let mode = slug_build_api::interpreter::rule_defs::build_config::get_compilation_mode();
            expected == mode
        }
        "cpu" => {
        ...
        "define" => {
            ...
            if let Some((def_key, def_val)) = expected.split_once('=') {
                slug_build_api::interpreter::rule_defs::build_config::get_define(def_key).as_deref()
                    == Some(def_val)
            } else {
                // Just check if the key exists
                slug_build_api::interpreter::rule_defs::build_config::get_define(expected).is_some()
            }
        }
        "stamp" => {
        ...
        match ctx.compute(key).await {
            Ok(Ok(aspect_value)) => {
                ...
                if let Some(existing) = aspect_results.get(&dep_label) {
                    let merged = slug_build_api::interpreter::rule_defs::provider::collection::merge_provider_collections(
                        existing,
...
/// Load `@slug_builtins//:exports.bzl` via DICE so analysis can reach
/// `rule_implementation_wrapper` and `aspect_implementation_wrapper`.
/// Returns `None` for workspaces where `@slug_builtins` is not
/// registered (legacy non-bzlmod projects without `[external_cells]
/// slug_builtins = bundled`); analysis falls back to direct impl
/// invocation in that case. Made `pub(crate)` so the aspect dispatch
/// path
/// (`super::aspect_calculation`) can reuse the same DICE round-trip.
pub(crate) async fn get_slug_builtins_module(
    ctx: &mut DiceComputations<'_>,
) -> slug_error::Result<Option<starlark::environment::FrozenModule>> {
    ...
    // round-trip entirely. Fresh slug builds always register
    // @slug_builtins via `slug_common::legacy_configs::cells`, but
    // legacy and external-test-cell setups may not.
    if alias_resolver.resolve("slug_builtins").is_err() {
        return Ok(None);
    }

    let kb_cell = match alias_resolver.resolve("slug_builtins") {
        Ok(c) => c,
...
pub async fn get_rule_spec(
    ctx: &mut DiceComputations<'_>,
    func: &StarlarkRuleType,
) -> slug_error::Result<impl RuleSpec + use<>> {
    let module = get_loaded_module(ctx, func).await?;
    let builtins_module = get_slug_builtins_module(ctx).await?;
        ...
        RuleType::Starlark(func) => {
            ...
            let (res, spans) = async_record_root_spans(async {
                ...
                span_async(start_event, async {
                    ...
                    let result: slug_error::Result<_> = try {
                        let result = span_async_simple(
                            slug_data::AnalysisStageStart {
                                stage: Some(slug_data::analysis_stage_start::Stage::EvaluateRule(
                                    (),
                                )),
                            },
                            run_analysis(
...
fn make_analysis_profile(res: &AnalysisResult) -> slug_error::Result<slug_data::AnalysisProfile> {
    let heap = res.providers()?.owner();

    Ok(slug_data::AnalysisProfile {
    ...
    let profile_datas = ctx
        .try_compute_join(all_deps.iter(), |ctx, node| {
            async move {
                ...
                slug_error::Ok(result.profile_data)
            }
            .boxed()
        })
        .await?;
...
mod tests {
    ...
    #[test]
    fn build_setting_lookup_normalization_runtime_miss_ignores_global_alias()
    -> slug_error::Result<()> {
