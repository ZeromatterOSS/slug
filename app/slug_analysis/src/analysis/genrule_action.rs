...
pub struct GenruleAction {
    ...
    /// package's root directory in the genfiles tree, which for slug is the
...
fn expand_genrule_cmd(
    ...
    //
    // For slug this means the rule's `__<target>__/` output root. Compute by
    ...
    //   "" for the main workspace and "external/<cell>" for external cells. Slug
    ...
    //   slug-specific prefix (`buck-out/v2/external_cells/...`); shell patterns
...
/// In Slug, all three resolve to the same artifact path.
    ...
    async fn execute(
        ...
        let req = match &self.shell {
            GenruleShell::Bash => CommandExecutionRequest::new(
                vec![slug_execute::shell::find_bash().to_owned()],
