...
use slug_build_api::analysis::anon_promises_dyn::RunAnonPromisesAccessorPair;
use slug_build_api::analysis::calculation::RuleAnalysisCalculation;
use slug_build_api::analysis::registry::AnalysisRegistry;
use slug_build_api::interpreter::rule_defs::cc_common::CcExpandIfEqual;
use slug_build_api::interpreter::rule_defs::cc_common::CcFeatureFlagSets;
use slug_build_api::interpreter::rule_defs::cc_common::CcFlagGroup;
use slug_build_api::interpreter::rule_defs::cc_common::CcFlagSet;
use slug_build_api::interpreter::rule_defs::cc_common::CcToolchainFeatures;
use slug_build_api::interpreter::rule_defs::cc_common::CcWithFeatureSet;
...
use slug_events::dispatch::Span as DispatchSpan;
use slug_events::dispatch::get_dispatcher;
use slug_events::dispatch::span_simple as dispatch_span_simple;
...
use slug_interpreter::types::rule::FROZEN_PROMISE_ARTIFACT_MAPPINGS_GET_IMPL;
use slug_interpreter::types::rule::FROZEN_RULE_GET_IMPL;
use slug_interpreter_for_build::rule::FrozenStarlarkRuleCallable;
...
use slug_node::rule_type::StarlarkRuleType;
...
/// `ExternalRunnerTestInfo` so Slug's test runner can execute them.
///
/// This bridges the gap between Bazel (where test rules are marked with `test=True`
/// and the executable comes from `DefaultInfo`) and Buck2/Slug (where test targets
...
fn maybe_inject_test_info<'v>(
    ...
    let (actual_list_res, is_struct) = if ListRef::from_value(list_res).is_none() {
        if let Some(pv) = slug_build_api::interpreter::rule_defs::provider::collection::extract_providers_from_struct(list_res) {
            (pv, true)
        } else {
            return Ok(list_res);
        }
...
///
/// Slug rule impls otherwise get an empty DefaultInfo auto-injected by
...
fn maybe_inject_implicit_default_info<'v>(
    ...
    ctx: starlark::values::ValueTyped<
        'v,
        slug_build_api::interpreter::rule_defs::context::AnalysisContext<'v>,
    >,
    list_res: Value<'v>,
    ...
    let default_info = slug_build_api::interpreter::rule_defs::provider::builtin::default_info::DefaultInfo::with_default_outputs(
        heap,
        artifacts,
    );
...
impl<'v> AttrResolutionContext<'v> for &'_ RuleAnalysisAttrResolutionContext<'v> {
    ...
    fn resolve_unkeyed_placeholder(
        &mut self,
        name: &str,
    ) -> slug_error::Result<Option<FrozenCommandLineArg>> {
...
pub fn resolve_query(
    ...
    query: &str,
    module: &Module,
) -> slug_error::Result<Arc<AnalysisQueryResult>> {
...
pub trait RuleSpec: Sync {
    ...
    fn promise_artifact_mappings<'v>(
        &self,
        eval: &mut Evaluator<'v, '_, '_>,
    ) -> slug_error::Result<SmallMap<String, Value<'v>>>;
...
fn eager_toolchain_loading_checkpoint(
    ...
    checkpoint_fields.extend(fields);
    slug_util::memory_checkpoint::checkpoint("analysis_eager_toolchain_loading", checkpoint_fields);
    tracing::warn!(
        target: "slug_memory",
            ...
            async move {
                let eval_result = match ctx.get_interpreter_results(package_label.dupe()).await {
                    ...
                    Err(e) => {
                        ...
                        return Ok::<_, slug_error::Error>((tc_label_str, Vec::new()));
                    }
                };
...
fn diagnostic_summary(error: &slug_error::Error) -> String {
...
/// when registered transitively. These back slug's bundled toolchains
...
fn should_eager_load_registered_toolchain(tc: &slug_bzlmod::RegisteredToolchain) -> bool {
...
fn collect_target_exec_properties(
    node: slug_node::nodes::configured::ConfiguredTargetNodeRef<'_>,
...
fn extract_toolchain_info_from_node(
    target_node: slug_node::nodes::unconfigured::TargetNodeRef<'_>,
...
/// `@aspect_bazel_lib+toolchains+coreutils_linux_amd64`. Slug does not yet carry
...
fn canonicalize_extension_sibling_label(owner_label: &str, dep_label: &str) -> String {
    ...
    let Some((module, extension, _owner_internal)) = slug_bzlmod::parse_canonical_name(owner_repo)
    else {
        return dep_label.to_owned();
    };
...
impl<'a> MetadataLabelContext<'a> {
    ...
    fn process_global_canonical_dynamic_extension_cell_name(
        ...
        self.allow_process_global_fallbacks
            .then(|| slug_core::cells::canonical_test_dynamic_extension_cell_name(cell_name))
            .flatten()
    }
    ...
    fn process_global_scoped_dynamic_extension_cell_name(
        ...
        slug_core::cells::resolve_test_scoped_bzlmod_repo_alias_for_current_cell(
            owner_cell,
            apparent_cell,
        )
        .or_else(|| {
            metadata_owner_scoped_repo_candidates(owner_cell)
                ...
                .find_map(|owner_module| {
                    slug_core::cells::resolve_test_scoped_bzlmod_repo_alias(
                        &owner_module,
                        apparent_cell,
                    )
                })
...
fn metadata_canonicalize_label_for_owner(
    ...
    if let Some(canonical_cell) =
        metadata_ctx.scoped_dynamic_extension_cell_name(owner_cell, apparent_cell)
    {
...
fn run_analysis_with_env<'a, 'd: 'a>(
    ...
    node: ConfiguredTargetNodeRef<'a>,
) -> impl Future<Output = slug_error::Result<AnalysisResult>> + 'a + Captures<'d> {
...
async fn run_analysis_with_env_underlying(
    ...
    BuckStarlarkModule::with_profiling_async(|env| async move {
        ...
        let (attributes, plugins) = dispatch_span_simple::<
            _,
            slug_data::AnalysisStageEnd,
            _,
            slug_error::Result<_>,
        >(
            slug_data::AnalysisStageStart {
        ...
        let configure_span = DispatchSpan::start(
            get_dispatcher(),
            slug_data::AnalysisStageStart {
                stage: Some(slug_data::analysis_stage_start::Stage::Configure(())),
            },
        );
        ...
        configure_span.end(slug_data::AnalysisStageEnd {});
        ...
        let (ctx, list_res) = reentrant_eval.with_evaluator(|mut eval| {
            ...
            if !exec_group_resolution_results.is_empty() {
                use slug_build_api::interpreter::rule_defs::context::ResolvedExecGroups;
            ...
            >(
                slug_data::AnalysisStageStart {
                    stage: Some(slug_data::analysis_stage_start::Stage::RunImpl(())),
                },
        ...
        // `slug test --include=small` works for Bazel tests tagged with `tags = ["small"]`.
...
pub fn get_user_defined_rule_spec(
    ...
    struct Impl {
        ...
        toolchain_types: Vec<(String, bool)>,
        exec_group_defs: Vec<(String, slug_node::rule::ExecGroupDef)>,
        /// Bundled `@slug_builtins//:exports.bzl`. If it exposes a
        ...
        /// migrated `ctx`-method bodies. None means @slug_builtins
    ...
    impl RuleSpec for Impl {
        fn invoke<'v>(
            ...
            slug_interpreter_for_build::subrule::set_current_rule_ctx_raw(ctx_bits);
            ...
            if let Some(wrapper) = self.lookup_subrule_implementation_wrapper(eval)? {
                ...
                slug_build_api::interpreter::rule_ctx_storage::set_current_subrule_wrapper_raw(
                    wrapper_bits,
                );
            }
            ...
            impl Drop for CtxGuard {
                fn drop(&mut self) {
                    slug_interpreter_for_build::subrule::clear_current_rule_ctx();
                    slug_build_api::interpreter::rule_ctx_storage::clear_current_subrule_wrapper();
                }
            }
            let _guard = CtxGuard;
            ...
            // `_make_rule_facade` in `@slug_builtins//:exports.bzl`).
    ...
    impl Impl {
        ...
        /// `@slug_builtins//:exports.bzl`. Returns `None` when the
        /// workspace doesn't have `@slug_builtins` registered (legacy
        ...
        fn lookup_rule_implementation_wrapper<'v>(
            &self,
            eval: &mut Evaluator<'v, '_, '_>,
        ) -> slug_error::Result<Option<Value<'v>>> {
...
mod tests {
    ...
    -> slug_error::Result<()> {
        ...
        let owner = "plan61_owner++metadata+owner";
        let canonical = "plan61_owner++metadata+scoped";
        let wrong_global = "plan61_wrong_owner++metadata+scoped";
        ...
        let resolver = test_bzlmod_cell_resolver(snapshot)?;
        ...
        assert_eq!(resolved.pkg().cell_name(), CellName::testing_new(canonical));
        assert_eq!(
            metadata_canonicalize_label_for_owner(
                &owner_label,
                label,
    ...
    #[test]
    fn metadata_owner_scoped_alias_runtime_miss_ignores_global_alias() -> slug_error::Result<()> {
        let apparent = "plan61_metadata_scoped_miss";
        let owner = "plan61_owner++metadata+owner_miss";
        let wrong_global = "plan61_wrong_owner++metadata+scoped_miss";
        slug_core::cells::register_test_scoped_bzlmod_repo_alias(
            "plan61_owner+".to_owned(),
            apparent.to_owned(),
            wrong_global.to_owned(),
    ...
    fn test_parse_impl_label_to_target_label() {
        ...
        // No @ prefix (Slug internal format)
        let label = parse_impl_label_to_target_label("local_config_cc//:cc-compiler-k8");
        assert!(label.is_some());
        let label = label.unwrap();
        assert_eq!(label.name().as_str(), "cc-compiler-k8");
    ...
    fn test_root_extension_toolchains_are_eager() {
        let root_extension = slug_bzlmod::RegisteredToolchain {
        ...
        let transitive_extension = slug_bzlmod::RegisteredToolchain {
        ...
        let bundled_transitive = slug_bzlmod::RegisteredToolchain {
