...
use slug_build_api::interpreter::rule_defs::provider::builtin::run_info::RunInfoCallable;
use slug_build_api::interpreter::rule_defs::provider::builtin::template_placeholder_info::FrozenTemplatePlaceholderInfo;
use slug_build_api::interpreter::rule_defs::resolved_macro::ResolvedMacro;
use slug_build_api::interpreter::rule_defs::resolved_macro::ResolvedStringWithMacros;
use slug_build_api::interpreter::rule_defs::resolved_macro::ResolvedStringWithMacrosPart;
...
use slug_node::attrs::attr_type::arg::ConfiguredMacro;
use slug_node::attrs::attr_type::arg::ConfiguredStringWithMacros;
use slug_node::attrs::attr_type::arg::ConfiguredStringWithMacrosPart;
...
fn resolve_configured_macro<'v>(
    ...
    ctx: &mut dyn AttrResolutionContext<'v>,
    pkg: PackageLabel,
) -> slug_error::Result<ResolvedMacro<'v>> {
    match configured_macro {
        ConfiguredMacro::Location { label, .. } => {
            ...
            let providers_value = ctx.get_dep(label)?.providers;
        ...
        ConfiguredMacro::Exe { label, .. } => {
            // Don't need to consider exec_dep as it already was applied when configuring the label.
            let providers = ctx.get_dep(label)?.providers;
