...
use slug_node::attrs::attr_type::configuration_dep::ConfigurationDepAttrType;
use slug_node::attrs::attr_type::configured_dep::ExplicitConfiguredDepAttrType;
use slug_node::attrs::attr_type::dep::DepAttrType;
use slug_node::attrs::attr_type::source::SourceAttrType;
use slug_node::attrs::attr_type::split_transition_dep::SplitTransitionDepAttrType;
    ...
    match (&attr_type.0.inner, this) {
        ...
        (AttrTypeInner::OneOf(oneof_ty), ConfiguredAttr::OneOf(inner, i)) => {
            let inner_ty = oneof_ty
                .xs
                .get(*i as usize)
                .ok_or_else(|| slug_error::internal_error!("oneof index ({}) out of bounds", i))?;
...
fn selected_source_variant_is_dependency_attr(
    oneof_ty: &slug_node::attrs::attr_type::one_of::OneOfAttrType,
    selected: u32,
) -> slug_error::Result<bool> {
    let selected_ty = oneof_ty
        .xs
        .get(selected as usize)
        .ok_or_else(|| slug_error::internal_error!("oneof index ({}) out of bounds", selected))?;
