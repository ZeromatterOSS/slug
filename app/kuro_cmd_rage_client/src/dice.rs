...
use kuro_cli_proto::UnstableDiceDumpRequest;
...
use kuro_fs::fs_util::create_dir_all;
use kuro_fs::fs_util::remove_all;
...
impl Drop for DiceDump {
    fn drop(&mut self) {
        if let Err(e) = remove_all(&self.dump_folder).with_buck_error_context(|| {
            format!(
                "Failed to remove Kuro DICE dump folder at `{}`. Please remove this manually as it could be quite large.",
                self.dump_folder.display()
            )
        }) { tracing::warn!("{:#}", e); };
    }
