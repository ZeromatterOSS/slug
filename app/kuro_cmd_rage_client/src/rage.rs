...
use kuro_client_ctx::thread_dump::thread_dump_command;
use kuro_client_ctx::upload_re_logs::upload_re_logs;
...
use kuro_data::RageResult;
use kuro_data::instant_event::Data;
use kuro_error::BuckErrorContext;
use kuro_event_log::file_names::do_find_log_by_trace_id;
use kuro_event_log::file_names::get_local_logs;
use kuro_event_log::read::EventLogPathBuf;
use kuro_event_log::read::EventLogSummary;
use kuro_events::BuckEvent;
use kuro_events::sink::remote::RemoteEventSink;
...
#[clap(
    name = "rage",
    about = "Record information about the previous failed kuro command",
...
pub struct RageCommand {
    ...
    /// Collect rage report about kuro in general, not about specific invocation
    #[clap(long, group = "invocation")]
    no_invocation: bool,
...
impl RageCommand {
    ...
    async fn exec_impl(self, mut ctx: ClientCommandContext<'_>) -> kuro_error::Result<()> {
        let paths = ctx.paths()?;
        ...
        let info = BuckdProcessInfo::load(&daemon_dir).map_err(kuro_error::Error::from);
        ...
        kuro_client_ctx::eprintln!("Collecting debug info...")?;
    ...
    async fn send_to_scuba(
        ...
        let kuro_revision = build_info.get_field(|o| Some(o.kuro_revision.to_owned()));
        ...
        insert_if_some(&mut string_data, "kuro_revision", kuro_revision.clone());
    ...
    where
        Fut: Future<Output = kuro_error::Result<T>> + 'a,
        T: std::fmt::Display + 'a,
    {
...
fn create_scribe_sink(ctx: &ClientCommandContext) -> kuro_error::Result<Option<RemoteEventSink>> {
...
async fn maybe_select_invocation(
    ...
    command: &RageCommand,
) -> kuro_error::Result<Option<EventLogPathBuf>> {
...
async fn user_prompt_select_log<'a>(
...
) -> kuro_error::Result<usize> {
    kuro_client_ctx::eprintln!("Which buck invocation would you like to report?\n")?;
    ...
    kuro_client_ctx::eprintln!("Selected invocation {}\n", selection)?;
    Ok(selection)
}
...
async fn output_rage(no_paste: bool, output: &str) -> kuro_error::Result<()> {
    if no_paste {
        kuro_client_ctx::println!("{}", output)?;
    } else {
        match generate_paste("Kuro Rage", output).await {
            Err(e) => {
                kuro_client_ctx::eprintln!(
            ...
            Ok(paste) => kuro_client_ctx::eprintln!(
                "\nPlease post in https://fb.workplace.com/groups/kurousers with the following link:\n\n{}\n",
                paste
            )?,
        }
    };
...
async fn generate_paste(title: &str, content: &str) -> kuro_error::Result<String> {
...
async fn upload_thread_dump(
    buckd: &kuro_error::Result<BuckdProcessInfo<'_>>,
    manifold: &ManifoldClient,
    manifold_id: &String,
) -> kuro_error::Result<String> {
