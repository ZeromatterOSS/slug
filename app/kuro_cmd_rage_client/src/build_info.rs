...
impl fmt::Display for BuildInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "kuro UI: https://www.internalfb.com/kuro/{}
timestamp: {}
command: {}
working dir: {}
kuro_revision: {}
...
pub(crate) async fn get(log: &EventLogPathBuf) -> kuro_error::Result<BuildInfo> {
    ...
    loop {
        ...
        if let Err(e) = res {
            kuro_client_ctx::eprintln!("Error found when iterating through logs: {:#}", e)?;
            break;
        }
    }
    ...
    let output = BuildInfo {
        ...
        kuro_revision: info.revision.unwrap_or_else(|| "".to_owned()),
...
fn extract_info(info: &mut LogInfo, event: Box<kuro_data::BuckEvent>) -> kuro_error::Result<()> {
    match event.data {
        Some(kuro_data::buck_event::Data::SpanStart(span)) => match &span.data {
            Some(kuro_data::span_start_event::Data::Command(action)) => {
                if info.revision.is_none() && action.metadata.contains_key("kuro_revision") {
                    if let Some(kuro_revision) = action.metadata.get("kuro_revision") {
                        info.revision.get_or_insert(kuro_revision.clone());
                    }
                }
            }
            _ => (),
        },
        Some(kuro_data::buck_event::Data::Instant(span)) => match &span.data {
            Some(kuro_data::instant_event::Data::Snapshot(snapshot)) => {
                info.daemon_uptime_s.get_or_insert(snapshot.daemon_uptime_s);
            }
            Some(kuro_data::instant_event::Data::ReSession(session)) => {
