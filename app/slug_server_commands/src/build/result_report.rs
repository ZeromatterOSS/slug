...
use slug_build_api::build::BuildProviderType;
use slug_build_api::build::BuildTargetResult;
use slug_build_api::build::ConfiguredBuildTargetResult;
use slug_build_api::build::ProviderArtifacts;
...
mod proto {
    pub(crate) use slug_cli_proto::BuildTarget;
    pub(crate) use slug_cli_proto::build_target::BuildOutput;
    pub(crate) use slug_cli_proto::build_target::build_output::BuildOutputProviders;
}

/// Simple container for multiple [`slug_error::Error`]s
pub(crate) struct BuildErrors {
    pub(crate) errors: Vec<slug_error::Error>,
}
...
/// Collects build results into a Result<Vec<proto::BuildTarget>, slug_error::Errors>. If any targets
...
impl<'a> ResultReporter<'a> {
    ...
    ) -> slug_error::Result<BuildTargetsAndErrors> {
        let mut out = Self {
            artifact_fs,
            options,
            results: Vec::new(),
