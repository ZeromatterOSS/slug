...
use slug_build_api::interpreter::rule_defs::provider::builtin::install_info::FrozenInstallInfo;
...
use slug_build_api::materialize::materialize_and_upload_artifact_group;
use slug_build_api::validation::validation_impl::VALIDATION_IMPL;
use slug_cli_proto::InstallRequest;
use slug_cli_proto::InstallResponse;
...
use slug_common::manifold::Bucket;
use slug_common::manifold::ManifoldClient;
use slug_common::manifold::Ttl;
...
use slug_core::target::name::TargetName;
use slug_data::BuildResult;
use slug_data::InstallEventInfoEnd;
use slug_data::InstallEventInfoStart;
...
use slug_events::dispatch::span_async_simple;
...
use slug_install_proto::DeviceMetadata;
use slug_install_proto::FileReadyRequest;
use slug_install_proto::InstallInfoRequest;
use slug_install_proto::ShutdownRequest;
use slug_install_proto::installer_client::InstallerClient;
...
pub(crate) async fn install_command(
    ...
    req: InstallRequest,
) -> slug_error::Result<InstallResponse> {
...
impl ServerCommandTemplate for InstallServerCommand {
    type StartEvent = slug_data::InstallCommandStart;
    type EndEvent = slug_data::InstallCommandEnd;
    type Response = InstallResponse;
    type PartialResult = NoPartialResult;

    fn end_event(&self, _response: &slug_error::Result<Self::Response>) -> Self::EndEvent {
        slug_data::InstallCommandEnd {
...
async fn collect_install_request_data<'a>(
    ...
    request: &InstallRequest,
) -> slug_error::Result<impl IntoIterator<Item = InstallRequestData<'a>> + use<'a>> {
...
fn get_random_tcp_port() -> slug_error::Result<u16> {
...
impl<'a> ConnectedInstaller<'a> {
    async fn connect(
        ...
        let timeout =
            Duration::from_secs(slug_env!("BUCK2_INSTALLER_TIMEOUT_S", type=u64)?.unwrap_or(120));
        let send_timeout = Duration::from_secs(
            slug_env!("BUCK2_INSTALLER_SEND_TIMEOUT_S", type=u64)?.unwrap_or(300),
        );

        let client: slug_error::Result<InstallerClient<Channel>> = span_async_simple(
            slug_data::ConnectToInstallerStart {
            ...
            slug_data::ConnectToInstallerEnd {},
        )
        .await;
    ...
    fn install_result(self, result: slug_error::Result<()>) -> InstallResult {
        InstallResult {
    ...
    async fn send_install_info(&mut self) -> slug_error::Result<()> {
        for (installed_target, install_files) in &self.install_request_data.installed_targets {
            ...
            if install_info_response.install_id != install_id {
                ...
                return Err(slug_error::slug_error!(
                    slug_error::ErrorTag::InstallIdMismatch,
    ...
    async fn send_shutdown_command(&self) -> slug_error::Result<()> {
    ...
    async fn send_file(&self, file: FileResult) -> slug_error::Result<()> {
        ...
        span_async(start, async {
            let mut outcome: slug_error::Result<()> = Ok(());
            ...
            if let Some(error_detail) = response.error_detail {
                let mut error: slug_error::Error = InstallError::ProcessingFileReadyFailure {
                    install_id: install_id.to_owned(),
                    artifact: name.to_owned(),
                    path: path.to_owned(),
                ...
                let category_tag = if let Ok(category) =
                    slug_install_proto::ErrorCategory::try_from(error_detail.category)
                {
                    match category {
                        slug_install_proto::ErrorCategory::Unspecified => {
                            ErrorTag::InstallerUnknown
                        }
                        slug_install_proto::ErrorCategory::Tier0 => ErrorTag::InstallerTier0,
                        slug_install_proto::ErrorCategory::Input => ErrorTag::InstallerInput,
                        slug_install_proto::ErrorCategory::Environment => {
...
async fn handle_install_request<'a>(
        ...
        .try_compute2(
            ...
            |ctx| {
                async move {
                    ...
                    // 1. slug passes a temp file for a tcp port output.
                    // 2. installer app choose unused tcp port and writes it into the passed file.
                    // 3. slug reads tcp port from file and use it to connect to the installer app. (`connect_to_installer` function)
                    ...
                    slug_error::Ok(installer.install(files_rx).await)
                }
                .boxed()
            },
        )
    ...
    let device_metadata: Vec<slug_data::DeviceMetadata> = device_metadata
        .lock()
        .await
        .iter()
        .map(|metadata| slug_data::DeviceMetadata {
            entry: metadata
                .entry
                .iter()
                .map(|e| slug_data::device_metadata::Entry {
                    key: e.key.clone(),
                    value: e.value.clone(),
                })
                .collect(),
    ...
    get_dispatcher().instant_event(slug_data::InstallFinished {
...
async fn upload_installer_logs(log_path: &AbsNormPathBuf) -> slug_error::Result<String> {
...
fn get_stdio(log_installer_console: bool) -> slug_error::Result<Stdio> {
