...
use slug_build_api::build::build_report::initialize_streaming_build_report;
use slug_build_api::build::build_report::stream_build_report;
use slug_build_api::build::build_report::write_build_report;
use slug_build_api::build::detailed_aggregated_metrics::types::DetailedAggregatedMetrics;
use slug_build_api::build::graph_properties::GraphPropertiesOptions;
...
use slug_cli_proto::build_request::Uploads;
use slug_cli_proto::build_request::build_providers::Action as BuildProviderAction;
...
impl ServerCommandTemplate for BuildServerCommand {
    type StartEvent = slug_data::BuildCommandStart;
    type EndEvent = slug_data::BuildCommandEnd;
    type Response = slug_cli_proto::BuildResponse;
...
fn expect_build_opts(req: &slug_cli_proto::BuildRequest) -> &CommonBuildOptions {
...
async fn build(
    ...
    request: &slug_cli_proto::BuildRequest,
) -> slug_error::Result<slug_cli_proto::BuildResponse> {
...
async fn process_streaming_build_result(
    ...
    let cwd: &slug_core::fs::project_rel_path::ProjectRelativePath = server_ctx.working_dir();
    let cell_resolver = ctx.get_cell_resolver().await?;
...
async fn maybe_stream_build_reports(
    build_future: impl std::future::Future<Output = slug_error::Result<BuildTargetResult>>,
...
) -> slug_error::Result<BuildTargetResult> {
    if build_opts
        .unstable_streaming_build_report_filename
        .is_empty()
    {
...
async fn process_build_result(
    ...
    Ok(slug_cli_proto::BuildResponse {
        build_targets,
        project_root,
...
async fn build_targets_in_universe(
    ...
    provider_labels
        .into_iter()
        .map(|p| {
            slug_util::async_move_clone!(providers_to_build, {
