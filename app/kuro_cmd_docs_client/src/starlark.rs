...
impl StreamingCommand for DocsStarlarkCommand {
    ...
    async fn exec_impl(
        ...
        let response = buckd
            ...
            .new_generic(
                client_context,
                kuro_cli_proto::new_generic::NewGenericRequest::Docs(DocsRequest::Starlark(
                    DocsStarlarkRequest {
        ...
        let kuro_cli_proto::new_generic::NewGenericResponse::Docs(response) = response else {
            return kuro_error!(
        ...
        if let Some(json_output) = response.json_output {
            kuro_client_ctx::println!("{}", json_output.trim_end())?;
        }
