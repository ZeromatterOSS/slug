...
use kuro_query::query::syntax::simple::functions::description::QUERY_ENVIRONMENT_DESCRIPTION_BY_TYPE;
...
impl OutputFormatOptions {
    fn emit_markdown(&self, markdown: &str) -> kuro_error::Result<()> {
        match self.format {
            OutputFormatArg::Markdown => {
                kuro_client_ctx::println!("{}", markdown)?;
            }
            OutputFormatArg::Rendered => {
                ...
                kuro_client_ctx::println!("{}", rendered)?;
            }
        }
