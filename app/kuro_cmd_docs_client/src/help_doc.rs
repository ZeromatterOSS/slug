...
#[clap(
    ...
    about = "Print out the kuro subcommand markdown docs"
)]
pub(crate) struct MarkdownHelpDocCommand {
    #[clap(
        name = "SUB_COMMAND",
        help = "The kuro sub command we want to generate the markdown doc"
    )]
    sub_cmd: String,
}
...
fn cmd_markdown_help(cmd: &Command) -> String {
    ...
    let content = cmd_content_markdown(cmd, vec!["kuro".to_owned()]).unwrap();
    format!("{header}\n{content}")
}
...
fn generate_common_options_doc(cmd: &Command) -> String {
    ...
    writeln!(markdown, "\nThis document provides an overview of common options that are available across multiple kuro commands.\n").unwrap();
...
fn cmd_header_markdown(cmd: &clap::Command) -> String {
    ...
    let contents = if name == "install" {
        "\
The `kuro install` command builds an installable target, typically a mobile app (.apk or .app bundle), and installs it using an installer server to some location, typically an emulator/simulator or external device.

## How kuro install works

The `InstallInfo` provider is used to make targets installable, it specifies an installer implementation (e.g. Android or Apple installer) and a set of files to install. For example (from `kuro audit providers`):
```python
InstallInfo(
...
".to_owned()
        ...
        format!(
            "This document provides an overview of the commands and options available under `kuro {name}`."
        )
    };
