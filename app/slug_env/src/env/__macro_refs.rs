...
pub fn convert_from_str<T>(v: &str) -> slug_error::Result<T>
where
    T: FromStr,
...
/// This macro is used to register environment variables that are used by Slug.
///
...
///    for self-testing of slug
///
/// The macro expands to an expression of type `slug_error::Result<Type>` if a default is set, and
/// `slug_error::Result<Option<Type>` otherwise.
pub macro slug_env {
    ($var:expr, bool $(, $($rest:tt)*)?) => {{
        let v: slug_error::Result<bool> = $crate::env::__macro_refs::slug_env!($var, type=bool, default=false, converter = |s| {
            match s.to_lowercase().as_str() {
                "1" | "true" => Ok(true),
                "0" | "false" => Ok(false),
                _ => Err(slug_error::slug_error!(slug_error::ErrorTag::Tier0, "Invalid bool value: {}", s).into()),
            }
        }, $($($rest)*)?);
        v
    }},
...
/// Register env name to be shown in `slug help-env`.
pub macro slug_env_name($var:expr) {{
...
#[allow(unused_macros)]
/// `parser` is `&str -> slug_error::Result<$stored_type>`, `processor` is `Option<& $stored_type> -> $output_type`
///
...
macro expand(
    ...
    let v: slug_error::Result<$output_ty> = ENV_HELPER.get().map($processor);
    v
}}
