...
type StructuredErrorHandler = Box<
    dyn for<'a> Fn(&'a str, &slug_error::Error, (&'a str, u32, u32), StructuredErrorOptions)
        + Send
        + Sync
        + 'static,
>;
...
pub fn slug_hard_error_env() -> slug_error::Result<Option<&'static str>> {
    slug_env!("BUCK2_HARD_ERROR")
}
...
/// * The error is a `slug_error::Error`.
///
/// Soft errors from Meta internal runs can be viewed
/// [in logview](https://www.internalfb.com/logview/overview/slug).
///
...
fn hard_error_config() -> slug_error::Result<Arc<HardErrorConfig>> {
    ...
    let config = slug_hard_error_env()?.unwrap_or_default();
...
pub fn reload_hard_error_config(var_value: &str) -> slug_error::Result<()> {
...
pub fn handle_soft_error(
...
) -> Result<slug_error::Error, slug_error::Error> {
    ...
    if is_open_source {
        ...
        let explicitly_disabled = matches!(slug_hard_error_env(), Ok(Some(v)) if v == "false");
...
pub fn initialize(handler: StructuredErrorHandler) -> slug_error::Result<()> {
    hard_error_config()?;
...
impl HardErrorConfigHolder {
    fn reload_hard_error_config(&self, var_value: &str) -> slug_error::Result<()> {
...
pub fn validate_logview_category(category: &str) -> slug_error::Result<()> {
...
pub(crate) mod tests {
    ...
    #[test]
    fn test_hard_error() -> slug_error::Result<()> {
