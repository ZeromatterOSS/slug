...
use kuro_build_api::build::build_report::write_bxl_build_report;
use kuro_build_api::bxl::result::BxlResult;
use kuro_build_api::bxl::result::PendingStreamingOutput;
...
use kuro_cli_proto::BxlRequest;
use kuro_cli_proto::BxlResponse;
...
use kuro_common::target_aliases::HasTargetAliasResolver;
...
use kuro_data::BxlEnsureArtifactsEnd;
use kuro_data::BxlEnsureArtifactsStart;
...
use kuro_server_ctx::bxl::GetBxlStreamingTracker;
...
impl BxlServerCommand {
    ...
    ) -> kuro_error::Result<kuro_cli_proto::BxlResponse> {
    ...
    ) -> kuro_error::Result<BxlCommandContext<'a>> {
    ...
    async fn resolve_cli_args(
        ...
        dice_ctx: &mut DiceTransaction,
    ) -> kuro_error::Result<BxlResolvedCliArgs> {
        let cur_package =
    ...
    async fn materialize_collected_artifacts(
        ...
        let mut errors: Vec<kuro_error::Error> = Vec::new();
    ...
    async fn write_build_report(
        ...
        dice_ctx: &mut DiceTransaction,
        server_ctx: &dyn ServerCommandContextTrait,
        ensured_artifact_errors: Vec<kuro_error::Error>,
        ...
        let serialized_build_report = if bxl_opts.unstable_print_build_report {
            ...
            let build_report_opts = BuildReportOpts {
                // These are all deprecated for `kuro build`, so don't need to support them
...
) -> kuro_error::Result<BxlFunctionLabel> {
