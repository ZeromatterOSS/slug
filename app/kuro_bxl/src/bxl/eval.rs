...
use kuro_data::BxlExecutionEnd;
use kuro_data::BxlExecutionStart;
use kuro_data::StarlarkFailNoStacktrace;
...
pub(crate) struct BxlEvalError {
    ...
    pub(crate) error: kuro_error::Error,
}

pub(crate) type Result<T> = std::result::Result<T, BxlEvalError>;

impl From<kuro_error::Error> for BxlEvalError {
    fn from(value: kuro_error::Error) -> Self {
        Self {
    ...
    let (_, futs) = unsafe {
        ...
        scope_and_collect_with_dice(ctx, |ctx, s| {
            s.spawn_cancellable(
                ...
                || Err(kuro_error!(kuro_error::ErrorTag::Tier0, "cancelled").into()),
            )
        })
    }
    .await;
...
) -> kuro_error::Result<OwnedFrozenValueTyped<FrozenBxlFunction>> {
...
pub(crate) async fn resolve_cli_args<'a>(
    ...
    match frozen_callable
        .to_clap(clap::Command::new(&spec.name).no_binary_name(true)) // patternlint-disable-line kuro-no-command-new
