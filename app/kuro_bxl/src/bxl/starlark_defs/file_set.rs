...
impl<'a> FileSetExpr<'a> {
    pub(crate) async fn get(self, bxl: &BxlContext<'_>) -> kuro_error::Result<Cow<'a, FileSet>> {
        let set = match self {
...
impl OwnedFileSetExpr {
    ...
    ) -> kuro_error::Result<Cow<'a, FileSet>> {
        let set = match self {
...
impl StarlarkReadDirSet {
    fn children(&self) -> kuro_error::Result<Vec<CellPath>> {
