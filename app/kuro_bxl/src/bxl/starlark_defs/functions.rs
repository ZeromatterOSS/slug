...
use kuro_interpreter::types::package_path::StarlarkPackagePath;
use kuro_interpreter_for_build::interpreter::package_file_calculation::EvalPackageFile;
use kuro_interpreter_for_build::super_package::package_value::SuperPackageValuesImpl;
...
#[derive(Debug, kuro_error::Error, Clone)]
#[kuro(tag = Input)]
...
impl<'v> PackagePathArg<'v> {
    fn pkg(&self, cell_alias_resolver: &CellAliasResolver) -> kuro_error::Result<PackageLabel> {
        match self {
...
pub(crate) fn register_read_package_value_function(builder: &mut GlobalsBuilder) {
    ...
    fn read_package_value<'v>(
        ...
        let metadata_key = MetadataKeyRef::new(key).map_err(kuro_error::Error::from)?;
