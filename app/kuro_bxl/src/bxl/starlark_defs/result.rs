...
enum BxlResultError {
    ...
    UnwrapOnError(kuro_error::Error),
...
impl StarlarkError {
    pub(crate) fn new(err: kuro_error::Error) -> Self {
        Self { err }
    }
}
...
pub(crate) enum StarlarkResultGen<T> {
    Ok(T),
    Err(#[freeze(identity)] kuro_error::Error),
}
...
impl<T> StarlarkResultGen<T> {
    pub(crate) fn from_result(res: kuro_error::Result<T>) -> Self {
        match res {
            Ok(val) => Self::Ok(val),
...
impl<'v, V: ValueLike<'v>> StarlarkResultGen<V> {
    fn unwrap(&self) -> kuro_error::Result<Value<'v>> {
        match self {
    ...
    fn unwrap_err(&self) -> kuro_error::Result<StarlarkError> {
        match self {
