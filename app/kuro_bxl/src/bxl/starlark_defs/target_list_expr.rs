...
use kuro_build_api::configure_targets::get_maybe_compatible_targets;
...
pub(crate) fn filter_incompatible<T: ErrorPrinter>(
    ...
    error_printer: &T,
) -> kuro_error::Result<TargetSet<ConfiguredTargetNode>> {
...
impl<'v> TargetListExpr<'v, TargetNode> {
    ...
    ) -> kuro_error::Result<Cow<'v, TargetSet<TargetNode>>> {
        let set = ctx
    ...
    pub(crate) async fn get_one(
        &self,
        ctx: &mut DiceComputations<'_>,
    ) -> kuro_error::Result<Option<TargetNode>> {
        Ok(match &self {
...
impl<'v> TargetListExpr<'v, ConfiguredTargetNode> {
    ...
    ) -> kuro_error::Result<Vec<MaybeCompatible<ConfiguredTargetNode>>> {
    ...
    pub(crate) async fn get_one(
        &self,
        dice: &mut DiceComputations<'_>,
    ) -> kuro_error::Result<Option<MaybeCompatible<ConfiguredTargetNode>>> {
        Ok(match &self {
    ...
    ) -> kuro_error::Result<TargetListExpr<'v, ConfiguredTargetNode>> {
        match arg {
        ...
        match ParsedPattern::<TargetPatternExtra>::parse_relaxed(
            ...
            pattern => {
                ...
                let maybe_compatible: Vec<_> = if keep_going {
                    ...
                    maybe_compatible_iter.collect::<kuro_error::Result<_>>()?
                };
            ...
            ConfiguredTargetListArg::TargetSet(s) => Ok(TargetListExpr::Iterable(
                dice.try_compute_join(s.0.iter(), |dice, node| {
                    async move {
                        ...
                        kuro_error::Ok(TargetExpr::Label(Cow::Owned(
...
impl<'v> TargetListExpr<'v, TargetNode> {
    pub(crate) async fn unpack(
        ...
        ctx: &BxlContext<'_>,
        dice: &mut DiceComputations<'_>,
    ) -> kuro_error::Result<TargetListExpr<'v, TargetNode>> {
        match value {
...
async fn unpack_string_literal(
    ...
    dice: &mut DiceComputations<'_>,
) -> kuro_error::Result<SingleOrCompatibleConfiguredTargets> {
    match ParsedPattern::<TargetPatternExtra>::parse_relaxed(
        ctx.target_alias_resolver(),
        // TODO(nga): Parse relaxed relative to cell root is incorrect.
        CellPathRef::new(ctx.cell_name(), CellRelativePath::empty()),
        ...
        pattern => {
            ...
            let maybe_compatible = maybe_compatible_iter.collect::<kuro_error::Result<_>>()?;
...
impl OwnedTargetNodeArg {
    ...
    pub(crate) async fn to_unconfigured_target_node(
        ...
        ctx: &BxlContextCoreData,
        dice: &mut DiceComputations<'_>,
    ) -> kuro_error::Result<Either<StarlarkTargetNode, StarlarkTargetSet<TargetNode>>> {
        match self {
    ...
    pub(crate) async fn to_unconfigured_target_set(
        ...
        ctx: &BxlContextCoreData,
        dice: &mut DiceComputations<'_>,
    ) -> kuro_error::Result<StarlarkTargetSet<TargetNode>> {
