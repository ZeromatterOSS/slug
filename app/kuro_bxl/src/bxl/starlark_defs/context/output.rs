...
use kuro_build_api::bxl::build_result::BxlBuildResult;
...
use kuro_server_ctx::bxl::BxlStreamingTracker;
...
impl EnsuredArtifactOrGroup {
    pub(crate) fn into_artifact_groups(self) -> kuro_error::Result<Vec<ArtifactGroup>> {
        match self {
...
impl OutputStreamState {
    ...
    pub(crate) fn take_state(&self) -> kuro_error::Result<OutputStreamOutcome> {
        ...
        let artifacts = state
            ...
            .flatten_ok()
            .collect::<kuro_error::Result<IndexSet<ArtifactGroup>>>()?;
        let pending_streaming_outputs = state
            ...
            .collect::<kuro_error::Result<Vec<(IndexSet<ArtifactGroup>, Vec<u8>)>>>()?;
...
fn output_stream_methods(builder: &mut MethodsBuilder) {
    ...
    /// of a bxl script, which will be displayed to stdout by kuro even when the script is cached.
    ...
    /// stdout by kuro even when the script is cached.
    ///
    /// Prints that are not result of the bxl should be printed via stderr via the stdlib `print`
    /// and `pprint`.
    ///
    ...
    /// It will be displayed to stdout by kuro even when the script is cached.
    /// Accepts an optional separator that defaults to " ".
    ///
        ...
        match artifacts {
            ...
            EnsureMultipleArtifactsArg::EnsuredArtifactArgs(list) => {
                let artifacts: Vec<EnsuredArtifact> = list.items.into_try_map(|artifact| {
                    ...
                    Ok::<EnsuredArtifact, kuro_error::Error>(artifact)
                })?;
                        ...
                        Ok((
                            label.get_hashed().map_err(|e| {
                                from_starlark_with_options(
                                    e,
                                    kuro_error::starlark_error::NativeErrorHandling::Unknown,
                                    false,
                                )
                            })?,
...
) -> kuro_error::Result<StarlarkCommandLineInputs> {
...
) -> kuro_error::Result<Vec<EnsuredArtifact>> {
