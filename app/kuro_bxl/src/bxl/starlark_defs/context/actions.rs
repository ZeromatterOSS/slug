...
) -> kuro_error::Result<BxlExecutionResolution> {
...
impl<'v> BxlActions<'v> {
    ...
    ) -> kuro_error::Result<BxlActions<'v>> {
    ...
    fn is_anon_target_or_dyn_action(&self) -> kuro_error::Result<bool> {
...
async fn alloc_deps<'v, 'c>(
    ...
    frozen_heap: &'v FrozenHeap,
    ctx: &'c mut DiceComputations<'_>,
) -> kuro_error::Result<ValueOfUnchecked<'v, DictType<StarlarkProvidersLabel, Dependency<'v>>>> {
    ...
    let deps: Vec<(StarlarkProvidersLabel, Dependency)> = analysis_results
        ...
        .map(|(configured, analysis_result)| {
            ...
            kuro_error::Ok((starlark_label, dependency))
        })
        .collect::<Result<_, _>>()?;
...
fn bxl_actions_methods(builder: &mut MethodsBuilder) {
    ...
    ) -> starlark::Result<ValueOfUnchecked<'v, DictType<StarlarkProvidersLabel, Dependency<'v>>>>
    {
        if this.is_anon_target_or_dyn_action()? {
            soft_error!(
                ...
                kuro_error!(kuro_error::ErrorTag::Input, "Anon target or dynamic action accesses bxl.Actions.exec_deps."),
                quiet: true
            )?;
        }
    ...
    ) -> starlark::Result<ValueOfUnchecked<'v, DictType<StarlarkProvidersLabel, Dependency<'v>>>>
    {
        if this.is_anon_target_or_dyn_action()? {
            soft_error!(
                "bxl_acessing_exec_platform",
                kuro_error!(kuro_error::ErrorTag::Input, "Anon target or dynamic action accesses bxl.Actions.toolchains."),
                quiet: true
            )?;
        }
        Ok(this.toolchains)
