...
use kuro_analysis::analysis::env::RuleAnalysisAttrResolutionContext;
...
use kuro_node::attrs::display::AttrDisplayWithContext;
...
fn attr_with_stripped_cfg(attr: &ConfiguredAttr) -> kuro_error::Result<CoercedAttr> {
    Ok(match attr {
        ...
        ConfiguredAttr::SplitTransitionDep(dep) => {
            ...
            CoercedAttr::Dep(
                deps.iter()
                    .next()
                    .ok_or_else(|| {
                        kuro_error::internal_error!("deps is empty after checking length")
                    })?
                    .dupe(),
            )
        }
            ...
            let unconfigured_parts = match &arg.string_with_macros {
                ...
                StringWithMacros::ManyParts(parts) => StringWithMacros::ManyParts(
                    parts
                        .iter()
                        .map(|part| match part {
                            kuro_node::attrs::attr_type::arg::StringWithMacrosPart::String(s) => {
                                kuro_node::attrs::attr_type::arg::StringWithMacrosPart::String(
                                    s.clone(),
                                )
                            }
                            kuro_node::attrs::attr_type::arg::StringWithMacrosPart::Macro(
                                write_to_file,
                                macr,
                            ) => kuro_node::attrs::attr_type::arg::StringWithMacrosPart::Macro(
                                *write_to_file,
                                match macr {
                                    kuro_node::attrs::attr_type::arg::MacroBase::Location {
                                        label,
                                        exec_dep,
                                    } => kuro_node::attrs::attr_type::arg::MacroBase::Location {
                                    ...
                                    kuro_node::attrs::attr_type::arg::MacroBase::Exe {
                                        label,
                                        exec_dep,
                                    } => kuro_node::attrs::attr_type::arg::MacroBase::Exe {
                                        label: label.unconfigured(),
                                    ...
                                    kuro_node::attrs::attr_type::arg::MacroBase::UserUnkeyedPlaceholder(
                                        var_name,
                                    ) => {
                                        kuro_node::attrs::attr_type::arg::MacroBase::UserUnkeyedPlaceholder(
                                            var_name.clone(),
                                    ...
                                    kuro_node::attrs::attr_type::arg::MacroBase::UserKeyedPlaceholder(
                                        box_value,
                                    ) => {
                                    ...
                                    kuro_node::attrs::attr_type::arg::MacroBase::Query(
                                        query_macro,
                                    ) => {
                                        kuro_node::attrs::attr_type::arg::MacroBase::Query(
                                            Box::new(
                                                kuro_node::attrs::attr_type::query::QueryMacroBase {
                                                    expansion_type: query_macro
                                                        .expansion_type
                                                        .clone(),
                                                    query: kuro_node::attrs::attr_type::query::QueryAttrBase {
                                                        query: query_macro.query.query.clone(),
                                                        resolved_literals: kuro_node::attrs::attr_type::query::ResolvedQueryLiterals(
                                                            query_macro
                                                                .query
                                                                .resolved_literals
                                                                .0
                                    ...
                                    kuro_node::attrs::attr_type::arg::MacroBase::Source(path) => {
                                        kuro_node::attrs::attr_type::arg::MacroBase::Source(
                                            path.clone(),
                                        )
                                    }
                                    kuro_node::attrs::attr_type::arg::MacroBase::UnrecognizedMacro(
                                        macr,
                                    ) => {
                                        kuro_node::attrs::attr_type::arg::MacroBase::UnrecognizedMacro(
                                            macr.clone(),
...
fn configured_target_node_value_methods(builder: &mut MethodsBuilder) {
    ...
    /// It will not return special attribute (attribute that start with 'buck.' in `kuro cquery -A` command).
    ///
    /// Sample usage:
    /// ```python
    ...
    fn resolved_attrs_eager<'v>(
        ...
        let dep_analysis: kuro_error::Result<Vec<(&ConfiguredTargetLabel, AnalysisResult)>> = ctx
            .via_dice(eval, |ctx| {
    ...
    fn sources(this: &StarlarkConfiguredTargetNode) -> starlark::Result<Vec<StarlarkArtifact>> {
        ...
        impl ConfiguredAttrTraversal for InputsCollector {
            fn dep(&mut self, _dep: &ConfiguredProvidersLabel) -> kuro_error::Result<()> {
                Ok(())
            }

            fn input(&mut self, path: SourcePathRef) -> kuro_error::Result<()> {
                self.inputs
    ...
    fn deps<'v>(
        this: &'v StarlarkConfiguredTargetNode,
        // ) -> kuro_error::Result<Vec<StarlarkConfiguredTargetNode>> {
