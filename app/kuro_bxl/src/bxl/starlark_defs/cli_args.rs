...
impl CliArgType {
    fn coerce_value<'v>(&self, value: Value<'v>) -> kuro_error::Result<CliArgValue> {
        Ok(match self {
    ...
    ) -> BoxFuture<'a, kuro_error::Result<Option<CliArgValue>>> {
        async move {
            Ok(match self {
                CliArgType::Bool => clap.value_of().map_or(Ok(None), |x| {
                    let r: kuro_error::Result<_> = try { CliArgValue::Bool(x.parse::<bool>()?) };
                    r.map(Some)
                })?,
                CliArgType::Int => clap.value_of().map_or(Ok(None), |x| {
                    let r: kuro_error::Result<_> = try {
                        CliArgValue::Int(
                            x.parse::<BigInt>()
...
mod tests {
    ...
    use kuro_core::provider::label::ProvidersLabel;
    use kuro_core::provider::label::testing::ProvidersLabelTestExt;
    use kuro_core::target::configured_target_label::ConfiguredTargetLabel;
    use kuro_core::target::label::label::TargetLabel;
    use kuro_interpreter::types::configured_providers_label::StarlarkProvidersLabel;
    use kuro_interpreter::types::target_label::StarlarkConfiguredTargetLabel;
    ...
    #[test]
    fn print_cli_arg_list() -> kuro_error::Result<()> {
        let args = vec![
    ...
    #[test]
    fn print_cli_arg_json() -> kuro_error::Result<()> {
    ...
    #[test]
    fn coerce_starlark() -> kuro_error::Result<()> {
        Heap::temp(|heap| {
            assert_eq!(
