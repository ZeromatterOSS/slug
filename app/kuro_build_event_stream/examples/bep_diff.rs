...
//! one by Kuro). Reports an event-type histogram and a per-type delta.
//!
//! Usage: `cargo run -p kuro_build_event_stream --example bep_diff -- <bazel.pb> <kuro.pb>`
...
fn main() {
    ...
    let lhs_path = args.next().expect("usage: bep_diff <bazel.pb> <kuro.pb>");
    let rhs_path = args.next().expect("usage: bep_diff <bazel.pb> <kuro.pb>");
