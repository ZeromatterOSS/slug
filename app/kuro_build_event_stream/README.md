# kuro_build_event_stream

Bazel Build Event Protocol (BEP) and Build Event Service (BES) schemas +
translation layer for Kuro. Exists so Kuro can speak enough BEP that
third-party BES consumers (BuildBuddy, EngFlow, Trunk, custom collectors)
render a Kuro invocation the same way they render a Bazel invocation.
...
```
kuro build --build_event_binary_file=events.pb   # length-delimited proto, Bazel-compatible
kuro build --build_event_text_file=events.txt    # Debug-formatted, form-feed delimited
```
...
and Kuro produced the following event-type delta (`examples/bep_diff`):

| Event | Bazel | Kuro | Notes |
|-------|-------|------|-------|
| Started | 1 | 1 | covered |
| Expanded (PatternExpanded) | 1 | 1 | covered |
| Configuration | 2 | 2 | covered |
| Configured (TargetConfigured) | 1 | 8 | Kuro analyzes toolchain targets too; reduce by filtering in 18.2 |
| ActionExecuted | 0 | 5 | Bazel omits successful actions; Kuro currently emits all |
...
kuro build //:main --build_event_binary_file=/tmp/kuro.pb

cargo run -p kuro_build_event_stream --example bep_diff -- /tmp/bazel.pb /tmp/kuro.pb
```
