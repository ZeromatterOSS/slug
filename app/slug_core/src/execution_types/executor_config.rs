...
impl ReGangWorker {
    pub fn parse(worker_map: SmallMap<&str, &str>) -> slug_error::Result<ReGangWorker> {
...
impl RemoteExecutorDependency {
    pub fn parse(dep_map: SmallMap<&str, &str>) -> slug_error::Result<RemoteExecutorDependency> {
...
impl RemoteExecutorUseCase {
    ...
    /// The "slug-default" use case. This is meant to be used when no use case is configured. It's
    /// not meant to be used for convenience when a use case is not available where it's needed!
    pub fn slug_default() -> Self {
        static USE_CASE: Lazy<RemoteExecutorUseCase> =
            Lazy::new(|| RemoteExecutorUseCase::new("slug-default".to_owned()));
        *USE_CASE
    }
}
