...
mod tests {
    ...
    #[tokio::test]
    async fn test() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_get_or_try_init() -> slug_error::Result<()> {
        let cell1 = AsyncOnceCell::new();

        assert_eq!(
            &43,
            cell1
                .get_or_try_init(async { slug_error::Ok(43) })
                .await
                .unwrap()
        );
        assert_eq!(
            &43,
            cell1
                .get_or_try_init(async { slug_error::Ok(55) })
                .await
                .unwrap()
        );
        ...
        let cell2 = AsyncOnceCell::new();
        cell2
            .get_or_try_init(async { Err(slug_error!(slug_error::ErrorTag::TestOnly, "foo")) })
            .await
            .unwrap_err();
        assert_eq!(
            &44,
            cell2
                .get_or_try_init(async { slug_error::Ok(44) })
                .await
                .unwrap()
        );
        assert_eq!(
            &44,
            cell2
                .get_or_try_init(async { slug_error::Ok(56) })
                .await
                .unwrap()
        );
