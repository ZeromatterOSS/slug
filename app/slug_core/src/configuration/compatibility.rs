...
impl<T> MaybeCompatible<T> {
    ...
    pub fn require_compatible(self) -> slug_error::Result<T> {
        match self {
...
impl IncompatiblePlatformReason {
    pub fn to_err(&self) -> slug_error::Error {
        match self.cause {
    ...
    pub fn to_soft_err(&self) -> slug_error::Error {
        match &self.cause {
