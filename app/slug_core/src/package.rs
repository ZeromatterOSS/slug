...
pub(crate) use slug_fs::paths::fmt::quoted_display;
...
///
/// A **valid** Slug package is defined by:
...
impl PackageLabel {
    #[inline]
    pub fn new(cell: CellName, path: &CellRelativePath) -> slug_error::Result<Self> {
        PackageLabel::from_cell_path(CellPathRef::new(cell, path))
    }

    #[inline]
    pub fn from_cell_path(path: CellPathRef) -> slug_error::Result<Self> {
    ...
    pub fn join(&self, path: &ForwardRelativePath) -> slug_error::Result<Self> {
        if path.is_empty() {
            Ok(self.dupe())
        } else {
    ...
    pub fn parent(&self) -> slug_error::Result<Option<PackageLabel>> {
