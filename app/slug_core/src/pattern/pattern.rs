...
pub(crate) fn split_providers_name(s: &str) -> slug_error::Result<(&str, ProvidersName)> {
    if let Some((t, flavors)) = split1_opt_ascii(s, AsciiChar::new('#')) {
        ...
        Ok((
            ...
            ProvidersName::NonDefault(triomphe::Arc::new(NonDefaultProvidersName::Named(
                slug_util::arc_str::ArcSlice::from_iter(names),
            ))),
        ))
    } else {
...
impl ParsedPattern<TargetPatternExtra> {
    ...
    pub fn as_target_label(self, original: &str) -> slug_error::Result<TargetLabel> {
...
impl ParsedPattern<ProvidersPatternExtra> {
    ...
    pub fn as_providers_label(self, original: &str) -> slug_error::Result<ProvidersLabel> {
...
impl<T: PatternType> ParsedPattern<T> {
    pub fn try_map<U: PatternType>(
        self,
        f: impl FnOnce(T) -> slug_error::Result<U>,
    ) -> slug_error::Result<ParsedPattern<U>> {
        match self {
    ...
    pub fn as_literal(self, original: &str) -> slug_error::Result<(TargetLabel, T)> {
...
impl<'a, T: PatternType> PatternParts<'a, T> {
    fn try_map<U: PatternType, F: FnOnce(T) -> slug_error::Result<U>>(
        self,
        f: F,
    ) -> slug_error::Result<PatternParts<'a, U>> {
        let PatternParts {
            cell_alias,
            pattern,
        } = self;
...
impl<'a, T: PatternType> PatternDataOrAmbiguous<'a, T> {
    fn try_map<U: PatternType>(
        self,
        f: impl FnOnce(T) -> slug_error::Result<U>,
    ) -> slug_error::Result<PatternDataOrAmbiguous<'a, U>> {
        match self {
    ...
    T: PatternType,
{
    ...
    /// `foo/bar:bar`.
    pub fn infer_target(self) -> slug_error::Result<PatternData<'a, T>> {
        match self {
    ...
    pub fn reject_ambiguity(self) -> slug_error::Result<PatternData<'a, T>> {
        match self {
            Self::PatternData(d) => Ok(d),
...
impl<'a, T: PatternType> PatternData<'a, T> {
    fn try_map<U: PatternType>(
        self,
        f: impl FnOnce(T) -> slug_error::Result<U>,
    ) -> slug_error::Result<PatternData<'a, U>> {
        match self {
...
pub fn maybe_split_cell_alias_and_relative_path(
    pattern: &str,
) -> slug_error::Result<Option<(CellAlias, &ForwardRelativePath)>> {
...
fn lex_provider_pattern(
    pattern: &str,
    strip_package_trailing_slash: bool,
) -> slug_error::Result<PatternParts<'_, ProvidersPatternExtra>> {
    ...
    let pattern = match split1_opt_ascii(pattern, AsciiChar::new(':')) {
        // Slug-style: //pkg: means all targets in package
        Some((package, "")) => PatternData::AllTargetsInPackage {
            package: normalize_package(package, strip_package_trailing_slash)?,
            modifiers,
        }
        .into(),
        // Bazel-style: //pkg:all also means all targets in package
        Some((package, "all")) => PatternData::AllTargetsInPackage {
            package: normalize_package(package, strip_package_trailing_slash)?,
            modifiers,
        }
        .into(),
...
fn lex_configuration_predicate(pattern: &str) -> slug_error::Result<ConfigurationPredicate> {
    let pattern = pattern
        .strip_prefix('(')
...
pub fn lex_configured_providers_pattern(
    pattern: &str,
    strip_package_trailing_slash: bool,
) -> slug_error::Result<PatternParts<'_, ConfiguredProvidersPatternExtra>> {
...
pub fn lex_target_pattern<T: PatternType>(
    pattern: &str,
    strip_package_trailing_slash: bool,
) -> slug_error::Result<PatternParts<'_, T>> {
...
fn normalize_package(
    package: &str,
    strip_package_trailing_slash: bool,
) -> slug_error::Result<&RelativePath> {
...
impl<'a> TargetParsingRel<'a> {
    ...
    fn join_relative(&self, path: &RelativePath) -> slug_error::Result<CellPath> {
        match self {
            ...
            TargetParsingRel::RequireAbsolute(_) => Err(slug_error::Error::from(
...
fn resolve_target_alias<T>(
    ...
    target_alias_resolver: &dyn TargetAliasResolver,
    lex: &PatternParts<T>,
) -> slug_error::Result<Option<ParsedPatternWithModifiers<T>>>
where
    T: PatternType,
{
    // Imported from Buck1
...
mod tests {
        ...
        ParsedPattern::Target(
            ...
            ProvidersPatternExtra {
                providers: providers.map_or(ProvidersName::Default, |n| {
                    ProvidersName::NonDefault(triomphe::Arc::new(NonDefaultProvidersName::Named(
                        slug_util::arc_str::ArcSlice::from_iter(
    ...
    fn fails<R>(x: slug_error::Result<R>, msgs: &[&str]) {
        match x {
            Err(e) => {
                let s = format!("{e:#}");
                for msg in msgs {
    ...
    impl TargetAliasResolver for NoAliases {
        fn get<'a>(&'a self, _name: &str) -> slug_error::Result<Option<&'a str>> {
            Ok(None)
        }
    }
    ...
    fn aliases(aliases: &[(&str, &str)]) -> impl TargetAliasResolver + use<> {
        ...
        impl TargetAliasResolver for Aliases {
            fn get<'a>(&'a self, name: &str) -> slug_error::Result<Option<&'a str>> {
                Ok(self
                    .0
                    .iter()
                    .find(|(a, _)| *a == name)
    ...
    #[test]
    fn parse_relative_pattern() -> slug_error::Result<()> {
        let package = CellPath::new(
            CellName::testing_new("root"),
            CellRelativePath::unchecked_new("package/path").to_owned(),
        );
    ...
    #[test]
    fn test_relaxed() -> slug_error::Result<()> {
        let package = CellPath::new(
            CellName::testing_new("root"),
    ...
    #[test]
    fn test_parsed_opt_absolute() -> slug_error::Result<()> {
        let package = CellPath::new(
            CellName::testing_new("root"),
            CellRelativePath::unchecked_new("package/path").to_owned(),
        );
    ...
    #[test]
    fn test_aliases() -> slug_error::Result<()> {
        let package = CellPath::new(
            CellName::testing_new("root"),
            CellRelativePath::unchecked_new("package").to_owned(),
        );
    ...
    #[test]
    fn parse_providers_pattern() -> slug_error::Result<()> {
        assert_eq!(
    ...
    #[test]
    fn parse_providers_pattern_with_alias() -> slug_error::Result<()> {
        let package = CellPath::new(
            CellName::testing_new("root"),
            CellRelativePath::unchecked_new("package").to_owned(),
        );
    ...
    #[test]
    fn test_parse_configured_providers_pattern() -> slug_error::Result<()> {
        assert_eq!(
    ...
    #[test]
    fn parsed_pattern_contains() -> slug_error::Result<()> {
    ...
    #[test]
    fn test_relative_pattern_with_parent() -> slug_error::Result<()> {
        let package = CellPath::new(
            CellName::testing_new("root"),
            CellRelativePath::unchecked_new("package/path").to_owned(),
        );
    ...
    #[test]
    fn test_parsed_pattern_with_modifiers_relaxed() -> slug_error::Result<()> {
    ...
    #[test]
    fn test_configured_parsed_pattern_with_modifiers() -> slug_error::Result<()> {
        let resolver = resolver();
        let alias_resolver = alias_resolver();
        let package = CellPath::new(
            CellName::testing_new("root"),
