...
pub fn is_ci() -> slug_error::Result<bool> {
    ...
    Ok(slug_env!("SANDCASTLE", applicability = internal)?.is_some() || slug_env!("CI", bool)?)
}
...
-> slug_error::Result<impl Iterator<Item = (&'static str, Option<&'static str>)>> {
    Ok([
        (
            "sandcastle_job_info",
            slug_env!("SANDCASTLE_JOB_INFO", applicability = internal)?,
        ),
        (
            "skycastle_workflow_run_id",
            slug_env!("SKYCASTLE_WORKFLOW_RUN_ID", applicability = internal)?,
        ),
        (
            "sandcastle_alias",
            slug_env!("SANDCASTLE_ALIAS", applicability = internal)?,
        ),
        (
            "skycastle_workflow_alias",
            slug_env!("SKYCASTLE_WORKFLOW_ALIAS", applicability = internal)?,
        ),
        (
            "sandcastle_type",
            slug_env!("SANDCASTLE_TYPE", applicability = internal)?,
        ),
    ]
    .into_iter())
}
