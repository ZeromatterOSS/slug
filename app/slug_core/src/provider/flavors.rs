...
pub fn map_flavors(flavors: &str, full_target: &str) -> slug_error::Result<ProvidersName> {
    ...
    match flavors_parts.iter().position(|x| is_platform_flavor(x)) {
        Some(index) => {
            ...
            soft_error!(
                "platform_flavor",
                slug_error::slug_error!(slug_error::ErrorTag::Input, "Platform flavor found in target: {}", full_target).into(),
                deprecation: true,
                quiet: true
            )?;
