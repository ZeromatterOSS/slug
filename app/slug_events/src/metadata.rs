...
use slug_core::ci::ci_identifiers;
...
pub fn collect(daemon: &DaemonId) -> HashMap<String, String> {
    ...
    if let Some(rev) = slug_build_info::revision() {
        map.insert("slug_revision".to_owned(), rev.to_owned());
    }

    if let Some(time) = slug_build_info::time_iso8601() {
        map.insert("slug_build_time".to_owned(), time.to_owned());
    }

    if let Some(ts) = slug_build_info::release_timestamp() {
        map.insert("slug_release_timestamp".to_owned(), ts.to_owned());
    }
...
pub fn username() -> slug_error::Result<Option<String>> {
    #[cfg(fbcode_build)]
    {
        use slug_error::conversion::from_any_with_tag;
        Ok(Some(user::current_username().map_err(|e| {
            from_any_with_tag(e, slug_error::ErrorTag::InvalidUsername)
        })?))
    }
    #[cfg(not(fbcode_build))]
    {
