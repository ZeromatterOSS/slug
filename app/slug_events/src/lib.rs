...
//!
//! Events and event streams for Slug.
//!
//! The `Event` enum is the set of events that Slug can produce. Events can be produced both during the course of
...
/// An event that can be produced by Slug. Events are points in time with additional metadata attached to them,
...
impl BuckEvent {
    ...
    pub fn trace_id(&self) -> slug_error::Result<TraceId> {
    ...
    pub fn event(&self) -> &slug_data::BuckEvent {
        &self.event
    }

    pub fn data(&self) -> &slug_data::buck_event::Data {
        self.event
            .data
            .as_ref()
    ...
    pub fn data_mut(&mut self) -> &mut slug_data::buck_event::Data {
        self.event
            .data
            .as_mut()
            .expect("data is set, it is validated")
    ...
    pub fn span_start_event(&self) -> Option<&slug_data::SpanStartEvent> {
        match self.data() {
            slug_data::buck_event::Data::SpanStart(start) => Some(start),
            _ => None,
        }
    }

    pub fn span_end_event(&self) -> Option<&slug_data::SpanEndEvent> {
        match self.data() {
            slug_data::buck_event::Data::SpanEnd(end) => Some(end),
            _ => None,
        }
    }

    pub fn command_start(&self) -> slug_error::Result<Option<&slug_data::CommandStart>> {
        match self.span_start_event() {
            ...
            Some(span_start_event) => {
                ...
                {
                    slug_data::span_start_event::Data::Command(command_start) => {
...
impl From<BuckEvent> for Box<slug_data::BuckEvent> {
    fn from(e: BuckEvent) -> Self {
        e.event
    }
}

impl TryFrom<Box<slug_data::BuckEvent>> for BuckEvent {
    type Error = slug_error::Error;

    fn try_from(event: Box<slug_data::BuckEvent>) -> slug_error::Result<BuckEvent> {
...
#[slug(tag = InvalidEvent)]
enum BuckEventError {
    #[error("The `slug_data::BuckEvent` provided has no `Timestamp`")]
    MissingTimestamp,
    #[error("The `slug_data::BuckEvent` provided has no `Data`")]
    MissingData,
...
pub fn init_late_bindings() {
    slug_core::event::EVENT_DISPATCH.init(&dispatch::EventDispatcherLateBinding);
}

#[cfg(test)]
mod tests {
    use slug_data::CommandStart;
    ...
    fn round_trip_success() {
        ...
        assert_eq!(
            test,
            BuckEvent::try_from(Box::<slug_data::BuckEvent>::from(test.clone())).unwrap()
        );
    }
