...
mod fbcode {
    ...
    impl RemoteEventSink {
        pub async fn send_now(&self, _event: BuckEvent) -> slug_error::Result<()> {
            Ok(())
        }
        pub async fn send_messages_now(&self, _events: Vec<BuckEvent>) -> slug_error::Result<()> {
            Ok(())
        }
    }
...
fn new_remote_event_sink_if_fbcode(
    fb: FacebookInit,
    config: ScribeConfig,
) -> slug_error::Result<Option<RemoteEventSink>> {
    #[cfg(fbcode_build)]
    {
