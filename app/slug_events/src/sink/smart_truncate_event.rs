...
fn truncate(s: &str, max_bytes: usize) -> String {
    ...
    slug_util::truncate::truncate(s, max_bytes.min(MAX_STRING_BYTES))
}

#[cfg_attr(not(fbcode_build), allow(dead_code))]
pub(crate) fn smart_truncate_event(d: &mut slug_data::buck_event::Data) {
    use slug_data::buck_event::Data;

    match d {
        Data::SpanEnd(s) => {
            use slug_data::span_end_event::Data;
        ...
        Data::Instant(inst) => {
            use slug_data::instant_event::Data;
            match &mut inst.data {
        ...
        Data::Record(rec) => {
            if let Some(slug_data::record_event::Data::InvocationRecord(invocation_record)) =
                &mut rec.data
            {
...
fn truncate_invocation_record(invocation_record: &mut slug_data::InvocationRecord) {
...
fn truncate_action_execution_end(action_execution_end: &mut slug_data::ActionExecutionEnd) {
    let per_command_size_budget = (500 * 1024) / action_execution_end.commands.len().max(1);

    let truncate_cmd = |cmd: &mut slug_data::CommandExecution, truncate_all: bool| {
...
fn truncate_command_end(command_end: &mut slug_data::CommandEnd, clear_target_patterns: bool) {
    use slug_data::command_end::Data;
...
fn truncate_file_watcher_stats(file_watcher_stats: &mut slug_data::FileWatcherStats) {
...
fn truncate_test_end(test_end: &mut slug_data::TestRunEnd) {
...
fn truncate_test_discovery_end(test_discovery_end: &mut slug_data::TestDiscoveryEnd) {
...
fn truncate_target_patterns(target_patterns: &mut Vec<slug_data::TargetPattern>) {
    ...
    for (index, target) in target_patterns.iter().enumerate() {
        ...
        if bytes > MAX_TARGET_PATTERNS_BYTES {
            ...
            let warn = format!("<<Truncated (reported {index} / {orig_len})>>");
            target_patterns.push(slug_data::TargetPattern { value: warn });
            break;
        }
    }
}
...
mod tests {
    ...
    fn make_invocation_record(data: slug_data::InvocationRecord) -> slug_data::buck_event::Data {
        slug_data::buck_event::Data::Record(slug_data::RecordEvent {
            data: Some(slug_data::record_event::Data::InvocationRecord(Box::new(
                data,
            ))),
        })
    }
    ...
    fn make_action_execution_end(
        data: slug_data::ActionExecutionEnd,
    ) -> slug_data::buck_event::Data {
        slug_data::buck_event::Data::SpanEnd(slug_data::SpanEndEvent {
            data: Some(slug_data::span_end_event::Data::ActionExecution(Box::new(
                data,
            ))),
            ..Default::default()
        })
    ...
    fn make_command_end(data: slug_data::CommandEnd) -> slug_data::buck_event::Data {
        slug_data::buck_event::Data::SpanEnd(slug_data::SpanEndEvent {
            data: Some(slug_data::span_end_event::Data::Command(data)),
            ..Default::default()
        })
    }

    fn make_build_command_end(
        unresolved_target_patterns: Vec<slug_data::TargetPattern>,
    ) -> slug_data::CommandEnd {
        slug_data::CommandEnd {
            data: Some(slug_data::command_end::Data::Build(
                slug_data::BuildCommandEnd {
    ...
    fn make_test_end(data: slug_data::TestRunEnd) -> slug_data::buck_event::Data {
        slug_data::buck_event::Data::SpanEnd(slug_data::SpanEndEvent {
            data: Some(slug_data::span_end_event::Data::TestEnd(data)),
            ..Default::default()
        })
    }

    fn make_test_discovery_end(data: slug_data::TestDiscoveryEnd) -> slug_data::buck_event::Data {
        slug_data::buck_event::Data::SpanEnd(slug_data::SpanEndEvent {
            data: Some(slug_data::span_end_event::Data::TestDiscovery(data)),
            ..Default::default()
        })
    }

    fn make_command_execution_with_stderr(stderr: String) -> slug_data::CommandExecution {
        slug_data::CommandExecution {
            details: Some(slug_data::CommandExecutionDetails {
                cmd_stderr: stderr,
                ..Default::default()
            }),
            ..Default::default()
    ...
    fn smart_truncate_resolved_target_patterns_clears_unresolved_one() {
        let mut record = slug_data::InvocationRecord::default();
        let mut record_expected = record.clone();

        let resolved_target_patterns = vec![slug_data::TargetPattern {
            value: "some_resolved_target".to_owned(),
        }];
        record.parsed_target_patterns = Some(slug_data::ParsedTargetPatterns {
            target_patterns: resolved_target_patterns.clone(),
        });
        // resolved_target_patterns is expected to be unchanged.
        record_expected.parsed_target_patterns = Some(slug_data::ParsedTargetPatterns {
            target_patterns: resolved_target_patterns,
        });

        let unresolved_target_patterns = vec![slug_data::TargetPattern {
    ...
    fn smart_truncate_action_execution_end_one_last_command_truncated() {
        ...
        let action_execution_end_with_stderrs = slug_data::ActionExecutionEnd {
            commands: vec![command_execution_with_stderr],
            ..Default::default()
        };
        let action_execution_end_last_stderr_omitted = slug_data::ActionExecutionEnd {
    ...
    fn smart_truncate_long_file_watcher_stats_truncated() {
        let file_watcher_event = slug_data::FileWatcherEvent {
            path: "0123456789".repeat(3 * 1024),
            ..Default::default()
        };
        let file_watcher_stats = slug_data::FileWatcherStats {
            events: vec![
        ...
        let file_watcher_stats_truncated = slug_data::FileWatcherStats {
            events: vec![
                file_watcher_event.clone(),
                file_watcher_event.clone(),
                file_watcher_event,
        ...
        let record_truncated = slug_data::InvocationRecord {
    ...
    fn smart_truncate_invocation_record_error_reports_truncated() {
        ...
        let mut event_data = make_invocation_record(slug_data::InvocationRecord {
            errors,
            ..Default::default()
        });
        smart_truncate_event(&mut event_data);

        let slug_data::buck_event::Data::Record(record_event) = event_data else {
            unreachable!()
        };
        let Some(slug_data::record_event::Data::InvocationRecord(invocation_record)) =
            record_event.data
        else {
            unreachable!()
        };
    ...
    fn smart_truncate_test_end_long_test_names_truncated() {
        ...
        let test_end = slug_data::TestRunEnd {
            suite: Some(slug_data::TestSuite {
                test_names,
                ..Default::default()
            }),
            ..Default::default()
        };
        let test_end_truncated = slug_data::TestRunEnd {
            suite: Some(slug_data::TestSuite {
    ...
    fn make_command_execution_with_stdout(stdout: String) -> slug_data::CommandExecution {
        slug_data::CommandExecution {
            details: Some(slug_data::CommandExecutionDetails {
                cmd_stderr: stdout,
                ..Default::default()
    ...
    fn smart_truncate_test_discovery_end_command_report_stdout_truncated() {
        let test_discovery_end = slug_data::TestDiscoveryEnd {
            command_report: Some(make_command_execution_with_stdout("blah".to_owned())),
            ..Default::default()
        };

        let test_discovery_end_truncated = slug_data::TestDiscoveryEnd {
            command_report: Some(make_command_execution_with_stdout("<<omitted>>".to_owned())),
            ..Default::default()
        };
