...
use slug_data::Location;
use slug_data::StructuredError;
...
/// RemoteEventSink is a ScribeSink backed by the Thrift-based client in the `slug_scribe_client` crate.
...
impl RemoteEventSink {
    ...
    pub fn new(
        ...
        category: String,
        config: ScribeConfig,
    ) -> slug_error::Result<RemoteEventSink> {
    ...
    pub async fn send_now(&self, event: BuckEvent) -> slug_error::Result<()> {
        self.send_messages_now(vec![event]).await
    }

    // Send multiple events now, bypassing internal message queue.
    pub async fn send_messages_now(&self, events: Vec<BuckEvent>) -> slug_error::Result<()> {
        let messages = events
            .into_iter()
            .map(|e| {
    ...
    fn encode_message(mut event: BuckEvent) -> Vec<u8> {
        ...
        let mut proto: Box<slug_data::BuckEvent> = event.into();
        ...
        if buf.len() > SCRIBE_MESSAGE_SIZE_LIMIT {
            ...
            let proto: Box<slug_data::BuckEvent> = BuckEvent::new(
                    SystemTime::now(),
                    TraceId::new(),
                    None,
                    None,
                    slug_data::buck_event::Data::Instant(InstantEvent {
                        data: Some(
                            StructuredError {
                                ...
                                quiet: false,
                                task: Some(true),
                                soft_error_category: Some(slug_data::SoftError {category: "oversized_scribe".to_owned(), is_quiet:false}),
    ...
    fn prepare_event(event: &mut slug_data::BuckEvent) {
        use slug_data::buck_event::Data;

        match &mut event.data {
            Some(Data::SpanEnd(s)) => match &mut s.data {
                Some(slug_data::span_end_event::Data::ActionExecution(action)) => {
    ...
    fn should_send_event(&self, data: &slug_data::buck_event::Data) -> bool {
        use slug_data::buck_event::Data;
        ...
        match data {
            ...
            Data::Record(r) => {
                use slug_data::record_event::Data;
...
fn get_is_cache_hit(details: &slug_data::CommandExecutionDetails) -> bool {
    use slug_data::command_execution_kind::Command::RemoteCommand;
...
pub(crate) fn scribe_category() -> slug_error::Result<String> {
    const DEFAULT_SCRIBE_CATEGORY: &str = "slug_events";
    ...
    Ok(
        slug_env!("BUCK2_SCRIBE_CATEGORY", applicability = internal)?
...
mod tests {
    ...
    fn test_encode_large_message() {
        ...
        let event = BuckEvent::new(
            ...
            None,
            slug_data::buck_event::Data::Instant(InstantEvent {
                data: Some(slug_data::instant_event::Data::StructuredError(
                    slug_data::StructuredError {
