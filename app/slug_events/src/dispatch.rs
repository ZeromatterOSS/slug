...
//! liberally duplicated and passed around to the depths of slug so that consumers can insert events into it.
...
impl EventDispatcher {
    ...
    pub fn instant_event<E: Into<slug_data::instant_event::Data>>(&self, data: E) {
        let instant = slug_data::InstantEvent {
            data: Some(data.into()),
        };
    ...
    pub fn console_message(&self, message: String) {
        self.instant_event(slug_data::ConsoleMessage { message })
    }

    pub fn console_warning(&self, message: String) {
        self.instant_event(slug_data::ConsoleWarning { message })
    }

    pub fn streaming_output(&self, message: String) {
        self.instant_event(slug_data::StdoutStreamingOutput { message })
    }
    ...
    pub fn partial_result(&self, data: slug_cli_proto::PartialResult) {
        self.sink.send(Event::PartialResult(data));
    }

    pub fn command_result(&self, data: slug_cli_proto::CommandResult) {
...
impl Span {
    ...
    fn send(&mut self, data: slug_data::span_end_event::Data) {
        self.sent = true;
        ...
        self.dispatcher.event_with_span_id(
            SpanEndEvent {
                ...
                data: Some(data),
                stats: Some(slug_data::SpanStats {
...
impl Drop for Span {
    ...
    fn drop(&mut self) {
        if !self.sent {
            self.send(slug_data::SpanCancelled {}.into())
        }
    }
}
...
pub fn get_dispatcher() -> EventDispatcher {
    let enforce_event_dispatcher_set = slug_env!("ENFORCE_DISPATCHER_SET", bool).unwrap();
    ...
    match get_dispatcher_opt() {
        ...
        None => {
            if enforce_event_dispatcher_set {
                ...
                // let _ignored = soft_error!(slug_error::slug_error!([], "Task local event dispatcher not set."));
...
/// Emits an InstantEvent annotated with the current trace ID
pub fn instant_event<E: Into<slug_data::instant_event::Data>>(data: E) {
    get_dispatcher().instant_event(data)
}
