...
#[cold]
#[track_caller]
pub fn slug_error_impl(tag: crate::ErrorTag, args: Arguments) -> crate::Error {
...
#[macro_export]
macro_rules! slug_error {
    ($tags:expr, $format:expr) => {
        $crate::slug_error!($tags, $format,)
    };
    ($tags:expr, $format:expr, $($arg:tt)*) => {
        $crate::macros::slug_error_impl($tags, format_args!($format, $($arg)*))
    };
}

/// Indicates a bug in slug.
#[macro_export]
