...
pub enum Tier {
    ...
    Environment,
    // Unexpected errors in slug or core dependencies.
...
impl ErrorLike for slug_data::ErrorReport {
    fn best_tag(&self) -> Option<ErrorTag> {
    ...
    fn error_rank(self: &slug_data::ErrorReport) -> u32 {
...
pub fn best_error<'a>(
    tags: impl IntoIterator<Item = &'a slug_data::ErrorReport>,
) -> Option<&'a slug_data::ErrorReport> {
...
// Slug is the fallback/default source area, use the first non-slug source area.
pub fn source_area(tags: impl IntoIterator<Item = ErrorTag>) -> ErrorSourceArea {
    tags.into_iter()
        .find_map(|tag| {
            let area = tag.source_area();
            if area != ErrorSourceArea::Slug {
                Some(area)
            } else {
                None
            }
        })
        .unwrap_or(ErrorSourceArea::Slug)
}
