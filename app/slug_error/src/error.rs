...
/// entirely replaced it in the Slug codebase. It has `From` impls from many common error types.
...
impl Error {
    ...
    pub fn new(
        ...
        let slug_error = crate::Error(Arc::new(ErrorKind::Root(Box::new(error_root))));
        slug_error.tag([error_tag])
    }
    ...
    pub fn action_error(&self) -> Option<&slug_data::ActionError> {
