...
#[derive(slug_error_derive::Error, Debug)]
#[error("foo")]
#[slug(input)]
pub struct Error1;
...
fn test_source_location_no_attrs() {
    ...
    assert_eq!(
        ...
        "slug_error/src/derive_tests.rs::NoAttrsStruct"
    );
    let e: crate::Error = NoAttrsEnum::Variant.into();
    assert_eq!(
        e.source_location().to_string(),
        "slug_error/src/derive_tests.rs::NoAttrsEnum::Variant"
    );
}
...
fn test_enum_with_type_option() {
    ...
    assert_eq!(
        e.source_location().to_string(),
        "slug_error/src/derive_tests.rs::EnumWithTypeOption::Variant",
    );
}
...
fn test_error_tags_vec_fn() {
    ...
    #[derive(slug_error_derive::Error, Debug)]
    #[error("Unused")]
    #[slug(tag = WatchmanTimeout, tags = calc_tags(*extra_tag))]
    struct TaggedError {
        extra_tag: bool,
    }
...
#[test]
fn test_recovery_through_transparent_slug_error() {
    #[derive(slug_error_derive::Error, Debug)]
    #[error("base_display")]
    #[slug(tag = Environment)]
    struct BaseError;
