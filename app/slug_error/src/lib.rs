...
/// you can call [`.tag()`](`crate::BuckErrorContext::tag`) on a [`slug_error::Result`][`Result`].
///
...
pub use slug_data::error::ErrorTag;
...
/// `thiserror` in `slug/app` will be replaced with this macro.
///
/// Currently, the only distinction from `thiserror::Error` is that an additional impl of
/// `AnyError` is generated for the type, which makes some of the interactions with `slug_error` more
...
/// #[error("My error type")]
/// #[slug(tag = slug_error::ErrorTag::Input)]
/// struct MyError;
///
/// let e = slug_error::Error::from(MyError);
/// assert_eq!(&format!("{}", e), "My error type");
/// ```
#[doc(inline)]
pub use slug_error_derive::Error;
