...
//! Starlark::Error <-> slug_error::Error conversion implementation.
...
fn error_with_starlark_context(
    ...
    let mut slug_error = e.clone();

    loop {
        match slug_error.0.as_ref() {
            ErrorKind::Root(root) => {
            ...
            ErrorKind::WithContext(context_value, inner) => {
                match context_value {
                    ...
                    ContextValue::StarlarkError(_) => {
                        return slug_error.context_for_starlark_backtrace(starlark_context);
                    }
...
mod tests {
    ...
    use crate as slug_error;
    use crate::context_value::StarlarkContext;
    use crate::slug_error;
    use crate::starlark_error::error_with_starlark_context;

    #[derive(slug_error::Error, Debug, Allocative)]
    #[error("FullMetadataError")]
    #[slug(tag = crate::ErrorTag::Tier0)]
    ...
    fn test_roundtrip_starlark() {
        // Tests slug_error->starlark->slug_error
    ...
    fn test_pop_base_error_from_context() {
        ...
        let e = slug_error!(crate::ErrorTag::StarlarkError, "{}", base_error);
