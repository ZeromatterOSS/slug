...
use slug_analysis::analysis::env::RuleAnalysisAttrResolutionContext;
use slug_analysis::analysis::env::get_deps_from_analysis_results;
use slug_analysis::attrs::resolve::configured_attr::ConfiguredAttrExt;
...
use slug_build_api::bxl::unconfigured_attribute::StarlarkCoercedAttr;
...
use slug_node::attrs::display::AttrDisplayWithContext;
...
fn attr_with_stripped_cfg(attr: &ConfiguredAttr) -> slug_error::Result<CoercedAttr> {
    Ok(match attr {
        ...
        ConfiguredAttr::SplitTransitionDep(dep) => {
            ...
            CoercedAttr::Dep(
                deps.iter()
                    .next()
                    .ok_or_else(|| {
                        slug_error::internal_error!("deps is empty after checking length")
                    })?
                    .dupe(),
            )
        }
            ...
            let unconfigured_parts = match &arg.string_with_macros {
                ...
                StringWithMacros::ManyParts(parts) => StringWithMacros::ManyParts(
                    parts
                        .iter()
                        .map(|part| match part {
                            slug_node::attrs::attr_type::arg::StringWithMacrosPart::String(s) => {
                                slug_node::attrs::attr_type::arg::StringWithMacrosPart::String(
                                    s.clone(),
                                )
                            }
                            slug_node::attrs::attr_type::arg::StringWithMacrosPart::Macro(
                                write_to_file,
                                macr,
                            ) => slug_node::attrs::attr_type::arg::StringWithMacrosPart::Macro(
                                *write_to_file,
                                match macr {
                                    slug_node::attrs::attr_type::arg::MacroBase::Location {
                                        label,
                                        exec_dep,
                                    } => slug_node::attrs::attr_type::arg::MacroBase::Location {
                                    ...
                                    slug_node::attrs::attr_type::arg::MacroBase::Exe {
                                        label,
                                        exec_dep,
                                    } => slug_node::attrs::attr_type::arg::MacroBase::Exe {
                                        label: label.unconfigured(),
                                    ...
                                    slug_node::attrs::attr_type::arg::MacroBase::UserUnkeyedPlaceholder(
                                        var_name,
                                    ) => {
                                        slug_node::attrs::attr_type::arg::MacroBase::UserUnkeyedPlaceholder(
                                            var_name.clone(),
                                    ...
                                    slug_node::attrs::attr_type::arg::MacroBase::UserKeyedPlaceholder(
                                        box_value,
                                    ) => {
                                    ...
                                    slug_node::attrs::attr_type::arg::MacroBase::Query(
                                        query_macro,
                                    ) => {
                                        slug_node::attrs::attr_type::arg::MacroBase::Query(
                                            Box::new(
                                                slug_node::attrs::attr_type::query::QueryMacroBase {
                                                    expansion_type: query_macro
                                                        .expansion_type
                                                        .clone(),
                                                    query: slug_node::attrs::attr_type::query::QueryAttrBase {
                                                        query: query_macro.query.query.clone(),
                                                        resolved_literals: slug_node::attrs::attr_type::query::ResolvedQueryLiterals(
                                                            query_macro
                                                                .query
                                                                .resolved_literals
                                                                .0
                                    ...
                                    slug_node::attrs::attr_type::arg::MacroBase::Source(path) => {
                                        slug_node::attrs::attr_type::arg::MacroBase::Source(
                                            path.clone(),
                                        )
                                    }
                                    slug_node::attrs::attr_type::arg::MacroBase::UnrecognizedMacro(
                                        macr,
                                    ) => {
                                        slug_node::attrs::attr_type::arg::MacroBase::UnrecognizedMacro(
                                            macr.clone(),
...
fn configured_target_node_value_methods(builder: &mut MethodsBuilder) {
    ...
    /// It will not return special attribute (attribute that start with 'buck.' in `slug cquery -A` command).
    ///
    /// Sample usage:
    /// ```python
    ...
    fn resolved_attrs_eager<'v>(
        ...
        let dep_analysis: slug_error::Result<Vec<(&ConfiguredTargetLabel, AnalysisResult)>> = ctx
            .via_dice(eval, |ctx| {
    ...
    fn sources(this: &StarlarkConfiguredTargetNode) -> starlark::Result<Vec<StarlarkArtifact>> {
        ...
        impl ConfiguredAttrTraversal for InputsCollector {
            fn dep(&mut self, _dep: &ConfiguredProvidersLabel) -> slug_error::Result<()> {
                Ok(())
            }

            fn input(&mut self, path: SourcePathRef) -> slug_error::Result<()> {
                self.inputs
    ...
    fn deps<'v>(
        this: &'v StarlarkConfiguredTargetNode,
        // ) -> slug_error::Result<Vec<StarlarkConfiguredTargetNode>> {
