...
use slug_build_api::interpreter::rule_defs::artifact::starlark_artifact_like::ValueAsInputArtifactLikeUnpack;
...
use slug_core::pattern::parse_package::parse_package;
use slug_interpreter::types::package_path::StarlarkPackagePath;
use slug_interpreter_for_build::interpreter::package_file_calculation::EvalPackageFile;
use slug_interpreter_for_build::super_package::package_value::SuperPackageValuesImpl;
...
#[derive(Debug, slug_error::Error, Clone)]
#[slug(tag = Input)]
...
impl<'v> PackagePathArg<'v> {
    fn pkg(&self, cell_alias_resolver: &CellAliasResolver) -> slug_error::Result<PackageLabel> {
        match self {
...
pub(crate) fn register_read_package_value_function(builder: &mut GlobalsBuilder) {
    ...
    fn read_package_value<'v>(
        ...
        let metadata_key = MetadataKeyRef::new(key).map_err(slug_error::Error::from)?;
