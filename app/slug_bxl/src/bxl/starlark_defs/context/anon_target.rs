...
use slug_analysis::analysis::calculation::get_loaded_module;
use slug_analysis::analysis::env::get_rule_impl;
use slug_analysis::analysis::env::promise_artifact_mappings;
use slug_analysis::analysis::env::transitive_validations;
use slug_build_api::analysis::AnalysisResult;
use slug_build_api::analysis::anon_promises_dyn::RunAnonPromisesAccessor;
...
use slug_build_api::bxl::anon_target::EVAL_BXL_FOR_ANON_TARGET;
...
use slug_build_api::interpreter::rule_defs::provider::ty::abstract_provider::AbstractProvider;
...
use slug_interpreter_for_build::attrs::StarlarkAttribute;
use slug_interpreter_for_build::rule::StarlarkRuleCallable;
...
async fn eval_bxl_for_anon_target_inner(
    ...
    BuckStarlarkModule::with_profiling(|env| {
        ...
        let (bxl_ctx, list_res) = reentrant_eval.with_evaluator(|eval| {
            ...
            let list_res = tokio::task::block_in_place(|| -> slug_error::Result<Value<'_>> {
    ...
    for BxlAnonPromisesAccessor<'me, 'v, 'a, 'e>
{
    fn with_evaluator(
        &mut self,
        closure: &mut dyn FnMut(&mut Evaluator<'v, 'a, 'e>) -> slug_error::Result<()>,
    ) -> slug_error::Result<()> {
        closure(self.0)
    }
