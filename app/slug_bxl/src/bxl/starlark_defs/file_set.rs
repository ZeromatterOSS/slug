...
use slug_common::file_ops::metadata::SimpleDirEntry;
...
impl<'a> FileSetExpr<'a> {
    pub(crate) async fn get(self, bxl: &BxlContext<'_>) -> slug_error::Result<Cow<'a, FileSet>> {
        let set = match self {
...
impl OwnedFileSetExpr {
    ...
    ) -> slug_error::Result<Cow<'a, FileSet>> {
        let set = match self {
...
impl StarlarkReadDirSet {
    fn children(&self) -> slug_error::Result<Vec<CellPath>> {
