...
enum BxlResultError {
    ...
    UnwrapOnError(slug_error::Error),
...
impl StarlarkError {
    pub(crate) fn new(err: slug_error::Error) -> Self {
        Self { err }
    }
}
...
pub(crate) enum StarlarkResultGen<T> {
    Ok(T),
    Err(#[freeze(identity)] slug_error::Error),
}
...
impl<T> StarlarkResultGen<T> {
    pub(crate) fn from_result(res: slug_error::Result<T>) -> Self {
        match res {
            Ok(val) => Self::Ok(val),
...
impl<'v, V: ValueLike<'v>> StarlarkResultGen<V> {
    fn unwrap(&self) -> slug_error::Result<Value<'v>> {
        match self {
    ...
    fn unwrap_err(&self) -> slug_error::Result<StarlarkError> {
        match self {
