...
impl CliArgType {
    fn coerce_value<'v>(&self, value: Value<'v>) -> slug_error::Result<CliArgValue> {
        Ok(match self {
    ...
    ) -> BoxFuture<'a, slug_error::Result<Option<CliArgValue>>> {
        async move {
            Ok(match self {
                CliArgType::Bool => clap.value_of().map_or(Ok(None), |x| {
                    let r: slug_error::Result<_> = try { CliArgValue::Bool(x.parse::<bool>()?) };
                    r.map(Some)
                })?,
                CliArgType::Int => clap.value_of().map_or(Ok(None), |x| {
                    let r: slug_error::Result<_> = try {
                        CliArgValue::Int(
                            x.parse::<BigInt>()
...
mod tests {
    ...
    #[test]
    fn print_cli_arg_list() -> slug_error::Result<()> {
        let args = vec![
    ...
    #[test]
    fn print_cli_arg_json() -> slug_error::Result<()> {
    ...
    #[test]
    fn coerce_starlark() -> slug_error::Result<()> {
        Heap::temp(|heap| {
            assert_eq!(
