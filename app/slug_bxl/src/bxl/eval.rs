...
use slug_data::BxlExecutionEnd;
use slug_data::BxlExecutionStart;
use slug_data::StarlarkFailNoStacktrace;
...
pub(crate) struct BxlEvalError {
    ...
    pub(crate) error: slug_error::Error,
}

pub(crate) type Result<T> = std::result::Result<T, BxlEvalError>;

impl From<slug_error::Error> for BxlEvalError {
    ...
    let (_, futs) = unsafe {
        ...
        scope_and_collect_with_dice(ctx, |ctx, s| {
            s.spawn_cancellable(
                ...
                || Err(slug_error!(slug_error::ErrorTag::Tier0, "cancelled").into()),
            )
        })
    }
    .await;
...
) -> slug_error::Result<OwnedFrozenValueTyped<FrozenBxlFunction>> {
...
pub(crate) async fn resolve_cli_args<'a>(
    ...
    match frozen_callable
        .to_clap(clap::Command::new(&spec.name).no_binary_name(true)) // patternlint-disable-line slug-no-command-new
