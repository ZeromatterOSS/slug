...
use slug_build_api::bxl::calculation::BXL_CALCULATION_IMPL;
use slug_build_api::bxl::calculation::BxlCalculationDyn;
...
impl BxlCalculationDyn for BxlCalculationImpl {
    ...
    ) -> slug_error::Result<BxlComputeResult> {
...
pub(crate) async fn eval_bxl(
    ...
    match ctx.compute(&internal::BxlComputeKey(bxl)).await {
        Ok(res) => res,
        Err(e) => Err(slug_error::Error::from(e).into()),
    }
}
