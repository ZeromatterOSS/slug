...
use slug_build_api::build::build_report::BuildReportOpts;
use slug_build_api::build::build_report::write_bxl_build_report;
use slug_build_api::bxl::result::BxlResult;
use slug_build_api::bxl::result::PendingStreamingOutput;
use slug_build_api::bxl::types::BxlFunctionLabel;
...
use slug_cli_proto::BxlRequest;
use slug_cli_proto::BxlResponse;
...
use slug_common::target_aliases::HasTargetAliasResolver;
...
use slug_data::BxlEnsureArtifactsEnd;
use slug_data::BxlEnsureArtifactsStart;
...
use slug_interpreter::parse_import::parse_import_with_config;
use slug_interpreter::paths::module::StarlarkModulePath;
use slug_server_ctx::bxl::GetBxlStreamingTracker;
...
impl BxlServerCommand {
    ...
    ) -> slug_error::Result<slug_cli_proto::BxlResponse> {
    ...
    ) -> slug_error::Result<BxlCommandContext<'a>> {
    ...
    async fn resolve_cli_args(
        ...
        dice_ctx: &mut DiceTransaction,
    ) -> slug_error::Result<BxlResolvedCliArgs> {
        let cur_package =
    ...
    async fn materialize_collected_artifacts(
        ...
        let mut errors: Vec<slug_error::Error> = Vec::new();
    ...
    async fn write_build_report(
        ...
        dice_ctx: &mut DiceTransaction,
        server_ctx: &dyn ServerCommandContextTrait,
        ensured_artifact_errors: Vec<slug_error::Error>,
        ...
        let serialized_build_report = if bxl_opts.unstable_print_build_report {
            ...
            let build_report_opts = BuildReportOpts {
                // These are all deprecated for `slug build`, so don't need to support them
...
) -> slug_error::Result<BxlFunctionLabel> {
