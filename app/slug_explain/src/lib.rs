...
pub fn output_format<T: QueryTarget>(data: TargetSet<T>) -> slug_error::Result<String> {
...
pub fn inline_fbs(
    ...
    let env = slug_env!("BUCK2_DUMP_FBS", applicability = testing)?;
    if let Some(fbs_dump) = env {
        fs::write(fbs_dump, &base64)?;
    }
    if !html_in.contains(HTML_PLACEHOLDER) {
        return Err(slug_error::slug_error!(
            slug_error::ErrorTag::Explain,
