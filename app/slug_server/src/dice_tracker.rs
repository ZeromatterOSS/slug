...
impl BuckDiceTracker {
    pub fn new(events: EventDispatcher) -> slug_error::Result<Self> {
        let (event_forwarder, receiver) = mpsc::unbounded();
        let snapshot_interval =
            slug_env!("BUCK2_DICE_SNAPSHOT_INTERVAL_MS", type=u64, default = 500)
                .map(Duration::from_millis)?;

        thread_spawn("slug-dice-tracker", move || {
