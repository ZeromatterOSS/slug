...
use slug_cli_proto::trace_io_response;
...
pub(crate) async fn trace_io_command(
    ...
    req: slug_cli_proto::TraceIoRequest,
) -> slug_error::Result<slug_cli_proto::TraceIoResponse> {
    let start_event = context
        .command_start_event(slug_data::TraceIoCommandStart {}.into())
        .await?;
    span_async(start_event, async move {
        ...
        let result = match (tracing_provider, respond_with_trace) {
            ...
            (Some(_), false) => Ok(slug_cli_proto::TraceIoResponse {
                enabled: true,
                trace: Vec::new(),
            ...
            (None, _) => Ok(slug_cli_proto::TraceIoResponse {
                enabled: false,
                trace: Vec::new(),
                external_entries: Vec::new(),
                relative_symlinks: Vec::new(),
        ...
        let end_event = command_end(&result, slug_data::TraceIoCommandEnd {});
...
async fn build_response_with_trace(
    ...
    Ok(slug_cli_proto::TraceIoResponse {
        enabled: true,
        trace,
        external_entries,
        relative_symlinks,
