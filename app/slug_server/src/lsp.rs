...
use slug_core::pattern::pattern::TargetParsingRel;
...
use slug_events::dispatch::with_dispatcher;
...
use slug_interpreter::allow_relative_paths::HasAllowRelativePaths;
...
use slug_interpreter_for_build::interpreter::dice_calculation_delegate::HasCalculationDelegate;
use slug_interpreter_for_build::interpreter::global_interpreter_state::HasGlobalInterpreterState;
use slug_interpreter_for_build::interpreter::interpreter_for_dir::ParseData;
...
impl DocsCacheManager {
    async fn new(fs: ProjectRoot, mut dice_ctx: DiceTransaction) -> slug_error::Result<Self> {
        Ok(Self {
    ...
    ) -> slug_error::Result<MutexGuard<'_, DocsCache>> {
...
async fn get_builtin_globals_docs(dice_ctx: &mut DiceTransaction) -> slug_error::Result<DocModule> {
    Ok(dice_ctx
...
impl DocsCache {
    async fn get_prelude_uri(
        ...
        Url::from_file_path(abs_path)
            .unwrap()
            .try_into()
            .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::Lsp))
    }
    ...
    async fn new_with_lookup<
        ...
        Fut: Future<Output = slug_error::Result<LspUrl>>,
    >(
...
impl<'a> BuckLspContext<'a> {
    async fn new(
        server_ctx: &'a dyn ServerCommandContextTrait,
    ) -> slug_error::Result<BuckLspContext<'a>> {
    ...
    async fn with_dice_ctx<F, Fut, R>(&self, exec: F) -> slug_error::Result<R>
    where
        F: FnOnce(DiceTransaction) -> Fut + Send,
        Fut: Future<Output = slug_error::Result<R>> + Send,
        R: Send,
    {
        self.server_ctx
    ...
    async fn import_path(&self, path: &Path) -> slug_error::Result<OwnedStarlarkModulePath> {
    ...
    async fn parse_file_from_contents_and_handle_diagnostic(
        ...
        uri: &LspUrl,
        content: String,
    ) -> slug_error::Result<LspEvalResult> {
    ...
    async fn parse_file_from_string(
        ...
        literal: &str,
    ) -> slug_error::Result<Option<StringLiteralResult>> {
                        ...
                        Ok(DicePackageListingResolver(&mut dice_ctx)
                            ...
                            .and_then(|listing| {
                                ...
                                match Url::from_file_path(path).unwrap().try_into() {
                                    ...
                                    Err(e) => Err(from_any_with_tag(e, slug_error::ErrorTag::Lsp)),
                                }
                            }))
                    })
                    .await?;
        ...
        self.runtime
            .block_on(with_dispatcher_async(dispatcher, async {
                match current_file {
                    ...
                    _ => Err(slug_error::Error::from(ResolveLoadError::WrongScheme(
                        "file://".to_owned(),
                        current_file.clone(),
                    ))
                    .into()),
    ...
    fn resolve_string_literal(
        ...
        self.runtime
            .block_on(with_dispatcher_async(dispatcher, async {
                ...
                //
                // slug_error::Error sort of obscures the root causes, so we just have
        ...
        self.runtime
            .block_on(with_dispatcher_async(dispatcher, async {
                let docs_cache = self
                    ...
                    .await?;
                slug_error::Ok(docs_cache.url_for_symbol(symbol).cloned())
            }))
            .map_err(|e| format!("{:#}", e))
    }
...
pub(crate) async fn run_lsp_server_command(
    ...
    partial_result_dispatcher: PartialResultDispatcher<slug_cli_proto::LspMessage>,
    req: StreamingRequestHandler<slug_cli_proto::LspRequest>,
) -> slug_error::Result<slug_cli_proto::LspResponse> {
    let start_event = ctx
        .command_start_event(slug_data::LspCommandStart {}.into())
        .await?;
    span_async(start_event, async move {
        let result = run_lsp_server(ctx, partial_result_dispatcher, req)
            .await
            .map_err(Into::into);
        let end_event = command_end(&result, slug_data::LspCommandEnd {});
...
async fn run_lsp_server(
    ctx: &dyn ServerCommandContextTrait,
    mut partial_result_dispatcher: PartialResultDispatcher<slug_cli_proto::LspMessage>,
    mut req: StreamingRequestHandler<LspRequest>,
) -> slug_error::Result<LspResponse> {
...
async fn recv_from_lsp(
    ...
    mut event_sender: UnboundedSender<slug_cli_proto::LspMessage>,
) -> slug_error::Result<()> {
    loop {
        let msg = to_client
            .recv()
        ...
        let res = slug_cli_proto::LspMessage { lsp_json };
        match event_sender.send(res).await {
            Ok(_) => {}
            Err(e) if e.is_disconnected() => break Ok(()),
            Err(e) => break Err(from_any_with_tag(e, slug_error::ErrorTag::Lsp)),
        }
    }
}
...
fn handle_incoming_lsp_message(
    ...
    message: slug_error::Result<LspRequest>,
) -> slug_error::Result<Option<LspResponse>> {
    if let Ok(m) = message {
...
fn handle_outgoing_lsp_message(
    partial_result_dispatcher: &mut PartialResultDispatcher<slug_cli_proto::LspMessage>,
    event: Option<slug_cli_proto::LspMessage>,
) -> Option<LspResponse> {
    match event {
        Some(event) => {
...
mod tests {
    ...
    #[test]
    fn cache_builds() -> slug_error::Result<()> {
        ...
        async fn lookup_function(location: &ImportPath) -> slug_error::Result<LspUrl> {
