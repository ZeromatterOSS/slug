...
use slug_execute::execute::blocking::BuckBlockingExecutor;
use slug_execute::execute::blocking::DirectIoExecutor;
...
use slug_execute_impl::materializers::deferred::AccessTimesUpdates;
use slug_execute_impl::materializers::deferred::DeferredMaterializer;
...
pub struct DaemonStateData {
    ...
    /// What slug state to store on disk, ex. materializer state on sqlite
...
impl DaemonStateData {
    pub fn dice_dump(&self, path: &Path, format: DiceDumpFormat) -> slug_error::Result<()> {
...
impl DaemonStatePanicDiceDump for DaemonStateData {
    fn dice_dump(&self, path: &Path, format: DiceDumpFormat) -> slug_error::Result<()> {
...
impl DaemonState {
    ...
    async fn init_data(
        ...
        cgroup_tree: Option<BuckCgroupTree>,
        daemon_id: DaemonId,
    ) -> slug_error::Result<Arc<DaemonStateData>> {
        if slug_env!(
        ...
        rt.spawn(async move {
            ...
            // since that file no longer drives any slug behaviour
            ...
            if fs.root().as_path().join(".buckconfig").exists() {
                tracing::warn!(
                    "Detected `.buckconfig` in workspace root `{}`. \
                     Slug no longer reads `.buckconfig` (Plan 35); the file is dead. \
            ...
            let default_digest_algorithm =
                slug_env!("BUCK_DEFAULT_DIGEST_ALGORITHM", type=DigestAlgorithmFamily)?;
            ...
            // run with whatever `[slug_re_client]` was in `.buckconfig`
            ...
            for (cell, instance) in cells.cells() {
                ...
                let bazelignore_rel =
                    instance
                        .path()
                        .join(slug_core::cells::paths::CellRelativePath::unchecked_new(
                            ".bazelignore",
                        ));
                let bazelignore_abs = fs.resolve(&bazelignore_rel);
                let bazelignore_spec = slug_fs::fs_util::read_to_string_if_exists(&bazelignore_abs)
                    .ok()
                    .flatten()
                    .map(|content| slug_common::ignores::bazelignore::parse_bazelignore(&content));
    ...
    ) -> slug_error::Result<Arc<dyn Materializer>> {
    ...
    fn init_scribe_sink(
        fb: FacebookInit,
        config: ScribeConfig,
    ) -> slug_error::Result<Option<Arc<dyn EventSinkWithStats>>> {
        facebook_only();
    ...
    pub async fn prepare_events(
        &self,
        trace_id: TraceId,
    ) -> slug_error::Result<(ChannelEventSource, EventDispatcher)> {
        // facebook only: logging events to Scribe.
        facebook_only();
        let (events, sink) = slug_events::create_source_sink_pair();
        let data = self.data();
    ...
    pub async fn prepare_command(
    ...
    ) -> slug_error::Result<BaseServerCommandContext> {
        let data = self.data();

        dispatcher.instant_event(slug_data::RestartConfiguration {
        ...
        dispatcher.instant_event(slug_data::TagEvent {
        ...
        dispatcher.instant_event(slug_data::IoProviderInfo { eden_version });
    ...
    pub fn validate_cwd(&self) -> slug_error::Result<()> {
    ...
    pub fn validate_buck_out_mount(&self) -> slug_error::Result<()> {
        #[cfg(fbcode_build)]
        {
...
fn apply_re_config_overlay(
    ...
    snapshot: &slug_common::init::ReConfigSnapshot,
) {
    let oss = &mut metadata.0;
    if let Some(ref addr) = snapshot.address {
        // `address` is the catch-all default in
        // `SlugOssReConfiguration::from_legacy_config`; populate every
        ...
        .or(snapshot.engine_address.as_deref())
    {
        ...
        // TLS. The prior `[slug_re_client] tls = false` opt-out is
...
fn apply_re_config_overlay(
    ...
    _snapshot: &slug_common::init::ReConfigSnapshot,
) {
    // Internal RE config has its own pathway; CLI-flag overlays for
    // OSS RE wiring are not relevant inside fbcode.
}

fn convert_algorithm_kind(kind: DigestAlgorithmFamily) -> slug_error::Result<DigestAlgorithm> {
    slug_error::Ok(match kind {
...
) -> slug_error::Result<HttpClientBuilder> {
...
mod tests {
    ...
    #[cfg(not(fbcode_build))]
    #[test]
    fn re_config_overlay_keeps_remote_cache_cache_only() {
        let mut metadata = RemoteExecutionStaticMetadata::default();
        let snapshot = slug_common::init::ReConfigSnapshot {
            address: None,
            cas_address: Some("grpc://127.0.0.1:50052".to_owned()),
            engine_address: None,
            action_cache_address: Some("grpc://127.0.0.1:50052".to_owned()),
            tls: None,
        ...
        let oss = &metadata.0;
        assert_eq!(oss.cas_address.as_deref(), Some("grpc://127.0.0.1:50052"));
        assert_eq!(oss.engine_address, None);
        assert_eq!(
            oss.action_cache_address.as_deref(),
            Some("grpc://127.0.0.1:50052")
        );
        assert!(!metadata.is_re_configured());
        assert_eq!(oss.http_headers.len(), 1);
        assert_eq!(oss.http_headers[0].key, "authorization");
        assert_eq!(oss.http_headers[0].value, "Bearer test");
        assert_eq!(oss.instance_name.as_deref(), Some("main"));
    ...
    async fn test_from_startup_config_defaults_internal() -> slug_error::Result<()> {
        ...
        assert_eq!(
            ...
            slug_certs::certs::supports_vpnless()
        );
        assert_eq!(
