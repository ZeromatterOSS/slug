...
use futures::stream;
use slug_build_api::configure_dice::configure_dice_for_buck;
use slug_build_api::spawner::BuckSpawner;
use slug_certs::validate::CertState;
use slug_certs::validate::check_cert_state;
use slug_certs::validate::validate_certs;
use slug_cli_proto::daemon_api_server::*;
use slug_cli_proto::*;
...
use slug_core::error::reload_hard_error_config;
use slug_core::error::reset_soft_error_counters;
...
use slug_fs::cwd::WorkingDirectory;
use slug_fs::fs_util;
use slug_fs::fs_util::DiskSpaceStats;
use slug_fs::fs_util::disk_space_stats;
use slug_fs::paths::abs_path::AbsPathBuf;
use slug_interpreter::starlark_profiler::config::StarlarkProfilerConfiguration;
use slug_profile::proto_to_profile_mode;
use slug_profile::starlark_profiler_configuration_from_request;
use slug_resource_control::buck_cgroup_tree::BuckCgroupTree;
use slug_resource_control::buck_cgroup_tree::PreppedBuckCgroups;
use slug_server_ctx::bxl::BXL_SERVER_COMMANDS;
use slug_server_ctx::late_bindings::AUDIT_SERVER_COMMAND;
use slug_server_ctx::late_bindings::OTHER_SERVER_COMMANDS;
use slug_server_ctx::late_bindings::QUERY_SERVER_COMMANDS;
use slug_server_ctx::late_bindings::STARLARK_SERVER_COMMAND;
use slug_server_ctx::late_bindings::TARGETS_SERVER_COMMANDS;
...
use slug_server_starlark_debug::run::run_dap_server_command;
use slug_test::executor_launcher::get_all_test_executors;
use slug_util::system_stats::system_memory_stats;
...
impl DaemonShutdown {
    ...
    fn start_shutdown(&self, reason: slug_data::DaemonShutdown, timeout: Option<Duration>) {
...
impl Interceptor for BuckCheckAuthTokenInterceptor {
    fn call(&mut self, request: Request<()>) -> Result<Request<()>, Status> {
        ...
        if slug_env!("BUCK2_TEST_FAIL_BUCKD_AUTH", bool, applicability = testing).unwrap() {
...
pub(crate) struct BuckdServerData {
    ...
    base_daemon_constraints: slug_cli_proto::DaemonConstraints,
...
impl BuckdServer {
    pub async fn run(
        ...
        #[cfg(fbcode_build)]
        {
            ...
            if !slug_env!("BUCK2_DISABLE_EDEN_HEALTH_CHECK", bool)?
    ...
    where
        F: for<'a> FnOnce(
            ...
            ) -> BoxFuture<'a, slug_error::Result<Res>>
            + Send
            + 'static,
        Req: TryFrom<StreamingRequest, Error = slug_error::Error> + Send + Sync + 'static,
        ...
        PartialRes: Into<partial_result::PartialResult> + Send + 'static,
    {
        ...
        let init_request = match req.message().await? {
            Some(
                m @ StreamingRequest {
                    request: Some(slug_cli_proto::streaming_request::Request::Context(_)),
                },
            ) => Ok(m),
    ...
    async fn run_streaming_fallible<Req, Res, PartialRes, F>(
        ...
        func: F,
    ) -> slug_error::Result<Response<ResponseStream>>
    where
        F: for<'a> FnOnce(
                &'a ServerCommandContext,
                PartialResultDispatcher<PartialRes>,
        ...
        PartialRes: Into<partial_result::PartialResult> + Send + 'static,
    {
        ...
        if slug_env!("BUCK2_TEST_FAIL_STREAMING", bool, applicability = testing).unwrap() {
        ...
        reload_hard_error_config(&client_ctx.slug_hard_error)?;
        ...
        dispatch.instant_event(slug_data::SystemInfo {
        ...
        let memory_reporter = daemon_state.data.memory_tracker.as_ref().map(|t| {
            slug_resource_control::memory_tracker::spawn_memory_reporter(dispatch.dupe(), t.dupe())
        });
        ...
        let resp = streaming(
            ...
            move |req, cancellations| {
                async move {
                    let result: slug_error::Result<Res> = try {
                        let base_context =
    ...
    async fn oneshot<
        ...
        Fut: Future<Output = slug_error::Result<Res>> + Send,
        F: FnOnce(Req) -> Fut,
    >(
        &self,
        req: Request<Req>,
...
fn error_to_command_result(e: slug_error::Error) -> CommandResult {
    CommandResult {
        result: Some(command_result::Result::Error(slug_data::ErrorReport::from(
            &e,
        ))),
    }
}
...
fn error_to_command_progress(e: slug_error::Error) -> CommandProgress {
    CommandProgress {
...
fn error_to_response_stream(e: slug_error::Error) -> Response<ResponseStream> {
    tonic::Response::new(Box::pin(stream::once(future::ready(Ok(
        slug_cli_proto::MultiCommandProgress {
...
fn pump_events(
    ...
    output_send: tokio::sync::mpsc::UnboundedSender<
        Result<slug_cli_proto::CommandProgress, tonic::Status>,
    >,
) {
...
fn streaming<Req, F>(
    ...
    daemon_shutdown_channel: oneshot::Receiver<slug_data::DaemonShutdown>,
    func: F,
    rt: &Handle,
    ...
    F: for<'a> FnOnce(Req, &'a CancellationContext) -> BoxFuture<'a, ()> + Send + 'static,
{
    ...
    if let Err(e) = merge_task {
        return error_to_response_stream(
            slug_error::Error::from(e).context("failed to spawn pump-events"),
        );
    };
...
impl<Req> StreamingCommandOptions<Req> for QueryCommandOptions {
    fn starlark_profiler_instrumentation_override(
        ...
        match self.profile_mode {
            ...
            Some(mode) => {
                let mode = slug_cli_proto::ProfileMode::try_from(mode)
...
impl DaemonApi for BuckdServer {
    async fn kill(&self, req: Request<KillRequest>) -> Result<Response<CommandResult>, Status> {
        ...
        self.oneshot(req, KillRunCommandOptions, move |req| async move {
            ...
            let reason = slug_data::DaemonShutdown {
                reason: req.reason,
                callers: req.callers,
            };
    ...
    async fn status(&self, req: Request<StatusRequest>) -> Result<Response<CommandResult>, Status> {
        ...
        self.oneshot(req, DefaultCommandOptions, move |req| async move {
            ...
            let extra_constraints = slug_cli_proto::ExtraDaemonConstraints {
    ...
    async fn flush_dep_files(
        ...
        self.oneshot(req, DefaultCommandOptions, move |req| async move {
            ...
            if retain_locally_produced_dep_files {
                slug_file_watcher::dep_files::flush_non_local_dep_files();
            } else {
                slug_file_watcher::dep_files::flush_dep_files();
            }
            Ok(GenericResponse {})
        })
        .await
    ...
    async fn unstable_dice_dump(
        ...
        let res: slug_error::Result<_> = try {
            let path = Path::new(&path);
            let format_proto =
                slug_cli_proto::unstable_dice_dump_request::DiceDumpFormat::try_from(inner.format)
            ...
            |ctx, partial_result_dispatcher, req| {
                Box::pin(async {
                    match req.profile_opts.as_ref().expect("Missing profile opts") {
                        slug_cli_proto::profile_request::ProfileOpts::TargetProfile(_) => {
                            profile_command(ctx, partial_result_dispatcher, req).await
                        }
                        slug_cli_proto::profile_request::ProfileOpts::BxlProfile(_) => {
...
) -> slug_error::Result<impl Future<Output = ()>> {
...
mod eden_health {
    ...
    use slug_eden::connection::EdenConnectionManager;
    use slug_eden::error::ErrorFromHangingMount;
    use slug_eden::semaphore;
    ...
    pub(crate) async fn edenfs_health_check(fb: fbinit::FacebookInit, root: ProjectRoot) {
        tokio::task::spawn(async move {
            ...
            loop {
                ...
                match EdenConnectionManager::new(fb, &root, Some(semaphore::slug_default())) {
                    Ok(Some(conn)) => {
                        let info = conn
                            .with_eden(|eden| {
                        ...
                        match info {
                            Err(e) if e.is_caused_by_hanging_mount() => {
                                ...
                                soft_error!(
                                    ...
                                    slug_error!(slug_error::ErrorTag::Input, "check failed with: {:#}", e),
                                    quiet: true
                                )
                                .ok();
                            }
                    ...
                    Err(e) => {
                        ...
                        soft_error!(
                            ...
                            slug_error!(slug_error::ErrorTag::Input, "client creation failed with: {:#}", e),
                            quiet: true
                        )
                        .ok();
                    }
