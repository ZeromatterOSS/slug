...
pub fn check_working_dir() -> slug_error::Result<()> {
    use std::fs;
    use std::io;
    ...
    if err.kind() == io::ErrorKind::NotConnected {
        let err = "Slug is running in an Eden mount but Eden restarted uncleanly. \
            This error is unrecoverable and you should restart Buck using `slug killall`.";
    ...
    if err.kind() != io::ErrorKind::NotFound {
        tracing::warn!(
            "Slug is unable to read its current working directory: {}. Consider restarting",
            err
        );
    }
