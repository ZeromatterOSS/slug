...
use slug_execute_impl::sqlite::materializer_db::MATERIALIZER_DB_SCHEMA_VERSION;
...
fn sqlite_db_setup_metadata_and_versions(
    ...
    daemon_id: &DaemonId,
) -> slug_error::Result<(HashMap<String, String>, HashMap<String, String>)> {
    let metadata = slug_events::metadata::collect(&daemon_id);
...
pub(crate) async fn maybe_initialize_materializer_sqlite_db(
    ...
    daemon_id: &DaemonId,
) -> slug_error::Result<(Option<MaterializerStateSqliteDb>, Option<MaterializerState>)> {
...
pub(crate) async fn maybe_initialize_incremental_sqlite_db(
    ...
    _root_config: &LegacyBuckConfig,
    daemon_id: &DaemonId,
) -> slug_error::Result<IncrementalDbState> {
...
// cache is disabled, causing slug to use stale cache when reading from the
// cache is re-enabled. One way this can happen is that someone can build on
// an older revision with a slug that doesn't understand the cache directory
// in between 2 builds on newer revisions with slug that reads from the cache
// (for ex., as a part of a bisect), then the state can become stale.
// There are 2 (not foolproof) mitigations planned:
// 1) Read from the logs what the last slug invocation was and check that the
// last slug supported on-disk state. If not, delete the disk state.
