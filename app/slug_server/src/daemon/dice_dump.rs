...
pub(crate) fn tar_dice_dump(dice_dump_folder: &Path) -> slug_error::Result<()> {
...
fn dice_dump_tsv(dice: &Arc<Dice>, path: &Path) -> slug_error::Result<()> {
    let path = path.to_path_buf();
...
fn dice_dump_bincode(dice: &Arc<Dice>, path: &Path) -> slug_error::Result<()> {
    let path = path.to_path_buf();
...
fn dice_dump_json_pretty(dice: &Arc<Dice>, path: &Path) -> slug_error::Result<()> {
    let path = path.to_path_buf();
    std::fs::create_dir_all(path.parent().unwrap())
        .buck_error_context("Failed to create directory")?;
    let out = File::create(&path)
