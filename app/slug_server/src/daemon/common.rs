...
use slug_build_api::actions::execute::dice_data::HasCommandExecutor;
...
use slug_execute_impl::executors::action_cache::ActionCacheChecker;
use slug_execute_impl::executors::action_cache::RemoteDepFileCacheChecker;
use slug_execute_impl::executors::action_cache_upload_permission_checker::ActionCacheUploadPermissionChecker;
use slug_execute_impl::executors::caching::CacheUploader;
use slug_execute_impl::executors::hybrid::FallbackTracker;
use slug_execute_impl::executors::hybrid::HybridExecutor;
use slug_execute_impl::executors::local::ForkserverAccess;
use slug_execute_impl::executors::local::LocalExecutor;
use slug_execute_impl::executors::re::ReExecutor;
use slug_execute_impl::executors::stacked::StackedExecutor;
use slug_execute_impl::executors::to_re_platform::RePlatformFieldsToRePlatform;
...
impl HasCommandExecutor for CommandExecutorFactory {
    fn get_command_executor(
    ...
    ) -> slug_error::Result<CommandExecutorResponse> {
        ...
        let response = match &executor_config.executor {
            ...
            Executor::RemoteEnabled(remote_options) => {
                ...
                let disable_caching =
                    slug_env!("BUCK2_TEST_DISABLE_CACHING", type=bool, applicability=testing)?
                ...
                let only_remote_dep_file_cache = slug_env!(
        ...
        let response = response
            .with_buck_error_context(|| format!("The desired execution strategy (`{:?}`) is incompatible with the executor config that was selected: {:?}", self.strategy, executor_config)).tag(slug_error::ErrorTag::Input)?;
...
fn get_default_re_properties(
    ...
    // `container-image`, etc.) and the slug-internal
