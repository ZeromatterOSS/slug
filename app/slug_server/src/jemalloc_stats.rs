...
use slug_common::memory::allocator_stats;

// TODO(raulgarcia4): Consider moving out this file to slug_common,
// similarly to what was done with slug_common::process_stats.
pub struct AllocatorStats {
    pub bytes_active: Option<u64>,
    pub bytes_allocated: Option<u64>,
}

pub fn get_allocator_stats() -> slug_error::Result<AllocatorStats> {
    fn set(stats: &serde_json::Value, val: &str, to: &mut Option<u64>) -> slug_error::Result<()> {
        if let serde_json::Value::Number(stat) = &stats["jemalloc"]["stats"][val] {
            *to = stat.as_u64();
            Ok(())
        } else {
            Err(slug_error::slug_error!(
                slug_error::ErrorTag::MallocStats,
...
#[cfg(test)]
mod tests {
    use slug_common::memory::has_jemalloc_stats;

    use crate::jemalloc_stats::get_allocator_stats;

    #[test]
    fn test_get_allocator_stats() -> slug_error::Result<()> {
