...
use slug_data::VersionControlRevision;
        ...
        while let Some(event) = tasks.next().await {
            if let Some(error) = &event.command_error {
                soft_error!(
                    ...
                    slug_error::slug_error!(slug_error::ErrorTag::Input, "{}", error),
                    quiet: true
                )
                .ok();
            }
...
) -> slug_data::VersionControlRevision {
    let mut revision = slug_data::VersionControlRevision::default();
...
async fn create_hg_data(
    revision: &mut slug_data::VersionControlRevision,
    revision_type: RevisionDataType,
    repo_root: &AbsNormPathBuf,
) {
    match revision_type {
...
async fn get_hg_status(revision: &mut slug_data::VersionControlRevision) {
...
async fn repo_type(repo_root: &AbsNormPathBuf) -> slug_error::Result<&'static RepoVcs> {
    static REPO_TYPE: OnceCell<slug_error::Result<RepoVcs>> = OnceCell::const_new();
    async fn repo_type_impl(repo_root: &AbsNormPathBuf) -> slug_error::Result<RepoVcs> {
