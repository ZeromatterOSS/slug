...
use slug_events::EventSinkStats;
...
use slug_util::process_stats::process_stats;
use slug_util::system_stats::UnixSystemStats;
...
impl SnapshotCollector {
    ...
    pub async fn create_snapshot(&self) -> slug_data::Snapshot {
        let mut snapshot = slug_data::Snapshot::default();
    ...
    fn add_daemon_metrics(&self, snapshot: &mut slug_data::Snapshot) {
        snapshot.blocking_executor_io_queue_size =
            self.daemon.blocking_executor.queue_size() as u64;
    }

    fn add_io_metrics(&self, snapshot: &mut slug_data::Snapshot) {
    ...
    fn add_re_metrics(&self, snapshot: &mut slug_data::Snapshot) {
        fn inner(
            snapshot: &mut slug_data::Snapshot,
    ...
    fn add_http_metrics(&self, snapshot: &mut slug_data::Snapshot) {
        snapshot.http_download_bytes = self.daemon.http_client.stats().get_downloaded_bytes();
    }

    fn add_dice_metrics(&self, snapshot: &mut slug_data::Snapshot) {
    ...
    fn add_materializer_metrics(&self, snapshot: &mut slug_data::Snapshot) {
        self.daemon.materializer.add_snapshot_stats(snapshot);
    }

    fn add_sink_metrics(&self, snapshot: &mut slug_data::Snapshot) {
    ...
    fn add_net_io_metrics(&self, snapshot: &mut slug_data::Snapshot) {
        if let Ok(Some(net_io_counters_per_nic)) = self.net_io_collector.collect() {
            snapshot.network_interface_stats = net_io_counters_per_nic
                ...
                .map(|(nic, counters)| {
                    (
                        nic,
                        slug_data::NetworkInterfaceStats {
                            tx_bytes: counters.bytes_sent,
                            rx_bytes: counters.bytes_recv,
                            network_kind: match counters.network_kind {
                                NetworkKind::WiFi => slug_data::NetworkKind::WiFi.into(),
                                NetworkKind::Ethernet => slug_data::NetworkKind::Ethernet.into(),
                                NetworkKind::Unknown => {
                                    slug_data::NetworkKind::UnknownNetKind.into()
                                }
                            },
                        },
                    )
    ...
    fn add_system_metrics(&self, snapshot: &mut slug_data::Snapshot) {
        ...
        {
            snapshot.unix_system_stats = Some(slug_data::UnixSystemStats {
                load1,
                load5,
                load15,
            });
    ...
    fn add_cpu_usage(&self, snapshot: &mut slug_data::Snapshot) {
    ...
    async fn add_memory_metrics(&self, snapshot: &mut slug_data::Snapshot) {
        #[cfg(not(unix))]
        {
        ...
        #[cfg(unix)]
        {
            use slug_resource_control::cgroup_files::MemoryStat;

            fn convert_stats(stats: &MemoryStat) -> slug_data::UnixCgroupMemoryStats {
                slug_data::UnixCgroupMemoryStats {
                    anon: stats.anon,
                    file: stats.file,
                    kernel: stats.kernel,
                }
            ...
            // Try to read Slug daemon memory information from cgroup
