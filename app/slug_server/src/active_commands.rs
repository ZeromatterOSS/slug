...
use slug_event_observer::span_tracker::RootData;
use slug_event_observer::span_tracker::Roots;
...
pub fn broadcast_instant_event<E: Into<slug_data::instant_event::Data> + Clone>(event: &E) -> bool {
...
pub fn broadcast_shutdown(shutdown: &slug_data::DaemonShutdown) {
    for cmd in ACTIVE_COMMANDS.lock().values() {
...
pub struct ActiveCommandHandle {
    ...
    daemon_shutdown_channel: Arc<Mutex<Option<oneshot::Sender<slug_data::DaemonShutdown>>>>,
...
impl ActiveCommandHandle {
    fn notify_shutdown(&self, shutdown: slug_data::DaemonShutdown) {
...
pub struct ActiveCommand {
    ...
    pub daemon_shutdown_channel: oneshot::Receiver<slug_data::DaemonShutdown>,
}
...
impl ActiveCommand {
    pub fn new(event_dispatcher: &EventDispatcher, sanitized_argv: Vec<String>) -> Self {
        ...
        if let Some(commands) = result {
            ...
            event_dispatcher.instant_event(slug_data::ConcurrentCommands {
                trace_ids: commands.keys().map(|cmd| cmd.to_string()).collect(),
            });

            // Notify other commands that they are concurrent with ours.
            for cmd in commands.values() {
                cmd.dispatcher.instant_event(slug_data::ConcurrentCommands {
...
mod tests {
    ...
    fn test_active_command_state() {
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            Some(root),
            None,
            slug_data::SpanStartEvent {
                data: Some(slug_data::AnalysisStart::default().into()),
            }
            .into(),
        ));
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            Some(root),
            slug_data::SpanStartEvent {
                data: Some(slug_data::AnalysisStageStart::default().into()),
            }
            .into(),
        ));
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            Some(child),
            Some(root),
            slug_data::SpanEndEvent {
                data: Some(slug_data::AnalysisStageEnd::default().into()),
                ..Default::default()
            }
            .into(),
        ));
        ...
        writer.peek_event(&BuckEvent::new(
            ...
            None,
            slug_data::SpanEndEvent {
                data: Some(slug_data::AnalysisEnd::default().into()),
                ..Default::default()
            }
            .into(),
        ));
        ...
        assert_matches!(event, Some(Event::Buck(event)) => {
            assert_matches!(
                ...
                slug_data::buck_event::Data::Instant(slug_data::InstantEvent {
                    data: Some(slug_data::instant_event::Data::ConcurrentCommands(
                        slug_data::ConcurrentCommands {
                            trace_ids,
                        }
                    ))
                }) => {
