...
impl DynamicActionsOutputArtifactBinder {
    ...
    ) -> kuro_error::Result<BoundBuildArtifact> {
...
) -> kuro_error::Result<Box<[ValueTyped<'v, StarlarkOutputArtifact<'v>>]>> {
...
pub(crate) fn analysis_actions_methods_dynamic_output(methods: &mut MethodsBuilder) {
    ...
    /// https://kuro.build/docs/rule_authors/dynamic_dependencies/.
    ///
    ...
    fn dynamic_output<'v>(
        ...
        if outputs.items.is_empty() {
            return Err(kuro_error::Error::from(DynamicOutputError::EmptyOutput).into());
        }
    ...
    /// overview of dynamic dependencies in kuro.
    ///
