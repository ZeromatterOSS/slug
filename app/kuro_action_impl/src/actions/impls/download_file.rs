...
use kuro_common::cas_digest::RawDigest;
...
use kuro_execute::materialize::http::http_head;
...
impl DownloadFileAction {
    ...
    ) -> kuro_error::Result<Option<FileMetadata>> {
        ...
        let size = match self.inner.size_bytes {
            ...
            None => {
                ...
                let head = http_head(client, url).await.map_err(|e| {
                    kuro_error::Error::from(e).tag([ErrorTag::DownloadFileHeadRequest])
                })?;
                ...
                head.headers()
                    ...
                    .map(|content_length| {
                        let content_length = content_length
                            .to_str()
                            .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::Http))
                        ...
                        kuro_error::Ok(content_length_number)
                    })
                    .transpose()
...
impl Action for DownloadFileAction {
    fn kind(&self) -> kuro_data::ActionKind {
        kuro_data::ActionKind::DownloadFile
    }
