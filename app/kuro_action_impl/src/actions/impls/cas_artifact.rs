...
use kuro_execute::directory::re_directory_to_re_tree;
...
enum CasArtifactActionDeclarationError {
    ...
    #[kuro(tag = ReCasArtifactWrongNumberOfOutputs)]
...
enum CasArtifactActionExecutionError {
    ...
    #[kuro(tag = ReCasArtifactInvalidExpiration)]
    InvalidExpiration {
        digest: FileDigest,
...
impl CasArtifactAction {
    ...
    ) -> kuro_error::Result<(ActionOutputs, ActionExecutionMetadata)> {
...
impl Action for CasArtifactAction {
    fn kind(&self) -> kuro_data::ActionKind {
        kuro_data::ActionKind::CasArtifact
    }
    ...
    async fn execute(
        ...
        let get_expiration = || async {
            kuro_error::Ok(
                re_client
                    ...
                    .tag(kuro_error::ErrorTag::ReCasArtifactGetDigestExpirationError)?
                    .1,
            )
        };
        ...
        if expiration < self.inner.expires_after {
            ...
            let error: kuro_error::Error = CasArtifactActionExecutionError::InvalidExpiration {
