...
use kuro_action_metadata_proto::DepFileInputs;
...
use kuro_common::cas_digest::CasDigestData;
...
use kuro_directory::directory::directory_hasher::DirectoryDigester;
use kuro_directory::directory::directory_ref::DirectoryRef;
use kuro_directory::directory::directory_selector::DirectorySelector;
...
use kuro_execute::directory::ActionFingerprintedDirectoryRef;
use kuro_execute::directory::ActionImmutableDirectory;
...
use kuro_execute::directory::ReDirectorySerializer;
use kuro_execute::directory::expand_selector_for_dependencies;
use kuro_execute::execute::action_digest_and_blobs::ActionDigestAndBlobs;
use kuro_execute::execute::action_digest_and_blobs::ActionDigestAndBlobsBuilder;
...
use kuro_file_watcher::dep_files::FLUSH_DEP_FILES;
use kuro_file_watcher::dep_files::FLUSH_NON_LOCAL_DEP_FILES;
use kuro_fs::fs_util;
...
use kuro_fs::paths::forward_rel_path::ForwardRelativePathNormalizer;
...
/// for debugging via `kuro audit dep-files`.
fn keep_directories() -> kuro_error::Result<bool> {
    kuro_env!("BUCK2_KEEP_DEP_FILE_DIRECTORIES", bool)
}
...
impl DepFileState {
    ...
    pub(crate) fn locked_compute_fingerprints<'a>(
        ...
        fs: &ArtifactFs,
    ) -> kuro_error::Result<MappedMutexGuard<'a, StoredFingerprints>> {
...
impl IntoRemoteDepFile for DepFileBundle {
    ...
    ) -> kuro_error::Result<Option<RemoteDepFile>> {
...
impl DepFileBundle {
    ...
    ) -> kuro_error::Result<(Option<(ActionOutputs, ActionExecutionMetadata)>, bool)> {
    ...
    pub(crate) async fn check_remote_dep_file_entry(
        ...
        let computed_filtered_fingerprints = self
            .shared_declared_inputs
            .clone()
            .ok_or_else(|| kuro_error::internal_error!("Dep files should have been declared"))?
            .unshare()
            .filter(dep_files, fs)?
...
) -> kuro_error::Result<DepFileBundle> {
...
pub(crate) async fn match_if_identical_action(
    ...
    declared_outputs: &[BuildArtifact],
    declared_dep_files: &DeclaredDepFiles,
) -> kuro_error::Result<(Option<ActionOutputs>, bool)> {
...
pub(crate) async fn match_or_clear_dep_file(
    ...
    declared_outputs: &[BuildArtifact],
    declared_dep_files: &DeclaredDepFiles,
) -> kuro_error::Result<Option<ActionOutputs>> {
    let previous_state = match get_dep_files(key) {
        Some(d) => d.dupe(),
        None => return Ok(None),
    };
...
async fn outputs_match(
    ...
    let output_matches = previous_state
        ...
        .collect::<kuro_error::Result<Vec<(ProjectRelativePathBuf, ArtifactValue)>>>()?;
...
async fn dep_files_match(
    ...
    .buck_error_context(
        "Error reading persisted dep files. \
            ...
            You may also use `kuro debug flush-dep-files` to drop all dep file state.",
    )?;
...
pub(crate) async fn read_dep_files(
    ...
    fs: &ArtifactFs,
    materializer: &dyn Materializer,
) -> kuro_error::Result<Option<ConcreteDepFiles>> {
...
fn compute_fingerprints(
    ...
    keep_directories: bool,
    fs: &ArtifactFs,
) -> kuro_error::Result<StoredFingerprints> {
...
impl PartitionedInputs<Vec<ArtifactGroup>> {
    ...
    fn to_directories(
        &self,
        ctx: &dyn ActionExecutionCtx,
    ) -> kuro_error::Result<PartitionedInputs<ActionDirectoryBuilder>> {
        fn reduce(
            ctx: &dyn ActionExecutionCtx,
            inputs: &[ArtifactGroup],
        ) -> kuro_error::Result<ActionDirectoryBuilder> {
        ...
        Ok(PartitionedInputs {
            ...
            tagged: self
                .tagged
                .iter()
                .map(|(tag, inputs)| kuro_error::Ok((tag.dupe(), reduce(ctx, inputs)?)))
                .collect::<Result<_, _>>()?,
        })
    }
}
...
impl DeclaredDepFiles {
    ...
    fn read(
        ...
        for declared_dep_file in self.tagged.values() {
            ...
            let read_dep_file: kuro_error::Result<()> = try {
...
impl ConcreteDepFiles {
    ...
    ) -> kuro_error::Result<Option<DirectorySelector>> {
