...
pub(crate) struct DynamicLambdaStaticFields {
    ...
    /// Execution platform inherited from the owner to use for actionsfbcode/kuro/app/kuro_action_impl/src/dynamic/deferred.rs
...
impl FrozenDynamicLambdaParams {
    pub(crate) fn attributes<'v>(
        &'v self,
    ) -> kuro_error::Result<Option<ValueOfUnchecked<'v, StructRef<'static>>>> {
    ...
    pub(crate) fn plugins<'v>(
        &'v self,
    ) -> kuro_error::Result<Option<ValueTypedComplex<'v, AnalysisPlugins<'v>>>> {
