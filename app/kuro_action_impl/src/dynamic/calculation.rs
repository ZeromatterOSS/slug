...
use kuro_build_api::dynamic::calculation::DYNAMIC_LAMBDA_CALCULATION_IMPL;
use kuro_build_api::dynamic::calculation::DynamicLambdaCalculation;
use kuro_build_api::dynamic::calculation::DynamicLambdaResult;
...
impl DynamicLambdaCalculation for DynamicLambdaCalculationImpl {
    ...
    ) -> kuro_error::Result<Arc<DynamicLambdaResult>> {
...
impl Key for DynamicLambdaDiceKey {
    type Value = kuro_error::Result<Arc<DynamicLambdaResult>>;
...
impl BuildSignalsNodeKeyImpl for DynamicLambdaDiceKey {
    fn critical_path_entry_proto(&self) -> Option<kuro_data::critical_path_entry2::Entry> {
        let entry = kuro_data::critical_path_entry2::Entry::DynamicAnalysis(
            kuro_data::critical_path_entry2::DynamicAnalysis {
                target: match self.0.holder_key() {
                    DeferredHolderKey::Base(BaseDeferredKey::TargetLabel(target)) => Some(
                        kuro_data::critical_path_entry2::dynamic_analysis::Target::StandardTarget(
                            target.as_proto(),
                        ),
                    ),
                    _ => None,
