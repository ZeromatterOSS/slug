...
impl KuroTestRunnerTcp {
    pub async fn run(self) -> kuro_error::Result<()> {
        ...
        let orchestrator_addr: SocketAddr = self
            .orchestrator_addr
            .parse()
            .map_err(|e| kuro_error::internal_error!("Invalid orchestrator address {:#}", e))?;
        let executor_addr: SocketAddr = self
            .executor_addr
            .parse()
            .map_err(|e| kuro_error::internal_error!("Invalid executor address {:#}", e))?;
