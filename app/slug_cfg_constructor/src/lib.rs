...
use slug_interpreter::factory::ReentrantStarlarkEvaluator;
...
) -> slug_error::Result<(Vec<String>, Value<'v>)> {
...
async fn analyze_constraints(
    ...
    refs: Vec<String>,
) -> slug_error::Result<SmallMap<String, FrozenProviderCollectionValue>> {
...
fn eval_post_constraint_analysis<'v>(
    ...
    eval.with_evaluator(|eval| -> slug_error::Result<ConfigurationData> {
...
impl CfgConstructorImpl for CfgConstructor {
    ...
    ) -> Pin<Box<dyn Future<Output = slug_error::Result<ConfigurationData>> + Send + 'a>> {
