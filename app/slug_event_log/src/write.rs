...
impl WriteEventLog {
    ...
    async fn log_invocation(&mut self, trace_id: TraceId) -> slug_error::Result<()> {
    ...
    async fn write_ln<'b, T, I>(&'b mut self, events: I) -> slug_error::Result<()>
    where
        T: SerializeForLog + 'b,
    ...
    async fn ensure_log_writers_opened(&mut self, event: &BuckEvent) -> slug_error::Result<()> {
...
) -> slug_error::Result<NamedEventLogWriter> {
    let current_exe = std::env::current_exe().buck_error_context("No current_exe")?;
    let mut command = slug_util::process::async_background_command(current_exe);
...
impl WriteEventLog {
    pub async fn write_events(&mut self, events: &[Arc<BuckEvent>]) -> slug_error::Result<()> {
    ...
    pub async fn flush_files(&mut self) -> slug_error::Result<()> {
...
impl SerializeForLog for Invocation {
    fn serialize_to_json(&self, buf: &mut Vec<u8>) -> slug_error::Result<()> {
        serde_json::to_writer(buf, &self.clone().to_proto())
            .buck_error_context("Failed to serialize event")
    }

    fn serialize_to_protobuf_length_delimited(&self, buf: &mut Vec<u8>) -> slug_error::Result<()> {
        self.clone().to_proto().encode_length_delimited(buf)?;
        Ok(())
    }

    // Always log invocation record to user event log for `slug log show` compatibility
    fn maybe_serialize_user_event(&self, buf: &mut Vec<u8>) -> slug_error::Result<bool> {
        serde_json::to_writer(buf, &self.clone().to_proto())
            .buck_error_context("Failed to serialize event")?;
        Ok(true)
    }
...
impl SerializeForLog for StreamValueForWrite<'_> {
    ...
    fn serialize_to_protobuf_length_delimited(&self, buf: &mut Vec<u8>) -> slug_error::Result<()> {
        ...
        let stream_val = slug_cli_proto::CommandProgressForWrite {
...
mod tests {
    ...
    impl WriteEventLog {
        async fn new_test(log: EventLogPathBuf) -> slug_error::Result<Self> {
            Ok(Self {
                ...
                sanitized_argv: Argv {
                    argv: vec!["slug".to_owned()],
                    expanded_argv: ExpandedArgv::from_literals(vec!["slug".to_owned()]),
                }
                .no_need_to_sanitize(),
    ...
    #[tokio::test]
    async fn test_protobuf_decoding_gzip() -> slug_error::Result<()> {
        test_protobuf_decoding(Encoding::PROTO_GZIP).await
    }

    #[tokio::test]
    async fn test_protobuf_decoding_zstd() -> slug_error::Result<()> {
        test_protobuf_decoding(Encoding::PROTO_ZSTD).await
    }

    async fn test_protobuf_decoding(encoding: Encoding) -> slug_error::Result<()> {
        //Create log dir
    ...
    #[tokio::test]
    async fn test_tick_makes_valid_log_zstd() -> slug_error::Result<()> {
        test_tick_makes_valid_log(Encoding::PROTO_ZSTD).await
    }

    async fn test_tick_makes_valid_log(encoding: Encoding) -> slug_error::Result<()> {
        if cfg!(windows) {
    ...
    fn test_stream_value_serialize_to_protobuf_length_delimited() {
        ...
        StreamValueForWrite::Event(event.event())
            ...
            .unwrap();
        let expected = slug_cli_proto::CommandProgress {
