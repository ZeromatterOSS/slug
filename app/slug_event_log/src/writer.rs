...
impl NamedEventLogWriter {
    ...
    pub(crate) async fn flush(&mut self) -> slug_error::Result<()> {
        match self.file.flush().await {
            ...
            Err(e) => Err(slug_error::Error::from(e).context(format!(
    ...
    fn serialize_event<'b, T>(&self, mut buf: &mut Vec<u8>, event: &T) -> slug_error::Result<()>
    where
        T: SerializeForLog + 'b,
    {
    ...
    async fn write_all(&mut self, buf: &[u8]) -> slug_error::Result<()> {
        match self.file.write_all(buf).await {
            ...
            Err(e) => Err(slug_error::Error::from(e).context("Failed to write event")),
        }
    }
