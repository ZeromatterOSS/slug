...
fn get_files_in_log_dir(logdir: &AbsNormPath) -> slug_error::Result<Vec<AbsNormPathBuf>> {
...
pub fn get_local_logs(logdir: &AbsNormPath) -> slug_error::Result<Vec<EventLogPathBuf>> {
...
pub fn do_find_log_by_trace_id(
    log_dir: &AbsNormPath,
    trace_id: &TraceId,
) -> slug_error::Result<EventLogPathBuf> {
...
pub fn retrieve_all_logs(paths: &InvocationPaths) -> slug_error::Result<Vec<EventLogPathBuf>> {
    let log_dir = paths.log_dir();
...
mod tests {
    ...
    #[tokio::test]
    async fn test_remove_old_logs_with_mix_of_files_and_folders() -> slug_error::Result<()> {
