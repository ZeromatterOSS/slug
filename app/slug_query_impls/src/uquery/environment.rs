...
use slug_query::query::environment::QueryEnvironmentAsNodeLookup;
...
use slug_query::query::syntax::simple::eval::file_set::FileNode;
...
use slug_query::query::syntax::simple::functions::HasModuleDescription;
use slug_query::query::syntax::simple::functions::docs::QueryEnvironmentDescription;
use slug_query::query::traversal::AsyncNodeLookup;
use slug_query::query::traversal::ChildVisitor;
...
pub(crate) trait UqueryDelegate: Send + Sync {
    async fn get_buildfile_names_by_cell(
        &self,
    ) -> slug_error::Result<HashMap<CellName, Arc<[FileNameBuf]>>>;

    /// Resolves a target pattern.
    async fn resolve_target_patterns(
        &self,
        pattern: &[&str],
    ) -> slug_error::Result<ResolvedPattern<TargetPatternExtra>>;
    ...
    async fn get_enclosing_packages(
        &self,
        path: &CellPath,
    ) -> slug_error::Result<Vec<PackageLabel>>;
...
pub(crate) struct PreresolvedQueryLiterals<T: QueryTarget> {
    resolved_literals: HashMap<String, slug_error::Result<TargetSet<T>>>,
}
...
impl<T: QueryTarget> PreresolvedQueryLiterals<T> {
    ...
    pub(crate) async fn pre_resolve(
        ...
        for (literal, result) in resolved_literal_results {
            resolved_literals.insert(literal, result.map_err(slug_error::Error::from));
        }
        Self { resolved_literals }
    }

    /// All the literals, or error if resolution of any failed.
    pub(crate) fn literals(&self) -> slug_error::Result<TargetSet<T>> {
...
impl<'c> UqueryEnvironment<'c> {
    ...
    async fn get_node(&self, target: &TargetLabel) -> slug_error::Result<TargetNode> {
        let package = self
            .delegate
            .ctx()
...
impl QueryEnvironment for UqueryEnvironment<'_> {
    ...
    async fn get_node(&self, node_ref: &TargetLabel) -> slug_error::Result<Self::Target> {
    ...
    async fn eval_literals(&self, literals: &[&str]) -> slug_error::Result<TargetSet<TargetNode>> {
        self.literals
    ...
    async fn dfs_postorder(
        ...
        visit: impl FnMut(TargetNode) -> slug_error::Result<()> + Send,
    ) -> slug_error::Result<()> {
    ...
    async fn owner(&self, paths: &FileSet) -> slug_error::Result<TargetSet<Self::Target>> {
...
pub(crate) async fn rbuildfiles<'c>(
    ...
    impl AsyncNodeLookup<Node> for Lookup {
        async fn get(&self, label: &NodeRef) -> slug_error::Result<Node> {
...
async fn split_universe_files<'c>(
    ...
    delegate: &'c dyn UqueryDelegate,
) -> slug_error::Result<(Vec<ArcCellPath>, Vec<ArcCellPath>)> {
...
async fn top_level_imports_by_build_file<'c>(
    ...
    delegate: &'c dyn UqueryDelegate,
) -> slug_error::Result<HashMap<ArcCellPath, Vec<ImportPath>>> {
...
async fn first_order_imports<'c>(
    ...
    delegate: &'c dyn UqueryDelegate,
) -> slug_error::Result<HashMap<ImportPath, Vec<ImportPath>>> {
...
) -> slug_error::Result<Vec<ImportPath>> {
    #[derive(Clone, Dupe)]
    struct Node(Arc<ImportPath>);
