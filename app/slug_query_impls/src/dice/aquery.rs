...
use slug_build_api::actions::query::ActionInput;
use slug_build_api::actions::query::ActionQueryNode;
use slug_build_api::actions::query::ActionQueryNodeRef;
use slug_build_api::actions::query::SetProjectionInputs;
use slug_build_api::actions::query::iter_action_inputs;
...
use slug_build_api::artifact_groups::TransitiveSetProjectionKey;
...
struct DiceAqueryNodesCache {
    action_nodes: Arc<NodeCache<ActionKey, slug_error::Result<ActionQueryNode>>>,
    tset_nodes: Arc<NodeCache<TransitiveSetProjectionKey, slug_error::Result<SetProjectionInputs>>>,
}
...
async fn convert_inputs<'c, 'a, Iter: IntoIterator<Item = &'a ArtifactGroup>>(
    ...
    inputs: Iter,
) -> slug_error::Result<Vec<ActionInput>> {
...
) -> BoxFuture<'c, slug_error::Result<SetProjectionInputs>> {
    async move {
...
async fn get_tset_node<'c>(
    ...
    ctx: &'c mut DiceComputations<'_>,
    key: TransitiveSetProjectionKey,
) -> slug_error::Result<SetProjectionInputs> {
...
fn compute_action_node<'c>(
    ...
    key: ActionKey,
    fs: Arc<ArtifactFs>,
) -> BoxFuture<'c, slug_error::Result<ActionQueryNode>> {
    async move {
...
async fn get_action_node<'c>(
    ...
    key: ActionKey,
    fs: Arc<ArtifactFs>,
) -> slug_error::Result<ActionQueryNode> {
    let copied_node_cache = node_cache.dupe();
    node_cache
        .action_nodes
        .get_or_compute(key, move |key| {
...
impl<'c, 'd> DiceAqueryDelegate<'c, 'd> {
    pub(crate) async fn new(
        base_delegate: DiceQueryDelegate<'c, 'd>,
    ) -> slug_error::Result<DiceAqueryDelegate<'c, 'd>> {
...
impl AqueryDelegate for DiceAqueryDelegate<'_, '_> {
    ...
    async fn expand_artifacts(
        ...
        slug_util::future::try_join_all(refs.iter().map(|n| self.get_node(n))).await
    }
