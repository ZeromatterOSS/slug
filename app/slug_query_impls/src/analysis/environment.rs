...
use slug_build_api::artifact_groups::ResolvedArtifactGroup;
...
use slug_node::nodes::configured_node_ref::ConfiguredTargetNodeRefNode;
use slug_node::nodes::configured_node_ref::ConfiguredTargetNodeRefNodeDeps;
use slug_node::nodes::configured_ref::ConfiguredGraphNodeRef;
use slug_node::query::query_functions::CONFIGURED_GRAPH_QUERY_FUNCTIONS;
use slug_query::query::environment::QueryEnvironment;
use slug_query::query::environment::TraversalFilter;
use slug_query::query::environment::deps;
use slug_query::query::graph::dfs::dfs_postorder;
use slug_query::query::graph::successors::AsyncChildVisitor;
use slug_query::query::syntax::simple::eval::error::QueryError;
use slug_query::query::syntax::simple::eval::file_set::FileSet;
use slug_query::query::syntax::simple::eval::set::TargetSet;
use slug_query::query::syntax::simple::eval::values::QueryValue;
use slug_query::query::syntax::simple::functions::DefaultQueryFunctionsModule;
use slug_query::query::syntax::simple::functions::QueryFunctions;
use slug_query::query::syntax::simple::functions::helpers::QueryBinaryOp;
use slug_query::query::syntax::simple::functions::helpers::QueryFunction;
use slug_query::query::traversal::NodeLookupId;
use slug_query::query::traversal::async_depth_limited_traversal;
use slug_query::query::traversal::async_fast_depth_first_postorder_traversal;
use slug_query::query_module;
...
pub(crate) trait ConfiguredGraphQueryEnvironmentDelegate: Send + Sync {
    fn eval_literal(&self, literal: &str) -> slug_error::Result<ConfiguredTargetNode>;

    async fn get_targets_from_template_placeholder_info(
        &self,
        template_name: &'static str,
        targets: TargetSet<ConfiguredGraphNodeRef>,
    ) -> slug_error::Result<TargetSet<ConfiguredGraphNodeRef>>;
}
...
impl QueryEnvironment for ConfiguredGraphQueryEnvironment<'_> {
    ...
    ) -> slug_error::Result<MaybeCompatible<Self::Target>> {
    ...
    async fn eval_literals(&self, literal: &[&str]) -> slug_error::Result<TargetSet<Self::Target>> {
    ...
    async fn eval_file_literal(&self, literal: &str) -> slug_error::Result<FileSet> {
    ...
    async fn dfs_postorder(
        ...
        visit: impl FnMut(Self::Target) -> slug_error::Result<()> + Send,
    ) -> slug_error::Result<()> {
    ...
    async fn owner(&self, _paths: &FileSet) -> slug_error::Result<TargetSet<Self::Target>> {
    ...
    async fn targets_in_buildfile(
        &self,
        _paths: &FileSet,
    ) -> slug_error::Result<TargetSet<Self::Target>> {
...
async fn get_template_info_provider_artifacts(
    ...
    template_name: &str,
) -> slug_error::Result<Vec<ArtifactGroup>> {
    let providers_label =
...
) -> slug_error::Result<IndexMap<ConfiguredTargetLabel, Artifact>> {
