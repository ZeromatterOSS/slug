...
use futures::Future;
use slug_common::scope::scope_and_collect_with_dispatcher;
use slug_error::BuckErrorContext;
use slug_events::dispatch::EventDispatcher;
use slug_query::query::environment::QueryEnvironment;
use slug_query::query::syntax::simple::eval::evaluator::QueryEvaluator;
use slug_query::query::syntax::simple::eval::literals::extract_target_literals;
use slug_query::query::syntax::simple::eval::multi_query::MultiQueryResult;
use slug_query::query::syntax::simple::eval::values::QueryEvaluationResult;
use slug_query::query::syntax::simple::eval::values::QueryEvaluationValue;
use slug_query::query::syntax::simple::functions::QueryFunctions;
use slug_query_parser::multi_query::MaybeMultiQuery;
use slug_query_parser::multi_query::MultiQueryItem;

pub(crate) async fn eval_query<
    F: QueryFunctions<Env = Env>,
    Env: QueryEnvironment,
    Fut: Future<Output = slug_error::Result<Env>> + Send,
>(
...
) -> slug_error::Result<QueryEvaluationResult<Env::Target>> {
...
) -> slug_error::Result<QueryEvaluationValue<<Env as QueryEnvironment>::Target>>
where
    F: QueryFunctions<Env = Env>,
    Env: QueryEnvironment,
    Fut: Future<Output = slug_error::Result<Env>>,
{
...
) -> slug_error::Result<MultiQueryResult<Env::Target>>
where
    Qf: QueryFunctions<Env = Env>,
    Env: QueryEnvironment,
    EnvFut: Future<Output = slug_error::Result<Env>> + Send,
{
            ...
            for (i, query) in queries.iter().enumerate() {
                ...
                scope.spawn_cancellable(
                    async move {
                        ...
                        let result: slug_error::Result<_> = result.await.map_err(|e| e.into());
                        (i, arg, result)
                    },
                    move || {
                        (
