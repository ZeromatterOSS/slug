...
use slug_build_api::actions::query::ActionQueryNode;
use slug_build_api::actions::query::ActionQueryNodeRef;
...
pub(crate) trait AqueryDelegate: Send + Sync {
    ...
    async fn get_node(&self, key: &ActionKey) -> slug_error::Result<ActionQueryNode>;

    async fn expand_artifacts(
        &self,
        artifacts: &[ArtifactGroup],
    ) -> slug_error::Result<Vec<ActionQueryNode>>;
...
impl<'c> AqueryEnvironment<'c> {
    ...
    async fn get_node(&self, label: &ActionQueryNodeRef) -> slug_error::Result<ActionQueryNode> {
...
impl QueryEnvironment for AqueryEnvironment<'_> {
    ...
    async fn get_node(&self, node_ref: &ActionQueryNodeRef) -> slug_error::Result<Self::Target> {
...
impl AsyncNodeLookup<ActionQueryNode> for AqueryNodeLookup<'_, '_> {
    async fn get(&self, label: &ActionQueryNodeRef) -> slug_error::Result<ActionQueryNode> {
