...
pub(crate) trait CqueryDelegate: Send + Sync {
    ...
    ) -> slug_error::Result<ConfiguredTargetNode>;
...
impl<'c> CqueryEnvironment<'c> {
    ...
    fn owner_correct(&self, path: &CellPath) -> slug_error::Result<Vec<ConfiguredTargetNode>> {
        let universe = self
            .universe
            .as_ref()
...
impl QueryEnvironment for CqueryEnvironment<'_> {
    ...
    async fn get_node(&self, node_ref: &ConfiguredTargetLabel) -> slug_error::Result<Self::Target> {
