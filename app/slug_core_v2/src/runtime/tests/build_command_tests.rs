...
fn epoch_certificate_reobserves_retained_materialization_and_symlink_lifecycle() {
    ...
    runtime.runtime.block_on(async {
        ...
        assert!(matches!(
            runtime
                .request_revision
                .finalize_native(&terminal, &certificate, selected, &initial, |_| Err(
                    RequestRevisionError::Observation("forced".to_owned())
                ),)
                .await,
...
async fn build_command_root_anchors_empty_and_preserves_ordered_package_results() {
    ...
    let mut empty =
        build_root_transaction_with_data(&dice, BuildRootEpoch::base(1).build(), user_data).await;
    ...
    let mut transaction = build_root_transaction_with_data(&dice, epoch.build(), user_data).await;
...
async fn resolved_run_view_reuses_exact_executable_filewrite_relation() {
    ...
    let mut accepted_tx =
        build_root_transaction(&dice, resolved_write_epoch(41, "setting_a", &replacements)).await;
...
async fn resolved_file_write_view_borrows_immutable_owner_context_without_closure_lookup() {
    ...
    let key =
        BuildCommandRootKey::new(workspace, &targets, build_test_configuration("target")).unwrap();
    let mut transaction =
    ...
    let retained = without_platform
        ...
        .unwrap();
    assert_eq!(
        resolved_setting_label(&without_platform),
        baseline_projection
    );
    assert!(Arc::ptr_eq(
...
async fn filewrite_aquery_text_matches_frozen_baseline() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "  SlugActionToken: slugact-display-v1:fc10121aa7a98fd9779a83ec5b695ef8940d8435e57cc7542482dfe491b91bce\n",
            "  Inputs: []\n",
...
async fn build_command_root_replays_root_module_and_build_create_edit_delete_restore() {
    ...
    let workspace = NormalizedAbsolutePath::new("/workspace").unwrap();
    let configuration = build_test_configuration("target");
    let empty = BuildCommandRootKey::new(workspace.clone(), &[], configuration.clone()).unwrap();
...
#[derive(Default)]
struct ObservedBuildTracker(Mutex<Vec<ObservedBuildActivation>>, Mutex<Vec<Vec<String>>>);
        ...
        Ok(match outcome {
            ...
            slug_bzlmod_v2::SourcePreparationOutcome::Complete(mut terminal) => {
                terminal.observations = PathObservationEpoch::from_shared(
                    terminal.observations.observations().iter().enumerate().map(
                        |(index, (demand, result))| {
...
fn observed_terminal_validation_requires_complete_values_and_exact_arcs() {
    ...
    snapshot.path_observations =
        PathObservationEpoch::from_shared([(first.dupe(), observed.get(&first).unwrap().dupe())])
            .unwrap();
    ...
    let normalized = NormalizedAbsolutePath::new(workspace.path().canonicalize().unwrap()).unwrap();
    ...
    let fresh_equal =
        PathObservationEpoch::new([(first.dupe(), observed.get(&first).unwrap().as_ref().clone())])
            .unwrap();
    ...
    let absent = preserve_equal_observation_arcs(&PathObservationEpoch::empty(), &fresh_changed);
    assert!(Arc::ptr_eq(
...
fn terminal_epoch_association_installs_terminal_arcs_and_preserves_unrelated_arcs() {
    ...
    assert!(Arc::ptr_eq(
        associated.get(&first).unwrap(),
        terminal.get(&first).unwrap()
    ));
    assert!(Arc::ptr_eq(
        associated.get(&second).unwrap(),
        terminal.get(&second).unwrap()
    ));
    assert!(Arc::ptr_eq(
        associated.get(&third).unwrap(),
        command.get(&third).unwrap()
    ));
}
...
fn observed_build_identity_and_epoch_union_are_restricted_and_left_stable() {
    ...
    let workspace = NormalizedAbsolutePath::new("/workspace").unwrap();
    let configuration = build_test_configuration("target");
    for targets in [vec![], vec![TargetPattern::parse("//pkg:t").unwrap()]] {
        let legacy =
            BuildCommandRootKey::new(workspace.clone(), &targets, configuration.clone()).unwrap();
    ...
    .unwrap();
    let observed_multi = BuildCommandRootObservationKey::new(multi.clone()).unwrap();
    assert_eq!(observed_multi.0, multi);
    assert!(observed_multi.initializes_request_revision());
    assert!(matches!(
        observed_multi.observed_selection_association(),
    ...
    .unwrap();
    assert_eq!(
        BuildCommandRootObservationKey::new(external.clone())
            .unwrap()
            .0,
        external
    );
    ...
    let compute_error =
        external_build_compute_error(BuildAnalysisMode::Observed, "source compute", left.dupe())
            .unwrap();
    ...
    assert!(matches!(
        error.kind,
        BuildCommandErrorKind::Infrastructure(_)
    ));
...
#[test]
fn observed_multi_reducer_and_selected_superset_preserve_total_order_and_arcs() {
    let first = BuildRootEpoch::demand("/first", PathObservationOperation::FileBytes);
    let second = BuildRootEpoch::demand("/second", PathObservationOperation::FileBytes);
    let first_result = Arc::new(PathObservationResult::FileBytes(
        PathOperationResult::Present(Arc::from(&b"first"[..])),
    ));
    let second_result = Arc::new(PathObservationResult::FileBytes(
        PathOperationResult::Present(Arc::from(&b"second"[..])),
    ));
    let certified_error = |demand: PathObservationDemand, result: Arc<PathObservationResult>| {
        observed_multi_branch_complete(
            Err(BuildCommandError::new(BuildCommandErrorKind::RootSource {
                observation: result.as_ref().clone(),
                source_certificate: Some(Box::new(SourceCertificate::new(demand, result))),
            })),
            PathObservationEpoch::empty(),
        )
    };
    let branches = vec![
        certified_error(first.dupe(), first_result.dupe()),
        certified_error(second.dupe(), second_result.dupe()),
    ];
    let PreparationOutcome::Complete(Ok((Err(selected), epoch, Some(certificate)))) =
        finish_observed_multi_build_branches(PathObservationEpoch::empty(), branches)
    else {
        panic!("two Complete semantic branches did not retain a certificate");
    };
    assert!(matches!(
        selected.kind,
        BuildCommandErrorKind::RootSource { ref observation, .. }
            if observation == first_result.as_ref()
    ));
    for (demand, result) in [(&first, &first_result), (&second, &second_result)] {
        assert!(Arc::ptr_eq(epoch.get(demand).unwrap(), result));
        assert!(Arc::ptr_eq(
            certificate.observations().get(demand).unwrap(),
            result
        ));
    }

    let need = build_test_need("/workspace/need");
    assert!(matches!(
        finish_observed_multi_build_branches(
            PathObservationEpoch::empty(),
            vec![
                certified_error(first.dupe(), first_result.dupe()),
    ...
    assert!(matches!(
        finish_observed_multi_build_branches(
            PathObservationEpoch::from_shared([(first.dupe(), first_result.dupe(),)]).unwrap(),
            vec![
                PreparationOutcome::Need(build_test_need("/workspace/need")),
                certified_error(first.dupe(), conflicting),
            ],
        ),
        PreparationOutcome::Complete(Err(_))
    ));
    let incompatible_a = slug_bzlmod_v2::SourcePreparationNeeds::root_module_bootstrap(
        slug_bzlmod_v2::RootModuleBootstrapRequest {
            workspace: NormalizedAbsolutePath::new("/workspace").unwrap(),
        },
    );
    let incompatible_b = slug_bzlmod_v2::SourcePreparationNeeds::root_module_bootstrap(
        slug_bzlmod_v2::RootModuleBootstrapRequest {
    ...
    assert!(matches!(
        finish_observed_multi_build_branches(
            PathObservationEpoch::empty(),
            vec![
                PreparationOutcome::Need(incompatible_a),
                PreparationOutcome::Need(incompatible_b),
            ],
        ),
        PreparationOutcome::Complete(Ok((
            Err(BuildCommandError {
                kind: BuildCommandErrorKind::Infrastructure(_),
            }),
            _,
            None
        )))
    ...
    snapshot.path_observations = PathObservationEpoch::from_shared([
        (first.dupe(), first_result.dupe()),
        (second.dupe(), second_result.dupe()),
    ])
    .unwrap();
    ...
    .unwrap();
    let fresh = PathObservationEpoch::new([(first.dupe(), first_result.as_ref().clone())]).unwrap();
    assert!(matches!(
        validate_observed_terminal_with_association(
            &fresh,
            &snapshot,
    ...
    let request = local_native_request(
    ...
    snapshot.selected = SelectedWorkspaceDemands::for_test([], [first.dupe(), second.dupe()]);
    let extra = BuildRootEpoch::demand("/extra", PathObservationOperation::Lstat);
    assert!(matches!(
        validate_observed_terminal_with_association(
            &PathObservationEpoch::new([
                (first, first_result.as_ref().clone()),
                (
                    extra,
                    PathObservationResult::Lstat(PathOperationResult::Missing),
                ),
...
fn public_multi_build_aggregates_sources_and_accepts_analysis_dependency_superset() {
    ...
    let stable_parent = tempfile::tempdir_in(stable_parent_root).unwrap();
    ...
    let observed = complete_observed_build(&observed);
    assert_eq!(observed.result().as_ref().as_ref().unwrap(), public);
    assert!(
        observed.observations().observations().len()
            < snapshot.path_observations.observations().len()
    );
    validate_observed_terminal_with_association(
    ...
    .unwrap();
    assert!(matches!(
        observed_key.terminal_demand_association(observed),
        TerminalDemandAssociation::ClosureOnly
    ));
    let certificate = observed.source_certificate().unwrap();
    for source in ["one.txt", "two.txt"] {
        assert!(
            certificate
                ...
                .observations()
                .iter()
                .any(|(demand, result)| demand.path().as_path().ends_with(source)
                    && Arc::ptr_eq(result, snapshot.path_observations.get(demand).unwrap()))
        );
    }
    assert!(
        certificate
    ...
    let edited = complete_observed_build(&edited_outcome);
    assert!(
        edited
            .source_certificate()
            .unwrap()
            ...
            .iter()
            .any(|(demand, result)| {
                demand.path().as_path().ends_with("one.txt")
                    && matches!(
                        result.as_ref(),
                        PathObservationResult::FileBytes(PathOperationResult::Present(bytes))
                            if bytes.as_ref() == b"ONE"
                    )
                    && Arc::ptr_eq(
                        result,
                        edited_snapshot.path_observations.get(demand).unwrap(),
                    )
            })
    );
    fs::write(workspace.path().join("one.txt"), b"one").unwrap();
    ...
    let failed = failure_run().unwrap();
    let failed_text = accepted_output_text(&failed);
    assert!(failed_text.is_empty());
    let failed_terminal = failed.terminal_for_test();
    let failed_error = failed_terminal.as_ref().as_ref().unwrap_err();
    assert!(matches!(
        &failed_error.kind,
        BuildCommandErrorKind::Analysis(_)
    ));
    let failed_snapshot = accepted_native_snapshot(&failure_runtime);
    assert!(failed_snapshot.selected.repository_requests().is_empty());
    assert!(failed_snapshot.selected.repository_validations().is_empty());
...
async fn observed_build_matches_legacy_arcs_events_and_isolates_key_families() {
    ...
    let slug_bzlmod_v2::SourcePreparationOutcome::Complete(legacy_result) = legacy_outcome else {
...
async fn observed_build_terminal_polarity_keeps_only_complete_semantic_prefixes() {
    let key = BuildCommandRootObservationKey::new(singleton_package_all_key("//pkg:all")).unwrap();
...
fn build_branch_collection_has_total_infrastructure_need_and_error_precedence() {
    ...
    let slug_bzlmod_v2::SourcePreparationOutcome::Need(combined) = collect_build_branches(vec![
...
fn neutral_identity_and_source_carrier_are_complete_only_and_exact() {
    ...
    let inputs = PathObservationEpoch::new([(demand.dupe(), result.as_ref().clone())]).unwrap();
    ...
    assert!(
        SingletonRootSingleBuildCommandKey::validity(&completed)
    ...
    let conflict = singleton_root_single_complete(
        Err(BuildCommandError::new(BuildCommandErrorKind::RootSource {
            ...
            source_certificate: Some(Box::new(SourceCertificate::new(demand, conflicting))),
        })),
        &inputs,
    );
    assert!(matches!(
...
async fn neutral_rule_and_filegroup_use_one_observed_family_and_child_events() {
    ...
    let mut transaction = build_root_transaction_with_data(&dice, epoch.dupe(), data).await;
    ...
    let mut cancelled_transaction =
        build_root_transaction_with_data(&cancelled, epoch.dupe(), cancelled_data).await;
            ...
            terminal.observations = terminal.observations.map(|observations| {
                PathObservationEpoch::from_shared(
                    observations
                        .observations()
                        .iter()
                        .map(|(demand, result)| (demand.dupe(), Arc::new(result.as_ref().clone()))),
                )
                .unwrap()
            });
...
fn public_neutral_source_preserves_exact_arcs_and_associates_terminal_epoch() {
    ...
    let driven = runtime
        .drive_command(
            NativeDemandRequestInputBundle::normalized_initial(),
            PointerDistinctNeutralRoot(SingletonRootSingleBuildCommandKey::new(key).unwrap()),
        )
        .unwrap();
...
fn public_external_single_uses_observed_family_and_full_source_certificate() {
    ...
    let source_result = |snapshot: &AcceptedNativeDemandSnapshot| {
        snapshot
            ...
            .max_by_key(|(demand, _)| demand.operation() == PathObservationOperation::FileBytes)
    ...
    assert_eq!(
        legacy.targets[0].completion,
        evaluation.targets[0].completion
    );
    assert!(
        legacy_tracker
            .take()
    ...
    let source_position = dependencies
        .iter()
        .position(|dependency| dependency.starts_with("observed-host-repository-source-file:"))
        .unwrap();
    ...
    assert!(
        snapshot
            ...
            .all(|(demand, result)| Arc::ptr_eq(
                result,
                warm_snapshot.path_observations.get(demand).unwrap()
            ))
    );
    ...
    fs::write(&source, b"A").unwrap();
    assert!(run(&target).unwrap().terminal_for_test().as_ref().is_ok());
...
async fn archive_override_restores_unsupported_route_after_nonregistry_bridge_fallback() {
    ...
    let route_key =
        slug_bzlmod_v2::RootRepositoryRouteKey::new(workspace.clone(), apparent).unwrap();
    ...
    let materialized_epoch = |epoch: &PathObservationEpoch| {
        ...
        let directory = || {
            Arc::new(PathObservationResult::Lstat(PathOperationResult::Present(
                slug_workspace_v2::PathLstat::new(PathNodeKind::Directory, 1, 1, 1, 1, 0o755),
            )))
        };
        PathObservationEpoch::from_shared(
            epoch
                ...
                .chain([
                    ...
                    (
                        demand("/immutable", PathObservationOperation::Lstat),
                        directory(),
                    ),
                    (
                        demand(
                            "/immutable/archive-override",
                    ...
                    (
                        demand(
                            "/immutable/archive-override/MODULE.bazel",
                            PathObservationOperation::Lstat,
                        ),
                        Arc::new(PathObservationResult::Lstat(PathOperationResult::Present(
    ...
    let request = need
        ...
        .unwrap()
        .clone();
    let mut legacy = local_materialized_transaction(
        &legacy_dice,
        &workspace,
        request,
        legacy_tracker,
    ...
    assert!(
        matches!(&legacy.kind, BuildCommandErrorKind::RepositoryRoute(error) if error.to_string() == original.to_string())
    );
    ...
    let request = need
        ...
        .unwrap()
        .clone();
    let mut observed = local_materialized_transaction(
        &observed_dice,
        &workspace,
        request,
        observed_tracker,
    ...
    let PreparationOutcome::Complete(Ok(observed)) = observed.compute(&observed_key).await.unwrap()
    else {
        panic!("observed archive bridge must complete after materialization")
    };
    let observed = observed.result().as_ref().as_ref().unwrap_err();
    assert!(
        matches!(&observed.kind, BuildCommandErrorKind::RepositoryRoute(error) if error.to_string() == original.to_string())
    );
