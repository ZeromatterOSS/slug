...
/// One prerequisite forest for the complete ordinary requested artifact set.
/// This borrowed view carries no source-read, execution or publication authority.
#[derive(Debug)]
pub struct RequestedActionPrerequisitePlan<'a> {
    selection: super::RequestedArtifacts<'a>,
    actions: Vec<PlannedAction<'a>>,
    artifact_producers: Vec<Option<usize>>,
}

impl<'a> RequestedActionPrerequisitePlan<'a> {
    pub fn selection(&self) -> &super::RequestedArtifacts<'a> {
        &self.selection
    }

    /// Prerequisites precede consumers; shared producers appear only once.
    pub fn actions(&self) -> &[PlannedAction<'a>] {
        &self.actions
    }

    /// One binding per selection().artifacts() entry. Sources have no producer;
    /// every derived artifact names its producer's index in actions().
    pub fn artifact_producers(&self) -> &[Option<usize>] {
    ...
    pub(super) fn new(
        ...
        let forest = plan_roots(closure, PlanRoots::Artifacts(selection.artifacts()))?;
        Ok(Self {
            selection,
            actions: forest.actions,
            artifact_producers: forest.producers,
        })
    }
}
...
enum PlanRoots<'a> {
    Action(Coordinate),
    Artifacts(&'a [AnalysisArtifact]),
}

#[derive(Debug)]
struct PlannedForest<'a> {
    actions: Vec<PlannedAction<'a>>,
...
fn plan_roots<'a>(
    ...
    let producer = |artifact: &AnalysisArtifact| -> Result<Option<Coordinate>, Arc<str>> {
        ...
        let coordinate = declared.get(&(owner, output)).copied().ok_or_else(|| {
            Arc::<str>::from(format!(
...
fn walk_prerequisites<'a>(
    ...
    let mut pending = seeds
        .iter()
        .rev()
        .filter_map(|seed| seed.map(Visit::Enter))
        .collect::<Vec<_>>();
        ...
        match visit {
            Visit::Enter(coordinate) => {
                ...
                for artifact in artifacts {
                    let input_producer = producer(&artifact)?;
                    inputs.push((artifact, input_producer));
                }
        ...
        .map(|seed| {
            seed.map(|key| {
                states
                    ...
                    .copied()
                    .flatten()
                    .expect("selected producer emitted")
            })
        })
        .collect();
    Ok(PlannedForest { actions, producers })
}
