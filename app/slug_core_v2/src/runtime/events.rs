...
impl AttemptEffectTracker {
    pub(super) fn install(
        ...
        self.owner.install(self.id, data, self.clone())
    }
...
impl ActivationTracker for AttemptEffectTracker {
    fn key_activated(
        &self,
        _key: &DynKey,
...
impl SealedCommandAttempt {
    pub(super) async fn select(
        ...
        self.owner.select(&self, &closure)
    }
}
...
mod tests {
    ...
    -> anyhow::Result<()> {
        ...
        let mut updater = dice.updater_with_data(user_data(&first)?);
        ...
        let mut terminal_transaction = dice.updater_with_data(user_data(&terminal)?).commit().await;
        ...
        let mut fresh_transaction = dice.updater_with_data(user_data(&fresh)?).commit().await;
        fresh_transaction.compute(&EventGraphKey::Parent).await?;
        let fresh_sealed = fresh.seal_terminal()?;
        assert!(
            fresh_sealed
                .select(&fresh_transaction)
                .await?
                .batches()
                .is_empty()
        );
    ...
    -> anyhow::Result<()> {
        ...
        let terminal = owner.begin_attempt()?;
        let mut updater = dice.updater_with_data(user_data(&terminal)?);
        updater.changed_to(vec![(EventMode, 1)])?;
        let mut terminal_transaction = updater.commit().await;
        terminal_transaction.compute(&EventGraphKey::Parent).await?;
        let sealed = terminal.seal_terminal()?;
        assert_eq!(
            selected_text(&sealed.select(&terminal_transaction).await?),
            ["parent"]
        );
    ...
    -> anyhow::Result<()> {
        ...
        let mut transaction = dice.updater_with_data(user_data(&attempt)?).commit().await;
        ...
        assert_eq!(
            selected_text(&sealed.select(&transaction).await?),
            ["shared", "left", "right"]
        );
        ...
        let mut updater = dice.updater_with_data(user_data(&stale)?);
        updater.changed_to(vec![(EventMode, 0)])?;
    ...
    async fn lifecycle_and_closure_failures_are_typed_and_select_nothing() -> anyhow::Result<()> {
        ...
        let mut occupied = UserComputationData {
            activation_tracker: Some(attempt.clone()),
            ..Default::default()
        };
        assert_eq!(
            attempt.install(&mut occupied),
            Err(CommandEffectError::ActivationTrackerAlreadyInstalled)
        );
        assert!(occupied.data.get::<CaptureEvaluationEvents>().is_err());
        let _installed = user_data(&attempt)?;
        let mut duplicate = UserComputationData::default();
        assert_eq!(
            attempt.install(&mut duplicate),
        ...
        let mut terminal = UserComputationData::default();
        assert_eq!(
            attempt.install(&mut terminal),
        ...
        let stale = owner.begin_attempt()?;
        stale.seal_retry()?;
        let mut stale_data = UserComputationData::default();
        assert_eq!(
            stale.install(&mut stale_data),
            Err(CommandEffectError::StaleAttempt)
        );
        ...
        let owner = CommandEffectOwner::new();
        let attempt = owner.begin_attempt()?;
        let mut updater = first_dice.updater_with_data(user_data(&attempt)?);
        updater.changed_to(vec![(EventMode, 0)])?;
        ...
        let mut transaction = first_dice
            .updater_with_data(user_data(&attempt)?)
            .commit()
            .await;
        transaction.compute(&EventGraphKey::Parent).await?;
        let sealed = attempt.seal_terminal()?;
        ...
        let mut transient_transaction = transient_dice
            .updater_with_data(user_data(&transient_attempt)?)
            .commit()
            .await;
