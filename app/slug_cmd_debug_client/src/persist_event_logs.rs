...
use slug_common::chunk_reader::ChunkReader;
use slug_common::manifold;
use slug_common::manifold::ManifoldChunkedUploader;
use slug_common::manifold::ManifoldClient;
use slug_core::soft_error;
use slug_data::InstantEvent;
use slug_data::PersistEventLogSubprocess;
...
impl PersistEventLogsCommand {
    ...
    ) -> (slug_error::Result<()>, slug_error::Result<()>) {
...
async fn create_log_file(local_path: String) -> Result<tokio::fs::File, slug_error::Error> {
...
impl<'a> Uploader<'a> {
    ...
    async fn upload_chunk(&mut self) -> slug_error::Result<()> {
    ...
    fn can_fill_chunk(&mut self) -> slug_error::Result<bool> {
        Ok(self
            .total_bytes
...
fn status_from_result(res: slug_error::Result<()>) -> (Vec<String>, Option<String>, bool) {
...
fn categorize_error(err: &slug_error::Error) -> &'static str {
    // This is for internal error tracking in `logview slug`
...
mod tests {
    ...
    fn test_categorize_error() {
        let err = slug_error!(slug_error::ErrorTag::Environment, "CertificateRequired");
        assert_eq!(categorize_error(&err), "persist_log_certificate_required");

        let err = slug_error!(slug_error::ErrorTag::Tier0, "Some other error");
