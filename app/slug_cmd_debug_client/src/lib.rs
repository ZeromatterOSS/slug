...
#[clap(about = "Hidden debug commands useful for testing slug")]
pub enum DebugCommand {
    ...
    /// Prints the hash of the slug binary
    ...
    /// Flushes all dep files known to Slug.
    ...
    /// Validates that Slug and disk agree on the state of files.
    FileStatus(FileStatusCommand),
    /// Prints slug daemon directory (`~/.buckd/xxx`).
    DaemonDir(DaemonDirCommand),
    /// Prints slug executable (this executable) path.
    Exe(ExeCommand),
