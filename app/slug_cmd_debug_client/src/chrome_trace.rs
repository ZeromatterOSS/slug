...
use slug_client_ctx::event_log_options::EventLogOptions;
use slug_client_ctx::events_ctx::EventsCtx;
use slug_client_ctx::exit_result::ExitResult;
use slug_client_ctx::path_arg::PathArg;
...
struct ChromeTraceFirstPass {
    ...
    pub long_analyses: HashSet<slug_events::span::SpanId>,
    pub long_loads: HashSet<slug_events::span::SpanId>,
    pub local_actions: HashSet<slug_events::span::SpanId>,
    pub critical_path_action_keys: HashSet<slug_data::ActionKey>,
...
impl ChromeTraceFirstPass {
    ...
    fn handle_event(&mut self, event: &BuckEvent) -> slug_error::Result<()> {
        match event.data() {
            slug_data::buck_event::Data::SpanStart(start) => {
                match start.data.as_ref() {
                    Some(slug_data::span_start_event::Data::ExecutorStage(exec)) => {
            ...
            slug_data::buck_event::Data::SpanEnd(end) => {
                match end.data.as_ref() {
                    Some(slug_data::span_end_event::Data::Analysis(_)) => {
                        if end
                            .duration
                            .as_ref()
                    ...
                    Some(slug_data::span_end_event::Data::Load(_)) => {
                        if end
                            .duration
                            .as_ref()
            ...
            slug_data::buck_event::Data::Instant(instant) => match instant.data.as_ref() {
                Some(slug_data::instant_event::Data::BuildGraphInfo(info)) => {
            ...
            slug_data::buck_event::Data::Record(_) => {}
        };
        Ok(())
    }
}
...
impl ChromeTraceClosedSpan {
    fn to_json(self) -> slug_error::Result<serde_json::Value> {
        Ok(json!(
            {
                "name": self.open.name,
        ...
        + Serialize,
{
    ...
    fn process_timestamp(&mut self, timestamp: SystemTime) -> slug_error::Result<()> {
    ...
    fn set(&mut self, timestamp: SystemTime, key: &str, amount: T) -> slug_error::Result<()> {
    ...
    fn bump(&mut self, timestamp: SystemTime, key: &str, amount: T) -> slug_error::Result<()> {
        self.process_timestamp(timestamp)?;
        let entry = self.counter_entry(key);
        entry.value += amount;
        Ok(())
    ...
    fn subtract(&mut self, timestamp: SystemTime, key: &str, amount: T) -> slug_error::Result<()> {
        self.process_timestamp(timestamp)?;
        let entry = self.counter_entry(key);
        entry.value -= amount;
        Ok(())
    ...
    fn flush(&mut self) -> slug_error::Result<()> {
    ...
    pub fn flush_all_to(&mut self, output: &mut Vec<serde_json::Value>) -> slug_error::Result<()> {
        self.flush()?;
...
struct SpanCounters {
    ...
    open_spans: HashMap<slug_events::span::SpanId, (&'static str, i32)>,
}
...
struct ChromeTraceWriter {
    trace_events: Vec<serde_json::Value>,
    open_spans: HashMap<slug_events::span::SpanId, ChromeTraceOpenSpan>,
...
impl ChromeTraceWriter {
    ...
    fn assign_track_for_span(
        ...
        event: &BuckEvent,
    ) -> slug_error::Result<Option<SpanTrackAssignment>> {
    ...
    pub fn to_writer<W>(mut self, file: W) -> slug_error::Result<()>
    where
        W: Write,
    {
        self.span_counters
    ...
    fn handle_event(&mut self, event: &Arc<BuckEvent>) -> slug_error::Result<()> {
        match event.data() {
            slug_data::buck_event::Data::SpanStart(slug_data::SpanStartEvent {
                ...
                let categorization = match start_data {
                    slug_data::span_start_event::Data::Command(_command) => Categorization::Show {
                    ...
                    slug_data::span_start_event::Data::ReUpload(_) => {
                        let name = "re_upload";
                        self.span_counters.bump_counter_while_span(event, name, 1)?;
                        Categorization::ShowIfParent { name: name.into() }
                    }
                    slug_data::span_start_event::Data::FinalMaterialization(..) => {
                        if on_critical_path {
                            Categorization::Show {
                    ...
                    slug_data::span_start_event::Data::FileWatcher(_file_watcher) => {
                        Categorization::Show {
                            category: SpanCategorization::CriticalPath,
            ...
            slug_data::buck_event::Data::SpanStart(slug_data::SpanStartEvent { data: None }) => {}
            slug_data::buck_event::Data::SpanEnd(end) => self.handle_event_end(end, event)?,
            slug_data::buck_event::Data::Instant(slug_data::InstantEvent {
                data: Some(instant_data),
            }) => {
                if let slug_data::instant_event::Data::Snapshot(snapshot) = instant_data {
                    self.process_memory_counters.set(
                        event.timestamp(),
                        "max_rss_gigabyte",
                        (snapshot.slug_max_rss) as f64 / Self::BYTES_PER_GIGABYTE,
                    )?;
                    ...
                    self.rate_of_change_counters
                        .set_average_rate_of_change_per_s(
                            ...
                            snapshot.slug_user_cpu_us,
                        )?;
                    self.rate_of_change_counters
                        .set_average_rate_of_change_per_s(
                            event.timestamp(),
                            "average_system_cpu_in_usecs_per_s",
                            snapshot.slug_system_cpu_us,
                        )?;
                ...
                } else if let slug_data::instant_event::Data::ResourceControlEvents(events) =
                    instant_data
                {
                    self.snapshot_counters.set(
                        event.timestamp(),
            ...
            // Data field is oneof and `None` means the event is produced with newer version of `.proto` file
            // which added a variant which is not available in version used when compiling this program.
            slug_data::buck_event::Data::Instant(slug_data::InstantEvent { data: None }) => {}
            slug_data::buck_event::Data::Record(_) => {}
        };
        Ok(())
    }
...
impl ChromeTraceCommand {
    ...
    async fn load_events(
        ...
        Invocation,
        BoxStream<'static, slug_error::Result<BuckEvent>>,
    )> {
...
impl BuckSubcommand for ChromeTraceCommand {
    ...
    async fn exec_impl(
        ...
        let log = if let Some(path) = self.path_flag {
...
impl ChromeTraceCommand {
    async fn trace_writer(log: EventLogPathBuf) -> slug_error::Result<ChromeTraceWriter> {
