...
use slug_cli_proto::TraceIoRequest;
use slug_cli_proto::TraceIoResponse;
use slug_cli_proto::trace_io_request;
...
use slug_client_ctx::daemon::client::connect::DesiredTraceIoState;
...
use slug_offline_archive::ExternalSymlink;
use slug_offline_archive::OfflineArchiveManifest;
use slug_offline_archive::RelativeSymlink;
use slug_offline_archive::RepositoryMetadata;
...
impl TraceIoCommand {
    ...
    ) -> slug_error::Result<CommandOutcome<TraceIoResponse>> {
        buckd
            .with_flushing()
            .trace_io(
                req,
    ...
    async fn exec_impl(
        ...
        match &self.trace_io_action {
            Subcommand::Status => {
                ...
                slug_client_ctx::println!("I/O tracing status: {}", resp.enabled)?;
            }
            Subcommand::ExportManifest { out } => {
                ...
                if let Some(output_path) = &out {
                    ...
                    slug_client_ctx::println!("{}", serialized)?;
                }
            }
