...
use slug_artifact::artifact::artifact_type::testing::ArtifactTestingExt;
...
use slug_build_api::actions::ActionErrors;
use slug_build_api::actions::registry::ActionsRegistry;
use slug_build_api::analysis::registry::AnalysisValueFetcher;
...
#[test]
fn declaring_artifacts() -> slug_error::Result<()> {
    Heap::temp(|heap| {
...
#[test]
fn claiming_conflicting_path() -> slug_error::Result<()> {
    let mut actions = ActionsRegistry::new(
...
#[test]
fn register_actions() -> slug_error::Result<()> {
    Heap::temp(|heap| {
        let base = BaseDeferredKey::TargetLabel(ConfiguredTargetLabel::testing_parse(
            "cell//pkg:foo",
            ConfigurationData::testing_new(),
...
#[test]
fn finalizing_actions() -> slug_error::Result<()> {
    Heap::temp(|heap| {
        let base = BaseDeferredKey::TargetLabel(ConfiguredTargetLabel::testing_parse(
            "cell//pkg:foo",
            ConfigurationData::testing_new(),
