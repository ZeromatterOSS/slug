...
use slug_build_api::actions::calculation::command_details;
...
use slug_build_api::actions::execute::dice_data::SetInvalidationTrackingConfig;
...
use slug_common::file_ops::testing::TestFileOps;
use slug_common::http::SetHttpClient;
use slug_common::legacy_configs::configs::LegacyBuckConfig;
use slug_common::legacy_configs::dice::inject_legacy_config_for_test;
use slug_configured::nodes::ConfiguredTargetNodeKey;
...
use slug_execute::execute::testing_dry_run::DryRunEntry;
...
fn mock_analysis_for_action_resolution(
    ...
    dice_builder = dice_builder.mock_and_return(
        ...
        slug_error::Ok(MaybeCompatible::Compatible(AnalysisResult::new(
...
) -> slug_error::Result<DiceTransaction> {
        ...
        fn get_command_executor(
            ...
            Ok(CommandExecutorResponse {
                ...
                output_trees_download_config:
                    slug_execute::re::output_trees_download_config::OutputTreesDownloadConfig::new(
                        None, true,
                    ),
            })
        }
...
#[tokio::test]
async fn test_get_action_for_artifact() -> slug_error::Result<()> {
...
#[tokio::test]
async fn test_build_action() -> slug_error::Result<()> {
    slug_certs::certs::maybe_setup_cryptography();
...
#[tokio::test]
async fn test_build_artifact() -> slug_error::Result<()> {
    slug_certs::certs::maybe_setup_cryptography();
    let temp_fs = ProjectRootTemp::new()?;
    let build_artifact = create_test_build_artifact();
    let registered_action = registered_action(
...
#[tokio::test]
async fn test_ensure_artifact_build_artifact() -> slug_error::Result<()> {
    slug_certs::certs::maybe_setup_cryptography();
    let temp_fs = ProjectRootTemp::new()?;
    let build_artifact = create_test_build_artifact();
    let registered_action = registered_action(
...
#[tokio::test]
async fn test_ensure_artifact_source_artifact() -> slug_error::Result<()> {
    slug_certs::certs::maybe_setup_cryptography();
...
#[tokio::test]
async fn test_ensure_artifact_external_symlink() -> slug_error::Result<()> {
    slug_certs::certs::maybe_setup_cryptography();
    let path = CellPath::new(
        CellName::testing_new("cell"),
