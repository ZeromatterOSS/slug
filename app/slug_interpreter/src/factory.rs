...
impl StarlarkEvaluatorProvider {
    ...
    pub async fn new(
        ...
        eval_kind: StarlarkEvalKind,
    ) -> slug_error::Result<StarlarkEvaluatorProvider> {
    ...
    ) -> slug_error::Result<ReentrantStarlarkEvaluator<'v, 'a, 'e>> {
        let (_, _v) = (slug_error::Ok(()), 1);
    ...
    pub fn with_evaluator<'v, 'a, 'e: 'a, R>(
        ...
        module: &'v BuckStarlarkModule,
        cancellation: CancellationPoller<'a>,
        closure: impl FnOnce(&mut Evaluator<'v, 'a, 'e>, bool) -> slug_error::Result<R>,
    ) -> slug_error::Result<(FinishedStarlarkEvaluation, R)> {
...
impl<'v, 'a, 'e: 'a> ReentrantStarlarkEvaluator<'v, 'a, 'e> {
    pub fn with_evaluator<R>(
        &mut self,
        closure: impl FnOnce(&mut Evaluator<'v, 'a, 'e>) -> slug_error::Result<R>,
    ) -> slug_error::Result<R> {
...
impl BuckStarlarkModule {
    ...
    pub async fn with_profiling_async<
        R,
        F: Future<Output = slug_error::Result<(ProfilingReportedToken, R)>>,
    >(
