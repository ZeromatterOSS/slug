...
struct Validate<'v>(#[trace(unsafe_ignore)] fn(Value<'v>) -> slug_error::Result<()>);
...
impl<'v> StarlarkPromise<'v> {
    ...
    pub fn map(
        ...
        f: StarlarkCallable<'v, (Value<'v>,), Value<'v>>,
        eval: &mut Evaluator<'v, '_, '_>,
    ) -> slug_error::Result<ValueTyped<'v, StarlarkPromise<'v>>> {
        match x.get() {
            Some(x) => Ok(eval
                .heap()
    ...
    pub fn validate(&self, f: fn(Value<'v>) -> slug_error::Result<()>) -> slug_error::Result<()> {
        match self.get() {
            Some(x) => f(x),
            _ => {
...
mod tests {
        ...
        fn promise_validate<'v>(
            ...
            promise.validate(|x| {
                if x.unpack_str() == Some("ok") {
                    ...
                    Err(slug_error!(slug_error::ErrorTag::Tier0, "VALIDATE_FAILED"))
                }
            })?;
            Ok(NoneType)
        }
    ...
    fn assert_promise<'v>(modu: &'v Module, content: &str) -> slug_error::Result<Value<'v>> {
        alloc_promises(modu);
    ...
    fn assert_promise_err<'v>(modu: &'v Module, content: &str, err: &str) -> slug_error::Error {
