...
impl StarlarkBuckRegex {
    ...
    fn is_match(&self, s: &str) -> slug_error::Result<bool> {
        match self {
...
pub fn register_buck_regex(builder: &mut GlobalsBuilder) {
    ...
    /// ## Fanciness
    ///
    /// Slug regexes support two backing implementations:
    ///
    ...
    fn regex<'v>(
        ...
        match fancy {
            false => Ok(StarlarkBuckRegex::Regular(
                regex::Regex::new(regex).map_err(slug_error::Error::from)?,
            )),
            true => Ok(StarlarkBuckRegex::Fancy(
                fancy_regex::Regex::new(regex).map_err(slug_error::Error::from)?,
            )),
        }
    }
