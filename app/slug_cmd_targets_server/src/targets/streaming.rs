...
//! Server-side implementation of `slug targets --streaming` command.
...
use slug_common::pattern::package_roots::find_package_roots_stream;
...
fn write_str(outputter: &mut dyn Write, s: &mut String) -> slug_error::Result<()> {
...
///                Passing from cli args `--keep-going` from `app/slug_client/src/commands/targets.rs`.
...
///               Passing from cli args `--imports` from `app/slug_client/src/commands/targets.rs`.
...
impl PreparePackageResult {
    ...
    fn record_error(&mut self, error: &slug_error::Error, formatter: &dyn TargetFormatter) {
...
) -> impl Stream<Item = slug_error::Result<(PackageLabel, PackageSpec<T>)>> + '_ {
        ...
        PackageSpec::Targets(targets) => {
            if keep_going {
                ...
                let targets = targets.into_try_map(|(target, TargetPatternExtra)| {
                    slug_error::Ok(result.resolve_target(target.as_ref())?.to_owned())
                })?;
