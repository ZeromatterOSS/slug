...
use slug_cli_proto::configured_targets_request::OutputFormat as ConfiguredOutputFormat;
use slug_cli_proto::targets_request;
use slug_cli_proto::targets_request::OutputFormat;
use slug_cli_proto::targets_request::TargetHashGraphType;
...
use slug_node::attrs::hacks::configured_value_to_json;
use slug_node::attrs::hacks::value_to_json;
use slug_node::attrs::inspect_options::AttrInspectOptions;
use slug_node::nodes::attributes::DEPS;
use slug_node::nodes::attributes::INPUTS;
use slug_node::nodes::attributes::ONCALL;
use slug_node::nodes::attributes::PACKAGE;
use slug_node::nodes::attributes::PACKAGE_VALUES;
use slug_node::nodes::attributes::TARGET_CALL_STACK;
use slug_node::nodes::attributes::TARGET_HASH;
use slug_node::nodes::attributes::TYPE;
...
fn package_error_to_stderr(package: PackageLabel, error: &slug_error::Error, stderr: &mut String) {
    writeln!(stderr, "Error parsing {package}\n{error:?}").unwrap();
}

fn target_error_to_stderr(error: &slug_error::Error, stderr: &mut String) {
...
pub trait ConfiguredTargetFormatter: Send + Sync {
    ...
    fn target_error(&self, error: &slug_error::Error, stdout: &mut String, stderr: &mut String) {
...
pub(crate) struct Stats {
    ...
    error_tags: BTreeSet<slug_error::ErrorTag>,
...
impl Stats {
    ...
    pub(crate) fn add_error(&mut self, e: &slug_error::Error) {
        self.error_tags.extend(e.tags());
        self.errors += 1;
    }

    pub(crate) fn to_error(&self) -> Option<slug_error::Error> {
        if self.errors == 0 {
            return None;
        }
        // Simpler error so that we don't print long errors twice (when exiting slug)
        ...
        let mut e = slug_error::Error::from(TargetsError::FailedToParse(self.errors, package_str));
...
) -> slug_error::Result<Arc<dyn TargetFormatter>> {
...
) -> slug_error::Result<ConfiguredOutputHandler> {
