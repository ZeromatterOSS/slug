...
use slug_cli_proto::TargetsResponse;
use slug_cli_proto::targets_request;
use slug_cli_proto::targets_request::Compression;
...
trait Compressor: Write + Send {
    fn finish(self: Box<Self>) -> slug_error::Result<()>;
}
...
fn outputter<'a, W: Write + Send + 'a>(
    ...
    stdout: W,
) -> slug_error::Result<(OutputType, Box<dyn Compressor + 'a>)> {
...
pub async fn targets_command(
    ...
    req: TargetsRequest,
) -> slug_error::Result<TargetsResponse> {
    run_server_command(
