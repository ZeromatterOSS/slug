...
///
/// kuro compilation speed is very important for us, so we try to make smaller crates
/// with as few dependencies between them as possible. So when we change one line of code,
/// we recompile as little as possible.
///
/// # Example
///
/// We have a crate `kuro_node` that defines target graph data types
...
/// `kuro_interpreter_for_build`, but code that needs to compute a target node,
/// calls the function defined in `kuro_node` crate without compile time dependency
/// on `kuro_interpreter_for_build` or `starlark` crates.
///
/// The best example of this pattern is probably `kuro_bxl` crate:
/// the only target depending on `kuro_bxl` is the final binary with `kuro.rs` file.
/// So when `kuro_bxl` changes, we recompile only `kuro_bxl` and `kuro.rs`.
/// At the moment of writing this comment, `kuro_bxl` had only one public function,
...
/// * in the file `app/kuro/bin/kuro.rs` call `init_late_bindings()` of the corresponding crate
...
impl<T> LateBinding<T> {
    ...
    #[inline]
    pub fn get(&self) -> kuro_error::Result<&T> {
        self.symbol
            .get()
