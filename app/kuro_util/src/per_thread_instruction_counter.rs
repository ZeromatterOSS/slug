...
impl PerThreadInstructionCounter {
    ...
    pub fn init() -> kuro_error::Result<Option<PerThreadInstructionCounter>> {
        Self::init_impl()
    }

    #[cfg(target_os = "linux")]
    fn init_impl() -> kuro_error::Result<Option<PerThreadInstructionCounter>> {
    ...
    pub fn collect(self) -> kuro_error::Result<u64> {
        self.collect_impl()
    }

    #[cfg(target_os = "linux")]
    fn collect_impl(mut self) -> kuro_error::Result<u64> {
    ...
    #[cfg(not(target_os = "linux"))]
    fn collect_impl(self) -> kuro_error::Result<u64> {
