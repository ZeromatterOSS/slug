...
const CLI_NAME: &str = "kuro-health-check";
const CLI_INFO_FILE: &str = "kuro-health-check-server-info";
...
pub(crate) struct HealthCheckRpcClient {
    ...
    connection: AsyncOnceCell<kuro_error::Result<HealthCheckServerConnection>>,
...
impl HealthCheckRpcClient {
    ...
    async fn connection(&self) -> kuro_error::Result<&HealthCheckServerConnection> {
    ...
    fn get_cli_path() -> kuro_error::Result<String> {
    ...
    async fn get_state_info_file_path(&self) -> kuro_error::Result<AbsNormPathBuf> {
...
impl HealthCheckService for HealthCheckRpcClient {
    async fn update_context(&mut self, event: HealthCheckContextEvent) -> kuro_error::Result<()> {
        let rpc_event: kuro_health_check_proto::HealthCheckContextEvent = event.try_into()?;
        self.rpc_client()
            .await?
    ...
    ) -> kuro_error::Result<Vec<Report>> {
        let snapshot: kuro_health_check_proto::HealthCheckSnapshotData = snapshot.try_into()?;
...
mod tests {
    ...
    async fn temp_state_info(
        temp_dir: &TempDir,
    ) -> kuro_error::Result<(AbsNormPathBuf, AbsNormPathBuf)> {
    ...
    #[tokio::test]
    async fn test_server_state_reset() -> kuro_error::Result<()> {
