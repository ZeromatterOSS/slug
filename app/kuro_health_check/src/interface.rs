...
/// Trait to generalize a kuro health check.
...
pub(crate) struct HealthCheckContext {
    ...
    pub command_data: Option<kuro_data::command_start::Data>,
    ...
    pub parsed_target_patterns: Option<kuro_data::ParsedTargetPatterns>,
    ...
    pub experiment_configurations: Option<kuro_data::SystemInfo>,
}

/// A subset of the Snapshot data specifically for health check use.
/// This struct contains timing metrics extracted from kuro_data::Snapshot.
#[derive(Dupe, Clone)]
...
pub enum HealthCheckEvent {
    ...
    // This snapshot passes a subset of the kuro_data::Snapshot data to health checks.
...
pub enum HealthCheckContextEvent {
    CommandStart(kuro_data::CommandStartWithTraceId),
    ParsedTargetPatterns(kuro_data::ParsedTargetPatterns),
    BranchedFromRevision(String),
    /// Sent only once and communicates if kuro is experiencing excess cache misses.
    HasExcessCacheMisses(),
    /// Configuration about the health checks.
    ExperimentConfigurations(kuro_data::SystemInfo),
}
