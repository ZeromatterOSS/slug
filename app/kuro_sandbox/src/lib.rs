...
mod linux {
    ...
    fn setup_sandbox(
        ...
        let staging_dir = if let (Some(buck_out), true) = (buck_out_root, !input_files.is_empty()) {
            match create_input_staging_dir(buck_out, input_files) {
                ...
                Err(e) => {
                    let msg = format!(
                        "[kuro-sandbox] input staging failed (continuing without input isolation): {}\n",
                        e
                    );
                    let _ =
        ...
        if let (Some(staging), Some(buck_out)) = (&staging_dir, buck_out_root) {
            if let Err(e) = bind_mount(staging, buck_out) {
                let msg = format!(
                    "[kuro-sandbox] buck-out overlay failed (input isolation disabled): {}\n",
                    e
                );
        ...
        for output_dir in output_dirs {
            ...
            if let Err(e) = make_writable_bind_mount(output_dir) {
                ...
                let msg = format!(
                    "[kuro-sandbox] writable bind mount failed for {}: {}\n",
                    output_dir.display(),
                    e
                );
                let _ = unsafe { libc::write(2, msg.as_ptr() as *const libc::c_void, msg.len()) };
    ...
    /// Create a staging directory at /tmp/kuro-sandbox-XXXXXX/ and bind-mount
    ...
    fn create_input_staging_dir(
        ...
        let mut template = b"/tmp/kuro-sandbox-XXXXXX\0".to_vec();
        ...
        for input_file in input_files {
            ...
            if let Some(parent) = target.parent() {
                if let Err(e) = std::fs::create_dir_all(parent) {
                    ...
                    let msg = format!(
                        "[kuro-sandbox] create_dir_all failed for {}: {}\n",
                        parent.display(),
                        e
                    );
                    let _ =
            ...
            if let Err(e) = bind_mount(input_file, &target) {
                let msg = format!(
                    "[kuro-sandbox] bind-mount input failed {} -> {}: {}\n",
                    input_file.display(),
                    target.display(),
                    e
                );
