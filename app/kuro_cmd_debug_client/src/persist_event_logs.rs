...
use kuro_common::chunk_reader::ChunkReader;
use kuro_common::manifold;
use kuro_common::manifold::ManifoldChunkedUploader;
use kuro_common::manifold::ManifoldClient;
use kuro_core::soft_error;
use kuro_data::InstantEvent;
use kuro_data::PersistEventLogSubprocess;
...
impl PersistEventLogsCommand {
    ...
    ) -> (kuro_error::Result<()>, kuro_error::Result<()>) {
...
async fn create_log_file(local_path: String) -> Result<tokio::fs::File, kuro_error::Error> {
...
impl<'a> Uploader<'a> {
    ...
    async fn upload_chunk(&mut self) -> kuro_error::Result<()> {
    ...
    fn can_fill_chunk(&mut self) -> kuro_error::Result<bool> {
        Ok(self
            .total_bytes
...
fn status_from_result(res: kuro_error::Result<()>) -> (Vec<String>, Option<String>, bool) {
...
fn categorize_error(err: &kuro_error::Error) -> &'static str {
    // This is for internal error tracking in `logview kuro`
...
mod tests {
    ...
    fn test_categorize_error() {
        let err = kuro_error!(kuro_error::ErrorTag::Environment, "CertificateRequired");
        assert_eq!(categorize_error(&err), "persist_log_certificate_required");

        let err = kuro_error!(kuro_error::ErrorTag::Tier0, "Some other error");
