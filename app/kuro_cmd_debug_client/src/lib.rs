...
#[clap(about = "Hidden debug commands useful for testing kuro")]
pub enum DebugCommand {
    ...
    /// Prints the hash of the kuro binary
    ...
    /// Flushes all dep files known to Kuro.
    ...
    /// Validates that Kuro and disk agree on the state of files.
    FileStatus(FileStatusCommand),
    /// Prints kuro daemon directory (`~/.buckd/xxx`).
    DaemonDir(DaemonDirCommand),
    /// Prints kuro executable (this executable) path.
    Exe(ExeCommand),
