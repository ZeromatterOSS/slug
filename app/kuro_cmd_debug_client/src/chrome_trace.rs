...
use kuro_client_ctx::event_log_options::EventLogOptions;
...
struct ChromeTraceFirstPass {
    ...
    pub long_analyses: HashSet<kuro_events::span::SpanId>,
    pub long_loads: HashSet<kuro_events::span::SpanId>,
    pub local_actions: HashSet<kuro_events::span::SpanId>,
    pub critical_path_action_keys: HashSet<kuro_data::ActionKey>,
...
impl ChromeTraceFirstPass {
    ...
    fn handle_event(&mut self, event: &BuckEvent) -> kuro_error::Result<()> {
        match event.data() {
            kuro_data::buck_event::Data::SpanStart(start) => {
                match start.data.as_ref() {
                    Some(kuro_data::span_start_event::Data::ExecutorStage(exec)) => {
            ...
            kuro_data::buck_event::Data::SpanEnd(end) => {
                match end.data.as_ref() {
                    Some(kuro_data::span_end_event::Data::Analysis(_)) => {
                        if end
                            .duration
                            .as_ref()
                    ...
                    Some(kuro_data::span_end_event::Data::Load(_)) => {
                        if end
                            .duration
                            .as_ref()
            ...
            kuro_data::buck_event::Data::Instant(instant) => match instant.data.as_ref() {
                Some(kuro_data::instant_event::Data::BuildGraphInfo(info)) => {
            ...
            kuro_data::buck_event::Data::Record(_) => {}
        };
        Ok(())
    }
}
...
impl ChromeTraceClosedSpan {
    fn to_json(self) -> kuro_error::Result<serde_json::Value> {
        Ok(json!(
            {
                "name": self.open.name,
        ...
        + Serialize,
{
    ...
    fn process_timestamp(&mut self, timestamp: SystemTime) -> kuro_error::Result<()> {
    ...
    fn set(&mut self, timestamp: SystemTime, key: &str, amount: T) -> kuro_error::Result<()> {
    ...
    fn bump(&mut self, timestamp: SystemTime, key: &str, amount: T) -> kuro_error::Result<()> {
        self.process_timestamp(timestamp)?;
        let entry = self.counter_entry(key);
        entry.value += amount;
        Ok(())
    ...
    fn subtract(&mut self, timestamp: SystemTime, key: &str, amount: T) -> kuro_error::Result<()> {
        self.process_timestamp(timestamp)?;
        let entry = self.counter_entry(key);
        entry.value -= amount;
        Ok(())
    ...
    fn flush(&mut self) -> kuro_error::Result<()> {
    ...
    pub fn flush_all_to(&mut self, output: &mut Vec<serde_json::Value>) -> kuro_error::Result<()> {
        self.flush()?;
...
struct SpanCounters {
    ...
    open_spans: HashMap<kuro_events::span::SpanId, (&'static str, i32)>,
}
...
struct ChromeTraceWriter {
    trace_events: Vec<serde_json::Value>,
    open_spans: HashMap<kuro_events::span::SpanId, ChromeTraceOpenSpan>,
...
impl ChromeTraceWriter {
    ...
    fn assign_track_for_span(
        ...
        event: &BuckEvent,
    ) -> kuro_error::Result<Option<SpanTrackAssignment>> {
    ...
    pub fn to_writer<W>(mut self, file: W) -> kuro_error::Result<()>
    where
        W: Write,
    {
        self.span_counters
    ...
    fn handle_event(&mut self, event: &Arc<BuckEvent>) -> kuro_error::Result<()> {
        match event.data() {
            kuro_data::buck_event::Data::SpanStart(kuro_data::SpanStartEvent {
                ...
                let categorization = match start_data {
                    kuro_data::span_start_event::Data::Command(_command) => Categorization::Show {
                    ...
                    kuro_data::span_start_event::Data::ReUpload(_) => {
                        let name = "re_upload";
                        self.span_counters.bump_counter_while_span(event, name, 1)?;
                        Categorization::ShowIfParent { name: name.into() }
                    }
                    kuro_data::span_start_event::Data::FinalMaterialization(..) => {
                        if on_critical_path {
                            Categorization::Show {
                    ...
                    kuro_data::span_start_event::Data::FileWatcher(_file_watcher) => {
                        Categorization::Show {
                            category: SpanCategorization::CriticalPath,
            ...
            kuro_data::buck_event::Data::SpanStart(kuro_data::SpanStartEvent { data: None }) => {}
            kuro_data::buck_event::Data::SpanEnd(end) => self.handle_event_end(end, event)?,
            kuro_data::buck_event::Data::Instant(kuro_data::InstantEvent {
                data: Some(instant_data),
            }) => {
                if let kuro_data::instant_event::Data::Snapshot(snapshot) = instant_data {
                    self.process_memory_counters.set(
                        event.timestamp(),
                        "max_rss_gigabyte",
                        (snapshot.kuro_max_rss) as f64 / Self::BYTES_PER_GIGABYTE,
                    )?;
                    ...
                    self.rate_of_change_counters
                        .set_average_rate_of_change_per_s(
                            ...
                            snapshot.kuro_user_cpu_us,
                        )?;
                    self.rate_of_change_counters
                        .set_average_rate_of_change_per_s(
                            event.timestamp(),
                            "average_system_cpu_in_usecs_per_s",
                            snapshot.kuro_system_cpu_us,
                        )?;
                ...
                } else if let kuro_data::instant_event::Data::ResourceControlEvents(events) =
                    instant_data
                {
                    self.snapshot_counters.set(
                        event.timestamp(),
            ...
            // Data field is oneof and `None` means the event is produced with newer version of `.proto` file
            // which added a variant which is not available in version used when compiling this program.
            kuro_data::buck_event::Data::Instant(kuro_data::InstantEvent { data: None }) => {}
            kuro_data::buck_event::Data::Record(_) => {}
        };
        Ok(())
    }
...
impl ChromeTraceCommand {
    ...
    async fn load_events(
        ...
        Invocation,
        BoxStream<'static, kuro_error::Result<BuckEvent>>,
    )> {
...
impl ChromeTraceCommand {
    async fn trace_writer(log: EventLogPathBuf) -> kuro_error::Result<ChromeTraceWriter> {
