...
use kuro_cli_proto::UnstableHeapDumpRequest;
...
impl StreamingCommand for HeapDumpCommand {
    ...
    async fn exec_impl(
            ...
            .unstable_heap_dump(
                UnstableHeapDumpRequest {
                    ...
                    test_executor_destination_path: test_executor_path
                        .map(|v| -> kuro_error::Result<String> { Ok(v.to_str()?.to_owned()) })
                        .transpose()?,
                },
                events_ctx,
            )
        ...
        if !is_open_source() {
            ...
            kuro_client_ctx::eprintln!("Heap dump written to `{}`", path.to_str()?)?;
        }
