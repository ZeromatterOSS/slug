...
use kuro_cmd_starlark_client::StarlarkSubcommand;
...
use kuro_server_ctx::late_bindings::StarlarkServerCommand;
...
impl StarlarkServerCommand for StarlarkServerCommandImpl {
    async fn starlark(
        ...
        let start_event = ctx
            .command_start_event(kuro_data::StarlarkCommandStart {}.into())
            .await?;
        span_async(
            start_event,
...
async fn server_starlark_command_inner(
    ...
    let result = parse_command_and_execute(context, partial_result_dispatcher, req)
        .await
        .map_err(Into::into);
    let end_event = command_end(&result, kuro_data::StarlarkCommandEnd {});
