...
impl Cache<'_> {
    async fn typecheck(&mut self, path: OwnedStarlarkPath) -> kuro_error::Result<()> {
        self.run(path).await?;
        Ok(())
    }
    ...
    async fn get(&mut self, path: OwnedStarlarkModulePath) -> kuro_error::Result<Interface> {
    ...
    #[async_recursion]
    async fn run(&mut self, path: OwnedStarlarkPath) -> kuro_error::Result<Interface> {
