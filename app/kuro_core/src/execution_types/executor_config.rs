...
impl ReGangWorker {
    pub fn parse(worker_map: SmallMap<&str, &str>) -> kuro_error::Result<ReGangWorker> {
...
impl RemoteExecutorDependency {
    pub fn parse(dep_map: SmallMap<&str, &str>) -> kuro_error::Result<RemoteExecutorDependency> {
...
impl RemoteExecutorUseCase {
    ...
    /// The "kuro-default" use case. This is meant to be used when no use case is configured. It's
    /// not meant to be used for convenience when a use case is not available where it's needed!
    pub fn kuro_default() -> Self {
        static USE_CASE: Lazy<RemoteExecutorUseCase> =
            Lazy::new(|| RemoteExecutorUseCase::new("kuro-default".to_owned()));
        *USE_CASE
    }
}
