...
pub fn map_flavors(flavors: &str, full_target: &str) -> kuro_error::Result<ProvidersName> {
    ...
    match flavors_parts.iter().position(|x| is_platform_flavor(x)) {
        Some(index) => {
            ...
            soft_error!(
                "platform_flavor",
                kuro_error::kuro_error!(kuro_error::ErrorTag::Input, "Platform flavor found in target: {}", full_target).into(),
                deprecation: true,
                quiet: true
            )?;
