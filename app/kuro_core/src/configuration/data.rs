...
enum ConfigurationLookupError {
    ...
    that buck has already loaded the configuration through some other mechanism. You can run `kuro cquery <some_target>`
...
fn emit_configuration_instant_event(cfg: &ConfigurationData) -> kuro_error::Result<()> {
    let constraints: Vec<kuro_data::Constraint> = cfg
        .data()?
        .constraints
        .iter()
        .map(|(k, v)| kuro_data::Constraint {
            setting: k.to_string(),
            value: v.to_string(),
        })
        .collect();

    // Sometimes this isn't going to be init'd in tests (oss or kuro), let's
    ...
    if let Ok(event_dispatch) = EVENT_DISPATCH.get() {
        event_dispatch.emit_instant_event_for_data(
            kuro_data::ConfigurationCreated {
                cfg: Some(kuro_data::ConfigurationWithConstraints {
...
impl ConfigurationData {
    ...
    pub fn from_platform(label: String, data: ConfigurationDataData) -> kuro_error::Result<Self> {
    ...
    pub fn lookup_bound(cfg: BoundConfigurationId) -> kuro_error::Result<Self> {
    ...
    pub fn data(&self) -> kuro_error::Result<&ConfigurationDataData> {
        match &self.0.configuration_platform {
            ConfigurationPlatform::Builtin(BuiltinPlatform::UnspecifiedExec) => {
...
impl ToProtoMessage for ConfigurationData {
    type Message = kuro_data::Configuration;

    fn as_proto(&self) -> Self::Message {
        kuro_data::Configuration {
