...
impl<T> MaybeCompatible<T> {
    ...
    pub fn require_compatible(self) -> kuro_error::Result<T> {
        match self {
...
impl IncompatiblePlatformReason {
    pub fn to_err(&self) -> kuro_error::Error {
        match self.cause {
    ...
    pub fn to_soft_err(&self) -> kuro_error::Error {
        match &self.cause {
