        ...
        ProjectRelativePathBuf::from(ForwardRelativePathBuf::concat([
            ...
            match &origin {
                ...
                ExternalCellOrigin::RepositoryRule(setup) => {
                    ForwardRelativePath::new(setup.repo_name.as_ref()).unwrap()
                }
                ExternalCellOrigin::ExtensionRepo(setup) => {
                    ForwardRelativePath::new(setup.canonical_name.as_ref()).unwrap()
                }
            },
            path.as_ref(),
        ]))
...
mod tests {
    ...
    #[test]
    fn buck_path_resolves() -> kuro_error::Result<()> {
    ...
    #[test]
    fn buck_output_path_resolves() -> kuro_error::Result<()> {
    ...
    #[test]
    fn buck_target_output_path_resolves() -> kuro_error::Result<()> {
        let path_resolver =
    ...
    #[test]
    fn test_resolve_test_discovery() -> kuro_error::Result<()> {
        let path_resolver =
            BuckOutPathResolver::new(ProjectRelativePathBuf::unchecked_new("buck-out".into()));
