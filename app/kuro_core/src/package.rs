...
///
/// A **valid** Kuro package is defined by:
...
impl PackageLabel {
    #[inline]
    pub fn new(cell: CellName, path: &CellRelativePath) -> kuro_error::Result<Self> {
        PackageLabel::from_cell_path(CellPathRef::new(cell, path))
    }

    #[inline]
    pub fn from_cell_path(path: CellPathRef) -> kuro_error::Result<Self> {
    ...
    pub fn join(&self, path: &ForwardRelativePath) -> kuro_error::Result<Self> {
        if path.is_empty() {
            Ok(self.dupe())
        } else {
    ...
    pub fn parent(&self) -> kuro_error::Result<Option<PackageLabel>> {
