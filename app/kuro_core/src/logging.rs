...
pub trait LogConfigurationReloadHandle: Send + Sync + 'static {
    fn update_log_filter(&self, format: &str) -> kuro_error::Result<()>;
}
...
impl LogConfigurationReloadHandle for NoopLogConfigurationReloadHandle {
    fn update_log_filter(&self, _filter: &str) -> kuro_error::Result<()> {
        Ok(())
    }
}
...
where
    ...
    R: Send + Sync + 'static,
{
    fn update_log_filter(&self, raw: &str) -> kuro_error::Result<()> {
        let filter = EnvFilter::try_new(raw)
            .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::LogFilter))
...
pub fn init_tracing_for_writer<W>(
    writer: W,
) -> kuro_error::Result<Arc<dyn LogConfigurationReloadHandle>>
where
    W: for<'writer> MakeWriter<'writer> + Send + Sync + 'static,
{
    ...
    let filter = match kuro_env!(ENV_VAR)? {
