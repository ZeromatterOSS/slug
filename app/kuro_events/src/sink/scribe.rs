...
/// RemoteEventSink is a ScribeSink backed by the Thrift-based client in the `kuro_scribe_client` crate.
...
impl RemoteEventSink {
    ...
    pub fn new(
        ...
        category: String,
        config: ScribeConfig,
    ) -> kuro_error::Result<RemoteEventSink> {
    ...
    pub async fn send_now(&self, event: BuckEvent) -> kuro_error::Result<()> {
        self.send_messages_now(vec![event]).await
    }

    // Send multiple events now, bypassing internal message queue.
    pub async fn send_messages_now(&self, events: Vec<BuckEvent>) -> kuro_error::Result<()> {
        let messages = events
            .into_iter()
            .map(|e| {
    ...
    fn encode_message(mut event: BuckEvent) -> Vec<u8> {
        ...
        let mut proto: Box<kuro_data::BuckEvent> = event.into();
        ...
        if buf.len() > SCRIBE_MESSAGE_SIZE_LIMIT {
            ...
            let proto: Box<kuro_data::BuckEvent> = BuckEvent::new(
                    SystemTime::now(),
                    TraceId::new(),
                    None,
                    None,
                    kuro_data::buck_event::Data::Instant(InstantEvent {
                        data: Some(
                            StructuredError {
                                ...
                                quiet: false,
                                task: Some(true),
                                soft_error_category: Some(kuro_data::SoftError {category: "oversized_scribe".to_owned(), is_quiet:false}),
    ...
    fn prepare_event(event: &mut kuro_data::BuckEvent) {
    ...
    fn should_send_event(&self, data: &kuro_data::buck_event::Data) -> bool {
        use kuro_data::buck_event::Data;
        ...
        match data {
            ...
            Data::Record(r) => {
                use kuro_data::record_event::Data;
...
fn get_is_cache_hit(details: &kuro_data::CommandExecutionDetails) -> bool {
    use kuro_data::command_execution_kind::Command::RemoteCommand;
...
pub(crate) fn scribe_category() -> kuro_error::Result<String> {
    const DEFAULT_SCRIBE_CATEGORY: &str = "kuro_events";
    ...
    Ok(
        kuro_env!("BUCK2_SCRIBE_CATEGORY", applicability = internal)?
...
mod tests {
    ...
    fn test_encode_large_message() {
        ...
        let event = BuckEvent::new(
            ...
            None,
            kuro_data::buck_event::Data::Instant(InstantEvent {
                data: Some(kuro_data::instant_event::Data::StructuredError(
                    kuro_data::StructuredError {
