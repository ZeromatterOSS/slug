...
fn truncate(s: &str, max_bytes: usize) -> String {
    ...
    kuro_util::truncate::truncate(s, max_bytes.min(MAX_STRING_BYTES))
}

#[cfg_attr(not(fbcode_build), allow(dead_code))]
pub(crate) fn smart_truncate_event(d: &mut kuro_data::buck_event::Data) {
    ...
    match d {
        ...
        Data::Instant(inst) => {
            use kuro_data::instant_event::Data;
            match &mut inst.data {
        ...
        Data::Record(rec) => {
            if let Some(kuro_data::record_event::Data::InvocationRecord(invocation_record)) =
                &mut rec.data
            {
...
fn truncate_invocation_record(invocation_record: &mut kuro_data::InvocationRecord) {
...
fn truncate_action_execution_end(action_execution_end: &mut kuro_data::ActionExecutionEnd) {
    let per_command_size_budget = (500 * 1024) / action_execution_end.commands.len().max(1);

    let truncate_cmd = |cmd: &mut kuro_data::CommandExecution, truncate_all: bool| {
...
fn truncate_command_end(command_end: &mut kuro_data::CommandEnd, clear_target_patterns: bool) {
    use kuro_data::command_end::Data;
...
fn truncate_file_watcher_stats(file_watcher_stats: &mut kuro_data::FileWatcherStats) {
...
fn truncate_test_end(test_end: &mut kuro_data::TestRunEnd) {
...
fn truncate_test_discovery_end(test_discovery_end: &mut kuro_data::TestDiscoveryEnd) {
...
fn truncate_target_patterns(target_patterns: &mut Vec<kuro_data::TargetPattern>) {
    ...
    for (index, target) in target_patterns.iter().enumerate() {
        ...
        if bytes > MAX_TARGET_PATTERNS_BYTES {
            ...
            let warn = format!("<<Truncated (reported {index} / {orig_len})>>");
            target_patterns.push(kuro_data::TargetPattern { value: warn });
            break;
        }
    }
}
...
mod tests {
    ...
    fn make_invocation_record(data: kuro_data::InvocationRecord) -> kuro_data::buck_event::Data {
        kuro_data::buck_event::Data::Record(kuro_data::RecordEvent {
            data: Some(kuro_data::record_event::Data::InvocationRecord(Box::new(
                data,
            ))),
        })
    }
    ...
    fn make_action_execution_end(
        data: kuro_data::ActionExecutionEnd,
    ) -> kuro_data::buck_event::Data {
        kuro_data::buck_event::Data::SpanEnd(kuro_data::SpanEndEvent {
            data: Some(kuro_data::span_end_event::Data::ActionExecution(Box::new(
                data,
            ))),
            ..Default::default()
        })
    ...
    fn make_command_end(data: kuro_data::CommandEnd) -> kuro_data::buck_event::Data {
        kuro_data::buck_event::Data::SpanEnd(kuro_data::SpanEndEvent {
            data: Some(kuro_data::span_end_event::Data::Command(data)),
            ..Default::default()
        })
    }

    fn make_build_command_end(
        unresolved_target_patterns: Vec<kuro_data::TargetPattern>,
    ) -> kuro_data::CommandEnd {
        kuro_data::CommandEnd {
            data: Some(kuro_data::command_end::Data::Build(
                kuro_data::BuildCommandEnd {
    ...
    fn make_test_end(data: kuro_data::TestRunEnd) -> kuro_data::buck_event::Data {
        kuro_data::buck_event::Data::SpanEnd(kuro_data::SpanEndEvent {
            data: Some(kuro_data::span_end_event::Data::TestEnd(data)),
            ..Default::default()
        })
    }

    fn make_test_discovery_end(data: kuro_data::TestDiscoveryEnd) -> kuro_data::buck_event::Data {
        kuro_data::buck_event::Data::SpanEnd(kuro_data::SpanEndEvent {
            data: Some(kuro_data::span_end_event::Data::TestDiscovery(data)),
            ..Default::default()
        })
    }

    fn make_command_execution_with_stderr(stderr: String) -> kuro_data::CommandExecution {
        kuro_data::CommandExecution {
            details: Some(kuro_data::CommandExecutionDetails {
                cmd_stderr: stderr,
                ..Default::default()
            }),
            ..Default::default()
    ...
    fn smart_truncate_resolved_target_patterns_clears_unresolved_one() {
        let mut record = kuro_data::InvocationRecord::default();
        let mut record_expected = record.clone();

        let resolved_target_patterns = vec![kuro_data::TargetPattern {
            value: "some_resolved_target".to_owned(),
        }];
        record.parsed_target_patterns = Some(kuro_data::ParsedTargetPatterns {
            target_patterns: resolved_target_patterns.clone(),
        });
        // resolved_target_patterns is expected to be unchanged.
        record_expected.parsed_target_patterns = Some(kuro_data::ParsedTargetPatterns {
            target_patterns: resolved_target_patterns,
        });

        let unresolved_target_patterns = vec![kuro_data::TargetPattern {
    ...
    fn smart_truncate_action_execution_end_one_last_command_truncated() {
        ...
        let action_execution_end_with_stderrs = kuro_data::ActionExecutionEnd {
            commands: vec![command_execution_with_stderr],
            ..Default::default()
        };
        let action_execution_end_last_stderr_omitted = kuro_data::ActionExecutionEnd {
    ...
    fn smart_truncate_long_file_watcher_stats_truncated() {
        let file_watcher_event = kuro_data::FileWatcherEvent {
            path: "0123456789".repeat(3 * 1024),
            ..Default::default()
        };
        let file_watcher_stats = kuro_data::FileWatcherStats {
            events: vec![
        ...
        let file_watcher_stats_truncated = kuro_data::FileWatcherStats {
            events: vec![
                file_watcher_event.clone(),
                file_watcher_event.clone(),
                file_watcher_event,
        ...
        let record_truncated = kuro_data::InvocationRecord {
    ...
    fn smart_truncate_invocation_record_error_reports_truncated() {
        ...
        let mut event_data = make_invocation_record(kuro_data::InvocationRecord {
            errors,
            ..Default::default()
        });
        smart_truncate_event(&mut event_data);

        let kuro_data::buck_event::Data::Record(record_event) = event_data else {
            unreachable!()
        };
        let Some(kuro_data::record_event::Data::InvocationRecord(invocation_record)) =
            record_event.data
        else {
            unreachable!()
        };
    ...
    fn smart_truncate_test_end_long_test_names_truncated() {
        ...
        let test_end = kuro_data::TestRunEnd {
            suite: Some(kuro_data::TestSuite {
                test_names,
                ..Default::default()
            }),
            ..Default::default()
        };
        let test_end_truncated = kuro_data::TestRunEnd {
            suite: Some(kuro_data::TestSuite {
    ...
    fn make_command_execution_with_stdout(stdout: String) -> kuro_data::CommandExecution {
        kuro_data::CommandExecution {
            details: Some(kuro_data::CommandExecutionDetails {
                cmd_stderr: stdout,
                ..Default::default()
    ...
    fn smart_truncate_test_discovery_end_command_report_stdout_truncated() {
        let test_discovery_end = kuro_data::TestDiscoveryEnd {
            command_report: Some(make_command_execution_with_stdout("blah".to_owned())),
            ..Default::default()
        };

        let test_discovery_end_truncated = kuro_data::TestDiscoveryEnd {
            command_report: Some(make_command_execution_with_stdout("<<omitted>>".to_owned())),
            ..Default::default()
        };
