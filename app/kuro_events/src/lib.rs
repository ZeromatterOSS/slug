...
//!
//! Events and event streams for Kuro.
//!
//! The `Event` enum is the set of events that Kuro can produce. Events can be produced both during the course of
...
/// An event that can be produced by Kuro. Events are points in time with additional metadata attached to them,
...
impl BuckEvent {
    ...
    pub fn trace_id(&self) -> kuro_error::Result<TraceId> {
    ...
    pub fn event(&self) -> &kuro_data::BuckEvent {
        &self.event
    }

    pub fn data(&self) -> &kuro_data::buck_event::Data {
        self.event
            .data
            .as_ref()
    ...
    pub fn data_mut(&mut self) -> &mut kuro_data::buck_event::Data {
        self.event
            .data
            .as_mut()
            .expect("data is set, it is validated")
    ...
    pub fn span_start_event(&self) -> Option<&kuro_data::SpanStartEvent> {
        match self.data() {
            kuro_data::buck_event::Data::SpanStart(start) => Some(start),
            _ => None,
        }
    }

    pub fn span_end_event(&self) -> Option<&kuro_data::SpanEndEvent> {
        match self.data() {
            kuro_data::buck_event::Data::SpanEnd(end) => Some(end),
            _ => None,
        }
    }

    pub fn command_start(&self) -> kuro_error::Result<Option<&kuro_data::CommandStart>> {
        match self.span_start_event() {
            ...
            Some(span_start_event) => {
                ...
                {
                    kuro_data::span_start_event::Data::Command(command_start) => {
...
impl From<BuckEvent> for Box<kuro_data::BuckEvent> {
    fn from(e: BuckEvent) -> Self {
        e.event
    }
}

impl TryFrom<Box<kuro_data::BuckEvent>> for BuckEvent {
    type Error = kuro_error::Error;

    fn try_from(event: Box<kuro_data::BuckEvent>) -> kuro_error::Result<BuckEvent> {
...
#[kuro(tag = InvalidEvent)]
enum BuckEventError {
    #[error("The `kuro_data::BuckEvent` provided has no `Timestamp`")]
    MissingTimestamp,
    #[error("The `kuro_data::BuckEvent` provided has no `Data`")]
    MissingData,
...
pub fn init_late_bindings() {
    kuro_core::event::EVENT_DISPATCH.init(&dispatch::EventDispatcherLateBinding);
}
...
mod tests {
    ...
    fn round_trip_success() {
        ...
        assert_eq!(
            test,
            BuckEvent::try_from(Box::<kuro_data::BuckEvent>::from(test.clone())).unwrap()
        );
    }
