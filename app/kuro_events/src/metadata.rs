...
use kuro_wrapper_common::BUCK2_WRAPPER_ENV_VAR;
...
pub fn collect(daemon: &DaemonId) -> HashMap<String, String> {
    ...
    if let Some(rev) = kuro_build_info::revision() {
        map.insert("kuro_revision".to_owned(), rev.to_owned());
    }

    if let Some(time) = kuro_build_info::time_iso8601() {
        map.insert("kuro_build_time".to_owned(), time.to_owned());
    }

    if let Some(ts) = kuro_build_info::release_timestamp() {
        map.insert("kuro_release_timestamp".to_owned(), ts.to_owned());
    }
...
pub fn username() -> kuro_error::Result<Option<String>> {
    #[cfg(fbcode_build)]
    {
        use kuro_error::conversion::from_any_with_tag;
        Ok(Some(user::current_username().map_err(|e| {
            from_any_with_tag(e, kuro_error::ErrorTag::InvalidUsername)
        })?))
    }
    #[cfg(not(fbcode_build))]
    {
