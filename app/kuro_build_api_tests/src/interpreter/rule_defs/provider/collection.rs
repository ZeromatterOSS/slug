...
#[test]
fn provider_collection_constructs_properly() -> kuro_error::Result<()> {
...
#[test]
fn provider_collection_fails_to_construct_on_bad_data() -> kuro_error::Result<()> {
    let mut tester = provider_collection_tester()?;
...
#[test]
fn returns_default_info() -> kuro_error::Result<()> {
    let mut tester = provider_collection_tester()?;
    tester.run_starlark_bzl_test(indoc!(
        r#"
...
#[test]
fn provider_collection_contains_methods_and_in_operator() -> kuro_error::Result<()> {
    let mut tester = provider_collection_tester()?;
    tester.add_import(
...
#[test]
fn provider_collection_get() -> kuro_error::Result<()> {
    let mut tester = provider_collection_tester()?;
    tester.add_import(
        &ImportPath::testing_new("root//providers:defs.bzl"),
        indoc!(
