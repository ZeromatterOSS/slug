...
use kuro_build_api::interpreter::rule_defs::required_test_local_resource::register_required_test_local_resource;
...
#[test]
fn test_construction() -> kuro_error::Result<()> {
    let test = indoc!(
        r#"
        def test():
...
#[test]
fn test_validation() -> kuro_error::Result<()> {
    let mut tester = tester();
...
#[test]
fn test_validation_at_freeze() -> kuro_error::Result<()> {
    let mut tester = tester();
