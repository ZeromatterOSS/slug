...
use kuro_build_api::actions::ActionErrors;
use kuro_build_api::actions::registry::ActionsRegistry;
use kuro_build_api::analysis::registry::AnalysisValueFetcher;
...
#[test]
fn declaring_artifacts() -> kuro_error::Result<()> {
    Heap::temp(|heap| {
...
#[test]
fn claiming_conflicting_path() -> kuro_error::Result<()> {
    let mut actions = ActionsRegistry::new(
...
#[test]
fn register_actions() -> kuro_error::Result<()> {
    Heap::temp(|heap| {
        let base = BaseDeferredKey::TargetLabel(ConfiguredTargetLabel::testing_parse(
            "cell//pkg:foo",
            ConfigurationData::testing_new(),
...
#[test]
fn finalizing_actions() -> kuro_error::Result<()> {
    Heap::temp(|heap| {
        let base = BaseDeferredKey::TargetLabel(ConfiguredTargetLabel::testing_parse(
            "cell//pkg:foo",
            ConfigurationData::testing_new(),
