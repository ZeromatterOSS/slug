...
use slug_data::FileWatcherEventType as Type;
use slug_data::FileWatcherKind as Kind;
...
pub(crate) enum EdenFsWatcherError {
    #[slug(tag = IoNotConnected)]
    #[error("Failed to connect to EdenFS {0}")]
    EdenConnectionError(slug_error::Error),
    #[slug(tag = Input)]
...
impl EdenFsFileWatcher {
    pub(crate) fn new(
        ...
        let manager = EdenConnectionManager::new(fb, project_root, Some(semaphore::slug_default()))
    ...
    ) -> slug_error::Result<ProcessChangeStatus> {
                        ...
                        soft_error!(
                            ...
                            slug_error::slug_error!(
                                ...
                                "EdenFS reported SmallChangeNotification::renamed directory: '{}' -> '{}'. \
                                 ...
                                 EdenFS Thrift API has changed and the slug code needs to be updated.",
    ...
    async fn skip_change(&self, change: &ChangeNotification) -> slug_error::Result<()> {
    ...
    async fn process_sapling_status(
            ...
            .buck_error_context("Failed to get Sapling status.")?
        {
            ...
            SaplingGetStatusResult::Normal(status) => {
                ...
                let results: slug_error::Result<Vec<_>> = status
                    .into_iter()
    ...
    async fn on_large_or_unknown_change(
        &self,
        dice: DiceTransactionUpdater,
    ) -> slug_error::Result<(FileWatcherStats, FileChangeTracker, DiceTransactionUpdater)> {
        ...
        let base_stats = self.base_file_watcher_stats().await?;
        let mut base_stats = slug_data::FileWatcherStats {
            fresh_instance: true,
            watchman_version: None,
            fresh_instance_data: Some(slug_data::FreshInstance {
    ...
    async fn update_mergebase(&self, to: &str) -> slug_error::Result<bool> {
    ...
    async fn base_file_watcher_stats(&self) -> slug_error::Result<slug_data::FileWatcherStats> {
