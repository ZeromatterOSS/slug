...
impl ExternalSymlink {
    ...
    pub fn interior_links(&self) -> slug_error::Result<Vec<AbsoluteSymlink>> {
...
///   `slug debug io-trace export-manifest`
...
impl RepositoryMetadata {
    pub fn from_cwd() -> slug_error::Result<Self> {
        Self::from_path(
            std::env::current_dir().buck_error_context("Error getting current directory")?,
        )
    }

    pub fn from_path<P: AsRef<Path>>(path: P) -> slug_error::Result<Self> {
...
pub fn hg<I, S>(args: I) -> slug_error::Result<String>
where
...
fn hg_in<I, S, P>(path: P, args: I) -> slug_error::Result<String>
where
    I: IntoIterator<Item = S>,
    S: AsRef<OsStr>,
    P: AsRef<Path>,
    ...
    if result.status.success() {
        ...
        if out.is_empty() {
            Err(slug_error::slug_error!(
                slug_error::ErrorTag::OfflineArchive,
...
mod tests {
    ...
    fn create_tree(entries: Vec<Entry>) -> slug_error::Result<TempDir> {
    ...
    #[test]
    fn test_full_target_no_remaining() -> slug_error::Result<()> {
    ...
    #[test]
    fn test_full_target_with_remaining() -> slug_error::Result<()> {
        let external_link = ExternalSymlink {
            link: ProjectRelativePathBuf::unchecked_new("foo/bar/baz".to_owned()),
            target: AbsPathBuf::new("/some/absolute/path")?,
    ...
    #[test]
    fn test_interior_links_no_remaining() -> slug_error::Result<()> {
        let external_link = ExternalSymlink {
            link: ProjectRelativePathBuf::unchecked_new("foo/bar/baz".to_owned()),
            target: AbsPathBuf::new("/some/absolute/path")?,
            remaining_path: None,
    ...
    #[test]
    fn test_interior_links_relative() -> slug_error::Result<()> {
    ...
    #[test]
    fn test_interior_links_absolute() -> slug_error::Result<()> {
        let tree = create_tree(vec![
            Entry::Dir("foo/bar/baz"),
            Entry::File("foo/bar/stuff.txt"),
            Entry::AbsoluteSymlink(Symlink {
