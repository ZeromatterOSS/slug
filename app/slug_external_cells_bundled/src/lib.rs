...
#[cfg(buck_build)]
mod slug_builtins {
    include!("slug_builtins/contents.rs");
}

#[cfg(not(buck_build))]
mod slug_builtins {
    include!(concat!(env!("OUT_DIR"), "/slug_builtins_include.rs"));
}
...
const SLUG_BUILTINS: BundledCell = BundledCell {
    name: "slug_builtins",
    files: slug_builtins::DATA,
    is_testing: false,
};
