...
fn imp() -> io::Result<()> {
    ...
    // Generate slug_builtins bundled files. The @slug_builtins cell
    // ships an `exports.bzl` whose public symbols are auto-injected
    // into every BUILD and `.bzl` file by the interpreter's
    // `bazel_builtins_autoload`. We stage the source `slug_builtins/`
    // directory into OUT_DIR alongside a build-time-generated
    // `_host_constants.bzl` (host OS/CPU constraint labels, consumed
    // by `_invoke_rule` in `exports.bzl` to serve
    // `ctx.target_platform_has_constraint` from Starlark). Walking the
    // staged tree (rather than the source tree) lets the bundled cell
    // carry the generated file.
    let slug_builtins_path = project_root.join("slug_builtins");
    assert!(
        slug_builtins_path.join("exports.bzl").exists(),
        "slug_builtins/exports.bzl not found"
    );
    println!("cargo:rerun-if-changed={}", slug_builtins_path.display());
    let slug_builtins_staged = out_dir.join("slug_builtins_staged");
    if slug_builtins_staged.exists() {
        std::fs::remove_dir_all(&slug_builtins_staged)?;
    }
    std::fs::create_dir_all(&slug_builtins_staged)?;
    for entry in walkdir::WalkDir::new(&slug_builtins_path) {
        let entry = entry.map_err(|e| e.into_io_error().unwrap())?;
        if !entry.file_type().is_file() {
            continue;
        }
        let rel = entry.path().strip_prefix(&slug_builtins_path).unwrap();
        let dest = slug_builtins_staged.join(rel);
        if let Some(parent) = dest.parent() {
            std::fs::create_dir_all(parent)?;
        }
        std::fs::copy(entry.path(), &dest)?;
    }
    // Empty strings stand in for unknown host OS/CPU and are filtered
    // out at use-time.
    let host_os_constraint = match std::env::consts::OS {
    ...
    let host_cpu_constraint = match std::env::consts::ARCH {
    ...
    let host_constants = format!(
        "# @generated by app/slug_external_cells_bundled/build.rs\n\
         #\n\
         # Host OS/CPU constraint labels detected at slug build time.\n\
         # Consumed by `_invoke_rule` in `exports.bzl` to serve\n\
         # `ctx.target_platform_has_constraint` from Starlark.\n\
         HOST_CONSTRAINT_LABELS = [\n    \"{host_cpu_constraint}\",\n    \"{host_os_constraint}\",\n]\n"
    );
    std::fs::write(
        slug_builtins_staged.join("_host_constants.bzl"),
        host_constants,
    )?;
    write_include_file(
        &slug_builtins_staged,
        std::fs::File::create(out_dir.join("slug_builtins_include.rs"))?,
    )?;
    ...
    // py_runtime+py_runtime_pair wired into a toolchain() target. Slug auto-
    ...
    //
    // The interpreter_path is detected at slug build time; if none is found,
    ...
    let stub_toolchain_bzl = concat!(
        ...
        "# toolchain types that slug must register to satisfy rule-level mandatory\n",
