...
fn imp() -> io::Result<()> {
    ...
    // `bazel_builtins_autoload`. We stage the source `kuro_builtins/`
    // directory into OUT_DIR alongside a build-time-generated
    // `_host_constants.bzl` (host OS/CPU constraint labels detected at
    // build time, consumed by `_invoke_rule` in `exports.bzl` to serve
    // `ctx.target_platform_has_constraint` from Starlark — Plan 28.4
    // Stage 3). Walking the staged tree (rather than the source tree)
    // lets the bundled cell carry the generated file.
    ...
    let kuro_builtins_staged = out_dir.join("kuro_builtins_staged");
    if kuro_builtins_staged.exists() {
        std::fs::remove_dir_all(&kuro_builtins_staged)?;
    }
    std::fs::create_dir_all(&kuro_builtins_staged)?;
    for entry in walkdir::WalkDir::new(&kuro_builtins_path) {
        let entry = entry.map_err(|e| e.into_io_error().unwrap())?;
        if !entry.file_type().is_file() {
            continue;
        }
        let rel = entry.path().strip_prefix(&kuro_builtins_path).unwrap();
        let dest = kuro_builtins_staged.join(rel);
        if let Some(parent) = dest.parent() {
            std::fs::create_dir_all(parent)?;
        }
        std::fs::copy(entry.path(), &dest)?;
    }
    // Mirrors the table in target_platform_has_constraint (Plan 28.4
    // pre-Stage-3) so the Starlark migration preserves byte-for-byte
    // behaviour. Empty strings stand in for unknown host OS/CPU and are
    // filtered out at use-time.
    ...
    let host_constants = format!(
        "# @generated by app/kuro_external_cells_bundled/build.rs\n\
         #\n\
         # Plan 28.4 Stage 3: host OS/CPU constraint labels detected at\n\
         # kuro build time. Consumed by `_invoke_rule` in `exports.bzl`\n\
         # to serve `ctx.target_platform_has_constraint` from Starlark.\n\
         # Must match the table in `target_platform_has_constraint` in\n\
         # `app/kuro_build_api/src/interpreter/rule_defs/context.rs`\n\
         # (deleted as part of the Stage 3 migration).\n\
         HOST_CONSTRAINT_LABELS = [\n    \"{host_cpu_constraint}\",\n    \"{host_os_constraint}\",\n]\n"
    );
    std::fs::write(
        kuro_builtins_staged.join("_host_constants.bzl"),
        host_constants,
    )?;
    write_include_file(
        &kuro_builtins_staged,
