...
use kuro_common::cas_digest::SHA256_SIZE;
...
use kuro_http::retries::AsKuroError;
...
impl Checksum {
    pub fn new(sha1: Option<&str>, sha256: Option<&str>) -> kuro_error::Result<Self> {
...
impl AsKuroError for HttpHeadError {
    fn as_kuro_error(self) -> kuro_error::Error {
        kuro_error::Error::from(self)
    }
}

impl AsKuroError for HttpDownloadError {
    fn as_kuro_error(self) -> kuro_error::Error {
        kuro_error::Error::from(self)
    }
}

pub async fn http_head(client: &HttpClient, url: &str) -> kuro_error::Result<Response<()>> {
...
pub async fn http_download(
    ...
    checksum: &Checksum,
    executable: bool,
) -> kuro_error::Result<TrackedFileDigest> {
    ...
    Ok(http_retry(
        || async {
            ...
            let file = fs_util::create_file(&abs_path)
                .map_err(|e| HttpDownloadError::IoError(kuro_error::Error::from(e)))?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_copy_and_hash_ok() -> kuro_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_copy_and_hash_invalid_primary_hash() -> kuro_error::Result<()> {
        assert_matches!(
    ...
    #[tokio::test]
    async fn test_copy_and_hash_invalid_secondary_hash() -> kuro_error::Result<()> {
        assert_matches!(
