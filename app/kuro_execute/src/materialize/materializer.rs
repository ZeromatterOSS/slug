...
#[derive(kuro_error::Error, Debug, Clone, Dupe)]
#[error(
    ...
    To proceed, you should restart Buck using `kuro killall`.

Debug information:
  Path: {}
  Digest origin: {}
  Directory:\n{}", .path, .info.origin.as_display_for_not_found(), format_directory_entry_leaves(.directory))]
#[kuro(tag = MaterializationError)]
pub struct CasNotFoundError {
    ...
    #[source]
    pub error: Arc<kuro_error::Error>,
}
...
pub trait Materializer: Allocative + Send + Sync + 'static {
    ...
    async fn declare_write<'a>(
        &self,
        generate: Box<dyn FnOnce() -> kuro_error::Result<Vec<WriteRequest>> + Send + 'a>,
    ) -> kuro_error::Result<Vec<ArtifactValue>>;
    ...
    ) -> kuro_error::Result<DeclareMatchOutcome>;
    ...
    async fn has_artifact_at(&self, path: ProjectRelativePathBuf) -> kuro_error::Result<bool>;
    ...
    async fn invalidate(&self, path: ProjectRelativePathBuf) -> kuro_error::Result<()> {
        self.invalidate_many(vec![path]).await
    }

    /// Declare an artifact at `path` exists. This will overwrite any pre-existing materialization
    /// methods for this file and indicate that no materialization is necessary.
    async fn invalidate_many(&self, paths: Vec<ProjectRelativePathBuf>) -> kuro_error::Result<()>;
    ...
    ) -> kuro_error::Result<BoxStream<'static, Result<(), MaterializationError>>>;
    ...
    /// Kuro significantly slower, after we stop deferring local copies of
    /// artifacts that are already on disk.
    async fn get_materialized_file_paths(
        &self,
        file_paths: Vec<ProjectRelativePathBuf>,
    ) -> kuro_error::Result<Vec<Result<ProjectRelativePathBuf, ArtifactNotMaterializedReason>>>;
    ...
    /// time.
    fn add_snapshot_stats(&self, _snapshot: &mut kuro_data::Snapshot) {}
...
impl dyn Materializer {
    ...
    fn check_declared_external_symlink(&self, value: &ArtifactValue) -> kuro_error::Result<()> {
        match value.entry() {
...
pub enum MaterializationMethodError {
    #[error(
        "Invalid value for buckconfig `[kuro] materializations`. Got `{0}`. Expected one of `all`, `deferred`, or `deferred_skip_final_artifacts`."
    )]
    InvalidValueForConfig(String),
}

impl MaterializationMethod {
    pub fn try_new_from_config_value(config_value: Option<&str>) -> kuro_error::Result<Self> {
        match config_value {
...
pub trait DeferredMaterializerExtensions: Send + Sync {
    ...
    fn fsck(
        &self,
    ) -> kuro_error::Result<BoxStream<'static, (ProjectRelativePathBuf, kuro_error::Error)>>;

    async fn refresh_ttls(&self, min_ttl: i64) -> kuro_error::Result<()>;

    async fn get_ttl_refresh_log(&self) -> kuro_error::Result<String>;
    ...
    async fn clean_stale_artifacts(
        ...
        dry_run: bool,
        tracked_only: bool,
    ) -> kuro_error::Result<kuro_cli_proto::CleanStaleResponse>;

    async fn test_iter(&self, count: usize) -> kuro_error::Result<String>;
    async fn flush_all_access_times(&self) -> kuro_error::Result<String>;

    /// Create a new DeferredMaterializerSubscription.
    async fn create_subscription(
        &self,
    ) -> kuro_error::Result<Box<dyn DeferredMaterializerSubscription>>;
}
