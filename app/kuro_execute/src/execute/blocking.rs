...
pub trait BlockingExecutor: Allocative + Send + Sync + 'static {
    ...
    async fn execute_dyn_io_inline<'a>(
        &self,
        f: Box<dyn FnOnce() -> kuro_error::Result<()> + Send + 'a>,
    ) -> kuro_error::Result<()>;
...
impl dyn BlockingExecutor {
    pub async fn execute_io_inline<F, T>(&self, f: F) -> kuro_error::Result<T>
    where
        F: FnOnce() -> kuro_error::Result<T> + Send,
        T: Send,
    {
        let mut res = None;
        ...
        .await
        .tag(kuro_error::ErrorTag::IoBlockingExecutor)?;
...
struct ThreadPoolIoRequest {
    io: Box<dyn IoRequest>,
    sender: oneshot::Sender<kuro_error::Result<()>>,
}
...
impl BuckBlockingExecutor {
    ...
    pub fn default_concurrency(fs: ProjectRoot) -> kuro_error::Result<Self> {
        let io_threads = kuro_env!("BUCK2_IO_THREADS", type=usize, default=4)?;
        let io_semaphore = kuro_env!("BUCK2_IO_SEMAPHORE", type=usize, default=kuro_util::threads::available_parallelism())?;
...
impl DirectIoExecutor {
    pub fn new(project_fs: ProjectRoot) -> kuro_error::Result<Self> {
        Ok(Self { project_fs })
    }
}
