...
impl CommandExecutionKind {
    pub fn as_enum(&self) -> kuro_data::ActionExecutionKind {
        match self {
            Self::Local { .. } => kuro_data::ActionExecutionKind::Local,
            Self::LocalWorker { .. } | Self::LocalWorkerInit { .. } => {
                kuro_data::ActionExecutionKind::LocalWorker
            }
            Self::Remote { details, .. } => match details.persistent_worker {
                false => kuro_data::ActionExecutionKind::Remote,
                true => kuro_data::ActionExecutionKind::RemoteWorker,
            },
            Self::ActionCache { .. } => kuro_data::ActionExecutionKind::ActionCache,
            Self::RemoteDepFileCache { .. } => kuro_data::ActionExecutionKind::RemoteDepFileCache,
        }
    }

    pub fn to_proto(&self, omit_details: bool) -> kuro_data::CommandExecutionKind {
        ...
        let command = Some(match self {
            Self::Local {
                ...
                if omit_details {
                    Command::OmittedLocalCommand(kuro_data::OmittedLocalCommand {
                        action_digest: digest.to_string(),
                    })
                } else {
                    Command::LocalCommand(kuro_data::LocalCommand {
                        action_digest: digest.to_string(),
                        argv: command.to_owned(),
                        env: env
                            .iter()
                            .map(|(key, value)| kuro_data::EnvironmentEntry {
                                key: key.clone(),
                                value: value.clone(),
                            })
                            .collect(),
            ...
            } => Command::RemoteCommand(kuro_data::RemoteCommand {
                action_digest: details.action_digest.to_string(),
                cache_hit: false,
                cache_hit_type: kuro_data::CacheHitType::Executed.into(),
            ...
            Self::ActionCache { details } => Command::RemoteCommand(kuro_data::RemoteCommand {
                action_digest: details.action_digest.to_string(),
                cache_hit: true,
                cache_hit_type: kuro_data::CacheHitType::ActionCache.into(),
                queue_time: None,
                details: details.to_proto(omit_details),
                remote_dep_file_key: None,
            ...
            Self::RemoteDepFileCache { details } => {
                Command::RemoteCommand(kuro_data::RemoteCommand {
                    action_digest: details.action_digest.to_string(),
                    cache_hit: true,
                    cache_hit_type: kuro_data::CacheHitType::RemoteDepFileCache.into(),
                    queue_time: None,
                    details: details.to_proto(omit_details),
            ...
            Self::LocalWorkerInit { command, env } => {
                Command::WorkerInitCommand(kuro_data::WorkerInitCommand {
                    argv: command.to_owned(),
                    env: env
                        .iter()
                        .map(|(key, value)| kuro_data::EnvironmentEntry {
            ...
            Self::LocalWorker {
                ...
                digest,
                fallback_exe,
            } => Command::WorkerCommand(kuro_data::WorkerCommand {
                action_digest: digest.to_string(),
                argv: command.to_owned(),
                env: env
                    .iter()
        ...
        kuro_data::CommandExecutionKind { command }
    }
}
...
impl RemoteCommandExecutionDetails {
    ...
    fn to_proto(&self, omit_details: bool) -> Option<kuro_data::RemoteCommandDetails> {
        if omit_details {
            return None;
        }

        Some(kuro_data::RemoteCommandDetails {
