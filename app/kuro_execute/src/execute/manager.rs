...
pub trait CommandExecutionManagerExt: Sized {
    ...
    fn error(
        self,
        stage: &'static str,
        error: impl Into<kuro_error::Error>,
    ) -> CommandExecutionResult {
...
fn inline_environment_metadata() -> kuro_data::InlineCommandExecutionEnvironmentMetadata {
    kuro_data::InlineCommandExecutionEnvironmentMetadata {
        sandcastle_instance_id:
            kuro_env!("SANDCASTLE_INSTANCE_ID", type = u64, applicability = internal)
                .ok()
                .flatten(),
    }
}
