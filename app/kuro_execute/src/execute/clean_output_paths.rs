...
#[cfg(unix)]
fn tag_environment_error(error: kuro_error::Error) -> kuro_error::Error {
    error
}
...
fn tag_cleanup_path_env_error(res: Result<(), IoError>) -> kuro_error::Result<()> {
    res.map_err(kuro_error::Error::from)
        .map_err(tag_environment_error)
}

#[tracing::instrument(level = "debug", skip(fs), fields(path = %path))]
pub fn cleanup_path(fs: &ProjectRoot, path: &ProjectRelativePath) -> kuro_error::Result<()> {
        ...
        path = match path.parent() {
            ...
            None => {
                return Err(kuro_error!(
                    kuro_error::ErrorTag::CleanOutputs,
