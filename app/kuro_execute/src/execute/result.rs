...
pub struct CommandExecutionMetadata {
    ...
    /// Additional stats.
    pub execution_stats: Option<kuro_data::CommandExecutionStats>,
...
impl CommandExecutionMetadata {
    ...
    pub fn to_proto(&self) -> kuro_data::CommandExecutionMetadata {
        let metadata = self.dupe();
        kuro_data::CommandExecutionMetadata {
...
pub struct CommandExecutionResult {
    ...
    /// Whether this was uploaded to cache, by Kuro.
    pub did_cache_upload: bool,
    /// Whether dep file information for this action was uploaded to cache, by Kuro.
...
pub struct CommandExecutionReport {
    ...
    pub inline_environment_metadata: kuro_data::InlineCommandExecutionEnvironmentMetadata,
}
...
impl CommandExecutionReport {
    pub async fn to_command_execution_proto(
        ...
        let status = match &self.status {
            CommandExecutionStatus::Success { .. } => {
                kuro_data::command_execution::Success {}.into()
            }
            CommandExecutionStatus::Cancelled { .. } => {
                kuro_data::command_execution::Cancelled {}.into()
            }
            CommandExecutionStatus::Failure { .. } => {
                kuro_data::command_execution::Failure {}.into()
            }
...
mod tests {
    ...
    use kuro_common::cas_digest::CasDigest;
    use kuro_util::time_span::TimeSpan;
    ...
    fn make_simple_report() -> CommandExecutionReport {
        ...
        let status = CommandExecutionStatus::Success {
            execution_kind: CommandExecutionKind::Local {
                ...
                command: vec!["fake_kuro".to_owned()],
                env: {
        ...
        let timing = CommandExecutionMetadata {
            ...
            execution_stats: Some(kuro_data::CommandExecutionStats {
                cpu_instructions_user: Some(4),
                cpu_instructions_kernel: Some(5),
                userspace_events: Some(kuro_data::CpuCounter {
                    count: 4,
                    time_enabled: 100,
                    time_running: 100,
                }),
                kernel_events: Some(kuro_data::CpuCounter {
                    count: 10,
                    time_enabled: 50,
                    time_running: 100,
                }),
    ...
    fn make_simple_proto() -> kuro_data::CommandExecution {
        // The field values correspond to what `make_simple_report()` builds.
        let command_execution_kind = kuro_data::CommandExecutionKind {
            command: Some(kuro_data::command_execution_kind::Command::LocalCommand(
                kuro_data::LocalCommand {
                    argv: vec!["fake_kuro".to_owned()],
                    env: vec![kuro_data::EnvironmentEntry {
        ...
        let command_execution_stats = kuro_data::CommandExecutionStats {
            cpu_instructions_user: Some(4),
            cpu_instructions_kernel: Some(5),
            userspace_events: Some(kuro_data::CpuCounter {
                count: 4,
        ...
        let command_execution_metadata = kuro_data::CommandExecutionMetadata {
        ...
        let command_execution_details = kuro_data::CommandExecutionDetails {
        ...
        kuro_data::CommandExecution {
            ...
            status: Some(kuro_data::command_execution::Status::Success(
                kuro_data::command_execution::Success {},
            )),
            inline_environment_metadata: Some(
                kuro_data::InlineCommandExecutionEnvironmentMetadata {
                    sandcastle_instance_id: Some(123),
                },
            ),
        }
    ...
    async fn test_to_command_execution_proto_omit_command_details() {
        ...
        expected_proto
            ...
            .unwrap()
            .command = Some(
            kuro_data::command_execution_kind::Command::OmittedLocalCommand(
                kuro_data::OmittedLocalCommand {
                    action_digest: format!("{}:{}", "0".repeat(64), "123"),
                },
            ),
        );
