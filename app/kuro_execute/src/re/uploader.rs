...
use kuro_common::file_ops::metadata::FileDigestKind;
...
impl Uploader {
    async fn find_missing<'a>(
        ...
        let now = Utc::now();
        let ttl_wanted = if kuro_core::is_open_source() {
            1
        } else {
            600i64
        };
        ...
        let digests_and_ttls_iterator = if deduplicate_get_digests_ttl_calls {
            ...
            {
                type Item = kuro_error::Result<(&'a TrackedFileDigest, i64)>;

                fn next(&mut self) -> Option<kuro_error::Result<(&'a TrackedFileDigest, i64)>> {
    ...
    pub async fn upload(
                            ...
                            if file.digest.to_re() == digest {
                                if should_error_for_missing_digest(info) {
                                    ...
                                    return Err(kuro_error::kuro_error!(
                                        kuro_error::ErrorTag::ReCasArtifactExpired,
        ...
        if !upload_files.is_empty() || !upload_blobs.is_empty() {
            ...
            .await
            .map_err(|e| {
                if e.tags().contains(&kuro_error::ErrorTag::ReInvalidArgument) {
                    kuro_error::kuro_error!(
                        kuro_error::ErrorTag::ReInvalidArgument,
...
fn error_for_missing_file(
    ...
    kuro_error::kuro_error!(
        kuro_error::ErrorTag::ReInvalidGetCasResponse,
        ...
        `kuro killall`. We would appreciate a bug report. Debug information: {:#}",
        digest,
        cause,
    )
}
...
fn add_injected_missing_digests<'a>(
    ...
    fn convert_digests(val: &str) -> kuro_error::Result<Vec<FileDigest>> {
        val.split(' ')
            .map(|digest| {
                let digest = TDigest::from_str(digest)
                    .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::InvalidDigest))
...
struct GetDigestsTtlDeduper<'s> {
    ...
    queries: HashMap<
        RequestId,
        Shared<BoxFuture<'s, kuro_error::Result<HashMap<TrackedFileDigest, i64>>>>,
    >,
}
...
impl<'s> GetDigestsTtlDeduper<'s> {
    ...
    fn get_ttls<'a>(
        ...
        impl Future<Output = kuro_error::Result<HashMap<TrackedFileDigest, i64>>> + 's,
        usize,
        usize,
    ) {
...
) -> BoxFuture<'s, kuro_error::Result<HashMap<TrackedFileDigest, i64>>> {
...
fn process_get_digest_ttls_response<T>(
    ...
    digest_config: DigestConfig,
) -> kuro_error::Result<impl Iterator<Item = kuro_error::Result<(T, i64)>>>
where
    T: Borrow<TrackedFileDigest> + Ord,
{
    ...
    let mut digest_ttls = digest_ttls.into_try_map(|d| {
        kuro_error::Ok((
            FileDigest::from_re(&d.digest, digest_config).map_err(kuro_error::Error::from)?,
            d.ttl,
        ))
    })?;
    digest_ttls.sort();
