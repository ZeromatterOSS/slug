...
use kuro_miniperf_proto::MiniperfCounter;
...
pub struct ActionCacheResult(pub ActionResultResponse, pub kuro_data::CacheType);
...
impl RemoteActionResult for ActionCacheResult {
    ...
    fn execution_kind(&self, details: RemoteCommandExecutionDetails) -> CommandExecutionKind {
        match self.1 {
            kuro_data::CacheType::ActionCache => CommandExecutionKind::ActionCache { details },
...
) -> kuro_error::Result<kuro_data::CommandExecutionStats> {
    Ok({
        ...
        kuro_data::CommandExecutionStats {
...
) -> kuro_error::Result<Option<MiniperfCounter>> {
