...
use kuro_data::ReQueueAcquiringDependencies;
use kuro_data::ReQueueCancelled;
use kuro_data::ReQueueNoWorkerAvailable;
use kuro_data::ReQueueOverQuota;
...
#[cfg(fbcode_build)]
use kuro_re_configuration::CASdMode;
...
impl RemoteExecutionClient {
    pub async fn new(re_config: &RemoteExecutionConfig) -> kuro_error::Result<Self> {
        if kuro_env!("BUCK2_TEST_FAIL_CONNECT", bool, applicability = testing)? {
    ...
    pub async fn new_retry(re_config: &RemoteExecutionConfig) -> kuro_error::Result<Self> {
    ...
    ) -> kuro_error::Result<Option<ActionResultResponse>> {
        self.data
            .action_cache
    ...
    pub async fn execute<'a>(
        ...
        re_gang_workers: &[kuro_core::execution_types::executor_config::ReGangWorker],
        use_case: RemoteExecutorUseCase,
    ...
    ) -> kuro_error::Result<ExecuteResponseOrCancelled> {
        self.data
            .executes
    ...
    ) -> kuro_error::Result<GetDigestsTtlResponse> {
        self.data
            .get_digests_ttl
    ...
    pub async fn get_digest_expirations(
        ...
        digests: Vec<TDigest>,
        metadata: RemoteExecutionMetadata,
    ) -> kuro_error::Result<Vec<(TDigest, DateTime<Utc>)>> {
        let now = Utc::now();
    ...
    ) -> kuro_error::Result<WriteActionResultResponse> {
        self.data
    ...
    pub fn get_experiment_name(&self) -> kuro_error::Result<Option<String>> {
        self.data
            .client
            .client()
            .get_experiment_name()
            .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::ReExperimentName))
    }
...
fn anticipated_queue_duration(
    ...
    if let Some(duration) = kuro_env!(
        "BUCK2_TEST_RE_QUEUE_ESTIMATE_S",
        type=u64,
        applicability = testing
    )
    // Stringify the error because we can't deal with kuro_errors here
...
impl RemoteExecutionClientImpl {
    async fn new(re_config: &RemoteExecutionConfig) -> kuro_error::Result<Self> {
        let op_name = "REClientBuilder";
        tracing::info!("Creating a new RE client");

        let res: kuro_error::Result<Self> = try {
            let download_concurrency =
                kuro_env!("BUCK2_RE_DOWNLOAD_CONCURRENCY", type=usize, default=256)?;
            ...
            let client = {
                ...
                // kuro makes find_missing calls for the same blobs
                ...
                if let Some(external_casd_address) = &static_metadata.shared_casd_address {
                    use kuro_re_configuration::CASdAddress;
                    use kuro_re_configuration::CopyPolicy as KuroCopyPolicy;
                    use remote_execution::RemoteCASdAddress;
                    use remote_execution::RemoteCacheConfig;
                    use remote_execution::RemoteCacheSyncConfig;

                    let policies: kuro_error::Result<(RemoteFetchPolicy, RemoteFetchPolicy)> =
                    ...
                    let remote_cache_config = {
                        let mut remote_cache_config = RemoteCacheConfig {
                            ...
                            sync_copy_policy: match static_metadata
                                ...
                                .clone()
                                .unwrap_or(KuroCopyPolicy::Copy)
                            {
                                KuroCopyPolicy::Copy => CopyPolicy::FULL_COPY,
                                KuroCopyPolicy::Reflink => CopyPolicy::SOFT_COPY,
                                KuroCopyPolicy::Hybrid => CopyPolicy::HYBRID_COPY,
                            },
                            ..Default::default()
                        };
                    ...
                    .unwrap_or(
                        if static_metadata.use_zippy_rich_client && cfg!(target_os = "linux") {
                            if static_metadata.shared_casd_address.is_some() {
                                "remote_execution/features/client_kuro_pc"
                            } else {
                                "remote_execution/features/client_kuro"
                            }
                        } else {
                            "remote_execution/features/client_kuro_alternative"
                        },
                    )
                    .to_owned();
    ...
    async fn execute_impl(
        ...
        use kuro_data::ReAfterAction;
        use kuro_data::ReBeforeAction;
        use kuro_data::ReExecute;
        use kuro_data::ReQueue;
        use kuro_data::ReUnknown;
        use kuro_data::ReWorkerDownload;
        use kuro_data::ReWorkerUpload;
        use kuro_data::re_stage;
    ...
    pub async fn execute<'a>(
        ...
        if kuro_env!("BUCK2_TEST_FAIL_RE_EXECUTE", bool, applicability = testing)? {
        ...
        let metadata = RemoteExecutionMetadata {
            ...
            buck_info: Some(BuckInfo {
                version: kuro_build_info::revision()
                    .map(|s| s.to_owned())
                    .unwrap_or_default(),
    ...
    async fn download_typed_blobs<T: Message + Default>(
        ...
        digests: Vec<TDigest>,
        use_case: RemoteExecutorUseCase,
    ) -> kuro_error::Result<(Vec<T>, TLocalCacheStats)> {
        if digests.is_empty() {
        ...
        if blobs.len() != expected_blobs {
            return Err(kuro_error!(
                kuro_error::ErrorTag::CasBlobCountMismatch,
    ...
    pub async fn download_blob(
        ...
        digest: &TDigest,
        use_case: RemoteExecutorUseCase,
    ) -> kuro_error::Result<(Vec<u8>, TLocalCacheStats)> {
    ...
    async fn materialize_files(
        ...
        files: Vec<NamedDigestWithPermissions>,
        use_case: RemoteExecutorUseCase,
    ) -> kuro_error::Result<TLocalCacheStats> {
        if kuro_env!(
        ...
        let futs = chunks(files, self.download_chunk_size).map(|chunk| async move {
            ...
            kuro_error::Ok(response.local_cache_stats)
        });

        let stat = kuro_util::future::try_join_all(futs).await?.iter().fold(
