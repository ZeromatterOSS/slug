...
impl RemoteExecutionConfig {
    async fn connect_now(&self) -> kuro_error::Result<RemoteExecutionClient> {
...
struct LazyRemoteExecutionClient {
    client: AsyncOnceCell<kuro_error::Result<RemoteExecutionClient>>,
...
impl LazyRemoteExecutionClient {
    ...
    async fn get(&self) -> kuro_error::Result<&RemoteExecutionClient> {
    ...
    async fn init(&self) -> kuro_error::Result<RemoteExecutionClient> {
...
impl ReConnectionManager {
    ...
    pub fn get_network_stats(&self) -> kuro_error::Result<RemoteExecutionClientStats> {
...
impl ReGetSessionId for ReConnectionManager {
    async fn get_session_id(&self) -> kuro_error::Result<String> {
...
impl UnconfiguredRemoteExecutionClient {
    ...
    fn lock(&self) -> kuro_error::Result<Arc<Arc<LazyRemoteExecutionClient>>> {
        self.data
            .upgrade()
    ...
    pub async fn get_session_id(&self) -> kuro_error::Result<String> {
...
impl ManagedRemoteExecutionClient {
    ...
    pub async fn download_blob(&self, digest: &TDigest) -> kuro_error::Result<Vec<u8>> {
        self.lock()?
            .get()
            .await?
    ...
    pub async fn upload_blob(&self, blob: InlinedBlobWithDigest) -> kuro_error::Result<TDigest> {
        self.lock()?
            .get()
            .await?
