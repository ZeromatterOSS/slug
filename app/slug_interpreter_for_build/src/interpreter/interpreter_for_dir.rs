...
use slug_interpreter::factory::FinishedStarlarkEvaluation;
use slug_interpreter::factory::StarlarkEvaluatorProvider;
use slug_interpreter::file_loader::InterpreterFileLoader;
use slug_interpreter::file_loader::LoadResolver;
use slug_interpreter::file_loader::LoadedModules;
use slug_interpreter::file_type::StarlarkFileType;
use slug_interpreter::import_paths::ImplicitImportPaths;
...
use slug_node::nodes::eval_result::EvaluationResultWithStats;
use slug_node::super_package::SuperPackage;
use slug_util::per_thread_instruction_counter::PerThreadInstructionCounter;
...
pub type ParseResult = Result<ParseData, slug_error::Error>;
...
pub fn get_starlark_warning_link() -> &'static str {
    if slug_core::is_open_source() {
        "https://slug.build/docs/users/faq/starlark_peak_mem"
    } else {
...
pub(crate) struct InterpreterForDir {
    ...
    /// Autoload path for `@slug_builtins//:exports.bzl`. Public symbols
...
impl LoadResolver for InterpreterLoadResolver {
    fn resolve_load(
        ...
        // If you load the same .bzl file twice via different aliases (e.g. fbcode//slug/prelude/foo.bzl and prelude.bzl)
...
impl InterpreterForDir {
    fn verbose_gc() -> slug_error::Result<bool> {
    ...
    fn is_ignore_attrs_for_profiling() -> slug_error::Result<bool> {
    ...
    pub(crate) fn new(
        ...
        // Resolve `@slug_builtins//:exports.bzl` once per InterpreterForDir.
        // The cell is auto-registered by
        // `slug_common::legacy_configs::cells` for every bzlmod project.
        // Legacy non-bzlmod workspaces without `[external_cells]
        // slug_builtins = bundled` in `.buckconfig` skip the autoload.
        let bazel_builtins_autoload = bzlmod_mode
            .then(|| {
                cell_info
                    .cell_alias_resolver()
                    .resolve("slug_builtins")
                    .ok()
                    .and_then(|_| {
                        parse_import(
                            cell_info.cell_alias_resolver(),
    ...
    fn create_env(
    ...
    ) -> slug_error::Result<BuckStarlarkModule> {
        let _ = starlark_path;

        // Inject names from `@slug_builtins//:exports.bzl::exported_toplevels`
        // into the consuming env. Skipped inside the slug_builtins cell
                    ...
                    .get(&StarlarkModulePath::LoadFile(builtins_path))
                {
                    ...
                    if let Some(exports_value) = exports_value {
                        ...
                        let dict = DictRef::from_value(value).with_internal_error(|| {
                            format!(
                                "@slug_builtins exports.bzl `exported_toplevels` must be a \
                                 dict, got: {}",
                                value.get_type()
                            )
                        })?;
                        for (k, v) in dict.iter() {
                            let name = k.unpack_str().with_internal_error(|| {
                                format!(
                                    "@slug_builtins exports.bzl `exported_toplevels` keys must \
                                     be strings, got: {}",
                                    k.get_type()
                                )
                            })?;
                    ...
                    if let StarlarkPath::BuildFile(_) = starlark_path {
                        ...
                        if let Some(exported_native_value) = exported_native_value {
                            ...
                            let dict = DictRef::from_value(value).with_internal_error(|| {
                                format!(
                                    "@slug_builtins exports.bzl `exported_native` must be a \
                                     dict, got: {}",
                                    value.get_type()
                                )
                            })?;
                            for (k, v) in dict.iter() {
                                let name = k.unpack_str().with_internal_error(|| {
                                    format!(
                                        "@slug_builtins exports.bzl `exported_native` keys must \
                                         be strings, got: {}",
                                        k.get_type()
                                    )
                                })?;
        ...
        .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::Interpreter))?;
    ...
    fn create_build_env(
        ...
        loaded_modules: &LoadedModules,
    ) -> slug_error::Result<(BuckStarlarkModule, ModuleInternals)> {
    ...
    pub(crate) fn parse(
        ...
        let ast = match AstModule::parse(
            ...
            Ok(ast) => ast,
            Err(e) => {
                return Ok(Err(slug_error::Error::from(e).context(format!(
                    "Error parsing: `{}`",
                    OwnedStarlarkPath::new(import)
                ))));
            }
        ...
        // Autoload @slug_builtins for BUILD and .bzl paths, except inside
        // the slug_builtins cell itself (a bundled .bzl can't load itself).
    ...
    ) -> slug_error::Result<(FinishedStarlarkEvaluation, EvalResult)> {
