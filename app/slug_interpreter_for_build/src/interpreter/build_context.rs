...
impl PerFileTypeContext {
    ...
    fn base_path(&self) -> slug_error::Result<CellPath> {
        match self {
    ...
    pub(crate) fn require_build(
        &self,
        function_name: &str,
    ) -> slug_error::Result<&ModuleInternals> {
        match self {
    ...
    pub(crate) fn into_build(self) -> slug_error::Result<ModuleInternals> {
        match self {
            PerFileTypeContext::Build(internals) => Ok(internals),
    ...
    pub(crate) fn require_package_file(
        &self,
        function_name: &str,
    ) -> slug_error::Result<&PackageFileEvalCtx> {
        match self {
    ...
    pub(crate) fn into_package_file(self) -> slug_error::Result<PackageFileEvalCtx> {
        match self {
            PerFileTypeContext::Package(ctx) => Ok(ctx),
...
impl<'a> BuildContext<'a> {
    ...
    ) -> slug_error::Result<&'a1 BuildContext<'a>> {
    ...
    pub fn require_package(&self) -> slug_error::Result<PackageLabel> {
    ...
    pub(crate) fn base_path(&self) -> slug_error::Result<CellPath> {
