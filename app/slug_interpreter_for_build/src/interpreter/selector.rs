...
impl<'v> StarlarkSelector<'v> {
    ...
    pub fn from_concat<I>(iter: I, heap: Heap<'v>) -> slug_error::Result<Value<'v>>
    where
        I: IntoIterator<Item = Value<'v>>,
    {
        fn values_to_selector<'v, I>(
            ...
            values: &mut I,
            heap: Heap<'v>,
        ) -> slug_error::Result<NoneOr<StarlarkSelector<'v>>>
        where
    ...
    fn select_map<'a, const RECURSE: bool>(
        ...
        slug_util::threads::check_stack_overflow()?;
...
/// The `Select` type represents a conditional attribute value in Slug.
///
...
/// # Resolution Timing
///
/// `Select` objects are resolved during Slug's **configuration phase**, which happens after
...
/// # Platform Selection
///
/// Slug determines the target platform through:
///
...
/// Example:
/// ```bash
/// slug build //app:main --target-platforms //platforms:linux-x86_64
/// ```
///
/// # Target Compatibility
///
/// Slug provides two attributes for platform compatibility:
///
...
/// - Slug docs: "Configurations By Example"
#[starlark_module]
