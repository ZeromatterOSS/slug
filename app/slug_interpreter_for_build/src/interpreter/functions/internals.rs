...
pub(crate) fn register_internals(builder: &mut GlobalsBuilder) {
    /// `fail()` but implemented using a slug error type instead of starlark's, for testing
    /// purposes.
    fn slug_fail<'v>(msg: &str, _eval: &mut Evaluator<'v, '_, '_>) -> starlark::Result<NoneType> {
        Err(slug_error::Error::from(BuckFail(msg.to_owned())).into())
    }
