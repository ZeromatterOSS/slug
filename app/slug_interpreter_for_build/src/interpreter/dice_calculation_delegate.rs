...
use slug_common::legacy_configs::dice::OpaqueLegacyBuckConfigOnDice;
...
use slug_interpreter::import_paths::HasImportPaths;
...
pub trait HasCalculationDelegate<'c, 'd> {
    ...
    ) -> slug_error::Result<DiceCalculationDelegate<'c, 'd>>;
}
...
impl<'c, 'd> HasCalculationDelegate<'c, 'd> for DiceComputations<'d> {
    async fn get_interpreter_calculator(
        ...
        impl Key for InterpreterConfigForDirKey {
            type Value = slug_error::Result<Arc<InterpreterForDir>>;
            async fn compute(
                &self,
...
impl<'c, 'd: 'c> DiceCalculationDelegate<'c, 'd> {
    async fn get_legacy_buck_config_for_starlark(
        &mut self,
    ) -> slug_error::Result<OpaqueLegacyBuckConfigOnDice> {
        self.ctx
    ...
    ) -> slug_error::Result<(AstModule, ModuleDeps)> {
    ...
    pub async fn prepare_package_file_eval(
        ...
        impl Key for PackageFileLookupKey {
            type Value = slug_error::Result<Option<Arc<PackageFilePath>>>;
    ...
    pub(crate) async fn eval_package_file(
        ...
        impl Key for PackageFileKey {
            type Value = slug_error::Result<SuperPackage>;
    ...
    pub async fn eval_build_file(
        ...
        let eval_result: slug_error::Result<_> = try {
            let ((), listing) = self
                .ctx
                .try_compute2(
            ...
            let start_event = slug_data::LoadBuildFileStart {
                cell: cell_str.clone(),
