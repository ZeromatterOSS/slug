...
use slug_interpreter::downstream_crate_starlark_defs::REGISTER_BUCK2_ACTION_IMPL_GLOBALS;
use slug_interpreter::downstream_crate_starlark_defs::REGISTER_BUCK2_ANON_TARGETS_GLOBALS;
use slug_interpreter::downstream_crate_starlark_defs::REGISTER_BUCK2_BUILD_API_GLOBALS;
use slug_interpreter::downstream_crate_starlark_defs::REGISTER_BUCK2_BUILD_API_INTERNALS;
use slug_interpreter::downstream_crate_starlark_defs::REGISTER_BUCK2_BXL_GLOBALS;
use slug_interpreter::downstream_crate_starlark_defs::REGISTER_BUCK2_CFG_CONSTRUCTOR_GLOBALS;
use slug_interpreter::downstream_crate_starlark_defs::REGISTER_BUCK2_TRANSITION_GLOBALS;
use slug_interpreter::starlark_promise::register_promise;
use slug_interpreter::types::cell_path::register_cell_path;
use slug_interpreter::types::cell_root::register_cell_root;
use slug_interpreter::types::configured_providers_label::register_providers_label;
use slug_interpreter::types::package_path::register_package_path;
use slug_interpreter::types::project_root::register_project_root;
use slug_interpreter::types::regex::register_buck_regex;
use slug_interpreter::types::target_label::register_target_label;
...
use crate::interpreter::functions::slug_runtime::register_slug_runtime;
...
// and `starlark_library_extensions_for_slug` are all the same, since all symbols are available
...
pub fn register_analysis_natives(builder: &mut GlobalsBuilder) {
    ...
    // slug-internal hooks consumed by the bundled
    // `@slug_builtins//:exports.bzl`. Each name is `slug_*`-prefixed.
    register_slug_runtime(builder);
}
...
pub fn starlark_library_extensions_for_slug() -> &'static [LibraryExtension] {
    &[
...
fn register_all_natives(builder: &mut GlobalsBuilder) {
    ...
    for ext in starlark_library_extensions_for_slug() {
        ext.add(builder);
    }
}
