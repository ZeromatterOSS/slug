...
use slug_build_api::interpreter::rule_defs::depset::bazel_depset_tset_definition;
use slug_build_api::interpreter::rule_defs::depset::depset_direct_and_transitive;
use slug_build_api::interpreter::rule_defs::depset::is_depset_value;
use slug_build_api::interpreter::rule_defs::depset::make_depset_from_lists;
use slug_build_api::interpreter::rule_defs::py_common::AnalysisTestResultInfoProvider;
use slug_build_api::interpreter::rule_defs::transitive_set::FrozenTransitiveSet;
use slug_build_api::interpreter::rule_defs::transitive_set::FrozenTransitiveSetDefinition;
use slug_build_api::interpreter::rule_defs::transitive_set::TransitiveSet;
use slug_build_api::interpreter::rule_defs::transitive_set::TransitiveSetLike;
use slug_build_api::interpreter::rule_defs::transitive_set::TransitiveSetOrdering;
...
fn depset_to_transitive_set<'v>(
    ...
    // sees it as one node value, so this remains a lossy Slug-specific bridge
...
fn canonical_repo_name_for_label_context_with_alias_resolver(
            ...
            alias_resolver.resolve_declared_or_runtime_alias(apparent_repo_name)
    {
        ...
        if is_root_repo_for_label_context(cell_name, root_cell_name) {
            return String::new();
        }
...
fn lexical_current_repo_name_for_label_context(
    file_cell: &str,
    alias_resolver: Option<&slug_core::cells::CellAliasResolver>,
    root_cell_name: Option<&str>,
) -> String {
    if file_cell.is_empty() || is_root_repo_for_label_context(file_cell, root_cell_name) {
        return String::new();
    }

    if let Some(alias_resolver) = alias_resolver
        && let Some(cell_name) = alias_resolver.resolve_declared_or_runtime_alias(file_cell)
    {
        if is_root_repo_for_label_context(cell_name.as_str(), root_cell_name) {
            return String::new();
        }
        return cell_name.as_str().to_owned();
    }
...
pub(crate) fn register_bzl_module_globals(globals: &mut GlobalsBuilder) {
    ...
    fn Label<'v>(
        ...
        let build_ctx = BuildContext::from_context(eval).ok();
        let alias_resolver =
            build_ctx.map(|build_ctx| build_ctx.cell_info().cell_alias_resolver().dupe());
        let root_cell_name =
            build_ctx.map(|build_ctx| build_ctx.cell_resolver().root_cell().as_str().to_owned());
        let current_repo = lexical_current_repo_name_for_label_context(
            &file_cell,
            alias_resolver.as_ref(),
            root_cell_name.as_deref(),
        );
        ...
        let resolved = slug_bzlmod::canonicalize_label_with_package_context_and_repo_resolver(
            ...
            None,
            |apparent_repo| {
                Some(slug_bzlmod::CanonicalRepoName::new(
    ...
    /// ```
    ///
    /// In Slug, this is currently a no-op stub that accepts the argument but
    ...
    fn visibility<'v>(#[starlark(require = pos)] _value: Value<'v>) -> starlark::Result<NoneType> {
        ...
        // Currently a no-op - Slug doesn't enforce .bzl file visibility yet.
        // The value can be:
...
pub(crate) fn register_module_natives(globals: &mut GlobalsBuilder) {
    ...
    fn licenses<'v>(
        ...
        // Currently a no-op - Slug doesn't track license metadata.
        Ok(NoneType)
    }
...
fn bazel_native_module(registry: &mut GlobalsBuilder) {
    ...
    fn glob<'v>(
        ...
        // package's listing. The slug `PackageFileListing` only stores
    ...
    ///
    /// Slug-specific bridge. This is not part of Bazel's public `native`
    ...
    ///
    /// This is a lossy Slug-specific bridge: projections, reductions,
    ...
    fn repository_name(eval: &mut Evaluator) -> starlark::Result<String> {
        ...
        let cell_name_str = cell_name.as_str();
        let is_root = build_ctx.cell_resolver().root_cell() == cell_name;
        if is_root {
            Ok("@".to_owned())
        } else {
            Ok(format!("@{}", cell_name_str))
        }
    }
...
/// Slug's reported Bazel version for compatibility with modern rules.
///
...
/// slug implements correctly with absolute paths) instead of the
/// `ln -sf relative_file(...)` fallback (which depends on a fully-loaded
/// `bazel_lib` and produces broken `var/...`-prefixed targets in slug).
///
...
mod tests {
    ...
    fn label_context_current_repo_keeps_empty_version_module_suffix() {
        assert_eq!(
            lexical_current_repo_name_for_label_context("rules_rust+", None, None),
            "rules_rust+"
        );
    }
    ...
    fn label_context_current_repo_does_not_alias_canonical_module_repo() {
        ...
        assert_eq!(
            lexical_current_repo_name_for_label_context("label_ctx_module+", None, None),
            "label_ctx_module+"
        );
    }

    #[test]
    fn label_context_current_repo_prefers_runtime_aliases_before_globals() -> slug_error::Result<()>
    {
        let apparent = "label_ctx_current_runtime_alias";
        ...
        assert_eq!(
            lexical_current_repo_name_for_label_context(apparent, Some(&resolver), None),
            canonical
        );
        assert_eq!(
    ...
    #[test]
    fn label_context_current_repo_uses_explicit_root_before_global_root() -> slug_error::Result<()>
    {
        let apparent = "root";
        let canonical = "label_ctx_current_owner++ext+root";
        let snapshot = BzlmodRuntimeCellInstallSnapshot {
            root_module_name: None,
            extension_cells: Vec::new(),
            scoped_aliases: Vec::new(),
        ...
        assert_eq!(
            lexical_current_repo_name_for_label_context(
                apparent,
                Some(&resolver),
                Some("workspace")
            ),
    ...
    #[test]
    fn label_context_current_repo_no_snapshot_miss_ignores_global_alias() -> slug_error::Result<()>
    {
        ...
        assert_eq!(
            lexical_current_repo_name_for_label_context(apparent, None, None),
            apparent
        );
        assert_eq!(
            slug_core::cells::resolve_test_dynamic_extension_cell_alias(apparent).as_deref(),
    ...
    #[test]
    fn label_context_explicit_repo_prefers_runtime_aliases_before_globals() -> slug_error::Result<()>
    {
        let apparent = "label_ctx_runtime_alias";
    ...
    #[test]
    fn label_context_explicit_repo_runtime_miss_ignores_global_alias() -> slug_error::Result<()> {
        ...
        assert_eq!(
            canonical_repo_name_for_label_context_with_alias_resolver(apparent, None, None),
            apparent
        );
        Ok(())
    }
...
pub(crate) fn register_bazel_native(globals: &mut GlobalsBuilder) {
    globals.namespace("native", |registry| {
        ...
        registry.set("bazel_version", SLUG_BAZEL_VERSION);
    });
}
