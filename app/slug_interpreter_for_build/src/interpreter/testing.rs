...
use slug_common::legacy_configs::configs::testing::parse_with_config_args;
...
use slug_node::nodes::targets_map::TargetsMap;
...
/// ignore the targets
pub fn run_simple_starlark_test(content: &str) -> slug_error::Result<()> {
...
pub fn cells(extra_root_config: Option<&str>) -> slug_error::Result<CellsData> {
...
pub fn expect_error<T>(result: slug_error::Result<T>, content: &str, expected: &str) {
    match result {
        Ok(_) => {
...
impl Tester {
    ...
    pub fn with_cells(cells_data: CellsData) -> slug_error::Result<Self> {
        let (
            cell_alias_resolver,
            cell_resolver,
            root_config,
    ...
    fn interpreter(&self) -> slug_error::Result<Arc<InterpreterForDir>> {
    ...
    ) -> slug_error::Result<EvaluationResult> {
    ...
    pub fn run_starlark_test(&mut self, content: &str) -> slug_error::Result<TargetsMap> {
    ...
    /// environment.
    pub fn run_starlark_bzl_test(&mut self, content: &str) -> slug_error::Result<()> {
        let import_path = ImportPath::testing_new("root//some/package:defs.bzl");
        let template = indoc!(
            r#"
