...
use slug_node::attrs::coerced_deps_collector::CoercedDeps;
use slug_node::attrs::coerced_deps_collector::CoercedDepsCollector;
use slug_node::attrs::coerced_path::CoercedPath;
...
use slug_node::attrs::spec::internal::NAME_ATTRIBUTE;
use slug_node::attrs::spec::internal::VISIBILITY_ATTRIBUTE;
use slug_node::attrs::values::AttrValues;
use slug_node::nodes::unconfigured::RuleKind;
use slug_node::nodes::unconfigured::TargetNode;
use slug_node::package::Package;
use slug_node::provider_id_set::ProviderIdSet;
use slug_node::rule::Rule;
...
pub(crate) mod rule_defs {
    ...
    fn config_setting_attributes() -> AttributeSpec {
        use slug_node::attrs::attr_type::dict::DictLiteral;
        use slug_node::attrs::attr_type::list::ListLiteral;
        use slug_util::arc_str::ArcSlice;
    ...
    /// `app/slug_analysis/src/analysis/native_rule_analysis.rs` is the
    ...
    fn make_removed_rule(kind: slug_node::rule_type::RemovedNativeRule) -> Arc<Rule> {
        make_native_rule(
    ...
    pub static REMOVED_ENVIRONMENT_GROUP_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_removed_rule(slug_node::rule_type::RemovedNativeRule::EnvironmentGroup));

    pub static REMOVED_EXECUTION_PLATFORM_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_removed_rule(slug_node::rule_type::RemovedNativeRule::ExecutionPlatform));

    pub static REMOVED_EXECUTION_PLATFORMS_RULE: Lazy<Arc<Rule>> = Lazy::new(|| {
        make_removed_rule(slug_node::rule_type::RemovedNativeRule::ExecutionPlatforms)
    });

    pub static REMOVED_SH_BINARY_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_removed_rule(slug_node::rule_type::RemovedNativeRule::ShBinary));

    pub static REMOVED_SH_TEST_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_removed_rule(slug_node::rule_type::RemovedNativeRule::ShTest));

    pub static REMOVED_SH_LIBRARY_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_removed_rule(slug_node::rule_type::RemovedNativeRule::ShLibrary));

    pub static REMOVED_CC_LIBRARY_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_removed_rule(slug_node::rule_type::RemovedNativeRule::CcLibrary));

    pub static REMOVED_CC_BINARY_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_removed_rule(slug_node::rule_type::RemovedNativeRule::CcBinary));

    pub static REMOVED_CC_TEST_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_removed_rule(slug_node::rule_type::RemovedNativeRule::CcTest));

    pub static REMOVED_CC_IMPORT_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_removed_rule(slug_node::rule_type::RemovedNativeRule::CcImport));

    pub static REMOVED_CC_SHARED_LIBRARY_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_removed_rule(slug_node::rule_type::RemovedNativeRule::CcSharedLibrary));

    pub static REMOVED_CC_TOOLCHAIN_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_removed_rule(slug_node::rule_type::RemovedNativeRule::CcToolchain));

    pub static REMOVED_CC_TOOLCHAIN_SUITE_RULE: Lazy<Arc<Rule>> =
        Lazy::new(|| make_removed_rule(slug_node::rule_type::RemovedNativeRule::CcToolchainSuite));
...
pub fn register_native_rules(globals: &mut GlobalsBuilder) {
    ...
    fn config_setting<'v>(
        ...
        let coerced_list = CoercedAttr::List(ListLiteral(slug_util::arc_str::ArcSlice::from_iter(
            coerced_cvs,
        )));
    ...
    fn package_group<'v>(
        ...
        slug_node::visibility::register_package_group(&group_label, packages.items, includes.items);
    ...
    /// execution would require integrating with the Slug query engine at build time.
    ///
...
///
/// Called from `slug_interpreter_for_build::init_late_bindings()`.
/// This bridges the circular dependency: `slug_build_api` defines `ANALYSIS_TEST_REGISTER`
/// but can't depend on `slug_interpreter_for_build` (which has `ModuleInternals`).
/// We initialize the binding here instead.
pub fn init_analysis_test_register() {
    use slug_build_api::interpreter::rule_defs::cc_common::ANALYSIS_TEST_REGISTER;
