...
use slug_node::nodes::targets_map::TargetsMapRecordError;
use slug_node::nodes::unconfigured::TargetNode;
use slug_node::oncall::Oncall;
...
impl ModuleInternals {
    ...
    pub fn record(&self, target_node: TargetNode) -> slug_error::Result<()> {
    ...
    pub(crate) fn set_oncall(&self, name: &str) -> slug_error::Result<()> {
...
fn register_implicit_source_file_targets(
    ...
    for file_path in package_listing.files().files() {
        ...
        // Skip if target name is not valid for Slug
