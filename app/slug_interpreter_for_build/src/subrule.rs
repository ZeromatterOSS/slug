...
// `slug_build_api::interpreter::rule_ctx_storage` so that provider constructors
// (e.g. DefaultInfo) can also reach it without a dependency on this crate.
pub use slug_build_api::interpreter::rule_ctx_storage::clear_current_rule_ctx;
pub use slug_build_api::interpreter::rule_ctx_storage::get_current_rule_ctx;
pub use slug_build_api::interpreter::rule_ctx_storage::get_current_subrule_wrapper;
pub use slug_build_api::interpreter::rule_ctx_storage::set_current_rule_ctx_raw;
...
impl<'v> StarlarkSubruleCallable<'v> {
    ...
    ) -> slug_error::Result<StarlarkSubruleCallable<'v>> {
...
impl<'v> StarlarkValue<'v> for StarlarkSubruleCallable<'v> {
    ...
    fn invoke(
        ...
        Err(slug_error::Error::from(SubruleError::SubruleCalledBeforeFreezing).into())
    }
...
impl<'v> StarlarkValue<'v> for FrozenStarlarkSubruleCallable {
    ...
    fn invoke(
        ...
        if let Some(ctx) = get_current_rule_ctx() {
            ...
            //
            // When @slug_builtins exposes a `subrule_implementation_wrapper`,
