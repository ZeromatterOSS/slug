...
//! handled separately in `slug_bzlmod`.
//!
//! ## What These Stubs Do
//!
...
impl<'v> StarlarkModuleExtension<'v> {
    ...
    ) -> slug_error::Result<StarlarkModuleExtension<'v>> {
...
impl<'v> StarlarkValue<'v> for StarlarkModuleExtension<'v> {
    ...
    fn invoke(
        ...
        Err(slug_error::Error::from(ModuleExtensionError::ExtensionCannotBeInvokedDirectly).into())
    }
    ...
    fn tag_class<'v>(
        ...
        if let Some(build_context) = eval.extra.and_then(|e| e.downcast_ref::<BuildContext>()) {
            match &build_context.additional {
                ...
                _ => {
                    return Err(
                        slug_error::Error::from(ModuleExtensionError::TagClassNotInBzl).into(),
                    );
                }
            }
        }
