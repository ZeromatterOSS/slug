...
use slug_node::metadata::super_package_values::SuperPackageValues;
...
impl SuperPackageValuesImpl {
    pub fn get(values: &dyn SuperPackageValues) -> slug_error::Result<&SuperPackageValuesImpl> {
        values
            .as_any()
    ...
    ) -> slug_error::Result<Arc<dyn SuperPackageValues>> {
...
impl SuperPackageValues for SuperPackageValuesImpl {
    ...
    fn package_values_json(&self) -> slug_error::Result<SmallMap<MetadataKey, serde_json::Value>> {
...
impl<'v> StarlarkPackageValue<'v> {
    pub(crate) fn new(value: Value<'v>) -> slug_error::Result<StarlarkPackageValue<'v>> {
...
impl OwnedFrozenStarlarkPackageValue {
    ...
    pub(crate) fn to_json_value(&self) -> slug_error::Result<serde_json::Value> {
        self.0
            .value()
            .to_json_value()
...
pub(crate) fn write_package_value_impl<'v>(
    ...
    let key = MetadataKeyRef::new(key).map_err(slug_error::Error::from)?;
    ...
    if package_file_extra.package_values.borrow().contains_key(key) {
        return Err(
            slug_error::Error::from(PackageValueError::KeyAlreadySetInThisFile(key.to_owned()))
                .into(),
        );
    }
    ...
    if !overwrite {
        if package_ctx.parent.package_values().contains_key(key) {
            return Err(
                slug_error::Error::from(PackageValueError::KeySetInParentFile(key.to_owned()))
                    .into(),
            );
        }
    }
