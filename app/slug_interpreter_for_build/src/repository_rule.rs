...
fn canonicalize_repo_attr_label(label: &str, context: Option<&RepoLabelContext>) -> String {
    let (repo, package) = context
        ...
        .unwrap_or(("", ""));
    slug_bzlmod::canonicalize_label_with_package_context(label, repo, package, None)
...
/// in slug, we need to extract them from the attr definition.
...
impl<'v> StarlarkRepositoryRule<'v> {
    ...
    ) -> slug_error::Result<StarlarkRepositoryRule<'v>> {
...
impl<'v> StarlarkValue<'v> for StarlarkRepositoryRule<'v> {
    ...
    fn invoke(
        ...
        Err(slug_error::Error::from(RepositoryRuleError::RuleCalledBeforeFreezing).into())
    }
...
impl<'v> StarlarkValue<'v> for FrozenStarlarkRepositoryRule {
    ...
    fn invoke(
        ...
        let name = kwargs
            ...
            .ok_or_else(|| slug_error::Error::from(RepositoryRuleError::NameAttributeRequired))?;
