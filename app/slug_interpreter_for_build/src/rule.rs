...
use slug_interpreter::late_binding_ty::ProviderReprLate;
...
use slug_interpreter::types::rule::FROZEN_RULE_GET_OUTPUTS;
use slug_interpreter::types::transition::transition_id_from_value;
use slug_node::attrs::attr::Attribute;
use slug_node::attrs::attr_type::AttrType;
use slug_node::attrs::attr_type::any::AnyAttrType;
...
use slug_node::rule::ExecGroupDef;
...
pub struct StarlarkRuleCallable<'v> {
    ...
    /// Names of attributes whose defaults were supplied by Slug's Bazel attr
...
enum RuleError {
    ...
    #[error(
        "Cannot specify both `impl` and `implementation` in rule(). Use only `implementation` (Bazel-style) or `impl` (Slug-style)"
    )]
    BothImplAndImplementation,
    #[error(
        "Missing `implementation` function in rule(). Specify either `implementation = _impl` (Bazel-style) or `impl = _impl` (Slug-style)"
    )]
...
impl<'v> StarlarkRuleCallable<'v> {
    fn new(
    ...
    ) -> slug_error::Result<StarlarkRuleCallable<'v>> {
        ...
        let sorted_validated_attrs = attrs
            ...
            .collect::<slug_error::Result<Vec<(String, Attribute)>>>()?;
                ...
                attrs.push((
                    ...
                    Attribute::new(
                        Some(Arc::new(CoercedAttr::Dict(
                            slug_node::attrs::attr_type::dict::DictLiteral(
                                slug_util::arc_str::ArcSlice::new([]),
                            ),
                        ))),
        ...
        // (rather than opaque `any`) lets slug resolve the labels so their
...
impl<'v> StarlarkValue<'v> for StarlarkRuleCallable<'v> {
    ...
    fn invoke(
        ...
        Err(slug_error::Error::from(RuleError::RuleCalledBeforeFreezing).into())
    }
...
fn unpack_frozen_rule(
    rule: FrozenValue,
) -> slug_error::Result<FrozenRef<'static, FrozenStarlarkRuleCallable>> {
...
fn parse_exec_groups<'v>(
    ...
    heap: Heap<'v>,
) -> slug_error::Result<Vec<(String, ExecGroupDef)>> {
...
fn parse_provides<'v>(provides: &UnpackListOrTuple<Value<'v>>) -> slug_error::Result<Vec<String>> {
...
fn parse_toolchains<'v>(
    ...
    heap: Heap<'v>,
) -> slug_error::Result<Vec<(String, bool)>> {
...
fn parse_rule_outputs<'v>(outputs: Option<Value<'v>>) -> slug_error::Result<Vec<(String, String)>> {
...
pub fn register_rule_function(builder: &mut GlobalsBuilder) {
    /// Define a rule. Supports both Bazel-style (`implementation`) and Slug-style (`impl`)
    ...
    fn rule<'v>(
            ...
            if build_setting_allows_multiple {
                let am_attr = StarlarkAttribute::new(Attribute::new(
                    Some(Arc::new(CoercedAttr::Bool(
                        slug_node::attrs::attr_type::bool::BoolLiteral(true),
                    ))),
        ...
        // Support positional arg, `implementation` (Bazel), and `impl` (Slug) parameter names
        ...
        let impl_fn = match (positional_impl, implementation, r#impl) {
            ...
            (None, None, None) => {
                return Err(slug_error::Error::from(RuleError::MissingImplementation).into());
            }
            _ => {
                return Err(slug_error::Error::from(RuleError::BothImplAndImplementation).into());
            }
        };
