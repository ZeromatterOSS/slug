...
use slug_core::pattern::pattern_type::PatternType;
...
use slug_node::attrs::coerced_path::CoercedDirectory;
...
use slug_query::query::syntax::simple::functions::QueryLiteralVisitor;
...
impl BuildAttrCoercionContext {
    ...
    pub fn parse_pattern<P: PatternType>(
        &self,
        value: &str,
    ) -> slug_error::Result<ParsedPattern<P>> {
    ...
    fn coerce_label_no_cache(&self, value: &str) -> slug_error::Result<ProvidersLabel> {
        ...
        match self.parse_pattern_bazel_compat::<ProvidersPatternExtra>(value) {
            Ok(result) => match result {
                ...
                ParsedPattern::Package(package) => {
                    return Err(BuildAttrCoercionContextError::RequiredLabel(format!(
                        "{}:",
    ...
    fn require_enclosing_package(
        &self,
        msg: &str,
    ) -> slug_error::Result<&(PackageLabel, PackageListing)> {
...
impl AttrCoercionContext for BuildAttrCoercionContext {
    fn coerce_providers_label(&self, value: &str) -> slug_error::Result<ProvidersLabel> {
    ...
    fn output_file_target(
        &self,
        value: &str,
    ) -> Option<slug_core::provider::label::ProvidersLabel> {
        let producer = self.output_file_registry.borrow().get(value).cloned();
        let producer = producer?;
        let (pkg_label, _) = self.enclosing_package.as_ref()?;
        let target_name = slug_core::target::name::TargetNameRef::new(&producer).ok()?;
        Some(slug_core::provider::label::ProvidersLabel::default_for(
            slug_core::target::label::label::TargetLabel::new(pkg_label.dupe(), target_name),
        ))
    }

    fn coerce_path(&self, value: &str, allow_directory: bool) -> slug_error::Result<CoercedPath> {
    ...
    fn coerce_target_pattern(
        &self,
        pattern: &str,
    ) -> slug_error::Result<ParsedPattern<TargetPatternExtra>> {
