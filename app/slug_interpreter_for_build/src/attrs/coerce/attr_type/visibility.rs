...
use slug_node::attrs::attr_type::visibility::VisibilityAttrType;
...
pub(crate) fn parse_visibility_from_strs(
    ctx: &dyn AttrCoercionContext,
    visibility: &[String],
) -> slug_error::Result<VisibilityWithinViewBuilder> {
...
pub(crate) fn parse_visibility_with_view(
    ...
    for item in list {
        ...
        if item == VisibilityPattern::PUBLIC || item == BAZEL_VISIBILITY_PUBLIC {
            ...
            // to the format expected by Slug's pattern matcher.
            let normalized_item = if item.ends_with(":__pkg__") {
