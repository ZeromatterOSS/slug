...
fn warn_attrs_deprecated() {
    ...
    WARNED.get_or_init(|| {
        tracing::warn!(
            "The `attrs.*` namespace is Buck2-specific and deprecated in Slug. \
...
/// a value from the command-line configuration. Slug doesn't have these flags, so we
...
/// attribute pre-analysis in `slug_analysis::analysis::aspect_calculation`.
...
pub(crate) trait AttributeExt {
    ...
    fn attr<'v>(
        ...
        doc: &str,
        coercer: AttrType,
    ) -> slug_error::Result<StarlarkAttribute>;
}
            ...
            Some(x) => {
                ...
                if value_type == "configuration_field" {
                    ...
                    match resolved_label {
                            ...
                            let coerce_ctx = match attr_coercion_context_for_bzl(eval) {
                                ...
                                Err(_) => {
                                    ...
                                    return Ok(StarlarkAttribute::new(Attribute::new(
                                        Some(Arc::new(
                                            slug_node::attrs::coerced_attr::CoercedAttr::None,
                                        )),
                                        doc,
                                        coercer,
                                    )));
                        ...
                        None => Some(Arc::new(slug_node::attrs::coerced_attr::CoercedAttr::None)),
                    }
                    ...
                    Some(Arc::new(slug_node::attrs::coerced_attr::CoercedAttr::None))
                } else {
...
) -> slug_error::Result<BuildAttrCoercionContext> {
...
fn dep_like_attr_handle_providers_arg(providers: Vec<Value>) -> slug_error::Result<ProviderIdSet> {
...
fn parse_bazel_dep_cfg(cfg: Option<Value<'_>>) -> slug_error::Result<BazelDepCfg> {
...
fn attr_module(registry: &mut GlobalsBuilder) {
    ...
    fn option<'v>(
        ...
        match attr.default() {
            Some(default) if !default.may_return_none() => Err(slug_error::Error::from(
    ...
    fn default_only<'v>(
        ...
        let Some(default) = inner.default().duped() else {
            return Err(slug_error::Error::from(AttrError::DefaultOnlyMustHaveDefault).into());
        };
...
///
/// This module provides Bazel 9.0-style attribute functions that map to Slug's internal
...
fn bazel_attr_module(registry: &mut GlobalsBuilder) {
    ...
    /// In Slug, this is handled via ctx.actions.declare_file() during analysis.
