...
// This file is the entry point for the target-completing delegate for slug
...
/// using:
/// - `source <(slug completion bash)`
/// - `source <(slug completion zsh)`
/// - `source (slug completion fish | psub)`
    ...
    for line in wrapper_iter.by_ref() {
        match line {
            ...
            COMPLETION_INSERTION_POINT => {
                ...
                slug_client_ctx::println!("{}", option_completions(shell, cmd)?)?;
            }
            s => {
                slug_client_ctx::println!("{}", s)?;
            }
        }
    }
