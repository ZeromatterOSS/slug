...
/// and cannot rely on slug's own parsing logic which (generally) requires
/// its targets to be well-formed.
///
/// Its goal is to create a label that slug commands will accept. These may,
...
/// 3. When corrections are necessary to create a label that slug will accept,
...
impl CompleteCommand {
    pub fn exec(
        ...
        let lockfile = slug_env!("COMPLETION_VERIFY_LOCKFILE", applicability = testing)?
            .map(AbsPath::new)
            .transpose()?;
