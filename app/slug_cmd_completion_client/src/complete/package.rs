...
impl<'a> PackageCompleter<'a> {
    ...
    async fn complete_partial_cells(&mut self, given_path: &str) -> slug_error::Result<()> {
...
mod tests {
    ...
    async fn test_completes_subdirs_as_well_as_target_colon_for_fully_qualified_cell() -> TestResult
    {
        ...
        assert_eq!(actual, vec!["cell1//:", "cell1//slug/", "cell1//slug:"]);
        Ok(())
    }
    ...
    async fn test_completes_directory_in_different_cell_to_canonical_name() -> TestResult {
        ...
        let actual = uut.complete("cell1/slug").await?;

        assert_eq!(actual, vec!["cell1//slug"]);
        Ok(())
    }
    ...
    async fn test_completes_partial_dir_in_different_cell_to_canonical_name() -> TestResult {
        ...
        let actual = uut.complete("cell1/k").await?;

        assert_eq!(actual, vec!["cell1//k"]);
        Ok(())
    }
    ...
    async fn test_completes_canonical_path_in_other_cell_to_with_slash_or_colon() -> TestResult {
        ...
        assert_eq!(actual, vec!["cell1//slug/", "cell1//slug:"]);
        Ok(())
    }
    ...
    async fn test_completes_cell_children_of_double_slash() -> TestResult {
        ...
        assert_eq!(actual, vec!["//:", "//slug/", "//slug:"]);
        Ok(())
    }
    ...
    {
        let (roots, cwd) = in_dir("cell1/slug/prelude")?;
        let uut = PackageCompleter::new(&cwd, &roots).await?;

        let actual = uut.complete("cell1_alias//k").await?;

        assert_eq!(actual, vec!["cell1_alias//slug/", "cell1_alias//slug:"]);
        Ok(())
    }

    #[tokio::test]
    async fn test_package_completion_only_uses_aliases_in_cells_definining_them() -> TestResult {
        let (roots, cwd) = in_dir("cell1/slug")?;
        let uut = PackageCompleter::new(&cwd, &roots).await?;

        let actual_result = uut.complete("cell1_alias//k").await;
