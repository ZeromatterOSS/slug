...
use kuro_util::process_stats::process_cpu_time_us;
...
pub(crate) fn maybe_schedule_termination() -> kuro_error::Result<()> {
    if let Some(duration) =
        kuro_core::kuro_env!("BUCK2_TERMINATE_AFTER", type=u64, applicability=testing)?
            .map(Duration::from_secs)
            .or_else(kuro_common::self_test_timeout::until_post_test_shutdown)
    {
        thread_spawn("kuro-terminate-after", move || {
