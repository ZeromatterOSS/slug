...
/// Kuro sets priority class = utility on macOS.
///
/// To disable this behavior, set this variable to `true`.
///
/// To experiment with other priority classes, set this variable to `true`,
/// and start `kuro` daemon like `taskpolicy -c utility kuro ...`.
fn enable_macos_qos() -> kuro_error::Result<bool> {
    Ok(!kuro_env!("BUCK2_DISABLE_MACOS_QOS", bool)?)
}

pub(crate) fn daemon_lower_priority(skip_macos_qos_flag: bool) -> kuro_error::Result<()> {
...
/// kuro daemon spawns processes like compilers which do not really need high priority.
...
#[cfg(target_os = "macos")]
fn do_lower_priority() -> kuro_error::Result<()> {
    use std::env;
    use std::ffi::CString;
    use std::io;
    use std::iter;
    ...
    impl Spawnattr {
        fn new() -> kuro_error::Result<Spawnattr> {
            unsafe {
    ...
    let mut argv: Vec<CString> = env::args()
        .map(|s| CString::new(s).map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::Tier0)))
        .chain(iter::once(Ok(CString::new("--skip-macos-qos").map_err(
            |e| from_any_with_tag(e, kuro_error::ErrorTag::Tier0),
        )?)))
