...
pub(crate) fn init_listener() -> kuro_error::Result<(std::net::TcpListener, ConnectionType)> {
...
fn verify_current_daemon(daemon_dir: &DaemonDir) -> kuro_error::Result<()> {
...
# This file is a cache directory tag created by Kuro.
...
impl DaemonCommand {
    fn run(
        ...
        let server_init_ctx = BuckdServerInitPreferences {
            detect_cycles: kuro_env!("DICE_DETECT_CYCLES_UNSTABLE", type=DetectCycles)?,
        ...
        tracing::info!("Starting Kuro daemon");
        ...
        kuro_common::rlimits::raise_file_descriptor_limits()?;
        ...
        let mut builder = new_tokio_runtime("kuro-rt");
        builder.enable_all();

        if let Some(num_tokio_workers) = server_init_ctx
            .daemon_startup_config
            .num_tokio_workers
            .or(kuro_env!("BUCK2_RUNTIME_THREADS", type=usize)?)
        {
        ...
        if let Some(threads) = kuro_env!("BUCK2_MAX_BLOCKING_THREADS", type=usize)? {
        ...
        let rt = new_tokio_runtime("kuro-tn")
            .enable_all()
    ...
    pub fn exec(
        ...
        if let Err(err) = res.as_ref() {
            fs_util::write(
                ...
                serde_json::to_string(&kuro_data::ErrorReport::from(err))?,
            )?;
        }
        res
    }
    ...
    #[cfg(unix)]
    fn redirect_output(stdout: File, stderr: File) -> kuro_error::Result<()> {
        ...
        unsafe {
            ...
            if stdout_fd == -1 || stderr_fd == -1 {
                return Err(kuro_error::kuro_error!(
                    kuro_error::ErrorTag::DaemonRedirect,
    ...
    #[cfg(unix)]
    fn daemonize(stdout: File, stderr: File) -> kuro_error::Result<()> {
    ...
    fn daemonize(_stdout: File, _stderr: File) -> kuro_error::Result<()> {
...
mod tests {
    ...
    use kuro_client_ctx::daemon::client::connect::new_daemon_api_client;
    use kuro_client_ctx::daemon_constraints::gen_daemon_constraints;
    use kuro_common::init::DaemonStartupConfig;
    use kuro_common::invocation_paths::InvocationPaths;
    ...
    async fn test_daemon_smoke() {
        ...
        kuro_core::client_only::CLIENT_ONLY_VAL.init(false);
