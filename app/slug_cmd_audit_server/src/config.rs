...
use slug_cmd_audit_client::config::OutputFormat;
use slug_cmd_audit_client::config::ValueStyle;
use slug_common::dice::cells::HasCellResolver;
use slug_common::legacy_configs::configs::LegacyBuckConfig;
use slug_common::legacy_configs::configs::LegacyBuckConfigLocation;
use slug_common::legacy_configs::configs::LegacyBuckConfigValue;
...
impl<'a> Match<'a> {
    fn parse(resolver: &CellAliasResolver, spec: &'a str) -> slug_error::Result<Self> {
...
impl<'a> Matches<'a> {
    fn parse(resolver: &CellAliasResolver, specs: &'a [String]) -> slug_error::Result<Self> {
        Ok(Self {
...
trait CellConfigRenderer {
    fn render_cell_header(&mut self, cell: CellName) -> slug_error::Result<()>;
    fn render_section_header(&mut self, section: &str) -> slug_error::Result<()>;
...
impl CellConfigRenderer for JsonCellConfigRenderer<'_> {
    fn render_cell_header(&mut self, _cell: CellName) -> slug_error::Result<()> {
        Ok(())
    }

    fn render_section_header(&mut self, _section: &str) -> slug_error::Result<()> {
        Ok(())
    }
