    ...
    async fn server_execute(
        ...
        Ok(server_ctx
            .with_dice_ctx(|server_ctx, mut ctx| async move {
                ...
                let results: Vec<(_, slug_error::Result<Vec<_>>)> = futures.collect().await;
                // This is expected to not return any errors, and so we're not careful about not propagating it.
                let to_absolute_path = move |include: ImportPath| -> slug_error::Result<_> {
                ...
                let absolutize_paths =
                    |paths: Vec<ImportPath>| -> slug_error::Result<Vec<AbsNormPathBuf>> {
                        paths.into_try_map(&to_absolute_path)
                    };
                let results: Vec<(String, slug_error::Result<Vec<AbsNormPathBuf>>)> = results
