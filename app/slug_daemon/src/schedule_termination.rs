...
use slug_util::process_stats::process_cpu_time_us;
...
pub(crate) fn maybe_schedule_termination() -> slug_error::Result<()> {
    if let Some(duration) =
        slug_core::slug_env!("BUCK2_TERMINATE_AFTER", type=u64, applicability=testing)?
            .map(Duration::from_secs)
            .or_else(slug_common::self_test_timeout::until_post_test_shutdown)
    {
        thread_spawn("slug-terminate-after", move || {
