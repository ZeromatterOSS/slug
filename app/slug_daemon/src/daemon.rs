...
pub(crate) fn init_listener() -> slug_error::Result<(std::net::TcpListener, ConnectionType)> {
...
fn verify_current_daemon(daemon_dir: &DaemonDir) -> slug_error::Result<()> {
...
fn verify_buck_out_dir(paths: &InvocationPaths) -> slug_error::Result<()> {
...
# This file is a cache directory tag created by Slug.
...
impl DaemonCommand {
    fn run(
        ...
        let server_init_ctx = BuckdServerInitPreferences {
            detect_cycles: slug_env!("DICE_DETECT_CYCLES_UNSTABLE", type=DetectCycles)?,
        ...
        tracing::info!("Starting Slug daemon");
        ...
        slug_common::rlimits::raise_file_descriptor_limits()?;
        ...
        let mut builder = new_tokio_runtime("slug-rt");
        builder.enable_all();

        if let Some(num_tokio_workers) = server_init_ctx
            .daemon_startup_config
            .num_tokio_workers
            .or(slug_env!("BUCK2_RUNTIME_THREADS", type=usize)?)
        {
        ...
        if let Some(threads) = slug_env!("BUCK2_MAX_BLOCKING_THREADS", type=usize)? {
        ...
        let rt = new_tokio_runtime("slug-tn")
            .enable_all()
    ...
    pub fn exec(
        ...
        if let Err(err) = res.as_ref() {
            fs_util::write(
                ...
                serde_json::to_string(&slug_data::ErrorReport::from(err))?,
            )?;
        }
        res
    }
    ...
    #[cfg(unix)]
    fn redirect_output(stdout: File, stderr: File) -> slug_error::Result<()> {
        ...
        unsafe {
            ...
            if stdout_fd == -1 || stderr_fd == -1 {
                return Err(slug_error::slug_error!(
                    slug_error::ErrorTag::DaemonRedirect,
    ...
    #[cfg(unix)]
    fn daemonize(stdout: File, stderr: File) -> slug_error::Result<()> {
    ...
    fn daemonize(_stdout: File, _stderr: File) -> slug_error::Result<()> {
...
mod tests {
    ...
    use slug_client_ctx::daemon::client::connect::new_daemon_api_client;
    use slug_client_ctx::daemon_constraints::gen_daemon_constraints;
    use slug_common::init::DaemonStartupConfig;
    use slug_common::invocation_paths::InvocationPaths;
    ...
    async fn test_daemon_smoke() {
        ...
        slug_core::client_only::CLIENT_ONLY_VAL.init(false);
