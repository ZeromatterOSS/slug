...
use kuro_analysis::analysis::calculation::get_rule_spec;
use kuro_analysis::analysis::env::RuleSpec;
use kuro_analysis::analysis::env::transitive_validations;
...
use kuro_build_api::analysis::anon_targets_registry::ANON_TARGET_REGISTRY_NEW;
use kuro_build_api::analysis::anon_targets_registry::AnonTargetsRegistryDyn;
...
use kuro_build_api::artifact_groups::promise::PromiseArtifactResolveError;
...
use kuro_build_api::deferred::calculation::EVAL_ANON_TARGET;
use kuro_build_api::deferred::calculation::GET_PROMISED_ARTIFACT;
...
use kuro_configured::execution::find_execution_platform_by_configuration;
...
use kuro_core::pattern::pattern::PatternData;
use kuro_core::pattern::pattern::lex_target_pattern;
...
use kuro_node::attrs::spec::internal::is_internal_attr;
...
impl Key for AnonTargetKey {
    type Value = kuro_error::Result<AnalysisResult>;
...
impl AnonTargetKey {
    fn downcast(key: Arc<dyn BaseDeferredKeyDyn>) -> kuro_error::Result<Self> {
        Ok(AnonTargetKey(
            key.into_any()
                .downcast()
                .ok()
    ...
    fn parse_target_label(x: &str) -> kuro_error::Result<TargetLabel> {
        let err = || {
            format!(
    ...
    fn create_name(rule_name: &str) -> kuro_error::Result<TargetLabel> {
    ...
    fn coerce_name(x: Value) -> kuro_error::Result<TargetLabel> {
    ...
    fn coerce_to_anon_target_attr(
        ...
        x: Value,
        ctx: &AnonAttrCtx,
    ) -> kuro_error::Result<AnonTargetAttr> {
    ...
    ) -> BoxFuture<'a, kuro_error::Result<AnalysisResult>> {
...
impl<'v> AnonTargetsRegistry<'v> {
    ...
    ) -> kuro_error::Result<&'v mut AnonTargetsRegistry<'v>> {
    ...
    pub(crate) fn anon_target_key(
        ...
        attributes: UnpackDictEntries<&'v str, Value<'v>>,
        owner_key: &BaseDeferredKey,
    ) -> kuro_error::Result<AnonTargetKey> {
    ...
    pub(crate) fn register_artifact(
        ...
        id: usize,
    ) -> kuro_error::Result<PromiseArtifact> {
...
impl<'v> AnonTargetsRegistryDyn<'v> for AnonTargetsRegistry<'v> {
    ...
    */
    ...
    fn assert_no_promises(&self) -> kuro_error::Result<()> {
