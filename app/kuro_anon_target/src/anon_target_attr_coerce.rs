...
use kuro_build_api::interpreter::rule_defs::resolved_macro::ResolvedStringWithMacros;
...
use kuro_node::attrs::attr_type::dict::DictAttrType;
...
pub trait AnonTargetAttrTypeCoerce {
    fn coerce_item(&self, ctx: &AnonAttrCtx, value: Value) -> kuro_error::Result<AnonTargetAttr>;
}
...
pub(crate) enum AnonTargetCoercionError {
    ...
    #[kuro(tag = Input)]
    OneOfMany(Vec<kuro_error::Error>),
...
impl AnonTargetCoercionError {
    ...
    pub fn one_of_many(mut errs: Vec<kuro_error::Error>) -> kuro_error::Error {
        if errs.is_empty() {
...
fn to_anon_target_any(value: Value, ctx: &AnonAttrCtx) -> kuro_error::Result<AnonTargetAttr> {
    if value.is_none() {
        Ok(AnonTargetAttr::None)
