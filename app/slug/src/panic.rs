...
//! The panic hook, shared by the slug CLI and daemon.
//!
//! The Slug CLI unwinds the stack on a panic, while the Slug daemon terminates without unwinding.
...
pub fn initialize() -> slug_error::Result<()> {
    ...
    slug_core::error::initialize(Box::new(move |category, err, loc, options| {
        let fb = slug_common::fbinit::get_or_init_fbcode_globals();
...
mod imp {
    ...
    use fbinit::FacebookInit;
    use slug_core::error::StructuredErrorOptions;
    use slug_data::Location;
    use slug_events::BuckEvent;
    use slug_events::daemon_id::get_daemon_id_for_panics;
    ...
    fn get_stack() -> Vec<slug_data::structured_error::StackFrame> {
        ...
        trace
            ...
            .map(|frame| {
                let symbols = frame
                    .symbols()
                    .iter()
                    .map(|symbol| slug_data::structured_error::Symbol {
                        name: symbol
                            .name()
                ...
                slug_data::structured_error::StackFrame {
    ...
    fn get_metadata_for_panic(options: &StructuredErrorOptions) -> HashMap<String, String> {
        ...
        #[cfg(not(client_only))]
        if let Some(commands) = slug_server::active_commands::try_active_commands() {
    ...
    pub(crate) fn write_soft_error(
        ...
        // commands.
        match slug_events::dispatch::get_dispatcher_opt() {
            Some(dispatcher) => {
            ...
            None => {
                ...
                #[cfg(not(client_only))]
                let warn = !slug_server::active_commands::broadcast_instant_event(&event)
                    && !options.quiet;
                if warn {
    ...
    fn panic_payload(
        ...
        message: String,
        backtrace: Vec<slug_data::structured_error::StackFrame>,
        options: &StructuredErrorOptions,
        soft_error_category: Option<slug_data::SoftError>,
        ...
        slug_data::StructuredError {
            ...
            soft_error_category: soft_error_category
                .map(|arg0: slug_data::SoftError| ToOwned::to_owned(&arg0)),
    ...
    fn write_to_scribe(fb: FacebookInit, data: slug_data::StructuredError) {
        ...
        let _err = thread_spawn("slug-write-panic-to-scribe", move || {
