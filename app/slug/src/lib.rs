...
use dupe::Dupe;
use slug_client::commands::build::BuildCommand;
use slug_client::commands::bxl::BxlCommand;
use slug_client::commands::clean::CleanCommand;
use slug_client::commands::ctargets::ConfiguredTargetsCommand;
use slug_client::commands::expand_external_cell::ExpandExternalCellsCommand;
use slug_client::commands::explain::ExplainCommand;
use slug_client::commands::fetch::FetchCommand;
use slug_client::commands::help_env::HelpEnvCommand;
use slug_client::commands::info::InfoCommand;
use slug_client::commands::init::InitCommand;
use slug_client::commands::install::InstallCommand;
use slug_client::commands::kill::KillCommand;
use slug_client::commands::killall::KillallCommand;
use slug_client::commands::lsp::LspCommand;
use slug_client::commands::profile::ProfileCommand;
use slug_client::commands::query::aquery::AqueryCommand;
use slug_client::commands::query::cquery::CqueryCommand;
use slug_client::commands::query::uquery::UqueryCommand;
use slug_client::commands::root::RootCommand;
use slug_client::commands::run::RunCommand;
use slug_client::commands::server::ServerCommand;
use slug_client::commands::status::StatusCommand;
use slug_client::commands::subscribe::SubscribeCommand;
use slug_client::commands::targets::TargetsCommand;
use slug_client::commands::test::TestCommand;
use slug_client::commands::version::VersionCommand;
use slug_client_ctx::argfiles::expand_argv;
use slug_client_ctx::bazelrc::inject_bazelrc_args;
use slug_client_ctx::bazelrc::normalize_args;
use slug_client_ctx::client_ctx::BuckSubcommand;
use slug_client_ctx::client_ctx::ClientCommandContext;
use slug_client_ctx::client_metadata::ClientMetadata;
use slug_client_ctx::client_metadata::parse_client_metadata;
use slug_client_ctx::common::BuckArgMatches;
use slug_client_ctx::exit_result::ExitResult;
use slug_client_ctx::immediate_config::ImmediateConfigContext;
use slug_client_ctx::version::BuckVersion;
use slug_cmd_audit_client::AuditCommand;
use slug_cmd_debug_client::DebugCommand;
use slug_cmd_log_client::LogCommand;
use slug_cmd_rage_client::rage::RageCommand;
use slug_cmd_starlark_client::StarlarkCommand;
use slug_common::argv::Argv;
use slug_common::invocation_paths_result::InvocationPathsResult;
use slug_common::invocation_roots::get_invocation_paths_result;
...
use slug_util::threads::thread_spawn_scoped;
...
fn parse_isolation_dir(s: &str) -> slug_error::Result<FileNameBuf> {
    FileNameBuf::try_from(s.to_owned()).buck_error_context("isolation dir must be a directory name")
}

/// Options of `slug` command, before subcommand.
#[derive(Clone, Debug, clap::Parser)]
#[clap(next_help_heading = "Universal Options")]
struct BeforeSubcommandOptions {
    /// The name of the directory that Slug creates within buck-out for writing outputs and daemon
    /// information. If one is not provided, Slug creates a directory with the default name.
    ///
    /// Instances of Slug share a daemon if and only if their isolation directory is identical.
    /// The isolation directory also influences the output paths provided by Slug,
    ...
    /// Metadata key-value pairs to inject into Slug's logging. Client metadata must be of the
    ...
    // is not supported for production work for slug and lots of places already set
    ...
    ///
    /// When set, slug will not load ~/.bazelrc or <workspace>/.bazelrc.
...
fn help() -> &'static str {
    concat!(
        "A build system\n",
        "\n",
        "Documentation: https://slug.build/docs/\n", // @oss-enable
        // @oss-disable: "Documentation: https://internalfb.com/intern/staticdocs/slug/docs/\n",
    )
}
...
pub fn exec(process: ProcessContext<'_>) -> ExitResult {
    ...
    let arg0_override = slug_env!("BUCK2_ARG0")?;
    ...
    let project_root = slug_common::invocation_roots::find_invocation_roots(&cwd)
        .ok()
    ...
    // slug flags (--keep-going) so both forms are accepted transparently.
    ...
    let matches = match clap.try_get_matches_from(argv.expanded_argv.args()) {
        ...
        Err(e) => {
            ...
            return if e.exit_code() == 0 {
                ...
                let e = slug_error::Error::from(e).tag([ErrorTag::ClapMatch]);
    ...
    // is an end user: https://fburl.com/scuba/slug_builds/n4klo51d
...
impl ParsedArgv {
    fn parse(argv: Argv, matches: clap::ArgMatches) -> slug_error::Result<Self> {
...
pub(crate) enum CommandKind {
    #[cfg(not(client_only))]
    #[clap(hide = true)]
    Daemon(slug_daemon::daemon::DaemonCommand),
    #[cfg(not(client_only))]
    #[clap(hide = true)]
    ...
    Build(BuildCommand),
    Bxl(BxlCommand),
    // TODO(nga): implement `slug help-buckconfig` too
    ...
    Debug(DebugCommand),
    #[clap(hide = true)]
    Complete(slug_cmd_completion_client::complete::CompleteCommand),
    Completion(slug_cmd_completion_client::completion::CompletionCommand),
    Docs(slug_cmd_docs_client::DocsCommand),
    #[clap(subcommand)]
...
impl CommandKind {
    pub(crate) fn exec(
        ...
        thread::scope(|scope| {
            ...
            match thread_spawn_scoped("slug-main", scope, move || {
                self.exec_no_daemon(
                    common_opts,
                    process,
                    immediate_config,
    ...
    fn exec_no_daemon(
        ...
        let start_in_process_daemon = if common_opts.no_buckd {
            #[cfg(not(client_only))]
            let v = slug_daemon::no_buckd::start_in_process_daemon(
        ...
        match self {
            ...
            CommandKind::Query(cmd) => {
                slug_client_ctx::eprintln!(
                    "WARNING: \"slug query\" is an alias for \"slug uquery\". Consider using \"slug cquery\" or \"slug uquery\" explicitly."
                )?;
