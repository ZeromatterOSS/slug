...
#[cfg(windows)]
pub(crate) fn check_user_allowed() -> slug_error::Result<()> {
    use std::io;
    use std::mem;
    ...
    use slug_wrapper_common::win::winapi_handle::WinapiHandle;
    ...
    if elevation_struct.TokenIsElevated == 1 {
        // In CI, if slug got run from an admin shell, we need not worry that a
        // subsequent invocation might come from a non-admin shell. It almost
        // certainly will not.
        if !is_ci()? {
            tracing::warn!(
                "You're running slug from an admin shell. Invocations from non-admin shells will likely fail going forward. To remediate, run `slug clean` in this admin shell, then switch to a non-admin shell."
            );
        }
    }
    Ok(())
...
pub(crate) fn check_user_allowed() -> slug_error::Result<()> {
    ...
    #[error("slug is not allowed to run as root (unless home dir is owned by root)")]
    #[slug(tag = Input)]
    struct RootError;
