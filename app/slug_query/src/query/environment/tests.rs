...
use slug_query::query::traversal::NodeLookup;
...
impl QueryTarget for TestTarget {
    ...
    fn attr_any_matches(
        _attr: &Self::Attr<'_>,
        _filter: &dyn Fn(&str) -> slug_error::Result<bool>,
...
impl NodeLookup<TestTarget> for TestEnv {
    fn get(&self, label: &<TestTarget as LabeledNode>::Key) -> slug_error::Result<TestTarget> {
        self.graph
            .get(label)
            .duped()
...
impl QueryEnvironment for TestEnv {
    ...
    async fn eval_file_literal(&self, _literal: &str) -> slug_error::Result<FileSet> {
        unimplemented!()
    }
...
impl TestEnv {
    ...
    fn set(&self, entries: &str) -> slug_error::Result<TargetSet<TestTarget>> {
...
#[tokio::test]
async fn test_one_path() -> slug_error::Result<()> {
...
#[tokio::test]
async fn test_many_paths() -> slug_error::Result<()> {
    let mut env = TestEnvBuilder::default();
    env.edge(1, 2);
    env.edge(2, 3);
    env.edge(1, 10);
...
#[tokio::test]
async fn test_distinct_paths() -> slug_error::Result<()> {
    let mut env = TestEnvBuilder::default();
    env.edge(1, 10);
    env.edge(10, 100);
    env.edge(2, 20);
...
#[tokio::test]
async fn test_no_path() -> slug_error::Result<()> {
    let mut env = TestEnvBuilder::default();
    env.edge(1, 10);
    env.edge(2, 20);
    let env = env.build();
...
#[tokio::test]
async fn test_nested_paths() -> slug_error::Result<()> {
    let mut env = TestEnvBuilder::default();
    env.edge(1, 2);
    env.edge(2, 3);
    env.edge(3, 4);
...
#[tokio::test]
async fn test_paths_with_cycles_present() -> slug_error::Result<()> {
    let mut env = TestEnvBuilder::default();
    env.edge(1, 2);
    env.edge(2, 3);
    env.edge(3, 4);
...
#[tokio::test]
async fn test_rdeps() -> slug_error::Result<()> {
    let mut env = TestEnvBuilder::default();
    env.edge(1, 2);
    env.edge(2, 3);
