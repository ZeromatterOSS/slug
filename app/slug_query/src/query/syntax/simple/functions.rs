...
use slug_query_derive::query_module;
use slug_query_parser::BinaryOp;
use slug_query_parser::Expr;
use slug_query_parser::spanned::Spanned;
...
pub trait QueryLiteralVisitor<'a> {
    fn target_pattern(&mut self, pattern: &'a str) -> slug_error::Result<()>;
}
...
impl<Env: QueryEnvironment> DefaultQueryFunctionsModule<Env> {
    ...
    /// For example:
    /// ```text
    /// $ slug uquery "allpaths('//foo:bar', '//foo/bar/lib:baz')"
    /// ```
    ...
    /// ```text
    /// $ slug uquery "allpaths(kind(java_library, '//...'), '//foo:bar')"
    /// ```
    ...
    ///
    /// ```text
    /// $ slug uquery "allpaths(//slug:slug, //slug/app/slug_validation:slug_validation)" --output-format=dot > result.dot
    ...
    ///
    /// ```text
    /// $ slug uquery 'somepath(//slug:slug, //slug/app/slug_node:slug_node)'
    ///
    /// //slug:slug
    /// //slug/app/slug:slug-bin
    /// //slug/app/slug_analysis:slug_analysis
    /// //slug/app/slug_node:slug_node
    /// ```
    async fn somepath(
        &self,
        evaluator: &QueryEvaluator<'_, Env>,
    ...
    /// For example:
    /// ```text
    /// $ slug uquery "attrfilter(deps, '//slug/app/slug_validation:slug_validation', '//...')"
    ///
    /// //slug/app/slug:slug-bin
    /// //slug/app/slug_server:slug_server
    /// //slug/app/slug_server:slug_server-unittest
    /// ```
    /// returns targets that contain `//slug/app/slug_validation:slug_validation` target in their `deps` attribute.
    async fn attrfilter(
        &self,
        attr: String,
        value: String,
    ...
    /// For example:
    /// ```text
    /// $ slug uquery "nattrfilter(deps, '//foo:bar', '//...')"
    /// ```
    ...
    /// For example:
    /// ```text
    /// $ slug uquery "attrregexfilter(deps, '.+validation$', '//...')"
    ///
    /// //slug/app/slug:slug-bin
    /// //slug/app/slug_server:slug_server
    /// //slug/app/slug_server:slug_server-unittest
    ...
    /// For example:
    /// ```text
    /// $ slug uquery 'buildfile(//slug:slug)'
    ///
    /// slug/BUCK
    /// ```
    ///
    ...
    /// Examples:
    /// ```text
    /// $ slug uquery "buildfile(//slug/app/slug_action_impl_tests:slug_action_impl_tests)"
    /// ```
    /// and
    /// ```text
    /// $ slug uquery "buildfile(owner(app/slug_action_impl_tests/src/context.rs))"
    /// ```
    /// both return `slug/app/slug_action_impl_tests/TARGETS`.
    ...
    /// For example:
    /// ```text
    /// $ slug uquery "rbuildfiles(//slug/BUCK, //slug/defs.bzl)"
    ///
    /// slug/defs.bzl
    /// slug/BUCK
    /// ```
    ...
    /// For example:
    /// ```text
    /// $ slug uquery 'allbuildfiles(//foo:bar)'
    ///
    /// foo/BUCK
    ...
    /// For example:
    /// ```text
    /// $ slug uquery "filter(validation$, //slug/app/...)"
    ///
    /// //slug/app/slug_validation:slug_validation
    /// ```
    /// returns all targets within `//slug/app` that have a label with a `validation` suffix.
    ...
    /// For example:
    /// ```text
    /// $ slug uquery "inputs(//slug/dice/...)"
    /// ```
    /// returns the direct inputs for the `//slug/dice/...` targets.
    ...
    /// For example:
    /// ```text
    /// $ slug query "kind('java.*', deps('//foo:bar'))"
    /// ```
    ...
    /// Not implemented.
    ///
    /// This function won't be implemented in the future, because slug query core does not support returning both files and targets from a single function.
    ///
    ...
    /// For example:
    /// ```text
    /// $ slug uquery "owner('app/slug/src/lib.rs')"
    ///
    /// //slug/app/slug:slug-unittest
    /// //slug/app/slug:slug
    /// ```
    ...
    /// For example:
    /// ```text
    /// $ slug uquery 'targets_in_buildfile(buildfile(//slug:slug))'
    ///
    /// //slug:slug
    /// //slug:slug_bundle
    /// //slug:symlinked_slug_and_tpx
    /// ```
    ...
    ///
    /// ```text
    /// $ slug uquery "rdeps(//slug/..., //slug/dice/dice:dice, 1)"
    /// ```
    /// returns all targets under `//slug/...` that depend on `//slug/dice/dice:dice`.
    async fn rdeps(
        &self,
        evaluator: &QueryEvaluator<'_, Env>,
        universe: TargetSet<Env::Target>,
    ...
    /// For example:
    /// ```text
    /// $ slug uquery "testsof(set(//slug/dice/dice:dice //slug/app/slug:slug))"
    ///
    /// //slug/dice/dice:dice-unittest
    /// //slug/app/slug:slug-unittest
    /// ```
    /// returns the tests associated with both `//slug/dice/dice:dice` and `//slug/app/slug:slug`.
    ///
    ...
    /// For example:
    /// ```text
    /// $ slug uquery "testsof(deps(//slug/app/slug:slug))"
    /// ```
    /// first finds the transitive closure of `//slug/app/slug:slug`,
    ...
    ///
    /// Example:
    /// `slug cquery "deps('//foo:bar', 1, first_order_deps())"` is equivalent to `slug cquery "deps('//foo:bar', 1)"`
    ...
    ///
    /// Example:
    /// `slug cquery "deps('//foo:bar', 1, target_deps())"`
    ...
    ///
    /// Example:
    /// `slug cquery "deps('//foo:bar', 1, exec_deps())"`
    ...
    ///
    /// Example:
    /// `slug cquery "deps('//foo:bar', 1, configuration_deps())"`
    ...
    ///
    /// Example:
    /// `slug cquery "deps('//foo:bar', 1, toolchain_deps())"`
    ...
    ///
    /// Example:
    /// `slug aquery "deps('//foo:bar') intersect deps('//baz:lib')"` is the same as
    /// `slug aquery "deps('//foo:bar') ^ deps('//baz:lib')"`
    ...
    ///
    /// Example:
    /// `slug aquery "deps('//foo:bar') except deps('//baz:lib')"` is the same as
    /// `slug aquery "deps('//foo:bar') - deps('//baz:lib')"`
    ...
    ///
    /// Example:
    /// `slug aquery "deps('//foo:bar') union deps('//baz:lib')"` is the same as
    /// `slug aquery "deps('//foo:bar') + deps('//baz:lib')"`
...
impl<Env: QueryEnvironment> DefaultQueryFunctions<Env> {
    ...
    pub fn attrfilter(
        ...
        value: &str,
        targets: &TargetSet<Env::Target>,
    ) -> slug_error::Result<TargetSet<Env::Target>> {
    ...
    pub fn filter_file_set(&self, regex: &str, files: &FileSet) -> slug_error::Result<FileSet> {
        files.filter_name(regex)
    }

    pub fn inputs(&self, targets: &TargetSet<Env::Target>) -> slug_error::Result<FileSet> {
        targets.inputs()
    }
    ...
    pub async fn testsof_with_default_target_platform(
        ...
        env: &Env,
        targets: &TargetSet<Env::Target>,
    ) -> slug_error::Result<Vec<MaybeCompatible<Env::Target>>> {
    ...
    where
        ...
        Ff: Fn(&FileSet, &FileSet) -> slug_error::Result<FileSet>,
    {
        match (left, right) {
