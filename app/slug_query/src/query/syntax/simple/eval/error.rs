...
pub enum QueryError {
    ...
    #[error("Argument `{1}` to `{0}` is not yet supported in slug")]
...
impl From<slug_error::Error> for QueryError {
    fn from(err: slug_error::Error) -> Self {
...
impl QueryError {
    pub fn drop_spans(err: Spanned<Self>) -> slug_error::Error {
        match err.value {
    ...
    pub fn convert_error(err: Spanned<Self>, input: &str) -> slug_error::Error {
        ...
        match err.value {
            ...
            e => {
                // TODO(cjhopman): This is going to drop the backtrace attached to the error, we should figure
                // out how to keep that.
                slug_error::slug_error!(slug_error::ErrorTag::Input, "{}:{}", e, context)
            }
        }
    }
}
