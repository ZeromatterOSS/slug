...
use kuro_build_api::interpreter::rule_defs::provider::builtin::install_info::FrozenInstallInfo;
...
use kuro_build_api::materialize::materialize_and_upload_artifact_group;
use kuro_build_api::validation::validation_impl::VALIDATION_IMPL;
use kuro_cli_proto::InstallRequest;
use kuro_cli_proto::InstallResponse;
...
use kuro_core::pattern::pattern::ModifiersError;
use kuro_core::pattern::pattern::PackageSpec;
...
use kuro_data::InstallEventInfoEnd;
use kuro_data::InstallEventInfoStart;
...
use kuro_install_proto::DeviceMetadata;
use kuro_install_proto::FileReadyRequest;
use kuro_install_proto::InstallInfoRequest;
use kuro_install_proto::ShutdownRequest;
use kuro_install_proto::installer_client::InstallerClient;
...
use kuro_util::future::try_join_all;
...
) -> kuro_error::Result<AbsNormPathBuf> {
...
pub(crate) async fn install_command(
    ...
    req: InstallRequest,
) -> kuro_error::Result<InstallResponse> {
...
impl ServerCommandTemplate for InstallServerCommand {
    type StartEvent = kuro_data::InstallCommandStart;
    type EndEvent = kuro_data::InstallCommandEnd;
    type Response = InstallResponse;
    type PartialResult = NoPartialResult;

    fn end_event(&self, _response: &kuro_error::Result<Self::Response>) -> Self::EndEvent {
        kuro_data::InstallCommandEnd {
            unresolved_target_patterns: self
                .req
                .target_patterns
                .iter()
                .map(|p| kuro_data::TargetPattern { value: p.clone() })
                .collect(),
        }
    }
...
async fn collect_install_request_data<'a>(
    ...
    request: &InstallRequest,
) -> kuro_error::Result<impl IntoIterator<Item = InstallRequestData<'a>> + use<'a>> {
...
fn get_random_tcp_port() -> kuro_error::Result<u16> {
...
impl<'a> ConnectedInstaller<'a> {
    async fn connect(
        ...
        let timeout =
            Duration::from_secs(kuro_env!("BUCK2_INSTALLER_TIMEOUT_S", type=u64)?.unwrap_or(120));
        let send_timeout = Duration::from_secs(
            kuro_env!("BUCK2_INSTALLER_SEND_TIMEOUT_S", type=u64)?.unwrap_or(300),
        );

        let client: kuro_error::Result<InstallerClient<Channel>> = span_async_simple(
            kuro_data::ConnectToInstallerStart {
            ...
            kuro_data::ConnectToInstallerEnd {},
        )
        .await;
    ...
    fn install_result(self, result: kuro_error::Result<()>) -> InstallResult {
        InstallResult {
    ...
    async fn send_install_info(&mut self) -> kuro_error::Result<()> {
        for (installed_target, install_files) in &self.install_request_data.installed_targets {
            ...
            if install_info_response.install_id != install_id {
                ...
                return Err(kuro_error::kuro_error!(
                    kuro_error::ErrorTag::InstallIdMismatch,
    ...
    async fn send_shutdown_command(&self) -> kuro_error::Result<()> {
    ...
    async fn send_file(&self, file: FileResult) -> kuro_error::Result<()> {
        ...
        span_async(start, async {
            let mut outcome: kuro_error::Result<()> = Ok(());
            ...
            if let Some(error_detail) = response.error_detail {
                let mut error: kuro_error::Error = InstallError::ProcessingFileReadyFailure {
                    install_id: install_id.to_owned(),
                    artifact: name.to_owned(),
                    path: path.to_owned(),
                ...
                let category_tag = if let Ok(category) =
                    kuro_install_proto::ErrorCategory::try_from(error_detail.category)
                {
                    match category {
                        kuro_install_proto::ErrorCategory::Unspecified => {
                            ErrorTag::InstallerUnknown
                        }
                        kuro_install_proto::ErrorCategory::Tier0 => ErrorTag::InstallerTier0,
                        kuro_install_proto::ErrorCategory::Input => ErrorTag::InstallerInput,
                        kuro_install_proto::ErrorCategory::Environment => {
...
async fn handle_install_request<'a>(
        ...
        .try_compute2(
            ...
            |ctx| {
                async move {
                    ...
                    // 1. kuro passes a temp file for a tcp port output.
                    // 2. installer app choose unused tcp port and writes it into the passed file.
                    // 3. kuro reads tcp port from file and use it to connect to the installer app. (`connect_to_installer` function)
                    ...
                    kuro_error::Ok(installer.install(files_rx).await)
                }
                .boxed()
            },
        )
    ...
    let device_metadata: Vec<kuro_data::DeviceMetadata> = device_metadata
        .lock()
        .await
        .iter()
        .map(|metadata| kuro_data::DeviceMetadata {
            entry: metadata
                .entry
                .iter()
                .map(|e| kuro_data::device_metadata::Entry {
                    key: e.key.clone(),
                    value: e.value.clone(),
                })
                .collect(),
    ...
    get_dispatcher().instant_event(kuro_data::InstallFinished {
...
async fn upload_installer_logs(log_path: &AbsNormPathBuf) -> kuro_error::Result<String> {
...
fn get_stdio(log_installer_console: bool) -> kuro_error::Result<Stdio> {
