...
use kuro_data::FileWatcherEvent;
...
use kuro_event_observer::display::display_anon_target;
use kuro_event_observer::display::display_bxl_key;
use kuro_event_observer::display::display_configured_target_label;
...
use kuro_explain::ActionEntryData;
use kuro_explain::ChangedFilesEntryData;
...
impl ActionEntry {
    fn format_action(
        &self,
        event: &kuro_data::SpanEndEvent,
    ) -> kuro_error::Result<Option<(String, ActionEntryData)>> {
        let action = &self.action;

        let action_execution = match &event.data {
                Some(kuro_data::span_end_event::Data::ActionExecution(action_exec)) => Some(action_exec),
                _ => None,
                ...
                if let Some(data) = event.data {
                    ...
                    match data {
                        kuro_data::buck_event::Data::SpanEnd(span) => {
                            ...
                            match &span.data {
                                Some(kuro_data::span_end_event::Data::FileWatcher(end)) => {
