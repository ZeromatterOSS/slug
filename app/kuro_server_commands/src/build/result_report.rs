...
mod proto {
    pub(crate) use kuro_cli_proto::BuildTarget;
    pub(crate) use kuro_cli_proto::build_target::BuildOutput;
    pub(crate) use kuro_cli_proto::build_target::build_output::BuildOutputProviders;
}

/// Simple container for multiple [`kuro_error::Error`]s
pub(crate) struct BuildErrors {
    pub(crate) errors: Vec<kuro_error::Error>,
}
...
/// Collects build results into a Result<Vec<proto::BuildTarget>, kuro_error::Errors>. If any targets
...
impl<'a> ResultReporter<'a> {
    ...
    ) -> kuro_error::Result<BuildTargetsAndErrors> {
        let mut out = Self {
            artifact_fs,
            options,
            results: Vec::new(),
