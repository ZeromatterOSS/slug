...
impl ServerCommandTemplate for BuildServerCommand {
    type StartEvent = kuro_data::BuildCommandStart;
    type EndEvent = kuro_data::BuildCommandEnd;
    type Response = kuro_cli_proto::BuildResponse;
...
fn expect_build_opts(req: &kuro_cli_proto::BuildRequest) -> &CommonBuildOptions {
...
async fn build(
    ...
    request: &kuro_cli_proto::BuildRequest,
) -> kuro_error::Result<kuro_cli_proto::BuildResponse> {
...
async fn process_streaming_build_result(
    ...
    let cwd: &kuro_core::fs::project_rel_path::ProjectRelativePath = server_ctx.working_dir();
    let cell_resolver = ctx.get_cell_resolver().await?;
...
async fn maybe_stream_build_reports(
    build_future: impl std::future::Future<Output = kuro_error::Result<BuildTargetResult>>,
...
async fn process_build_result(
    ...
    Ok(kuro_cli_proto::BuildResponse {
        build_targets,
        project_root,
...
async fn build_targets_in_universe(
    ...
    provider_labels
        .into_iter()
        .map(|p| {
            kuro_util::async_move_clone!(providers_to_build, {
