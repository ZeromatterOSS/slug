...
/// Internal test runner implementation for Slug.
///
/// This is a basic test runner intended to be used by the open-source Slug build
/// if no external test runner is provided. This ensures that `slug test` works
/// out-of-the-box for open-source users.
///
/// **This is intended for open-source use only.**
pub struct SlugTestRunner {
...
impl SlugTestRunner {
    ...
    pub async fn run_all_tests(&self) -> slug_error::Result<()> {
        let receiver;
        {
        ...
        let run_verdict = receiver
            ...
            // Use an arbitrarily large buffer -- execution throttling will be handled by the Slug
    ...
    async fn report_test_result(&self, test_result: TestResult) -> slug_error::Result<()> {
        self.orchestrator_client
    ...
    TestResult {
        ...
        details: {
            let stdout = match &execution_result.stdout {
                slug_test_api::data::ExecutionStream::Inline(d) => {
