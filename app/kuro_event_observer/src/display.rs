...
use kuro_data::AnonTarget;
use kuro_data::BxlFunctionKey;
use kuro_data::BxlFunctionLabel;
use kuro_data::ConfiguredTargetLabel;
use kuro_data::FileWatcherKind;
use kuro_data::TargetLabel;
use kuro_data::action_key;
...
pub fn display_anon_target(ctl: &AnonTarget) -> kuro_error::Result<String> {
    if let AnonTarget {
...
pub fn display_analysis_target(
    target: &kuro_data::analysis_start::Target,
    opts: TargetDisplayOptions,
) -> kuro_error::Result<String> {
    use kuro_data::analysis_start::Target;
    match target {
        ...
        Target::DynamicLambda(dynamic) => {
            use kuro_data::dynamic_lambda_owner::Owner;
            match dynamic
                .owner
                .as_ref()
...
pub fn display_bxl_key(ctl: &BxlFunctionKey) -> kuro_error::Result<String> {
...
pub fn display_event(event: &BuckEvent, opts: TargetDisplayOptions) -> kuro_error::Result<String> {
    let res: kuro_error::Result<_> = try {
        let data = match event.data() {
            kuro_data::buck_event::Data::SpanStart(start) => start.data.as_ref().unwrap(),
            _ => Err(kuro_error::Error::from(ParseEventError::UnexpectedEvent))?,
        };

        let res: kuro_error::Result<_> = match data {
            ...
            Data::MatchDepFiles(kuro_data::MatchDepFilesStart {
            ...
            Data::SharedTask(kuro_data::SharedTaskStart { owner_trace_id }) => Ok(format!(
            ...
            Data::DiceSynchronizeSection(..) => Ok("Synchronizing kuro internal state".to_owned()),
            Data::DiceCleanup(..) => Ok("Cleaning up graph state".to_owned()),
            Data::ExclusiveCommandWait(kuro_data::ExclusiveCommandWaitStart { command_name }) => {
            ...
            Data::DeferredPreparationStage(prep) => {
                use kuro_data::deferred_preparation_stage_start::Stage;
            ...
            Data::DynamicLambda(lambda) => {
                use kuro_data::dynamic_lambda_start::Owner;
            ...
            Data::ConnectToInstaller(kuro_data::ConnectToInstallerStart { tcp_port }) => {
...
fn display_file_watcher(provider: i32) -> &'static str {
    match kuro_data::FileWatcherProvider::try_from(provider) {
        Ok(kuro_data::FileWatcherProvider::Watchman) => "Watchman",
        Ok(kuro_data::FileWatcherProvider::RustNotify) => "notify",
        Ok(kuro_data::FileWatcherProvider::FsHashCrawler) => "fs_hash_crawler",
        Ok(kuro_data::FileWatcherProvider::EdenFs) => "EdenFS",
        Err(_) => "unknown mechanism",
    }
}

pub fn display_analysis_stage(stage: &kuro_data::analysis_stage_start::Stage) -> &'static str {
    use kuro_data::analysis_stage_start::Stage;
...
pub fn display_file_watcher_end(file_watcher_end: &kuro_data::FileWatcherEnd) -> Vec<String> {
...
pub fn display_executor_stage(
    stage: &kuro_data::executor_stage_start::Stage,
) -> Option<&'static str> {
    use kuro_data::executor_stage_start::Stage;

    let label = match stage {
        Stage::Prepare(..) => "prepare",
        Stage::CacheQuery(cache_query) => {
            match kuro_data::CacheType::try_from(cache_query.cache_type).unwrap() {
                kuro_data::CacheType::ActionCache => "re_action_cache",
                kuro_data::CacheType::RemoteDepFileCache => "re_dep_file_cache",
            }
        }
        Stage::CacheHit(..) => "re_download",
        Stage::Re(re) => {
            use kuro_data::re_stage::Stage;
        ...
        Stage::Local(local) => {
            use kuro_data::local_stage::Stage;
...
) -> kuro_error::Result<Option<Lines>> {
    let kuro_data::TestResult {
        name,
        status,
        duration,
        details,
...
pub struct ActionErrorDisplay<'a> {
    ...
    pub reason: String,
    pub command: Option<&'a kuro_data::CommandExecutionDetails>,
    pub error_diagnostics: Option<&'a kuro_data::ActionErrorDiagnostics>,
}
    ...
    fn simple_format_inner(
        ...
        if let Some(error_diagnostics) = self.error_diagnostics {
            match error_diagnostics.data.as_ref().unwrap() {
                kuro_data::action_error_diagnostics::Data::SubErrors(sub_errors) => {
                ...
                kuro_data::action_error_diagnostics::Data::HandlerInvocationError(error) => {
...
pub fn get_action_error_reason(error: &kuro_data::ActionError) -> kuro_error::Result<String> {
    use kuro_data::action_error::Error;
    ...
    Ok(
            ...
            .buck_error_context("Internal error: Missing error in action error")?
        {
            ...
            Error::CommandExecutionError(kuro_data::CommandExecutionError {}) => {
...
pub fn display_action_error(
    error: &kuro_data::ActionError,
    opts: TargetDisplayOptions,
) -> kuro_error::Result<ActionErrorDisplay<'_>> {
...
fn failure_reason_for_command_execution(
    command_execution: &kuro_data::CommandExecution,
) -> kuro_error::Result<String> {
    use kuro_data::command_execution::Cancelled;
    use kuro_data::command_execution::Error;
    use kuro_data::command_execution::Failure;
    use kuro_data::command_execution::Status;
    use kuro_data::command_execution::Success;
    use kuro_data::command_execution::Timeout;
    use kuro_data::command_execution::WorkerFailure;
