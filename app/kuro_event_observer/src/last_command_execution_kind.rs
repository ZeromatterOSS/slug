...
pub fn get_last_command_execution_kind(
    ...
    if let Some(command_kind) = last_command_kind {
        ...
        match command_kind.command.as_ref() {
            ...
            Some(Command::RemoteCommand(kuro_data::RemoteCommand {
                cache_hit: true,
                cache_hit_type,
                ..
            })) => {
                match kuro_data::CacheHitType::try_from(*cache_hit_type).unwrap() {
                    // ActionCache is 0, so this should be backwards compatible
                    kuro_data::CacheHitType::ActionCache => LastCommandExecutionKind::Cached,
                    kuro_data::CacheHitType::RemoteDepFileCache => {
                        LastCommandExecutionKind::RemoteDepFileCached
                    }
                    kuro_data::CacheHitType::Executed => LastCommandExecutionKind::Remote,
                }
            }
            Some(Command::RemoteCommand(kuro_data::RemoteCommand {
                cache_hit: false, ..
...
pub fn get_last_command_execution_time(action: &kuro_data::ActionExecutionEnd) -> ExecTime {
