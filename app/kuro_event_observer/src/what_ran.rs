...
impl<'a> WhatRanOptionsRegex<'a> {
    pub fn from_options(options: &'a WhatRanOptions) -> kuro_error::Result<Self> {
...
pub enum WhatRanRelevantAction {
    ActionExecution(kuro_data::ActionExecutionStart),
    TestDiscovery(kuro_data::TestDiscoveryStart),
    TestRun(kuro_data::TestRunStart),
    SetupLocalResources(kuro_data::SetupLocalResourcesStart),
}

impl WhatRanRelevantAction {
    /// Extract a relevant action from an event's data, if we can find one.
    pub fn from_buck_data(data: &kuro_data::buck_event::Data) -> Option<Self> {
        match data {
            kuro_data::buck_event::Data::SpanStart(span) => match &span.data {
                Some(kuro_data::span_start_event::Data::ActionExecution(action)) => {
                    Some(Self::ActionExecution(action.clone()))
                }
                Some(kuro_data::span_start_event::Data::TestDiscovery(suite)) => {
                    Some(Self::TestDiscovery(suite.clone()))
                }
                Some(kuro_data::span_start_event::Data::TestStart(test)) => {
                    Some(Self::TestRun(test.clone()))
                }
                Some(kuro_data::span_start_event::Data::LocalResources(setup)) => {
...
pub trait WhatRanOutputWriter {
    fn emit_command(&mut self, command: WhatRanOutputCommand<'_>) -> kuro_error::Result<()>;
}
...
pub enum CommandReproducer {
    CacheQuery(kuro_data::CacheQuery),
    CacheHit(kuro_data::CacheHit),
    LocalDepFileCacheHit,
    ReExecute(kuro_data::ReExecute),
    LocalExecute(kuro_data::LocalExecute),
    WorkerExecute(kuro_data::WorkerExecute),
    WorkerInit(kuro_data::WorkerInit),
}
...
impl CommandReproducer {
    pub fn executor(&self) -> String {
        match self {
            ...
            Self::CacheHit(cache) => {
                ...
                match kuro_data::CacheHitType::try_from(cache_type) {
                    Ok(kuro_data::CacheHitType::RemoteDepFileCache) => {
    ...
    pub fn from_buck_data(
        data: &kuro_data::buck_event::Data,
        options: &WhatRanOptions,
    ) -> Option<Self> {
        match data {
            kuro_data::buck_event::Data::SpanStart(span) => match &span.data {
                Some(kuro_data::span_start_event::Data::ExecutorStage(executor_stage)) => {
                    match &executor_stage.stage {
                        Some(kuro_data::executor_stage_start::Stage::CacheQuery(cache_hit))
                            if options.emit_cache_queries =>
                        {
                            return Some(CommandReproducer::CacheQuery(cache_hit.clone()));
                        }
                        Some(kuro_data::executor_stage_start::Stage::CacheHit(cache_hit))
                            if !options.skip_cache_hits =>
                        {
                            return Some(CommandReproducer::CacheHit(cache_hit.clone()));
                        }
                        Some(kuro_data::executor_stage_start::Stage::Re(re_stage))
                            if !options.skip_remote_executions =>
                        {
                            match &re_stage.stage {
                                Some(kuro_data::re_stage::Stage::Execute(execute)) => {
                        ...
                        Some(kuro_data::executor_stage_start::Stage::Local(local_stage)) => {
                            if !options.skip_local_executions {
                                match &local_stage.stage {
                                    ...
                                    Some(kuro_data::local_stage::Stage::WorkerExecute(
                                        worker_execute,
                                    )) => {
                                    ...
                                    Some(kuro_data::local_stage::Stage::WorkerInit(
                                        worker_init,
                                    )) => {
...
pub struct Command<'a> {
    env: &'a Vec<kuro_data::EnvironmentEntry>,
    argv: &'a Vec<String>,
}

impl<'a> From<&'a kuro_data::LocalCommand> for Command<'a> {
    fn from(command: &'a kuro_data::LocalCommand) -> Self {
        Command {
            env: &command.env,
            argv: &command.argv,
        }
...
impl<'a> From<&'a kuro_data::WorkerCommand> for Command<'a> {
    fn from(command: &'a kuro_data::WorkerCommand) -> Self {
        Command {
            env: &command.env,
            argv: &command.argv,
        }
...
impl<'a> From<&'a kuro_data::WorkerInitCommand> for Command<'a> {
    fn from(command: &'a kuro_data::WorkerInitCommand) -> Self {
        Command {
            env: &command.env,
            argv: &command.argv,
        }
...
pub fn worker_command_as_fallback_to_string(command: &kuro_data::WorkerCommand) -> String {
...
fn executor_with_platform(execute: &kuro_data::ReExecute) -> String {
...
mod tests {
    ...
    fn test_executor_with_platform_no_platform() {
        let execute = kuro_data::ReExecute::default();
        let result = executor_with_platform(&execute);
