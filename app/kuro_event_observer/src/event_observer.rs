    ...
    E: EventObserverExtra,
{
    pub fn new(trace_id: TraceId) -> Self {
        Self {
            ...
            system_info: kuro_data::SystemInfo::default(),
            session_info: SessionInfo {
    ...
    pub async fn observe(&mut self, event: &Arc<BuckEvent>) -> kuro_error::Result<()> {
                ...
                Instant(instant) => {
                        ...
                        .buck_error_context("Missing `data` in `Instant`")?
                    {
                        ...
                        TestDiscovery(discovery) => {
                            use kuro_data::test_discovery::Data::*;
    ...
    pub fn system_info(&self) -> &kuro_data::SystemInfo {
        &self.system_info
    }
...
pub trait EventObserverExtra: Send {
    ...
    fn observe(&mut self, event: &Arc<BuckEvent>) -> kuro_error::Result<()>;
}
...
impl EventObserverExtra for NoopEventObserverExtra {
    ...
    fn observe(&mut self, _event: &Arc<BuckEvent>) -> kuro_error::Result<()> {
        // Noop
        Ok(())
    }
}
