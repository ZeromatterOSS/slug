...
impl ReState {
    ...
    pub fn add_re_session(&mut self, session: &kuro_data::RemoteExecutionSessionCreated) {
        self.session_id = Some(session.session_id.clone());
    }

    pub fn update(&mut self, snapshot: &kuro_data::Snapshot) {
    ...
    fn render_detailed(&self, two_snapshots: &TwoSnapshots) -> kuro_error::Result<Vec<Line>> {
        let mut r = Vec::new();
        if let (Some(first), Some((_, last))) = (&self.first_snapshot, &two_snapshots.last) {
