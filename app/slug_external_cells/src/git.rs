...
impl IoRequest for GitFetchIoRequest {
    fn execute(
        self: Box<Self>,
        project_fs: &slug_core::fs::project::ProjectRoot,
    ) -> slug_error::Result<()> {
        ...
        fn run_git(cwd: &AbsNormPath, f: impl FnOnce(&mut Command)) -> slug_error::Result<()> {
...
pub(crate) async fn get_file_ops_delegate(
    ...
    cell: CellName,
    setup: GitCellSetup,
) -> slug_error::Result<Arc<GitFileOpsDelegate>> {
    #[derive(
        dupe::Dupe,
        Clone,
        Debug,
    ...
    impl Key for GitFileOpsDelegateKey {
        type Value = slug_error::Result<Arc<GitFileOpsDelegate>>;
