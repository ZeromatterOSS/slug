...
/// then you can fix forward by bumping the `slug.sqlite_materializer_state_version`
...
impl MaterializerStateSqliteDb {
    ...
    /// or the DB has a different set of versions than the versions this slug expects, we
    ...
    ) -> slug_error::Result<(Self, slug_error::Result<MaterializerState>)> {
        io_executor
            .execute_io_inline(|| {
...
mod tests {
    ...
    #[test]
    fn test_initialize_sqlite_db() -> slug_error::Result<()> {
        fn testing_metadatas() -> Vec<HashMap<String, String>> {
            let metadata = slug_events::metadata::collect(&DaemonId::new());
