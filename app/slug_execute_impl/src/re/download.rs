...
use slug_execute::directory::re_tree_to_directory;
use slug_execute::execute::action_digest::TrackedActionDigest;
        ...
        e => {
            let materialized_inputs = if materialize_failed_re_action_inputs {
                executor_stage_async(
                    slug_data::ReStage {
                        stage: Some(slug_data::MaterializeFailedInputs {}.into()),
                    },
                    async move {
...
impl CasDownloader<'_> {
    ...
    async fn extract_artifacts<'a>(
        ...
        requested_outputs: impl IntoIterator<Item = CommandExecutionOutputRef<'a>>,
        output_spec: &dyn RemoteActionResult,
    ) -> slug_error::Result<ExtractedArtifacts> {
        let now = Utc::now();
    ...
    ) -> slug_error::Result<IndexMap<CommandExecutionOutput, ArtifactValue>> {
...
fn re_forward_path(re_path: &str) -> slug_error::Result<&ForwardRelativePath> {
