...
use slug_execute::directory::directory_to_re_tree;
...
use slug_execute::execute::cache_uploader::UploadCache;
...
impl CacheUploader {
    ...
    async fn upload_local_outputs(
        ...
        span_async(
            ...
            async {
                ...
                .await
                .map_err(|e: slug_error::Error| slug_error::Error::from(e))
                .unwrap_or_else(CacheUploadOutcome::Failed);

                let cache_upload_end_event = slug_data::CacheUploadEnd {
                    key: Some(info.target.as_proto_action_key()),
                    name: Some(info.target.as_proto_action_name()),
                    action_digest: digest_str.clone(),
    ...
    async fn upload_dep_file(
        ...
        span_async(
            ...
            async {
                ...
                let end_event = slug_data::DepFileUploadEnd {
                    key: Some(info.target.as_proto_action_key()),
                    name: Some(info.target.as_proto_action_name()),
                    remote_dep_file_key: remote_dep_file_key.clone(),
                    success: outcome.uploaded(),
    ...
    async fn check_upload_permission(
        &self,
        info: &CacheUploadInfo<'_>,
    ) -> slug_error::Result<Result<(), CacheUploadOutcome>> {
    ...
    async fn upload_files_and_directories(
    ...
    ) -> slug_error::Result<Result<TActionResult2, CacheUploadRejectionReason>> {
        ...
        let uploads = async {
            slug_util::future::try_join_all(upload_futs)
                .await
    ...
    fn log_and_create_result(
        ...
        if !self.uploaded() && error_on_cache_upload {
            Err(slug_error::slug_error!(
                slug_error::ErrorTag::CacheUploadFailed,
                "cache_upload_failed"
            ))
        } else {
            Ok(self)
...
fn systemtime_to_ttimestamp(time: SystemTime) -> slug_error::Result<TTimestamp> {
