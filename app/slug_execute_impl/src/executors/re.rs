...
use slug_execute::execute::blobs::ActionBlobs;
use slug_execute::execute::kind::CommandExecutionKind;
use slug_execute::execute::kind::RemoteCommandExecutionDetails;
...
use slug_execute::execute::result::CommandExecutionErrorType;
...
use slug_execute::re::action_identity::ReActionIdentity;
use slug_execute::re::client::CancellationReason;
use slug_execute::re::client::ExecuteResponseOrCancelled;
use slug_execute::re::error::RemoteExecutionError;
use slug_execute::re::error::get_re_error_tag;
...
impl ReExecutor {
    async fn upload(
        ...
        let upload_response = span_async(slug_data::ReUploadStart {}, async move {
            let res = re_client
                .upload(
                    &self.project_fs,
                    &self.materializer,
            ...
            match res {
                ...
                Err(e) => (Err(e), slug_data::ReUploadEnd::default()),
            }
        })
        .await;
    ...
    ) -> ControlFlow<CommandExecutionResult, (CommandExecutionManager, ExecuteResponseWithQueueStats)>
    {
        ...
        let execute_response =
            if let Some(timeout) = slug_common::self_test_timeout::maybe_cap_timeout(None) {
...
#[slug(tier0, tag = get_re_error_tag(&inner.code))]
