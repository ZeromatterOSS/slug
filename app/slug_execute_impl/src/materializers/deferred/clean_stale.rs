...
use slug_common::liveliness_observer::LivelinessObserverSync;
...
use slug_data::CleanStaleResultKind;
use slug_data::CleanStaleStats;
...
pub struct CleanStaleArtifactsExtensionCommand {
    ...
    pub sender: Sender<BoxFuture<'static, slug_error::Result<CleanResult>>>,
}
...
enum PendingCleanResult {
    ...
    Pending(BoxFuture<'static, slug_error::Result<CleanResult>>),
}
...
impl From<CleanResult> for slug_cli_proto::CleanStaleResponse {
    fn from(result: CleanResult) -> Self {
        let message = match result.kind {
            ...
            CleanStaleResultKind::SkippedDeferWriteDisabled => {
                Some("Skipping clean, set slug.defer_write_actions to use clean --stale")
            }
            CleanStaleResultKind::SkippedSqliteDisabled => {
                Some("Skipping clean, set slug.sqlite_materializer_state to use clean --stale")
            }
...
fn create_result(
    result: Result<CleanResult, slug_error::Error>,
    trace_id: Option<TraceId>,
    daemon_id: &DaemonId,
    total_duration_s: u64,
) -> slug_data::CleanStaleResult {
...
impl CleanStaleArtifactsCommand {
    pub(super) fn create_clean_fut<T: IoHandler>(
        ...
        trace_id: Option<TraceId>,
        daemon_id: DaemonId,
    ) -> BoxFuture<'static, slug_error::Result<CleanResult>> {
        ...
        async move {
            ...
            let result: Result<CleanResult, slug_error::Error> = result.map_err(|e| e.into());
            let result_event: slug_data::CleanStaleResult = create_result(
                result.clone(),
                trace_id,
                &daemon_id,
    ...
    fn create_pending_clean_result<T: IoHandler>(
        &self,
        processor: &mut DeferredMaterializerCommandProcessor<T>,
    ) -> slug_error::Result<PendingCleanResult> {
    ...
    fn scan_and_create_clean_fut<T: IoHandler>(
        ...
        for (path, file_type) in found_paths
            ...
            .take(2000)
        {
            self.dispatcher.instant_event(slug_data::UntrackedFile {
                path: path.to_string(),
...
#[derive(Debug, Clone, slug_error::Error)]
...
pub(crate) struct CleanStaleError {
    db_size: usize,
    stats: slug_data::CleanStaleStats,
}

fn stats_for_paths(paths: &Vec<FoundPath>) -> slug_data::CleanStaleStats {
    let mut stats = slug_data::CleanStaleStats::default();
    for path in paths {
        match path {
...
fn create_clean_fut<T: IoHandler>(
    ...
    cancellations: &'static CancellationContext,
    liveliness_observer: Arc<dyn LivelinessObserverSync>,
) -> slug_error::Result<BoxFuture<'static, slug_error::Result<CleanResult>>> {
    let io = io.dupe();
    ...
    let fut = async move {
        ...
        let res = slug_util::future::try_join_all(
            found_paths
                .into_iter()
                .filter_map(|x| match x {
...
impl IoRequest for CleanInvalidatedPathRequest {
    fn execute(self: Box<Self>, project_fs: &ProjectRoot) -> slug_error::Result<()> {
        if !self.liveliness_observer.is_alive_sync() {
            return Err(slug_error!(ErrorTag::CleanInterrupt, "Interrupt").into());
        }
...
pub fn get_size(path: &AbsNormPath) -> slug_error::Result<u64> {
    let mut result = 0;
    if path.is_dir() {
...
impl CleanStaleConfig {
    pub fn from_buck_config(_root_config: &LegacyBuckConfig) -> slug_error::Result<Option<Self>> {
        Ok(None)
    }
}
