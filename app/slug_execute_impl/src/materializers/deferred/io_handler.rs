...
use slug_execute::digest::CasDigestFromReExt;
...
use slug_execute::materialize::http::http_download;
...
impl IoHandler for DefaultIoHandler {
    ...
    fn clean_path<'a>(
        ...
        self.io_executor
            ...
            .map(|r| r.map_err(slug_error::Error::from))
            .boxed()
    }
    ...
    async fn materialize_entry(
        ...
        let materialization_start = slug_data::MaterializationStart {
...
fn maybe_tombstone_digest(digest: &FileDigest) -> slug_error::Result<&FileDigest> {
    ...
    fn convert_digests(val: &str) -> slug_error::Result<HashSet<FileDigest>> {
        val.split(' ')
            .map(|digest| {
    ...
    let tombstoned_digests = slug_env!(
...
pub(super) fn create_ttl_refresh(
    ...
    min_ttl: Duration,
    digest_config: DigestConfig,
) -> Option<impl Future<Output = slug_error::Result<()>> + use<>> {
        ...
        for (use_case, digests_to_refresh) in digests_to_refresh {
            ...
            for chunk in digests_to_refresh.as_slice().chunks(REFRESH_CHUNK_SIZE) {
                ...
                let mut digests_expires = digests_expires.into_try_map(|(digest, expires)| {
                    slug_error::Ok((FileDigest::from_re(&digest, digest_config)?, expires))
                })?;
...
impl WriteIoRequest {
    fn execute_inner(&self, project_fs: &ProjectRoot) -> slug_error::Result<()> {
...
impl IoRequest for CleanIoRequest {
    fn execute(self: Box<Self>, project_fs: &ProjectRoot) -> slug_error::Result<()> {
        // NOTE: No spans here! We should perhaps add one, but this needs to be considered
        // carefully as it's a lot of spans, and we haven't historically emitted those for writes.
        let res = cleanup_path(project_fs, &self.path).map_err(slug_error::Error::from);
