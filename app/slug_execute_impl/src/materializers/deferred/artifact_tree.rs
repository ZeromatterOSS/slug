...
pub(crate) type CleaningFuture = Shared<BoxFuture<'static, slug_error::Result<()>>>;
    ...
    pub(crate) fn matches_entry(
        ...
        match (&self.0, entry) {
            ...
            (DirectoryEntry::Leaf(l1), DirectoryEntry::Leaf(l2)) => {
                // In Windows, the 'executable bit' absence can cause Slug to re-download identical artifacts.
...
pub(crate) trait MaterializationMethodToProto {
    fn to_proto(&self) -> slug_data::MaterializationMethod;
}
    ...
    fn to_proto(&self) -> slug_data::MaterializationMethod {
        match self {
            ArtifactMaterializationMethod::LocalCopy { .. } => {
                slug_data::MaterializationMethod::LocalCopy
            }
            ArtifactMaterializationMethod::CasDownload { .. } => {
                slug_data::MaterializationMethod::CasDownload
            }
            ArtifactMaterializationMethod::Write { .. } => slug_data::MaterializationMethod::Write,
            ArtifactMaterializationMethod::HttpDownload { .. } => {
                slug_data::MaterializationMethod::HttpDownload
            }
            #[cfg(test)]
...
impl ArtifactTree {
    ...
    ) -> slug_error::Result<Vec<(ProjectRelativePathBuf, ProcessingFuture)>> {
        ...
        #[cfg(test)]
        {
            ...
            for path in &invalidated_paths {
                if path.as_str() == "test/invalidate/failure" {
                    return Err(slug_error!(slug_error::ErrorTag::Tier0, "Injected error"));
                }
            }
        }
