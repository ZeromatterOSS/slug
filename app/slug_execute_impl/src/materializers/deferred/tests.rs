...
use slug_execute::directory::insert_file;
use slug_execute::materialize::materializer::DeferredMaterializerSubscription;
...
#[test]
fn test_find_artifacts() -> slug_error::Result<()> {
...
mod state_machine {
    ...
    use slug_fs::fs_util;
    use slug_fs::fs_util::ReadDir;
    use slug_fs::paths::RelativePathBuf;
    use slug_fs::paths::abs_norm_path::AbsNormPathBuf;
    use slug_fs::paths::forward_rel_path::ForwardRelativePath;
    use slug_util::threads::ignore_stack_overflow_checks_for_future;
    ...
    impl IoHandler for StubIoHandler {
        ...
        async fn immediate_write<'a>(
            self: &Arc<Self>,
            _gen: Box<dyn FnOnce() -> slug_error::Result<Vec<WriteRequest>> + Send + 'a>,
        ...
        fn clean_path<'a>(
            ...
            command_sender: Arc<MaterializerSender<Self>>,
            _cancellations: &'a CancellationContext,
        ) -> BoxFuture<'a, Result<(), slug_error::Error>> {
        ...
        async fn materialize_entry(
            ...
            if (*self.fail_paths.lock()).contains(&path) || *self.fail.lock() {
                ...
                Err(slug_error::slug_error!(
                    slug_error::ErrorTag::MaterializationError,
                    "Injected error"
                )
                .into())
            } else {
        ...
        fn create_ttl_refresh(
            ...
            _tree: &ArtifactTree,
            _min_ttl: Duration,
        ) -> Option<BoxFuture<'static, slug_error::Result<()>>> {
            unimplemented!()
        }
    ...
    fn make_processor_for_io(
        ...
        let (daemon_dispatcher_events, daemon_dispatcher_sink) =
            slug_events::create_source_sink_pair();
    ...
    #[tokio::test]
    async fn test_declare_reuse() -> slug_error::Result<()> {
    ...
    fn make_artifact_value_with_symlink_dep(
        ...
        digest_config: DigestConfig,
    ) -> slug_error::Result<ArtifactValue> {
    ...
    #[tokio::test]
    async fn test_materialize_symlink_and_target() -> slug_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
    ...
    #[tokio::test]
    async fn test_materialize_symlink_first_then_target() -> slug_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
    ...
    #[tokio::test]
    async fn test_subscription_subscribe_also_materializes() -> slug_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let (mut dm, mut channel) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
            let value = ArtifactValue::file(digest_config.empty_file());
    ...
    #[tokio::test]
    async fn test_invalidate_error() -> slug_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async{
            let (mut dm, _) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
    ...
    #[tokio::test]
    async fn test_materialize_dep_error() -> slug_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            // Construct a tree with a symlink and its target, materialize both at once
            let symlink_path = make_path("foo/bar_symlink");
            let target_path = make_path("foo/bar_target");
            ...
            dm.materialize_artifact(&symlink_path, EventDispatcher::null())
                .buck_error_context("Expected a future")?
                .await
                .map_err(|err| slug_error!(slug_error::ErrorTag::MaterializationError, "error materializing {:?}", err))?;
            assert_eq!(
                dm.io.take_log(),
                &[
            ...
            dm.materialize_artifact(&symlink_path, EventDispatcher::null())
                .buck_error_context("Expected a future")?
                .await
                .map_err(|err| slug_error!(slug_error::ErrorTag::MaterializationError, "error materializing 2 {:?}", err))?;
    ...
    #[tokio::test]
    async fn test_retry() -> slug_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let (mut dm, mut channel) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
    ...
    #[tokio::test]
    async fn test_clean_stale() -> slug_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
    ...
    #[tokio::test]
    async fn test_clean_stale_interrupt() -> slug_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let path = make_path("buck-out/v2/gen/foo/bar");
            let project_root = temp_root();
            let io = Arc::new(StubIoHandler::new(project_root.clone()));
    ...
    #[tokio::test]
    async fn test_clean_stale_schedule() -> slug_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let path = make_path("buck-out/v2/gen/foo/bar");
            let project_root = temp_root();
            ...
            let receive_clean_result = |events: &mut ChannelEventSource| {
                ...
                match event.unpack_buck().unwrap().data() {
                    slug_data::buck_event::Data::Instant(instant) => match instant.data.as_ref() {
                        Some(slug_data::instant_event::Data::CleanStaleResult(res)) => {
                            Some(res.clone())
                        }
                        _ => None,
                    },
            ...
            while i < 5 {
                ...
                if let slug_data::CleanStaleStats {
    ...
    #[tokio::test]
    async fn test_has_artifact_at() -> slug_error::Result<()> {
        ignore_stack_overflow_checks_for_future(async {
            let (mut dm, _) = make_processor(Default::default());
            let digest_config = dm.io.digest_config();
