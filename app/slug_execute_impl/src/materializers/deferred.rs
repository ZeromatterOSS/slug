...
use slug_directory::directory::directory_iterator::DirectoryIteratorPathStack;
use slug_directory::directory::entry::DirectoryEntry;
use slug_directory::directory::walk::unordered_entry_walk;
use slug_error::BuckErrorContext;
use slug_events::dispatch::EventDispatcher;
use slug_events::dispatch::current_span;
...
use slug_execute::materialize::materializer::ArtifactNotMaterializedReason;
use slug_execute::materialize::materializer::CasDownloadInfo;
use slug_execute::materialize::materializer::CasNotFoundError;
use slug_execute::materialize::materializer::CopiedArtifact;
use slug_execute::materialize::materializer::DeclareArtifactPayload;
use slug_execute::materialize::materializer::DeclareMatchOutcome;
use slug_execute::materialize::materializer::DeferredMaterializerExtensions;
use slug_execute::materialize::materializer::HttpDownloadInfo;
...
fn access_time_update_max_buffer_size() -> slug_error::Result<usize> {
    slug_env!("BUCK_ACCESS_TIME_UPDATE_MAX_BUFFER_SIZE", type=usize, default=5000)
}
...
pub enum AccessTimesUpdatesError {
    #[error(
        "Invalid value for buckconfig `[slug] update_access_times`. Got `{0}`. Expected one of `full`, `partial`  or `disabled`."
    )]
    InvalidValueForConfig(String),
}

impl AccessTimesUpdates {
    pub fn try_new_from_config_value(config_value: Option<&str>) -> slug_error::Result<Self> {
        match config_value {
...
struct TtlRefreshHistoryEntry {
    at: DateTime<Utc>,
    outcome: Option<slug_error::Result<()>>,
}
...
impl From<slug_error::Error> for MaterializeEntryError {
    fn from(e: slug_error::Error) -> MaterializeEntryError {
        Self::Error(e)
    }
}
...
impl<T: IoHandler + Allocative> Materializer for DeferredMaterializerAccessor<T> {
    ...
    async fn declare_write<'a>(
        &self,
        generate: Box<dyn FnOnce() -> slug_error::Result<Vec<WriteRequest>> + Send + 'a>,
    ) -> slug_error::Result<Vec<ArtifactValue>> {
    ...
    ) -> slug_error::Result<DeclareMatchOutcome> {
    ...
    async fn has_artifact_at(&self, path: ProjectRelativePathBuf) -> slug_error::Result<bool> {
        let (sender, recv) = oneshot::channel();
    ...
    async fn invalidate_many(&self, paths: Vec<ProjectRelativePathBuf>) -> slug_error::Result<()> {
        let (sender, recv) = oneshot::channel();
        ...
        invalidate_fut.await.map_err(slug_error::Error::from)
    }

    async fn materialize_many(
        &self,
        artifact_paths: Vec<ProjectRelativePathBuf>,
    ) -> slug_error::Result<BoxStream<'static, Result<(), MaterializationError>>> {
    ...
    ) -> slug_error::Result<Vec<Result<ProjectRelativePathBuf, ArtifactNotMaterializedReason>>>
    {
        if paths.is_empty() {
            return Ok(Vec::new());
        }
    ...
    fn add_snapshot_stats(&self, snapshot: &mut slug_data::Snapshot) {
...
impl DeferredMaterializerAccessor<DefaultIoHandler> {
    ...
    pub fn new(
        ...
        let materializer_state_info = slug_data::MaterializerStateInfo {
        ...
        let command_thread = thread_spawn("slug-dm", {
            move || {
