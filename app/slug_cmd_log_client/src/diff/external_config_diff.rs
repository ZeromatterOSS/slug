...
fn insert_config_value(dict: &mut BTreeMap<String, String>, config: &slug_data::ConfigValue) {
...
fn process_buckconfig_data(mut dict: &mut BTreeMap<String, String>, event: &slug_data::BuckEvent) {
    use slug_data::buckconfig_component::Data::ConfigFile;
    use slug_data::buckconfig_component::Data::ConfigValue;
    use slug_data::buckconfig_component::Data::GlobalExternalConfigFile;
    use slug_data::config_file::Data::GlobalExternalConfig;
    use slug_data::config_file::Data::ProjectRelativePath;

    event.data.as_ref().into_iter().for_each(|data| match data {
        slug_data::buck_event::Data::Instant(end) => {
            end.data.as_ref().into_iter().for_each(|data| match data {
                slug_data::instant_event::Data::BuckconfigInputValues(input) => input
                    .components
                    .iter()
...
) -> slug_error::Result<BTreeMap<String, String>> {
...
impl BuckSubcommand for ExternalConfigDiffCommand {
    ...
    async fn exec_impl(
        ...
        slug_client_ctx::println!("{}", json_diffs)?;
        ExitResult::success()
    }
}
