...
fn get_action_execution_data(
    ...
    event.data.as_ref().and_then(|data| match data {
        slug_data::buck_event::Data::SpanEnd(end) => {
                ...
                slug_data::span_end_event::Data::ActionExecution(data) => {
...
) -> slug_error::Result<LinkedHashMap<ActionKey, ActionExecutionData>> {
...
fn print_divergence_msg(
    ...
    slug_client_ctx::println!("{}", output.join("\n"))?;
...
impl BuckSubcommand for ActionDivergenceCommand {
    ...
    async fn exec_impl(
        ...
        if !divergence_found {
            slug_client_ctx::println!("No divergent actions found.")?;
        }
        ExitResult::success()
    }
}
