    ...
    while let Some(event) = events.try_next().await? {
        match event {
            StreamValue::Event(event) => match event.data {
                Some(slug_data::buck_event::Data::Instant(instant)) => match instant.data {
                    Some(slug_data::instant_event::Data::BuildGraphInfo(build_graph)) => {
                        match path_kind {
                            PathKind::Critical => {
...
async fn log_critical_path(
    path: &Vec<slug_data::CriticalPathEntry2>,
    format: LogCommandOutputFormat,
) -> slug_error::Result<()> {
    ...
    slug_client_ctx::stdio::print_with_writer::<slug_error::Error, _>(async move |w| {
        ...
        for entry in path {
            use slug_data::critical_path_entry2::Entry;
            ...
            critical_path.name = match entry_data {
                Entry::Analysis(analysis) => {
                    use slug_data::critical_path_entry2::analysis::Target;
                ...
                Entry::DynamicAnalysis(analysis) => {
                    use slug_data::critical_path_entry2::dynamic_analysis::Target;
                ...
                Entry::ActionExecution(action_execution) => {
                    use slug_data::critical_path_entry2::action_execution::Owner;
                    ...
                    critical_path.execution_kind = Some(
                        slug_data::ActionExecutionKind::try_from(action_execution.execution_kind)
                            .unwrap_or(slug_data::ActionExecutionKind::NotSet)
                            .as_str_name(),
                    );
                ...
                Entry::FinalMaterialization(materialization) => {
                    use slug_data::critical_path_entry2::final_materialization::Owner;
