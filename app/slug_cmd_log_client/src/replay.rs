...
use slug_client_ctx::common::ui::get_console_with_root;
use slug_client_ctx::daemon::client::NoPartialResultHandler;
use slug_client_ctx::event_log_options::EventLogOptions;
use slug_client_ctx::events_ctx::DaemonEventsCtx;
use slug_client_ctx::events_ctx::EventsCtx;
use slug_client_ctx::exit_result::ExitResult;
use slug_client_ctx::signal_handler::with_simple_sigint_handler;
use slug_client_ctx::subscribers::superconsole::timekeeper::Clock;
use slug_client_ctx::subscribers::superconsole::timekeeper::Timekeeper;
use slug_client_ctx::subscribers::superconsole::timekeeper::duration_between_timestamps;
use slug_client_ctx::ticker::Tick;
...
impl BuckSubcommand for ReplayCommand {
    ...
    async fn exec_impl(
        ...
        if !speed.is_finite() || speed <= 0.0 {
            return ExitResult::from(slug_error::Error::from(ReplayError::InvalidSpeed(speed)));
        }

        if !seek.is_finite() || seek < 0.0 {
            return ExitResult::from(slug_error::Error::from(ReplayError::InvalidSeek(seek)));
        }
        ...
        let work = async {
            ...
            let res = res??;
            for e in &res.errors {
                slug_client_ctx::eprintln!("{}", e.message)?;
            }
...
impl TryFrom<slug_cli_proto::command_result::Result> for ReplayResult {
    type Error = slug_cli_proto::command_result::Result;

    fn try_from(v: slug_cli_proto::command_result::Result) -> Result<Self, Self::Error> {
        use slug_cli_proto::command_result::Result;
...
async fn make_replayer(
    ...
    impl Stream<Item = slug_error::Result<StreamValue>> + Unpin,
    Invocation,
    Timekeeper,
)> {
...
async fn replay_events_into(
    sink: UnboundedSender<slug_error::Result<StreamValue>>,
    events: impl Stream<Item = slug_error::Result<StreamValue>>,
    syncher: Syncher,
    mut speed_update_requests: mpsc::Receiver<SpeedUpdateRequest>,
    first_event: slug_error::Result<StreamValue>,
...
async fn find_next_event_with_delay(
    sink: &UnboundedSender<slug_error::Result<StreamValue>>,
    events: &mut (impl Stream<Item = slug_error::Result<StreamValue>> + Unpin),
    min_timestamp: Option<prost_types::Timestamp>,
) -> Option<(slug_error::Result<StreamValue>, prost_types::Timestamp)> {
