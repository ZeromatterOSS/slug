...
use slug_client_ctx::subscribers::recorder::process_memory;
...
use slug_event_observer::humanized::HumanizedBytesPerSecond;
...
impl Stats {
    fn update_with_event(&mut self, event: &slug_data::BuckEvent) {
        match &event.data {
            Some(slug_data::buck_event::Data::SpanEnd(end)) => match end.data.as_ref() {
                Some(slug_data::span_end_event::Data::ReUpload(data)) => {
                    self.total_bytes_uploaded += data.bytes_uploaded.unwrap_or_default();
                }
                Some(slug_data::span_end_event::Data::Materialization(data)) => {
                    self.total_files_materialized += data.file_count;
                    self.total_bytes_materialized += data.total_bytes;
                }
                Some(slug_data::span_end_event::Data::ActionExecution(data)) => {
                ...
                Some(slug_data::span_end_event::Data::Command(_command)) => {
                    self.duration = end.duration;
                }
                _ => {}
            },
            Some(slug_data::buck_event::Data::Instant(instant_event)) => {
                match instant_event.data.as_ref() {
                    ...
                    Some(slug_data::instant_event::Data::SystemInfo(system_info)) => {
                    ...
                    Some(slug_data::instant_event::Data::VersionControlRevision(vcs)) => {
...
fn get_event_timestamp(event: &slug_data::BuckEvent) -> Option<SystemTime> {
    ...
    async fn exec_impl(
        ...
        match self.format {
            SummaryFormat::Table => {
                ...
                slug_client_ctx::println!("build ID: {}", invocation.trace_id)?;
                slug_client_ctx::println!("{}", stats)?;
            ...
            SummaryFormat::Json => {
                let json = build_summary_to_json(&invocation.trace_id.to_string(), &summary)
                    .map_err(|e| {
                        slug_error::conversion::from_any_with_tag(e, slug_error::ErrorTag::Tier0)
                    })?;
                slug_client_ctx::println!("{}", json)?;
            }
            SummaryFormat::Csv => {
                let csv = build_summary_to_csv(&summary).map_err(|e| {
                    slug_error::conversion::from_any_with_tag(e, slug_error::ErrorTag::Tier0)
                })?;
                slug_client_ctx::println!("{}", csv)?;
            }
        }
        ExitResult::success()
    }
