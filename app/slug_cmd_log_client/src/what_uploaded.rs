...
fn get_action_record(
    state: &HashMap<u64, slug_data::ActionExecutionStart>,
...
struct ReUploadEvent<'a> {
    ...
    pub inner: &'a slug_data::ReUploadEnd,
}
            ...
            while let Some(event) = events.try_next().await? {
                match event {
                    ...
                    StreamValue::Event(event) => match event.data {
                        Some(slug_data::buck_event::Data::SpanStart(start)) => match start.data {
                        ...
                        Some(slug_data::buck_event::Data::SpanEnd(end)) => {
                            match end.data.as_ref() {
                                Some(slug_data::span_end_event::Data::ReUpload(u)) => {
