...
fn get_record(materialization: &slug_data::MaterializationEnd) -> Record {
    let method = match materialization
        .method
        .and_then(|v| slug_data::MaterializationMethod::try_from(v).ok())
    {
        Some(slug_data::MaterializationMethod::CasDownload) => "cas",
        Some(slug_data::MaterializationMethod::LocalCopy) => "copy",
        Some(slug_data::MaterializationMethod::HttpDownload) => "http",
        Some(slug_data::MaterializationMethod::Write) => "write",
        _ => "<unknown>",
    };
    Record {
            ...
            while let Some(event) = events.try_next().await? {
                match event {
                    StreamValue::Event(event) => match &event.data {
                        Some(slug_data::buck_event::Data::SpanEnd(slug_data::SpanEndEvent {
                            data: Some(slug_data::span_end_event::Data::Materialization(m)),
                            ..
                        })) if m.success =>
