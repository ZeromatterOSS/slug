...
/// Robots, please use `slug log show`.
...
impl BuckSubcommand for WhatCmdCommand {
    ...
    async fn exec_impl(
        ...
        slug_client_ctx::println!("# cd {}", invocation.working_dir)?;
        if expand {
            slug_client_ctx::println!("{}", invocation.display_expanded_command_line())?;
        } else {
            slug_client_ctx::println!("{}", invocation.display_command_line())?;
        }
        ExitResult::success()
    }
}
