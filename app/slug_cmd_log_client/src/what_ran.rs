...
/// (if unsure, use `slug root --kind project` to find it), then run the command. The command is
...
pub struct WhatRanCommand {
    ...
    /// That is command were running if slug process was killed,
    /// or command currently running if slug is running build now.
    #[clap(long)]
    pub incomplete: bool,
...
impl BuckSubcommand for WhatRanCommand {
    ...
    async fn exec_impl(
        ...
        slug_client_ctx::stdio::print_with_writer::<slug_error::Error, _>(async move |w| {
...
fn should_emit_finished_action(
    data: &Option<slug_data::span_end_event::Data>,
    options: &WhatRanCommandOptions,
) -> bool {
        ...
        match &mut self.format {
            ...
            LogCommandOutputFormatWithWriter::Json(w) => {
                let reproducer = match &command.repro {
                    ...
                    CommandReproducer::CacheHit(cache_hit) => match cache_hit.cache_type() {
                        slug_data::CacheType::ActionCache => JsonReproducer::Cache {
                            digest: &cache_hit.action_digest,
            ...
            LogCommandOutputFormatWithWriter::Csv(writer) => {
                ...
                writer
                    ...
                    .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::LogCmd))?;
                Ok(())
            }
        }
    }
...
fn into_index_map(platform: &Option<slug_data::RePlatform>) -> IndexMap<&str, &str> {
...
mod tests {
    ...
    #[test]
    fn serialize_what_ran_command_no_extr() -> slug_error::Result<()> {
  ...
  "duration": "1"
    ...
    #[test]
    fn serialize_what_ran_command_with_extra() -> slug_error::Result<()> {
  ...
  "extra": {
    ...
    #[test]
    fn serialize_what_ran_command_in_re() -> slug_error::Result<()> {
