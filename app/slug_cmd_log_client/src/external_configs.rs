            ...
            match event {
                StreamValue::Event(event) => match event.data {
                    Some(slug_data::buck_event::Data::Instant(instant)) => match instant.data {
                        Some(slug_data::instant_event::Data::BuckconfigInputValues(configs)) => {
...
fn write_config_value<'a>(
    config_value: &'a slug_data::ConfigValue,
    origin_path: &'a str,
...
async fn log_external_configs(
    components: &[slug_data::BuckconfigComponent],
    format: LogCommandOutputFormat,
) -> slug_error::Result<()> {
    slug_client_ctx::stdio::print_with_writer::<slug_error::Error, _>(async move |w| {
        ...
        for component in components {
            use slug_data::buckconfig_component::Data;
            use slug_data::config_file::Data as CData;
