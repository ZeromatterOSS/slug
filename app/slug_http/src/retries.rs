...
where
    Exec: Fn() -> F,
    E: AsSlugError + HttpErrorForRetry + std::fmt::Display + Send + Sync + 'static,
        ...
        if err.is_retryable() {
            if let Some(b) = backoff.peek() {
                ...
                tracing::warn!(
                    ...
                    b.as_secs(),
                    // Print as a slug_error to make sure we get the source
                    err.as_slug_error()
                );
                continue;
            }
...
mod tests {
    ...
    impl AsSlugError for HttpTestError {
        fn as_slug_error(self) -> slug_error::Error {
    ...
    fn ok_response() -> Result<String, HttpTestError> {
        Ok("Success".to_owned()).map_err(|_: slug_error::Error| test_error(StatusCode::IM_A_TEAPOT))
    }
