...
use slug_interpreter_for_build::nodes::attr_spec::AttributeSpecExt;
...
#[test]
fn rule_creates_callable() -> slug_error::Result<()> {
...
#[test]
fn udr_is_recorded() -> slug_error::Result<()> {
    let content = indoc!(
        r#"
        def impl(ctx):
            pass
...
#[test]
fn option_allows_none() -> slug_error::Result<()> {
    let mut tester = rule_tester();
...
#[test]
fn returns_documentation() -> slug_error::Result<()> {
    let bzl = indoc::indoc!(
        r#"def impl(ctx):
            pass
