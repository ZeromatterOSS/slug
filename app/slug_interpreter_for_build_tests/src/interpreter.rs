...
use slug_interpreter_for_build::interpreter::testing::CellsData;
...
fn test_root_import() {
    let mut tester = Tester::with_cells(
        slug_interpreter_for_build::interpreter::testing::cells(Some(indoc!(
            r#"
            [buildfile]
...
#[test]
fn test_package_import() -> slug_error::Result<()> {
    let mut tester = Tester::with_cells(slug_interpreter_for_build::interpreter::testing::cells(
        Some(indoc!(
            r#"
            [buildfile]
...
#[test]
fn test_builtins() -> slug_error::Result<()> {
    ...
    // appear at the top level. `slug_fail` is registered in
    // `register_all_internals` (slug_interpreter_for_build::interpreter::functions::internals).
    let mut tester = Tester::new().unwrap();
