...
use slug_interpreter_for_build::interpreter::context::SetInterpreterContext;
...
/// Test mixed Bazel and Slug style in same rule
#[tokio::test]
async fn test_mixed_bazel_slug_style() {
    let fs = ProjectRootTemp::new().unwrap();

    // Mix Bazel's attr.* with Slug's attrs.* in same rule definition
    fs.write_file(
        "rules.bzl",
        indoc!(
            r#"
                ...
                # Mix attr.* (Bazel) and attrs.* (Slug) in same file
                mixed_rule = rule(
                    implementation = _impl,  # Bazel-style parameter
                    attrs = {
                        "bazel_deps": attr.label_list(default = []),      # Bazel-style
                        "slug_deps": attrs.list(attrs.dep(), default = []), # Slug-style
                        "bazel_str": attr.string(default = ""),           # Bazel-style
                        "slug_str": attrs.string(default = ""),           # Slug-style
                    },
                )
            "#
        ),
