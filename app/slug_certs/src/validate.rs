...
#[slug(environment, tag = NoValidCerts)]
...
async fn verify(path: &OsString) -> slug_error::Result<()> {
...
pub async fn validate_certs() -> slug_error::Result<()> {
...
pub async fn check_cert_state(cert_state: CertState) -> Option<slug_error::Error> {
