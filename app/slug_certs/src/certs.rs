...
/// certificate store.
async fn load_system_root_certs() -> slug_error::Result<RootCertStore> {
    let root_certs = if let Some(path) = find_root_ca_certs() {
        ...
        if !native_certs_results.certs.is_empty() {
            ...
            let native_certs_error = native_certs_results
                .errors
                .pop()
                .map(slug_error::Error::from)
                .unwrap_or(slug_error!(
                    slug_error::ErrorTag::NoValidCerts,
            ...
            if cfg!(fbcode_build) {
                ...
                let context = format!(
                    ...
                    Please try `getchef -reason 'chef broken'`{windows_message}, `Fix My <OS>` via the f-menu, then `slug killall`.
...
async fn load_key<P: AsRef<Path>>(key: P) -> slug_error::Result<PrivateKeyDer<'static>> {
    let key = key.as_ref();
...
async fn load_cert_pair<P: AsRef<Path>>(
    cert: P,
    key: P,
) -> slug_error::Result<(Vec<CertificateDer<'static>>, PrivateKeyDer<'static>)> {
...
pub async fn tls_config_with_system_roots() -> slug_error::Result<ClientConfig> {
...
pub async fn tls_config_with_single_cert<P: AsRef<Path>>(
    cert_path: P,
    key_path: P,
) -> slug_error::Result<ClientConfig> {
    let system_roots = load_system_root_certs().await?;
    ...
    ClientConfig::builder()
        ...
        .map_err(|e| from_any_with_tag(e, slug_error::ErrorTag::Certs))
...
pub(crate) async fn load_certs<P: AsRef<Path>>(
    cert_path: P,
) -> slug_error::Result<Vec<CertificateDer<'static>>> {
