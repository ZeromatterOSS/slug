...
use kuro_execute_local::StdRedirectPaths;
...
use kuro_execute_local::status_decoder::MiniperfStatusDecoder;
use kuro_forkserver_proto::CommandRequest;
use kuro_forkserver_proto::RequestEvent;
use kuro_forkserver_proto::SetLogFilterRequest;
use kuro_forkserver_proto::SetLogFilterResponse;
use kuro_forkserver_proto::forkserver_server::Forkserver;
use kuro_fs::fs_util;
...
type RunStream =
    Pin<Box<dyn Stream<Item = Result<kuro_forkserver_proto::CommandEvent, Status>> + Send>>;

struct ValidatedCommand {
    exe: PathBuf,
    argv: Vec<Vec<u8>>,
    env: Vec<kuro_forkserver_proto::EnvDirective>,
    cwd: PathBuf,
...
impl ValidatedCommand {
    fn try_from(cmd_request: CommandRequest) -> kuro_error::Result<Self> {
        let CommandRequest {
            exe,
            argv,
            env,
...
impl UnixForkserverService {
    ...
    ) -> kuro_error::Result<CommandRequest> {
    ...
    fn configure_environment(
        ...
        env_directives: &[kuro_forkserver_proto::EnvDirective],
    ) -> kuro_error::Result<()> {
        use kuro_forkserver_proto::env_directive::Data;
    ...
    ) -> kuro_error::Result<(Command, Option<AbsNormPathBuf>)> {
    ...
    async fn create_command_stream(
        cmd: Command,
        validated_cmd: ValidatedCommand,
        cancellation: impl futures::Future<Output = kuro_error::Result<GatherOutputStatus>>
        + Send
        + Unpin
        + 'static,
        miniperf_output: Option<AbsNormPathBuf>,
        freeze_rx: impl ActionFreezeEventReceiver,
    ) -> kuro_error::Result<RunStream> {
            ...
            tokio::spawn(async move {
                ...
                while let Some(m) = stream.next().await {
                    let err = match m {
                        Ok(m) => match m.data {
                            Some(d) => match d {
                                kuro_forkserver_proto::request_event::Data::CommandRequest(_) => {
                                    kuro_error::internal_error!("Non-leading command request")
                                }
                                kuro_forkserver_proto::request_event::Data::CancelRequest(_) => {
                                    drop(cancel_tx.send(Ok(GatherOutputStatus::Cancelled)));
                                    break;
                                }
                                kuro_forkserver_proto::request_event::Data::FreezeRequest(_) => {
                                    drop(freeze_tx.send(ActionFreezeEvent::Freeze));
                                    continue;
                                }
                                kuro_forkserver_proto::request_event::Data::UnfreezeRequest(_) => {
...
impl MiniperfContainer {
    fn new(forkserver_state_dir: &AbsNormPath) -> kuro_error::Result<Option<Self>> {
        let miniperf_bin: Option<&'static [u8]>;

        #[cfg(all(fbcode_build, target_os = "linux"))]
        {
            miniperf_bin = Some(kuro_miniperf_data::get());
        }
