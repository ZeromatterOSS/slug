...
struct ForkserverClientInner {
    ...
    #[allocative(skip)]
    error: Arc<ArcSwapOption<kuro_error::Error>>,
    pid: u32,
    #[allocative(skip)]
    rpc: kuro_forkserver_proto::forkserver_client::ForkserverClient<Channel>,
}

impl ForkserverClient {
    pub(crate) async fn new(mut child: Child, channel: Channel) -> kuro_error::Result<Self> {
        let rpc = kuro_forkserver_proto::forkserver_client::ForkserverClient::new(channel)
        ...
        tokio::task::spawn(kuro_util::async_move_clone!(error, {
            let err = match child.wait().await {
                Ok(status) => ForkserverError::Exited(status),
                Err(e) => ForkserverError::WaitError(e),
            };

            let err = kuro_error::Error::from(err).context("Forkserver is unavailable");
    ...
    pub async fn execute<C>(
        &self,
        req: kuro_forkserver_proto::CommandRequest,
        cancel: C,
        ...
        C: Future<Output = ()> + Send + 'static,
    {
        ...
        let cancel_stream = stream::once(cancel.map(|()| kuro_forkserver_proto::RequestEvent {
            data: Some(kuro_forkserver_proto::CancelRequest {}.into()),
        }));
        let freeze_stream = freeze_rx.map(|e| {
            let data = match e {
                ActionFreezeEvent::Freeze => kuro_forkserver_proto::FreezeRequest {}.into(),
                ActionFreezeEvent::Unfreeze => kuro_forkserver_proto::UnfreezeRequest {}.into(),
            };
            kuro_forkserver_proto::RequestEvent { data: Some(data) }
        });

        let stream = stream::once(future::ready(kuro_forkserver_proto::RequestEvent {
            data: Some(req.into()),
        }))
    ...
    pub async fn set_log_filter(&self, log_filter: String) -> kuro_error::Result<()> {
        self.inner
            .rpc
            .clone()
            .set_log_filter(Request::new(kuro_forkserver_proto::SetLogFilterRequest {
                log_filter,
            }))
            .await?;
