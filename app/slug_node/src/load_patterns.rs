...
use slug_common::file_ops::trait_::DiceFileOps;
use slug_common::pattern::package_roots::collect_package_roots;
...
struct Builder<'c, 'd> {
    ...
    load_package_futs:
        FuturesUnordered<BoxFuture<'c, (PackageLabel, slug_error::Result<Arc<EvaluationResult>>)>>,
}
...
async fn resolve_patterns_and_load_buildfiles<'c, T: PatternType>(
    ...
    ResolvedPattern<T>,
    impl Stream<Item = (PackageLabel, slug_error::Result<Arc<EvaluationResult>>)> + use<'c, T>,
)> {
...
pub struct LoadedPatterns<T: PatternType> {
    results: BTreeMap<PackageLabelWithModifiers, slug_error::Result<PackageLoadedPatterns<T>>>,
}
...
impl<T: PatternType> LoadedPatterns<T> {
    pub fn iter(
        ...
        Item = (
            ...
            &slug_error::Result<PackageLoadedPatterns<T>>,
        ),
    > {
    ...
    pub fn iter_loaded_targets(
        &self,
    ) -> impl Iterator<Item = slug_error::Result<TargetNodeRef<'_>>> {
        self.results
            .values()
...
fn apply_spec<T: PatternType>(
    ...
    load_results: BTreeMap<PackageLabel, slug_error::Result<Arc<EvaluationResult>>>,
    skip_missing_targets: MissingTargetBehavior,
) -> slug_error::Result<LoadedPatterns<T>> {
    let mut all_targets: BTreeMap<_, slug_error::Result<PackageLoadedPatterns<T>>> =
        BTreeMap::new();
...
pub async fn load_patterns<T: PatternType>(
    ...
    ctx.with_linear_recompute(|ctx| async move {
        ...
        let mut results: BTreeMap<PackageLabel, slug_error::Result<Arc<EvaluationResult>>> =
            BTreeMap::new();
        while let Some((pkg, load_res)) = load_package_futs.next().await {
            results.insert(pkg, load_res.map_err(slug_error::Error::from));
        }
