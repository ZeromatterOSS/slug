...
use slug_query::query::syntax::simple::eval::label_indexed::LabelIndexed;
use slug_query::query::syntax::simple::eval::set::TargetSet;
use slug_util::late_binding::LateBinding;
use slug_util::self_ref::RefData;
use slug_util::self_ref::SelfRef;
...
pub static UNIVERSE_FROM_LITERALS: LateBinding<
    for<'c> fn(
        ...
        -> Pin<Box<dyn Future<Output = slug_error::Result<CqueryUniverse>> + Send + 'c>>,
...
impl<'a> CqueryUniverseInner<'a> {
    ...
    ) -> slug_error::Result<CqueryUniverseInner<'a>> {
...
impl CqueryUniverse {
    ...
    pub fn build(universe: &TargetSet<ConfiguredTargetNode>) -> slug_error::Result<CqueryUniverse> {
        span(slug_data::CqueryUniverseBuildStart {}, || {
            let r = SelfRef::try_new(universe.clone(), |universe| {
                CqueryUniverseInner::build_inner(universe)
            })
            .map(|data| CqueryUniverse { data });
            (r, slug_data::CqueryUniverseBuildEnd {})
        })
    }
    ...
    pub fn owners(&self, path: &CellPath) -> slug_error::Result<Vec<ConfiguredTargetNode>> {
...
mod tests {
    use dupe::Dupe;
    use slug_common::pattern::resolve::ResolvedPattern;
    use slug_core::configuration::bound_label::BoundConfigurationLabel;
    use slug_core::configuration::data::ConfigurationData;
    use slug_core::configuration::hash::ConfigurationHash;
    ...
    use slug_query::__derive_refs::indexmap::IndexMap;
    use slug_query::query::syntax::simple::eval::set::TargetSet;
    ...
    async fn test_get_from_package_by_configured_provider_pattern() {
        fn providers_name() -> ProvidersName {
            ProvidersName::NonDefault(triomphe::Arc::new(NonDefaultProvidersName::Named(
                slug_util::arc_str::ArcSlice::new([ProviderName::new("P".to_owned()).unwrap()]),
            )))
        }
