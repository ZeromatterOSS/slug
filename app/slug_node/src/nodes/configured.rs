...
impl ConfiguredTargetNode {
    ...
    pub fn new_forward(
        ...
        Ok(ConfiguredTargetNode(triomphe::Arc::new(Hashed::new(
            ConfiguredTargetNodeData {
                ...
                // so we can call `slug test` on Forward.
    ...
    pub fn attr_as_target_compatible_with(
        attr: ConfiguredAttr,
    ) -> impl Iterator<Item = slug_error::Result<ConfigurationSettingKey>> {
    ...
    pub fn tests(&self) -> impl Iterator<Item = ConfiguredProvidersLabel> + use<> {
        ...
        impl ConfiguredAttrTraversal for TestCollector {
            ...
            fn label(&mut self, label: &ConfiguredProvidersLabel) -> slug_error::Result<()> {
    ...
    #[inline]
    pub fn execution_platform(&self) -> slug_error::Result<&ExecutionPlatform> {
