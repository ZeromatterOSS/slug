...
impl ConfiguredAttr {
    ...
    fn concat_not_supported(&self, attr_ty: &'static str) -> slug_error::Error {
        slug_error!(
    ...
    fn unpack_oneof(self) -> slug_error::Result<(Self, u32)> {
        match self {
    ...
    fn unpack_oneof_i(self, expected_i: u32, oneof: &OneOfAttrType) -> slug_error::Result<Self> {
    ...
    fn concat_oneof(
        self,
        items: &mut dyn Iterator<Item = slug_error::Result<Self>>,
        oneof: &OneOfAttrType,
    ...
    pub(crate) fn try_into_configuration_dep(self) -> slug_error::Result<ProvidersLabel> {
        match self {
    ...
    pub(crate) fn try_into_list(self) -> slug_error::Result<Vec<ConfiguredAttr>> {
        match self {
