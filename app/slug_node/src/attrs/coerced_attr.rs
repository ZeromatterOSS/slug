...
impl CoercedSelector {
    ...
    ) -> slug_error::Result<CoercedSelector> {
    ...
    fn check_all_keys_unique(
        ...
        fn duplicate_key(key: &ConfigurationSettingKey) -> slug_error::Error {
            slug_error!(
    ...
    pub fn to_json(&self, ctx: &AttrFmtContext) -> slug_error::Result<serde_json::Value> {
...
impl CoercedAttr {
    ...
    pub fn select_the_most_specific<'a, 'x>(
        ...
        >,
    ) -> slug_error::Result<Option<&'a CoercedAttr>> {
    ...
    ) -> slug_error::Result<&'a CoercedAttr> {
        let CoercedSelector {
            entries, default, ..
        } = select;
