...
use kuro_query::query::syntax::simple::eval::label_indexed::LabelIndexed;
...
pub static UNIVERSE_FROM_LITERALS: LateBinding<
    for<'c> fn(
        ...
        -> Pin<Box<dyn Future<Output = kuro_error::Result<CqueryUniverse>> + Send + 'c>>,
...
impl<'a> CqueryUniverseInner<'a> {
    ...
    ) -> kuro_error::Result<CqueryUniverseInner<'a>> {
...
impl CqueryUniverse {
    ...
    pub fn build(
        ...
        span(kuro_data::CqueryUniverseBuildStart {}, || {
            ...
            (r, kuro_data::CqueryUniverseBuildEnd {})
        })
    }
    ...
    pub fn owners(&self, path: &CellPath) -> kuro_error::Result<Vec<ConfiguredTargetNode>> {
...
mod tests {
    ...
    use kuro_core::configuration::hash::ConfigurationHash;
    ...
    use kuro_core::pattern::pattern::PackageSpec;
    use kuro_core::pattern::pattern_type::ConfigurationPredicate;
    ...
    async fn test_get_from_package_by_configured_provider_pattern() {
        fn providers_name() -> ProvidersName {
            ProvidersName::NonDefault(triomphe::Arc::new(NonDefaultProvidersName::Named(
                kuro_util::arc_str::ArcSlice::new([ProviderName::new("P".to_owned()).unwrap()]),
            )))
        }
