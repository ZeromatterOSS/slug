...
pub trait CoercedAttrTraversal<'a> {
    ...
    fn exec_dep(&mut self, dep: &'a ProvidersLabel) -> kuro_error::Result<()> {
        self.dep(dep)
    }

    fn toolchain_dep(&mut self, dep: &'a ProvidersLabel) -> kuro_error::Result<()> {
        self.dep(dep)
    }
    ...
    fn plugin_dep(&mut self, dep: &'a TargetLabel, _kind: &PluginKind) -> kuro_error::Result<()> {
        let p = ProvidersLabel::default_for(dep.dupe());
        self.dep(&p)
    }

    fn input(&mut self, input: SourcePathRef) -> kuro_error::Result<()>;

    fn label(&mut self, _label: &'a ProvidersLabel) -> kuro_error::Result<()> {
        Ok(())
    }
}
