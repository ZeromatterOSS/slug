...
impl TargetNode {
    ...
    pub fn visibility(&self) -> kuro_error::Result<&VisibilitySpecification> {
    ...
    pub fn attr<'a>(
        ...
        key: &str,
        opts: AttrInspectOptions,
    ) -> kuro_error::Result<Option<CoercedAttrFull<'a>>> {
    ...
    pub fn tests(&self) -> impl Iterator<Item = &ProvidersLabel> {
        ...
        impl<'a> CoercedAttrTraversal<'a> for TestCollector<'a> {
            ...
            fn dep(&mut self, _dep: &ProvidersLabel) -> kuro_error::Result<()> {
                Ok(())
            }

            fn label(&mut self, label: &'a ProvidersLabel) -> kuro_error::Result<()> {
    ...
    #[inline]
    pub fn metadata(&self) -> kuro_error::Result<Option<&MetadataMap>> {
...
impl<'a> TargetNodeRef<'a> {
    ...
    pub fn metadata(self) -> kuro_error::Result<Option<&'a MetadataMap>> {
    ...
    pub fn target_modifiers(self) -> kuro_error::Result<Option<&'a TargetModifiersValue>> {
        ...
        let map: Map<String, Value> = target
            ...
            .map(|(target_name, values)| {
                let mut json_values: Map<String, Value> = values
                    ...
                    .collect::<kuro_error::Result<Map<String, Value>>>()?;
                json_values.insert(
                    "__type__".to_owned(),
