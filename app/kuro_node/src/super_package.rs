...
impl SuperPackageData {
    pub fn new(
        ...
        cfg_constructor: Option<Arc<dyn CfgConstructorImpl>>,
        test_config_unification_rollout: bool,
    ) -> kuro_error::Result<SuperPackageData> {
        let modifier_key =
            MetadataKeyRef::new(MODIFIER_METADATA_KEY).map_err(kuro_error::Error::from)?;
...
impl SuperPackage {
    ...
    pub fn empty<T: SuperPackageValues + Default>() -> kuro_error::Result<SuperPackage> {
        SuperPackage::new(
            Arc::new(T::default()),
