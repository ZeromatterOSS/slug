...
impl TransitiveValidationKey {
    ...
    async fn validate_current_node(
        ...
        let artifacts = info
            ...
            .collect::<slug_error::Result<Vec<Artifact>>>()?;
...
impl Key for TransitiveValidationKey {
    type Value = slug_error::Result<CachedValidationResult>;
...
enum TreatValidationFailureAsError {
    ...
    Transient(slug_error::Error),
}

impl From<slug_error::Error> for TreatValidationFailureAsError {
...
impl From<DiceError> for TreatValidationFailureAsError {
    fn from(value: DiceError) -> Self {
        TreatValidationFailureAsError::Transient(slug_error::Error::from(value))
    }
}
...
fn tighten_cached_validation_result(
    result: slug_error::Result<CachedValidationResult>,
) -> Result<(), TreatValidationFailureAsError> {
    match result {
