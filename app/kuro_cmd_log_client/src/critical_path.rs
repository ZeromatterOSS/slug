    ...
    while let Some(event) = events.try_next().await? {
        match event {
            StreamValue::Event(event) => match event.data {
                Some(kuro_data::buck_event::Data::Instant(instant)) => match instant.data {
                    Some(kuro_data::instant_event::Data::BuildGraphInfo(build_graph)) => {
                        match path_kind {
                            PathKind::Critical => {
...
async fn log_critical_path(
    path: &Vec<kuro_data::CriticalPathEntry2>,
    format: LogCommandOutputFormat,
) -> kuro_error::Result<()> {
    ...
    kuro_client_ctx::stdio::print_with_writer::<kuro_error::Error, _>(async move |w| {
        ...
        for entry in path {
            use kuro_data::critical_path_entry2::Entry;
            ...
            critical_path.name = match entry_data {
                Entry::Analysis(analysis) => {
                    use kuro_data::critical_path_entry2::analysis::Target;
                ...
                Entry::DynamicAnalysis(analysis) => {
                    use kuro_data::critical_path_entry2::dynamic_analysis::Target;
                ...
                Entry::ActionExecution(action_execution) => {
                    use kuro_data::critical_path_entry2::action_execution::Owner;
                    ...
                    critical_path.execution_kind = Some(
                        kuro_data::ActionExecutionKind::try_from(action_execution.execution_kind)
                            .unwrap_or(kuro_data::ActionExecutionKind::NotSet)
                            .as_str_name(),
                    );
                ...
                Entry::FinalMaterialization(materialization) => {
                    use kuro_data::critical_path_entry2::final_materialization::Owner;
