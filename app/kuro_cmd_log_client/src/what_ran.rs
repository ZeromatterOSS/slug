...
use kuro_event_observer::what_ran;
...
use kuro_event_observer::what_ran::WhatRanOutputCommandExtra;
...
/// (if unsure, use `kuro root --kind project` to find it), then run the command. The command is
...
pub struct WhatRanCommand {
    ...
    /// That is command were running if kuro process was killed,
    /// or command currently running if kuro is running build now.
    #[clap(long)]
    pub incomplete: bool,
...
impl BuckSubcommand for WhatRanCommand {
    ...
    async fn exec_impl(
        ...
        kuro_client_ctx::stdio::print_with_writer::<kuro_error::Error, _>(async move |w| {
...
impl WhatRanCommandState {
    async fn execute(
        mut events: impl Stream<Item = kuro_error::Result<StreamValue>> + Unpin + Send,
        output: &mut impl WhatRanOutputWriter,
        options: &WhatRanCommandOptions,
    ) -> Result<(), ClientIoError> {
    ...
    fn event(
        ...
        if let Some(data) = event.data {
            ...
            match &data {
                kuro_data::buck_event::Data::SpanEnd(span) => {
                        ...
                        self.known_actions.remove(&SpanId::from_u64(event.span_id)?)
                    {
                        if should_emit_finished_action(&span.data, options) {
                            ...
                            let (execution_kind, std_err, duration, scheduling_mode) = match &span
                                .data
                            {
                                Some(kuro_data::span_end_event::Data::ActionExecution(
                                    action_exec,
                                )) => (
                            ...
                            if execution_kind
                                == Some(kuro_data::ActionExecutionKind::LocalDepFile as i32)
                            {
                                entry
                                    .reproducers
...
fn should_emit_finished_action(
    data: &Option<kuro_data::span_end_event::Data>,
    options: &WhatRanCommandOptions,
) -> bool {
    ...
    match data {
        Some(kuro_data::span_end_event::Data::ActionExecution(action)) => {
        ...
        match &mut self.format {
            ...
            LogCommandOutputFormatWithWriter::Json(w) => {
                let reproducer = match &command.repro {
                    ...
                    CommandReproducer::CacheHit(cache_hit) => match cache_hit.cache_type() {
                        kuro_data::CacheType::ActionCache => JsonReproducer::Cache {
                            digest: &cache_hit.action_digest,
                            action_key: cache_hit.action_key.as_deref(),
                        },
                        kuro_data::CacheType::RemoteDepFileCache => {
            ...
            LogCommandOutputFormatWithWriter::Csv(writer) => {
                ...
                writer
                    ...
                    .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::LogCmd))?;
                Ok(())
            }
        }
    }
...
fn into_index_map(platform: &Option<kuro_data::RePlatform>) -> IndexMap<&str, &str> {
...
mod tests {
    ...
    #[test]
    fn serialize_what_ran_command_no_extr() -> kuro_error::Result<()> {
  ...
  "duration": "1"
    ...
    #[test]
    fn serialize_what_ran_command_with_extra() -> kuro_error::Result<()> {
  ...
  "extra": {
    ...
    #[test]
    fn serialize_what_ran_command_in_re() -> kuro_error::Result<()> {
