...
fn get_record(materialization: &kuro_data::MaterializationEnd) -> Record {
    let method = match materialization
        .method
        .and_then(|v| kuro_data::MaterializationMethod::try_from(v).ok())
    {
        Some(kuro_data::MaterializationMethod::CasDownload) => "cas",
        Some(kuro_data::MaterializationMethod::LocalCopy) => "copy",
        Some(kuro_data::MaterializationMethod::HttpDownload) => "http",
        Some(kuro_data::MaterializationMethod::Write) => "write",
        _ => "<unknown>",
    };
    Record {
            ...
            while let Some(event) = events.try_next().await? {
                match event {
                    StreamValue::Event(event) => match &event.data {
                        Some(kuro_data::buck_event::Data::SpanEnd(kuro_data::SpanEndEvent {
                            data: Some(kuro_data::span_end_event::Data::Materialization(m)),
                            ..
                        })) if m.success =>
