...
fn get_action_execution_data(
    ...
    event.data.as_ref().and_then(|data| match data {
        kuro_data::buck_event::Data::SpanEnd(end) => {
                ...
                kuro_data::span_end_event::Data::ActionExecution(data) => {
...
) -> kuro_error::Result<LinkedHashMap<ActionKey, ActionExecutionData>> {
...
fn print_divergence_msg(
    ...
    kuro_client_ctx::println!("{}", output.join("\n"))?;
...
impl BuckSubcommand for ActionDivergenceCommand {
    ...
    async fn exec_impl(
        ...
        if !divergence_found {
            kuro_client_ctx::println!("No divergent actions found.")?;
        }
        ExitResult::success()
    }
}
