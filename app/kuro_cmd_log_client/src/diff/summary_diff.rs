...
//! `kuro log diff summary`: rollup-vs-rollup delta between two builds.
//!
...
impl BuckSubcommand for SummaryDiffCommand {
    ...
    async fn exec_impl(
        ...
        if self.fail_on_regression && had_regression {
            ExitResult::err(kuro_error::kuro_error!(
                ErrorTag::Bail,
...
async fn rollup_log(path: &EventLogPathBuf, top_n: usize) -> kuro_error::Result<BuildSummary> {
