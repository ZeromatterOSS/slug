...
fn insert_config_value(dict: &mut BTreeMap<String, String>, config: &kuro_data::ConfigValue) {
...
fn process_buckconfig_data(mut dict: &mut BTreeMap<String, String>, event: &kuro_data::BuckEvent) {
    ...
    event.data.as_ref().into_iter().for_each(|data| match data {
        kuro_data::buck_event::Data::Instant(end) => {
            end.data.as_ref().into_iter().for_each(|data| match data {
                kuro_data::instant_event::Data::BuckconfigInputValues(input) => input
                    .components
                    .iter()
...
) -> kuro_error::Result<BTreeMap<String, String>> {
...
impl BuckSubcommand for ExternalConfigDiffCommand {
    ...
    async fn exec_impl(
        ...
        kuro_client_ctx::println!("{}", json_diffs)?;
        ExitResult::success()
    }
}
