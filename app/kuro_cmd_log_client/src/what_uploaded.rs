...
fn get_action_record(
    state: &HashMap<u64, kuro_data::ActionExecutionStart>,
...
struct ReUploadEvent<'a> {
    ...
    pub inner: &'a kuro_data::ReUploadEnd,
}
            ...
            while let Some(event) = events.try_next().await? {
                match event {
                    ...
                    StreamValue::Event(event) => match event.data {
                        Some(kuro_data::buck_event::Data::SpanStart(start)) => match start.data {
                        ...
                        Some(kuro_data::buck_event::Data::SpanEnd(end)) => {
                            match end.data.as_ref() {
                                Some(kuro_data::span_end_event::Data::ReUpload(u)) => {
