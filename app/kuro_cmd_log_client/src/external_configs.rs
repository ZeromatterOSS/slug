...
use kuro_data::GlobalExternalConfig;
            ...
            match event {
                StreamValue::Event(event) => match event.data {
                    Some(kuro_data::buck_event::Data::Instant(instant)) => match instant.data {
                        Some(kuro_data::instant_event::Data::BuckconfigInputValues(configs)) => {
...
fn write_config_value<'a>(
    config_value: &'a kuro_data::ConfigValue,
    origin_path: &'a str,
...
async fn log_external_configs(
    components: &[kuro_data::BuckconfigComponent],
    format: LogCommandOutputFormat,
) -> kuro_error::Result<()> {
    kuro_client_ctx::stdio::print_with_writer::<kuro_error::Error, _>(async move |w| {
        ...
        for component in components {
            use kuro_data::buckconfig_component::Data;
            use kuro_data::config_file::Data as CData;
