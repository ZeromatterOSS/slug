...
impl Stats {
    fn update_with_event(&mut self, event: &kuro_data::BuckEvent) {
        match &event.data {
            Some(kuro_data::buck_event::Data::SpanEnd(end)) => match end.data.as_ref() {
                Some(kuro_data::span_end_event::Data::ReUpload(data)) => {
                    self.total_bytes_uploaded += data.bytes_uploaded.unwrap_or_default();
                }
                Some(kuro_data::span_end_event::Data::Materialization(data)) => {
                    self.total_files_materialized += data.file_count;
                    self.total_bytes_materialized += data.total_bytes;
                }
                Some(kuro_data::span_end_event::Data::ActionExecution(data)) => {
                ...
                Some(kuro_data::span_end_event::Data::Command(_command)) => {
                    self.duration = end.duration;
                }
                _ => {}
            },
            Some(kuro_data::buck_event::Data::Instant(instant_event)) => {
                match instant_event.data.as_ref() {
                    ...
                    Some(kuro_data::instant_event::Data::SystemInfo(system_info)) => {
                    ...
                    Some(kuro_data::instant_event::Data::VersionControlRevision(vcs)) => {
...
fn get_event_timestamp(event: &kuro_data::BuckEvent) -> Option<SystemTime> {
