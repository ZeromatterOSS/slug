...
use kuro_client_ctx::common::ui::get_console_with_root;
use kuro_client_ctx::daemon::client::NoPartialResultHandler;
use kuro_client_ctx::event_log_options::EventLogOptions;
use kuro_client_ctx::events_ctx::DaemonEventsCtx;
use kuro_client_ctx::events_ctx::EventsCtx;
use kuro_client_ctx::exit_result::ExitResult;
use kuro_client_ctx::signal_handler::with_simple_sigint_handler;
use kuro_client_ctx::subscribers::superconsole::timekeeper::Clock;
use kuro_client_ctx::subscribers::superconsole::timekeeper::Timekeeper;
use kuro_client_ctx::subscribers::superconsole::timekeeper::duration_between_timestamps;
use kuro_client_ctx::ticker::Tick;
...
impl BuckSubcommand for ReplayCommand {
    ...
    async fn exec_impl(
        ...
        if !speed.is_finite() || speed <= 0.0 {
            return ExitResult::from(kuro_error::Error::from(ReplayError::InvalidSpeed(speed)));
        }

        if !seek.is_finite() || seek < 0.0 {
            return ExitResult::from(kuro_error::Error::from(ReplayError::InvalidSeek(seek)));
        }
        ...
        let work = async {
            ...
            let res = res??;
            for e in &res.errors {
                kuro_client_ctx::eprintln!("{}", e.message)?;
            }
...
impl TryFrom<kuro_cli_proto::command_result::Result> for ReplayResult {
    type Error = kuro_cli_proto::command_result::Result;

    fn try_from(v: kuro_cli_proto::command_result::Result) -> Result<Self, Self::Error> {
        use kuro_cli_proto::command_result::Result;
...
async fn make_replayer(
    ...
    impl Stream<Item = kuro_error::Result<StreamValue>> + Unpin,
    Invocation,
    Timekeeper,
)> {
...
async fn replay_events_into(
    sink: UnboundedSender<kuro_error::Result<StreamValue>>,
    events: impl Stream<Item = kuro_error::Result<StreamValue>>,
    syncher: Syncher,
    mut speed_update_requests: mpsc::Receiver<SpeedUpdateRequest>,
    first_event: kuro_error::Result<StreamValue>,
...
async fn find_next_event_with_delay(
    sink: &UnboundedSender<kuro_error::Result<StreamValue>>,
    events: &mut (impl Stream<Item = kuro_error::Result<StreamValue>> + Unpin),
    min_timestamp: Option<prost_types::Timestamp>,
) -> Option<(kuro_error::Result<StreamValue>, prost_types::Timestamp)> {
