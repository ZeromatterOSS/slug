...
//!
//! `slug` supports limited concurrency for commands.
//! If there are no buckconfig changes, nor file changes, then commands can be allowed to execute
//! concurrently. Otherwise, `slug` will block waiting for other commands to finish.
...
use slug_data::DiceBlockConcurrentCommandEnd;
use slug_data::DiceBlockConcurrentCommandStart;
use slug_data::DiceEqualityCheck;
use slug_data::DiceSynchronizeSectionEnd;
use slug_data::DiceSynchronizeSectionStart;
use slug_data::ExclusiveCommandWaitEnd;
use slug_data::ExclusiveCommandWaitStart;
use slug_data::NoActiveDiceState;
...
impl CommandData {
    fn format_argv(&self) -> String {
        ...
        // Skip the "/path/to/slug" part so we can just emit "slug" for the start of the cmd
        iter.next();

        let cmd = format!("slug {}", iter.join(" "));
        truncate(&cmd, 500)
    }

    fn notify_tainted(&self) {
        self.dispatcher.instant_event(slug_data::TagEvent {
...
impl ConcurrencyHandler {
    ...
    async fn wait_for_others(
        ...
        let (mut transaction, tainted) = loop {
            match &data.dice_status {
                DiceStatus::Cleanup { future, epoch } => {
                    ...
                    event_dispatcher
                        .span_async(
                            slug_data::DiceCleanupStart { epoch: epoch as _ },
                            async move { (future.await, slug_data::DiceCleanupEnd {}) },
                        )
                        .await;
                ...
                DiceStatus::Available { active } => {
                    ...
                    let transaction = async {
                        ...
                        let transaction = event_dispatcher
                            .span_async(slug_data::DiceStateUpdateStart {}, async {
                                (
                                    async {
                                        let transaction =
                                            transaction.commit_with_data(user_data).await;
                                        slug_error::Ok(transaction)
                                    }
                                    .await,
                                    slug_data::DiceStateUpdateEnd {},
                                )
                            })
                            .await?;
                        slug_error::Ok(transaction)
            ...
            .await?
        {
            ...
            let current_external_and_local_configs: Vec<slug_data::BuckconfigComponent> =
                external_configs
            ...
            event_dispatcher.instant_event(slug_data::TagEvent {
                tags: get_experiment_tags(&current_external_and_local_configs),
            });
            event_dispatcher.instant_event(slug_data::BuckconfigInputValues {
...
mod tests {
    ...
    use slug_core::is_open_source;
    use slug_events::BuckEvent;
    use slug_events::create_source_sink_pair;
    ...
    #[tokio::test]
    async fn parallel_invocation_different_traceid_blocks() -> slug_error::Result<()> {
        let dice = make_default_dice().await;
    ...
    #[tokio::test]
    async fn parallel_invocation_exit_when_different_state() -> slug_error::Result<()> {
        ...
        let fut3_error: slug_error::Error = fut3_result.unwrap_err().into();
        assert!(
            fut3_error
                .tags()
                .contains(&slug_error::ErrorTag::DaemonIsBusy),
        );
    ...
    #[tokio::test]
    async fn parallel_invocation_exit_when_preemptible() -> slug_error::Result<()> {
        let dice = make_default_dice().await;
        ...
        let fut1_error: slug_error::Error = fut1_result.unwrap_err().into();
        assert!(
            fut1_error
                .tags()
                .contains(&slug_error::ErrorTag::DaemonPreempted),
        );
    ...
    #[tokio::test]
    async fn test_cleanup_stage() -> slug_error::Result<()> {
    ...
    async fn wait_for_exclusive_span_start(
        source: &mut ChannelEventSource,
        cmd: Option<&str>,
    ) -> slug_error::Result<Option<SpanId>> {
        let cmd = cmd.map(|c| c.to_owned());
        Ok(wait_for_event(
            source,
            Box::new(|e: &BuckEvent| {
                if let Some(span_start) = &e.span_start_event() {
                    if let Some(slug_data::span_start_event::Data::ExclusiveCommandWait(data)) =
                        &span_start.data
                    {
        ...
        wait_for_event(
            ...
            Box::new(|e: &BuckEvent| {
                if let Some(span_end) = &e.span_end_event() {
                    if let Some(slug_data::span_end_event::Data::ExclusiveCommandWait(_)) =
                        &span_end.data
                    {
    ...
    #[tokio::test]
    async fn exclusive_command_lock() -> slug_error::Result<()> {
        let dice = make_default_dice().await;
        let concurrency = ConcurrencyHandler::new(dice.dupe());
    ...
    #[tokio::test]
    async fn test_thundering_herd() -> slug_error::Result<()> {
        let dice = make_default_dice().await;
        ...
        slug_util::future::try_join_all(tasks).await?;
    ...
    #[tokio::test]
    async fn test_updates_are_synchronized() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_exit_when_not_idle_with_same_state() -> slug_error::Result<()> {
        let dice = make_default_dice().await;
        let concurrency = ConcurrencyHandler::new(dice.dupe());
        ...
        let fut2 = tokio::spawn(slug_util::async_move_clone!(concurrency, {
            concurrency
                .enter(
                    null_sink_with_trace(traces2),
                    &NoChanges,
        ...
        let fut2_error: slug_error::Error = fut2_result.unwrap_err().into();
        assert!(
            fut2_error
                .tags()
                .contains(&slug_error::ErrorTag::DaemonIsBusy),
    ...
    #[tokio::test]
    async fn test_exit_when_not_idle_with_different_state() -> slug_error::Result<()> {
        let dice = make_default_dice().await;
        let concurrency = ConcurrencyHandler::new(dice.dupe());
    ...
    #[tokio::test]
    async fn test_multiple_exit_when_not_idle_commands_with_same_state() -> slug_error::Result<()> {
        let dice = make_default_dice().await;
        let concurrency = ConcurrencyHandler::new(dice.dupe());
        ...
        let fut3 = tokio::spawn(slug_util::async_move_clone!(concurrency, {
            concurrency
                .enter(
                    null_sink_with_trace(traces3),
                    &NoChanges,
    ...
    #[tokio::test]
    async fn test_exit_when_not_idle_with_preemptible_command() -> slug_error::Result<()> {
        let dice = make_default_dice().await;
        let concurrency = ConcurrencyHandler::new(dice.dupe());
    ...
    #[tokio::test]
    async fn test_exit_when_not_idle_gets_preempted() -> slug_error::Result<()> {
        let dice = make_default_dice().await;
        let concurrency = ConcurrencyHandler::new(dice.dupe());
        ...
        let fut1 = tokio::spawn({
            ...
            async move {
                ...
                if let Err(ref e) = result {
                    let error: slug_error::Error = e.clone().into();
                    if error
                        .tags()
                        .contains(&slug_error::ErrorTag::DaemonPreempted)
                    {
    ...
    #[tokio::test]
    async fn test_enter_duration_parameter_populated() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_enter_duration_parameter_zero_for_non_exclusive() -> slug_error::Result<()> {
    ...
    #[tokio::test]
    async fn test_file_watcher_sync_duration_captured() -> slug_error::Result<()> {
