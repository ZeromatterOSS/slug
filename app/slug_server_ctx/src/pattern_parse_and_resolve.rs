...
use slug_common::pattern::parse_from_cli;
...
async fn resolve_patterns_to_targets<T: PatternType>(
...
) -> slug_error::Result<Vec<TargetLabelWithExtra<T>>> {
    ...
    for (package_with_modifiers, spec) in resolved_pattern.specs {
        ...
        match spec {
            slug_core::pattern::pattern::PackageSpec::Targets(targets) => {
            ...
            slug_core::pattern::pattern::PackageSpec::All() => {
...
pub async fn parse_and_resolve_provider_labels_from_cli_args(
    ...
    target_patterns: &[String],
    cwd: &ProjectRelativePath,
) -> slug_error::Result<Vec<ProvidersLabel>> {
...
pub async fn parse_and_resolve_provider_labels_with_modifiers_from_cli_args(
    ...
    target_patterns: &[String],
    cwd: &ProjectRelativePath,
) -> slug_error::Result<Vec<ProvidersLabelWithModifiers>> {
