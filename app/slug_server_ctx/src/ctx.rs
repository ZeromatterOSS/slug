...
use slug_data::CommandCriticalEnd;
use slug_data::CommandCriticalStart;
use slug_data::DiceCriticalSectionEnd;
use slug_data::DiceCriticalSectionStart;
...
pub struct PreviousCommandDataInternal {
    pub external_and_local_configs: Vec<slug_data::BuckconfigComponent>,
...
impl PreviousCommandData {
    pub fn process_current_command(
        ...
        current_external_and_local_configs: Vec<slug_data::BuckconfigComponent>,
        ...
        }) = self.data.as_ref()
        {
            if *current_external_and_local_configs != *external_configs {
                event_dispatcher.instant_event(slug_data::PreviousCommandWithMismatchedConfig {
...
pub trait ServerCommandContextTrait: Send + Sync {
    ...
    ) -> slug_error::Result<DiceAccessor<'a>>;
...
pub trait ServerCommandDiceContext {
    async fn with_dice_ctx<'v, F, Fut, R>(&'v self, exec: F) -> slug_error::Result<R>
    where
                            ...
                            |mut dice, early_command_timing| async move {
                                ...
                                events
                                    .span_async(
                                        ...
                                        async move {
                                            let res = slug_build_signals::env::scope(
                                                build_signals,
                                                self.events().dupe(),
