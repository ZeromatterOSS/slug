...
#[pin_project]
pub struct StreamingRequestHandler<T: TryFrom<StreamingRequest, Error = slug_error::Error>> {
    #[pin]
    client_stream: tonic::Streaming<StreamingRequest>,
    _phantom: PhantomData<T>,
}

impl<T: TryFrom<StreamingRequest, Error = slug_error::Error>> StreamingRequestHandler<T> {
    ...
    pub async fn message(&mut self) -> slug_error::Result<T> {
    ...
    fn map_poll(v: Option<Result<StreamingRequest, Status>>) -> Option<slug_error::Result<T>> {
        match v {
            None => None,
...
impl<T: TryFrom<StreamingRequest, Error = slug_error::Error>> Stream
    for StreamingRequestHandler<T>
{
    type Item = slug_error::Result<T>;
