...
pub(crate) fn validate_request(
    ...
    validate_metadata(receipt, repositories)?;
...
fn validate_metadata(
    ...
    let generated = repositories
        ...
        .collect::<SmallSet<_>>();
    validate_metadata_parts(&receipt.metadata, receipt.root_usage, &generated)
}

fn validate_metadata_parts(
    metadata: &slug_bzlmod_v2::ModuleExtensionMetadata,
    root_usage: Option<slug_bzlmod_v2::RootModuleExtensionUsage>,
    generated: &SmallSet<CompactString>,
) -> Result<(), HostValidateModuleExtensionRequestError> {
    let Some(root_usage) = root_usage else {
        return Ok(());
    };
    let resolve = |name: &str,
                   selection: &ModuleExtensionRepositorySelection|
     -> Result<SmallSet<CompactString>, HostValidateModuleExtensionRequestError> {
        match selection {
            ModuleExtensionRepositorySelection::Unspecified => Ok(SmallSet::new()),
            ModuleExtensionRepositorySelection::All => Ok(generated.clone()),
            ModuleExtensionRepositorySelection::Explicit(values) => {
                for value in values {
                    if !generated.contains(value) {
                        return Err(HostValidateModuleExtensionRequestError::Join(
                            format!(
                                "{name} contained repository '{value}' which was not generated by the extension"
                            )
                            .into(),
                        ));
                    }
    ...
    if !regular.is_empty() && !root_usage.has_non_dev_dependency() {
        return Err(HostValidateModuleExtensionRequestError::Join(
            "root_module_direct_deps must be empty when the root module has no non-dev usage"
                .into(),
        ));
    }
    if !dev.is_empty() && !root_usage.has_dev_dependency() {
        return Err(HostValidateModuleExtensionRequestError::Join(
            "root_module_direct_dev_deps must be empty when the root module has no dev usage"
                .into(),
        ));
    }
    Ok(())
...
"#;
    ...
    #[test]
    fn metadata_validation_is_root_presence_conditioned_and_checks_generated_names() {
        let generated = SmallSet::from_iter([CompactString::from("generated")]);
        let metadata = slug_bzlmod_v2::ModuleExtensionMetadata::new(
            ModuleExtensionRepositorySelection::explicit(["missing".into()]),
            ModuleExtensionRepositorySelection::explicit([]),
            false,
            slug_bzlmod_v2::Facts::default(),
        );
        assert!(validate_metadata_parts(&metadata, None, &generated).is_ok());
        assert!(matches!(
            validate_metadata_parts(
                &metadata,
                Some(slug_bzlmod_v2::RootModuleExtensionUsage::new(true, false)),
                &generated,
            ),
            Err(HostValidateModuleExtensionRequestError::Join(message))
                if message.contains("was not generated")
        ));

        let all_regular = slug_bzlmod_v2::ModuleExtensionMetadata::new(
        ...
        assert!(matches!(
            validate_metadata_parts(
                &all_regular,
                Some(slug_bzlmod_v2::RootModuleExtensionUsage::new(false, true)),
                &generated,
            ),
            Err(HostValidateModuleExtensionRequestError::Join(message))
                if message.contains("no non-dev usage")
        ));
        assert!(
            validate_metadata_parts(
                &all_regular,
        ...
        let both_missing = slug_bzlmod_v2::ModuleExtensionMetadata::new(
            ModuleExtensionRepositorySelection::explicit(["missing_regular".into()]),
            ModuleExtensionRepositorySelection::explicit(["missing_dev".into()]),
            false,
            slug_bzlmod_v2::Facts::default(),
        );
        assert!(matches!(
            validate_metadata_parts(
                &both_missing,
                Some(slug_bzlmod_v2::RootModuleExtensionUsage::new(true, true)),
                &generated,
            ),
            Err(HostValidateModuleExtensionRequestError::Join(message))
                if message.contains("root_module_direct_dev_deps")
                    && message.contains("missing_dev")
        ));
    }
    #[derive(Default)]
    ...
    #[tokio::test]
    async fn extension_metadata_capture_accepts_reproducible_and_fails_nonempty_facts() {
        let dice = Arc::new(Dice::builder().build(DetectCycles::Enabled));
        let root_module = module("use_repo(e, first='first')", "");
        let reproducible = r#"
repo=repository_rule(lambda ctx: None)
def impl(ctx):
    repo(name='first')
...
ext=module_extension(implementation=impl)
"#;
        let value = compute_with_extension(&dice, &root_module, reproducible).await;
        assert!(matches!(
            value,
            SourcePreparationOutcome::Complete(ref value) if value.is_ok()
        ));

        let explicit = reproducible.replace(
            "reproducible=True",
            "root_module_direct_deps=['first'], root_module_direct_dev_deps=[], reproducible=True",
        );
        let value = compute_with_extension(&dice, &root_module, &explicit).await;
        assert!(matches!(
            value,
            SourcePreparationOutcome::Complete(ref value) if value.is_ok()
        ));
        let missing = explicit.replace("['first']", "['missing']");
        let value = compute_with_extension(&dice, &root_module, &missing).await;
        assert!(matches!(
            value,
            SourcePreparationOutcome::Complete(ref value) if value.is_err()
        ));

        let dual_invalid_module = module("use_repo(e, bad='missing_import')", "");
        let value = compute_with_extension(&dice, &dual_invalid_module, &missing).await;
        assert!(matches!(
            value,
            SourcePreparationOutcome::Complete(value)
                if matches!(
                    ...
                    Err(HostValidatedGeneratedRepositorySpecsError {
                        inner: PrivateValidationError::Join { message, .. },
                    }) if message.contains("root_module_direct_deps")
                )
        ));

        let nonempty_facts = reproducible.replace(
            "reproducible=True",
            "reproducible=True, facts={'digest': 'value'}",
        );
        let value = compute_with_extension(&dice, &root_module, &nonempty_facts).await;
        assert!(matches!(
            value,
            SourcePreparationOutcome::Complete(ref value) if value.is_err()
        ));
