...
use slug_common::io::fs::ReadUncheckedOptions;
use slug_core;
...
use crate::semaphore::slug_default;
...
impl EdenIoProvider {
    pub async fn new(
        ...
        let eden_semaphore = slug_default();
        ...
        if let Some(eden_version) = &eden_version {
            if eden_version.as_str() < min_eden_version {
                tracing::warn!(
                    ...
                    Update Eden then restart Slug to use Eden I/O.",
                    eden_version,
                    min_eden_version
                );
                return Ok(None);
    ...
    ) -> slug_error::Result<PathMetadataResult> {
