...
pub enum IsMountReadyError {
    ...
    #[slug(tag = IoEdenMountDoesNotExist)]
    MountDoesNotExist { mount: AbsPathBuf },
    #[error(transparent)]
    #[slug(tag = IoEdenRequestError)]
...
pub enum ConnectAndRequestError<E> {
    #[error(transparent)]
    #[slug(tag = IoEdenConnectionError)]
    ConnectionError(slug_error::Error),
