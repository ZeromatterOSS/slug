...
pub mod semaphore {
    ...
    /// A slug-specific semaphore that is used to limit the number of outstanding requests to
    /// EdenFS. Reads slug specific environment variable "BUCK2_EDEN_SEMAPHORE" to determine the
    /// number of permits.
    pub fn slug_default() -> Semaphore {
        let permits =
            slug_env!("BUCK2_EDEN_SEMAPHORE", type=usize, default=DEFAULT_MAX_OUTSTANDING_REQUESTS, applicability=internal)
