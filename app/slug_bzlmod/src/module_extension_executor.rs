...
//! implementation lives in `slug_interpreter_for_build` which has access to the
//! Starlark interpreter and can call `build_module_context()`.
//!
//! ## Why Late Binding?
//!
//! The crate dependency direction is:
//!   `slug_interpreter_for_build` depends on `slug_bzlmod`
//!
//! But extension execution needs to:
//!   1. Use `AggregatedExtension` from `slug_bzlmod`
//!   2. Build `ModuleContext` via `build_module_context()` from `slug_interpreter_for_build`
...
/// lives in `slug_interpreter_for_build` where it can access:
...
///
/// Initialized by `slug_interpreter_for_build::init_late_bindings()`.
///
/// # Example Usage
///
/// ```ignore
