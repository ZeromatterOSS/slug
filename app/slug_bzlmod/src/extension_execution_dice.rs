...
pub fn build_canonical_names(
    ...
    specs
        .keys()
        .map(|internal| {
            let canonical = format!("{}+{}", repo_prefix, internal);
...
pub fn extract_extension_name(extension_id: &str) -> String {
    // Look for %extension_name, leaving a possible second %isolation_key out.
    let mut parts = extension_id.splitn(3, '%');
    let _bzl_file = parts.next();
...
/// Extract the optional isolation key from an extension ID.
pub fn extract_extension_isolation_key(extension_id: &str) -> Option<String> {
    let mut parts = extension_id.splitn(3, '%');
    let _bzl_file = parts.next()?;
    let _extension_name = parts.next()?;
    parts.next().map(str::to_owned)
}

/// Bazel-shaped canonical repository prefix for repos generated by one
/// extension instance.
///
/// Computes a unique name following Bazel 9's `calculateUniqueNameForUsedExtensionId`
/// and `makeUniqueNameCandidate` logic: after appending a single `+` to each
/// name, no resulting string is a prefix of any other such string.
pub fn extension_repo_prefix(extension_id: &str, root_module_name: &str) -> String {
    make_unique_name_candidate(extension_id, root_module_name, 1)
}

/// Compute unique extension repo prefixes for a set of extension IDs.
///
/// Parity: `BazelDepGraphFunction.calculateUniqueNameForUsedExtensionId`.
/// Iterates over extension IDs, tries `makeUniqueNameCandidate(id, 1)`, then
/// increments attempt on collision, producing unambiguous prefixes.
pub fn compute_extension_unique_names(
    ...
    for extension_id in extension_ids {
        let mut attempt = 1;
        loop {
            let candidate = make_unique_name_candidate(extension_id, root_module_name, attempt);
            if !used.contains(&candidate) {
                used.insert(candidate.clone());
                assigned.insert(extension_id.clone(), candidate);
                break;
            }
            attempt += 1;
        }
...
fn make_unique_name_candidate(
    ...
    let Some(isolation_key) = extract_extension_isolation_key(extension_id) else {
        return format!("{}+{}{}", owning_module, ext_name, disambiguator);
    };
    let (module_name, module_version, exported_name) =
        isolation_key_unique_name_parts(&isolation_key);
    format!(
        "{}+_{}{}+{}+{}+{}",
        owning_module, ext_name, disambiguator, module_name, module_version, exported_name
    )
}
...
fn isolation_key_unique_name_parts(isolation_key: &str) -> (String, String, String) {
    let Some((module_key, exported_name)) = isolation_key.rsplit_once('+') else {
        return ("".to_owned(), "".to_owned(), isolation_key.to_owned());
    };
    if module_key == "<root>" {
        return ("".to_owned(), "".to_owned(), exported_name.to_owned());
    }
    let Some((module_name, module_version)) = module_key.split_once('@') else {
        return (
            module_key.to_owned(),
            "".to_owned(),
    ...
    let module_version = if module_version == "_" {
        ""
    } else {
        module_version
    };
...
/// **WARNING**: This function does NOT hash `.bzl` file content. It only hashes
/// the extension ID. If the extension's `.bzl` implementation changes on disk
/// while the extension ID remains the same, DICE will produce a stale cache
/// hit. Prefer `compute_bzl_transitive_digest_from_file_states` which hashes
/// actual file content. This function is retained only for test and bootstrap
/// callers where file states are not yet available.
pub fn compute_bzl_transitive_digest(extension_id: &str) -> String {
    ...
    tracing::warn!(
        "compute_bzl_transitive_digest: using v1 (extension-id-only) digest \
         for '{}'. This does not capture .bzl file changes. \
         Prefer compute_bzl_transitive_digest_from_file_states.",
        extension_id
    );
...
mod tests {
    ...
    fn test_extract_extension_name() {
        ...
        assert_eq!(
            extract_extension_name("//:my_extension.bzl%my_ext%<root>+first"),
            "my_ext"
        );
        // Fallback cases
    ...
    #[test]
    fn isolated_extension_repo_prefix_uses_bazel_unique_name_shape() {
        assert_eq!(
            extension_repo_prefix("//:ext.bzl%ext%<root>+first", "root"),
            "_main+_ext+++first"
        );
        assert_eq!(
            extension_repo_prefix("@dep//:ext.bzl%ext%dep@1.0+second", "root"),
            "dep++_ext+dep+1.0+second"
        );
    }
    ...
    #[tokio::test]
    async fn extension_aggregation_key_rejects_stale_resolution_digest() -> slug_error::Result<()> {
        let workspace_id = crate::WorkspaceId::for_project_root(PathBuf::from(
            "/tmp/slug-plan61-extension-aggregation-digest",
        ));
        ...
        assert!(
            err.to_string()
                .contains("extension aggregation data digest"),
            "{err:?}"
        );
        Ok(())
    }
