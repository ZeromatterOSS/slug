...
pub fn convert_from_str<T>(v: &str) -> kuro_error::Result<T>
where
    T: FromStr,
...
/// This macro is used to register environment variables that are used by Kuro.
///
...
///    for self-testing of kuro
///
/// The macro expands to an expression of type `kuro_error::Result<Type>` if a default is set, and
/// `kuro_error::Result<Option<Type>` otherwise.
pub macro kuro_env {
    ($var:expr, bool $(, $($rest:tt)*)?) => {{
        let v: kuro_error::Result<bool> = $crate::env::__macro_refs::kuro_env!($var, type=bool, default=false, converter = |s| {
            match s.to_lowercase().as_str() {
                "1" | "true" => Ok(true),
                "0" | "false" => Ok(false),
                _ => Err(kuro_error::kuro_error!(kuro_error::ErrorTag::Tier0, "Invalid bool value: {}", s).into()),
            }
        }, $($($rest)*)?);
        v
    }},
...
/// Register env name to be shown in `kuro help-env`.
pub macro kuro_env_name($var:expr) {{
...
#[allow(unused_macros)]
/// `parser` is `&str -> kuro_error::Result<$stored_type>`, `processor` is `Option<& $stored_type> -> $output_type`
///
...
macro expand(
    ...
    let v: kuro_error::Result<$output_ty> = ENV_HELPER.get().map($processor);
    v
}}
