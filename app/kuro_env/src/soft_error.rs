...
use crate::env::kuro_env;

type StructuredErrorHandler = Box<
    dyn for<'a> Fn(&'a str, &kuro_error::Error, (&'a str, u32, u32), StructuredErrorOptions)
        + Send
        + Sync
        + 'static,
>;
...
pub fn kuro_hard_error_env() -> kuro_error::Result<Option<&'static str>> {
    kuro_env!("BUCK2_HARD_ERROR")
}
...
/// * The error is a `kuro_error::Error`.
///
/// Soft errors from Meta internal runs can be viewed
/// [in logview](https://www.internalfb.com/logview/overview/kuro).
///
...
fn hard_error_config() -> kuro_error::Result<Arc<HardErrorConfig>> {
    ...
    let config = kuro_hard_error_env()?.unwrap_or_default();
...
pub fn reload_hard_error_config(var_value: &str) -> kuro_error::Result<()> {
...
) -> Result<kuro_error::Error, kuro_error::Error> {
...
pub fn initialize(handler: StructuredErrorHandler) -> kuro_error::Result<()> {
    hard_error_config()?;
...
impl HardErrorConfigHolder {
    fn reload_hard_error_config(&self, var_value: &str) -> kuro_error::Result<()> {
...
pub fn validate_logview_category(category: &str) -> kuro_error::Result<()> {
...
pub(crate) mod tests {
    ...
    #[test]
    fn test_hard_error() -> kuro_error::Result<()> {
