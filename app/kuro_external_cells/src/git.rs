...
impl IoRequest for GitFetchIoRequest {
    fn execute(
        self: Box<Self>,
        project_fs: &kuro_core::fs::project::ProjectRoot,
    ) -> kuro_error::Result<()> {
        ...
        fn run_git(cwd: &AbsNormPath, f: impl FnOnce(&mut Command)) -> kuro_error::Result<()> {
...
pub(crate) async fn get_file_ops_delegate(
    ...
    cell: CellName,
    setup: GitCellSetup,
) -> kuro_error::Result<Arc<GitFileOpsDelegate>> {
    #[derive(
        dupe::Dupe,
        Clone,
        Debug,
    ...
    impl Key for GitFileOpsDelegateKey {
        type Value = kuro_error::Result<Arc<GitFileOpsDelegate>>;
