...
use kuro_common::io::fs::is_executable;
...
use kuro_external_cells_bundled::BundledCell;
use kuro_external_cells_bundled::BundledFile;
use kuro_external_cells_bundled::get_bundled_data;
use kuro_fs::fs_util;
...
fn load_nano_prelude() -> kuro_error::Result<BundledCell> {
    let path = env::var("NANO_PRELUDE")
        ...
        Consider `export NANO_PRELUDE=$HOME/fbsource/fbcode/kuro/tests/e2e_util/nano_prelude`",
        )?;
    if path.is_empty() {
...
fn nano_prelude() -> kuro_error::Result<BundledCell> {
...
pub(crate) fn find_bundled_data(cell_name: CellName) -> kuro_error::Result<BundledCell> {
...
impl BundledFileOpsDelegate {
    ...
    async fn read_dir(&self, path: &CellRelativePath) -> kuro_error::Result<Arc<[RawDirEntry]>> {
    ...
    fn read_file_if_exists(
        &self,
        path: &CellRelativePath,
    ) -> kuro_error::Result<Option<&'static str>> {
...
) -> kuro_error::Result<BundledFileOpsDelegate> {
...
pub(crate) async fn get_file_ops_delegate(
    ctx: &mut DiceComputations<'_>,
    cell_name: CellName,
) -> kuro_error::Result<Arc<BundledFileOpsDelegate>> {
    #[derive(
        dupe::Dupe,
        Clone,
        Copy,
    ...
    impl Key for BundledFileOpsDelegateKey {
        type Value = kuro_error::Result<Arc<BundledFileOpsDelegate>>;
