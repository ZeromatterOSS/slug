...
impl CgroupFile {
    ...
    async fn read_to_short_buf(&self) -> kuro_error::Result<([u8; 32], usize)> {
        let file = self.0.dupe();
        tokio::task::spawn_blocking(move || {
            ...
            kuro_error::Ok((data, filled))
        })
        .await?
        .with_buck_error_context(|| format!("Reading cgroup file {}", self.1))
    }

    async fn read_to_buf(&self) -> kuro_error::Result<Vec<u8>> {
        let file = self.0.dupe();
        tokio::task::spawn_blocking(move || {
    ...
    pub(crate) async fn read_to_string(&self) -> kuro_error::Result<String> {
        let buf = self.read_to_buf().await?;
        Ok(String::from_utf8(buf)?)
    }

    // FIXME(JakobDegen): Ought probably to have some types to represent the files
    pub(crate) async fn read_memory_stat(&self) -> kuro_error::Result<MemoryStat> {
        MemoryStat::parse(&self.read_to_string().await?)
            .buck_error_context("Failed to parse memory.stat")
    }

    pub(crate) async fn read_max_or_int(&self) -> kuro_error::Result<Option<u64>> {
    ...
    pub(crate) async fn read_int(&self) -> kuro_error::Result<u64> {
        let (data, filled) = self.read_to_short_buf().await?;
        let data = &data[..filled];
        std::str::from_utf8(data)
            .ok()
    ...
    pub(crate) async fn read_resource_pressure(&self) -> kuro_error::Result<ResourcePressure> {
...
impl MemoryStat {
    fn parse(content: &str) -> kuro_error::Result<Self> {
...
impl ResourcePressure {
    fn parse(s: &str) -> kuro_error::Result<Self> {
        Self::parse_inner(s).ok_or_else(|| {
            kuro_error::Error::from(CgroupFileError::UnexpectedFormat(
