...
impl<M: MemoryMonitoring, K: CgroupKind> Cgroup<M, K> {
    ...
    pub(crate) async fn read_enabled_controllers(&self) -> kuro_error::Result<EnabledControllers> {
    ...
    pub async fn set_memory_high(&self, memory_high: &str) -> kuro_error::Result<()> {
        CgroupFile::open(
            self.dir.dupe(),
    ...
    pub async fn set_memory_max(&self, memory_max: &str) -> kuro_error::Result<()> {
        CgroupFile::open(
            self.dir.dupe(),
    ...
    async fn read_resource_constraints(&self) -> kuro_error::Result<EffectiveResourceConstraints> {
        let read = |f| async move {
            let f = CgroupFile::open(self.dir.dupe(), FileNameBuf::unchecked_new(f), false).await?;
            kuro_error::Ok(f.read_max_or_int().await?)
        };
    ...
    pub(crate) async fn read_effective_resouce_constraints(
        &self,
    ) -> kuro_error::Result<EffectiveResourceConstraints> {
...
impl Cgroup<NoMemoryMonitoring, CgroupKindUndecided> {
    pub(crate) fn sync_try_from_path(path: CgroupPathBuf) -> kuro_error::Result<Self> {
    ...
    pub async fn try_from_path(path: CgroupPathBuf) -> kuro_error::Result<Self> {
    ...
    ) -> kuro_error::Result<Cgroup<NoMemoryMonitoring, CgroupKindInternal>> {
...
impl<K: CgroupKind> Cgroup<NoMemoryMonitoring, K> {
    pub(crate) async fn enable_memory_monitoring(
        self,
    ) -> kuro_error::Result<Cgroup<WithMemoryMonitoring, K>> {
        let open = |f| {
            let d = &self.dir;
...
impl<K: CgroupKind> Cgroup<WithMemoryMonitoring, K> {
    pub async fn read_memory_stat(&self) -> kuro_error::Result<MemoryStat> {
        self.memory.memory_stat.read_memory_stat().await
    }

    pub async fn read_memory_current(&self) -> kuro_error::Result<u64> {
        self.memory.memory_current.read_int().await
    }

    pub async fn read_swap_current(&self) -> kuro_error::Result<u64> {
        self.memory.swap_current.read_int().await
    }

    pub async fn read_memory_pressure(&self) -> kuro_error::Result<f64> {
        Ok(self
            .memory
            .memory_pressure
...
impl<M: MemoryMonitoring, K: CgroupKind> Cgroup<M, K> {
    pub async fn freeze(&self) -> kuro_error::Result<CgroupFreezeGuard> {
