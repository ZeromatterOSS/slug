...
impl EventSenderState {
    pub(crate) fn new(
        ...
        Self {
            metadata: kuro_events::metadata::collect(daemon_id),
    ...
    pub(crate) fn maybe_send_scheduled_event(
            ...
            .is_none_or(|last| now.duration_since(last) > freq)
        {
            ...
            self.send_event(
                kuro_data::ResourceControlEventKind::Scheduled,
                None,
                actions_running,
                actions_suspended,
            );
    ...
    pub(crate) fn send_event(
        &mut self,
        kind: kuro_data::ResourceControlEventKind,
...
impl ResourceControlEventMostly {
    pub(crate) fn complete(self, uuid: &TraceId) -> kuro_data::ResourceControlEvents {
        kuro_data::ResourceControlEvents {
            uuid: uuid.to_string(),
            kind: self.kind.into(),
            ...
            ancestor_cgroup_constraints: Some(kuro_data::AncestorCgroupConstraints {
