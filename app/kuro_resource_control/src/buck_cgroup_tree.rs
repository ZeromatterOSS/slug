...
fn parse_procfs_cgroup_output(out: &str) -> kuro_error::Result<CgroupPathBuf> {
...
impl PreppedBuckCgroups {
    ...
    ///  2. Kuro may manage the child cgroups by itself without interference from outside things;
    ...
    pub fn prep_current_process() -> kuro_error::Result<Self> {
...
mod tests {
    ...
    fn test_cgroup_info_parse() {
        let cgroup = "\
0::/user.slice/user-532497.slice/user@532497.service/kuro.cg\n";
        assert_eq!(
            "/sys/fs/cgroup/user.slice/user-532497.slice/user@532497.service/kuro.cg",
