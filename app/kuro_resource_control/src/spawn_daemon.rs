...
use kuro_common::init::ResourceControlStatus;
use kuro_fs::paths::abs_norm_path::AbsNormPath;
use kuro_util::process;
...
async fn systemd_check_available() -> kuro_error::Result<()> {
...
mod tests {
    ...
    async fn test_always_unavailable_on_nonlinux() {
        let _error = kuro_error::Error::from(SystemdNotAvailableReason::UnsupportedPlatform);
        assert!(matches!(
