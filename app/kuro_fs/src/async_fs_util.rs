...
pub async fn open<P: AsRef<AbsPath>>(path: P) -> kuro_error::Result<tokio::fs::File> {
...
pub async fn read<P: AsRef<AbsPath>>(path: P, buf: &mut [u8]) -> kuro_error::Result<usize> {
...
pub async fn read_to_string<P: AsRef<AbsPath>>(path: P) -> kuro_error::Result<String> {
    let path = path.as_ref().to_owned();
...
pub async fn create_dir_all<P: AsRef<AbsPath>>(dir: P) -> kuro_error::Result<()> {
    let dir = dir.as_ref().to_owned();
    Ok(tokio::task::spawn_blocking(move || fs_util::create_dir_all(dir)).await??)
}

pub async fn metadata<P: AsRef<AbsPath>>(path: P) -> kuro_error::Result<Metadata> {
    let path = path.as_ref().to_owned();
