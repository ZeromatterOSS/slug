...
/// Kuro having full control over how FS IO works is beneficial for implementing
/// IO counters and retry policies that are optimized for Kuro and the EdenFS
...
pub use kuro_env::soft_error::soft_error;
...
impl IoError {
    ...
    pub fn categorize_for_source_file(self) -> kuro_error::Error {
        if self.e.kind() == io::ErrorKind::NotFound {
            kuro_error::Error::from(self).tag([ErrorTag::Input]).into()
        } else {
            self.into()
        }
    }
...
#[kuro(tags = io_error_tags(e, *is_eden))]
#[error("{op}")]
pub struct IoError {
    op: String,
    #[source]
            ...
            Ok(v) => {
                if let Some(err) = last_error_kind {
                    ...
                    soft_error!(
                        ...
                        kuro_error!(kuro_error::ErrorTag::Input, "{}", err.to_string()),
                        quiet: true
                    )
                    .ok();
                }
...
pub fn symlink<P, Q>(original: P, link: Q) -> kuro_error::Result<()>
where
    P: AsRef<Path>,
    Q: AsRef<AbsPath>,
{
...
#[cfg(unix)]
fn symlink_impl(original: &Path, link: &AbsPath) -> kuro_error::Result<()> {
...
fn symlink_impl(original: &Path, link: &AbsPath) -> kuro_error::Result<()> {
    ...
    fn permission_check(result: io::Result<()>) -> kuro_error::Result<()> {
        match result {
            ...
            link.parent()
                .ok_or_else(|| {
                    kuro_error::kuro_error!(
                        kuro_error::ErrorTag::SymlinkParentMissing,
...
pub fn set_current_dir<P: AsRef<AbsPath>>(path: P) -> kuro_error::Result<()> {
...
pub fn set_executable<P: AsRef<AbsPath>>(path: P, executable: bool) -> kuro_error::Result<()> {
    let path = path.as_ref();
...
pub fn canonicalize<P: AsRef<AbsPath>>(path: P) -> kuro_error::Result<AbsNormPathBuf> {
...
pub fn canonicalize_if_exists<P: AsRef<AbsPath>>(
    path: P,
) -> kuro_error::Result<Option<AbsNormPathBuf>> {
    let _guard = IoCounterKey::Canonicalize.guard();
...
pub fn simplified(path: &AbsPath) -> kuro_error::Result<&AbsPath> {
...
pub fn disk_space_stats<P: AsRef<AbsPath>>(path: P) -> kuro_error::Result<DiskSpaceStats> {
    #[cfg(not(windows))]
    fn disk_space_stats_impl(path: &AbsPath) -> kuro_error::Result<DiskSpaceStats> {
        use std::ffi::CString;
    ...
    fn disk_space_stats_impl(path: &AbsPath) -> kuro_error::Result<DiskSpaceStats> {
        ...
        use kuro_util::os::win::os_str::os_str_to_wide_null_term;
...
// TODO(T143971518) Avoid RelativePath usage in kuro
pub fn relative_path_from_system(path: &Path) -> kuro_error::Result<Cow<'_, RelativePath>> {
...
mod tests {
    ...
    #[test]
    fn if_exists_read_dir() -> kuro_error::Result<()> {
    ...
    #[test]
    fn create_and_remove_symlink_dir() -> kuro_error::Result<()> {
    ...
    #[test]
    fn create_and_remove_symlink_file() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = tempdir.path().join("root");
        let root = AbsPath::new(&root)?;
        create_dir_all(root)?;
    ...
    #[test]
    fn test_symlink_with_target_length_over_max_path() -> kuro_error::Result<()> {
    ...
    #[test]
    fn symlink_to_file_which_doesnt_exist() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn symlink_to_symlinked_dir() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn relative_symlink_to_nonexistent_file() -> kuro_error::Result<()> {
    ...
    #[test]
    fn relative_symlink_to_nonexistent_dir() -> kuro_error::Result<()> {
    ...
    #[test]
    fn relative_symlink_from_symlinked_dir_windows() -> kuro_error::Result<()> {
    ...
    #[test]
    fn absolute_symlink_to_nonexistent_file_in_nonexistent_dir() -> kuro_error::Result<()> {
        // tmp -- dir1 (doesn't exists) -- file1 (doesn't exist)
        //     \
    ...
    #[test]
    fn remove_file_removes_symlink_to_directory() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn remove_file_does_not_remove_directory() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
        let dir_path = root.join("dir");
    ...
    #[test]
    fn remove_file_broken_symlink() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
    ...
    #[test]
    fn remove_file_non_existing_file() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn remove_all_nonexistent() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn remove_all_regular() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn remove_all_dir() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn remove_all_broken_symlink() -> kuro_error::Result<()> {
        fn ls(path: &Path) -> kuro_error::Result<Vec<PathBuf>> {
    ...
    #[cfg(unix)]
    #[test]
    fn remove_all_path_contains_regular_file() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn remove_dir_all_does_not_remove_file() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn test_read_to_string_if_exists() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
        let f1 = root.join("f1");
        let f2 = root.join("f2");
    ...
    #[test]
    fn test_read_if_exists() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
        let f1 = root.join("f1");
        let f2 = root.join("f2");
    ...
    #[cfg(windows)]
    #[test]
    fn test_windows_relative_path() -> kuro_error::Result<()> {
        assert_eq!(
    ...
    #[cfg(unix)]
    #[test]
    fn test_relative_path() -> kuro_error::Result<()> {
        assert_eq!(
            fs_util::relative_path_from_system(Path::new("foo/bar"))?,
            RelativePath::new("foo/bar")
        );
    ...
    #[cfg(unix)]
    #[test]
    fn test_remove_all_removes_readonly_path() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn test_create_file_if_not_exists() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn test_disk_space_stats() -> kuro_error::Result<()> {
        let tempdir = tempfile::tempdir()?;
        let root = AbsPath::new(tempdir.path())?;
    ...
    #[test]
    fn test_retry_io() -> kuro_error::Result<()> {
        use std::io::ErrorKind;
    ...
    #[test]
    fn test_io_error_tag() -> kuro_error::Result<()> {
        ...
        let kuro_error = kuro_error::Error::from(file.err().unwrap());
