...
impl AbsNormPath {
    ...
    /// }
    /// ```
    pub fn new<P: ?Sized + AsRef<Path>>(p: &P) -> kuro_error::Result<&AbsNormPath> {
    ...
    fn strip_prefix_impl(&self, base: &AbsNormPath) -> kuro_error::Result<&Path> {
        if self.windows_prefix()? == base.windows_prefix()? {
            ...
            Err(kuro_error::kuro_error!(
                kuro_error::ErrorTag::InvalidAbsPath,
    ...
    /// # kuro_error::Ok(())
    /// ```
    pub fn windows_prefix(&self) -> kuro_error::Result<OsString> {
        use std::os::windows::ffi::OsStringExt;
        use std::path::Prefix;

        match self.0.components().next().ok_or_else(|| {
            kuro_error::kuro_error!(kuro_error::ErrorTag::InvalidAbsPath, "AbsPath is empty.")
        })? {
            std::path::Component::Prefix(prefix_component) => match prefix_component.kind() {
                ...
                prefix => Err(kuro_error::kuro_error!(
                    kuro_error::ErrorTag::InvalidAbsPath,
                    "Unknown prefix kind: {:?}.",
                    prefix
                )),
    ...
    /// # kuro_error::Ok(())
    /// ```
    pub fn strip_windows_prefix(&self) -> kuro_error::Result<&Path> {
...
impl AbsNormPathBuf {
    pub fn new(path: PathBuf) -> kuro_error::Result<AbsNormPathBuf> {
    ...
    pub fn from(s: String) -> kuro_error::Result<Self> {
    ...
    /// # kuro_error::Ok(())
    /// ```
    pub fn push_normalized<P: AsRef<RelativePath>>(&mut self, path: P) -> kuro_error::Result<()> {
...
impl TryFrom<String> for AbsNormPathBuf {
    ...
    /// }
    /// ```
    fn try_from(s: String) -> kuro_error::Result<AbsNormPathBuf> {
...
impl TryFrom<OsString> for AbsNormPathBuf {
    ...
    // no allocation
    fn try_from(s: OsString) -> kuro_error::Result<AbsNormPathBuf> {
...
impl TryFrom<PathBuf> for AbsNormPathBuf {
    ...
    /// }
    /// ```
    fn try_from(p: PathBuf) -> kuro_error::Result<AbsNormPathBuf> {
...
impl FromStr for AbsNormPathBuf {
    ...
    fn from_str(s: &str) -> kuro_error::Result<Self> {
...
fn verify_abs_path(path: &AbsPath) -> kuro_error::Result<()> {
...
mod tests {
    ...
    #[test]
    fn abs_paths_work_in_maps() -> kuro_error::Result<()> {
    ...
    #[test]
    fn abs_path_is_comparable() -> kuro_error::Result<()> {
        let foo_string = make_absolute("/foo");
        let bar_string = make_absolute("/bar");
    ...
    #[cfg(not(windows))]
    #[test]
    fn absolute_path_display_is_readable() -> kuro_error::Result<()> {
