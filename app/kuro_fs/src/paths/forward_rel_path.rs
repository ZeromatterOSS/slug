...
impl ForwardRelativePath {
    ...
    #[inline]
    pub fn new<S: ?Sized + AsRef<Path>>(s: &S) -> kuro_error::Result<&ForwardRelativePath> {
    ...
    pub fn new_trim_trailing_slashes<S: ?Sized + AsRef<Path>>(
        path: &S,
    ) -> kuro_error::Result<&ForwardRelativePath> {
    ...
    pub fn join_normalized<P: AsRef<RelativePath>>(
        &self,
        path: P,
    ) -> kuro_error::Result<ForwardRelativePathBuf> {
...
impl ForwardRelativePathBuf {
    #[inline]
    pub fn new(s: String) -> kuro_error::Result<ForwardRelativePathBuf> {
...
impl<'a> TryFrom<&'a str> for &'a ForwardRelativePath {
    ...
    /// ```
    #[inline]
    fn try_from(s: &'a str) -> kuro_error::Result<&'a ForwardRelativePath> {
...
impl<'a> TryFrom<&'a Path> for &'a ForwardRelativePath {
    ...
    /// # kuro_error::Ok(())
    /// ```
    fn try_from(s: &'a Path) -> kuro_error::Result<&'a ForwardRelativePath> {
        let s = s
            .as_os_str()
            .to_str()
...
impl<'a> TryFrom<&'a RelativePath> for &'a ForwardRelativePath {
    ...
    /// ```
    #[inline]
    fn try_from(p: &'a RelativePath) -> kuro_error::Result<&'a ForwardRelativePath> {
...
impl TryFrom<String> for ForwardRelativePathBuf {
    ...
    /// ```
    #[inline]
    fn try_from(s: String) -> kuro_error::Result<ForwardRelativePathBuf> {
        ForwardRelativePathVerifier::verify_str(&s)?;
        Ok(ForwardRelativePathBuf(s))
    }
}
...
impl TryFrom<PathBuf> for ForwardRelativePathBuf {
    ...
    /// # kuro_error::Ok(())
    /// ```
    fn try_from(p: PathBuf) -> kuro_error::Result<ForwardRelativePathBuf> {
...
impl TryFrom<RelativePathBuf> for ForwardRelativePathBuf {
    ...
    /// ```
    #[inline]
    fn try_from(p: RelativePathBuf) -> kuro_error::Result<ForwardRelativePathBuf> {
...
impl ForwardRelativePathVerifier {
    fn verify_str(rel_path: &str) -> kuro_error::Result<()> {
        #[cold]
        #[inline(never)]
        fn err(rel_path: &str) -> kuro_error::Error {
...
mod tests {
    ...
    #[test]
    fn forward_path_is_comparable() -> kuro_error::Result<()> {
    ...
    #[test]
    fn path_buf_is_clonable() -> kuro_error::Result<()> {
    ...
    #[test]
    fn relative_path_display_is_readable() -> kuro_error::Result<()> {
