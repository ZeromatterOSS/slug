...
impl AbsPath {
    pub fn new<P: AsRef<Path> + ?Sized>(path: &P) -> kuro_error::Result<&AbsPath> {
        // Wrapper function to make sure the lifetimes are right
        fn inner(path: &Path) -> kuro_error::Result<&AbsPath> {
            if path.is_absolute() {
    ...
    pub fn to_str(&self) -> kuro_error::Result<&str> {
        match self.0.to_str() {
            Some(s) => Ok(s),
    ...
    pub fn strip_prefix<P: AsRef<AbsPath>>(&self, prefix: P) -> kuro_error::Result<&Path> {
    ...
    pub fn as_maybe_relativized_str(&self) -> kuro_error::Result<&str> {
        Ok(cwd::maybe_relativize_str(self.to_str()?))
    }
}

impl AbsPathBuf {
    pub fn new<P: AsRef<Path>>(path: P) -> kuro_error::Result<Self> {
    ...
    pub fn into_string(self) -> kuro_error::Result<String> {
        self.into_os_string()
            .into_string()
...
impl FromStr for AbsPathBuf {
    ...
    fn from_str(s: &str) -> kuro_error::Result<AbsPathBuf> {
