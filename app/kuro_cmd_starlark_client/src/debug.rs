...
pub fn write_dap_message(out: &mut impl Write, msg: &[u8]) -> kuro_error::Result<()> {
...
fn send_message_to_dap_client(msg: &[u8]) -> kuro_error::Result<()> {
...
impl StreamingCommand for StarlarkDebugAttachCommand {
        ...
        let stream = ide_message_stream::<_, debugserver_types::Request>(ctx.stdin()).filter_map(
            |m| async move {
                match m {
                    ...
                    Err(e) => {
                        let _ignored = kuro_client_ctx::eprintln!(
    ...
    fn extra_subscribers(&self) -> Vec<Box<dyn EventSubscriber>> {
        ...
        impl ConvertToDap {
            fn write_console(&self, msg: &str) -> kuro_error::Result<()> {
                ...
                for ev in events {
                    match unpack_event(ev)? {
                        UnpackedBuckEvent::Instant(_, _, data) => match data {
                            kuro_data::instant_event::Data::StructuredError(soft_error) => {
                                if !soft_error.quiet {
...
impl PartialResultHandler for DapPartialResultHandler {
    type PartialResult = kuro_cli_proto::DapMessage;

    async fn handle_partial_result(
        &mut self,
        mut _ctx: PartialResultCtx<'_>,
        partial_res: kuro_cli_proto::DapMessage,
    ) -> kuro_error::Result<()> {
