...
impl BuckStarlarkDebuggerServer {
    pub(crate) fn new(
        ...
        Self {
            to_state,
            eval_semaphore: Arc::new(Semaphore::new(kuro_util::threads::available_parallelism())),
    ...
    pub(crate) async fn start_eval(
        ...
        description: &str,
    ) -> kuro_error::Result<Box<dyn StarlarkDebugController>> {
        ...
        let (hook_id, eval_wrapper) = match recv.await {
            ...
            Err(..) => {
                ...
                // That could be due to the debugger detaching from kuro. This does not indicate
    ...
    pub(crate) fn send_request(&self, req: dap::Request) -> kuro_error::Result<()> {
    ...
    pub(crate) fn detach(&self) -> kuro_error::Result<()> {
...
impl VariableId {
    ...
    pub fn new(top_frame: bool, thread_id: u32, variable_id: u32) -> kuro_error::Result<Self> {
...
impl DebugServer for ServerState {
    ...
    ) -> kuro_error::Result<Option<serde_json::Value>> {
    ...
    fn set_breakpoints(
    ...
    ) -> kuro_error::Result<dap::SetBreakpointsResponseBody> {
        ...
        let resolved = resolve_breakpoints(&x, &self.get_ast(&project_relative)?)
            .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::StarlarkServer))?;
    ...
    fn attach(&mut self, _x: dap::AttachRequestArguments) -> kuro_error::Result<()> {
        Ok(())
    }

    fn threads(&mut self) -> kuro_error::Result<dap::ThreadsResponseBody> {
    ...
    fn configuration_done(&mut self) -> kuro_error::Result<()> {
        Ok(())
    }
    ...
    ) -> kuro_error::Result<dap::StackTraceResponseBody> {
    ...
    fn scopes(&mut self, x: dap::ScopesArguments) -> kuro_error::Result<dap::ScopesResponseBody> {
    ...
    ) -> kuro_error::Result<dap::VariablesResponseBody> {
    ...
    fn source(&mut self, _x: dap::SourceArguments) -> kuro_error::Result<dap::SourceResponseBody> {
        Err(StarlarkDebuggerError::Unimplemented.into())
    }
    ...
    fn next(&mut self, x: dap::NextArguments) -> kuro_error::Result<()> {
        let hook = self.find_hook_by_pseudo_thread(x.thread_id)?;
        hook.adapter
            .step(StepKind::Over)
            .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::StarlarkServer))?;
    ...
    fn step_in(&mut self, x: dap::StepInArguments) -> kuro_error::Result<()> {
        let hook = self.find_hook_by_pseudo_thread(x.thread_id)?;
        hook.adapter
            .step(StepKind::Into)
            .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::StarlarkServer))?;
    ...
    fn step_out(&mut self, x: dap::StepOutArguments) -> kuro_error::Result<()> {
        let hook = self.find_hook_by_pseudo_thread(x.thread_id)?;
        hook.adapter
            .step(StepKind::Out)
            .map_err(|e| from_any_with_tag(e, kuro_error::ErrorTag::StarlarkServer))?;
    ...
    ) -> kuro_error::Result<dap::EvaluateResponseBody> {
    ...
    fn disconnect(&mut self, _x: dap::DisconnectArguments) -> kuro_error::Result<()> {
        Ok(())
    }
}
...
impl ServerState {
    ...
    fn get_snapshot(&self) -> kuro_data::DebugAdapterSnapshot {
        ...
        for hook_state in self.current_hooks.values() {
            if let Some(v) = &hook_state.stopped_at {
                let handle_snapshot = current_handles
                    ...
                    .or_insert_with(|| kuro_data::DebugAdapterCommandSnapshot {
                        stopped_evals: Vec::new(),
                    });
                handle_snapshot
                    .stopped_evals
                    .push(kuro_data::DebugAdapterStoppedEval {
        ...
        kuro_data::DebugAdapterSnapshot {
    ...
    fn handle_message(&mut self, msg: ServerMessage) -> kuro_error::Result<bool> {
        match msg {
    ...
    fn new_hook(
        ...
        handle: BuckStarlarkDebuggerHandle,
        description: String,
    ) -> kuro_error::Result<(HookId, Option<Box<dyn DapAdapterEvalHook>>)> {
    ...
    fn drop_hook(&mut self, hook_id: HookId) -> kuro_error::Result<()> {
    ...
    fn eval_stopped(&mut self, hook_id: HookId) -> kuro_error::Result<()> {
    ...
    fn find_hook_by_pseudo_thread(&self, thread_id: i64) -> kuro_error::Result<&HookState> {
    ...
    fn get_ast(&self, source: &ProjectRelativePath) -> kuro_error::Result<AstModule> {
