...
pub(crate) trait DebugServer {
    ...
    fn attach(&mut self, x: dap::AttachRequestArguments) -> kuro_error::Result<()>;
    ...
    fn disconnect(&mut self, x: dap::DisconnectArguments) -> kuro_error::Result<()>;

    /// See <https://microsoft.github.io/debug-adapter-protocol/specification#Requests_Source>
    fn source(&mut self, x: dap::SourceArguments) -> kuro_error::Result<dap::SourceResponseBody>;
}
...
pub(crate) fn err_response(req: &dap::Request, err: &kuro_error::Error) -> dap::Response {
    dap::Response {
...
pub(crate) fn dispatch(
    ...
    r: &dap::Request,
) -> kuro_error::Result<dap::Response> {
    fn arg<T: for<'a> Deserialize<'a>>(r: &dap::Request) -> kuro_error::Result<T> {
    ...
    fn ret_some<T: Serialize>(
        r: &dap::Request,
        v: kuro_error::Result<T>,
    ) -> kuro_error::Result<dap::Response> {
        ret(r, v.map(|v| Some(v)))
    }

    fn ret_none(r: &dap::Request, v: kuro_error::Result<()>) -> kuro_error::Result<dap::Response> {
        ret::<()>(r, v.map(|_| None))
    }
    ...
    match r.command.as_str() {
        ...
        _ => Err(kuro_error::kuro_error!(
            kuro_error::ErrorTag::Input,
            "Kuro debugserver didn't recognize command: {}",
            r.command
        )),
    }
}
