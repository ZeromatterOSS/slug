...
/// Messages from the debugger server to its client (the cli `kuro starlark debug-attach` which
/// then forwards them along through its stdout).
#[derive(Debug)]
pub(crate) enum ToClientMessage {
    Event(dap::Event),
    Response(dap::Response),
    Shutdown(kuro_error::Result<()>),
}

impl ToClientMessage {
    fn pretty_string(&self) -> kuro_error::Result<String> {
        match self {
...
pub async fn run_dap_server_command(
    ...
    partial_result_dispatcher: PartialResultDispatcher<kuro_cli_proto::DapMessage>,
    req: StreamingRequestHandler<kuro_cli_proto::DapRequest>,
) -> kuro_error::Result<kuro_cli_proto::DapResponse> {
    let start_event = ctx
        .command_start_event(kuro_data::StarlarkDebugAttachCommandStart {}.into())
        .await?;
    span_async(start_event, async move {
        let result = run_dap_server(ctx, partial_result_dispatcher, req)
            .await
            .map_err(Into::into);
        let end_event = command_end(&result, kuro_data::StarlarkDebugAttachCommandEnd {});
...
async fn run_dap_server(
    ctx: &dyn ServerCommandContextTrait,
    mut partial_result_dispatcher: PartialResultDispatcher<kuro_cli_proto::DapMessage>,
    mut req: StreamingRequestHandler<kuro_cli_proto::DapRequest>,
) -> kuro_error::Result<kuro_cli_proto::DapResponse> {
        ...
        select! {
            request = req.next() => {
                let request = match request {
                    ...
                    None => {
                        // client disconnected.
                        break kuro_cli_proto::DapResponse {};
                    }
                };
...
fn handle_outgoing_message(
    seq: &mut u32,
    partial_result_dispatcher: &mut PartialResultDispatcher<kuro_cli_proto::DapMessage>,
    message: ToClientMessage,
) -> kuro_error::Result<Option<kuro_cli_proto::DapResponse>> {
    ...
    let (dap_json, response) = match message {
        ...
        ToClientMessage::Shutdown(res) => {
            ...
            (
                ...
                Some(kuro_cli_proto::DapResponse {}),
            )
        }
    };

    partial_result_dispatcher.emit(kuro_cli_proto::DapMessage { dap_json });
    Ok(response)
}
