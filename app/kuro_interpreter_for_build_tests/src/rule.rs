...
use kuro_interpreter_for_build::nodes::attr_spec::AttributeSpecExt;
...
#[test]
fn rule_creates_callable() -> kuro_error::Result<()> {
...
#[test]
fn udr_is_recorded() -> kuro_error::Result<()> {
    let content = indoc!(
        r#"
        def impl(ctx):
            pass
...
#[test]
fn option_allows_none() -> kuro_error::Result<()> {
    let mut tester = rule_tester();
...
#[test]
fn returns_documentation() -> kuro_error::Result<()> {
    let bzl = indoc::indoc!(
        r#"def impl(ctx):
            pass
    ...
    for param in &mut params.named_only {
        match param.name.as_str() {
            ...
            "tags" => param.default_value = Some("[]".to_owned()),
            "testonly" => param.default_value = Some("False".to_owned()),
            "deprecation" => param.default_value = Some("None".to_owned()),
            "features" => param.default_value = Some("[]".to_owned()),
            "applicable_licenses" => param.default_value = Some("[]".to_owned()),
            _ => {}
        }
    }
