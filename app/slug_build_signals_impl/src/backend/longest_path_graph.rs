...
use slug_critical_path::Graph;
use slug_critical_path::GraphBuilder;
use slug_critical_path::OptionalVertexId;
use slug_critical_path::PushError;
use slug_critical_path::VertexData;
use slug_critical_path::VertexKeys;
use slug_critical_path::compute_critical_path_potentials;
...
pub(crate) struct LongestPathGraphBackend {
    builder: slug_error::Result<GraphBuilder<NodeKey, NodeData>>,
...
impl BuildListenerBackend for LongestPathGraphBackend {
    ...
    fn finish(self) -> slug_error::Result<BuildInfo> {
        let (graph, keys, data) = {
...
) -> slug_error::Result<(DetailedCriticalPath, Vec<(ConfiguredTargetLabel, Duration)>)> {
    ...
    let critical_path_for_top_level_targets = top_level_targets
        ...
        .filter_map(|t| {
            let max_cost = (|| {
                ...
                slug_error::Result::Ok(Duration::from_micros(path_cost.runtime))
            })();
...
fn compute_slowest_paths(
    ...
    keys: &VertexKeys<NodeKey>,
    data: &VertexData<NodeData>,
) -> slug_error::Result<DetailedCriticalPath> {
    let mut last = None;
