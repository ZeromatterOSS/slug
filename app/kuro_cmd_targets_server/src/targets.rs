...
use kuro_cli_proto::targets_request::Compression;
...
trait Compressor: Write + Send {
    fn finish(self: Box<Self>) -> kuro_error::Result<()>;
}
...
fn outputter<'a, W: Write + Send + 'a>(
    ...
    stdout: W,
) -> kuro_error::Result<(OutputType, Box<dyn Compressor + 'a>)> {
...
pub async fn targets_command(
    ...
    req: TargetsRequest,
) -> kuro_error::Result<TargetsResponse> {
    run_server_command(
...
impl ServerCommandTemplate for TargetsServerCommand {
    type StartEvent = kuro_data::TargetsCommandStart;
    type EndEvent = kuro_data::TargetsCommandEnd;
