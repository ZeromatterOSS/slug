...
use kuro_build_api::configure_targets::get_compatible_targets;
use kuro_common::file_ops::dice::DiceFileComputations;
use kuro_common::file_ops::error::FileReadErrorContext;
use kuro_common::file_ops::metadata::PathMetadata;
use kuro_common::file_ops::metadata::PathMetadataOrRedirection;
...
trait FileHasher: Send + Sync {
    ...
    async fn hash_path(&self, path: &CellPath) -> kuro_error::Result<Vec<u8>>;
}
...
impl FileHasher for PathsAndContentsHasher {
    async fn hash_path(&self, cell_path: &CellPath) -> kuro_error::Result<Vec<u8>> {
        #[async_recursion]
        async fn hash_item(
...
/// This trait is purposely defined here instead of in kuro_node crate
...
pub(crate) trait TargetHashingTargetNode: QueryTarget {
    ...
    async fn get_target_nodes(
        ...
        loaded_targets: Vec<(
            ...
            kuro_error::Result<Vec<TargetNode>>,
        )>,
        global_cfg_options: &GlobalCfgOptions,
    ) -> kuro_error::Result<TargetSet<Self>>;
}
...
pub struct TargetHashes {
    ...
    target_mapping: HashMap<TargetLabel, kuro_error::Result<BuckTargetHash>>,
}
...
impl TargetHashes {
    pub fn get(&self, label: &TargetLabel) -> Option<&kuro_error::Result<BuckTargetHash>> {
        ...
        T::Key: ConfiguredOrUnconfiguredTargetLabel,
    {
        let mut hashes: HashMap<
            T::Key,
            Shared<DropcancelJoinHandle<kuro_error::Result<BuckTargetHash>>>,
        > = HashMap::new();
        ...
        let mut target_mapping: HashMap<TargetLabel, kuro_error::Result<BuckTargetHash>> =
            HashMap::new();
        ...
        T::Key: ConfiguredOrUnconfiguredTargetLabel,
    {
        let hashing_futures: Vec<_> = targets
            ...
            .map(|target| {
                let file_hasher = file_hasher.dupe();
                async move {
                    let hash_result: kuro_error::Result<BuckTargetHash> = try {
                        let mut hasher = TargetHashes::new_hasher(use_fast_hash);
                        TargetHashes::hash_node(&target, &mut *hasher);
                    ...
                    (
                        ...
                        hash_result.map_err(kuro_error::Error::from),
                    )
                }
                .boxed()
                .shared()
        ...
        let target_mapping: HashMap<TargetLabel, kuro_error::Result<BuckTargetHash>> =
    ...
    fn hash_deps(
        dep_hashes: Vec<kuro_error::Result<BuckTargetHash>>,
    ...
    fn hash_files(
        file_digests: Vec<(CellPath, kuro_error::Result<Vec<u8>>)>,
