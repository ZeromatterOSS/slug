...
use kuro_cli_proto::configured_targets_request::OutputFormat as ConfiguredOutputFormat;
use kuro_cli_proto::targets_request;
use kuro_cli_proto::targets_request::OutputFormat;
use kuro_cli_proto::targets_request::TargetHashGraphType;
...
use kuro_node::attrs::hacks::configured_value_to_json;
use kuro_node::attrs::hacks::value_to_json;
use kuro_node::attrs::inspect_options::AttrInspectOptions;
use kuro_node::nodes::attributes::DEPS;
use kuro_node::nodes::attributes::INPUTS;
use kuro_node::nodes::attributes::ONCALL;
use kuro_node::nodes::attributes::PACKAGE;
use kuro_node::nodes::attributes::PACKAGE_VALUES;
use kuro_node::nodes::attributes::TARGET_CALL_STACK;
use kuro_node::nodes::attributes::TARGET_HASH;
use kuro_node::nodes::attributes::TYPE;
...
fn package_error_to_stderr(package: PackageLabel, error: &kuro_error::Error, stderr: &mut String) {
    writeln!(stderr, "Error parsing {package}\n{error:?}").unwrap();
}

fn target_error_to_stderr(error: &kuro_error::Error, stderr: &mut String) {
...
pub trait ConfiguredTargetFormatter: Send + Sync {
    ...
    fn target_error(&self, error: &kuro_error::Error, stdout: &mut String, stderr: &mut String) {
...
pub(crate) struct Stats {
    ...
    error_tags: BTreeSet<kuro_error::ErrorTag>,
...
impl Stats {
    ...
    pub(crate) fn add_error(&mut self, e: &kuro_error::Error) {
        self.error_tags.extend(e.tags());
        self.errors += 1;
    }

    pub(crate) fn to_error(&self) -> Option<kuro_error::Error> {
        if self.errors == 0 {
            return None;
        }
        // Simpler error so that we don't print long errors twice (when exiting kuro)
        ...
        let mut e = kuro_error::Error::from(TargetsError::FailedToParse(self.errors, package_str));
...
) -> kuro_error::Result<Arc<dyn TargetFormatter>> {
...
) -> kuro_error::Result<ConfiguredOutputHandler> {
