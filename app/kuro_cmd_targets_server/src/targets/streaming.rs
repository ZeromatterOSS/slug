...
//! Server-side implementation of `kuro targets --streaming` command.
...
use kuro_common::pattern::package_roots::find_package_roots_stream;
...
fn write_str(outputter: &mut dyn Write, s: &mut String) -> kuro_error::Result<()> {
...
///                Passing from cli args `--keep-going` from `app/kuro_client/src/commands/targets.rs`.
...
///               Passing from cli args `--imports` from `app/kuro_client/src/commands/targets.rs`.
...
impl PreparePackageResult {
    ...
    fn record_error(&mut self, error: &kuro_error::Error, formatter: &dyn TargetFormatter) {
...
) -> impl Stream<Item = kuro_error::Result<(PackageLabel, PackageSpec<T>)>> + '_ {
        ...
        PackageSpec::Targets(targets) => {
            if keep_going {
                ...
                let targets = targets.into_try_map(|(target, TargetPatternExtra)| {
                    kuro_error::Ok(result.resolve_target(target.as_ref())?.to_owned())
                })?;
