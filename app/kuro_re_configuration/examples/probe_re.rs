//! Smoke test: build a `KuroOssReConfiguration` from CLI args + the BuildBuddy
//! API key, hand it to `REClientBuilder::build_and_connect`, and call
//! `Capabilities`. If this returns ok, the wire path between kuro's REv2
...
fn main() {
    ...
    let config = KuroOssReConfiguration {
